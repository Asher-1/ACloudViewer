	...
	m_data( new ccDefaultPluginData )
{
	...
	if ( !opened )
	{
		CVLog::Error( QStringLiteral( "Could not load plugin resources: %1" ).arg( resourcePath ) );
		return;
	}
	...
	{
		CVLog::Error( QStringLiteral( "Could not parse plugin info: %1" ).arg( jsonError.errorString() ) );
		return;
	}
}
