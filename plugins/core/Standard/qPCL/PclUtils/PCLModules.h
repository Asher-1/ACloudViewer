...
/// @file PCLModules.h
/// @brief PCL algorithm modules: surface reconstruction, segmentation,
...
/** Grid projection surface reconstruction.
 *  @param cloudWithNormals Input point cloud with normals
 *  @param outMesh Output mesh
 *  @param resolution Grid resolution (default 0.2)
 *  @param paddingSize Padding size (default 2)
 *  @param maxSearchLevel Max octree search level (default 8)
 *  @return 1 on success
 */
int GridProjection(const PointCloudNormal::ConstPtr &cloudWithNormals,
                   PCLMesh &outMesh,
 ...
 *  @param treeDepth Octree depth (default 8)
 *  @param isoDivideDepth Iso-surface subdivision depth
 *  @param solverDivideDepth Solver subdivision depth
 *  @param scale Scale factor (default 1.25)
 *  @param samplesPerNode Samples per octree node
 *  @param useConfidence Use confidence weighting
 *  @param useManifold Enforce manifold output
 *  @param outputPolygons Output polygon soup
 *  @return 1 on success
 */
 ...
 *  @param cloudWithNormals Input point cloud with normals
 *  @param outMesh Output mesh
 *  @param trigulationSearchRadius Search radius for triangulation
 *  @param weightingFactor Mu weighting factor
 *  @param maxNearestNeighbors Max neighbors for search
 *  @param maxSurfaceAngle Max surface angle (degrees)
 *  @param minAngle Min angle (degrees)
 *  @param maxAngle Max angle (degrees)
 *  @param normalConsistency Enforce normal consistency
 *  @return 1 on success
 */
...
/** Project points onto a model (plane, etc.).
 *  @param originCloud Input point cloud
 *  @param projectedCloud Output projected cloud
 *  @param coefficients Model coefficients
 *  @param modelType SAC model type (default SACMODEL_PLANE)
 *  @return 1 on success
 */
int GetProjection(const PointCloudT::ConstPtr &originCloud,
                  PointCloudT::Ptr &projectedCloud,
                  const pcl::ModelCoefficients::ConstPtr coefficients,
                  const int &modelType = 0 /*pcl::SACMODEL_PLANE*/
);

/** Project points onto a plane (coefficient form).
 *  @param originCloud Input point cloud
 *  @param projectedCloud Output projected cloud
 *  @param coefficientA,coefficientB,coefficientC,coefficientD Plane equation
 * Ax+By+Cz+D=0
 *  @param modelType SAC model type (default SACMODEL_PLANE)
 *  @return 1 on success
 */
...
/**
 * @brief Basic Region Growing segmentation
 * @param cloud Input point cloud
 * @param clusters Output cluster indices
 * @param cloud_segmented Output colored segmented cloud
 * @param k K-nearest neighbors
 * @param min_cluster_size Minimum cluster size
 * @param max_cluster_size Maximum cluster size
 * @param neighbour_number Neighbor count for normal estimation
 * @param smoothness_theta Smoothness threshold (radians)
 * @param curvature Curvature threshold
 * @return 1 on success
 */
int GetRegionGrowing(const PointCloudT::ConstPtr cloud,
...
/**
 * @brief Color-based Region Growing segmentation
 * @param cloud Input RGB point cloud
 * @param clusters Output cluster indices
 * @param cloud_segmented Output colored segmented cloud
 * @param min_cluster_size Minimum cluster size
 * @param neighbors_distance Max distance for neighbors
 * @param point_color_diff Point color difference threshold
 * @param region_color_diff Region color difference threshold
 * @return 1 on success
 */
int GetRegionGrowingRGB(const PointCloudRGB::ConstPtr cloud,
                        std::vector<pcl::PointIndices> &clusters,
                        PointCloudRGB::Ptr cloud_segmented,
                        int min_cluster_size,
 ...
 *  @param methodType SAC method (default SAC_RANSAC)
 *  @param modelType SAC model (default SACMODEL_PLANE)
 *  @param distanceThreshold Inlier distance threshold
 *  @param probability RANSAC probability
 *  @param maxIterations Max RANSAC iterations
 *  @param minRadiusLimits Min radius for sphere/cylinder
 *  @param maxRadiusLimits Max radius for sphere/cylinder
 *  @param normalDisWeight Normal distance weight
 *  @return 1 on success
 */
int GetSACSegmentation(const PointCloudT::ConstPtr cloud,
...
not really good, so please give parameters) \note Different types can be passed
as input for this function:
                - PointXYZI
                - PointNormal
                - PointXYZRGB
        \note If a PointType with a scale field is passed as output type, scales
will be returned together with the return cloud
**/
...
/// @class FeatureCloud
/// @brief Computes surface normals and FPFH features for point clouds.
class FeatureCloud {
public:
    // A bit of shorthand
    ...
    /** @param normalRadius Search radius for normal estimation
     */
    void setNormalRadius(float normalRadius) { m_normalRadius = normalRadius; }
    /** @param featureRadius Search radius for FPFH features
     */
    void setFeatureRadius(float featureRadius) {
        m_featureRadius = featureRadius;
    }
    /** @param maxThreadCount Max threads for parallel computation
     */
    void setmaxThreadCount(int maxThreadCount) {
        m_maxThreadCount = maxThreadCount;
    }

    /** @param xyz Input point cloud (triggers processing)
     */
    ...
    /** @param pcd_file Path to PCD file
     */
    void loadInputCloud(const std::string &pcd_file);

    /** @return Pointer to 3D point cloud
     */
    PointCloudT::Ptr getPointCloud() const { return (m_xyz); }

    /** @return Pointer to surface normals
     */
    SurfaceNormals::Ptr getSurfaceNormals() const { return (m_normals); }

    /** @return Pointer to FPFH feature descriptors
     */
