...
/**
 * @file PCLModules.cpp
 * @brief Implementation of PCL algorithm modules (segmentation, surface,
 * recognition).
 */
...
int GetRegionGrowing(const PointCloudT::ConstPtr cloud,
    ...
    regionGrowing.setNumberOfNeighbours(
            neighbour_number);  // example default: 30
    //
    ...
    CVLog::Print(
            QString("[Basic Region Growing] Number of clusters is equal to %1")
                    .arg(clusters.size()));
    if (clusters.size() != 0) {
        CVLog::Print(
                QString("[Basic Region Growing] First cluster has %1 points.")
                        .arg(clusters[0].indices.size()));
    }
...
int GetRegionGrowingRGB(const PointCloudRGB::ConstPtr cloud,
    ...
    pcl::search::KdTree<PointRGB>::Ptr kdtree(
    ...
    // Set the distance threshold, to know which points will be considered
    // neighbors.
    ...
    regionGrowing.extract(clusters);
    CVLog::Print(
            QString("[RGB Region Growing] Number of clusters is equal to %1")
                    .arg(clusters.size()));
    if (clusters.size() != 0) {
        CVLog::Print(
                QString("[RGB Region Growing] First cluster has %1 points.")
                        .arg(clusters[0].indices.size()));
    }
...
int GetSACSegmentation(const PointCloudT::ConstPtr cloud,
    ...
    switch (modelType) {
        ...
        case pcl::SACMODEL_NORMAL_SPHERE: {
            ...
            if (!ComputeNormals<PointT, NormalT>(cloud, cloudNormas, -1,
                                                 false) ||
            ...
            float normalWeight =
                    normalDisWeight > 0 ? normalDisWeight : 0.1 /*default*/;
...
template int GetMarchingCubes<PointNT>(const PointCloudNormal::ConstPtr inCloud,
...
void TemplateMatching::align(FeatureCloud &template_cloud,
...
// Align all of template clouds set by addTemplateCloud to the target specified
// by setTargetCloud ()
void TemplateMatching::alignAll(
        std::vector<TemplateMatching::Result, Eigen::aligned_allocator<Result>>
                &results) {
