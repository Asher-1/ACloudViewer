...
MinimumCutSegmentationDlg::MinimumCutSegmentationDlg(ecvMainAppInterface* app)
    ...
    connect(label2DCloudComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onLabelChanged(int)));
}
...
void MinimumCutSegmentationDlg::refreshLabelComboBox() {
    if (m_app) {
        ...
        for (size_t i = 0; i < labels.size(); ++i) {
            if (labels[i]->isA(CV_TYPES::LABEL_2D))  // as filterChildren only
                                                     // test 'isKindOf'
            {
...
void MinimumCutSegmentationDlg::updateForeGroundPoint() {
    ...
    cc2DLabel* label =
            get2DLabelFromCombo(label2DCloudComboBox, m_app->dbRootObject());
    if (!label) {
        return;
    }
    const CCVector3* center = label->getPickedPoint(0).cloud->getPoint(
            label->getPickedPoint(0).index);
    if (!center) {
        return;
    }
