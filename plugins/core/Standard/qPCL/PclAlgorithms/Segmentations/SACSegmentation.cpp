...
void SACSegmentation::getParametersFromDialog() {
    ...
    m_distanceThreshold =
            static_cast<float>(m_dialog->disThresholdSpinBox->value());
    ...
    m_normalDisWeight =
            static_cast<float>(m_dialog->normalDisWeightSpinBox->value());
    ...
    m_maxRemainingRatio =
            static_cast<float>(m_dialog->maxRemainingRatioSpinBox->value());
...
int SACSegmentation::compute() {
    ...
    if (!extractRecursive(cloudFiltered, cloudRemained, cloudExtractions,
                          m_maxIterations, m_probability, m_minRadiusLimits,
                          m_maxRadiusLimits, m_distanceThreshold, m_methodType,
                          m_modelType, m_normalDisWeight, m_maxRemainingRatio,
                          m_exportExtraction, m_recursiveMode, m_app)) {
        return -53;
    }
    ...
    if (group) {
        if (m_recursiveMode) {
            ...
            group->setName(group->getName() +
                           tr("-SingleSegmentation[Distance Threshold %1]")
                                   .arg(m_distanceThreshold));
        }

        unsigned count = group->getChildrenNumber();
        m_app->dispToConsole(tr("[SACSegmentation::compute] %1 extracted "
                                "segment(s) where created from cloud '%2'")
                                     .arg(cloudExtractions.size())
                                     .arg(cloud->getName()));

        if (error) {
            m_app->dispToConsole(tr("Error(s) occurred during the generation "
                                    "of segments! Result may be incomplete"),
...
int extractRecursive(PointCloudT::Ptr cloudFilterd,
                     PointCloudT::Ptr cloudRemained,
                     std::vector<PointCloudT::Ptr>& cloudExtractions,
                     int maxIterations,
                     float probability,
...
QString SACSegmentation::getErrorMessage(int errorCode) {
    switch (errorCode) {
        ...
        case -51:
            return tr(
                    "must select one of 'Export Extraction' and "
                    "'exportRemaining' or both");
        case -52:
            return tr(
        ...
        case -53:
            return tr(
                    "SAC Segmentation could not estimate a model for the given "
                    "dataset. Try relaxing your parameters");
        case -54:
