...
EuclideanClusterSegmentation::EuclideanClusterSegmentation()
	: BasePclModule(PclModuleDescription(tr("EuclideanCluster Segmentation"),
...
int EuclideanClusterSegmentation::openInputDialog()
{
	...
	if (!m_dialog)
		m_dialog = new EuclideanClusterDlg(m_app ? m_app->getActiveWindow() : 0);
...
int EuclideanClusterSegmentation::compute()
{
	...
	if (!PCLModules::EuclideanCluster<PointT>(
		xyzCloud, cluster_indices, m_clusterTolerance,
	...
	ccHObject* group = ecvTools::GetClousterGroup(
		cloud, clusterIndices, m_minClusterSize, m_randomClusterColor, error);
	...
	if (group)
	{
		...
		m_app->dispToConsole(
			tr("[EuclideanClusterSegmentation] %1 cluster(s) where created from cloud '%2'").
			arg(cluster_indices.size()).arg(cloud->getName()));
...
QString EuclideanClusterSegmentation::getErrorMessage(int errorCode)
{
	...
	case -53:
		return tr("EuclideanCluster Segmentation could not get any cluster or "
