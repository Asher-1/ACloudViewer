...
CorrespondenceMatching::CorrespondenceMatching()
	: BasePclModule(PclModuleDescription(tr("Correspondence Matching"),
...
int CorrespondenceMatching::openInputDialog()
{
	...
	if (!m_dialog)
		m_dialog = new CorrespondenceMatchingDialog(m_app);
...
void CorrespondenceMatching::getParametersFromDialog()
{
	...
	m_scales.clear();
	{
		...
		{
			m_app->dispToConsole(tr("Invalid scale parameters!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	...
	m_modelClouds.clear();
	{
		...
		{
			if (m_app)
				m_app->dispToConsole(tr("At least one cloud (model #1 or #2) was not defined!"),
					ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
		assert(cloud1 != cloud2);
		if (cloud1)
			m_modelClouds.push_back(cloud1);
		if (cloud2)
			m_modelClouds.push_back(cloud2);
	}
...
int CorrespondenceMatching::compute()
{
	...
	std::vector< std::vector< Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> > > rotoTranslationsVec;
	...
	pcl::PointCloud<DescriptorType>::Ptr sceneDescriptors(new pcl::PointCloud<DescriptorType>());
	...
	PointCloudT::Ptr scene = cc2smReader(m_sceneCloud).getXYZ2();
	{
		...
		if (m_useVoxelGrid)
		{
			...
			if (!PCLModules::VoxelGridFilter<PointType>(scene, tempCloud,
	...
	for (size_t i = 0; i < m_modelClouds.size(); ++i)
	{
		...
		if (modelCloud->width * modelCloud->height != 0)
		{
			...
			if (PCLModules::ComputeNormals<PointType, NormalType>(
				modelCloud, model_normals, m_normalKSearch, true, m_maxThreadCount))
			{
	...
	if (!PCLModules::GetUniformSampling<PointType>(scene, sceneKeypoints, m_sceneSearchRadius))
	{
		return -1;
	}
	CVLog::Print(tr("Scene total points: %1; Selected Keypoints: %2").arg(scene->size()).arg(sceneKeypoints->size()));
	for (size_t i = 0; i < modelClouds.size(); ++i)
	{
		PointCloudT::Ptr model_keypoints (new PointCloudT());
		if (!PCLModules::GetUniformSampling<PointType>(modelClouds[i], model_keypoints, m_modelSearchRadius))
		{
			return -1;
		}
		modelsKeypoints.push_back(model_keypoints);
		CVLog::Print(tr("Model %1 total points: %2; Selected Keypoints: %3").arg(i+1).arg(modelClouds[i]->size()).arg(model_keypoints->size()));
	}
	...
	for (size_t i = 0; i < modelClouds.size(); ++i)
	{
		...
		if (!PCLModules::EstimateShot<PointType, NormalType, DescriptorType>(
			modelClouds[i], modelsKeypoints[i], modelsWithNormal[i], model_descriptors,
			m_shotDescriptorRadius, m_maxThreadCount))
		{
			return -1;
		}
	...
	for (size_t j = 0; j < modelClouds.size(); ++j)
	{
		...
		CVLog::Print(tr("Correspondences found for model %1 : %2").arg(j+1).arg(model_scene_corrs->size()));
	...
	for (size_t j = 0; j < modelClouds.size(); ++j)
	{
		std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> > transformMatrix;
		...
		if (m_gcMode)
		{
			if (!PCLModules::EstimateGeometricConsistencyGrouping<PointType, PointType>(
				model_keypoints, sceneKeypoints, model_scene_corrs,
				transformMatrix, clustered_corrs, m_consensusResolution, m_gcMinClusterSize))
			{
				return -1;
			}
		}
		else
		{
			...
			if (!PCLModules::EstimateLocalReferenceFrame<PointType, NormalType, RFType>(
				model, model_keypoints, model_normals, model_rf, m_lrfRadius))
			{
				return -1;
			}

			if (!PCLModules::EstimateLocalReferenceFrame<PointType, NormalType, RFType>(
				scene, sceneKeypoints, sceneNormals, scene_rf, m_lrfRadius))
			{
				return -1;
			}

			//  Clustering
			if (!PCLModules::EstimateHough3DGrouping<PointType, PointType, RFType, RFType>(
				model_keypoints, sceneKeypoints, model_rf, scene_rf, model_scene_corrs,
				transformMatrix, clustered_corrs, m_houghBinSize, m_houghThreshold))
			{
				return -1;
			}
	...
	for (size_t j = 0; j < rotoTranslationsVec.size(); ++j)
	{
		...
		CVLog::Print(tr("Model %1 instances found number : %2").arg(j + 1).arg(rotoTranslationsVec[j].size()));
		...
		*/
		...
		for (size_t i = 0; i < rotoTranslationsVec[j].size(); ++i)
		{
			...
			pcl::transformPointCloud(*modelClouds[j], *rotated_model, rotoTranslationsVec[j][i]);
			...
			std::vector<pcl::PointCloud<PointType>::ConstPtr> registered_instances;
			{
				...
				for (size_t i = 0; i < rotoTranslationsVec[j].size(); ++i)
				{
					...
					if (PCLModules::ICPRegistration<PointType, PointType>(
						scene, instances[i], registered, m_icpMaxIterations, m_icpCorrDistance))
					{
						CVLog::Print(tr("Model %1 Instances %2 Aligned!").arg(j + 1).arg(i + 1));
					}
					else
					{
						CVLog::Print(tr("Model %1 Instances %2 Not Aligned!").arg(j + 1).arg(i + 1));
					}
...
//			CVLog::Print(tr("--- Hypotheses Verification End ---------"));
	...
	// 8. export result
	ccHObject* ecvGroup = new ccHObject(tr("correspondence-grouping-cluster(s)"));
		...
		for (int i = 0; i < static_cast<int>(instancesVec[j].size()); ++i)
		{
			...
			if (hypothesesMaskVec[j][i])
			{
				...
				ecvColor::Rgb col = ecvColor::Generator::Random();
				{
					...
					if (cloud)
					{
						QString outName = tr("%1-correspondence-%2-%3").arg(cloud->getName()).arg(j+1).arg(i+1);
...
void CorrespondenceMatching::applyTransformation(ccHObject* entity, const ccGLMatrixd& mat)
{
...
QString CorrespondenceMatching::getErrorMessage(int errorCode)
{
	...
	case -53:
		return tr("Correspondence Matching does not returned any point. Try relaxing your parameters");
	}
