...
TemplateAlignmentDialog::TemplateAlignmentDialog(ecvMainAppInterface* app)
    ...
    connect(template1CloudComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onCloudChanged(int)));
    connect(template2CloudComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onCloudChanged(int)));
...
void TemplateAlignmentDialog::refreshCloudComboBox() {
    if (m_app) {
        ...
        // the global cloud!
        template1CloudComboBox->setCurrentIndex(
                cloudCount > 0 ? (cloudCount > 2 ? 1 : 0) : -1);
        template2CloudComboBox->setCurrentIndex(
                cloudCount > 1 ? (cloudCount > 2 ? 2 : 1) : -1);

        if (cloudCount < 1 && m_app)
            m_app->dispToConsole(tr("You need at least 1 loaded clouds to "
                                    "perform alignment"),
...
float TemplateAlignmentDialog::getMaxCorrespondenceDistance() const {
    return static_cast<float>(maxCorrespondenceDistanceSpinBox->value() *
                              maxCorrespondenceDistanceSpinBox->value());
}
