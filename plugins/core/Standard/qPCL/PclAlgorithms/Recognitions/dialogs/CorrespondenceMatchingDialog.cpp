	...
	, m_app(app)
{
	...
	connect(model1CloudComboBox,	SIGNAL(currentIndexChanged (int)),	this,	SLOT(onCloudChanged(int)));
	connect(model2CloudComboBox,	SIGNAL(currentIndexChanged (int)),	this,	SLOT(onCloudChanged(int)));
...
void CorrespondenceMatchingDialog::refreshCloudComboBox()
{
	if (m_app)
	{
		...
		model1CloudComboBox->setCurrentIndex(cloudCount > 0 ? (cloudCount > 2 ? 1 : 0) : -1);
		model2CloudComboBox->setCurrentIndex(cloudCount > 1 ? (cloudCount > 2 ? 2 : 1) : -1);

		if (cloudCount < 1 && m_app)
			m_app->dispToConsole(tr("You need at least 1 loaded clouds to perform matching"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
	}
...
ccPointCloud* CorrespondenceMatchingDialog::getCloudFromCombo(QComboBox* comboBox, ccHObject* dbRoot)
{
