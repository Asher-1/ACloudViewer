...
TemplateAlignment::TemplateAlignment()
	: BasePclModule(PclModuleDescription(tr("Template Alignment"),
...
int TemplateAlignment::openInputDialog()
{
	...
	if (!m_dialog)
		m_dialog = new TemplateAlignmentDialog(m_app);
...
void TemplateAlignment::getParametersFromDialog()
{
	...
	m_templateClouds.clear();
	{
		...
		{
			if (m_app)
				m_app->dispToConsole(tr("At least one cloud (class #1 or #2) was not defined!"),
					ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
		assert(cloud1 != cloud2);
		if (cloud1)
			m_templateClouds.push_back(cloud1);
		if (cloud2)
			m_templateClouds.push_back(cloud2);
	}
...
int TemplateAlignment::compute()
{
	...
	{
		PointCloudT::Ptr cloudFiltered = cc2smReader(m_templateClouds[i]).getXYZ2();
	...
	CVLog::Print(tr("(values less than 0.00002 are good) Best fitness score: %1").arg(best_alignment.fitness_score));
...
void TemplateAlignment::applyTransformation(ccHObject* entity, const ccGLMatrixd& mat)
{
...
QString TemplateAlignment::getErrorMessage(int errorCode)
{
	...
	case -53:
		return tr("Template Alignment does not returned any point. Try relaxing your parameters");
	}
