...
ConvexConcaveHullReconstruction::ConvexConcaveHullReconstruction()
	: BasePclModule(PclModuleDescription(tr("ConvexConcaveHull Reconstruction"),
...
int ConvexConcaveHullReconstruction::openInputDialog()
{
	...
	if (!m_dialog)
		m_dialog = new ConvexConcaveHullDlg(m_app ? m_app->getActiveWindow() : 0);
...
int ConvexConcaveHullReconstruction::compute()
{
	...
	if (m_alpha > 0)
	{
		result = PCLModules::GetConcaveHullReconstruction<PointT>(xyzCloud, mesh, m_dimension, m_alpha);
	}
	else
	{
		result = PCLModules::GetConvexHullReconstruction<PointT>(xyzCloud, mesh, m_dimension);
	}
	if (result < 0)
		return -1;
	...
	if (m_alpha > 0)
	{
		CVLog::Print(tr("[Concave-Reconstruction] %1 points, %2 face(s)").arg(vertCount).arg(faceCount));
		out_mesh->setName(tr("Concave Reconstruction"));
	}
	else
	{
		CVLog::Print(tr("[Convex-Reconstruction] %1 points, %2 face(s)").arg(vertCount).arg(faceCount));
...
QString ConvexConcaveHullReconstruction::getErrorMessage(int errorCode)
{
	...
	case -53:
		return tr("Convex Concave Hull Reconstruction does not returned any point. Try relaxing your parameters");
    default:
        break;
	}
