...
ExtractSIFT::ExtractSIFT()
	: BasePclModule(PclModuleDescription(tr("Extract SIFT"),
...
int ExtractSIFT::compute()
{
	...
	QString name;
	if (m_mode == RGB)
		name = tr("SIFT Keypoints_%1_rgb_%2_%3_%4").arg(m_nr_octaves).arg(m_min_scale).arg(m_nr_scales_per_octave).arg(m_min_contrast);
	else
		name = tr("SIFT Keypoints_%1_%2_%3_%4_%5").arg(m_nr_octaves).arg(m_field_to_use_no_space.c_str()).arg(m_min_scale).arg(m_nr_scales_per_octave).arg(m_min_contrast);
