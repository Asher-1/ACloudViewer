...
static void SearchKDTree(fgr::KDTree& tree,
    ...
    for (size_t i = 0; i < cols; i++) query[i] = input(i);
...
static bool Similar(const Eigen::Vector3f& ACloud1,
                    const Eigen::Vector3f& BCloud1,
                    const Eigen::Vector3f& ACloud2,
                    const Eigen::Vector3f& BCloud2,
                    double maxRelativeScaleDiff) {
    float ABCloud1 = (BCloud1 - ACloud1).norm();
    float ABCloud2 = (BCloud2 - ACloud2).norm();
    return (maxRelativeScaleDiff * ABCloud1 < ABCloud2) &&
           (ABCloud2 * maxRelativeScaleDiff < ABCloud1);
}
...
void CApp::AdvancedMatching(bool crossCheck /*=true*/,
    ...
    Correspondences corres;
    {
        ...
        for (int indexInC2 = 0; indexInC2 < cloud2Size; ++indexInC2) {
            ...
            SearchKDTree(feature_tree_cloud1, cloud2Features[indexInC2],
                         nearestIndexInC1, squareDistance);
            assert(nearestIndexInC1 != -1);

            // Then check the 'backward' correspondance starting from the
            // nearest point in cloud1 (if not already done)
            if (cloud1_to_cloud2[nearestIndexInC1] == -1) {
                int nearestIndexInC2 = -1;
                SearchKDTree(feature_tree_cloud2,
                             cloud1Features[nearestIndexInC1], nearestIndexInC2,
                             squareDistance);
                assert(nearestIndexInC2 != -1);
                cloud1_to_cloud2[nearestIndexInC1] = nearestIndexInC2;

                if (!crossCheck) {
                    // add the corresponding 'backward' correspondance from
                    // cloud1 to cloud2 to the global correspondances list
                    corres.push_back(Pair(nearestIndexInC1, nearestIndexInC2));
                }
            }

            if (!crossCheck ||
                indexInC2 == cloud1_to_cloud2[nearestIndexInC1]) {
    ...
    CVLog::Print(QString("Number of pairs %1 cross-check: %2")
                         .arg(crossCheck ? "with" : "without")
                         .arg(corres.size()));
    ...
    if (tupleConstraint) {
        ...
        for (size_t i = 0; i < number_of_trial; i++) {
            ...
            if (rand1 == rand0) continue;
            size_t rand2 = (static_cast<size_t>(rand()) % ncorr);
            if (rand2 == rand0 || rand2 == rand1) continue;
            ...
            if (Similar(ptCloud1_0, ptCloud1_1, ptCloud2_0, ptCloud2_1,
                        tuple_scale_) &&
                Similar(ptCloud1_0, ptCloud1_2, ptCloud2_0, ptCloud2_2,
                        tuple_scale_) &&
                Similar(ptCloud1_1, ptCloud1_2, ptCloud2_1, ptCloud2_2,
                        tuple_scale_)) {
                // confirm the pairs
        ...
        CVLog::Print("Number of pairs after tuple constraint: " +
                     QString::number(corres.size()));
    }
...
void CApp::NormalizePoints() {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        float maxNorm = 0;
        for (size_t ii = 0; ii < npti; ++ii) {
            float norm = pointClouds_[i][ii].norm();  // because we extract mean
                                                      // in the previous stage.
            if (norm > maxNorm) maxNorm = norm;
        }

        if (maxNorm > scale) scale = maxNorm;
    }
...
bool CApp::OptimizePairwise(bool decrease_mu) {
    ...
    if (correspondances_.size() < 10) return false;
    ...
    for (size_t itr = 0; itr < iteration_number_; itr++) {
        ...
        aff_mat.linear() = static_cast<Eigen::Matrix3d>(
...
Eigen::Matrix4f CApp::GetOutputTrans() const {
    ...
    transTemp.block<3, 1>(0, 3) = -R * means_[AlignedIndex] + t * globalScale_ +
                                  means_[ReferenceIndex];
    transTemp(3, 3) = 1;
