...
void GeneralFilters::getParametersFromDialog() {
    ...
    switch (m_dialog->tab->currentIndex()) {
        ...
        case 0: {
            m_comparisonField = m_dialog->getComparisonField(m_minMagnitude,
                                                             m_maxMagnitude);
        ...
        case 2: {
            // m_sampleAllData =
            // m_dialog->downsampleAllDataCheckBox->isChecked();
        ...
        case 3: {
            bool m_extractRemainings =
                    m_dialog->extractRemainingCheckBox->isChecked();
            int m_maxWindowSize = m_dialog->maxWindowSizeSpinBox->value();
            float m_slope = static_cast<float>(m_dialog->slopeSpinBox->value());
            float m_initialDistance = static_cast<float>(
                    m_dialog->initialDistanceSpinBox->value());
            float m_maxDistance =
                    static_cast<float>(m_dialog->maxDistanceSpinBox->value());
    ...
    switch (m_filterType) {
        case GeneralFilters::PASS_FILTER: {
            if (m_hasColors) {
                ...
                if (!PCLModules::PassThroughFilter<PointRGB>(
                            rgbCloud, tempCloud, m_comparisonField,
                            m_minMagnitude, m_maxMagnitude)) {
                    return -1;
                }
                ...
                if (!PCLModules::PassThroughFilter<PointT>(
                            xyzCloud, tempCloud, m_comparisonField,
                            m_minMagnitude, m_maxMagnitude)) {
                    return -1;
                }
                TO_PCL_CLOUD(*tempCloud, *out_cloud_sm);
        ...
        case GeneralFilters::CR_FILTER: {
            ...
            PCLModules::ConditionParameters param;
            {
                ...
                for (const QString& type : m_comparisonTypes) {
                    ...
                    if (type == "GT") {
                        comparison.comparison_type_ = PCLModules::
                                ConditionParameters::ComparisonType::GT;
                    } else if (type == "GE") {
                        comparison.comparison_type_ = PCLModules::
                                ConditionParameters::ComparisonType::GE;
                    } else if (type == "LT") {
                        comparison.comparison_type_ = PCLModules::
                                ConditionParameters::ComparisonType::LT;
                    } else if (type == "LE") {
                        comparison.comparison_type_ = PCLModules::
                                ConditionParameters::ComparisonType::LE;
                    } else if (type == "EQ") {
                        comparison.comparison_type_ = PCLModules::
                                ConditionParameters::ComparisonType::EQ;
                    }
        ...
        case GeneralFilters::VOXEL_FILTER: {
            if (m_hasColors) {
                ...
                if (!PCLModules::VoxelGridFilter<PointRGB>(
                            rgbCloud, tempCloud, m_leafSize, m_leafSize,
                            m_leafSize)) {
                    return -1;
                }
        ...
        case GeneralFilters::PM_FILTER: {
            if (m_hasColors) {
                ...
                if (!PCLModules::ProgressiveMpFilter<PointRGB>(
                            rgbCloud, ground, m_maxWindowSize, m_slope,
                            m_initialDistance, m_maxDistance) ||
                ...
                if (m_extractRemainings) {
                    if (!PCLModules::ExtractIndicesFilter<PointRGB>(
                                rgbCloud, ground, nullptr, tempCloud)) {
                        return -1;
                    }
                } else {
                    if (!PCLModules::ExtractIndicesFilter<PointRGB>(
                                rgbCloud, ground, tempCloud, nullptr)) {
                        return -1;
                    }
                }
                ...
                pcl::PointIndicesPtr ground(new pcl::PointIndices);
                if (!PCLModules::ProgressiveMpFilter<PointT>(
                            xyzCloud, ground, m_maxWindowSize, m_slope,
                            m_initialDistance, m_maxDistance) ||
                    ground->indices.size() == 0) {
                    return -1;
                }
                ...
                if (m_extractRemainings) {
                    if (!PCLModules::ExtractIndicesFilter<PointT>(
                                xyzCloud, ground, nullptr, tempCloud)) {
                        return -1;
                    }
                } else {
                    if (!PCLModules::ExtractIndicesFilter<PointT>(
                                xyzCloud, ground, tempCloud, nullptr)) {
                        return -1;
                    }
                }
        ...
        case GeneralFilters::HULL_FILTER: {
            ...
            if (m_hasColors) {
                PointCloudRGB::Ptr tempCloud(new PointCloudRGB);
                if (!PCLModules::CropHullFilter<PointRGB>(
                            rgbCloud, boundary, tempCloud, m_dimension)) {
                    return -1;
                }
                TO_PCL_CLOUD(*tempCloud, *out_cloud_sm);
            } else {
                PointCloudT::Ptr tempCloud(new PointCloudT);
                if (!PCLModules::CropHullFilter<PointT>(
                            xyzCloud, boundary, tempCloud, m_dimension)) {
                    return -1;
                }
                TO_PCL_CLOUD(*tempCloud, *out_cloud_sm);
            }
...
QString GeneralFilters::getErrorMessage(int errorCode) {
    switch (errorCode) {
        ...
        case -53:
            return tr(
                    "General Filter does not returned any point. Try relaxing "
                    "your parameters");
    }
