...
qPclIO::qPclIO(QObject* parent)
    : QObject(parent), ccIOPluginInterface(":/ECV/plugin/qPclIO/info.json") {}
...
public:
    PCDCommand()
        : ccCommandLineInterface::Command("Set PCD output format",
    ...
    bool process(ccCommandLineInterface& cmd) override {
        if (cmd.arguments().size() == 0) {
            return cmd.error(
                    QObject::tr("Missing argument after %1: output format type")
                            .arg(COMMAND_PCD_OUTPUT_FORMAT));
        }
...
void qPclIO::registerCommands(ccCommandLineInterface* cmd) {
    if (cmd) {
        cmd->registerCommand(
                ccCommandLineInterface::Command::Shared(new PCDCommand));
    }
}
