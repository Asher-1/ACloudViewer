...
ccPolyline* ProfileLoader::Load(QString filename, CCVector3& origin, ecvMainAppInterface* app/*=0*/)
{
	...
	{
		if (app)
			app->dispToConsole(QString("Failed to open file for reading! Check access rights"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return 0;
	}
	...
	for (unsigned n=0; n<1; ++n) //fake loop for easy break ;)
	{
		//read origin
		{
			...
			{
				if (app)
					app->dispToConsole(QString("Malformed file (origin header expected on first line)"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				error = true;
				break;
			}
			QString centerLine = stream.readLine();
			{
				...
				{
					if (app)
						app->dispToConsole(QString("Malformed file (origin coordinates expected on second line)"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					error = true;
					break;
				}
			}
		...
		//read elevations
		{
			...
			{
				if (app)
					app->dispToConsole(QString("Malformed file (radii/heights header expected on third line)"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				error = true;
				break;
			}
			...
			while (!line.isEmpty())
			{
				...
				{
					if (app)
						app->dispToConsole(QString("Malformed file (radius/height couple expected from the 4th line and afterwards)"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					error = true;
					break;
				}
				...
				catch (const std::bad_alloc&)
				{
					//not enough memory
					if (app)
						app->dispToConsole(QString("Not enough memory!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					error = true;
					break;
				}
			...
			//convert 2D points to polyline
			{
				...
				{
					if (app)
						app->dispToConsole(QString("Not enough points in profile?!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
