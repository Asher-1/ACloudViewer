...
//	const double xMax = profileBBMax.x;
	...
	{
		if (app)
			app->dispToConsole(QString("Internal error: null profile?!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	{
		if (app)
			app->dispToConsole(QString("Failed to open '%1' file for writing!").arg(filename),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
