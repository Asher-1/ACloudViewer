...
public:
    struct ProfileMetaData {
        ...
        PointCoordinateType heightShift;  //!< virtual shift of the profile
                                          //!< coordinates along the revolution
                                          //!< axis and relative to the origin
        bool hasAxis;  //!< whether a custom revolution axis has been defined or
                       //!< not
        CCVector3 axis;  //!< custom revolution axis
    };

    //! Returns the whole set of meta-data associated to a given
    //! polyline/profile
    /** \return whether the profile is valid or not
     **/
    static bool GetPoylineMetaData(const ccPolyline* polyline,
    ...
    **/
    ...
    //! Returns the revolution 'dimension' associated to a given profile
    //! (polyline)
    /** Requires the right meta-data to be set (see SetPoylineRevolDim).
            \return 0 (X), 1 (Y), 2 (Z) or -1 if no revolution dimension is
    defined
    **/
    ...
    **/
     ...
     **/
    static void SetPolylineHeightShift(ccPolyline* polyline,
    ...
    **/
    static bool GetPolylineHeightShift(const ccPolyline* polyline,
                                       PointCoordinateType& heightShift);
    ...
    //! Projects a cloud (scalar field) on a revolution surface to generate a 2D
    //! map
    /** Projection can be either cylindrical or spherical.
            Warning: for cylindrical projection, the 2D map 'height' will be
    expressed relatively to the 'revolutionOrigin' and not the cloud's
    (implicit) origin.
    **/
    static QSharedPointer<Map> CreateMap(
            ccPointCloud* cloud,
            ccScalarField* sf,
            const ccGLMatrix& cloudToSurface,  // e.g. translation to the
                                               // revolution origin
    ...
    //! Returns the constant 'n' factor of a Lambert conform conical projection
    //! with two intersecting parallels
    static double ConicalProjectN(double phi1, double phi2);

    //! Projects a given latitude with a Lambert conform conical projection with
    //! two intersecting parallels
    ...
    static bool ConvertCloudToCylindrical(
            ccPointCloud* cloud,
            const ccGLMatrix& cloudToSurface,  // e.g. translation to the
                                               // revolution origin
            unsigned char revolutionAxisDim,
     ...
     **/
    static bool ConvertCloudToConical(
            ccPointCloud* cloud,
            const ccGLMatrix& cloudToSurface,  // e.g. translation to the
                                               // revolution origin
            unsigned char revolutionAxisDim,
    ...
    //! Computes min and max latitudes of a point cloud (relatively to an axis
    //! and an 'origin')
    static bool ComputeMinAndMaxLatitude_rad(
            ccPointCloud* cloud,
            double& minLat_rad,
            double& maxLat_rad,
    ...
    static QImage ConvertMapToImage(
    ...
    static ccMesh* ConvertProfileToMesh(
            ccPolyline* profile,
            const ccGLMatrix&
                    cloudToProfile,  // e.g. translation to the profile origin
            bool counterclockwise,
            unsigned angularSteps = 36,
            QImage mapTexture = QImage());

    //! Various "measures" that can be computed with the map (surfacces,
    //! volumes)
    struct Measures {
        double total;
        double theoretical;
    ...
    //! Computes various surfaces and volumes (see Surfaces & Volumes
    //! structures)
    /** Warning: the 'positive' resp. 'negative' volumes coorespond to
    differences of volumes (i.e the volume that is exterior resp. interori to
    the theoretical profile). However, the 'positive' resp. 'negative' surfaces
    correspond to the surface of all the elements that have a positive resp.
    negative deviation.
    **/
