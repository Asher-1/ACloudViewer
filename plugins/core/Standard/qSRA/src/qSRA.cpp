...
QList<QAction *> qSRA::getActions()
{
	...
	if (!m_doLoadProfile)
	{
		...
		m_doLoadProfile->setToolTip(tr("Loads the 2D profile of a Surface of Revolution (from a dedicated ASCII file)"));
	...
	{
		m_doCompareCloudToProfile = new QAction(tr("Cloud-SurfRev radial distance"), this);
		m_doCompareCloudToProfile->setToolTip(tr("Computes the radial distances between a cloud and a Surface of Revolution (polyline/profile, cone or cylinder)"));
	...
	if (!m_doProjectCloudDists)
	{
		...
		m_doProjectCloudDists->setToolTip(tr("Creates the 2D deviation map (radial distances) from a Surface or Revolution (unroll)"));
...
void qSRA::onNewSelection(const ccHObject::Container& selectedEntities)
{
	...
	{
		//we expect a cloud...
		int cloudIndex = selectedEntities[0]->isA(CV_TYPES::POINT_CLOUD) ? 0 : selectedEntities[1]->isA(CV_TYPES::POINT_CLOUD) ? 1 : -1;
		if (cloudIndex != -1)
		{
			//... and either a polyline or a cone/cylinder
			validSelection = (selectedEntities[1 - cloudIndex]->isA(CV_TYPES::POLY_LINE) || selectedEntities[1 - cloudIndex]->isKindOf(CV_TYPES::CONE));
		}
	}
...
ccHObject* GetDefaultContainer(ecvMainAppInterface* app)
{
	...
	app->dbRootObject()->filterChildren(groups,true,CV_TYPES::HIERARCHY_OBJECT);
...
void qSRA::loadProfile() const
{
	...
	{
		if (m_app)
			m_app->dispToConsole(tr("Failed to load file '%1'!").arg(filename), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	m_app->dispToConsole(tr("[qSRA] File '%1' successfully loaded").arg(filename), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
}
...
void qSRA::computeCloud2ProfileRadialDist() const
{
	...
	bool tempPolyline = false;
	{
		for (unsigned i = 0; i < 2; ++i)
		{
			...
			else if (!polyline && selectedEntities[i]->isKindOf(CV_TYPES::CONE))
			{
	...
	else
	{
		if (m_app)
			m_app->dispToConsole(
				tr("Select exactly one cloud and one Surface of Revolution (polyline/profile, cone or cylinder)"),
...
void qSRA::doProjectCloudDistsInGrid(ccPointCloud* cloud, ccPolyline* polyline) const
{
	...
	ccScalarField* sf = 0;
	{
		...
		if (sfIdx < 0)
		{
			...
			{
				CVLog::Error(tr("Cloud has no no '%1' field and no active scalar field!").arg(RADIAL_DIST_SF_NAME));
				
				//additional indications
				if (m_doCompareCloudToProfile)
				{
					CVLog::Warning(tr("You can compute the radial distances with the '%1' method").arg(m_doCompareCloudToProfile->text()));
				}
				return;
			}
		}
