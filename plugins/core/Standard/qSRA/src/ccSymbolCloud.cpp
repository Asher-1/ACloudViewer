...
ccSymbolCloud::ccSymbolCloud(QString name/*=QString()*/)
	...
	, m_showSymbols(true)
	, m_showLabels(true)
	, m_labelAlignFlags(ecvDisplayTools::ALIGN_HMIDDLE | ecvDisplayTools::ALIGN_VBOTTOM)
{
}
	...
	if (MACRO_Draw2D(context) && MACRO_Foreground(context))
	{
		...
		//draw symbols + labels
		{
			for (unsigned i = 0; i < numberOfPoints; i++)
			{
				...
				{
					drawSymbolAt(Q2D.x - context.glW / 2, Q2D.y - context.glH / 2, m_symbolSize / 2);
				}
				...
				{
					//draw label
					ecvDisplayTools::DisplayText(	m_labels[i],
