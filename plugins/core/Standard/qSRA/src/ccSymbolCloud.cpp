...
bool ccSymbolCloud::reserve(unsigned numberOfPoints) {
    ...
    if (m_labels.size()) return reserveLabelArray(numberOfPoints);

    return true;
}

bool ccSymbolCloud::resize(unsigned numberOfPoints) {
    if (!ccPointCloud::resize(numberOfPoints)) return false;

    if (m_labels.size()) return resizeLabelArray(numberOfPoints);
...
void ccSymbolCloud::clearLabelArray() { m_labels.clear(); }
...
void ccSymbolCloud::drawMeOnly(CC_DRAW_CONTEXT& context) {
    ...
    if (MACRO_Draw2D(context) && MACRO_Foreground(context)) {
        ...
        // default color
        const ecvColor::Rgb* color = &context.pointsDefaultCol;
        ...
        // draw symbols + labels
        {
            for (unsigned i = 0; i < numberOfPoints; i++) {
                ...
                // we must reset the color each time as the call to displayText
                // may change the active color! glFunc->glColor3ubv(color->rgb);

                // draw associated symbol
                if (m_showSymbols && m_symbolSize > 0.0) {
                    drawSymbolAt(Q2D.x - context.glW / 2,
                                 Q2D.y - context.glH / 2, m_symbolSize / 2);
                }
