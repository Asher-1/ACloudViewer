													...
													ecvMainAppInterface* app/*=0*/)
{
	...
	{
		if (app)
			app->dispToConsole(QString("Internal error: invalid input parameters"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	{
		if (app)
			app->dispToConsole(QString("Invalid polyline (not enough vertices)"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	{
		if (app)
			app->dispToConsole(QString("Invalid polyline (bad or missing meta-data)"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	{
		if (app)
			app->dispToConsole(QString("Failed to allocate a new scalar field for computing distances! Try to free some memory ..."), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	if (storeRadiiAsSF)
	{
		...
		{
			if (app)
				app->dispToConsole(QString("Failed to allocate a new scalar field for storing radii! You should try to free some memory ..."), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			//return false;
		}
		else
		{
																					...
																					ecvMainAppInterface* app/*=0*/)
{
	...
	{
		if (app)
			app->dispToConsole(QString("[DistanceMapGenerationTool] Internal error: invalid input structures!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
	...
	{
		if (app)
			app->dispToConsole(QString("[DistanceMapGenerationTool] Internal error: invalid grid parameters!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return QSharedPointer<Map>(0);
	}
	...
	{
		if (app)
			app->dispToConsole(QString("[DistanceMapGenerationTool] Cloud is empty! Nothing to do!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return QSharedPointer<Map>(0);
	}
	...
	unsigned xSteps = 0;
	{
		...
		{
			if (app)
				app->dispToConsole(QString("[DistanceMapGenerationTool] Invalid longitude step/boundaries! Can't generate a proper map!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return QSharedPointer<Map>(0);
		}
	}
	...
	unsigned ySteps = 0;
	{
		...
		{
			if (app)
				app->dispToConsole(QString("[DistanceMapGenerationTool] Invalid latitude step/boundaries! Can't generate a proper map!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return QSharedPointer<Map>(0);
		}
	}
	...
	if (app)
		app->dispToConsole(QString("[DistanceMapGenerationTool] Projected map size: %1 x %2 (%3 cells)").arg(xSteps).arg(ySteps).arg(cellCount),ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	{
		if (app)
			app->dispToConsole(QString("[DistanceMapGenerationTool] Not enough memory!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return QSharedPointer<Map>(0);
	}
	...
	else if (emptyCellfillOption == FILL_INTERPOLATE)
	{
		...
		if (fillCount)
		{
			...
			catch (...)
			{
				//out of memory
				if (app)
					app->dispToConsole(QString("[DistanceMapGenerationTool] Not enough memory to interpolate!"),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			}
			...
			if (the2DPoints.capacity() == fillCount)
			{
				...
				{
					if (app)
						app->dispToConsole(QString("[DistanceMapGenerationTool] Interpolation failed: Triangle lib. said '%1'").arg(errorStr),ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				}
				else
				{
													...
													ecvMainAppInterface* app/*=0*/)
{
	...
	{
		if (app)
			app->dispToConsole(QString("[SaveMapAsCSVMatrix] Internal error: invalid input map!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	{
		if (app)
			app->dispToConsole(QString("[SaveMapAsCSVMatrix] Failed to open file for writing! Check access rights"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
			...
			for (unsigned i = 0; i < map->xSteps; ++i, ++cell)
			{
				...
				if (cell->count != 0)
				{
					...
					if (relativePos < 0.0)
						...
						relativePos = 1.0;
					rgb = colorScale->getColorByRelativePos(relativePos, colorScaleSteps, &ecvColor::lightGrey);
				}
