...
bool isAvailable() {
    if (!g_checked) {
        // probeAvailable() lives in BevRemapCudaProbe.cpp: on Windows the
        // cudart DLL is delay-loaded so the probe is SEH-protected there
        // (nvcc cannot parse __try/__except in .cu); on Linux/macOS cudart is
        // statically linked and the call just returns an error code.
        g_available = probeAvailable();
        g_checked = true;
        if (g_available) {
            CVLog::Print("[BevCuda] CUDA remap backend available");
        }
    }
    return g_available;
}
