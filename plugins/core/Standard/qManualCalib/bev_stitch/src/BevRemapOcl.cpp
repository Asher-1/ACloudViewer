...
// macOS ships <OpenCL/cl.h> (Xcode SDK usr/include + OpenCL.framework) instead
// of the Linux/Windows-style <CL/cl.h>; follow OpenCV's opencl_core convention.
...
#else  // !MCALIB_BEV_OPENCL_ENABLED

// Stub implementation used when the OpenCL backend is disabled (no OpenCL
// development package / CL/cl.h header available). Callers guard every
// bev_ocl:: call with MCALIB_BEV_OPENCL_ENABLED, so these are never invoked;
// they only exist to keep the library linkable if this file is compiled
// without the macro for any reason.

namespace mcalib {
namespace bev_ocl {

bool probePlatform() { return false; }

bool isAvailable() { return false; }

bool remap(const cv::Mat&, cv::Mat&, const cv::Mat&, const cv::Mat&) {
    return false;
}

bool projectPoints(const float*,
                   int,
                   const float[9],
                   const float[3],
                   float,
...
bool projectPointsKb(const float*,
                     int,
                     const float[9],
                     const float[3],
                     float,
...
bool initAlphaFusion(cv::Size,
                     const std::vector<std::string>&,
                     const std::map<std::string, cv::Mat>&) {
    return false;
}

void updateAlphaFusionWeights(const std::map<std::string, cv::Mat>&) {}

void releaseAlphaFusion() {}

bool alphaFusion(const std::vector<std::string>&,
                 const std::map<std::string, cv::Mat>&,
                 cv::Mat&) {
    return false;
}
...
#endif  // MCALIB_BEV_OPENCL_ENABLED
