...
BevRemapMode BirdEyeView::getActiveRemapMode() const {
    // Resolve the REQUESTED mode against the current runtime (CUDA/OpenCL
    // availability). Must not consult existing backends: buildRemapMaps/
    // buildAlphaFusion use this value to detect a backend switch (e.g. user
    // changes OpenCL -> CPU) and would otherwise always see the old backend's
    // mode and keep reusing it ("CPU (resolved OpenCL)" symptom).
