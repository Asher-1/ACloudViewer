...
// The sample bag is no longer shipped in the repo. It is downloaded on demand
// into the shared test-data repository (~/cloudViewer_data/extract/
// qcalib_test_data) by the plugin's "use test data" button. We resolve the bag
// from the repository first, then fall back to the in-tree path (dev builds or
// user-provided custom data). If none exists the bag-based tests are skipped.
static std::string resolveTestBag() {
    const std::string rel = "bags/sample_aligned.bag";
    std::vector<std::string> candidates;

    if (const char* root = std::getenv("CLOUDVIEWER_DATA_ROOT")) {
        candidates.push_back(std::string(root) + "/extract/qcalib_test_data/" +
                             rel);
    }
    if (const char* home = std::getenv("HOME")) {
        candidates.push_back(std::string(home) + "/cloudViewer_data/extract/" +
                             "qcalib_test_data/" + rel);
    }
#ifdef _WIN32
    if (const char* up = std::getenv("USERPROFILE")) {
        candidates.push_back(std::string(up) + "/cloudViewer_data/extract/" +
                             "qcalib_test_data/" + rel);
    }
#endif
    // In-tree / user-provided custom data (dev builds, CI cache, etc.)
    candidates.push_back(testDataPath(rel.c_str()));
...
static std::string TEST_BAG = resolveTestBag();
...
int main() {
    ...
    if (TEST_BAG.empty()) {
        std::cout << "SKIP: sample_aligned.bag not found. Download it via the "
                     "plugin's "
                     "'use test data' button (cached at "
                     "~/cloudViewer_data/extract/qcalib_test_data) or place it "
                     "at "
                  << testDataPath("bags/sample_aligned.bag")
                  << ". Running only bag-independent tests." << std::endl;
