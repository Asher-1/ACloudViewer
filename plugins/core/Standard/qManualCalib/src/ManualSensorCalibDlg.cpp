...
#include "TestDataDownloadHelper.h"
...
ManualSensorCalibDlg::~ManualSensorCalibDlg() {
    ...
    if (m_batchExportWatcher) {
        ...
        m_batchExportWatcher->waitForFinished();
        delete m_batchExportWatcher;
        m_batchExportWatcher = nullptr;
    }
...
void ManualSensorCalibDlg::setupUI() {
    ...
    // -- One-click test data (downloads + extracts sample ROS bag/configs) --
    auto* testDataRow = new QHBoxLayout;
    testDataRow->setSpacing(2);
    m_btnTestData = new QPushButton(QStringLiteral("🧪  use test data"));
    m_btnTestData->setToolTip(tr(
            "Auto-download the sample ROS bag + sensor configs, then load "
            "them. "
            "User-selected config/bag remains supported via steps 1-2 below."));
    testDataRow->addWidget(m_btnTestData);
    controlLayout->addLayout(testDataRow);
    auto* testDataHint = new QLabel(
            tr("Download & load the sample bag + configs (no custom data "
               "needed)."));
    testDataHint->setStyleSheet("color: #888; font-size: 11px;");
    testDataHint->setWordWrap(true);
    controlLayout->addWidget(testDataHint);
    connect(m_btnTestData, &QPushButton::clicked, this,
            &ManualSensorCalibDlg::onUseTestData);
    ...
    // Only list backends that are actually usable on this machine: probe the
    // runtime (cudaAvailable/openclAvailable), not the compile-time flags, so
    // e.g. a CUDA build running on a host without a CUDA toolkit / NVIDIA
    // driver does not offer CUDA here. The plugin itself always loads; Auto
    // and CPU are always offered, OpenCL only when a platform is present.
    const bool cudaOk = mcalib::BevRemapper::cudaAvailable();
    const bool oclOk = mcalib::BevRemapper::openclAvailable();
    ...
    if (cudaOk) {
        m_cmbBevRemapBackend->addItem(
                tr("CUDA"), static_cast<int>(mcalib::BevRemapMode::CUDA));
        m_cmbBevRemapBackend->setItemData(m_cmbBevRemapBackend->count() - 1,
                                          tr("NVIDIA GPU remap + alpha fusion"),
                                          Qt::ToolTipRole);
    }
    if (oclOk) {
        m_cmbBevRemapBackend->addItem(
        ...
        m_cmbBevRemapBackend->setItemData(
                m_cmbBevRemapBackend->count() - 1,
                tr("OpenCL GPU remap + alpha fusion (macOS default)"),
                Qt::ToolTipRole);
    }
    m_cmbBevRemapBackend->addItem(tr("CPU"),
                                  static_cast<int>(mcalib::BevRemapMode::CPU));
    m_cmbBevRemapBackend->setItemData(m_cmbBevRemapBackend->count() - 1,
                                      tr("Force CPU parallel remap + blend"),
                                      Qt::ToolTipRole);
...
void ManualSensorCalibDlg::onUseTestData() {
    qmcalib_ui::runManualCalibTestDataProvision(this, [this](bool ok) {
        if (!ok) {
            m_lblStatus->setText(tr("Test data download/extract failed."));
            return;
        }
        const QString root = ecvTestDataRepository::extractPath(
                ecvTestDataRepository::Dataset::ManualCalib);
        const QString configs =
                ecvTestDataRepository::getManualCalibConfigDir(root);
        const QString bag = ecvTestDataRepository::getManualCalibBagPath(root);
        if (configs.isEmpty() || bag.isEmpty()) {
            m_lblStatus->setText(
                    tr("Test data extracted but config/bag not found."));
            return;
        }
        // Reuse the standard load path so cfg + bag are parsed
        // identically to user-selected data.
        if (!loadConfigFromDir(configs)) return;
        loadBagFromInputPath(bag, /*autoSelectSession=*/true);
    });
}
...
bool ManualSensorCalibDlg::loadConfigFromDir(const QString& dir) {
...
void ManualSensorCalibDlg::onLoadBag() {
    ...
    m_bagPath = selected;
    loadBagFromInputPath(selected);
}

bool ManualSensorCalibDlg::loadBagFromInputPath(const QString& selected,
                                                bool autoSelectSession) {
    ...
    if (autoSelectSession) {
        // Auto-select the preferred / first session for one-click test data.
        session_index = 0;
    } else {
        session_index = mcalib::ui::pickBagSessionIndex(
    ...
    m_bagReader = std::make_shared<mcalib::RosBagReader>();
...
void ManualSensorCalibDlg::exportBevImages(const std::string& output_dir,
    ...
    startBatchExportJob(tr("BEV Batch Export"), nullptr, output_qdir,
                        [reader = m_bagReader, ctx, output_dir, options](
