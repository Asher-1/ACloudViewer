...
void ManualAvmAdjustDlg::setupUI() {
    ...
    m_btnTestData->setToolTip(
            tr("Auto-download the sample sensor configs + ROS bag, then load "
               "them. User-selected config/bag remains supported via steps 1-2 "
               "below."));
    controlLayout->addWidget(m_btnTestData);
    auto* testDataHint = new QLabel(
            tr("Download & load the sample configs + bag (no custom data "
               "needed)."));
    ...
    connect(m_btnTestData, &QPushButton::clicked, this,
            &ManualAvmAdjustDlg::onUseTestData);
...
void ManualAvmAdjustDlg::loadConfigFromDir(const QString& dir) {
    m_configPath = dir;
...
void ManualAvmAdjustDlg::onUseTestData() {
...
void ManualAvmAdjustDlg::onLoadBag() {
    ...
    loadBagFromInputPath(selected, /*autoSelectSession=*/false);
}

void ManualAvmAdjustDlg::loadBagFromInputPath(const QString& selected,
    ...
    if (autoSelectSession) {
        ...
        for (size_t i = 0; i < discovery.sessions.size(); ++i) {
            if (!preferred.empty() &&
                discovery.sessions[i].session_key == preferred) {
                session_index = static_cast<int>(i);
                break;
            }
        }
    } else {
