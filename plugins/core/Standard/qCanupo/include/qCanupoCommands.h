...
struct CommandCanupoClassif : public ccCommandLineInterface::Command {
    CommandCanupoClassif()
        : ccCommandLineInterface::Command("Canupo Classify",
                                          COMMAND_CANUPO_CALSSIFY) {}

    virtual bool process(ccCommandLineInterface& cmd) override {
        cmd.print("[CANUPO]");
        if (cmd.arguments().empty()) {
            return cmd.error(QString("Missing parameter: classifier filename "
                                     "(.prm) after \"-%1\"")
                                     .arg(COMMAND_CANUPO_CALSSIFY));
        }
        ...
        while (!cmd.arguments().empty()) {
            ...
            if (ccCommandLineInterface::IsCommand(argument,
                                                  COMMAND_CANUPO_CONFIDENCE)) {
                // local option confirmed, we can move on
                cmd.arguments().pop_front();

                if (cmd.arguments().empty()) {
                    return cmd.error(QString("Missing parameter: confidence "
                                             "threshold after '%1'")
                                             .arg(COMMAND_CANUPO_CONFIDENCE));
                }
                ...
                if (!ok || params.confidenceThreshold < 0.0) {
                    return cmd.error(QString("Invalid parameter: confidence "
                                             "threshold after '%1'")
                                             .arg(COMMAND_CANUPO_CONFIDENCE));
                }

                cmd.print(QString("Confidence threshold set to %1")
                                  .arg(params.confidenceThreshold));
            } else {
        ...
        for (CLCloudDesc& desc : cmd.clouds()) {
            ...
            if (qCanupoProcess::Classify(classifierFilename, params, desc.pc,
                                         corePoints, corePointsDescriptors,
                                         realCorePoints, nullptr, nullptr,
                                         cmd.silentMode())) {
                if (cmd.autoSaveMode()) {
