...
public:
    ...
    /** Scales are always called in decreasing order.
            ...
            \param[out] invalidScale whether this scale is 'invalid' (i.e.
    parameters couldn't be computed, default one have been returned instead)
...
struct CorePointDesc {
    //! Parameters array (its size should be 'number of scales' * 'dimensions
    //! per scale')
