        ...
        for (size_t i = 0; i < clouds.size(); ++i) {
            if (clouds[i]->isA(CV_TYPES::POINT_CLOUD))  // as filterChildren
                ...
                cpOtherCloudComboBox->addItem(
                        qCanupoTools::GetEntityName(clouds[i]),
                        QVariant(clouds[i]->getUniqueID()));
        }
    }

    connect(browseToolButton, SIGNAL(clicked()), this,
            SLOT(browseClassifierFile()));
    connect(mscBrowseToolButton, SIGNAL(clicked()), this,
            SLOT(browseMscFile()));
...
double qCanupoClassifDialog::getConfidenceTrehshold() const {
    if (!useConfThresholdGroupBox->isChecked()) return 0.0;
...
qCanupoClassifDialog::getCorePointsCloudSource() const {
...
ccPointCloud* qCanupoClassifDialog::getCorePointsCloud() {
    if (cpUseCloudRadioButton->isChecked()) {
        ...
        return qCanupoTools::GetCloudFromCombo(cpOtherCloudComboBox,
                                               m_app->dbRootObject());
    } else {
        return 0;
    }
...
void qCanupoClassifDialog::loadParamsFromPersistentSettings() {
    ...
    QString currentPath =
            settings.value("CurrentPath", QApplication::applicationDirPath())
                    .toString();
    QString mscCurrentPath =
            settings.value("MscCurrentPath", QApplication::applicationDirPath())
                    .toString();

    bool useConfThreshold =
            settings.value("UseConfThreshold",
                           useConfThresholdGroupBox->isChecked())
                    .toBool();
    ...
    bool additionalSF =
            settings.value("AdditionalSF",
                           generateAdditionalSFsCheckBox->isChecked())
                    .toBool();
    bool roughnessSF = settings.value("RoughnessSF",
                                      generateRoughnessSFsCheckBox->isChecked())
                               .toBool();
    int maxThreadCount =
    ...
    if (subsampleEnabled) cpSubsampleRadioButton->setChecked(true);
...
void qCanupoClassifDialog::browseClassifierFile() {
    ...
    if (Classifier::Load(filename, classifiers, scales, error, &header, true)) {
        ...
        fileDesc << QString("Classifier(s) in file: %1")
                            .arg(header.classifierCount);

        assert(header.descID != 0);
        ScaleParamsComputer* computer =
                ScaleParamsComputer::GetByID(header.descID);
        QString descriptorName =
                computer ? computer->getName() : QString("INVALID");
        fileDesc << QString("Descriptor ID: %1 (%2)")
                            .arg(header.descID)
                            .arg(descriptorName);
        ...
        for (size_t i = 0; i < scales.size(); ++i)
            scalesStr.append(QString("%1 ").arg(
                    scales[scales.size() - 1 -
                           i]));  // reverse order as scales are sorted from
                                  // biggest to smallest
        scalesStr.append("]");
        fileDesc << scalesStr;
        ...
        classifInfoTextEdit->setText(
                "Error: failed to open/read the input file");
        buttonBox->button(QDialogButtonBox::Ok)->setEnabled(false);
    }
}
