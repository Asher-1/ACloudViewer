...
void qCanupoPlugin::onNewSelection(const ccHObject::Container& selectedEntities)
{
	if (m_classifyAction)
	{
		...
		m_classifyAction->setEnabled(selectedEntities.size() == 1 && selectedEntities[0]->isA(CV_TYPES::POINT_CLOUD));
	}
...
void qCanupoPlugin::doClassifyAction()
{
	...
	if (m_selectedEntities.empty() || !m_selectedEntities.front()->isA(CV_TYPES::POINT_CLOUD))
	{
	...
	case qCanupoClassifDialog::OTHER:
	{
		...
		{
			assert(false);
			m_app->dispToConsole(tr("Internal error: failed to access core pointss?!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	...
	case qCanupoClassifDialog::SUBSAMPLED:
	{
		...
		if (!refCloud)
		{
			m_app->dispToConsole(tr("Failed to compute sub-sampled core points!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
		...
		if (realCorePoints)
		{
			realCorePoints->setName(cloud->getName() + tr(".core points (subsampled @ %1)").arg(samplingDist));
		...
		{
			m_app->dispToConsole(tr("Can't save subsampled cloud (not enough memory)!"));
		}
	...
	case qCanupoClassifDialog::MSC_FILE:
	{
		...
		else if (!error.isNull())
		{
			...
			m_app->dispToConsole(tr("[qCanupo] ") + error, ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		}
	...
	{
		assert(false);
		m_app->dispToConsole(tr("Internal error: no core point source specified?!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	}
	...
	if (qCanupoProcess::Classify(classifierFilename, params, cloud, corePoints, corePointsDescriptors, realCorePoints, m_app, m_app->getActiveWindow()))
	{
...
void qCanupoPlugin::doTrainAction()
{
	...
	//{
	//	m_app->dispToConsole("Select two point clouds!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
	//	return;
	//}
	//
	...
	{
		if (m_app)
			m_app->dispToConsole(tr("At least one cloud (class #1 or #2) was not defined!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	//check that the selected descriptor (computer) is valid
	{
		...
		{
			if (m_app)
				m_app->dispToConsole(tr("Internal error: unhandled descriptor ID (%1)!").arg(descriptorID), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
		if (computer->needSF()
		...
		{
			if (m_app)
				m_app->dispToConsole(tr("To compute this type of descriptor, all clouds must have an active scalar field!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	}
	...
	while (true)
	{
		// sub-sample clouds (if necessary)
		{
			...
			if (evaluationCloud)
			{
				...
				if (!evaluationPoints)
				{
					m_app->dispToConsole(tr("Failed to compute sub-sampled version of evaluation points!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					break;
				}
			}
			...
			if (!corePoints1)
			{
				m_app->dispToConsole(tr("Failed to compute sub-sampled version of cloud #1!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				break;
			}
		...
		CorePointDescSet descriptors1;
		{
			...
			if (!qCanupoTools::ComputeCorePointsDescriptors(corePoints1,
				...
				octree*/))
			{
				m_app->dispToConsole(tr("Failed to compute core points descriptors: %1").arg(errorStr), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				break;
			}
			else if (invalidDescriptors)
			{
				m_app->dispToConsole(tr("[qCanupo] Some descriptors couldn't be computed on cloud#1 (min scale may be too small)!"), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			}
		}
		...
		CorePointDescSet descriptors2;
		{
			...
			{
				m_app->dispToConsole(tr("[qCanupo] Some descriptors couldn't be computed on cloud#2 (min scale may be too small)!"), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			}
		}
		...
		if (evaluationPoints)
		{
			...
			{
				m_app->dispToConsole(tr("[qCanupo] Some descriptors couldn't be computed on evaluation cloud (min scale may be too small)!"), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			}
		}
		...
		//now for the Classifier training!
		{
			...
			if (!c2DDlg.trainClassifier())
			{
				...
				m_app->dispToConsole(tr("[qCanupo] Classifier training failed..."), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				break;
			}
