									...
									ecvMainAppInterface* app/*=0*/)
{
	...
	{
		if (app)
			app->dispToConsole("Invalid descriptors!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	LDATrainer trainer;
	{
		...
		{
			if (app)
				app->dispToConsole("Oups, it seems the LDA classifier just crashed!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return false;
		}
	}
	...
	//proceed to boundary evaluation
	{
		...
		{
			if (app)
				app->dispToConsole("[Internal error] Invalid output MSC cloud!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return false;
		}
		...
		{
			if (app)
				app->dispToConsole("Not enough memory to determine the classifier behavior!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return false;
		}
		...
		{
			if (app)
				app->dispToConsole("Not enough memory to display colors!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		}
		else
		{
