								...
								bool silent/*=false*/)
{
	...
	unsigned descriptorID = DESC_DIMENSIONALITY;
	{
		...
		{
			if (app)
				app->dispToConsole(QString("An error occured: ") + error, ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return false;
		}
		if (classifiers.empty())
		{
			if (app)
				app->dispToConsole(QString("Invalid classifier file!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return false;
		}
		...
		for (size_t i = 0; i < classifiers.size(); ++i)
		{
			...
			{
				if (app)
					app->dispToConsole(QString("Unhandled descriptor type! (ID = %1)").arg(classifiers[i].descriptorID), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				return false;
			}
			if (i != 0 && classifiers[i].descriptorID != classifiers[0].descriptorID)
			{
				if (app)
					app->dispToConsole(QString("Can't handle mixed descriptor types!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				return false;
			}
		}
	...
	if (realCorePoints)
	{
		...
		if (mscMetaData.isValid())
		{
			...
			{
				if (app)
					app->dispToConsole("[qCanupo] Failed to read core point cloud associated MSC meta data?!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				mscMetaData.clear();
			}
		}
	}
	...
	try
	{
		for (int step = 0; step < 1; ++step) //fake loop for easy break
		{
			...
			if (!octree)
			{
				...
				{
					if (app)
						app->dispToConsole("Failed to compute input cloud octree!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					break;
				}
			...
			if (computeDescriptors)
			{
				//check that the selected descriptor (computer) is valid
				{
					...
					{
						if (app)
							app->dispToConsole(QString("Internal error: unhandled descriptor ID (%1)!").arg(descriptorID), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
						break;
					}
					...
					{
						if (app)
							app->dispToConsole(QString("To compute this type of descriptor, the core points cloud must have an active scalar field!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
						break;
					}
				}
				...
				if (generateRoughnessSF)
				{
					...
					for (size_t s=0; s<scaleCount; ++s)
					{
						...
						{
							m_app->dispToConsole("Not enough memory to store per-level roughness!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				...
				{
					if (app)
						app->dispToConsole(QString("Failed to compute core points descriptors: %1").arg(errorStr), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					break;
				}
				else if (invalidDescriptors)
				{
					if (app)
						app->dispToConsole("[qCanupo] Some descriptors couldn't be computed (min scale may be too small)!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				}
			}
			...
			//main classification process
			{
				...
				else
				{
					...
					{
						if (app)
							app->dispToConsole("Failed to compute core points octree! (not enough memory?)", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
						...
						if (classifiers.size() == 1)
						{
							...
							if (confidence < params.confidenceThreshold)
							{
								...
								if (params.useActiveSFForConfidence)
								{
									...
									{
										if (app)
											app->dispToConsole("Internal error: failed to refine the classification on the boundary (not enough memory)", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
										return false;
									}
				...
				{
					if (app)
						app->dispToConsole("[qCanupo] Process cancelled by user!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
					break;
				}
				...
				// eventually label the points
				{
					...
					int classLabelSFIdx = -1;
					{
						...
						{
							if (app)
								app->dispToConsole("Not enough memory!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
							break;
						}
					}
					...
					int confidenceSFIdx = -1;
					{
						...
						{
							app->dispToConsole("[qCanupo] Not enough memory to store the confidence values!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
						}
					}
					...
					{
						if (app)
							app->dispToConsole("[qCanupo] Per-level 'x-y' values can only be extracted from descriptor with 2 dimensions per scale!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
					...
					if (generateAdditionalSF)
					{
						...
						for (size_t s = 0; s < scaleCount; ++s)
						{
							...
							{
								if (app)
									app->dispToConsole("Not enough memory to store per-level 'x-y' values!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					...
					if (generateRoughnessSF)
					{
						...
						else
						{
							...
							for (size_t s = 0; s < scaleCount; ++s)
							{
								...
								{
									if (app)
										app->dispToConsole("Not enough memory to store per-level roughness!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					...
					{
						if (app)
							app->dispToConsole("Internal error: failed to get nearest core points?!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				...
				if (realCorePoints && !useExistingMetaData)
				{
					...
					if (proceed)
					{
						...
						{
							if (app)
								app->dispToConsole("[qCanupo] Failed to save MSC meta-dataa (not enough memory?)", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
						}
					}
				}
			}
