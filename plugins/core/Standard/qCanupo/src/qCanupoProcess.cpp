...
float RefinePointClassif(const Classifier& classifier,
    ...
    try {
        ...
        int n = octree->getPointsInSphericalNeighbourhood(
                *corePoints->getPoint(coreIndex),
                largestRadius,  // we use the biggest neighborhood
                neighbors, octreeLevel);

        // for each scene data point, find the corresponding core point and
        ...
        for (int j = 0; j < n; ++j) {
            ...
            if (corePoints == cloud) {
                // if we used the whole cloud as core points, then each point is
                // a core point!
                nearestCoreIndex = currentPointIndex;
            } else {
                double maxSquareDist = 0;
                cloudViewer::ReferenceCloud Yk(corePoints);
                if (corePointsOctree->findPointNeighbourhood(
                            cloud->getPoint(currentPointIndex), &Yk, 1,
                            coreOctreeLevel, maxSquareDist) == 1) {
        ...
        // some local info? TODO: min size for considering this information is
        // reliable?
        ...
        if (class1SFValues.empty()) {
            ...
            distToBoundary = -static_cast<float>(class1SFValues.size()) /
                             n;  // dist(class 1) < 0
            if (distToBoundary > -0.5f) {
                // too close
                return confidence;
            ...
            for (size_t k = 0; k < smallestSet->size(); ++k) {
                ...
                // dichomed is the first index so that largestSet[dichomed] >=
                // smallestSet[k]...
                size_t dichomed =
                        std::upper_bound(largestSet->begin(), largestSet->end(),
                                         smallestSet->at(k)) -
                        largestSet->begin();
                if (dichomed == 0) {
                ...
                // classification on either side, take largest and reverse roles
                // if necessary
                float c1 = static_cast<float>(nlabove) / largestSet->size() +
                           static_cast<float>(nsbelow) / smallestSet->size();
                float c2 = static_cast<float>(largestSet->size() - nlabove) /
                                   largestSet->size() +
                           static_cast<float>(smallestSet->size() - nsbelow) /
                                   smallestSet->size();
                ...
                if (qCanupoTools::Fpeq<float>(bestConfidence, conf)) {
                    bestSplit.push_back(
                            (smallestSet->at(k) +
                             largestSet->at(std::min(dichomed,
                                                     largestSet->size() - 1))) /
                            2);
            ...
            // TODO: sometimes (rarely) there are mistakes in the reference core
            // points and we're dealing with similar classes
            ...
            if (/*old*/ confidence < bestConfidence) {
                ...
                if (bestSplitDir[bsi] == 1) distToBoundary = -distToBoundary;
        ...
        // update confidence
        float newConfidence =
                1.0f / (exp(-fabs(distToBoundary)) + 1.0f);  // in [0.5 ; 1]
...
bool qCanupoProcess::Classify(
    ...
    unsigned descriptorID = DESC_DIMENSIONALITY;
    {
        ...
        if (!Classifier::Load(classifierFilename, classifiers, scales, error)) {
            if (app)
                app->dispToConsole(QString("An error occured: ") + error,
                                   ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
            return false;
        }
        if (classifiers.empty()) {
            if (app)
                app->dispToConsole(QString("Invalid classifier file!"),
                                   ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
            return false;
        }

        // check that the descriptor ID of every classifier is handled by this
        // version!
        for (size_t i = 0; i < classifiers.size(); ++i) {
            if (!ScaleParamsComputer::GetByID(classifiers[i].descriptorID)) {
                if (app)
                    app->dispToConsole(
                            QString("Unhandled descriptor type! (ID = %1)")
                                    .arg(classifiers[i].descriptorID),
                            ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                return false;
            }
            if (i != 0 &&
                classifiers[i].descriptorID != classifiers[0].descriptorID) {
                if (app)
                    app->dispToConsole(
                            QString("Can't handle mixed descriptor types!"),
                            ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                return false;
            }
        }
    ...
    if (realCorePoints) {
        ...
        if (mscMetaData.isValid()) {
            ...
            if (validMetaData) {
                ...
                if (app)
                    app->dispToConsole(
                            "[qCanupo] Failed to read core point cloud "
                            "associated MSC meta data?!",
    ...
    try {
        for (int step = 0; step < 1; ++step)  // fake loop for easy break
        {
            ...
            if (!corePointsDescriptors.scales().empty()) {
                if (qCanupoTools::TestVectorsOverlap(
                            scales, corePointsDescriptors.scales()) <
                    scales.size()) {
                    if (!silent &&
                        QMessageBox::question(
                                parentWidget, "Scales mismatch",
                                "Available descriptors/scales data mismatch "
                                "with classifier's! Compute new descriptors or "
                                "cancel?",
                                QMessageBox::Yes,
                    ...
                    computeDescriptors =
                            false;  // no need to compute the descriptors as we
                                    // already have them!
                }

                // check descriptor type!
                if (!computeDescriptors &&
                    corePointsDescriptors.descriptorID() != descriptorID) {
                    if (!silent &&
                        QMessageBox::question(
                                parentWidget, "Descriptor type mismatch",
                                "Available descriptors have been computed with "
                                "another descriptor type! Compute new "
                                "descriptors or cancel?",
                                QMessageBox::Yes,
                                QMessageBox::Cancel) == QMessageBox::Cancel) {
                        // cancel process
                        break;
            ...
            if (!octree) {
                ...
                if (!octree) {
                    if (app)
                        app->dispToConsole(
                                "Failed to compute input cloud octree!",
                                ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                    break;
            ...
            if (computeDescriptors) {
                // check that the selected descriptor (computer) is valid
                {
                    ...
                    if (!computer) {
                        if (app)
                            app->dispToConsole(
                                    QString("Internal error: unhandled "
                                            "descriptor ID (%1)!")
                                            .arg(descriptorID),
                                    ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                        break;
                    ...
                    if (computer->needSF() &&
                        ((realCorePoints &&
                          realCorePoints->getCurrentDisplayedScalarField() ==
                                  nullptr) ||
                         (!realCorePoints &&
                          cloud->getCurrentDisplayedScalarField() ==
                                  nullptr)  // if realCorePoints == 0, it means
                                            // that the subsampled cloud
                                            // couldn't be converted to a real
                                            // cloud!
                         )) {
                        if (app)
                            app->dispToConsole(
                                    QString("To compute this type of "
                                            "descriptor, the core points cloud "
                                            "must have an active scalar "
                                            "field!"),
                                    ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                        break;
                    }
                ...
                if (generateRoughnessSF) {
                    ...
                    for (size_t s = 0; s < scaleCount; ++s) {
                        QString sfName =
                                QString(CANUPO_PER_LEVEL_ROUGHNESS_SF_NAME) +
                                QString(" @ scale %1").arg(scales[s]);

                        coreRoughnessSFs[s] =
                                new ccScalarField(qPrintable(sfName));
                        if (!coreRoughnessSFs[s]->resize(corePoints->size(),
                                                         NAN_VALUE)) {
                            m_app->dispToConsole(
                                    "Not enough memory to store per-level "
                                    "roughness!",
                                    ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                ...
                if (!qCanupoTools::ComputeCorePointsDescriptors(
                    ...
                    if (app)
                        app->dispToConsole(
                                QString("Failed to compute core points "
                                        "descriptors: %1")
                                        .arg(errorStr),
                                ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                    break;
            ...
            {
                // advanced options
                assert(!params.useActiveSFForConfidence ||
                       cloud->getCurrentDisplayedScalarField() != nullptr);
                ...
                if (corePoints == cloud) {
                    ...
                    if (!corePointsOctree->build(&pDlg)) {
                        if (app)
                            app->dispToConsole(
                                    "Failed to compute core points octree! "
                                    "(not enough memory?)",
                                    ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                ...
                const PointCoordinateType& largestRadius =
                        scales.front() / 2;  // largest scale is the first by
                                             // construction in canupo
                unsigned char coreOctreeLevel =
                        corePointsOctree
                ...
                // set the class of all core points that are far enough from
                // hyperplane
                ...
                while (!pendingPoints.empty()) {
                    ...
                    pDlg.reset();
                    pDlg.setInfo(QObject::tr("Remaining points to classify: "
                                             "%1\nSource points: %2")
                                         .arg(pendingPoints.size())
                                         .arg(cloud->size()));
                    pDlg.setMethodTitle(QObject::tr("Classification"));
                    cloudViewer::NormalizedProgress nProgress(
                            &pDlg, corePoints->size());
                    pDlg.start();

                    for (size_t i = 0; i < pendingPoints.size(); ++i) {
                        unsigned coreIndex = pendingPoints[i];
                        CorePointDesc& coreDesc =
                                corePointsDescriptors[coreIndex];

                        // most common case
                        if (classifiers.size() == 1) {
                            const Classifier& classifier = classifiers.front();
                            float distToBoundary =
                                    classifier.classify(coreDesc);

                            float confidence =
                                    1.0f / (exp(-fabs(distToBoundary)) +
                                            1.0f);  // in [0.5 ; 1]
                            confidence =
                                    2 * (confidence - 0.5f);  // map to [0;1]
                            ...
                            if (confidence < params.confidenceThreshold) {
                                ...
                                if (params.useActiveSFForConfidence) {
                                    // use the scalar field to refine the
                                    // classification
                                    float newConfidence = RefinePointClassif(
                                            classifier, confidence,
                                            distToBoundary, cloud,
                                            octree.data(), octreeLevel,
                                            corePoints, corePointsOctree,
                                            coreOctreeLevel, coreIndex,
                                            largestRadius, corePointClasses);
                            ...
                            if (!unreliable) {
                                ...
                                unreliablePointIndexes.push_back(
                        ...
                        } else  // more than one classifier
                        {
                            ...
                            // apply all classifiers and look for the most
                            // represented class
                            for (std::vector<Classifier>::const_iterator
                                         classifierIt = classifiers.begin();
                                 classifierIt != classifiers.end();
                                 ++classifierIt) {
                                const Classifier& classifier = *classifierIt;

                                // uniformize the order, distToBoundary>0
                                // selects the larger class of both
                                float distToBoundary = classifier.classify(
                                        coreDesc);  // DGM: the descriptors may
                                                    // have more values than the
                                                    // number of scales!
                                // if (classifier.class1 > classifier.class2)
                                //	distToBoundary = -distToBoundary;

                                // int minclass = std::min(classifier.class1,
                                // classifier.class2); int maxclass =
                                // std::max(classifier.class1,
                                // classifier.class2);
                                ...
                                if (minConfidences.find(theClass) ==
                                    minConfidences.end()) {
                            ...
                            if (!votes.empty()) {
                                ...
                                // in case of equality, use the distances to the
                                // decision boundary
                                int bestClassLabel = bestClasses.front();
                                if (bestClasses.size() > 1) {
                                    for (size_t j = 1; j < bestClasses.size();
                                         ++j) {
                                        if (minConfidences[bestClasses[j]] >
                                            minConfidences[bestClassLabel])
                                            bestClassLabel = bestClasses[j];
                                    }
                                }

                                corePointClasses[i] = bestClassLabel;
                                corePointConfidences[i] =
                                        minConfidences[bestClassLabel];
                            } else if (params.useActiveSFForConfidence) {
                                // this point can't be classified this way
                                unreliablePointIndexes.push_back(
                ...
                if (processCanceled) {
                    if (app)
                        app->dispToConsole(
                                "[qCanupo] Process cancelled by user!",
                                ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
                    break;
                }
                ...
                // eventually label the points
                {
                    ...
                    {
                        classLabelSFIdx = cloud->getScalarFieldIndexByName(
                                "CANUPO.class");
                        if (classLabelSFIdx < 0)
                            classLabelSFIdx =
                                    cloud->addScalarField("CANUPO.class");
                        if (classLabelSFIdx >= 0) {
                            classLabelSF =
                                    cloud->getScalarField(classLabelSFIdx);
                    ...
                    {
                        confidenceSFIdx = cloud->getScalarFieldIndexByName(
                                "CANUPO.confidence");
                        if (confidenceSFIdx < 0)
                            confidenceSFIdx =
                                    cloud->addScalarField("CANUPO.confidence");
                        if (confidenceSFIdx >= 0) {
                            confidenceSF =
                                    cloud->getScalarField(confidenceSFIdx);
                            confidenceSF->fill(NAN_VALUE);
                        } else if (app) {
                            app->dispToConsole(
                                    "[qCanupo] Not enough memory to store the "
                                    "confidence values!",
                                    ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
                        }
                    }
                    ...
                    if (generateAdditionalSF &&
                        corePointsDescriptors.dimPerScale() != 2) {
                        if (app)
                            app->dispToConsole(
                                    "[qCanupo] Per-level 'x-y' values can only "
                                    "be extracted from descriptor with 2 "
                    ...
                    if (generateAdditionalSF) {
                        // remove any previously generated scalar field starting
                        // by CANUPO_PER_LEVEL_ADDITIONAL_SF_NAME
                        {
                            QStringList toDelete;
                            for (unsigned i = 0;
                                 i < cloud->getNumberOfScalarFields(); ++i) {
                                QString sfName(
                                        cloud->getScalarField(i)->getName());
                                if (sfName.startsWith(
                        ...
                        for (size_t s = 0; s < scaleCount; ++s) {
                            QString sfName =
                                    QString(CANUPO_PER_LEVEL_ADDITIONAL_SF_NAME) +
                                    QString(" @ scale %1").arg(scales[s]);

                            // SF with same name (if any) should have already
                            // been removed!
                            assert(cloud->getScalarFieldIndexByName(
                                           qPrintable(sfName)) < 0);

                            scaleSFs[s] = new ccScalarField(qPrintable(sfName));
                            if (!scaleSFs[s]->resizeSafe(cloud->size(), true,
                                                         NAN_VALUE)) {
                                if (app)
                                    app->dispToConsole(
                                            "Not enough memory to store "
                                            "per-level 'x-y' values!",
                                            ecvMainAppInterface::
                                                    ERR_CONSOLE_MESSAGE);
                    ...
                    if (generateRoughnessSF) {
                        // remove any previously generated scalar field starting
                        // by CANUPO_PER_LEVEL_ROUGHNESS_SF_NAME
                        {
                            QStringList toDelete;
                            for (unsigned i = 0;
                                 i < realCorePoints->getNumberOfScalarFields();
                                 ++i) {
                                QString sfName(
                                        cloud->getScalarField(i)->getName());
                                if (sfName.startsWith(
                                            CANUPO_PER_LEVEL_ROUGHNESS_SF_NAME))
                                    toDelete << sfName;
                            }

                            for (int j = 0; j < toDelete.size(); ++j)
                                cloud->deleteScalarField(
                                        cloud->getScalarFieldIndexByName(
                                                qPrintable(toDelete[j])));
                        }

                        // if the output cloud has the same number of points as
                        // the core points cloud, no need to duplicate the
                        // scalar fields!
                        if (corePoints->size() == cloud->size()) {
                            for (size_t s = 0; s < coreRoughnessSFs.size();
                                 ++s) {
                                // SF with same name (if any) should have
                                // already been removed!
                                assert(cloud->getScalarFieldIndexByName(
                                               coreRoughnessSFs[s]->getName()) <
                                       0);

                                coreRoughnessSFs[s]->computeMinAndMax();
                                cloud->addScalarField(coreRoughnessSFs[s]);
                            }
                            coreRoughnessSFs.clear();  // don't want to release
                                                       // them anymore!
                            generateRoughnessSF =
                                    false;  // no need to bother anymore
                        } else {
                            size_t scaleCount = scales.size();
                            roughnessSFs.resize(scaleCount, 0);
                            assert(coreRoughnessSFs.size() ==
                                   roughnessSFs.size());
                            // for each scale
                            for (size_t s = 0; s < scaleCount; ++s) {
                                // same name as the per-core points version
                                roughnessSFs[s] = new ccScalarField(
                                        coreRoughnessSFs[s]->getName());

                                // SF with same name (if any) should have
                                // already been removed!
                                assert(cloud->getScalarFieldIndexByName(
                                               roughnessSFs[s]->getName()) < 0);

                                if (!roughnessSFs[s]->resize(cloud->size(),
                                                             NAN_VALUE)) {
                                    if (app)
                                        app->dispToConsole(
                                                "Not enough memory to store "
                    ...
                    pDlg.reset();
                    pDlg.setInfo(
                            QObject::tr("Core points: %1\nSource points: %2")
                                    .arg(corePoints->size())
                                    .arg(cloud->size()));
                    pDlg.setMethodTitle(QObject::tr("Labelling"));
                    cloudViewer::NormalizedProgress nProgress(&pDlg,
                                                              cloud->size());
                    pDlg.start();
                    ...
                    for (unsigned i = 0; i < cloud->size(); ++i) {
                        ...
                        if (corePoints == cloud) {
                            // if we used the whole cloud as core points, then
                            // each point is a core point
                            ...
                            assert(corePointsOctree);
                            if (corePointsOctree->findPointNeighbourhood(
                                        P, &Yk, 1, coreOctreeLevel,
                                        maxSquareDist) == 1) {
                                nearestCorePointIndex =
                                        Yk.getPointGlobalIndex(0);
                            } else {
                                assert(false);
                                error = true;
                                break;
                        ...
                        assert(classLabelSF);
                        ScalarType classVal = static_cast<ScalarType>(
                                corePointClasses[nearestCorePointIndex]);
                        if (classVal >= 0) classLabelSF->setValue(i, classVal);
                        // otherwise, it is already defaulted to NaN

                        if (confidenceSF) {
                            ScalarType confVal = static_cast<ScalarType>(
                                    corePointConfidences
                                            [nearestCorePointIndex]);
                        ...
                        if (generateAdditionalSF) {
                            unsigned dimPerScale =
                                    corePointsDescriptors.dimPerScale();
                            assert(dimPerScale == 2);
                            const CorePointDesc& desc = corePointsDescriptors
                                    [nearestCorePointIndex];
                            assert(desc.params.size() ==
                                   scaleSFs.size() * dimPerScale);
                            for (size_t s = 0; s < scaleSFs.size(); ++s) {
                                ScalarType val =
                                        (desc.params[s * dimPerScale] -
                                         desc.params[s * dimPerScale + 1]);
                    ...
                    if (error) {
                        if (app)
                            app->dispToConsole(
                                    "Internal error: failed to get nearest "
                                    "core points?!",
                                    ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
                        ...
                        if (confidenceSF) confidenceSF->computeMinAndMax();
                    ...
                    cloud->showSF(cloud->getCurrentDisplayedScalarField() !=
                                  nullptr);
                }
                ...
                if (realCorePoints && !useExistingMetaData) {
                    ...
                    if (mscMetaData.isValid()) {
                        proceed = (silent ||
                                   QMessageBox::question(
                                           ...
                                           "Core points cloud already has "
                                           "associated MSC meta-data, should "
                                           "we overwrite them?",
                    ...
                    if (proceed) {
                        ...
                        if (mscMetaData.isValid()) {
                            realCorePoints->setMetaData(s_canupoMSCMetaData,
                                                        mscMetaData);
                            if (app)
                                app->dispToConsole(
                                        QString("[qCanupo] MSC descriptors "
                                                "have been saved as meta-data "
                                                "(cloud '%1')")
                                                .arg(realCorePoints
                                                             ->getName()));
                        } else {
                            if (app)
                                app->dispToConsole(
                                        "[qCanupo] Failed to save MSC "
                                        "meta-dataa (not enough memory?)",
                                        ecvMainAppInterface::
                                                WRN_CONSOLE_MESSAGE);
                        }
                    }
