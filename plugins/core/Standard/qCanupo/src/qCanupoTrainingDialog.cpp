...
qCanupoTrainingDialog::qCanupoTrainingDialog(ecvMainAppInterface* app)
    ...
    if (m_app) {
        ...
        // the global  cloud!
        class1CloudComboBox->setCurrentIndex(
                cloudCount > 0 ? (cloudCount > 2 ? 1 : 0) : -1);
        class2CloudComboBox->setCurrentIndex(
                cloudCount > 1 ? (cloudCount > 2 ? 2 : 1) : -1);
        originCloudComboBox->setCurrentIndex(cloudCount > 2 ? 0 : -1);

        if (cloudCount < 2 && app)
            app->dispToConsole(
                    "You need at least 2 loaded clouds to train a classifier "
                    "(one per class)",
    ...
    connect(cloud1ClassSpinBox, SIGNAL(valueChanged(int)), this,
            SLOT(onClassChanged(int)));
    connect(cloud2ClassSpinBox, SIGNAL(valueChanged(int)), this,
            SLOT(onClassChanged(int)));
    connect(class1CloudComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onCloudChanged(int)));
    connect(class2CloudComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onCloudChanged(int)));
...
ccPointCloud* qCanupoTrainingDialog::getClass1Cloud() {
    ...
    return qCanupoTools::GetCloudFromCombo(class1CloudComboBox,
                                           m_app->dbRootObject());
}

ccPointCloud* qCanupoTrainingDialog::getClass2Cloud() {
    // return the cloud currently selected in the combox box
    return qCanupoTools::GetCloudFromCombo(class2CloudComboBox,
                                           m_app->dbRootObject());
}

ccPointCloud* qCanupoTrainingDialog::getOriginPointCloud() {
    // return the cloud currently selected in the combox box
    return useOriginalCloudCheckBox->isChecked()
                   ? qCanupoTools::GetCloudFromCombo(originCloudComboBox,
                                                     m_app->dbRootObject())
                   : 0;
}
...
ccPointCloud* qCanupoTrainingDialog::getEvaluationCloud() {
    // return the cloud currently selected in the combox box
    return evaluateParamsCheckBox->isChecked()
                   ? qCanupoTools::GetCloudFromCombo(evaluationCloudComboBox,
                                                     m_app->dbRootObject())
                   : 0;
}
...
void qCanupoTrainingDialog::loadParamsFromPersistentSettings() {
    ...
    // read out parameters
    double minScale = settings.value("MinScale", minScaleDoubleSpinBox->value())
                              .toDouble();
    double step =
            settings.value("Step", stepScaleDoubleSpinBox->value()).toDouble();
    double maxScale = settings.value("MaxScale", maxScaleDoubleSpinBox->value())
                              .toDouble();
    QString scalesList =
            settings.value("ScalesList", scalesListLineEdit->text()).toString();
    bool scalesRampEnabled = settings.value("ScalesRampEnabled",
                                            scalesRampRadioButton->isChecked())
                                     .toBool();

    unsigned maxPoints =
            settings.value("MaxPoints", maxPointsSpinBox->value()).toUInt();
    int classifParam =
            settings.value("ClassifParam", paramComboBox->currentIndex())
                    .toInt();
    int maxThreadCount =
