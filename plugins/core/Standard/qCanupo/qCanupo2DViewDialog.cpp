	...
	, m_pickingRadius(5)
{
	...
	//setup 2D view
	{
		...
		//ecvDisplayTools::SetInteractionMode(ecvDisplayTools::PAN_ONLY() | ecvDisplayTools::INTERACT_SEND_ALL_SIGNALS);
		...
		connect(ecvDisplayTools::TheInstance(), SIGNAL(leftButtonClicked(int, int)),			this, SLOT(addOrSelectPoint(int, int)));
		connect(ecvDisplayTools::TheInstance(), SIGNAL(rightButtonClicked(int, int)),			this, SLOT(removePoint(int, int)));
		connect(ecvDisplayTools::TheInstance(), SIGNAL(mouseMoved(int, int, Qt::MouseButtons)),	this, SLOT(moveSelectedPoint(int, int, Qt::MouseButtons)));
		connect(ecvDisplayTools::TheInstance(), SIGNAL(buttonReleased()),						this, SLOT(deselectPoint()));*/
	}
...
{
	if (!m_poly || !ecvDisplayTools::GetCurrentScreen())
		return -1;
...
void qCanupo2DViewDialog::addOrSelectPoint(int x, int y)
{
	...
	//picking radius
	double maxPickingDist = static_cast<double>(m_pickingRadius) * ecvDisplayTools::ComputeActualPixelSize();
