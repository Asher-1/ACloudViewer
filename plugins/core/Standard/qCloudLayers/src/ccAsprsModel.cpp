...
ccAsprsModel::ccAsprsModel(QObject* parent) : QAbstractTableModel(parent) {
    load();
...
QVariant ccAsprsModel::data(const QModelIndex& index, int role) const {
    ...
    // for the others, we only provide the values for the 'Display' and 'Edit'
    // roles
...
bool ccAsprsModel::setData(const QModelIndex& index,
    ...
    switch (index.column()) {
        case VISIBLE: {
            if (role == Qt::CheckStateRole) {
                item.visible = static_cast<Qt::CheckState>(value.toInt()) ==
                               Qt::Checked;
...
QModelIndex ccAsprsModel::createNewItem() {
    ...
    m_data.append(
            {false, "UNNAMED", getUnusedCode(), Qt::GlobalColor::black, 0});
    endInsertRows();
...
bool ccAsprsModel::isNameExist(const QString& name) const {
    auto item = std::find_if(
            ...
            [name](const AsprsItem& item) { return item.name == name; });
...
bool ccAsprsModel::isCodeExist(int code) const {
    auto item = std::find_if(
            m_data.begin(), m_data.end(),
            [code](const AsprsItem& item) { return item.code == code; });
    return item != m_data.end();
}

static void AddClass(QSettings& settings,
...
static void ReadClass(const QSettings& settings,
                      const QString& className,
...
ccAsprsModel::AsprsItem* ccAsprsModel::find(QString name) {
    auto it = std::find_if(m_data.begin(), m_data.end(),
                           [name](const ccAsprsModel::AsprsItem& item) {
...
ccAsprsModel::AsprsItem* ccAsprsModel::find(int code) {
    auto it = std::find_if(m_data.begin(), m_data.end(),
                           [code](const ccAsprsModel::AsprsItem& item) {
...
int ccAsprsModel::getUnusedCode() const {
    auto it = std::max_element(m_data.cbegin(), m_data.cend(),
                               [](const AsprsItem& a, const AsprsItem& b) {
