...
ccCloudLayersDlg::ccCloudLayersDlg(ecvMainAppInterface* app, QWidget* parent)
    ...
    connect(pbDelete, &QPushButton::clicked, this,
            &ccCloudLayersDlg::deleteClicked);
    connect(pbStart, &QPushButton::clicked, this,
            &ccCloudLayersDlg::startClicked);
    ...
    connect(cbScalarField, qOverload<int>(&QComboBox::currentIndexChanged),
            this, &ccCloudLayersDlg::scalarFieldIndexChanged);
    connect(cbInput, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ccCloudLayersDlg::inputClassIndexChanged);
    connect(cbOutput, qOverload<int>(&QComboBox::currentIndexChanged), this,
            &ccCloudLayersDlg::outputClassIndexChanged);

    // color picker
    connect(tableView, &QTableView::doubleClicked, this,
            &ccCloudLayersDlg::tableViewDoubleClicked);

    // asprs model changed signals
    connect(&m_asprsModel, &ccAsprsModel::codeChanged, this,
    ...
    m_mouseCircle = new ccMouseCircle(m_app, m_app->getActiveWindow());
...
void ccCloudLayersDlg::reject() {
    if (m_helper && m_helper->hasChanges()) {
        if (QMessageBox::question(
                    ...
                    "The cloud has been modified, are you sure you want exit?",
...
void ccCloudLayersDlg::initTableView() {
    ...
    tableView->setItemDelegateForColumn(ccAsprsModel::COLOR,
                                        new ccColorStyledDelegate(this));

    tableView->horizontalHeader()->setSectionResizeMode(ccAsprsModel::NAME,
                                                        QHeaderView::Stretch);
    tableView->horizontalHeader()->setSectionResizeMode(
            ccAsprsModel::VISIBLE, QHeaderView::ResizeToContents);
    tableView->horizontalHeader()->setSectionResizeMode(
            ccAsprsModel::CODE, QHeaderView::ResizeToContents);
    tableView->horizontalHeader()->setSectionResizeMode(
            ccAsprsModel::COLOR, QHeaderView::ResizeToContents);
    tableView->horizontalHeader()->setSectionResizeMode(
            ccAsprsModel::COUNT, QHeaderView::ResizeToContents);
}
...
void ccCloudLayersDlg::loadSettings() {
    ...
    settings.beginGroup("qCloudLayers");
    {
        ...
        if (!item) {
            ...
            cbInput->setCurrentIndex(m_asprsModel.indexOf(inputName) +
                                     m_presets.size());
        }
...
void ccCloudLayersDlg::deleteClicked() {
    ...
    for (QModelIndex index : mapIndices) {
        QModelIndex sourceIndex =
                static_cast<QSortFilterProxyModel*>(tableView->model())
                        ->mapToSource(index);
        sourceIndices.append(sourceIndex);
    }

    ccAsprsModel::AsprsItem* to = m_asprsModel.getData().size() > 0
                                          ? &(m_asprsModel.getData().front())
                                          : nullptr;
    for (int i = mapIndices.size(); i > 0; --i) {
        ccAsprsModel::AsprsItem& from =
                m_asprsModel.getData()[sourceIndices[i - 1].row()];
...
void ccCloudLayersDlg::tableViewDoubleClicked(const QModelIndex& index) {
    if (index.column() != ccAsprsModel::COLOR) return;
...
void ccCloudLayersDlg::updateInputOutput() {
    ...
    for (int i = 0; i < data.size(); ++i) cbInput->addItem(data[i].name);

    cbOutput->clear();
    for (int i = 0; i < data.size(); ++i) cbOutput->addItem(data[i].name);
}
