...
ccCloudLayersHelper::ccCloudLayersHelper(ecvMainAppInterface* app,
    ...
    if (m_cloud) {
        ...
        if (m_cloud->hasColors()) {
            ...
            m_formerCloudColors = m_cloud->rgbColors()->clone();
            if (!m_formerCloudColors) {
                CVLog::Error("Not enough memory to backup previous colors");
            }
        } else {
            // check memory for rgb colors
            if (!m_cloud->resizeTheRGBTable()) {
                CVLog::Error("Not enough memory to show colors");
            }
        }
...
void ccCloudLayersHelper::apply(QList<ccAsprsModel::AsprsItem>& items) {
    if (m_scalarFieldIndex >= m_cloud->getNumberOfScalarFields()) return;

    m_cloud->setRGBColor(ecvColor::black);
...
int ccCloudLayersHelper::apply(ccAsprsModel::AsprsItem& item,
                               bool redrawDisplay) {
    ecvColor::Rgba ecvColor = ecvColor::FromQColora(item.color);
    ecvColor.a = item.visible ? ecvColor::MAX : 0;
    ...
    for (auto it = sf->begin(); it != sf->end(); ++it, ++counter) {
        if ((*it) == code) {
            m_cloud->setPointColor(counter, ecvColor);
            ++affected;
        }
    }

    if (redrawDisplay) m_cloud->redrawDisplay();
...
void ccCloudLayersHelper::restoreState() {
    cloudViewer::ScalarField* sf =
            (m_cloud ? m_cloud->getScalarField(m_scalarFieldIndex) : nullptr);
    if (!sf) {
        assert(false);
        return;
    }
...
void ccCloudLayersHelper::project(ccGLCameraParameters camera,
    ...
    for (unsigned i = start; i < end; ++i) {
        ...
        m_projectedPoints[i] =
                CCVector2(static_cast<PointCoordinateType>(Q2D.x - half_w),
                          static_cast<PointCoordinateType>(Q2D.y - half_h));
        m_pointInFrustum[i] = pointInFrustum;
    }
}

PointCoordinateType ccCloudLayersHelper::ComputeSquaredEuclideanDistance(
        const CCVector2& a, const CCVector2& b) {
    return (b - a).norm2();
}
...
void ccCloudLayersHelper::mouseMove(const CCVector2& center,
    ...
    if (m_parameters.output == nullptr ||
        ((!m_parameters.anyPoints && !m_parameters.visiblePoints) &&
    ...
    ScalarType inputCode =
            m_parameters.input != nullptr
                    ? static_cast<ScalarType>(m_parameters.input->code)
                    : 0;
    ScalarType outputCode = static_cast<ScalarType>(m_parameters.output->code);

    unsigned char alpha = m_parameters.output->visible ? ecvColor::MAX : 0;
    ecvColor::Rgba outputColor = ecvColor::Rgba(
...
void ccCloudLayersHelper::projectCloud(const ccGLCameraParameters& camera) {
    ...
    for (unsigned i = 0; i < processorCount; ++i) {
        ...
        if (i == processorCount - 1) end = cloudSize;

        threads[i] = new std::thread(&ccCloudLayersHelper::project, this,
                                     camera, start, end);
    }

    for (auto it = threads.begin(); it != threads.end(); ++it) (*it)->join();

    for (auto it = threads.begin(); it != threads.end(); ++it) delete (*it);
}
