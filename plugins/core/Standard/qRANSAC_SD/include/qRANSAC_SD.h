...
//! Wrapper to Schnabel et al. library for automatic shape detection in point
//! cloud
/** "Efficient RANSAC for Point-Cloud Shape Detection", Ruwen Schnabel, Roland
Wahl, and Reinhard Klein, in Computer Graphics Forum (June 2007), 26:2(214-226)
...
public:
    ...
    struct RansacParams {
        ...
        unsigned supportPoints;  // this is the minimal numer of points required
                                 // for a primitive
        float maxNormalDev_deg;  // maximal normal deviation from ideal shape
                                 // (in degrees)
        float probability;       // probability that no better candidate was
                                 // overlooked during sampling
        bool randomColor;     // should the resulting detected shapes sub point
