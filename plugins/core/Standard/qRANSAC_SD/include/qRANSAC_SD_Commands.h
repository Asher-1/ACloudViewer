...
struct CommandRANSAC : public ccCommandLineInterface::Command {
    CommandRANSAC()
        : ccCommandLineInterface::Command(QObject::tr("RANSAC"),
                                          COMMAND_RANSAC) {}

    virtual bool process(ccCommandLineInterface& cmd) override {
        if (cmd.clouds().empty()) {
            return cmd.error(QObject::tr("No point cloud to attempt RANSAC on "
                                         "(be sure to open one with \"-O "
        ...
        QStringList paramNames =
                QStringList()
                << EPSILON_ABSOLUTE << EPSILON_PERCENTAGE_OF_SCALE
                << BITMAP_EPSILON_PERCENTAGE_OF_SCALE << BITMAP_EPSILON_ABSOLUTE
                << SUPPORT_POINTS << MAX_NORMAL_DEV << PROBABILITY
                << ENABLE_PRIMITIVE << OUT_CLOUD_DIR << OUT_MESH_DIR
                << OUT_GROUP_DIR << OUT_PAIR_DIR << OUTPUT_INDIVIDUAL_PRIMITIVES
                << OUTPUT_INDIVIDUAL_SUBCLOUDS << OUTPUT_GROUPED
                << OUTPUT_INDIVIDUAL_PAIRED_CLOUD_PRIMITIVE;
        QStringList primitiveNames = QStringList() << PRIM_PLANE << PRIM_SPHERE
                                                   << PRIM_CYLINDER << PRIM_CONE
                                                   << PRIM_TORUS;
            ...
            while (paramNames.contains(param)) {
                ...
                if (param == EPSILON_ABSOLUTE) {
                    if (cmd.arguments().empty()) {
                        return cmd.error(
                                QObject::tr("Missing parameter: number after "
                                            "\"-%1 %2\"")
                                        .arg(COMMAND_RANSAC, EPSILON_ABSOLUTE));
                    }
                    bool ok;
                    ...
                    if (!ok || (val <= 0.0f || val >= 1.0f)) {
                        return cmd.error(
                                "Invalid number for epsilon percentage must be "
                                "a float greater than 0.0 and less than 1.0!");
                    }
                    cmd.print(QObject::tr("\tEpsilon : %1").arg(val));
                    epsilonPercentage = val;
                } else if (param == BITMAP_EPSILON_ABSOLUTE) {
                    if (cmd.arguments().empty()) {
                        return cmd.error(QObject::tr("Missing parameter: "
                                                     "number after \"-%1 %2\"")
                                                 .arg(COMMAND_RANSAC,
                    ...
                    if (!ok || (val <= 0.0f || val >= 1.0f)) {
                        return cmd.error(
                                "Invalid number for Bitmap epsilon must be a "
                                "float greater than 0.0 and less than 1.0!!");
                    }
                    cmd.print(QObject::tr("\tBitmap Epsilon : %1").arg(val));
                    ...
                    if (cmd.arguments().empty()) {
                        return cmd.error(
                                QObject::tr("Missing parameter: number after "
                                            "\"-%1 %2\"")
                                        .arg(COMMAND_RANSAC, SUPPORT_POINTS));
                    }
                    bool ok;
                    unsigned count = cmd.arguments().takeFirst().toUInt(&ok);
                    if (!ok) {
                        return cmd.error(
                                "Invalid number of for support points!");
                    }
                    ...
                    if (cmd.arguments().empty()) {
                        return cmd.error(
                                QObject::tr("Missing parameter: number after "
                                            "\"-%1 %2\"")
                                        .arg(COMMAND_RANSAC, MAX_NORMAL_DEV));
                    }
                    bool ok;
                    float val = cmd.arguments().takeFirst().toFloat(&ok);
                    if (!ok) {
                        return cmd.error(
                                "Invalid number for Max Normal Deviation!");
                    }
                    cmd.print(QObject::tr("\tMax Normal Deviation : %1")
                                      .arg(val));
                    ...
                    if (cmd.arguments().empty()) {
                        return cmd.error(
                                QObject::tr("Missing parameter: number after "
                                            "\"-%1 %2\"")
                                        .arg(COMMAND_RANSAC, PROBABILITY));
                    }
                    bool ok;
                    float val = cmd.arguments().takeFirst().toFloat(&ok);
                    if (!ok) {
                    ...
                    if (!makePathIfPossible(cmd, param, &outputCloudsDir,
                                            &outputIndividualClouds)) {
                        return false;
                    }
                } else if (param == OUT_MESH_DIR) {
                    if (!makePathIfPossible(cmd, param, &outputMeshesDir,
                                            &outputIndividualPrimitives)) {
                        return false;
                    }
                } else if (param == OUT_GROUP_DIR) {
                    if (!makePathIfPossible(cmd, param, &outputGroupDir,
                                            &outputGrouped)) {
                        return false;
                    }
                } else if (param == OUT_PAIR_DIR) {
                    if (!makePathIfPossible(cmd, param, &outputPairDir,
                                            &outputIndividualPairs)) {
                        return false;
                    }
                    ...
                    if (cmd.arguments().empty()) {
                        return cmd.error(
                                QObject::tr("Missing parameter: primitive type "
                                            "after \"-%1 %2\"")
                                        .arg(COMMAND_RANSAC, ENABLE_PRIMITIVE));
                    }
                    ...
                    if (primCount > 0) {
                        ...
                        return cmd.error(
                                QObject::tr("No valid parameter: primitive "
                                            "type after \"-%1 %2\"")
                                        .arg(COMMAND_RANSAC, ENABLE_PRIMITIVE));
                    }
                }
        ...
        if (primCount == 0) {
            cmd.print(QObject::tr("\tDefault Shape Search == %1")
                              .arg(PRIM_PLANE));
            params.primEnabled[qRansacSD::RPT_PLANE] = true;
        }
        if (!outputIndividualClouds && !outputIndividualPrimitives &&
            !outputGrouped && !outputIndividualPairs) {
            cmd.print(
                    QObject::tr("\tDefault output == %1").arg(OUTPUT_GROUPED));
            outputGrouped = true;
        }
        ...
        for (CLCloudDesc clCloud : cmd.clouds()) {
            ...
            ccHObject* group = qRansacSD::executeRANSAC(clCloud.pc, params,
                                                        cmd.silentMode());

            if (group) {
                if (outputGrouped) {
                    CLGroupDesc clGroup(group,
                                        clCloud.basename + "_" +
                                                clCloud.pc->getName() +
                                                "_RANSAC_DETECTED_SHAPES",
                                        outputGroupDir != "" ? outputGroupDir
                                                             : clCloud.path);
                    QString errorStr = cmd.exportEntity(
                            clGroup, QString(), nullptr,
                ...
                if (outputIndividualPrimitives || outputIndividualClouds ||
                    outputIndividualPairs) {
                    ccHObject::Container meshGroup;
                    unsigned meshCount = group->filterChildren(meshGroup, true,
                                                               CV_TYPES::MESH);
                    ...
                    for (auto meshObj : meshGroup) {
                        ...
                        if (mesh) {
                            QString suffix;
                            if (meshObj->isA(CV_TYPES::PLANE)) {
                                suffix = QString("_PLANE_%1")
                                                 .arg(planeCount, 4, 10,
                                                      QChar('0'));
                                planeCount++;
                            } else if (meshObj->isA(CV_TYPES::SPHERE)) {
                                suffix = QString("_SPHERE_%1")
                                                 .arg(sphereCount, 4, 10,
                                                      QChar('0'));
                                sphereCount++;
                            } else if (meshObj->isA(CV_TYPES::CYLINDER)) {
                                suffix = QString("_CYLINDER_%1")
                                                 .arg(cylinderCount, 4, 10,
                                                      QChar('0'));
                                cylinderCount++;
                            } else if (meshObj->isA(CV_TYPES::CONE)) {
                                suffix = QString("_CONE_%1")
                                                 .arg(coneCount, 4, 10,
                                                      QChar('0'));
                                coneCount++;
                            } else if (meshObj->isA(CV_TYPES::TORUS)) {
                                suffix = QString("_TORUS_%1")
                                                 .arg(torusCount, 4, 10,
                                                      QChar('0'));
                                torusCount++;
                            }
                            auto cld = ccHObjectCaster::ToPointCloud(
                                    mesh->getParent());
                            if (outputIndividualPairs) {
                                CLGroupDesc clPair(
                                        cld,
                                        clCloud.basename + "_" +
                                                clCloud.pc->getName() + suffix +
                                                QString("_pair"),
                                        outputPairDir != "" ? outputPairDir
                                                            : clCloud.path);
                                QString errorStr = cmd.exportEntity(
                                        clPair, QString(), nullptr,
                                        ccCommandLineInterface::ExportOption::
                                                ForceHierarchy);
                                if (!errorStr.isEmpty()) {
                                    cmd.warning(errorStr);
                            ...
                            CLCloudDesc clCloudp(
                                    cld,
                                    clCloud.basename + "_" +
                                            clCloud.pc->getName() + suffix +
                                            QString("_cloud"),
                                    outputCloudsDir != "" ? outputCloudsDir
                                                          : clCloud.path);
                            ...
                            CLMeshDesc clMesh(
                                    ...
                                    clCloud.basename + "_" +
                                            clCloud.pc->getName() + suffix,
                                    outputMeshesDir != "" ? outputMeshesDir
                                                          : clCloud.path);
            ...
            if (cmd.autoSaveMode()) {
                QString errorStr =
                        cmd.exportEntity(clCloud);  // The original cloud may
                                                    // have had normals added
                if (!errorStr.isEmpty()) {
                    cmd.warning(errorStr);
                }
            }
    ...
    bool makePathIfPossible(ccCommandLineInterface& cmd,
                            const QString& param,
                            QString* outputPath,
                            bool* performOutput) {
        if (cmd.arguments().empty()) {
