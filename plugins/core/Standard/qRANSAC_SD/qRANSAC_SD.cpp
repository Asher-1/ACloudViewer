...
void qRansacSD::doAction()
{
	...
	RansacParams params;
	{
		...
		s_maxNormalDev_deg = static_cast<float>(rsdDlg.maxNormDevAngleSpinBox->value());
...
ccHObject* qRansacSD::executeRANSAC(ccPointCloud* ccPC, const RansacParams& params, bool silent)
{
	...
	PointCloud cloud;
	{
		...
		{
			CVLog::Error("[qRansacSD] Could not create temporary cloud, Not enough memory!");
			return nullptr;
		}
	...
	if (shapes.size() > 0)
	{
		...
		for (MiscLib::Vector<DetectedShape>::const_iterator it = shapes.begin(); it != shapes.end(); ++it)
		{
			...
			bool saveNormals = true;
			{
				...
				{
					refPcShape.addPointIndex(static_cast<unsigned int>(cloud[shapeCloudIndex - j].index));
				}
				int warnings = 0;
			...
			case RPT_CONE: //cone
			{
				...
				if (minRadius < params.minRadius || maxRadius > params.maxRadius)
				{
					count -= shapePointsCount;
					delete pcShape;
					continue;
		...
		if (group)
		{
			...
			CVLog::Warning("[qRansacSD] Input cloud has been automatically hidden!");
