...
void ReadPoints3DText(const std::string& path,
    ...
    while (std::getline(file, line)) {
        ...
        // Color
        std::getline(line_stream, item, ' ');
        std::cerr << "point3D.Color(0) = "
                  << static_cast<uint8_t>(std::stoi(item)) << std::endl;

        std::getline(line_stream, item, ' ');
        std::cerr << "point3D.Color(1) = "
                  << static_cast<uint8_t>(std::stoi(item)) << std::endl;

        std::getline(line_stream, item, ' ');
        std::cerr << "point3D.Color(2) = "
                  << static_cast<uint8_t>(std::stoi(item)) << std::endl;
        ...
        while (!line_stream.eof()) {
            ...
            std::cerr << "track_el.image_id = " << std::stoul(item)
                      << std::endl;

            std::getline(line_stream, item, ' ');
            std::cerr << "track_el.point2D_idx = " << std::stoul(item)
                      << std::endl;
...
bool Mesh::saveToObj(const std::string& filename) const {
    ...
    for (auto itr = vVertices.begin(); itr != vVertices.end(); ++itr) {
        pMesh->mVertices[itr - vVertices.begin()] = aiVector3D(
                vVertices[j].x(), vVertices[j].y(), vVertices[j].z());
        if (hasNormals())
            pMesh->mNormals[itr - vVertices.begin()] = aiVector3D(
                    _normals[j].x(), _normals[j].y(), _normals[j].z());
        if (hasTexCoords())
            pMesh->mTextureCoords[0][itr - vVertices.begin()] =
                    aiVector3D(_texcoords[j][0], _texcoords[j][1], 0);
        j++;
    }
...
bool Mesh::saveToBinaryPLY(const std::string& filename,
                           bool universal,
    ...
    if (file) {
        ...
        for (uint i = 0; i < _vertices.size(); ++i) {
            ...
            if (hasColors()) {
                ...
                if (universal)
                    bytes << uint8(c[0] * (UINT8_MAX - 1))
                          << uint8(c[1] * (UINT8_MAX - 1))
                          << uint8(c[2] *
                                   (UINT8_MAX -
                                    1));  // ! converting colors explicitly
                else
                    bytes << uint16(c[0] * (UINT16_MAX - 1))
        ...
        for (uint i = 0; i < _triangles.size(); ++i) {
            ...
            bytes << uint8(3);
            for (uint j = 0; j < 3; ++j) bytes << uint32(tri[j]);
        }

        file.write(reinterpret_cast<const char*>(bytes.buffer()),
                   bytes.bufferSize());
        file.close();
        SIBR_LOG << "Saving '" << filename << "'... done" << std::endl;
        return true;
    }
    SIBR_LOG << "error: cannot write to file '" << filename << "'."
             << std::endl;
    return false;
}

bool Mesh::saveToASCIIPLY(const std::string& filename,
                          bool universal,
                          const std::string& textureName) const {
    assert(_vertices.size());
...
bool Mesh::load(const std::string& filename, const std::string& dataset_path) {
    ...
    if (!sibr::fileExists(filename)) {
        SIBR_LOG << "Warning: can't load mesh '" << filename << "."
                 << std::endl;
        return false;
    }
    Assimp::Importer importer;
    // importer.SetPropertyBool(AI_CONFIG_PP_FD_REMOVE, true); // cause Assimp
    // to remove all degenerated faces as soon as they are detected
    ...
    if (scene->mNumMeshes == 0) {
        SIBR_LOG
                << "error: the loaded model file ('" << filename
    ...
    for (uint meshId = 0; meshId < scene->mNumMeshes; ++meshId) {
        ...
        for (uint i = 0; i < mesh->mNumVertices; ++i)
            _vertices[offsetVertices + i] =
                    converter * convertVec(mesh->mVertices[i]);

        if (mesh->HasVertexColors(0) && mesh->mColors[0]) {
            _colors.resize(offsetVertices + mesh->mNumVertices);
            for (uint i = 0; i < mesh->mNumVertices; ++i) {
                _colors[offsetVertices + i] =
                        Vector3f(mesh->mColors[0][i].r, mesh->mColors[0][i].g,
                                 mesh->mColors[0][i].b);
            }
        }
        ...
        if (mesh->HasNormals()) {
            ...
            for (uint i = 0; i < mesh->mNumVertices; ++i) {
                _normals[offsetVertices + i] =
                        converter * convertVec(mesh->mNormals[i]);
            }
        }
        ...
        if (mesh->HasTextureCoords(0)) {
            ...
            std::string texFileName =
                    dataset_path + "/capreal/" + _textureImageFileName;
            if (!fileExists(texFileName))
                texFileName = parentDirectory(parentDirectory(dataset_path)) +
                              "/capreal/" + _textureImageFileName;
            if (!fileExists(texFileName))
                texFileName = parentDirectory(dataset_path) + "/capreal/" +
                              _textureImageFileName;
            ...
            if (!mesh->HasVertexColors(0) && fileExists(texFileName)) {
                ...
                for (uint ci = 0; ci < mesh->mNumVertices; ++ci) {
                    ...
                    Vector3ub col = texImg((uv[0] * texImg.w()),
                                           uint((1 - uv[1]) * texImg.h()));
                    _colors[offsetVertices + ci] = Vector3f(
                            float(col[0]) / 255.0, float(col[1]) / 255.0,
                            float(col[2]) / 255.0);
                }
        ...
        for (uint i = 0; i < mesh->mNumFaces; ++i) {
            ...
            if (f->mNumIndices != 3)
                SIBR_LOG << "warning: discarding a face (not a triangle, num "
                            "indices: "
                         << f->mNumIndices << ")" << std::endl;
            else {
                Vector3u tri = Vector3u(offsetVertices + f->mIndices[0],
                                        offsetVertices + f->mIndices[1],
                                        offsetVertices + f->mIndices[2]);
                if (tri[0] < 0 || tri[0] >= _vertices.size() || tri[1] < 0 ||
                    tri[1] >= _vertices.size() || tri[2] < 0 ||
                    tri[2] >= _vertices.size())
                    SIBR_WRG << "face num [" << i
                             << "] contains invalid vertex id(s)" << std::endl;
                else {
    ...
    SIBR_LOG << "Mesh '" << filename << " successfully loaded. "
             << scene->mNumMeshes << " meshes were loaded with a total of "
             << " (" << _triangles.size() << ") faces and "
             << " (" << _vertices.size() << ") vertices detected. Init GL ..."
             << std::endl;
...
bool Mesh::loadSfM(const std::string& filename,
    ...
    if (!sibr::fileExists(fname)) {
        SIBR_LOG << "Warning: can't load mesh '" << fname << "." << std::endl;
        return false;
    }
    Vertices verts;
    Colors cols;
    ...
    for (uint i = 0; i < numverts; ++i) _vertices[i] = verts[i];

    _colors.resize(numverts);

    for (uint i = 0; i < numverts; ++i)
        _colors[i] = Vector3f(cols[i].x(), cols[i].y(), cols[i].z());

    _meshPath = dataset_path + "/points3D.bin";
    _renderingOptions.mode = PointRenderMode;

    SIBR_LOG << "SfM Mesh '" << filename << " successfully loaded. " << " ("
             << _vertices.size() << ") vertices detected. Init GL ..."
             << std::endl;
    SIBR_LOG << "Init GL mesh complete " << std::endl;
...
bool sibr::Mesh::loadMtsXML(const std::string& xmlFile) {
    ...
    std::string pathFolder =
            boost::filesystem::path(xmlFile).parent_path().string();
    ...
    for (rapidxml::xml_node<>* node = nodeScene->first_node("shape"); node;
    ...
    for (rapidxml::xml_node<>* node = nodeScene->first_node("shape"); node;
        ...
        if (strcmp(node->first_attribute()->name(), "type") == 0 &&
            ...
            if (nodeTrans) {
                ...
                sibr::Matrix4f m1;
                m1 << std::stof(splitM1[0]), std::stof(splitM1[1]),
                        std::stof(splitM1[2]), std::stof(splitM1[3]),
                        std::stof(splitM1[4]), std::stof(splitM1[5]),
                        std::stof(splitM1[6]), std::stof(splitM1[7]),
                        std::stof(splitM1[8]), std::stof(splitM1[9]),
                        std::stof(splitM1[10]), std::stof(splitM1[11]),
                        std::stof(splitM1[12]), std::stof(splitM1[13]),
                        std::stof(splitM1[14]), std::stof(splitM1[15]);
                ...
                std::vector<std::string> splitM2(
                        ...
                        std::istream_iterator<std::string>());
                sibr::Matrix4f m2;
                m2 << std::stof(splitM2[0]), std::stof(splitM2[1]),
                        std::stof(splitM2[2]), std::stof(splitM2[3]),
                        std::stof(splitM2[4]), std::stof(splitM2[5]),
                        std::stof(splitM2[6]), std::stof(splitM2[7]),
                        std::stof(splitM2[8]), std::stof(splitM2[9]),
                        std::stof(splitM2[10]), std::stof(splitM2[11]),
                        std::stof(splitM2[12]), std::stof(splitM2[13]),
                        std::stof(splitM2[14]), std::stof(splitM2[15]);
            ...
            const std::string filename =
                    nodeRef->first_attribute("value")->value();
            const std::string meshPath = pathFolder + "/" + filename;
...
void Mesh::save(const std::string& filename,
    ...
    if (ext == "obj") {
        ...
        // Meshlab does not support uint16 colors, if you want to use the mesh
        // in such program, set 'universal' = true
        if (universal) {
...
void Mesh::vertices(const std::vector<float>& vertices) {
    ...
    while (it != vertices.end()) {
        ...
        for (int i = 0; i < 3; ++i, ++it) vertex[i] = (*it);
...
void Mesh::triangles(const std::vector<uint>& triangles) {
    ...
    while (it != triangles.end()) {
        ...
        for (int i = 0; i < 3; ++i, ++it) triangle[i] = (*it);
...
void Mesh::texCoords(const std::vector<float>& texcoords) {
    ...
    while (it != texcoords.end()) {
        ...
        for (int i = 0; i < 2; ++i, ++it) texcoord[i] = (*it);
...
void Mesh::normals(const std::vector<float>& normals) {
    ...
    while (it != normals.end()) {
        ...
        for (int i = 0; i < 3; ++i, ++it) normal[i] = (*it);
...
void Mesh::generateNormals(void) {
    ...
    auto normalizeNormal = [](const Vector3f& normal) -> Vector3f {
        ...
        if (len > std::numeric_limits<float>::epsilon()) return normal / len;
    ...
    for (const Vector3u& tri : _triangles) {
        ...
        if (tri[0] > _vertices.size() || tri[1] > _vertices.size() ||
            tri[2] > _vertices.size()) {
            SIBR_ERR << "Incorrect indices (" << i << ") " << tri[0] << ":"
                     << tri[1] << ":" << tri[2] << std::endl;
        } else {
    ...
    for (uint i = 0; i < _normals.size(); ++i) {
        Vector3f n = std::accumulate(vertexNormals[i].begin(),
                                     vertexNormals[i].end(),
...
void Mesh::generateSmoothNormals(int numIter) {
    ...
    for (int it = 0; it < numIter; it++) {
        ...
        for (int i = 0; i < _normals.size(); ++i) {
            Vector3f n = std::accumulate(vertexNormalsIter[i].begin(),
                                         vertexNormalsIter[i].end(),
                                         Vector3f(0.f, 0.f, 0.f));
        ...
        // To avoid float overflow after multiple iterations, we need to
        // normalize. But we can't just normalize each normal separately because
        // we want to preserve the relative triangle area weighting. So instead
        // we just send everything in [0,1] each time apart from the last
        // iteration.
...
void Mesh::laplacianSmoothing(int numIter, bool updateNormals) {
    ...
    /// \todo TODO: we could also detect vertices on the edges of the mesh to
...
void Mesh::adaptativeTaubinSmoothing(int numIter, bool updateNormals) {
    ...
    for (const sibr::Vector3u& tri : _triangles) {
        ...
        for (int i = 0; i < 3; i++) vs.push_back(_vertices[tri[i]]);

        for (int i = 0; i < 3; i++) {
            float angle = acos((vs[i] - vs[(i + 2) % 3])
                                       .normalized()
                                       .dot((vs[(i + 1) % 3] - vs[(i + 2) % 3])
                                                    .normalized()));
            cotanW[tri[i]][tri[(i + 1) % 3]].emplace(1.0f /
                                                     (tan(angle) + 0.00001f));
            cotanW[tri[(i + 1) % 3]][tri[i]].emplace(1.0f /
                                                     (tan(angle) + 0.00001f));
        }
    }
...
#pragma omp parallel for
        for (int vid = 0; vid < verticesSize; ++vid) {
            ...
            for (const auto& c : colorsLocal) {
                ...
                varColor += sibr::Vector3f(pow(c.x() - meanColor.x(), 2),
                                           pow(c.y() - meanColor.y(), 2),
                                           pow(c.z() - meanColor.z(), 2));
            }
...
Mesh Mesh::generateSubMesh(std::function<bool(int)> func) const {
    ...
    if (hasColors()) oldColors = colors();

    sibr::Mesh::Normals oldNormals;
    if (hasNormals()) oldNormals = normals();

    sibr::Mesh::UVs oldUVs;
    if (hasTexCoords()) oldUVs = texCoords();

    for (int v = 0; v < vertices().size(); v++) {
        if (func(v)) {
            newVertices.push_back(vertices()[v]);

            if (hasColors()) newColors.push_back(oldColors[cmptVert]);
            if (hasNormals()) newNormals.push_back(oldNormals[cmptVert]);
            if (hasTexCoords()) newUVs.push_back(oldUVs[cmptVert]);
    ...
    for (sibr::Vector3u t : triangles()) {
        if (mapIdVert[t.x()] != -1 && mapIdVert[t.y()] != -1 &&
            mapIdVert[t.z()] != -1) {
            sibr::Vector3u newt(mapIdVert[t.x()], mapIdVert[t.y()],
                                mapIdVert[t.z()]);
    ...
    if (hasColors()) newMesh.colors(newColors);
    if (hasNormals()) newMesh.normals(newNormals);
    if (hasTexCoords()) newMesh.texCoords(newUVs);
...
void Mesh::forceBufferGLUpdate(bool adjacency) const {
    if (!_gl.bufferGL) {
        SIBR_ERR << "Tried to forceBufferGL on a non OpenGL Mesh" << std::endl;
        return;
    }
...
void Mesh::render(bool depthTest,
    ...
    if (!_gl.bufferGL) {
        SIBR_ERR << "Tried to render a non OpenGL Mesh" << std::endl;
        return;
    }
    ...
    if (_gl.dirtyBufferGL) forceBufferGLUpdate(adjacency);
...
void Mesh::renderSubMesh(unsigned int begin,
                         unsigned int end,
                         bool depthTest,
                         bool backFaceCulling,
                         RenderMode mode,
                         bool frontFaceCulling,
                         bool invertDepthTest) const {
    if (!_gl.bufferGL) {
        SIBR_ERR << "Tried to render a non OpenGL Mesh" << std::endl;
        return;
    }
    if (_gl.dirtyBufferGL) forceBufferGLUpdate();
...
Mesh::SubMesh Mesh::extractSubMesh(const std::vector<int>& newVerticesIds,
                                   VERTEX_LIST_CHOICE v_choice) const {
    ...
    for (const auto& t : triangles()) {
        sibr::Vector3i newVerticesId =
                t.cast<int>().unaryExpr([&oldToNewVertexId](int v_id) {
                    return oldToNewVertexId[v_id];
                });
        if (newVerticesId
                    .unaryViewExpr([](int v_id) { return v_id >= 0 ? 1 : 0; })
                    .all()) {
...
const std::string Mesh::getMeshFilePath(void) const { return _meshPath; }

void Mesh::getBoundingSphere(Vector3f& outCenter,
                             float& outRadius,
                             bool referencedOnly,
                             bool usePCcenter) const {
    ...
    if (usePCcenter) {
        ...
        if (tri.size() == 0) {
            SIBR_WRG << "No triangles found for evaluation of sphere center, "
                        "result will be NaN";
        }
        for (const Vector3u& t : tri) {
            ...
            currentArea =
                    ((vert[t[1]] - vert[t[0]]).cross(vert[t[2]] - vert[t[0]]))
                            .norm() /
                    2.0;
        ...
        outCenter =
                Vector3f(float(xCenter / totalArea), float(yCenter / totalArea),
                         float(zCenter / totalArea));
    }
...
sibr::Mesh::Ptr sibr::Mesh::getEnvSphere(sibr::Vector3f center,
                                         float radius,
    ...
    for (int lat = lowLimit; lat <= highLimit; lat++) {
        for (int lgt = 0; lgt <= 360; lgt++) {
            sibr::Vector3f point =
                    cos(0.5 * M_PI * lat / 90.0f) *
                            (cos(2 * M_PI * lgt / 360.0f) * north +
                             sin(2 * M_PI * lgt / 360.0f) * east) +
    ...
    for (int lat = lowLimit; lat < highLimit; lat++) {
        for (int lgt = 0; lgt < 360; lgt++) {
            ...
            tri.push_back(sibr::Vector3u(lgtShift, lgtShift + delta,
                                         lgtShift + 361 + delta));
            tri.push_back(sibr::Vector3u(lgtShift, lgtShift + 361 + delta,
                                         lgtShift + 361));
        }
    }
...
sibr::Mesh::Ptr Mesh::getTestCube(bool withGraphics) {
    ...
    std::vector<sibr::Vector3u> indices = {
            {0, 1, 2}, {0, 2, 3}, {7, 4, 0}, {7, 0, 3}, {4, 5, 1}, {4, 1, 0},
            {5, 6, 2}, {5, 2, 1}, {3, 2, 6}, {3, 6, 7}, {6, 5, 4}, {6, 4, 7}};
...
Mesh::Ptr Mesh::getSphereMesh(const Vector3f& center,
                              float radius,
                              bool withGraphics,
                              int precision) {
    ...
    for (int t = 0; t < nTheta; ++t) {
        ...
        for (int p = 0; p < nPhi; ++p) {
            ...
            normals[p + nPhi * t] =
                    Vector3d(sinT * cosP, sinT * sinP, cosT).cast<float>();
...
sibr::Mesh::Ptr Mesh::subDivide(float limitSize, size_t maxRecursion) const {
    struct Less {
        bool operator()(const sibr::Vector3f& a,
                        const sibr::Vector3f& b) const {
            return a < b;
        }
    };
    ...
    struct Triangle {
        Triangle()
            : edges_ids(std::vector<int>(3, -1)),
              edges_flipped(std::vector<bool>(3, false)) {}
    ...
    for (const auto& t : triangles()) {
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            const sibr::Vector3f midPoint =
                    0.5f * (vertices()[v0] + vertices()[v1]);
            sibr::Vector3f midNormal(0.0f, 0.0f, 0.0f);
            if (hasNormals()) {
                midNormal =
                        (0.5f * (normals()[v0] + normals()[v1])).normalized();
            }
            ...
            if (mapEdges.count(midPoint) == 0) {
                ...
                const Edge edge = {
                        midPoint, midNormal, {t_id}, length, {v0, v1}};
    ...
    for (const Triangle& t : tris) {
        ...
        if (t.edges_ids[0] == -1 && t.edges_ids[1] == -1 &&
            t.edges_ids[2] == -1) {
            continue;
        }
        ...
        const sibr::Vector3i corners_ids =
                sibr::Vector3i(0, 1, 2).unaryViewExpr([&](int i) {
                    return edges[t.edges_ids[i]]
                            .v_ids[t.edges_flipped[i] ? 1 : 0];
                });
        const sibr::Vector3i midpoints_ids =
                sibr::Vector3i(0, 1, 2).unaryViewExpr([&](int i) {
                    return edge_to_divided_edges[t.edges_ids[i]] >= 0
                                   ? nOldVertices + edge_to_divided_edges
                                                            [t.edges_ids[i]]
                                   : -1;
                });

        if (ks[2] >= 0) {
            newTriangles.push_back(sibr::Vector3u(
                    corners_ids[0], midpoints_ids[0], midpoints_ids[2]));
            newTriangles.push_back(sibr::Vector3u(
                    corners_ids[1], midpoints_ids[1], midpoints_ids[0]));
            newTriangles.push_back(sibr::Vector3u(
                    corners_ids[2], midpoints_ids[2], midpoints_ids[1]));
            newTriangles.push_back(midpoints_ids.cast<unsigned>());
        } else if (ks[1] >= 0) {
            const int candidate_edge_1_v_id_1 = corners_ids[non_ks[0]];
            const int candidate_edge_1_v_id_2 =
                    midpoints_ids[(non_ks[0] + 1) % 3];
            const int candidate_edge_2_v_id_1 =
                    corners_ids[(non_ks[0] + 1) % 3];
            const int candidate_edge_2_v_id_2 =
                    midpoints_ids[(non_ks[0] + 2) % 3];
            const float candidate_edge_1_norm =
                    (newVertices[candidate_edge_1_v_id_1] -
                     newVertices[candidate_edge_1_v_id_2])
                            .norm();
            const float candidate_edge_2_norm =
                    (newVertices[candidate_edge_2_v_id_1] -
                     newVertices[candidate_edge_2_v_id_2])
                            .norm();
            if (candidate_edge_1_norm < candidate_edge_2_norm) {
                newTriangles.push_back(
                        sibr::Vector3u(candidate_edge_1_v_id_1,
                                       corners_ids[(non_ks[0] + 1) % 3],
                                       candidate_edge_1_v_id_2));
                newTriangles.push_back(
                        sibr::Vector3u(candidate_edge_1_v_id_2,
                                       midpoints_ids[(non_ks[0] + 2) % 3],
                                       candidate_edge_1_v_id_1));
            } else {
                newTriangles.push_back(sibr::Vector3u(candidate_edge_2_v_id_1,
                                                      candidate_edge_2_v_id_2,
                                                      corners_ids[non_ks[0]]));
                newTriangles.push_back(
                        sibr::Vector3u(candidate_edge_2_v_id_1,
                                       midpoints_ids[(non_ks[0] + 1) % 3],
                                       candidate_edge_2_v_id_2));
            }
            newTriangles.push_back(
                    sibr::Vector3u(midpoints_ids[(non_ks[0] + 1) % 3],
                                   corners_ids[(non_ks[0] + 2) % 3],
                                   midpoints_ids[(non_ks[0] + 2) % 3]));
        } else if (ks[0] >= 0) {
            newTriangles.push_back(sibr::Vector3u(
                    midpoints_ids[ks[0]], corners_ids[(ks[0] + 1) % 3],
                    corners_ids[(ks[0] + 2) % 3]));
            newTriangles.push_back(sibr::Vector3u(midpoints_ids[ks[0]],
                                                  corners_ids[(ks[0] + 2) % 3],
                                                  corners_ids[ks[0]]));
        } else {
...
float Mesh::meanEdgeSize() const {
    ...
    for (const auto& t : triangles()) {
        ...
        sumSizes += (double)((v1 - v2).norm() + (v2 - v3).norm() +
                             (v3 - v1).norm());
    }
...
void Mesh::merge(const Mesh& other) {
    ...
    if (_vertices.empty()) {
        ...
        for (Vector3u& t : triangles) t += triOffset;

        if (hasNormals())
            _normals.insert(_normals.end(), other.normals().begin(),
                            other.normals().end());

        if (hasColors())
            _colors.insert(_colors.end(), other.colors().begin(),
                           other.colors().end());

        if (hasTexCoords())
            _texcoords.insert(_texcoords.end(), other.texCoords().begin(),
                              other.texCoords().end());

        _vertices.insert(_vertices.end(), other.vertices().begin(),
                         other.vertices().end());
        _triangles.insert(_triangles.end(), triangles.begin(), triangles.end());
    }

    if (withGraphics) _gl.bufferGL.reset(new MeshBufferGL);
}

void sibr::Mesh::makeWhole(void) {
    if (!hasNormals()) _normals = Normals(vertices().size());

    if (!hasColors()) _colors = Colors(vertices().size());

    if (!hasTexCoords()) _texcoords = UVs(vertices().size());
}
...
void Mesh::eraseTriangles(const std::vector<uint>& faceIDList) {
    ...
    for (uint faceID : faceIDList) faceToErase[faceID] = true;
    ...
    if (hasColors()) {
        ...
        for (uint i = 0; i < indexRemap.size(); ++i)
            if (indexRemap[i] != -1) newColors[indexRemap[i]] = colors().at(i);
        colors(newColors);
    }
    ...
    if (hasTexCoords()) {
        ...
        for (uint i = 0; i < indexRemap.size(); ++i)
            if (indexRemap[i] != -1) newUVs[indexRemap[i]] = texCoords().at(i);
        texCoords(newUVs);
    }
