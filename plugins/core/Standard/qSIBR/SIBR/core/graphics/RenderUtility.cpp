...
static const std::vector<float>& getCameraStubVertices(
        float camStubSize = 0.1f) {
    std::vector<float> _vBuffer(3 * 5);
    _vBuffer[3 * 0 + 0] = 1 * camStubSize;
    _vBuffer[3 * 0 + 1] = 1 * camStubSize;
    _vBuffer[3 * 0 + 2] = -3 * camStubSize;
    _vBuffer[3 * 1 + 0] = -1 * camStubSize;
    _vBuffer[3 * 1 + 1] = 1 * camStubSize;
    _vBuffer[3 * 1 + 2] = -3 * camStubSize;
    _vBuffer[3 * 2 + 0] = -1 * camStubSize;
    _vBuffer[3 * 2 + 1] = -1 * camStubSize;
    _vBuffer[3 * 2 + 2] = -3 * camStubSize;
    _vBuffer[3 * 3 + 0] = 1 * camStubSize;
    _vBuffer[3 * 3 + 1] = -1 * camStubSize;
    _vBuffer[3 * 3 + 2] = -3 * camStubSize;
    _vBuffer[3 * 4 + 0] = 0 * camStubSize;
    _vBuffer[3 * 4 + 1] = 0 * camStubSize;
    _vBuffer[3 * 4 + 2] = 0 * camStubSize;
    return _vBuffer;
}
...
void RenderUtility::sendVertsTexToGPU(GLuint vertTexVBO,
                                      GLfloat vert[],
                                      GLfloat tcoord[],
                                      int svert,
                                      int stcoord) {
...
/*static*/ void RenderUtility::renderScreenQuad(bool reverse,
                                                GLfloat tex_coor[]) {
    ...
    static int rqContextId = -1;
    if (rqContextId != Window::contextId) {
        rqContextId = Window::contextId;
        firstTime = true;
    }

    if (reverse) {
        GLfloat tmp[] = {0, 1, 0, 0, 1, 0, 1, 1};
        if (tex_coor) std::memcpy(tmp, tex_coor, sizeof tmp);

        std::memcpy(tcoord, tmp, sizeof tcoord);
        if (!firstTime)  // re-transfer to GPUs
            sendVertsTexToGPU(vertTexVBO, vert, tcoord, sizeof(vert),
                              sizeof(tcoord));
    } else {
...
void RenderUtility::renderScreenQuad() {
    ...
    if (lastContextId != Window::contextId || FfirstTime) {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(Find), Find,
                     GL_STATIC_DRAW);

        glGenBuffers(1, &FvertTexVBO);
        sendVertsTexToGPU(FvertTexVBO, Fvert, Ftcoord, sizeof(Fvert),
                          sizeof(Ftcoord));
...
/*static*/ void RenderUtility::useDefaultVAO(void) {
    ...
    static int vaoContextId = -1;
    if (vaoContextId != Window::contextId) {
        vaoContextId = Window::contextId;
        gDefaultVAO = 0;
    }

    if (!gDefaultVAO) glGenVertexArrays(1, &gDefaultVAO);
