...
Input Input::subInput(const sibr::Input& global,
                      const sibr::Viewport& viewport,
                      const bool mouseOutsideDisablesKeyboard) {
    Input sub = global;
    sub._mousePrevPos -=
            sibr::Vector2i(viewport.finalLeft(), viewport.finalTop());
...
KeyCombination operator&&(const KeyCombination& combA,
                          const KeyCombination& combB) {
    return KeyCombination(combA.numKeys + combB.numKeys,
                          combA.isTrue && combB.isTrue);
}
