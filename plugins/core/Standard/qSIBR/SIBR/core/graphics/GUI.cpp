...
bool showImGuiWindow(const std::string& windowTitle,
                     const IRenderTarget& rt,
    ...
    if (updateLayout) {
        ImGui::SetNextWindowPos(
                ImVec2(viewport.finalLeft(), viewport.finalTop()));
        ImGui::SetNextWindowSize(ImVec2(0, 0));
        ImGui::SetNextWindowContentSize(
                ImVec2(viewport.finalWidth(), viewport.finalHeight()));
    }

    if (::ImGui::Begin(windowTitle.c_str(), NULL, flags)) {
        // Get the current cursor position (where your window is)
        ImVec2 pos =
                /*ImGui::GetItemRectMin() + */ ::ImGui::GetCursorScreenPos();
        Vector2f offset, size;
        Vector2i availRegionSize(::ImGui::GetContentRegionAvail().x,
                                 ::ImGui::GetContentRegionAvail().y);

        fitImageToDisplayRegion(viewport.finalSize(), availRegionSize, offset,
                                size);
        ...
        ImGui::SetCursorPos(
                ImVec2(offset.x(), ImGui::GetTitleBarHeight() + offset.y()));
        ImGui::InvisibleButton(
                (windowTitle + "--TEXTURE-INVISIBLE_BUTTON").c_str(),
                ImVec2(size.x(), size.y()));
        if (!invalidTexture) {
            ::ImGui::GetWindowDrawList()->AddImage(
                    (void*)(intptr_t)(rt.handle(handle)), pos,
                    ImVec2(pos.x + size.x(), pos.y + size.y()), ImVec2(0, 1),
                    ImVec2(1, 0));
        }
...
Mesh::Ptr generateMeshForText(const std::string& text,
                              unsigned int& separationIndex) {
    // Technically we don't care if we already are in the middle of a ImGui
    // frame. as long as we clear the draw list. ImGui will detect the empty
    // draw lists and cull them.
    ImGui::PushID(1234567809);
    ImGui::SetNextWindowPos(ImVec2(0, 0));
    ImGui::Begin(text.c_str(), nullptr,
    ...
    // Store the separation idnex between the background and the text
    // foreground.
...
void fitImageToDisplayRegion(const Vector2f& imgSize,
...
void ZoomInterraction::updateZoom(const sibr::Vector2f& canvasSize) {
    ...
    if (zoomData.underMofidication) {
        ImGui::GetWindowDrawList()->AddRect(
                ImVec2(zoomData.firstClickPixel[0],
                       zoomData.firstClickPixel[1]),
                ImVec2(zoomData.secondClickPixel[0],
                       zoomData.secondClickPixel[1]),
                IM_COL32(255, 0, 0, 255), 0, 0, 2);
    }

    if (d.isReleasedRight && zoomData.underMofidication) {
        zoomData.underMofidication = false;
        if ((zoomData.tmpBottonRight - zoomData.tmpTopLeft)
                    .cwiseProduct(canvasSize)
                    .cwiseAbs()
                    .minCoeff() > 10) {
            zoomData.center =
                    0.5f * (zoomData.tmpBottonRight + zoomData.tmpTopLeft);
            zoomData.diagonal =
                    0.5f *
                    (zoomData.tmpBottonRight - zoomData.tmpTopLeft).cwiseAbs();
    ...
    if (d.isHoovered && d.scroll != 0) {
        zoomData.diagonal = zoomData.diagonal.cwiseProduct(
                pow(1.15f, -d.scroll) * sibr::Vector2f(1, 1));
    }
    ...
    if (!target.contains(current)) {
        ...
        for (int c = 0; c < 4; ++c) {
            ...
            if ((current.corner(cType) - inside.corner(cType)).isZero()) {
                ...
                zoomData.center +=
                        (inside.corner(opposite) - current.corner(opposite));
                break;
            }
        }
    }
...
void SegmentSelection::update(const CallBackData& callback,
                              const sibr::Vector2i& size,
                              const ZoomData& zoom) {
    sibr::Vector2i pos = zoom.scaled(size.cast<float>())
                                 .uvFromBoxPos(callback.positionRatio)
                                 .cast<int>();

    if (callback.isHoovered && callback.isClickedRight &&
        callback.ctrlPressed && (!first || valid)) {
...
void SegmentSelection::computeRasterizedLine() {
    ...
    for (int i = 0; i <= l; ++i) {
        rasterizedLine[i] =
                (firstPosIm.cast<float>() + (i / (float)l) * diff.cast<float>())
                        .cast<int>();
    }
}

void DisplayImageGui(GLuint texture,
                     const sibr::Vector2i& displaySize,
                     const sibr::Vector2f& uv0,
                     const sibr::Vector2f& uv1) {
    ImGui::Image((void*)(intptr_t)(texture),
                 ImVec2(float(displaySize[0]), float(displaySize[1])),
                 ImVec2(uv0[0], uv0[1]), ImVec2(uv1[0], uv1[1]));
}

void ImageWithCallback(GLuint texture,
                       const sibr::Vector2i& displaySize,
    ...
    if (data.isHoovered) {
        ...
        data.positionRatio =
                (data.mousePos - data.itemPos).cwiseQuotient(data.itemSize);
    }
}

void ImageWithZoom(GLuint texture,
                   const sibr::Vector2i& displaySize,
                   ZoomInterraction& zoom) {
    ImageWithCallback(texture, displaySize, zoom.callBackData,
                      zoom.zoomData.topLeft(), zoom.zoomData.bottomRight());
...
void PlotMultiLines(const char* label,
                    ...
                    float scale_max,
                    ImVec2 graph_size) {
    // Note: code extracted from ImGui and udpated to display multiple lines on
    // the same graph.
        ...
        for (int vid = 0; vid < values.size(); ++vid) {
            ...
            float histogram_zero_line_t =
                    (scale_min * scale_max < 0.0f)
                            ? (-scale_min * inv_scale)
                            : (scale_min < 0.0f ? 0.0f
                                                : 1.0f);  // Where does the zero
                                                          // line stands

            const ImU32 col_base =
                    GetColorU32(colors[vid >= colors.size() ? 0 : vid]);
