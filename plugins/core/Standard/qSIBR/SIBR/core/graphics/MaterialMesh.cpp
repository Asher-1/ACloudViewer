...
bool MaterialMesh::load(const std::string& filename) {
    ...
    if (!scene) {
        SIBR_WRG << "Warning: can't load mesh '" << filename << "' ("
        ...
        if (randomUV) {
            ...
            for (uint i = 0; i < mesh->mNumVertices; ++i) {
                ...
                float v = static_cast<float>(rand()) /
                          static_cast<float>(RAND_MAX);
                _texcoords[offsetVertices + i] = Vector2f(u * 5.f, v * 5.f);
            }
        }
...
sibr::Matrix4f parseTransform(const rapidxml::xml_node<>* nodeTrans) {
    ...
    if (nodeTrans) {
        ...
        const auto getAxesValues = [&](rapidxml::xml_node<>* n, float& x,
                                       float& y, float& z) {
            getfValue(n, "x", x);
            getfValue(n, "y", y);
            getfValue(n, "z", z);
        ...
        for (rapidxml::xml_node<>* node = nodeTrans->first_node(); node;
            ...
            if (strcmp(transformType, "matrix") == 0) {
                std::string matrixValue =
                        node->first_attribute("value")->value();
                std::istringstream issMatrix(matrixValue);
                std::vector<std::string> split(
                        std::istream_iterator<std::string>{issMatrix},
                        std::istream_iterator<std::string>());
                nodeMatrix << std::stof(split[0]), std::stof(split[1]),
                        std::stof(split[2]), std::stof(split[3]),
                        std::stof(split[4]), std::stof(split[5]),
                        std::stof(split[6]), std::stof(split[7]),
                        std::stof(split[8]), std::stof(split[9]),
                        std::stof(split[10]), std::stof(split[11]),
                        std::stof(split[12]), std::stof(split[13]),
                        std::stof(split[14]), std::stof(split[15]);
            } else if (strcmp(transformType, "translate") == 0) {
                getAxesValues(node, nodeMatrix(0, 3), nodeMatrix(1, 3),
                              nodeMatrix(2, 3));
            } else if (strcmp(transformType, "scale") == 0) {
                float scale = 1.f;
                getfValue(node, "value", scale);
                getAxesValues(node, nodeMatrix(0, 0), nodeMatrix(1, 1),
                              nodeMatrix(2, 2));
                ...
                transform.rotate(Eigen::Quaternionf(Eigen::AngleAxisf(
                        angleRadians,
                ...
                throw std::runtime_error(
                        std::string(
                                "Mitsuba xml parser: Unknown tranform type: ") +
                        transformType);
            }
...
bool shouldFlipNormals(const rapidxml::xml_node<>* shape) {
    ...
    for (const rapidxml::xml_node<>* node = shape->first_node("boolean"); node;
         node = node->next_sibling("boolean")) {
        if (node->first_attribute("name")->value() ==
            std::string("flipNormals")) {
...
std::string parseFilename(const rapidxml::xml_node<>* shape) {
    assert(shape);
    for (const rapidxml::xml_node<>* node = shape->first_node("string"); node;
...
bool MaterialMesh::loadMtsXML(const std::string& xmlFile, bool loadTextures) {
    ...
    for (rapidxml::xml_node<>* node = nodeScene->first_node("shape"); node;
         node = node->next_sibling("shape")) {
        for (rapidxml::xml_attribute<>* browserAttributes =
            ...
            if (strcmp(browserAttributes->name(), "type") == 0 &&
                ...
                if (idToShapegroups.find(_id) == idToShapegroups.end()) {
                    SIBR_WRG << "Could not find shapegroup " << _id << "!!!"
                             << std::endl;
                    continue;
                }
                ...
                sibr::Matrix4f objectToWorld =
                        shapeGroup.objectToWorld * parseTransform(nodeTrans);
                sibr::MaterialMesh instance;
                for (const auto& shape : shapeGroup.shapes) {
                    std::cout << shape.filename;
                    const std::string meshPath =
                            pathFolder + "/" + shape.filename;
                    ...
                    sibr::Matrix4f matrix = shape.toWorld * objectToWorld;
                    {
                        sibr::MaterialMesh::Vertices vertices(
                                toWorldMesh.vertices().size());
                        for (int v = 0; v < toWorldMesh.vertices().size();
                             v++) {
                            sibr::Vector4f v4(toWorldMesh.vertices()[v].x(),
                                              toWorldMesh.vertices()[v].y(),
                                              toWorldMesh.vertices()[v].z(),
                                              1.0);
                    ...
                    if (toWorldMesh.hasNormals()) {
                        sibr::Mesh::Normals normals(
                                toWorldMesh.normals().size());
                        const sibr::Matrix3f normalTMatrix =
                                matrix.block(0, 0, 3, 3).inverse().transpose();
            ...
            // Create the "unique" shapes
                ...
                while (nodeOpt) {
                    const std::string name =
                            nodeOpt->first_attribute("name")->value();
                    const std::string value =
                            nodeOpt->first_attribute("value")->value();
                ...
                if (nodeRefMat) {
                    const std::string matName =
                            nodeRefMat->first_attribute("id")->value();
    ...
    SIBR_LOG << "Loaded mesh: " << vertices().size() << " verts, "
             << meshIds().size() << " ids." << std::endl;
    // Load all the materials
    for (rapidxml::xml_node<>* node = nodeScene->first_node("bsdf"); node;
        ...
        if (attribute != nullptr) {
            ...
            while (!queue.empty()) {
                ...
                for (rapidxml::xml_node<>* nodeTexture =
                    ...
                    if (strcmp(nodeTexture->first_attribute("name")->value(),
                        ...
                        // std::cout << "DiffuseReflectance Texture found" <<
                        // std::endl;
                        rapidxml::xml_node<>* firstTexture =
                                nodeTexture->first_node("texture");
                        if (firstTexture == nullptr) {
                            firstTexture = nodeTexture;
                        }
                        for (rapidxml::xml_node<>* nodeString =
                                     firstTexture->first_node("string");
                             nodeString;
                             nodeString = nodeString->next_sibling("string")) {
                            std::string textureName =
                                    nodeString->first_attribute("value")
                                            ->value();
                            sibr::ImageRGBA::Ptr texture(new sibr::ImageRGBA());
                            // If we skip loading the textures, still set them
                            // as empty images.
                            if (!loadTextures ||
                                texture->load(pathFolder + "/" + textureName)) {
                    ...
                    for (std::string colorsFormat : colorsFormatList)
                        for (rapidxml::xml_node<>* nodeTexture =
                             ...
                             nodeTexture = nodeTexture->next_sibling(
                                     colorsFormat.c_str())) {
                            if (strcmp(nodeTexture->first_attribute("name")
                                               ->value(),
                                       "diffuseReflectance") == 0 ||
                                strcmp(nodeTexture->first_attribute("name")
                                               ->value(),
                                       "reflectance") == 0 ||
                                strcmp(nodeTexture->first_attribute("name")
                                               ->value(),
                                ...
                                /*std::cout << "DiffuseReflectance Color found"
                                        << std::endl;*/
                                rapidxml::xml_node<>* firstTexture =
                                        nodeTexture->first_node(
                                                colorsFormat.c_str());
                                if (firstTexture == nullptr) {
                                    firstTexture = nodeTexture;
                                }
                                std::string colorString =
                                        nodeTexture->first_attribute("value")
                                                ->value();
                                sibr::Vector3f colorMaterial;
                                float redComponent, greenComponent,
                                        blueComponent;
#ifdef SIBR_OS_WINDOWS
                                sscanf_s(colorString.c_str(), "%f, %f, %f",
                                         &redComponent, &greenComponent,
                                         &blueComponent);
#else
                                ...
                                const sibr::ImageRGBA::Pixel color(
                                        ...
                                        static_cast<const unsigned char>(
                                                blueComponent * 255),
                                        255);
                                sibr::ImageRGBA::Ptr texture(
                                        new sibr::ImageRGBA(1, 1, color));
                                if (texture) {
            ...
            if (_diffuseMaps[nameMat].get() == nullptr) {
                ...
                sibr::ImageRGBA::Ptr texture(new sibr::ImageRGBA(1, 1, color));
                SIBR_WRG << "Warning: No color and no texture found for "
                         << nameMat << ", "
                ...
                /*if (_hasTagsCoveringFile) {
                        _tagsCoveringMaps[nameMat] = _listCoveringImagesTags.at(
                                _tagsCoveringMaps.size() %
                _listCoveringImagesTags.size());
                }*/
            }
        }
    }
            ...
            for (rapidxml::xml_node<>* nodeTexture =
                ...
                if (strcmp(nodeTexture->first_attribute("name")->value(),
                    ...
                    strcmp(nodeTexture->first_attribute("type")->value(),
                           "scale") == 0) {
                    // std::cout << "Found opacity mask:" << nameMat <<
                    // std::endl;

                    for (rapidxml::xml_node<>* nodeString =
                                 nodeTexture->first_node("texture")->first_node(
                                         "string");
                         nodeString;
                         nodeString = nodeString->next_sibling("string")) {
                        std::string textureName =
                        ...
                        if (!loadTextures ||
                            ...
                            SIBR_ERR << "Opacity layer for: " << nameMat
                                     << " not found" << std::endl;
                        }
                    }
                    if (breakBool) break;
...
void MaterialMesh::loadCoveringTagsTexture(
    ...
    for (const std::string filename : listFilesTags) {
        ...
        if (textureTag->load(filename)) {
            ...
            SIBR_ERR << "Diffuse layer for: " << filename << " not found"
                     << std::endl;
        }
    }
    if (_listCoveringImagesTags.size() > 0) _hasTagsCoveringFile = true;
...
void MaterialMesh::fillColorsWithIndexMaterials(void) {
    ...
    for (unsigned int i = 0; i < _matIds.size(); i++) {
        colorsIdsMaterials.at(_triangles.at(i)[0]) =
                randomsColors.at(_matIds.at(i));
        colorsIdsMaterials.at(_triangles.at(i)[1]) =
                randomsColors.at(_matIds.at(i));
        colorsIdsMaterials.at(_triangles.at(i)[2]) =
                randomsColors.at(_matIds.at(i));
    }
...
void MaterialMesh::fillColorsWithMatIds() {
    ...
    for (unsigned int i = 0; i < _matIds.size(); i++) {
        ...
        const sibr::Vector3u col = {uchar(matId & 0xff),
                                    uchar((matId >> 8) & 0xff),
                                    uchar((matId >> 16) & 0xff)};
...
Mesh MaterialMesh::generateSubMaterialMesh(int material) const {
    ...
    if (hasTexCoords()) oldTexCoords = texCoords();
    ...
    if (hasTexCoords()) newMesh.texCoords(newTexCoords);
...
void MaterialMesh::subdivideMesh2(float threshold) {
    ...
    while (mustChange) {
        ...
        for (unsigned int i = 0; i < triangles().size(); i++) {
            ...
            int tMatId;
            if (i < matIds().size()) tMatId = matIds().at(i);
            ...
            if (areaHeronsFormula(a, b, c) >= (_averageArea * threshold)) {
                ...
                if (hasTexCoords()) {
                    newTexCoord =
                            (texCoords().at(t.x()) + texCoords().at(t.y()) +
                             texCoords().at(t.z())) /
                            3.f;
                }
                ...
                newTriangles.push_back(
                        sibr::Vector3u(t.x(), t.y(), newIndexVertex));
                newTriangles.push_back(
                        sibr::Vector3u(t.y(), t.z(), newIndexVertex));
                newTriangles.push_back(
                        sibr::Vector3u(t.z(), t.x(), newIndexVertex));
                if (i < matIds().size()) {
                ...
                if (i < matIds().size()) newMatIds.push_back(tMatId);
            }
        }
    ...
    while (mustChange) {
        ...
        for (unsigned int i = 0; i < triangles().size(); i++) {
            ...
            if (localMaximum >= (_averageSize * threshold)) {
                ...
                sibr::Vector3f *v1Pos, *v2Pos, *v1Color, *v2Color, *v1Normal,
                        *v2Normal;
                ...
                if (longestSide == 0) {
                    ...
                    v1Normal = &aNormal;
                    v2Normal = &bNormal;
                    v1TexCoords = &aTexCoords;
                    v2TexCoords = &bTexCoords;
                    ...
                    v1Normal = &bNormal;
                    v2Normal = &cNormal;
                    v1TexCoords = &bTexCoords;
                    v2TexCoords = &cTexCoords;
                    ...
                    v1Normal = &cNormal;
                    v2Normal = &aNormal;
                    v1TexCoords = &cTexCoords;
                    v2TexCoords = &aTexCoords;
                }
                ...
                newNormal =
                        sibr::Vector3f((v1Normal->x() + v2Normal->x()) / 2.f,
                                       (v1Normal->y() + v2Normal->y()) / 2.f,
                                       (v1Normal->z() + v2Normal->z()) / 2.f);
                newTexCoord = sibr::Vector2f(
                        (v1TexCoords->x() + v2TexCoords->x()) / 2.f,
                ...
                if (longestSide == 0) {
                    newTriangles.push_back(
                            sibr::Vector3u(t.x(), newIndexVertex, t.z()));
                    newTriangles.push_back(
                            sibr::Vector3u(newIndexVertex, t.y(), t.z()));
                } else if (longestSide == 1) {
                    newTriangles.push_back(
                            sibr::Vector3u(t.x(), t.y(), newIndexVertex));
                    newTriangles.push_back(
...
void MaterialMesh::initAlbedoTextures(void) {
    ...
    for (auto it = matId2Name().begin(); it != matId2Name().end(); ++it) {
        ...
        if (texturePtrOpacity && texturePtr) {
            _opacityTextures[i] =
                    std::shared_ptr<sibr::Texture2DRGB>(new sibr::Texture2DRGB(
                            *texturePtrOpacity, SIBR_GPU_LINEAR_SAMPLING));
        ...
        if (_hasTagsCoveringFile && _tagsCoveringMaps[*it]) {
            ...
            _tagsCoveringTexture[*it] =
                    std::shared_ptr<sibr::Texture2DRGB>(new sibr::Texture2DRGB(
                            *texturePtrTag, SIBR_GPU_LINEAR_SAMPLING));
...
void MaterialMesh::renderAlbedo(bool depthTest,
                                bool backFaceCulling,
                                RenderMode mode,
                                bool frontFaceCulling,
                                bool invertDepthTest,
                                bool specificMaterial,
                                std::string nameOfSpecificMaterial) const {
    if (_subMeshes.empty()) {
        return;
    }
    ...
    for (auto it = matId2Name().begin();
         it != matId2Name().end() && !textureFound; ++it) {
        ...
        if (!specificMaterial || *it == nameOfSpecificMaterial)
            if (_albedoTextures[i] != nullptr) {
                ...
                if (_hasTagsCoveringFile && coveringTagImage &&
                    ...
                    if (_switchTags.find(*it) != _switchTags.end() &&
                        _switchTags.at(*it) && _hasTagsFile && _tagTexture)
                        glBindTexture(GL_TEXTURE_2D, _idTagTexture);
                    else
                        glBindTexture(GL_TEXTURE_2D,
                                      _idTagsCoveringTexture.at(*it));
                    ...
                    if (_switchTags.find(*it) != _switchTags.end() &&
                        ...
                        glBindTexture(GL_TEXTURE_2D,
                                      _idTagsCoveringTexture.at(texName));
                    else
                        glBindTexture(GL_TEXTURE_2D, _idTagTexture);
                }
...
void MaterialMesh::renderThreeSixty(bool depthTest,
                                    bool backFaceCulling,
                                    RenderMode mode,
                                    bool frontFaceCulling,
...
void MaterialMesh::render(bool depthTest,
                          bool backFaceCulling,
                          RenderMode mode,
                          bool frontFaceCulling,
                          bool invertDepthTest,
...
void MaterialMesh::merge(const MaterialMesh& other) {
    ...
    for (unsigned int i = 0; i < other.matId2Name().size(); ++i) {
        ...
        for (unsigned int j = 0; j < _matId2Name.size() && !foundSimilarity;
             ++j) {
    ...
    _matId2Name.insert(_matId2Name.end(), matId2Name.begin(), matId2Name.end());
    _opacityMaps.insert(other.opacityMaps().begin(), other.opacityMaps().end());
    _diffuseMaps.insert(other.diffuseMaps().begin(), other.diffuseMaps().end());
    ...
    if (thisHasIds && otherHasIds) {
        ...
        _maxMeshId += 1;
        MaterialMesh::MeshIds newMeshIds(other.vertices().size(),
                                         int(_maxMeshId));
        _meshIds.insert(_meshIds.end(), newMeshIds.begin(), newMeshIds.end());

    } else if (otherHasIds) {
        // in that case give a new ID to the current mesh and insert the other
        // IDs.
        _maxMeshId = other._maxMeshId + 1;
        _meshIds = MaterialMesh::MeshIds(oldVerticesCount, int(_maxMeshId));
        _meshIds.insert(_meshIds.end(), other.meshIds().begin(),
                        other.meshIds().end());
    }
}
...
sibr::MaterialMesh::Ptr MaterialMesh::invertedFacesMesh2() const {
    ...
    auto invertedFacesMaterialMesh =
            std::make_shared<MaterialMesh>(*invertedFacesMesh);
...
void MaterialMesh::addEnvironmentMap(float* forcedCenterX,
    ...
    std::vector<PartOfSphere> partsOfSphere = {PartOfSphere::BOTTOM,
                                               PartOfSphere::UP};
    // partsOfSphere.push_back("bottom");
    // partsOfSphere.push_back("up");

    for (PartOfSphere part : partsOfSphere) {
        std::shared_ptr<Mesh> pSphere =
                getEnvSphere(center, radius, Vector3f(0.f, 1.f, 0.f),
                             Vector3f(1.f, 0.f, 0.f), part);
        ...
        const sibr::ImageRGBA::Pixel color(0, 255, 255, 255);
        ...
        sibr::ImageRGB::Ptr textureOpacity(
                new sibr::ImageRGB(1, 1, opacityAlpha));
