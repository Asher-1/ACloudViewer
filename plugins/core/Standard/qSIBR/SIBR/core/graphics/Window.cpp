...
static void glErrorCallback(GLenum src,
                            GLenum type,
                            GLuint id,
                            GLenum severity,
                            GLsizei size,
    ...
    if (type != GL_DEBUG_TYPE_ERROR) {
        // SIBR_LOG << "[API]" << "(" << src << "," << type << "," << id << ","
        // << severity << "): " << std::string(str, size) << std::endl;
        return;
    }
    std::string errStr;
    switch (src) {
...
static void glfwKeyboardCallback(
    ...
    // We only pass the key input to our code if the interface isn't currently
    // using it.
...
static void glfwCursorPosCallback(GLFWwindow* /*window*/, double x, double y) {
    // We  pass the mouse position to our code iff the interface doesn't need
    // it.
...
static void glfwMouseButtonCallback(GLFWwindow* window,
                                    ...
                                    int action,
                                    int mods) {
    // We only pass the mouse input to our code if the interface isn't currently
    // using it.
    if (!ImGui::GetIO().WantCaptureMouse) {
        if (action == GLFW_PRESS) {
            sibr::Input::global().mouseButton().press(
                    (sibr::Mouse::Code)button);
        } else if (action == GLFW_RELEASE) {
            sibr::Input::global().mouseButton().release(
                    (sibr::Mouse::Code)button);
        }
    } else {
        // We have to pass release events in the case where we pressed while
        // inside our views, and released outside.
        if (sibr::Input::global().mouseButton().isActivated(
                    (sibr::Mouse::Code)button)) {
            sibr::Input::global().mouseButton().release(
                    (sibr::Mouse::Code)button);
        }
...
/*static*/ bool Window::contextIsRunning(void) { return windowCounter > 0; }

Window::AutoInitializer::AutoInitializer(const WindowArgs& args)
    : _useGUI(!args.no_gui && !args.offscreen) {
    if (windowCounter == 0) {
        SIBR_LOG << "Initialization of GLFW" << std::endl;
        glfwSetErrorCallback(glfwErrorCallback);

        if (!glfwInit()) SIBR_ERR << "cannot init glfw" << std::endl;
        if (!args.offscreen) sibr::Input::global().key().clearStates();
    }
    ++windowCounter;
}
...
Window::AutoInitializer::~AutoInitializer(void) {
    ...
    if (windowCounter == 0) {
        if (_useGUI) {
            ImGui_ImplGlfwGL3_Shutdown();  /// \todo TODO: not sure if safe with
                                           /// multi-context
...
Window::Window(uint w,
               uint h,
               const std::string& title,
               const WindowArgs& args,
               const std::string& defaultSettingsFilename)
    : _hiddenInit(args),
      _useGUI(!args.no_gui && !args.offscreen),
      _shouldClose(false) {
...
Window::Window(const std::string& title,
    ...
    // Here autoInitializer is already initialized, thus glfwInit() has been
    // called
    setup(winSize.x() - 2 * margins.x(), winSize.y() - 2 * margins.y(), title,
          args, defaultSettingsFilename);
...
void Window::swapBuffer(void) {
    ...
    // Keep the call below in all cases to avoid accumulating all interfaces in
    // one frame.
    if (_useGUI) ImGui_ImplGlfwGL3_NewFrame();
}
...
void Window::resetSettingsToDefault() {
    ...
    if (iniFilename != "" && fileExists(iniFilename)) {
        if (remove(iniFilename.c_str()))
            SIBR_WRG << "Settings file " << iniFilename
                     << " was not removed due to an error." << std::endl;
        else
            SIBR_LOG << "Settings file " << iniFilename
                     << " was removed successfully." << std::endl;
    } else {
        SIBR_WRG << "Settings file " << iniFilename << " not found."
                 << std::endl;
    }
...
void Window::loadSettings() {
    ...
    LoadIniSettingsFromDisk(std::string(getResourcesDirectory() + "/core/" +
                                        _defaultImguiSettingsFilename)
                                    .c_str());
...
void Window::setup(int width,
                   int height,
                   const std::string& title,
                   const WindowArgs& args,
                   const std::string& defaultSettingsFilename) {
    // IMPORTANT NOTE: if you got compatibility problem with old opengl
    // function, try to load compat 3.2 instead of core 4.2
...
#else
    if (args.offscreen)
        SIBR_WRG << "Offscreen enabled without EGL implementation. Using "
                    "native context (Offscreen might run into issues if no "
    ...
    _glfwWin =
            GLFWwindowptr(glfwCreateWindow(width, height, title.c_str(),
                                           (args.fullscreen && !args.offscreen)
    ...
    if (_glfwWin == nullptr)
        SIBR_ERR << "failed to create a glfw window (is your graphics driver "
                    "updated ?)"
                 << std::endl;
    ...
    GLenum err = glewInit();
#ifdef GLEW_EGL
    //		if (err != GLEW_OK && (!args.offscreen || err !=
    // GLEW_ERROR_NO_GLX_DISPLAY)) // Small hack for glew, this error occurs but
    // does not concern offscreen
    if (err != GLEW_OK &&
        (!args.offscreen))  // Small hack for glew, this error occurs but does
                            // not concern offscreen
#else
    if (err != GLEW_OK)
#endif
        SIBR_ERR << "cannot initialize GLEW (used to load OpenGL function)"
                 << std::endl;
    (void)glGetError();  // I notice that glew might do wrong things during its
                         // init() some drivers complain about it. So I reset
                         // OpenGL's errors to discard this.

    glfwSetWindowUserPointer(_glfwWin.get(), this);
    /// \todo TODO: fix, width and height might be erroneous. SR
    viewport(Viewport(0.f, 0.f, (float)width,
                      (float)height));  /// \todo TODO: bind both
    ...
    // SR: we don't use it by default because you won't get callstack/file/line
    // info.
    if (args.gl_debug) {
    ...
    if (!args.offscreen) {
        ...
        glfwGetMonitorPhysicalSize(glfwGetPrimaryMonitor(), &widthmm,
                                   &heightmm);
        const float defaultDPI = 96.0f;
        sibr::Vector2i dsize = desktopSize();

        _scaling = sibr::clamp(
                std::round(dsize.x() / (widthmm / 25.4f) / defaultDPI), 1.0f,
                2.0f);
    ...
    /** \todo
    TODO: fix issue on some HiDPI screens + interaction with GUI labels
    generation.
...
void Window::setFullscreen(const bool fullscreen) {
    ...
    if (fullscreen) {
        ...
        glfwSetWindowMonitor(_glfwWin.get(), monitor, 0, 0, mode->width,
                             mode->height, mode->refreshRate);
        // There is a bug in glfw (see
        // https://github.com/glfw/glfw/issues/1072). We have to manually re-set
        // the swap interval.
        glfwSwapInterval(_useVSync ? 1 : 0);
    } else {
        glfwSetWindowMonitor(_glfwWin.get(), NULL, _oldPosition[0],
                             _oldPosition[1], _oldSize[0], _oldSize[1], 0);
    }
}
...
void Window::size(int w, int h) {
    ...
    // viewport(Viewport(0.f, 0.f, (float)(s[0]-1), (float)(s[1]-1))); // TODO:
    // bind both
}
...
bool Window::isVsynced(void) const { return _useVSync; }

bool Window::isGUIEnabled(void) const { return _useGUI; }
...
void Window::enableCursor(bool enable) {
    glfwSetInputMode(_glfwWin.get(), GLFW_CURSOR,
                     enable ? GLFW_CURSOR_NORMAL : GLFW_CURSOR_HIDDEN);
}

GLFWwindow* Window::GLFW(void) { return _glfwWin.get(); }
