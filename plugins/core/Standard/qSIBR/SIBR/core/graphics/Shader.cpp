...
GLuint GLShader::compileShader(const char* shader_code, GLuint type) {
    ...
    switch (type) {
        ...
        default:
            std::runtime_error(
                    "Shader types other than vertex/fragment/geometry not "
                    "supported");
    }
    ...
    if (!compiled) {
        ...
        SIBR_WRG << "GLSL " << shader_type.c_str()
                 << " shader compilation failed for program " << m_Name.c_str()
                 << std::endl
                 << infoLog << std::endl;
        delete[] infoLog;
        (void)glGetError();
...
GLShader::GLShader(void)
    : m_Shader(0), m_Name(""), m_Strict(false), m_Active(false) {}

GLShader::~GLShader(void) { terminate(); }

void GLShader::init(GLuint s_handle) { m_Shader = s_handle; }

void GLShader::setStrict(bool s) { m_Strict = s; }

GLuint GLShader::shader(void) const { return m_Shader; }

std::string GLShader::name(void) const { return m_Name; }

bool GLShader::isReady(void) const { return m_Shader != 0; }

bool GLShader::isActive(void) const { return m_Active; }

bool GLShader::isStrict(void) const { return m_Strict; }
...
bool GLShader::init(std::string name,
    ...
    if (!shader_linked) {
        ...
        SIBR_WRG << "GLSL program failed to link " << m_Name.c_str()
                 << std::endl
                 << "Shader linking log:" << std::endl
                 << infoLog << std::endl;
        delete[] infoLog;

        if (exitOnError) SIBR_ERR << "GLSL program failed to link" << std::endl;
    }
...
bool GLShader::reload(std::string vp_code,
                      std::string fp_code,
                      std::string gp_code) {
    {  // Simple way to test if it compiles
        sibr::GLShader tester;
        if (tester.init(m_Name, vp_code, fp_code, gp_code) == false) {
            SIBR_WRG << "Can't reload shader '" << m_Name
                     << "' (see previous log entries)" << std::endl;
            return false;
        }
    }
...
void GLShader::getBinary(std::vector<char>& binary) {
    ...
    if (count <= 0) {
        SIBR_WRG << "GL driver does not support program binary export."
                 << std::endl;
        return;
    }
    int length = 0;
...
GLParameter::GLParameter(void)
    : m_Shader(NULL), m_Handle(-1), m_Name(""), m_Strict(false) {}

bool GLParameter::isInitialized(void) const {
    return (m_Handle != -1 && m_Shader != NULL);
}

GLint GLParameter::handle(void) const { return m_Handle; }
...
void GLParameter::init(sibr::GLShader& shader, std::string name) {
    ...
    if (m_Handle == -1) {
        std::string message = "GLParameter " + m_Name +
                              " does not exist in shader " + shader.name();
        if (m_Strict) {
...
std::string loadFile(const std::string& filename,
                     const GLShader::Define::List& defines) {
    ...
    for (const GLShader::Define& define : defines) {
        ...
        for (std::string& line : lines) {
            ...
            if (formatted.find(tag) == 0) {
                std::size_t pos = line.find(define.nameToSearch) +
                                  define.nameToSearch.size();
                line.insert(pos,
                            std::string(" (") + define.valueToSet + ") //");
                break;
            }
        }
    }
    ...
    std::string out;
    for (std::string& line : lines) out = out + line + '\n';
    return out;
}
