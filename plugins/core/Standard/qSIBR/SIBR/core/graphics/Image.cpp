...
float imageTypeCVRange(int cvDepth) {
    ...
    // };
    static float ranges[] = {imageTypeRange<uint8>(),  imageTypeRange<int8>(),
                             imageTypeRange<uint16>(), imageTypeRange<int16>(),
                             imageTypeRange<int32>(),  imageTypeRange<float>(),
    ...
    for (uint y = 0; y < out.h(); ++y) {
        for (uint x = 0; x < out.w(); ++x) {
            unsigned char const *p =
                    reinterpret_cast<unsigned char const *>(&imgF(x, y).x());
...
#pragma omp parallel for
    for (int y = 0; y < int(out.h()); ++y) {
        for (uint x = 0; x < out.w(); ++x) {
            unsigned char *p =
                    reinterpret_cast<unsigned char *>(&out(x, y).x());
            for (std::size_t i = 0; i != sizeof(float); ++i) {
                p[i] = imgRGBA(x, y)[i];
            }
        }
...
#pragma omp parallel for
    for (int y = 0; y < int(imgF.h()); ++y) {
        for (uint x = 0; x < imgF.w(); ++x) {
            for (int k = 0; k < 3; k++) {
                unsigned char const *p =
                        reinterpret_cast<unsigned char const *>(&imgF(x, y)[k]);
                for (std::size_t i = 0; i != sizeof(float); ++i) {
...
#pragma omp parallel for
    for (int y = 0; y < int(out.h()); ++y) {
        for (uint x = 0; x < out.w(); ++x) {
            for (int k = 0; k < 3; k++) {
                unsigned char *p =
                        reinterpret_cast<unsigned char *>(&out(x, y)[k]);
                for (std::size_t i = 0; i != sizeof(float); ++i) {
...
sibr::ImageRGBA convertNormalMapToSphericalHalf(const sibr::ImageRGB32F &imgF) {
    ...
    for (uint i = 0; i < out.h(); ++i) {
        for (uint j = 0; j < out.w(); ++j) {
            ...
            const double theta =
                    std::atan2((double)imgF(j, i)[1], (double)imgF(j, i)[0]);
            phi_uint = (uint)((phi / M_PI) * (1 << 16));
            theta_uint = (uint)((0.5 * (theta / M_PI + 1.0)) * (1 << 16));

            unsigned char *out_ptr =
                    reinterpret_cast<unsigned char *>(&out(j, i)[0]);
            out_ptr[0] = phi_ptr[0];
            out_ptr[1] = phi_ptr[1];
...
sibr::ImageRGB32F convertSphericalHalfToNormalMap(
        const sibr::ImageRGBA &imgRGBA) {
    uint phi_uint;
    uint theta_uint;
    unsigned char *phi_ptr = reinterpret_cast<unsigned char *>(&phi_uint);
    unsigned char *theta_ptr = reinterpret_cast<unsigned char *>(&theta_uint);
    ...
    for (uint i = 0; i < out.h(); ++i) {
        for (uint j = 0; j < out.w(); ++j) {
            unsigned char const *out_ptr =
                    reinterpret_cast<unsigned char const *>(&imgRGBA(j, i)[0]);
            phi_ptr[0] = out_ptr[0];
            phi_ptr[1] = out_ptr[1];
            ...
            float theta =
                    ((float)phi_uint * 2.0f / (1 << 16) - 1.0f) * float(M_PI);
...
Image<unsigned char, 3> coloredClass(
        const Image<unsigned char, 1>::Ptr imClass) {
    const int color_list[25][3] = {
            {255, 179, 0},  {128, 62, 117}, {166, 189, 215}, {193, 0, 32},
            {0, 128, 255},  {0, 125, 52},   {246, 118, 142}, {0, 83, 138},
            {255, 122, 92}, {0, 255, 0},    {255, 142, 0},   {179, 40, 81},
            {244, 200, 0},  {127, 24, 13},  {147, 170, 0},   {89, 51, 21},
            {241, 58, 19},  {35, 44, 22},   {83, 55, 122},   {255, 0, 128},
            {128, 255, 0},  {128, 0, 255},  {206, 162, 98},  {128, 128, 128},
            {255, 255, 255}};

    std::vector<Vector3ub> colors(256);
    for (int i = 0; i < 255; i++) {
        colors[i] = Vector3ub(color_list[i % 25][0], color_list[i % 25][1],
                              color_list[i % 25][2]);
    }
...
void showFloat(const Image<float, 1> &im,
               bool logScale,
               double min,
               double max) {
    ...
    for (unsigned int j = 0; j < im.h(); j++) {
        for (unsigned int i = 0; i < im.w(); i++) {
            if (logScale)
                imIntensity(i, j).x() = static_cast<unsigned char>(std::max(
                        0.0,
                        std::min((log(im(i, j).x()) - min) * 255 / (max - min),
                                 255.0)));
            else
                imIntensity(i, j).x() = static_cast<unsigned char>(std::max(
                        0.0, std::min((im(i, j).x() - min) * 255 / (max - min),
                                      255.0)));
        }
    }
    cv::Mat colorMat;
...
// Adopted from http://www.64lines.com/jpeg-width-height. Gets the JPEG size
// from the file stream passed to the function, file reference:
...
sibr::Vector2i IImage::get_jpeg_size(std::ifstream &file) {
    ...
    file.get();
    file.get();  // Skip the rest of JPG identifier.

    std::streampos block_length =
            static_cast<std::streampos>(file.get() * 256 + file.get() - 2);
    for (;;) {
        ...
        if (!file.good() || file.get() != 0xFF) break;

        // If the block is not the "Start of frame", skip to the next block
        if (file.get() != 0xC0) {
            block_length = static_cast<std::streampos>(file.get() * 256 +
                                                       file.get() - 2);
            continue;
        }
...
// Adopted from
// http://stackoverflow.com/questions/22638755/image-dimensions-without-loading
// kudos to Lukas (http://stackoverflow.com/users/643315/lukas).
sibr::Vector2i IImage::imageResolution(const std::string &file_path) {
    enum ValidFormats { PNG = 0, BMP, TGA, JPG, JPEG, VALID_COUNT };

    std::string valid_extensions[] = {"png", "bmp", "tga", "jpg", "jpeg"};
    ...
    if (extension_id == VALID_COUNT) return Eigen::Vector2i(-1, -1);

    std::ifstream file(file_path, std::ios::binary);
    if (!file.good()) return Eigen::Vector2i(-1, -1);
