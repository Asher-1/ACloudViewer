...
ByteStream& operator<<(ByteStream& stream, const Camera& c) {
    ...
    float znear = c.znear();
    float zfar = c.zfar();
    return stream << t << fovy << aspect << znear << zfar;
}
...
ByteStream& operator>>(ByteStream& stream, Camera& c) {
    ...
    float znear = 0.f;
    float zfar = 0.f;
    stream >> t >> fovy >> aspect >> znear >> zfar;
    c.transform(t);
    c.fovy(fovy);
    c.aspect(aspect);
    c.znear(znear);
...
bool Camera::frustumTest(const Vector3f& position3d,
                         const Vector2f& pixel2d) const {
    return (pixel2d.cwiseAbs().array() < (1.0f - 1e-5f)).all() &&
           (dir().dot(position3d - position()) > 0);
}
...
Matrix4f Camera::proj(void) const {
    // std::cout << "FOV: " << _fov << "Aspect" << _aspect << "ZNEAR" << _znear
    // << "ZFAR" << _zfar << std::endl << std::flush;
    if (ortho())
...
/*static*/ Camera Camera::interpolate(const Camera& from,
                                      const Camera& to,
                                      float dist01) {
    dist01 = std::max(0.f, std::min(1.f, dist01));
    Transform3f t =
            Transform3f::interpolate(from._transform, to._transform, dist01);
    Camera out = from;
    out._transform = t;
    ...
    if (from.ortho()) {
        out.orthoRight(dist01 * from.orthoRight() +
                       (1.0f - dist01) * to.orthoRight());
        out.orthoTop(dist01 * from.orthoTop() +
                     (1.0f - dist01) * to.orthoTop());
    }
    return out;
}

void Camera::setStereoCam(bool isLeft, float focal, float iod) {
    _matViewProj = sibr::perspectiveStereo(_fov, _aspect, _znear, _zfar, focal,
                                           iod, isLeft) *
                   view();
    _invMatViewProj = _matViewProj.inverse();
}
