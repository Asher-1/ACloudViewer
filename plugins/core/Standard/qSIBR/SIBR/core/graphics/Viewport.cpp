...
void Viewport::bind(uint screenWidth, uint screenHeight) const {
    glViewport((GLint)(left() * screenWidth), (GLint)(top() * screenHeight),
...
void Viewport::bind(void) const {
    ...
    //	&& "Too small viewport detected (Set a parent viewport from a window
    // using Viewport::parent(...) and Window::viewport()");

    glViewport((GLint)(finalLeft()), (GLint)(finalTop()),
               (GLsizei)(finalWidth()), (GLsizei)(finalHeight()));
}

bool Viewport::contains(float x, float y) const {
    return (x > finalLeft() && x < finalRight() && y > finalTop() &&
            y < finalBottom());
}

bool Viewport::contains(int x, int y) const {
    return (x > (int)finalLeft() && x < (int)finalRight() &&
            y > (int)finalTop() && y < (int)finalBottom());
}
...
bool Viewport::isEmpty() const { return width() == 0.0 && height() == 0.0; }
