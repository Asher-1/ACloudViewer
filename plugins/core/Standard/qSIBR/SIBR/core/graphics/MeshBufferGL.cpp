...
static std::vector<TTo> prepareVertexData(const std::vector<TFrom>& fromData,
                                          uint numVertices) {
    ...
    if (fromData.empty()) return std::vector<TTo>();  // empty
...
template <typename TTo, typename TFrom>
static void appendVertexData(std::vector<TTo>& toData,
                             const std::vector<TFrom>& fromData) {
    if (fromData.empty()) return;
...
MeshBufferGL::~MeshBufferGL(void) { free(); }
...
void MeshBufferGL::fetchIndices(const Mesh& mesh, bool adjacency) {
    ...
    if (adjacency) {
        std::unordered_map<std::pair<GLuint, GLuint>, GLuint, hash_pair>
                trianglesByEdges(mesh.triangles().size() * 3);
        indices.reserve(mesh.triangles().size() * 6);

        for (int i = 0; i < mesh.triangles().size(); i++) {
            // store triangles vertices by edges

            trianglesByEdges.insert(
        ...
        for (int i = 0; i < mesh.triangles().size(); i++) {
            ...
            // 1
            indices.push_back(trianglesByEdges[{mesh.triangles()[i][1],
                                                mesh.triangles()[i][0]}]);
            // 2
            indices.push_back(mesh.triangles()[i][1]);
            // 3
            indices.push_back(trianglesByEdges[{mesh.triangles()[i][2],
                                                mesh.triangles()[i][1]}]);
            // 4
            indices.push_back(mesh.triangles()[i][2]);
            // 5
            indices.push_back(trianglesByEdges[{mesh.triangles()[i][0],
                                                mesh.triangles()[i][2]}]);
        }
        ...
        indices.insert(indices.begin(), mesh.triangleArray(),
                       mesh.triangleArray() + mesh.triangles().size() * 3);
...
void MeshBufferGL::build(const Mesh& mesh, bool adjacency) {
    ...
    if (adjacency) fetchIndices(mesh, true);

    uint numVertices = (uint)mesh.vertices().size();
    _vertexCount = numVertices;
    // SIBR_DEBUG(mesh.triangles().size());
    std::vector<GLfloat> vertices =
            prepareVertexData<GLfloat>(mesh.vertices(), numVertices);
    std::vector<GLfloat> colors =
            prepareVertexData<GLfloat>(mesh.colors(), numVertices);
    std::vector<GLfloat> texcoords =
            prepareVertexData<GLfloat>(mesh.texCoords(), numVertices);
    std::vector<GLfloat> normals =
            prepareVertexData<GLfloat>(mesh.normals(), numVertices);
    ...
    vertexData.reserve(getVectorDataSize(vertices) + getVectorDataSize(colors) +
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(uint8) * vertexData.size(),
                 vertexData.data(), GL_STATIC_DRAW);
    CHECK_GL_ERROR;

    glVertexAttribPointer(VertexAttribLocation, 3, GL_FLOAT, GL_FALSE, 0,
                          (uint8_t*)(0));
    glEnableVertexAttribArray(VertexAttribLocation);
    glVertexAttribPointer(ColorAttribLocation, 3, GL_FLOAT, GL_FALSE, 0,
                          (uint8_t*)(0) + getVectorDataSize(vertices));
    glEnableVertexAttribArray(ColorAttribLocation);
    glVertexAttribPointer(TexCoordAttribLocation, 2, GL_FLOAT, GL_FALSE, 0,
                          (uint8_t*)(0) + getVectorDataSize(vertices) +
                                  getVectorDataSize(colors));
    glEnableVertexAttribArray(TexCoordAttribLocation);
    glVertexAttribPointer(NormalAttribLocation, 3, GL_FLOAT, GL_FALSE, 0,
                          (uint8_t*)(0) + getVectorDataSize(vertices) +
                                  getVectorDataSize(colors) +
                                  getVectorDataSize(texcoords));
    ...
    /// \todo TODO:
    /// We could ignore attrib that are empty (where mesh.colors().empty() ==
    /// true, don't do anything with this). This could improve a bit
    /// performances.
...
void MeshBufferGL::draw(bool adjacency) const {
    ...
    if (adjacency) {
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _bufferIds[BUFADJINDEX]);
        glDrawElements(GL_TRIANGLES_ADJACENCY, _adjacentIndexCount,
...
void MeshBufferGL::draw(unsigned int begin,
                        unsigned int end,
    ...
    if (adjacency) {
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _bufferIds[BUFADJINDEX]);
        glDrawElements(GL_TRIANGLES_ADJACENCY, count, GL_UNSIGNED_INT,
                       (void*)(sizeof(GLuint) * begin));
    } else {
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _bufferIds[BUFINDEX]);
...
void MeshBufferGL::bind(void) const { glBindVertexArray(_vaoId); }

void MeshBufferGL::unbind(void) const { glBindVertexArray(0); }
