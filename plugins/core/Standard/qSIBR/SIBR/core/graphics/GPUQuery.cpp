...
GPUQuery::GPUQuery(GLenum type, size_t count) : _count(count), _type(type) {
    if (count < 2) {
        SIBR_WRG << "Using a buffer of size >= 2 is recommended to avoid "
...
uint64 GPUQuery::value() {
    ...
    // We want the ID of the previous frame, taking into account that we have
    // incremented the counter once more when ending it. So minus 1. Except if
    // you have only one query, in which case we query this one, but it will
    // stall the GPU.
