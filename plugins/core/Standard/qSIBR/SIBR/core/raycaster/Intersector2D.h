...
struct quad {
    ...
    */
    void save(std::string path) {
        sibr::ByteStream bs;
        bs << q1.x() << q1.y() << q1.z() << q2.x() << q2.y() << q2.z() << q3.x()
           << q3.y() << q3.z() << q4.x() << q4.y() << q4.z();
        bs.saveToFile(path);
    }
    ...
    */
    void load(std::string path) {
        sibr::ByteStream bs;
        bs.load(path);
        bs >> q1.x() >> q1.y() >> q1.z() >> q2.x() >> q2.y() >> q2.z() >>
...
/** This class provides utilities to compute point/line/triangle/quad
intersections. \ingroup sibr_raycaster
*/
...
public:
    ...
    /**
    Having defined a straight line in the 2D plane, this method can be used to
    know in which half-space (defined by the line) a point lies. \param p1 the
    2D point to locate wrt to the line. \param p2 a 2D point on the line. \param
    p3 another 2D point on the line. \return a signed value indicating on which
    side of the line the point is.
    */
    ...
    */
    static bool PointInTriangle(sibr::Vector2f pt,
                                sibr::Vector2f v1,
                                sibr::Vector2f v2,
                                sibr::Vector2f v3);
    ...
    */
    static bool LineLineIntersect(sibr::Vector2f a,
                                  sibr::Vector2f b,
                                  sibr::Vector2f c,
                                  sibr::Vector2f d);
    ...
    */
    static bool TriTriIntersect(sibr::Vector2f t0_0,
                                sibr::Vector2f t0_1,
                                sibr::Vector2f t0_2,
                                sibr::Vector2f t1_0,
                                sibr::Vector2f t1_1,
    ...
    */
    static bool QuadQuadIntersect(sibr::Vector2f q0_0,
                                  sibr::Vector2f q0_1,
                                  sibr::Vector2f q0_2,
                                  sibr::Vector2f q0_3,
                                  sibr::Vector2f q1_0,
    ...
    \param cams an array of cameras against which frusta the intersections tests
    should be performed. \return a double-array of booleans denoting, for each
    camera, for each quad, if the quad intersects the frustum volume.
    */
    static std::vector<std::vector<bool>> frustrumQuadsIntersect(
