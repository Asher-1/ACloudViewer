...
/*static*/ void CameraRaycaster::computePixelDerivatives(
    ...
    sibr::Vector2f screenWorldSize;
    {
        ...
        screenWorldSize =
                sibr::Vector2f(heightWorldSize * aspect, heightWorldSize);
    }
    ...
    // upLeftOffset = upLeftOffset + dx/2.f + dy/2.f;	// Used to start from
    // the center of a pixel
    upLeftOffset += cam.position();
}

void CameraRaycaster::computeClippingPlanes(
    ...
    SIBR_LOG << " [CameraRaycaster] computeAutoClippingPlanes() : "
             << std::flush;
...
#pragma omp parallel for
    for (int cam_id = 0; cam_id < (int)cams.size(); ++cam_id) {
        ...
        sibr::CameraRaycaster::computePixelDerivatives(cam, dx, dy,
                                                       upLeftOffset);
        ...
        for (int i = 0; i < (int)cam.h(); i += deltaPix) {
            for (int j = 0; j < (int)cam.w(); j += deltaPix) {
                sibr::Vector3f worldPos = ((float)j + 0.5f) * dx +
                                          ((float)i + 0.5f) * dy + upLeftOffset;
                sibr::Vector3f dir = (worldPos - cam.position()).normalized();

                sibr::RayHit hit =
                        raycaster.intersect(sibr::Ray(cam.position(), dir));
...
sibr::Vector3f CameraRaycaster::computeRayDir(const sibr::InputCamera& cam,
                                              const sibr::Vector2f& pixel) {
    sibr::Vector3f dx, dy, upLeftOffset;
    CameraRaycaster::computePixelDerivatives(cam, dx, dy, upLeftOffset);

    sibr::Vector3f worldPos = pixel.x() * dx + pixel.y() * dy +
                              upLeftOffset;  // at dist 1 from cam center
    return (worldPos - cam.position()).normalized();
}

bool CameraRaycaster::init(void) { return _raycaster.init(); }

void CameraRaycaster::addMesh(const sibr::Mesh& mesh) {
    _raycaster.addMesh(mesh);
}

void CameraRaycaster::castForEachPixel(const sibr::InputCamera& cam,
                                       ICameraRaycasterProcessor* processors[],
                                       uint nbProcessors,
                                       const std::string& optLogMsg) {
    // SIBR_PROFILESCOPE;
    ...
    for (uint py = 0; py < cam.h(); ++py) {
        for (uint px = 0; px < cam.w(); ++px) {
            // progress.walk();
            sibr::Vector3f worldPos =
                    (float)px * dx + (float)py * dy + upLeftOffset;
            // Cast a ray
...
RaycastingCamera::RaycastingCamera(const sibr::InputCamera& cam)
    : sibr::InputCamera(cam) {
    CameraRaycaster::computePixelDerivatives(*this, dx, dy,
                                             upLeftOffsetMinusPos);
    upLeftOffsetMinusPos -= position();

    std::vector<sibr::Vector2f> corners = {{-1, -1}, {-1, 1}, {1, 1}, {1, -1}};
    ...
    frustum_planes = {
            // HPlane::Through(pts_near[0], pts_near[3], pts_near[2]), //
            // near_plane,
            HPlane::Through(pts_far[0], pts_far[2], pts_far[3]),   // far_plane
            HPlane::Through(pts_near[2], pts_far[2], pts_far[1]),  // top_plane,
            HPlane::Through(pts_near[3], pts_near[0],
                            pts_far[3]),  // bottom_plane,
            HPlane::Through(pts_far[0], pts_near[0], pts_far[1]),  // left_plane
            HPlane::Through(pts_near[3], pts_far[3],
                            pts_far[2])  // right_plane;
    };

    // sibr::Vector3f pt = unproject({ 0, 0, 0 });
    // std::cout << " debug planes : ";
    // for (uint i = 0; i < frustum_planes.size(); ++i) {
    //	std::cout << sibr::Vector4f(pt[0], pt[1], pt[2],
    // 1).dot(frustum_planes[i].coeffs()) << " ";
    // }
    // std::cout << std::endl;
}

sibr::Vector3f RaycastingCamera::rayDirNotNormalized(
        const sibr::Vector2f& pixel) const {
...
bool RaycastingCamera::isInsideFrustum(const sibr::Vector3f& pt,
                                       float eps) const {
    for (uint i = 0; i < frustum_planes.size(); ++i) {
        if (sibr::Vector4f(pt[0], pt[1], pt[2], 1)
                    .dot(frustum_planes[i].coeffs()) < -eps) {
            return false;
        }
    }
    return true;
...
sibr::Vector2f RaycastingCamera::projectImg_outside_frustum_correction(
        const Vector3f& pt3d) const {
    ...
    return 0.5f * (pos2dGL.xy() + sibr::Vector2f(1, 1))
