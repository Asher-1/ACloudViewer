...
namespace sibr {
/*static*/ SIBR_RAYCASTER_EXPORT const Raycaster::geomId
        Raycaster::InvalidGeomId = RTC_INVALID_GEOMETRY_ID;
/*static*/ bool Raycaster::g_initRegisterFlag = false;
/*static*/ Raycaster::RTCDevicePtr Raycaster::g_device = nullptr;

/*static*/ void Raycaster::rtcErrorCallback(void* userPtr,
                                            RTCError code,
                                            const char* msg) {
    std::string err;
    ...
    switch (code) {
        case RTC_ERROR_UNKNOWN:
            err = std::string("RTC_ERROR_UNKNOWN");
            break;
        case RTC_ERROR_INVALID_ARGUMENT:
            err = std::string("RTC_ERROR_INVALID_ARGUMENT");
            break;
        case RTC_ERROR_INVALID_OPERATION:
            err = std::string("RTC_ERROR_INVALID_OPERATION");
            break;
        case RTC_ERROR_OUT_OF_MEMORY:
            err = std::string("RTC_ERROR_OUT_OF_MEMORY");
            break;
        case RTC_ERROR_UNSUPPORTED_CPU:
            err = std::string("RTC_ERROR_UNSUPPORTED_CPU");
            break;
        case RTC_ERROR_CANCELLED:
            err = std::string("RTC_ERROR_CANCELLED");
            break;
        default:
            err = std::string("invalid error code");
            break;
    }
...
bool Raycaster::init(RTCSceneFlags sceneType) {
    if (!g_device) {
        ...
        if (g_device == nullptr) {
            SIBR_LOG << "Cannot create an embree device : "
                     << rtcGetDeviceError(*g_device.get()) << std::endl;
        }

        rtcSetDeviceErrorFunction(*g_device.get(), &Raycaster::rtcErrorCallback,
                                  nullptr);  // Set callback error function
    ...
    _scene = std::make_shared<RTCScene>(  // define a new scene
                                          /// \todo create a new static scene
                                          /// optimized for primary rays (TODO:
                                          /// test perf with RTC_SCENE_ROBUST)
            rtcNewScene(*g_device.get()));  // set a custom deleter

    if (_scene == nullptr)
        SIBR_LOG << "Cannot create an embree scene" << std::endl;
    else {
        // SIBR_LOG << "Embree device and scene created" << std::endl;
        // SIBR_LOG << "Warning Backface culling state : "<<
        // rtcGetDeviceProperty(*g_device,
        // RTC_DEVICE_PROPERTY_BACKFACE_CULLING_ENABLED) << std::endl;
        return true;  // Success
    }
    return false;  // Fail
}
...
Raycaster::geomId Raycaster::addGenericMesh(const sibr::Mesh& mesh,
                                            RTCBuildQuality type) {
    if (init() == false) return Raycaster::InvalidGeomId;

    const sibr::Mesh::Vertices& vertices = mesh.vertices();
    const sibr::Mesh::Triangles& triangles = mesh.triangles();

    RTCGeometry geom_0 = rtcNewGeometry(
            *g_device.get(),
            RTC_GEOMETRY_TYPE_TRIANGLE);  // EMBREE_FIXME: check if geometry
                                          // gets properly committed
    ...
    {  // Fill vertices of the geometry
        Vertex* vert = (Vertex*)rtcSetNewGeometryBuffer(
                geom_0, RTC_BUFFER_TYPE_VERTEX, 0, RTC_FORMAT_FLOAT3,
                4 * sizeof(float), vertices.size());
    ...
    {  // Fill triangle indices of the geometry
        Triangle* tri = (Triangle*)rtcSetNewGeometryBuffer(
                geom_0, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3,
                3 * sizeof(int), triangles.size());
...
// are always stored in mesh.vertices -- we only xform the vertices in the
// embree buffer
void Raycaster::xformRtcMeshOnly(sibr::Mesh& mesh,
                                 geomId mesh_id,
                                 sibr::Matrix4f& mat,
                                 sibr::Vector3f& centerPt,
                                 float& maxlen) {
    struct Vertex {
        float x, y, z, a;
    };
    Vertex* vert = (Vertex*)rtcGetGeometryBufferData(
            rtcGetGeometry(*_scene.get(), mesh_id), RTC_BUFFER_TYPE_VERTEX,
            0) /* EMBREE_FIXME: check if this should be rtcSetNewGeometryBuffer
                */
            ;
    ...
    for (uint i = 0; i < mesh.vertices().size(); ++i) {
        ...
        averagePt += v;
        float d = sibr::Vector3f(
                          sibr::Vector4f(averagePt / (float)((i == 0) ? 1 : i))
                                  .xyz() -
                          v.xyz())
                          .norm();
...
bool Raycaster::hitSomething(const Ray& inray, float minDist) {
    ...
    if (init() == false)
        ...
        // embree4: rtcOccluded1 signature changed, context parameter is now
        // optional (NULL)
...
std::array<bool, 8> Raycaster::hitSomething8(const std::array<Ray, 8>& inray,
                                             float minDist) {
    assert(minDist >= 0.f);
    ...
    if (init() == false)
        ...
        // embree4: rtcOccluded8 signature changed, context parameter is now
        // optional (NULL)
...
RayHit Raycaster::intersect(const Ray& inray, float minDist) {
    ...
    if (init() == false)
        ...
        // embree4: rtcIntersect1 signature changed, context parameter is now
        // optional (NULL)
    ...
    sibr::Vector3f normal =
            sibr::Vector3f(rh.hit.Ng_x, rh.hit.Ng_y, rh.hit.Ng_z);

    // Return the result.
    return RayHit(inray, rh.ray.tfar, coord, normal, prim);
}

std::array<RayHit, 8> Raycaster::intersect8(const std::array<Ray, 8>& inray,
                                            const std::vector<int>& valid8,
                                            float minDist) {
    assert(minDist >= 0.f);
    ...
    if (init() == false)
        ...
        // embree4: rtcIntersect8 signature changed, context parameter is now
        // optional (NULL)
    ...
    for (int r = 0; r < 8; r++) {
        if (valid8[r])
            res[r] = {inray[r], rh.ray.tfar[r],
                      RayHit::BCCoord{rh.hit.u[r], rh.hit.v[r]},
                      sibr::Vector3f(rh.hit.Ng_x[r], rh.hit.Ng_y[r],
                                     rh.hit.Ng_z[r]),
                      RayHit::Primitive{
#ifdef SIBR_OS_WINDOWS
                              (uint)rh.hit.primID[r], (uint)rh.hit.geomID[r],
                              (uint)rh.hit.instID[r]
#else
                              // Considering RTC_MAX_INSTANCE_LEVEL_COUNT to be
                              // 1 (Single-level instancing); see
                              // https://www.embree.org/api.html#rtchit
                              (uint)rh.hit.primID[r], (uint)rh.hit.geomID[r],
                              (uint)rh.hit.instID[0][r]
#endif
...
void Raycaster::clearGeometry() { _scene.reset(); }

sibr::Vector3f Raycaster::smoothNormal(const sibr::Mesh& mesh,
                                       const RayHit& hit) {
    if (!mesh.hasNormals()) {
        SIBR_ERR << " cannot compute smoothed normals if the mesh does not "
                    "have normals "
                 << std::endl;
    }
    const sibr::Mesh::Normals& normals = mesh.normals();
    ...
    return (wcoord * normals[tri[0]] + ucoord * normals[tri[1]] +
            vcoord * normals[tri[2]])
            .normalized();
}

sibr::Vector3f Raycaster::smoothColor(const sibr::Mesh& mesh,
                                      const RayHit& hit) {
    if (!mesh.hasColors()) {
        SIBR_ERR << " cannot compute smoothed color if the mesh does not have "
                    "colors "
                 << std::endl;
    }
    ...
    return wcoord * colors[tri[0]] + ucoord * colors[tri[1]] +
           vcoord * colors[tri[2]];
}

sibr::Vector2f Raycaster::smoothUV(const sibr::Mesh& mesh, const RayHit& hit) {
    if (!mesh.hasTexCoords()) {
        SIBR_ERR << " cannot compute UV if the mesh does not have texcoords "
                 << std::endl;
    }
