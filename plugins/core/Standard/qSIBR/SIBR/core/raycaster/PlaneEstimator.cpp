...
PlaneEstimator::PlaneEstimator(const std::vector<sibr::Vector3f>& vertices,
                               bool excludeBB) {
    ...
    if (vertices.size() > 200000) {
        std::cout
                << "Found more than 200000 points reducing point cloud size ..."
                << std::endl;
        ...
        if (excludeBB)
            std::cout << bboxReject
                      << " points where rejected becaused considered on the "
                         "bounding box"
                      << std::endl;
    } else {
        _Points = vertices;
...
void PlaneEstimator::computePlanes(const int numPlane,
                                   const float delta,
                                   const int numTry) {
    _planeComputed = true;  // we know that the planes were computed

    std::cout << "Original number of points " << _remainPoints3D.rows()
              << std::endl;
    for (int i = 0; i < numPlane; i++) {
        if (_remainPoints3D.rows() < _numPoints3D * 5 / 100) {
            std::cout << "Not enough points remaining, stop searching. " << i
                      << " planes found." << std::endl;
            break;
        }

        sibr::Vector3f color(((float)rand() / (RAND_MAX)),
                             ((float)rand() / (RAND_MAX)),
                             ((float)rand() / (RAND_MAX)));
        ...
        int vote = -1;
        sibr::Vector4f plane =
                estimatePlane(delta, numTry, mask, vote, covMean);

        if (vote < _numPoints3D * 2 / 100 && i >= 12) {
            std::cout
                    << "Not enough points in candidate plane, stop searching. "
                    << i << " planes found." << std::endl;
            break;
        }
        //
        ...
        std::cout << "vote :" << vote << " notSel " << notSel
                  << " supposed total " << _remainPoints3D.rows() << std::endl;
        _remainPoints3D = remainPoints3DTemp;
        _remainNormals3D = remainNormals3DTemp;
        //_remainImPos=remainImPosTemp;
        std::cout << "Remaining number of points " << _remainPoints3D.rows()
                  << std::endl;
...
sibr::Vector4f PlaneEstimator::estimatePlane(
        const float delta,
        const int numTry,
        Eigen::MatrixXi& bestMask,
        int& bestVote,
        std::pair<Eigen::MatrixXf, sibr::Vector3f>& bestCovMean) {
...
#pragma omp critical
    ...
    std::cout << "Best vote " << bestVote << " Best plane " << bestPlane
              << std::endl;
    /*
    ...
    bestCovMean.first = (dataCentered*dataCentered.adjoint()) /
    float(dataCentered.cols() - 1); bestCovMean.second = center;

    std::cout << "Cov Matrix : " << bestCovMean.first << " Mean : " <<
    bestCovMean.second << std::endl; std::cout << "Cov Determinant : " <<
    bestCovMean.first.determinant() << std::endl;

    Eigen::JacobiSVD<Eigen::MatrixXf> svd(dataCentered, Eigen::ComputeFullU |
    Eigen::ComputeThinV);

    std::cout << "old normal" << " " << bestPlane.xyz();
    //the normal to the fitting plane is the eigenvector associated to the
    smallest eigenvalue (i.e. the direction in which the variance of all points
    is the smallest) sibr::Vector3f normal = svd.matrixU().col(2);
    normal.normalize();
    ...
    // normal coherency
    // Eigen::ArrayXf dotWithOriNormal=(_remainNormals3D *
    // bestPlane.xyz()).array().cwiseAbs(); bestMaskNormals =
    // (bestMask.array().cast<float>()*dotWithOriNormal);

    std::cout << "Vote refined " << bestVote << " Plane refined " << bestPlane
              << /*" Normals coherency " << bestMaskNormals.sum()/bestVote  <<*/
            std::endl;
...
std::pair<int, float> PlaneEstimator::votePlane(const sibr::Vector4f plane,
                                                const float delta,
                                                Eigen::MatrixXi& mask,
                                                float normalDot) {
    ...
    // std::cout << "size " << _points3D.size() << " " << normal.size() << " d "
    // << d << std::endl;
    ...
    distances =
            (distances - d * Eigen::ArrayXf::Ones(distances.rows())).cwiseAbs();
    ...
    mask = (distances < delta &&
            (dotWithOriNormal > normalDot || dotWithOriNormal == 0))
                   .cast<int>();

    Eigen::ArrayXf voteW =
            (distances + 0.1f * delta * Eigen::ArrayXf::Ones(distances.rows()));
...
sibr::Vector4f PlaneEstimator::estimateGroundPlane(sibr::Vector3f roughUp) {
    if (_planeComputed) {
        ...
        std::cout << "Error : Plane not computed, you should call "
                     "computePlanes first"
                  << std::endl;
        SIBR_ERR;
...
sibr::Vector3f PlaneEstimator::estimateMedianVec(
        const std::vector<sibr::Vector3f>& ups) {
...
sibr::Mesh PlaneEstimator::getMeshPlane(sibr::Vector4f plane,
                                        sibr::Vector3f center,
                                        float radius) {
    sibr::Mesh planeMesh;

    sibr::Vector3f projCenter =
            center -
            (center - plane.w() * plane.xyz()).dot(plane.xyz()) * plane.xyz();
    ...
    for (int i = 0; i < numP; i++) {
        vert.push_back(projCenter + radius * cos(2 * M_PI * i / numP) * u +
                       radius * sin(2 * M_PI * i / numP) * v);
