...
BasicIBRScene::BasicIBRScene(const BasicIBRAppArgs& myArgs,
                             bool noRTs,
                             bool noMesh) {
    BasicIBRScene();
    // parse metadata file
    ...
    std::cout << "Number of input Images to read: " << _data->imgInfos().size()
              << std::endl;

    if (_data->imgInfos().size() != _data->numCameras())
        SIBR_ERR << "List Image file size do not match number of input cameras "
                    "in Bundle file!"
                 << std::endl;
...
void BasicIBRScene::createFromCustomData(const IParseData::Ptr& data,
                                         const uint width,
...
void BasicIBRScene::createFromData(const uint width) {
    ...
    if (_currentOpts.cameras) {
        ...
        std::cout << "Number of Cameras set up: "
                  << _cams->inputCameras().size() << std::endl;
    }
    ...
    if (_currentOpts.images) {
        ...
        std::cout << "Number of Images loaded: " << _imgs->inputImages().size()
                  << std::endl;

        if (width == 0) {  // default
            if (_imgs->inputImages()[0]->w() > 1920) {
                SIBR_LOG << "Limiting width to 1920 for performance; use "
                            "--texture-width to override"
                         << std::endl;
                mwidth = 1920;
            }
        }
    ...
    if (_currentOpts.mesh) {
        ...
        float eps = 0.1f;
        if (inCams.size() > 0 &&
            (abs(inCams[0]->znear() - 0.1) < eps ||
             abs(inCams[0]->zfar() - 1000.0) < eps ||
             abs(inCams[0]->zfar() - 100.0) < eps) &&
            _proxies->proxy().triangles().size() > 0) {
            std::vector<sibr::Vector2f> nearsFars;
            CameraRaycaster::computeClippingPlanes(_proxies->proxy(), inCams,
                                                   nearsFars);
        ...
        // Assumes that the texture is stored next to the mesh in the same
        // directory This information comes from Assimp and the mtl file if
        // available
        if ((textureImageFileName =
                     _proxies->proxy().getTextureImageFileName()) != "") {
            texturePath = sibr::parentDirectory(_data->meshPath()) + "/" +
                          textureImageFileName;
            // check if full path given
            if (!sibr::fileExists(texturePath) &&
                sibr::fileExists(textureImageFileName))
