...
bool ParseData::parseBundlerFile(const std::string& bundler_file_path) {
    ...
    if (!bundle_file.is_open()) {
        SIBR_ERR << "Bundler file does not exist at " + bundler_file_path
                 << std::endl;
    }
    ...
    for (int i = 0; i < _numCameras; i++) {
        ...
        _camInfos[i] = InputCamera::Ptr(new InputCamera(
                infos.camId, infos.width, infos.height, m, _activeImages[i]));
...
bool ParseData::parseSceneMetadata(const std::string& scene_metadata_path) {
...

    {
        if (line.compare("[list_images]") == 0) {
            safeGetline(scene_metadata,
                        line);  // ignore template specification line
            ...
            while (safeGetline(scene_metadata, line)) {
                ...
                if (splitS.size() > 1) {
                    ...
                    // infos.filename.erase(infos.filename.find_last_of("."),
                    // std::string::npos);
            ...
            for (int i = 0; i < _imgInfos.size(); i++) _activeImages[i] = false;
            ...
            while (safeGetline(scene_metadata, line)) {
                ...
                if (splitS.size() >= 1) {
                    for (auto& s : splitS)
                        if (!s.empty()) _activeImages[stoi(s)] = true;
                    splitS.clear();
                    break;
                } else
                    break;
            ...
            for (int i = 0; i < _imgInfos.size(); i++) _activeImages[i] = true;

            while (safeGetline(scene_metadata, line)) {
                split(splitS, line, is_any_of(" "));
                if (splitS.size() >= 1) {
                    for (auto& s : splitS)
                        if (!s.empty()) _activeImages[stoi(s)] = false;
                    splitS.clear();
                    break;
                } else
                    break;
...
void ParseData::getParsedBundlerData(
        const std::string& dataset_path,
        const std::string& customPath,
        const std::string& scene_metadata_filename) {
    ...
    if (!parseSceneMetadata(_basePathName + "/" + scene_metadata_filename)) {
        SIBR_ERR << "Scene Metadata file does not exist at /" + _basePathName +
                            "/."
                 << std::endl;
    }

    if (!parseBundlerFile(_basePathName + "/cameras/bundle.out")) {
        SIBR_ERR << "Bundle file does not exist at /" + _basePathName +
                            "/cameras/."
                 << std::endl;
    }
    ...
    if (_meshPath.empty()) {
        ...
        _meshPath = (sibr::fileExists(_meshPath))
                            ? _meshPath
                            : _basePathName + "/meshes/recon.ply";
    }
}

void ParseData::getParsedMeshroomData(const std::string& dataset_path,
                                      const std::string& customPath) {
    _basePathName = dataset_path;

    std::string meshRoomCachePath = sibr::listSubdirectories(
            _basePathName + "/StructureFromMotion/")[0];

    _camInfos = sibr::InputCamera::loadMeshroom(
            _basePathName + "/StructureFromMotion/" + meshRoomCachePath);

    if (_camInfos.empty()) {
        SIBR_ERR << "Could not load Meshroom sfm file at /" + _basePathName +
                            "/StructureFromMotion/"
                 << meshRoomCachePath << std::endl;
    }

    _imgPath =
            _basePathName + "/PrepareDenseScene/" +
            sibr::listSubdirectories(_basePathName + "/PrepareDenseScene/")[0];

    populateFromCamInfos();

    _meshPath = _basePathName + "/Texturing/" +
                sibr::listSubdirectories(_basePathName + "/Texturing/")[0] +
                "/texturedMesh.obj";
}

void ParseData::getParsedBlenderData(const std::string& dataset_path) {
    _camInfos =
            InputCamera::loadTransform(dataset_path + "/transforms_test.json",
                                       800, 800, "png", 0.01f, 1000.0f);
    auto testInfos = InputCamera::loadTransform(
            dataset_path + "/transforms_train.json", 800, 800, "png", 0.01f,
            1000.0f, _camInfos.size());
    _camInfos.insert(_camInfos.end(), testInfos.begin(), testInfos.end());

    _basePathName = dataset_path;

    if (_camInfos.empty()) {
        SIBR_ERR << "Colmap camera calibration file does not exist at /" +
                            _basePathName + "/sparse/."
                 << std::endl;
    }
...
void ParseData::getParsedColmap2Data(const std::string& dataset_path,
                                     const int fovXfovY_flag,
                                     const bool capreal_flag) {
    _basePathName = dataset_path + "/sparse/0/";

    _camInfos = sibr::InputCamera::loadColmapBin(_basePathName, 0.01f, 1000.0f,
                                                 fovXfovY_flag);

    if (_camInfos.empty()) {
        _camInfos = sibr::InputCamera::loadColmap(_basePathName, 0.01f, 1000.0f,
                                                  fovXfovY_flag);
    }
...
void colmapSave(const std::string& filename,
                const std::vector<InputCamera::Ptr>& xformPath,
                float scale) {
    ...
    std::string colmapPathCams =
            parentDirectory(filename) + std::string("/cameras.txt");

    std::cerr << std::endl;
    std::cerr << std::endl;
    std::cerr << "Writing colmap path to " << parentDirectory(filename)
              << std::endl;
    ...
    outputColmapPathCams << "# Camera list with one line of data per camera:"
                         << std::endl;
    outputColmapPathCams << "#   CAMERA_ID, MODEL, WIDTH, HEIGHT, PARAMS[]"
                         << std::endl;
    outputColmapPathCams << "# Number of cameras: " << xformPath.size()
                         << std::endl;

    SIBR_WRG << "No focal x given making it equal to focaly * aspect ratio; "
                "use result at own risk. Should have a colmap dataset as input"
             << std::endl;

    for (int i = 0; i < xformPath.size(); i++) {
        float focalx = xformPath[i]->focal() *
                       xformPath[i]->aspect();  // use aspect ratio
        outputColmapPathCams
                << i + 1 << " PINHOLE " << xformPath[i]->w() * scale << " "
                << xformPath[i]->h() * scale << " "
                << xformPath[i]->focal() * scale << " " << focalx * scale << " "
                << xformPath[i]->w() * scale * 0.5 << " "
                << xformPath[i]->h() * scale * 0.5 << std::endl;
    }

    outputColmapPath << "# Image list with two lines of data per image:"
                     << std::endl;
    outputColmapPath
...
void ParseData::getParsedChunkedData(const std::string& dataset_path) {
    ...
    for (int i = 0; i < camdirs.size(); i++) {
        auto cam = std::make_shared<InputCamera>(0, 0, 0, 0, 0, 0,
                                                 _camInfos.size());
        ...
        if (mat(2, 2) > 0.9 || cam->position().x() < (x) * 100.9 ||
            cam->position().x() > (x + 1) * 100.9 ||
            cam->position().y() < y * 100.9 ||
            cam->position().y() > (y + 1) * 100.9)
            continue;
...
void ParseData::getParsedColmapData(const std::string& dataset_path,
    ...
    _camInfos = sibr::InputCamera::loadColmap(_basePathName + "/sparse", 0.01f,
                                              1000.0f, fovXfovY_flag);
    ...
    if (capreal_flag) {
        ...
        _meshPath = (sibr::fileExists(_meshPath))
                            ? _meshPath
                            : dataset_path + "/capreal/mesh.ply";
    } else {
        _meshPath = dataset_path + "/colmap/stereo/meshed-delaunay.ply";
    }
}

void ParseData::getParsedNVMData(const std::string& dataset_path,
                                 const std::string& customPath,
                                 const std::string& nvm_path) {
    _basePathName = dataset_path + customPath + nvm_path;

    _camInfos = sibr::InputCamera::loadNVM(_basePathName + "/scene.nvm", 0.001f,
                                           1000.0f);
    if (_camInfos.empty()) {
        SIBR_ERR << "Error reading NVM dataset at /" + _basePathName
                 << std::endl;
    }
...
void ParseData::getParsedData(const BasicIBRAppArgs& myArgs,
    ...
    std::string bundler =
            myArgs.dataset_path.get() + customPath + "/cameras/bundle.out";
    std::string colmap =
            myArgs.dataset_path.get() + "/colmap/stereo/sparse/images.txt";
    ...
    std::string nvmscene =
            myArgs.dataset_path.get() + customPath + "/nvm/scene.nvm";
    std::string meshroom =
            myArgs.dataset_path.get() + "/../../StructureFromMotion/";
    std::string meshroom_sibr =
            myArgs.dataset_path.get() + "/StructureFromMotion/";
    ...
    if (datasetTypeStr == "sibr") {
        if (!sibr::fileExists(bundler))
            SIBR_ERR << "Cannot use dataset_type " + myArgs.dataset_type.get() +
                                " at /" + myArgs.dataset_path.get() + "."
                     << std::endl
                     << "Reason : bundler folder (" << bundler
        ...
        if (!(sibr::fileExists(caprealobj) || sibr::fileExists(caprealply)))
            SIBR_ERR << "Cannot use dataset_type " + myArgs.dataset_type.get() +
                                " at /" + myArgs.dataset_path.get() + "."
                     << std::endl
                     << "Reason : capreal mesh (" << caprealobj << ", "
                     << caprealply << ") does not exist" << std::endl;
        ...
        if (!sibr::fileExists(nvmscene))
            SIBR_ERR << "Cannot use dataset_type " + myArgs.dataset_type.get() +
                                " at /" + myArgs.dataset_path.get() + "."
                     << std::endl
                     << "Reason : nvmscene folder (" << nvmscene
                     << ") does not exist" << std::endl;

        _datasetType = Type::NVM;
    } else if (datasetTypeStr == "meshroom") {
        if (!(sibr::directoryExists(meshroom) ||
              sibr::directoryExists(meshroom_sibr)))
            SIBR_ERR << "Cannot use dataset_type " + myArgs.dataset_type.get() +
                                " at /" + myArgs.dataset_path.get() + "."
                     << std::endl
                     << "Reason : meshroom folder (" << meshroom << ", "
                     << meshroom_sibr << ") does not exist" << std::endl;
        ...
        if (!sibr::fileExists(blender))
            SIBR_ERR << "Cannot use dataset_type " + myArgs.dataset_type.get() +
                                " at /" + myArgs.dataset_path.get() + "."
                     << std::endl
                     << "Reason : blender transform (" << blender
                     << ") does not exist" << std::endl;
        ...
        if (!sibr::fileExists(gaussian))
            SIBR_ERR << "Cannot use dataset_type " + myArgs.dataset_type.get() +
                                " at /" + myArgs.dataset_path.get() + "."
                     << std::endl
                     << "Reason : Gaussian transform (" << blender
                     << ") does not exist" << std::endl;
        ...
        if (sibr::fileExists(bundler)) {
            ...
            _datasetType = Type::GAUSSIAN;
        } else if (sibr::fileExists(colmap) && (sibr::fileExists(caprealobj) ||
                                                sibr::fileExists(caprealply))) {
            _datasetType = Type::COLMAP_CAPREAL;
            ...
            SIBR_ERR << "Cannot determine type of dataset at /" +
                                myArgs.dataset_path.get() + customPath
                     << std::endl;
        }
    }
    ...
    switch (_datasetType) {
        case Type::GAUSSIAN:
            getParsedGaussianData(myArgs.dataset_path);
            break;
        case Type::BLENDER:
            getParsedBlenderData(myArgs.dataset_path);
            break;
        case Type::SIBR:
            getParsedBundlerData(myArgs.dataset_path, customPath,
                                 myArgs.scene_metadata_filename);
            break;
        case Type::COLMAP_CAPREAL:
            getParsedColmapData(myArgs.dataset_path,
                                myArgs.colmap_fovXfovY_flag, true);
            break;
        case Type::COLMAP:
            getParsedColmapData(myArgs.dataset_path,
                                myArgs.colmap_fovXfovY_flag, false);
            break;
        case Type::COLMAP2:
            getParsedColmap2Data(myArgs.dataset_path,
                                 myArgs.colmap_fovXfovY_flag, false);
            break;
        case Type::CHUNKED:
            getParsedChunkedData(myArgs.dataset_path);
            break;
        case Type::NVM:
    ...
    for (const auto& cam : _camInfos) minCamID = std::min(minCamID, cam->id());
    for (auto& cam : _camInfos) cam->_id -= minCamID;
    for (auto& img : _imgInfos) img.camId -= minCamID;
    ...
    for (int c = 0; c < _numCameras; c++) {
        maxId = (maxId > int(_imgInfos[c].camId)) ? maxId
                                                  : int(_imgInfos[c].camId);
