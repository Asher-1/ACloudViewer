    ...
    if (data->imgInfos().empty() == false) {
        ...
        for (int i = 0; i < data->imgInfos().size(); ++i) {
            if (data->activeImages()[i]) {
                ...
                _inputImages[i]->load(
                        data->imgPath() + "/" + data->imgInfos().at(i).filename,
                        false);
            } else {
        ...
        SIBR_WRG << "cannot load images (ImageListFile is empty. Did you use "
                    "ImageListFile::load(...) before ?";
...
/// \todo UN-TESTED code!!!!
void InputImages::loadFromPath(const IParseData::Ptr& data,
...
#pragma omp parallel for
    for (int i = 0; i < data->imgInfos().size(); ++i) {
        if (data->activeImages()[i]) {
            std::string imgPath = data->basePathName() + "/images/" + prefix +
                                  sibr::imageIdToString(i) + postfix;
            if (!_inputImages[i]->load(imgPath, false)) {
                SIBR_WRG << "could not load input image : " << imgPath
                         << std::endl;
            }
        }
    }
...
void InputImages::alphaBlendInputImages(const std::vector<sibr::ImageRGB>& back,
                                        std::vector<sibr::ImageRGB>& alphas) {
    for (uint i = 0; i < _inputImages.size(); i++) {
        // check size
        if (_inputImages[i]->w() != alphas[i].w() ||
            _inputImages[i]->h() != alphas[i].h())
            alphas[i] = alphas[i].resized(_inputImages[i]->w(),
                                          _inputImages[i]->h());
        for (uint x = 0; x < _inputImages[i]->w(); x++)
            for (uint y = 0; y < _inputImages[i]->h(); y++) {
                ...
                Vector3f alpha(float(a[0] / 255.), float(a[1] / 255.),
                               float(a[2] / 255.));
                float al = alpha[0];  // assume grey for now
                Vector3f val = Vector3f(float(p[0]), float(p[1]), float(p[2]));
                Vector3f bval =
                        Vector3f(float(bp[0]), float(bp[1]), float(bp[2]));
                Vector3f out;
                if (alpha[0] > 0.4) {
                    out = (val - bval + al * bval) /
                          al;  // foreground solving for a
...
#define UP(x) ((x) > 255.f ? 255.f : x)
                ...
                _inputImages[i](x, y) = ImageRGB::Pixel(
                        (uint)out[0], (uint)out[1], (uint)out[2]);
            }
    }
}
}  // namespace sibr
