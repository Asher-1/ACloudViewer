...
Volume3u loadVideoVolume(sibr::Video& video) {
    ...
    Volume3u volume(video.getNumFrames(), video.getResolution()[0],
                    video.getResolution()[1]);
...
SIBR_VIDEO_EXPORT std::vector<sibr::Volume3u> gaussianPyramid(
        const sibr::Volume3u& vid, uint num_levels) {
    if (num_levels == 0) {
...
SIBR_VIDEO_EXPORT std::vector<sibr::Volume3u> gaussianPyramidTemporal(
        const sibr::Volume3u& vid, uint num_levels) {
    if (num_levels == 0) {
        num_levels = optimal_num_levels(vid.l);
    }
...
std::vector<sibr::Volume3u> laplacianPyramid(const sibr::Volume3u& vid,
                                             uint num_levels) {
    if (num_levels == 0) {
        num_levels = optimal_num_levels(vid.l);
    }
...
std::vector<sibr::Volume3u> laplacianPyramidTemporalDouble(
        const sibr::Volume3u& vid, uint num_levels) {
    if (num_levels == 0) {
        num_levels = 1;
        int length = vid.l;
    ...
    for (int i = 0; i < (int)num_levels - 1; ++i) {
        ...
        up = down.pyrUpTemporal((current_v.l + 1) / 2)
                     .pyrUpTemporal(current_v.l);
        current_v.substract(up);
        current_v.shift(128);
        out.push_back(current_v.convertTo<uchar>());
...
SIBR_VIDEO_EXPORT sibr::Volume3u collapseLaplacianPyramid(
        const std::vector<sibr::Volume3u>& pyr, double shift) {
...
SIBR_VIDEO_EXPORT sibr::Volume3u laplacianBlending(
        const sibr::Volume3u& vA,
        const sibr::Volume3u& vB,
        std::vector<sibr::Volume1u>& pyrM) {
...
cv::Mat VideoUtils::cropFromSize(const cv::Mat& mat,
    ...
    sibr::Vector2i topLeft =
            ((currentSize - targetSize).cast<float>() / 2.0).cast<int>();
    sibr::Vector2i bottomRight =
            ((currentSize + targetSize).cast<float>() / 2.0).cast<int>();
    cv::Rect roi(cv::Point(topLeft[0], topLeft[1]),
                 cv::Point(bottomRight[0], bottomRight[1]));
...
void VideoUtils::getMeanVariance(cv::VideoCapture& cap,
                                 cv::Mat& outMean,
                                 cv::Mat& outVariance,
                                 const sibr::Vector2i& finalSize) {
    cap.set(cv::VideoCaptureProperties::CAP_PROP_POS_FRAMES, 0);
    bool first = true;
    cv::Mat mean, meanSq, out;
    float sum = 0;
    int f_id = 0, current_seg = -1;
    bool doResize = (finalSize[0] != cap.get(cv::CAP_PROP_FRAME_WIDTH) ||
                     finalSize[1] != cap.get(cv::CAP_PROP_FRAME_HEIGHT));
    while (true) {
        std::cout << "." << std::flush;
    ...
    mean /= sum;
    cv::Mat var = cv::min(255.0f * 255.0f,
                          cv::max(0.0f, meanSq / sum - mean.mul(mean)));
    cv::sqrt(var, var);
    var *= 5.0;
...
void VideoUtils::getMeanVariance2(cv::VideoCapture& cap,
                                  cv::Mat& outMean,
                                  cv::Mat& outVariance,
                                  const sibr::Vector2i& finalSize,
                                  float starting_point_s) {
...
cv::Mat VideoUtils::getMedian(sibr::Video& vid,
                              float time_skiped_begin,
                              float time_skiped_end) {
    ...
    for (int i = 0; i < median.rows; ++i) {
        for (int j = 0; j < median.cols; ++j) {
            ...
            for (int c = 0; c < 3; ++c) {
                ...
                for (int t = 0; t < L; ++t) {
                    values[t] =
                            volume.at<uchar>(t, 3 * (i * median.cols + j) + c);
                }
                // std::sort(values.begin(), values.end());
                // medianColor[c] = values[values.size() / 2];
                std::nth_element(values.begin(), values.begin() + L / 2,
                                 values.end());
...
cv::Mat3b VideoUtils::getMedian(const std::string& path,
                                float time_percentage_crop) {
    sibr::Video vid(path);
...
cv::Mat VideoUtils::getBackgroundImage(sibr::Video& vid,
                                       int numBins,
                                       float time_skip_begin,
                                       float time_skip_end) {
    cv::Mat volume = vid.getVolume(time_skip_begin, time_skip_end);
    volume = volume.t();
    return getBackgroundImage(volume, vid.getResolution()[0],
                              vid.getResolution()[1], numBins);
}

cv::Mat VideoUtils::getBackgroundImage(const cv::Mat volume,
                                       int w,
                                       int h,
                                       int numBins) {
    ...
    for (int i = 0; i < bg.rows; ++i) {
        for (int j = 0; j < bg.cols; ++j) {
            ...
            for (int c = 0; c < 3; ++c) {
                for (int t = 0; t < L; ++t) {
                    values[t][c] =
                            volume.at<uchar>(3 * (i * bg.cols + j) + c, t);
                }
            }
...
void VideoUtils::getBackGroundVideo(sibr::Video& vid,
                                    PyramidLayer& out_mask,
                                    PyramidLayer& out_video,
                                    cv::Mat& out_img,
                                    const sibr::ImageRGB& meanImg,
                                    int threshold,
                                    int numBins,
                                    float time_skip_begin,
                                    float time_skip_end) {
    ...
    const int w = vid.getResolution()[0], h = vid.getResolution()[1],
              L = volume.cols;
    out_mask.w = w;
    out_mask.l = L;
    out_mask.h = h;
    ...
    std::cout << w << " " << h << " " << L << " use mean img " << useMeanImg
              << std::endl;
#pragma omp parallel for
    for (int i = 0; i < h; ++i) {
        for (int j = 0; j < w; ++j) {
            ...
            out_img.at<cv::Vec3b>(i, j) =
                    sibr::toOpenCV<uchar, uchar, 3>(mode_color);
            ...
            if (useMeanImg) {
                ...
                for (int di = -radius; di <= radius; ++di) {
                    ...
                    for (int dj = -radius; dj <= radius; ++dj) {
                        ...
                        sumColorSq +=
                                meanImg(jj, ii).cast<float>().cwiseProduct(
                                        meanImg(jj, ii).cast<float>());
                    }
                }
                sumColor /= (float)(num);
                sumColorSq = sumColorSq / (float)num -
                             sumColor.cwiseProduct(sumColor);
            ...
            for (int t = 0; t < L; ++t) {
                ...
                for (int c = 0; c < 3; ++c) {
                    // out_mask.volume.at<uchar>(t, 3 * (i*w + j) + c) = (cdf <
                    // 0.75f ? 0 : 255); out_video.volume.at<uchar>(t, 3 * (i*w
                    // + j) + c) = viz_color[c];

                    if ((color.cast<int>() - mode_color.cast<int>()).norm() <
                        threshold) {
...
sibr::Volume1u VideoUtils::getBackgroundVolume(const sibr::Volume3u& volume,
                                               int threshold,
                                               int numBins) {
...
sibr::Volume1f VideoUtils::getBackgroundVolumeF(const sibr::Volume3u& volume,
                                                int numBins) {
    const int L = volume.l;
...
#pragma omp parallel for
    for (int i = 0; i < volume.h; ++i) {
        for (int j = 0; j < volume.w; ++j) {
            ...
            for (int t = 0; t < L; ++t) {
                const auto& color = values[t];
                out_mask.pixelAt(t, i, j) =
                        (float)(color.cast<int>() - mode_color.cast<int>())
                                .norm();
            }
        }
    }
...
void VideoUtils::computeSaveSimpleFlow(sibr::Video& vid, bool show) {
    ...
    std::string filepath = folder + "/" + path.stem().string() + "_sflow_" +
                           std::to_string(layers) + "_" +
                           std::to_string(block_size) + +"_" +
                           std::to_string(max_flow) + ".mp4";

    sibr::FFVideoEncoder encoder(filepath, 30, {2 * vol.w, 2 * vol.h});
    for (int t = 0; t < vol.l - 1; ++t) {
        cv::Mat flow;
        cv::optflow::calcOpticalFlowSF(vol.frame(t), vol.frame(t + 1), flow,
                                       layers, block_size, max_flow);
        ...
        for (int i = 0; i < vol.h; i += r) {
            for (int j = 0; j < vol.w; j += r) {
                ...
                if (isfinite(f[0]) && isfinite(f[1])) {
                    if (cv::norm(f) > 0.5) {
                        cv::line(viz, cv::Point(j, i),
                                 cv::Point(int(j + f[1]), int(i + f[0])),
                                 {255, 0, 255}, 2);
                    }
                } else {
        ...
        cv::resize(viz, viz, cv::Size(2 * viz.cols, 2 * viz.rows), 0, 0,
                   cv::INTER_NEAREST);
...
#pragma omp parallel for
    for (int t = 0; t < volume.l; ++t) {
        cv::ximgproc::guidedFilter(volume.frame(t), mask.frame(t),
                                   bilateral_mask.frame(t), radius_bila, eps);
        // cv::medianBlur(mask.frame(t), median_mask.frame(t), 7);
        cv::medianBlur(bilateral_mask.frame(t), bilateral_mask_median.frame(t),
                       5);
        median_bilateral_mask_binary.frame(t) =
                bilateral_mask_median.frame(t) > threshold;
    }

    sibr::Volume3u video_masked_bilateral_bin =
            volume.applyMaskBinary(median_bilateral_mask_binary);
    ...
    const std::string folder =
            filepath.parent_path().string() + "/masks/bilateral/";
    sibr::makeDirectory(folder);

    const std::string basename = folder + filepath.stem().string() + "_bila_" +
                                 std::to_string(radius_bila) + "_" +
                                 std::to_string((int)(10 * eps));
    ...
    median_bilateral_mask_binary.saveToVideoFile(basename + "_median_binary" +
                                                 extension);
...
void VideoUtils::computeSaveVideoMaskBlur(Video& vid, int time_window) {
    ...
    const std::string in_filename =
            filepath.parent_path().string() + "/masks/bilateral/" +
            filepath.stem().string() + "_bila_21_100_median_binary.mp4";
    const std::string out_folder =
            filepath.parent_path().string() + "/masks/bilateral_tblur/";
    sibr::makeDirectory(out_folder);
    const std::string out_filename =
            out_folder + "/" + filepath.stem().string() + "_mask_tblur.mp4";

    sibr::Volume3u volume = sibr::loadVideoVolume(in_filename);
    std::cout << "volume.mat.isContinuous() : " << volume.mat.isContinuous()
              << std::endl;
    ...
    for (int i = 0; i < out.h; ++i) {
        for (int j = 0; j < out.w; ++j) {
            for (int t = 0; t < out.l; ++t) {
                for (int u = std::max(0, t - time_win);
                     u < std::min(out.l - 1, t + time_win); ++u) {
...
cv::Mat VideoUtils::getTemporalSpatialRatio(sibr::Video& vid,
                                            PyramidLayer& out_ratio,
                                            const sibr::ImageRGB& spatial_ratio,
                                            int numBins,
                                            float time_skip_begin,
                                            float time_skip_end) {
    cv::Mat volume = vid.getVolume(time_skip_begin, time_skip_end).t();
    ...
    for (int i = 0; i < h; ++i) {
        for (int j = 0; j < w; ++j) {
            ...
            for (int t = 0; t < L; ++t) {
                ...
                sibr::Vector3f norm_temporal =
                        (color.cast<int>() - mode_color.cast<int>())
                                .cwiseAbs()
                                .cast<float>();
                sibr::Vector3f norm_spatial =
                        spatial_ratio(j, i).cwiseAbs().cast<float>().array() +
                        10;
                sibr::Vector3f ratios =
                        norm_temporal.cwiseQuotient(norm_spatial);

                for (int c = 0; c < 3; ++c) {
                    out_ratio.volume.at<uchar>(t, 3 * (i * w + j) + c) =
                            sibr::clamp<uchar>((uchar)(128 * ratios[c]), 0,
                                               255);
                    ...
                    // }
                    // out_ratio.volume.at<uchar>(t, 3 * (i*w + j) + c) =
                    // sibr::clamp<uchar>((uchar)(64*ratios[c]),0,255);
                }
            }
        }
    }
...
cv::Mat VideoUtils::getLaplacian(cv::Mat mat,
                                 int size,
                                 bool smooth,
                                 bool absolute) {
    cv::Mat grey, laplacian, abs;
    if (smooth) {
        cv::GaussianBlur(mat, mat, cv::Size(size, size), 0, 0,
                         cv::BORDER_DEFAULT);
    }
    grey = getGrey(mat);
...
int VideoUtils::rotationAngleFromMetadata(const std::string& videoPath) {
    ...
    std::string tmpFilePath =
            parentAbs + "/" + vidPath.stem().string() + "_tmp.txt";

    std::string cmd = "ffprobe -i \"" + vidPath.string() + "\" > \"" +
                      tmpFilePath + "\" 2>&1";
...
void VideoUtils::ECCtransform(cv::Mat matA,
                              cv::Mat matB,
                              cv::Mat& correctedB,
                              cv::Mat& diff,
                              int cvMotion) {
...
void VideoUtils::smallAlignmentVideo(sibr::Video& vid,
                                     const std::string& outputVidPath,
                                     bool viz) {
    struct Match {
        cv::Point2f in, out;
        float error;
    ...
    // cv::VideoWriter out(outputVidPath, codec_OpenH264, vid.getFrameRate(),
    // cv::Size(vid.getResolution()[0], vid.getResolution()[1]));
    sibr::FFVideoEncoder out(outputVidPath, vid.getFrameRate(),
                             vid.getResolution());
    ...
    cv::goodFeaturesToTrack(initGray, features, numFeatures,
                            features_to_track_quality, 10);
    ...
    for (;;) {
        ...
        cv::calcOpticalFlowPyrLK(initGray, gray, features, nextFeatures, status,
                                 errors, cv::Size(15, 15), 0);
        ...
        std::vector<cv::Point2f> inputFeatures(numBestMatch),
                outputFeatures(numBestMatch);
        ...
        if (viz) {
            ...
            for (int i = 0; i < numBestMatch; ++i) {
                cv::circle(corresp_viz, matchs[i].in, 5, cv::Scalar(0, 255, 0),
                           2);
                cv::circle(corresp_viz, matchs[i].out, 5, cv::Scalar(255, 0, 0),
                           2);
            }
        ...
        cv::Mat homography = cv::findHomography(inputFeatures, outputFeatures,
                                                cv::RANSAC, ransac_repro_error);

        cv::Mat correctedFrame;
        cv::warpPerspective(nextFrame, correctedFrame, homography.inv(),
                            nextFrame.size());
...
void VideoUtils::smallAlignmentVideo2(sibr::Video& vid,
                                      const std::string& outputVidPath,
                                      bool viz) {
    struct Match {
        cv::Point2f in, out;
    ...
    for (;;) {
        ...
        cv::goodFeaturesToTrack(initGray, features, numFeatures,
                                features_to_track_quality, features_min_dist);

        cv::calcOpticalFlowPyrLK(initGray, gray, features, nextFeatures, status,
                                 errors, cv::Size(5, 5), 0);

        std::vector<Match> matchs;
        for (int i = 0; i < (int)status.size(); ++i) {
            auto v = features[i] - nextFeatures[i];

            if (status[i] == 1 && cv::norm(cv::Vec2f(v.x, v.y), cv::NORM_INF) <
                                          max_displacement) {
                matchs.push_back({features[i], nextFeatures[i], errors[i]});
            }
        }
        ...
        cv::Mat correctedFrame;
        cv::warpPerspective(nextFrame, correctedFrame, completeHomography.inv(),
                            nextFrame.size());
...
void VideoUtils::simpleFlow(
        cv::VideoCapture& cap,
        float ratio,
        std::function<
                bool(cv::Mat prev, cv::Mat next, cv::Mat flow, int flow_id)> f,
    ...
    while (true) {
        ...
        cv::resize(next, next,
                   cv::Size((int)(ratio * next.size().width),
                            (int)(ratio * next.size().height)));
...
void VideoUtils::simpleFlowViz(cv::VideoCapture& cap, float ratio) {
    simpleFlow(
            cap, ratio,
            [](cv::Mat prev, cv::Mat next, cv::Mat flow, int flow_id) {
...
void VideoUtils::simpleFlowSave(
        cv::VideoCapture& cap,
        float ratio,
        std::function<std::string(int flow_id)> naming_f) {
    std::cout << " saving flow " << std::flush;
    simpleFlow(cap, ratio,
               [&](cv::Mat prev, cv::Mat next, cv::Mat flow, int flow_id) {
                   std::cout << "." << std::flush;
                   cv::Mat viz = getFlowViz(flow);
...
void VideoUtils::deepFlow(
    ...
    while (true) {
        ...
        auto size = cv::Size((int)(ratio * next.size().width),
                             (int)(ratio * next.size().height));
...
void PyramidLayer::saveToVideoFile(const std::string& filename,
                                   double framerate) {
...
void PyramidLayer::show(PyramidLayer A,
                        PyramidLayer B,
                        PyramidLayer C,
                        PyramidLayer D,
                        int s,
...
#pragma omp parallel for
    for (int t = 0; t < layer.l; ++t) {
        ...
        if (params.splacialDS) {
            cv::GaussianBlur(sliceIn, sliceOut,
                             cv::Size(2 * params.spatial_radius + 1,
                                      2 * params.spatial_radius + 1),
                             0);
        } else {
...
//	PyramidLayer temporalBlur(const PyramidLayer & layer, const
// PyramidParameters &  params, float scaling)
//	{
//		//cv::Mat kernel = cv::Mat::ones(cv::Size(2 *
// params.temporal_radius + 1, 1), CV_32FC1);
//		//kernel = kernel / cv::norm(kernel, cv::NORM_L1);
//
//		const cv::Mat kernel = (scaling / 16.0f)*(cv::Mat_<float>(1, 5)
//<< 1, 4, 6, 4, 1);
//
//		//layer.show(0);
//
...
//
//		cv::filter2D(vol, vol, -1, kernel, { 2,0 }, 0,
// cv::BORDER_DEFAULT);
//
////#pragma omp parallel for
...
////				for (int c = 0; c < 3; ++c) {
////					cv::Mat slice = vol.row(3 * (i*layer.w +
/// j) + c); /					cv::filter2D(slice, slice, -1,
/// kernel, { 2,0 }, 0, cv::BORDER_DEFAULT); // cv::Point(-1,-1) BORDER_ISOLATED
/// cv::BORDER_REPLICATE /				} / } /		}
//
//		//std::cout << vol.row(3 * (0 * layer.w + 0) + 0) << std::endl;
//		//std::cout << vol.row(3 * (0 * layer.w + 0) + 1) << std::endl;
...
PyramidLayer temporalBlur(const PyramidLayer& layer,
                          const PyramidParameters& params,
                          float scaling) {
    const cv::Mat kernel =
            (scaling / 16.0f) * (cv::Mat_<float>(5, 1) << 1, 4, 6, 4, 1);
...
void temporalBlurInPlace(PyramidLayer& layer,
                         const PyramidParameters& params,
                         float scaling) {
    const cv::Mat kernel =
            (scaling / 16.0f) * (cv::Mat_<float>(5, 1) << 1, 4, 6, 4, 1);
    cv::filter2D(layer.volume, layer.volume, -1, kernel, {-1, -1}, 0.0,
                 cv::BORDER_DEFAULT);
}

PyramidLayer decimate(const PyramidLayer& layer,
                      const PyramidParameters& params) {
...
#pragma omp parallel for
    for (int t = 0; t < out.l; ++t) {
        ...
        // cv::resize(sliceCurrent, sliceDecimated, sliceDecimated.size(), 0, 0,
        // CV_INTER_NN);
    }
...
PyramidLayer upscale(const PyramidLayer& layerUp,
                     const PyramidLayer& layerDown,
                     const PyramidParameters& params) {
    // std::cout << layerUp.w << " " << layerUp.h << " " << layerUp.l <<
    // std::endl; std::cout << layerDown.w << " " << layerDown.h << " " <<
    // layerDown.l << std::endl;
    ...
    for (int t = 0; t < layerDown.l; ++t) {
        ...
        //	if (2 * t + 1 < layerUp.l) {
        //		sliceUp.copyTo(out.volume.row(2 * t + 1).reshape(3,
        // layerUp.h));
        //	}
    }
...
PyramidLayer downscale(const PyramidLayer& layer,
                       const PyramidParameters& params) {
    ...
    if (params.splacialDS) {
        out = PyramidLayer((layer.w + 1) / 2, (layer.h + 1) / 2,
                           (layer.l + 1) / 2);
    } else {
    ...
    for (int t = 0; t < out.l; ++t) {
        ...
        //	continue;
        // }
        // cv::Mat sliceCurrent = blured.volume.row(2 * t + 1).reshape(3,
        // layer.h);
    ...
    //	cv::pyrDown(layer.volume.row(i), blur.volume.row(i),
    // blur.volume.row(i).size());
    // }
...
SIBR_VIDEO_EXPORT cv::Mat slice(
        const PyramidLayer& layer, int i, int j, bool vertical, bool center) {
    cv::Mat out;
    if (vertical) {
        ...
        for (int t = 0; t < layer.l; ++t) {
            for (int i = 0; i < layer.h; ++i) {
                for (int c = 0; c < 3; ++c) {
                    out.at<cv::Vec3b>(t, i)[c] = (uchar)sibr::clamp(
                            (int)layer.volume.at<float>(
                                    t, 3 * (i * layer.w + j) + c) +
                                    (center ? 128 : 0),
                            0, 255);
                }
...
VideoGaussianPyramid buildVideoGaussianPyramid(const cv::Mat& volume,
                                               int w,
                                               int h,
                                               int nLevels,
                                               const PyramidParameters& params,
...
VideoGaussianPyramid buildVideoGaussianPyramid(sibr::Video& vid,
                                               int nLevels,
                                               const PyramidParameters& params,
                                               bool show) {
    return buildVideoGaussianPyramid(vid.getVolume(), vid.getResolution()[0],
                                     vid.getResolution()[1], nLevels, params,
                                     show);
}

SIBR_VIDEO_EXPORT VideoLaplacianPyramid
buildVideoLaplacianPyramid(PyramidLayer vid,
                           int nLevels,
                           const PyramidParameters& params,
                           bool show) {
...
VideoLaplacianPyramid buildVideoLaplacianPyramid(
        sibr::Video& vid,
        int nLevels,
        const PyramidParameters& params,
        bool show) {
    PyramidLayer layer(vid.getVolume(), vid.getResolution()[0],
                       vid.getResolution()[1]);
    return buildVideoLaplacianPyramid(layer, nLevels, params, show);
}

VideoLaplacianPyramid buildVideoLaplacianPyramidFullyReduced(
        PyramidLayer vid,
        int nLevels,
        const PyramidParameters& params,
        bool show) {
    VideoLaplacianPyramid standardPyramid =
            buildVideoLaplacianPyramid(vid, nLevels, params, show);
...
SIBR_VIDEO_EXPORT void convertReducedVideoPyramidTo128(
        VideoLaplacianPyramid& vid) {
...
PyramidLayer videoLaplacianBlending(sibr::Video& vidA,
                                    sibr::Video& vidB,
                                    PyramidLayer mask_volume) {
    int num_lvls = 6;
    auto pyrA = sibr::buildVideoLaplacianPyramid(vidA, num_lvls);
    auto pyrB = sibr::buildVideoLaplacianPyramid(vidB, num_lvls);
    auto pyrM = sibr::buildVideoGaussianPyramid(
            mask_volume.volume, mask_volume.w, mask_volume.h, num_lvls);
...
PyramidLayer videoLaplacianBlending(PyramidLayer vidA,
                                    PyramidLayer vidB,
                                    PyramidLayer mask_volume,
    ...
    auto pyrM = sibr::buildVideoGaussianPyramid(mask_volume.volume,
                                                mask_volume.w, mask_volume.h,
                                                num_lvls, params, show);
    ...
    for (int l = 0; l < num_lvls; ++l) {
        ...
        layer.volume =
                A_lvl.mul(normalized_mask) + B_lvl.mul(normalized_mask_r);
        // layer.show();
...
std::vector<FullContribData> videoLaplacianBlendingContrib(
        PyramidLayer vidA,
        PyramidLayer vidB,
        PyramidLayer mask_volume,
        PyramidParameters params) {
    ...
    auto pyrM = sibr::buildVideoGaussianPyramid(
            mask_volume.volume, mask_volume.w, mask_volume.h, num_lvls, params);
...
void videoLaplacianBlendingDebug(PyramidLayer vidA,
                                 PyramidLayer vidB,
                                 PyramidLayer mask_volume,
                                 PyramidParameters params) {
    int num_lvls = params.num_levels;
...
//	return  std::distance(bins.begin(), std::max_element(bins.begin(),
// bins.end()));
// }
