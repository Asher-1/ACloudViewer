...
bool Video::load(const std::string& path) {
    ...
    if (loaded) {
        nFrames =
                (int)cap.get(cv::VideoCaptureProperties::CAP_PROP_FRAME_COUNT);
        frameRate = (double)cap.get(cv::VideoCaptureProperties::CAP_PROP_FPS);
        resolution[0] =
                (int)cap.get(cv::VideoCaptureProperties::CAP_PROP_FRAME_WIDTH);
        resolution[1] =
...
cv::Mat Video::getVolume(float time_skiped_begin, float time_skiped_end) {
    ...
    const int finishing_frame =
            getNumFrames() - (int)(time_skiped_end * getFrameRate()) - 1;
...
bool Video::exists() const { return sibr::fileExists(getFilepath().string()); }

void Video::checkLoad() {
    if (!loaded) {
        if (!load(filepath.string())) {
            SIBR_ERR << "[Video] Could not open video " << filepath
                     << std::endl;
        }
    }
}
...
VideoPlayer::VideoPlayer(const std::string& filepath,
                         const std::function<cv::Mat(cv::Mat)>& f)
...
void VideoPlayer::onGui(float ratio_display) {
    ...
    if (ImGui::SliderInt("timeline", &current_frame_slider, 1,
                         getNumFrames())) {
    ...
    if (getDisplayTex() && getDisplayTex()->handle()) {
        std::string infos =
                "size : " + std::to_string((int)getDisplayTex()->w()) + " " +
                std::to_string((int)getDisplayTex()->h()) +
                ", framerate : " + std::to_string(getFrameRate());
        ImGui::Text(infos.c_str());
        sibr::Vector2f displayTexSize(getDisplayTex()->w(),
                                      getDisplayTex()->h());
        sibr::Vector2i viewResolution =
                (ratio_display * displayTexSize).cast<int>();

        sibr::ImageWithCallback(getDisplayTex()->handle(), viewResolution,
                                callBackData, zoomData.topLeft(),
                                zoomData.bottomRight());
...
void VideoPlayer::updateGPU() {
    if (getLoadingTex().get()) {
        ...
        getLoadingTex() = std::shared_ptr<sibr::Texture2DRGB>(
                new sibr::Texture2DRGB(tmpFrame));
    }
}
