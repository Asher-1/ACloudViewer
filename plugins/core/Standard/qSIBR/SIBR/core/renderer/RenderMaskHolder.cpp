...
const std::vector<RenderMaskHolder::MaskPtr>& RenderMaskHolder::getMasks(
        void) const {
    return _masks;
}

bool RenderMaskHolder::useMasks(void) const { return _masks.empty() == false; }

void RenderMaskHolder::uploadMaskGPU(sibr::ImageL8& img,
                                     int i,
                                     std::vector<RenderTargetLum::Ptr>& masks,
                                     bool invert) {
    ...
    textureShader.init(
            ...
            sibr::loadFile(sibr::Resources::Instance()->getResourceFilePathName(
                    "texture.vp")),
            invert ? sibr::loadFile(sibr::getShadersDirectory("core") +
                                    "/texture-invert.frag")
    ...
    img.flipH();
    std::shared_ptr<sibr::Texture2DLum> rawInputImage(
            new sibr::Texture2DLum(img));
    img.flipH();
...
void RenderMaskHolder::loadMasks(const sibr::BasicIBRScene::Ptr& ibrScene,
    ...
    if (boost::filesystem::exists(maskDir)) {
        for (int i = 0; i < (int)ibrScene->cameras()->inputCameras().size();
             i++) {
            sibr::ImageRGB mask;
            std::string filename = maskDir + "/" + preFileName +
                                   sibr::imageIdToString(i) + postFileName;
            ...
            if (boost::filesystem::exists(filename)) {
                ...
                if (ibrScene->cameras()->inputCameras()[i]->isActive())
                    SIBR_ERR << "[RenderMaskHolder] couldnt find " << filename
                             << std::endl;
                else {  /// push back empty mask so array is consistent
                    /// \todo TODO GD -- this is wasteful, should fine better
                    /// way
                    std::shared_ptr<sibr::RenderTargetLum> maskRTPtr;
        ...
        SIBR_ERR << "[RenderMaskHolder] Cant find directory " << maskDir
                 << std::endl;
}
...
const RenderMaskHolderArray::MaskArrayPtr& RenderMaskHolderArray::getMasks(
        void) const {
    return _masks;
}

void RenderMaskHolderArray::loadMasks(const sibr::BasicIBRScene::Ptr& ibrScene,
                                      const std::string& maskDir,
                                      const std::string& preFileName,
                                      const std::string& postFileName,
                                      int w,
                                      int h) {
    std::string maskdir =
            (maskDir == "" ? ibrScene->data()->basePathName() + "/images/"
                           : maskDir);
    ...
    if (!boost::filesystem::exists(maskdir)) {
        ...
        for (int i = 0; i < numInputImgs; i++) {
            ...
            if (mask.empty()) {
                SIBR_ERR << "[RenderMaskHolderArray] couldnt find or read "
                         << filename << std::endl;
            }
