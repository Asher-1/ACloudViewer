...
NormalRenderer::NormalRenderer(
        int w, int h, bool generate, bool useFloats, bool imSpace) {
    _generate = generate;
    ...
    if (_generate) {
        _normalShader.init(
                "NormalShader",
                sibr::loadFile(
                        sibr::Resources::Instance()->getResourceFilePathName(
...
void NormalRenderer::render(const sibr::InputCamera& cam,
                            const Mesh& mesh,
    ...
    //	std::cout << cam.znear() << " " << cam.zfar() << " " << cam.viewproj()
    //<< std::endl;
