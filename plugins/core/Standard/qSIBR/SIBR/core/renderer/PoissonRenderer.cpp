...
PoissonRenderer ::PoissonRenderer(uint w, uint h) : _size(w, h) {
    std::string vp =
            sibr::loadFile(sibr::getShadersDirectory("core") + "/texture.vert");
    _jacobiShader.init("Jacobi", vp,
                       sibr::loadFile(sibr::getShadersDirectory("core") +
                                      "/poisson_jacobi.frag"));
    _restrictShader.init("Restrict", vp,
                         sibr::loadFile(sibr::getShadersDirectory("core") +
                                        "/poisson_restrict.frag"));
    _interpShader.init("Interp", vp,
                       sibr::loadFile(sibr::getShadersDirectory("core") +
                                      "/poisson_interp.frag"));
    _divergShader.init("Diverg", vp,
                       sibr::loadFile(sibr::getShadersDirectory("core") +
                                      "/poisson_diverg.frag"));
    ...
    for (uint i = 0; i < _poisson_div_RT.size(); i++) {
        ...
        _poisson_div_RT[i].reset(
                new sibr::RenderTargetRGBA(ww, hh, SIBR_CLAMP_UVS));
    }
    _poisson_RT.reset(new sibr::RenderTargetRGBA(
            w, h, SIBR_CLAMP_UVS | SIBR_GPU_LINEAR_SAMPLING));
    _poisson_tmp_RT.reset(new sibr::RenderTargetRGBA(
            w, h, SIBR_CLAMP_UVS | SIBR_GPU_LINEAR_SAMPLING));
    _enableFix = true;
}
...
uint PoissonRenderer::render(uint texture) {
    ...
    for (int k = 0; k < int(_poisson_div_RT.size()) - 1; k++) {
        ...
        glViewport(0, 0, _poisson_div_RT[k + 1]->w(),
                   _poisson_div_RT[k + 1]->h());
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, _poisson_div_RT[k]->texture());
        _restrict_scale.set(float(_poisson_div_RT[k]->w()) /
                            _poisson_div_RT[k + 1]->w());
        RenderUtility::renderScreenQuad();
    ...
    for (int k = (int)_poisson_div_RT.size() - 1; k >= 0; k--) {
        for (uint i = 0; i < POISSON_ITERATIONS; i++) {
            double h =
                    pow((float)MULTIGRID_SCALE,
                        (int)k);  // Jacobi relaxation filter kernel taken from
            double hsq =
                    h * h;  // Real-Time Gradient-Domain Painting, SIGGRAPH '08
            double xh0 =
                    -2.1532 + 1.5070 / h +
                    0.5882 /
                            hsq;  // http://graphics.cs.cmu.edu/projects/gradient-paint/
            ...
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, _poisson_tmp_RT->texture());
            _jacobi_weights.set((float)xh, (float)e, (float)c,
                                (float)(1.0 / (m - xh)));
            _jacobi_scale.set(isFirst ? (float(_poisson_tmp_RT->w()) /
                                         _poisson_div_RT[k]->w())
                                      : 1.0f);
            RenderUtility::renderScreenQuad();
            _poisson_RT->unbind();
        ...
        if (k > 0) {
            ...
            glBindTexture(GL_TEXTURE_2D, _poisson_div_RT[k - 1]->texture());
            _interp_scale.set(float(_poisson_div_RT[k - 1]->w()) /
                              _poisson_div_RT[k]->w());
            RenderUtility::renderScreenQuad();
            _poisson_RT->unbind();
            _interpShader.end();
...
void PoissonRenderer::process(const RenderTargetRGBA::Ptr& src,
                              RenderTargetRGBA::Ptr& dst) {
