...
FPSCamera::FPSCamera(void) : _hasBeenInitialized(false) {
    ...
    UIShortcuts::global().add("[FPS camera] l",
                              "rotate camera +Y (look right)");
    ...
    UIShortcuts::global().add("[FPS camera] w",
                              "move camera -Z (move forward)");
    UIShortcuts::global().add("[FPS camera] s",
                              "move camera +Z (move backward)");
    UIShortcuts::global().add("[FPS camera] a", "move camera -X (strafe left)");
    UIShortcuts::global().add("[FPS camera] d",
                              "move camera +X (strafe right)");
...
void FPSCamera::snap(const std::vector<InputCamera::Ptr>& cams) {
    ...
    for (const auto& cam : cams) {
        float dist = 1.0f / std::max(1e-6f, distance(_currentCamera.position(),
                                                     cam->position()));
        sumDir += dist * cam->dir();
        sumUp += dist * cam->up();
    }
...
void FPSCamera::update(const sibr::Input& input,
...
const sibr::InputCamera& FPSCamera::getCamera(void) const {
    if (!_hasBeenInitialized) {
        SIBR_ERR << " FPS Camera : camera not initialized before use"
                 << std::endl
...
void FPSCamera::moveUsingWASD(const sibr::Input& input, float deltaTime) {
    ...
    if (_goalAltitude != -1) {
        ...
        const sibr::Vector3f custom_forward =
                _currentCamera.right().cross(worldUp);
        const sibr::Vector3f translation_right =
                (_speedFpsCam * move.x()) * _currentCamera.right();

        sibr::Vector3f translation =
                _speedFpsCam * (move.z() * custom_forward) + translation_right;
        // const float altitudeDiff = _goalAltitude -
        // _currentCamera.position().z();
        translation[2] = _goalAltitude - _currentCamera.position().z();

        _currentCamera.translate(translation);
    } else {
        _currentCamera.translate(move * _speedFpsCam,
...
void FPSCamera::moveUsingMousePan(const sibr::Input& input, float deltaTime) {
    ...
    sibr::Vector3f move(input.mouseButton().isActivated(sibr::Mouse::Left)
                                ? input.mouseDeltaPosition().x() * speed
                                : 0.f,
                        input.mouseButton().isActivated(sibr::Mouse::Right)
                                ? input.mouseDeltaPosition().y() * speed
                                : 0.f,
                        input.mouseButton().isActivated(sibr::Mouse::Middle)
                                ? input.mouseDeltaPosition().y() * speed
                                : 0.f);
