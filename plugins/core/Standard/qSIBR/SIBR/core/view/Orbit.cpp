...
Orbit::Orbit(void) : _hasBeenInitialized(false), _orbitPointClicked(false) {
    UIShortcuts::global().add("[Orbit cam] alt+click",
                              "Select new orbit center.");
    ...
    UIShortcuts::global().add("[Orbit cam] 5",
                              "flip up vector (look upside down)");
    UIShortcuts::global().add("[Orbit cam] alt+1-9", "automatic move");
    UIShortcuts::global().add("[Orbit cam] 0",
                              "stop automatic move, restore previous cam");
    UIShortcuts::global().add("[Orbit cam] .",
                              "stop automatic move, keep current cam");
}

void Orbit::update(const sibr::Input& input,
    ...
    } else if ((input.key().isReleased(sibr::Key::KPNum0) ||
                input.key().isReleased(sibr::Key::Num0)) &&
...
void Orbit::interpolateOrbit() {
    ...
    float theta =
            (_orbit.status == OrbitParameters::FORWARD_X ? (float)(SIBR_2PI * k)
                                                         : _orbit.theta);
    float phi =
            (_orbit.status == OrbitParameters::FORWARD_Y ? (float)(SIBR_2PI * k)
                                                         : _orbit.phi);
    float roll =
            (_orbit.status == OrbitParameters::FORWARD_Z ? (float)(SIBR_2PI * k)
                                                         : _orbit.roll);
    ...
    _currentCamera =
            sibr::InputCamera(n, _currentCamera.w(), _currentCamera.h());
}

void Orbit::updateOrbitParameters(const sibr::Input& input,
    ...
    sibr::CameraRaycaster::computePixelDerivatives(_currentCamera, dx, dy,
                                                   upLeftOffset);
    ...
    if (raycaster != nullptr) {
        ...
        // sibr::Vector3f dir =
        // sibr::CameraRaycaster::computeRayDir(_currentCamera,
        // input.mousePosition().cast<float>()).normalized();
        sibr::RayHit hit =
                raycaster->intersect(sibr::Ray(_currentCamera.position(), dir));

        // If hit at the proxy surface, compute the corresponding worls
        // position, save it.
        if (hit.hitSomething()) {
            _orbit.center =
                    _currentCamera.position() + hit.dist() * dir.normalized();

            // \todo TODO: SR reimplement the fitting of planes by either
            // passing cameras all the way down, or something else.
            ...
            _orbit.radius =
                    (_orbit.initialCamera.position() - _orbit.center).norm();
...
void Orbit::fromCamera(const sibr::InputCamera& cam,
                       const std::shared_ptr<sibr::Raycaster> raycaster) {
    _orbit.initialCamera = cam;
    _currentCamera = cam;
    _hasBeenInitialized = true;

    // If no point has already been selected by the user, we simply pick it
    // automatically by intersecting cam dir and the mesh.
...
void Orbit::updateOrbitParametersCentered(
        const std::shared_ptr<sibr::Raycaster> raycaster) {
    if (raycaster != nullptr) {
        sibr::RayHit hit = raycaster->intersect(
                sibr::Ray(_currentCamera.position(), _currentCamera.dir()));
        // If hit at the proxy surface, compute the corresponding world
        // position, save it.
        if (hit.hitSomething()) {
            sibr::Vector3f intersection(
                    _currentCamera.position() +
                    hit.dist() * _currentCamera.dir().normalized());
            ...
            _orbit.initialCamera = _currentCamera;
            // orbitPointClicked -->; don't set it, the center is picked
            // automatically.
        }
    }
}

sibr::Vector4f Orbit::computeFittingPlaneCameras(
        sibr::Vector3f& clickedPoint,
    ...
    std::vector<sibr::Vector3f> colors(positions.size(),
                                       sibr::Vector3f(1, 0, 0));
    ...
    // smallest eigenvalue (i.e. the direction in which the variance of all
    ...
    std::cout << " \t  plane ( clicked point + input cams ) : " << n.x()
              << "*x + " << n.y() << "*y + " << n.z() << "*z + " << d
              << std::endl;
