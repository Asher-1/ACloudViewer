...
void MonoRdrMode::render(ViewBase& view,
    ...
    ////else
    ////	glViewport(0,0, _w * _ibr->args().rt_factor,
    ///(_ibr->args().fullscreen ? screenHeight : _h) * _ibr->args().rt_factor);
    // viewport.use();
    ////glViewport(0,0, size().x(), size().y());

    // glActiveTexture(GL_TEXTURE0); glBindTexture(GL_TEXTURE_2D,
    // _destRT->texture()); RenderUtility::renderScreenQuad(false
    // /*_ibr->isPortraitAcquisition()*/); _quadShader.end();
...
void StereoAnaglyphRdrMode::render(ViewBase& view,
                                   const sibr::Camera& eye,
                                   const sibr::Viewport& viewport,
                                   IRenderTarget* optDest) {
    int w = (int)viewport.finalWidth();
    ...
    _stereoShader.begin();
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, _leftRT->texture());
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, _rightRT->texture());
