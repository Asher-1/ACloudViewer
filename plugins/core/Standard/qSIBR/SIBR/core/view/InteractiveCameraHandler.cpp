...
InteractiveCameraHandler::InteractiveCameraHandler(const bool supportRecording)
    : _trackball(true) {
    _currentMode = FPS;
    ...
    if (_supportRecording) {
        ...
        sibr::UIShortcuts::global().add(
                "ctrl+c", "save camera path (enter filename in the prompt)");
        sibr::UIShortcuts::global().add(
                "shift+c", "load camera path (enter filename in the prompt)");
        sibr::UIShortcuts::global().add("alt+c", "start recording camera path");
    }
}

// save default camera for a scene
void InteractiveCameraHandler::saveDefaultCamera(
...
void InteractiveCameraHandler::loadDefaultCamera(
        const sibr::InputCamera& cam, const std::string& datasetPath) {
    ...
    if (camFile.good()) {
        ...
        SIBR_LOG << "Loaded  " << datasetPath << "/default_camera.bin"
                 << std::endl;
        fromCamera(savedCam, false);
    }
}

void InteractiveCameraHandler::setup(
        const std::vector<InputCamera::Ptr>& cams,
        const sibr::Vector2u& resolution,
        const sibr::Viewport& viewport,
        const std::shared_ptr<sibr::Raycaster> raycaster) {
...
void InteractiveCameraHandler::setup(
        const Eigen::AlignedBox<float, 3>& areaOfInterest,
        const sibr::Viewport& viewport,
        const std::shared_ptr<sibr::Raycaster> raycaster) {
    _raycaster = raycaster;
    _viewport = viewport;
...
void InteractiveCameraHandler::setup(
        ...
        const sibr::Viewport& viewport,
        const std::shared_ptr<sibr::Raycaster> raycaster,
        const sibr::Vector2f& clippingPlanes) {
    ...
    SIBR_LOG << "Interactive camera using (" << idealCam.znear() << ","
             << idealCam.zfar() << ") near/far planes." << std::endl;

    setup(idealCam, viewport, raycaster);
}

void InteractiveCameraHandler::setup(const std::shared_ptr<sibr::Mesh> mesh,
                                     const sibr::Viewport& viewport) {
...
void InteractiveCameraHandler::fromCamera(const sibr::InputCamera& cam,
                                          bool interpolate,
    ...
    if (updateResolution) {
        // Viewport might have not been set, in this case defer the full camera
        // update until after the viewport has been updated, ie in onUpdate().
    ...
    if (_raycaster != nullptr) {
        sibr::RayHit hit = _raycaster->intersect(
                sibr::Ray(idealCam.position(), idealCam.dir()));
        // If hit at the proxy surface, save the distance between the camera and
        // the mesh, to use as a trackball radius.
        if (hit.hitSomething()) {
            _radius = hit.dist();
        }
    }
...
void InteractiveCameraHandler::fromTransform(const Transform3f& transform,
                                             bool interpolate,
                                             bool updateResolution) {
...
int InteractiveCameraHandler::findNearestCamera(
        const std::vector<InputCamera::Ptr>& inputCameras,
        const bool& useRotation) const {
    if (inputCameras.size() == 0) return -1;

    int selectedCam = 0;
    int numCams = inputCameras.size();

    std::vector<uint> sortByDistance =
            sibr::IBRBasicUtils::selectCamerasSimpleDist(
                    inputCameras, _currentCamera, numCams);
    std::vector<uint> sortByAngle =
            sibr::IBRBasicUtils::selectCamerasAngleWeight(
                    inputCameras, _currentCamera, numCams);
...
void InteractiveCameraHandler::setupInterpolationPath(
        const std::vector<InputCamera::Ptr>& cameras) {
    ...
    for (int i = 0; i < cameras.size(); i++) {
        if (cameras[i]->isActive()) {
            if (cameras[i]->id() < cameras.size()) {
                ...
                std::cout << "Cameras ID inconsistent. Setting default "
                             "interpolation path."
                          << std::endl;
                defaultPath = true;
                break;
    ...
    if (defaultPath) {
        ...
        std::sort(_interpPath.begin(), _interpPath.end(),
...
void InteractiveCameraHandler::interpolate() {
    ...
    // If we reach the last frame of the interpolation b/w two cameras, skip to
    // next camera.
    ...
    float k = std::min(
            std::max(((_interpFactor) / (float)SIBR_INTERPOLATE_FRAMES), 1e-6f),
            1.0f - 1e-6f);
...
void InteractiveCameraHandler::snapToCamera(const int i) {
    if (!_interpPath.empty()) {
        unsigned int nearestCam =
                (i == -1 ? findNearestCamera(_interpPath) : i);
        nearestCam = sibr::clamp(nearestCam, (unsigned int)(0),
                                 (unsigned int)(_interpPath.size() - 1));
        fromCamera(*_interpPath[nearestCam], true, false);
    }
}

float InteractiveCameraHandler::getInterpolatedHeight(
        const std::vector<InputCamera::Ptr>& inputCameras) {
    const uint numCams = inputCameras.size();
    std::vector<uint> sortByDistance =
            sibr::IBRBasicUtils::selectCamerasSimpleDist(
                    inputCameras, _currentCamera, numCams, true);
    Vector3f pos0 = 0.5f * (_interpPath[sortByDistance[0]]->position() +
                            _interpPath[sortByDistance[1]]->position());
    Vector3f pos1 = 0.5f * (_interpPath[sortByDistance[2]]->position() +
                            _interpPath[sortByDistance[3]]->position());
    ...
    const float height =
            t * pos0.z() +
            (1 - t) *
                    pos1.z();  // (cam1->position().z() - cam0->position().z());
    return height;
}
...
void InteractiveCameraHandler::update(const sibr::Input& input,
                                      float deltaTime,
                                      const sibr::Viewport& viewport) {
    if (!viewport.isEmpty()) {
        _viewport = viewport;
    ...
    } else if (input.key().isActivated(sibr::Key::LeftShift) &&
               input.key().isReleased(sibr::Key::Y)) {
        if (_currentMode == TRACKBALL) {
            _trackball.drawThis = !_trackball.drawThis;
            SIBR_LOG << "[Trackball] Display visual guides: "
                     << (_trackball.drawThis ? "on" : "off") << "."
                     << std::endl;
        }
    }
    // only free key
    else if (input.key().isReleased(sibr::Key::M)) {
        _cameraRecorder.saveImage("", _currentCamera, _currentCamera.w(),
                                  _currentCamera.h());
        ...
        if (input.key().isActivated(Key::LeftShift) &&
            (input.key().isActivated(Key::LeftAlt) ||
             input.key().isActivated(Key::LeftControl)) &&
            input.key().isReleased(Key::C)) {
            ...
            int w, h;
            for (uint i = 0; i < 10; ++i) std::cout << std::endl;
            std::cout << "Enter a filename for loading a camera path:"
                      << std::endl;
        ...
        } else if (input.key().isActivated(Key::LeftControl) &&
                   input.key().isReleased(Key::C)) {
            std::string filename;
            for (uint i = 0; i < 10; ++i) std::cout << std::endl;
            std::cout << "Enter a filename for saving a camera path:"
                      << std::endl;
            safeGetline(std::cin, filename);
            ...
            if (_fribrExport) {
                const int height =
                        int(std::floor(1920.0f / _currentCamera.aspect()));
                _cameraRecorder.saveAsFRIBRBundle(filename + "_fribr/", 1920,
                                                  height);
            }
            _cameraRecorder.stop();
        } else if (input.key().isActivated(Key::LeftAlt) &&
                   input.key().isReleased(Key::C)) {
            _cameraRecorder.reset();
            _cameraRecorder.record();
        } else if (input.key().isActivated(Key::RightAlt) &&
                   input.key().isReleased(Key::C)) {
            std::string filename;
            for (uint i = 0; i < 10; ++i) std::cout << std::endl;
            std::cout << "Enter a filename for saving a camera path:"
    ...
    if (!_cameraRecorder.isPlaying()) {
        switch (_currentMode) {
            ...
            default:
                ...
                if (_altitudeInterp) {
                    _fpsCamera.setGoalAltitude(
                            getInterpolatedHeight(_interpPath));
                } else {
        ...
        if (_shouldSmooth && _currentMode != INTERPOLATION) {
            const sibr::Camera newcam = sibr::Camera::interpolate(
                    _previousCamera, _currentCamera, IBRVIEW_SMOOTHCAM_POWER);
            _currentCamera = sibr::InputCamera(newcam, _currentCamera.w(),
                                               _currentCamera.h());
        }
    }

    // Note this call has three modes: record (only read the arg camera) |
    // playback (overwrite the arg camera) | do nothing (do nothing)
...
void InteractiveCameraHandler::onGUI(const std::string& suffix) {
    ...
    if (ImGui::Begin(fullName.c_str())) {
        ...
        ImGui::Combo("Mode", (int*)&_currentMode,
                     "FPS\0Orbit\0Interp.\0Trackball\0None\0\0");
        ...
        if (ImGui::Button("Load camera")) {
            ...
            if (sibr::showFilePicker(selectedFile, Default)) {
                if (!selectedFile.empty()) {
                    ...
                    SIBR_LOG << "Loaded saved camera (" << selectedFile << ")."
                             << std::endl;
                    fromCamera(savedCam, false);
                }
            }
        ...
        if (ImGui::Button("Save camera (bin)")) {
            ...
            if (sibr::showFilePicker(selectedFile, Save)) {
                if (!selectedFile.empty()) {
                    if (selectedFile[selectedFile.size() - 1] == '/' ||
                        selectedFile[selectedFile.size() - 1] == '\\') {
                        selectedFile.append("default_camera.bin");
                    }
                    _currentCamera.saveToBinary(selectedFile);
                    SIBR_LOG << "Saved camera (" << selectedFile << ")."
                             << std::endl;
                }
            }
        }
        ...
        if (ImGui::InputInt("Snap to", &_currentCamId, 1, 10)) {
            _currentCamId =
                    sibr::clamp(_currentCamId, 0, int(_interpPath.size()) - 1);
            snapToCamera(_currentCamId);
        }
        ...
        if (ImGui::Button("Save key cameras...")) {
            std::string outpath;
            if (sibr::showFilePicker(outpath, Save, "", "lookat") &&
                !outpath.empty()) {
    ...
    if (_supportRecording) {
        ...
        if (ImGui::Begin(fullName.c_str())) {
            ...
            if (ImGui::InputFloat("Speed##CamRecorder",
                                  &_cameraRecorder.speed(), 0.1f)) {
                _cameraRecorder.speed() =
                        sibr::clamp(_cameraRecorder.speed(), 0.0f, 1.0f);
            }
            ...
            if (ImGui::Button("Load path")) {
                if (sibr::showFilePicker(selectedFile, Default)) {
                    if (!selectedFile.empty()) {
                        ...
                        _cameraRecorder.reset();
                        if (boost::filesystem::extension(selectedFile) ==
                            ".out")
                            _cameraRecorder.loadBundle(selectedFile,
                                                       _currentCamera.w(),
                                                       _currentCamera.h());
                        else if (boost::filesystem::extension(selectedFile) ==
                                 ".lookat")
                            _cameraRecorder.loadLookat(selectedFile,
                                                       _currentCamera.w(),
                                                       _currentCamera.h());
                        else if (boost::filesystem::extension(selectedFile) ==
                                 ".txt")
                            _cameraRecorder.loadColmap(selectedFile,
                                                       _currentCamera.w(),
                                                       _currentCamera.h());
                        else
            ...
            if (ImGui::Button("Save path")) {
                ...
                if (sibr::showFilePicker(selectedFile, Save)) {
                    if (!selectedFile.empty()) {
                        ...
                        _cameraRecorder.saveAsBundle(selectedFile + ".out",
                                                     _currentCamera.h());
                        _cameraRecorder.saveAsColmap(selectedFile,
                                                     _currentCamera.h(),
                                                     _currentCamera.w());
                        _cameraRecorder.saveAsLookAt(selectedFile + ".lookat");
                        if (_fribrExport) {
                            const int height = int(std::floor(
                                    1920.0f / _currentCamera.aspect()));
                            _cameraRecorder.saveAsFRIBRBundle(
                                    selectedFile + "_fribr/", 1920, height);
                        }
                    }
                }
            }
