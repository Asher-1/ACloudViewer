...
TrackBall::TrackBall(bool _verbose)
    : hasBeenInitialized(false),
      shadersCompiled(false),
      state(TrackBallState::IDLE),
      verbose(_verbose),
      fixedCamera(InputCamera()),
      tempCamera(InputCamera()) {
    drawThis = true;
}

void TrackBall::update(const sibr::Input& input,
...
const InputCamera& TrackBall::getCamera(void) const {
    if (!hasBeenInitialized) {
        SIBR_ERR << " TrackBall : camera not initialized before use"
                 << std::endl
                 << "\t you should use either fromMesh(), fromCamera() or "
                    "load() "
                 << std::endl;
    }
...
void TrackBall::save(std::string& filePath) const {
    if (boost::filesystem::exists(filePath)) {
        char c;
        SIBR_LOG << " a track ball already exists, override ? y/n ... "
                 << std::flush;
        std::cin >> c;
        if (c != 'y') {
    ...
    if (file.is_open()) {
        ...
        file << fixedCamera.fovy() << " " << fixedCamera.znear() << " "
             << fixedCamera.zfar() << std::endl;
...
void TrackBall::fromCamera(const InputCamera& cam,
                           const Viewport& viewport,
                           const float& radius) {
    fixedCamera = cam;
...
bool TrackBall::fromBoundingBox(const Eigen::AlignedBox<float, 3>& box,
                                const Viewport& viewport) {
    if (box.isEmpty() || (box.diagonal().array() == 0.0f).any()) {
        SIBR_LOG << " [WARNING] TrackBall::fromMesh : cannot create camera "
                    "from flat mesh "
                 << std::endl;
        return false;
    } else {
...
void TrackBall::update(const Input& input,
                       const Viewport& viewport,
                       std::shared_ptr<Raycaster> raycaster) {
    if (!hasBeenInitialized || input.empty()) {
        return;
    }
...
void TrackBall::updateAspectWithViewport(const Viewport& viewport) {
    fixedCamera.size(static_cast<uint>(viewport.finalWidth()),
                     static_cast<uint>(viewport.finalHeight()));
    fixedCamera.aspect(viewport.finalHeight() / viewport.finalWidth());
}

void TrackBall::updateTrackBallStatus(const Input& input,
                                      const Viewport& viewport) {
    currentPoint2D = input.mousePosition();

    if (input.key().isActivatedOnly(Key::T) &&
        input.key().isActivatedOnly(Key::V)) {
        verbose = !verbose;
        if (verbose) {
    ...
    } else if (input.mouseButton().isReleased(Mouse::Right) ||
               input.mouseButton().isReleased(Mouse::Left)) {
        if (state != TrackBallState::IDLE) {
            state = TrackBallState::IDLE;
...
void TrackBall::updateTrackBallCamera(const Input& input,
                                      const Viewport& viewport,
                                      std::shared_ptr<Raycaster> raycaster) {
...
void TrackBall::updateBallCenter(const Input& input,
                                 std::shared_ptr<Raycaster> raycaster) {
    if (raycaster.get() == nullptr ||
        !input.mouseButton().isPressed(Mouse::Left)) {
        return;
    }
    ...
    if (fixedCamera.ortho()) {
        ...
        worldPos = fixedCamera.position() +
                   (2.0f * clickPos.x() / (float)fixedCamera.w() - 1.0f) *
                           fixedCamera.orthoRight() * fixedCamera.right() +
                   (2.0f * ((float)fixedCamera.h() - 1 - clickPos.y()) /
                            (float)fixedCamera.h() -
                    1.0f) * fixedCamera.orthoTop() *
                           fixedCamera.up();
        dir = fixedCamera.dir();

    } else {
        dir = CameraRaycaster::computeRayDir(
                      fixedCamera, input.mousePosition().cast<float>())
                      .normalized();
        worldPos = fixedCamera.position();
    }
    ...
    if (hit.hitSomething()) {
        printMessage(
                " TrackBall::updateBallCenter : updating center from mesh ");
        ...
        printMessage(
                " TrackBall::updateBallCenter : could not intersect mesh ");
    }
}

void TrackBall::updateRotationSphere(const Input& input,
                                     const Viewport& viewport) {
    if (!isInTrackBall2dRegion(input.mousePosition(), viewport) ||
        input.mousePosition() == lastPoint2D) {
        return;
    }
    ...
    Vector3f rotationAxisScreenSpace(
            (lastPointSphere.cross(newPointSphere)).normalized());
    Vector4f axis;
    axis << rotationAxisScreenSpace, 0.0f;
    Vector3f rotationAxisWorldSpace(
            (fixedCamera.view().inverse() * axis).xyz());

    float angleCos = newPointSphere.dot(lastPointSphere);
    if (std::abs(angleCos) < 1.0f) {
        float rotationAngle = -2.0f * acos(angleCos);
        Eigen::Quaternionf rot(
                Eigen::AngleAxisf(rotationAngle, rotationAxisWorldSpace));
...
void TrackBall::updateRotationRoll(const Input& input,
                                   const Viewport& viewport) {
    if (isInTrackBall2dRegion(input.mousePosition(), viewport)) {
        return;
    }
    ...
    Vector2f viewportCenter(
            0.5f * (viewport.finalLeft() + viewport.finalRight()),
            0.5f * (viewport.finalTop() + viewport.finalBottom()));
    float clockwise = (areClockWise(viewportCenter, lastPoint2D.cast<float>(),
                                    input.mousePosition().cast<float>())
                               ? -1.0f
                               : 1.0f);
    float diagonal =
            std::sqrt((float)(viewport.finalWidth() * viewport.finalWidth() +
                              viewport.finalHeight() * viewport.finalHeight()));
    float rollAngle = clockwise * (float)M_PI *
                      (float)(lastPoint2D - input.mousePosition()).norm() /
                      diagonal;

    Eigen::Quaternionf rot(
            Eigen::AngleAxisf(rollAngle, -fixedCamera.dir().normalized()));
    Vector3f newUp = rot * fixedCamera.up().normalized();

    tempCamera.setLookAt(fixedCamera.position(), fixedCenter, newUp);
}

void TrackBall::updateTranslationPlane(const Input& input,
                                       const Viewport& viewport,
                                       std::shared_ptr<Raycaster> raycaster) {
    if (!isInTrackBall2dRegion(input.mousePosition(), viewport)) {
        return;
    ...
    if (input.mouseButton().isPressed(Mouse::Right)) {
        ...
        trackballPlane = Eigen::Hyperplane<float, 3>(
                fixedCamera.dir().normalized(), pointOnPlane);
    }
    ...
    tempCamera.setLookAt(fixedCamera.position() + shift3D, tempCenter,
                         fixedCamera.up());
}

void TrackBall::updateTranslationZ(const Input& input,
                                   const Viewport& viewport) {
    if (isInTrackBall2dRegion(input.mousePosition(), viewport)) {
        return;
    }
    ...
    Vector2f shift2Df(
            shift2D.cast<float>().array() /
            Vector2f(viewport.finalWidth(), viewport.finalHeight()).array());

    int whichDir = (std::abs(shift2D.x()) > std::abs(shift2D.y()) ? 0 : 1);

    float shift = 4.0f * (fixedCenter - fixedCamera.position()).norm() *
                  (whichDir == 0 ? -1.0f : 1.0f) * shift2Df[whichDir];
...
void TrackBall::updateRadius(const Input& input) {
    if (input.key().getNumActivated() != 0) {
        return;
    }
    if (!fixedCamera.ortho()) {
...
void TrackBall::updateZnearZFar(const Input& input) {
    ...
    } else if (input.key().isActivatedOnly(sibr::Key::Z) &&
               input.key().isActivatedOnly(Key::LeftShift)) {
...
bool TrackBall::isInTrackBall2dRegion(const Vector2i& pos2D,
    ...
    return pos_x >= min_ratio && pos_x <= max_ratio && pos_y >= min_ratio &&
           pos_y <= max_ratio;
}

Vector3f TrackBall::mapToSphere(const Vector2i& pos2D,
                                const Viewport& viewport) const {
    int xMin = (int)0;
    ...
    Vector2i clampPos =
            pos2D.cwiseMin(Vector2i(xMax, yMax)).cwiseMax(Vector2i(xMin, yMin));
    ...
    return Vector3d(sinx, siny, sinx2siny2 < 1.0 ? sqrt(1.0 - sinx2siny2) : 0.0)
            .cast<float>();
}
...
Vector3f TrackBall::mapTo3Dplane(const Vector2i& pos2D) const {
    ...
    if (fixedCamera.ortho()) {
        worldPos = fixedCamera.position() +
                   (2.0f * pos2D.x() / (float)fixedCamera.w() - 1.0f) *
                           fixedCamera.orthoRight() * fixedCamera.right() +
                   (2.0f * ((float)fixedCamera.h() - 1 - pos2D.y()) /
                            (float)fixedCamera.h() -
                    1.0f) * fixedCamera.orthoTop() *
                           fixedCamera.up();
        dir = fixedCamera.dir();
        ...
        dir = CameraRaycaster::computeRayDir(fixedCamera, pos2D.cast<float>())
                      .normalized();
        worldPos = fixedCamera.position();
    }
...
bool TrackBall::areClockWise(const Vector2f& a,
                             const Vector2f& b,
                             const Vector2f& c) const {
...
void TrackBall::initTrackBallShader(void) {
    ...
    trackBallShader.init("trackBallShader", trackBallVertexShader,
                         trackBallFragmentShader);
