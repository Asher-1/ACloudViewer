...
void MultiViewBase::onUpdate(Input& input) {
    if (input.key().isActivated(Key::LeftControl) &&
        input.key().isPressed(Key::LeftAlt) && input.key().isPressed(Key::P)) {
        _onPause = !_onPause;
    }
    if (_onPause) {
        return;
    ...
    _deltaTime = (float)(std::chrono::duration_cast<std::chrono::microseconds>(
                                 timeNow - _timeLastFrame)
                                 .count()) /
                 1000000.0f;
    ...
    for (auto& subview : _subViews) {
        if (subview.second.view->active()) {
            auto subInput =
                    !subview.second.view->isFocused()
                            ? Input()
                            : Input::subInput(input, subview.second.viewport,
                                              false);

            if (subview.second.handler) {
                subview.second.handler->update(subInput, _deltaTime,
                                               subview.second.viewport);
            }

            subview.second.updateFunc(subview.second.view, subInput,
                                      subview.second.viewport, _deltaTime);
        }
    }
    ...
    for (auto& subview : _ibrSubViews) {
        ...
        if (fView.view->active()) {
            auto subInput =
                    !fView.view->isFocused()
                            ? Input()
                            : Input::subInput(input, fView.viewport, false);

            if (fView.handler) {
                fView.handler->update(subInput, _deltaTime, fView.viewport);
            }

            fView.cam = fView.updateFunc(fView.view, subInput, fView.viewport,
                                         _deltaTime);
...
void MultiViewBase::addSubView(const std::string& title,
                               ViewBase::Ptr view,
                               const Vector2u& res,
                               const ImGuiWindowFlags flags) {
    const ViewUpdateFunc updateFunc = [](ViewBase::Ptr& vi, Input& in,
                                         const Viewport& vp, const float dt) {
        vi->onUpdate(in, vp);
    };
...
void MultiViewBase::addSubView(const std::string& title,
    ...
    const Viewport viewport(
            ...
            (res.y() > 0 ? res.y() : (float)_defaultViewResolution.y()) +
                    titleBarHeight);
    RenderTargetRGB::Ptr rtPtr(new RenderTargetRGB((uint)viewport.finalWidth(),
                                                   (uint)viewport.finalHeight(),
                                                   SIBR_CLAMP_UVS));
    _subViews[title] = {view, rtPtr, viewport, title, flags, updateFunc};
}

void MultiViewBase::addIBRSubView(const std::string& title,
                                  ViewBase::Ptr view,
    ...
    if (_ibrSubViews.count(title) > 0) {
        ...
        _ibrSubViews[title] = {view,  rtPtr,      viewport,       title,
                               flags, updateFunc, defaultFuncUsed};
...
void MultiViewBase::addIBRSubView(const std::string& title,
                                  ...
                                  const Vector2u& res,
                                  const ImGuiWindowFlags flags) {
    const auto updateFunc = [](ViewBase::Ptr& vi, Input& in, const Viewport& vp,
                               const float dt) {
        vi->onUpdate(in, vp);
        return InputCamera();
    };
...
void MultiViewBase::addSubMultiView(const std::string& title,
                                    MultiViewBase::Ptr multiview) {
...
Viewport& MultiViewBase::getIBRSubViewport(const std::string& title) {
    ...
    SIBR_ERR << " No subviewport with name <" << title << "> found."
             << std::endl;
...
void MultiViewBase::renderSubView(SubView& subview) {
    if (!_onPause) {
        ...
        // Offline video dumping, continued. We ignore additional rendering as
        // those often are GUI overlays.
        if (subview.handler != NULL &&
            (subview.handler->getCamera().needVideoSave() ||
        ...
        subview.renderFunc(subview.view, renderViewport,
                           std::static_pointer_cast<IRenderTarget>(subview.rt));
    ...
    if (_enableGUI) {
        ...
        subview.view->setFocus(showImGuiWindow(
                subview.view->name(), *subview.rt, subview.flags,
                subview.viewport, false, subview.shouldUpdateLayout));
        ImGui::PopStyleVar();
    }
...
const Viewport MultiViewBase::getViewport(void) const {
    return Viewport(0.0f, 0.0f, (float)_defaultViewResolution.x(),
                    (float)_defaultViewResolution.y());
}

void MultiViewBase::addCameraForView(const std::string& name,
                                     ICameraHandler::Ptr cameraHandler) {
...
void MultiViewBase::addAdditionalRenderingForView(
        const std::string& name, const AdditionalRenderFunc renderFunc) {
    if (_subViews.count(name) > 0) {
...
int MultiViewBase::numSubViews() const {
    return static_cast<int>(_subViews.size() + _ibrSubViews.size() +
                            _subMultiViews.size());
}

void MultiViewBase::captureView(const std::string& subviewName,
    ...
    if (_subViews.count(subviewName)) {
        ...
        SIBR_WRG << "No View in the MultiViewManager with " << subviewName
                 << " as a name!" << std::endl;
    }
}

void MultiViewBase::captureView(const SubView& view,
                                const std::string& path,
                                const std::string& filename) {
    ...
    if (!filename.empty()) {
        ...
        const std::string autoName =
                view.view->name() + "_" + sibr::timestamp();
...
void MultiViewBase::mosaicLayout(const Viewport& vp) {
    ...
    Viewport usedVP = Viewport(vp.finalLeft(), vp.finalTop() + verticalShift,
                               vp.finalRight(), vp.finalBottom());
    ...
    for (auto& view : _ibrSubViews) {
        ...
        view.second.viewport =
                Viewport(usedVP, col * itemRatio[0], row * itemRatio[1],
                         (col + 1) * itemRatio[0], (row + 1) * itemRatio[1]);
...
MultiViewBase::SubView::SubView(ViewBase::Ptr view_,
                                RenderTargetRGB::Ptr rt_,
                                const sibr::Viewport viewport_,
    ...
    renderFunc = [](ViewBase::Ptr&, const Viewport&,
                    const IRenderTarget::Ptr&) {};
    view->setName(name_);
}

MultiViewBase::BasicSubView::BasicSubView(ViewBase::Ptr view_,
                                          RenderTargetRGB::Ptr rt_,
                                          const sibr::Viewport viewport_,
                                          const std::string& name_,
                                          const ImGuiWindowFlags flags_,
...
void MultiViewBase::BasicSubView::render(const IRenderingMode::Ptr& rm,
                                         const Viewport& renderViewport) const {
    rt->bind();
    renderViewport.bind();
...
MultiViewBase::IBRSubView::IBRSubView(ViewBase::Ptr view_,
                                      RenderTargetRGB::Ptr rt_,
                                      const sibr::Viewport viewport_,
                                      const std::string& name_,
                                      const ImGuiWindowFlags flags_,
                                      IBRViewUpdateFunc f_,
                                      const bool defaultUpdateFunc_)
    : SubView(view_, rt_, viewport_, name_, flags_),
      updateFunc(f_),
      defaultUpdateFunc(defaultUpdateFunc_) {
    cam = sibr::InputCamera();
}

void MultiViewBase::IBRSubView::render(const IRenderingMode::Ptr& rm,
                                       const Viewport& renderViewport) const {
    if (rm) {
...
MultiViewManager::MultiViewManager(Window& window, bool resize)
    ...
    if (resize) {
        window.size(Window::desktopSize().x() - 200,
    ...
    if (_showGUI && ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("Menu")) {
            ...
            if (ImGui::MenuItem("Row layout")) {
                ...
                itemSize[0] = std::round(
                        float(itemSize[0]) /
                        float(_subViews.size() + _ibrSubViews.size()));
                const float verticalShift = ImGui::GetTitleBarHeight();
                float vid = 0.0f;
                for (auto& view : _ibrSubViews) {
                    // Compute position on grid.
                    view.second.viewport =
                            Viewport(vid * itemSize[0], verticalShift,
        ...
        if (ImGui::BeginMenu("Views")) {
            ...
            if (ImGui::BeginMenu("Front when focus")) {
                for (auto& subview : _subViews) {
                    const bool isLockedInBackground =
                            subview.second.flags &
                            ImGuiWindowFlags_NoBringToFrontOnFocus;
                    if (ImGui::MenuItem(subview.first.c_str(), "",
                                        !isLockedInBackground)) {
        ...
        if (ImGui::BeginMenu("Capture")) {
            if (ImGui::MenuItem("Set export directory...")) {
                ...
                if (showFilePicker(selectedDirectory,
            ...
            if (ImGui::MenuItem("Export Video")) {
                ...
                if (showFilePicker(saveFile, FilePickerMode::Save)) {
                    ...
                    if (!_videoFrames.empty()) {
                        SIBR_LOG << "Exporting video to : " << outputVideo
                                 << " ..." << std::flush;
                        FFVideoEncoder vdoEncoder(
                                outputVideo, 30,
                                Vector2i(_videoFrames[0].cols,
                                         _videoFrames[0].rows));
                        ...
                        SIBR_WRG << "No frames to export!! Check save frames "
                                    "in camera options for the view you want "
                                    "to render and play the path and re-export!"
                                 << std::endl;
                    }
                }
            }
...
void MultiViewManager::toggleGUI() {
    _showGUI = !_showGUI;
    if (!_showGUI) {
        SIBR_LOG << "[MultiViewManager] GUI is now hidden, use Ctrl+Alt+G to "
                    "toggle it back on."
                 << std::endl;
    }
    toggleSubViewsGUI();
