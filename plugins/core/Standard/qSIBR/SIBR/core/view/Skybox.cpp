...
bool Skybox::load(const std::string& skyFolder) {
    if (!sibr::directoryExists(skyFolder)) return false;
    ...
    std::array<const char*, 6> filenames = {"right.jpg",   "left.jpg",
                                            "top.jpg",     "bottom.jpg",
                                            "forward.jpg", "back.jpg"};
    ...
    for (uint i = 0; i < filenames.size(); ++i) {
        ...
        if (images[i].load(file) == false) {
            SIBR_ERR << "cannot open " << file << " (loading the skybox)"
                     << std::endl;
        }
    }
    _cubemap.reset(new TextureCubeMapRGB(images[0], images[1], images[2],
...
void Skybox::render(const Camera& eye, const sibr::Vector2u& imgSize) {
    ...
    _shader.begin();
    CHECK_GL_ERROR;
    _paramAspect.set(Vector2f(float(imgSize.x()) / float(imgSize.y()),
                              float(imgSize.y()) / float(imgSize.x())));
    CHECK_GL_ERROR;
