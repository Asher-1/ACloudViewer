...
void FPSCounter::init(const sibr::Vector2f& position) { _position = position; }
...
void FPSCounter::render() {
    ...
    if (_position.x() != -1) {
        ...
        ImGui::SetNextWindowSize(ImVec2(0, ImGui::GetTitleBarHeight()),
...
void FPSCounter::update(bool doRender) {
    ...
    float deltaTime =
            std::chrono::duration<float>(now - _lastFrameTime).count();
    update(deltaTime);
    if (doRender) {
        render();
    }
