...
ImageView::ImageView(bool interactiveMode) {
    ...
    _showChannels[0] = _showChannels[1] = _showChannels[2] = _showChannels[3] =
            true;
    _bgColor = {0.25f, 0.25f, 0.25f};
    _pos = {0.0f, 0.0f};
    _scale = 1.0f;
    // When in "fixed" mode, don't respect the aspect ratio, to make sure that
    // the full image is visible to the viewer.
...
void ImageView::onUpdate(Input& input, const Viewport& vp) {
    ...
    if (input.mouseButton().isActivated(Mouse::Left)) {
        sibr::Vector2f delta =
                input.mouseDeltaPosition().cast<float>().cwiseQuotient(
                        vp.finalSize());
        delta[1] *= -1.0f;
        _pos = _pos.get() + delta;
    }
...
void ImageView::onGUI() {
    ...
    if (ImGui::Begin(guiName.c_str())) {
        ImGui::Text("Size: %dx%d. Scale: %.2f%%", int(_size.get()[0]),
                    int(_size.get()[1]), 100.0f * _scale);
        ...
        ImGui::Text("Channels");
        ImGui::SameLine();
        ImGui::Checkbox("R", &_showChannels[0]);
        ImGui::SameLine();
        ImGui::Checkbox("G", &_showChannels[1]);
        ImGui::SameLine();
        ImGui::Checkbox("B", &_showChannels[2]);
        ImGui::SameLine();
        ...
        if (_lockChannels) {
            // Only display one value and ensure synchronisation between the RGB
            // components.
            editBounds =
                    ImGui::DragFloat("Min.", &_minVal.get()[0], dragSpeed) ||
                    editBounds;
            editBounds =
                    ImGui::DragFloat("Max.", &_maxVal.get()[0], dragSpeed) ||
                    editBounds;
        } else {
            editBounds =
                    ImGui::DragFloat4("Min.", &_minVal.get()[0], dragSpeed) ||
                    editBounds;
            editBounds =
                    ImGui::DragFloat4("Max.", &_maxVal.get()[0], dragSpeed) ||
                    editBounds;
        }
        // Ensure internal state consistency.
        if (editBounds && _lockChannels) {
            _minVal.get()[3] = _minVal.get()[2] = _minVal.get()[1] =
                    _minVal.get()[0];
            _maxVal.get()[3] = _maxVal.get()[2] = _maxVal.get()[1] =
                    _maxVal.get()[0];
        }
        // Ensure ordering.
        if (editBounds) {
...
void ImageView::setRenderTarget(const IRenderTarget& rt, uint handle) {
    ...
    _size.get()[0] = float(rt.w());
    _size.get()[1] = float(rt.h());
}

void ImageView::setTexture(const ITexture2D& tex) {
    _tex = nullptr;
    _texHandle = tex.handle();
    _size.get()[0] = float(tex.w());
    _size.get()[1] = float(tex.h());
}
