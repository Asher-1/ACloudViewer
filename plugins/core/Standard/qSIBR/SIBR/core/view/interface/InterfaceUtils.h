 ...
 * \ingroup sibr_view
 */
template <UVspace space>
struct UV : public sibr::Vector2f {
    ...
    template <UVspace otherSpace>
    operator UV<otherSpace>() const;
};
...
template <>
template <>
inline UV11::operator UV01() const {
    return UV01(0.5f * x() + 1, 0.5f * y() + 1);
}

template <>
template <>
inline UV01::operator UV11() const {
    return UV11(2.0f * x() - 1, 2.0f * y() - 1);
}

template <>
template <>
inline UV01::operator UV10() const {
    return UV10(x(), 1.0f - y());
}

template <>
template <>
inline UV10::operator UV01() const {
    return UV01(x(), 1.0f - y());
}

template <>
template <>
inline UV10::operator UV11() const {
    return UV11(UV01(*this));
}

template <>
template <>
inline UV11::operator UV10() const {
...
public:
    ...
    void rectangle(const sibr::Vector3f& color,
                   const UV11& tl,
                   const UV11& br,
                   bool fill,
                   float alpha);
    void rectanglePixels(const sibr::Vector3f& color,
                         const sibr::Vector2f& center,
                         const sibr::Vector2f& diagonal,
                         bool fill,
                         float alpha,
                         const sibr::Vector2f& winSize);
    void circle(const sibr::Vector3f& color,
                const UV11& center,
                float radius,
                bool fill,
                float alpha,
                const sibr::Vector2f& scaling = sibr::Vector3f(1, 1),
                int precision = 50);
    void circlePixels(const sibr::Vector3f& color,
                      const sibr::Vector2f& center,
                      float radius,
                      bool fill,
                      float alpha,
                      const sibr::Vector2f& winSize,
                      int precision = 50);
    void linePixels(const sibr::Vector3f& color,
                    const sibr::Vector2f& ptA,
                    const sibr::Vector2f& ptB,
                    const sibr::Vector2f& winSize);
...
struct DoubleClick {
    ...
    bool detected(const sibr::Input& input, bool should_be_close = true) {
        if (input.mouseButton().isPressed(mKey)) {
            // std::cout << "timer.deltaTimeFromLastTic<sibr::Timer::s>() : " <<
            // timer.deltaTimeFromLastTic<>() << std::endl;
            if (timer.deltaTimeFromLastTic<>() < detection_timing_in_ms &&
                (!should_be_close ||
                 (firstPosition - input.mousePosition()).cwiseAbs().maxCoeff() <
                         10)) {
                return true;
            }
...
static void show(const sibr::Texture2DArray<T_Type, T_NumComp>& texArray,
    ...
    if (inChargeOfWindow) {
        sibr::Vector2i winSize = (useCustomSize ? sibr::Vector2i(w, h)
                                                : sibr::Vector2i(1600, 1200));
...
// public:
//	static void rectangle(const sibr::Vector3f & color, const UV11 & tl,
// const UV11 & br, bool fill, float alpha); 	static void circle(const
// sibr::Vector3f & color, const UV11 & center, float radius, bool fill, float
// alpha, int precision = 50);
// };
