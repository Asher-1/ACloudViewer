...
MultiViewInterface::MultiViewInterface() {
    ...
    imagesViewBase = std::make_shared<MultiViewInterfaceView>(
            this, MultiViewInterfaceView::ViewType::IMAGES);
    meshViewBase = std::make_shared<MultiViewInterfaceView>(
            this, MultiViewInterfaceView::ViewType::MESH);
}

void MultiViewInterface::displayLoop(
        sibr::Window& window, std::function<void(MultiViewInterface*)> f) {
    if (layersData.size() == 0) {
        SIBR_ERR << " cant display interface without image layer added"
                 << std::endl;
    }
    ...
    window.size(window.size().x(),
                (int)std::ceil(window.size().x() / scalesData[0].imRatio));
    ...
    if (cpuMesh.get()) {
        meshViewer.setMainMesh(*cpuMesh, sibr::Mesh::FillRenderMode, false,
                               true);
    } else {
    ...
    while (window.isOpened()) {
        ...
        winSize = window.size().cast<float>();
        imagesInput = sibr::Input::subInput(sibr::Input::global(),
                                            imagesView.viewport);
        meshInput =
                sibr::Input::subInput(sibr::Input::global(), meshView.viewport);
...
void MultiViewInterface::addCameras(
        const std::vector<InputCamera::Ptr>& input_cams) {
    cams = input_cams;
}
...
void MultiViewInterface::update(sibr::Window& window,
                                const sibr::Input& input) {
    updateImageView(imagesView.viewport, imagesInput);
    updateMeshView(meshInput, window);
}

void MultiViewInterface::updateImageView(const sibr::Viewport& viewport,
                                         const sibr::Input& input) {
    ...
    imgPixelScreenSize = screenPosPixelsFloat({0, {1, 1}}, winSize) -
                         screenPosPixelsFloat({0, {0, 0}}, winSize);
...
void MultiViewInterface::render() {
    ...
    //	int r = 50;
    //	utils.rectanglePixels(sibr::Vector3f(1, 0, 1),
    // sibr::Input::global().mousePosition().cast<float>(), sibr::Vector2f(r,
    // r), true, 0.15f, imagesViewSize);
    // utils.circlePixels(sibr::Vector3f(0, 1, 1),
    // sibr::Input::global().mousePosition().cast<float>(), r, true, 0.15f,
    // imagesViewSize);
    //
    // }
}
...
sibr::ViewBase::Ptr MultiViewInterface::getViewBase(
        MultiViewInterfaceView::ViewType type) {
...
void MultiViewInterface::onGui() {
    ...
    if (imagesLayers.size() != 1) {
        ImGui::SliderInt("Laplacian scale", &currentScale, 0,
                         (int)imagesLayers.size() - 1);
        ImGui::Separator();
    }
    ...
    if (nLayers > 1) {
        ...
        for (size_t n = 0; n < nLayers; ++n) {
            if (ImGui::Selectable(layersData[n].name.c_str(),
                                  currentLayer == n)) {
                currentLayer = (int)n;
            }
        }
    ...
    if (currentScale == 0 && currentActivePos.isDefined) {
        std::stringstream ss;
        ss << "Image : " << currentActivePos.im
           << ", pixel : " << currentActivePos.pos << std::endl;
        ImGui::Text(ss.str().c_str());
        if (currentActivePos.isDefined) {
            // std::cout << imagesPtr[currentLayer][currentActivePos.im] <<
            // std::endl; std::cout <<
            // imagesPtr[currentLayer][currentActivePos.im]->size() <<
            // std::endl;
            /*				if
               (imagesPtr[currentLayer][currentActivePos.im]) {
...
PixPos MultiViewInterface::pixFromScreenPos(const sibr::Vector2i& posScreen,
                                            const sibr::Vector2f& winSize) {
    UV01 uvScreen =
            UV10::from((posScreen.cast<float>() + 0.5f * sibr::Vector2f(1, 1))
                               .cwiseQuotient(winSize));

    // std::cout << uvScreen.transpose() << std::endl;

    sibr::Vector2f posF =
            viewRectangle.tl() +
            (viewRectangle.br() - viewRectangle.tl()).cwiseProduct(uvScreen);
    posF.y() = 1.0f - posF.y();
...
UV01 MultiViewInterface::screenPos(const PixPos& pix) {
    sibr::Vector2f pos = (pix.pos.cast<float>().cwiseQuotient(
                                  scalesData[currentScale].imSize) +
                          sibr::Vector2f(pix.im % grid.x(), pix.im / grid.x()))
                                 .cwiseQuotient(grid.cast<float>());
    pos.y() = 1.0f - pos.y();
    return UV01::from(
            (pos - viewRectangle.tl())
                    .cwiseQuotient(viewRectangle.br() - viewRectangle.tl()));
}

UV01 MultiViewInterface::screenPosPixelCenter(const PixPos& pix) {
    sibr::Vector2f pos =
            ((pix.pos.cast<float>() + sibr::Vector2f(0.5, 0.5))
                     .cwiseQuotient(scalesData[currentScale].imSize) +
             sibr::Vector2f(pix.im % grid.x(), pix.im / grid.x()))
                    .cwiseQuotient(grid.cast<float>());
    pos.y() = 1.0f - pos.y();
    return UV01::from(
...
sibr::Vector2i MultiViewInterface::screenPosPixels(
        const PixPos& pix, const sibr::Vector2f& winSize) {
    return (screenPos(pix).cwiseProduct(winSize)).cast<int>();
}

sibr::Vector2f MultiViewInterface::screenPosPixelsFloat(
        const PixPos& pix, const sibr::Vector2f& winSize) {
    return screenPosPixelCenter(pix).cwiseProduct(winSize);
}

// void MultiViewInterface::setupFromImSizeAndNumIm(LayerData & layerData, const
// sibr::Vector2i & imSize)
//{
...
void MultiViewInterface::addHighlightPixel(const PixPos& pix,
                                           const sibr::Vector2f& winSize) {
...
void MultiViewInterface::highlightPixel(const PixPos& pix,
                                        const sibr::Viewport& viewport,
                                        const sibr::Vector3f& color,
                                        const sibr::Vector2f& pixScreenSize) {
    ...
    if ((pixBR - pixTl)
                .cwiseProduct(viewport.finalSize())
                .cwiseAbs()
                .minCoeff() < 2.0f) {
        // if pixel size in screen space is tinier than 2 screen pix
        utils.rectanglePixels(
                color,
                0.5f * (pixTl + pixBR).cwiseProduct(viewport.finalSize()),
                pixScreenSize, true, 0.15f, viewport.finalSize());
    } else {
...
void MultiViewInterface::displayImages(const sibr::Viewport& viewport) {
    // const sibr::Vector2f & winSize;
    // std::cout << imagesView.viewport.left() << " " <<
    // imagesView.viewport.right() << " " << imagesView.viewport.top() << " " <<
    // imagesView.viewport.bottom() << std::endl;
    viewport.bind();
    ...
    utils.numImgsGL.set((int)imagesLayers[currentScale][currentLayer]->depth() -
                        1);
    ...
    glBindTexture(GL_TEXTURE_2D_ARRAY,
                  imagesLayers[currentScale][currentLayer]->handle());
    sibr::RenderUtility::renderScreenQuad();

    utils.multiViewShader.end();

    for (int i = 0; i < (int)imagesLayers[currentScale][currentLayer]->depth();
         ++i) {
        UV01 imTl = screenPos(PixPos(i, sibr::Vector2i(0, 0)));
        UV01 imBR = screenPos(
                PixPos(i, scalesData[currentScale].imSize.cast<int>()));
        utils.rectangle(sibr::Vector3f(0, 0, 0), imTl, imBR, false, 1.0);
    }

    // utils.rectangle(sibr::Vector3f(0, 1, 0), UV01(0.25,0.25), UV01(0.75,
    // 0.75), true, 0.15); utils.circle(sibr::Vector3f(0, 0, 1), UV01(0.33,
    // 0.33), 0.1, true, 0.25);
    ...
    if (reproMesh.get()) {
        ...
        sibr::Vector2i viewPortSize(imagesView.viewport.finalWidth(),
                                    imagesView.viewport.finalHeight());
        ...
        // std::cout << " winbox " <<
        // (winBox.center()-0.5*winBox.diagonal()).transpose() << " " <<
        // (winBox.center() + 0.5*winBox.diagonal()).transpose() << std::endl;
        for (int i = 0; i < (int)cams.size(); ++i) {
            ...
            sibr::Vector2i tlImgPix = screenPosPixels(
                    PixPos(i, sibr::Vector2i(0, cams[i]->h() - 1)),
                    viewPortSize.cast<float>());
            sibr::Vector2i brImgPix = screenPosPixels(
                    PixPos(i, sibr::Vector2i(cams[i]->w() - 1, 0)),
                    viewPortSize.cast<float>());

            Eigen::AlignedBox2d box;
            box.extend(tlImgPix.cast<double>());
            box.extend(brImgPix.cast<double>());
            // std::cout << "\t box " << (box.center() -
            // 0.5*box.diagonal()).transpose() << " " << (box.center() +
            // 0.5*box.diagonal()).transpose() << std::endl;
            ...
            // tlImgPix =
            // renderBox.corner(Eigen::AlignedBox2d::CornerType::BottomLeft).cast<int>();
            //	brImgPix =
            // renderBox.corner(Eigen::AlignedBox2d::CornerType::TopRight).cast<int>();
            // std::cout << "\t renderBox " << (renderBox.center() -
            // 0.5*renderBox.diagonal()).transpose() << " " <<
            // (renderBox.center() + 0.5*renderBox.diagonal()).transpose() <<
            // std::endl;

            // std::cout << tlImgPix.x() << " " << tlImgPix.y() << " " <<
            // (brImgPix - tlImgPix).x() << " " << (brImgPix - tlImgPix).y() <<
            // std::endl;
            glViewport(tlImgPix.x(), tlImgPix.y(),
                       std::abs((brImgPix - tlImgPix).x()),
                       std::abs((brImgPix - tlImgPix).y()));
...
void MultiViewInterface::displayZoom(const sibr::Viewport& viewport) {
    if (zoomSelection.isActive) {
        viewport.bind();
        UV01 tl = UV01::from(zoomSelection.first.cast<float>().cwiseQuotient(
                viewport.finalSize()));
        UV01 br = UV01::from(zoomSelection.second.cast<float>().cwiseQuotient(
                viewport.finalSize()));
        utils.rectangle(sibr::Vector3f(1, 0, 0), tl, br, true, 0.15f);
    }
}

void MultiViewInterface::displayHighlightedPixels(const sibr::Vector3f& color,
                                                  float alpha) {
    if (!highlightedPixelsMesh.get()) {
        return;
    }
...
void MultiViewInterface::updateMeshView(const sibr::Input& input,
                                        sibr::Window& window) {
    if (meshView.isActive) {
        ...
        if (sibr::Input::global().key().isPressed(sibr::Key::Left)) {
            ...
            imagesView.viewport =
                    sibr::Viewport(&window.viewport(), 0, 0, 1, 1);
        ...
        if (sibr::Input::global().key().isPressed(sibr::Key::Right)) {
            ...
            window.size(oldWinSize.x() * 2, oldWinSize.y());
            meshView.viewport =
                    sibr::Viewport(&window.viewport(), 0.5, 0, 1, 1);
            imagesView.viewport =
                    sibr::Viewport(&window.viewport(), 0, 0, 0.5, 1);
...
void MultiViewInterface::updateZoomBox(const sibr::Input& input,
                                       const sibr::Vector2f& winSize) {
    ...
    if (input.mouseButton().isPressed(sibr::Mouse::Code::Right) &&
        !zoomSelection.isActive) {
        zoomSelection.isActive = true;
        zoomSelection.first = input.mousePosition();
        zoomSelection.first.y() =
                (int)winSize.y() - zoomSelection.first.y() - 1;
    }
    if (input.mouseButton().isActivated(sibr::Mouse::Code::Right) &&
        zoomSelection.isActive) {
        zoomSelection.second = input.mousePosition();
        zoomSelection.second.y() =
                (int)winSize.y() - zoomSelection.second.y() - 1;
    }
    if (input.mouseButton().isReleased(sibr::Mouse::Code::Right) &&
        zoomSelection.isActive) {
        sibr::Vector2f currentTL =
                (zoomSelection.first.cwiseMin(zoomSelection.second))
                        .cast<float>();
        sibr::Vector2f currentBR =
                (zoomSelection.first.cwiseMax(zoomSelection.second))
                        .cast<float>();

        if (((currentBR - currentTL).array() > sibr::Vector2f(10, 10).array())
                    .all()) {
            sibr::Vector2f tlPix = viewRectangle.tl().cwiseProduct(winSize) +
                                   (viewRectangle.br() - viewRectangle.tl())
                                           .cwiseProduct(currentTL);
            sibr::Vector2f brPix = viewRectangle.tl().cwiseProduct(winSize) +
                                   (viewRectangle.br() - viewRectangle.tl())
                                           .cwiseProduct(currentBR);
...
void MultiViewInterface::updateCurrentLayer(const sibr::Input& input) {
    ...
    std::vector<sibr::Key::Code> keys = {
            sibr::Key::Num1, sibr::Key::Num2, sibr::Key::Num3,
            sibr::Key::Num4, sibr::Key::Num5, sibr::Key::Num6,
            sibr::Key::Num7, sibr::Key::Num8, sibr::Key::Num9};
...
void MultiViewInterface::updateCenter(const sibr::Input& input,
                                      const sibr::Vector2f& winSize) {
    if (dclick.detected(input)) {
        // std::cout << "dclick : " << std::endl;
        sibr::Vector2f translation =
                (dclick.firstPosition.cast<float>().cwiseQuotient(winSize) -
                 sibr::Vector2f(0.5, 0.5))
                        .cwiseProduct(viewRectangle.br() - viewRectangle.tl());
        translation.y() = -translation.y();
        viewRectangle.center += translation;
    }
}

void MultiViewInterface::updateDrag(const sibr::Input& input,
                                    const sibr::Vector2f& winSize) {
    if (input.mouseButton().isPressed(sibr::Mouse::Left)) {
        ...
        drag.center = viewRectangle.center;
               ...
               input.mouseButton().isReleased(sibr::Mouse::Left)) {
        drag.isActive = false;
    }
    if (drag.isActive && input.mouseButton().isActivated(sibr::Mouse::Left)) {
        sibr::Vector2f translation =
                (input.mousePosition() - drag.position)
                        .cast<float>()
                        .cwiseQuotient(winSize)
                        .cwiseProduct(viewRectangle.br() - viewRectangle.tl());
        translation.y() = -translation.y();
...
void MultiViewInterfaceView::onUpdate(Input& input,
                                      const sibr::Viewport& viewport) {
