    ...
    struct MeshParams {
        MeshParams()
            : depthTest(true),
              ...
              color(sibr::Vector3f(0.7f, 0.7f, 0.7f)) {}
        std::shared_ptr<sibr::Mesh> mesh;
...
public:
    ...
    void addMesh(std::shared_ptr<sibr::Mesh> meshPtr,
                 sibr::Mesh::RenderMode mode = sibr::Mesh::FillRenderMode);

    void addLines(const std::vector<sibr::Vector3f>& listPoints,
                  const sibr::Vector3f& color);

    void addPoint(const sibr::Vector3f& point, const sibr::Vector3f& color);
    void addPoints(const std::vector<sibr::Vector3f>& listPoints,
                   const sibr::Vector3f& color);
    void cleanPoints();
    void cleanLines();
...
public:
    ...
    MeshViewer(const sibr::Vector2i& screenRes,
    ...
    void renderLoop(
            const std::function<void(MeshViewer*)>& f = [](MeshViewer* m) {},
