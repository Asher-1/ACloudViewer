...
const std::string InterfaceUtilities::multiViewFragmentShader =
        ...
        "(float(numImgs)/grid.x) + 1) { discard; } else { \n"
...
void InterfaceUtilities::initMultiViewShader() {
    multiViewShader.init("InterfaceUtilitiesMultiViewShader",
...
void InterfaceUtilities::initMeshViewShader() {
    meshViewShader.init("InterfaceUtilitiesMeshViewShader", meshVertexShader,
...
void InterfaceUtilities::rectangle(const sibr::Vector3f& color,
                                   const UV11& tl,
                                   const UV11& br,
                                   bool fill,
                                   float alpha) {
...
void InterfaceUtilities::rectanglePixels(const sibr::Vector3f& color,
...
void InterfaceUtilities::circle(const sibr::Vector3f& color,
                                const UV11& center,
                                float radius,
                                bool fill,
                                float alpha,
                                const sibr::Vector2f& scaling,
                                int precision) {
    static int n;
    ...
    bool updateMeshes =
            (lastContextId != sibr::Window::contextId) || (n != precision);
    if (updateMeshes) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            vertices[i] = sibr::Vector3f((float)cos(angle), (float)sin(angle),
                                         (float)0.0);
        }
...
void InterfaceUtilities::circlePixels(const sibr::Vector3f& color,
                                      const sibr::Vector2f& center,
                                      float radius,
                                      bool fill,
                                      float alpha,
    ...
    sibr::Vector2f scaling =
            radius * sibr::Vector2f(1, 1).cwiseQuotient(winSize);

    circle(color, centerUV, 1.0f, fill, alpha, scaling, precision);
}

void InterfaceUtilities::linePixels(const sibr::Vector3f& color,
    ...
    line.triangles({sibr::Vector3u(0, 0, 1)});
