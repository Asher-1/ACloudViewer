...
const std::string sibr::MeshRenderer::meshFragmentShader =
        ...
        "			\n"
        "	out_color = vec4((1.0 - kd -ks)*forcedColor + kd*diffuse + "
        "ks*specular , 1.0);	 	\n"
        "}								"
        "								"
        "					\n";
...
void sibr::MeshRenderer::render(const sibr::Camera& eye) {
    ...
    for (auto& meshParam : listMeshes) {
        if (meshParam.mode == sibr::Mesh::LineRenderMode) {
            ...
            meshParam.mesh->render(meshParam.depthTest, false,
                                   sibr::Mesh::LineRenderMode);
            shaderLines.end();
        } else {
            shaderMesh.begin();
            ...
            meshParam.mesh->render(meshParam.depthTest,
                                   meshParam.backFaceCulling, meshParam.mode);
            shaderMesh.end();
        }
    }
...
void sibr::MeshRenderer::addMesh(std::shared_ptr<sibr::Mesh> meshPtr,
...
void sibr::MeshRenderer::addLines(const std::vector<sibr::Vector3f>& listPoints,
...
void sibr::MeshRenderer::addPoint(const sibr::Vector3f& point,
                                  const sibr::Vector3f& color) {
...
void sibr::MeshRenderer::addPoints(
        const std::vector<sibr::Vector3f>& list_points,
        const sibr::Vector3f& color) {
    std::vector<sibr::Vector3f> colors(list_points.size(), color);

    points.points.reserve(points.points.size() + list_points.size());
    points.points.insert(points.points.end(), list_points.begin(),
                         list_points.end());
...
void sibr::MeshRenderer::resetMeshes() { listMeshes.resize(0); }
...
void sibr::MeshRenderer::updateMeshPoints(void) {
    ...
    for (int vertex_id = 0; vertex_id < (int)points.points.size();
         vertex_id++) {
        for (int c = 0; c < 3; ++c) {
...
void sibr::MeshRenderer::updateMeshLines(void) {
    ...
    for (int vertex_id = 0; vertex_id < (int)lines.points.size();
         vertex_id += 2) {
        for (int c = 0; c < 3; ++c) {
...
sibr::MeshViewer::MeshViewer() {
    ...
    interactCam->switchMode(
            sibr::InteractiveCameraHandler::InteractionMode::TRACKBALL);
    inChargeOfWindow = false;
    fpsCounter.init(sibr::Vector2f(10, 10));
}

sibr::MeshViewer::MeshViewer(const sibr::Vector2i& screenRes,
                             const sibr::Mesh& mesh,
                             bool launchRenderingLoop) {
    window.reset(new Window(screenRes[0], screenRes[1], "MeshViewer"));
    renderer = std::make_shared<MeshRenderer>();
    interactCam = std::make_shared<sibr::InteractiveCameraHandler>(
            new sibr::InteractiveCameraHandler());
    interactCam->setFPSCameraSpeed(1);
    interactCam->switchMode(
            sibr::InteractiveCameraHandler::InteractionMode::TRACKBALL);
...
void sibr::MeshViewer::setMainMesh(const sibr::Mesh& mesh,
                                   sibr::Mesh::RenderMode mode,
                                   bool updateCam,
                                   bool setupRaycaster) {
...
void sibr::MeshViewer::setMainMesh(sibr::Window& win,
                                   const sibr::Mesh& mesh,
                                   sibr::Mesh::RenderMode mode,
                                   bool updateCam,
                                   bool setupRaycaster) {
...
void sibr::MeshViewer::renderLoop(sibr::Window& window) {
    ...
    while (doLoop && window.isOpened()) {
        ...
        interactCam->update(sibr::Input::global(), 1 / 60.0f,
                            window.viewport());
...
void sibr::MeshViewer::renderLoop(const std::function<void(MeshViewer*)>& f,
...
void sibr::MeshViewer::demo() {
    ...
    meshViewer.renderer->addPoints(meshPtr->vertices(),
                                   sibr::Vector3f(0, 1, 0));

    for (const auto& tri : meshPtr->triangles()) {
        for (int k = 0; k < 3; ++k) {
            meshViewer.renderer->addLines(
                    {meshPtr->vertices()[tri[k]],
                     meshPtr->vertices()[tri[(k + 1) % 3]]},
                    sibr::Vector3f(1, 0, 0));
        }
    }
