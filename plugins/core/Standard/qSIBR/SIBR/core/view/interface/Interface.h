...
// typedef void (*CallBackFunction)(int event, int x, int y, int flags, void*
// userdata);
...
struct PixPos {
    ...
    void cout() const {
        std::cout << im << " : " << pos.transpose() << std::endl;
    }
    sibr::Vector2i pos;
    int im;
    bool isDefined;
...
struct SubView {
    ...
    sibr::Vector2i getViewportPosition(const sibr::Vector2i& winPos) {
        return winPos -
               sibr::Vector2f(viewport.finalLeft(), viewport.finalTop())
                       .cast<int>();
    }
};
...
public:
    ...
    MultiViewInterfaceView(MultiViewInterface* interfacePtr, ViewType type)
        : interfacePtr(interfacePtr), viewType(type) {}

    virtual void onRenderIBR(IRenderTarget& /*dst*/,
...
/**
        This class provides basic rendering utilities for a list of images + a
   mesh. \ingroup sibr_view
*/
...
public:
    ...
    */
    void displayLoop(
            sibr::Window& window,
            std::function<void(MultiViewInterface*)> = [](MultiViewInterface*) {
            });
    ...
    virtual void updateImageView(const sibr::Viewport& viewport,
    ...
    sibr::ViewBase::Ptr getViewBase(
            MultiViewInterfaceView::ViewType type =
                    MultiViewInterfaceView::ViewType::IMAGES);
    void onGui();
...
public:
    ...
    int currentLayer;
    int numImgs;
    std::vector<std::vector<sibr::ITexture2DArray::Ptr>>
            imagesLayers;  // for each scale, each image layer
    ...
    PixPos pixFromScreenPos(const sibr::Vector2i& pos,
                            const sibr::Vector2f& winSize);

    UV01 screenPos(const PixPos& pix);
    UV01 screenPosPixelCenter(const PixPos& pix);

    sibr::Vector2i screenPosPixels(const PixPos& pix,
                                   const sibr::Vector2f& winSize);
    sibr::Vector2f screenPosPixelsFloat(const PixPos& pix,
                                        const sibr::Vector2f& winSize);

    void addHighlightPixel(const PixPos& pix, const sibr::Vector2f& winSize);
    void renderHighlightPixels();

    void highlightPixel(const PixPos& pix,
                        const sibr::Viewport& viewport,
                        const sibr::Vector3f& color = {0, 1, 0},
                        const sibr::Vector2f& pixScreenSize = {5.0f, 5.0f});
    ...
    virtual void updateMeshView(const sibr::Input& input,
                                const sibr::Viewport& viewport);
    ...
    bool checkNewLayer(
            const std::vector<sibr::Image<T_Type, T_NumComp>>& images) {
        if (imagesLayers.size() == 0) {
            imagesLayers.resize(scalingOptions.numScale);
            for (int scale = 0; scale < scalingOptions.numScale; ++scale) {
                int w_s = static_cast<int>(
                        std::ceil(images[0].w() * pow(2.0f, -scale)));
                int h_s = static_cast<int>(
                        std::ceil(images[0].h() * pow(2.0f, -scale)));
...
public:
    template <typename T_Type, unsigned int T_NumComp>
    void addImageLayer(
            const std::vector<sibr::ImagePtr<T_Type, T_NumComp>>& images,
    ...
    void addImageLayer(
        ...
        for (int scale = 0; scale < scalingOptions.numScale; ++scale) {
            std::vector<sibr::Image<T_Type, T_NumComp>> resized_imgs(
                    images.size());
            if (scale != 0) {
#pragma omp parallel for
                for (int im = 0; im < (int)images.size(); ++im) {
                    const sibr::Vector2i scaleSize =
                            scalesData[scale].imSize.cast<int>();
                    resized_imgs[im] = images[im].resized(
                            scaleSize[0], scaleSize[1],
                            scalingOptions.interpolation_method_cv);
                }
            } else {
            ...
            const std::vector<sibr::Image<T_Type, T_NumComp>>& imgs =
                    (scale == 0 ? images : resized_imgs);
            auto layer =
                    std::make_shared<sibr::Texture2DArray<T_Type, T_NumComp>>();
        ...
        std::string layerName =
                (name == "" ? "Layer" + std::to_string(layersData.size())
                            : name);
...
#pragma omp parallel for
            ...
            if (scale == 0) {
                ...
                std::vector<const sibr::IImage*> layerPtrs(images.size());
                for (int im = 0; im < (int)images.size(); ++im) {
                    auto lambdaImgPtr = std::make_shared<Lambda_Out_Image_Type>(
                            lambdaImgs[im].clone());

                    firstLayerPtrs[im] =
                            std::static_pointer_cast<IImage>(lambdaImgPtr);
            ...
            typename sibr::Texture2DArray<Lambda_Out_Type,
                                          Lambda_Out_N>::Ptr layer =
                    std::make_shared<sibr::Texture2DArray<Lambda_Out_Type,
                                                          Lambda_Out_N>>();
