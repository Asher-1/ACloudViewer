...
std::vector<uint> IBRBasicUtils::selectCamerasSimpleDist(
        const std::vector<InputCamera::Ptr>& cams,
    ...
    for (uint i = 0; i < cams.size(); ++i) {
        if (cams.at(i)->isActive()) {
            ...
            if (distOnly) {
                ...
                dist.insert(std::make_pair(
                        d, i));  // sort distances in increasing order
            }
        }
    }
...
std::vector<uint> IBRBasicUtils::selectCamerasAngleWeight(
        const std::vector<InputCamera::Ptr>& cams,
        const sibr::Camera& eye,
        uint count) {
    ...
    for (uint i = 0; i < cams.size(); ++i) {
        if (cams.at(i)->isActive()) {
            ...
            if (a > 0.707)  // cameras with 45 degrees
            {
                ...
                float currNormalAngle =
                        inverseLerp(0.f, midAngle,
                                    angleRadian(rotation, cams[i]->rotation()));
                float factor = currNormalDist * (1.f - angleWeight) +
                               currNormalAngle * angleWeight;

                factors.insert(std::make_pair(
                        factor, i));  // sort distances in increasing order
            }
        }
    }
