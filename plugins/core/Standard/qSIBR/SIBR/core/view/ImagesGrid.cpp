...
void ImagesGrid::onUpdate(Input& input, const Viewport& vp) {
    ...
    if (current_level_tex) {
        ...
        imSizePixels =
                imSizePixels
                        .cwiseQuotient(pow(2.0, current_lod) * Vector2f(1, 1))
                        .unaryExpr([](float f) { return std::floor(f); });
    ...
    if (currentActivePix && input.key().isActivated(Key::LeftControl) &&
        input.mouseButton().isReleased(Mouse::Code::Left)) {
    ...
    if (!imgs_list.empty()) {
        std::vector<int> selected_ims(std::begin(imgs_list),
                                      std::end(imgs_list));
...
void ImagesGrid::onRender(const Viewport& viewport) {
    ...
    draw_utils.image_grid(num_imgs, current_level_tex->handle(), grid_adjusted,
                          viewRectangle.tl(), viewRectangle.br(), current_lod,
                          current_layer->flip_texture);
...
void ImagesGrid::onGUI() {
    if (ImGui::Begin("grid_gui")) {
        ...
        if (currentActivePix) {
            GUI_TEXT("current pix : " << currentActivePix.im << ", "
                                      << currentActivePix.pos.transpose());

            Vector4f value = current_layer->imgs_texture_array->readBackPixel(
                    currentActivePix.im, currentActivePix.pos[0],
                    currentActivePix.pos[1], current_lod);
...
void ImagesGrid::addImagesToHighlight(const std::string& name,
                                      const std::vector<int>& imgs,
                                      const Vector3f& col,
                                      float alpha_fill) {
    images_to_highlight[name] = {imgs, col, alpha_fill};
}

void ImagesGrid::addPixelsToHighlight(const std::string& name,
                                      const std::vector<MVpixel>& pixs,
                                      const Vector3f& col,
                                      float alpha_fill) {
    pixels_to_highlight[name] = {pixs, col, alpha_fill};
}

const MVpixel& ImagesGrid::getCurrentPixel() { return currentActivePix; }
...
void ImagesGrid::listImagesLayerGUI() {
    if (ImGui::CollapsingHeader("images_layers")) {
        ...
        for (auto imgs_it = images_layers.begin();
             imgs_it != images_layers.end(); ++imgs_it) {
            if (ImGui::Selectable(imgs_it->name.c_str(),
            ...
            GUI_TEXT(tex_arr->depth()
                     << " x " << tex_arr->w() << " x " << tex_arr->h());
            ImGui::NextColumn();

            ImGui::Checkbox(("flip##" + imgs_it->name).c_str(),
                            &imgs_it->flip_texture);
...
void ImagesGrid::optionsGUI() {
    if (ImGui::CollapsingHeader("grid_options")) {
        ...
        static const std::vector<const char*> selection_mode_str = {
                "no selection", "image", "pixel"};
        for (int i = 0; i < (int)selection_mode_str.size(); ++i) {
            if (i != 0) {
                ImGui::SameLine();
            }
            if (ImGui::RadioButton(selection_mode_str[i],
...
void DrawUtilities::baseRendering(const Mesh& mesh,
...
void DrawUtilities::rectangle(const Vector3f& color,
                              const Vector2f& tl,
                              const Vector2f& br,
                              bool fill,
                              float alpha,
    ...
    rectangleMesh->vertices({{tl.x(), tl.y(), 0},
                             {tl.x(), br.y(), 0},
                             {br.x(), br.y(), 0},
                             {br.x(), tl.y(), 0}});

    if (fill) {
        rectangleMesh->triangles({{0, 1, 2}, {0, 2, 3}});

        baseRendering(*rectangleMesh, Mesh::FillRenderMode, color, {0, 0},
                      {1, 1}, alpha, vp);
    }

    rectangleMesh->triangles({{0, 0, 1}, {1, 1, 2}, {2, 2, 3}, {3, 3, 0}});

    baseRendering(*rectangleMesh, Mesh::LineRenderMode, color, {0, 0}, {1, 1},
                  1.0f, vp);
}

void DrawUtilities::rectanglePixels(const Vector3f& color,
                                    const Vector2f& center,
                                    const Vector2f& diagonalPixs,
                                    bool fill,
                                    float alpha,
                                    const Viewport& vp) {
...
void DrawUtilities::circle(const Vector3f& color,
                           const Vector2f& center,
                           float radius,
                           bool fill,
                           float alpha,
    ...
    for (int i = 0; i < n; ++i) {
        ...
        vertices[i] =
                Vector3f((float)cos(angle), (float)sin(angle), (float)0.0);
    }
    ...
    if (fill) {
        baseRendering(*circleFilledMesh, Mesh::FillRenderMode, color, {0, 0},
                      {radius, radius}, alpha, {});
    }
    baseRendering(*circleMesh, Mesh::LineRenderMode, color, {0, 0},
                  {radius, radius}, 1.0f, {});
}

void DrawUtilities::circlePixels(const Vector3f& color,
                                 const Vector2f& center,
                                 float radius,
                                 bool fill,
                                 float alpha,
                                 const Vector2f& winSize,
                                 int precision) {
...
void DrawUtilities::linePixels(const Vector3f& color,
                               const Vector2f& ptA,
                               const Vector2f& ptB,
                               const Vector2f& winSize) {
    ...
    Mesh line;
    line.vertices({{uvA.x(), uvA.y(), 0.0f}, {uvB.x(), uvB.y(), 0.0f}});
    line.triangles({Vector3u(0, 0, 1)});

    baseRendering(line, Mesh::LineRenderMode, color, {0, 0}, {1.0, 1.0}, 1.0f,
                  {});
}

void DrawUtilities::image_grid(int num_imgs,
                               uint texture,
                               const Vector2f& grid,
                               const Vector2f& tl,
                               const Vector2f& br,
...
void DrawUtilities::initBaseShader() {
    const std::string translationScalingVertexShader =
            ...
            "void main(void) {						"
            "								\n"
            "	gl_Position = vec4(scaling*in_vertex.xy+translation,0.0, "
            "1.0);		\n"
            "}								"
            "								"
            "		\n";
    ...
    baseShader.init("InterfaceUtilitiesBaseShader",
                    translationScalingVertexShader, colorAlphaFragmentShader);
...
void DrawUtilities::initGridShader() {
    ...
    const std::string gridFragmentShader =
            ...
            "				\n"
            " if ( n< 0 || n > numImgs || mods.x >= grid.x || mods.y >= "
            "(float(numImgs)/grid.x) ) { discard; } else { \n"
            "	out_color = textureLod(texArray,vec3(fracs.x, flip_texture ? "
            "1.0 -fracs.y : fracs.y,n), lod);	}		\n"
    ...
    gridShader.init("InterfaceUtilitiesMultiViewShader", gridVertexShader,
                    gridFragmentShader);
...
MVpixel GridMapping::pixFromScreenPos(const Vector2i& pos,
                                      const Vector2f& size) {
    Vector2f uvScreen =
            (pos.cast<float>() + 0.5 * Vector2f(1, 1)).cwiseQuotient(size);

    Vector2f posF = viewRectangle.tl() +
                    2.0 * viewRectangle.diagonal.cwiseProduct(uvScreen);
    posF = posF.cwiseProduct(grid_adjusted);

    // std::cout << posF.transpose() << " " << numImgs << std::endl;

    if (posF.x() < 0 || posF.y() < 0 ||
        posF.x() >= grid_adjusted.x() /* || posF.y() >= grid.y()  */) {
        return MVpixel();
    }
...
Vector2f GridMapping::uvFromMVpixel(const MVpixel& pix, bool use_center) {
    Vector2f pos =
            ((pix.pos.cast<float>() + (use_center ? 0.5 : 0) * Vector2f(1, 1))
                     .cwiseQuotient(imSizePixels) +
             Vector2f(pix.im % num_per_row, pix.im / num_per_row))
                    .cwiseQuotient(grid_adjusted);
    pos = (pos - viewRectangle.tl()).cwiseQuotient(viewRectangle.diagonal) -
          Vector2f(1, 1);
    pos.y() = -pos.y();
    return pos;
}
...
void GridMapping::updateZoomBox(const Input& input, const sibr::Viewport& vp) {
    ...
    if (input.mouseButton().isPressed(Mouse::Code::Right) &&
        !input.key().isActivated(Key::LeftControl) && !zoomSelection) {
    ...
    if (zoomSelection) {
        ...
        const auto clamp = [](Vector2f& v, float w, float h) {
            v = v.cwiseMax(Vector2f(1, 1)).cwiseMin(Vector2f(w - 2, h - 2));
        };
        ...
        if (input.mouseButton().isReleased(Mouse::Code::Right)) {
            ...
            if (((currentBR - currentTL).array() > Vector2f(5, 5).array())
                        .all()) {
                Vector2f tlPix = viewRectangle.tl().cwiseProduct(size) +
                                 (viewRectangle.br() - viewRectangle.tl())
                                         .cwiseProduct(currentTL);
                Vector2f brPix = viewRectangle.tl().cwiseProduct(size) +
                                 (viewRectangle.br() - viewRectangle.tl())
        ...
        } else if (!input.mouseButton().isActivated(Mouse::Code::Right) &&
                   input.isInsideViewport(aligned_vp)) {
            zoomSelection.isActive = false;
        }
    }
}
...
void GridMapping::updateDrag(const Input& input, const Vector2f& size) {
    ...
    if (drag.isActive && input.mouseButton().isActivated(Mouse::Left)) {
        Vector2f translation =
                2.0 * (input.mousePosition() - drag.position)
                              .cast<float>()
                              .cwiseQuotient(size)
                              .cwiseProduct(viewRectangle.diagonal);
...
void GridMapping::displayZoom(const Viewport& viewport, DrawUtilities& utils) {
    if (zoomSelection) {
        Vector2f tl = 2.0 * zoomSelection.first.cast<float>().cwiseQuotient(
                                    _vp.finalSize()) -
                      Vector2f(1, 1);
        Vector2f br = 2.0 * zoomSelection.second.cast<float>().cwiseQuotient(
                                    _vp.finalSize()) -
                      Vector2f(1, 1);
        tl.y() = -tl.y();
        br.y() = -br.y();
...
void GridMapping::highlightPixel(const MVpixel& pix,
                                 const Viewport& viewport,
                                 const Vector3f& color,
                                 const Vector2f& pixScreenSize) {
    Vector2f pixTl = uvFromMVpixel(pix);
    Vector2f pixBR = uvFromMVpixel(MVpixel(pix.im, pix.pos + Vector2i(1, 1)));

    viewport.bind();

    if ((pixBR - pixTl).cwiseProduct(viewport.finalSize()).norm() <
        pixScreenSize.diagonal().norm()) {
        // if pixel size in screen space is too tiny
        draw_utils.rectanglePixels(color, 0.5 * (pixTl + pixBR), pixScreenSize,
                                   true, 0.15f, viewport);
    } else {
...
void GridMapping::highlightImage(int im,
