...
MeshData MeshData::dummy =
        MeshData("dummy", Mesh::Ptr(), DUMMY, Mesh::FillRenderMode);

MeshData::MeshData(const std::string& _name,
                   Mesh::Ptr mesh_ptr,
                   MeshType mType,
...
MeshData MeshData::getNormalsMeshData() const {
    MeshData data(name + "_normals", meshPtr,
                  normalMode == PER_TRIANGLE ? TRIANGLES : POINTS);
    data.setColor(normalsColor).setDepthTest(depthTest);
    data.normalsLength = (normalsInverted ? -normalsLength : normalsLength);
    return data;
}

MeshData::operator bool() const { return meshType != DUMMY; }
...
void MeshData::renderGeometry() const {
    ...
    if (renderMode == Mesh::PointRenderMode) {
        ...
        meshPtr->render(depthTest, backFaceCulling, renderMode,
...
void MeshData::onGUI(const std::string& name) {
    ...
    if (ImGui::BeginCombo(("##render_mode_" + name).c_str(),
                          renderModeStrs[(int)renderMode].data())) {
        for (int t = (int)meshType; t >= 0; --t) {
            if (ImGui::Selectable(renderModeStrs[t].data(),
                                  t == (int)renderMode)) {
    ...
    if (ImGui::BeginCombo(("##color_mode_" + name).c_str(),
                          colorModeStrs[(int)colorMode].data())) {
        if (meshPtr && meshPtr->hasColors()) {
            if (ImGui::Selectable(colorModeStrs[VERTEX].data(),
                                  colorMode == VERTEX)) {
                colorMode = VERTEX;
            }
        }
        if (ImGui::Selectable(colorModeStrs[USER_DEFINED].data(),
    ...
    if (colorMode == USER_DEFINED) {
        ImGui::SameLine();
        ImGui::ColorEdit3(("##color_picker_" + name).c_str(), &userColor[0],
    ...
    if (ImGui::BeginPopup(("##Options_popup_" + name).c_str())) {
        ...
        if (meshType == TRIANGLES) {
            ...
            ImGui::Checkbox(("Swap back/front##" + name).c_str(),
                            &frontFaceCulling);
        }
        if (renderMode == Mesh::PointRenderMode) {
        ...
        if (meshType == TRIANGLES) {
            ...
            if (showNormals) {
                static const std::string normalModeStrs[2] = {"Per-triangle",
                                                              "Per-vertex"};
                if (ImGui::BeginCombo(("##normal_mode_" + name).c_str(),
                                      normalModeStrs[(int)normalMode].data())) {
                    if (ImGui::Selectable(normalModeStrs[PER_TRIANGLE].data(),
                                          normalMode == PER_TRIANGLE)) {
                        normalMode = PER_TRIANGLE;
                    }
                    if (meshPtr && meshPtr->hasNormals()) {
                        if (ImGui::Selectable(normalModeStrs[PER_VERTEX].data(),
                ...
                ImGui::Checkbox(("NormalInverted##" + name).c_str(),
                                &normalsInverted);
                ImGui::PushItemWidth(90);
                ImGui::SliderFloat(("NormalSize##" + name).c_str(),
                                   &normalsLength, 0.001f, 10.0f, "%.3f", 3.0f);
                ImGui::PopItemWidth();
                ImGui::ColorEdit3(
                        ("NormalsColor##color_picker_" + name).c_str(),
                        &normalsColor[0], ImGuiColorEditFlags_NoInputs);
            }
            if (!meshPtr->hasNormals()) {
                ...
                ImGui::Checkbox(("Phong shading##" + name).c_str(),
                                &phongShading);
            }
            ImGui::Separator();
        }
...
void ShaderAlphaMVP::initShader(const std::string& name,
...
void ColorMeshShader::initShader(const std::string& name,
                                 const std::string& vert,
                                 const std::string& frag,
                                 const std::string& geom) {
...
void PointShader::initShader(const std::string& name,
                             const std::string& vert,
                             const std::string& frag,
                             const std::string& geom) {
...
void NormalRenderingShader::initShader(const std::string& name,
                                       const std::string& vert,
                                       const std::string& frag,
                                       const std::string& geom) {
    ColorMeshShader::initShader(name, vert, frag, geom);
...
void NormalRenderingShader::setUniforms(const Camera& eye,
                                        const MeshData& data) {
    ColorMeshShader::setUniforms(eye, data);
    normals_size.set(data.normalsLength);
}

void MeshShadingShader::initShader(const std::string& name,
                                   const std::string& vert,
                                   const std::string& frag,
                                   const std::string& geom) {
    ColorMeshShader::initShader(name, vert, frag, geom);
...
MultiMeshManager::MultiMeshManager(const std::string& _name) : name(_name) {
    ...
    camera_handler.switchMode(
            InteractiveCameraHandler::InteractionMode::TRACKBALL);
}
...
void MultiMeshManager::setIntialView(const std::string& dataset_path) {
    ...
    if (topViewFile.good()) {
        SIBR_LOG << "Loaded saved topview (" << topViewPath << ")."
                 << std::endl;
...
void MultiMeshManager::initShaders() {
    ...
    colored_mesh_shader.initShader(
            "colored_mesh_shader", loadFile(folder + "alpha_colored_mesh.vert"),
    ...
    per_vertex_normals_shader.initShader(
            "per_vertex_normal_shader",
            loadFile(folder + "alpha_colored_per_vertex_normals.vert"),
            loadFile(folder + "alpha_colored_mesh.frag"),
            loadFile(folder + "alpha_colored_per_vertex_normals.geom"));
    per_triangle_normals_shader.initShader(
            "per_triangle_normal_shader",
    ...
    for (const auto& mesh_data : list_meshes) {
        ...
        if (mesh_data.showNormals) {
            if (mesh_data.normalMode == MeshData::PER_VERTEX) {
                per_vertex_normals_shader.render(
                        camera_handler.getCamera(),
                        mesh_data.getNormalsMeshData());
            } else {
                per_triangle_normals_shader.render(
                        camera_handler.getCamera(),
                        mesh_data.getNormalsMeshData());
            }
        }
...
void MultiMeshManager::list_mesh_onGUI() {
    ...
    if (ImGui::CollapsingHeader(("Meshes list##" + name).c_str(),
        ...
        if (ImGui::Button("load Mesh##MeshesList") &&
            showFilePicker(loaded_mesh_path, FilePickerMode::Default, "",
                           "obj,ply")) {
            ...
            if (mesh->load(loaded_mesh_path)) {
                ...
                mesh_name = (mesh_name == "") ? mesh_path.stem().string()
                                              : loaded_mesh_str;
        ...
        ImGui::SameLine();
        ImGui::InputText("mesh name##MeshesList", loaded_mesh_str,
                         IM_ARRAYSIZE(loaded_mesh_str));

        // 0 name | 1 snapto delete | 2 active | 3 rendering mode | 4 alpha | 5
        // color | 6 Options
        ...
        ImGui::Text("Color");
        ImGui::SameLine();
        ImGui::ColorEdit3(("Background##" + name).c_str(), &backgroundColor[0],
                          ImGuiColorEditFlags_NoInputs);
        ImGui::NextColumn();
        ...
        for (auto mesh_it = list_meshes.begin(); mesh_it != list_meshes.end();
             ++mesh_it) {
            auto& mesh = *mesh_it;
            if (ImGui::Selectable(mesh.name.c_str(),
                                  (selected_mesh_it_is_valid &&
                                   mesh_it == selected_mesh_it))) {
            ...
            if (ImGui::Button(("SnapTo##" + mesh_it->name).c_str()) &&
                mesh_it->meshPtr) {
                auto box = mesh_it->meshPtr->getBoundingBox();
                if ((box.diagonal().array() > 1e-6f).all()) {
                    InputCamera cam = camera_handler.getCamera();
                    cam.setLookAt(box.center() + 2.0f * box.diagonal(),
                                  box.center(), {0, 1, 0});
            ...
            if (ImGui::Button(("OnlyMe##" + mesh.name).c_str())) {
                for (auto other_it = list_meshes.begin();
                     other_it != list_meshes.end(); ++other_it) {
...
MeshData& MultiMeshManager::addMesh(const std::string& name,
                                    Mesh::Ptr mesh,
                                    bool use_raycaster) {
    if (!mesh) {
...
MeshData& MultiMeshManager::addMesh(const std::string& name,
    ...
    data.colorMode = mesh->hasColors() ? MeshData::ColorMode::VERTEX
                                       : MeshData::ColorMode::USER_DEFINED;
    data.normalMode = (mesh->hasNormals() ? MeshData::PER_VERTEX
                                          : MeshData::PER_TRIANGLE);
...
MeshData& MultiMeshManager::addMeshAsLines(const std::string& name,
                                           Mesh::Ptr mesh) {
    if (!mesh) {
        SIBR_WRG << "no mesh ptr in " << name;
        return MeshData::dummy;
...
MeshData& MultiMeshManager::addLines(const std::string& name,
...
MeshData& MultiMeshManager::addPoints(const std::string& name,
...
MeshData& MultiMeshManager::addMeshData(MeshData& data, bool create_raycaster) {
    ...
    for (collision_it = list_meshes.begin(); collision_it != list_meshes.end();
         ++collision_it) {
