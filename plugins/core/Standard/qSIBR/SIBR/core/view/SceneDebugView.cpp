...
Mesh::Ptr generateCamFrustum(const InputCamera& cam,
                             float near,
                             float far,
                             bool useCam) {
    static const Mesh::Triangles tris = {
            {0, 0, 1}, {1, 1, 2}, {2, 2, 3}, {3, 3, 0}, {4, 4, 5}, {5, 5, 6},
            {6, 6, 7}, {7, 7, 4}, {0, 0, 4}, {1, 1, 5}, {2, 2, 6}, {3, 3, 7},
    };
    ...
    if (!useCam) {
        ...
        for (const auto& c : cam.getImageCorners())
            dirs.push_back(CameraRaycaster::computeRayDir(
                    cam, c.cast<float>() + 0.5f * Vector2f(1, 1)));
    }
...
Mesh::Ptr generateCamFrustumColored(const InputCamera& cam,
                                    const Vector3f& col,
                                    float znear,
                                    float zfar) {
...
Mesh::Ptr generateCamQuadWithUvs(const InputCamera& cam, float dist) {
    static const Mesh::Triangles quadTriangles = {{0, 1, 2}, {0, 2, 3}};
    static const Mesh::UVs quadUVs = {{0, 1}, {1, 1}, {1, 0}, {0, 0}};
...
LabelsManager::CameraInfos::CameraInfos(const InputCamera& cam,
                                        uint id,
                                        bool highlight)
...
void LabelsManager::setupLabelsManagerShader() {
    _labelShader.init("text-imgui",
                      loadFile(Resources::Instance()->getResourceFilePathName(
                              "text-imgui.vp")),
                      loadFile(Resources::Instance()->getResourceFilePathName(
                              "text-imgui.fp")));
...
void LabelsManager::setupLabelsManagerMeshes(
        const std::vector<InputCamera::Ptr>& cams) {
    _labelMeshes.clear();
    for (const auto& cam : cams) {
        unsigned int sepIndex = 0;
        _labelMeshes[cam->id()] = {};
        _labelMeshes[cam->id()].mesh =
                generateMeshForText(std::to_string(cam->id()), sepIndex);
        _labelMeshes[cam->id()].splitIndex = sepIndex;
    }
}

void LabelsManager::renderLabels(const Camera& eye,
                                 const Viewport& vp,
                                 const std::vector<CameraInfos>& cams_info) {
    glEnable(GL_BLEND);
    ...
    glBindTexture(GL_TEXTURE_2D,
                  (GLuint)(intptr_t)(ImGui::GetFont()->ContainerAtlas->TexID));
    ...
    for (const auto& camInfos : cams_info) {
        ...
        // Draw the label.
        // TODO: we could try to use depth testing to have the labels overlap
        // properly. As the label is put at the position of the camera, the
        // label will intersect with the frustum mesh, causing artifacts. One
        // way of solving this would be to just shift the label away a bit and
        // enable depth testing (+ GL_LEQUAl for the text).
        ...
        label.mesh->renderSubMesh(label.splitIndex,
                                  int(label.mesh->triangles().size()) * 3,
                                  false, false);
    }
    _labelShader.end();
    glDisable(GL_BLEND);
...
void ImageCamViewer::initImageCamShaders() {
    const std::string vertex_str = loadFile(
            Resources::Instance()->getResourceFilePathName("uv_mesh.vert"));

    _shader2D.init("cameraImageShader", vertex_str,
                   loadFile(Resources::Instance()->getResourceFilePathName(
                           "alpha_uv_tex.frag")));
    _mvp2D.init(_shader2D, "mvp");
    _alpha2D.init(_shader2D, "alpha");

    _shaderArray.init("cameraImageShaderArray", vertex_str,
                      loadFile(Resources::Instance()->getResourceFilePathName(
...
void ImageCamViewer::renderImage(
        const Camera& eye,
        const InputCamera& cam,
        const std::vector<RenderTargetRGBA32F::Ptr>& rts,
        int cam_id) {
...
void ImageCamViewer::renderImage(const Camera& eye,
                                 const InputCamera& cam,
                                 uint tex2Darray_handle,
                                 int cam_id) {
    const auto quad = generateCamQuadWithUvs(cam, _pathScaling);
    _shaderArray.begin();
...
void ImageCamViewer::renderImage(const Camera& eye,
                                 const InputCamera& cam,
                                 const RenderTargetRGBA32F::Ptr& rt) {
    const auto quad = generateCamQuadWithUvs(cam, _pathScaling);
    _shader2D.begin();
    _mvp2D.set(eye.viewproj());
    _alpha2D.set(_alphaImage);
...
void ImageCamViewer::updateImgRt(const InputCamera& cam,
                                 const sibr::ImageRGBA& img) {
    uint w = cam.w();
    uint h = cam.h();
    ...
    textureShader.init("Texture",
                       ...
                       loadFile(Resources::Instance()->getResourceFilePathName(
                               "texture.fp")));
    uint interpFlag = (SIBR_SCENE_LINEAR_SAMPLING & SIBR_SCENE_LINEAR_SAMPLING)
                              ? SIBR_GPU_LINEAR_SAMPLING
                              : 0;  // LINEAR_SAMPLING Set to default

    std::cerr << ".";
    ImageRGBA cloned_img = std::move(img.clone());
    cloned_img.flipH();

    std::shared_ptr<Texture2DRGBA> rawInputImage(
            new Texture2DRGBA(cloned_img, interpFlag));
    ...
    // Render texture (mapped to screenquad geometry) in the framebuffer
    // (renderTarget)
    //  so that it matches afterwards the same camera settings when applied as a
    //  texture
...
SceneDebugView::SceneDebugView(const IIBRScene::Ptr& scene,
                               const InteractiveCameraHandler::Ptr& camHandler,
                               const BasicDatasetArgs& myArgs,
    ...
    if (!_scene->cameras()->inputCameras().empty()) {
        camera_handler.fromTransform(
                _scene->cameras()->inputCameras()[0]->transform(), true, false);
        camera_handler.setupInterpolationPath(
...
SceneDebugView::SceneDebugView(const IIBRScene::Ptr& scene,
                               ...
                               const InteractiveCameraHandler::Ptr& camHandler,
                               const BasicDatasetArgs& myArgs)
    : SceneDebugView(scene, camHandler, myArgs) {
    SIBR_WRG << "Deprecated SceneDebugView constructor, use the version "
                "without viewport passed as argument."
             << std::endl;
}
...
void SceneDebugView::onUpdate(Input& input,
    ...
    if (input.key().isActivated(Key::LeftControl) &&
        input.mouseScroll() != 0.0) {
        _userCameraScaling = std::max(
                0.001f, _userCameraScaling + (float)input.mouseScroll() * 0.1f);
    }
    if (input.key().isActivated(Key::LeftControl) &&
        input.key().isReleased(Key::P)) {
    ...
    if (_pathScaling != _lastPathScaling) {
        ...
        for (const auto& camInfos : _cameras) {
            ...
            (camInfos.highlight ? _used_cams : _non_used_cams)
                    ->merge(*generateCamFrustum(camInfos.cam, 0.0f,
                                                _pathScaling));
        }
        _lastPathScaling = _pathScaling;
        addMeshAsLines("used cams", _used_cams)
                .setColor({0, 1, 0})
                .setDepthTest(true);
    }
}
...
void SceneDebugView::onRender(const Viewport& viewport) {
    ...
    if (_displayImg) {
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        // renderFromTex(camera_handler.getCamera(),
        // _cameras[_cameraIdInfoGUI].cam, _imgTex->handle());
        renderImage(camera_handler.getCamera(), _cameras[_cameraIdInfoGUI].cam,
                    _imgRt);
        glDisable(GL_BLEND);
    }
...
void SceneDebugView::setScene(const IIBRScene::Ptr& scene,
                              bool preserveCamera) {
    _scene = scene;
    const InputCamera cameraBack = camera_handler.getCamera();
    setup();
    camera_handler.setup(_scene->cameras()->inputCameras(),
                         camera_handler.getViewport(),
                         camera_handler.getRaycaster());
...
void SceneDebugView::gui_options() {
    if (ImGui::CollapsingHeader("OptionsSceneDebugView##")) {
        ...
        ImGui::InputFloat("User camera scale", &_userCameraScaling, 0.1f,
                          10.0f);
...
void SceneDebugView::gui_cameras() {
    if (ImGui::CollapsingHeader("Cameras##SceneDebugView")) {
        ImGui::SliderInt("Camera ID info", &_cameraIdInfoGUI, 0,
                         static_cast<int>(_cameras.size()) - 1);

        // Snap topView cam to closest input camera in mainView
        if (ImGui::Button(std::string("Snap to closest").c_str())) {
            _cameraIdInfoGUI = _userCurrentCam->findNearestCamera(
                    _scene->cameras()->inputCameras());
            ...
            InputCamera cam = InputCamera(_cameras[_cameraIdInfoGUI].cam,
                                          (int)size[0], (int)size[1]);

            if (_displayImg) _displayImg = false;

            if (ratio_src < ratio_dst) {
                float fov_h = 2 * atan(tan(input_cam->fovy() / 2) * ratio_src /
                                       ratio_dst);
                cam.fovy(fov_h);
            } else {
        ...
        static std::vector<std::string> cam_info_option_str = {
                "size", "focal", "fov_y", "aspect"};
        if (ImGui::BeginCombo("Info",
                              cam_info_option_str[_camInfoOption].c_str())) {
            for (int i = 0; i < (int)cam_info_option_str.size(); ++i) {
                if (ImGui::Selectable(cam_info_option_str[i].c_str(),
                                      _camInfoOption == i)) {
        ...
        // for (uint i = 0; i < _cameras.size(); ++i)
        {
            ...
            if (ImGui::Button(("SnapTo##" + name).c_str())) {
                ...
                if (ratio_src < ratio_dst) {
                    float fov_h = 2 * atan(tan(input_cam->fovy() / 2) *
                                           ratio_src / ratio_dst);
                    cam.fovy(fov_h);
                } else {
                    cam.fovy(input_cam->fovy());
                ...
                if (ImGui::Button("DisplayImg##")) {
                    if (_imgToFetch != _cameras[_cameraIdInfoGUI].cam.name()) {
                        ...
                        if (!fileExists(fullPath)) {
                            fullPath =
                                    fullPath.substr(0, fullPath.length() - 3) +
                                    "JPG";
                        }
...
void SceneDebugView::setup() {
    if (_scene) {
        ...
        setupMeshes();
        _user_cam = generateCamFrustum(_userCurrentCam->getCamera(), 0.0f,
                                       _userCameraScaling, false);
        _cameras.clear();

        int index = 0;
        for (const auto& inputCam : _scene->cameras()->inputCameras()) {
            const bool isUsed =
                    _scene->cameras()->isCameraUsedForRendering(index);
            _cameras.push_back(CameraInfos(*inputCam, inputCam->id(), isUsed));

            if (inputCam->isActive())
                (isUsed ? _used_cams : _non_used_cams)
                        ->merge(*generateCamFrustum(*inputCam, 0.0f,
                                                    _pathScaling));
            index++;
        }
        addMeshAsLines("scene cam", _user_cam)
                .setColor({1, 0, 0})
                .setDepthTest(true);
        addMeshAsLines("used cams", _used_cams)
                .setColor({0, 1, 0})
                .setDepthTest(true);
        addMeshAsLines("non used cams", _non_used_cams)
                .setColor({0, 0, 1})
                .setDepthTest(true);
    }
    ...
    if (topViewFile.good()) {
        SIBR_LOG << "Loaded saved topview (" << _topViewPath << ")."
                 << std::endl;
...
void SceneDebugView::setupMeshes() {
    ...
    if (!proxyMesh->hasColors() && !proxyMesh->hasTexCoords()) {
        ...
        // in sibr subdir
        else if (fileExists(fn = _scene->data()->basePathName() +
                                 "/../capreal/mesh.ply")) {
            if (sdv_mesh.load(fn, _scene->data()->basePathName()))
                success = true;
        }
    ...
    addMeshAsLines("guizmo", RenderUtility::createAxisGizmo())
            .setDepthTest(false)
            .setColorMode(MeshData::ColorMode::VERTEX);
}
