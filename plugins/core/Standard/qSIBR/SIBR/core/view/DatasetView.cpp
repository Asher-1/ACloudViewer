...
DatasetView::DatasetView(const BasicIBRScene& scene,
                         const Vector2u& defaultRenderingRes,
                         const Vector2i& defaultViewRes)
...
void DatasetView::onUpdate(Input& input) {
    ...
    if (meshInput.key().isActivated(Key::LeftControl) &&
        meshInput.mouseButton().isActivated(Mouse::Right)) {
        RaycastingCamera cam =
                RaycastingCamera(getMMM()->getCameraHandler().getCamera());
        ...
        if (hit.hitSomething()) {
            ...
            repro(currentRepro);
            getGrid()->addPixelsToHighlight("zinputRepro", {}, {1, 0, 0},
                                            0.25f);
        }
    }

    Input gridInput = Input::subInput(input, getGridView().viewport);
    if (gridInput.key().isActivated(Key::LeftControl) &&
        gridInput.mouseButton().isActivated(Mouse::Right)) {
        ...
        if (pix) {
            ...
            if (hit.hitSomething()) {
                ...
                currentRepro.repros.clear();
                repro(currentRepro);
                getGrid()->addPixelsToHighlight("zinputRepro", {pix}, {1, 0, 0},
                                                0.25f);
            }
        }
    }
...
void DatasetView::repro(ReprojectionData& data) {
    ...
    for (int im = 0; im < (int)cams.size(); ++im) {
        ...
        if (data.occlusionTest) {
            ...
            if (hit.hitSomething() &&
                std::abs(hit.dist() - dist) / dist > 0.01f) {
                continue;
            }
        }
...
void DatasetView::displayRepro(const ReprojectionData& data) {
    ...
    // getGrid()->addImagesToHighlight("reproImgs", repro_imgs, { 0,1,0 },
    // 0.1f);
}
...
MeshData& DatasetView::proxyData() { return getMMM()->getMeshData("proxy"); }
