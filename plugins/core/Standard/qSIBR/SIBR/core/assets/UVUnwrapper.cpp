...
bool progressCallback(xatlas::ProgressCategory category,
                      int progress,
                      void *userData) {
...
void setPixel(uint8_t *dest,
              int destWidth,
              int x,
              int y,
              const sibr::Vector3ub &color) {
...
static void rasterizeLine(uint8_t *dest,
                          int destWidth,
                          const int *p1,
                          const int *p2,
                          const sibr::Vector3ub &color) {
    ...
    current[0] = p1[0];
    current[1] = p1[1];
    while (setPixel(dest, destWidth, current[0], current[1], color),
           current[0] != p2[0] || current[1] != p2[1]) {
        const int e2 = err;
        if (e2 > -dx) {
            err -= dy;
            current[0] += sx;
...
*/
void rasterizeTriangle(uint8_t *dest,
                       int destWidth,
                       const int *t0,
                       const int *t1,
                       const int *t2,
    ...
    for (int i = 0; i < total_height; i++) {
        ...
        const float beta = (float)(i - (second_half ? t1[1] - t0[1] : 0)) /
                           float(segment_height);
        int A[2], B[2];
        for (int j = 0; j < 2; j++) {
            A[j] = int(t0[j] + (t2[j] - t0[j]) * alpha);
            B[j] = int(second_half ? t1[j] + (t2[j] - t1[j]) * beta
                                   : t0[j] + (t1[j] - t0[j]) * beta);
        }
...
UVUnwrapper::UVUnwrapper(const sibr::Mesh &mesh, unsigned int res)
    : _mesh(mesh) {
    _size = res;
    // Create empty atlas.
    ...
    SIBR_LOG << "[UVMapper] Adding one mesh with " << mesh.vertices().size()
             << " vertices and " << mesh.triangles().size() << " triangles."
             << std::endl;
    // For now consider everything as one mesh. Splitting in components *might*
    // help.
    ...
    if (error != xatlas::AddMeshError::Success) {
        ...
        SIBR_ERR << "\r[UVMapper] Error adding mesh: "
                 << xatlas::StringForEnum(error) << std::endl;
    }
    // Not necessary. Only called here so geometry totals are printed after the
    // AddMesh progress indicator
...
sibr::Mesh::Ptr UVUnwrapper::unwrap() {
    ...
    SIBR_LOG << "[UVMapper] Generation took: "
             << timer.deltaTimeFromLastTic<Timer::s>() << "s." << std::endl;
    SIBR_LOG << "[UVMapper] Output resolution: " << _atlas->width << "x"
             << _atlas->height << std::endl;
    SIBR_LOG << "[UVMapper] Generated " << _atlas->chartCount << " charts, "
             << _atlas->atlasCount << " atlases." << std::endl;
    for (uint32_t i = 0; i < _atlas->atlasCount; i++) {
        SIBR_LOG << "[UVMapper] \tAtlas " << i
                 << ": utilisation: " << _atlas->utilization[i] * 100.0f << "%"
                 << std::endl;
    }
    ...
    SIBR_LOG << "[UVMapper] Output geometry data: " << totalVertices
             << " vertices, " << totalFaces << " triangles." << std::endl;
    // Write meshes.
    ...
    for (uint32_t i = 0; i < _atlas->meshCount; i++) {
        ...
        for (uint32_t v = 0; v < xmesh.vertexCount; v++) {
            ...
            texcoords.emplace_back(vertex.uv[0] / float(_atlas->width),
                                   vertex.uv[1] / float(_atlas->height));
        }
...
const std::vector<uint> &UVUnwrapper::mapping() const { return _mapping; }

std::vector<ImageRGB::Ptr> UVUnwrapper::atlasVisualization() const {
    if (!_atlas || _atlas->width <= 0 || _atlas->height <= 0) {
        SIBR_WRG << "[UVMapper] Atlas has not been created/processed."
                 << std::endl;
        return {};
    }
            ...
            for (uint32_t k = 0; k < chart->faceCount; k++) {
                ...
                for (int l = 0; l < 3; l++) {
                    const xatlas::Vertex &v =
                            xmesh.vertexArray
                                    [xmesh.indexArray[chart->faceArray[k] * 3 +
                                                      l]];
                    verts[l][0] = int(v.uv[0]);
                    verts[l][1] = int(v.uv[1]);
                }
                uint8_t *imageData =
                        &outputChartsImage[chart->atlasIndex * imageDataSize];
                rasterizeTriangle(imageData, _atlas->width, verts[0], verts[1],
                                  verts[2], color);
                rasterizeLine(imageData, _atlas->width, verts[0], verts[1],
                              white);
                rasterizeLine(imageData, _atlas->width, verts[1], verts[2],
                              white);
                rasterizeLine(imageData, _atlas->width, verts[2], verts[0],
                              white);
            }
        }
    }
...
UVUnwrapper::~UVUnwrapper() { xatlas::Destroy(_atlas); }
