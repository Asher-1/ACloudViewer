...
void CameraRecorder::use(Camera& cam) {
    if (_recording) {
        ...
        if (!_playNoInterp) {
            ...
            //  If we reach the last frame of the interpolation b/w two cameras,
            //  skip to next camera.
            ...
            sibr::Camera& camStart =
                    _cameras[std::min(int(_pos), int(_cameras.size()) - 1)];
            sibr::Camera& camNext =
                    _cameras[std::min(int(_pos) + 1, int(_cameras.size()) - 1)];
            ...
            cam = _cameras[_pos];
            _pos++;
            if (_pos == _cameras.size()) _playNoInterp = false;
        }
...
void CameraRecorder::savingVideo(bool saveVideo) { _savingVideo = saveVideo; }
...
bool CameraRecorder::load(const std::string& filename) {
    ...
    if (stream.load(filename) == false) return false;
...
void CameraRecorder::save(const std::string& filename) {
    ...
    stream << num;
    for (const Camera& cam : _cameras) stream << cam;

    stream.saveToFile(filename);
    SIBR_LOG << "[CameraRecorder] - Saved " << num << " cameras to " << filename
             << std::endl;
}
...
void CameraRecorder::loadBundle(const std::string& filePath, int w, int h) {
    ...
    for (int i = 0; i < numImages; i++) {
        ...
        cameras[i]->znear(0.2f);
        cameras[i]->zfar(250.f);
    }
...
void CameraRecorder::loadColmap(const std::string& filePath, int w, int h) {
    ...
    SIBR_LOG << "DEBUG: colmap path dir " << colmapDir.parent_path().string()
             << std::endl;

    std::vector<InputCamera::Ptr> path = InputCamera::loadColmap(
            colmapDir.parent_path().string(), float(0.01), 1000, false);
...
void CameraRecorder::loadLookat(const std::string& filePath, int w, int h) {
    ...
    std::vector<InputCamera::Ptr> path = InputCamera::loadLookat(
            filePath, std::vector<Vector2u>{Vector2u(w, h)});
    for (const InputCamera::Ptr cam : path) {
        _cameras.push_back(*cam);
    }
}

void CameraRecorder::saveAsBundle(const std::string& filePath,
                                  const int height,
                                  const int step) {
    std::ofstream out(filePath.c_str(), std::ios::binary);
    if (!out.good()) {
        SIBR_LOG << "ERROR: cannot write to the file '" << filePath << "'"
                 << std::endl;
        return;
    }
    ...
    for (int i = 0; i < _cameras.size(); i += step) {
        ...
        float m[15];
        m[0] = 0.5f * height / tan(cam.fovy() / 2.f);
        m[1] = 0.0f;
        m[2] = 0.0f;
    ...
    SIBR_LOG << "[CameraRecorder] - Saved " << _cameras.size() << " cameras to "
             << filePath << " (using fovy " << _cameras[0].fovy() << ")."
             << std::endl;
}

void CameraRecorder::saveAsColmap(const std::string& filePath,
                                  const int height,
                                  const int width) {
    ...
    out_images << "#   IMAGE_ID, QW, QX, QY, QZ, TX, TY, TZ, CAMERA_ID, NAME"
               << std::endl;
    out_images << "#   POINTS2D[] as (X, Y, POINT3D_ID)" << std::endl;
    for (int i = 0; i < _cameras.size(); i++) {
        sibr::Matrix3f tmp =
                _cameras[i].rotation().toRotationMatrix() * converter;
        sibr::Matrix3f Qinv = tmp.transpose();
        sibr::Quaternionf q = quatFromMatrix(Qinv);
        sibr::Vector3f t =
                converter *
                _cameras[i].rotation().toRotationMatrix().transpose() *
                (-_cameras[i].position());

        out_images << i << " " << -_cameras[i].rotation().x() << " "
                   << -_cameras[i].rotation().w() << " "
                   << -_cameras[i].rotation().z() << " "
                   << _cameras[i].rotation().y() << " "
                   << _cameras[i].view()(0, 3) << " "
                   << -_cameras[i].view()(1, 3) << " "
                   << -_cameras[i].view()(2, 3) << " " << i << " "
                   << "00000000.png" << std::endl
                   << std::endl;

        float focal = 0.5f * height / tan(_cameras[i].fovy() / 2.f);
        // float focal = 1.0f / (tan(0.5f * cam.fovy()) * 2.0f / float(height));
        out_cameras << i << " " << "PINHOLE" << " " << width << " " << height
                    << " " << focal << " " << focal << " " << width / 2.0 << " "
                    << height / 2.0 << std::endl;
    }
...
void CameraRecorder::saveAsFRIBRBundle(const std::string& dirPath,
                                       const int width,
                                       const int height) {
    ...
    if (out.good() && outList.good()) {
        ...
        from_cv << 1, 0, 0, 0, -1, 0, 0, 0, -1;
        for (int i = 0; i < _cameras.size(); ++i) {
            const sibr::Camera cam = _cameras[i];

            sibr::Matrix3f orientation = cam.rotation().toRotationMatrix();
            sibr::Vector3f position = cam.position();
            sibr::Matrix3f rotation_cv =
                    converter.transpose() * orientation.transpose() * converter;
        ...
        SIBR_LOG << "[CameraRecorder] - Saved " << _cameras.size()
                 << " cameras to " << dirPath << "." << std::endl;
    }
}
...
bool CameraRecorder::loadPath(const std::string& pathFileName, int w, int h) {
    ...
    if (!fileExists(pathFileName)) {
        SIBR_WRG << "Camera path " << pathFileName << " doesnt exist. "
                 << std::endl;
        return false;
    }
    _ow = w, _oh = h;
...
void CameraRecorder::recordOfflinePath(const std::string& outPathDir,
                                       ViewBase::Ptr view,
    ...
    if (outPathDir == "pathOutput" &&
        _savingPath != "") {  // default to path parent, saved by loadPath
        outpathd = _savingPath + "/" + "pathOutput";
        dstFolder = outpathd;
        if (!directoryExists(outpathd) &&
    ...
    std::cout << "Rendering path with " << _cameras.size() << " cameras to "
              << outpathd << std::endl;
...
void CameraRecorder::saveImage(const std::string& outPathDir,
                               const Camera& cam,
                               int w,
                               int h) {
    sibr::ImageRGBA32F::Ptr outImage;
