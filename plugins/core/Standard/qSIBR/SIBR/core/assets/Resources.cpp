...
/// \todo TODO: If you care about security (did someone want to hack/use your
/// app to hide a virus/retrieve informations from this compiled code), comment
...
Resources* Resources::Instance() {
    if (_instance == 0) _instance = new Resources;
    return _instance;
}
...
Resources::Resources() {
    ...
    if (rscFile.good()) {
        ...
        std::ifstream rscFile2(sibr::getInstallDirectory() +
                               "/bin/ibr_resources.ini");
    ...
    if (pathFile.good()) {
        for (std::string line; safeGetline(pathFile, line);) {
            ...
            if (curEnv != NULL) currentEnv = std::string(curEnv);
            ...
            std::cout << "[Resources] env: " << name << "="
                      << getenv(name.c_str()) << std::endl;
        }
    }
}
...
std::string Resources::getResourceFilePathName(std::string const& filename,
    ...
    // Weird bug -- GD: I have no idea why, but if I dont call this the paths
    ...
    if (success = rscFileTest.good()) return filename;
    for (std::string rscPath : _rscPaths) {
        std::string filePathName =
                sibr::getInstallDirectory() + "/" + rscPath + "/" + filename;
