...
void MeshTexturing::setMesh(const sibr::Mesh::Ptr mesh) {
    ...
    if (!_mesh->hasTexCoords()) {
        SIBR_ERR << "[Texturing] The mesh to texture needs to have UVs"
                 << std::endl;
        return;
    }
...
#pragma omp parallel for
    ...
    // Our version of Embree being compiled with backface culling, we have to
...
void MeshTexturing::interpolate(const sibr::RayHit& hit,
                                sibr::Vector3f& vertex,
                                sibr::Vector3f& normal) const {
    ...
    vertex = wCoord * vertices[tri[0]] + uCoord * vertices[tri[1]] +
             vCoord * vertices[tri[2]];
    normal = (wCoord * normals[tri[0]] + uCoord * normals[tri[1]] +
              vCoord * normals[tri[2]])
                     .normalized();
}

void MeshTexturing::reproject(const std::vector<InputCamera::Ptr>& cameras,
    ...
    sibr::LoadingProgress progress(
            h, "[Texturing] Gathering color samples from cameras");
    SIBR_LOG << "[Texturing] Gathering color samples from " << cameras.size()
             << " cameras ..." << std::endl;
    ...
    for (int py = 0; py < h; ++py) {
        for (int px = 0; px < w; ++px) {
            ...
            for (int cid = 0; cid < cameras.size(); ++cid) {
                ...
                const RayHit hitOcc =
                        _worldRaycaster.intersect(Ray(cam->position(), occDir));
                if (hitOcc.hitSomething() && (hitOcc.dist() + 0.0001f) < dist) {
                    continue;
                }

                // Reproject, read color.
                const sibr::Vector2f pos =
                        cam->projectImgSpaceInvertY(vertex).xy();
                const sibr::Vector3f col =
                        images[cid]->bilinear(pos).cast<float>().xyz();
            ...
            std::sort(samples.begin(), samples.end(),
                      [](const SampleInfos& a, const SampleInfos& b) {
                          return a.weight > b.weight;
                      });

            // Re-weight and accumulate the samples.
            // The code is written this way to support 'best sampleRatio of all
            // samples' approaches.
        ...
        if ((py % 1000) == 0) progress.walk(1000);
    }
}
...
sibr::ImageRGB::Ptr MeshTexturing::getTexture(uint options) const {
    ...
    /// \todo For extra large images, this might crash because of internal
    /// openCV indexing limitations.
...
sibr::ImageRGB32F::Ptr MeshTexturing::poissonFill(
        const sibr::ImageRGB32F& image, const sibr::ImageL8& mask) {
...
bool MeshTexturing::hitTest(int px, int py, RayHit& finalHit) {
    ...
    const RayHit hit =
            _uvsRaycaster.intersect(Ray({u, v, 1.0f}, {0.0f, 0.0f, -1.0f}));
    if (hit.hitSomething()) {
        finalHit = hit;
        return true;
    }
    // Just in case of backface culling, try the other side.
    const RayHit hitBack =
            _uvsRaycaster.intersect(Ray({u, v, -1.0f}, {0.0f, 0.0f, 1.0f}));
...
bool MeshTexturing::sampleNeighborhood(int px, int py, RayHit& hit) {
    ...
    // Sample a 3x3 neighborhood to counter-act aliasing/interpolation later on,
    // as long as we don't get a hit. The order is important, to first fetch in
    // line/column and then in diagonal. Sorry for the cache...
