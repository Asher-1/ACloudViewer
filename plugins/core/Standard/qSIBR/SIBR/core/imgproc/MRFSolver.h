...
public:
    typedef std::shared_ptr<std::function<double(int, int)>>
            UnaryFuncPtr;  ///< Unary cost function that depend on node
                           ///< attributes and its label.
    typedef std::shared_ptr<std::function<double(int, int, int, int)>>
            PairwiseFuncPtr;  ///< Pairwise cost function that depend on node
                              ///< attributes and their labels.

    typedef std::shared_ptr<std::function<double(int)>>
            UnaryLabelOnlyFuncPtr;  ///< Unary cost function that only depend on
                                    ///< the label.
    typedef std::shared_ptr<std::function<double(int, int)>>
            PairwiseLabelOnlyFuncPtr;  ///< Pairwise cost function that only
     ...
     *\param neighborMap connectivity map, for each node, list of neighboring
     *nodes linear indices \param numIterations number of expansion iterations
     *to perform \param unaryLabelOnly optional unary cost that only depends on
     *the label: f(lab0), else provide nullptr \param unaryFull unary (per node)
     *cost function evaluator, receiving the node linear index and label:
     *f(ind0, lab0) \param pairwiseLabelsOnly optional pairwise cost that only
     *depends on the labels: f(lab0, lab1), else provide nullptr \param
     *pairwiseFull pairwise (per pair of nodes) cost function evaluator,
     *receiving the nodes linear indices and their labels: f(ind0, ind1, lab0,
     *lab1) \note the "*LabelsOnly" functions are optional and are precomputed
     *and cached for optimized resolution.
     */
    MRFSolver(std::vector<int> labels,
    ...
    /// Solve using alpha expansion. When you have only two labels, use
    /// solveBinaryLabels instead
    void solveLabels(void);

    /// Solve for binary labels: if you only more than two labels, call
    /// solveLabels instead.
    void solveBinaryLabels(void);

    /** For each pixel, get the estimated label (call either solveLabels or
     solveBinaryLabels before). \return a list of labels, one per pixel */
...
private:
    ...
    std::vector<std::vector<int>>*
            _neighborMap;  ///< For each variable, gives the list of its
                           ///< neighbor variables
    int _numIterations;    ///< Number of iterations in alpha expansion.

    std::vector<double> _UnaryLabelOnly;  ///< Unaries only requiring label.
    std::shared_ptr<std::function<double(int, int)>>
            _unaryFull;  ///< Unaries requiring label and variable.
    std::vector<std::vector<double>>
            _PairwiseLabelsOnly;  ///< Pairwises only requiring labels.
    std::shared_ptr<std::function<double(int, int, int, int)>>
            _pairwiseFull;  ///< Pairwises requiring labels and variables.
