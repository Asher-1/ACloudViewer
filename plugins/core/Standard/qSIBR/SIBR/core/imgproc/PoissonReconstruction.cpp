...
PoissonReconstruction::PoissonReconstruction(const cv::Mat3f& gradientsX,
...
void PoissonReconstruction::solve(void) {
    ...
    for (int p = 0; p < (int)_pixels.size(); p++) {
        ...
        std::vector<sibr::Vector2i> nPos(
                getNeighbors(pos, _img_target.cols, _img_target.rows));
        int num_neighbors = 0;
        ...
        for (int n_id = 0; n_id < nPos.size(); n_id++) {
            ...
            if (isInMask(npos)) {  // pair inside mask
                ...
                new_term += _img_target.at<cv::Vec3f>(
                        npos.y(), npos.x());  // color of target
            }
        }
...
void PoissonReconstruction::parseMask(void) {
    ...
    // std::cerr << "size : " <<  _mask.cols << " x " << _mask.rows <<
    // std::endl;
    ...
    for (int j = 0; j < (int)_mask.rows; j++) {
        for (int i = 0; i < (int)_mask.cols; i++) {
            ...
            if (!isInMask(pos)) {
                std::vector<sibr::Vector2i> neighbors =
                        getNeighbors(pos, _img_target.cols, _img_target.rows);
                for (int n_id = 0; n_id < neighbors.size();
                     n_id++) {  // if at least one neighbor is in mask,
...
std::vector<sibr::Vector2i> PoissonReconstruction::getNeighbors(
        sibr::Vector2i pos, int width, int height) {
    std::vector<sibr::Vector2i> output;
    int offset_list_4[4][2] = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};

    for (int i = 0; i < 4; i++) {
        sibr::Vector2i n_pos(pos[0] + offset_list_4[i][0],
                             pos[1] + offset_list_4[i][1]);
        int x = n_pos.x();
        int y = n_pos.y();
...
void PoissonReconstruction::computeGradients(const cv::Mat3f& src,
                                             cv::Mat3f& gradX,
                                             cv::Mat3f& gradY) {
...
void PoissonReconstruction::checkConnectivity(void) {
    // R(x) : 0 -> not connected to boundary, 1 -> connected, G(y) : 0 -> not
    // checked, 1 -> checked
    ...
    for (int p = 0; p < (int)_boundaryPixels.size(); p++) {
        ...
        connectivity(pos.x(), pos.y()).x() =
                1;  // boundaries are connected to boundaries
    }
...
#pragma omp parallel for
    for (int j = 0; j < (int)_mask.rows; j++) {
        for (int i = 0; i < (int)_mask.cols; i++) {
            ...
            if (std::abs(_mask.at<float>(j, i)) < 0.5f &&
                cv::norm(_img_target.at<cv::Vec3f>(j, i)) == 0.0f) {
                std::vector<sibr::Vector2i> neighbors(getNeighbors(
                        sibr::Vector2i(i, j), _mask.cols, _mask.rows));
                std::vector<bool> neighIsBlack(neighbors.size(), false);
                int black_neighbor = false;
                for (uint n_id = 0; n_id < neighbors.size() && !black_neighbor;
                     n_id++) {
                    sibr::Vector2i npos(neighbors[n_id]);
                    if (cv::norm(_img_target.at<cv::Vec3f>(npos.y(),
                                                           npos.x())) == 0.0f) {
                ...
                if (!black_neighbor && neighbors.size() > 0) {
                    ...
                    for (uint n_id = 0; n_id < neighbors.size(); n_id++) {
                        ...
                        sibr::Vector2i npos(neighbors[n_id]);
                        new_color +=
                                _img_target.at<cv::Vec3f>(npos.y(), npos.x());
                        ++count;
                    }
                    _img_target.at<cv::Vec3f>(j, i) =
                            ((1.0f / (float)count) * new_color);
                }
            }
        }
    }
