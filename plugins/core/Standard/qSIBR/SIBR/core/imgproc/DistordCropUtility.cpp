...
void DistordCropUtility::addPixelToQueue(
        const sibr::Vector2i& pixel,
        const sibr::ImageRGB& img,
        std::priority_queue<sibr::Vector2i>& queue,
        sibr::Array2d<bool>& arrayVisited,
        Vector3i backgroundColor,
        int threshold_black_color) {
    if (!arrayVisited(pixel.x(), pixel.y()) &&
        isBlack(img(pixel.x(), pixel.y()), backgroundColor,
                threshold_black_color)) {
        queue.push(pixel);
        arrayVisited(pixel.x(), pixel.y()) = true;
    }
}

void DistordCropUtility::findBounds(sibr::Array2d<bool>& isBlack,
                                    DistordCropUtility::Bounds& bounds,
    ...
    while (wasUpdated) {
        ...
        for (int x = bounds.xMin; x <= bounds.xMax; ++x) {
            wasUpdated = wasUpdated || isBlack(x, bounds.yMax) ||
                         isBlack(x, bounds.yMin);
        }
        for (int y = bounds.yMin; y <= bounds.yMax; ++y) {
            wasUpdated = wasUpdated || isBlack(bounds.xMax, y) ||
                         isBlack(bounds.xMin, y);
        }
        ...
        if (bounds.xMax - bounds.xMin < thinest_bounding_box_size ||
            bounds.yMax - bounds.yMin < thinest_bounding_box_size) {
            break;
        }
    }
}

DistordCropUtility::Bounds DistordCropUtility::getBounds(
        const sibr::ImageRGB& img,
        Vector3i backgroundColor,
        int threshold_black_color,
        int thinest_bounding_box_size,
    ...
    //  add first row and last row of pixels to the pixelsQueue (if they are
    //  black) and marked them as visited
    for (int x = 0; x < w; ++x) {
        addPixelToQueue(sibr::Vector2i(x, 0), img, pixelsQueue, wasVisited,
                        backgroundColor, threshold_black_color);
        addPixelToQueue(sibr::Vector2i(x, h - 1), img, pixelsQueue, wasVisited,
                        backgroundColor, threshold_black_color);
    }

    // add left col and right col of pixels to the pixelsQueue (if they are
    // black) and marked them as visited
    for (int y = 0; y < h; ++y) {
    ...
    // neighbors shifts
    sibr::Vector2i shiftsArray[4] = {
            sibr::Vector2i(1, 0), sibr::Vector2i(-1, 0), sibr::Vector2i(0, -1),
            sibr::Vector2i(0, 1)};
    std::vector<sibr::Vector2i> shifts(
            shiftsArray,
            shiftsArray + sizeof(shiftsArray) / sizeof(sibr::Vector2i));
    ...
    bounds.width = int(float(int(img.w()) - proposedWidth) * toleranceFactor +
                       float(proposedWidth));
    bounds.height = int(float(int(img.h()) - proposedHeight) * toleranceFactor +
                        float(proposedHeight));

    return bounds;
}

sibr::Vector2i DistordCropUtility::calculateAvgResolution(
        const std::vector<Path>& imagePaths,
        std::vector<sibr::Vector2i>& resolutions,
        const int batch_size) {
    const int nrBatches =
            static_cast<int>(ceil((float)(imagePaths.size()) / batch_size));
    resolutions.resize(imagePaths.size());
    std::vector<std::pair<std::pair<long, long>, unsigned>> sumAndNrItems(
            nrBatches);
    ...
    for (int batchId = 0; batchId < nrBatches; batchId++) {
        const int nrItems =
                (batchId != nrBatches - 1)
                        ? batch_size
                        : ((nrBatches * batch_size != int(imagePaths.size()))
                                   ? (int(imagePaths.size()) -
                                      (batch_size * batchId))
                                   : batch_size);
        long sumOfWidths = 0;
...
#pragma omp parallel for
        for (int localImgIndex = 0; localImgIndex < nrItems; localImgIndex++) {
            ...
            chunkOfInputImages.at(localImgIndex)
                    .load(imagePaths.at(globalImgIndex).string(), false);

#pragma omp critical
            {
                sumOfWidths += long(chunkOfInputImages[localImgIndex].w());
                sumOfHeights += long(chunkOfInputImages[localImgIndex].h());
                resolutions[localImgIndex].x() =
                        chunkOfInputImages[localImgIndex].w();
                resolutions[localImgIndex].y() =
                        chunkOfInputImages[localImgIndex].h();
            }
        }
...
sibr::Vector2i DistordCropUtility::findBiggestImageCenteredBox(
        const Path& root,
        const std::vector<Path>& imagePaths,
        std::vector<sibr::Vector2i>& resolutions,
        int avgWidth,
    ...
    if (avgWidth == 0 || avgHeight == 0) {
        std::cout << "about to calculate avg resolution. use python "
                     "get_image_size script if dataset has too many images\n";
        sibr::Vector2i avgResolution =
                calculateAvgResolution(imagePaths, resolutions, batch_size);
        avgWidth = avgResolution.x();
        avgHeight = avgResolution.y();
    }

    std::cout << "[distordCrop] average resolution " << avgWidth << "x"
              << avgHeight
              << " and nr resolutions given: " << resolutions.size() << "\n";

    // discard images with different resolution
    std::vector<uint> preExcludedCams;
    for (unsigned i = 0; i < resolutions.size(); i++) {
        bool shrinkHorizontally = ((resolutions[i].x() < avgWidth) &&
                                   ((avgWidth - resolutions[i].x()) >
                                    avgWidth * resolutionThreshold))
                                          ? true
                                          : false;
        bool shrinkVertically = ((resolutions[i].y() < avgHeight) &&
                                 ((avgHeight - resolutions[i].y()) >
                                  avgHeight * resolutionThreshold))
                                        ? true
                                        : false;
        if (shrinkHorizontally || shrinkVertically) {
            preExcludedCams.push_back(i);
            std::cout << "[distordCrop] excluding input image " << i
                      << " resolution=" << resolutions[i].x() << "x"
                      << resolutions[i].y() << "\n";
        }
    }

    std::cout << "[distordCrop] nr pre excluded images "
              << preExcludedCams.size() << "\n";
    ...
    for (int batchId = 0; batchId < nrBatches; batchId++) {
        ...
        // load images in parallel (OpenMP 2.0 doesn't allow unsigned int as
        // index. must be signed integral type)
#pragma omp parallel for
        for (int localImgIndex = 0; localImgIndex < nrItems; localImgIndex++) {
            const uint globalImgIndex =
                    uint((batchId * batch_size) + localImgIndex);
            // if cam was discarded, do nothing
            if (std::find(preExcludedCams.begin(), preExcludedCams.end(),
                          globalImgIndex) == preExcludedCams.end()) {
                // only now load the img
                chunkOfInputImages.at(localImgIndex)
                        .load(imagePaths.at(globalImgIndex).string(), false);
                allBounds.at(globalImgIndex) =
                        getBounds(chunkOfInputImages.at(localImgIndex),
                                  backgroundColor, threshold_black_color,
                                  thinest_bounding_box_size, toleranceFactor);
            }
        }
    }
    ...
    for (auto& bounds : allBounds) {
        bool wasPreExcluded =
                std::find(preExcludedCams.begin(), preExcludedCams.end(),
                          im_id) != preExcludedCams.end();

        if (!wasPreExcluded &&
            bounds.xRatio > threshold_ratio_bounding_box_size &&
            bounds.yRatio > threshold_ratio_bounding_box_size) {
            ...
            minWidth = (minWidth < 0 || bounds.width < minWidth) ? bounds.width
                                                                 : minWidth;
            ...
            std::cout << wasPreExcluded << " " << bounds.xRatio << " "
                      << threshold_ratio_bounding_box_size << " "
                      << bounds.yRatio << " "
                      << threshold_ratio_bounding_box_size << std::endl;
        }
...
sibr::Vector2i DistordCropUtility::findMinImageSize(
        const Path& root, const std::vector<Path>& imagePaths) {
