...
MRFSolver::MRFSolver(std::vector<int> labels_list,
    ...
    if (unaryLabelOnly.get()) {
        SIBR_LOG << "[MRFSolver] unaryLabelOnly exists, precomputing."
                 << std::endl;
        ...
        SIBR_LOG << "[MRFSolver] unaryLabelOnly does not exist, skipping."
                 << std::endl;
    }

    // storing values for the pairwise part only requiring labels
    if (pairwiseLabelsOnly.get()) {
        SIBR_LOG << "[MRFSolver] pairwiseLabelsOnly exists, precomputing."
                 << std::endl;
        ...
        for (int l_id1 = 0; l_id1 < _labList.size(); l_id1++) {
            ...
            for (int l_id2 = 0; l_id2 < _labList.size(); l_id2++) {
                _PairwiseLabelsOnly[l_id1][l_id2] =
                        (*pairwiseLabelsOnly)(_labList[l_id1], _labList[l_id2]);
            }
        }
    } else {
        SIBR_LOG << "[MRFSolver] pairwiseLabelsOnly does not exist, skipping."
                 << std::endl;
    }
...
void MRFSolver::solveLabels(void) {
    ...
    SIBR_LOG << "[MRFSolver] Initialization : minimizing unaries..."
             << std::flush;
    ...
    SIBR_LOG << "[MRFSolver] Energies: U: " << computeEnergyU()
             << ", W: " << computeEnergyW() << std::endl;

    // Alpha-expansion algorithm
    SIBR_LOG << "[MRFSolver] Alpha-expansion [label,flow]..." << std::endl;
    for (int it = 0; it < _numIterations; it++) {
        SIBR_LOG << "[MRFSolver] Iteration " << (it + 1) << "/"
                 << (_numIterations) << ": " << std::endl;
        ...
        for (int label_id = 0; label_id < (int)_labList.size(); label_id++) {
            ...
            for (int p = 0; p < num_nodes; p++) {
                if (_graph->what_segment(p) == GraphType::SINK) {
                    if (_labels[p] != label_id) {
                        ++num_change;
                    }
                    _labels[p] = label_id;
                }
            }
            SIBR_LOG << "[MRFSolver]\t\tLabel " << label
                     << ": modifications = " << num_change
...
void MRFSolver::buildGraphAlphaExp(int label_iteration_id) {
    ...
    for (int p = 0; p < num_nodes; p++) {
        ...
        if (_labels[p] == label_iteration_id) {
            _graph->add_tweights(node_id, unaryTotal(p, label_iteration_id),
                                 infty);
        } else {
            _graph->add_tweights(node_id, unaryTotal(p, label_iteration_id),
                                 unaryTotal(p, _labels[p]));
        }
        ++node_id;
    }
    ...
    for (int p = 0; p < num_nodes; p++) {
        ...
        for (int q_id = 0; q_id < (int)neighors.size(); q_id++) {
            ...
            if (_labels[p] != _labels[q]) {
                ...
                _graph->add_tweights(
                        node_id, 0,
                        pairwiseTotal(q, p, _labels[q], _labels[p]));

                double pairwise_q_a =
                        pairwiseTotal(q, p, _labels[q], label_iteration_id);
                _graph->add_edge(q, node_id, pairwise_q_a, pairwise_q_a);

                double pairwise_p_a =
                        pairwiseTotal(q, p, label_iteration_id, _labels[p]);
...
void MRFSolver::solveBinaryLabels(void) {
    ...
    if (numLabels < 2) {
        SIBR_ERR << "[MRFSolver] solveBinaryLabels, expected 2 labels, only "
                 << numLabels << " labels " << std::endl;
    } else if (numLabels > 2) {
        SIBR_WRG << "[MRFSolver] solveBinaryLabels, found " << numLabels
                 << " labels, only the first two will be used." << std::endl;
    }
...
std::vector<double> MRFSolver::getUnariesEnergies(void) {
    if (_labels.size() == 0) {
        std::cerr << "[MRFSolver] warning getUnariesEnergies without nodes"
                  << std::endl;
...
double MRFSolver::getTotalEnergy(void) { return _energy; }
