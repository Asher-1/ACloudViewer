...
void LoadingProgress::walk(size_t step) {
    ...
    _currentStep += step;
    if (std::chrono::duration<float>(clock::now() - _lastReport).count() >=
                _interval ||
...
void LoadingProgress::report(void) const {
    if (_status.empty())
        ...
        SIBR_LOG << "Progression [ " << current() * 100.f << "% ] - " << _status
                 << std::endl;
}
