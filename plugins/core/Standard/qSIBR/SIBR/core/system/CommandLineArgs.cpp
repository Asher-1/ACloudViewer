...
CommandLineArgs& CommandLineArgs::getGlobal() {
    ...
    if (!global.init && first) {
        SIBR_WRG << "CommandLineArgs::parseMainArgs(ac, av) was not called "
                    "rigth after main(ac, av) \n default value (empty command "
                    "line) will be used"
                 << std::endl;
        first = false;
    }
...
#ifdef WIN32
        if (tgreen) restoreConsole();
#endif
    }
    std::cout << std::endl;
}

void CommandLineArgs::registerCommand(const std::string& key,
...
void CommandLineArgs::registerRequiredCommand(const std::string& key,
                                              const std::string& description) {
    // Register the command.
    std::string defaultDesc = description.empty() ? "" : " ";
...
AppArgs::AppArgs() {
    Path path =
            CommandLineArgs::getGlobal().getRequired<std::string>("app_path");
