...
namespace sibr {
std::string strSearchAndReplace(const std::string& src,
                                const std::string& search,
                                const std::string& replaceby) {
    size_t pos = src.find(search);
    if (pos != std::string::npos) {
        std::string out;
        out = src.substr(0, pos) + replaceby +
              src.substr(pos + search.size(), src.size() - pos + search.size());
        return out;
    }
    return src;
}
...
std::vector<std::string> split(const std::string& str, char delim) {
    ...
    while (std::getline(ss, to, delim)) out.push_back(to);
    return out;
}
...
SIBR_SYSTEM_EXPORT std::string to_lower(const std::string& str) {
    ...
    for (size_t i = 0; i < str.length(); ++i) out.push_back(tolower(str[i]));

    return out;
}

SIBR_SYSTEM_EXPORT bool find_any(const std::vector<std::string>& needles,
