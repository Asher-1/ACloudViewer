...
sibr::ULRV3Renderer::ULRV3Renderer(const std::vector<InputCamera::Ptr>& cameras,
    ...
    const unsigned int maxCamerasAllowed =
            std::min((unsigned int)maxSlicesSize,
                     (unsigned int)(maxBlockSize / bytesPerCamera));
    std::cout << "[ULRV3Renderer] "
              << "MAX_UNIFORM_BLOCK_SIZE: " << maxBlockSize
              << ", MAX_ARRAY_TEXTURE_LAYERS: " << maxSlicesSize
              << ", meaning at most " << maxCamerasAllowed << " cameras."
              << std::endl;
    ...
    glBufferData(GL_UNIFORM_BUFFER, sizeof(CameraUBOInfos) * _maxNumCams,
                 &_cameraInfos[0], GL_DYNAMIC_DRAW);
...
void sibr::ULRV3Renderer::setupShaders(const std::string& fShader,
                                       const std::string& vShader) {
    // Create shaders.
    std::cout << "[ULRV3Renderer] Setting up shaders for " << _maxNumCams
              << " cameras." << std::endl;
    ...
    _ulrShader.init("ULRV3",
                    sibr::loadFile(sibr::getShadersDirectory("") + "/" +
                                   vShader + ".vert"),
                    sibr::loadFile(sibr::getShadersDirectory("") + "/" +
                                           fShader + ".frag",
                                   defines));
    _depthShader.init("ULRV3Depth",
                      sibr::loadFile(sibr::getShadersDirectory("ulr") +
                                     "/ulr_intersect.vert"),
                      sibr::loadFile(sibr::getShadersDirectory("ulr") +
                                             "/ulr_intersect.frag",
                                     defines));
...
void sibr::ULRV3Renderer::process(
    ...
    if (_profiling) {
        glFinish();
        // std::cout << "\nDepth Pass: " <<
        // _depthPassTimer.deltaTimeFromLastTic() << " ms" << std::endl;
    ...
    if (_profiling) {
        glFinish();
        // std::cout << "\nBlend Pass: " <<
        // _blendPassTimer.deltaTimeFromLastTic() << " ms" << std::endl;
...
void sibr::ULRV3Renderer::updateCameras(const std::vector<uint>& camIds) {
    ...
    glBindBuffer(GL_UNIFORM_BUFFER, _uboIndex);
    glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(CameraUBOInfos) * _maxNumCams,
                    &_cameraInfos[0]);
    glBindBuffer(GL_UNIFORM_BUFFER, 0);
}
...
void sibr::ULRV3Renderer::stopProfile() {
    ...
    const std::vector<std::vector<float>> counts = {_depthCost, _blendCost};
    ...
    for (int i = 0; i < names.size(); ++i) {
        ...
        profileStr += "-----------\n";
        profileStr += names[i] +
                      " num frames: " + std::to_string(counts[i].size()) + "\n";
        profileStr += names[i] + " min/max: " + std::to_string(miniF) + "/" +
                      std::to_string(maxiF) + "\n";
        profileStr += names[i] + " avg/stddev: " + std::to_string(avgF) + "/" +
                      std::to_string(std::sqrt(varF)) + "\n";
    }

    std::cout << profileStr << std::endl;
}

void sibr::ULRV3Renderer::renderProxyDepth(const sibr::Mesh& mesh,
                                           const sibr::Camera& eye) {
    // Bind and clear RT.
    _depthRT->bind();
