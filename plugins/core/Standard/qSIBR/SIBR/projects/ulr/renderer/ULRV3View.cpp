...
sibr::ULRV3View::ULRV3View(const sibr::BasicIBRScene::Ptr& ibrScene,
                           uint render_w,
                           uint render_h)
    ...
    //  Renderers.
    _ulrRenderer.reset(
            new ULRV3Renderer(ibrScene->cameras()->inputCameras(), w, h));
...
void sibr::ULRV3View::setScene(const sibr::BasicIBRScene::Ptr& newScene) {
    ...
    _ulrRenderer.reset(new ULRV3Renderer(newScene->cameras()->inputCameras(), w,
                                         h, shaderName));
...
void sibr::ULRV3View::onRenderIBR(sibr::IRenderTarget& dst,
    ...
    _ulrRenderer->process(_scene->proxies()->proxy(), eye,
...
void sibr::ULRV3View::onGUI() {
    ...
    if (ImGui::Begin(guiName.c_str())) {
        // Poisson settings.
        ImGui::Checkbox("Poisson ", &_poissonBlend);
        ImGui::SameLine();
        ...
        ImGui::InputFloat("Epsilon occlusion",
                          &_ulrRenderer->epsilonOcclusion(), 0.001f, 0.01f);

        ImGui::Separator();
        // Rendering mode selection.
        if (ImGui::Combo("Rendering mode", (int*)(&_renderMode),
                         "Standard\0One image\0Leave one out\0Every N\0\0")) {
            updateCameras(true);
        }

        // Get the desired index, make sure it falls in the cameras range.
        if (_renderMode == ONE_CAM || _renderMode == LEAVE_ONE_OUT) {
            const bool changedIndex =
                    ImGui::InputInt("Selected image", &_singleCamId, 1, 10);
            _singleCamId = sibr::clamp(
                    _singleCamId, 0,
                    (int)_scene->cameras()->inputCameras().size() - 1);
            if (changedIndex) {
                // If we are in "leave one out" or "one camera only" mode, we
                // have to update the list of enabled cameras.
                updateCameras(false);
            }
        }
        ...
        if (ImGui::Combo("Weights mode", (int*)(&_weightsMode),
                         "Standard ULR\0Variance based\0Fast ULR\0\0")) {
...
void sibr::ULRV3View::updateCameras(bool allowResetToDefault) {
    // If we are here, the rendering mode or the selected index have changed, we
    // need to update the enabled cameras.
    std::vector<uint> imgs_ulr;
    ...
    if (_renderMode == RenderMode::ONE_CAM) {
        ...
        if (cams[_singleCamId]->isActive()) {
            ...
            std::cerr << "The camera is not active, using all cameras."
                      << std::endl;
        }
