...
namespace sibr {
ULRV2Renderer::ULRV2Renderer(const std::vector<InputCamera::Ptr>& cameras,
                             const uint w,
                             const uint h,
                             const unsigned int maxCams,
...
void ULRV2Renderer::setupULRshader(const std::string& fShader,
                                   const std::string& vShader) {
    std::cerr << "[ULRV2Renderer] Trying to initialize shaders for at most "
              << _numCams << " cameras." << std::endl;
    /// \todo TODO SR: handle the case were we require more shader texture slots
    /// than we are allowed too. Seems to be around 90 on Quadro K4200. We can
    /// either do multiple passes (fi 40 cams per pass), or try to use texture
    /// arrays to avoid this problem. If this happens to you, lower the maximum
    /// number of cameras picked by the ulr algo.
    ...
    for (uint i = 0; i < (uint)_numCams; i++) {
        ...
        _selected_cams[i].init(_ulrShader,
                               sibr::sprint("selected_cams[%d]", i));
...
void ULRV2Renderer::process(
        const std::vector<uint>& imgs_ulr,
    ...
    if (altMesh != nullptr) {
        altMesh->render(
                true, _shouldCull);  // enable depth test - disable back culling
    } else {
    ...
    _use_soft_visibility.set(soft_visibility_maps != nullptr &&
                             soft_visibility_maps->handle());
