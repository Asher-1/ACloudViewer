...
void ULRRenderer::process(
    ...
    if (altMesh != nullptr)
        altMesh->render(true,
                        true);  // enable depth test - disable back culling
    else
        scene->proxies()->proxy().render(
                true, true);  // enable depth test - disable back culling
    ...
    for (uint i = 0; i < imgs_ulr.size(); i++) {
        if (scene->cameras()->inputCameras()[imgs_ulr[i]]->isActive()) {
            const sibr::InputCamera& cam =
                    *scene->cameras()->inputCameras()[imgs_ulr[i]];
            ...
            glBindTexture(GL_TEXTURE_2D, _depth_RT->texture());
            glActiveTexture(GL_TEXTURE2);
            glBindTexture(GL_TEXTURE_2D, _ulr1_RT->texture(0));
            glActiveTexture(GL_TEXTURE3);
            glBindTexture(GL_TEXTURE_2D, _ulr1_RT->texture(1));
            glActiveTexture(GL_TEXTURE4);
            glBindTexture(GL_TEXTURE_2D, _ulr1_RT->texture(2));
            glActiveTexture(GL_TEXTURE5);
            glBindTexture(GL_TEXTURE_2D, _ulr1_RT->texture(3));
            if (useMasks()) {
                glActiveTexture(GL_TEXTURE6);
                glBindTexture(GL_TEXTURE_2D,
                              getMasks()[imgs_ulr[i]]->texture());
            }
    ...
    glBindTexture(GL_TEXTURE_2D, _depth_RT->texture());
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, _ulr0_RT->texture(0));
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, _ulr0_RT->texture(1));
    glActiveTexture(GL_TEXTURE3);
    glBindTexture(GL_TEXTURE_2D, _ulr0_RT->texture(2));
    glActiveTexture(GL_TEXTURE4);
    glBindTexture(GL_TEXTURE_2D, _ulr0_RT->texture(3));
    sibr::RenderUtility::renderScreenQuad();
    dst.unbind();
    _ulrShaderPass2.end();
