...
namespace sibr {
ULRV2View::~ULRV2View() { _altMesh.reset(); }

ULRV2View::ULRV2View(const sibr::BasicIBRScene::Ptr& ibrScene,
                     uint render_w,
                     uint render_h)
    : _scene(ibrScene),
      sibr::ViewBase(render_w, render_h),
      _renderMode(ULRV2View::RenderMode::NORMAL),
      _singleCamId(0) {
    _altMesh.reset();
    _altMesh = nullptr;
    _numDistUlr = 4, _numAnglUlr = 0;
    std::cerr << "[ULR] setting number of images to blend " << _numDistUlr
              << " " << _numAnglUlr << std::endl;

    _ulr.reset(new ULRV2Renderer(ibrScene->cameras()->inputCameras(), render_w,
                                 render_h, _numDistUlr + _numAnglUlr));
    uint w = render_w;
    uint h = render_h;
...
void ULRV2View::onRenderIBR(sibr::IRenderTarget& dst, const sibr::Camera& eye) {
    ...
    if (_renderMode == RenderMode::ONLY_ONE_CAM) {
        ...
        int id_cam =
                std::max(0, std::min((int)imgs_ulr.size() - 1, _singleCamId));
        int cam = imgs_ulr[id_cam];
        imgs_ulr = std::vector<uint>(1, cam);
        // std::cout << " -> ulr debug single cam, id : " << _singleCamId << ",
        // cam : "; for (auto i : imgs_ulr) { std::cout << i << " ";
        //}
        // std::cout << std::endl;
    ...
    if (_noPoissonBlend) {
        _ulr->process(imgs_ulr, eye, _scene, _altMesh, _inputRTs, dst);
...
void ULRV2View::onUpdate(Input& input) {
    if (input.key().isReleased(sibr::Key::Tab)) {
        ...
        std::cout << "ULR using "
                  << (testAltlULRShader ? "all cams" : "standard ulr")
                  << std::endl;
    }
}
...
void ULRV2View::onGUI() {
    ...
    if (ImGui::Begin(guiName.c_str())) {
        ...
        ImGui::InputFloat("Epsilon occlusion", &_ulr->epsilonOcclusion(),
                          0.001f, 0.01f);
        ImGui::Combo("Rendering mode", (int*)(&_renderMode),
                     "Standard\0One image\0Leave one out\0\0");
        if (ImGui::InputInt("Selected image", &_singleCamId, 1, 10)) {
            _renderMode = RenderMode::ONLY_ONE_CAM;
        }
        _singleCamId =
                sibr::clamp(_singleCamId, 0,
                            (int)_scene->cameras()->inputCameras().size() - 1);
        // ImGui::SliderInt("Selected image", &_singleCamId, 0,
        // scene().inputCameras().size() - 1);
        ImGui::PopItemWidth();
    }
    ImGui::End();
}

void ULRV2View::computeVisibilityMap(const sibr::ImageL32F& depthMap,
                                     sibr::ImageRGBA& out) {
    const float threshold_3d = 2.5f;
    const std::vector<sibr::Vector2i> shifts = {
            {1, 0}, {0, 1}, {-1, 0}, {0, -1}};
    ...
    for (uint i = 0; i < depthMap.h(); i++) {
        for (uint j = 0; j < depthMap.w(); j++) {
            ...
            for (const auto& shift : shifts) {
                ...
                if (!depthMap.isInRange(npos)) {
                    continue;
                }
                if (std::abs(depthMap(npos).x() - currentDepth) >
                    threshold_3d) {
                    edgeMap(pos).x() = 0;
                    break;
                }
    ...
    cv::distanceTransform(edgeMap.toOpenCVnonConst(), distance, cv::DIST_L2,
...
std::vector<uint> ULRV2View::chosen_cameras(const sibr::Camera& eye) {
    ...
    std::multimap<float, uint>
            dang;  // angular distance from inputs to novel camera
    for (uint i = 0; i < _scene->cameras()->inputCameras().size(); i++) {
        const sibr::InputCamera& inputCam =
                *_scene->cameras()->inputCameras()[i];
        if (inputCam.isActive()) {
            ...
            distMap.insert(std::make_pair(
                    dist, i));  // sort distances in increasing order
            dang.insert(std::make_pair(acos(angle),
                                       i));  // sort angles in increasing order
        }
    }
    for (uint i = 0; i < _scene->cameras()->inputCameras().size(); i++) {
        const sibr::InputCamera& inputCam =
                *_scene->cameras()->inputCameras()[i];
        if (inputCam.isActive() &&
            distMap.size() <= (_numDistUlr + _numAnglUlr) / 2) {
            float dist = sibr::distance(inputCam.position(), eye.position());
            distMap.insert(std::make_pair(
                    dist, i));  // sort distances in increasing order
        }
    ...
    std::multimap<float, uint>::const_iterator d_it(
            distMap.begin());  // select the _numDistUlr closest cameras
    for (int i = 0; d_it != distMap.end() && i < _numDistUlr; d_it++, i++) {
        imgs_id.push_back(d_it->second);
    }

    std::multimap<float, uint>::const_iterator a_it(
            dang.begin());  // select the NUM_ANG_ULR closest cameras
...
std::vector<uint> ULRV2View::chosen_cameras_angdist(const sibr::Camera& eye) {
    ...
    struct camAng {
        ...
        float ang, dist;
        int id;
        static bool compare(const camAng& a, const camAng& b) {
            return a.ang / a.dist > b.ang / b.dist;
        }
    };
...
std::vector<uint> ULRV2View::chosen_camerasNew(const sibr::Camera& eye) {
    ...
    struct camDist {
        ...
        float dist;
        int id;
        static bool compare(const camDist& a, const camDist& b) {
    ...
    for (int id = 0; id < (int)cams.size(); ++id) {
        const auto& cam = *cams[id];
        allDist.push_back(
                camDist((cam.position() - eye.position()).norm(), id));
    }
...
void ULRV2View::setNumBlend(short int dist, short int angle) {
    ...
    std::cerr << "[ULR] setting number of images to blend " << _numDistUlr
              << " " << _numAnglUlr << std::endl;
    _ulr.reset(new ULRV2Renderer(_scene->cameras()->inputCameras(),
                                 _scene->cameras()->inputCameras()[0]->w(),
                                 _scene->cameras()->inputCameras()[0]->h(),
                                 _numDistUlr + _numAnglUlr));
    _ulr->setMasks(copyMasks);
}

void ULRV2View::loadMasks(const sibr::BasicIBRScene::Ptr& ibrScene,
                          int w,
                          int h,
                          const std::string& maskDir,
                          const std::string& preFileName,
                          const std::string& postFileName) {
    std::string finalMaskDir =
            (maskDir == "" ? ibrScene->data()->basePathName() + "/masks/"
                           : maskDir);
    std::string finalPostFileName =
            (postFileName == "" ? "-mask.jpg" : postFileName);
    _ulr->loadMasks(ibrScene, finalMaskDir, preFileName, finalPostFileName, w,
                    h);
}
