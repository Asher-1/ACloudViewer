...
int main(int ac, char** av) {
    ...
    // Window setup
    sibr::Window window(
            PROGRAM_NAME, sibr::Vector2i(50, 50), myArgs,
            getResourcesDirectory() + "/ulr/" + PROGRAM_NAME + ".ini");
    ...
    rendering_width = (rendering_width <= 0)
                              ? scene->cameras()->inputCameras()[0]->w()
                              : rendering_width;
    rendering_height = (rendering_height <= 0)
                               ? scene->cameras()->inputCameras()[0]->h()
                               : rendering_height;
    Vector2u usedResolution(rendering_width, rendering_height);
    std::cerr << " USED RES " << usedResolution << " scene w h " << scene_width
              << " : " << scene_height << " NAME "
              << scene->cameras()->inputCameras()[0]->name() << std::endl;
    ...
    scene->renderTargets()->initRGBandDepthTextureArrays(
            scene->cameras(), scene->images(), scene->proxies(), flags, true,
            myArgs.force_aspect_ratio);
    ...
    if (myArgs.masks) {
        if (!myArgs.maskParams.get().empty()) {
            if (!myArgs.maskParamsExtra.get().empty()) {
                ...
                ulrView->getULRrenderer()->loadMasks(
                        scene, myArgs.maskParams.get(), "", ".png");
            }
        } else {
    ...
    std::shared_ptr<sibr::Raycaster> raycaster =
            std::make_shared<sibr::Raycaster>();
    raycaster->init();
    raycaster->addMesh(scene->proxies()->proxy());

    // Camera handler for main view.
    sibr::InteractiveCameraHandler::Ptr generalCamera(
            new InteractiveCameraHandler());
    generalCamera->setup(scene->cameras()->inputCameras(),
                         Viewport(0, 0, (float)usedResolution.x(),
                                  (float)usedResolution.y()),
                         raycaster);

    // Add views to mvm.
    MultiViewManager multiViewManager(window, false);
    multiViewManager.addIBRSubView("ULR view", ulrView, usedResolution,
    ...
    if (myArgs.offscreen || myArgs.pathFile.get() != "") {
        generalCamera->getCameraRecorder().loadPath(
                myArgs.pathFile.get(), usedResolution.x(), usedResolution.y());
        generalCamera->getCameraRecorder().recordOfflinePath(
                myArgs.outPath, multiViewManager.getIBRSubView("ULR view"), "");
        if (!myArgs.noExit) exit(0);
    }

    // Top view
    const std::shared_ptr<sibr::SceneDebugView> topView(
            new sibr::SceneDebugView(scene, generalCamera, myArgs));
...
int legacyV2main(ULRAppArgs& myArgs) {
    {
        ...
        ULRV2View::Ptr ulrView(
                new ULRV2View(scene, sceneResWidth, sceneResHeight));
        ...
        std::vector<sibr::ImageL32F> depths3D(
                scene->cameras()->inputCameras().size());
            ...
            for (int imId = 0; imId < numImages; ++imId) {
                ...
                for (int i = 0; i < w; i++) {
                    for (int j = 0; j < h; j++) {
                        ...
                        sibr::Vector3f pos3dMesh(cam.unprojectImgSpaceInvertY(
                                pixelPos, depthMapSIBR(i, j).x()));
        ...
        if (myArgs.masks) {
            if (!myArgs.maskParams.get().empty()) {
                ulrView->loadMasks(scene, usedResolution.x(),
                                   usedResolution.y(), myArgs.maskParams.get(),
                                   "", ".png");
            } else {
                ulrView->loadMasks(scene, usedResolution.x(),
                                   usedResolution.y());
...
#pragma omp parallel for
            for (int imId = 0; imId < numImages; ++imId) {
                ...
                softVisibilities[imId] =
                        std::move(sibr::convertRGBAtoL32F(tempVisibility));
                cv::Mat temp;
                cv::resize(softVisibilities[imId].toOpenCV(), temp,
                           cv::Size(wSoft, hSoft), 0, 0, cv::INTER_NEAREST);
                softVisibilities[imId].fromOpenCV(temp);
            }

            soft_visibility_textures.createFromImages(
                    softVisibilities,
                    SIBR_GPU_LINEAR_SAMPLING | SIBR_FLIP_TEXTURE);
        ...
        if (myArgs.pathFile.get() != "") {
            generalCamera->getCameraRecorder().loadPath(myArgs.pathFile.get(),
                                                        usedResolution.x(),
                                                        usedResolution.y());
            generalCamera->getCameraRecorder().recordOfflinePath(
                    myArgs.outPath, multiViewManager.getIBRSubView("ULR view"),
        ...
        while (window.isOpened()) {
            ...
            if (sibr::Input::global().key().isPressed(sibr::Key::Z)) {
                if (ulrView->_ulr->getSoftVisibilityMaps()) {
                    ...
                    ulrView->_ulr->getSoftVisibilityMaps() =
                            &soft_visibility_textures;
                }
            }
