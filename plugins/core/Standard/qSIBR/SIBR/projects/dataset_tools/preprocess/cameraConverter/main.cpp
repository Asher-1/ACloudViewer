...
struct CameraConverterArgs : virtual AppArgs {
    ...
    RequiredArg<std::string> colmapPath = {
            "colmapPath", "path to colmap recon for camera file"};
    Arg<std::string> transfo = {"transfo", "", "matrix file"};
    Arg<sibr::Vector2u> inputRes = {
            "ires",
            {1920, 1080},
            "input camera resolution (not required for all formats)"};
    Arg<sibr::Vector2u> outputRes = {
            "ores",
            {1920, 1080},
            "output camera resolution (not required for all formats)"};
    Arg<bool> inverse = {"inverse", "reverse the transformation"};
    Arg<bool> bundleImageList = {"images_list",
                                 "for a bundle output, output list_images.txt"};
    Arg<bool> bundleImageFiles = {"images_files",
                                  "for a bundle output, output empty images in "
                                  "a 'visualize' subdirectory"};
    Arg<std::string> inImageFilePath = {
            "in_images_files", "",
            "for a bundle input images file directory (for list_images etc)"};
...
void colmapSave(const std::string& filename,
    ...
    if (focalx == -1) {
        focalx = xformPath[0]->focal() *
                 xformPath[0]->aspect();  // use aspect ratio
        SIBR_WRG << "No focal x given making it equal to focaly * aspect "
                    "ratio; use result at own risk. Should have a colmap "
                    "dataset as input"
                 << std::endl;
    } else {
    ...
    for (int i = 0; i < xformPath.size(); i++) {
        outputColmapPathCams
                << i + 1 << " PINHOLE " << xformPath[0]->w() * scale << " "
                << xformPath[0]->h() * scale << " "
                << xformPath[0]->focal() * scale << " " << focalx * scale << " "
                << xformPath[0]->w() * scale * 0.5 << " "
                << xformPath[0]->h() * scale * 0.5 << std::endl;
    }
    ...
    for (int i = 0; i < xformPath.size(); i++) {
        ...
        outputColmapPath << i << " " << q.w() << " " << -q.x() << " " << -q.y()
                         << " " << -q.z() << " " << t.x() << " " << t.y() << " "
                         << t.z() << " " << 1 << " " << "pathImage" << i
                         << std::endl;
...
/** The camera converter is a utility to convert a camera path from a file
 * format to another, with additional options. You can specify a 4x4
 * transformation matrix to apply, stored in a txt file (values separated by
 * spaces/newlines, f.i. the output of CloudCompare alignment). If you want to
 * apply the inverse transformation, use the --inverse flag. Supported inputs:
 * path (*), lookat (*), bundle, nvm (*), colmap Supported outputs: path,
 * bundle(*), lookat, colmap (images.txt)
...
int main(int ac, char** av) {
    ...
    if (ext == "path") {
        ...
        if (std::string(args.inImageFilePath) == "")
            SIBR_ERR << "Please provide image file directory for bundler input "
                        "(use option -in_images_files "
                        "DIRECTORY_CONTAINING_LIST_IMAGES.txt )\nIf necessary "
                        "use the generate_list_images.py script to generate "
                        "list_images.txt "
                     << std::endl;
        cams = InputCamera::loadBundle(args.input, 0.01, 1000,
                                       args.inImageFilePath, true);
    } else if (ext == "nvm") {
        cams = InputCamera::loadNVM(args.input, 0.01f, 1000.0f,
                                    {args.inputRes});
        ...
        SIBR_ERR << "Unsupported path file extension: " << ext << "."
                 << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (args.colmapPath != "") {
        ...
        if (directoryExists(cm_sparse_path)) {
            ...
            std::cerr << "Found " << camsFx.size() << " cameras fovx "
                      << camsFx[0]->focalx() << std::endl;
    ...
    if (!transf.isIdentity()) {
        SIBR_LOG << "Applying transformation: " << std::endl
                 << transf << std::endl;
        for (auto& cam : cams) {
    ...
    if (outExt == "path") {
        ...
        for (const auto& cam : cams) {
            ...
            InputCamera::Ptr oc;
            outCams.push_back(
                    oc = std::make_shared<InputCamera>(*cam, outW, outH));
            // reset focal
            oc->setFocal(cam->focal());
        }
        sibr::InputCamera::saveAsBundle(outCams, args.output,
                                        args.bundleImageList,
                                        args.bundleImageFiles, false);
        ...
        SIBR_ERR << "Unsupported output file extension: " << outExt << "."
                 << std::endl;
        return EXIT_FAILURE;
    }
    SIBR_LOG << "Saved transformed cameras to \"" << args.output.get() << "\"."
             << std::endl;
