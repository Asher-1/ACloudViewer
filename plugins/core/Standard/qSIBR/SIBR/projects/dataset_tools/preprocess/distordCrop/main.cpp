...
const int PROCESSING_BATCH_SIZE =
        150;  // process PROCESSING_BATCH_SIZE images together

sibr::Vector3i backgroundColor = sibr::Vector3i(0, 0, 0);

/*
        if input image resolution is too different from the avg, it will be
   discarded automatically at the beginning
*/
float resolutionThreshold = 0.15f;

/*
        tolerance factor is used to allow somehow some black borders in the
   final images. if tolerance factor is zero, then all black borders are remove.
...
struct DistordCropAppArgs : virtual sibr::BasicIBRAppArgs {
    ...
    sibr::Arg<sibr::Vector3i> backgroundColor = {"backgroundColor",
                                                 sibr::Vector3i(0, 0, 0)};
};

/*
utility program that determines a new resolution taking into account that some
input images have black borders added by reality capture. the second output of
the program [optional] is a excludeImages.txt file containing the id of the
images that didn't pass the threshold test (they would have to be cropped to
much). current pipeline (IBR_recons_RC.py) doesn't used that file properly.

we might need to call process_cam_selection manually passing as argument the
excludeImages.txt in order to actually remove the cameras that didn't pass the
threshold test

update: reality capture (using the 'fit' option when exporting bundle) sometimes
produces datasets that have images not only with black borders but also with a
completely different resolution. We need to take into account those datasets
too.
*/
...
int main(const int argc, const char* const* argv) {
    ...
    BOOST_FOREACH (Path const& p, std::make_pair(it, eod)) {
        if (is_regular_file(p) &&
            (p.extension() == ".jpg" || p.extension() == ".JPG" ||
             p.extension() == ".PNG" || p.extension() == ".png") &&
            appUtils.is_number(p.stem().string())) {
    ...
    if (resolutions.size() == 0) {
        std::cout << "[distordCrop] WARNING : no resolution.txt file found"
                  << std::endl;
        return 0;
    }

    if (imagePaths.size() == 0) {
        std::cout << "[distordCrop] WARNING: no images found: need "
                     ".jpg,.JPG,.png,.PNG "
                  << std::endl;
        return 0;
    }
    ...
    if (resolutions.size() != imagePaths.size()) {
        std::cout << "[distordCrop] WARNING : different number of input images "
                     "and resolutions written in resolutions.txt"
                  << std::endl;
        return 0;
    }
    ...
    if (sameSize) {
        ...
        sibr::Vector2i minSize = appUtils.findBiggestImageCenteredBox(
                root, imagePaths, resolutions, avgWidth, avgHeight,
                PROCESSING_BATCH_SIZE, resolutionThreshold,
                threshold_ratio_bounding_box_size, backgroundColor,
                threshold_black_color, thinest_bounding_box_size,
                toleranceFactor);

        std::cout << "[distordCrop] minSize " << minSize[0] << "x" << minSize[1]
                  << std::endl;
        minWidth = minSize[0];
        minHeight = minSize[1];
    } else {
    ...
    while ((new_half_w % 4) != 0) {
        --new_half_w;
    }
    while ((new_half_h % 4) != 0) {
        --new_half_h;
    }
    ...
    if (file) {
        ...
        std::cout << "[distordCrop]  ERROR cant open file : " << outputFilePath
                  << std::endl;
        return 1;
    }

    std::cout << "[distordCrop] done, new size is " << 2 * new_half_w << " x "
              << 2 * new_half_h << std::endl;
