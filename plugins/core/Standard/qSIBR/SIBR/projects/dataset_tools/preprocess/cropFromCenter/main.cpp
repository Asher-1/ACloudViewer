...
/*
Crop input images from center so they end up with resolution <crop_width> x
<crop_height> if scale down factor is also passed, after the image has been
cropped, it will be scaled down by that value
*/
static const char* USAGE =
        "Usage: cropFromCenter --inputFile <path_to_input_file> --outputPath "
        "<path_to_output_folder> --avgResolution <width x height> "
        "--cropResolution <width x height> [--scaleDownFactor <alpha> "
        "--targetResolution <width x height>] \n";
// const char* USAGE						= "Usage:
// cropFromCenter --inputFile <path_to_input_file> --outputPath
// <path_to_output_folder> --avgResolution <width x height> --cropResolution
// <widht x height> [--scaleDownFactor <alpha> --targetResolution <width x
// height>] \n";
static const char* TAG = "[cropFromCenter]";
...
struct CropAppArgs : virtual sibr::BasicIBRAppArgs {
    ...
    sibr::Arg<sibr::Vector2i> targetResolutionArg = {"targetResolution",
                                                     {0, 0}};
};

void printUsage() { std::cout << USAGE << std::endl; }

bool getParamas(int argc,
                const char** argv,
    ...
    if (inputFile.empty() || outputFolder.empty() ||
        avgResolution == sibr::Vector2i(0, 0) ||
        cropResolution == sibr::Vector2i(0, 0)) {
        return false;
    }
...
int main(const int argc, const char** argv) {
    ...
    if (!getParamas(argc, argv, inputFileName, outputFolder,
                    avgInitialResolution, cropResolution, scaleDownFactor,
                    targetResolution)) {
    ...
    // cv::Size resizedSize (finalResolution[0], cropResolution[1] *
    // ((float)(finalResolution[0]) / cropResolution[0]));
    cv::Size resizedSize(int(cropResolution[0] * scaleDownFactor),
                         int(cropResolution[1] * scaleDownFactor));
    ...
    std::vector<std::string> pathToImgs =
            appUtility.getPathToImgs(inputFileName);
    std::vector<sibr::CropScaleImageUtility::Image> listOfImages(
            pathToImgs.size());
    std::vector<sibr::CropScaleImageUtility::Image> listOfImagesScaledDown(
            scaleDown ? pathToImgs.size() : 0);

    // calculate nr batches
    const int nrBatches = static_cast<int>(
            ceil((float)(pathToImgs.size()) / PROCESSING_BATCH_SIZE));
    ...
    for (int batchId = 0; batchId < nrBatches; batchId++) {
        const int nrItems =
                (batchId != nrBatches - 1)
                        ? batchSize
                        : ((nrBatches * batchSize != int(pathToImgs.size()))
                                   ? (int(pathToImgs.size()) -
                                      (batchSize * batchId))
                                   : batchSize);
...
#pragma omp parallel for
        for (int localImgIndex = 0; localImgIndex < nrItems; localImgIndex++) {
            ...
            // boostPath.filename()).string();

            std::stringstream ss;
            ss << std::setfill('0') << std::setw(8) << globalImgIndex
               << boostPath.extension().string();
            std::string outputFileName = (outputFolder / ss.str()).string();
            std::string scaledDownOutputFileName =
                    (scaledDownOutputFolder / ss.str()).string();

            cv::Mat img = cv::imread(pathToImgs[globalImgIndex], 1);

            cv::Rect areOfIntererst =
                    cv::Rect((img.cols - cropResolution[0]) / 2,
                             (img.rows - cropResolution[1]) / 2,
                             cropResolution[0], cropResolution[1]);
            ...
            if (scaleDown) {
                cv::Mat resizedImg;
                cv::resize(croppedImg, resizedImg, resizedSize, 0, 0,
                           cv::INTER_LINEAR);
    ...
    appUtility.logExecution(avgInitialResolution, int(pathToImgs.size()),
                            elapsedTime, scaleDown, LOG_FILE_NAME);

    // write list_images.txt
    appUtility.writeListImages((outputFolder / "list_images.txt").string(),
                               listOfImages);

    // write list_images.txt and scale_factor in scaled down directoy if needed
    if (scaleDown) {
        appUtility.writeListImages(
                (scaledDownOutputFolder / "list_images.txt").string(),
                listOfImagesScaledDown);
        appUtility.writeScaleFactor(
                (scaledDownOutputFolder / SCALED_DOWN_FILENAME).string(),
                scaleDownFactor);

        if (targetResolution != sibr::Vector2i(0, 0)) {
            appUtility.writeTargetResolution(
                    (scaledDownOutputFolder / "target_resolution.txt").string(),
                    targetResolution);
        }
    }
