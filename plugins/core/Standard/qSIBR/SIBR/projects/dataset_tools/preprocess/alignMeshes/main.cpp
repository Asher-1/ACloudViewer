...
static const char* usage =
        ""
        "Usage: " PROGRAM_NAME
        " -path <reference scene path> -path2 <scene to align path> -outPath "
        "<mesh output path>"
        "\n";

double distPatch(sibr::ImageRGB& im1,
                 sibr::Vector2i& tpos,
                 sibr::ImageRGB& im2,
                 sibr::Vector2i& spos,
                 int size) {
    ...
    for (int i = -size; i <= size; i++) {
        for (int j = -size; j <= size; j++) {
            ...
            if (!im2.isInRange(debug.x(), debug.y()))
                std::cout << "Pos patch is : "
                          << sibr::Vector2i(spos.x() + i, spos.y() + j)
                          << std::endl;
            dist += (im1(tpos.x() + i, tpos.y() + j).cast<double>() -
                     im2(spos.x() + i, spos.y() + j).cast<double>())
                            .squaredNorm();
        }
    }
    return dist;
...
// Convienience function to find the Median Absolute Deviation from a vector of
// deviations. Note that it isn't a strict median( just a quick approximation. )
...
// Weight function.( Takes a list of standardized adjusted residuals and returns
// the square-root-weights for each one ) Currently, the Bisquares estimator is
// used. Note that this function should return the square root of the actual
// weight value since both X and Y are multiplied by this vector.
...
void irls(Eigen::MatrixX4f mX,
          Eigen::VectorXf vY,
          Eigen::Vector4f& mCoeffs,
          float tune) {
    Eigen::MatrixXf mX_ = mX;
    Eigen::VectorXf vY_ = vY;
    // Find the least squares coefficients.
    Eigen::Vector4f vC =
            mX_.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(vY);
    ...
    Eigen::VectorXf mH =
            (mX_ * (((mX_.transpose() * mX_).inverse()) * mX_.transpose()))
                    .diagonal();
    Eigen::MatrixXf mH_ = (Eigen::VectorXf::Constant(mH.rows(), 1, 1) - mH)
                                  .cwiseSqrt()
                                  .cwiseInverse()
                                  .asDiagonal();
    ...
    for (int i = 0;; i++) {
        ...
        // Regress the weighted X and Y to find weighted least squares
        // optimisation.
        Eigen::Vector4f vC_ =
                mX_.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV)
                        .solve(vY_);

        // Find mean deviation in coefficients.
        float meanDiff = (vC - vC_).cwiseAbs().mean();
        std::cout << "MD=" << meanDiff << ",";
        // Terminate if the deviation is too small or number of iterations has
        // been exceeded.
...
static bool isRawRC(std::string pathRC) {
    ...
    if (!mesh2Align.load(pathRC + "/recon.ply")) {
        SIBR_WRG << "***** No file "
                 << pathRC + "/recon.ply ; make sure your mesh has the correct "
                             "name !!";
        return false;
    }
    if (!fileExists(pathRC + "/bundle.out")) {
        SIBR_WRG << "***** No file "
                 << pathRC + "/bundle.out ; make sure your bundle file has the "
                             "correct name !!";
        return false;
    }
    if (!fileExists(pathRC + "/list_images.txt")) {
        SIBR_WRG << "***** No file "
                 << pathRC + "/list_images.txt ; make sure you generate the "
                             "list_images.txt file ";
        return false;
    }
    return true;
}

static void loadRawRC(std::string pathRC,
                      std::vector<sibr::InputCamera::Ptr>& cams2Align,
                      std::vector<sibr::ImageRGB::Ptr>& imgs2Align,
                      sibr::Mesh& mesh2Align) {
    cams2Align =
            sibr::InputCamera::loadBundle(pathRC + "/bundle.out", 0.01f,
                                          1000.0f, pathRC + "/list_images.txt");
        ...
        if (!img.load(pathRC + "/" + cams2Align[c]->name()))
            if (!img.load(pathRC + "/" + cams2Align[c]->name() + ".png"))
                if (!img.load(pathRC + "/" + cams2Align[c]->name() + ".jpg")) {
                    SIBR_ERR << "Error loading dataset to align from "
                             << pathRC;
...
static bool isRawSynthetic(std::string pathSynthetic) {
    ...
    if (!fileExists(pathSynthetic + "/scene.obj")) {
        SIBR_WRG << "***** No file "
                 << pathSynthetic +
                            "/scene.obj ; make sure your mesh has the correct "
                            "name !!";
        return false;
    }
    if (!fileExists(pathSynthetic + "/cameras.lookat")) {
        SIBR_WRG << "***** No file "
                 << pathSynthetic +
                            "/cameras.lookat ; make sure your bundle file has "
                            "the correct name !!";
        return false;
    }
    if (!directoryExists(pathSynthetic + "/images")) {
        SIBR_WRG << "***** No file "
                 << pathSynthetic +
                            "/images ; make sure you have images folder inside "
                            "the scene ";
        return false;
    }
    return true;
...
static void loadRawSynthetic(std::string pathSynthetic,
                             std::vector<sibr::InputCamera::Ptr>& cams2Align,
                             std::vector<sibr::ImageRGB::Ptr>& imgs2Align,
                             sibr::Mesh& mesh2Align) {
    cams2Align = sibr::InputCamera::loadLookat(
            pathSynthetic + "/cameras.lookat",
            std::vector<sibr::Vector2u>{sibr::Vector2u(1920, 1080)}, 0.01f,
            1000.0f);
    SIBR_WRG << "We assume a size of the synthetic images of 1920*1080. If it "
                "is not your case, this loading will not work properly";
    ...
    for (int c = 0; c < cams2Align.size(); c++) {
        ...
        if (!img.load(pathSynthetic + "/images/" + cams2Align[c]->name()))
            if (!img.load(pathSynthetic + "/images/" + cams2Align[c]->name() +
                          ".png"))
                if (!img.load(pathSynthetic + "/images/" +
                              cams2Align[c]->name() + ".jpg")) {
                    SIBR_ERR << "Error loading dataset to align from "
                             << pathSynthetic;
                    SIBR_ERR << "Problem loading images from raw RC, exiting ";
                }
...
int assignImages(std::vector<sibr::ImageRGB::Ptr>& imgs2Align,
                 ...
                 float threshold) {
    int assignCnt = 0;
    std::cout << "Assigning " << imgs2Align.size()
              << " cameras from the set to align to the fixed one: ";
    // We then look for closest match and assign it only if the distance between
    // the images is half the median distance This prevent issues in the case
    // were a camera is missing from one set
    for (int i = 0; i < imgs2Align.size(); i++) {
        ...
        cv::Rect centerROI(im2Align.w() / 8, im2Align.h() / 8,
                           6 * im2Align.w() / 8, 6 * im2Align.h() / 8);

        for (int j = 0; j < imgsRef.size(); j++) {
            if (assignedCam.find(j) != assignedCam.end()) continue;
            ...
            for (int dx = -wIm / 8; dx <= wIm / 8; dx += 4) {
                for (int dy = -hIm / 8; dy <= hIm / 8; dy += 4) {
                    cv::Rect shiftROI(dx + wIm / 8, dy + hIm / 8, 6 * wIm / 8,
                                      6 * hIm / 8);
                    double d = cv::norm(imRef.toOpenCV()(shiftROI),
                                        im2Align.toOpenCV()(centerROI));
                    if (d < minDist) {
                        minDist = d;
                    }
                }
        ...
        std::cerr << " SIZe " << dists.size() << " min " << minImDist
                  << " half " << threshold * dists[dists.size() / 2]
                  << std::endl;
        if (dists.size() > 5 &&
            minImDist < threshold * dists[dists.size() / 2]) {
            alignCamToRef[i] = bestIm;
            assignedCam.emplace(bestIm);
            std::wcout << i << " -> " << bestIm << " -- "
                       << cams2Align[i]->name().c_str() << " -> "
                       << camsRef[bestIm]->name().c_str() << std::endl;
            assignCnt++;
        } else {
            alignCamToRef[i] = -1;
            std::wcout << i << " -> " << "Not assigned " << std::endl;
            std::wcout << i << " BEST MATCH -> " << bestIm << " -- "
                       << cams2Align[i]->name().c_str() << " -> "
                       << camsRef[bestIm]->name().c_str() << std::endl;
        }
...
InputCamera rot90CC(InputCamera::Ptr& in) {
    ...
    rotCam.setLookAt(rotCam.position(), rotCam.position() + rotCam.dir(),
                     rotCam.right());
    return rotCam;
}

struct AlignMeshesArgs : virtual BasicIBRAppArgs {
    RequiredArg<std::string> pathRef = {"pathRef", "Path to the fixed scene"};
    RequiredArg<std::string> pathToAlign = {"path2Align",
                                            "Path to the scene to align"};
    RequiredArg<std::string> outPath = {"out",
                                        "Path to the folder where to write the "
                                        "transformed mesh and the matrix"};
    Arg<bool> forceLandscape = {
            "forceLandscape",
            "Option to force all images to be in landscape orientation before "
            "image assignation and correspondances computation"};
    Arg<bool> saveScene = {
            "saveScene",
            "If true saves entire scene, else only save the transformed mesh "
            "and transform.txt file in out dir"};
};
...
int main(int ac, char** av) {
    ...
    std::cout << "This method relies on images, cameras and meshes of both "
                 "scenes."
              << std::endl;

    //	Here is the data strctures that we will use for this program to make it
    ...
    if (myArgs.pathRef.get() == "") SIBR_ERR << "Reference path empty";
    ...
    try {
        ...
        if ((refSceneType = sceneRef->data()->datasetType()) !=
            IParseData::Type::EMPTY) {
            meshRef = sceneRef->proxies()->proxy();
            imgsRef = sceneRef->images()->inputImages();
            camsRef = sceneRef->cameras()->inputCameras();
        } else
            SIBR_ERR << "Error loading reference dataset from "
                     << myArgs.pathRef.get();
    }

    catch (...) {
        std::cout << "Trying to load Raw RealityCapture or Synthetic data"
                  << std::endl;
        if (isRawRC(argsRefScene.dataset_path)) {  // try "raw RC" option
            ...
            loadRawSynthetic(argsRefScene.dataset_path, camsRef, imgsRef,
                             meshRef);
        } else
            SIBR_ERR << "Error loading reference dataset from "
                     << myArgs.pathRef.get();
    ...
    try {
        ...
        if (isRawRC(argsAlignScene.dataset_path)) {  // try "raw RC" option
            loadRawRC(argsAlignScene.dataset_path, cams2Align,
                      imgs2AlignOriginal, mesh2Align);
        } else if (isRawSynthetic(argsAlignScene.dataset_path)) {
            loadRawSynthetic(argsAlignScene.dataset_path, cams2Align,
                             imgs2AlignOriginal, mesh2Align);
        } else
            SIBR_ERR << "Error loading dataset to align from "
                     << myArgs.pathToAlign.get();
    ...
    if (myArgs.forceLandscape) {
        for (int c = 0; c < camsRef.size(); c++) {
            if (imgsRef[c]->h() > imgsRef[c]->w()) {
                // rotate the image
                cv::rotate(imgsRef[c]->toOpenCV(),
                           imgsRef[c]->toOpenCVnonConst(),
        ...
        for (int c = 0; c < cams2Align.size(); c++) {
            if (imgs2AlignOriginal[c]->h() > imgs2AlignOriginal[c]->w()) {
                cv::rotate(imgs2AlignOriginal[c]->toOpenCV(),
                           imgs2AlignOriginal[c]->toOpenCVnonConst(),
                           cv::ROTATE_90_COUNTERCLOCKWISE);
                // rotate the camera
    ...
    // We resize the input images to the same width to account for possible
    // rescale between the two scenes
#pragma omp parallel for
    for (int c = 0; c < camsRef.size(); c++) {
        *imgsRef[c] = imgsRef[c]->resized(
                1024, 1024.0f * imgsRef[c]->h() / imgsRef[c]->w(),
                cv::INTER_LINEAR);
    }
...
#pragma omp parallel for
    for (int c = 0; c < cams2Align.size(); c++) {
        *imgs2Align[c] = imgs2AlignOriginal[c]->resized(
                1024,
                1024.0f * imgs2AlignOriginal[c]->h() /
                        imgs2AlignOriginal[c]->w(),
                cv::INTER_LINEAR);
    }
    ...
    // We now match images between the two scenes as we cannot rely on
    // correspondance between cameras First we make all image 512*512
    int resizeW = 512;
...
#pragma omp parallel for
    for (int i = 0; i < imgs2Align.size(); i++) {
        imgs2AlignSmall[i] =
                imgs2Align[i]->resized(resizeW, resizeW, cv::INTER_AREA);
...
#pragma omp parallel for
    ...
    int cnt = assignImages(imgs2Align, imgs2AlignSmall, imgsRef, imgsRefSmall,
                           alignCamToRef, camsRef, cams2Align, resizeW,
                           assignedCam, 0.7);
    ...
    for (int im = 0; im < imgs2Align.size(); im++) {
        if (alignCamToRef[im] < 0) continue;
        ...
        // Compute im center to transpose position from imref to im2Align. The
        // center should stay aligned with the crop. Small errors are absorbed
        // by the shift estimation.
        ...
        // We render the two depth map of the two meshes to be able to recover
        // 3D position from pixel position
        ...
        // showFloat(depthMap2Align.resized(1024, 1024 * hCam2Align /
        // wCam2Align));
        const int stride = std::max(16.0, sqrt(imRef.w() * imRef.h() *
                                               imgs2Align.size() / 50000.0));
...
#pragma omp parallel for
        for (int i = patchRadius; i < imRef.w(); i += stride) {
            for (int j = patchRadius; j < imRef.h(); j += stride) {
                ...
                // Corresponding position is estimated from the center because
                // the two center of the images should be aligned
                sibr::Vector2i pos2Align =
                        im2AlignCenter + posRef - imRefCenter;

                if (imRef.isInRange(i, j) &&
                    imRef(i, j) != Vector3ub(0, 0, 0) &&
                    im2Align.isInRange(pos2Align.x(), pos2Align.y()) &&
                    im2Align.isInRange(
                            pos2Align.x() - (shiftMax + patchRadius),
                            pos2Align.y() - (shiftMax + patchRadius)) &&
                    im2Align.isInRange(
                            pos2Align.x() + shiftMax + patchRadius + 1,
                            pos2Align.y() + shiftMax + patchRadius + 1)) {
                    // To visualize feature match in images
                    imRefCopy(i, j) = sibr::Vector3ub(255, 0, 0);
                    im2AlignCopy(pos2Align.x(), pos2Align.y()) =
                            sibr::Vector3ub(255, 0, 0);
                    ...
                    for (int k = -shiftMax; k <= shiftMax; k++) {
                        for (int l = -shiftMax; l <= shiftMax; l++) {
                            ...
                            double dist =
                                    distPatch(imRef, posRef, im2Align,
                                              pos2AlignShifted, patchRadius);

                            if (dist < minDist ||
                                (dist == minDist &&
                                 shift.norm() < bestShift.norm())) {
                                bestShift = shift;
                                minDist = dist;
                            }
                        }
                    ...
                    sibr::Vector2i posImFullRef(posRef.x() * ratioRefW,
                                                posRef.y() * ratioRefH);
                    sibr::Vector2i posImFull2Align(
                            pos2AlignShifted.x() * ratio2AlignW,
                            pos2AlignShifted.y() * ratio2AlignH);

                    if (depthMapRef(posImFullRef.x(), posImFullRef.y()).x() !=
                                1 &&
                        depthMap2Align(posImFull2Align.x(), posImFull2Align.y())
                                        .x() != 1) {
                        // To see feature match in images
                        im2AlignCopy(pos2Align.x() + bestShift.x(),
                                     pos2Align.y() + bestShift.y()) =
                                sibr::Vector3ub(0, 255, 0);

                        float dRef =
                                depthMapRef(posImFullRef.x(), posImFullRef.y())
                                        .x();
                        sibr::Vector3f pos3DRef =
                                camRef->unprojectImgSpaceInvertY(posImFullRef,
                                                                 dRef);

                        float d2Align = depthMap2Align(posImFull2Align.x(),
                                                       posImFull2Align.y())
                                                .x();
                        sibr::Vector3f pos3D2Align =
                                cam2Align->unprojectImgSpaceInvertY(
                                        posImFull2Align, d2Align);

                        // Add those feature to the list
#pragma omp critical
                        {
                            if (pos3DRef == pos3DRef &&
                                pos3D2Align == pos3D2Align) {
                                listFeatPRef.push_back(pos3DRef);
                                listFeatP2Align.push_back(pos3D2Align);

                                dist2CamRef.push_back(
                                        (pos3DRef - camRef->position()).norm());
                                dist2Cam2Align.push_back(
                                        (pos3D2Align - cam2Align->position())
                                                .norm());
                            } else {
    ...
    // Now we will remove outliers that appears for several reason, one of them
    // is obviously the part of the images where the content changed. We compute
    // the median scale factor using the distances to cameras, and we only keep
    // matches that respect this median
    ...
    std::cout << "Cleaned matches: " << listFeatPRef.size() << " to "
              << listStrongFeatPRef.size() << std::endl;
    ...
    Eigen::Map<Eigen::Matrix<float, -1, 3, Eigen::RowMajor>> mX3(XData.data(),
                                                                 numX, 3);

    // Convert 3-column matrix into a 4-column one using all 1s for the last
    // column.
...
#define TUNING_CONSTANT 4.685
    ...
    // Run IRLS considering each of the Y-matrix's calumns as the target
    // Y-vector individually. We get one row of the solution matrix at each step
    // which is then put together to form the complete solution.
    ...
    mFinal << vCoeffs0.x(), vCoeffs0.y(), vCoeffs0.z(), vCoeffs0.w(),
            ...
            vCoeffs2.x(), vCoeffs2.y(), vCoeffs2.z(), vCoeffs2.w(), 0, 0, 0, 1;

    std::cout << "Matrix is:" << std::endl;
    std::cout << vCoeffs0.x() << " " << vCoeffs0.y() << " " << vCoeffs0.z()
              << " " << vCoeffs0.w() << std::endl;
    std::cout << vCoeffs1.x() << " " << vCoeffs1.y() << " " << vCoeffs1.z()
              << " " << vCoeffs1.w() << std::endl;
    std::cout << vCoeffs2.x() << " " << vCoeffs2.y() << " " << vCoeffs2.z()
              << " " << vCoeffs2.w() << std::endl;
    ...
    myfile << vCoeffs0.x() << " " << vCoeffs0.y() << " " << vCoeffs0.z() << " "
           << vCoeffs0.w() << std::endl;
    myfile << vCoeffs1.x() << " " << vCoeffs1.y() << " " << vCoeffs1.z() << " "
           << vCoeffs1.w() << std::endl;
    myfile << vCoeffs2.x() << " " << vCoeffs2.y() << " " << vCoeffs2.z() << " "
           << vCoeffs2.w() << std::endl;
    ...
    std::cout << " Output vertices num : " << alignedMesh.vertices().size()
              << std::endl;
    ...
    if (myArgs.saveScene) {
        ...
        sibr::InputCamera::saveAsBundle(outCams,
                                        outPath + "/cameras/bundle.out");
        ...
        // could preserve texture name, but is probably cleaner to have standard
        // name
...
void computeRT(std::vector<sibr::Vector3f> A,
               std::vector<sibr::Vector3f> B,
               Matrix3f S,
               Matrix3f& R,
               sibr::Vector3f& T) {
    ...
    if (R.determinant() < 0) {
        // R.col(2) *= -1;
        // std::cout << "Warning : determinant of rotation matrix is negative,
        // multiplying last column by -1" << std::endl;
    }
...
float computeS(std::vector<sibr::Vector3f> A,
               std::vector<sibr::Vector3f> B,
               float& minScale,
               float& maxScale) {
    ...
    for (int i = 0; i < A.size(); i++) {
        ...
        if (scale_i > maxScale) maxScale = scale_i;
        if (scale_i < minScale) minScale = scale_i;
    }
