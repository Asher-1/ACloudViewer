...
static void* User_ReadOpen(ImGuiContext*,
                           ImGuiSettingsHandler*,
                           const char* name) {
    return (void*)0x1;
}

static void User_ReadLine(ImGuiContext*,
...
static void User_WriteAll(ImGuiContext* imgui_ctx,
                          ImGuiSettingsHandler* handler,
                          ImGuiTextBuffer* buf) {
    // Write a buffer
...
int main(int ac, char** av) {
    ...
    sibr::Window window(
            ...
            getResourcesDirectory() + "/gaussians/" + PROGRAM_NAME + ".ini");
    ...
    if (!cfgFile.good()) {
        SIBR_ERR << "Could not find config file 'cfg_args' at "
    ...
    bool white_background =
            cfgLine.substr(rng.first, rng.second - rng.first).find("True") !=
            -1;
    ...
    try {
        ...
        SIBR_LOG << "Did not find specified input folder, loading from model "
                    "path"
                 << std::endl;
    ...
    rendering_width = (rendering_width <= 0) ? std::min(1200U, scene_width)
                                             : rendering_width;
    rendering_height = (rendering_height <= 0) ? std::min(1200U, scene_width) /
                                                         scene_aspect_ratio
                                               : rendering_height;
    ...
    GaussianView::Ptr gaussianView(new GaussianView(
            scene, sceneResWidth, sceneResHeight, plyfile.c_str(), &messageRead,
    ...
    if (myArgs.rendering_mode == 1)
        multiViewManager.renderingMode(
                IRenderingMode::Ptr(new StereoAnaglyphRdrMode()));

    multiViewManager.addIBRSubView(
            "Point view", gaussianView, usedResolution,
    ...
    // save images
    generalCamera->getCameraRecorder().setViewPath(gaussianView,
