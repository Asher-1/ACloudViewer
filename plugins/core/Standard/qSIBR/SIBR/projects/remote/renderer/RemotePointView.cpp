            ...
            while (keep_running) {
                {
                    ...
                    // Serialize our arbitrary data to something simple, yet
                    ...
                    sendData[jViewMat] = std::vector<float>(
                            (float*)&_remoteInfo.view,
                            ((float*)&_remoteInfo.view) + 16);
                    sendData[jViewProjMat] = std::vector<float>(
                            (float*)&_remoteInfo.viewProj,
                            ((float*)&_remoteInfo.viewProj) + 16);

                    std::string message = sendData.dump();
                    uint32_t messageLength = message.size();
                    boost::asio::write(sock,
                                       boost::asio::buffer(&messageLength,
                                                           sizeof(uint32_t)));
                    boost::asio::write(sock,
                                       boost::asio::buffer(message.c_str(),
                                                           messageLength));
                }

                uint32_t bytes_to_receive = _remoteInfo.imgResolution.x() *
                                            _remoteInfo.imgResolution.y() * 3;
                if (bytes_to_receive > 0) {
                    std::lock_guard<std::mutex> ilg(_imageDataMutex);
                    _imageData.resize(bytes_to_receive);
                    boost::asio::read(sock,
                                      boost::asio::buffer(_imageData.data(),
                                                          _imageData.size()));
                    {
                        std::lock_guard<std::mutex> lg(_renderDataMutex);
                ...
                boost::asio::read(sock, boost::asio::buffer(&sceneLength,
                                                            sizeof(uint32_t)));
                std::vector<char> sceneName(sceneLength);
                boost::asio::read(sock, boost::asio::buffer(sceneName.data(),
                                                            sceneLength));
...
sibr::RemotePointView::RemotePointView(std::string ip, uint port)
    : sibr::ViewBase(0, 0), _ip(ip), _port(port) {
    ...
    _networkThread =
            std::make_unique<std::thread>(&RemotePointView::send_receive, this);
}
...
void sibr::RemotePointView::onRenderIBR(sibr::IRenderTarget& dst,
    ...
    {
        std::lock_guard<std::mutex> lg(_renderDataMutex);
        if (eye.view() != _remoteInfo.view ||
            eye.viewproj() != _remoteInfo.viewProj) {
    ...
    if (_showSfM || _timestampReceived == 0 ||
    ...
    } else {
        {
            ...
            if (_imageResize) {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, _resolution.x(),
                             _resolution.y(), 0, GL_RGB, GL_UNSIGNED_BYTE, 0);
                glBindTexture(GL_TEXTURE_2D, 0);
                _imageResize = false;
            }
            if (_imageDirty &&
                _imageData.size() == 3 * _resolution.x() * _resolution.y()) {
                glTextureSubImage2D(_imageTexture, 0, 0, 0, _resolution.x(),
                                    _resolution.y(), GL_RGB, GL_UNSIGNED_BYTE,
                                    _imageData.data());
                _imageDirty = false;
            }
        }
