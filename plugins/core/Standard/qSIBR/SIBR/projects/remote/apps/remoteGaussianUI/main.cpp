...
void resetScene(RemoteAppArgs myArgs,
    ...
    try {
        ...
        SIBR_ERR << "Problem loading model info from input path "
                 << myArgs.dataset_path
                 << ". Consider overriding path to model directory using "
                    "--path.";
    }
    ...
    float divider =
            ...
            std::min(1200.f, (float)scene->cameras()->inputCameras()[0]->w());
    ...
    rendering_width =
            (rendering_width <= 0)
                    ? scene->cameras()->inputCameras()[0]->w() / divider
                    : rendering_width;
    rendering_height =
            (rendering_height <= 0)
                    ? scene->cameras()->inputCameras()[0]->h() / divider
                    : rendering_height;
    ...
    multiViewManager.addIBRSubView("Point view", pointBasedView,
    ...
    // save images
    generalCamera->getCameraRecorder().setViewPath(pointBasedView,
...
int main(int ac, char** av) {
    ...
    sibr::Window window(
            ...
            getResourcesDirectory() + "/remote/" + PROGRAM_NAME + ".ini");

    // Add views to mvm.
    MultiViewManager multiViewManager(window, false);
    BasicIBRScene::Ptr scene;
    RemotePointView::Ptr remoteView(
            new RemotePointView(myArgs.ip, myArgs.port));
    std::shared_ptr<sibr::SceneDebugView> topView;
    ...
    if (pathOverride) {
        resetScene(myArgs, rendering_width, rendering_height, scene, remoteView,
                   topView, multiViewManager);
    }

    // Main looooooop.
    while (window.isOpened()) {
        if (!pathOverride && remoteView->sceneName() != "" &&
            remoteView->sceneName() != currentName) {
            currentName = remoteView->sceneName();
            myArgs.dataset_path = currentName;
            resetScene(myArgs, rendering_width, rendering_height, scene,
                       remoteView, topView, multiViewManager);
        }
