...
const char* usage =
        ""
        "Usage: " PROGRAM_NAME
        " -path <dataset-path or mesh-path>"
        "\n";

struct TexturedMeshAppArgs : virtual BasicIBRAppArgs {
...
std::string findCaprealMesh(const std::string& caprealDir) {
    ...
    if (!fileExists(meshPath)) m2 = meshPath = caprealDir + "/mesh.obj";
    if (!fileExists(meshPath)) m3 = meshPath = caprealDir + "/mesh.ply";

    if (!fileExists(meshPath))
        SIBR_ERR << "Can't find mesh, tried: " << m1 << ":" << m2 << ":" << m3
                 << std::endl;
...
std::string findCaprealTexture(const std::string& caprealDir) {
    ...
    if (!fileExists(texImgPath)) t3 = texImgPath = caprealDir + "/mesh.png";
    if (!fileExists(texImgPath))
        t4 = texImgPath = caprealDir + "/mesh_u1_v1.png";
    if (!fileExists(texImgPath))
        SIBR_ERR << "Cant find texture, tried " << t1 << ":" << t2 << ":" << t3
                 << ":" << t4 << std::endl;
    return texImgPath;
}
...
int main(int ac, char** av) {
    {
        ...
        if (myArgs.noScene) {
            ...
            */
            ...
            std::cerr << "Reading " << myArgs.dataset_path.get() + "/capreal"
                      << std::endl;
            std::string caprealDir = myArgs.dataset_path.get() + "/capreal";
            if (!directoryExists(caprealDir))
                caprealDir =
                        parentDirectory(myArgs.dataset_path.get()) + "/capreal";
        ...
        if (sibr::fileExists(texImgPath)) {
            ...
            scene->inputMeshTextures().reset(new sibr::Texture2DRGB(
                    inputTextureImg, SIBR_GPU_LINEAR_SAMPLING));
        }
        /* HACK GD
        else {
                SIBR_ERR << "No mesh and texture found! Please specify path to
        mesh using --path and path to the mesh texture using --texture!" <<
        std::endl; return 0;
        }
        */
        ...
        TexturedMeshView::Ptr texturedView(
                new TexturedMeshView(scene, sceneResWidth, sceneResHeight));
        ...
        if (scene->cameras()->inputCameras().size() == 0)
            generalCamera->setup(scene->proxies()->proxyPtr(),
        ...
        multiViewManager.addIBRSubView("TM View", texturedView, usedResolution,
        ...
        const std::shared_ptr<sibr::SceneDebugView> topView(
                new sibr::SceneDebugView(scene, multiViewManager.getViewport(),
                                         generalCamera, myArgs));
        ...
        if (myArgs.pathFile.get() != "") {
            ...
            generalCamera->getCameraRecorder().recordOfflinePath(
                    myArgs.outPath, multiViewManager.getIBRSubView("TM view"),
                    "texturedmesh");
