	...
	, m_initialCloud(nullptr)
{
	...
	//create 3D view
	{
		...
		m_glView->setInteractionMode(ecvGenericGLDisplay::MODE_TRANSFORM_CAMERA);
	...
	//connect signals/slots
	{
		...
		{
			connect(src, SIGNAL(itemPicked(ccHObject*,unsigned,int,int,CCVector3)),
					this, SLOT(handlePickedItem(ccHObject*,unsigned,int,int,CCVector3)));
			connect(src, SIGNAL(leftButtonClicked(int,int)),
					this, SLOT(onLeftButtonClicked(int,int)));
			connect(src, SIGNAL(mouseMoved(int,int,Qt::MouseButtons)),
					this, SLOT(onMouseMoved(int,int,Qt::MouseButtons)));
			connect(src, SIGNAL(buttonReleased()),
		...
		//view buttons
		connect(topViewToolButton,    &QToolButton::clicked, [&]() { if (m_glView) m_glView->setView(CC_TOP_VIEW  ); });
		connect(frontViewToolButton,  &QToolButton::clicked, [&]() { if (m_glView) m_glView->setView(CC_FRONT_VIEW); });
		connect(leftViewToolButton,   &QToolButton::clicked, [&]() { if (m_glView) m_glView->setView(CC_LEFT_VIEW ); });
		connect(backViewToolButton,   &QToolButton::clicked, [&]() { if (m_glView) m_glView->setView(CC_BACK_VIEW ); });
		connect(rightViewToolButton,  &QToolButton::clicked, [&]() { if (m_glView) m_glView->setView(CC_RIGHT_VIEW); });
		connect(bottomViewToolButton, &QToolButton::clicked, [&]() { if (m_glView) m_glView->setView(CC_BOTTOM_VIEW); });
	}
...
void qBroomDlg::AutomationArea::clear()
{
	...
	if (polyline)
	{
		...
		{
			static_cast<ecvGenericGLDisplay*>(polyline->getDisplay())->removeFromOwnDB(polyline);
		}
...
void qBroomDlg::Picking::clear()
{
	...
	for (size_t i = 0; i < labels.size(); ++i)
	{
		...
		if (label)
		{
			...
			if (label->getDisplay())
			{
				static_cast<ecvGenericGLDisplay*>(label->getDisplay())->removeFromOwnDB(label);
			}
...
bool qBroomDlg::CloudBackup::backupColors()
{
	...
	if (ref->hasColors())
	{
		...
		{
			colors->setValue(i, ecvColor::Rgba(ref->getPointColor(i), ecvColor::MAX));
		}
	}
...
bool qBroomDlg::setCloud(ccPointCloud* cloud, bool ownCloud/*=false*/, bool autoRedraw/*=true*/)
{
	...
	if (cloud)
	{
		...
		{
			ecvViewportParameters viewport = m_cloud.originDisplay->getViewportParameters();
...
void qBroomDlg::updateAutomationAreaPolyline(int x, int y)
{
	...
	{
		*const_cast<CCVector3*>(vertices->getPoint(static_cast<unsigned>(i))) = m_autoArea.clickedPoints[i] + normalOffset;
	}
	...
	else if (m_autoArea.clickedPoints.size() == 2)
	{
		...
		*const_cast<CCVector3*>(vertices->getPoint(2)) = P13D + dist * Yp + normalOffset;
		*const_cast<CCVector3*>(vertices->getPoint(3)) = P03D + dist * Yp + normalOffset;
...
bool qBroomDlg::startAutomation()
{
	...
	if (lostTrack)
	{
		CVLog::Warning("Holes or too steep slope encountered during automation process");
	}
...
{
	assert(m_glView && m_glView->getPickingMode() == ecvGenericGLDisplay::POINT_PICKING);
...
void qBroomDlg::stopBroomPicking()
{
	...
	m_glView->displayNewMessage(QString(), ecvGenericGLDisplay::UPPER_CENTER_MESSAGE); //clear the area
	m_glView->setPickingMode(ecvGenericGLDisplay::NO_PICKING);
...
void qBroomDlg::onReposition()
{
	...
	//display message
	m_glView->displayNewMessage("Pick two points on the cloud", ecvGenericGLDisplay::UPPER_CENTER_MESSAGE, false, 3600, ecvGenericGLDisplay::CUSTOM_MESSAGE);
...
void qBroomDlg::onAutomate()
{
	...
	m_glView->setInteractionMode(ecvGenericGLDisplay::INTERACT_SEND_ALL_SIGNALS);
	...
	//display message
	m_glView->displayNewMessage("Pick two corners of the area to clean", ecvGenericGLDisplay::UPPER_CENTER_MESSAGE, false, 3600, ecvGenericGLDisplay::CUSTOM_MESSAGE);
...
void qBroomDlg::onLeftButtonClicked(int x, int y)
{
	...
	if (m_picking.mode == Picking::AUTO_AREA_PICKING)
	{
		...
		CCVector3 P3D;
		{
			...
			if (!Intersection(broomTrans, M03D.toPC(), M13D.toPC(), P3D))
			{
				CVLog::Warning("Failed to project the clicked point on the bromm plane");
				return;
			}
	...
	else //selection of the broom
	{
		...
		m_broomSelected = false;
		{
			...
			CCVector3d P;
			CCVector3d vtkPos = m_glView->toVtkCoordinates(x, y);
			m_broomSelected = m_broomBox->trianglePicking(	CCVector2d(vtkPos.x, vtkPos.y),
															camera,
															nearestTriIndex,
														nearestSquareDist,
														P );
		...
		if (m_broomSelected)
		{
			...
			if (stickCheckBox->isChecked())
			{
				...
				CCVector3d vtkPos = m_glView->toVtkCoordinates(m_lastMousePos.x(), m_lastMousePos.y());
			...
			// here, the isSignalOnlyInteraction guard (line 1230) returns early,
			// preventing VTK from starting camera rotation on this press.
			m_glView->setInteractionMode(ecvGenericGLDisplay::INTERACT_SEND_ALL_SIGNALS);
	...
	QPoint mousePos(x, y);
	{
		...
		CCVector3d broomDelta(0,0,0);
		{
			...
			if (!stickToTheFloor || !m_hasLastMousePos3D)
			{
				...
				m_glView->getViewportParameters().viewMat.transposed().applyRotation(u);
				broomDelta = u;
			}
...
bool qBroomDlg::moveBroom(ccGLMatrix& broomTrans, CCVector3d& broomDelta, bool stickToTheFloor) const
{
	...
	if (stickToTheFloor)
	{
		...
		if (count < 10)
		{
			CVLog::Warning("Failed to extract enough point inside the broom. Lost track.");
			return false;
		}
		else
		{
			cloudViewer::DgmOctreeReferenceCloud neighboursCloud(&bn.neighbours);
			cloudViewer::Neighbourhood n(&neighboursCloud);
			const CCVector3* N = n.getLSPlaneNormal();
			if (N)
			{
			...
			{
				CVLog::Error("Failed to detect a planar surface below the broom. Lost track.");
				return false;
			}
		}
	}
