...
// intersection between a plane (the broom plane) and a line (represented by two
// points)
static bool Intersection(const ccGLMatrix& broomTrans,
                         const CCVector3& A,
                         const CCVector3& B,
                         CCVector3& I) {
...
static const ecvGenericGLDisplay::INTERACTION_FLAGS BROOM_DEFAULT_INTERACTION =
        ecvGenericGLDisplay::MODE_TRANSFORM_CAMERA |
        ecvGenericGLDisplay::INTERACT_SIG_LB_CLICKED;

qBroomDlg::qBroomDlg(ecvMainAppInterface* app /*=nullptr*/)
    ...
    int selectionMode = selectionModeComboBox->currentIndex();
    {
        ...
        bool stickToTheFloor =
                settings.value("stickToTheFloor", stickCheckBox->isChecked())
                        .toBool();
        bool animateAutomation =
                settings.value("animatedAutomation",
                               animateAutomationCheckBox->isChecked())
                        .toBool();
    ...
    // create 3D view
    {
        // Pass nullptr as parent — CloudCompare creates ccGLWindow with nullptr
        // parent too.  If we pass QMainWindow*, the QVTKOpenGLNativeWidget is
        // reparented when addWidget() moves it into viewFrame, which destroys
        // and recreates its QOpenGLContext and invalidates VTK's GL state,
        // producing a blank window.
        m_glView = ecvGenericGLDisplay::CreateView(nullptr, true, false);
        assert(m_glView);

        m_glView->setPerspectiveState(false, true);
        m_glView->setInteractionMode(
                ecvGenericGLDisplay::MODE_TRANSFORM_CAMERA);
        m_glView->setPickingMode(ecvGenericGLDisplay::NO_PICKING);

        // add window to the input frame (if any)
        viewFrame->setLayout(new QHBoxLayout());
        viewFrame->layout()->addWidget(m_glView->asWidget());
        // QVTKWidgetCustom's constructor calls hide() to prevent X11 stacking
        // issues before layout placement.  The MainWindow path handles this via
        // buildCell()->show(), but embedded dialogs must do it themselves.
        m_glView->asWidget()->show();
    }

    // items
    {
        m_broomBox->setEnabled(true);
        m_broomBox->setDisplay(m_glView);

        m_selectionBox->setEnabled(true);
        m_selectionBox->setDisplay(m_glView);

        m_boxes->setEnabled(false);
        m_boxes->setDisplay(m_glView);
        m_boxes->addChild(m_broomBox);
        m_boxes->addChild(m_selectionBox);
        m_glView->addToOwnDB(m_boxes);
    }
    ...
    // connect signals/slots
    {
        if (QObject* src = m_glView->signalSource()) {
            connect(src,
                    SIGNAL(itemPicked(ccHObject*, unsigned, int, int,
                                      CCVector3)),
                    this,
                    SLOT(handlePickedItem(ccHObject*, unsigned, int, int,
                                          CCVector3)));
            connect(src, SIGNAL(leftButtonClicked(int, int)), this,
                    SLOT(onLeftButtonClicked(int, int)));
            connect(src, SIGNAL(mouseMoved(int, int, Qt::MouseButtons)), this,
                    SLOT(onMouseMoved(int, int, Qt::MouseButtons)));
            connect(src, SIGNAL(buttonReleased()), this,
                    SLOT(onButtonReleased()));
        }

        connect(cleanHeightDoubleSpinBox,
                qOverload<double>(&QDoubleSpinBox::valueChanged), this,
                &qBroomDlg::onCleanHeightChanged);
        connect(broomLengthDoubleSpinBox,
                qOverload<double>(&QDoubleSpinBox::valueChanged), this,
                &qBroomDlg::onDimensionChanged);
        connect(broomWidthDoubleSpinBox,
                qOverload<double>(&QDoubleSpinBox::valueChanged), this,
                &qBroomDlg::onDimensionChanged);
        connect(broomThicknessDoubleSpinBox,
                qOverload<double>(&QDoubleSpinBox::valueChanged), this,
                &qBroomDlg::onDimensionChanged);

        connect(selectionModeComboBox,
                qOverload<int>(&QComboBox::currentIndexChanged), this,
                &qBroomDlg::onSelectionModeChanged);
        connect(undoPushButton, &QAbstractButton::clicked, this,
                &qBroomDlg::doUndo);
        connect(undo10PushButton, &QAbstractButton::clicked, this,
                &qBroomDlg::doUndo10);
        connect(repositionPushButton, &QAbstractButton::clicked, this,
                &qBroomDlg::onReposition);
        connect(automatePushButton, &QAbstractButton::clicked, this,
                &qBroomDlg::onAutomate);

        connect(cancelPushButton, &QAbstractButton::clicked, this,
                &qBroomDlg::cancel);
        connect(applyPushButton, &QAbstractButton::clicked, this,
                &qBroomDlg::apply);
        connect(validatePushButton, &QAbstractButton::clicked, this,
                &qBroomDlg::validate);

        // view buttons
        connect(topViewToolButton, &QToolButton::clicked, [&]() {
            if (m_glView) m_glView->setView(CC_TOP_VIEW);
        });
        connect(frontViewToolButton, &QToolButton::clicked, [&]() {
            if (m_glView) m_glView->setView(CC_FRONT_VIEW);
        });
        connect(leftViewToolButton, &QToolButton::clicked, [&]() {
            if (m_glView) m_glView->setView(CC_LEFT_VIEW);
        });
        connect(backViewToolButton, &QToolButton::clicked, [&]() {
            if (m_glView) m_glView->setView(CC_BACK_VIEW);
        });
        connect(rightViewToolButton, &QToolButton::clicked, [&]() {
            if (m_glView) m_glView->setView(CC_RIGHT_VIEW);
        });
        connect(bottomViewToolButton, &QToolButton::clicked, [&]() {
            if (m_glView) m_glView->setView(CC_BOTTOM_VIEW);
        });
    }

    // VTK adaptation of CC's dynamic interaction-mode switching:
    //
    // Default mode (BROOM_DEFAULT_INTERACTION):
    //   MODE_TRANSFORM_CAMERA | INTERACT_SIG_LB_CLICKED
    //   → VTK trackball handles camera rotation on left-drag
    //   → leftButtonClicked signal emitted on left-press (line 1163
    //     of QVTKWidgetCustom::mousePressEvent, BEFORE VTK forwarding)
    //
    // When broom is picked (onLeftButtonClicked → trianglePicking HIT):
    //   Switch to INTERACT_SEND_ALL_SIGNALS inside the signal handler,
    //   which runs before VTK processes the press event (line 1230 guard),
    //   preventing VTK from starting camera rotation.
    //
    // When broom is released (onButtonReleased):
    //   Switch back to BROOM_DEFAULT_INTERACTION.
...
qBroomDlg::~qBroomDlg() {
    ...
    m_autoArea.clear();
    if (m_glView) {
        m_glView->getOwnDB()->removeAllChildren();
        if (QObject* src = m_glView->signalSource()) {
            src->disconnect(this);
            delete src;
        }
...
cc2DLabel* qBroomDlg::Picking::addLabel(ccGenericPointCloud* cloud,
                                        unsigned pointIndex) {
...
void qBroomDlg::AutomationArea::clear() {
    ...
    if (polyline) {
        ...
        if (polyline->getDisplay()) {
            static_cast<ecvGenericGLDisplay*>(polyline->getDisplay())
                    ->removeFromOwnDB(polyline);
        }
...
void qBroomDlg::Picking::clear() {
    ...
    for (size_t i = 0; i < labels.size(); ++i) {
        ...
        if (label) {
            // auto remove from display
            if (label->getDisplay()) {
                static_cast<ecvGenericGLDisplay*>(label->getDisplay())
                        ->removeFromOwnDB(label);
            }
...
bool qBroomDlg::CloudBackup::backupColors() {
    ...
    if (ref->hasColors()) {
        ...
        for (unsigned i = 0; i < ref->size(); ++i) {
            colors->setValue(
                    i, ecvColor::Rgba(ref->getPointColor(i), ecvColor::MAX));
        }
    }
...
void qBroomDlg::CloudBackup::restore() {
    ...
    if (!hadOctree) {
        // we can only delete the octree if it has not already been added to the
        // DB tree!!!
        if (!ref->getParent()) {
            ref->deleteOctree();
        }
...
bool qBroomDlg::setCloud(ccPointCloud* cloud,
                         bool ownCloud /*=false*/,
    ...
    if (m_cloud.ref) {
        m_glView->removeFromOwnDB(m_cloud.ref);
        m_cloud.restore();
        m_cloud.clear();
    }
    ...
    if (cloud) {
        ...
        if (pointCount < 10) {
            CVLog::Error("Cloud is too small!");
            return false;
        }
        ...
        if (!cloud->hasColors()) {
            ...
            // or we simply set white colors
                ...
                success = cloud->setRGBColor(ecvColor::white);
            }
            if (!success) {
                CVLog::Error("Not enough memory");
                return false;
        ...
        // from now on, the cloud will be automatically deleted if it's 'owned'
        // by the dialog
        m_cloud.ownCloud = ownCloud;

        m_cloud.ref->setDisplay(m_glView);
        m_glView->addToOwnDB(m_cloud.ref);

        if (m_cloud.originDisplay) {
            ecvViewportParameters viewport =
                    m_cloud.originDisplay->getViewportParameters();
            ccBBox bbox = m_cloud.ref->getOwnBB();
            CCVector3d center;
            if (bbox.isValid()) {
                center = CCVector3d::fromArray(bbox.getCenter().u);
                viewport.setPivotPoint(center, true);
            }
            m_glView->setViewportParameters(viewport);

            // setViewportParameters sets the VTK camera focal point to the
            // pivot, but the VTK interactor style has its own rotation
            // center that must be set separately.  setPivotPoint calls
            // VtkVis::setCenterOfRotation internally.
            if (bbox.isValid()) {
                m_glView->setPivotPoint(center, false);
            }
        } else {
            ccBBox bbox = m_cloud.ref->getOwnBB();
            m_glView->updateConstellationCenterAndZoom(&bbox);
        }
...
void qBroomDlg::onSelectionModeChanged(int mode) {
    ...
    m_selectionMode =
            static_cast<SelectionModes>(selectionModeComboBox->currentIndex());
    ...
    if (m_selectionMode != formerMode) {
        removeSelectedPointsCheckBox->setChecked(
                mode != 0);  // mode '0' is inside (--> we'll probably want to
                             // keep these points)
        if (m_glView) {
            updateBroomBox();
            updateSelectionBox();
...
void qBroomDlg::updateAutomationAreaPolyline(int x, int y) {
    ...
    if (m_autoArea.clickedPoints.empty() ||
        m_autoArea.clickedPoints.size() > 2) {
        assert(false);
        return;
    }
    ...
    m_glView->getGLCameraParameters(camera);
    ...
    if (!m_autoArea.polyline) {
        ...
        vertices->addPoint(CCVector3(0, 0, 0));
        vertices->addPoint(CCVector3(0, 0, 0));
        m_autoArea.polyline->addPointIndex(
                0, 2);  // we only display a segment at first
        m_autoArea.polyline->setEnabled(true);
        m_autoArea.polyline->setVisible(true);
        m_autoArea.polyline->setWidth(3);
        m_autoArea.polyline->setColor(ecvColor::red);
        m_autoArea.polyline->showColors(true);
        m_autoArea.polyline->setTempColor(ecvColor::red);
        // VTK has no built-in 2D overlay polyline support. Use 3D mode
        // with actual world coordinates so the polyline renders correctly
        // in the VTK pipeline.  CC uses set2DMode(true) with projected
        // screen coordinates, relying on OpenGL's glOrtho overlay.
        m_autoArea.polyline->set2DMode(false);
        m_autoArea.polyline->setClosed(true);
        m_autoArea.polyline->setDisplay(m_glView);
        m_glView->addToOwnDB(m_autoArea.polyline);
    } else {
        vertices = dynamic_cast<ccPointCloud*>(
                m_autoArea.polyline->getAssociatedCloud());
    }
    ...
    // Offset along the broom plane normal so the polyline
    // sits visibly above the point cloud surface.
    // Use a more generous offset (2% of broom length) to avoid Z-fighting.
    CCVector3 normalOffset = Zp * (broomLengthDoubleSpinBox->value() * 0.02);
    ...
    CCVector3 P3D;
    {
        CCVector3d M03D;
        CCVector3d M13D;
        CCVector3d vtkPos = m_glView->toVtkCoordinates(x, y);
        camera.unproject(CCVector3d(vtkPos.x, vtkPos.y, 0), M03D);
        camera.unproject(CCVector3d(vtkPos.x, vtkPos.y, 1), M13D);
        if (!Intersection(broomTrans, M03D.toPC(), M13D.toPC(), P3D)) {
            return;
        }
    }

    // Use actual 3D world coordinates for the polyline vertices.
    for (size_t i = 0; i < std::min<size_t>(m_autoArea.clickedPoints.size(), 2);
         ++i) {
        *const_cast<CCVector3*>(vertices->getPoint(static_cast<unsigned>(i))) =
                m_autoArea.clickedPoints[i] + normalOffset;
    }

    if (m_autoArea.clickedPoints.size() == 1) {
        *const_cast<CCVector3*>(vertices->getPoint(1)) = P3D + normalOffset;
        ...
        CCVector3 Yp = Zp.cross(Xp);
        Yp.normalize();

        PointCoordinateType dist = (P3D - P03D).dot(Yp);

        *const_cast<CCVector3*>(vertices->getPoint(2)) =
                P13D + dist * Yp + normalOffset;
        *const_cast<CCVector3*>(vertices->getPoint(3)) =
                P03D + dist * Yp + normalOffset;

        if (m_autoArea.polyline->size() == 2) {
            m_autoArea.polyline->addPointIndex(
                    2, 4);  // we now display a rectangle
        }
    }

    vertices->invalidateBoundingBox();
    m_autoArea.polyline->invalidateBoundingBox();
    m_autoArea.polyline->setRedraw(true);
    m_autoArea.polyline->setForceRedraw(true);
...
void qBroomDlg::getBroomDimensions(BroomDimensions& dimensions) const {
    ...
    dimensions.length =
            static_cast<PointCoordinateType>(broomLengthDoubleSpinBox->value());
    dimensions.width = static_cast<PointCoordinateType>(
                               broomWidthDoubleSpinBox->value() / 100.0) *
                       dimensions.length;
    dimensions.thick = static_cast<PointCoordinateType>(
                               broomThicknessDoubleSpinBox->value() / 100.0) *
                       dimensions.length;
    dimensions.height =
            static_cast<PointCoordinateType>(cleanHeightDoubleSpinBox->value());
}
...
bool qBroomDlg::startAutomation() {
    ...
    if ((BB0.x > 0 && BB0.y > 0) || (BB1.x < 0 && BB1.y < 0)) {
        QMessageBox::critical(this, "Misplaced area",
                              "The broom is not inside the automation area");
        return false;
    }
    ...
    if (false) {
        ...
        planeTrans.setTranslation(initialBroomTrans.getTranslationAsVec3D() +
                                  center.x * X + center.y * Y);
        ccPlane* plane = new ccPlane(diagonal.x, diagonal.y, &planeTrans,
                                     "automation area");
        m_glView->addToOwnDB(plane, false);
        m_glView->redraw(false, true);
    }
    ...
    QProgressDialog progressDialog("Automation in progress...", "Cancel", 0, 0,
                                   this);
    progressDialog.show();
    ...
    while (true) {
        ...
        bool lastRow = false;
        {
            ...
            if (sign < 0.0 && broomCenter.dot(X) <= BB0.x + broom.length / 2) {
                lastRow = true;
                broomCenter = origin + (BB0.x + broom.length / 2) * X +
                              broomCenter.dot(Y) * Y;
                broomTrans.setTranslation(broomCenter);
            } else if (sign > 0.0 &&
                       broomCenter.dot(X) >= BB1.x - broom.length / 2) {
                lastRow = true;
                broomCenter = origin + (BB1.x - broom.length / 2) * X +
                              broomCenter.dot(Y) * Y;
                broomTrans.setTranslation(broomCenter);
            }
        }
        ...
        // test backward
        {
            ...
            while (!lastColumn) {
                ...
                {
                    CCVector3 broomCenter =
                            backwardBroomTrans.getTranslationAsVec3D() - origin;
                    if (broomCenter.dot(Y) - broom.width / 2 + broomDeltaY <=
                        BB0.y) {
                        lastColumn = true;
                        broomDeltaY =
                                BB0.y - broomCenter.dot(Y) + broom.width / 2;
                    }
                }

                CCVector3d broomDelta = broomDeltaY * Yd;

                if (!moveBroom(backwardBroomTrans, broomDelta,
                               stickToTheFloor) ||
        ...
        if (cancelled) {
            CVLog::Warning("Automation process cancelled by the user");
            break;
        }
        ...
        // test forward
        {
            ...
            while (!lastColumn) {
                ...
                {
                    CCVector3 broomCenter =
                            forwardBroomTrans.getTranslationAsVec3D() - origin;
                    if (broomCenter.dot(Y) + broom.width / 2 + broomDeltaY >=
                        BB1.y) {
                        lastColumn = true;
                        broomDeltaY =
                                BB1.y - broomCenter.dot(Y) - broom.width / 2;
                    }
                }

                CCVector3d broomDelta = broomDeltaY * Yd;

                if (!moveBroom(forwardBroomTrans, broomDelta,
                               stickToTheFloor) ||
                    broomDelta.norm() < broom.width / 20.0) {
                    // hole or too steep slope? we stop
                    lostTrack = true;
    ...
    if (lostTrack) {
        CVLog::Warning(
                "Holes or too steep slope encountered during automation "
                "process");
    }
...
bool qBroomDlg::positionBroom(const CCVector3& P0, const CCVector3& P1) {
    ...
    // static_cast<PointCoordinateType>(broomWidthDoubleSpinBox->value() /
    // 100.0) * length; PointCoordinateType thick =
    // static_cast<PointCoordinateType>(broomThicknessDoubleSpinBox->value() /
    // 100.0) * length;
...
#define USE_CYLINDRICAL_NEIGHBORHOOD
    ...
    cloudViewer::DgmOctree::CylindricalNeighbourhood nn;
    nn.center = (P0 + P1) / 2;
    nn.dir = X;
    nn.dir.normalize();
    nn.radius = length * 0.05;  // 5% of the broom length?
    nn.level = octree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
            nn.radius);
    ...
    cloudViewer::DgmOctree::BoxNeighbourhood nn;
    {
        BroomDimensions broom;
        getBroomDimensions(broom);
        ...
        nn.level = octree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
                std::max(broom.length / 5, std::max(broom.width, broom.thick)));
    }
    ...
    if (count < 3) {
        CVLog::Error("Not enough points between the two picked points");
        return false;
    }

    cloudViewer::DgmOctreeReferenceCloud neighboursCloud(&nn.neighbours);

    cloudViewer::Neighbourhood n(&neighboursCloud);
    const CCVector3* N = n.getLSPlaneNormal();
    if (!N) {
        CVLog::Error("Failed to detect a planar surface below the two points");
        return false;
    }
    ...
    m_boxes->setRedrawFlagRecursive(true);
    m_boxes->setForceRedrawRecursive(true);
    onDimensionChanged(0.0);  // to update the display

    m_glView->setInteractionMode(BROOM_DEFAULT_INTERACTION);

    return true;
}

void qBroomDlg::handlePickedItem(
        ccHObject* entity, unsigned itemIdx, int x, int y, const CCVector3&) {
    assert(m_glView &&
           m_glView->getPickingMode() == ecvGenericGLDisplay::POINT_PICKING);
    ...
    label->setDisplay(m_glView);
    m_glView->addToOwnDB(label);
    m_glView->redraw(false, true);

    if (m_picking.labels.size() == 2) {
        const cc2DLabel::PickedPoint& L0 =
                m_picking.labels[0]->getPickedPoint(0);
        const cc2DLabel::PickedPoint& L1 =
                m_picking.labels[1]->getPickedPoint(0);
...
void qBroomDlg::stopBroomPicking() {
    freezeUI(!m_boxes->isEnabled());  // we only unfreeze the UI if the broom is
                                      // valid (enabled)

    repositionPushButton->setText("Reposition");
    m_glView->displayNewMessage(
            QString(),
...
void qBroomDlg::onReposition() {
    ...
    m_glView->setInteractionMode(BROOM_DEFAULT_INTERACTION);
    m_glView->setPickingMode(ecvGenericGLDisplay::POINT_PICKING);
    ...
    // display message
    m_glView->displayNewMessage("Pick two points on the cloud",
                                ecvGenericGLDisplay::UPPER_CENTER_MESSAGE,
                                false, 3600,
...
void qBroomDlg::onAutomate() {
    ...
    // display message
    m_glView->displayNewMessage("Pick two corners of the area to clean",
                                ecvGenericGLDisplay::UPPER_CENTER_MESSAGE,
                                false, 3600,
                                ecvGenericGLDisplay::CUSTOM_MESSAGE);
...
void qBroomDlg::onLeftButtonClicked(int x, int y) {
    ...
    if (m_picking.mode == Picking::AUTO_AREA_PICKING) {
        ...
        CCVector3 P3D;
        {
            ...
            if (!Intersection(broomTrans, M03D.toPC(), M13D.toPC(), P3D)) {
                CVLog::Warning(
                        "Failed to project the clicked point on the bromm "
                        "plane");
                return;
            }
        ...
        switch (m_autoArea.clickedPoints.size()) {
            case 1:
                // Already in INTERACT_SEND_ALL_SIGNALS (set in onAutomate).
                break;
            case 2:
                // nothing to do
                break;
    ...
    } else  // selection of the broom
    {
        ...
        m_broomSelected = false;
        {
            ...
            CCVector3d P;
            CCVector3d vtkPos = m_glView->toVtkCoordinates(x, y);
            m_broomSelected = m_broomBox->trianglePicking(
                    CCVector2d(vtkPos.x, vtkPos.y), camera, nearestTriIndex,
                    nearestSquareDist, P);
        }
        ...
        if (m_broomSelected) {
            ...
            if (stickCheckBox->isChecked()) {
                ...
                CCVector3d vtkPos = m_glView->toVtkCoordinates(
                        m_lastMousePos.x(), m_lastMousePos.y());
                CCVector3d A2D(vtkPos.x, vtkPos.y, 0);
                CCVector3d B2D(vtkPos.x, vtkPos.y, 1);
                CCVector3d A3D;
                CCVector3d B3D;
                if (camera.unproject(A2D, A3D) && camera.unproject(B2D, B3D)) {
                    m_hasLastMousePos3D = Intersection(
                            m_boxes->getGLTransformation(), A3D.toPC(),
                            B3D.toPC(), m_lastMousePos3D);
                    ...
                    if (false && m_hasLastMousePos3D) {
                        ...
                        la->setDisplay(m_glView);
                        la->setVisible(true);
                        la->setEnabled(true);
                        fakeCloud->addChild(la);
                        m_glView->addToOwnDB(fakeCloud, false);
                    }
                }
            }

            // Switch to signal-only mode INSIDE this handler.
            // QVTKWidgetCustom::mousePressEvent emits leftButtonClicked
            // (line 1163) BEFORE forwarding to VTK (line 1237).  By switching
            // here, the isSignalOnlyInteraction guard (line 1230) returns
            // early, preventing VTK from starting camera rotation on this
            // press.
            m_glView->setInteractionMode(
                    ecvGenericGLDisplay::INTERACT_SEND_ALL_SIGNALS);
            m_broomBox->setTempColor(ecvColor::red);
            m_broomBox->setRedraw(true);
            m_glView->redraw(false, true);
        }
    }
}
    ...
    QPoint mousePos(x, y);
    {
        ...
        CCVector3d broomDelta(0, 0, 0);
        {
            ...
            if (!stickToTheFloor || !m_hasLastMousePos3D) {
                double pixSize = m_glView->computeActualPixelSize();
                QPoint d = mousePos - m_lastMousePos;
                CCVector3d u(d.x() * pixSize, -d.y() * pixSize, 0);
                m_glView->getViewportParameters()
                        .viewMat.transposed()
                        .applyRotation(u);
                broomDelta = u;
            }
            ...
            if (stickToTheFloor) {
                ...
                if (camera.unproject(A2D, A3D) && camera.unproject(B2D, B3D)) {
                    hasMousePos3D =
                            Intersection(m_boxes->getGLTransformation(),
                                         A3D.toPC(), B3D.toPC(), mousePos3D);
                }
                ...
                if (m_hasLastMousePos3D && hasMousePos3D) {
                    ...
                    // but we can remove the normal component (the broom stays
                    // on the floor)
                    CCVector3d Zbox = broomTrans.getColumnAsVec3D(2);
                    broomDelta -= (broomDelta.dot(Zbox) *
                                   Zbox);  // filter the Z component
                }
        ...
        if (!lostTrack) {
            ...
            m_broomBox->setTempColor(ecvColor::yellow);
            m_broomSelected = false;
        }
...
bool qBroomDlg::moveBroom(ccGLMatrix& broomTrans,
    ...
    // translate the broom
    broomTrans.setTranslation(broomTrans.getTranslationAsVec3D() +
                              broomDelta.toPC());

    if (stickToTheFloor) {
        // extract the points inside the broom
        cloudViewer::DgmOctree::BoxNeighbourhood bn;
        {
            ...
            bn.level =
                    octree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
                            std::max(broom.length / 5,
                                     std::max(broom.width, broom.thick)));
        }
        ...
        if (count < 10) {
            CVLog::Warning(
                    "Failed to extract enough point inside the broom. Lost "
                    "track.");
            return false;
        } else {
            cloudViewer::DgmOctreeReferenceCloud neighboursCloud(
                    &bn.neighbours);
            cloudViewer::Neighbourhood n(&neighboursCloud);
            const CCVector3* N = n.getLSPlaneNormal();
            if (N) {
                ...
                CVLog::Error(
                        "Failed to detect a planar surface below the broom. "
                        "Lost track.");
                return false;
            }
        }
...
bool qBroomDlg::selectPoints(const ccGLMatrix& broomTrans,
    ...
    cloudViewer::DgmOctree::BoxNeighbourhood bn;
    {
        ...
        switch (m_selectionMode) {
            case INSIDE:
                bn.dimensions =
                        CCVector3(broom.length, broom.width, broom.thick);
                break;

            case ABOVE:
            case ABOVE_AND_BELOW:  // we start by ABOVE and we'll treat BELOW
                                   // later
                bn.dimensions =
                        CCVector3(broom.length, broom.width, broom.height);
        ...
        PointCoordinateType radius =
                std::max(bn.dimensions.x,
                         std::max(bn.dimensions.y, bn.dimensions.z)) /
                5;  // emprirical ;)
        bn.level = octree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
                radius);
    }
    ...
    if (m_selectionMode == ABOVE_AND_BELOW) {
        ...
        bn.center =
                broomCenter - ((broom.thick + broom.height) / 2) * broomNormal;
    }
...
void qBroomDlg::onButtonReleased() {
    ...
    // During automation area picking, stay in INTERACT_SEND_ALL_SIGNALS
    // to preserve INTERACT_SIG_MOUSE_MOVED for the polyline preview.
    if (m_picking.mode == Picking::AUTO_AREA_PICKING) {
        return;
    }

    // Restore camera interaction when the broom is released.
    m_glView->setInteractionMode(BROOM_DEFAULT_INTERACTION);
...
void qBroomDlg::updateSelectionBox() {
    ...
    m_selectionBox->setTempColor(ecvColor::red);
...
void qBroomDlg::updateBroomBox() {
    ...
    m_broomBox->enableStippling(
            m_selectionMode ==
            INSIDE);  // the cleaning area box is already stippled!
    m_broomBox->setTempColor(ecvColor::yellow);
}
...
bool qBroomDlg::selectPoint(unsigned index) {
    ...
    m_cloud.ref->setPointColor(index, ecvColor::red);
...
void qBroomDlg::undo(uint32_t undoCount) {
    if (!m_cloud.ref || m_selectionTable.size() != m_cloud.ref->size()) {
        assert(false);
        return;
    }
...
ccPointCloud* qBroomDlg::createSegmentedCloud(ccPointCloud* cloud,
                                              bool removeSelected,
                                              bool& error) {
    error = false;
    ...
    unsigned selectedCount = 0;
    {
        for (unsigned i = 0; i < cloud->size(); ++i)
            if (m_selectionTable[i] != 0) ++selectedCount;
    ...
    cloudViewer::ReferenceCloud selection(cloud);
    {
        ...
        for (unsigned i = 0; i < cloud->size(); ++i) {
            if ((removeSelected &&
                 m_selectionTable[i] == 0)  // keep non selected
                ||
...
void qBroomDlg::apply() {
    ...
    ecvViewportParameters formerViewport = m_glView->getViewportParameters();
    m_cloud.restore();

    bool error;
    ccPointCloud* newCloud = createSegmentedCloud(
            m_cloud.ref, removeSelectedPointsCheckBox->isChecked(), error);
    if (!newCloud) {
        if (error) {
            displayError(tr("Not enough memory"));
        }
    ...
    m_glView->setViewportParameters(formerViewport);
    m_glView->redraw(false, true);
}
...
void qBroomDlg::closeEvent(QCloseEvent* e) {
    if (!m_undoPositions.empty() || m_cloud.ownCloud) {
        if (QMessageBox::warning(
                    this, "Cancel",
                    "The selection/segmentation will be lost. Do you confirm?",
...
void qBroomDlg::savePersistentSettings() {
    ...
    settings.setValue("animatedAutomation",
                      animateAutomationCheckBox->isChecked());
}
...
void qBroomDlg::validate() {
    ...
    if (!m_undoPositions.empty()) {
        bool error;
        newCloud = createSegmentedCloud(
                cloud, removeSelectedPointsCheckBox->isChecked(), error);
        if (!newCloud) {
            if (ownCloud) {
                if (error) {
                    displayError(
                            tr("Not enough memory to apply the last "
                               "segmentation"));
                }
                newCloud = cloud;
            } else if (error) {
    ...
    if (newCloud) {
        ...
        // Pass autoRedraw=false: the dialog's embedded view is still the
        // active view at this point, so addToDB's internal refreshAll()
        // would only redraw the embedded view, not the main window.
        // The main window is refreshed by doAction's refreshAll() after
        // the dialog closes.
        m_app->addToDB(newCloud, false, true, false, false);

        // setDisplayAll MUST come AFTER addToDB.  addToDB calls
        // associateToActiveView() which binds the cloud to the current
        // active view (the embedded dialog view here).  setDisplayAll()
        // overrides that binding so the cloud is visible in ALL windows
        // -- including the main view that becomes active after the
        // dialog closes.  CC uses setDisplay(nullptr) for the same
        // "show everywhere" semantic.
        newCloud->setDisplayAll();
        newCloud->setRedraw(true);
