...
Classify3DMASCDialog::Classify3DMASCDialog(ecvMainAppInterface* app, bool trainMode/*=false*/)
	...
	, m_app(app)
{
	...
	if (m_app)
	{
		...
		{
			m_app->dbRootObject()->filterChildren(clouds, true, CV_TYPES::POINT_CLOUD, true);
		}
		...
		if (cloudCount == 0 && app)
		{
			app->dispToConsole(QString("You need at least 1 loaded cloud to ") + (trainMode ? "train a classifier" : "classify it..."), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		}
	}
...
void Classify3DMASCDialog::setCloudRoles(const QList<QString>& roles, QString& corePointsLabel, const QMap<QString, QString>& rolesAndNames)
{
	...
	for (size_t i = 0; i < clouds.size(); ++i)
	{
		if (clouds[i]->isA(CV_TYPES::POINT_CLOUD)) //as filterChildren only tests 'isKindOf'
		{
			QVariant uniqueID(clouds[i]->getUniqueID());
