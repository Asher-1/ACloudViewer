...
cloudViewer::ScalarField* Feature::PrepareSF(ccPointCloud* cloud, const char* resultSFName, SFCollector* generatedScalarFields/*=nullptr*/, SFCollector::Behavior behavior/*=SFCollector::CAN_REMOVE*/)
{
	if (!cloud || !resultSFName)
	{
	...
	if (sfIdx >= 0)
	{
		// CVLog::Warning("Existing SF: " + QString(resultSFName) + ", do not store in generatedScalarFields");
		resultSF = cloud->getScalarField(sfIdx);
	}
	else
	{
		// CVLog::Warning("SF does not exist, create it: " + QString(resultSFName)  + ", SFCollector::Behavior " + QString::number(behavior));
...
bool Feature::PerformMathOp(cloudViewer::ScalarField* sf1, const cloudViewer::ScalarField* sf2, Feature::Operation op)
{
...
bool Feature::PerformMathOp(const IScalarFieldWrapper& sf1, const IScalarFieldWrapper& sf2, Operation op, cloudViewer::ScalarField* outSF)
{
