...
public:  // methods
    //! Default constructor
    Feature(double p_scale = std::numeric_limits<double>::quiet_NaN(),
    ...
    virtual bool prepare(
            ...
            SFCollector* generatedScalarFields = nullptr) = 0;

    //! Finishes the feature preparation (update the scalar field, etc.)
    virtual bool finish(const CorePoints& corePoints,
                        QString& error) { /* does nothing by default*/
        return true;
    }
...
public:  // helpers
    //! Creates (or resets) a scalar field with the given name on the input core
    //! points cloud
    static bool CheckSFExistence(ccPointCloud* cloud, const char* resultSFName);

    //! Creates (or resets) a scalar field with the given name on the input core
    //! points cloud
    static cloudViewer::ScalarField* PrepareSF(
            ccPointCloud* cloud,
            const char* resultSFName,
            SFCollector* generatedScalarFields /*= nullptr*/,
            SFCollector::Behavior behavior);

    //! Performs a mathematical operation between two scalars
    static ScalarType PerformMathOp(double s1, double s2, Operation op);

    //! Performs a mathematical operation between two scalar fields (they must
    //! have the same size!)
    static bool PerformMathOp(cloudViewer::ScalarField* sf1,
                              const cloudViewer::ScalarField* sf2,
                              Operation op);

    //! Performs a mathematical operation between two scalar fields (they must
    //! have the same size!)
    static bool PerformMathOp(const IScalarFieldWrapper& sf1,
...
public:  // members
    ...
    bool sf2WasAlreadyExisting;  // only considered if a second scalar field may
                                 // be necessary (PointFeature,
                                 // NeighborhoodFeature)
};
}  // namespace masc
