...
bool NeighborhoodFeature::checkValidity(QString corePointRole,
    ...
    if (stat != Feature::NO_STAT) {
        error = "Neighborhood features shouldn't be associated to a STAT "
                "measure";
        return false;
    }

    if (cloud2 && op == NO_OPERATION) {
        error = "Feature has a second cloud associated but no MATH operation "
                "is defined";
        return false;
    }
...
bool NeighborhoodFeature::prepare(
    ...
    if (sf1WasAlreadyExisting) {
        ...
        if (generatedScalarFields->scalarFields.contains(
                    sf1))  // i.e. the SF is existing but was not present at the
    ...
    if (!sf1) {
        error = QString("Failed to prepare scalar %1 @ scale %2")
                        .arg(resultSFName)
                        .arg(scale);
        return false;
    }
    ...
    if (cloud2 && op != Feature::NO_OPERATION && !sf1WasAlreadyExisting) {
        QString resultSFName2 = ToString(type) + "_" + cloud2Label + "@" +
                                QString::number(scale);

        assert(!sf2);

        sf2WasAlreadyExisting =
                CheckSFExistence(corePoints.cloud, qPrintable(resultSFName2));
        sf2 = PrepareSF(corePoints.cloud, qPrintable(resultSFName2),
                        generatedScalarFields,
                        sf2WasAlreadyExisting ? SFCollector::ALWAYS_KEEP
                                              : SFCollector::ALWAYS_REMOVE);

        if (!sf2) {
            error = QString("Failed to prepare scalar field for %1 @ scale %2")
                            .arg(cloud2Label)
                            .arg(scale);
            return false;
        }
...
bool NeighborhoodFeature::finish(const CorePoints& corePoints, QString& error) {
    ...
    if (sf1) {
        ...
        {
            int sfIndex1 =
                    corePoints.cloud->getScalarFieldIndexByName(sf1->getName());
    ...
    if (sf2 && !sf1WasAlreadyExisting) {
        ...
        //		{
        //			int sfIndex2 =
        // corePoints.cloud->getScalarFieldIndexByName(sf2->getName());
        // if (sfIndex2 >= 0)
        //			{
...
bool NeighborhoodFeature::computeValue(
    ...
    switch (type) {
        ...
        case PLANA:
        case VERT: {
            cloudViewer::Neighbourhood::GeomFeature f;
            switch (type) {
                case PCA1:
                    f = cloudViewer::Neighbourhood::PCA1;
                    break;
                case PCA2:
                    f = cloudViewer::Neighbourhood::PCA2;
                    break;
                case PCA3:
                    f = cloudViewer::Neighbourhood::SurfaceVariation;
                    break;
                case SPHER:
                    f = cloudViewer::Neighbourhood::Sphericity;
                    break;
                case LINEA:
                    f = cloudViewer::Neighbourhood::Linearity;
                    break;
                case PLANA:
                    f = cloudViewer::Neighbourhood::Planarity;
                    break;
                case VERT:
                    f = cloudViewer::Neighbourhood::Verticality;
                    break;
                default:
                    // impossible
                    assert(false);
            ...
            cloudViewer::DgmOctreeReferenceCloud neighboursCloud(
                    &pointsInNeighbourhood, static_cast<unsigned>(kNN));
            cloudViewer::Neighbourhood Z(&neighboursCloud);
        ...
        case DipDir:
            if (kNN >= 3) {
                ...
                if (N) {
                    ...
                    ccNormalVectors::ConvertNormalToDipAndDipDir(Np, dip_deg,
                                                                 dipDir_deg);
        ...
        case CURV: {
            cloudViewer::DgmOctreeReferenceCloud neighboursCloud(
                    &pointsInNeighbourhood, static_cast<unsigned>(kNN));
            cloudViewer::Neighbourhood Z(&neighboursCloud);
            outputValue = Z.computeCurvature(
                    queryPoint,
                    cloudViewer::Neighbourhood::
                            MEAN_CURV);  // TODO: is it really the default one?
        } break;
            ...
            if (kNN >= 3) {
                ...
                if (G) {
                    ...
                    if (r > std::numeric_limits<double>::epsilon()) {
                        ...
                        // Ratio of distance to center of mass and radius of
                        // sphere
                        outputValue = d / r;
                    }
                }
        ...
        // case ORIENF:
        default: {
            CVLog::Warning("Unhandled feature");
            assert(false);
            return false;
        }
    }
