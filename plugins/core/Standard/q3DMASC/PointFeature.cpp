...
IScalarFieldWrapper::Shared PointFeature::retrieveField(ccPointCloud* cloud, QString& error)
{
	...
	case PointFeature::Intensity:
	{
		cloudViewer::ScalarField* sf = Tools::RetrieveSF(cloud, LAS_FIELD_NAMES[LAS_INTENSITY], false);
		if (!sf)
		{
	...
	case PointFeature::NbRet:
	{
		cloudViewer::ScalarField* sf = Tools::RetrieveSF(cloud, LAS_FIELD_NAMES[LAS_NUMBER_OF_RETURNS], false);
		if (!sf)
		{
	...
	case PointFeature::RetNb:
	{
		cloudViewer::ScalarField* sf = Tools::RetrieveSF(cloud, LAS_FIELD_NAMES[LAS_RETURN_NUMBER], false);
		if (!sf)
		{
	...
	case PointFeature::EchoRat:
	{
		...
		cloudViewer::ScalarField* numberOfRetSF = Tools::RetrieveSF(cloud, LAS_FIELD_NAMES[LAS_NUMBER_OF_RETURNS], false);
		if (!numberOfRetSF)
		{
			error = "Can't compute the 'echo ratio' field: no 'Number of Return' SF available";
			return nullptr;
		}
		cloudViewer::ScalarField* retNumberSF = Tools::RetrieveSF(cloud, LAS_FIELD_NAMES[LAS_RETURN_NUMBER], false);
		if (!retNumberSF)
		{
	...
	case PointFeature::NIR:
	{
		cloudViewer::ScalarField* sf = Tools::RetrieveSF(cloud, s_NIRSFName, false);
		if (!sf)
		{
	...
	case PointFeature::M3C2:
	{
		cloudViewer::ScalarField* sf = Tools::RetrieveSF(cloud, s_M3C2SFName, true);
		if (!sf)
		{
	...
	case PointFeature::PCV:
	{
		cloudViewer::ScalarField* sf = Tools::RetrieveSF(cloud, s_PCVSFName, true);
		if (!sf)
		{
...
#pragma omp parallel for num_threads(std::max(1, omp_get_max_threads() - 2))
	...
	if (!cancelled)
	{
		...
		if (i && (i % tenth) == 0)
		{
			...
			if (density < 1.1)
			{
				if (octreeLevel + 1 < cloudViewer::DgmOctree::MAX_OCTREE_LEVEL)
					++octreeLevel;
			}
			...
			CVLog::Print(QString("[Adaptative octree level] Mean neighborhood size: %1 --> new level = %2").arg(meanNeighborhoodSize / tenth).arg(octreeLevel));
...
bool PointFeature::computeStat(const cloudViewer::DgmOctree::NeighboursSet& pointsInNeighbourhood, const IScalarFieldWrapper::Shared& sourceField, double& outputValue) const
{
