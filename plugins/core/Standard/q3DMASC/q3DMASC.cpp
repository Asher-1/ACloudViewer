...
void q3DMASCPlugin::doClassifyAction()
{
	...
	{
		m_app->dispToConsole("Failed to read classifier file (see Console)", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	if (cloudLabels.empty())
	{
		m_app->dispToConsole("Invalid classifier file (no cloud label defined)", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	else if (cloudLabels.size() > 4 + (cloudLabels.contains("TEST") ? 1 : 0))
	{
		m_app->dispToConsole("This classifier uses more than 4 clouds (the GUI version cannot handle it)", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	{
		m_app->dispToConsole("No classifier or invalid classifier", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	//apply classifier
	{
		...
		{
			m_app->dispToConsole(errorMessage, ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			generatedScalarFields.releaseSFs(false);
			return;
		}
...
void q3DMASCPlugin::doTrainAction()
{
	...
	if (useCloudsFromDB)
	{
		...
		{
			m_app->dispToConsole("This classifier uses more than 4 different clouds (the GUI version cannot handle it)", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			return;
		}
		...
		m_app->dispToConsole("Training cloud: " + mainCloudLabel, ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	{
		m_app->dispToConsole("Failed to load the training file", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}

	if (!corePoints.origin)
	{
		m_app->dispToConsole("Core points not defined", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	{
		m_app->dispToConsole("Missing 'Classification' field on core points cloud", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	{
		if (it.value())
			m_app->dispToConsole(it.key() + " = " + it.value()->getName(), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		else
	...
	if (loadedClouds.contains("TEST"))
	{
		...
		if (testCloud != corePoints.origin && testCloud != corePoints.cloud)
		{
			...
			{
				m_app->dispToConsole("Failed to load the training file (for TEST)", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				return;
			}
		}
	}
	...
	{
		m_app->dispToConsole("Failed to compute/prepare the core points!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		delete group;
		return;
	}
	...
	for (int iteration = 0; ; ++iteration)
	{
		...
		if (features.empty())
		{
			m_app->dispToConsole("No feature selected!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		}
		else
		{
			...
			if (!testCloud)
			{
				...
				{
					assert(false);
					m_app->dispToConsole("Invalid test data ratio", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
					trainSubset.clear();
					testSubset.clear();
				}
				...
				{
					if (!trainSubset)
						trainSubset.reset(new cloudViewer::ReferenceCloud(corePoints.cloud));
					trainSubset->clear();

					if (!testSubset)
						testSubset.reset(new cloudViewer::ReferenceCloud(corePoints.cloud));
					testSubset->clear();
					...
					{
						m_app->dispToConsole("Not enough memory to generate the test subsets", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
						generatedScalarFields.releaseSFs(false);
						generatedScalarFieldsTest.releaseSFs(false);
						return;
					}
...
//				trainDlg.shouldSaveClassifier(); // useless?
			...
			//test the trained classifier
			{
				...
				m_app->dispToConsole(resultText, ecvMainAppInterface::STD_CONSOLE_MESSAGE);
				...
				if (trainDlg.getSaveTrace())
				{
					...
					if (!tracePath.isEmpty())
					{
						...
						{
							m_app->dispToConsole("Classifier succesfully saved to " + outputFilePath, ecvMainAppInterface::STD_CONSOLE_MESSAGE);
						...
						{
							CVLog::Warning(QString("Can't open file '%1' for writing").arg(filename));
						}
		...
		while (true) // ew!
		{
			...
			if (trainDlg.shouldSaveClassifier())
			{
				...
				{
					m_app->dispToConsole("Classifier succesfully saved to " + outputFilename, ecvMainAppInterface::STD_CONSOLE_MESSAGE);
					trainDlg.setClassifierSaved();
				}
				else
				{
