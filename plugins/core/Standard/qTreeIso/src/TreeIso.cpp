...
bool TreeIso::Init_seg_pcd(ccPointCloud* pc,
    ...
    perform_cut_pursuit(K, regStrength1, pc_sub, edgeWeight, Eu, Ev,
    ...
    if (outSFIndex < 0) {
        ...
        if (outSFIndex < 0) {
            CVLog::Error("[TreeIso] Not enough memory!");
            return false;
        }
    }
    cloudViewer::ScalarField* outSF = pc->getScalarField(outSFIndex);
    ...
    CVLog::Print(QString("[TreeIso] Init segs took %1 seconds").arg(elapsed));
...
bool TreeIso::Intermediate_seg_pcd(ccPointCloud* pc,
                                   const unsigned PR_MIN_NN2,
    ...
    if (initSFIndex < 0) {
        CVLog::Error("[TreeIso] Please run initial segmentation first!");
        return false;
    }
    cloudViewer::ScalarField* initSF = pc->getScalarField(initSFIndex);
    ...
    for (size_t i = 0; i < n_clusters; ++i) {
        ...
        if (currentClusterPos.size() > 1) {
            ...
            decimate_vec(currentClusterPos, PR_DECIMATE_RES2,
                         currentClusterPosDec);
    ...
    knn_cpp_nearest_neighbors(clusterCentroids, PR_MIN_NN2, minIdxsC, minIdxsD,
                              8);
    ...
    std::vector<
            knncpp::KDTreeMinkowskiX<float, knncpp::EuclideanDistance<float>>>
            knn_kdtrees;
    for (size_t i = 0; i < n_centroids; ++i) {
        std::vector<Vec3d> currentClusterDec = currentClusterDecs[i];
        Eigen::MatrixXf currentClusterDecMat(currentClusterDec[0].size(),
                                             currentClusterDec.size());
        for (size_t k = 0; k < currentClusterDec.size(); k++) {
            currentClusterDecMat.col(k) = Eigen::VectorXf::Map(
                    &currentClusterDec[k][0], currentClusterDec[k].size());
        }
    ...
    for (size_t i = 0; i < n_centroids; ++i) {
        knncpp::KDTreeMinkowskiX<float, knncpp::EuclideanDistance<float>>
                knn_kdtree(currentClusterDecMats[minIdxsC[i][0]]);
        knn_cpp_build(knn_kdtree);
        for (size_t j = 1; j < n_K; ++j) {
            if (minIdxsD[i][j] > 0) {
                std::vector<std::vector<size_t>> min_c;
                std::vector<Vec3d> min_D2;
                float min_D = knn_cpp_query_min_d(
                        knn_kdtree, currentClusterDecMats[minIdxsC[i][j]], 1);
                nnDists[i][j] = min_D;
            }
        }
    }
    ...
    for (auto& vec : clusterVGroup) {
        currentClusterDecsIDs.insert(currentClusterDecsIDs.end(),
                                     std::make_move_iterator(vec.begin()),
                                     std::make_move_iterator(vec.end()));
    }
    for (auto& vec : currentClusterDecs) {
        currentClusterDecsFlat.insert(currentClusterDecsFlat.end(),
                                      std::make_move_iterator(vec.begin()),
                                      std::make_move_iterator(vec.end()));
        currentClusterDecsICsFlatIndex.insert(
                currentClusterDecsICsFlatIndex.end(), vec.size());
    }
    ...
    std::vector<Vec3d> Ds;
    knn_cpp_nearest_neighbors(currentClusterDecsFlat, PR_MIN_NN2, minIdxs, Ds,
                              8);
    ...
    perform_cut_pursuit2d(PR_MIN_NN2, PR_REG_STRENGTH2, currentClusterDecsFlat,
                          edgeWeight, Eu, Ev, in_component2d);
    ...
    for (size_t i = 0; i < currentClusterDecs.size(); i++) {
        ...
        std::vector<uint32_t> v(in_component2d.begin() + ia_counter,
                                in_component2d.begin() + ia_counter +
                                        currentClusterDecsICsFlatIndex[i]);
        for (size_t j = 0; j < N; j++) {
            currentClusterDecsICsReverse.push_back(
                    v[currentClusterDecsICs[i][j]]);
        }
    ...
    sort_indexes(currentClusterDecsIDs, currentClusterDecsIDsSortedIdx,
                 currentClusterDecsIDsSorted);
    std::vector<uint32_t> currentClusterDecsICsReverseSorted;
    currentClusterDecsICsReverseSorted.reserve(
            currentClusterDecsICsReverse.size());
    std::transform(currentClusterDecsIDsSortedIdx.begin(),
                   currentClusterDecsIDsSortedIdx.end(),
                   std::back_inserter(currentClusterDecsICsReverseSorted),
    ...
    auto elapsed = Since(start).count() / 1000;
    CVLog::Print(QString("[TreeIso] Intermediate segs took %1 seconds")
                         .arg(elapsed));

    return true;
}

bool TreeIso::Final_seg_pcd(ccPointCloud* pc,
    ...
    if (groupIdx < 0) {
        CVLog::Error("[TreeIso] Please run intermeditate segmentation first!");
        return false;
    }

    cloudViewer::ScalarField* initSF = pc->getScalarField(initIdx);
    ...
    cloudViewer::ScalarField* groupSF = pc->getScalarField(groupIdx);
    if (!groupSF) {
        assert(false);
        return false;
    }
    ...
    if (clusterVGroup.size() > 1) {
        ...
        while ((n_to_merge_ids != 0) &&
            ...
            size_t knncpp_nn =
                    (PR_MIN_NN3 < n_clusters ? PR_MIN_NN3 : n_clusters);
            std::vector<std::vector<uint32_t>> groupNNIdxC;
            std::vector<Vec3d> groupNNCDs;
            knn_cpp_nearest_neighbors(centroid2DFeatures, knncpp_nn,
                                      groupNNIdxC, groupNNCDs, 8);
            Vec3d mds;
            ...
            for (size_t i = 0; i < nGroups; ++i) {
                ...
                float currentGroupRelHt =
                        (zFeatures[i] - minZ) / lenFeatures[i];

                if (abs(currentGroupRelHt) > PR_REL_HEIGHT_LENGTH_RATIO) {
                    if (iter == 1) {
                        float initialLenRatio =
                                lenFeatures[i] / median_col(lenFeatures);
            ...
            std::set_difference(allIds.begin(), allIds.end(),
                                toMergeIds.begin(), toMergeIds.end(),
                                std::inserter(remainIds, remainIds.begin()));
            ...
            size_t knncpp_nn2 =
                    (PR_MIN_NN3 < remainIds.size() ? PR_MIN_NN3
                                                   : remainIds.size());

            knncpp::KDTreeMinkowskiX<float, knncpp::EuclideanDistance<float>>
                    knn_kdtree(groupCentroidsRemain);
            knn_cpp_build(knn_kdtree);
            std::vector<std::vector<size_t>> groupNNIdx;
            std::vector<Vec3d> groupNNIdxDists;

            knn_cpp_query(knn_kdtree, groupCentroidsToMerge, knncpp_nn2,
                          groupNNIdx, groupNNIdxDists);
            ...
            for (size_t i = 0; i < nToMergeIds; ++i) {
                ...
                get_subset(clusterCentroids, clusterVGroup[toMergeId],
                ...
                for (size_t j = 0; j < nNNs; ++j) {
                    ...
                    float lineSegs2 = zFeatures[toMergeId] +
                                      lenFeatures[toMergeId] -
                                      zFeatures[remainId];
                    float lineSegs1 = zFeatures[remainId] +
                                      lenFeatures[remainId] -
                                      zFeatures[toMergeId];

                    float verticalOverlapRatio =
                            (lineSegs2 > lineSegs1 ? lineSegs1 : lineSegs2) /
                            (lineSegs1 > lineSegs2 ? lineSegs1 : lineSegs2);
                    float horizontalOverlapRatio;
                    if ((boost::geometry::num_points(groupHulls[toMergeId]) >
                         3) &
                        (boost::geometry::num_points(groupHulls[remainId]) >
                         3)) {
                        multi_polygon intersection;
                        boost::geometry::intersection(groupHulls[toMergeId],
                                                      groupHulls[remainId],
                                                      intersection);
                        float intersect_area =
                                boost::geometry::area(intersection);
                        float area1 =
                                boost::geometry::area(groupHulls[toMergeId]);
                        float area2 =
                                boost::geometry::area(groupHulls[remainId]);
                    ...
                    get_subset(clusterCentroids, clusterVGroup[remainId],
                               nnClusterCentroids);

                    knncpp::KDTreeMinkowskiX<float,
                                             knncpp::EuclideanDistance<float>>
                            knn_kdtree2(nnClusterCentroids);
                    knn_cpp_build(knn_kdtree2);
                    std::vector<std::vector<size_t>> min3D_idx;
                    std::vector<Vec3d> min3D_dists;

                    knn_cpp_query(knn_kdtree2, currentClusterCentroids, 1,
                                  min3D_idx, min3D_dists);
                    float min3DSpacing = min_col(min3D_dists[0]);
                    min3DSpacings.push_back(min3DSpacing);

                    Eigen::MatrixXf nnClusterCentroids2D =
                            nnClusterCentroids.block(0, 0, 2,
                                                     nnClusterCentroids.cols());
                    Eigen::MatrixXf currentClusterCentroids2D =
                            currentClusterCentroids.block(
                                    0, 0, 2, currentClusterCentroids.cols());
                    Eigen::VectorXf nnClusterCentroids2Dmean =
                            nnClusterCentroids2D.rowwise().mean();
                    Eigen::VectorXf currentClusterCentroids2Dmean =
                            currentClusterCentroids2D.rowwise().mean();
                    float min2DSpacing =
                            sqrt((nnClusterCentroids2Dmean[0] -
                                  currentClusterCentroids2Dmean[0]) *
                                         (nnClusterCentroids2Dmean[0] -
                                          currentClusterCentroids2Dmean[0]) +
                                 (nnClusterCentroids2Dmean[1] -
                                  currentClusterCentroids2Dmean[1]) *
                                         (nnClusterCentroids2Dmean[1] -
                                          currentClusterCentroids2Dmean[1]));

                    verticalOverlapRatio =
                            verticalOverlapRatio > 0 ? verticalOverlapRatio : 0;

                    float score = exp(
                            -(1 - horizontalOverlapRatio) *
                                    (1 - horizontalOverlapRatio) -
                            PR_VERTICAL_WEIGHT * (1 - verticalOverlapRatio) *
                                    (1 - verticalOverlapRatio) -
                            ((min3DSpacing < min2DSpacing ? min3DSpacing
                                                          : min2DSpacing) /
                             sigmaD) *
                                    ((min3DSpacing < min2DSpacing
                                              ? min3DSpacing
                                              : min2DSpacing) /
                                     sigmaD));
                ...
                std::transform(scoreSort.begin(), scoreSort.end(),
                               std::back_inserter(scoreSortRatio),
                               [score_highest](float value) {
                                   return value / score_highest;
                               });
                ...
                if (nScoreSortCandidateIdx == 1) {
                    mergeNNId = groupU
                            [filteredRemainIds
                                     [scoreSortI[scoreSortCandidateIdx[0]]]];

                } else if (nScoreSortCandidateIdx > 1) {
                    std::vector<float> min3DSpacingsFiltered;
                    std::vector<size_t> scoreSortIFiltered;
                    get_subset(scoreSortI, scoreSortCandidateIdx,
                               scoreSortIFiltered);
                    get_subset(min3DSpacings, scoreSortIFiltered,
                               min3DSpacingsFiltered);

                    size_t filterMinSpacingIdx =
                            arg_min_col(min3DSpacingsFiltered);
                    mergeNNId =
                            groupU[filteredRemainIds
                                           [scoreSortI[filterMinSpacingIdx]]];
                } else {
                    continue;
                }

                std::vector<uint32_t> currentVGroup =
                        groupVGroup[toMergeIds[i]];
        ...
        auto elapsed = Since(start).count() / 1000;
        CVLog::Print(QString("[TreeIso] Final segs took: %1 seconds !!!")
                             .arg(elapsed));
    }
...
bool TreeIso::Init_seg(const unsigned min_nn1,
    ...
    if (selectedEntities.empty()) {
        assert(false);
        app->dispToConsole("[TreeIso] Select at least one cloud",
    ...
    if (!ent || !ent->isA(CV_TYPES::POINT_CLOUD)) {
        app->dispToConsole("[TreeIso] Not a point cloud",
                           ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return false;
    }
    ccPointCloud* pointCloud = static_cast<ccPointCloud*>(ent);

    if (Init_seg_pcd(pointCloud, min_nn1, regStrength1, PR_DECIMATE_RES1,
                     progressDlg)) {
        ent->redrawDisplay();
        return true;
    } else {
...
bool TreeIso::Intermediate_seg(const unsigned PR_MIN_NN2,
    ...
    if (Intermediate_seg_pcd(pointCloud, PR_MIN_NN2, PR_REG_STRENGTH2,
                             PR_DECIMATE_RES2, PR_MAX_GAP, progressDlg)) {
        ent->redrawDisplay();
        return true;
    } else {
        return false;
...
bool TreeIso::Final_seg(const unsigned PR_MIN_NN3,
    ...
    if (Final_seg_pcd(pointCloud, PR_MIN_NN3, PR_REL_HEIGHT_LENGTH_RATIO,
                      PR_VERTICAL_WEIGHT, progressDlg)) {
        ent->redrawDisplay();
        return true;
    } else {
        return false;
...
void knn_cpp_query(
    ...
    for (Eigen::Index i = 0; i < indices.rows(); i++) {
        res_idx[i] = std::vector<size_t>(
                indices.row(i).data(), indices.row(i).data() + indices.cols());
    }
    res_dists.resize(distances.rows(), Vec3d(distances.cols()));
    for (Eigen::Index i = 0; i < distances.rows(); i++) {
        res_dists[i] = Vec3d(distances.row(i).data(),
                             distances.row(i).data() + distances.cols());
    }
}

float knn_cpp_query_min_d(
        knncpp::KDTreeMinkowskiX<float, knncpp::EuclideanDistance<float>>&
                kdtree,
        Eigen::MatrixXf& query_points,
        size_t k) {
...
void knn_cpp_nearest_neighbors(const std::vector<Vec3d>& dataset,
    ...
    knncpp::KDTreeMinkowskiX<float, knncpp::EuclideanDistance<float>> kdtree(
            mat);
    kdtree.setBucketSize(16);
    kdtree.setSorted(true);
    kdtree.setThreads(n_thread);
    ...
    for (Eigen::Index i = 0; i < indices.cols(); i++) {
        res_idx[i] = std::vector<uint32_t>(
                indices.col(i).data(), indices.col(i).data() + indices.rows());
    }
    res_dists.resize(distances.cols(), Vec3d(distances.rows()));
    for (Eigen::Index i = 0; i < distances.cols(); i++) {
        res_dists[i] = Vec3d(distances.col(i).data(),
                             distances.col(i).data() + distances.rows());
    }
}
...
void unique_group(std::vector<T>& arr,
    ...
    // detect the location (before sorted) where a row in the sorted array is
    // different from the previous row (as ia), and add one for the reverse
    // index as ic
...
// convert ccPointCloud to a vector of points (centering the points about their
// gravity center)
template <typename T>
void toTranslatedVector(const ccPointCloud* pc,
    ...
    for (unsigned i = 0; i < pointCount; ++i) {
        ...
        y[i] = {static_cast<float>(pv->x), static_cast<float>(pv->y),
...
bool perform_cut_pursuit(const uint32_t K,
    ...
    CP::cut_pursuit<float>(nNod, nEdg, nObs, y, Eu, Ev, edgeWeight, nodeWeight,
                           solution, in_component, components, regStrength,
                           cutoff, mode, speed, weight_decay, verbose);
    return true;
}
...
void sort_indexes(std::vector<ValueType>& v,
    ...
    std::stable_sort(idx.begin(), idx.end(), [&v](IndexType i1, IndexType i2) {
        return v[i1] < v[i2];
    });
