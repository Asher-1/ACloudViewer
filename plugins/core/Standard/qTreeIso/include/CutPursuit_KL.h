...
namespace CP {
template <typename T>
struct CutPursuit_KL : public CutPursuit<T> {
    std::pair<T, T> compute_energy() override {
        ...
        for (VertexIterator<T> i_ver = boost::vertices(this->main_graph).first;
            ...
            for (uint32_t i_dim = 0; i_dim < this->dim;
                 i_dim++) {  // smoothing as a linear combination with the
                             // uniform probability
                smoothedObservation =
    ...
    inline void init_labels(
        ...
        std::vector<std::vector<T>> smooth_kernels(2,
                                                   std::vector<T>(this->dim));
        T total_weight[2];
        uint32_t nb_comp = static_cast<uint32_t>(this->components.size());
        T best_energy, current_energy;
        // #pragma omp parallel for private(kernels, total_weight, best_energy,
        // current_energy) if (this->parameter.parallel && nb_comp>8)
        // schedule(dynamic)
        for (uint32_t i_com = 0; i_com < nb_comp; i_com++) {
            uint32_t comp_size =
                    static_cast<uint32_t>(this->components[i_com].size());
            std::vector<bool> potential_label(comp_size);
            std::vector<T> energy_array(comp_size);
            std::vector<T> constant_part(comp_size);
            std::vector<std::vector<T>> smooth_obs(comp_size,
                                                   std::vector<T>(2, 0));
            ...
            for (uint32_t init_kmeans = 0;
                ...
                uint32_t first_kernel = std::rand() % comp_size,
                         second_kernel = 0;
                for (uint32_t i_dim = 0; i_dim < this->dim;
                     i_dim++) {  // fill the first kernel
                    kernels[0][i_dim] =
                            vertex_attribute_map(
                                    this->components[i_com][first_kernel])
                                    .observation[i_dim];
                    smooth_kernels[0][i_dim] =
                            this->parameter.smoothing / this->dim +
                            (1 - this->parameter.smoothing) * kernels[0][i_dim];
                }
                // now compute the square distance of each pouint32_t to this
                // kernel
                ...
                if (best_energy ==
                    0) {  // all the points in this components are identical
                    for (uint32_t i_ver = 0; i_ver < comp_size; i_ver++) {
                        binary_label[vertex_index_map(
                                this->components[i_com][i_ver])] = false;
                    }
                    break;
                }
                ...
                for (uint32_t i_dim = 0; i_dim < this->dim;
                    ...
                    kernels[1][i_dim] =
                            vertex_attribute_map(
                                    this->components[i_com][second_kernel])
                                    .observation[i_dim];
                    smooth_kernels[1][i_dim] =
                            this->parameter.smoothing / this->dim +
                            (1 - this->parameter.smoothing) * kernels[1][i_dim];
                }
                //----main kmeans loop-----
                for (uint32_t ite_kmeans = 0;
                    ...
                    // kernel------------------- #pragma omp parallel for if
                    //(this->parameter.parallel && nb_comp<=8) schedule(dynamic)
                    for (uint32_t i_ver = 0; i_ver < comp_size; i_ver++) {
                    ...
                    for (uint32_t i_ver = 0; i_ver < comp_size; i_ver++) {
                        if (vertex_attribute_map(this->components[i_com][i_ver])
                                    .weight == 0) {
                            continue;
                        }
    ...
    inline void compute_centers(VectorOfCentroids<T>& centers,
        ...
        for (uint32_t i_com = 0; i_com < nb_comp; i_com++) {
            ...
            if ((total_weight[0] == 0) || (total_weight[1] == 0)) {
                ...
                for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
                    centers.centroids[i_com][0][i_dim] =
                            vertex_attribute_map(this->components[i_com].back())
                                    .value[i_dim];
                    centers.centroids[i_com][1][i_dim] =
                            vertex_attribute_map(this->components[i_com].back())
                                    .value[i_dim];
    ...
    inline void set_capacities(const VectorOfCentroids<T>& centers) {
        ...
        uint32_t nb_comp = static_cast<uint32_t>(this->components.size());
        T cost_B, cost_notB, smoothedValueB, smoothedValueNotB,
                smoothedObservation;  // the cost of being in B or not B, local
                                      // for each component
            ...
            for (uint32_t i_ver = 0; i_ver < this->components[i_com].size();
                ...
                for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
                    smoothedObservation =
                            ...
                            (1 - this->parameter.smoothing) *
                                    vertex_attribute_map(desc_v)
                                            .observation[i_dim];
                    smoothedValueB = this->parameter.smoothing / this->dim +
                                     (1 - this->parameter.smoothing) *
                                             centers.centroids[i_com][0][i_dim];
                    smoothedValueNotB =
                            this->parameter.smoothing / this->dim +
                    ...
                    cost_B += smoothedObservation *
                              (log(smoothedObservation) - log(smoothedValueB));
                    cost_notB +=
                            smoothedObservation *
                            (log(smoothedObservation) - log(smoothedValueNotB));
                }
                if (cost_B > cost_notB) {
    ...
    std::pair<std::vector<T>, T> compute_merge_gain(
        ...
        for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
            ...
            smoothedValueMerged =
                    this->parameter.smoothing / this->dim +
                    (1 - this->parameter.smoothing) * merge_value[i_dim];
            gain -= reduced_vertex_attribute_map(comp1).weight *
                            smoothedValue1 *
                            (log(smoothedValue1) - log(smoothedValueMerged)) +
                    reduced_vertex_attribute_map(comp2).weight *
                            smoothedValue2 *
                            (log(smoothedValue2) - log(smoothedValueMerged));
        }
