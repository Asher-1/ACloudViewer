...
namespace CP {
template <typename T>
struct CutPursuit_L2 : public CutPursuit<T> {
    //=============================================================================================
    //=============================     COMPUTE ENERGY
    //===========================================
    //=============================================================================================
    ...
    size_t split()
        ...
        for (uint32_t i_step = 1; i_step <= this->parameter.flow_steps;
             i_step++) {
    ...
    inline void init_labels(
            std::vector<bool>&
                    binary_label) {  //-----initialize the labelling for each
                                     // components with
                                     // kmeans------------------------------
        VertexAttributeMap<T> vertex_attribute_map =
                boost::get(boost::vertex_bundle, this->main_graph);
...
#pragma omp parallel for if (nb_comp < omp_get_num_threads()) \
        shared(potential_label) schedule(static)
                    ...
                    if ((total_weight[0] == 0) || (total_weight[1] == 0)) {
                        // std::cout << "kmeans error : " << comp_size <<
                        // std::endl;
                        break;
                    }
                    for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
