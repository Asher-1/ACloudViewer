...
namespace CP {
template <typename T>
struct CutPursuit_SPG : public CutPursuit<T> {
    //=============================================================================================
    //=============================     COMPUTE ENERGY
    //===========================================
    //=============================================================================================
    std::pair<T, T> compute_energy() override {
        ...
        // the first element pair_energy of is the fidelity and the second the
        // penalty
        std::pair<T, T> pair_energy;
        T energy = 0;
        // #pragma omp parallel for private(i_dim) if (this->parameter.parallel)
        // schedule(static) reduction(+:energy,i)
        for (uint32_t ind_ver = 0; ind_ver < this->nVertex; ind_ver++) {
            VertexDescriptor<T> i_ver =
    ...
    size_t split()
            override {  // split the graph by trying to find the best binary
                        ...
                        // for each components we associate the value h_1 and
                        // h_2 to vertices in B or notB the affectation as well
                        // as h_1 and h_2 are computed alternatively
        // tic();
        //--------loading
        // structures---------------------------------------------------------------
        ...
        VertexIndexMap<T> vertex_index_map =
                boost::get(boost::vertex_index, this->main_graph);
        ...
        // centers is the value of each binary component in the optimal
        // partition
        ...
        T unary_weight = pow(this->parameter.weight_decay,
                             -float(this->parameter.flow_steps));
        for (uint32_t i_step = 0; i_step < this->parameter.flow_steps;
             i_step++) {
            ...
            for (uint32_t ind_com = 0; ind_com < nb_comp; ind_com++) {
                ...
                for (uint32_t i_ver = 0;
                     i_ver < this->components[ind_com].size(); i_ver++) {
                    binary_label[vertex_index_map(
        ...
        size_t saturation = this->activate_edges(false);
        return saturation;
    }
    ...
    inline void init_labels(
            std::vector<bool>& binary_label,
            bool spatial_part) {  //-----initialize the labelling for each
                                  // components with
                                  // kmeans------------------------------
        VertexAttributeMap<T> vertex_attribute_map =
                boost::get(boost::vertex_bundle, this->main_graph);
        ...
        uint32_t nb_comp = static_cast<uint32_t>(this->components.size());
        // ind_com;
        // #pragma omp parallel for private(ind_com) //if (nb_comp>=8)
        // schedule(dynamic)
...
#pragma omp parallel for if (nb_comp >= omp_get_num_threads()) schedule(dynamic)
        ...
        for (uint32_t ind_com = 0; ind_com < nb_comp; ind_com++) {
            ...
            T current_energy;
            uint32_t comp_size =
                    static_cast<uint32_t>(this->components[ind_com].size());
            ...
            for (uint32_t init_kmeans = 0;
                 init_kmeans < this->parameter.kmeans_resampling;
                 init_kmeans++) {  // proceed to several initilialisation of
                                   // kmeans and pick up the best one
                //----- initialization with KM++ ------------------
                uint32_t first_kernel = std::rand() % comp_size,
                         second_kernel = 0;  // first kernel attributed
                for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
                    kernels[0][i_dim] =
                            vertex_attribute_map(
...
#pragma omp parallel for if (nb_comp < omp_get_num_threads()) \
        shared(best_energy) schedule(static)
                ...
                }  // we now generate a random number to determinate which node
                   // will be the second kernel
                T random_sample = ((T)(rand())) / ((T)(RAND_MAX));
                current_energy = best_energy * random_sample;
                for (uint32_t i_ver = 0; i_ver < comp_size; i_ver++) {
                    current_energy -= energy_array[i_ver];
                    if (current_energy <
                        0) {  // we have selected the second kernel
                        second_kernel = i_ver;
                        break;
                    }
                }
                for (uint32_t i_dim = 0; i_dim < this->dim;
                     i_dim++) {  // now fill the second kernel
                    kernels[1][i_dim] =
                            vertex_attribute_map(
                                    this->components[ind_com][second_kernel])
                                    .observation[i_dim];
                }
                //----main kmeans loop-----
                for (uint32_t ite_kmeans = 0;
                     ite_kmeans < this->parameter.kmeans_ite; ite_kmeans++) {
                    //--affectation step: associate each node with its closest
                    // kernel-------------------
#ifdef OPENMP
#pragma omp parallel for if (nb_comp < omp_get_num_threads()) \
        shared(potential_label) schedule(static)
                    ...
                    for (uint32_t i_ver = 0; i_ver < comp_size; i_ver++) {
                        ...
                        for (uint32_t i_dim = 0; i_dim < dim_spat; i_dim++) {
                            distance_kernels[0] += pow(
                                    vertex_attribute_map(
                                            this->components[ind_com][i_ver])
                                                    .observation[i_dim] -
                                            kernels[0][i_dim],
                                    2);
                            distance_kernels[1] += pow(
                                    vertex_attribute_map(
                                            this->components[ind_com][i_ver])
                                                    .observation[i_dim] -
                                            kernels[1][i_dim],
                        ...
                        potential_label[i_ver] =
                                distance_kernels[0] > distance_kernels[1];
                    }
...
#pragma omp parallel for if (nb_comp < omp_get_num_threads()) \
    ...
    //==========================================
    //=============================================================================================
    inline void compute_centers(VectorOfCentroids<T>& centers,
                                const uint32_t& nb_comp,
...
#pragma omp parallel for if (nb_comp >= omp_get_num_threads()) schedule(dynamic)
    ...
    //==========================================
    //=============================================================================================
    inline void compute_center(std::vector<std::vector<T>>& center,
                               const uint32_t& ind_com,
                               const std::vector<bool>& binary_label) {
        // compute for each component the values of the centroids corresponding
        // to the optimal binary partition
        VertexAttributeMap<T> vertex_attribute_map =
                boost::get(boost::vertex_bundle, this->main_graph);
        VertexIndexMap<T> vertex_index_map =
                boost::get(boost::vertex_index, this->main_graph);
        ...
        for (uint32_t i_ver = 0; i_ver < this->components[ind_com].size();
             i_ver++) {
            if (vertex_attribute_map(this->components[ind_com][i_ver]).weight ==
                0) {
                continue;
            }
            if (binary_label[vertex_index_map(
                        this->components[ind_com][i_ver])]) {
                total_weight[0] +=
                        vertex_attribute_map(this->components[ind_com][i_ver])
                                .weight;
                for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
                    center[0][i_dim] +=
                            vertex_attribute_map(
        ...
        if ((total_weight[0] == 0) || (total_weight[1] == 0)) {
            ...
            for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
                center[0][i_dim] =
                        vertex_attribute_map(this->components[ind_com][0])
                                .value[i_dim];
                center[1][i_dim] =
                        vertex_attribute_map(this->components[ind_com][0])
                                .value[i_dim];
...
#pragma omp parallel for if (nb_comp >= omp_get_num_threads()) schedule(dynamic)
        ...
        for (uint32_t ind_com = 0; ind_com < nb_comp; ind_com++) {
            ...
            T cost_B, cost_notB;  // the cost of being in B or not B, local for
                                  // each component
            if (this->saturated_components[ind_com]) {
                continue;
            }
            for (uint32_t i_ver = 0; i_ver < this->components[ind_com].size();
                ...
                cost_B = 0;
                cost_notB = 0;
                if (vertex_attribute_map(desc_v).weight ==
                    0) {  // no observation - no cut
                ...
                for (uint32_t i_dim = 0; i_dim < this->dim; i_dim++) {
                    cost_B += 0.5 * vertex_attribute_map(desc_v).weight *
                              (pow(centers.centroids[ind_com][0][i_dim], 2) -
                               2 * (centers.centroids[ind_com][0][i_dim] *
                                    vertex_attribute_map(desc_v)
                                            .observation[i_dim]));
                    cost_notB += 0.5 * vertex_attribute_map(desc_v).weight *
                                 (pow(centers.centroids[ind_com][1][i_dim], 2) -
                                  2 * (centers.centroids[ind_com][1][i_dim] *
                                       vertex_attribute_map(desc_v)
                                               .observation[i_dim]));
                }
                if (cost_B > cost_notB) {
        ...
        for (boost::tie(i_edg, i_edg_end) = boost::edges(this->main_graph);
            ...
            if (!edge_attribute_map(*i_edg).isActive) {
                edge_attribute_map(*i_edg).capacity =
                        edge_attribute_map(*i_edg).weight *
                        this->parameter.reg_strenth / unary_weight;
            } else {
    ...
    //=========================================
    //=============================================================================================
    std::pair<std::vector<T>, T> compute_value(
            const uint32_t& ind_com) override {
        VertexAttributeMap<T> vertex_attribute_map =
                boost::get(boost::vertex_bundle, this->main_graph);
        T total_weight = 0;
...
#endif
        for (uint32_t ind_ver = 0; ind_ver < this->components[ind_com].size();
             ++ind_ver) {
            total_weight +=
