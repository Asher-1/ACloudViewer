...
static ccMesh* GetMeshFromCombo(QComboBox* comboBox, ccHObject* dbRoot) {
    ...
    if (!item || !item->isA(CV_TYPES::MESH)) {
        assert(false);
        return nullptr;
    }
...
qVoxFallDialog::qVoxFallDialog(ccMesh* mesh1,
    ...
    connect(showMesh1CheckBox, &QAbstractButton::toggled, this,
            &qVoxFallDialog::setMesh1Visibility);
    connect(showMesh2CheckBox, &QAbstractButton::toggled, this,
            &qVoxFallDialog::setMesh2Visibility);

    connect(swapMeshesToolButton, &QAbstractButton::clicked, this,
...
void qVoxFallDialog::swapMeshes() { setMeshes(m_mesh2, m_mesh1); }
...
void qVoxFallDialog::loadParamsFrom(const QSettings& settings) {
    // read parameters
    double voxelSize =
            settings.value("VoxelSize", voxelSizeDoubleSpinBox->value())
                    .toDouble();
    double azimuth =
            settings.value("Azimuth", azDoubleSpinBox->value()).toDouble();
    bool exportMeshesEnabled =
            settings.value("ExportMeshesEnabled", exportCheckBox->isChecked())
                    .toBool();
    bool lossGainEnabled =
            settings.value("LossGainEnabled", lossCheckBox->isChecked())
                    .toBool();
