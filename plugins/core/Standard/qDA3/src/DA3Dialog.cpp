...
QString DA3Dialog::modelCacheDir() {
    return QString::fromUtf8(da::default_model_cache_dir().c_str());
}
...
void DA3Dialog::setupUi() {
    ...
    m_modeCombo->addItem("Depth + Pose",
                         static_cast<int>(Mode::DepthPose));
    ...
    m_modeCombo->addItem("Export COLMAP",
                         static_cast<int>(Mode::ExportCOLMAP));
    m_modeCombo->addItem("Quantize Model",
                         static_cast<int>(Mode::Quantize));
    ...
    modelLayout->addWidget(new QLabel("Threads:"), 4, 0);
    ...
    browseFolderBtn->setToolTip("Select a folder for batch depth estimation on all images inside");
    ...
    m_dbImageCombo->setToolTip("Select an image already loaded in the viewer's DB tree");
    ...
    m_exportDepthBtn->setToolTip("Save the last depth estimation as a grayscale image file");
    ...
    m_exportAllBtn->setToolTip("Export all depth maps to the output directory as images");
...
void DA3Dialog::populateModelCombos() {
...
void DA3Dialog::syncModeModelConstraints() {
    ...
    const bool isGiantOrCustom = (sz == ModelSize::Giant || filename == "CUSTOM"
    ...
    auto* modeStdModel = qobject_cast<QStandardItemModel*>(m_modeCombo->model());
    if (modeStdModel) {
        for (int i = 0; i < m_modeCombo->count(); ++i) {
            auto m = static_cast<Mode>(m_modeCombo->itemData(i).toInt());
            auto* item = modeStdModel->item(i);
            if (!item) continue;
            if (m == Mode::Reconstruct)
                item->setEnabled(isGiantOrCustom);
        }
    }

    auto* modelStdModel = qobject_cast<QStandardItemModel*>(m_modelCombo->model());
    if (modelStdModel) {
    ...
    if (needsGiant && !isGiantOrCustom) {
        ...
        for (int i = 0; i < m_modeCombo->count(); ++i) {
            if (static_cast<Mode>(m_modeCombo->itemData(i).toInt()) == Mode::DepthSingle) {
...
void DA3Dialog::onBrowseCustomModel() {
    QString path = QFileDialog::getOpenFileName(
            this, "Select GGUF Model", QString(),
...
void DA3Dialog::onBrowseCustomMetricModel() {
    QString path = QFileDialog::getOpenFileName(
            this, "Select Metric Model", QString(),
            "GGUF Models (*.gguf);;All Files (*)");
...
bool DA3Dialog::ensureAllModelsAvailable() {
    ...
    auto collectMissing = [&](QComboBox* combo,
        ...
        for (const auto& bm : catalog) {
            if (bm.filename == data) { needed.append(bm); break; }
        }
    };
    ...
    if (result != QMessageBox::Yes) {
        appendLog(tr("[Info] Download declined by user. Please download or "
...
void DA3Dialog::startDownload(const DA3BuiltinModel& model) {
    ...
    QFile* outFile = new QFile(tmpDest, m_currentDownload);
    ...
    connect(m_currentDownload, &QNetworkReply::readyRead, this,
            [this, outFile]() { outFile->write(m_currentDownload->readAll()); });
    ...
    connect(m_currentDownload, &QNetworkReply::finished, this,
            [this, outFile, dest, tmpDest, model]() {
                outFile->close();
                bool ok = m_currentDownload->error() == QNetworkReply::NoError;
                if (ok) {
                    QFile::remove(dest);
...
void DA3Dialog::onModeChanged(int index) {
    ...
    bool showOutput = isExport || isQuantize || isDepth || (mode == Mode::Reconstruct);
...
void DA3Dialog::onBrowseFile() {
    ...
    const QString imageFilter =
            ...
            "*.ppm *.pgm *.pbm);;All Files (*)";
    if (mode == Mode::DepthMultiView || mode == Mode::Reconstruct) {
        QStringList paths = QFileDialog::getOpenFileNames(
                this, "Select Image(s)", QString(), imageFilter);
        if (!paths.isEmpty()) m_inputPath->setText(paths.join(";"));
    } else {
        QString path = QFileDialog::getOpenFileName(
                this, "Select Input Image", QString(), imageFilter);
...
void DA3Dialog::onBrowseFolder() {
    ...
    filters << "*.png" << "*.jpg" << "*.jpeg" << "*.bmp" << "*.tif" << "*.tiff"
            ...
            << "*.WEBP" << "*.GIF";
    QStringList files = folder.entryList(filters, QDir::Files, QDir::Name);
    ...
    for (const auto& f : files) {
        fullPaths << folder.absoluteFilePath(f);
    }
    m_inputPath->setText(fullPaths.join(";"));
    appendLog(tr("[Info] Found %1 image(s) in folder: %2")
                      .arg(files.size()).arg(dir));
}
