...
// M5 gate: GIANT depth (DualDPT) + pose (CameraDec) via the EXISTING
// metadata-driven DptHead/CamPose on the giant config dims (head dim_in 3072,
// features 256, out_channels [256,512,1024,1024]; cam dim_in 3072). Feeds the
// dumped feat_g_*
// + cam_g_39 to isolate head/cam from the backbone. SKIP (77) if artifacts
// absent.
#include <array>
#include <cstdlib>
#include <string>
...
int main() {
    ...
    if (!head.depth(feats, H, W, depth, conf)) {
        std::fprintf(stderr, "giant depth failed\n");
        return 1;
    }
    ...
    std::array<float, 9> K;
    if (!cp.pose(ct, H, W, pe, ext, K)) {
        std::fprintf(stderr, "giant pose failed\n");
        return 1;
    }
