...
int main() {
    ...
    const double QR_Q[9] = {-0.9186304243, 0.2313704865,  0.3202902456,
                            -0.2296576061, -0.9722910038, 0.0436759426,
                            0.3215206485,  -0.0334350414, 0.9463120894};
    const double QR_R[9] = {-2.1771541057, 0.2939617358, 0.2985548879,
    ...
    const double QL_Q[9] = {0.9164203548,  -0.3845934904, 0.1107320206,
                            0.3213748094,  0.8720541554,  0.3691067353,
                            -0.2385203663, -0.3026704433, 0.9227668382};
    const double QL_L[9] = {2.1604923708,  0.0,          0.0,
    ...
    ql_decomposition(A, Ql, Lq);
    {
        ...
        check(close(Lq[1], 0, 1e-12) && close(Lq[2], 0, 1e-12) &&
                      close(Lq[5], 0, 1e-12),
    ...
    {
        // M = V diag(1,5,9) V^T won't be hand-built; instead use a small
        // explicit symmetric matrix and verify M v = lambda v for the smallest
        // eigenpair.
        const int n = 4;
        double M[16] = {4, 1, 0, 2, 1, 3, 1, 0, 0, 1, 2, 1, 2, 0, 1, 5};
        double v[4];
        ...
        // smallest eigenvalue of this matrix is ~1.0856 (sanity: < all diag
        // mins won't hold, but it must be the minimum eigenvalue). Verify it's
        // <= every diagonal-derived bound by checking no other eigvec gives a
        // smaller Rayleigh quotient via random probes.
        check(lam < 2.0 && lam > 0.5,
              "jacobi smallest eigenvalue in expected range", lam, 0);
    }

    // ---- weighted homography recovery ----
    {
        // Known homography (det>0). Apply to source points -> dst, recover.
        const double Htrue[9] = {1.2,  0.1,  0.3,   -0.05, 0.9,
                                 -0.2, 0.02, -0.01, 1.0};
        double src[2 * 12], dst[2 * 12], w[12];
        const double pts[12][2] = {{0.1, 0.2},   {-0.3, 0.5},  {0.7, -0.4},
                                   {-0.6, -0.8}, {0.4, 0.9},   {-0.2, 0.1},
                                   {0.8, 0.3},   {-0.7, 0.6},  {0.3, -0.5},
                                   {-0.4, -0.2}, {0.55, 0.15}, {-0.15, -0.65}};
        for (int m = 0; m < 12; ++m) {
    ...
    if (g_fail) {
        std::fprintf(stderr, "test_linalg: %d FAILURES\n", g_fail);
        return 1;
    }
