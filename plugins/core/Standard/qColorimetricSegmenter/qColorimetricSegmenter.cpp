...
static void ShowDurationNow(const std::chrono::high_resolution_clock::time_point& startTime)
{
	...
	CVLog::Print("Time to execute: " + QString::number(duration_ms) + " milliseconds");
}
...
void ColorimetricSegmenter::filterRgb()
{
	...
	{
		m_app->dispToConsole("[ColorimetricSegmenter] No active 3D view", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	for (ccPointCloud* cloud : clouds)
	{
		if (cloud && cloud->hasColors())
		{
			...
			m_app->dispToConsole("[ColorimetricSegmenter] Cloud successfully filtered ! ", ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		}
	}
...
static ecvColor::Rgb ComputeAverageColor(const ccPointCloud& cloud, CVLib::ReferenceCloud* subset)
{
	...
	ecvColor::Rgb res(	static_cast<ColorCompType>(std::min(redSum   / count, static_cast<size_t>(ecvColor::MAX))),
						static_cast<ColorCompType>(std::min(greenSum / count, static_cast<size_t>(ecvColor::MAX))),
						static_cast<ColorCompType>(std::min(blueSum  / count, static_cast<size_t>(ecvColor::MAX))));
...
void ColorimetricSegmenter::filterRgbWithSegmentation()
{
	...
	for (ccPointCloud* cloud : clouds)
	{
		if (cloud->hasColors())
		{
			...
			//m_app->dispToConsole(QString("[ColorimetricSegmenter] regions %1").arg(regions->size()), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
			...
			for (Region& r : mergedRegions)
			{
					...
					)
				{
					...
					{
						m_app->dispToConsole("Not enough memory", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
						return;
					}
				}
			}
...
void ColorimetricSegmenter::HistogramClustering()
{
	...
	for (ccPointCloud* cloud : clouds)
	{
		if (cloud->hasColors())
		{
			...
			{
				ecvColor::Rgb averageColor = ComputeAverageColor(*histCloud, it->second);
...
static ccPointCloud* ComputeKmeansClustering(ccPointCloud* theCloud, unsigned K, int maxIterationCount)
{
	...
	if (K >= pointCount)
	{
		CVLog::Warning("Cloud %1 has less point than the expected number of classes.");
		return nullptr;
	}
...
void ColorimetricSegmenter::KmeansClustering()
{
	...
	for (ccPointCloud* cloud : clouds)
	{
		...
		if (!kcloud)
		{
			m_app->dispToConsole(QString("[ColorimetricSegmenter] Failed to cluster cloud %1").arg(cloud->getName()), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			continue;
		}
