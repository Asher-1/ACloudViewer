...
QString getPluginsWrappersPath()
{
    ...
    QString pluginPath = QString("%1/lib/plugins-python").arg(envPrefix);
#else
...
{
#ifdef PYCC_STAND_ALONE
    // We put the python wrapper in $venvPrefix/lib(64)/plugins-python
    py::module_ sys = py::module_::import("sys");
    const QString envPrefix = sys.attr("prefix").cast<QString>();
    const QString platlibdir = guessPlatlibdir();
    QString pluginPath = QString("%1/%2/plugins-python").arg(envPrefix, platlibdir);
    ...
    QString pluginPath = QString("%1/plugins-python").arg(theDir.absolutePath());
...
void load_pluginWrappers(py::module_ &m)
{
    ...
    pluginWrappers.doc() = "Module we wrap around some ACloudViewer plugins";
    ...
    while (iter.hasNext())
    {
        ...
        try
        {
            ...
            CVLog::Print(
                QString("[PythonRuntime] Plugin python wrapper found: %1").arg(pluginName));
        }
