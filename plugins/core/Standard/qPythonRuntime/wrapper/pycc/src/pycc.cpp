...
// Some plugins in ACloudViewer are written to always display some
// kind of dialogs.
//
...
#endif
    // Whe not built as stand alone, the QApplication is ACloudViewer
...
void define_pycc(py::module &m)
{
    ...
    // some ACloudViewer facilities we have to initialize ourselves
    // to have a more complete experience
    CVLog::RegisterInstance(new PyPrintLogger);
    ...
    {
        CVLog::Warning("Failed to import wrapped plugins: ", e.what());
    }
    ...
    {
        CVLog::Warning("Failed to import pycc_runtime content: ", e.what());
    }
#endif
}
