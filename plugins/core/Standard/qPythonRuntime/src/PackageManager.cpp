      ...
      m_shouldUseUserOption(false)
{
    ...
    // "C:\Programs\ACloudViewer\plugins\Python" and that requires admin rights to add/modify.
...
void PackageManager::refreshInstalledPackagesList()
{
    ...
    {
        CVLog::Warning("Failed to start python process");
        return;
    }
    loop.exec();
    ...
    const QtCompatRegExp regex(R"((\S*)(?:\s*)(\S*)(?:\s*)(\S?))");
    ...
    for (int i{2}; i < lines.size() - 1; ++i)
    {
        ...
        QRegularExpressionMatch match = regex.match(currentLine.toString());
        if (match.hasMatch())
        {
            for (int j = 1; j < 3; ++j)
            {
...
void PackageManager::handleInstallPackage()
{
    ...
    {
        CVLog::Error("Failed to run install commands", m_pythonProcess->error());
        CVLog::Warning(m_pythonProcess->errorString());
    }
    refreshInstalledPackagesList();
}
...
void PackageManager::handleUninstallPackage()
{
    ...
    for (const QTableWidgetItem *item : selectedItems)
    {
        ...
        if (m_pythonProcess->exitCode() != 0)
        {
            CVLog::Error("Failed to run uninstall commands", m_pythonProcess->error());
            CVLog::Warning(m_pythonProcess->errorString());
        }
    }
    refreshInstalledPackagesList();
