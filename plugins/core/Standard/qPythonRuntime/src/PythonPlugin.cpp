      ...
      m_actionLauncher(new PythonActionLauncher(&m_pluginManager, &m_interp))
{
    ...
    if (PythonConfig::IsInsideEnvironment())
    {
        ...
        plgPrint() << "ACloudViewer was loaded from within a " << config.type() << "env. "
                   << "Will try to use it";
    }
    else
    {
...
void PythonPlugin::setMainAppInterface(ecvMainAppInterface *app)
{
    LogPythonHome();
    LogPythonPath();
    ...
    {
        CVLog::Warning("[PythonRuntime] Failed to load autodiscovered custom python plugins: %s",
                       e.what());
    }
    ...
    {
        CVLog::Warning("[PythonRuntime] Failed to load custom python plugins : %s", e.what());
    }
