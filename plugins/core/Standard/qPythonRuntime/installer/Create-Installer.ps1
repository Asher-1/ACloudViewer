param(
    ...
    [String]$ACloudViewerInstallFolder,
...
$IsFolder = Test-Path -Path $ACloudViewerInstallFolder -PathType Container
if ($IsFolder -eq $False) {
    throw '$ACloudViewerInstallFolder does no point to a Folder'
}

# we need to find two DLLs
# A python3.dll
# A python3<minor version>.dll
$PythonDlls = Get-ChildItem -Path $ACloudViewerInstallFolder -Filter "python3*.dll"
...
$PythonEnvPrefix = Join-Path -Resolve -Path $ACloudViewerInstallFolder -ChildPath "plugins" -AdditionalChildPath "Python"
...
&heat `
    dir "$ACloudViewerInstallFolder\plugins\Python" `
    -scom `
    -frag `
    -srd `
    -sreg `
...
&candle `
    -arch x64 `
    -dSourceInstallPath="$ACloudViewerInstallFolder" `
    -dPythonSuffix="$Python3DllSuffix" `
    -dPLUGIN_STANDARD_QM3C2="$PLUGIN_STANDARD_QM3C2" `
    .\Installer.wxs `
    .\PythonEnvironment.wxs `

$OutputInstallerName = "PythonRuntime-Setup-$LocalizationName-Python$Python3DllSuffix-$EnvTypeName.msi"
...
&light `
    ...
    -b "$ACloudViewerInstallFolder\plugins\Python" `
    -b "$ACloudViewerInstallFolder\resources" `
    -b "$(Get-Location)" `
...
# Basically:
# it 'harvest' the "$ACloudViewerInstallFolder\plugins\Python" directory
