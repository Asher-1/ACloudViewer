...
bool qM3C2Process::Compute(const qM3C2Dialog& dlg, QString& errorMessage, ccPointCloud*& outputCloud, bool allowDialogs, QWidget* parentWidget/*=nullptr*/, ecvMainAppInterface* app/*=nullptr*/)
{
	errorMessage.clear();
	outputCloud = nullptr;
	...
	if (!s_M3C2Params.corePoints && samplingDist > 0)
	{
		...
		if (s_M3C2Params.corePoints)
		{
			...
			if (app)
			{
				app->dispToConsole(QString("[M3C2] Sub-sampled cloud has been saved ('%1')").arg(s_M3C2Params.corePoints->getName()), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	if (!error)
	{
		...
		if (app)
			app->dispToConsole(QString("[M3C2] Core points: %1").arg(s_M3C2Params.corePoints->size()), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	if (!error && s_M3C2Params.coreNormals && corePointsHaveBeenSubsampled)
	{
		...
		{
			app->dispToConsole("Failed to allocate memory for core points normals!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		}
	}
	...
	while (!error) //fake loop for easy break
	{
		...
		if (app)
			app->dispToConsole(QString("[M3C2] Initialization & normal computation: %1 s.").arg(initTime_ms / 1000.0, 0, 'f', 3), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		...
		if (dlg.exportStdDevInfoCheckBox->isChecked())
		{
			...
			{
				if (app)
					app->dispToConsole("Failed to allocate memory for cloud #1 std. dev. values!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			...
			{
				if (app)
					app->dispToConsole("Failed to allocate memory for cloud #2 std. dev. values!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		...
		if (dlg.exportDensityAtProjScaleCheckBox->isChecked())
		{
			...
			{
				if (app)
					app->dispToConsole("Failed to allocate memory for cloud #1 density values!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			...
			{
				if (app)
					app->dispToConsole("Failed to allocate memory for cloud #2 density values!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		...
		if (app)
			app->dispToConsole(QString("[M3C2] Working subdivision level (cloud #1): %1").arg(s_M3C2Params.level1), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		...
		if (app)
			app->dispToConsole(QString("[M3C2] Working subdivision level (cloud #2): %1").arg(s_M3C2Params.level2), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		...
		{
			if (app)
				app->dispToConsole("Failed to allocate memory for exporting normals!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		...
		else
		{
			...
			//we display init. timing only if no error occurred!
			if (app)
				app->dispToConsole(QString("[M3C2] Distances computation: %1 s.").arg(static_cast<double>(distTime_ms) / 1000.0, 0, 'f', 3), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		}
