                                                        ...
                                                        cloudViewer::GenericProgressCallback* progressCb/*=nullptr*/)
{
	...
	{
		assert(false);
		CVLog::Warning("[qM3C2Tools::UpdateNormalOrientationsWithCloud] Cloud/normals set mismatch!");
		return false;
	}
	...
	if (!fastMode)
	{
		...
		while (!hasBestNormLevel || !hasBestProjLevel)
		{
			...
			if (level == 6 && (!hasBestNormLevel || !hasBestProjLevel))
			{
				...
				hasBestNormLevel = true;
				if (app)
					app->dispToConsole(QString("[M3C2::auto] We failed to converge for the best projection level, so we will stop here!"), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				break;
			}
