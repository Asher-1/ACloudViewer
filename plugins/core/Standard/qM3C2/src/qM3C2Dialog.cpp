...
qM3C2Dialog::qM3C2Dialog(ccPointCloud* cloud1, ccPointCloud* cloud2, ecvMainAppInterface* app)
...
void qM3C2Dialog::loadParamsFrom(const QSettings& settings)
{
	...
	case qM3C2Normals::USE_CORE_POINTS_NORMALS:
	{
		...
		if (!found)
		{
			CVLog::Warning("Can't restore the previous normal computation method (cloud #1 or core points has no normals)");
		}
	}
	break;
...
void qM3C2Dialog::loadParamsFromFile()
{
	...
	{
		QSettings settings("qM3C2");
		QString currentPath = settings.value("currentPath", ecvFileUtils::defaultDocPath()).toString();
