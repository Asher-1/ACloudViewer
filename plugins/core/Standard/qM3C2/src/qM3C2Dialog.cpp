...
qM3C2Dialog::qM3C2Dialog(ccPointCloud* cloud1,
                         ccPointCloud* cloud2,
    ...
    connect(showCloud1CheckBox, SIGNAL(toggled(bool)), this,
            SLOT(setCloud1Visibility(bool)));
    connect(showCloud2CheckBox, SIGNAL(toggled(bool)), this,
            SLOT(setCloud2Visibility(bool)));

    connect(loadParamsToolButton, SIGNAL(clicked()), this,
            SLOT(loadParamsFromFile()));
    connect(saveParamsToolButton, SIGNAL(clicked()), this,
            SLOT(saveParamsToFile()));
    connect(swapCloudsToolButton, SIGNAL(clicked()), this, SLOT(swapClouds()));
    connect(guessParamsPushButton, SIGNAL(clicked()), this,
            SLOT(guessParamsSlow()));

    connect(projDestComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(projDestIndexChanged(int)));

    connect(cpOtherCloudComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(updateNormalComboBox()));
    connect(normalSourceComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(onUpdateNormalComboBoxChanged(int)));
    connect(cpUseCloud1RadioButton, SIGNAL(toggled(bool)), this,
            SLOT(updateNormalComboBox()));
    connect(cpSubsampleRadioButton, SIGNAL(toggled(bool)), this,
            SLOT(updateNormalComboBox()));
    connect(cpUseOtherCloudRadioButton, SIGNAL(toggled(bool)), this,
            SLOT(updateNormalComboBox()));
    ...
    if (m_app) {
        ...
        for (size_t i = 0; i < clouds.size(); ++i) {
            if (clouds[i]->isA(CV_TYPES::POINT_CLOUD))  // as filterChildren
                                                        // only test 'isKindOf'
            {
                cpOtherCloudComboBox->addItem(
                        GetEntityName(clouds[i]),
                        QVariant(clouds[i]->getUniqueID()));
                normOriCloudComboBox->addItem(
                        GetEntityName(clouds[i]),
                        QVariant(clouds[i]->getUniqueID()));
            }
        }
...
void qM3C2Dialog::setupPrecisionMapsTab() {
    ...
    if (m_cloud1->hasScalarFields() && m_cloud2->hasScalarFields()) {
        ...
        bool auto1 = PopulatePMFields(c1SxComboBox, c1SyComboBox, c1SzComboBox,
                                      *m_cloud1);
        bool auto2 = PopulatePMFields(c2SxComboBox, c2SyComboBox, c2SzComboBox,
                                      *m_cloud2);
...
void qM3C2Dialog::onUpdateNormalComboBoxChanged(int) {
    int selectedItem = normalSourceComboBox->currentIndex() >= 0
                               ? normalSourceComboBox->currentData().toInt()
                               : -1;
    switch (selectedItem) {
...
void qM3C2Dialog::updateNormalComboBox() {
    int previouslySelectedItem =
            normalSourceComboBox->currentIndex() >= 0
                    ? normalSourceComboBox->currentData().toInt()
                    : -1;
    int lastIndex = -1;
    normalSourceComboBox->clear();
    normalSourceComboBox->addItem("Compute normals (on core points)",
                                  QVariant(qM3C2Normals::DEFAULT_MODE));
    ++lastIndex;
    ...
    if (m_cloud1 && m_cloud1->hasNormals()) {
        normalSourceComboBox->addItem(
                "Use cloud #1 normals",
                QVariant(qM3C2Normals::USE_CLOUD1_NORMALS));
        ++lastIndex;
        if (previouslySelectedItem == qM3C2Normals::USE_CLOUD1_NORMALS ||
    ...
    if (cpUseOtherCloudRadioButton->isChecked()) {
        ...
        ccPointCloud* otherCloud =
                GetCloudFromCombo(cpOtherCloudComboBox, m_app->dbRootObject());
        if (otherCloud && otherCloud->hasNormals()) {
            normalSourceComboBox->addItem(
                    "Use core points normals",
                    QVariant(qM3C2Normals::USE_CORE_POINTS_NORMALS));
            ++lastIndex;
            if (previouslySelectedItem ==
                        qM3C2Normals::USE_CORE_POINTS_NORMALS ||
                previouslySelectedItem < 0) {
                normalSourceComboBox->setCurrentIndex(lastIndex);
...
void qM3C2Dialog::projDestIndexChanged(int index) {
    useOriginalCloudCheckBox->setEnabled(getExportOption() ==
                                         PROJECT_ON_CORE_POINTS);
}

bool qM3C2Dialog::keepOriginalCloud() const {
    return useOriginalCloudCheckBox->isEnabled() &&
           useOriginalCloudCheckBox->isChecked();
}
...
unsigned qM3C2Dialog::getMinPointsForStats(unsigned defaultValue /*=5*/) const {
    return useMinPoints4StatCheckBox->isChecked()
                   ? static_cast<unsigned>(
                             std::max(0, minPoints4StatSpinBox->value()))
                   : defaultValue;
}
...
void qM3C2Dialog::loadParamsFrom(const QSettings& settings) {
    // read out parameters
    double normalScale =
            settings.value("NormalScale", normalScaleDoubleSpinBox->value())
                    .toDouble();
    int normModeInt =
            settings.value("NormalMode",
                           static_cast<int>(getNormalsComputationMode()))
                    .toInt();
    double normMinScale =
            settings.value("NormalMinScale", minScaleDoubleSpinBox->value())
                    .toDouble();
    double normStep =
            settings.value("NormalStep", stepScaleDoubleSpinBox->value())
                    .toDouble();
    double normMaxScale =
            settings.value("NormalMaxScale", maxScaleDoubleSpinBox->value())
                    .toDouble();
    bool normUseCorePoints =
            settings.value("NormalUseCorePoints",
                           normUseCorePointsCheckBox->isChecked())
                    .toBool();
    int normPreferredOri =
            settings.value("NormalPreferedOri",
                           normOriPreferredComboBox->currentIndex())
                    .toInt();

    double seachScale =
            settings.value("SearchScale", cylDiameterDoubleSpinBox->value())
                    .toDouble();
    double searchDepth =
            settings.value("SearchDepth", cylHalfHeightDoubleSpinBox->value())
                    .toDouble();

    double subsampleRadius = settings.value("SubsampleRadius",
                                            cpSubsamplingDoubleSpinBox->value())
                                     .toDouble();
    bool subsampleEnabled = settings.value("SubsampleEnabled",
                                           cpSubsampleRadioButton->isChecked())
                                    .toBool();

    double registrationError =
            settings.value("RegistrationError", rmsDoubleSpinBox->value())
                    .toDouble();
    bool registrationErrorEnabled =
            settings.value("RegistrationErrorEnabled", rmsCheckBox->isChecked())
                    .toBool();

    bool useSinglePass4Depth =
            settings.value("UseSinglePass4Depth",
                           useSinglePass4DepthCheckBox->isChecked())
                    .toBool();
    bool positiveSearchOnly =
            settings.value("PositiveSearchOnly",
                           positiveSearchOnlyCheckBox->isChecked())
                    .toBool();
    bool useMedian = settings.value("UseMedian", useMedianCheckBox->isChecked())
                             .toBool();

    bool useMinPoints4Stat =
            settings.value("UseMinPoints4Stat",
                           useMinPoints4StatCheckBox->isChecked())
                    .toBool();
    int minPoints4Stat =
            settings.value("MinPoints4Stat", minPoints4StatSpinBox->value())
                    .toInt();

    int projDestIndex =
            settings.value("ProjDestIndex", projDestComboBox->currentIndex())
                    .toInt();
    bool useOriginalCloud =
            settings.value("UseOriginalCloud",
                           useOriginalCloudCheckBox->isChecked())
                    .toBool();

    bool exportStdDevInfo =
            settings.value("ExportStdDevInfo",
                           exportStdDevInfoCheckBox->isChecked())
                    .toBool();
    bool exportDensityAtProjScale =
            settings.value("ExportDensityAtProjScale",
                           exportDensityAtProjScaleCheckBox->isChecked())
                    .toBool();

    int maxThreadCount =
            settings.value("MaxThreadCount", maxThreadCountSpinBox->maximum())
                    .toInt();

    bool usePrecisionMaps = settings.value("UsePrecisionMaps",
                                           precisionMapsGroupBox->isChecked())
                                    .toBool();
    double pm1Scale = settings.value("PM1Scale", pm1ScaleDoubleSpinBox->value())
                              .toDouble();
    double pm2Scale = settings.value("PM2Scale", pm2ScaleDoubleSpinBox->value())
                              .toDouble();
    ...
    switch (normModeInt) {
        ...
        case qM3C2Normals::USE_CORE_POINTS_NORMALS: {
            ...
            if (!found) {
                CVLog::Warning(
                        "Can't restore the previous normal computation method "
                        "(cloud #1 or core points has no normals)");
            }
        } break;
...
void qM3C2Dialog::saveParamsTo(QSettings& settings) {
    ...
    settings.setValue("NormalUseCorePoints",
                      normUseCorePointsCheckBox->isChecked());
    settings.setValue("NormalPreferedOri",
                      normOriPreferredComboBox->currentIndex());
    ...
    settings.setValue("UseSinglePass4Depth",
                      useSinglePass4DepthCheckBox->isChecked());
    settings.setValue("PositiveSearchOnly",
                      positiveSearchOnlyCheckBox->isChecked());
    settings.setValue("UseMedian", useMedianCheckBox->isChecked());

    settings.setValue("UseMinPoints4Stat",
                      useMinPoints4StatCheckBox->isChecked());
    ...
    settings.setValue("UseOriginalCloud",
                      useOriginalCloudCheckBox->isChecked());

    settings.setValue("ExportStdDevInfo",
                      exportStdDevInfoCheckBox->isChecked());
    settings.setValue("ExportDensityAtProjScale",
                      exportDensityAtProjScaleCheckBox->isChecked());
...
bool qM3C2Dialog::loadParamsFromFile(QString filename) {
    ...
    if (!fileSettings.contains("M3C2VER")) {
        QMessageBox::critical(this, "Invalid file",
                              "File doesn't seem to be a valid M3C2 parameters "
                              "file ('M3C2VER' not found)!");
        return false;
    }
...
void qM3C2Dialog::guessParams(bool fastMode /*=false*/) {
    if (!m_cloud1 || !m_cloud2) return;

    // see article: ideal = 30 while default = 5
    unsigned minPoints4Stats = getMinPointsForStats() * 6;

    qM3C2Tools::GuessedParams params;
    if (qM3C2Tools::GuessBestParams(m_cloud1, m_cloud2, minPoints4Stats, params,
                                    fastMode, m_app)) {
        ...
        if (params.preferredDimension >= 0 && params.preferredDimension < 3) {
            //*2 because for each dimension, the preferred
            // orientation is either + or -
            normOriPreferredComboBox->setCurrentIndex(
                    params.preferredDimension * 2);
        }
