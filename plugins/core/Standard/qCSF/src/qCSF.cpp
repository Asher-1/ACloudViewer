...
void qCSF::doAction() {
    ...
    if (!m_app->haveOneSelection()) {
        m_app->dispToConsole(tr("Select only one cloud!"),
    ...
    if (!ent || !ent->isA(CV_TYPES::POINT_CLOUD)) {
        m_app->dispToConsole(tr("Select a real point cloud!"),
                             ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }
    ...
    ccMesh* clothMesh = 0;
    if (!csf.do_filtering(groundIndexes, offGroundIndexes, ExportClothMesh,
                          clothMesh, m_app)) {
        m_app->dispToConsole(tr("Process failed"),
                             ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }

    m_app->dispToConsole(
            tr("[CSF] %1% of points classified as ground points")
                    .arg((groundIndexes.size() * 100.0) / count, 0, 'f', 2),
            ecvMainAppInterface::STD_CONSOLE_MESSAGE);
    m_app->dispToConsole(
            tr("[CSF] Timing: %1 s.").arg(timer.elapsed() / 1000.0, 0, 'f', 1),
            ecvMainAppInterface::STD_CONSOLE_MESSAGE);
    ...
    if (!offgroundpoint) {
        m_app->dispToConsole(tr("Failed to extract the off-ground subset (not "
                                "enough memory)"),
                             ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
        if (!groundpoint) {
            // nothing to do!
...
void qCSF::registerCommands(ccCommandLineInterface* cmd) {
    ...
    cmd->registerCommand(
            ccCommandLineInterface::Command::Shared(new CommandCSF));
}
