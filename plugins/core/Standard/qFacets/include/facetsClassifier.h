...
public:
    ...
    static PointCoordinateType CommputeHDistBetweenFacets(const ccFacet* f1,
                                                          const ccFacet* f2) {
        CCVector3 AB = f1->getCenter() - f2->getCenter();
        return std::min(fabs(AB.dot(f1->getNormal())),
                        fabs(AB.dot(f2->getNormal())));
    }

    //! Generates a given sub-family color
    static void GenerateSubfamilyColor(ecvColor::Rgb& col,
                                       double dip,
                                       double dipDir,
        ...
        if (subFamilyCount > 1) {
            ...
            // V = 0.5 + 0.5 *
            // static_cast<double>(subFamilyIndex-1)/static_cast<double>(subFamilyCount-1);
        }

        col = ecvColor::Convert::hsl2rgb(H, S, L);
        if (darkCol) {
            *darkCol = ecvColor::Convert::hsl2rgb(H, S, c_darkColorRatio);
        }
    }
    ...
    static void GetFamilyIndexes(ccFacet* facet,
        ...
        if (iDipDir == ddSteps) iDipDir--;
    }

    static inline QString GetFamilyName(double dip,
                                        double dipDir,
                                        double angularStep_deg) {
        // return QString("(%1+/-%2) /
        // (%3+/-%4)").arg(dip).arg(angularStep_deg/2).arg(dipDir).arg(angularStep_deg/2);
    ...
    static bool ProcessFamiliy(ccHObject* parent,
        ...
        // create family group
        ccHObject* familyGroup = new ccHObject(
                QString("F%1_").arg(familyIndex, 2, 10, QChar('0')) +
                familyName);
        if (parent) parent->addChild(familyGroup);

        // tag all facets consequently
        for (FacetSet::iterator it = family.begin(); it != family.end(); ++it) {
            (*it)->setMetaData(s_OriFamilyKey,
                               QVariant(static_cast<uint>(familyIndex)));
        ...
        if (count == 1 || count == 2) {
            ...
            family[0]->setMetaData(
                    s_OriSubFamilyKey,
                    QVariant(static_cast<uint>(++subFamilyIndex)));
            ccHObject* subFamilyGroup =
                    new ccHObject(GetSubFamilyName(subFamilyIndex));
            ...
            if (count == 2) {
                PointCoordinateType dist =
                        CommputeHDistBetweenFacets(family[0], family[1]);
                if (dist <= maxDist) {
                    // same sub-family as #1
                    family[1]->setMetaData(
                            s_OriSubFamilyKey,
                            QVariant(static_cast<uint>(subFamilyIndex)));
            ...
            if (distMat.isValid()) {
                for (unsigned it1 = 0; it1 + 1 != count; ++it1) {
                    for (unsigned it2 = it1 + 1; it2 != count; ++it2) {
                        PointCoordinateType dist = CommputeHDistBetweenFacets(
                                family.at(it1), family.at(it2));
                ...
                while (true) {
                    ...
                    for (unsigned i = 0; i + 1 < count; ++i) {
                        if (distMat.getValue(i, i) ==
                            0)  // not already assigned
                        {
                            for (unsigned j = i + 1; j < count; ++j) {
                                if (distMat.getValue(j, j) ==
                                    0)  // not already assigned
                                {
                                    if (bestCoupleDist < 0 ||
                                        bestCoupleDist >
                                                distMat.getValue(i, j)) {
                                        bestCouple.first = i;
                                        bestCouple.second = j;
                    ...
                    while (true) {
                        ...
                        for (unsigned i = 0; i < count; ++i) {
                                ...
                                0)  // not already assigned
                            {
                                ...
                                for (unsigned j = 0; j < subFamily.size();
                                     ++j) {
                                    const PointCoordinateType& dist =
                                            distMat.getValue(i, subFamily[j]);
                                    if (dist < maxDist) {
                                        // still elligible?
                    ...
                    // we can set the class for all the sub-family elements
                    {
                        ...
                        for (unsigned j = 0; j < subFamily.size(); ++j) {
                            ...
                            facet->setMetaData(s_OriSubFamilyKey,
                                               QVariant(static_cast<uint>(
                                                       subFamilyIndex)));
                ...
                // don't forget to set the class for the remaining isolated
                // elements
                {
                    for (unsigned i = 0; i < count; ++i) {
                        if (distMat.getValue(i, i) ==
                            0)  // not already assigned
                        {
                            ...
                            facet->setMetaData(s_OriSubFamilyKey,
                                               QVariant(static_cast<uint>(
                                                       ++subFamilyIndex)));
                            ccHObject* subFamilyGroup = new ccHObject(
                                    GetSubFamilyName(subFamilyIndex));
                            familyGroup->addChild(subFamilyGroup);
                            subFamilyGroup->addChild(facet);
                        }
                    }
        ...
        if (subFamilyCount) {
            ...
            for (unsigned i = 0; i < count; ++i) {
                ...
                GenerateSubfamilyColor(col, dip, dipDir, subFamilyIndex,
                                       subFamilyCount, &darkCol);
                facet->setColor(col);
                if (facet->getContour()) {
    ...
    static bool ByOrientation(ccHObject* facetGroup,
                              double angularStep_deg,
                              double maxDist) {
        ...
        if (facetGroup)
            facetGroup->filterChildren(facets, true, CV_TYPES::FACET);
        ...
        if (facetCount == 1) {
            ...
            GetFamilyIndexes(facet, dSteps, ddSteps, angularStep_deg, iDip,
                             iDipDir);

            error = !ProcessFamiliy(facetGroup, family, 1, iDip, iDipDir,
                                    angularStep_deg, maxDist);
        } else {
            ...
            if (!grid) {
                ...
                QProgressDialog pDlg("Families classification", QString(), 0,
                                     static_cast<int>(facetCount));
                pDlg.show();
                ...
                for (size_t i = 0; i < facetCount; ++i) {
                    ...
                    unsigned iDip = 0, iDipDir = 0;
                    GetFamilyIndexes(facet, dSteps, ddSteps, angularStep_deg,
                                     iDip, iDipDir);
                ...
                if (!error) {
                    ...
                    QProgressDialog pDlg("Sub-families classification",
                                         QString(), 0,
                                         static_cast<int>(setCount));
                    pDlg.show();
                    QApplication::processEvents();
                    ...
                    for (unsigned j = 0; j < dSteps; ++j) {
                        for (unsigned i = 0; i < ddSteps; ++i, ++set) {
                            // new family?
                            if (*set) {
                                error = !ProcessFamiliy(
                                        facetGroup, **set, ++familyIndex, j, i,
                                        angularStep_deg, maxDist);
                                if (error) {
                                    // stop the process
                                    // i = ddSteps;
        ...
        // check that no parent-less facets remain!
        {
            for (size_t i = 0; i < facetCount; ++i) {
                ...
                if (!facet->getParent()) facetGroup->addChild(facet);
            }
        }
