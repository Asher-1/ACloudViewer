...
public:
    //! Default constructor
    FacetDensityGrid()
        : grid(0), rSteps(0), ddSteps(0), step_deg(0), step_R(0) {
...
StereogramWidget::StereogramWidget(QWidget* parent)
    : QLabel("", parent),
      ...
      m_densityColorScaleSteps(
              ccColorScale::MAX_STEPS < 256
                      ? ccColorScale::MAX_STEPS
                      : 256)  // DGM: we can't pass a constant initializer
                              // (MAX_STEPS) by reference
      ,
      m_ticksFreq(3),
      m_showHSVRing(false),
...
StereogramWidget::~StereogramWidget() {
    if (m_densityGrid) delete m_densityGrid;
}
...
bool StereogramWidget::init(double angularStep_deg,
    ...
    if (entity->isA(CV_TYPES::HIERARCHY_OBJECT)) {
        ...
        {
            entity->filterChildren(planes, true, CV_TYPES::PLANE);
            count = planes.size();
        }
    }
    // or a cloud?
        ...
        if (cloud->hasNormals()) count = cloud->size();
    }
    ...
    // densityGrid->dSteps = static_cast<unsigned>(ceil(90.0 /
    // densityGrid->step_deg)); R steps (R in [0,1])
    densityGrid->rSteps =
            static_cast<unsigned>(ceil(1.0 / densityGrid->step_R));
    // dip direction steps (dip dir. in [0,360])
    densityGrid->ddSteps =
            static_cast<unsigned>(ceil(360.0 / densityGrid->step_deg));
    ...
    if (densityGrid->grid) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            // unsigned iDip =
            // static_cast<unsigned>(floor(static_cast<double>(dip)/densityGrid->step_deg));
            // if (iDip == densityGrid->dSteps)
            //	iDip--;
            unsigned iDipDir = static_cast<unsigned>(
                    floor(static_cast<double>(dipDir) / densityGrid->step_deg));
            if (iDipDir == densityGrid->ddSteps) iDipDir--;

            double dip_rad = cloudViewer::DegreesToRadians(dip);
            double R = sin(dip_rad) / (1.0 + cos(dip_rad));

            unsigned iR = static_cast<unsigned>(
                    floor(static_cast<double>(R) / densityGrid->step_R));
            if (iR == densityGrid->rSteps) iR--;
        ...
        // compute min and max density
        {
            ...
            // std::minmax_element(densityGrid->grid,densityGrid->grid+cellCount);
    ...
    if (m_trackMouseClick && m_radius > 0) {
        if (e->button() == Qt::LeftButton) {
            ...
            if (squareDistToCenter <= m_radius * m_radius) {
                ...
                m_clickDip_deg = std::min(
                        90.0,
                        90.0 * sqrt(static_cast<double>(squareDistToCenter)) /
                                static_cast<double>(m_radius));
                m_clickDipDir_deg = cloudViewer::RadiansToDegrees(
                        atan2(static_cast<double>(AB.y()),
                              static_cast<double>(AB.x())));
                if (m_clickDipDir_deg < 0) m_clickDipDir_deg += 360.0;
                m_clickDipDir_deg +=
                        90.0;  // stereogram starts at 12 o'clock (not 3)
                if (m_clickDipDir_deg >= 360.0) m_clickDipDir_deg -= 360.0;
...
void StereogramWidget::enableMouseTracking(bool state,
    ...
    if (state) {
        ...
        // to give to the parent dialog some feedback about the current marker
        // position!
        emit pointClicked(m_clickDip_deg, m_clickDipDir_deg);
    }
}
...
void StereogramWidget::paintEvent(QPaintEvent* event) {
    ...
    if (m_showHSVRing) {
        ...
        // TODO
        if (hsvThickness > 0) {
            QRect rectangle(center.x() - diameter / 2 + 1,
                            center.y() - diameter / 2 + 1, diameter - 2,
                            diameter - 2);
            int angle_span = static_cast<int>(m_angularStep_deg *
                                              16.0);  // see QPainter::drawPie
            QBrush brush;
            brush.setStyle(Qt::SolidPattern);
            painter.setPen(Qt::NoPen);

            // dip direction steps (dip dir. in [0,360])
            unsigned ddSteps = static_cast<unsigned>(
                    ceil(360.0 / std::max(m_angularStep_deg, 1.0)));
            for (unsigned j = 0; j < ddSteps; ++j) {
                ...
                FacetsClassifier::GenerateSubfamilyColor(
                        col, 90.0, dipDir_deg + 0.5 * m_angularStep_deg, 0, 1);
                brush.setColor(QColor(static_cast<int>(col.r),
                                      static_cast<int>(col.g),
                                      static_cast<int>(col.b), 255));
                painter.setBrush(brush);

                int angle_start = static_cast<int>(
                        (360.0 - dipDir_deg - m_angularStep_deg + 90.0) *
                        16.0);  // see QPainter::drawPie
    ...
    if (m_angularStep_deg > 0) {
        ...
        // dip direction steps (dip dir. in [0,360])
        unsigned ddSteps =
                static_cast<unsigned>(ceil(360.0 / m_angularStep_deg));
        ...
        for (unsigned i = 1; i < dSteps; ++i) {
            ...
            if (dip_deg < 90.0) {
                ...
                if (R > 1) painter.drawEllipse(center, R - 1, R - 1);
            }
        }
        ...
        for (unsigned j = 1; j <= ddSteps * ticksFreq; ++j) {
            ...
            if (dipDir_deg < 360.0) {
                QPoint X(
                        static_cast<int>(
                                sin(cloudViewer::DegreesToRadians(dipDir_deg)) *
                                radius),
                        -static_cast<int>(
                                cos(cloudViewer::DegreesToRadians(dipDir_deg)) *
                                radius));
    ...
    // draw density map
    if (m_densityGrid && m_densityColorScale && m_densityGrid->grid &&
        m_densityGrid->minMaxDensity[1] != 0) {
        ...
        for (unsigned j = 0; j < m_densityGrid->ddSteps; ++j) {
            double dipDir0_rad =
                    (j)*cloudViewer::DegreesToRadians(m_densityGrid->step_deg);
            double dipDir1_rad = (j + 1) * cloudViewer::DegreesToRadians(
                                                   m_densityGrid->step_deg);
            ...
            for (unsigned i = 0; i < m_densityGrid->rSteps; ++i, ++d) {
                if (*d != 0) {
                    ...
                    const ecvColor::Rgb* col =
                            m_densityColorScale->getColorByRelativePos(
                                    relPos, m_densityColorScaleSteps);
                    brush.setColor(QColor(static_cast<int>(col->r),
                                          static_cast<int>(col->g),
                                          static_cast<int>(col->b), 255));
                    painter.setBrush(brush);
                    ...
                    poly.setPoint(
                            0,
                            center + QPoint(static_cast<int>(sin_dipDir0 * R0),
                                            -static_cast<int>(cos_dipDir0 *
                                                              R0)));
                    poly.setPoint(
                            1,
                            center + QPoint(static_cast<int>(sin_dipDir0 * R1),
                                            -static_cast<int>(cos_dipDir0 *
                                                              R1)));
                    poly.setPoint(
                            2,
                            center + QPoint(static_cast<int>(sin_dipDir1 * R1),
                                            -static_cast<int>(cos_dipDir1 *
                                                              R1)));
                    poly.setPoint(
                            3,
                            center + QPoint(static_cast<int>(sin_dipDir1 * R0),
                                            -static_cast<int>(cos_dipDir1 *
                                                              R0)));
    ...
    if (m_meanDipDir_deg >= 0) {
        ...
        painter.setPen(pen);
        // draw main direction
        QPoint X(static_cast<int>(
                         sin(cloudViewer::DegreesToRadians(m_meanDipDir_deg)) *
                         radius),
                 -static_cast<int>(
                         cos(cloudViewer::DegreesToRadians(m_meanDipDir_deg)) *
                         radius));
    ...
    if (m_trackMouseClick) {
        ...
        double R0 =
                radius *
                (std::max(0.0, m_clickDip_deg - m_clickDipSpan_deg / 2) / 90.0);
        double R1 = radius *
                    (std::min(90.0, m_clickDip_deg + m_clickDipSpan_deg / 2) /
                     90.0);

        // draw radial limits
        {
            QPoint X0(static_cast<int>(sin(cloudViewer::DegreesToRadians(
                                               m_clickDipDir_deg -
                                               m_clickDipDirSpan_deg / 2)) *
                                       R0),
                      -static_cast<int>(cos(cloudViewer::DegreesToRadians(
                                                m_clickDipDir_deg -
                                                m_clickDipDirSpan_deg / 2)) *
                                        R0));
            QPoint X1(static_cast<int>(sin(cloudViewer::DegreesToRadians(
                                               m_clickDipDir_deg -
                                               m_clickDipDirSpan_deg / 2)) *
                                       R1),
                      -static_cast<int>(cos(cloudViewer::DegreesToRadians(
        ...
        {
            int angle_start =
                    static_cast<int>((360.0 - m_clickDipDir_deg -
                                      m_clickDipDirSpan_deg / 2 + 90.0) *
                                     16.0);  // see QPainter::drawPie
            int angle_span = static_cast<int>(m_clickDipDirSpan_deg *
                                              16.0);  // see QPainter::drawPie

            QRectF rect0(center.x() - R0, center.y() - R0, 2 * R0, 2 * R0);
            painter.drawArc(rect0, angle_start, angle_span);

            QRectF rect1(center.x() - R1, center.y() - R1, 2 * R1, 2 * R1);
            painter.drawArc(rect1, angle_start, angle_span);
        }
    }
}

StereogramDialog::StereogramDialog(ecvMainAppInterface* app /*=0*/)
    ...
    {
        if (!frame->layout()) frame->setLayout(new QHBoxLayout());
    ...
    if (m_app) {
        ...
        m_colorScaleSelector = new ccColorScaleSelector(
                csManager, this,
                QString::fromUtf8(":/CC/plugin/qFacets/images/gearIcon.png"));
        m_colorScaleSelector->init();
        ccColorScale::Shared scale =
                csManager->getDefaultScale(ccColorScalesManager::BGYR);
        if (scale) {
        ...
        colorRampGroupBox->layout()->addItem(new QSpacerItem(
        ...
        m_classifWidget->setDensityColorScale(
                ccColorScalesManager::GetDefaultScale());
    }

    connect(colorScaleStepsSpinBox, SIGNAL(valueChanged(int)), this,
            SLOT(onDensityColorStepsChanged(int)));
    connect(ticksFreqSpinBox, SIGNAL(valueChanged(int)), this,
            SLOT(onTicksFreqChanged(int)));
    connect(showHSVColorsCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(onHSVColorsToggled(bool)));

    // interactive filtering mechanism
    connect(filterFacetsGroupBox, SIGNAL(toggled(bool)), this,
            SLOT(onFilterEnabled(bool)));
    connect(dipSpanDoubleSpinBox, SIGNAL(valueChanged(double)), this,
            SLOT(onFilterSizeChanged(double)));
    connect(dipDirSpanDoubleSpinBox, SIGNAL(valueChanged(double)), this,
            SLOT(onFilterSizeChanged(double)));
    connect(dipDoubleSpinBox, SIGNAL(valueChanged(double)), this,
            SLOT(onFilterCenterChanged(double)));
    connect(dipDirDoubleSpinBox, SIGNAL(valueChanged(double)), this,
            SLOT(onFilterCenterChanged(double)));
    connect(m_classifWidget, SIGNAL(pointClicked(double, double)), this,
            SLOT(onPointClicked(double, double)));
    connect(exportPushButton, SIGNAL(clicked()), this,
            SLOT(exportCurrentSelection()));
}

bool StereogramDialog::init(double angularStep_deg,
                            ccHObject* facetGroup,
                            double resolution_deg /*=2.0*/) {
    if (!m_classifWidget) return false;
    ...
    meanDirLabel->setText(
            QString("[Mean] ") +
            ccNormalVectors::ConvertDipAndDipDirToString(
                    static_cast<PointCoordinateType>(meanDip_deg),
...
void StereogramDialog::onFilterEnabled(bool state) {
    if (m_classifWidget) {
        m_classifWidget->enableMouseTracking(state,
                                             dipSpanDoubleSpinBox->value(),
                                             dipDirSpanDoubleSpinBox->value());
...
void StereogramDialog::onFilterSizeChanged(double) {
    if (m_classifWidget) {
        m_classifWidget->enableMouseTracking(filterFacetsGroupBox->isChecked(),
                                             dipSpanDoubleSpinBox->value(),
                                             dipDirSpanDoubleSpinBox->value());
...
void StereogramDialog::onFilterCenterChanged(double) {
    if (m_classifWidget) {
        m_classifWidget->setTrackedCenter(dipDoubleSpinBox->value(),
                                          dipDirDoubleSpinBox->value());
        m_classifWidget->update();
...
void StereogramDialog::onPointClicked(double dip_deg, double dipDir_deg) {
    ...
    if (filterFacetsGroupBox->isChecked()) {
        ...
        if (m_classifWidget) m_classifWidget->update();
...
void StereogramDialog::updateFacetsFilter(bool enable) {
    ...
    PointCoordinateType dipFilter =
            static_cast<PointCoordinateType>(dipDoubleSpinBox->value());
    PointCoordinateType dipDirFilter =
            static_cast<PointCoordinateType>(dipDirDoubleSpinBox->value());

    PointCoordinateType halfDipSpan =
            static_cast<PointCoordinateType>(dipSpanDoubleSpinBox->value()) / 2;
    PointCoordinateType halfDipDirSpan =
    ...
    if (entity->isA(CV_TYPES::HIERARCHY_OBJECT)) {
        ...
        for (size_t i = 0; i < facets.size(); ++i) {
            ...
            if (enable) {
                ...
                visible =
                        ((dDip <= halfDipSpan || dDip >= 360.0 - halfDipSpan) &&
                         (dDipDir <= halfDipDirSpan ||
                          dDipDir >= 360.0 - halfDipDirSpan));
            }
            ...
            if (polyline) {
                ...
                polyline->setWidth(
                        static_cast<PointCoordinateType>(visible ? 2 : 1));
            }
        }
    }
    // or a cloud?
...
void StereogramDialog::exportCurrentSelection() {
    ...
    QString selectionSuffix = QString(" [dip=(%1 - %2)][dipDir=(%3 - %4)]")
                                      .arg(dipFilter - halfDipSpan)
                                      .arg(dipFilter + halfDipSpan)
                                      .arg(dipDirFilter - halfDipDirSpan)
                                      .arg(dipDirFilter + halfDipDirSpan);
    ...
    if (entity->isA(CV_TYPES::HIERARCHY_OBJECT)) {
        ...
        if (facets.empty()) return;
        ccHObject* newGroup =
                new ccHObject(entity->getName() + selectionSuffix);
        ...
        for (size_t i = 0; i < facets.size(); ++i) {
            ...
            if (visible) {
                ...
                if (facetClone) newGroup->addChild(facetClone);
            }
        }
    ...
    // or a cloud?
        ...
        ccPointCloud* cloud = static_cast<ccPointCloud*>(entity);
        if (!cloud->isVisibilityTableInstantiated()) return;

        ccGenericPointCloud* subset =
...
void StereogramDialog::onDensityColorStepsChanged(int value) {
    if (m_classifWidget && value >= ccColorScale::MIN_STEPS) {
        m_classifWidget->setDensityColorScaleSteps(
...
void StereogramDialog::colorScaleChanged(int) {
    if (!m_classifWidget || !m_colorScaleSelector) return;
