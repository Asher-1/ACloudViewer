...
qFacets::~qFacets()
{
	if (s_fcDlg)
	{
		...
		{
			CVLog::Warning(tr("closing facets dialog failed! [%1]").arg(e.what()));
		}
	}
}
...
{	
	...
	if (!m_doFuseKdTreeCells)
	{
		...
		m_doFuseKdTreeCells->setToolTip(tr("Detect planar facets by fusing Kd-tree cells"));
	...
	{
		m_fastMarchingExtraction = new QAction(tr("Extract facets (Fast Marching)"), this);
		m_fastMarchingExtraction->setToolTip(tr("Detect planar facets with Fast Marching"));
	...
	if (!m_doExportFacets)
	{
		...
		m_doExportFacets->setToolTip(tr("Exports one or several facets to a shapefile"));
	...
	{
		m_doExportFacetsInfo = new QAction(tr("Export facets info (CSV)"), this);
		m_doExportFacetsInfo->setToolTip(tr("Exports various information on a set of facets (ASCII CSV file)"));
	...
	{
		m_doClassifyFacetsByAngle = new QAction(tr("Classify facets by orientation"), this);
		m_doClassifyFacetsByAngle->setToolTip(tr("Classifies facets based on their orienation (dip & dip direction)"));
	...
	if (!m_doShowStereogram)
	{
		...
		m_doShowStereogram->setToolTip(tr("Computes and displays a stereogram (+ interactive filtering)"));
...
{
	if (m_doFuseKdTreeCells)
		m_doFuseKdTreeCells->setEnabled(selectedEntities.size() == 1 && selectedEntities.back()->isA(CV_TYPES::POINT_CLOUD));
	if (m_fastMarchingExtraction)
		m_fastMarchingExtraction->setEnabled(selectedEntities.size() == 1 && selectedEntities.back()->isA(CV_TYPES::POINT_CLOUD));
	if (m_doExportFacets)
		m_doExportFacets->setEnabled(selectedEntities.size() != 0);
	if (m_doExportFacetsInfo)
		m_doExportFacetsInfo->setEnabled(selectedEntities.size() != 0);
	if (m_doClassifyFacetsByAngle)
		m_doClassifyFacetsByAngle->setEnabled(selectedEntities.size() == 1 && selectedEntities.back()->isA(CV_TYPES::HIERARCHY_OBJECT));
	if (m_doShowStereogram)
		m_doShowStereogram->setEnabled(selectedEntities.size() == 1 && (selectedEntities.back()->isA(CV_TYPES::HIERARCHY_OBJECT) || selectedEntities.back()->isA(CV_TYPES::POINT_CLOUD)));
}
...
void qFacets::extractFacets(CellsFusionDlg::Algorithm algo)
{
	...
	if (!pc)
	{
		m_app->dispToConsole(tr("Select one and only one point cloud!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}

	if (algo != CellsFusionDlg::ALGO_FAST_MARCHING && algo != CellsFusionDlg::ALGO_KD_TREE)
	{
		m_app->dispToConsole(tr("Internal error: invalid algorithm type!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	{
		m_app->dispToConsole(
			tr("Couldn't allocate a new scalar field for computing fusion labels! Try to free some memory ..."), 
	...
	if (algo == CellsFusionDlg::ALGO_KD_TREE)
	{
		...
		if (kdtree.build(s_errorMaxPerFacet / 2, errorMeasure, s_minPointsPerFacet, 1000, &pDlg))
		{
			...
			m_app->dispToConsole(tr("[qFacets] Kd-tree construction timing: %1 s").arg(static_cast<double>(elapsedTime_ms) / 1.0e3, 0, 'f', 3), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		...
		else
		{
			...
			if (group)
			{
				...
				case CellsFusionDlg::ALGO_KD_TREE:
					group->setName(group->getName() + tr(" [Kd-tree][error < %1][angle < %2 deg.]").arg(s_errorMaxPerFacet).arg(s_kdTreeFusionMaxAngle_deg));
					break;
				case CellsFusionDlg::ALGO_FAST_MARCHING:
					group->setName(group->getName() + tr(" [FM][level %2][error < %1]").arg(s_octreeLevel).arg(s_errorMaxPerFacet));
					break;
				default:
					break;
				}
				...
				m_app->dispToConsole(
					tr("[qFacets] %1 facet(s) where created from cloud '%2'").arg(count).arg(pc->getName()));

				if (error)
				{
					m_app->dispToConsole(
						tr("Error(s) occurred during the generation of facets! Result may be incomplete"),
						ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				}
				else
				{
			...
			{
				m_app->dispToConsole(
					tr("An error occurred during the generation of facets!"),
					ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			}
			else
			{
				m_app->dispToConsole(
					tr("No facet remains! Check the parameters (min size, etc.)"),
	...
	{
		m_app->dispToConsole(
			tr("An error occurred during the fusion process!"),
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
	}
	...
	while (!components.empty())
	{
		...
		if (compIndexes && compIndexes->size() >= minPointsPerComponent)
		{
			...
			else
			{
				...
				if (facet)
				{
					QString facetName = tr("facet %1 (rms=%2)").arg(ccGroup->getChildrenNumber()).arg(facet->getRMS());
...
void qFacets::getFacetsInCurrentSelection(FacetSet& facets) const
{
	...
	for ( ccHObject *entity : m_app->getSelectedEntities() )
	{
		...
		else //if (entity->isA(CV_TYPES::HIERARCHY_OBJECT)) //recursively tests group's children
		{
...
void qFacets::exportFacets()
{
	...
	QString facetsSavePath = settings.value("exportPath", ecvFileUtils::defaultDocPath()).toString();
	...
	for (FacetSet::iterator it = facets.begin(); it != facets.end(); ++it)
	{
		...
		if (!useNativeOrientation)
		{
			...
			{
				m_app->dispToConsole(
					tr("Failed to change the orientation of polyline '%1'! (not enough memory)").arg(poly->getName()),
					ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				continue;
			}
	...
	if (toSave.getChildrenNumber())
	{
		...
		{
			m_app->dispToConsole(tr("[qFacets] File '%1' successfully saved").arg(filename), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		}
		else
		{
			m_app->dispToConsole(tr("[qFacets] Failed to save file '%1'!").arg(filename), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		}
	}
}
...
void qFacets::classifyFacetsByAngle()
{
	...
	const ccHObject::Container& selectedEntities = m_app->getSelectedEntities();
	if (!m_app->haveOneSelection() || !selectedEntities.back()->isA(CV_TYPES::HIERARCHY_OBJECT))
	{
	...
	double maxDist)
{
	...
	if (group->isA(CV_TYPES::HIERARCHY_OBJECT))
	{
		...
		if (!success)
		{
			m_app->dispToConsole(tr("An error occurred while classifying the facets! (not enough memory?)"),
				ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	}
...
void qFacets::exportFacetsInfo()
{
	...
	{
		m_app->dispToConsole(
			tr("Failed to open file for writing! Check available space and access rights"),
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
