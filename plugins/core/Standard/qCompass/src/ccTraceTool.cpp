...
ccTraceTool::ccTraceTool() : ccTool() {}
...
void ccTraceTool::pointPicked(ccHObject* insertPoint,
                              unsigned itemIdx,
                              ccPointCloud* cloud,
                              const CCVector3& P) {
    // try and fetch the trace object (returns null if the id is invalid)
    ccTrace* t =
            dynamic_cast<ccTrace*>(m_app->dbRootObject()->find(m_trace_id));
    ...
    // if cost function is gradient/curvature then check appropriate SFs have
    // been defined
    if (ccTrace::COST_MODE & ccTrace::GRADIENT)  // gradient cost is active
    {
        if (m_precompute_gradient &
            !t->isGradientPrecomputed())  // not already computed
        {
            // give user a chance to bail - computations can take looooong
            QMessageBox::StandardButton q;
            q = QMessageBox::question(
                    m_app->getMainWindow(), "Calculate gradient?",
                    "Precompute Gradient? This can be slow, but once complete "
                    "will greatly decrease future computation times.",
    ...
    {
        if (m_precompute_curvature &
            !t->isCurvaturePrecomputed())  // not already computed?
        {
            QMessageBox::StandardButton q;
            q = QMessageBox::question(
                    m_app->getMainWindow(), "Calculate curvature?",
                    "Precompute Curvature? This can be slow, but once complete "
                    "will greatly decrease future computation times.",
                    QMessageBox::Yes | QMessageBox::No);
            if (q == QMessageBox::Yes)  // do compute
            {
    ...
    if (t->waypoint_count() >= 2) {
        ...
        {
            //... problem?
            m_app->dispToConsole(QString("[ccCompass] Failed to optimize trace "
...
void ccTraceTool::cancel() {
    ...
    if (t) {
        ...
        if (!m_preExisting)  // delete new traces (i.e. that were "picked-up" by
    ...
    if (selectedEntities.size() > 0)  // non-empty selection
    {
        ...
        if (selectedEntities[0]->getUniqueID() == m_trace_id) {
            return;  // we're already on it
                           ...
                           selectedEntities[0]))  // try pick-up the selection
        {
...
void ccTraceTool::finishCurrentTrace() {
    ...
    if (t) {
        ...
        bool shift = QApplication::keyboardModifiers().testFlag(
                Qt::ShiftModifier);  // shift is pressed
        if (shift) {
            fitPlane = !fitPlane;
            m_parentPlaneDeleted =
                    false;  // probably want different outcome to before
            m_childPlaneDeleted = false;
            m_changed = true;
        }

        // fit plane
        if ((fitPlane && m_changed) ||
            (m_parentPlaneDeleted || m_childPlaneDeleted)) {
            // calculate fit plane
            ...
            if (p) {
                ...
                bool child =
                        ccCompass::mapMode;  // in map mode, plane is stored as
                                             // a child; in compass mode, plane
                                             // is stored as parent
                ...
                m_app->dispToConsole(
                        QString("[ccCompass] Trace orientation estimate = " +
                                p->getName()),
                        ecvMainAppInterface::STD_CONSOLE_MESSAGE);
            } else {
                m_app->dispToConsole(
                        QString("[ccCompass] Not enough topography to fit "
                                "plane to trace."),
                        ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
            }
        }
...
bool ccTraceTool::pickupTrace(ccHObject* obj) {
    ...
    if (t)                                     // the object is a ccTrace!
    {
        ...
        // check if trace has previously fitted planes before optimizing (and
        // delete them if so)
        if (ccFitPlane::isFitPlane(t->getParent())) {
            ccHObject* parent = t->getParent();
            parent->detachChild(t);            // remove trace from the plane
            parent->getParent()->addChild(t);  // add trace to the plane's
                                               // parent
            ...
            for (unsigned idx = 0; idx < t->getChildrenNumber(); idx++) {
                ...
                if (ccFitPlane::isFitPlane(child)) {
                    m_app->removeFromDB(
                            child);  // n.b. removeFromDB also deletes the child
