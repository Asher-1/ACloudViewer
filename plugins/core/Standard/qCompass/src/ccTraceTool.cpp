...
void ccTraceTool::pointPicked(ccHObject* insertPoint, unsigned itemIdx, ccPointCloud* cloud, const CCVector3& P)
{
	...
	if (t->waypoint_count() >= 2)
	{
		...
		{
			//... problem?
			m_app->dispToConsole(QString("[ccCompass] Failed to optimize trace path... please try again."), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
	...
	if (t)
	{
		...
		if ( (fitPlane && m_changed) || (m_parentPlaneDeleted || m_childPlaneDeleted))
		{
			...
			if (p)
			{
				...
				m_app->dispToConsole(QString("[ccCompass] Trace orientation estimate = " + p->getName()), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
			}
			else
			{
				m_app->dispToConsole(QString("[ccCompass] Not enough topography to fit plane to trace."), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			}
		}
