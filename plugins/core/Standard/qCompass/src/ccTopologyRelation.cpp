...
ccTopologyRelation::ccTopologyRelation(ccPointCloud* associatedCloud,
                                       int older_id,
                                       int younger_id,
                                       int type)
...
ccTopologyRelation::ccTopologyRelation(ccPolyline* obj) : ccPointPair(obj) {
...
void ccTopologyRelation::constructGraphic(ccGeoObject* older,
                                          ccGeoObject* younger) {
    ...
    CCVector3 old_P =
            older->getBB_recursive().getCenter();  // first geoObject centroid
    CCVector3 young_P = younger->getBB_recursive()
                                .getCenter();  // second geoObject centroid

    // add start/end points too point cloud
    c->addPoint(old_P);
    c->addPoint(young_P);

    // add polyline points (these will then cause the underlying points in the
    // cloud to be drawn)
    ...
    switch (m_type) {
        ...
        case ccTopologyRelation::NOT_YOUNGER_THAN:
            setName(older->getName() + " Not Younger than " +
                    younger->getName());
            break;
        default:
            setName(older->getName() + " Unknown Relationship to " +
                    younger->getName());
    }
...
int ccTopologyRelation::getYoungerID() { return m_younger_id; }

// returns the older GeoObjects
int ccTopologyRelation::getOlderID() { return m_older_id; }

// return the type of this relationship
int ccTopologyRelation::getType() { return m_type; }

// get the type of this relationship
void ccTopologyRelation::setType(int topologyType) { m_type = topologyType; }
