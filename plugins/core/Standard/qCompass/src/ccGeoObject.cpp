...
ccGeoObject::ccGeoObject(QString name, ecvMainAppInterface* app, bool singleSurface)
	: ccHObject(name)
{
	m_app = app;
...
void ccGeoObject::assignGID()
{
	...
	_gID = static_cast<unsigned int>(std::hash<std::string>{}(QString::asprintf("%s%d", getName().toLatin1().data(), getUniqueID()).toStdString()));
}
...
ccTopologyRelation* ccGeoObject::addRelationTo(ccGeoObject* obj2, int type, ecvMainAppInterface* app)
{
	...
	if (out)
	{
		...
		app->dispToConsole("Relation already exists!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return nullptr;
	}
