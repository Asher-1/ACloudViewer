...
namespace ccCompassImport {
	...
	void importFoliations( ecvMainAppInterface *app )
	{
		...
		if (sel.empty())
		{
			app->dispToConsole("Please select a point cloud containing your field data (this can be loaded from a text file)",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
		...
		if (dipSF == dipDirSF)
		{
			app->dispToConsole("Error: Dip and Dip-Direction scalar fields must be different!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	...
	void importLineations( ecvMainAppInterface *app )
	{
		...
		if (dipSF == dipDirSF)
		{
			app->dispToConsole("Error: Trend and plunge scalar fields must be different!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
