              ...
              p->getName())  // create an identical plane
{
    ...
    QString dipAndDipDirStr = QString("%1/%2")
                                      .arg((int)dip, 2, 10, QChar('0'))
                                      .arg((int)dipdir, 3, 10, QChar('0'));
...
void ccFitPlane::updateAttributes(float rms, float search_r) {
    ...
    ccNormalVectors::ConvertNormalToDipAndDipDir(N, dip, dipdir);
    // ccNormalVectors::ConvertNormalToStrikeAndDip(N, strike, dip); //n.b. this
    // returns result using the british RHR?!?)
...
bool ccFitPlane::isFitPlane(ccHObject* object) {
    ...
    return false;
    /*return object->isKindOf(CV_TYPES::PLANE) //ensure object is a plane
