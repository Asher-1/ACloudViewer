...
{
	if (pickedObject->isA(CV_TYPES::PLANE)) //we want to be able to pick planes
	{
		//select the object
...
void ccThicknessTool::pointPicked(ccHObject* insertPoint, unsigned itemIdx, ccPointCloud* cloud, const CCVector3& P)
{
	...
	if (!m_referencePlane)
	{
		m_app->dispToConsole("[ccCompass] Please select a fit-plane to constrain true-thickness calculations.", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
...
{ 
	...
	//display instructions
	ecvDisplayTools::DisplayNewMessage("Select reference plane for thickness measurement.", ecvDisplayTools::LOWER_LEFT_MESSAGE);
