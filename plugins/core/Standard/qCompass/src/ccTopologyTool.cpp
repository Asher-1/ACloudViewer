...
void ccTopologyTool::onNewSelection(const ccHObject::Container& selectedEntities)
{
	...
	if (o)
	{
		...
		if (!first) //no... this is the first point (m_firstPick is invalid)
		{
			...
			ecvDisplayTools::DisplayNewMessage("Select second (younger) GeoObject.", ecvDisplayTools::LOWER_LEFT_MESSAGE);
		}
	...
	{
		//no - throw error
		m_app->dispToConsole("[ccCompass] Please select a GeoObject", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
	}
}
...
{ 
	...
	ecvDisplayTools::DisplayNewMessage("Select first (older) GeoObject.", ecvDisplayTools::LOWER_LEFT_MESSAGE);
}
