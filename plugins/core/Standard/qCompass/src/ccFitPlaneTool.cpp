...
void ccFitPlaneTool::pointPicked(ccHObject* insertPoint, unsigned itemIdx, ccPointCloud* cloud, const CCVector3& P)
{
	...
	CVLib::DgmOctree::NeighboursSet set;
	...
	CVLib::DgmOctreeReferenceCloud nCloud(&set, n);
	...
	if (pPlane) //valid fit
	{
		...
		m_app->dispToConsole(QString("[ccCompass] Surface orientation estimate = " + pPlane->getName()), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	}
}
