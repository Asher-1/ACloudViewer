...
void ccCompass::doAction()
{
	...
	{
		m_app->dispToConsole(tr("[ccCompass] Could not find valid 3D window."), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
...
bool ccCompass::startPicking()
{
	...
	{
		m_app->dispToConsole(
			tr("[ccCompass] Could not retrieve valid picking hub. Measurement aborted."), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	{
		m_app->dispToConsole(
			tr("Another tool is already using the picking mechanism. Stop it first"), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
...
ccHObject* ccCompass::getInsertPoint()
{
	...
	if (ccCompass::mapMode || m_geoObject)
	{
		...
		{
			m_app->dispToConsole(
				tr("[ccCompass] Error: Please select a GeoObject to digitize to."), 
				ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		}
		...
		else
		{
			...
			{
				m_app->dispToConsole(
					tr("[ccCompass] Warning: Could not retrieve valid mapping region for the active GeoObject."), 
	...
	else
	{
		...
		{
			if (m_app->dbRootObject()->getChild(i)->getName() == tr("measurements"))
			{
			...
			else
			{
				...
				{
					if (m_app->dbRootObject()->getChild(i)->getChild(c)->getName() == tr("measurements"))
					{
...
void ccCompass::pointPicked(ccHObject* entity, unsigned itemIdx, int x, int y, const CCVector3& P)
{
	...
	if (entity->isKindOf(CV_TYPES::POINT_CLOUD))
	{
		...
		if (!cloud)
		{
			CVLog::Warning(tr("[Item picking] Shit's fubar (Picked point is not in pickable entities DB?)!"));
			return;
		}
...
void ccCompass::fitPlaneToGeoObject()
{
	...
	m_app->dispToConsole(tr("[Compass] fitPlane"), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	if (points->size() > 0)
	{
		...
		{
			m_app->dispToConsole(
				tr("[Compass] Not enough 3D information to generate sensible fit plane."), 
				ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		}
	}
...
inline double logWishSF(CVLib::SquareMatrixd X, int nobserved)
{
...
inline double logWishart(CVLib::SquareMatrixd X, int nobserved, double phi, double theta, double alpha, double e1, double e2, double e3, double lsf)
{
	//--------------------------------------------------
...
void ccCompass::estimateStructureNormals()
{
	...
	QCheckBox calcThickChk(tr("Calculate thickness")); calcThickChk.setChecked(calcThickness);
	...
	//tooltips
	minSizeText.setToolTip(tr("The minimum size of the normal-estimation window."));
	maxSizeText.setToolTip(tr("The maximum size of the normal-estimation window."));
	dofText.setToolTip(tr("Sets the degrees of freedom parameter for the Wishart distribution. Due to non-independent data/errors in traces, this should be low (~10). Higher give more confident results - use with care!"));
	distanceText.setToolTip(tr("The furthest distance to search for points on the opposite surface of a GeoObject during thickness calculations."));
	sampleText.setToolTip(tr("Sample n orientation estimates at each point in each trace to quantify uncertainty."));
	likPowerText.setToolTip(tr("Fudge factor to change the balance between the prior and likelihood functions. Advanced use only - see docs for details."));
	strideText.setToolTip(tr("Standard deviation of the normal distribution used to calculate monte-carlo jumps during sampling. Larger numbers sample more widely but are slower to run."));
	...
	if (maxsize < minsize) {
		m_app->dispToConsole(
			tr("[ccCompass] Error - provided maxsize is less than minsize? Get your shit together..."), 
				ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}

	m_app->dispToConsole(
		tr("[ccCompass] Estimating structure normals. This may take a while..."),
		ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	for (ccHObject* o : m_app->getSelectedEntities())
	{
		...
		{
			o->filterChildren(objs, true, CV_TYPES::HIERARCHY_OBJECT); //n.b. geoObjects are simpy considered to be hierarchy objects by CC
		}
		...
		for (ccHObject* o2 : objs) {
			if (ccGeoObject::isGeoObject(o2)) {
				...
				if (g) {//could possibly be null if non-loaded geo-objects exist
					...
					ccHObject::Container objs;
					g->filterChildren(objs, true, CV_TYPES::POLY_LINE); //pinch nodes inherit the polyline clas
		...
		if (ccTrace::isTrace(o)) { //selected object is a trace
			...
			o->filterChildren(objs, true, CV_TYPES::POLY_LINE); //n.b. geoObjects are simpy considered to be hierarchy objects by CC
		}
		for (ccHObject* o2 : objs) {
	...
	if (datasets.empty()) { //no data found
		m_app->dispToConsole(
			tr("[ccCompass] No GeoObjects or Traces could be found to estimate structure normals for. Please select some!"), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
	}
	...
	for (int _d = 0; _d < datasets.size(); _d++)
	{
		...
		prg.setInfo(tr("Processing %1 of %2 datasets: Calculating fit planes...").arg( _d+1 ).arg( datasets.size() ));
		prg.update(0.0f);
		...
		for (unsigned r = 0; r < 2; r++) 
		{
			...
			if (points[r]->size() < minsize) {
				m_app->dispToConsole(tr("[ccCompass] Warning: Region %1 contains less than minsize points. Region ignored.").arg(regions[r]->getUniqueID()), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				delete points[r];
				points[r] = nullptr;
				regions[r] = nullptr;
				continue;
			...
			//*********************************************************
			CVLib::Neighbourhood Z(points[r]); //put points for this surface into a neighbourhood and get the sorting direction (principal eigenvector)
			const CCVector3* longAxis = Z.getLSPlaneX(); //n.b. this is a normal vector
			if (longAxis == nullptr) {
				//fail friendly if eigens could not be computed
				m_app->dispToConsole(tr("[ccCompass] Warning: Could not compute eigensystem for region %1. Region ignored.").arg(regions[r]->getUniqueID()), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			...
			CVLib::ReferenceCloud* nCloud = new  CVLib::ReferenceCloud(points[r]);
			...
			std::vector<CVLib::SquareMatrixd> bestX(px.size()); //keep track of best COV matrix for each trace (for oversampling later)
			...
			if (hasNormals) {
				...
				{ 
					m_app->dispToConsole(
						tr("[ccCompass] Warning: Cannot compensate for outcrop-surface bias as point cloud has no normals. Structure normal estimates may be misleading or incorrect."), 
						ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			...
			for (unsigned _min = 0; _min < px.size() - minsize; _min++)
			{
				...
				for (unsigned _max = _min + minsize; _max < std::min(static_cast<unsigned>(px.size()), _min + maxsize); _max++)
				{
					...
					//-----------------------------------------------------------------------------
					CVLib::SquareMatrixd X(3); //scale matrix
					for (unsigned p = _min; p <= _max; p++)
					{
					...
					CVLib::SquareMatrixd cov(3); //convert to covariance matrix
					...
					for (unsigned p = _min; p <= _max; p++)
					{
						if (lpd > bestPd[p]) //this is a better Pd
						{
							...
							segmentID[p] = _max * static_cast<int>(px.size()) + _min;
							bestX[p] = X;
			...
			if (!hasValidSNE) { //if segments between pinch nodes are too small, then we will not get any valid fit-planes
				m_app->dispToConsole(tr("[ccCompass] Warning: Region %1 contains no valid points (PinchNodes break the trace into small segments?). Region ignored.").arg(regions[r]->getUniqueID()), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				delete points[r];
				points[r] = nullptr;
				regions[r] = nullptr;
				continue;
			...
			//build scalar fields
			CVLib::ScalarField* startSF = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("StartPoint")));
			CVLib::ScalarField* endSF = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("EndPoint")));
			CVLib::ScalarField* idSF = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("SegmentID")));
			CVLib::ScalarField* weightSF = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("Weight")));
			CVLib::ScalarField* trend = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("Trend")));
			CVLib::ScalarField* plunge = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("Plunge")));
			CVLib::ScalarField* pointID = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("PointID"))); //used for linking samples representing the same point
			...
			if (oversample > 1)
			{
				...
				samples[r]->reserve(static_cast<unsigned int>(px.size()*oversample));
				samples[r]->reserveTheNormsTable();
				CVLib::ScalarField* startSF = samples[r]->getScalarField(samples[r]->addScalarField(new ccScalarField("StartPoint")));
				CVLib::ScalarField* endSF = samples[r]->getScalarField(samples[r]->addScalarField(new ccScalarField("EndPoint")));
				CVLib::ScalarField* idSF = samples[r]->getScalarField(samples[r]->addScalarField(new ccScalarField("SegmentID")));
				CVLib::ScalarField* weightSF = samples[r]->getScalarField(samples[r]->addScalarField(new ccScalarField("Weight")));
				CVLib::ScalarField* trend = samples[r]->getScalarField(samples[r]->addScalarField(new ccScalarField("Trend")));
				CVLib::ScalarField* plunge = samples[r]->getScalarField(samples[r]->addScalarField(new ccScalarField("Plunge")));
				CVLib::ScalarField* pointID = samples[r]->getScalarField(samples[r]->addScalarField(new ccScalarField("PointID")));
				...
				{
					//update progress dialog
					prg.setInfo(tr("Processing %1 of %2 datasets: Sampling points...").arg( _d + 1 ).arg( datasets.size() ));
					...
					while (count < oversample)
					{
						...
						{
							m_app->dispToConsole(
								tr("[ccCompass] Warning - MCMC sampler failed so sampling will be incomplete."), 
								ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
							break;
						}
						iter++;
		...
		if (regions[0] != nullptr && regions[1] != nullptr && calcThickness) //have both surfaces been defined?
		{
			if (points[0]->size() > 0 && points[1]->size() > 0) { //do both surfaces have points in them?
				prg.setInfo(tr("Processing %1 of %2 datasets: Estimating thickness...").arg( _d + 1 ).arg( datasets.size() ));
				for (int r = 0; r < 2; r++)
				{
					//make scalar field
					CVLib::ScalarField* thickSF = points[r]->getScalarField(points[r]->addScalarField(new ccScalarField("Thickness")));
					...
					CVLib::ScalarField* thickSF_sample = nullptr;
					CVLib::ScalarField* idSF_sample = nullptr;
					...
					CVLib::ReferenceCloud* nCloud = new  CVLib::ReferenceCloud(points[compID]);
					...
					for (unsigned p = 0; p < points[r]->size(); p++)
					{
						...
						d = CVLib::DistanceComputationTools::computePoint2PlaneDistance(nCloud->getPoint(0), pEq);
...
void ccCompass::estimateStrain()
{
	...
	{
		m_app->dispToConsole(
			tr("[ccCompass] Error - no traces or SNEs found to compute estimate strain with."), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	QCheckBox externalSNEChk(tr("Use external SNE:")); externalSNEChk.setChecked(useExternalSNE);
	QCheckBox buildBlocksChk(tr("Build graphics:")); buildBlocksChk.setChecked(buildGraphics);
	...
	boxSizeText.setToolTip(tr("The voxel size for computing strain. This should be large enough that most boxes contain SNEs."));
	externalSNEChk.setToolTip(tr("Use SNE orientation estimates for outside the current cell if none are avaliable within it."));
	buildBlocksChk.setToolTip(tr("Build graphic strain ellipses and grid domains. Useful for validation."));
	exagText.setToolTip(tr("Exaggerate the shape of strain ellipses for easier visualisation."));
		...
		if (g != nullptr)
		{
			for (unsigned p = 0; p < lines[i]->size(); p++)
			{
				...
				else
				{
					...
					const QString message = tr("[ccCompass] Error: cell %1 is outside of mesh bounds (with total size = %2 [%3,%4,%5])." )
											.arg( idx )
	...
	//init strain tensors
	CVLib::SquareMatrixd I(3); I.toIdentity();
	std::vector<CVLib::SquareMatrixd> F(nx*ny*nz, CVLib::SquareMatrixd(I)); //deformation gradient tensors
		...
		for (int y = 0; y < ny; y++)
		{
			for (int z = 0; z < nz; z++)
			{
				...
				for (ccGeoObject* g : geoObjectBins[idx])
				{
					...
					ccHObject::Container objs;
					g->filterChildren(objs, true, CV_TYPES::POINT_CLOUD,true); //look for SNE
					for (ccHObject* c : objs)
					{
					...
					CVLib::SquareMatrixd e(3); e.toIdentity();
					e.setValue(0, 0, (binSize + average_thickness) / binSize); //stretch matrix in local coordinates
					CVLib::SquareMatrixd F_increment = B*(e*B.transposed());// transform to global coords
		...
		for (int y = 0; y < ny; y++)
		{
			for (int z = 0; z < nz; z++)
			{
				...
				if (nStructures[idx] > 0)
				{
					...
					CVLib::SquareMatrixd B = F[idx] * F[idx].transposed();
					Jacobi<double>::ComputeEigenValuesAndVectors(B, eigVectors, eigValues, true); //get eigens

					CVLib::SquareMatrixd U_local(3); U_local.toIdentity();  //calculate stretch matrix in local (un-rotated coordinates)
					U_local.setValue(0, 0, sqrt(eigValues[0])); U_local.setValue(1, 1, sqrt(eigValues[1])); U_local.setValue(2, 2, sqrt(eigValues[2]));
					CVLib::SquareMatrixd U = eigVectors.transposed() * (U_local * eigVectors); //transform back into global coordinates
					...
					if (buildGraphics)
					{
						...
						CVLib::SquareMatrixd transMat(3);
...
void ccCompass::estimateP21()
{
	...
	{
		m_app->dispToConsole(
			tr("[ccCompass] Error - no polylines or traces found to compute P21."), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	for (ccPolyline* p : lines)
	{
		...
		{
			m_app->dispToConsole(
				tr("[ccCompass] Error - cannot calculate P21 intensity for structures digitised from different point clouds."), 
				ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	...
	boxSizeText.setToolTip(tr("The search radius used to define the region to compute P21 within."));
	subsampleText.setToolTip(tr("Only sample P21 on the each n'th point in the original outcrop model (decreases calculation time)."));
	...
	m_app->dispToConsole(
		tr("[ccCompass] Estimating P21 Intensity using a search radius of of %1.").arg(searchR), 
		ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	CVLib::DgmOctree::NeighboursSet region;
...
void ccCompass::convertToPointCloud()
{
	...
	for (ccHObject* o : m_app->getSelectedEntities())
	{
		...
		else
		{
			...
			ccHObject::Container objs;
			o->filterChildren(objs, true, CV_TYPES::POLY_LINE | CV_TYPES::HIERARCHY_OBJECT);
			for (ccHObject* c : objs)
			{
	...
	for (ccGeoObject* o : objs)
	{
		...
		//make point cloud
		ccPointCloud* points = new ccPointCloud(tr("ConvertedLines")); //create point cloud for storing points
		int sfid = points->addScalarField(new ccScalarField("Region")); //add scalar field containing region info
		CVLib::ScalarField* sf = points->getScalarField(sfid);
		...
		{
			m_app->dispToConsole(
				tr("[Compass] No polylines or traces converted - none found."), 
				ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			delete points;
		}
	}
...
void ccCompass::distributeSelection()
{
	...
	m_app->dbRootObject()->filterChildren(search, true, CV_TYPES::HIERARCHY_OBJECT, false);
	...
	for (ccHObject* obj : selection)
	{
		...
		{
			m_app->dispToConsole(
				tr("[Compass] Warning: No GeoObject could be found that matches %1.").arg(obj->getName()),
				ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		}
	}
...
{
	//m_app->dispToConsole("ccCompass: Changing to Map mode. Measurements will be associated with GeoObjects.", ecvMainAppInterface::STD_CONSOLE_MESSAGE);
...
{
	//m_app->dispToConsole("ccCompass: Changing to Compass mode. Measurements will be stored in the \"Measurements\" folder.", ecvMainAppInterface::STD_CONSOLE_MESSAGE);
...
void ccCompass::addGeoObject(bool singleSurface) //creates a new GeoObject
{
	...
	//get name
	name = QInputDialog::getText(m_app->getMainWindow(), tr("New GeoObject"), tr("GeoObject Name:"), QLineEdit::Normal, name);
	...
	for (unsigned i = 0; i < m_app->dbRootObject()->getChildrenNumber(); i++)
	{
		if (m_app->dbRootObject()->getChild(i)->getName() == tr("interpretation"))
		{
		...
		else
		{
			...
			for (unsigned c = 0; c < m_app->dbRootObject()->getChild(i)->getChildrenNumber(); c++)
			{
				if (m_app->dbRootObject()->getChild(i)->getChild(c)->getName() == tr("interpretation"))
				{
...
void ccCompass::importFoliations()
{
	...
	{
		m_app->dispToConsole(
			tr("Please select a point cloud containing your field data (this can be loaded from a text file)"),
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	{
		m_app->dispToConsole(
			tr("Error: Dip and Dip-Direction scalar fields must be different!"), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
...
void ccCompass::exportToSVG()
{
	...
	QString filename = QFileDialog::getSaveFileName(m_dlg,
		tr("SVG Output file"), "", tr("SVG files (*.svg)"));
	if (filename.isEmpty())
	{
	...
	m_app->dbRootObject()->filterChildren(objects, true, CV_TYPES::OBJECT, false); //get list of all children!
	...
	if (svg_file.open(QIODevice::WriteOnly))
	{
		...
		int width  = std::abs(static_cast<int>(ecvDisplayTools::GlWidth()  * zoom)); //glWidth and glHeight are negative on some machines??
		int height = std::abs(static_cast<int>(ecvDisplayTools::GlHeight() * zoom));
		...
		if (count > 0)
		{
			m_app->dispToConsole(tr("[ccCompass] Successfully saved %1 polylines to .svg file.").arg(count));
		}
		else
		{
			//remove file
			svg_file.remove();
			m_app->dispToConsole(
				tr("[ccCompass] Could not write polylines to .svg - no polylines found!"),
				ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		}
	}
}
	...
	if (object->isA(CV_TYPES::POLY_LINE) || ccTrace::isTrace(object))
	{
		...
		*out << "<polyline fill=\"none\" stroke=\"black\" points=\"";
		...
		if (params.perspective)
		{
			...
			//m_app->getActiveWindow()->redraw(false, false); //not sure if this is needed or not?
...
void ccCompass::onSave()
{
	...
	*/
	...
	if (plane_file.open(QIODevice::WriteOnly) && trace_file.open(QIODevice::WriteOnly) && lineation_file.open(QIODevice::WriteOnly) && thickness_file.open(QIODevice::WriteOnly))
	{
		...
		if (traces)
		{
			m_app->dispToConsole(tr("[ccCompass] Successfully exported trace data."), 
				ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		}
		else
		{
		...
		if (lineations)
		{
			m_app->dispToConsole(tr("[ccCompass] Successfully exported lineation data."), 
				ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		}
		else
		{
		...
		if (thicknesses)
		{
			m_app->dispToConsole(tr("[ccCompass] Successfully exported thickness data."), 
				ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		}
		else
		{
	...
	{
		m_app->dispToConsole(
			tr("[ccCompass] Could not open output files... ensure CC has write access to this location."), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
	}
}
...
int ccCompass::writePlanes(ccHObject* object, QTextStream* out, const QString &parentName)
{
	...
	if (object->isKindOf(CV_TYPES::PLANE) | ccFitPlane::isFitPlane(object))
	{
		...
		m_app->dbRootObject()->filterChildren(clouds, true, CV_TYPES::POINT_CLOUD, false);
	...
	else if (object->isKindOf(CV_TYPES::PLANE)) //not one of our planes, but a plane anyway (so we'll export it)
	{
...
int ccCompass::writeToXML(const QString &filename)
{
	...
	if (file.open(QIODevice::WriteOnly)) //open the file
	{
		...
		m_app->dbRootObject()->filterChildren(&pointClouds, true, CV_TYPES::POINT_CLOUD, true);*/
