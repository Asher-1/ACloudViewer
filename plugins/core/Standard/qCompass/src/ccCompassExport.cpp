...
namespace ccCompassExport {
	...
	void saveCSV( ecvMainAppInterface *app, const QString &filename )
	{
		...
		if (plane_file.open(QIODevice::WriteOnly) && trace_file.open(QIODevice::WriteOnly) && lineation_file.open(QIODevice::WriteOnly) && thickness_file.open(QIODevice::WriteOnly))
		{
			...
			if (planes)
			{
				app->dispToConsole("[ccCompass] Successfully exported plane data.", ecvMainAppInterface::STD_CONSOLE_MESSAGE);
			}
			else
			{
				app->dispToConsole("[ccCompass] No plane data found.", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				plane_file.remove();
			}
			if (traces)
			{
				app->dispToConsole("[ccCompass] Successfully exported trace data.", ecvMainAppInterface::STD_CONSOLE_MESSAGE);
			}
			else
			{
				app->dispToConsole("[ccCompass] No trace data found.", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				trace_file.remove();
			}
			if (lineations)
			{
				app->dispToConsole("[ccCompass] Successfully exported lineation data.", ecvMainAppInterface::STD_CONSOLE_MESSAGE);
			}
			else
			{
				app->dispToConsole("[ccCompass] No lineation data found.", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				lineation_file.remove();
			}
			if (thicknesses)
			{
				app->dispToConsole("[ccCompass] Successfully exported thickness data.", ecvMainAppInterface::STD_CONSOLE_MESSAGE);
			}
			else
			{
				app->dispToConsole("[ccCompass] No thickness data found.", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				thickness_file.remove();
			}
		}
		else
		{
			app->dispToConsole("[ccCompass] Could not open output files... ensure CC has write access to this location.", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		}
	...
	void saveSVG(ecvMainAppInterface *app, const QString &filename, float zoom)
	{
		...
		app->dbRootObject()->filterChildren(objects, true, CV_TYPES::OBJECT, false); //get list of all children!
		...
		QString pngFile = QString("%1/%2.png").arg(fileInfo.absolutePath()).arg(fileInfo.baseName());
		...
		if (svg_file.open(QIODevice::WriteOnly))
		{
			...
			int width  = std::abs(static_cast<int>(ecvDisplayTools::GlWidth()  * zoom));
			...
			else
			{
				//remove file
				svg_file.remove();
				app->dispToConsole("[ccCompass] Could not write polylines to .svg - no polylines found!",ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			}
		}
	}
	...
	{			
		...
		if (file.open(QIODevice::WriteOnly)) //open the file
		{
			...
			rootObject->filterChildren(&pointClouds, true, CV_TYPES::POINT_CLOUD, true);*/
			...
			app->dispToConsole("[ccCompass] Successfully exported data-tree to xml.", ecvMainAppInterface::STD_CONSOLE_MESSAGE);			
		}
		else
		{
			app->dispToConsole("[ccCompass] Could not open output files... ensure CC has write access to this location.", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
