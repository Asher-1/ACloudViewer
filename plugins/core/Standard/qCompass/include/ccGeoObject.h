...
/*
ccGeoObjects are a data-organisation structure comprised of four ccHObjects, a
parent (representing the "GeoObject") and 3 children, representing the "upper
boundary", "lower boundary" and "interior" of the GeoObject.
*/

class ccGeoObject : public ccHObject {
public:
    ccGeoObject(QString name, ecvMainAppInterface* app, bool singleSurface);
    ccGeoObject(ccHObject* obj, ecvMainAppInterface* app);

    // returns the pointCloud associated with this ccGeoObject's interior (or
    // null if the interior is undefined)
    ccPointCloud* getAssociatedCloud();

    // returns the ccHObject parent of the specified mapping region (see below
    // for mapping region consts)
    ...
    ccTopologyRelation* addRelationTo(ccGeoObject* obj2,
                                      int type,
...
protected:
    ...
    // the dataset this object "belongs" too. Assigned when an "interior" gets
    // defined.
    ...
    // group containing lower boundary polylines/traces and associated
    // measurments
...
private:
    ...
    // searches and activates/disactivates children belonging to the ccHObject.
    // This is used when the DBTree selection changes.
    void recurseChildren(ccHObject* par, bool highlight);

    // return the topology relationship between this ccHObject and another
    // (WIP).
...
public:
    ...
    // returns true if object is a single-surface GeoObject rather than a
    // generic GeoObject
    static bool isSingleSurfaceGeoObject(ccHObject* object);

    // traverses up the DbTree, starting at object, until a ccGeoObject is
    // found. If none is found this will return null.
    static ccGeoObject* getGeoObjectParent(ccHObject* object);

    // traverses up the DbTree, starting at object, until a ccGeoObject
    // representing the interior, upper or lower surface. returns -1 if no
    // ccGeoObject is found, otherwise ccGeoObject::INTERIOR,
