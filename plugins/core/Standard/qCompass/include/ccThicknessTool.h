...
protected:
    ...
    CCVector3* m_startPoint = nullptr;    // first point used to calculate
                                          // thickness in two-point mode
    std::vector<int> m_hiddenObjects;  // used to hide all point clouds (first),
                                       // then all planes (second).
    int m_graphic_id =
            -1;  // used to store partially completed lineation graphics
private:
    float planeToPointDistance(
            ccPlane* plane, CCVector3 P);  // calculate point-to-plane distances
    ccHObject* buildGraphic(CCVector3 endPoint,
                            float thickness);  // build a "thickness" graphic
    ...
    // gets the interior part of the currently selected GeoObject (or returns
    // the insertPoint if it's not a GeoObject)
    ccHObject* getInsertInterior(ccHObject* insertPoint);

public:
    static ecvColor::Rgb ACTIVE_COLOR;
    static bool TWO_POINT_MODE;  // if true, two points + planar orientation
                                 // used to calculate thickness. If false, then
