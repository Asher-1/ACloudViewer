...
class ccCompass : public QObject,
...
protected slots:
    ...
    void pointPicked(ccHObject* entity,
                     unsigned itemIdx,
                     int x,
                     int y,
                     const CCVector3& P);
    ...
    void
    recalculateSelectedTraces();  // recalculate any selected traces (for
                                  // updating with a different cost function)
    void mergeGeoObjects();       // merges the selected GeoObjects
    void fitPlaneToGeoObject();   // calculates best-fit plane for the upper and
                                  // lower surfaces of the selected GeoObject
    void recalculateFitPlanes();  // recalcs fit planes for traces and
                                  // GeoObjects
    void estimateStructureNormals();  // Estimate the normal vector to the
                                      // structure this trace represents at each
                                      // point in this trace.
    ...
    void convertToPointCloud();  // converts selected traces or geoObjects to
                                 // point clouds
    void distributeSelection();  // distributes selected objects into GeoObjects
                                 // with the same name
    ...
    // map mode dialog
    void writeToInterior();  // new digitization will be added to the GeoObjects
                             // interior
    void writeToUpper();     // new digitization will be added to the GeoObjects
                             // upper boundary
    void writeToLower();  // new digitiziation will be added to the GeoObjects
                          // lower boundary
    ...
    // drawing options
    void hideAllPointClouds(ccHObject* o);  // hides all point clouds and adds
                                            // them to the m_hiddenObjects list
...
protected:
    ...
    // used to get the place/object that new measurements or interpretation
    // should be stored
    ...
    // checks if the passed object, or any of it's children, represent unloaded
    // ccCompass objects (e.g. traces, fitplanes etc).
    void tryLoading();
    void tryLoading(ccHObject* obj,
    ...
    ccGeoObject* m_geoObject =
            nullptr;          // the GeoObject currently being written to
    ...
    int writeObjectXML(
            ccHObject* object,
            QXmlStreamWriter* out);  // writes the provided object (recursive)

    // checks if an object was made by this app (i.e. returns true if we are
    // responsible for a given layer)
    bool madeByMe(ccHObject* object);

    // used by the SNE algorithms
    // static double prior(double phi, double theta, double nx, double ny,
    // double nz); //prior distribution for orientations (depends on outcrop
    // orientation) static double logWishSF(double X[3][3], int nobserved);
    // //calculate log scale-factor for wishart dist. This only needs to be done
    // once per X, so is pulled out of the wish function for performance static
    // double logWishart(double X[3][3], int nobserved, double phi, double
    // theta, double alpha, double e1, double e2, double e3, double lsf);
    // //calculate log wishart probability density for an observed covariance
    // and proposed eigen system static double wishartExp1D(double X[3][3], int
    // nobserved, double phi, double theta, double e1, double e2, double e3,
    // double lsf, unsigned steps=500); //integrate over alpha static double**
    // sampleMCMC(double icov[3][3], int nobserved, CCVector3* normal, int
    // nsamples = 1000, double proposalWidth = 0.075); //sample posterior with
    // MCMC
