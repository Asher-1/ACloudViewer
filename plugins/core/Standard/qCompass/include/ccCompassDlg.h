...
public:
    ...
    /*
    Returns a flag describing the currently selected ccTrace::COST_MODE (used to
    build the cost function for optimisation)
    */
    int getCostMode();

    /*
    Returns true if the m_plane_fit action is checked -> used to check if the
    user expects us to fit a plane to finished traces.
    */
    bool planeFitMode();
    ...
    QAction *m_follows;      // activates topology tool - stratigraphic younging
                             // relations
    QAction *m_equivalent;   // activates topology tool - coeval/equivalent
                             // relations
    //--
    QAction *m_fitPlaneToGeoObject;  // fits a plane to the upper/lower surfaces
                                     // of all points in a GeoObject
    QAction *m_recalculateFitPlanes;
    QAction *m_estimateNormals;  // estimate structure normals
    QAction *m_estimateP21;     // estimate the intensity of structures (doesn't
    ...
    QAction *m_distributeSelection;  // tool for distributing imported data
                                     // (meshes etc.) into GeoObjects based on
                                     // shared names.
...
protected slots:
    ...
    void setDistCost() {
        clearCost();
        m_dist->setChecked(true);
    }
    void setScalarCost() {
        clearCost();
        m_scalar->setChecked(true);
    }
    void setInvScalarCost() {
        clearCost();
        m_scalar_inv->setChecked(true);
    }
