...
ccMPlaneDlgController::ccMPlaneDlgController(ecvMainAppInterface *app)
    ...
    connect(m_dialog, &ccMPlaneDlg::signalCloseButtonPressed, this,
            &ccMPlaneDlgController::onCloseButtonPressed);
    connect(m_dialog, &ccMPlaneDlg::signalTabChanged, this,
            &ccMPlaneDlgController::onNewTab);
    connect(m_dialog, &ccMPlaneDlg::signalMeasureNameChanged, this,
            &ccMPlaneDlgController::onMeasureNameChanged);
    connect(m_dialog, &ccMPlaneDlg::signalMeasurementModeChanged, this,
            &ccMPlaneDlgController::onMeasurementModeChanged);
    connect(m_dialog, &ccMPlaneDlg::signalFittingPointDelete, this,
            &ccMPlaneDlgController::onFittingPointDelete);
    connect(m_dialog, &ccMPlaneDlg::signalShowNormalCheckBoxClicked, this,
            &ccMPlaneDlgController::onNormalCheckBoxClicked);
    connect(m_dialog, &ccMPlaneDlg::signalSaveButtonClicked, this,
...
void ccMPlaneDlgController::onCloseButtonPressed() {
    ...
    // m_app->getActiveWindow()->redraw(true, false);
...
void ccMPlaneDlgController::onNewTab(int tabIndex) {
    m_mode = tabIndex == 0 ? CC_Mode::CC_POINT_SELECTION
                           : CC_Mode::CC_MEASUREMENT;
}

void ccMPlaneDlgController::onMeasureNameChanged(QTableWidgetItem *item) {
    if (!m_data->renameMeasurement(item->text(), item->row())) {
        m_app->dispToConsole(
                QString("[MPlane] Measurement point %1 already exists!")
                        .arg(item->text()),
                ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        m_dialog->renameMeasurement(
                m_data->getMeasurementPoints()[item->row()].getName(),
                item->row());
    }
}
...
void ccMPlaneDlgController::onSaveButtonClicked() {
    ...
    if (file.open(QIODevice::ReadWrite | QIODevice::Truncate)) {
        ...
        stream << "measurement,x-coord,y-coord,z-coord,distance"
               << QtCompat::endl;
        for (ccMPlanePoint point : m_data->getMeasurementPoints()) {
            stream << point.getName() << "," << point.getCoordinates().x << ","
                   << point.getCoordinates().y << ","
                   << point.getCoordinates().z << "," << point.getDistance()
                   << QtCompat::endl;
        }
    }
}

void ccMPlaneDlgController::onItemPicked(
    ...
    if (entity && entity->isKindOf(CV_TYPES::POINT_CLOUD)) {
        ...
        if (sourceCloud != m_selectedCloud) {
            m_app->dispToConsole(
                    "[MPlane] Can only pick points from selected cloud.",
                    ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        } else {
            if (m_mode == CC_Mode::CC_POINT_SELECTION) {
                pickFittingPoint(item);
                updatAllMeasurementEntities();
                m_dialog->selectFittingPoint(
                        m_data->getActualFittingPointIndex());
            } else {
...
void ccMPlaneDlgController::startPicking() {
    if (!m_app->pickingHub()) {
        m_app->dispToConsole(
                "[MPlane] Could not retrieve valid picking hub. Measurement "
                "aborted.",
                ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
    }
    ...
    if (!m_app->pickingHub()->addListener(this, true, true)) {
        m_app->dispToConsole(
                "[MPlane] Another tool is already using the picking mechanism. "
                "Stop it first",
                ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
    }
...
void ccMPlaneDlgController::pickFittingPoint(
    ...
    m_dialog->addFittingPoint(m_data->getActualFittingPointIndex() - 1,
                              item.P3D);
...
void ccMPlaneDlgController::pickMeasurementPoint(
        const ccPickingListener::PickedItem &item) {
    float distance = m_device->measurePointToPlaneDistance(
            m_data->getPlane(), item.P3D, m_signedMeasurement);
...
void ccMPlaneDlgController::updatePlane() {
    ccPlane *plane = m_device->fitPlaneToPoints(m_data->getFittingPoints(),
                                                MPLANE_PLANE_NAME);
...
void ccMPlaneDlgController::updateScalarfield() {
    m_device->createScalarFieldForCloud(m_data->getPlane(), m_selectedCloud,
                                        m_signedMeasurement);
}
...
void ccMPlaneDlgController::updateMeasurements() {
    ...
    for (auto i = 0; i < points.size(); ++i) {
        float distance = m_device->measurePointToPlaneDistance(
                m_data->getPlane(), points[i].getCoordinates(),
                m_signedMeasurement);
