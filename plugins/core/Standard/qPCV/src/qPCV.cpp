...
void qPCV::onNewSelection(const ccHObject::Container& selectedEntities)
{
	if (m_action)
	{
		...
		{
			if (obj && (obj->isKindOf(CV_TYPES::POINT_CLOUD) || obj->isKindOf(CV_TYPES::MESH)))
			{
...
void qPCV::doAction()
{
	...
	if (!cloudsWithNormals.empty() && dlg.useCloudRadioButton->isChecked())
	{
		...
		{
			m_app->dispToConsole("Not enough memory to generate the set of rays", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	...
	else
	{
		...
		{
			m_app->dispToConsole("Failed to generate the set of rays", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	}
	...
	{
		assert(false);
		m_app->dispToConsole("No ray was generated?!", ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		return;
	}
	...
	for (ccHObject* obj : candidates)
	{
		...
		if (sfIdx < 0)
		{
			m_app->dispToConsole("Couldn't allocate a new scalar field for computing PCV field ! Try to free some memory ...", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
		...
		if (!success)
		{
			...
			m_app->dispToConsole(tr("An error occurred during entity '%1' illumination!").arg(objName), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		}
		else
		{
			...
			if (sf)
			{
				...
				{
					m_app->dispToConsole(tr("Entity '%1' normals have been automatically disabled").arg(objName), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				}
		...
		{
			m_app->dispToConsole(tr("Process has been cancelled by the user"), ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
			break;
		}
	}
