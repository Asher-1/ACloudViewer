...
bool PCVCommand::Process(const ccHObject::Container& candidates,
    ...
    for (ccHObject* obj : candidates) {
        ...
        if (cloud == nullptr) {
            assert(false);
            if (app)
                app->dispToConsole(QObject::tr("Invalid object type"),
        ...
        if (sfIdx < 0) {
            if (app)
                app->dispToConsole(
                        ...
                        "PCV field! Try to free some memory...",
                        ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
            return false;
        }
        ...
        if (!success) {
            ...
            if (app)
                app->dispToConsole(QObject::tr("An error occurred during "
                                               "entity '%1' illumination!")
                                           .arg(objName),
                                   ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
            ++errorCount;
        } else {
            ...
            if (sf) {
                ...
                if (obj->hasNormals() && obj->normalsShown()) {
                    if (app)
                        app->dispToConsole(
                                QObject::tr("Entity '%1' normals have been "
        ...
        if (progressDlg && progressDlg->wasCanceled()) {
            if (app)
                app->dispToConsole(
                        QObject::tr("Process has been cancelled by the user"),
                        ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
            ++errorCount;
            break;
        }
...
bool PCVCommand::process(ccCommandLineInterface& cmd) {
    ...
    while (!cmd.arguments().empty()) {
        ...
        // PCV::Launch mode360 defaults to true. To make absence / presence of
        // command map to false / true, the command is named "180," and is the
        // inverse of mode360.
