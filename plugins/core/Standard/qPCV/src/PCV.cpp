...
static bool SampleSphere(unsigned N, std::vector<CCVector3>& dirs) {
    ...
    try {
        ...
        double beta = acos(1.0 - 2.0 / N);  // return in [0,pi/2] as '1-2/N'
                                            // goes from 0 to 1 when N --> inf
        ...
        m.resize(L, 0);
        {
            ...
            for (int i = 1; i < L; ++i) {
                ...
                if ((mbar[i] - f) >= 0.5) f = ceil(mbar[i] + alpha - fuzz);
...
bool PCV::GenerateRays(unsigned numberOfRays,
    ...
    // we keep only the light directions that meets input parameters (non
    // predictible if not in 360?mode!)
    if (!mode360) {
...
bool PCV::Launch(std::vector<CCVector3>& rays,
    ...
    if (!vertices || !vertices->enableScalarField()) return false;
    ...
    // for each vertex we keep count of the number of light directions for which
    // it is "illuminated"
    ...
    CVLib::NormalizedProgress nProgress(progressCb, numberOfRays);
    if (progressCb) {
        if (progressCb->textCanBeEdited()) {
            progressCb->setMethodTitle("ShadeVis");
            QString infoStr;
            if (!entityName.isEmpty()) infoStr = entityName + "\n";
