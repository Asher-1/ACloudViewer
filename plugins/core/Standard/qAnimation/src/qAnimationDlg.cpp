        ...
        for (unsigned it = 0; it < iterationCount; ++it)
        {
            ...
            for (size_t i = 0; i < segmentCount; ++i)
            {
                ...
                if (!openPoly || i + 1 != segmentCount)
                {
                    ...
                    interpolatedStep.duration_sec = sP.duration_sec * (PC_ONE - ratio);
...
void qAnimationDlg::onAutoStepsDurationToggled(bool state)
{
    ...
    CVLog::PrintDebug(QString("Total time = %1 / Smooth time = %2").arg(totalTime).arg(totalTimeSmooth));
...
void qAnimationDlg::render(bool asSeparateFrames)
{
    ...
    if (!asSeparateFrames)
    {
        ...
        encoder.reset(new QVideoEncoder(outputFilename,
                                        ecvDisplayTools::GlWidth() * animScale,
                                        ecvDisplayTools::GlHeight() * animScale,
                                        bitrate, gop, static_cast<unsigned>(fpsSpinBox->value())));
        QStringList errors;
...
//    m_view3d->setLODEnabled(false);
    ...
    for (int frameIndex = 0; frameIndex < frameCount; )
    {
        ...
        if (deltaTime <= step1.duration_sec)
        {
            ...
            {
                image = image.scaled(image.width() / superRes, image.height() / superRes,
