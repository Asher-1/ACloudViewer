...
void qCork::onNewSelection(const ccHObject::Container& selectedEntities)
{
	if (m_action)
	{
		...
		m_action->setEnabled(	selectedEntities.size() == 2
							&&	selectedEntities[0]->isKindOf(CV_TYPES::MESH)
							&&	selectedEntities[1]->isKindOf(CV_TYPES::MESH));
	}
}
...
bool ToCorkMesh(const ccMesh* in, CorkMesh& out, ecvMainAppInterface* app = 0)
{
	...
	{
		if (app)
			app->dispToConsole("[Cork] Invalid input mesh!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		assert(false);
		return false;
	}
	...
	{
		if (app)
			app->dispToConsole("[Cork] Not enough memory!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return false;
	}
	...
	{
		if (app)
			app->dispToConsole(QString("[Cork] Input mesh '%1' is empty?!").arg(in->getName()),ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		return false;
	}
...
ccMesh* FromCorkMesh(const CorkMesh& in, ecvMainAppInterface* app = 0)
{
	...
	{
		if (app)
			app->dispToConsole("[Cork] Output mesh is empty?!",ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		return 0;
	}
...
bool doPerformBooleanOp()
{
	...
	try
	{
		...
		if (false)
		{
			...
			{
				if (s_params.app)
					s_params.app->dispToConsole(QString("[Cork] Mesh '%1' is self-intersecting! Result may be jeopardized!").arg(s_params.nameA),ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				s_params.meshesAreOk = false;
			}
			else if (!s_params.corkA->isClosed())
			{
				if (s_params.app)
					s_params.app->dispToConsole(QString("[Cork] Mesh '%1' is not closed! Result may be jeopardized!").arg(s_params.nameA),ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				s_params.meshesAreOk = false;
			}
			if (s_params.corkB->isSelfIntersecting())
			{
				if (s_params.app)
					s_params.app->dispToConsole(QString("[Cork] Mesh '%1' is self-intersecting! Result may be jeopardized!").arg(s_params.nameB),ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				s_params.meshesAreOk = false;
			}
			else if (!s_params.corkB->isClosed())
			{
				if (s_params.app)
					s_params.app->dispToConsole(QString("[Cork] Mesh '%1' is not closed! Result may be jeopardized!").arg(s_params.nameB),ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
				s_params.meshesAreOk = false;
			}
		}
		...
		default:
			assert(false);
			if (s_params.app)
				s_params.app->dispToConsole("Unhandled operation?!",ecvMainAppInterface::WRN_CONSOLE_MESSAGE); //DGM: can't issue an error message (i.e. with dialog) in another thread!
			break;
		}
	}
	...
	{
		if (s_params.app)
			s_params.app->dispToConsole(QString("Exception caught: %1").arg(e.what()),ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
		return false;
	}
...
void qCork::doAction()
{
	...
	{
		assert(false);
		m_app->dispToConsole("Select two and only two meshes!",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	//launch process
	{
		...
		{
			if (m_app)
				m_app->dispToConsole(s_params.meshesAreOk ? "Computation failed!" : "Computation failed! (check console)",ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			//an error occurred
			return;
		}
	}
