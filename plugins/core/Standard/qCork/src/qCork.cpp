...
bool ToCorkMesh(const ccMesh* in, CorkMesh& out, ecvMainAppInterface* app = 0) {
    ...
    try {
        ...
        if (app)
            app->dispToConsole("[Cork] Not enough memory!",
                               ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return false;
    }
    ...
    if (outVerts.empty() || outTris.empty()) {
        if (app)
            app->dispToConsole(QString("[Cork] Input mesh '%1' is empty?!")
                                       .arg(in->getName()),
...
ccMesh* FromCorkMesh(const CorkMesh& in, ecvMainAppInterface* app = 0) {
    ...
    if (inTris.empty() || inVerts.empty()) {
        if (app)
            app->dispToConsole("[Cork] Output mesh is empty?!",
                               ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
        return 0;
    }
...
bool doPerformBooleanOp() {
    // invalid parameters
    if (!s_params.corkA || !s_params.corkB) return false;
    ...
    try {
        ...
        if (false) {
            if (s_params.corkA->isSelfIntersecting()) {
                if (s_params.app)
                    s_params.app->dispToConsole(
                            QString("[Cork] Mesh '%1' is self-intersecting! "
                                    "Result may be jeopardized!")
                                    .arg(s_params.nameA),
                            ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
                ...
                if (s_params.app)
                    s_params.app->dispToConsole(
                            QString("[Cork] Mesh '%1' is not closed! Result "
                                    "may be jeopardized!")
                                    .arg(s_params.nameA),
                            ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
                s_params.meshesAreOk = false;
            ...
            default:
                ...
                if (s_params.app)
                    s_params.app->dispToConsole(
                            "Unhandled operation?!",
                            ecvMainAppInterface::
                                    WRN_CONSOLE_MESSAGE);  // DGM: can't issue
                                                           // an error message
                                                           // (i.e. with dialog)
                                                           // in another thread!
                break;
...
void qCork::doAction() {
    ...
    if (selNum != 2 || !selectedEntities[0]->isKindOf(CV_TYPES::MESH) ||
        !selectedEntities[1]->isKindOf(CV_TYPES::MESH)) {
        assert(false);
        m_app->dispToConsole("Select two and only two meshes!",
                             ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }
    ...
    if (!cDlg.exec()) return;
    if (cDlg.isSwapped()) std::swap(meshA, meshB);

    // try to convert both meshes to CorkMesh structures
    CorkMesh corkA;
    if (!ToCorkMesh(meshA, corkA, m_app)) return;
    CorkMesh corkB;
    if (!ToCorkMesh(meshB, corkB, m_app)) return;

    // launch process
    {
        // run in a separate thread
        QProgressDialog pDlg("Operation in progress", QString(), 0, 0,
                             m_app->getMainWindow());
