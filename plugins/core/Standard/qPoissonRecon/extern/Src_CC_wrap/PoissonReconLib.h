...
public:
    ...
    struct Parameters {
        ...
        /** Running at depth d corresponds to solving on a 2^d x 2^d x 2^d.
                Note that since the reconstructor adapts the octree to the
        sampling density, the specified reconstruction depth is only an upper
        bound.
        **/
        int depth = 8;

        //! The target width of the finest level octree cells (ignored if depth
        //! is specified)
        ...
        /** Specifies the factor of the bounding cube that the input samples
         *should fit into.
         **/
        float scale = 1.1f;

        //! The minimum number of sample points that should fall within an
        //! octree node as the octree construction is adapted to sampling
        //! density.
        /** This parameter specifies the minimum number of points that should
        fall within an octree node. For noise-free samples, small values in the
        range [1.0 - 5.0] can be used. For more noisy samples, larger values in
        the range [15.0 - 20.0] may be needed to provide a smoother,
        noise-reduced, reconstruction.
        **/
        float samplesPerNode = 1.5f;

        //! The importance that interpolation of the point samples is given in
        //! the formulation of the screened Poisson equation.
        /** The results of the original (unscreened) Poisson Reconstruction can
         *be obtained by setting this value to 0.
         **/
        float pointWeight = 2.0f;

        //! The number of solver iterations
        /** Number of Gauss-Seidel relaxations to be performed at each level of
        ...
        //! This flag tells the reconstructor to read in color values with the
        //! input points and extrapolate those to the vertices of the output.
        bool withColors = true;

        //! Data pull factor
        /** If withColors is rue, this floating point value specifies the
         *relative importance of finer color estimates over lower ones.
         **/
        float colorPullFactor = 32.0f;

        //! Normal confidence exponent
        /** Exponent to be applied to a point's confidence to adjust its weight.
         *(A point's confidence is defined by the magnitude of its normal.)
         **/
        float normalConfidence = 0.0;

        //! Normal confidence bias exponent
        /** Exponent to be applied to a point's confidence to bias the
         *resolution at which the sample contributes to the linear system.
         *(Points with lower confidence are biased to contribute at coarser
         *resolutions.)
         **/
        ...
        //! This parameter specifies the number of threads across which the
        //! solver should be parallelized
        int threads = 1;

        /** The parameters below are accessible via the command line but are not
         * described in the official documentation **/

        //! The depth beyond which the octree will be adapted.
        /** At coarser depths, the octree will be complete, containing all 2^d x
         *2^d x 2^d nodes.
         **/
        int fullDepth = 5;
