...
void qPoissonRecon::doAction()
{
	...
	{
		m_app->dispToConsole(tr("Select a cloud!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	if (!pc->hasNormals())
	{
		m_app->dispToConsole(tr("Cloud must have normals!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	ccPointCloud* newPC = new ccPointCloud(tr("vertices"));
	...
	{
		//start message
		m_app->dispToConsole(tr("[PoissonRecon] Job started (level %1)").arg(s_params.depth), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
		...
		QString progressLabel(tr("Reconstruction in progress\n"));
		if (s_depthMode)
	...
	if (!result)
	{
		...
		delete newMesh;
		newMesh = nullptr;
		m_app->dispToConsole(tr("Reconstruction failed!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	
	//success message
	m_app->dispToConsole(
		tr("[PoissonRecon] Job finished (%1 triangles, %2 vertices)").arg(newMesh->size()).arg(newPC->size()), 
		ecvMainAppInterface::STD_CONSOLE_MESSAGE);

	newMesh->setName(tr("Mesh[%1] (level %2)").arg(pc->getName()).arg(s_params.depth));
