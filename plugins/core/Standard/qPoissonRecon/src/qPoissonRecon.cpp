...
public:
    explicit PoissonReconParamDlg(QWidget* parent = nullptr)
        : QDialog(parent, Qt::Tool), Ui::PoissonReconParamDialog() {
        setupUi(this);

        threadSpinBox->setRange(
                1, PoissonReconLib::Parameters::GetMaxThreadCount());
    }
};
...
void qPoissonRecon::onNewSelection(
...
void qPoissonRecon::doAction() {
    ...
    if (!m_app->haveOneSelection()) {
        m_app->dispToConsole("Select only one cloud!",
    ...
    if (!ent->isA(CV_TYPES::POINT_CLOUD)) {
        m_app->dispToConsole("Select a cloud!",
                             ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }
    ...
    if (!pc->hasNormals()) {
        m_app->dispToConsole("Cloud must have normals!",
                             ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }
    ...
    s_params.samplesPerNode =
            static_cast<float>(prpDlg.samplesPerNodeSpinBox->value());
    ...
    s_params.pointWeight =
            static_cast<float>(prpDlg.weightDoubleSpinBox->value());
    ...
    bool result = false;
    {
        // start message
        m_app->dispToConsole(
                QString("[PoissonRecon] Job started (level %1 - %2 threads)")
                        .arg(s_params.depth)
                        .arg(s_params.threads),
                ecvMainAppInterface::STD_CONSOLE_MESSAGE);

        // progress dialog (Qtconcurrent::run can't be canceled!)
        QProgressDialog pDlg(tr("Initialization"), QString(), 0, 0,
                             m_app->getMainWindow());
        pDlg.setWindowTitle("Poisson Reconstruction");
        pDlg.setCancelButton(
                nullptr);  // it's not possible to cancel PoissonRecon currently
        pDlg.show();
        ...
        if (s_depthMode)
            ...
            progressLabel +=
                    QString("resolution: %1").arg(s_params.finestCellWidth);
    ...
    if (!result) {
        ...
        delete newMesh;
        newMesh = nullptr;
        m_app->dispToConsole("Reconstruction failed!",
                             ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
        return;
    }
    ...
    // success message
    m_app->dispToConsole(
            QString("[PoissonRecon] Job finished (%1 triangles, %2 vertices)")
                    .arg(newMesh->size())
                    .arg(newPC->size()),
            ecvMainAppInterface::STD_CONSOLE_MESSAGE);
