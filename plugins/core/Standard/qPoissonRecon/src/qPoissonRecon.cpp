//##########################################################################
//#                                                                        #
//#                EROWCLOUDVIEWER PLUGIN: qPoissonRecon                      #
//#                                                                        #
...
public:
	...
	virtual void getColor(size_t index, Real* rgb) const
	{
		...
		const ecvColor::Rgb& color = m_cloud.getPointColor(static_cast<unsigned>(index));
...
{
public:
	explicit MeshWrapper(ccMesh& mesh, ccPointCloud& vertices, CVLib::ScalarField* densitySF = nullptr)
		: m_mesh(mesh)
		, m_vertices(vertices)
...
static CVLib::ScalarField* s_densitySF = nullptr;
...
void qPoissonRecon::doAction()
{
	...
	{
		m_app->dispToConsole("Select only one cloud!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	{
		m_app->dispToConsole("Select a cloud!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	if (!pc->hasNormals())
	{
		m_app->dispToConsole("Cloud must have normals!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	{
		//start message
		m_app->dispToConsole(QString("[PoissonRecon] Job started (level %1)").arg(s_params.depth), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
	...
	if (!result)
	{
		...
		delete newMesh;
		newMesh = nullptr;
		m_app->dispToConsole("Reconstruction failed!", ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	
	//success message
	m_app->dispToConsole(QString("[PoissonRecon] Job finished (%1 triangles, %2 vertices)").arg(newMesh->size()).arg(newPC->size()), ecvMainAppInterface::STD_CONSOLE_MESSAGE);
