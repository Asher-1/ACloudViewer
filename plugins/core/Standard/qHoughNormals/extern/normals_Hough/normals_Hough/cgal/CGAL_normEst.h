...
public:
    ...
    typedef typename CGAL::Orthogonal_k_neighbor_search<TreeTraits>
            Neighbor_search;
     ...
     * @param normals
     */
    CGAL_Normal_Estimator(std::vector<Point3> &points,
...
private:
     ...
     */
    inline void generate_rotation_matrix(std::vector<Matrix3> &rotMat,
        ...
        if (rotations == 0) {
            ...
            for (int i = 0; i < rotations; i++) {
                ...
                Matrix3 Rt(1, 0, 0, 0, cos(theta), -sin(theta), 0, sin(theta),
                           cos(theta));
                Matrix3 Rph(cos(phi), 0, sin(phi), 0, 1, 0, -sin(phi), 0,
                            cos(phi));
                Matrix3 Rps(cos(psi), -sin(psi), 0, sin(psi), cos(psi), 0, 0, 0,
                            1);
                Matrix3 Rtinv(1, 0, 0, 0, cos(theta), sin(theta), 0,
                              -sin(theta), cos(theta));
                Matrix3 Rphinv(cos(phi), 0, -sin(phi), 0, 1, 0, sin(phi), 0,
                               cos(phi));
                Matrix3 Rpsinv(cos(psi), sin(psi), 0, -sin(psi), cos(psi), 0, 0,
                               0, 1);
     ...
     */
    inline void generate_random_points_vector(std::vector<Point3> &points,
                                              int point_number) {
    ...
    inline void assign_points_to_cubes(std::vector<int> cubes[],
        ...
        for (unsigned int i = 0; i < points2.size(); i++) {
            ...
            int x = std::max(
                    0, std::min(int((points[i].x() - refPoint2.x()) / step +
                                    (n_cubes / 2.f)),
                                n_cubes - 1));
            int y = std::max(
                    0, std::min(int((points[i].y() - refPoint2.y()) / step +
                                    (n_cubes / 2.f)),
                                n_cubes - 1));
            int z = std::max(
                    0, std::min(int((points[i].z() - refPoint2.z()) / step +
                                    (n_cubes / 2.f)),
                                n_cubes - 1));
     ...
     */
    inline void find_a_triplet(Tree &tree,
                               float radius1,
                               float radius2,
                               Triplet &triplet,
                               Point3 &pt) {
    ...
    inline void generate_list_of_triplets(std::vector<Triplet> &triplets,
        ...
        while (ittrip != triplets.end() && itpoints != points.end()) {
            ...
            if (points_search.size() != 0) {
                Point3 new_point =
                        points_search[(*itint) % points_search.size()];
                bool is_valid = true;
                for (int i = 0; i < idx; i++) {
    ...
    float normal_at_point(const int d1,
        ...
        nls[n] = votesV[i1 + j1 * d1] /
                 sqrt(votesV[i1 + j1 * d1] * votesV[i1 + j1 * d1]);
        delete[] votes;
        delete[] votesV;
    ...
    inline Vector3 normal_selection(int &rotations,
                                    std::vector<Vector3> &normals_vec,
        ...
        switch (selection_type) {
            ...
            case 2:  // mb
            {
                ...
                while (number_to_test > 0) {
                    ...
                    normals_fin.push_back(std::pair<Vector3, float>(
                    ...
                    for (int i = 0; i < rotations; i++) {
                        if (normals_use[i] &&
                            acos(normals_vec[idx] * normals_vec[i]) <
                                    tol_angle_rad) {
                            normals_use[i] = false;
                            number_to_test--;
...
#pragma omp for schedule(guided)
        ...
        Tree tree(pts.begin(), pts.end());
            ...
            for (int n = 0; n < (int)pts.size(); n++) {
                ...
                for (Neighbor_search_iterator it = search.begin();
                     it != search.end() && points_size < neighbor_number;
                     ++it) {
                ...
                for (int i = 0; i < rotations; i++) {
                    ...
                    for (unsigned int pt = 0; pt < points_size; pt++) {
                        points[pt] = points2[pt].transform(
                                rotMat[(n + i) % rotMat.size()]);
                    }
                    normals_conf[i] = normal_at_point(d1, d2, points, n,
                                                      triplets, conf_interv);

                    normals_vec[i] = nls[n].transform(
        ...
        Tree tree(pts.begin(), pts.end());
            ...
            for (int n = 0; n < (int)pts.size(); n++) {
                ...
                if (points_size < lower_neighbor_bound_neighbors) {
                    ...
                    // getting the list of neighbors
                    Neighbor_search search(tree, pts[n],
                                           lower_neighbor_bound_neighbors);

                    for (Neighbor_search_iterator it = search.begin();
                         it != search.end() &&
                         points_size < lower_neighbor_bound_neighbors;
                         ++it) {
                ...
                if (rotations <= 1) {
                    ...
                    list_of_triplets(trip, points_size, rotations * n_planes,
                                     vecInt);
                }
...
#pragma omp critical
            ...
            if (points_size != neighbor_number) {
                continue;
            }
            radius = sqrt(radius);
            ...
            for (int i = 0; i < rotations; i++) {
                ...
                for (unsigned int tr = 0; tr < triplets.size(); tr++) {
                    triplets[tr](0) = triplets[tr](0).transform(
                            rotMat[(n + i) % rotMat.size()]);
                    triplets[tr](1) = triplets[tr](1).transform(
                            rotMat[(n + i) % rotMat.size()]);
                    triplets[tr](2) = triplets[tr](2).transform(
                            rotMat[(n + i) % rotMat.size()]);
                }

                normals_conf[i] = normal_at_point(d1, d2, points_size, n,
                                                  triplets, conf_interv);

                normals_vec[i] =
                        nls[n].transform(rotMatInv[(n + i) % rotMat.size()]);
            }
...
#pragma omp for schedule(guided)
        ...
        Tree tree(pts.begin(), pts.end());
            ...
            for (int n = 0; n < (int)pts.size(); n++) {
                ...
                for (int i = 0; i < rotations; i++) {
                    assign_points_to_cubes(cubes, radius, pts[n], points,
                                           points2,
                                           rotMat[(n + i) % rotMat.size()]);
...
#pragma omp for schedule(guided)
        ...
        Tree tree(pts.begin(), pts.end());
            ...
            for (int n = 0; n < (int)pts.size(); n++) {
                ...
                for (int i = 0; i < rotations; i++) {
                    assign_points_to_cubes(cubes, radius2, pts[n], points,
                                           points2,
                                           rotMat[(n + i) % rotMat.size()]);
