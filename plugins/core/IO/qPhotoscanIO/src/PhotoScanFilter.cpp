...
{
    CVLog::Warning("--> " + stream.name().toString() + (stream.isStartElement() ? " [start]" : "") + (stream.isEndElement() ? " [end]" : ""));
	for (int i = 0; i < stream.attributes().size(); ++i)
	{
        CVLog::Warning(QString("\t") + stream.attributes().at(i).qualifiedName().toString());
	}
}
...
static QString CreateTempFile(QuaZip& zip, QString zipFilename)
{
	...
	{
        CVLog::Warning(QString("[Photoscan] Failed to locate '%1' in the Photoscan archive").arg(zipFilename));
		return QString();
	}
	...
	{
        CVLog::Warning(QString("[Photoscan] Failed to extract '%1' from Photoscan archive").arg(zipFilename));
		return QString();
	}
	...
	{
        CVLog::Warning(QString("[Photoscan] Failed to create temp file '%1'").arg(tempFilename));
		return QString();
	}
										...
										LoadParameters& parameters)
{
	...
	{
		//empty archive?
        CVLog::Warning(QString("[Photoscan] Couldn't find '%1' in Photoscan archive").arg(s_defaultXMLFilename));
		return CC_FERR_NO_LOAD;
	}
	...
	{
        CVLog::Warning(QString("[Photoscan] Failed to locate '%1' in the Photoscan archive").arg(s_defaultXMLFilename));
	...
	{
        CVLog::Warning(QString("[Photoscan] Failed to extract '%1' from Photoscan archive").arg(s_defaultXMLFilename));
		return CC_FERR_NO_LOAD;
	}
	...
	while (true)
	{
		if (!stream.readNextStartElement())
		{
			...
			if (!sections.empty())
			{
                CVLog::PrintDebug(" < " + stream.name().toString() + QString(" [%1]").arg(ToName(sections.back())));
				sections.pop_back();
				//stream.skipCurrentElement();
				continue;
			}
        ...
        CVLog::PrintDebug(" > " + stream.name().toString());
	...
	for (CameraDesc& camera : cameras)
	{
		...
		//{
        //	CVLog::Warning(QString("[Photoscan] Failed to locate image '%1' in the Photoscan archive").arg(camera.imageFilename));
		//	continue;
		//}
		...
		//{
        //	CVLog::Warning(QString("[Photoscan] Failed to extract '%1' from Photoscan archive").arg(camera.imageFilename));
		//	continue;
		//}
		...
		{
            CVLog::Warning(QString("[Photoscan] Failed to load image '%1'").arg(camera.imageFilename));
			continue;
		}
	...
	if (!wasCanceled)
	{
		...
		for (CloudDesc& desc : clouds)
		{
			...
			{
                CVLog::Warning(QString("[Photoscan] OC3 format not supported. Can't import %1 from the Photoscan archive").arg(desc.type));
				continue;
			}
			...
			{
                CVLog::Warning(QString("[Photoscan] Failed to extract '%1' from Photoscan archive").arg(desc.filename));
			}
