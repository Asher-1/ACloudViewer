...
struct QUAZIP_EXPORT QuaZipFileInfo64 {
    ...
    /// Last modification date and time.
    /**
     * This is the time stored in the standard ZIP header. This format only
     * allows to store time with 2-second precision, so the seconds will always
     * be even and the milliseconds will always be zero. If you need more
     * precise date and time, you can try to call the getNTFSmTime() function or
    ...
    /**
      ...
      used for compatibility with the old code expecting QuaZipFileInfo, in the
      cases when it's impossible or otherwise unadvisable (due to ABI
      compatibility reasons, for example) to modify that old code to use
      QuaZipFileInfo64.
     ...
     * @param fineTicks If not NULL, the fractional part of milliseconds
     * returned there, measured in 100-nanosecond ticks. Will be set to zero if
     * there is no NTFS extra field.
     * @sa dateTime
     * @sa getNTFSaTime()
     * @sa getNTFScTime()
