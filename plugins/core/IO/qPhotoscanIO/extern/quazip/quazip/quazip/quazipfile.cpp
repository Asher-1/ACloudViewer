...
QString QuaZipFile::getActualFileName() const {
    ...
    if (p->zip == NULL || (openMode() & WriteOnly)) return QString();
    QString name = p->zip->getCurrentFileName();
    if (name.isNull()) p->setZipError(p->zip->getZipError());
    return name;
}
...
void QuaZipFile::setZipName(const QString &zipName) {
    if (isOpen()) {
        qWarning(
                "QuaZipFile::setZipName(): file is already open - can not set "
                "ZIP name");
        return;
    }
    if (p->zip != NULL && p->internal) delete p->zip;
...
void QuaZipFile::setFileName(const QString &fileName,
    ...
    if (!p->internal) {
        qWarning(
                "QuaZipFile::setFileName(): should not be used when not using "
                "internal QuaZip");
        return;
    }
    if (isOpen()) {
        qWarning(
                "QuaZipFile::setFileName(): can not set file name for already "
                "opened file");
        return;
    }
    p->fileName = fileName;
    if (p->fileName.startsWith('/')) p->fileName = p->fileName.mid(1);
...
void QuaZipFilePrivate::setZipError(int zipError) const {
    ...
    if (zipError == UNZ_OK)
        ...
        q->setErrorString(
                QuaZipFile::tr("ZIP/UNZIP API error %1").arg(zipError));
}
    ...
    if ((mode & ReadOnly) && !(mode & WriteOnly)) {
        if (p->internal) {
            ...
            if (p->zip->getMode() != QuaZip::mdUnzip) {
                qWarning(
                        "QuaZipFile::open(): file open mode %d incompatible "
                        "with ZIP open mode %d",
...
bool QuaZipFile::open(OpenMode mode,
    ...
    if ((mode & WriteOnly) && !(mode & ReadOnly)) {
        if (p->internal) {
            qWarning(
                    "QuaZipFile::open(): write mode is incompatible with "
        ...
        if (p->zip->getMode() != QuaZip::mdCreate &&
            ...
            qWarning(
                    "QuaZipFile::open(): file open mode %d incompatible with "
                    "ZIP open mode %d",
                    (int)mode, (int)p->zip->getMode());
            return false;
        }
        ...
        p->setZipError(zipOpenNewFileInZip3_64(
                ...
                &info_z, info.extraLocal.constData(), info.extraLocal.length(),
                info.extraGlobal.constData(), info.extraGlobal.length(),
                p->zip->getCommentCodec()
                        ->fromUnicode(info.comment)
                        .constData(),
                method, level, (int)raw, windowBits, memLevel, strategy,
...
qint64 QuaZipFile::csize() const {
    ...
    p->setZipError(unzGetCurrentFileInfo64(p->zip->getUnzFile(), &info_z, NULL,
                                           0, NULL, 0, NULL, 0));
    if (p->zipError != UNZ_OK) return -1;
