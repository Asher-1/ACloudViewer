...
private:
    ...
    bool entryInfoList(QStringList nameFilters,
                       QDir::Filters filter,
                       QDir::SortFlags sort,
...
QuaZipDir::QuaZipDir(QuaZip *zip, const QString &dir)
    ...
    if (d->dir.startsWith('/')) d->dir = d->dir.mid(1);
}
...
public:
    ...
    bool operator()(const QuaZipFileInfo64 &info1,
...
bool QuaZipDirPrivate::entryInfoList(QStringList nameFilters,
    ...
    if (fltr == QDir::NoFilter) fltr = this->filter;
    if (fltr == QDir::NoFilter) fltr = QDir::AllEntries;
    QStringList nmfltr = nameFilters;
    if (nmfltr.isEmpty()) nmfltr = this->nameFilters;
    ...
    do {
        ...
        if (relativeName.contains('/')) {
            ...
            if (dirsFound.contains(relativeName)) continue;
            isDir = true;
        }
        dirsFound.insert(relativeName);
        if ((fltr & QDir::Dirs) == 0 && isDir) continue;
        if ((fltr & QDir::Files) == 0 && !isDir) continue;
        if (!nmfltr.isEmpty() && !QDir::match(nmfltr, relativeName)) continue;
        bool ok;
        QuaZipFileInfo64 info =
    ...
    if (srt == QDir::NoSort) srt = sorting;
#ifdef QUAZIP_QUAZIPDIR_DEBUG
...
QList<QuaZipFileInfo64> QuaZipDir::entryInfoList64(
...
bool QuaZipDir::exists(const QString &filePath) const {
    if (filePath == "/" || filePath.isEmpty()) return true;
...
void QuaZipDir::setPath(const QString &path) {
    ...
    if (newDir == "/") {
        ...
        if (newDir.startsWith('/')) newDir = newDir.mid(1);
        d->dir = newDir;
    }
}
