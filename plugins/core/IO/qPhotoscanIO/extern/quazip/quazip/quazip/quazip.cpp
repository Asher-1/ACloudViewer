...
bool QuaZipPrivate::goToFirstUnmappedFile() {
    ...
    if (mode != QuaZip::mdUnzip) {
        qWarning(
                "QuaZipPrivate::goToNextUnmappedFile(): ZIP is not open in "
                "mdUnzip mode");
        return false;
    }
...
bool QuaZip::open(Mode mode, zlib_filefunc_def *ioApi) {
    ...
    if (ioDevice == NULL) {
        if (p->zipName.isEmpty()) {
            qWarning(
                    "QuaZip::open(): set either ZIP file name or IO device "
                    "first");
            return false;
        } else {
    ...
    switch (mode) {
        case mdUnzip:
            if (ioApi == NULL) {
                if (p->autoClose) flags |= UNZ_AUTO_CLOSE;
        ...
        case mdAdd:
            if (ioApi == NULL) {
                if (p->autoClose) flags |= ZIP_AUTO_CLOSE;
...
void QuaZip::close() {
        ...
        case mdAdd:
            p->zipError =
                    zipClose(p->zipFile_f,
                             p->comment.isNull()
                                     ? NULL
                                     : p->commentCodec->fromUnicode(p->comment)
                                               .constData());
            break;
        default:
    ...
    p->clearDirectoryMap();
    if (p->zipError == UNZ_OK) p->mode = mdNotOpen;
}
...
int QuaZip::getEntriesCount() const {
    ...
    if ((fakeThis->p->zipError =
                 unzGetGlobalInfo64(p->unzFile_f, &globalInfo)) != UNZ_OK)
        return p->zipError;
...
QString QuaZip::getComment() const {
    ...
    if ((fakeThis->p->zipError = unzGetGlobalComment(
...
bool QuaZip::setCurrentFile(const QString &fileName, CaseSensitivity cs) {
    ...
    if (p->hasCurrentFile_f) return p->hasCurrentFile_f;
...
bool QuaZip::goToNextFile() {
    ...
    p->hasCurrentFile_f = p->zipError == UNZ_OK;
    if (p->zipError == UNZ_END_OF_LIST_OF_FILE) p->zipError = UNZ_OK;
    return p->hasCurrentFile_f;
}
...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo64 *info) const {
    ...
    if (p->mode != mdUnzip) {
        qWarning(
                "QuaZip::getCurrentFileInfo(): ZIP is not open in mdUnzip "
                "mode");
        return false;
    }
    ...
    if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(
                 p->unzFile_f, NULL, fileName.data(), fileName.size(),
                 extra.data(), extra.size(), comment.data(), comment.size())) !=
        UNZ_OK)
        return false;
...
QString QuaZip::getCurrentFileName() const {
    ...
    if (p->mode != mdUnzip) {
        qWarning(
                "QuaZip::getCurrentFileName(): ZIP is not open in mdUnzip "
                "mode");
        return QString();
    }
    ...
    if ((fakeThis->p->zipError = unzGetCurrentFileInfo64(
                 p->unzFile_f, NULL, fileName.data(), fileName.size(), NULL, 0,
                 NULL, 0)) != UNZ_OK)
        return QString();
