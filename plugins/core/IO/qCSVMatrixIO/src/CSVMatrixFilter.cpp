									...
									LoadParameters& parameters)
{
	...
	while (file.error() == QFile::NoError && !file.atEnd())
	{
		...
		if (line.size() == 0)
		{
			CVLog::Warning(QString("[CSVMatrixFilter] Line %1 is empty (ignored)").arg(lineIndex));
			continue;
		}
		...
		{
			width = parts.size();
			CVLog::Print(QString("[CSVMatrixFilter] Detected width: %1").arg(width));
		}
		else if (width != parts.size())
		{
			CVLog::Warning(QString("[CSVMatrixFilter] Line %1 has not the same width as the previous ones!").arg(lineIndex));
	...
	if (result == CC_FERR_NO_ERROR)
	{
		...
		if (s_loadAsMesh)
		{
			...
			else
			{
				...
				{
					CVLog::Warning(QString("[CSVMatrixFilter] Failed to compute normals on mesh (not enough memory?)"));
				}
			}
		}
		...
		if (s_useTexture)
		{
			...
			{
				CVLog::Warning(QString("[CSVMatrixFilter] Failed to load texture from file '%1'").arg(filename));
			}
			else
			{
				if (mesh)
				{
					...
					{
						CVLog::Warning("[CSVMatrixFilter] Not enough memory to map the texture on the mesh!");
						texCoords->release();
						texCoords = 0;
					}
				}
				else
				{
					...
					{
						CVLog::Warning(QString("[CSVMatrixFilter] To map a texture on the cloud, the image size should equal or greater than the grid size (%1x%2)").arg(colCount).arg(rowCount));
					}
					...
					{
						CVLog::Warning("[CSVMatrixFilter] Not enough memory to map the texture on the cloud!");
					}
				}
			}
		}
		...
		CVLog::Print(QString("[CSVMatrixFilter] Number of rows: %1").arg(rowCount));
