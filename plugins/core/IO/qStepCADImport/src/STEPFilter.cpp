											...
											LoadParameters& parameters )
{
	...
	for (Standard_Integer n = 1; n <= rootCount; n++)
	{
		...
		{
            CVLog::Warning(QString("[STEP] Failed to transfer root #%1").arg(n));
		}
	}
	// Root transfers
    ...
    CVLog::Print("[STEP] Number of triangles (after tesselation) = " + QString::number(triCount));
    CVLog::Print("[STEP] Number of vertices (after tesselation)  = " + QString::number(vertCount));
