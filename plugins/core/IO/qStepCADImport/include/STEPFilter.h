...
// ##########################################################################
// #                                                                        #
// #                 CloudViewer PLUGIN: qSTEPCADImport                    #
// #                                                                        #
...
public:
    ...
    CC_FILE_ERROR loadFile(const QString& fullFilename,
                           ccHObject& container,
    ...
    /** \param value linear deflection (in [1e-2, 1e-6])
                    The smaller this value is, the smaller the triangles will
    be. In some cases, if this linear deflection is too big, the tesselation may
    crash (precisely the instruction BRep_Tool::Triangulation(face, location)).
