...
bool PNFilter::canSave(CV_CLASS_ENUM type,
                       bool& multiple,
...
CC_FILE_ERROR PNFilter::saveToFile(ccHObject* entity,
    ...
    if (!theCloud) {
        CVLog::Warning("[PN] This filter can only save one cloud at a time!");
        return CC_FERR_BAD_ENTITY_TYPE;
    }
    unsigned numberOfPoints = theCloud->size();

    if (numberOfPoints == 0) {
        CVLog::Warning("[PN] Input cloud is empty!");
    ...
    if (theCloud->isShifted())
        CVLog::Warning(QString("[PNFilter::save] Can't recenter or rescale "
                               "cloud '%1' when saving it in a PN file!")
                               .arg(theCloud->getName()));

    bool hasNorms = theCloud->hasNormals();
    if (!hasNorms)
        CVLog::Warning(QString("[PNFilter::save] Cloud '%1' has no normal (we "
                               "will save points with a default normal)!")
                               .arg(theCloud->getName()));
    ...
    for (unsigned i = 0; i < numberOfPoints; i++) {
        // write point
        {
            ...
            if (out.write(reinterpret_cast<const char*>(Pfloat.u),
                          3 * sizeof(float)) < 0) {
        ...
        if (out.write(reinterpret_cast<const char*>(norm), 3 * sizeof(float)) <
            0) {
            result = CC_FERR_WRITING;
            break;
        }
...
CC_FILE_ERROR PNFilter::loadFile(const QString& filename,
                                 ccHObject& container,
    ...
    for (unsigned i = 0; i < numberOfPoints; i++) {
        ...
        if (pointsRead == fileChunkPos + fileChunkSize) {
            if (loadedCloud) container.addChild(loadedCloud);
