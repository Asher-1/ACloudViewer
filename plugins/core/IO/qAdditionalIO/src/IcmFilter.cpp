...
CC_FILE_ERROR IcmFilter::loadFile(const QString& filename,
                                  ccHObject& container,
                                  LoadParameters& parameters) {
    if (CheckForSpecialChars(filename)) {
        CVLog::Warning(
                QString("[ICM] Input filename contains special characters. It "
                        "might be rejected by the I/O filter..."));
    }
    ...
    if (!filter) {
        CVLog::Warning(QString("[ICM] No I/O filter found for loading file "
                               "'%1' (type = '%2')")
                               .arg(cloudFileName, subFileType));
        fclose(fp);
    ...
    ccHObject* entities = FileIOFilter::LoadFromFile(
            QString("%1/%2").arg(path, cloudFileName), parameters, filter,
            result);
    if (!entities) {
        fclose(fp);
        return CC_FERR_READING;
    ...
    if (!fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp)) {
        CVLog::Error("[ICM] Read error (IMAGES_DESCRIPTOR)! No image loaded");
        fclose(fp);
        return CC_FERR_READING;
    } else {
        ...
        int n = LoadCalibratedImages(entities, path, imagesDescriptorFileName,
                                     entities->getBB_recursive());
        CVLog::Print("[ICM] %i image(s) loaded ...", n);
    }
...
int IcmFilter::LoadCalibratedImages(ccHObject* entities,
                                    const QString& path,
    ...
    // ouverture du fichier
    QString completeImageDescFilename =
            QString("%1/%2").arg(path, imageDescFilename);

    if (CheckForSpecialChars(completeImageDescFilename)) {
        CVLog::Warning(QString("[ICM] File '%1' contains special characters. "
                               "It might be rejected by the I/O filter...")
                               .arg(completeImageDescFilename));
    }

    FILE* fp = fopen(qPrintable(completeImageDescFilename), "rt");
    if (fp == nullptr) {
        CVLog::Error(QString("[IcmFilter::loadCalibratedImages] Error opening "
                             "file %1!")
                             .arg(completeImageDescFilename));
        return -1;
    }
    ...
    while (fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp) != nullptr) {
        if (line[0] == 'D' && line[1] == 'E' && line[2] == 'F') {
            ...
            QString errorStr;
            if (!CI->load(QString("%1/%2").arg(path, imageFileName),
                          errorStr)) {
                CVLog::Warning(QString("[IcmFilter] Failed to load image %1 "
                                       "(%2)! Process stopped...")
                                       .arg(imageFileName, errorStr));
                delete CI;
                fclose(fp);
                return loadedImages;
            }

            CVLog::Print("[IcmFilter] Image '%s' loaded", imageFileName);
            CI->setEnabled(false);
            ...
            // FOV
            if (!fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp)) {
                CVLog::Print("[IcmFilter] Read error (fieldOfView)!");
                delete CI;
                fclose(fp);
                return loadedImages;
            }
            ...
            float t[3];
            if (!fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp)) {
                CVLog::Error("[IcmFilter] Read error (position)!");
                // delete CI;
                fclose(fp);
                return loadedImages;
            }
            ...
            CVLog::Print("\t Camera pos=(%f,%f,%f)", t[0], t[1], t[2]);

            // Description
            char desc[MAX_ASCII_FILE_LINE_LENGTH];
            if (!fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp)) {
                CVLog::Error("[IcmFilter] Read error (description)!");
                // delete CI;
                fclose(fp);
                return loadedImages;
            }
            ...
            CVLog::Print("\t Description: '%s'", desc);

            // Orientation
            float axis[3], angle_rad;
            if (!fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp)) {
                CVLog::Error("[IcmFilter] Read error (orientation)!");
                fclose(fp);
                return loadedImages;
            }
            ...
            CVLog::Print("\t Camera orientation=(%f,%f,%f)+[%f]", axis[0],
                         axis[1], axis[2], angle_rad);
            ...
            ccGLMatrix mat;
            mat.initFromParameters(angle_rad, CCVector3::fromArray(axis),
                                   CCVector3::fromArray(t));
