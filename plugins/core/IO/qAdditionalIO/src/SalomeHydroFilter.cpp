...
bool SalomeHydroFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
CC_FILE_ERROR SalomeHydroFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	try
	{
		...
		else if (entity->isA(CV_TYPES::HIERARCHY_OBJECT))
		{
			for (unsigned i=0; i<entity->getChildrenNumber(); ++i)
				if (entity->getChild(i) && entity->getChild(i)->isA(CV_TYPES::POLY_LINE))
	...
	for (size_t i=0; i<candidates.size(); ++i)
	{
		...
		{
			//invalid size
			CVLog::Warning(QString("[Salome Hydro] Polyline '%1' does not have enough vertices")
...
CC_FILE_ERROR SalomeHydroFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	while (true)
	{
		...
		if (currentLine.isNull() || currentLine.isEmpty())
		{
			...
			if (currentVertices)
			{
				if (currentVertices->size() < 2)
				{
					...
					CVLog::Warning("[Salome Hydro] An invalid polyline (single vertex) will be ignored");
				}
				else
				{
		...
		else
		{
			...
			if (parts.size() == 3)
			{
				...
				if (firstPoint)
				{
					if (HandleGlobalShift(P, Pshift, preserveCoordinateShift, parameters))
					{
						...
						CVLog::Warning("[Salome Hydro] Polylines will be recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
					}
					firstPoint = false;
				}
			...
			{
				CVLog::Warning("[Salome Hydro] Malformed file: 3 values per line expected");
