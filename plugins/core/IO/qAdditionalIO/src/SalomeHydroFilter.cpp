...
SalomeHydroFilter::SalomeHydroFilter()
    : FileIOFilter({"_SalomeHydro Filter",
                    ...
                    QStringList{"poly"}, "poly",
                    QStringList{"Salome Hydro polylines (*.poly)"},
                    QStringList{"Salome Hydro polylines (*.poly)"},
                    Import | Export}) {}

bool SalomeHydroFilter::canSave(CV_CLASS_ENUM type,
                                bool& multiple,
...
CC_FILE_ERROR SalomeHydroFilter::saveToFile(ccHObject* entity,
    ...
    if (candidates.empty()) return CC_FERR_NO_SAVE;
    ...
    for (size_t i = 0; i < candidates.size(); ++i) {
        ...
        if (vertCount < 2) {
            // invalid size
            CVLog::Warning(QString("[Salome Hydro] Polyline '%1' does not have "
                                   "enough vertices")
...
CC_FILE_ERROR SalomeHydroFilter::loadFile(const QString& filename,
                                          ccHObject& container,
        ...
        if (currentLine.isNull() || currentLine.isEmpty()) {
            ...
            if (currentVertices) {
                if (currentVertices->size() < 2) {
                    ...
                    CVLog::Warning(
                            "[Salome Hydro] An invalid polyline (single "
                            "vertex) will be ignored");
                } else {
                    ...
                    newPoly->setName(
                            QString(QString("Polyline #%1").arg(++index)));
            ...
            if (parts.size() == 3) {
                ...
                if (firstPoint) {
                    if (HandleGlobalShift(P, Pshift, preserveCoordinateShift,
                        ...
                        CVLog::Warning(
                                "[Salome Hydro] Polylines will be recentered! "
                ...
                if (currentVertices->size() == currentVertices->capacity()) {
                    if (!currentVertices->reserve(currentVertices->size() +
                                                  64)) {
                        delete currentVertices;
                        currentVertices = 0;
                        result = CC_FERR_NOT_ENOUGH_MEMORY;
                ...
                CVLog::Warning(
                        "[Salome Hydro] Malformed file: 3 values per line "
                        "expected");
