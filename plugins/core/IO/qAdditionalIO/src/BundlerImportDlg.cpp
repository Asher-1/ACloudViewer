...
BundlerImportDlg::BundlerImportDlg(QWidget* parent) : QDialog(parent) {
    ...
    connect(buttonBox, &QDialogButtonBox::accepted, this,
            &BundlerImportDlg::acceptAndSaveSettings);
    connect(browseImageListFileToolButton, &QAbstractButton::clicked, this,
            &BundlerImportDlg::browseImageListFilename);
    connect(browseAltKeypointsFileToolButton, &QAbstractButton::clicked, this,
            &BundlerImportDlg::browseAltKeypointsFilename);
}
...
void BundlerImportDlg::initFromPersistentSettings() {
    ...
    // read parameters
    double scaleFactor =
            settings.value("scaleFactor", imageScaleDoubleSpinBox->value())
                    .toDouble();
    bool orthoRectifyAsCloud =
            settings.value("orthoRectifyAsClouds",
                           orthoRectifyAsCloudCheckBox->isChecked())
                    .toBool();
    bool orthoRectifyAsImage =
            settings.value("orthoRectifyAsImages",
                           orthoRectifyAsImageCheckBox->isChecked())
                    .toBool();
    bool undistortImages = settings.value("undistortImages",
                                          undistortImagesCheckBox->isChecked())
                                   .toBool();
    bool generateColoredDTM =
            settings.value("generateColoredDTM",
                           generateColoredDTMGroupBox->isChecked())
                    .toBool();
    bool keepImagesInMemory =
            settings.value("keepImagesInMemory",
                           keepImagesInMemoryCheckBox->isChecked())
                    .toBool();
    bool importImages =
            settings.value("importImages", imagesGroupBox->isChecked())
                    .toBool();
    bool useAltKeypoints = /*settings.value("useAltKeypoints",
                              altKeypointsCheckBox->isChecked()).toBool()*/
            false;  // DGM: if we don't handle the filename, it's too dangerous
    bool importKeypoints = settings.value("importKeypoints",
                                          importKeypointsGroupBox->isChecked())
                                   .toBool();
    int dtmVerticesCount =
            settings.value("dtmVerticesCount", dtmVerticesSpinBox->value())
                    .toInt();
    int orthoRectMethod =
            settings.value("orthoRectMethod",
                           orthoRectMethodComboBox->currentIndex())
                    .toInt();
...
void BundlerImportDlg::acceptAndSaveSettings() {
    ...
    if (customVertAxisRadioButton->isChecked()) {
        bool success;
        ccGLMatrixd::FromString(applyTransfoMatrixTextEdit->toPlainText(),
                                success);
        if (!success) {
            QMessageBox::critical(this, "Invalid matrix",
                                  "Invalid input 4x4 matrix!");
            return;
        }
    }
    ...
    settings.setValue("orthoRectifyAsClouds",
                      orthoRectifyAsCloudCheckBox->isChecked());
    settings.setValue("orthoRectifyAsImages",
                      orthoRectifyAsImageCheckBox->isChecked());
    settings.setValue("undistortImages", undistortImagesCheckBox->isChecked());
    settings.setValue("generateColoredDTM",
                      generateColoredDTMGroupBox->isChecked());
    settings.setValue("keepImagesInMemory",
                      keepImagesInMemoryCheckBox->isChecked());
    ...
    settings.setValue("orthoRectMethod",
                      orthoRectMethodComboBox->currentIndex());
...
bool BundlerImportDlg::importKeypoints() const {
    return importKeypointsGroupBox->isEnabled() &&
           importKeypointsGroupBox->isChecked();
}
...
bool BundlerImportDlg::undistortImages() const {
    return imagesGroupBox->isEnabled() &&
           undistortImagesCheckBox->isEnabled() &&
           undistortImagesCheckBox->isChecked();
}
...
BundlerImportDlg::OrthoRectMethod
BundlerImportDlg::getOrthorectificationMethod() const {
...
bool BundlerImportDlg::getOptionalTransfoMatrix(ccGLMatrix& mat) {
    if (xVertAxisRadioButton->isChecked()) {
        ...
        bool success = false;
        mat = ccGLMatrix::FromString(applyTransfoMatrixTextEdit->toPlainText(),
                                     success);
        return success;
    }
