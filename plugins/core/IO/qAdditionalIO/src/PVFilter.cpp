...
PVFilter::PVFilter()
    : FileIOFilter(
              {"_Point+Value Filter",
               ...
               QStringList{"pv"}, "pv", QStringList{"Point+Value cloud (*.pv)"},
               QStringList{"Point+Value cloud (*.pv)"}, Import | Export}) {}

bool PVFilter::canSave(CV_CLASS_ENUM type,
                       bool& multiple,
...
CC_FILE_ERROR PVFilter::saveToFile(ccHObject* entity,
    ...
    if (theCloud->isShifted())
        CVLog::Warning(QString("[PVFilter::save] Can't recenter or rescale "
                               "cloud '%1' when saving it in a PN file!")
                               .arg(theCloud->getName()));

    // for point clouds with multiple SFs, we must set the currently displayed
    // one as 'input' SF if (theCloud->isA(CV_TYPES::POINT_CLOUD))
    //{
    ...
    if (!hasSF)
        CVLog::Warning(QString("[PVFilter::save] Cloud '%1' has no displayed "
                               "scalar field (we will save points with a "
    ...
    for (unsigned i = 0; i < numberOfPoints; i++) {
        ...
        // write scalar value
        if (hasSF) val = static_cast<float>(theCloud->getPointScalarValue(i));
...
CC_FILE_ERROR PVFilter::loadFile(const QString& filename,
                                 ccHObject& container,
    ...
    if ((fileSize % singlePointSize) != 0) return CC_FERR_MALFORMED_FILE;
            ...
            if (loadedCloud) {
                ...
                if (sfIdx >= 0) {
                    cloudViewer::ScalarField* sf =
                            loadedCloud->getScalarField(sfIdx);
                    sf->computeMinAndMax();
