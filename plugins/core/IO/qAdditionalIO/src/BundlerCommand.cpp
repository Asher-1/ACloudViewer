...
static const char COMMAND_BUNDLER[] =
        "BUNDLER_IMPORT";  // Import Bundler file + orthorectification
...
bool BundlerCommand::process(ccCommandLineInterface &cmd) {
            ...
            if (cmd.arguments().empty())
                return cmd.error(
                        QObject::tr("Missing parameter: value after \"-%1\"")
                                .arg(COMMAND_BUNDLER_SCALE_FACTOR));
            ...
            if (!conversionOk)
                return cmd.error(
                        QObject::tr("Invalid parameter: value after \"-%1\"")
                           ...
                           argument, COMMAND_BUNDLER_UNDISTORT)) {
            // local option confirmed, we can move on
            cmd.arguments().pop_front();

            undistortImages = true;
        } else if (ccCommandLineInterface::IsCommand(
                           argument, COMMAND_BUNDLER_COLOR_DTM)) {
            // local option confirmed, we can move on
            cmd.arguments().pop_front();

            if (cmd.arguments().empty())
                return cmd.error(QObject::tr("Missing parameter: vertices "
                                             "count after \"-%1\"")
                                         .arg(COMMAND_BUNDLER_COLOR_DTM));
            bool conversionOk = false;
            coloredDTMVerticesCount =
                    cmd.arguments().takeFirst().toUInt(&conversionOk);
            if (!conversionOk)
                return cmd.error(QObject::tr("Invalid parameter: vertices "
                                             "count after \"-%1\"")
                                         .arg(COMMAND_BUNDLER_COLOR_DTM));
    ...
    BundlerFilter().loadFileExtended(qPrintable(bundlerFilename), tempContainer,
                                     parameters, altKeypointsFilename,
                                     undistortImages, generateColoredDTM,
                                     coloredDTMVerticesCount, scaleFactor);
    return true;
}
