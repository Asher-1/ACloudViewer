...
bool PovFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
CC_FILE_ERROR PovFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	std::vector<ccGenericPointCloud*> clouds;
	{
		for (unsigned i = 0; i < hClouds.size(); ++i)
		{
			...
			hClouds[i]->filterChildren(cloudSensors, false, CV_TYPES::GBL_SENSOR);
			if (!cloudSensors.empty())
			{
				clouds.push_back(ccHObjectCaster::ToGenericPointCloud(hClouds[i]));
				if (cloudSensors.size() > 1)
					CVLog::Warning(QString("Found more than one GBL sensor associated to entity '%1'. Only the first will be saved!").arg(hClouds[i]->getName()));
...
CC_FILE_ERROR PovFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	{
		CVLog::Warning("[PovFilter::loadFile] Unhandled rotation order description! (%s)", sensorType);
		fclose(fp);
		return CC_FERR_READING;
	}
	...
	CVLog::Print("[PovFilter::loadFile] POV FILE [Type %s - base=%f - unit: %s]", sensorType, base, unitsType);
	...
	while (fgets(line, MAX_ASCII_FILE_LINE_LENGTH, fp))
	{
		if ((line[0] == '#') && (line[1] == 'P'))
		{
			...
			if (!filter)
			{
				CVLog::Warning(QString("[POV] No I/O filter found for loading file '%1' (type = '%2')").arg(subFileName, subFileType));
				fclose(fp);
			...
			if (entities)
			{
				...
				for (size_t i = 0; i < clouds.size(); ++i)
				{
					...
					{
						CVLog::Warning(QString("[PovFilter::loadFile] failed to create sensor on cloud #%1 (%2)").arg(i).arg(theCloud->getName()));
						delete gls;
						gls = nullptr;
					}
			...
			else
			{
				...
				{
					CVLog::Print("[PovFilter::loadFile] File (%s) not found or empty!", subFileName);
				}
			}
		}
	}
