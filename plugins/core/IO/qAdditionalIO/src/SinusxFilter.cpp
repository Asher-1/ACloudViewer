...
SinusxFilter::SinusxFilter()
    : FileIOFilter({"_Sinusx Filter",
                    ...
                    QStringList{"sx", "sinusx"}, "sx",
                    QStringList{"Sinusx curve (*.sx)"},
                    QStringList{"Sinusx curve (*.sx)"}, Import | Export}) {}

bool SinusxFilter::canSave(CV_CLASS_ENUM type,
                           bool& multiple,
...
CC_FILE_ERROR SinusxFilter::saveToFile(ccHObject* entity,
    ...
    // write header
    outFile << "C Generated by CLOUDVIEWER " << QtCompat::endl;
    ...
    for (size_t i = 0; i < profiles.size(); ++i) {
        ...
        if (vertCount < 2) {
            // invalid size
            CVLog::Warning(QString("[Sinusx] Polyline '%1' does not have "
                                   "enough vertices")
        ...
        if (is2D && poly->hasMetaData(ccPolyline::MetaKeyUpDir())) {
            ...
            if (!ok) upDir = 2;  // restore default value
        }

        // new block
        outFile << "B S"
                << QtCompat::endl;  // B + curve type (S = set of 3D points)
        outFile << "CN " << poly->getName() << QtCompat::endl;  // CN + name
        outFile << "CP 1 " << (poly->isClosed() ? 1 : 0)
                << QtCompat::endl;  // CP + isLinked + isClosed
        outFile << "CP " << (upDir == 2 ? 0 : (upDir == 1 ? 2 : 1))
                << QtCompat::endl;  // base plane: 0 = (XY), 1 = (YZ), 2 = (ZX)
...
CC_FILE_ERROR SinusxFilter::loadFile(const QString& filename,
                                     ccHObject& container,
    ...
    while (!currentLine.isEmpty() && file.error() == QFile::NoError) {
        ...
        if (currentLine.startsWith("C ")) {
            ...
            if (curveType == INVALID) {
                CVLog::Warning(QString("[SinusX] Unhandled curve type '%1' on "
                                       "line '%2'!")
                                       .arg(curveTypeChar)
                                       .arg(lineNumber));
                result = CC_FERR_MALFORMED_FILE;
            ...
            // TODO: what about the local coordinate system and scale?! (7 last
            // values)
            if (tokens.size() > 7) {
            }
            ...
            if (currentLine.startsWith("CN")) {
                ...
                switch (cpIndex) {
                    case 0:  // first 'CP' line
                    {
                        ...
                        if (ok) {
                            ...
                            if (ok) {
                                if (isConnected == 0) {
                                    ...
                                    //--> we simply hide the polyline and
                                    // display its vertices
                        ...
                        if (!ok) {
                            CVLog::Warning(
                                    QString("[SinusX] Line %1 is corrupted "
                                            "(expected: 'CP connected_flag "
                                            "closed_flag')")
                                            .arg(lineNumber));
                            result = CC_FERR_MALFORMED_FILE;
                            continue;
                    ...
                    case 1:  // second 'CP' line
                    {
                        if (curveType == CUREV_S) {
                            ++cpIndex;
                            // no break: we go directly to the next case
                            // (cpIndex = 2)
                        } else if (curveType == CURVE_P) {
                            // nothing particular for profiles (they are not
                            // handled in the same way in CC!)
                            ++cpIndex;
                            break;
                            ...
                            if (ok) {
                                ...
                                if (ok)
                                    currentPoly->setMetaData(
                                            ccPolyline::MetaKeyConstAltitude(),
                                            QVariant(z));
                            }
                            if (!ok) {
                                CVLog::Warning(QString("[SinusX] Line %1 is "
                                                       "corrupted (expected: "
                                                       "'CP const_altitude')")
                                                       .arg(lineNumber));
                                result = CC_FERR_MALFORMED_FILE;
                            ...
                            int skipped = tokens.size() -
                                          1;  // all but the 'CP' keyword
                            while (skipped < 16 && !currentLine.isEmpty() &&
                    ...
                    case 2: {
                        ...
                        if (ok) {
                            ...
                            if (ok)
                                currentPoly->setMetaData(
                                        ccPolyline::MetaKeyUpDir(),
                                        QVariant(vertDir));
                        }
                        if (!ok) {
                            CVLog::Warning(
                                    QString("[SinusX] Line %1 is corrupted "
                                            "(expected: 'CP base_plane')")
                                            .arg(lineNumber));
                            result = CC_FERR_MALFORMED_FILE;
                            continue;
                        }
                    ...
                    if (ok) {
                        ...
                        if (ok) {
                            ...
                            if (ok) {
                                ...
                                if (currentVertices->size() ==
                                            currentVertices->capacity() &&
                                    !currentVertices->reserve(
                                ...
                                if (firstVertex/*currentVertices->size() == 0*/)
								{
                                    ...
                                    if (HandleGlobalShift(
                                                Pd, Pshift,
                                                preserveCoordinateShift,
                                                parameters)) {
                                        if (preserveCoordinateShift) {
                                            if (currentPoly)
                                                currentPoly->setGlobalShift(
                                                        Pshift);
                                            else
                                                currentVertices->setGlobalShift(
                                                        Pshift);
                                        }
                                        CVLog::Warning(
                                ...
                                currentVertices->addPoint(
                                        CCVector3::fromArray((Pd + Pshift).u));
                            }
                        }
                    }
                }
