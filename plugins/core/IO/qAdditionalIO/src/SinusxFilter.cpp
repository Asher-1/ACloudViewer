...
bool SinusxFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
CC_FILE_ERROR SinusxFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	for (size_t i = 0; i < profiles.size(); ++i)
	{
		...
		{
			//invalid size
			CVLog::Warning(QString("[Sinusx] Polyline '%1' does not have enough vertices")
...
CC_FILE_ERROR SinusxFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	while (!currentLine.isEmpty() && file.error() == QFile::NoError)
	{
		...
		else if (currentLine.startsWith("B"))
		{
			...
			{
				CVLog::Warning(QString("[SinusX] Line %1 is corrupted").arg(lineNumber));
			...
			{
				CVLog::Warning(QString("[SinusX] Unhandled curve type '%1' on line '%2'!").arg(curveTypeChar).arg(lineNumber));
				result = CC_FERR_MALFORMED_FILE;
				continue;
			}
		...
		else if (currentPoly)
		{
				...
				case 0: //first 'CP' line
					{
						...
						if (!ok)
						{
							CVLog::Warning(QString("[SinusX] Line %1 is corrupted (expected: 'CP connected_flag closed_flag')").arg(lineNumber));
							result = CC_FERR_MALFORMED_FILE;
							continue;
						}
						++cpIndex;
				...
				case 1: //second 'CP' line
					{
						...
						else if (curveType == CURVE_N)
						{
							...
							if (!ok)
							{
								CVLog::Warning(QString("[SinusX] Line %1 is corrupted (expected: 'CP const_altitude')").arg(lineNumber));
								result = CC_FERR_MALFORMED_FILE;
								continue;
							}
							++cpIndex;
				...
				case 2:
					{
						...
						if (!ok)
						{
							CVLog::Warning(QString("[SinusX] Line %1 is corrupted (expected: 'CP base_plane')").arg(lineNumber));
							result = CC_FERR_MALFORMED_FILE;
							continue;
						}
					}
			...
			else if (!currentLine.isEmpty())
			{
							...
							if (ok)
							{
								...
								if (firstVertex/*currentVertices->size() == 0*/)
								{
									...
									if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, parameters))
									{
										...
										CVLog::Warning("[SinusX::loadFile] Polyline has been recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
									}
								}
				...
				if (!ok)
				{
					CVLog::Warning(QString("[SinusX] Line %1 is corrupted (expected: 'X Y Z Key ...')").arg(lineNumber));
					result = CC_FERR_MALFORMED_FILE;
					continue;
				}
			}
