...
namespace LasDetails
{
	...
	const std::vector<unsigned>* PointFormatsAvailableForVersion(QString version)
	{
		...
		CVLog::Warning("Unknown LAS version: " + version);
		return nullptr;
	}
	...
	LasVersion SelectBestVersion(const ccPointCloud& cloud)
	{
		...
		if (!isExtendedRequired)
		{
			...
			{
				const cloudViewer::ScalarField* classification = cloud.getScalarField(classificationIdx);
			...
			{
				const cloudViewer::ScalarField* returnNumber = cloud.getScalarField(returnNumberIdx);
			...
			if (numReturnsIdx != -1)
			{
				const cloudViewer::ScalarField* numReturns = cloud.getScalarField(numReturnsIdx);
