...
CC_FILE_ERROR TileLasReader(laszip_POINTER laszipReader, const QString& originName, const LasTilingOptions& options)
{
	...
	ecvProgressDialog progressDialog(true);
	...
	for (unsigned i = 0; i < pointCount; ++i)
	{
		...
		if (writer == nullptr)
		{
			...
			{
				CVLog::Warning("[LAS] Failed to create tile writer");
				error = CC_FERR_THIRD_PARTY_LIB_FAILURE;
				break;
			}
	...
	if (error == CC_FERR_NO_ERROR)
	{
		...
		CVLog::Print(QString("[LAS] File tiled in %1m%2s%3ms").arg(minutes).arg(seconds).arg(elapsed_ms));
	}
