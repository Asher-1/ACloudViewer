...
void SaveImage(const ccImage* image, const QString& scanGUID, e57::ImageFile& imf, e57::VectorNode& images2D)
{
	...
	//Description
	//imageNode.set("description", e57::StringNode(imf, "Imported from ACloudViewer (EDF R&D / Telecom ParisTech)"));
	...
	try
	{
		...
		if (result == CC_FERR_NO_ERROR)
		{
			...
			for (size_t i = 0; i < scanCount; ++i)
			{
				...
				{
					//progress bar
					cloudViewer::NormalizedProgress nprogress(progressDlg.data(), imageCount);
					if (progressDlg)
					{
						progressDlg->setMethodTitle(QObject::tr("Write E57 file"));
...
static ccHObject* LoadScan(const e57::Node& node, QString& guidStr, ecvProgressDialog* progressDlg = nullptr)
{
	...
	//local progress bar
	cloudViewer::NormalizedProgress nprogress(progressDlg, static_cast<unsigned>(pointCount / chunkSize));
	if (progressDlg)
	{
...
CC_FILE_ERROR E57Filter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		if (root.isDefined("/data3D"))
		{
			...
			cloudViewer::NormalizedProgress nprogress(progressDlg.data(), showGlobalProgress ? scanCount : 100);
