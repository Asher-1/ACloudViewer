...
static void SavePoseInformation(e57::StructureNode& parentNode, const e57::ImageFile& imf, const ccGLMatrixd& poseMat)
{
	...
	CVLib::SquareMatrixd transMat(poseMat.data(), true);
	double q[4];
...
static bool SaveScan(ccPointCloud* cloud, e57::StructureNode& scanNode, e57::ImageFile& imf, e57::VectorNode& data3D, QString& guidStr, ecvProgressDialog* progressDlg = nullptr)
{
	...
	//progress bar
	CVLib::NormalizedProgress nprogress(progressDlg, pointCount);
	if (progressDlg)
	{
...
CC_FILE_ERROR E57Filter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	try
	{
		...
		{
			progressDlg.reset(new ecvProgressDialog(true, parameters.parentWidget));
		...
		if (result == CC_FERR_NO_ERROR)
		{
			...
			for (size_t i = 0; i < scanCount; ++i)
			{
				...
				unsigned imageCount = cloud->filterChildren(images, false, CV_TYPES::IMAGE);

				if (imageCount != 0)
				{
					//progress bar
					CVLib::NormalizedProgress nprogress(progressDlg.data(), imageCount);
					if (progressDlg)
					{
						progressDlg->setMethodTitle(QObject::tr("Write E57 file"));
...
static ccHObject* LoadScan(const e57::Node& node, QString& guidStr, ecvProgressDialog* progressDlg = nullptr)
{
	...
	//log
	CVLog::Print(QString("[E57] Reading new scan node (%1)").arg(scanNode.elementName().c_str()));

	if (!scanNode.isDefined("points"))
	{
		CVLog::Warning(QString("[E57Filter] No point in scan '%1'!").arg(scanNode.elementName().c_str()));
		return nullptr;
	}
		...
		!header.pointFields.cartesianZField)
	{
		...
		{
			CVLog::Warning(QString("[E57Filter] No readable point in scan '%1'! (only cartesian and spherical coordinates are supported right now)").arg(scanNode.elementName().c_str()));
			return nullptr;
		}
		sphericalMode = true;
	}
	...
	if (validPoseMat)
	{
		...
		if (FileIOFilter::HandleGlobalShift(T, Tshift, preserveCoordinateShift, s_loadParameters))
		{
			...
			CVLog::Warning("[E57Filter::loadFile] Cloud %s has been recentered! Translation: (%.2f ; %.2f ; %.2f)", qPrintable(guidStr), Tshift.x, Tshift.y, Tshift.z);
		}
	...
	//local progress bar
	CVLib::NormalizedProgress nprogress(progressDlg, static_cast<unsigned>(pointCount / chunkSize));
	if (progressDlg)
	{
		...
		for (unsigned i = 0; i < size; ++i)
		{
				...
				&& (!validPoseMat || !poseMatWasShifted) )
			{
				...
				if (FileIOFilter::HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, s_loadParameters))
				{
					...
					CVLog::Warning("[E57Filter::loadFile] Cloud %s has been recentered! Translation: (%.2f ; %.2f ; %.2f)", qPrintable(guidStr), Pshift.x, Pshift.y, Pshift.z);
				}
			}
	...
	if (realCount == 0)
	{
		CVLog::Warning(QString("[E57] No valid point in scan '%1'!").arg(scanNode.elementName().c_str()));
		delete cloud;
		return nullptr;
	}
	...
	if (returnIndexSF)
	{
		...
		CVLog::Warning("[E57] Cloud has multiple echoes: use 'Edit > Scalar Fields > Filter by value' to extract one component");
		cloud->showSF(true);
	}
...
static ccHObject* LoadImage(const e57::Node& node, QString& associatedData3DGuid)
{
	...
	//log
	CVLog::Print(QString("[E57] Reading new image node (%1)").arg(imageNode.elementName().c_str()));
	...
	if (imageNode.isDefined("description"))
		CVLog::Print(QString("[E57] Description: %1").arg(e57::StringNode(imageNode.get("description")).value().c_str()));
	...
	{
		CVLog::Warning(QString("[E57] Image %1 has no associated camera representation!").arg(name.c_str()));
		return nullptr;
	}
	...
	{
		CVLog::Warning("[E57] Image format not handled (only jpg and png for the moment)");
		return nullptr;
	}
...
//#endif
	...
	case E57_CYLINDRICAL:
	case E57_SPHERICAL:
		CVLog::Warning("[E57] Unhandled camera type (image will be loaded as is)");
...
CC_FILE_ERROR E57Filter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		if (root.isDefined("/data3D"))
		{
			...
			CVLib::NormalizedProgress nprogress(progressDlg.data(), showGlobalProgress ? scanCount : 100);
