...
AssimpIfc::AssimpIfc()
    : IoAbstractLoader({"_IFC Filter", FileIOFilter::DEFAULT_PRIORITY,
                        QStringList{"ifc", "stp", "step"}, "ifc",
...
void AssimpIfc::_recursiveRename(ccHObject *ioContainer) {
    ...
    if (ioContainer->getName() == QLatin1String("$RelAggregates")) {
        ioContainer->setName(
                QCoreApplication::translate("qMeshIO", "Unnamed Group"));
    } else {
        ...
        if (match.hasMatch()) {
            ...
            // Then rename our node with "name" put "type" and "GUID" into our
            // meta data
