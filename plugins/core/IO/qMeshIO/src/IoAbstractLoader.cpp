...
class _ProgressHandler : public QProgressDialog,
...
public:
    ...
    bool Update(float inPercent) override {
        ...
        setLabelText(QStringLiteral("Loading %1: %2%")
                             .arg(cText, QString::number(value)));
...
public:
    ...
    void load(ccHObject &ioContainer) {
        if (cScene->HasMeshes()) {
            CVLog::Print(QStringLiteral("[qMeshIO] The file '%1' has %2 meshes")
                                 .arg(cFileName, QLocale::system().toString(
                                                         cScene->mNumMeshes)));
        }

        if (cScene->HasCameras()) {
            CVLog::Print(
                    QStringLiteral("[qMeshIO] The file '%1' has %2 cameras")
                            .arg(cFileName, QLocale::system().toString(
                                                    cScene->mNumCameras)));
        }
...
private:
    ...
    void _recursiveAddNode(const aiNode *inNode,
                           const aiScene *inScene,
                           ccHObject *ioParentObject) {
#ifdef QT_DEBUG
        ...
        if (cNodeHasTransform) {
            ccGLMatrix transform =
                    IoUtils::convertMatrix(inNode->mTransformation);
        ...
        if (inNode->mMetaData != nullptr) {
            ...
            for (unsigned int i = 0; i < data->mNumProperties; ++i) {
                ...
                QVariant metaValue =
                        IoUtils::convertMetaValueToVariant(data, i);

#ifdef QT_DEBUG
                std::cout << "Setting meta: " << cMetaKey << " = "
                          << metaValue.toString().toLatin1().constData()
                          << std::endl;
#endif
    ...
    void _pruneTree(ccHObject *ioCurrentObject) {
        ...
        // Because the indices change when we delete children, save a list and
        ...
        // If we are not a "naked" hierarchy object, then we contain useful
        // info, so return
        ...
        if (childCount == 0) {
#ifdef QT_DEBUG
            std::cout << "pruning: "
                      << ioCurrentObject->getName().toLatin1().constData()
                      << "  from parent: "
            ...
            if (child != nullptr) {
#ifdef QT_DEBUG
                std::cout << "reparenting: "
                          << child->getName().toLatin1().constData() << " from "
                          << ioCurrentObject->getName().toLatin1().constData()
...
CC_FILE_ERROR IoAbstractLoader::loadFile(
        const QString &inFileName,
    ...
    if (cScene == nullptr) {
        CVLog::Warning(QStringLiteral("[qMeshIO] The file '%1' has errors: %2")
                               .arg(cFileName, importer.GetErrorString()));
