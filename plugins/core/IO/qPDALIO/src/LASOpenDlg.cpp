...
LASOpenDlg::LASOpenDlg(QWidget* parent)
    ...
    connect(applyAllButton, &QAbstractButton::clicked, this,
            &LASOpenDlg::onApplyAll);
    connect(browseToolButton, &QAbstractButton::clicked, this,
            &LASOpenDlg::onBrowse);
    connect(tileGroupBox, &QGroupBox::toggled, applyAllButton,
            &QWidget::setDisabled);
...
void LASOpenDlg::resetApplyAll() { m_autoSkip = false; }
...
bool FieldIsPresent(const std::vector<std::string>& dimensions,
    ...
    for (const std::string& dimension : dimensions) {
        if (QString(dimension.c_str()).toUpper() ==
            QString(LAS_FIELD_NAMES[field]).toUpper())
            return true;
    }
...
void LASOpenDlg::setDimensions(const std::vector<std::string>& dimensions) {
    ...
    returnNumberCheckBox->setEnabled(
            FieldIsPresent(dimensions, LAS_RETURN_NUMBER));
    numberOfReturnsCheckBox->setEnabled(
            FieldIsPresent(dimensions, LAS_NUMBER_OF_RETURNS));
    scanDirFlagCheckBox->setEnabled(
            FieldIsPresent(dimensions, LAS_SCAN_DIRECTION));
    edgeOfFlightCheckBox->setEnabled(
            FieldIsPresent(dimensions, LAS_FLIGHT_LINE_EDGE));
    scanAngleRankCheckBox->setEnabled(
            FieldIsPresent(dimensions, LAS_SCAN_ANGLE_RANK));
    userDataCheckBox->setEnabled(FieldIsPresent(dimensions, LAS_USER_DATA));
    pointSourceIDCheckBox->setEnabled(
            FieldIsPresent(dimensions, LAS_POINT_SOURCE_ID));
...
bool LASOpenDlg::doLoad(LAS_FIELDS field) const {
    switch (field) {
        ...
        case LAS_Z:
            return true;
        case LAS_INTENSITY:
            return intensityCheckBox->isEnabled() &&
                   intensityCheckBox->isChecked();
        case LAS_RETURN_NUMBER:
            return returnNumberCheckBox->isEnabled() &&
                   returnNumberCheckBox->isChecked();
        case LAS_NUMBER_OF_RETURNS:
            return numberOfReturnsCheckBox->isEnabled() &&
                   numberOfReturnsCheckBox->isChecked();
        case LAS_SCAN_DIRECTION:
            return scanDirFlagCheckBox->isEnabled() &&
                   scanDirFlagCheckBox->isChecked();
        case LAS_FLIGHT_LINE_EDGE:
            return edgeOfFlightCheckBox->isEnabled() &&
                   edgeOfFlightCheckBox->isChecked();
        case LAS_CLASSIFICATION:
            return classifCheckBox->isEnabled() &&
                   classifCheckBox->isChecked() &&
                   !decomposeClassifGroupBox->isChecked();
        case LAS_SCAN_ANGLE_RANK:
            return scanAngleRankCheckBox->isEnabled() &&
                   scanAngleRankCheckBox->isChecked();
        case LAS_USER_DATA:
            return userDataCheckBox->isEnabled() &&
                   userDataCheckBox->isChecked();
        case LAS_POINT_SOURCE_ID:
            return pointSourceIDCheckBox->isEnabled() &&
                   pointSourceIDCheckBox->isChecked();
        case LAS_RED:
        ...
        case LAS_EXTRA:
            return extraFieldGroupBox->isEnabled() &&
                   extraFieldGroupBox->isChecked();
        case LAS_CLASSIF_VALUE:
            return classifCheckBox->isEnabled() &&
                   classifCheckBox->isChecked() &&
                   decomposeClassifGroupBox->isChecked() &&
                   classifValueCheckBox->isChecked();
        case LAS_CLASSIF_SYNTHETIC:
            return classifCheckBox->isEnabled() &&
                   classifCheckBox->isChecked() &&
                   decomposeClassifGroupBox->isChecked() &&
                   classifSyntheticCheckBox->isChecked();
        case LAS_CLASSIF_KEYPOINT:
            return classifCheckBox->isEnabled() &&
                   classifCheckBox->isChecked() &&
                   decomposeClassifGroupBox->isChecked() &&
                   classifKeypointCheckBox->isChecked();
        case LAS_CLASSIF_WITHHELD:
            return classifCheckBox->isEnabled() &&
                   classifCheckBox->isChecked() &&
                   decomposeClassifGroupBox->isChecked() &&
                   classifWithheldCheckBox->isChecked();
        case LAS_CLASSIF_OVERLAP:
            return classifCheckBox->isEnabled() &&
                   classifCheckBox->isChecked() &&
                   decomposeClassifGroupBox->isChecked() &&
                   classifOverlapCheckBox->isChecked();
        case LAS_INVALID:
        default:
            assert(false);
            return false;
...
void LASOpenDlg::setInfos(QString filename,
    ...
    // number of points
    pointCountLineEdit->setText(
            QLocale().toString(static_cast<uint>(pointCount)));
