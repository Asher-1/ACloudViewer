...
CC_FILE_ERROR LASFilter::saveToFile(ccHObject* entity,
    ...
    bool hasScaleMetaData = false;
    {
        ...
        if (hasScaleMetaData) {
            ...
            if (hasScaleMetaData) {
                originalLasScale.z =
                        theCloud->getMetaData(LAS_SCALE_Z_META_DATA)
                                .toDouble(&hasScaleMetaData);
            }
        }
    }
    ...
    if (!hasOffsetMetaData) {
        ...
        if (ecvGlobalShiftManager::NeedShift(bbMax - lasOffset)) {
            CVLog::Warning(
                    "[LAS] The former LAS_OFFSET doesn't seem to be optimal. "
                    "Using the minimum bounding-box corner instead.");
            CCVector3d globaShift =
                    theCloud->getGlobalShift();  //'global shift' is the
                                                 // opposite of LAS offset ;)

            if (ecvGlobalShiftManager::NeedShift(bbMax + globaShift)) {
                CVLog::Warning(
                        "[LAS] Using the minimum bounding-box corner instead.");
                lasOffset.x = bbMin.x;
                lasOffset.y = bbMin.y;
                lasOffset.z = 0;
            } else {
                CVLog::Warning(
                        "[LAS] Using the previous Global Shift instead.");
    ...
    if (parameters.alwaysDisplaySaveDialog) {
        ...
        if (hasScaleMetaData) {
            ...
            if (!canUseOriginalScale) {
                s_saveDlg->labelOriginal->setText(QObject::tr(
    ...
    auto convertOne = [&](PointRef& point) {
            ...
            switch (lasField.type) {
                ...
                case LAS_TIME:
                    point.setField(pdalId,
                                   lasField.getSafeValue(ptsWritten) +
        ...
        for (const ExtraLasField::Shared& extraField : extraFieldsToSave) {
            point.setField(extraField->pdalId,
                           extraField->getSafeValue(ptsWritten) +
...
struct LasCloudChunk {
    ...
    void createFieldsToLoad(const IdList& extraFieldsToLoad,
                            const StringList& extraNamesToLoad) {
        if (!s_lasOpenDlg) {
            assert(false);
            return;
        }
...
CC_FILE_ERROR LASFilter::loadFile(const QString& filename,
    ...
    try {
        ...
        if (nullptr == layout) {
            CVLog::Warning("PDAL failed to retrieve the file layout");
            return CC_FERR_THIRD_PARTY_LIB_FAILURE;
        }
            ...
            for (const pdal::Dimension::Id pdalId : layout->dims()) {
                switch (pdalId) {
                    ...
                    default:
                        ...
                        QString desanitizedSFName = LasField::DesanitizeString(
                                QString::fromStdString(dim.name));
        ...
        bool loadRGBComponent[3] = {s_lasOpenDlg->doLoad(LAS_RED),
                                    s_lasOpenDlg->doLoad(LAS_GREEN),
                                    s_lasOpenDlg->doLoad(LAS_BLUE)};
        bool loadColor = (loadRGBComponent[0] || loadRGBComponent[1] ||
                          loadRGBComponent[2]);
        ...
        for (unsigned i = 0; i < extraDims.size(); ++i) {
            if (s_lasOpenDlg->doLoadEVLR(i)) {
                extraDimsArg += extraDims[i].name + "=" +
                                interpretationName(extraDims[i].dimType) + ",";
        ...
        auto ccProcessOne = [&](PointRef& point) {
            ...
            if (nbPointsRead == 0) {
                ...
                if (lasOffsetXY.norm2() != 0 &&
                    ((nullptr == parameters.coordinatesShiftEnabled) ||
                     (false == *parameters.coordinatesShiftEnabled))) {
                    if (csModeBackup !=
            ...
            if (loadColor) {
                uint16_t red = loadRGBComponent[0]
                                       ? point.getFieldAs<uint16_t>(Id::Red)
                                       : 0;
                uint16_t green = loadRGBComponent[1]
                                         ? point.getFieldAs<uint16_t>(Id::Green)
                                         : 0;
                uint16_t blue = loadRGBComponent[2]
                ...
                if (!loadedCloud->hasColors()) {
                    if (red || green || blue) {
                        if (loadedCloud->reserveTheRGBTable()) {
                            ...
                            // skipped points
                            for (unsigned i = 0; i + 1 < loadedCloud->size();
                                 ++i) {
                ...
                if (pushColor) {
                    // we test if the color components are coded on 16 bits
                    // (standard) or only on 8 bits (it happens ;)
            ...
            for (LasField::Shared& field : fieldsToLoad) {
                ...
                if (field->sf) {
                    ...
                    if (!ignoreDefaultFields || value != field->firstValue ||
                        ...
                        field->sf = new ccScalarField(qPrintable(
                                LasField::DesanitizeString(field->getName())));
                        if (field->sf->reserveSafe(fileChunkSize)) {
                            ...
                            if (field->type == LAS_TIME) {
                                ...
                                if (value < 1.0e5) {
                                    ...
                                    CVLog::Warning(
                                            "[LAS] Time SF has been shifted "
                                            "but accuracy may not be preserved "
                                            "(%.2f)",
                                            timeShift);
                                } else {
                                    CVLog::Warning(
                                            "[LAS] Time SF has not been "
                                            "shifted. Accuracy may not be "
                                            "preserved.");
                                }
                                field->firstValue = value;
                            }
