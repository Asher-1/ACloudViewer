...
static const char s_LAS_SRS_Key[] =
        "LAS.spatialReference.nosave";  // DGM: added the '.nosave' suffix
                                        // because this custom type can't be
                                        // streamed properly

//! Custom ("Extra bytes") field (EVLR)
struct ExtraLasField : LasField {
    //! Default constructor
    ExtraLasField(QString name,
                  Id id,
...
bool LASFilter::canSave(CV_CLASS_ENUM type,
                        bool& multiple,
...
pdal::Dimension::Id typeToId(LAS_FIELDS sfType, uint8_t pointFormat) {
    switch (sfType) {
        ...
        case LAS_FIELDS::LAS_CLASSIF_WITHHELD:
            return pointFormat < 6 ? pdal::Dimension::Id::Classification
                                   : pdal::Dimension::Id::ClassFlags;
        case LAS_FIELDS::LAS_CLASSIF_OVERLAP:
            return pointFormat < 6
                           ? pdal::Dimension::Id::Unknown
                           : pdal::Dimension::Id::ClassFlags;  // only for point
                                                               // formats >= 6
            // Invalid flag
...
CC_FILE_ERROR LASFilter::saveToFile(ccHObject* entity,
    ...
    if (!theCloud) {
        CVLog::Warning("[LAS] This filter can only save one cloud at a time");
        return CC_FERR_BAD_ENTITY_TYPE;
    }

    unsigned int numberOfPoints = theCloud->size();
    if (numberOfPoints == 0) {
        CVLog::Warning("[LAS] Cloud is empty!");
    ...
    if (theCloud->isA(CV_TYPES::POINT_CLOUD)) {
        ...
        for (unsigned i = 0; i < pc->getNumberOfScalarFields(); ++i) {
            ccScalarField* sf =
                    static_cast<ccScalarField*>(pc->getScalarField(i));
            ...
            auto pos = std::find_if(fieldsToSave.begin(), fieldsToSave.end(),
                                    name_matches);
            if (pos == fieldsToSave.end()) {
                ExtraLasField::Shared extraField(
                        new ExtraLasField(QString(sf->getName()), Id::Unknown));
    ...
    // let the user choose between the original scale and the 'optimal' one (for
    // accuracy, not for compression ;)
    bool hasScaleMetaData = false;
    CCVector3d originalLasScale(0, 0, 0);
    originalLasScale.x = theCloud->getMetaData(LAS_SCALE_X_META_DATA)
                                 .toDouble(&hasScaleMetaData);
    if (hasScaleMetaData) {
        originalLasScale.y = theCloud->getMetaData(LAS_SCALE_Y_META_DATA)
                                     .toDouble(&hasScaleMetaData);
        if (hasScaleMetaData) {
            originalLasScale.z = theCloud->getMetaData(LAS_SCALE_Z_META_DATA)
                                         .toDouble(&hasScaleMetaData);
        }
    }
    ...
    lasOffset.x = theCloud->getMetaData(LAS_OFFSET_X_META_DATA)
                          .toDouble(&hasOffsetMetaData);
    if (hasOffsetMetaData) {
        lasOffset.y = theCloud->getMetaData(LAS_OFFSET_Y_META_DATA)
                              .toDouble(&hasOffsetMetaData);
        if (hasOffsetMetaData) {
            lasOffset.z = theCloud->getMetaData(LAS_OFFSET_Z_META_DATA)
                                  .toDouble(&hasOffsetMetaData);
        }
    }
    ...
    if (!hasOffsetMetaData && theCloud->isShifted()) {
        lasOffset = -theCloud->getGlobalShift();  //'global shift' is the
    ...
    {
        double maxScale = std::max(optimalScale.x,
    ...
    CCVector3d lasScale =
            (canUseOriginalScale ? originalLasScale : optimalScale);

    if (parameters.alwaysDisplaySaveDialog) {
        if (!s_saveDlg)
            s_saveDlg = QSharedPointer<LASSaveDlg>(new LASSaveDlg(nullptr));

        s_saveDlg->bestAccuracyLabel->setText(QString("(%1, %2, %3)")
                                                      .arg(optimalScale.x)
                                                      .arg(optimalScale.y)
                                                      .arg(optimalScale.z));

        if (hasScaleMetaData) {
            s_saveDlg->origAccuracyLabel->setText(
                    QString("(%1, %2, %3)")
                            .arg(originalLasScale.x)
                            .arg(originalLasScale.y)
                            .arg(originalLasScale.z));

            if (!canUseOriginalScale) {
                s_saveDlg->labelOriginal->setText(
                        QObject::tr(
                                "Original scale is too small for this cloud "
                                " "));  // add two whitespaces to avoid issues
                                        // with italic characters justification
    ...
    minPointFormat = LasField::UpdateMinPointFormat(minPointFormat, hasColors,
                                                    false, true);

    if (theCloud->hasMetaData(
                LAS_POINT_FORMAT_META_DATA))  // DGM: is it really necessary?
    {
        bool ok = false;
        unsigned previousPointFormat =
                theCloud->getMetaData(LAS_POINT_FORMAT_META_DATA).toUInt(&ok);
        if (ok && previousPointFormat < 256) {
            minPointFormat = std::max(static_cast<uint8_t>(previousPointFormat),
                                      minPointFormat);
        } else {
            CVLog::Warning("Invalid point_format metadata");
        }
    }
    ...
    auto convertOne = [&](PointRef& point) {
        if (ptsWritten == numberOfPoints) return false;
        ...
        if (hasColors) {
            ...
            const ecvColor::Rgb& rgb = theCloud->getPointColor(ptsWritten);
        ...
        for (const LasField& lasField : fieldsToSave) {
            ...
            switch (lasField.type) {
                ...
                case LAS_BLUE:
                    ...
                    break;
                case LAS_TIME:
                    point.setField(pdalId,
                                   lasField.sf->getValue(ptsWritten) +
                                           lasField.sf->getGlobalShift());
                    break;
                case LAS_CLASSIFICATION:
                    classification = static_cast<uint8_t>(
                            static_cast<int>(
                                    lasField.sf->getValue(ptsWritten)) &
                            255);
                    break;
                case LAS_CLASSIF_VALUE:
                    classification = static_cast<uint8_t>(
                            static_cast<int>(
                                    lasField.sf->getValue(ptsWritten)) &
                            (minPointFormat < 6 ? 31 : 255));
                    break;
                case LAS_CLASSIF_SYNTHETIC:
                    if (lasField.sf->getValue(ptsWritten) != 0) {
                        if (minPointFormat < 6)
                            classification |=
                                    32;  // bit #5 of the 'Classification' field
                        else
                            classFlags |= 1;  // bit #0 of the 'Classification
                                              // Flags' field
                    }
                    break;
                case LAS_CLASSIF_KEYPOINT:
                    if (lasField.sf->getValue(ptsWritten) != 0) {
                        if (minPointFormat < 6)
                            classification |=
                                    64;  // bit #6 of the 'Classification' field
                        else
                            classFlags |= 2;  // bit #1 of the 'Classification
                                              // Flags' field
                    }
                    break;
                case LAS_CLASSIF_WITHHELD:
                    if (lasField.sf->getValue(ptsWritten) != 0) {
                        if (minPointFormat < 6)
                            classification |= 128;  // bit #7 of the
                                                    // 'Classification' field
                        else
                            classFlags |= 4;  // bit #2 of the 'Classification
                                              // Flags' field
                    }
                    break;
                case LAS_CLASSIF_OVERLAP:
                    if (lasField.sf->getValue(ptsWritten) != 0) {
                        if (minPointFormat >= 6)
                            classFlags |= 8;  // bit #3 of the 'Classification
                                              // Flags' field
                        else
                            assert(false);
                    }
        ...
        if (hasClassifFlags) point.setField(Id::ClassFlags, classFlags);

        // extra las fields
        for (const ExtraLasField::Shared& extraField : extraFieldsToSave) {
            point.setField(extraField->pdalId,
                           extraField->sf->getValue(ptsWritten) +
                                   extraField->sf->getGlobalShift());
        }
    ...
    try {
        ...
        if (theCloud->hasMetaData(LAS_GLOBAL_ENCODING_META_DATA)) {
            bool ok = false;
            unsigned int global_encoding =
                    theCloud->getMetaData(LAS_GLOBAL_ENCODING_META_DATA)
                            .toUInt(&ok);
            if (ok) {
        ...
        if (theCloud->hasMetaData(LAS_PROJECT_UUID_META_DATA)) {
            QString uuid = theCloud->getMetaData(LAS_PROJECT_UUID_META_DATA)
                                   .toString();
        ...
        if (theCloud->hasMetaData(LAS_VERSION_MINOR_META_DATA)) {
            bool ok = false;
            int minor_version =
                    theCloud->getMetaData(LAS_VERSION_MINOR_META_DATA)
                            .toInt(&ok);
            if (ok &&
                minor_version != 0)  // PDAL can read but not write LAS 1.0
                writerOptions.add("minor_version", minor_version);
            else if (!ok)
                CVLog::Warning(QString("Invalid minor_version metadata"));
        }
        ...
        for (const ExtraLasField::Shared extraField : extraFields) {
            ...
            Type t = Type::Double;
            extraField->pdalId =
                    table.layout()->registerOrAssignDim(dimName, t);
        }

        writer.prepare(table);
        writer.execute(table);
    } catch (const pdal::pdal_error& p) {
        CVLog::Error(QString("PDAL exception: %1").arg(p.what()));
        return CC_FERR_THIRD_PARTY_LIB_EXCEPTION;
    } catch (const std::exception& e) {
        CVLog::Error(QString("PDAL generic exception: %1").arg(e.what()));
        return CC_FERR_THIRD_PARTY_LIB_EXCEPTION;
    } catch (...) {
...
public:
    ...
    bool init(unsigned int width,
        ...
        for (unsigned int i = 0; i < width; ++i) {
            for (unsigned int j = 0; j < height; ++j) {
                ...
                QString filename = absoluteBaseFilename +
                                   QString("_%1_%2.%3")
    ...
    void addPoint(const PointViewPtr& buffer, unsigned int pointIndex) {
        ...
        CCVector3d Prel =
                CCVector3d(buffer->getFieldAs<double>(Id::X, pointIndex),
        ...
        unsigned int i =
                std::min(static_cast<unsigned int>(std::max(ii, 0)), w - 1);
        unsigned int j =
                std::min(static_cast<unsigned int>(std::max(ji, 0)), h - 1);
    ...
    void writeAll() {
        for (unsigned int i = 0; i < tilePointViews.size(); ++i) {
            ...
            writerOptions.add("filename",
                              fileNames[i].toLocal8Bit().toStdString());
            if (tilePointViews[i]->empty()) continue;
            try {
                ...
                CVLog::Error(QString("PDAL exception '%1'").arg(e.what()));
            }
        }
    }
...
struct LasCloudChunk {
    ...
    bool reserveSize(unsigned int nbPoints) {
        ...
        if (!success) delete loadedCloud;

        return success;
    }

    void createFieldsToLoad(IdList extraFieldsToLoad,
        ...
        if (s_lasOpenDlg->doLoad(LAS_CLASSIFICATION))
            lasFields.push_back(LasField::Shared(
                    new LasField(LAS_CLASSIFICATION, 0, 0,
                                 255)));  // unsigned char: between 0 and 255
        if (s_lasOpenDlg->doLoad(LAS_CLASSIF_VALUE))
            lasFields.push_back(LasField::Shared(new LasField(
                    LAS_CLASSIF_VALUE, 0, 0, 31)));  // 5 bits: between 0 and 31
        if (s_lasOpenDlg->doLoad(LAS_CLASSIF_SYNTHETIC))
            lasFields.push_back(LasField::Shared(new LasField(
                    LAS_CLASSIF_SYNTHETIC, 0, 0, 1)));  // 1 bit: 0 or 1
        if (s_lasOpenDlg->doLoad(LAS_CLASSIF_KEYPOINT))
            lasFields.push_back(LasField::Shared(new LasField(
                    LAS_CLASSIF_KEYPOINT, 0, 0, 1)));  // 1 bit: 0 or 1
        if (s_lasOpenDlg->doLoad(LAS_CLASSIF_WITHHELD))
            lasFields.push_back(LasField::Shared(new LasField(
                    LAS_CLASSIF_WITHHELD, 0, 0, 1)));  // 1 bit: 0 or 1
        if (s_lasOpenDlg->doLoad(LAS_CLASSIF_OVERLAP))
            lasFields.push_back(LasField::Shared(new LasField(
                    LAS_CLASSIF_OVERLAP, 0, 0, 1)));  // 1 bit: 0 or 1
        ...
        for (unsigned int i = 0; i < extraNamesToLoad.size(); ++i) {
            ...
            ExtraLasField* eField =
                    new ExtraLasField(name, extraFieldsToLoad[i]);
            lasFields.emplace_back(eField);
        }
    }

    void addLasFieldsToCloud() {
        if (loadedCloud == nullptr) return;
        ...
        while (!lasFields.empty()) {
            ...
            if (field && field->sf) {
                ...
                if (field->type == LAS_CLASSIFICATION ||
                    ...
                    field->sf->setColorRampSteps(static_cast<unsigned>(
                ...
                CVLog::Warning(
                        QString("[LAS] All '%1' values were the same (%2)! We "
                                "ignored them...")
...
/*
        The following functions until readExtraBytesVlr are copied from PDAL
...
void ExtraBytesIf::setType(uint8_t lastype) {
    ...
    if (m_type == Dimension::Type::None) m_fieldCnt = 0;
}
...
void ExtraBytesIf::readFrom(const char* buf) {
    ...
    extractor >> dummy32;
    for (size_t i = 0; i < 3; ++i) extractor >> dummy64;  // No data field.
    for (size_t i = 0; i < 3; ++i) extractor >> dummyd;   // Min.
    for (size_t i = 0; i < 3; ++i) extractor >> dummyd;   // Max.
    for (size_t i = 0; i < 3; ++i) extractor >> m_scale[i];
    for (size_t i = 0; i < 3; ++i) extractor >> m_offset[i];
    extractor.get(m_description, 32);

    setType(type);
    if (m_type == Dimension::Type::None) m_size = options;
    if (!(options & SCALE_MASK))
        for (size_t i = 0; i < 3; ++i) m_scale[i] = 1.0;
    if (!(options & OFFSET_MASK))
        for (size_t i = 0; i < 3; ++i) m_offset[i] = 0.0;
}
...
std::vector<ExtraDim> ExtraBytesIf::toExtraDims() {
    ...
    if (m_type == Dimension::Type::None) {
        ...
        for (size_t i = 0; i < m_fieldCnt; ++i) {
            ExtraDim ed(m_name + std::to_string(i), m_type, m_scale[i],
                        m_offset[i]);
            eds.push_back(ed);
        }
    }
...
static bool ReadExtraBytesVlr(LasHeader& header,
    ...
    if (size % sizeof(ExtraBytesSpec) != 0) {
        CVLog::Warning("[LAS] Bad size for extra bytes VLR. Ignoring.");
        return false;
    }
    size_t count = size / sizeof(ExtraBytesSpec);
    CVLog::PrintDebug("[LAS] VLR count: " + QString::number(count));
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            for (const ExtraDim& ed : eds) {
                CVLog::PrintDebug(
                        QString("[LAS] VLR #%1: %2")
                                .arg(i + 1)
                                .arg(QString::fromStdString(ed.m_name)));
                extraDims.push_back(ed);
            }
        }
    } catch (const std::bad_alloc&) {
        CVLog::Warning(
                "[LAS] Not enough memory to retrieve the extra bytes fields.");
        return false;
    }
...
CC_FILE_ERROR LASFilter::loadFile(const QString& filename,
                                  ccHObject& container,
    ...
    try {
        ...
        // The VLR record describing the extra bytes has been added to LAS 1.4
        // to formalize a process that has been used in prior versions of LAS.
        // So PDAL doesn't read this VLR if the version is <= 1.3.
        // The idea is to read the VLR manually, make a string of the names and
        // data types, and pass that back to the PDAL reader.
        std::vector<ExtraDim> extraDims;
        ...
        CCVector3d lasScale(lasHeader.scaleX(), lasHeader.scaleY(),
                            lasHeader.scaleZ());
        CCVector3d lasOffset(lasHeader.offsetX(), lasHeader.offsetY(),
                             lasHeader.offsetZ());

        const uint8_t pointFormat = lasHeader.pointFormat();
        CVLog::Print("[LAS] Point format: " + QString::number(pointFormat));
        ...
        for (const ExtraDim& dim : extraDims) {
            s_lasOpenDlg->addEVLR(
                    QString("%1").arg(QString::fromStdString(dim.m_name)));
        }

        if (parameters.sessionStart) {
            // we do this AFTER restoring the previous context because it may
            // still be good that the previous configuration is restored even
            // though the user needs to confirm it
            s_lasOpenDlg->resetApplyAll();
        }

        if (parameters.alwaysDisplayLoadDialog &&
            !s_lasOpenDlg->autoSkipMode() && !s_lasOpenDlg->exec()) {
            return CC_FERR_CANCELED_BY_USER;
        }

        bool ignoreDefaultFields =
                s_lasOpenDlg->ignoreDefaultFieldsCheckBox->isChecked();

        unsigned int short rgbColorMask[3] = {0, 0, 0};
        if (s_lasOpenDlg->doLoad(LAS_RED)) rgbColorMask[0] = (~0);
        if (s_lasOpenDlg->doLoad(LAS_GREEN)) rgbColorMask[1] = (~0);
        if (s_lasOpenDlg->doLoad(LAS_BLUE)) rgbColorMask[2] = (~0);
        bool loadColor =
                (rgbColorMask[0] || rgbColorMask[1] || rgbColorMask[2]);

        // by default we read colors as triplets of 8 bits integers but we might
        // dynamically change this if we encounter values using 16 bits (16 bits
        // is the standard!)
        unsigned char colorCompBitShift = 0;
        bool forced8bitRgbMode = s_lasOpenDlg->forced8bitRgbMode();
        ecvColor::Rgb rgb(0, 0, 0);
        ...
        for (unsigned i = 0; i < extraDims.size(); ++i) {
            if (s_lasOpenDlg->doLoadEVLR(i)) {
                extraDimsArg +=
                        extraDims[i].m_name + "=" +
                        interpretationName(extraDims[i].m_dimType.m_type) + ",";
                extraNamesToLoad.push_back(extraDims[i].m_name);
            }
        }

        if (!extraNamesToLoad.empty()) {
            // If extra fields are requested, reload the file with the new
            // extra_dims parameters
            Options las_opts2;
        ...
        if (tiling) {
            ...
            auto w = static_cast<unsigned int>(
                    s_lasOpenDlg->wTileSpinBox->value());
            auto h = static_cast<unsigned int>(
                    s_lasOpenDlg->hTileSpinBox->value());

            QString outputBaseName = s_lasOpenDlg->outputPathLineEdit->text() +
            ...
            if (parameters.parentWidget) {
                ...
                pDlg->setInfo(
                        QObject::tr("Please wait... reading in progress"));
                pDlg->setRange(0, 0);
                pDlg->setModal(true);
                pDlg->start();
            }
            ...
            QObject::connect(&reader, SIGNAL(finished()), pDlg.data(),
                             SLOT(reset()));
            ...
            for (PointId idx = 0; idx < pointView->size(); ++idx) {
                if (pDlg->isCancelRequested()) return CC_FERR_CANCELED_BY_USER;
            ...
            if (parameters.parentWidget) {
                ...
                pDlg->setMethodTitle(QObject::tr("LAS file"));
                pDlg->setInfo(
                        QObject::tr("Please wait... writing in progress"));
                pDlg->setRange(0, 0);
                pDlg->setModal(true);
                pDlg->start();
            }
            ...
            QObject::connect(&writer, SIGNAL(finished()), pDlg.data(),
                             SLOT(reset()));
            writer.setFuture(
                    QtConcurrent::run([&tiler]() { tiler.writeAll(); }));
        ...
        unsigned int nbOfChunks =
                (nbOfPoints / CC_MAX_NUMBER_OF_POINTS_PER_CLOUD) + 1;
        ...
        auto ccProcessOne = [&](PointRef& point) {
            ...
            if (pointChunk.getLoadedCloud() == nullptr) {
                ...
                if (!srs.empty()) {
                    ...
                    CVLog::Print("[LAS] Spatial reference: " + wkt);
                    pointChunk.loadedCloud->setMetaData(s_LAS_SRS_Key, wkt);
                } else {
                    CVLog::Print("[LAS] Spatial reference: None");
                }

                pointChunk.createFieldsToLoad(extraDimensionsIds,
                                              extraNamesToLoad);
            }
            ...
            if (nbPointsRead == 0) {
                CCVector3d P(static_cast<PointCoordinateType>(
                                     point.getFieldAs<int>(Id::X)),
                             static_cast<PointCoordinateType>(
                                     point.getFieldAs<int>(Id::Y)),
                             static_cast<PointCoordinateType>(
                                     point.getFieldAs<int>(Id::Z)));
                ...
                if (lasOffset.norm2() != 0 &&
                    (!parameters.coordinatesShiftEnabled ||
                     !*parameters.coordinatesShiftEnabled)) {
                    if (csModeBackup !=
                        ecvGlobalShiftManager::
                                NO_DIALOG)  // No dialog, practically means that
                                            // we don't want any shift!
                    {
                        useLasOffset = true;
                        Pshift = -lasOffset;
                        if (csModeBackup !=
            ...
            CCVector3 P(static_cast<PointCoordinateType>(
                                point.getFieldAs<double>(Id::X) + Pshift.x),
                        static_cast<PointCoordinateType>(
                                point.getFieldAs<double>(Id::Y) + Pshift.y),
                        static_cast<PointCoordinateType>(
                                point.getFieldAs<double>(Id::Z) + Pshift.z));
            ...
            if (loadColor) {
                ...
                unsigned short green =
                        point.getFieldAs<unsigned short>(Id::Green) &
                        rgbColorMask[1];
                unsigned short blue =
                        point.getFieldAs<unsigned short>(Id::Blue) &
                        rgbColorMask[2];

                // if we don't have reserved a color field yet, we check that
                // color is not black
                bool pushColor = true;
                if (!loadedCloud->hasColors()) {
                    if (red || green || blue) {
                        if (loadedCloud->reserveTheRGBTable()) {
                            // we must set the color (black) of all previously
                            // skipped points
                            for (unsigned int i = 0;
                                 i < loadedCloud->size() - 1; ++i) {
                                loadedCloud->addRGBColor(ecvColor::black);
                            }
                        } else {
                            CVLog::Warning(
                                    "[LAS]: Not enough memory, color field "
                                    "will be ignored!");
                            loadColor = false;  // no need to retry with the
                                                // other chunks anyway
                            pushColor = false;
                        }
                    } else  // otherwise we ignore it for the moment (we'll add
                ...
                if (pushColor) {
                    // we test if the color components are on 16 bits (standard)
                    // or only on 8 bits (it happens ;)
                    if (!forced8bitRgbMode && colorCompBitShift == 0) {
                        if ((red & 0xFF00) || (green & 0xFF00) ||
                            (blue & 0xFF00)) {
                            // the color components are on 16 bits!
                            CVLog::Print(
                                    "[LAS] Color components are coded on 16 "
                                    "bits");
                            colorCompBitShift = 8;
                            // we fix all the previously read colors
                            for (unsigned int i = 0;
                                 i < loadedCloud->size() - 1; ++i) {
                                loadedCloud->setPointColor(
                                        i, ecvColor::black);  // 255 >> 8 = 0!
                            }
                        }
                    }
                    rgb.r = static_cast<ColorCompType>(red >>
                                                       colorCompBitShift);
                    rgb.g = static_cast<ColorCompType>(green >>
                                                       colorCompBitShift);
                    rgb.b = static_cast<ColorCompType>(blue >>
                                                       colorCompBitShift);
            ...
            for (LasField::Shared& field : fieldsToLoad) {
                ...
                switch (field->type) {
                    ...
                    case LAS_CLASSIF_VALUE:
                        if (pointFormat <= 5)
                            value = (point.getFieldAs<int>(pdalId) &
                                     31);  // bit #0-4 of the 'Classification'
                                           // field
                        else
                            value = point.getFieldAs<int>(pdalId);
                        break;
                    case LAS_CLASSIF_SYNTHETIC:
                        if (pointFormat <= 5)
                            value = (point.getFieldAs<int>(pdalId) & 32)
                                            ? 1.0
                                            : 0.0;  // bit #5 of the
                                                    // 'Classification' field
                        else
                            value = (point.getFieldAs<int>(pdalId) & 1)
                                            ? 1.0
                                            : 0.0;  // bit #0 of the
                    ...
                    case LAS_CLASSIF_KEYPOINT:
                        if (pointFormat <= 5)
                            value = (point.getFieldAs<int>(pdalId) & 64)
                                            ? 1.0
                                            : 0.0;  // bit #6 of the
                                                    // 'Classification' field
                        else
                            value = (point.getFieldAs<int>(pdalId) & 2)
                                            ? 1.0
                                            : 0.0;  // bit #1 of the
                                                    // 'Classification Flags'
                                                    // field
                    ...
                    case LAS_CLASSIF_WITHHELD:
                        if (pointFormat <= 5)
                            value = (point.getFieldAs<int>(pdalId) & 128)
                                            ? 1.0
                                            : 0.0;  // bit #7 of the
                                                    // 'Classification' field
                        else
                            value = (point.getFieldAs<int>(pdalId) & 4)
                                            ? 1.0
                                            : 0.0;  // bit #2 of the
                                                    // 'Classification Flags'
                                                    // field
                    ...
                    case LAS_CLASSIF_OVERLAP:
                        if (pointFormat <= 5) {
                            ...
                            value = (point.getFieldAs<int>(pdalId) & 8)
                                            ? 1.0
                                            : 0.0;  // bit #3 of the
                                                    // 'Classification Flags'
                                                    // field
                ...
                if (field->sf) {
                    ...
                    if (!ignoreDefaultFields || value != field->firstValue ||
                        ...
                        field->sf =
                                new ccScalarField(qPrintable(field->getName()));
                        if (field->sf->reserveSafe(fileChunkSize)) {
                            ...
                            auto defaultValue = static_cast<ScalarType>(
                                    field->defaultValue);
            ...
            if (loadedCloud) {
                if (loadedCloud->size()) {
                    ...
                    if (loadColor && !thisChunkHasColors) {
                        CVLog::Warning(
                                "[LAS] Color field was all black! We ignored "
                                "it...");
                    }
                    ...
                    if (n != 0) {
                        if (n == 1) {
                            container.getChild(0)->setName(chunkName +
                                                           QString(" #1"));
                        }
                    ...
                    loadedCloud->setMetaData(LAS_SCALE_X_META_DATA,
                                             QVariant(lasScale.x));
                    loadedCloud->setMetaData(LAS_SCALE_Y_META_DATA,
                                             QVariant(lasScale.y));
                    loadedCloud->setMetaData(LAS_SCALE_Z_META_DATA,
                                             QVariant(lasScale.z));
                    loadedCloud->setMetaData(LAS_OFFSET_X_META_DATA,
                                             QVariant(lasOffset.x));
                    loadedCloud->setMetaData(LAS_OFFSET_Y_META_DATA,
                                             QVariant(lasOffset.y));
                    loadedCloud->setMetaData(LAS_OFFSET_Z_META_DATA,
                                             QVariant(lasOffset.z));
                    loadedCloud->setMetaData(
                            LAS_GLOBAL_ENCODING_META_DATA,
                            QVariant(lasHeader.globalEncoding()));
                    ...
                    loadedCloud->setMetaData(
                            ...
                            QVariant(lasHeader.versionMajor()));
                    loadedCloud->setMetaData(
                            LAS_VERSION_MINOR_META_DATA,
                            QVariant(lasHeader.versionMinor()));
                    loadedCloud->setMetaData(LAS_POINT_FORMAT_META_DATA,
                                             QVariant(pointFormat));
