...
public:
    ...
    virtual bool setOrientation(const CCVector3 &x,
                                const CCVector3 &y,
                                const CCVector3 &z);
    ...
    virtual GenericItem *getRoot() {
        ...
        while (item->owner) item = item->owner;
        return item;
    }
    ...
    virtual std::pair<int, int> write(std::ostream &output,
                                      int nbtabs = 0) const = 0;
...
public:
    ...
    virtual void remove(Loop *l) {
        if (l == loop) loop = nullptr;
    }
    virtual bool push(GenericItem *l);
    virtual void remove(GenericItem *i) {
        if (loop == i) loop = nullptr;
    }
    virtual bool setValue(Token t, PointCoordinateType value) {
        if (t == PDMS_HEIGHT) {
            height = value;
...
public:
    ...
    explicit DistanceValue(Token t = PDMS_INVALID_TOKEN)
        : NumericalValue(t), unit(PDMS_INVALID_TOKEN) {}
...
public:
    ...
    explicit Reference(Token t = PDMS_INVALID_TOKEN)
        : Command(t), token(PDMS_INVALID_TOKEN) {
        memset(refname, 0, c_max_str_length);
    }
    Reference(const Reference &ref) : Command(ref), token(ref.token) {
...
public:
    ...
    explicit Coordinates(Token t = PDMS_INVALID_TOKEN) : Command(t) {
        current = -1;
    }
    virtual bool handle(Token t);
    virtual bool handle(PointCoordinateType numvalue);
...
public:
    ...
    Orientation() : Command(PDMS_ORIENTATION) {
        current = nullptr;
        component = -1;
    }
    virtual bool handle(Token t);
...
public:
    ...
    virtual bool isValid() const { return strlen(name) > 0; }
    virtual bool execute(PdmsObjects::GenericItem *&item) const;
};
...
public:
    ...
    ElementCreation() : Command(PDMS_CREATE) {
