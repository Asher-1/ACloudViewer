...
//! PDMS Lexer
/** PDMS sessions are made to provide an input stream to the parser (file,
 *keyboard, pipe, ...)
 **/
class PdmsLexer {
public:
...
/** Use this parser the following way:
        ...
        3- parse the current session content. if the session content changes,
you can parse it as many times as you wish (the loaded object is overwritten
each time) 4- get the result (either in session or in parser)
**/
class PdmsParser {
public:
    //! Default constructor
