...
void PdmsObjects::Stack::Clear()
{
	...
	while (!s_elementsStack.empty())
	{
		...
		if (item)
		{
			//CVLog::Print(QString("[PDMS] Should be deleting %1").arg(item->name));
			delete item;
		}
	}
...
PointCoordinateType NumericalValue::getValue() const
{
	...
	case PDMS_Y_BOTTOM_SHEAR:
		return static_cast<PointCoordinateType>(CV_DEG_TO_RAD) * value;
	default:
		return value;
	}
}
...
bool Orientation::axisFromCoords(const Coordinates &coords, CCVector3 &u)
{
	...
	{
		PointCoordinateType alpha = static_cast<PointCoordinateType>(CV_DEG_TO_RAD) * u[0];
		PointCoordinateType beta = static_cast<PointCoordinateType>(CV_DEG_TO_RAD) * u[1];
