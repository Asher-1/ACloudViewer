...
void PdmsObjects::Stack::Clear() {
    ...
    while (!s_elementsStack.empty()) {
        ...
        if (item) {
            // CVLog::Print(QString("[PDMS] Should be deleting
            // %1").arg(item->name));
            delete item;
        }
    }
...
void PdmsObjects::Stack::Destroy(GenericItem *&item) {
    if (item && s_elementsStack.remove(item)) {
        // CVLog::Print(QString("[PDMS] Destroying %1").arg((item)->name));
        delete item;
        item = nullptr;
    }
}
...
bool NumericalValue::isValid() const { return valueChanges <= 1; }
...
PointCoordinateType NumericalValue::getValue() const {
    switch (command) {
        ...
        case PDMS_Y_BOTTOM_SHEAR:
            return cloudViewer::DegreesToRadians(value);
        default:
            return value;
    }
}
...
bool Reference::isNameReference() const { return strlen(refname) > 0; }

bool Reference::isTokenReference() const { return token != PDMS_INVALID_TOKEN; }
...
bool Reference::execute(PdmsObjects::GenericItem *&item) const {
    ...
    if (command == PDMS_LAST) {
        if (s_elementsStack.size() < 2) return false;
        ...
        if (isSet() == 1) {
            while (true) {
                if (isNameReference() && strcmp(refname, (*it)->name) == 0)
                    break;
                if (isTokenReference() && (*it)->getType() == token) break;
                if (it == s_elementsStack.begin()) return false;
                --it;
            }
        }
        item = *it;
    ...
    // Request for an element (hierarchical or design element only)
        ...
        if (PdmsToken::isGroupElement(token)) {
            // Go up in the hierarchy to find a matching, or the first group
            // which can own the request item
            result = item;
            while (result && result->getType() > token) result = result->owner;
            if (!result) result = &defaultWorld;
    ...
    // If the reference command is PDMS_OWNER, then we have to change the
    // request item owner
...
bool Coordinates::handle(Token t) {
    ...
    // Check that current active command cannot handle this token (if it cannot,
    // check that it is valid before continuing)
    if (current >= 0) {
        if (coords[current].handle(t)) return true;
        if (!coords[current].isValid()) return false;
    }
...
int Coordinates::getNbComponents(bool onlyset) const {
    ...
    for (int i = 0; i < 3; i++) {
        if (PdmsToken::isCoordinate(coords[i].command)) {
            if (!onlyset || coords[nb].valueChanges >= 1) nb++;
        }
    }
    return nb;
}
...
bool Position::handle(Token t) {
    // Check if current activ command can handle this token (if it cannot, check
    // that it is valid before continuing)
    if (current) {
        if (current->handle(t)) return true;
        if (!current->isValid()) return false;
    }
    // If no current command is activ, then token must be either a reference
    // command or a coordinate ID Handle PDMS_WRT (and check it has not been
    // handled yet)
    if (t == PDMS_WRT) {
        current = &ref;
...
bool Position::execute(PdmsObjects::GenericItem *&item) const {
    ...
    if (ref.isValid()) {
        refpos = item;
        if (!ref.execute(refpos)) return false;
    }
...
bool Orientation::handle(Token t) {
    ...
    // Handle coordinates ID : here, we should create a new orientation axis
    // (since no current element is activ)
    if (PdmsToken::isCoordinate(t)) {
...
bool Orientation::getAxes(CCVector3 &x, CCVector3 &y, CCVector3 &z) const {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        switch (orientation[i].command) {
            case PDMS_X:
            case PDMS_EST:
                if (!axisFromCoords(orientation[i], x)) return false;
                break;
            ...
            case PDMS_Y:
            case PDMS_NORTH:
                if (!axisFromCoords(orientation[i], y)) return false;
                break;
            ...
            case PDMS_Z:
            case PDMS_UP:
                if (!axisFromCoords(orientation[i], z)) return false;
                break;
...
bool Orientation::axisFromCoords(const Coordinates &coords, CCVector3 &u) {
    if (!coords.getVector(u)) return false;

    if (coords.getNbComponents(true) == 2) {
        PointCoordinateType alpha = cloudViewer::DegreesToRadians(u[0]);
        PointCoordinateType beta = cloudViewer::DegreesToRadians(u[1]);
...
int Orientation::getNbComponents() const {
    int nb = 0;
    while (nb < 3 && orientation[nb].command) nb++;
    return nb;
}
...
bool Orientation::execute(PdmsObjects::GenericItem *&item) const {
    ...
    for (unsigned i = 0; i < 3; i++) {
        ...
        if (refs[i].isValid()) {
            refori = item;
            if (!refs[i].execute(refori)) return false;
        }
...
bool ElementCreation::execute(PdmsObjects::GenericItem *&item) const {
    ...
    try {
        switch (elementType) {
            ...
            case PDMS_NBOX:
                newElement = new Box;
                static_cast<Box *>(newElement)->negative =
                        (elementType == PDMS_NBOX);
                break;
            case PDMS_PYRAMID:
            ...
            case PDMS_NEXTRU:
                ...
                static_cast<Extrusion *>(newElement)->negative =
                        (elementType == PDMS_NEXTRU);
                break;
            case PDMS_LOOP:
                newElement = new Loop;
                break;
...
bool ElementEnding::execute(PdmsObjects::GenericItem *&item) const {
    ...
    switch (command) {
        case PDMS_OWNER:
            // If the ending command is PDMS_OWNER, then we simply go back to
            // the item owner
            result = item ? item->owner : 0;
            break;
        case PDMS_END:
            // Bug realworks : ignore END EXTRU command
            if (end.isTokenReference() &&
                (end.token == PDMS_EXTRU || end.token == PDMS_NEXTRU))
                return true;
            // In the general case, we have to find the references item (default
            // : this one), and go back to its creator
            result = item;
            ...
            break;
        case PDMS_LAST:
            if (!end.execute(result)) return false;
            break;
        default:
            return false;
    }
...
bool ElementCreation::splitPath(const char *str) {
    ...
    while (str[i]) {
        if (str[i] == '/') {
            if (i != 0) path.push_back(std::string(str, i));
            str = &str[i + 1];
            i = 0;
        } else {
            i++;
...
bool HierarchyNavigation::execute(PdmsObjects::GenericItem *&item) const {
    ...
    // Go back to the first creator object that matches or can contain the
    // command hierarchy level
    while (result && command < result->getType()) result = result->creator;

    // If we went to the root, we have to create a new hierarchy level and set
    // it as the new root
    if (!result) {
        try {
...
GenericItem::GenericItem()
    ...
    orientationReferences[0] = orientationReferences[1] =
            orientationReferences[2] = nullptr;
...
bool GenericItem::completeOrientation() {
    bool ok[3] = {isOrientationValid(0), isOrientationValid(1),
                  isOrientationValid(2)};

    unsigned nb = static_cast<unsigned>(ok[0]) + static_cast<unsigned>(ok[1]) +
...
bool GenericItem::convertCoordinateSystem() {
    ...
    if (positionReference) {
        ...
        // New position is reference origin plus
        // sum(reference_axis[i]*point_coordinate[i])
        GenericItem *ref = positionReference;
        if (!ref->isCoordinateSystemUpToDate && ref->owner == this)
            return false;
        CCVector3 p = position;
        for (unsigned i = 0; i < 3; i++)
            position[i] = ref->orientation[0][i] * p[0] +
                          ref->orientation[1][i] * p[1] +
                          ref->orientation[2][i] * p[2];
        position += ref->position;
    }
    // The same for orientation
    for (unsigned k = 0; k < 3; k++) {
        if (!isOrientationValid(k)) continue;
        if (orientationReferences[k]) {
            ...
            CCVector3 axis[3];
            {
                for (unsigned j = 0; j < 3; j++) axis[j] = orientation[j];
            }
            {
                for (unsigned j = 0; j < 3; j++)
                    for (unsigned i = 0; i < 3; i++)
                        orientation[j][i] =
                                ref->orientation[0][i] * axis[j][0] +
                                ref->orientation[1][i] * axis[j][1] +
                                ref->orientation[2][i] * axis[j][2];
            }
        }
    }

    if (!completeOrientation()) return false;
...
DesignElement::~DesignElement() {
    for (std::list<DesignElement *>::iterator it = nelements.begin();
         it != nelements.end(); ++it) {
        GenericItem *item = *it;
        if (item) {
            Stack::Destroy(item);
...
bool DesignElement::push(GenericItem *i) {
    if (i->isDesignElement()) {
        ...
        if (element->negative) {
            ...
            if (element->owner) element->owner->remove(element);
            element->owner = this;
            return true;
        }
    }
...
GroupElement::~GroupElement() { clear(true); }

void GroupElement::clear(bool del) {
    if (del) {
        for (std::list<DesignElement *>::iterator eit = elements.begin();
             eit != elements.end(); ++eit) {
            GenericItem *item = *eit;
            if (*eit) Stack::Destroy(item);
        }
        for (std::list<GroupElement *>::iterator hit = subhierarchy.begin();
             hit != subhierarchy.end(); ++hit) {
            GenericItem *item = *hit;
            if (*hit) Stack::Destroy(item);
        }
    }
    elements.clear();
    subhierarchy.clear();
...
bool GroupElement::push(GenericItem *i) {
    // In each case, the insertion of a new element consists in finding the list
    // in which it should be added

    // If the request item is a group, we have to find its new place in the
    // hierarchy
    if (PdmsToken::isGroupElement(i->getType())) {
        // If this group can contain the request item, then insert it in the
        // group list
...
bool GroupElement::convertCoordinateSystem() {
    // Important : check that the object is not up to date, to avoid infinite
    // loops
    if (isCoordinateSystemUpToDate) return true;

    if (!GenericItem::convertCoordinateSystem()) return false;
    for (std::list<DesignElement *>::iterator eit = elements.begin();
         eit != elements.end(); ++eit)
        if (!(*eit)->convertCoordinateSystem()) return false;
    for (std::list<GroupElement *>::iterator hit = subhierarchy.begin();
         hit != subhierarchy.end(); ++hit)
        if (!(*hit)->convertCoordinateSystem()) return false;
    return true;
}

GenericItem *GroupElement::scan(const char *str) {
    // scan all elements contained in this group, beginning with this one, while
    // none matches the requested name
    GenericItem *item = GenericItem::scan(str);
    for (std::list<DesignElement *>::iterator eit = elements.begin();
         eit != elements.end() && !item; ++eit)
        item = (*eit)->scan(str);
    for (std::list<GroupElement *>::iterator hit = subhierarchy.begin();
         hit != subhierarchy.end() && !item; ++hit)
        item = (*hit)->scan(str);
    return item;
}
...
std::pair<int, int> GroupElement::write(std::ostream &output,
                                        int nbtabs) const {
    {
        for (int i = 0; i < nbtabs; i++) output << "\t";
    }
    output << "NEW ";
    ...
    if (strlen(name)) output << " /" << name;
    output << std::endl;

    std::pair<int, int> nb(1, 0);

    for (std::list<GroupElement *>::const_iterator hit = subhierarchy.begin();
         hit != subhierarchy.end(); ++hit) {
    ...
    for (std::list<DesignElement *>::const_iterator eit = elements.begin();
         eit != elements.end(); ++eit) {
...
bool SCylinder::setValue(Token t, PointCoordinateType value) {
    switch (t) {
        ...
        case PDMS_X_TOP_SHEAR:
            xtshear = value;
            if (fabs(xtshear) > 90.) return false;
            break;
        case PDMS_Y_TOP_SHEAR:
            ytshear = value;
            if (fabs(ytshear) > 90.) return false;
            break;
        case PDMS_X_BOTTOM_SHEAR:
            xbshear = value;
            if (fabs(xbshear) > 90.) return false;
            break;
        case PDMS_Y_BOTTOM_SHEAR:
            ybshear = value;
            if (fabs(ybshear) > 90.) return false;
            break;
        default:
            return false;
    }
...
std::pair<int, int> SCylinder::write(std::ostream &output, int nbtabs) const {
    ...
    for (i = 0; i <= nbtabs; i++) output << "\t";
    output << "AT X " << position[0] << " Y " << position[1] << " Z "
           << position[2] << std::endl;
    for (i = 0; i <= nbtabs; i++) output << "\t";
    output << "ORI ";
    output << "X is X " << orientation[0][0] << " Y " << orientation[0][1]
           << " Z " << orientation[0][2];
    output << " AND Z is X " << orientation[2][0] << " Y " << orientation[2][1]
           << " Z " << orientation[2][2] << std::endl;
...
bool CTorus::setValue(Token t, PointCoordinateType value) {
    switch (t) {
        case PDMS_ANGLE:
            angle = value;
            if (fabs(angle) > (2. * M_PI)) return false;
            break;
        case PDMS_INSIDE_RADIUS:
            inside_radius = value;
            break;
...
PointCoordinateType CTorus::surface() const {
    PointCoordinateType r = static_cast<PointCoordinateType>(0.5) *
                            (outside_radius - inside_radius);
    PointCoordinateType R = outside_radius - r;
    return (angle / static_cast<PointCoordinateType>(2.0 * M_PI)) *
           (static_cast<PointCoordinateType>(4.0 * PDMS_SQR(M_PI)) * r * R);
}
...
PointCoordinateType RTorus::surface() const {
    PointCoordinateType inside = static_cast<PointCoordinateType>(2.0 * M_PI) *
                                 inside_radius * height;
    PointCoordinateType outside = static_cast<PointCoordinateType>(2.0 * M_PI) *
                                  outside_radius * height;
    PointCoordinateType updown =
            static_cast<PointCoordinateType>(2.0 * M_PI) *
            (PDMS_SQR(outside_radius) - PDMS_SQR(inside_radius));
    return (angle / static_cast<PointCoordinateType>(2.0 * M_PI)) *
           (inside + outside + updown);
}
...
Dish::Dish() : diameter(0), height(0), radius(0) {}
...
PointCoordinateType Dish::surface() const {
    if (cloudViewer::GreaterThanEpsilon(radius)) {
        PointCoordinateType r =
                static_cast<PointCoordinateType>(0.5f * diameter);
        ...
        if (2 * height > diameter) {
            ...
            return static_cast<PointCoordinateType>(M_PI) *
                   (PDMS_SQR(r) + (a * r * height / sin(a)));
        } else {
            PointCoordinateType a = acos(height / r);
            return static_cast<PointCoordinateType>(M_PI) *
                   (PDMS_SQR(r) +
                    ((PDMS_SQR(height) / sin(a)) * log((1 + sin(a)) / cos(a))));
        }
    }
    return static_cast<PointCoordinateType>(M_PI) * diameter * height;
}
...
PointCoordinateType Cone::surface() const {
    ...
    PointCoordinateType a1 = static_cast<PointCoordinateType>(M_PI) * r1 *
                             sqrt(PDMS_SQR(r1) + PDMS_SQR(h1));
    PointCoordinateType a2 = static_cast<PointCoordinateType>(M_PI) * r2 *
                             sqrt(PDMS_SQR(r2) + PDMS_SQR(h1 + height));
...
PointCoordinateType Box::surface() const {
    return 2 * ((lengths[0] * lengths[1]) + (lengths[1] * lengths[2]) +
                (lengths[2] * lengths[0]));
}
...
PointCoordinateType Extrusion::surface() const {
    ...
    if (loop) {
        ...
        while (it1 != loop->loop.end()) {
            if (it2 == loop->loop.end()) it2 = loop->loop.begin();
            p += ((*it1)->v - (*it2)->v).norm();
            ++it1;
            ++it2;
        }
