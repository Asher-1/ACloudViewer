...
bool PdmsLexer::gotoNextToken() {
    ...
    // Special case: in meta group, the lexer splits Meta Group comments into
    // appropriated tokens
    if (metaGroupMask) {
        metaGroupMask++;
    ...
    if (metaGroupMask) return gotoNextToken();
...
void PdmsLexer::parseCurrentToken() {
    ...
    if (tokenBuffer[0] == '/')
        ...
        currentToken = PDMS_COMMENT_BLOCK;
             ...
             ('0' <= tokenBuffer[0] && tokenBuffer[0] <= '9'))
        ...
        if (location != dictionary.end()) currentToken = location->second;
    }
}
...
PointCoordinateType PdmsLexer::valueFromBuffer() {
    ...
    {
        if ((tokenBuffer[index - 1] >= '0' && tokenBuffer[index - 1] <= '9') ||
            tokenBuffer[index - 1] == '.')
            break;
        index--;
        length++;
    ...
    for (index = 0; index < length; index++)
        if (tokenBuffer[index] == ',') tokenBuffer[index] = '.';

    // convert value
    PointCoordinateType value =
            static_cast<PointCoordinateType>(atof(tokenBuffer));

    return value;
}

const char* PdmsLexer::nameFromBuffer() const { return &tokenBuffer[1]; }
...
void PdmsLexer::pushIntoDictionary(const char* str, Token token, int minSize) {
    ...
    if (minSize == 0 || minSize > n) minSize = n;
...
bool PdmsFileSession::moveForward() {
    ...
    tokenBuffer[n] = '\0';
    if (tokenBuffer[0] != '/') upperStr(tokenBuffer);
    return (n > 0);
}
...
void PdmsFileSession::skipComment() {
    switch (currentToken) {
        case PDMS_COMMENT_LINE:
            // skip line only if the end of line has not been read in current
            // buffer
            if (!m_eol) {
                int n = 0;
                int car = 0;
            ...
            break;
        case PDMS_COMMENT_BLOCK: {
            // comment block opening symbol has been met. Search for comment
            // block ending symbol don't forget that some other comments could
            // be embedded in this comment
    ...
    if (strncmp(tokenBuffer, "ENTERING IN GROUP:", 18) == 0) {
        ...
        // The meta group name starts after the "entering in group:" statement,
        // after the last slash But we still store the whole path
...
void PdmsFileSession::printWarning(const char* str) {
    if (currentToken == PDMS_EOS)
        std::cerr << "[" << m_filename << "]@postprocessing : " << str
                  << std::endl;
    else
        std::cerr << "[" << m_filename << "]@[line " << m_currentLine << "]::["
                  << tokenBuffer << "] : " << str << std::endl;
}
...
bool PdmsParser::processCurrentToken() {
    ...
    switch (currentToken) {
        ...
        case PDMS_NUM_VALUE:
            ...
            if (!currentCommand ||
                !currentCommand->handle(session->valueFromBuffer())) {
        ...
        case PDMS_NAME_STR:
            // There should be a active command, and it should handle the char
            // string
            if (!currentCommand ||
                !currentCommand->handle(session->nameFromBuffer())) {
                session->printWarning(
                        "Last token cannot be associated with a name");
                return false;
            }
            break;
        ...
        default:
            ...
            if (currentCommand) {
                ...
                // Else, the token must be a new command. We execute the active
                // command and delete it
                ...
                if (!success) {
                    assert(false);
                    session->printWarning(
                            "Unable to resolve previous command (this token "
                            "may be unexpected in current command)");
                    return false;
                }
                ...
                if (item) {
                    ...
                    if (!root) {
                        ...
                        assert(false);
                        session->printWarning(
                                "Trying to create a second root for elements "
                                "hierarchy");
                        return false;
                    }
                }
            ...
            if (currentToken == PDMS_RETURN) {
                ...
                // Here, we must create a new command from the token speciffied
                // by the lexer
...
bool PdmsParser::parseSessionContent() {
    ...
    // else check that the current item doesn't belong to a new root
        ...
        session->printWarning(
                "there could be several hierarchy root specified in this file");
    if (root) root->convertCoordinateSystem();
...
PdmsObjects::GenericItem* PdmsParser::getLoadedObject(bool forgetIt) {
    ...
    if (session) result = session->getLoadedObject();
    if (forgetIt) {
        currentItem = nullptr;
        root = nullptr;
    }
