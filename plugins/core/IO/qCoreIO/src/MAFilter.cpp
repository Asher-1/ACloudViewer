...
static void ReleaseEdgeList(
        edge**& theEdges,
        unsigned numberOfVertexes,
...
MAFilter::MAFilter()
    : FileIOFilter({"_Maya ASCII Filter",
                    ...
                    QStringList(), "ma", QStringList(),
                    QStringList{"Maya ASCII mesh (*.ma)"}, Export}) {}

bool MAFilter::canSave(CV_CLASS_ENUM type,
                       bool& multiple,
...
CC_FILE_ERROR MAFilter::saveToFile(ccHObject* entity,
    ...
    if (!theMesh) {
        CVLog::Error("[MA] This filter can only save one mesh at a time!");
    ...
    // if (theCloud->isA(CV_TYPES::POINT_CLOUD))
    //	sf =
    // static_cast<ccPointCloud*>(theCloud)->getCurrentDisplayedScalarField();

    // if (!sf)
    //	CVLog::Warning("No displayed scalar field! Values will all be 0!\n");

    // open ASCII file for writing
    FILE* fp = fopen(qPrintable(filename), "wt");

    if (!fp) return CC_FERR_WRITING;
    ...
    cloudViewer::NormalizedProgress nprogress(
            pDlg.data(), ((2 + coloursAdjustment) * numberOfTriangles +
                          (3 + coloursAdjustment) * numberOfVertexes));
    ...
    if (fprintf(fp,
                "\tsetAttr \".clst[0].clsn\" -type \"string\" "
                "\"colorSet%i\";\n",
                currentMesh + 1) < 0) {
        fclose(fp);
        return CC_FERR_WRITING;
    ...
    if (hasColors) {
        ...
        if (fprintf(fp,
                    "createNode mesh -n \"polySurfaceShape%i\" -p "
                    "\"Mesh%i\";\n",
                    currentMesh + 1, currentMesh + 1) < 0) {
            fclose(fp);
            return CC_FERR_WRITING;
        ...
        if (fprintf(fp,
                    "\tsetAttr \".uvst[0].uvsn\" -type \"string\" "
                    "\"map1\";\n") < 0) {
            fclose(fp);
            return CC_FERR_WRITING;
        }
    ...
    int hard = 0;  // Maya edges cab be "hard" or "soft" ...
    {
        ...
        for (unsigned i = 0; i < numberOfTriangles; ++i) {
            const cloudViewer::VerticesIndexes* tsi =
                    theMesh->getNextTriangleVertIndexes();  // DGM:
            ...
            for (unsigned char k = 0; k < 3; ++k) {
                ...
                if (currentEdgeIndex < 0)  // create a new edge
                {
                    ...
                    // newEdge->edgeIndex = ++lastEdgeIndexPushed; //don't write
                    // the edge right now
                    newEdge->edgeIndex = 0;
                    ++lastEdgeIndexPushed;
    ...
    // now write the edges
    {
        ...
        if (fprintf(fp, "\tsetAttr -s %u \".ed[0:%u]\"", numberOfEdges,
                    numberOfEdges - 1) < 0) {
            fclose(fp);
    ...
    // vertex colors
    if (hasColors) {
        assert(theCloud->isA(CV_TYPES::POINT_CLOUD));
        ccPointCloud* pc = static_cast<ccPointCloud*>(theCloud);

        if (fprintf(fp,
                    "createNode polyColorPerVertex -n "
                    "\"polyColorPerVertex%i\";\n",
                    currentMesh + 1) < 0) {
            fclose(fp);
            return CC_FERR_WRITING;
        }
        ...
        {
            for (unsigned i = 0; i < numberOfVertexes; ++i) {
                const ecvColor::Rgb& c = pc->getPointColor(i);
                ecvColor::Rgbf col(static_cast<float>(c.r) / ecvColor::MAX,
                ...
                if (nf > 0) {
                    if (fprintf(fp, "\tsetAttr -s %i \".vclr[%u].vfcl\";\n", nf,
                                i) < 0) {
                        fclose(fp);
                        delete[] theFacesIndexes;  // DGM: we are missing some
                                                   // faces here, aren't we?
                        return CC_FERR_WRITING;
                    }
                    ...
                    while (f) {
                        if (fprintf(fp,
                                    "\tsetAttr \".vclr[%u].vfcl[%i].frgb\" "
                                    "-type \"float3\" %f %f %f;\n",
                                    i, f->faceIndex, col.r, col.g, col.b) < 0) {
                            fclose(fp);
                            delete[] theFacesIndexes;  // DGM: we are missing
                                                       // some faces here,
                                                       // aren't we?
                            return CC_FERR_WRITING;
                        }
    ...
    if (hasColors) {
        if (fprintf(fp,
                    "connectAttr \"polyColorPerVertex%i.out\" "
                    "\"MeshShape%i.i\";\n",
                    currentMesh + 1, currentMesh + 1) < 0 ||
            fprintf(fp,
                    "connectAttr \"polySurfaceShape%i.o\" "
                    "\"polyColorPerVertex%i.ip\";\n",
                    currentMesh + 1, currentMesh + 1) < 0) {
            fclose(fp);
            return CC_FERR_WRITING;
        }
    ...
    if (fprintf(fp,
                "connectAttr \"MeshShape%i.iog\" \":initialShadingGroup.dsm\" "
                "-na;\n",
                currentMesh + 1) < 0) {
        fclose(fp);
        return CC_FERR_WRITING;
