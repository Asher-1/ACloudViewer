...
bool LASFWFFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
CC_FILE_ERROR LASFWFFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	if (!cloud)
	{
		CVLog::Warning("[LAS_FWF] This filter can only save one cloud at a time!");
	...
	try
	{
		...
		laswriteopener.set_file_name(CVTools::fromQString(filename).c_str());
		...
		if (hasFWF)
		{
			...
			{
				CVLog::Warning(QString("[LAS_FWF] An error occurred while writing the FWF data file!\n(%1)").arg(fwFilename));
				hasFWF = false;
			}
			else
			{
				CVLog::Print(QString("[LAS_FWF] FWF data file written: %1").arg(fwFilename));
			}
		}
		...
		{
			progressDialog.reset(new ecvProgressDialog(false, parameters.parentWidget));
...
bool PrepareLASField(ccScalarField*& field, LAS_FIELDS type, unsigned totalCount, unsigned currentCount, ScalarType defaultValue = 0)
{
	...
	{
		CVLog::Warning(QString("[LAS] Not enough memory to load a field: '%1'").arg(LAS_FIELD_NAMES[type]));
		field->release();
		field = 0;
		return false;
	}
...
CC_FILE_ERROR LASFWFFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		lasreadopener.set_file_name(CVTools::fromQString(filename).c_str());
		...
		CVLog::Print(QString("[LASLib] " + QObject::tr("Reading %1 points").arg(pointCount)));

		//progress dialog
		QScopedPointer<ecvProgressDialog> progressDialog(0);
		if (parameters.parentWidget)
		{
			progressDialog.reset(new ecvProgressDialog(true, parameters.parentWidget));
		...
		for (int fakeIteration = 0; hasFWF && fakeIteration < 1; ++fakeIteration)
		{
			...
			{
				CVLog::Warning(QString("Not enough memory to import the waveform data"));
				hasFWF = false;
				break;
			}
			...
			if (!fwfReader)
			{
				CVLog::Warning(QString("Failed to open/read the associated waveform data packets"));
				hasFWF = false;
				break;
			}
			...
			if (lasreader->header.start_of_waveform_data_packet_record != 0)
			{
				...
				{
					CVLog::Warning(QString("Failed to read the associated waveform data packets"));
					hasFWF = false;
					break;
				}
				...
				{
					CVLog::Warning(QString("Failed to find the associated waveform data packets header"));
					hasFWF = false;
					break;
				}
				...
				if (fwfDataCount == 0)
				{
					CVLog::Warning(QString("Invalid waveform data packet size (0). We'll load all the remaining part of the file!"));
			...
			else
			{
				...
				if (!fwfDataSource.open(QFile::ReadOnly))
				{
					CVLog::Warning(QString("Failed to read the associated waveform data packets file (looking for '%1')").arg(wdpFilename));
					hasFWF = false;
					break;
				}
			...
			if (cloud->size() == 0) //first point
			{
				...
				if (lasShift.norm2() != 0 && (!parameters.coordinatesShiftEnabled || !*parameters.coordinatesShiftEnabled))
				{
					...
					if (	csModeBackup != ecvGlobalShiftManager::NO_DIALOG
						&&	csModeBackup != ecvGlobalShiftManager::NO_DIALOG_AUTO_SHIFT)
					{
			...
			if (!ignoreColors)
			{
				if (!hasColors)
				{
					...
					if (mergedColorComp != 0)
					{
						...
						{
							//not enough memory!
							CVLog::Warning("[LAS] Not enough memory to load RGB colors!");
							ignoreColors = true;
						}
						else
						{
				...
				if (hasColors)
				{
					...
					ecvColor::Rgb color(	static_cast<unsigned char>((point.rgb[0] >> colorBitDec) & 255),
