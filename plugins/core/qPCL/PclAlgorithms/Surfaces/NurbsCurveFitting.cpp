...
NurbsCurveFitting::NurbsCurveFitting()
	: BasePclModule(PclModuleDescription(tr("BSpline Curve Fitting"),
...
int NurbsCurveFitting::openInputDialog()
{
	...
	if (!m_dialog)
		m_dialog = new NurbsCurveFittingDlg(m_app ? m_app->getActiveWindow() : 0);
...
void NurbsCurveFitting::getParametersFromDialog()
{
	...
	m_exportProjectedCloud =	m_dialog->exportProjectedCloudCheckBox->isChecked();
	m_closed =					m_dialog->closedCurveCheckBox->isChecked();
	m_curveFitting3D =			m_dialog->curve3DFittingCheckBox->isChecked();
	m_exportProjectedCloud = m_closed && m_curveFitting3D ? false : m_exportProjectedCloud;
	m_curveRscale =		static_cast<float>(m_dialog->curveRscaleSpinBox->value());
	m_curveSmoothness =	static_cast<float>(m_dialog->curveSmoothnessSpinBox->value());
}
...
int NurbsCurveFitting::compute()
{
	...
	if (m_useVoxelGrid)
	{
		double leafSize = 3 * PCLModules::ComputeCloudResolution<PointT>(xyzCloud);
		if (leafSize > 0)
		{
	...
	if (m_curveFitting3D)
	{
		if (!PCLModules::BSplineCurveFitting3D<PointT>(
			...
			m_curveResolution, m_curveSmoothness, m_curveRscale, m_closed))
		{
			return -1;
		}
	...
	else
	{
		...
		PCLModules::CurveFittingMethod fittingMethod = (PCLModules::CurveFittingMethod) m_minimizationType;
	...
	if (!m_closed || !m_curveFitting3D)
	{
		...
		Eigen::Matrix4f inverseXoyTransformation(makeZPosMatrix.inverse().data());
		pcl::transformPointCloud(*outCurve, *outCurve, inverseXoyTransformation);
	}
	...
	ccPolyline* curvePoly = nullptr;
	{
		...
		CVLog::Print(tr("[NurbsCurveFitting] curve : %1 point(s)").arg(curvePoly->size()));
	}

	ccHObject* outObject = nullptr;
	if (m_exportProjectedCloud)
	{
		outObject = new ccHObject(
			cloud ? cloud->getName() : "" + QString(" [curve-fitting]-"));
		if (outObject)
		{
	...
	{
		if (cloud->getParent())
			cloud->getParent()->addChild(outObject);
...
QString NurbsCurveFitting::getErrorMessage(int errorCode)
{
	...
	case -53:
		return tr("Nurbs Curve Fitting does not returned any point. Try relaxing your parameters");
	}
