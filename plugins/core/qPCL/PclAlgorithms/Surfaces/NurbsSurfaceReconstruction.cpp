...
NurbsSurfaceReconstruction::NurbsSurfaceReconstruction()
	: BasePclModule(PclModuleDescription(tr("Nurbs Surface Triangulation"),
...
void NurbsSurfaceReconstruction::getParametersFromDialog()
{
	...
	m_interiorSmoothness =	static_cast<float>(m_dialog->interiorSSpinBox->value());
	m_boundaryWeight =		static_cast<float>(m_dialog->boundaryWSpinBox->value());
	m_boundarySmoothness =	static_cast<float>(m_dialog->boundarySSpinBox->value());
}

int NurbsSurfaceReconstruction::checkParameters()
{
	return 1;
}
...
int NurbsSurfaceReconstruction::compute()
{
	...
	{
		if (!PCLModules::NurbsSurfaceFitting<PointT>(
			xyzCloud, nurbsParams, mesh, outCurve))
		{
			return -1;
		}
	}
	...
	{
		if (!PCLModules::NurbsSurfaceFitting<PointT>(
			xyzCloud, nurbsParams, mesh, nullptr))
		{
			return -1;
		}
	}
	...
	CVLog::Print(tr("[NurbsSurfaceReconstruction] %1 points, %2 face(s)").arg(vertCount).arg(faceCount));
	...
	{
		if (cloud->getParent())
			cloud->getParent()->addChild(curvePoly);
...
QString NurbsSurfaceReconstruction::getErrorMessage(int errorCode)
{
	...
	case -53:
		return tr("Nurbs Surface Triangulation does not returned any point. Try relaxing your parameters");
	}
