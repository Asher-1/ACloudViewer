...
bool SoiFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
	//not supported
	return false;
}
...
CC_FILE_ERROR SoiFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	while ((strcmp((char*)line.substr(0, 4).c_str(), "#CC#") != 0) && (eof != nullptr))
	{
		if (strcmp(line.substr(0, 4).c_str(), "#NP#") == 0)
		{
			...
			//CVLog::Print("[SoiFilter::loadFile] Total number of points: %i",nbPointsTotal);
		}
		else if (strcmp(line.substr(0, 4).c_str(), "#NS#") == 0)
		{
			std::string numScans(line, 4, line.size() - 4);
			nbScansTotal = strtol(numScans.c_str(), &pEnd, 0);
			//CVLog::Print("[SoiFilter::loadFile] Total number of scans: %i",nbScansTotal);
		}
	...
	{
		CVLog::Warning("[SoiFilter::loadFile] No points or scans defined in this file!");
		fclose(fp);
	...
	{
		pDlg.reset(new ecvProgressDialog(false, parameters.parentWidget)); //cancel is not supported
		pDlg->setMethodTitle(QObject::tr("Open SOI file"));
		pDlg->setInfo(QObject::tr("%1 scans / %2 points").arg(nbScansTotal).arg(nbPointsTotal));
		pDlg->start();
	}
	CVLib::NormalizedProgress nprogress(pDlg.data(), nbPointsTotal);
	...
	for (unsigned k = 0; k < nbScansTotal; k++)
	{
		...
		if (strcmp(line.substr(0, 4).c_str(), "#pt#") == 0)
		{
			...
			//CVLog::Print("[SoiFilter::loadFile] Scan %i - points: %i",k+1,nbOfPoints);
		}
		else
		{
