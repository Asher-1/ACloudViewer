...
bool IcmFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
CC_FILE_ERROR IcmFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	{
		CVLog::Warning(QString("[ICM] Input filename contains special characters. It might be rejected by the I/O filter..."));
	}
	...
	if (!filter)
	{
		CVLog::Warning(QString("[ICM] No I/O filter found for loading file '%1' (type = '%2')").arg(cloudFileName,subFileType));
		fclose(fp);
...
int IcmFilter::LoadCalibratedImages(ccHObject* entities, const QString& path, const QString& imageDescFilename, const ccBBox& globalBBox)
{
	...
	{
		CVLog::Warning(QString("[ICM] File '%1' contains special characters. It might be rejected by the I/O filter...").arg(completeImageDescFilename));
	}

	FILE* fp = fopen(qPrintable(completeImageDescFilename), "rt");
	if (fp == nullptr)
	{
		CVLog::Error(QString("[IcmFilter::loadCalibratedImages] Error opening file %1!").arg(completeImageDescFilename));
		return -1;
	}
	...
	while (fgets(line, MAX_ASCII_FILE_LINE_LENGTH , fp) != nullptr)
	{
		if (line[0] == 'D' && line[1] == 'E' && line[2] == 'F')
		{
			...
			{
				CVLog::Warning(QString("[IcmFilter] Failed to load image %1 (%2)! Process stopped...").arg(imageFileName,errorStr));
				delete CI;
				fclose(fp);
				return loadedImages;
			}
			...
			float fov_deg = fov_rad*static_cast<float>(CV_RAD_TO_DEG);
			...
			CVLog::Print("\t Camera orientation=(%f,%f,%f)+[%f]",axis[0],axis[1],axis[2],angle_rad);
