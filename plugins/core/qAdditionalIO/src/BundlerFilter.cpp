...
bool BundlerFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
	//no output yet
	return false;
}
												...
												float _scaleFactor/*=1.0f*/)
{
	...
	{
		CVLog::Error("Only version 0.3 and 0.4 of Bundler files are supported!");
	...
	if (list.size() != 2)
	{
		CVLog::Error("[Bundler] Second line should be <num_cameras> <num_points>!");
		return CC_FERR_MALFORMED_FILE;
	}
	...
	//Read Bundler '.out' file
	{
		...
		if (!useAltKeypoints && (importKeypoints || orthoRectifyImages || generateColoredDTM))
		{
			...
			if (importKeypoints)
			{
				...
				if (!hasColors)
					CVLog::Warning("[Bundler] Not enough memory to load colors!");
				else
			...
			for (unsigned i = 0; i < ptsCount; ++i)
			{
				...
				if (i == 0)
				{
					...
					if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, parameters))
					{
						...
						CVLog::Warning("[Bundler] Cloud has been recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
					}
				}
				...
				if (colorParts.size() == 3)
				{
					if (hasColors)
					{
						...
						keypointsCloud->addRGBColor(static_cast<ColorCompType>(std::min<int>(R, ecvColor::MAX)),
													static_cast<ColorCompType>(std::min<int>(G, ecvColor::MAX)),
													static_cast<ColorCompType>(std::min<int>(B, ecvColor::MAX)));
					}
				...
				else if (colorParts.size() > 3)
				{
					...
					CVLog::Warning("[Bundler] Keypoint #%i has no associated color!",i);
					if (hasColors)
				...
				if (storeKeypoints || !camUsage.empty())
				{
					...
					if (!parts.isEmpty())
					{
						...
						{
							CVLog::Warning("[Bundler] View list for point #%i is invalid!",i);
						}
						else
						{
							unsigned pos = 1;
							for (unsigned n=0; n<nviews; ++n)
							{
								...
								if (storeKeypoints)
								{
									...
									{
										CVLog::Warning("[Bundler] Not enough memory to store keypoints!");
			...
			if (!camUsage.empty())
			{
				...
				{
					if (!camUsage[i])
						CVLog::Warning(QString("[Bundler] Camera #%1 has no associated keypoints!").arg(i+1));
				}
			}
			...
			if (applyOptMatrix)
			{
				...
				CVLog::Print("[Bundler] Keypoints cloud has been transformed with input matrix!");
	...
	if (useAltKeypoints)
	{
		...
		if (	!altKeypointsContainer
			...
			||	(!altKeypointsContainer->getChild(0)->isKindOf(CV_TYPES::POINT_CLOUD) && !altKeypointsContainer->getChild(0)->isKindOf(CV_TYPES::MESH)))
		{
			if (!altKeypointsContainer)
				CVLog::Error(QString("[Bundler] Failed to load alternative keypoints file:\n'%1'").arg(altKeypointsFilename));
			else
				CVLog::Error("[Bundler] Can't use this kind of entities as keypoints (need one and only one cloud or mesh)");
	...
	QStringList imageFilenames;
	{
		...
		{
			CVLog::Error(QString("[Bundler] Failed to open image list file! (%1)").arg(imageListFilename));
		...
		for (unsigned lineIndex=0; lineIndex<camCount; ++lineIndex)
		{
			...
			{
				CVLog::Error(QString("[Bundler] Couldn't extract image name from line %1 in file '%2'!").arg(lineIndex).arg(imageListFilename));
				break;
			}
		}
	}
	...
	if (imageFilenames.size() < static_cast<int>(camCount)) //not enough images!
	{
		if (imageFilenames.isEmpty())
			CVLog::Error(QString("[Bundler] No filename could be extracted from file '%1'!").arg(imageListFilename));
		else
			CVLog::Error(QString("[Bundler] Only %1 filenames (out of %2) could be extracted\nfrom file '%3'!").arg(imageFilenames.size()).arg(camCount).arg(imageListFilename));
		if (!importKeypoints && keypointsCloud)
			delete keypointsCloud;
		if (!importKeypoints && altEntity)
			delete altEntity;
	...
	if (parameters.parentWidget)
	{
		ipDlg.reset(new ecvProgressDialog(true, parameters.parentWidget)); //cancel available
	...
	if (generateColoredDTM)
	{
		...
		if (parameters.parentWidget)
		{
			toDlg.reset(new ecvProgressDialog(true, parameters.parentWidget)); //cancel available
		...
		if (!baseDTMMesh)
		{
			...
			if (!dummyMesh)
			{
				CVLog::Warning(QString("[Bundler] Failed to generate DTM! (%1)").arg(errorStr));
			}
		}
		...
		if (dummyMesh)
		{
			...
			if (mntSamples)
			{
				...
				{
					//not enough memory
					CVLog::Error("Not enough memory to store DTM colors! DTM generation cancelled");
					delete mntSamples;
					mntSamples = 0;
	...
	for (unsigned i = 0; i < camCount; ++i)
	{
		...
		{
			CVLog::Error(QString("[Bundler] %1 (image '%2')").arg(errorStr, imageFilenames[i]));
			delete image;
			image = 0;
			break;
		}
		...
		ccCameraSensor* sensor = 0;
		{
			...
			if (applyOptMatrix)
			{
				...
				//CVLog::Print("[Bundler] Camera cloud has been transformed with input matrix!");
				//this transformation is of no interest for the user
		...
		if (orthoRectifyImages)
		{
			...
			{
				CVLog::Warning(QString("[Bundler] Not enough keypoints descriptors for image '%1'!").arg(image->getName()));
			}
			else if (!keypointsImageBB.isValid() || keypointsImageBB.getDiagNorm() < PC_ONE)
			{
				CVLog::Warning(QString("[Bundler] Keypoints descriptors for image '%1' are invalid (= all the same)").arg(image->getName()));
			}
			else
			{
				if (orthoRectifyImagesAsImages)
				{
					...
					if (orthoImage)
					{
						...
						CVLog::Print(QString("[Bundler] Ortho-rectified version of image '%1' (%2 x %3) saved to '%4'").arg(imageFilenames[i]).arg(info.w).arg(info.h).arg(imageDir.absoluteFilePath(info.name)));
...
#define TBI_DEFAULT_TILE_POW 10 // 2^10 = 1024
					...
					{
						CVLog::Warning(QString("[Bundler] Failed to ortho-rectify image '%1'!").arg(image->getName()));
					}
				}
				...
				if (orthoRectifyImagesAsClouds)
				{
					...
					{
						CVLog::Warning(QString("[Bundler] Failed to ortho-rectify image '%1' as a cloud!").arg(image->getName()));
					}
				}
			}
		}
		...
		if (sensor && undistortImages)
			if (!sensor->undistort(image, true))
				CVLog::Warning(QString("[Bundler] Failed to undistort image '%1'!").arg(image->getName()));
	...
	if (orthoRectifyImages && OR_pixelSize > 0)
	{
		...
		{
			CVLog::Warning("Failed to save orthorectification log file! (ortho_rectification_log.txt)");
		}
	}
			...
			if (mntCloud->reserve(sampleCount) && mntCloud->reserveTheRGBTable())
			{
				...
				for (unsigned i=0; i<sampleCount; ++i,col+=4)
				{
					if (col[3] > 0) //accum
					{
						...
						ecvColor::Rgb avgCol(static_cast<ColorCompType>(col[0] / col[3]),
				...
				if (realCount != 0)
				{
					...
					CVLog::Warning("[Bundler] DTM vertices successfully generated: clean it if necessary then use 'Edit > Mesh > Compute Delaunay 2D (Best LS plane)' then 'Smooth' to get a proper mesh");
					...
					if (!parameters.alwaysDisplayLoadDialog)
					{
						...
						if (bf.saveToFile(mntCloud, outputFile, parameters) == CC_FERR_NO_ERROR)
							CVLog::Print(QString("[Bundler] Color DTM vertices automatically saved to '%2'").arg(outputFile));
						else
							CVLog::Warning(QString("[Bundler] Failed to save DTM vertices to '%2'").arg(outputFile));
					}
				}
				else
				{
					CVLog::Warning("[Bundler] Failed to generate DTM! (no point viewed in images?)");
				}
			}
			else
			{
				CVLog::Warning("[Bundler] Failed to generate DTM vertices cloud! (not enough memory?)");
				delete mntCloud;
				mntCloud = 0;
			}
		}
