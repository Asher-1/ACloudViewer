...
void DxfProfilesExportDlg::initFromPersistentSettings()
{
	...
	const QString	defaultVertProfile( ecvFileUtils::defaultDocPath() + "/vert_profiles.dxf" );
	const QString	defaultHorizProfile( ecvFileUtils::defaultDocPath() + "/horiz_profiles.dxf" );
...
{
	return vertProfilesGroupBox->isChecked() ? 
		vertOutputFileLineEdit->text() :
		QString();
}
...
{
	return horizProfilesGroupBox->isChecked() ? 
		horizOutputFileLineEdit->text() : QString();
}
