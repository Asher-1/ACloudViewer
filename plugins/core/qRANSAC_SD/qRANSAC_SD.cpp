...
static double   s_proba				= 0.75;	// default : 0.01, probability that no better candidate was overlooked during sampling
static bool		s_primEnabled[5]	= {true, true, true, false, false};

static float	s_minRadius = 0.001f;		// minimum radius for sphere, cylinder, cone, torus
static float	s_maxRadius = 0.5f;			// maximum radius for sphere, cylinder, cone, torus
...
void qRansacSD::doAction()
{
	...
	//for parameters persistence
	{
		// update min radius and max radius limits
		{
		...
		//consistency check
		{
			...
			if (primCount==0)
			{
				m_app->dispToConsole(tr("No primitive type selected!"), ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				return;
			}
		}
	...
	RansacShapeDetector::Options ransacOptions;
	{
		...
		ransacOptions.m_normalThresh	= static_cast<float>(cos(rsdDlg.maxNormDevAngleSpinBox->value() * CV_DEG_TO_RAD));
	...
	if (!hasNorms)
	{
		QProgressDialog pDlg(tr("Computing normals (please wait)"),
			QString(), 0, 0, m_app->getActiveWindow());
		pDlg.setWindowTitle(tr("Ransac Shape Detection"));
		pDlg.show();
		...
		{
			m_app->dispToConsole(tr("Not enough memory to compute normals!"),
				ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
			return;
		}
	}
...

	{
		...
		QProgressDialog pDlg(tr("Operation in progress (please wait)"), QString(),0,0,m_app->getActiveWindow());
		pDlg.setWindowTitle(tr("Ransac Shape Detection"));
		pDlg.show();
		QApplication::processEvents();
	...
	if (remaining == count)
	{
		m_app->dispToConsole(tr("Segmentation failed..."), 
			ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	if (shapes.size() > 0)
	{
		...
		for (MiscLib::Vector<DetectedShape>::const_iterator it = shapes.begin(); it != shapes.end(); ++it)
		{
			...
			{
				m_app->dispToConsole(tr("Inconsistent result!"),
					ecvMainAppInterface::ERR_CONSOLE_MESSAGE);
				break;
			}
			...
			case 1: //sphere
				...
				if (radius < s_minRadius || radius > s_maxRadius)
				{
				...
				pcShape->setName(tr("Sphere (r=%1)").arg(radius,0,'f'));
			...
			case 2: //cylinder
				...
				if (r < s_minRadius || r > s_maxRadius)
				{
					count -= shapePointsCount;
					delete pcShape;
					continue;
				...
				pcShape->setName(tr("Cylinder (r=%1/h=%2)").arg(r,0,'f').arg(h,0,'f'));
			...
			case 3: //cone
				...
				if (minRadius < s_minRadius || maxRadius > s_maxRadius)
				{
					count -= shapePointsCount;
					delete pcShape;
					continue;
				...
				pcShape->setName(tr("Cone (alpha=%1/h=%2)").arg(alpha,0,'f').arg(maxHeight-minHeight,0,'f'));
			...
			case 4: //torus
				...
				{
					m_app->dispToConsole(
						tr("[qRansacSD] Apple-shaped torus are not handled by CLOUDVIEWER !"),
				...
				else
				{
					...
					pcShape->setName(tr("Torus (r=%1/R=%2)").arg(minRadius,0,'f').arg(maxRadius,0,'f'));
			...
			if (!group)
				group = new ccHObject(tr("Ransac Detected Shapes (%1)").arg(ent->getName()));
		...
		if (group)
		{
			...
			pc->setEnabled(false);
			m_app->dispToConsole(
				tr("[qRansacSD] Input cloud has been automtically hidden!"),
				ecvMainAppInterface::WRN_CONSOLE_MESSAGE);
