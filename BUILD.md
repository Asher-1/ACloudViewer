...
| **macOS** | 13+ (Intel or Apple Silicon) | Xcode CLI (`xcode-select --install`) | ARM64 / x64 |
...
| | | **Linux:** `libAICore.so` links `CUDA::cudart_static` (no DT_NEEDED on `libcudart.so.*`). MMQ kernels replace cuBLAS (no `libcublas.so.*`). **Only the NVIDIA driver is needed at runtime** — no CUDA toolkit required. |
...
| `AICore_BUNDLE_CUDA_RUNTIME` | OFF | **`option()`** in `cmake/AICoreOptions.cmake`: redist CUDA runtime into installer; **requires `AICore_USE_CUDA=ON`**. See per-platform notes above — on Linux this option is **deprecated** (cudart_static + FORCE_MMQ eliminate the need); on Windows bundles cudart64_*.dll etc. into `lib/cuda-runtime/` (auto-added to `%PATH%` by `ACloudViewer.bat`). |
...
> **Windows only:** For driver-only CUDA installers, add
> `-DAICore_BUNDLE_CUDA_RUNTIME=ON` to bundle CUDA runtime DLLs into
> `lib/cuda-runtime/`. On Linux this option is **no-op** (cudart_static +
> FORCE_MMQ eliminate the need).
...
> explicitly requested with `AICore_USE_CUDA=ON`. On Linux, `cudart_static` +
> FORCE_MMQ eliminate the need for a CUDA Toolkit — only the NVIDIA driver is
> required at runtime. Vulkan users need only a compatible GPU driver/ICD.
>
...
but on Linux it is **no-op** (the option exists for cross-platform script
compatibility):
```bash
# Linux (bundle_cuda_runtime has no effect):
build_gui_app with_conda package_installer with_aicore_cuda
```
