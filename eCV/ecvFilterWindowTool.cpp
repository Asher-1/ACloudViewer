...
ecvFilterWindowTool::ecvFilterWindowTool(QMainWindow* parent)
    ...
    connect(resetButton, &QToolButton::clicked, this,
            &ecvFilterWindowTool::reset);
    connect(cancelButton, &QToolButton::clicked, this,
            &ecvFilterWindowTool::cancel);
    connect(exportButton, &QToolButton::clicked, this,
            &ecvFilterWindowTool::exportSlice);
    connect(exportMultButton, &QToolButton::clicked, this,
            &ecvFilterWindowTool::exportMultSlices);
    connect(extractContourToolButton, &QToolButton::clicked, this,
            &ecvFilterWindowTool::extractContour);
    connect(removeLastContourToolButton, &QToolButton::clicked, this,
            &ecvFilterWindowTool::removeLastContour);
    connect(restoreToolButton, &QToolButton::clicked, this,
            &ecvFilterWindowTool::restoreLastBox);
    ...
    // selection modes
    connect(actionSegmentationExtraction, &QAction::triggered, this,
            &ecvFilterWindowTool::doSetClipMode);
    connect(actionPolylineExtraction, &QAction::triggered, this,
            &ecvFilterWindowTool::doSetPolylineMode);
    ...
    connect(this, &ccOverlayDialog::shortcutTriggered, this,
            &ecvFilterWindowTool::onShortcutTriggered);
}

ecvFilterWindowTool::~ecvFilterWindowTool() { releaseAssociatedEntities(); }
...
void ecvFilterWindowTool::cancel() { stop(true); }
...
void ecvFilterWindowTool::stop(bool accepted) {
    reset();
    MainWindow::TheInstance()->addWidgetToQMdiArea(
            ecvDisplayTools::GetMainScreen());
...
void ecvFilterWindowTool::removeLastContour() {
    if (s_lastContourUniqueIDs.empty()) return;

    MainWindow* mainWindow = MainWindow::TheInstance();
    if (mainWindow) {
        for (size_t i = 0; i < s_lastContourUniqueIDs.size(); ++i) {
            ccHObject* obj = mainWindow->db()->find(
                    static_cast<int>(s_lastContourUniqueIDs[i]));
            if (obj) {
...
ccHObject* ecvFilterWindowTool::getSlice(ccHObject* obj, bool silent) {
    ...
    if (obj->isKindOf(CV_TYPES::POINT_CLOUD)) {
        ...
        if (!mesh) {
            if (!silent) CVLog::Error("Failed to segment the mesh!");
            return nullptr;
        }
        return mesh;
    }
    ...
    for (unsigned char d = 0; d < 3; ++d) {
        if (processDim[d]) {
            if (cloudViewer::LessThanEpsilon(cellSizePlusGap.u[d])) {
                CVLog::Error(
                        "Box size (plus gap) is null! Can't apply repetitive "
                        "process!");
                return 0;
            }

            // don't forget the user defined gap between 'cells'
            PointCoordinateType a =
                    (localBox.minCorner().u[d] - gridOrigin.u[d]) /
                    cellSizePlusGap.u[d];
            PointCoordinateType b =
                    (localBox.maxCorner().u[d] - gridOrigin.u[d]) /
                    cellSizePlusGap.u[d];

            indexMins[d] = static_cast<int>(
                    floor(a + static_cast<PointCoordinateType>(1.0e-6)));
            indexMaxs[d] =
                    static_cast<int>(ceil(
                            b - static_cast<PointCoordinateType>(1.0e-6))) -
                    1;
...
ccBBox ecvFilterWindowTool::getOwnBB() const { return m_box; }
...
bool ecvFilterWindowTool::extractSlicesAndContours(
    ...
    if (!singleContourMode && repeatDimensionsSum == 0) {
        assert(false);
        CVLog::Error(
                "No dimension selected to repeat the segmentation process?!");
        return false;
    }
    ...
    try {
        ...
        } else  // repeat mode
        {
            if (!clouds.empty())  // extract sections from clouds
            {
                ...
                unsigned cellCount = ComputeGridDimensions(
                        localBox, repeatDimensions, indexMins, indexMaxs,
                        gridDim, gridOrigin, cellSizePlusGap);

                // we'll potentially create up to one (ref.) cloud per input
                // loud and per cell
                ...
                for (size_t ci = 0; ci != clouds.size(); ++ci) {
                    ...
                    cloudViewer::NormalizedProgress nProgress(progressDialog,
                                                              pointCount);
                    for (unsigned i = 0; i < pointCount; ++i) {
                        ...
                        if (gap == 0 ||
                            ((P.x - static_cast<PointCoordinateType>(xi)) *
                                             cellSizePlusGap.x <=
                                     cellSize.x &&
                             (P.y - static_cast<PointCoordinateType>(yi)) *
                                             cellSizePlusGap.y <=
                                     cellSize.y &&
                             (P.z - static_cast<PointCoordinateType>(zi)) *
                                             cellSizePlusGap.z <=
                                     cellSize.z)) {
                            int cloudIndex =
                                    ((zi - indexMins[2]) *
                                     ...
                                     (yi - indexMins[1])) *
                                            static_cast<int>(gridDim[0]) +
                                    (xi - indexMins[0]);
                            assert(cloudIndex >= 0 &&
                                   static_cast<size_t>(cloudIndex) *
                                                           clouds.size() +
                                                   ci <
                                           refClouds.size());

                            cloudViewer::ReferenceCloud*& destCloud =
                                    refClouds[cloudIndex * clouds.size() + ci];
                            if (!destCloud) {
                                destCloud =
                                        new cloudViewer::ReferenceCloud(cloud);
                                ++subCloudsCount;
                            }
                ...
                if (progressDialog) {
                    progressDialog->setWindowTitle(
                            QObject::tr("Section extraction"));
                    progressDialog->setInfo(
                            QObject::tr("Section(s): %L1").arg(subCloudsCount));
                    progressDialog->setMaximum(
                            static_cast<int>(subCloudsCount));
                    ...
                    for (int j = indexMins[1]; j <= indexMaxs[1]; ++j) {
                        for (int k = indexMins[2]; k <= indexMaxs[2]; ++k) {
                            ...
                            for (size_t ci = 0; ci != clouds.size(); ++ci) {
                                ccGenericPointCloud* cloud = clouds[ci];
                                cloudViewer::ReferenceCloud* destCloud =
                                        refClouds[cloudIndex * clouds.size() +
                                                  ci];
                                if (destCloud)  // some slices can be empty!
                                {
                                    ...
                                    int warnings = 0;
                                    ccPointCloud* sliceCloud =
                                            cloud->isA(CV_TYPES::POINT_CLOUD)
                                                    ? static_cast<
                                                              ccPointCloud*>(
                                                              cloud)
                                                              ->partialClone(
                ...
                // release memory
                {
                    for (size_t i = 0; i < refClouds.size(); ++i)
                        if (refClouds[i]) delete refClouds[i];
                    refClouds.clear();
                }
            ...
            if (!meshes.empty())  // extract sections from meshes
            {
                ...
                if (progressDialog) {
                    ...
                    progressDialog->setInfo(
                            QObject::tr("Up to (%1 x %2 x %3) = %4 section(s)")
                                    .arg(gridDim[0])
                                    .arg(gridDim[1])
                                    .arg(gridDim[2])
                                    .arg(cellCount));
                    progressDialog->setMaximum(
                            static_cast<int>(cellCount * meshes.size()));
                    progressDialog->show();
                    QApplication::processEvents();
                }
                ...
                for (int i = indexMins[0]; i <= indexMaxs[0]; ++i) {
                    for (int j = indexMins[1]; j <= indexMaxs[1]; ++j) {
                        for (int k = indexMins[2]; k <= indexMaxs[2]; ++k) {
                            ...
                            CCVector3 C = gridOrigin +
                                          CCVector3(i * cellSizePlusGap.x,
                                                    j * cellSizePlusGap.y,
                                                    k * cellSizePlusGap.z);
                            ...
                            for (size_t mi = 0; mi != meshes.size(); ++mi) {
                                ...
                                ccHObject* croppedEnt = ccCropTool::Crop(
                                        mesh, cropBox, true, _transformation);
                                if (croppedEnt) {
                                    if (generateRandomColors) {
                                        ccPointCloud* croppedVertices =
                                                ccHObjectCaster::ToPointCloud(
                                                        mesh->getAssociatedCloud());
                                        if (croppedVertices) {
                                            ecvColor::Rgb col = ecvColor::
                                                    Generator::Random();
                                            if (!croppedVertices->setRGBColor(
                                                        col)) {
                                                CVLog::Error(
                                                        "Not enough memory!");
                                                error = true;
                                    ...
                                    QString slicePosStr =
                                            QString("(%1 ; %2 ; %3)")
                                                    ...
                                                    .arg(C.y)
                                                    .arg(C.z);
                                    croppedEnt->setName(QString("slice @ ") +
                                                        slicePosStr);
        ...
        if (!error && extractContours) {
            ...
            if (progressDialog) {
                ...
                progressDialog->setInfo(
                        QObject::tr("Contour(s): %L1").arg(filteredNum));
            ...
            if (repeatDimensionsSum == 1 && !projectOnBestFitPlane) {
                for (int i = 0; i < 3; ++i)
                    if (repeatDimensions[i]) preferredDim = i;
            }

            ccGLMatrix invLocalTrans = localTrans.inverse();
            PointCoordinateType* preferredOrientation =
                    (preferredDim != -1 ? invLocalTrans.getColumn(preferredDim)
                                        : 0);
            ...
            for (unsigned i = 0; i != filteredNum; ++i) {
                ...
                if (extractFlatContour(sliceCloud, multiPass, maxEdgeLength,
                                       polys, splitContours,
                                       preferredOrientation, visualDebugMode)) {
                    if (!polys.empty()) {
                        ...
                        CVLog::Warning(
                                QString("%1: points are too far from each "
                                        "other! Increase the max edge length")
                                        .arg(sliceCloud->getName()));
                        warningsIssued = true;
                    }
                } else {
                ...
                if (progressDialog && !visualDebugMode) {
                    if (progressDialog->wasCanceled()) {
                        error = true;
                        CVLog::Warning(
                                QString("[ExtractSlicesAndContours] Process "
                                        "canceled by user"));
                        // early stop
                        break;
                    }
        ...
        if (error) {
            ...
            CVLog::Warning(
                    "[ExtractSlicesAndContours] Warnings were issued during "
...
void ecvFilterWindowTool::extractSlicesAndContours(bool extractSlices,
                                                   bool extractContours,
                                                   bool singleContourMode) {
    ...
    ccClippingBoxRepeatDlg repeatDlg(singleContourMode,
                                     MainWindow::TheInstance());
    repeatDlg.extractContoursGroupBox->setEnabled(true);

    // by default we set the 'flat/repeat' dimension to the smallest box
    // dimension
    {
        CCVector3 diagVec = getOwnBB().getDiagVec();
        unsigned char flatDim = 0;
        if (diagVec.y < diagVec.x) flatDim = 1;
        if (diagVec.z < diagVec.u[flatDim]) flatDim = 2;
        if (singleContourMode)
    ...
    if (s_maxEdgeLength < 0) s_maxEdgeLength = getOwnBB().getDiagNorm();
    ...
    s_defaultGap = repeatDlg.gapDoubleSpinBox->value();
    bool projectOnBestFitPlane =
            repeatDlg.projectOnBestFitCheckBox->isChecked();
    ...
    if (!extractSlicesAndContours(
                clouds, meshes, m_box, singleContourMode, processDim,
                outputSlices, extractContours,
                static_cast<PointCoordinateType>(s_maxEdgeLength),
                outputContours, static_cast<PointCoordinateType>(s_defaultGap),
                s_multiPass, s_splitContours, projectOnBestFitPlane,
                visualDebugMode, generateRandomColors, &pDlg)) {
        // process failed (error message has already been issued)
        return;
    }

    CVLog::Print("[ecvFilterWindowTool] Processed finished in %.2f s.",
    ...
    if (!outputSlices.empty()) {
        ...
        QMessageBox::warning(0, "Process finished",
                             QString("%1 slices have been generated.\n(you may "
                                     "have to close the tool and hide the "
                                     "initial cloud to see them...)")
                                     .arg(sliceGroup->getChildrenNumber()));
        MainWindow::TheInstance()->addToDB(sliceGroup);
    } else if (!singleContourMode) {
        CVLog::Warning(
                "[ecvFilterWindowTool] Repeat process generated no output!");
    }

    // contour polylines
    if (!outputContours.empty()) {
        ccHObject* contourGroup = new ccHObject(
                baseName.isEmpty() ? QString("Contours")
                                   : baseName + QString(".contours"));
