...
public:
    ...
    QString exportEntity(CLEntityDesc& entityDesc,
                         ...
                         ccCommandLineInterface::ExportOptions options =
                                 ExportOption::NoOptions) override;
    ...
    bool error(const QString& message)
            const override;  // must always return false!
    bool saveClouds(QString suffix = QString(),
                    bool allAtOnce = false,
                    const QString* allAtOnceFileName = nullptr) override;
    ...
    bool importFile(QString filename,
                    ...
                    FileIOFilter::Shared filter =
                            FileIOFilter::Shared(nullptr)) override;
...
private:  // members
    //! Current cloud(s) export format (can be modified with the
    //! 'COMMAND_CLOUD_EXPORT_FORMAT' option)
    QString m_cloudExportFormat;
    //! Current cloud(s) export extension (warning: can be anything)
    QString m_cloudExportExt;
    //! Current mesh(es) export format (can be modified with the
    //! 'COMMAND_MESH_EXPORT_FORMAT' option)
    QString m_meshExportFormat;
    //! Current mesh(es) export extension (warning: can be anything)
    QString m_meshExportExt;
    //! Current hierarchy(ies) export format (can be modified with the
    //! 'COMMAND_HIERARCHY_EXPORT_FORMAT' option)
