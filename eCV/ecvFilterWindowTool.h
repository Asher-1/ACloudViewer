...
class ecvFilterWindowTool : public ccOverlayDialog,
                            public Ui::GraphicalFilteringWindowDlg {
    Q_OBJECT
public:
    //! Default constructor
    ...
    **/
    ...
    /** \param clouds input clouds (may be empty if meshes are defined)
            ...
            \param singleContourMode if true, a single cut is made (the process
    is not repeated) and only the contour is extracted (not the slice) \param
    processDimensions If singleContourMode is true: the dimension normal to the
    slice should be true (and the others false). Otherwise: the dimensions along
    which to repeat the cuting process should be true. \param outputSlices
    output slices (if successful) \param extractContours whether to extract
    contours or not \param maxEdgeLength max contour edge length (the smaller,
    the tighter the contour will be) \param outputContours output contours (if
    successful) \param gap optional gap between each slice \param multiPass
    multi-pass contour extraction \param splitContours whether to split the
    contour when the segment can't be smaller than the specified 'maxEdgeLength'
            \param projectOnBestFitPlane to project the points on the slice best
    fitting plane (otherwise the plane normal to the \param visualDebugMode
    displays a 'debugging' window during the contour extraction process \param
    generateRandomColors randomly colors the extracted slices \param
...
protected:
    ...
    void extractSlicesAndContours(bool extractSlices,
                                  bool extractContours,
                                  bool singleContourMode);

    bool extractFlatContour(ccPointCloud* sliceCloud,
                            bool allowMultiPass,
                            PointCoordinateType maxEdgeLength,
