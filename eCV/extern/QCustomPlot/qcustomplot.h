...
public:
        ...
        /*!
          Defines the different units in which the image resolution can be
          specified in the export functions.
...
enum PlottingHint {
    ...
    phFastPolylines =
            0x001  ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster
                   ...
                   ///< segment
                   ///<                joins, thus is most effective for pen
                   ///<                sizes larger than 1. It is only used for
                   ///<                solid line pens.
    ,
    phImmediateRefresh =
            0x002  ///< <tt>0x002</tt> causes an immediate repaint() instead of
                   ///< a soft update() when QCustomPlot::replot() is called
                   ///< with parameter \ref QCustomPlot::rpRefreshHint.
                   ///<                This is set by default to prevent the
                   ///<                plot from freezing on fast consecutive
                   ///<                replots (e.g. user drags ranges with
                   ///<                mouse).
    ,
...
enum Interaction {
    ...
    iSelectOther =
            0x080  ///< <tt>0x080</tt> All other objects are selectable (e.g.
                   ///< your own derived layerables, other layout elements,...)
};
...
public:
    explicit QCPPaintBufferGlPbuffer(const QSize &size,
...
protected:
    ...
    // non-virtual methods:
    void trimTicks(const QCPRange &range,
                   QVector<double> &ticks,
...
public:
    ...
    void setTickOrigin(double origin);  // hides base class method but calls
                                        // baseclass implementation ("using"
                                        // throws off IDEs and doxygen)
...
public:
    ...
    void limitIteratorsToDataRange(const_iterator &begin,
                                   const_iterator &end,
    ...
    if (oldSize > 0 &&
        !qcpLessThanSortKey<DataType>(
                *constBegin(), *(data.constEnd() -
                                 1)))  // prepend if new data keys are all
...
void QCPDataContainer<DataType>::add(const QVector<DataType> &data,
    ...
    if (alreadySorted && oldSize > 0 &&
        !qcpLessThanSortKey<DataType>(
                *constBegin(),
                *(data.constEnd() -
                  1)))  // prepend if new data is sorted and keys are all
                        // smaller than or equal to existing ones
    {
        if (mPreallocSize < n) preallocateGrow(n);
        mPreallocSize -= n;
...
void QCPDataContainer<DataType>::add(const DataType &data) {
    if (isEmpty() ||
        !qcpLessThanSortKey<DataType>(
                data,
                *(constEnd() - 1)))  // quickly handle appends if new data key
...
void QCPDataContainer<DataType>::performAutoSqueeze() {
                              ...
                              // respect to total used size
    {
        shrinkPostAllocation =
                postAllocSize >
                usedSize * 1.5;  // QVector grow strategy is 2^n for static
                                 // data. Watch out not to oscillate!
                      ...
                      // memory, below 1k points don't even bother
    {
        ...
        shrinkPreAllocation =
                mPreallocSize >
                usedSize * 1.5;  // preallocation can grow into postallocation,
                                 // so can be smaller
    }
...
protected:
    ...
    // non-virtual methods:
    double rectDistance(const QRectF &rect,
                        const QPointF &pos,
...
signals:
    ...
    void plottableClick(QCPAbstractPlottable *plottable,
                        int dataIndex,
                        QMouseEvent *event);
...
double QCPAbstractPlottable1D<DataType>::selectTest(const QPointF &pos,
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it = begin;
        ...
        if (keyRange.contains(mainKey) &&
            valueRange.contains(
                    mainValue))  // make sure data point is inside visible
                                 // range, for speedup in cases where sort key
                                 // isn't main key and we iterate over all
                                 // points
        {
    ...
    if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
        ...
        while (i < lineDataSize) {
            if (qIsNaN(lineData.at(i).y()) || qIsNaN(lineData.at(i).x()) ||
                qIsInf(lineData.at(i).y()))  // NaNs create a gap in the line.
                                             // Also filter Infs which make
                                             // drawPolyline block
            {
...
public:
    ...
    */
    ...
    // non-property methods:
    void addData(const QVector<double> &keys,
                 const QVector<double> &values,
                 bool alreadySorted = false);
...
public:
    ...
    */
    ...
    // non-property methods:
    void addData(const QVector<double> &t,
                 const QVector<double> &keys,
                 const QVector<double> &values,
                 bool alreadySorted = false);
...
protected:
    ...
    // non-virtual methods:
    void getCurveLines(QVector<QPointF> *lines,
                       const QCPDataRange &dataRange,
                       double penWidth) const;
    void getScatters(QVector<QPointF> *scatters,
                     const QCPDataRange &dataRange,
                     double scatterWidth) const;
    int getRegion(double key,
                  double value,
                  double keyMin,
                  double valueMax,
                  double keyMax,
...
public:
    ...
    QCPStatisticalBoxData(double key,
                          double minimum,
...
class QCP_LIB_DECL QCPErrorBars : public QCPAbstractPlottable,
    ...
    Q_OBJECT
    /// \cond INCLUDE_QPROPERTIES
    Q_PROPERTY(QSharedPointer<QCPErrorBarsDataContainer> data READ data WRITE
                       setData)
...
protected:
    ...
    // non-virtual methods:
    QLineF getRectClippedLine(const QCPVector2D &start,
