...
public:  // raster export
    ...
    static bool ExportGeoTiff(
            ...
            unsigned char Z,
            double customHeightForEmptyCells =
...
protected:  // standard methods
    ...
    ccRasterGrid::EmptyCellFillOption getFillEmptyCellsStrategyExt(
            double& emptyCellsHeight,
            double& minHeight,
            double& maxHeight) const;

    //! Returns whether a given field count should be exported as SF (only if a
    //! cloud is generated!)
    bool exportAsSF(ccRasterGrid::ExportableFields field) const;

    //! Returns whether the output cloud should use the original cloud or the
    //! grid as 'support'
