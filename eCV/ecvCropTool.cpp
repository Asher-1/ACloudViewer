...
ccHObject* ccCropTool::Crop(ccHObject* entity, const ccBBox& box, bool inside/*=true*/, const ccGLMatrix* meshRotation/*=0*/)
{
	...
	if (entity->isA(CV_TYPES::POINT_CLOUD))
	{
		...
		{
			//process failed!
			CVLog::Warning(QString("[Crop] Failed to crop cloud '%1'!").arg(cloud->getName()));
			return 0;
		}
		...
		if (selection->size() == 0)
		{
			...
			CVLog::Warning(QString("[Crop] No point of the cloud '%1' falls %2side the input box!").arg(cloud->getName(), (inside ? "in" : "out")));
			delete selection;
			return 0;
		}
	...
	else if (entity->isKindOf(CV_TYPES::MESH))
	{
		...
		if (meshRotation)
		{
			...
			if (!rotatedVertices)
			{
				CVLog::Warning(QString("[Crop] Failed to crop mesh '%1'! (not enough memory)").arg(mesh->getName()));
				return 0;
			}
		...
		if (tempMesh)
		{
			...
			if (croppedVertices)
			{
				...
				{
					//no points fall inside selection!
					CVLog::Warning(QString("[Crop] No triangle of the mesh '%1' falls %2side the input box!").arg(mesh->getName(), (inside ? "in" : "out")));
					delete croppedMesh;
					croppedMesh = 0;
				}
				else
				{
					...
					if (mesh->hasNormals())
					{
						...
						if (!success)
						{
							CVLog::Warning("[Crop] Failed to compute normals on the output mesh (not enough memory)");
						}
					...
					if (params.trackOrigIndexes)
					{
						...
						try
						{
							...
							if (origVertices->hasColors() || origVertices->hasScalarFields())
							{
								...
								if (origVertices->hasColors())
								{
									...
									if (!importColors)
										CVLog::Warning("[Crop] Failed to transfer RGB colors on the output mesh (not enough memory)");
								}
								...
								{
									origVertices_pc = origVertices->isA(CV_TYPES::POINT_CLOUD) ? static_cast<ccPointCloud*>(origVertices) : 0;
									...
									for (unsigned i = 0; i < sfCount; ++i)
									{
										...
										{
											CVLog::Warning("[Crop] Failed to transfer one or several scalar fields on the output mesh (not enough memory)");
							...
							if (mesh->hasMaterials())
							{
								...
								{
									CVLog::Warning("[Crop] Failed to transfer materials on the output mesh (not enough memory)");
								}
								...
								if (mesh->hasPerTriangleTexCoordIndexes())
								{
									...
									{
										CVLog::Warning("[Crop] Failed to transfer texture coordinates on the output mesh (not enough memory)");
										delete texCoords;
										texCoords = 0;
									}
								}
						...
						{
							CVLog::Warning("[Crop] Failed to transfer per-vertex features (color, SF values, etc.) on the output mesh (not enough memory)");
			...
			{
				CVLog::Warning("[Crop] Failed to create output mesh vertices (not enough memory)");
			}
		}
