...
ccHObject* ccCropTool::Crop(ccHObject* entity,
    ...
    if (entity->isA(CV_TYPES::POINT_CLOUD)) {
        ...
        if (!selection) {
            // process failed!
            CVLog::Warning(QString("[Crop] Failed to crop cloud '%1'!")
                                   .arg(cloud->getName()));
            return 0;
        }
        ...
        if (selection->size() == 0) {
            ...
            CVLog::Warning(
                    QString("[Crop] No point of the cloud '%1' falls %2side "
                            "the input box!")
                            .arg(cloud->getName(), (inside ? "in" : "out")));
            delete selection;
            return 0;
        }
        ...
        params.trackOrigIndexes = mesh->hasColors() ||
                                  mesh->hasScalarFields() ||
                                  mesh->hasMaterials();
        ...
        if (meshRotation) {
            ...
            if (!rotatedVertices) {
                CVLog::Warning(QString("[Crop] Failed to crop mesh '%1'! (not "
                                       "enough memory)")
                                       .arg(mesh->getName()));
                return 0;
            }
        ...
        cloudViewer::SimpleMesh* tempMesh =
                inside ? params.insideMesh : params.outsideMesh;
        ...
        if (tempMesh) {
            ccPointCloud* croppedVertices =
                    ccPointCloud::From(tempMesh->vertices());
            if (croppedVertices) {
                if (meshRotation) {
                    // apply inverse transformation
                    croppedVertices->setGLTransformation(
                            meshRotation->inverse());
                ...
                if (croppedMesh->size() == 0) {
                    // no points fall inside selection!
                    CVLog::Warning(QString("[Crop] No triangle of the mesh "
                                           "'%1' falls %2side the input box!")
                                           .arg(mesh->getName(),
                                                (inside ? "in" : "out")));
                    delete croppedMesh;
                    croppedMesh = 0;
                    ...
                    if (mesh->hasNormals()) {
                        ...
                        if (!success) {
                            CVLog::Warning(
                                    "[Crop] Failed to compute normals on the "
                                    "output mesh (not enough memory)");
                        }
                        croppedMesh->showNormals(success &&
                                                 mesh->normalsShown());
                    }

                    // import other features if necessary
                    if (params.trackOrigIndexes) {
                        const std::vector<unsigned>& origTriIndexes =
                                inside ? params.origTriIndexesMapInside
                                       : params.origTriIndexesMapOutside;

                        try {
                            // per vertex features (RGB color & scalar fields)
                            if (origVertices->hasColors() ||
                                origVertices->hasScalarFields()) {
                                // we use flags to avoid processing the same
                                // vertex multiple times
                                std::vector<bool> vertProcessed(
                                        croppedVertices->size(), false);
                                ...
                                if (origVertices->hasColors()) {
                                    ...
                                    if (!importColors)
                                        CVLog::Warning(
                                                "[Crop] Failed to transfer RGB "
                                                "colors on the output mesh "
                                                "(not enough memory)");
                                }
                                ...
                                if (origVertices->hasScalarFields()) {
                                    ...
                                    for (unsigned i = 0; i < sfCount; ++i) {
                                        int sfIdx = croppedVertices->addScalarField(
                                                origVertices_pc
                                                        ? origVertices_pc
                                                                  ->getScalarField(
                                                                          i)
                                        ...
                                        if (sfIdx >= 0) {
                                            ...
                                            sf->fill(NAN_VALUE);
                                            if (origVertices_pc) {
                                                // import display parameters if
                                                // possible
                                                ccScalarField* originSf = static_cast<
                                                        ccScalarField*>(
                                                        origVertices_pc
                                                                ->getScalarField(
                                                                        i));
                                                ...
                                                sf->importParametersFrom(
                                                        originSf);
                                            }
                                            importedSFs.push_back(sf);
                                        } else {
                                            CVLog::Warning(
                                                    "[Crop] Failed to transfer "
                                                    "one or several scalar "
                                                    "fields on the output mesh "
                                                    "(not enough memory)");
                                            // we can stop right now as all SFs
                                            // have the same size!
                                            break;
                                        }
                                    }
                                    ...
                                    if (origVertices_pc)
                                        croppedVertices->setCurrentDisplayedScalarField(std::max(
                                                static_cast<int>(
                                                        croppedVertices
                                                                ->getNumberOfScalarFields()) -
                                                        1,
                                ...
                                if (importColors || importSFs) {
                                    ...
                                    for (unsigned i = 0;
                                         i < croppedMesh->size(); ++i) {
                                        // get the origin triangle
                                        unsigned origTriIndex =
                                                origTriIndexes[i];
                                        const cloudViewer::VerticesIndexes* tsio =
                                                mesh->getTriangleVertIndexes(
                                                        origTriIndex);

                                        // get the new triangle
                                        const cloudViewer::VerticesIndexes* tsic =
                                                croppedMesh
                                                        ->getTriangleVertIndexes(
                                                                i);

                                        // we now have to test the 3 vertices of
                                        // the new triangle
                                        for (unsigned j = 0; j < 3; ++j) {
                                            unsigned vertIndex = tsic->i[j];

                                            if (vertProcessed[vertIndex]) {
                                                // vertex has already been
                                                // process
                                                continue;
                                            }

                                            const CCVector3* Vcj =
                                                    croppedVertices->getPoint(
                                                            vertIndex);

                                            // we'll deduce its color and SFs
                                            // values by triangulation
                                            if (importColors) {
                                                ecvColor::Rgb col;
                                                if (mesh->interpolateColors(
                                                            origTriIndex, *Vcj,
                                                            col)) {
                                                    croppedVertices
                                                            ->setPointColor(
                                            ...
                                            if (importSFs) {
                                                CCVector3d w;
                                                mesh->computeInterpolationWeights(
                                                        origTriIndex, *Vcj, w);
                                                ...
                                                for (unsigned s = 0;
                                                    ...
                                                    if (origVertices_pc) {
                                                        ...
                                                        assert(s == 0);
                                                        scalarValues.x =
                                                                origVertices->getPointScalarValue(
                                                                        tsio->i1);
                                                        scalarValues.y =
                                                                origVertices->getPointScalarValue(
                                                                        tsio->i2);
                                                    ...
                                                    ScalarType sVal = static_cast<
                                                            ScalarType>(
                                                            scalarValues.dot(
                                                                    w));
                                                    importedSFs[s]->setValue(
                                                            vertIndex, sVal);
                                                }
                                            }
                                    ...
                                    for (size_t s = 0; s < importedSFs.size();
                                         ++s) {
                                        importedSFs[s]->computeMinAndMax();
                                    }

                                    croppedVertices->showColors(
                                            importColors &&
                                            origVertices->colorsShown());
                                    croppedVertices->showSF(
                                            importSFs &&
                                            origVertices->sfShown());
                                    croppedMesh->showColors(
                                            importColors &&
                                            mesh->colorsShown());
                                    croppedMesh->showSF(importSFs &&
                                                        mesh->sfShown());
                                }
                            }
                            ...
                            if (mesh->hasMaterials()) {
                                const ccMaterialSet* origMaterialSet =
                                        mesh->getMaterialSet();
                                assert(origMaterialSet);

                                if (origMaterialSet &&
                                    !origMaterialSet->empty() &&
                                    croppedMesh
                                            ->reservePerTriangleMtlIndexes()) {
                                    std::vector<int> materialUsed(
                                            origMaterialSet->size(), -1);
                                    ...
                                    for (unsigned i = 0;
                                        ...
                                        unsigned origTriIndex =
                                                origTriIndexes[i];
                                        int mtlIndex =
                                                mesh->getTriangleMtlIndex(
                                                        origTriIndex);
                                        croppedMesh->addTriangleMtlIndex(
                                                mtlIndex);
                                    ...
                                    // import materials
                                    {
                                        ...
                                        {
                                            for (size_t i = 0;
                                                 i < materialUsed.size(); ++i)
                                                if (materialUsed[i] == 1)
                                                    ++materialUsedCount;
                                        }
                                        if (materialUsedCount ==
                                            materialUsed.size()) {
                                            // nothing to do, we use all input
                                            // materials
                                            croppedMesh->setMaterialSet(
                                                    origMaterialSet->clone());
                                        } else {
                                            // create a subset of the input
                                            // materials
                                            ccMaterialSet* matSet =
                                                    new ccMaterialSet(
                                                            origMaterialSet
                                                                    ->getName());
                                            {
                                                ...
                                                for (size_t i = 0;
                                                     ...
                                                     ++i) {
                                                    if (materialUsed[i] >= 0) {
                                                        matSet->push_back(ccMaterial::Shared(
                                                                new ccMaterial(
                                                                        *origMaterialSet
                                                                                 ->at(i))));
                                                        // update index
                                                ...
                                                if (mtlIndex >= 0) {
                                                    assert(mtlIndex <
                                                           static_cast<int>(
                                                                   materialUsed
                                                                           .size()));
                                                    croppedMesh->setTriangleMtlIndex(
                                                            i,
                                                            materialUsed
                                                                    [mtlIndex]);
                                                }
                                    ...
                                    croppedMesh->showMaterials(
                                            mesh->materialsShown());
                                } else {
                                    CVLog::Warning(
                                            "[Crop] Failed to transfer "
                                            "materials on the output mesh (not "
                                            "enough memory)");
                                }
                                ...
                                if (mesh->hasPerTriangleTexCoordIndexes()) {
                                    ...
                                    if (croppedMesh
                                        ...
                                        for (unsigned i = 0;
                                            ...
                                            mesh->getTriangleTexCoordinates(
                                                    origTriIndex, tx1, tx2,
                                                    tx3);
                                            // get the new triangle
                                            const cloudViewer::VerticesIndexes* tsic =
                                                    croppedMesh
                                        ...
                                        croppedMesh->setTexCoordinatesTable(
                                                texCoords);
                                    } else {
                                        CVLog::Warning(
                                                "[Crop] Failed to transfer "
                                                "texture coordinates on the "
                                                "output mesh (not enough "
                                                "memory)");
                                        delete texCoords;
                                        texCoords = 0;
                                    }
                            ...
                            CVLog::Warning(
                                    "[Crop] Failed to transfer per-vertex "
                                    "features (color, SF values, etc.) on the "
                                    "output mesh (not enough memory)");
                ...
                CVLog::Warning(
                        "[Crop] Failed to create output mesh vertices (not "
                        "enough memory)");
            }
        }
    ...
    // unhandled entity
    CVLog::Warning("[Crop] Unhandled entity type");
    return 0;
}
