...
void ccHistogramWindow::setCurveValues(const std::vector<double>& curveValues)
{
	...
	{
		CVLog::Warning("[ccHistogramWindow::setCurveValues] Not enough memory!");
	}
}
...
bool ccHistogramWindow::computeBinArrayFromSF(size_t binCount)
{
	...
	{
		assert(false);
		CVLog::Error("[ccHistogramWindow::computeBinArrayFromSF] Need an associated SF!");
		return false;
	}
	...
	{
		assert(false);
		CVLog::Error("[ccHistogramWindow::computeBinArrayFromSF] Invalid number of classes!");
		return false;
	}
	...
	if (binCount == m_associatedSF->getHistogram().size())
	{
		...
		{
			CVLog::Warning("[ccHistogramWindow::computeBinArrayFromSF] Not enough memory!");
			return false;
		}
		return true;
	}
...
void ccHistogramWindow::refreshBars()
{
		...
		&&	m_associatedSF->getColorScale())
	{
		...
		for (int i = 0; i < histoSize; ++i)
		{
			...
			const ecvColor::Rgb* col = m_associatedSF->getColor(static_cast<ScalarType>(keyData[i]));
...
void ccHistogramWindow::refresh()
{
	...
	if (m_sfInteractionMode && m_associatedSF)
	{
		...
		if (colorScale)
		{
			const ecvColor::Rgb* col = colorScale->getColorByRelativePos(m_associatedSF->symmetricalScale() ? 0.5 : 0, m_associatedSF->getColorRampSteps());
			if (col)
		...
		if (colorScale)
		{
			const ecvColor::Rgb* col = colorScale->getColorByRelativePos(1.0, m_associatedSF->getColorRampSteps());
			if (col)
	...
	else if (m_drawVerticalIndicator) //vertical hint
	{
		...
		int precision = static_cast<int>(ecvGui::Parameters().displayedNumPrecision);
...
bool ccHistogramWindowDlg::exportToCSV(QString filename) const
{
	...
	{
		CVLog::Warning("[Histogram] Histogram has no associated values (can't save file)");
		return false;
	}
	...
	{
		CVLog::Warning(QString("[Histogram] Failed to save histogram to file '%1'").arg(filename));
		return false;
	}
...
QString ccHistogramWindowDlg::getSaveFilename(const QString& dialogTitle, const QString& baseName, const QString& imageSavePath, QWidget* parentWidget/*=0*/)
{
...
void ccHistogramWindowDlg::onExportToImage()
{
	...
	QString outputFilename = getSaveFilename("Select output file",
		m_win->windowTitle(),
		currentPath,
		this);
	...
	{
		CVLog::Print(QString("[Histogram] Image '%1' successfully saved").arg(outputFilename));
	}
	else
	{
