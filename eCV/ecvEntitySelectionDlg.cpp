	...
	, m_ui(new Ui_EntitySelectionDialog)
{
	...
	if (multiSelectionEnabled)
	{
		...
		connect(m_ui->selectAllPushButton,  &QPushButton::clicked, this, &ecvEntitySelectionDialog::selectAll);
		connect(m_ui->selectNonePushButton, &QPushButton::clicked, this, &ecvEntitySelectionDialog::selectNone);
	}
	else
	{
...
void ecvEntitySelectionDialog::selectAll()
{
...
{
	ecvEntitySelectionDialog epDlg(entities, false, selectedIndex, parent, label);
	if (!epDlg.exec())
	{
		return -1;
	}
...
bool ecvEntitySelectionDialog::SelectEntities(	const ccHObject::Container& entities,
