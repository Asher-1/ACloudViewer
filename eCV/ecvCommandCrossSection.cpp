...
bool CommandCrossSection::process(ccCommandLineInterface &cmd) {
    ...
    if (cmd.arguments().empty())
        return cmd.error(
                QString("Missing parameter: XML parameters file after \"-%1\"")
                        .arg(COMMAND_CROSS_SECTION));
    ...
    QString outputFilePath;
    {
        ...
        if (!file.open(QFile::ReadOnly | QFile::Text)) {
            return cmd.error(
                    QString("Couldn't open XML file '%1'").arg(xmlFilename));
        }
        ...
        while (stream.readNextStartElement())  // loop over the elements
        {
            if (stream.name() == s_xmlBoxThickness) {
                ...
                if (!ok || dim < 0 || dim > 2) {
                    return cmd.error(QString("Invalid XML file (invalid value "
                                             "for '<%1>')")
                                             .arg(s_xmlRepeatDim));
                }
                repeatDim[dim] = true;
                ...
                if (!QDir(inputFilePath).exists()) {
                    return cmd.error(QString("Invalid file path (directory "
                                             "pointed by '<%1>' doesn't exist)")
                                             .arg(s_xmlFilePath));
                }
                //++mandatoryCount;
                ...
                if (!QDir(outputFilePath).exists()) {
                    return cmd.error(
                            QString("Invalid output file path (directory "
                                    "pointed by '<%1>' doesn't exist)")
                                    .arg(s_outputXmlFilePath));
                }
                //++mandatoryCount;
            } else {
                cmd.warning(QString("Unknown element: %1")
        ...
        if (mandatoryCount < 1 ||
            (!repeatDim[0] && !repeatDim[1] && !repeatDim[2])) {
            return cmd.error(
                    QString("Some mandatory elements are missing in the XML "
                            "file (see documentation)"));
        }
    }

    // safety checks
    if (cloudViewer::LessThanEpsilon(boxThickness.x) ||
        cloudViewer::LessThanEpsilon(boxThickness.y) ||
        cloudViewer::LessThanEpsilon(boxThickness.z)) {
        return cmd.error(QString("Invalid box thickness"));
    }

    CCVector3 repeatStep =
            boxThickness + CCVector3(repeatGap, repeatGap, repeatGap);
    if ((repeatDim[0] && cloudViewer::LessThanEpsilon(repeatStep.x)) ||
        (repeatDim[1] && cloudViewer::LessThanEpsilon(repeatStep.y)) ||
        (repeatDim[2] && cloudViewer::LessThanEpsilon(repeatStep.z))) {
        return cmd.error(QString(
                "Repeat gap can't be equal or smaller than 'minus' box width"));
    }
    ...
    for (int f = 0; f < iterationCount; ++f) {
        ...
        if (fromFiles) {
            ...
            bool result = false;
            {
                // hack: replace the current argument list by a fake 'load file'
                // sequence
        ...
        // repeat crop process on each file (or do it only once on the currently
        // loaded entities)
        {
            ...
            for (size_t i = 0; i < entities.size(); ++i) {
                ...
                if (!bbox.isValid()) {
                    cmd.warning(
                            QString("Entity '%1' has an invalid bounding-box!")
                                    .arg(ent->getName()));
                    continue;
                }

                // browse to/create a subdirectory with the (base) filename as
                // name
                QString basename;
                if (fromFiles) {
                    basename = QFileInfo(filename).baseName();
                } else {
                    basename = i < cmd.clouds().size()
                                       ? cmd.clouds()[i].basename
                                       : cmd.meshes()[i - cmd.clouds().size()]
                                                 .basename;
                }

                if (entities.size() > 1) basename += QString("_%1").arg(i + 1);
                ...
                if (outputDir.cd(basename)) {
                    ...
                    cmd.warning(QString("Subdirectory '%1' already exists")
                                        .arg(basename));
                } else if (outputDir.mkdir(basename)) {
                    outputDir.cd(basename);
                } else {
                    cmd.warning(
                            QString("Failed to create subdirectory '%1' (check "
                                    "access rights and base name validity!)")
                                    .arg(basename));
                    continue;
                }
                ...
                // place the initial box at the beginning of the entity bounding
                // box
                CCVector3 C0 = autoCenter ? bbox.getCenter() : boxCenter;
                unsigned steps[3] = {1, 1, 1};
                for (unsigned d = 0; d < 3; ++d) {
                    if (repeatDim[d]) {
                        PointCoordinateType boxHalfWidth =
                                boxThickness.u[d] / 2;
                        PointCoordinateType distToMinBorder =
                                C0.u[d] - boxHalfWidth - bbox.minCorner().u[d];
                        int stepsToMinBorder = static_cast<int>(
                                ceil(distToMinBorder / repeatStep.u[d]));
                        C0.u[d] -= stepsToMinBorder * repeatStep.u[d];

                        PointCoordinateType distToMaxBorder =
                                bbox.maxCorner().u[d] - C0.u[d] - boxHalfWidth;
                        int stepsToMaxBoder = static_cast<int>(
                                ceil(distToMaxBorder / repeatStep.u[d]) + 1);
                        assert(stepsToMaxBoder >= 0);
                        steps[d] = std::max<unsigned>(stepsToMaxBoder, 1);
                    }
                }

                cmd.print(QString("Will extract up to (%1 x %2 x %3) = %4 "
                                  "sections")
                                  .arg(steps[0])
                                  .arg(steps[1])
                                  .arg(steps[2])
                                  .arg(steps[0] * steps[1] * steps[2]));

                // now extract the slices
                for (unsigned dx = 0; dx < steps[0]; ++dx) {
                    for (unsigned dy = 0; dy < steps[1]; ++dy) {
                        for (unsigned dz = 0; dz < steps[2]; ++dz) {
                            CCVector3 C = C0 + CCVector3(dx * repeatStep.x,
                                                         dy * repeatStep.y,
                                                         dz * repeatStep.z);
                            ccBBox cropBox(C - boxThickness / 2,
                                           C + boxThickness / 2);
                            ...
                            ccHObject *croppedEnt =
                                    ccCropTool::Crop(ent, cropBox, inside);
                            if (croppedEnt) {
                                QString outputBasename =
                                        basename + QString("_%1_%2_%3")
                                                           .arg(C.x)
                                                           .arg(C.y)
                                                           .arg(C.z);
                                QString errorStr;
...
bool CommandCrossSection::readVector(const QXmlStreamAttributes &attributes,
                                     CCVector3 &P,
                                     QString element,
                                     const ccCommandLineInterface &cmd) {
    if (attributes.size() < 3) {
        return cmd.error(QString("Invalid XML file (3 attributes expected for "
                                 "element '<%1>')")
                                 .arg(element));
    }
    ...
    for (int i = 0; i < attributes.size(); ++i) {
        ...
        if (!ok) {
            return cmd.error(
                    QString("Invalid XML file (numerical attribute expected "
                            "for attribute '%1' of element '<%2>')")
                            .arg(name, element));
        }
    }

    if (count < 3) {
        return cmd.error(QString("Invalid XML file (attributes 'X','Y' and 'Z' "
                                 "are mandatory for element '<%1>')")
                                 .arg(element));
    }
