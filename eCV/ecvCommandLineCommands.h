	...
	virtual bool process(ccCommandLineInterface& cmd) override
	{
		...
		while (!cmd.arguments().empty())
		{
			...
			else if (ccCommandLineInterface::IsCommand(argument, COMMAND_OPEN_SHIFT_ON_LOAD))
			{
				...
				loadParams.shiftHandlingMode = ecvGlobalShiftManager::NO_DIALOG;
	...
	virtual bool process(ccCommandLineInterface& cmd) override
	{
		...
		else if (method == "SPATIAL")
		{
			...
			for (size_t i = 0; i < cmd.clouds().size(); ++i)
			{
				...
				CVLib::ReferenceCloud* refCloud = CVLib::CloudSamplingTools::resampleCloudSpatially(cloud, static_cast<PointCoordinateType>(step), modParams, 0, cmd.progressDialog());
				if (!refCloud)
				{
					return cmd.error("Subsampling process failed!");
				}
...
struct CommandStatTest : public ccCommandLineInterface::Command
{
	...
	virtual bool process(ccCommandLineInterface& cmd) override
	{
		...
		//distribution
		CVLib::GenericDistribution* distrib = 0;
		{
			if (cmd.arguments().empty())
