	...
	, m_ui(new Ui::ColorFromScalarDialog)
{
	...
	if (m_cloud->getNumberOfScalarFields() > 0)
	{
		...
		{
			assert(false);
			CVLog::Error("[ccColorFromScalarDlg] Get current scalar field failed!");
	...
	{
		CVLog::Error("[ccColorFromScalarDlg] Current cloud has no scalar fields!");
		m_systemInvalid = true;
		disableAllButCancel();
	}
...
void ccColorFromScalarDlg::updateHistogram(int n)
{
	if (!m_systemInvalid)
	{
		...
		{
			CVLog::Error("[ccColorFromScalarDlg] updateHistogram called with an invalid channel index");
			return;
		}
...
void ccColorFromScalarDlg::updateSpinBoxLimits(int n)
{
	if (!m_systemInvalid)
	{
		if (n >= c_channelCount || n < 0)
		{
			CVLog::Error("[ccColorFromScalarDlg] updateSpinBoxLimits called with an invalid channel index");
			return;
		}
		bool fixed[c_channelCount] = { m_ui->fixR->isChecked(), m_ui->fixG->isChecked(), m_ui->fixB->isChecked(), m_ui->fixA->isChecked() };
		if (fixed[n])
...
void ccColorFromScalarDlg::updateChannel(int n)
{
	if (!m_systemInvalid)
	{
		if (n >= c_channelCount || n < 0)
		{
			CVLog::Error("[ccColorFromScalarDlg] updateChannel called with an invalid channel index");
			return;
		}
...
void ccColorFromScalarDlg::setDefaultSatValuePerChannel(int n)
{
	if (!m_systemInvalid)
	{
		if (n >= c_channelCount || n < 0)
		{
			CVLog::Error("[ccColorFromScalarDlg] setDefaultSatValuePerChannel called with an invalid channel index");
			return;
		}
...
void ccColorFromScalarDlg::minChanged(int n, double val, bool slider)
{
	if (n >= c_channelCount || n < 0)
	{
		CVLog::Error("[ccColorFromScalarDlg] minChanged called with an invalid channel index");
		return;
	}
	...
	{
		//CVLog::Warning(QString("minChanged Channel %1 failed New min > max, val = %2, maxval = %3").arg(n).arg(val).arg(m_maxSat[n]));
	}
}
...
void ccColorFromScalarDlg::maxChanged(int n, double val, bool slider)
{
	if (n >= c_channelCount || n < 0)
	{
		CVLog::Error("[ccColorFromScalarDlg] maxChanged called with an invalid channel index");
		return;
	}
	...
	{
		//CVLog::Warning(QString("maxChanged Channel %1 failed New max < min, val = %2, minval = %3").arg(n).arg(val).arg(m_minSat[n]));
	}
}
	...
	if (!m_systemInvalid)
	{
		...
		if (m_ui->toggleRGB->isChecked())
		{
			...
			for (unsigned p = 0; p < m_cloud->size(); p++)
			{
				...
				m_cloud->setPointColor(p, ecvColor::FromQColora(QColor(col[0], col[1], col[2], col[3])));
			}
		}
		else //map scalar values to HSV (and then to RGB)
		{
			...
			for (unsigned p = 0; p < m_cloud->size(); p++)
			{
				...
				ecvColor::Rgb rgb = ecvColor::Convert::hsv2rgb(col[0] * 360.0f, col[1], col[2]);
				m_cloud->setPointColor(p, ecvColor::Rgba(rgb, static_cast<int>(col[3] * ecvColor::MAX)));
			}
		}
