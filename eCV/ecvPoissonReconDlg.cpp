...
struct Parameters {
    ...
    **/
    ...
    //! The target width of the finest level octree cells (ignored if depth is
    //! specified)
    float finestCellWidth = 0.0f;

    //! The ratio between the diameter of the cube used for reconstruction and
    //! the diameter of the samples' bounding cube.
    /** Specifies the factor of the bounding cube that the input samples should
     *fit into.
     **/
    float scale = 1.1f;
    ...
    within an octree node. For noise-free samples, small values in the range
    [1.0 - 5.0] can be used. For more noisy samples, larger values in the range
    [15.0 - 20.0] may be needed to provide a smoother, noise-reduced,
    reconstruction.
    **/
    float samplesPerNode = 1.5f;

    //! The importance that interpolation of the point samples is given in the
    //! formulation of the screened Poisson equation.
    ...
    /** Number of Gauss-Seidel relaxations to be performed at each level of the
     *octree hierarchy.
     **/
    int iters = 8;

    //! If this flag is enabled, the sampling density is written out with the
    //! vertices
    bool density = false;

    //! This flag tells the reconstructor to read in color values with the input
    //! points and extrapolate those to the vertices of the output.
    bool withColors = true;

    //! Data pull factor
    /** If withColors is rue, this floating point value specifies the relative
     *importance of finer color estimates over lower ones.
     **/
    float colorPullFactor = 32.0f;

    //! Normal confidence exponent
    /** Exponent to be applied to a point's confidence to adjust its weight. (A
     *point's confidence is defined by the magnitude of its normal.)
     **/
    float normalConfidence = 0.0;

    //! Normal confidence bias exponent
    /** Exponent to be applied to a point's confidence to bias the resolution at
     *which the sample contributes to the linear system. (Points with lower
     *confidence are biased to contribute at coarser resolutions.)
     **/
    ...
    /** At coarser depths, the octree will be complete, containing all 2^d x 2^d
     *x 2^d nodes.
     **/
    int fullDepth = 5;
