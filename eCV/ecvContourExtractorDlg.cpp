...
{
	float pixSize = std::max(box.getDiagVec().x / std::max(20, ecvDisplayTools::GetScreenRect().width() - 20),
		box.getDiagVec().y / std::max(20, ecvDisplayTools::GetScreenRect().height() - 20));
