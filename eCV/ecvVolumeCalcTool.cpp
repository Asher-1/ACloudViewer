										...
										QWidget* parentWidget/*=0*/)
{
	...
	if (ground)
	{
									...
									pDlg.data()))
		{
			...
			CVLog::Print(QString("[Volume] Ground raster grid: size: %1 x %2 / heights: [%3 ; %4]").arg(groundRaster.width).arg(groundRaster.height).arg(groundRaster.minHeight).arg(groundRaster.maxHeight));
		}
		else
		{
			return false;
	...
	if (ceil)
	{
								...
								pDlg.data()))
		{
			...
			CVLog::Print(QString("[Volume] Ceil raster grid: size: %1 x %2 / heights: [%3 ; %4]").arg(ceilRaster.width).arg(ceilRaster.height).arg(ceilRaster.minHeight).arg(ceilRaster.maxHeight));
		}
		else
		{
			return false;
...
void ccVolumeCalcTool::exportGridAsCloud() const
{
	...
	if (mainWindow)
	{
		...
		CVLog::Print(QString("[Volume] Cloud '%1' successfully exported").arg(rasterCloud->getName()));
	}
	else
	{
		assert(false);
