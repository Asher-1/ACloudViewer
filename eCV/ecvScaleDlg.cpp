...
ccScaleDlg::ccScaleDlg(QWidget* parent /*=0*/)
    : QDialog(parent), Ui::ScaleDialog() {
    setupUi(this);

    connect(sameForAllCheckBox, &QCheckBox::toggled, this,
            &ccScaleDlg::allDimsAtOnceToggled);
    connect(fxSpinBox,
            static_cast<void (QDoubleSpinBox::*)(double)>(
                    &QDoubleSpinBox::valueChanged),
            this, &ccScaleDlg::fxUpdated);
    ...
    fxSpinBox->setValue(
            s_lastScales.x);  // always last in case 'same for all' is checked!
}
...
CCVector3d ccScaleDlg::getScales() const {
    return CCVector3d(fxSpinBox->value(), fySpinBox->value(),
                      fzSpinBox->value());
}
