...
ecvFilterByLabelDlg::ecvFilterByLabelDlg(QWidget* parent)
    ...
    connect(splitPushButton, &QAbstractButton::clicked, this,
            &ecvFilterByLabelDlg::onSplit);
    connect(exportSelectedToolButton, &QAbstractButton::clicked, this,
            &ecvFilterByLabelDlg::onExportSelected);
    connect(exportUnselectedToolButton, &QAbstractButton::clicked, this,
            &ecvFilterByLabelDlg::onExportUnSelected);
    connect(selectAllRadioButton, &QAbstractButton::clicked, this,
            &ecvFilterByLabelDlg::selectAllClasses);
    connect(unselectAllRadioButton, &QAbstractButton::clicked, this,
            &ecvFilterByLabelDlg::selectAllClasses);
    connect(cancelToolButton, &QAbstractButton::clicked, this,
            &ecvFilterByLabelDlg::cancel);
    connect(toggleSelectedToolButton, &QAbstractButton::clicked, this,
            &ecvFilterByLabelDlg::toggleSelectedVisibility);
}
...
bool ecvFilterByLabelDlg::start() {
    ...
    for (unsigned j = 0; j < pc->size(); j++) {
        ...
        if (value - valueId != 0) {
            CVLog::Error(
                    "[ecvFilterByLabelDlg::start] only support int type scalar "
                    "fields!");
            clear();
            return false;
        }
...
void ecvFilterByLabelDlg::createCheckboxesWithLabels() {
    ...
    for (size_t i = 0; i < m_labels.size(); ++i) {
        ...
        if (m_toFilter.second) {
            col = *m_toFilter.second->getScalarValueColor(
...
void ecvFilterByLabelDlg::stop(bool state) { ccOverlayDialog::stop(state); }
...
void ecvFilterByLabelDlg::getSelectedFilterClasses(
        std::vector<ScalarType>& filteredClasses) {
    ...
    foreach (QCheckBox* ncheckBox, list) {
        if (ncheckBox && ncheckBox->isChecked()) {
            int index = ClassMap::FindindexByValue(
                    CVTools::FromQString(ncheckBox->text()));
...
void ecvFilterByLabelDlg::clear() { m_labels.clear(); }

bool ecvFilterByLabelDlg::setInputEntity(ccHObject* entity) {
    ccGenericPointCloud* cloud = ccHObjectCaster::ToGenericPointCloud(entity);
    if (!cloud || !cloud->isKindOf(CV_TYPES::POINT_CLOUD)) return false;

    ccPointCloud* pc = static_cast<ccPointCloud*>(cloud);
    ccScalarField* sf = pc->getCurrentDisplayedScalarField();
    if (!sf) {
        CVLog::Warning(tr("Entity [%1] has no active scalar field !")
                               .arg(pc->getName()));
        return false;
    } else {
        ...
        if (m_maxVald >= 20) {
            CVLog::Warning(tr("Entity [%1] scalar field value range is bigger "
                              "than 20!")
                                   .arg(pc->getName()));
            return false;
        }
    ...
    if (pointNumber < 10) {
        CVLog::Warning(QString("[ecvFilterByLabelDlg::setInputEntity] "
                               "Skip entity [%1] as the point number of it is "
                               "%2 lower than min limit 10!")
...
void ecvFilterByLabelDlg::apply() {
    ...
    if (selectedLabels.empty()) {
        CVLog::Warning(
                "[ecvFilterByLabelDlg::apply] no filter labels selected, "
                "please select some and try again!");
        return;
    }
    ...
    if (ent->isKindOf(CV_TYPES::MESH)) {
        ...
        if (ecvFilterByLabelDlg::SPLIT == m_mode ||
            ...
            if (ent->isA(
                ...
                resultInside = ccHObjectCaster::ToSubMesh(ent)
        ...
        if (ecvFilterByLabelDlg::SPLIT == m_mode ||
            ...
            if (ent->isA(
                        CV_TYPES::
                                MESH) /*|| ent->isKindOf(CV_TYPES::PRIMITIVE)*/)  // TODO
                resultOutside = ccHObjectCaster::ToMesh(ent)
                                        ->createNewMeshFromSelection(false);
            else if (ent->isA(CV_TYPES::SUB_MESH))
                resultOutside = ccHObjectCaster::ToSubMesh(ent)
                                        ->createNewSubMeshFromSelection(false);
        }
