...
ccPluginUIManager::ccPluginUIManager( ecvMainAppInterface *appInterface, QWidget *parent )
	: QObject( parent )
...
{	
	...
	for ( ccPluginInterface *plugin : plugins )
	{
		...
		switch ( plugin->getType() )
		{
			...
			{
				ccPclPluginInterface *pclPlugin = static_cast<ccPclPluginInterface*>(plugin);
				...
				// store the plugin's interface pointer in the QAction data so we can access it in enablePCLAlgorithm()
				QVariant v;
				...
				connect(action, &QAction::triggered, this, &ccPluginUIManager::enablePCLAlgorithm);
	...
	m_pclAlgorithmsToolbar->setEnabled( !m_pclAlgorithmMenu->isEmpty() );	// [sic] we have toolbar actions if we have them in the menu
...
void ccPluginUIManager::addActionsToMenu( const QString &moduleName, const QList<QAction *> &actions )
{
	// If the plugin has more than one action we create its own menu
	if ( actions.size() > 1 )
	{
...
void ccPluginUIManager::addActionsToToolBar( const QString &moduleName, const QList<QAction *> &actions )
{	
	// If the plugin has more than one action we create its own tool bar
	if ( actions.size() > 1 )
	{
...
void ccPluginUIManager::enablePCLAlgorithm()
{
	...
	ccPclPluginInterface *plugin = action->data().value<ccPclPluginInterface *>();
	...
	ecvConsole::Print("Note: go to << PCL ALgorithms >> to disable PCL ALGORITHM");
}
