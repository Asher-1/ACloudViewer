...
{
	const QString emailFormat( "&lt;<a href=\"mailto:%1?Subject=CLOUDVIEWER  %2\">%1</a>&gt;" );
	QString	formattedText;
...
namespace
{
	...
	private:
		static void	_init()
		{
			...
			sIconMap[ECV_STD_PLUGIN] = QIcon(":/CC/pluginManager/images/std_plugin.png");
			sIconMap[ECV_PCL_ALGORITHM_PLUGIN] = QIcon(":/CC/pluginManager/images/algorithm_plugin.png");
			sIconMap[ECV_IO_FILTER_PLUGIN] = QIcon(":/CC/pluginManager/images/io_plugin.png");
		}
...
void ccPluginInfoDlg::updatePluginInfo( const ccPluginInterface *plugin )
{
	...
	switch ( plugin->getType() )
	{
		...
		case ECV_PCL_ALGORITHM_PLUGIN:
		{
			if ( iconPixmap.isNull() )
			{
				iconPixmap = QPixmap( ":/CC/pluginManager/images/algorithm_plugin.png" ).scaled( iconSize );
			}
