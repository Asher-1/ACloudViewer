...
ecvRecentFiles::ecvRecentFiles( QWidget *parent ) :
	QObject( parent )
{
...
{
	QStringList	list = ecvSettingManager::getValue(ecvPS::LoadFile(), s_settingKey).toStringList();
...
void ecvRecentFiles::updateMenu()
{
	...
	for ( const QString &recentFile : recentList )
	{
		...
		connect( recentAction, &QAction::triggered, this, &ecvRecentFiles::openFileFromAction );
