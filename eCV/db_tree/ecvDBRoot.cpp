...
static bool CanDetachCloud(const ccHObject* obj)
{
	...
	bool blocked = (	(parent->isKindOf(CV_TYPES::MESH) && (ccHObjectCaster::ToGenericMesh(parent)->getAssociatedCloud() == obj))
						||	(parent->isKindOf(CV_TYPES::POLY_LINE) && (dynamic_cast<ccPointCloud*>(ccHObjectCaster::ToPolyline(parent)->getAssociatedCloud()) == obj)) );
...
private:
	...
	void	init()
	{
		...
		mDefaultIcons = { {}, QIcon(QStringLiteral(":/Resources/images/dbLockSymbol.png")) };
		
		const int	hObjectIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbHObjectSymbol.png")),
							QIcon(QStringLiteral(":/Resources/images/dbHObjectSymbolLocked.png")) } );
		
		const int	cloudIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbCloudSymbol.png")),
							QIcon(QStringLiteral(":/Resources/images/dbCloudSymbolLocked.png")) } );
		
		const int	geomIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbMiscGeomSymbol.png")),
							QIcon(QStringLiteral(":/Resources/images/dbMiscGeomSymbolLocked.png")) } );
		
		const int	meshIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbMeshSymbol.png")),
							QIcon(QStringLiteral(":/Resources/images/dbMeshSymbolLocked.png")) } );
		
		const int	subMeshIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbSubMeshSymbol.png")),
							QIcon(QStringLiteral(":/Resources/images/dbSubMeshSymbolLocked.png")) } );
		
		const int	polyLineIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbPolylineSymbol.png")),
							{} } );
		
		const int	octreeIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbOctreeSymbol.png")),
							QIcon(QStringLiteral(":/Resources/images/dbOctreeSymbolLocked.png")) } );
		
		const int	calibratedImageIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbCalibratedImageSymbol.png")),
							{} } );
		
		const int	imageIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbImageSymbol.png")),
							{} } );
		
		const int	sensorIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbGBLSensorSymbol.png")),
							{} } );
		
		const int	cameraSensorIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbCamSensorSymbol.png")),
							{} } );
		
		const int	materialSetIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbMaterialSymbol.png")),
							{} } );
		
		const int	containerIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbContainerSymbol.png")),
							QIcon(QStringLiteral(":/Resources/images/dbContainerSymbolLocked.png")) } );
		
		const int	labelIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbLabelSymbol.png")),
							{} } );
		
		const int	viewportObjIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbViewportSymbol.png")),
							{} } );
		
		const int	viewportLabelIndex = mIconList.count();
		mIconList.append( { QIcon(QStringLiteral(":/Resources/images/dbAreaLabelSymbol.png")),
							{} } );
		
		mIconMap = {
			{ CV_TYPES::HIERARCHY_OBJECT, hObjectIndex },
...
void ccDBRoot::removeElements(ccHObject::Container& objects)
{
	...
	for (ccHObject* object : objects)
	{
		...
		if (!parent)
		{
			CVLog::Warning(tr("[ccDBRoot::removeElements] Internal error: object '%1' has no parent").arg(object->getName()));
			continue;
		}
...
void ccDBRoot::removeElement(ccHObject* object)
{
	...
	if (!parent)
	{
		CVLog::Warning(tr("[ccDBRoot::removeElement] Internal error: object has no parent"));
		return;
	}
...
void ccDBRoot::deleteSelectedEntities()
{
	...
	for (unsigned i = 0; i < selCount; ++i)
	{
		...
		{
			CVLog::Warning(tr("Object '%1' can't be deleted this way (locked)").arg(obj->getName()));
			continue;
		}
		...
		if (!isDescendent)
		{
			...
			if (obj->isKindOf(CV_TYPES::POINT_CLOUD) && !CanDetachCloud(obj))
			{
				...
				{
					CVLog::Warning(tr("Vertices can't be deleted without their parent mesh"));
...
QVariant ccDBRoot::data(const QModelIndex &index, int role) const
{
	...
	case Qt::CheckStateRole:
	{
		...
		if ( item->getClassID() == CV_TYPES::HIERARCHY_OBJECT )
		{
			...
			unsigned int	count = item->filterChildren( drawableObjects, true, CV_TYPES::HIERARCHY_OBJECT, true );
	...
	if (index.isValid())
	{
		...
		else if (role == Qt::CheckStateRole)
		{
			...
			if (item)
			{
				...
				{
					cc2DViewportLabel* label = ccHObjectCaster::To2DViewportLabel(item);
					if (label)
					{
						label->updateLabel();
					}
...
QModelIndex ccDBRoot::index(ccHObject* object)
{
	...
	if (!parent)
	{
		...
		//CVLog::Error(QString("An error occurred while creating DB tree index: object '%1' has no parent").arg(object->getName()));
		return QModelIndex();
	}
			...
			if (additiveSelection)
			{
				...
				if (!obj->isSelected())
				{
					...
					if (!selectedIndexes.empty())
					{
						...
						if (obj->isA(CV_TYPES::LABEL_2D) != static_cast<ccHObject*>(selectedIndexes[0].internalPointer())->isA(CV_TYPES::LABEL_2D))
						{
							CVLog::Warning(tr("[Selection] Labels and other entities can't be mixed (release the CTRL key to start a new selection)"));
							return;
						}
					}
				}
	...
	QItemSelection newSelection;
	{
		...
		bool keepLabels = false;
		{
			...
			if (formerSelectedIndexes.isEmpty() || !incremental)
				...
				keepLabels = static_cast<ccHObject*>(formerSelectedIndexes[0].internalPointer())->isA(CV_TYPES::LABEL_2D); //yes if previously selected entities were already labels
		}
...
int ccDBRoot::countSelectedEntities(CV_CLASS_ENUM filter)
{
	...
	if (selCount == 0 || filter == CV_TYPES::OBJECT)
		return selCount;
	...
	if (info)
	{
		...
		for (size_t i = 0; i < info->selCount; ++i)
		{
			...
			else if (obj->isKindOf(CV_TYPES::MESH))
			{
				...
				if (obj->isKindOf(CV_TYPES::PLANE))
					info->planeCount++;
			}
			...
			{
				info->sensorCount++;
				if (obj->isKindOf(CV_TYPES::GBL_SENSOR))
					info->gblSensorCount++;
...
Qt::ItemFlags ccDBRoot::flags(const QModelIndex &index) const
{
	...
	if (item && !item->isLocked()) //locked items cannot be drag-dropped
	{
		if (item->isA(CV_TYPES::HIERARCHY_OBJECT)								||
			(item->isKindOf(CV_TYPES::POINT_CLOUD) && CanDetachCloud(item))		|| //vertices can't be displaced
			(item->isKindOf(CV_TYPES::MESH) && !item->isA(CV_TYPES::SUB_MESH))	|| //a sub-mesh can't leave its parent mesh
...
bool ccDBRoot::dropMimeData(const QMimeData* data, Qt::DropAction action, int destRow, int destColumn, const QModelIndex& destParent)
{
	...
	while (!stream.atEnd())
	{
		...
		if (!item)
			continue;
		//CVLog::Print(QString("[Drag & Drop] Source: %1").arg(item->getName()));

		//old parent
		ccHObject* oldParent = item->getParent();
		//CVLog::Print(QString("[Drag & Drop] Parent: %1").arg(oldParent ? oldParent->getName() : "none")));
		...
		if (oldParent)
		{
			if (item->isKindOf(CV_TYPES::POINT_CLOUD))
			{
				...
				if (oldParent->isKindOf(CV_TYPES::MESH) && ccHObjectCaster::ToGenericMesh(oldParent)->getAssociatedCloud() == item)
				{
					if (oldParent != newParent)
					{
						CVLog::Error(tr("Vertices can't leave their parent mesh"));
						return false;
					}
				}
			}
			else if (item->isKindOf(CV_TYPES::MESH))
			{
				...
				else if (oldParent->isKindOf(CV_TYPES::POINT_CLOUD) && ccHObjectCaster::ToGenericMesh(item)->getAssociatedCloud() == oldParent)
				{
					if (oldParent != newParent)
					{
						CVLog::Error(tr("Meshes can't leave their associated cloud (vertices set)"));
						return false;
					}
				}
			}
			else if (/*item->isKindOf(CV_TYPES::PRIMITIVE) || */item->isKindOf(CV_TYPES::IMAGE))
			{
				if (oldParent != newParent)
				{
					CVLog::Error(tr("This kind of entity can't leave their parent"));
					return false;
				}
			}
			else if (oldParent != newParent)
			{
				...
				//		if (!canMove)
				//		{
				//			CVLog::Error("Labels (or group of) can't leave their parent");
				//			return false;
				//		}
				//	}
				//}
...
void ccDBRoot::alignCameraWithEntity(bool reverse)
{
	...
	//output the transformation matrix that would make this normal points towards +Z
	{
		...
		CVLog::Print(tr("[Orientation] You can copy this matrix values (CTRL+C) and paste them in the 'Apply transformation tool' dialog"));
	}
}
...
void ccDBRoot::selectByTypeAndName()
{
	...
	if (scDlg.getTypeIsUsed()) // we are using type checking
	{
		...
		switch (type)
		{
		case CV_TYPES::HIERARCHY_OBJECT: //returned if no type is selected (i.e. all objects are selected!)
...
void ccDBRoot::editLabelScalarValue()
{
	...
	bool ok = false;
	double newValue = QInputDialog::getDouble(MainWindow::TheInstance(), tr("Edit scalar value"), QString("%1 (%2) =").arg(sf->getName()).arg(P.index), s, -2147483647, 2147483647, 6, &ok);
	if (!ok)
	{
			...
			for (int i = 0; i < selCount; ++i)
			{
				...
				if (!item->isA(CV_TYPES::HIERARCHY_OBJECT))
				{
					...
					if (selCount == 1)
					{
						...
						else if (item->isA(CV_TYPES::PLANE) || item->isA(CV_TYPES::FACET))
						{
							hasExactlyOnePlanarEntity = true;
							hasExactlyOnePlane = item->isKindOf(CV_TYPES::PLANE);
						}
...
QItemSelectionModel::SelectionFlags ccCustomQTreeView::selectionCommand(const QModelIndex& index, const QEvent* event/*=0*/) const
{
	if (index.isValid())
	{
		...
		if (!selectedIndexes.empty() && !selectionModel()->isSelected(index))
		{
			...
			if (selectedItem && selectedItem->isA(CV_TYPES::LABEL_2D) != static_cast<ccHObject*>(selectedIndexes[0].internalPointer())->isA(CV_TYPES::LABEL_2D))
