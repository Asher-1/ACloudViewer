...
struct dbTreeSelectionInfo {
    ...
    void reset() { memset(this, 0, sizeof(dbTreeSelectionInfo)); }
};
...
public:
    ...
    /** \param dbTreeWidget widget for DB tree display
            \param propertiesTreeWidget widget for selected entity's properties
    tree display \param parent widget QObject parent
    **/
    ccDBRoot(ccCustomQTreeView* dbTreeWidget,
    ...
    **/
    ...
    //! Returns the number of selected entities in DB tree (optionally with a
    //! given type)
    int countSelectedEntities(CV_CLASS_ENUM filter = CV_TYPES::OBJECT);

    //! Returns selected entities in DB tree (optionally with a given type and
    ...
    virtual QMap<int, QVariant> itemData(
...
public slots:
    ...
    /** If ctrl is pressed by the user at the same time,
            ...
            \param forceAdditiveSelection whether to force additive selection
    (just as if CTRL key is pressed) or not
    **/
    ...
    /** \param entities set of the entities to 'select'
            \param incremental whether to 'add' the input set to the selected
    entities set or to use it as replacement
    **/
    void selectEntities(const ccHObject::Container& entities,
...
protected:
    ...
    //! Toggles a given property (enable state, visibility, normal, color, SF,
    //! etc.) on selected entities
...
protected:
    ...
    /** \param reverse whether to use the entity's normal (false) or its inverse
      *(true)
     **/
