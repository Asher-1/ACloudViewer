...
void QUIWidget::createTrayMenu()
{
	...
	//恢复窗口
	connect(restoreWinAction, SIGNAL(triggered(bool)), this, SLOT(showWindow()));
	...
	m_systemTray->showMessage("托盘", "托盘管理", QSystemTrayIcon::Information, 10000);
	m_systemTray->setContextMenu(trayMenu); //绑定托盘菜单
	m_systemTray->show();   //在任务栏显示图标

	connect(m_systemTray, &QSystemTrayIcon::activated, this, &QUIWidget::activeTray);//点击托盘，执行相应的动作
	connect(m_systemTray, &QSystemTrayIcon::messageClicked, this, &QUIWidget::showWindow);//点击消息框，显示主窗口
}
...
void QUIWidget::showEvent(QShowEvent* event)
{
	...
	if (this->mainWidget)
	{
		...
		QVariant  geometry = ecvSettingManager::getValue(ecvPS::MainWindowSettings(), ecvPS::MainWinGeom());

		if (geometry.isValid())
		{
			this->mainWidget->restoreGeometry(geometry.toByteArray());
			this->mainWidget->restoreState(ecvSettingManager::getValue(ecvPS::MainWindowSettings(), ecvPS::MainWinState()).toByteArray());
		}
...
void QUIWidget::closeEvent(QCloseEvent *event)
{
	if (this->mainWidget)
	{
		if ((this->mainWidget->db() && this->mainWidget->db()->getRootEntity()->getChildrenNumber() == 0)
