...
void QUIWidget::writeInfo(const QString &info, const QString &filePath) {
    QString fileName = QString("%1/%2/%3_runinfo_%4.txt")
                               .arg(QUIWidget::appPath())
                               .arg(filePath)
                               .arg(QUIWidget::appName())
...
void QUIWidget::setStyle(const QString &qssFile,
    ...
    if (file.open(QFile::ReadOnly)) {
        QString qss = QLatin1String(file.readAll());
        getQssColor(qss, textColor, panelColor, borderColor, normalColorStart,
                    normalColorEnd, darkColorStart, darkColorEnd, highColor);
...
void QUIWidget::getQssColor(const QString &qss,
    ...
    if (indexNormalColorStart >= 0) {
        normalColorStart = str.mid(
                indexNormalColorStart + flagNormalColorStart.length(), 7);
    }
    ...
    if (indexNormalColorEnd >= 0) {
        normalColorEnd =
                str.mid(indexNormalColorEnd + flagNormalColorEnd.length(), 7);
    }
    ...
    if (indexDarkColorStart >= 0) {
        darkColorStart =
                str.mid(indexDarkColorStart + flagDarkColorStart.length(), 7);
    }
    ...
    if (indexDarkColorEnd >= 0) {
        darkColorEnd =
                str.mid(indexDarkColorEnd + flagDarkColorEnd.length(), 7);
    }
...
void QUIWidget::setFormInCenter(QWidget *frm) {
    ...
    int deskHeight = qApp->primaryScreen()->geometry().height();
...
//     SLOT(quit())); eventLoop.exec(); responseData = reply->readAll(); return
...
QUIWidget::~QUIWidget() { delete widgetMain; }

QLabel *QUIWidget::getLabIco() const { return this->labIco; }

QLabel *QUIWidget::getLabTitle() const { return this->labTitle; }

QToolButton *QUIWidget::getBtnMenu() const { return this->btnMenu; }

QPushButton *QUIWidget::getBtnMenuMin() const { return this->btnMenu_Min; }

QPushButton *QUIWidget::getBtnMenuMax() const { return this->btnMenu_Max; }

QPushButton *QUIWidget::getBtnMenuMClose() const { return this->btnMenu_Close; }

QString QUIWidget::getTitle() const { return this->title; }

Qt::Alignment QUIWidget::getAlignment() const { return this->alignment; }

QSize QUIWidget::sizeHint() const { return QSize(600, 450); }

QSize QUIWidget::minimumSizeHint() const { return QSize(200, 150); }
...
void QUIWidget::initControl() {
    ...
    sizePolicy4.setHeightForWidth(
            btnMenu_Min->sizePolicy().hasHeightForWidth());
    ...
    sizePolicy3.setHeightForWidth(
            btnMenu_Max->sizePolicy().hasHeightForWidth());
    ...
    sizePolicy3.setHeightForWidth(
            btnMenu_Close->sizePolicy().hasHeightForWidth());
    ...
    connect(this->btnMenu_Min, SIGNAL(clicked(bool)), this,
            SLOT(on_btnMenu_Min_clicked()));
    connect(this->btnMenu_Max, SIGNAL(clicked(bool)), this,
            SLOT(on_btnMenu_Max_clicked()));
    connect(this->btnMenu_Close, SIGNAL(clicked(bool)), this,
            SLOT(on_btnMenu_Close_clicked()));
}
...
void QUIWidget::setWindowLogo(const QString &icon) {
    if (this->mainWidget) {
        this->mainWidget->setWindowIcon(QIcon(icon));
    }
}
...
void QUIWidget::showEvent(QShowEvent *event) {
    ...
    if (this->mainWidget) {
        if (!this->mainWidget->m_FirstShow) {
            return;
        }

        QVariant geometry = ecvSettingManager::getValue(
                ecvPS::MainWindowSettings(), ecvPS::MainWinGeom());

        if (geometry.isValid()) {
            this->mainWidget->restoreGeometry(geometry.toByteArray());
            this->mainWidget->restoreState(
                    ecvSettingManager::getValue(ecvPS::MainWindowSettings(),
                                                ecvPS::MainWinState())
                            .toByteArray());
        }

        this->mainWidget->m_FirstShow = false;
        ...
        if (isFullScreen()) {
            this->mainWidget->updateFullScreenMenu(true);
        }
    }
}

void QUIWidget::closeEvent(QCloseEvent *event) {
    if (this->mainWidget) {
        if ((this->mainWidget->db() &&
             this->mainWidget->db()->getRootEntity()->getChildrenNumber() ==
                     0) ||
        ...
        if (MainWindow::s_autoSaveGuiElementPos) {
            this->mainWidget->saveGUIElementsPos();
        }
    }
}

void QUIWidget::moveEvent(QMoveEvent *event) {
    QDialog::moveEvent(event);

    if (this->mainWidget) {
        this->mainWidget->updateOverlayDialogsPlacement();
    }
}
...
void QUIWidget::keyPressEvent(QKeyEvent *event) {
    switch (event->key()) {
        case Qt::Key_Escape: {
            if (this->mainWidget) {
                this->mainWidget->cancelPreviousPickingOperation(true);
            }
...
QUIMessageBox::~QUIMessageBox() { delete widgetMain; }
...
void QUIMessageBox::initControl() {
    ...
    labIcoMain->setStyleSheet(
            QString::fromUtf8("border-image: url(:/image/msg_info.png);"));

    horizontalLayout1->addWidget(labIcoMain);

    horizontalSpacer1 =
            new QSpacerItem(5, 0, QSizePolicy::Minimum, QSizePolicy::Minimum);
    ...
    horizontalSpacer2 = new QSpacerItem(40, 20, QSizePolicy::Expanding,
                                        QSizePolicy::Minimum);
    ...
    QIcon icon;
    icon.addFile(QString::fromUtf8(":/image/btn_ok.png"), QSize(),
    ...
    QIcon icon1;
    icon1.addFile(QString::fromUtf8(":/image/btn_close.png"), QSize(),
                  QIcon::Normal, QIcon::Off);
    ...
    connect(this->btnMenu_Close, SIGNAL(clicked(bool)), this,
            SLOT(on_btnMenu_Close_clicked()));
    connect(this->btnCancel, SIGNAL(clicked()), this,
            SLOT(on_btnMenu_Close_clicked()));
}

void QUIMessageBox::initForm() {
    IconHelper::Instance()->setIcon(labIco, QUIConfig::IconMain,
                                    QUIConfig::FontSize + 2);
    IconHelper::Instance()->setIcon(btnMenu_Close, QUIConfig::IconClose,
                                    QUIConfig::FontSize);
...
QUIInputBox::~QUIInputBox() { delete widgetMain; }
...
QString QUIInputBox::getValue() const { return this->value; }
...
void IconHelper::setStyle(QWidget *widget,
    ...
    QString strBorder;
    if (type == "top") {
        strBorder = QString("border-width:%1px 0px 0px 0px;padding:%1px %2px "
                            "%2px %2px;")
                            .arg(borderWidth)
                            .arg(borderWidth * 2);
    } else if (type == "right") {
        strBorder = QString("border-width:0px %1px 0px 0px;padding:%2px %1px "
                            "%2px %2px;")
                            .arg(borderWidth)
                            .arg(borderWidth * 2);
    } else if (type == "bottom") {
        strBorder = QString("border-width:0px 0px %1px 0px;padding:%2px %2px "
                            "%1px %2px;")
                            .arg(borderWidth)
                            .arg(borderWidth * 2);
    } else if (type == "left") {
        strBorder = QString("border-width:0px 0px 0px %1px;padding:%2px %2px "
                            "%2px %1px;")
                            .arg(borderWidth)
                            .arg(borderWidth * 2);
    }
    ...
    QStringList qss;
    qss.append(QString("QWidget[flag=\"%1\"] "
                       "QAbstractButton{border-style:none;border-radius:0px;"
                       "padding:5px;color:%2;background:%3;}")
                       .arg(type)
                       .arg(normalTextColor)
                       .arg(normalBgColor));
    ...
    qss.append(QString("QWidget[flag=\"%1\"] QAbstractButton:hover,"
                       ...
                       "border-style:solid;%2border-color:%3;color:%4;"
                       "background:%5;}")
                       .arg(type)
                       .arg(strBorder)
                       .arg(borderColor)
...
void IconHelper::setStyle(QWidget *widget,
    ...
    qss.append(QString("QWidget#%1{background:%2;}")
    ...
    qss.append(QString("QToolButton{background-color:%1;color:%2;}")
                       .arg(normalBgColor)
                       .arg(normalTextColor));
    qss.append(QString("QToolButton:hover,QToolButton:pressed,QToolButton:"
                       "checked{background-color:%1;color:%2;}")
                       .arg(darkBgColor)
                       .arg(darkTextColor));
