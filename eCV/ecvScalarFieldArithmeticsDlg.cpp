										...
										QWidget* parent/*=0*/)
{
	...
	{
		CVLog::Warning("[ccScalarFieldArithmeticsDlg::apply] No input cloud, or cloud has no SF?!");
		assert(false);
		return false;
	}
	...
	if (op == INVALID)
	{
		CVLog::Warning("[ccScalarFieldArithmeticsDlg::apply] Invalid/unhandled operation");
		assert(false);
		return false;
	}
	...
	CVLib::ScalarField* sf1 = 0;
	{
		...
		{
			CVLog::Warning("[ccScalarFieldArithmeticsDlg::apply] Invalid SF1 index!");
			assert(false);
			return false;
		}
	...
	if (op <= DIVIDE)
	{
		...
		{
			CVLog::Warning("[ccScalarFieldArithmeticsDlg::apply] Invalid SF2 index/descriptor!");
			assert(false);
			return false;
		}
	...
	if (!inplace)
	{
		...
		if (sfIdx >= 0)
		{
			...
			{
				CVLog::Warning(QString("[ccScalarFieldArithmeticsDlg::apply] Resulting scalar field would have the same name as one of the operand (%1)! Rename it first...").arg(sfName));
				return false;
			}
		...
		if (sfIdx < 0)
		{
			CVLog::Warning("[ccScalarFieldArithmeticsDlg::apply] Failed to create destination SF! (not enough memory?)");
			return false;
		}
	}
	else
