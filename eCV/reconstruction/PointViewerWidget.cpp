...
void PointViewerWidget::Show(const point3D_t point3D_id) {
    ...
    setWindowTitle(
            QString::fromStdString("Point " + std::to_string(point3D_id)));
    ...
    std::vector<std::pair<TrackElement, std::string>>
            track_idx_image_name_pairs;
    ...
    for (const auto& track_el : track_idx_image_name_pairs) {
        const Image& image =
                model_viewer_widget_->images[track_el.first.image_id];
        ...
        const Eigen::Vector2d proj_point2D = ProjectPointToImage(
                point3D.XYZ(), image.ProjectionMatrix(), camera);
...
void PointViewerWidget::ResizeInfoTable() {
    ...
    int height = info_table_->horizontalHeader()->height() +
                 2 * info_table_->frameWidth();
...
void PointViewerWidget::UpdateImages() {
    ...
    for (size_t i = 0; i < location_pixmaps_.size(); ++i) {
        QLabel* image_id_label =
                new QLabel(QString::number(image_ids_[i]), this);
        ...
        QLabel* error_label =
                new QLabel(QString::number(reproj_errors_[i]), this);
        ...
        image_label->setPixmap(pixmap.scaledToWidth(zoom_ * pixmap.width(),
