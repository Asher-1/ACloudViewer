...
void ReconstructionWidget::close() {
    if (project_widget_->IsValid() && *options_.project_path == "") {
        ...
        reply = QMessageBox::question(
                this, "",
                tr("You have not saved your reconstruction project. Do you "
                   "want to save it?"),
...
void ReconstructionWidget::CreateWidgets() {
    ...
    dense_reconstruction_widget_ =
            new DenseReconstructionWidget(this, &options_);
...
void ReconstructionWidget::CreateActions() {
    ...
    action_project_new_ = new QAction(QIcon(":/media/project-new.png"),
                                      tr("New project"), this);
    ...
    action_project_open_ = new QAction(QIcon(":/media/project-open.png"),
    ...
    action_project_edit_ = new QAction(QIcon(":/media/project-edit.png"),
                                       tr("Edit project"), this);
    connect(action_project_edit_, &QAction::triggered, this,
            &ReconstructionWidget::ProjectEdit);

    action_project_save_ = new QAction(QIcon(":/media/project-save.png"),
    ...
    connect(action_export_as_, &QAction::triggered, this,
            &ReconstructionWidget::ExportAs);
    ...
    connect(action_quit_, &QAction::triggered, this,
            &ReconstructionWidget::close);
    ...
    action_feature_extraction_ =
            new QAction(QIcon(":/media/feature-extraction.png"),
                        tr("Feature extraction"), this);
    ...
    action_feature_matching_ =
            new QAction(QIcon(":/media/feature-matching.png"),
    ...
    action_bundle_adjustment_ =
            new QAction(QIcon(":/media/bundle-adjustment.png"),
                        tr("Bundle adjustment"), this);
    ...
    connect(action_render_reset_view_, &QAction::triggered,
            model_viewer_widget_, &ModelViewerWidget::ResetView);
    ...
    connect(reconstruction_manager_widget_,
    ...
    connect(action_log_show_, &QAction::triggered, this,
            &ReconstructionWidget::showLog);

    action_grab_image_ = new QAction(QIcon(":/media/grab-image.png"),
                                     tr("Grab image"), this);
    connect(action_grab_image_, &QAction::triggered, this,
            &ReconstructionWidget::GrabImage);

    action_grab_movie_ = new QAction(QIcon(":/media/grab-movie.png"),
                                     tr("Grab movie"), this);
    connect(action_grab_movie_, &QAction::triggered, model_viewer_widget_,
            &ModelViewerWidget::GrabMovie);

    action_undistort_ = new QAction(QIcon(":/media/undistort.png"),
                                    tr("Undistortion"), this);
    ...
    connect(action_render_, &QAction::triggered, this,
            &ReconstructionWidget::Render, Qt::BlockingQueuedConnection);

    action_render_now_ = new QAction(tr("Render now"), this);
    connect(action_render_now_, &QAction::triggered, this,
            &ReconstructionWidget::RenderNow, Qt::BlockingQueuedConnection);
    ...
    connect(action_reconstruction_finish_, &QAction::triggered, this,
            &ReconstructionWidget::ReconstructionFinish,
...
void ReconstructionWidget::CreateStatusbar() {
    ...
    connect(statusbar_timer_, &QTimer::timeout, this,
            &ReconstructionWidget::UpdateTimer);
...
void ReconstructionWidget::CreateControllers() {
    ...
    mapper_controller_.reset(new IncrementalMapperController(
            options_.mapper.get(), *options_.image_path,
            *options_.database_path, &reconstruction_manager_));
...
void ReconstructionWidget::ProjectSave() {
    if (!ExistsFile(*options_.project_path)) {
        QSettings settings;
        settings.beginGroup("Reconstruction");
        QString last_project_path =
                settings.value("project_path", "").toString();
        settings.endGroup();
    ...
    if (ExistsFile(project_path)) {
        ...
        QMessageBox::StandardButton reply = QMessageBox::question(
                ...
                tr("Directory does not contain a <i>project.ini</i>. To "
                   ...
                   "now (or press <i>No</i> to only visualize the "
                   "reconstruction)?"),
                QMessageBox::Yes | QMessageBox::No);
        if (reply == QMessageBox::Yes) {
            edit_project = true;
...
void ReconstructionWidget::ImportFrom() {
    QSettings settings;
    settings.beginGroup("Reconstruction");
    QString last_model_import_path =
            settings.value("model_import_path", "").toString();

    const std::string import_path =
            QFileDialog::getOpenFileName(this, tr("Select source..."),
                                         last_model_import_path)
                    .toUtf8()
                    .constData();
    ...
    thread_control_widget_->StartFunction("Importing...", [this,
                                                           import_path]() {
    ...
    if (ExistsFile(cameras_path) || ExistsFile(images_path) ||
        ...
        QMessageBox::StandardButton reply = QMessageBox::question(
                ...
                StringPrintf(
                        ...
                        "exist in the selected destination. Do you want to "
                        "overwrite them?",
                        cameras_name.c_str(), images_name.c_str(),
                        points3D_name.c_str())
                        .c_str(),
                QMessageBox::Yes | QMessageBox::No);
...
void ReconstructionWidget::ExportAs() {
    ...
    QString last_export_path =
            settings.value("model_export_path", "").toString();
...
void ReconstructionWidget::SetOptions() {
    ...
    data_items << "Individual images"
               ...
               << "Internet images";
    bool data_ok;
    const QString data_item = QInputDialog::getItem(
            this, "", "Data:", data_items, 0, false, &data_ok);
    if (!data_ok) {
        return;
    }
