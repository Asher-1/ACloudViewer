...
void DrawKeypoints(QPixmap* pixmap,
    ...
    const int pen_width =
            std::max(pixmap->width(), pixmap->height()) / 2048 + 1;
    ...
    for (const auto& point : points) {
        painter.drawEllipse(point.x - radius2, point.y - radius2, radius,
                            radius);
    }
}
...
QPixmap DrawMatches(const QPixmap& image1,
    ...
    for (const auto& point : points2) {
        painter.drawEllipse(image1.width() + point.x - radius2,
                            point.y - radius2, radius, radius);
    }
    ...
    for (const auto& match : matches) {
        ...
        painter.setPen(pen);
        painter.drawLine(
                QPoint(points1[idx1].x, points1[idx1].y),
