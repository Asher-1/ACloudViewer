...
#include "HierarchicalMapperController.h"
...
void MergeClusters(
        const colmap::SceneClustering::Cluster& cluster,
        std::unordered_map<const colmap::SceneClustering::Cluster*,
                           ReconstructionManager>* reconstruction_managers) {
    // Extract all reconstructions from all child clusters.
    std::vector<colmap::Reconstruction*> reconstructions;
    for (const auto& child_cluster : cluster.child_clusters) {
        ...
        auto& reconstruction_manager =
                reconstruction_managers->at(&child_cluster);
    ...
    while (reconstructions.size() > 1) {
        ...
        for (size_t i = 0; i < reconstructions.size(); ++i) {
            for (size_t j = 0; j < i; ++j) {
                ...
                if (reconstructions[i]->Merge(*reconstructions[j],
                                              kMaxReprojError)) {
    ...
    auto ReconstructCluster =
            [&, this](const SceneClustering::Cluster& cluster,
                ...
                for (const auto image_id : cluster.image_ids) {
                    custom_options.image_names.insert(
                            image_id_to_name.at(image_id));
                }

                IncrementalMapperController mapper(
                        &custom_options, options_.image_path,
    ...
    std::sort(leaf_clusters.begin(), leaf_clusters.end(),
              [](const SceneClustering::Cluster* cluster1,
                 const SceneClustering::Cluster* cluster2) {
                  return cluster1->image_ids.size() >
