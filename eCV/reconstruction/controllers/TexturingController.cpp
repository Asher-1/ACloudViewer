...
#ifdef USE_PCL_BACKEND
    PCLTextureMesh::Ptr texturedMesh = PclTools::CreateTexturingMesh(
            options_.meshed_file_path, *camera_trajectory_,
            options_.show_cameras, options_.verbose);
    if (texturedMesh) {
        if (!PclTools::SaveOBJFile(
                    options_.textured_file_path, *texturedMesh,
                    static_cast<unsigned>(options_.save_precision))) {
            CVLog::Warning("Saving textured mesh to %s failed!",
