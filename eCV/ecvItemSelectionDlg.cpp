...
ccItemSelectionDlg::ccItemSelectionDlg(bool multiSelectionEnabled,
    ...
    : QDialog(parent, Qt::Tool), Ui::ItemSelectionDlg() {
    setupUi(this);
...
void ccItemSelectionDlg::setItems(const QStringList& items,
    ...
    for (int i = 0; i < items.size(); ++i) {
        ...
        listWidget->insertItem(static_cast<int>(i),
                               new QListWidgetItem(items[i]));
    }

    // default selection
    if (defaultSelectedIndex >= 0 && defaultSelectedIndex < items.size()) {
        listWidget->item(defaultSelectedIndex)->setSelected(true);
        // listWidget->setItemSelected(listWidget->item(defaultSelectedIndex),
        // true);
    }
}
