...
public:
    ...
    /** Number of classes can be freely modified afterwards (if enabled).
            ...
            \param numberOfClassesCanBeChanged whether to allow the user to
    modify the number of classes \param showNaNValuesInGrey show NaN values (in
    gray) \return success
    **/
    ...
    //! Creates histogram from a bin array (each bin = number of elements per
    //! class)
    ...
    **/
    ...
    /** The curve will only appear over an histogram
            \param curveValues curve points 'Y' coordinates only (regularly
    sampled between the min and max histogram values)
    **/
