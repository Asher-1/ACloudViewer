...
ccComputeOctreeDlg::ccComputeOctreeDlg(const ccBBox& baseBBox,
                                       double minCellSize,
                                       QWidget* parent /*=0*/)
    : QDialog(parent), Ui::ComputeOctreeDialog(), m_bbEditorDlg(0) {
    setupUi(this);

    headerLabel->setText(QString("Max subdivision level: %1")
                                 .arg(ccOctree::MAX_OCTREE_LEVEL));
    ...
    if (minCellSize > 0.0) {
        ...
        CVLog::Warning(
                "[ccComputeOctreeDlg] Invalid minimum cell size specified!");
    ...
    if (baseBBox.isValid()) {
        ...
        CVLog::Warning(
                "[ccComputeOctreeDlg] Invalid base bounding-box specified!");
