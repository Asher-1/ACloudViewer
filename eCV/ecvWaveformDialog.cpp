	...
	, m_waveMax(0)
{
	...
	if (cloud && cloud->size())
	{
		...
		{
			CVLog::Print(QString("[ccWaveDialog] Cloud '%1': max FWF amplitude = %2").arg(cloud->getName()).arg(m_waveMax));
		}
		else
		{
...
void ccWaveDialog::onPointPickingButtonToggled(bool state)
{
	...
	if (state)
	{
		...
		{
			CVLog::Error("Another tool is currently using the point picking mechanism.\nYou'll have to close it first.");
