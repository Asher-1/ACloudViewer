	...
	, m_cloud(cloud)
{
    ...
    connect(buttonBox,	&QDialogButtonBox::accepted,	this,	&ccRasterizeTool::testAndAccept);
    connect(buttonBox,	&QDialogButtonBox::rejected,	this,	&ccRasterizeTool::testAndReject);

    connect(gridStepDoubleSpinBox,	static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged),	this,	&ccRasterizeTool::updateGridInfo);
    connect(gridStepDoubleSpinBox,	static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged),	this,	&ccRasterizeTool::gridOptionChanged);
    connect(emptyValueDoubleSpinBox,	static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged),	this,	&ccRasterizeTool::gridOptionChanged);
    connect(dimensionComboBox,		static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),			this,	&ccRasterizeTool::projectionDirChanged);
    connect(heightProjectionComboBox,	static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),			this,	&ccRasterizeTool::projectionTypeChanged);
    connect(scalarFieldProjection,	static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),			this,	&ccRasterizeTool::sfProjectionTypeChanged);
    connect(fillEmptyCellsComboBox,	static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),			this,	&ccRasterizeTool::fillEmptyCellStrategyChanged);

    connect(resampleCloudCheckBox,		&QAbstractButton::toggled,	this,	&ccRasterizeTool::resampleOptionToggled);
    connect(updateGridPushButton,			&QAbstractButton::clicked,	this,	&ccRasterizeTool::updateGridAndDisplay);
    connect(generateCloudPushButton,		&QAbstractButton::clicked,	this,	[this](){ generateCloud(true); } );
    connect(generateImagePushButton,		&QAbstractButton::clicked,	this,	&ccRasterizeTool::generateImage);
    connect(generateRasterPushButton,		&QAbstractButton::clicked,	this,	&ccRasterizeTool::generateRaster);
    connect(generateASCIIPushButton,		&QAbstractButton::clicked,	this,	&ccRasterizeTool::generateASCIIMatrix);
    connect(generateMeshPushButton,		&QAbstractButton::clicked,	this,	&ccRasterizeTool::generateMesh);
    connect(generateContoursPushButton,	&QAbstractButton::clicked,	this,	&ccRasterizeTool::generateContours);
    connect(exportContoursPushButton,		&QAbstractButton::clicked,	this,	&ccRasterizeTool::exportContourLines);
    connect(clearContoursPushButton,		&QAbstractButton::clicked,	this,	&ccRasterizeTool::removeContourLines);

    connect(generateHillshadePushButton, &QAbstractButton::clicked,	this,	&ccRasterizeTool::generateHillshade);

    connect(activeLayerComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this, [this] (int index)
    {
	...
	if (gridBBox.isValid())
	{
        ...
        connect(editGridToolButton, &QAbstractButton::clicked, this, &ccRasterizeTool::showGridBoxEditor);
	}
	else
	{
		...
		if (copyHillshadeSF)
		{
			...
			if (hillshadeSFIdx >= 0)
			{
				...
				if (hillshadeField->currentSize() == cloudGrid->size())
				{
					...
					{
						CVLog::Warning("[Rasterize] Not enough memory to export the hillshade field");
					}
				}
			}
		}
...
ccPointCloud* ccRasterizeTool::generateCloud(bool autoExport/*=true*/) const
{
	...
	if (rasterCloud && autoExport)
	{
		...
		if (m_cloud->isEnabled())
		{
			...
			CVLog::Warning("[Rasterize] Previously selected entity (source cloud) has been hidden!");
		}
		...
		if (mainWindow)
		{
			...
			CVLog::Print(QString("[Rasterize] Cloud '%1' successfully exported").arg(rasterCloud->getName()));
		}
		else
		{
			assert(false);
...
void ccRasterizeTool::generateMesh() const
{
	...
	if (rasterCloud)
	{
		...
		if (rasterMesh)
		{
			...
			CVLog::Print(QString("[Rasterize] Mesh '%1' successfully exported").arg(rasterMesh->getName()));
		}
		else
		{
...
void ccRasterizeTool::generateRaster() const
{
	...
	if (activeLayerComboBox->currentData().toInt() == LAYER_SF && m_cloud->isA(CV_TYPES::POINT_CLOUD))
	{
	...
	assert(false);
	CVLog::Error("[Rasterize] GDAL not supported by this version! Can't generate a raster...");
									...
									int visibleSfIndex/*=-1*/)
{
	...
	if (totalBands == 0)
	{
		CVLog::Error("Can't output a raster with no band! (check export parameters)");
		return false;
	}

	GDALAllRegister();
	CVLog::PrintDebug("(GDAL drivers: %i)", GetGDALDriverManager()->GetDriverCount());
	...
	{
		CVLog::Error("[GDAL] Driver %s doesn't support Create() method", pszFormat);
		return false;
	}
	...
	if (!poDstDS)
	{
		CVLog::Error("[GDAL] Failed to create output raster (not enough memory?)");
		return false;
	}
	...
	if (exportBands.rgb)
	{
		...
		if (error)
		{
			CVLog::Error("[GDAL] An error occurred while writing the color bands!");
			GDALClose(poDstDS);
			return false;
		}
	}
	...
	if (exportBands.height)
	{
		...
		for (unsigned j = 0; j < grid.height; ++j)
		{
			...
			{
				CVLog::Error("[GDAL] An error occurred while writing the height band!");
				if (scanline)
					CPLFree(scanline);
				GDALClose(poDstDS);
				return false;
			...
			if (exportBands.allSFs || (exportBands.visibleSF && visibleSfIndex == static_cast<int>(k)))
			{
				...
				for (unsigned j = 0; j < grid.height; ++j)
				{
											...
											GDT_Float64, 0, 0 ) != CE_None)
					{
						...
						CVLog::Error(QString("[GDAL] An error occurred while writing a scalar field band!"));
	...
	CVLog::Print(QString("[Rasterize] Raster '%1' successfully saved").arg(outputFilename));
	return true;
...
void ccRasterizeTool::addNewContour(ccPolyline* poly, double height, unsigned subIndex)
{
	...
	if (poly->size() > 1)
	{
		...
		if (ecvDisplayTools::GetMainWindow())
			ecvDisplayTools::AddToOwnDB(poly);
...
void ccRasterizeTool::generateContours()
{
...
#ifdef CV_GDAL_SUPPORT //use GDAL (more robust) - otherwise we will use an old code found on the Internet (with a strange behavior)
	...
	//feed the scan lines
	{
		...
		for (unsigned j = 0; j < m_grid.height; ++j)
		{
			...
			if (error != CE_None)
			{
				CVLog::Error("[GDAL] An error occurred during countour lines generation");
				break;
			}
		}
	...
	try
	{
		...
		while (v <= activeLayer->getMax() && !memoryError)
		{
			...
			CVLog::PrintDebug(QString("[Rasterize][Isolines] value=%1 : %2 lines").arg(v).arg(lineCount));
	...
	CVLog::Print(QString("[Rasterize] %1 iso-lines generated (%2 levels)").arg(m_contourLines.size()).arg(levelCount));
...
void ccRasterizeTool::exportContourLines()
{
	...
	CVLog::Print(QString("Contour lines have been successfully exported to DB (group name: %1)").arg(group->getName()));
}
...
void ccRasterizeTool::generateImage() const
{
	...
	if (!bitmap8.isNull())
	{
			...
			&&	m_rasterCloud->getCurrentDisplayedScalarField()->getColorScale())
		{
			...
			{
				const ecvColor::Rgb* col = colorScale->getColorByRelativePos(i / static_cast<double>(steps - 1), steps, &ecvColor::lightGrey);
...
void ccRasterizeTool::generateASCIIMatrix() const
{
	...
	QSettings settings;
	settings.beginGroup(ecvPS::HeightGridGeneration());
	QString asciiGridSavePath = settings.value("savePathASCIIGrid", ecvFileUtils::defaultDocPath()).toString();
	...
	if (!pFile)
	{
		CVLog::Warning(QString("[ccHeightGridGeneration] Failed to write '%1' file!").arg(outputFilename));
	}
	...
	CVLog::Print(QString("[Rasterize] Raster matrix '%1' successfully saved").arg(outputFilename));
}
