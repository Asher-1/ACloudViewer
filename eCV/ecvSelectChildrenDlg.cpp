...
ccSelectChildrenDlg::ccSelectChildrenDlg(QWidget* parent /*=0*/)
    : QDialog(parent, Qt::Tool), mUI(new Ui::SelectChildrenDialog) {
    mUI->setupUi(this);
...
void ccSelectChildrenDlg::addType(QString typeName, CV_CLASS_ENUM type) {
...
CV_CLASS_ENUM ccSelectChildrenDlg::getSelectedType() {
    ...
    return static_cast<CV_CLASS_ENUM>(
            mUI->typeComboBox->itemData(currentIndex).value<qint64>());
}
