...
The usage of CloudViewer convenient visualization functions ``draw_geometries`` and ``draw_geometries_with_custom_animation`` is straightforward. Everything can be done with the GUI. Press :kbd:`h` inside the visualizer window to see helper information. For more details, see :doc:`visualization`.
...
.. code-block:: python
    ...
    def main():
        ...
        cv3d.visualization.draw([{"name": "cube", "geometry": model, "material": material}])
