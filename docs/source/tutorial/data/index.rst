...
.. code-block:: python
    ...
    model = cv3d.io.read_triangle_model(dataset.path)
...
.. code-block:: python
    ...
    dataset = cv3d.data.FlightHelmetModel()
    model = cv3d.io.read_triangle_model(dataset.path)
...
.. code-block:: python
    ...
    dataset = cv3d.data.AvocadoModel()
    model = cv3d.io.read_triangle_model(dataset.path)
...
.. code-block:: python
    ...
    dataset = cv3d.data.DamagedHelmetModel()
    model = cv3d.io.read_triangle_model(dataset.path)
...
.. code-block:: python
    ...
    mat_data = cv3d.data.MetalTexture()

    mat = cv3d.visualization.rendering.MaterialRecord()
    mat.shader = "defaultLit"
    mat.albedo_img = cv3d.io.read_image(mat_data.albedo_texture_path)
    mat.normal_img = cv3d.io.read_image(mat_data.normal_texture_path)
    mat.roughness_img = cv3d.io.read_image(mat_data.roughness_texture_path)
    mat.metallic_img = cv3d.io.read_image(mat_data.metallic_texture_path)
...
.. code-block:: cpp
    ...
    mat.albedo_img = io::ReadImage(mat_data.albedo_texture_path);
    mat.normal_img = io::ReadImage(mat_data.normal_texture_path);
    mat.roughness_img = io::ReadImage(mat_data.roughness_texture_path);
    mat.metallic_img = io::ReadImage(mat_data.metallic_texture_path);
...
.. code-block:: python
    ...
    mat_data = cv3d.data.PaintedPlasterTexture()
...
.. code-block:: python
    ...
    mat_data = cv3d.data.TilesTexture()
...
.. code-block:: python
    ...
    mat_data = cv3d.data.TerrazzoTexture()
...
.. code-block:: python
    ...
    mat_data = cv3d.data.WoodTexture()
...
.. code-block:: python
    ...
    mat_data = cv3d.data.WoodFloorTexture()
...
.. code-block:: cpp
    ...
    auto img = io::ReadImage(img_data.path);
...
.. code-block:: cpp
    ...
    for(size_t i = 0; i < dataset.GetDepthPaths().size(); ++i) {
        auto color_raw = io::ReadImage(dataset.GetColorPaths()[i]);
        auto depth_raw = io::ReadImage(dataset.GetDepthPaths()[i]);
    ...
    auto pcd = io::ReadPointCloud(dataset.GetReconstructionPath());
...
.. code-block:: python
    ...
    dataset = cv3d.data.SampleFountainRGBDImages()
    ...
    mesh = cv3d.io.read_triangle_mesh(dataset.reconstruction_path)
...
.. code-block:: cpp
    ...
    io::ReadPinholeCameraTrajectory(dataset.GetKeyframePosesLogPath(),
                                    camera_trajectory);
    auto mesh = io::ReadTriangleMesh(dataset.GetReconstructionPath());
...
.. code-block:: python
    ...
    dataset = cv3d.data.SampleNYURGBDImage()
    ...
    rgbd_image = cv3d.geometry.RGBDImage.create_from_nyu_format(
        color, depth, convert_rgb_to_intensity=False)
...
.. code-block:: python
    ...
    dataset = cv3d.data.SampleSUNRGBDImage()
    color_raw = cv3d.io.read_image(dataset.color_path)
    depth_raw = cv3d.io.read_image(dataset.depth_path)
...
.. code-block:: cpp
    ...
    auto color_raw = io::ReadImage(dataset.GetColorPath());
    auto depth_raw = io::ReadImage(dataset.GetDepthPath());
...
.. code-block:: python
    ...
    dataset = cv3d.data.SampleTUMRGBDImage()
    color_raw = cv3d.io.read_image(dataset.color_path)
    depth_raw = cv3d.io.read_image(dataset.depth_path)
...
.. code-block:: python
    ...
    dataset = cv3d.data.LoungeRGBDImages()
...
.. code-block:: python
    ...
    dataset = cv3d.data.BedroomRGBDImages()
...
.. code-block:: python
    ...
    dataset = cv3d.data.DemoCropPointCloud()
    pcd = cv3d.io.read_point_cloud(dataset.point_cloud_path)
    vol = cv3d.visualization.read_selection_polygon_volume(dataset.cropped_json_path)
    chair = vol.crop_point_cloud(pcd)

.. code-block:: cpp

    data::DemoCropPointCloud dataset;
    auto pcd = io::ReadPointCloud(dataset.GetPointCloudPath());
...
.. code-block:: python
    ...
    dataset = cv3d.data.DemoFeatureMatchingPointClouds()

    pcd0 = cv3d.io.read_point_cloud(dataset.point_cloud_paths[0])
    pcd1 = cv3d.io.read_point_cloud(dataset.point_cloud_paths[1])

    fpfh_feature0 = cv3d.io.read_feature(dataset.fpfh_feature_paths[0])
    fpfh_feature1 = cv3d.io.read_feature(dataset.fpfh_feature_paths[1])

    l32d_feature0 = cv3d.io.read_feature(dataset.l32d_feature_paths[0])
    l32d_feature1 = cv3d.io.read_feature(dataset.l32d_feature_paths[1])

.. code-block:: cpp

    data::DemoFeatureMatchingPointClouds dataset;

    auto pcd0 = io::ReadPointCloud(dataset.GetPointCloudPaths()[0]);
    auto pcd1 = io::ReadPointCloud(dataset.GetPointCloudPaths()[1]);
...
.. code-block:: python
    ...
    dataset = cv3d.data.DemoPoseGraphOptimization()
    pose_graph_fragment = cv3d.io.read_pose_graph(dataset.pose_graph_fragment_path)
    pose_graph_global = cv3d.io.read_pose_graph(dataset.pose_graph_global_path)

.. code-block:: cpp

    data::DemoPoseGraphOptimization dataset;
    auto pose_graph_fragment = io::ReadPoseGraph(
                        dataset.GetPoseGraphFragmentPath());
    auto pose_graph_global = io::ReadPoseGraph(
...
.. code-block:: python
    ...
    dataset = cv3d.data.RedwoodIndoorLivingRoom1()
    assert Path(gt_download_dir).is_dir()
    pcd = cv3d.io.read_point_cloud(dataset.point_cloud_path)

    im_rgbds = []
    for color_path, depth_path in zip(dataset.color_paths, dataset.depth_paths):
        im_color = cv3d.io.read_image(color_path)
        im_depth = cv3d.io.read_image(depth_path)
        im_rgbd = cv3d.geometry.RGBDImage.create_from_color_and_depth(
            im_color, im_depth)
...
.. code-block:: cpp
    ...
    for (size_t i = 0; i < dataset.GetColorPaths().size(); ++i) {
        auto im_color = io::ReadImage(dataset.GetColorPaths()[i]);
        auto im_depth = io::ReadImage(dataset.GetDepthPaths()[i]);
    ...
    for (size_t i = 0; i < dataset.GetColorPaths().size(); ++i) {
        auto im_color = io::ReadImage(dataset.GetColorPaths()[i]);
        auto im_depth =
                io::ReadImage(dataset.GetNoisyDepthPaths()[i]);
        auto im_rgbd = geometry::RGBDImage::CreateFromColorAndDepth(*im_color,
                                                                    *im_depth);
...
.. code-block:: python
    ...
    dataset = cv3d.data.RedwoodIndoorLivingRoom2()
    assert Path(gt_download_dir).is_dir()
    pcd = cv3d.io.read_point_cloud(dataset.point_cloud_path)
...
.. code-block:: python
    ...
    dataset = cv3d.data.RedwoodIndoorOffice1()
    assert Path(gt_download_dir).is_dir()
    pcd = cv3d.io.read_point_cloud(dataset.point_cloud_path)
...
.. code-block:: python
    ...
    dataset = cv3d.data.RedwoodIndoorOffice2()
    assert Path(gt_download_dir).is_dir()
    pcd = cv3d.io.read_point_cloud(dataset.point_cloud_path)
