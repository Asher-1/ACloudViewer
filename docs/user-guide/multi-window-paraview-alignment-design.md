...
| Singleton API | ~1000+ external refs | **Routed** — `ecvDisplayTools` is **not** a singleton; engine instance held by `ecvViewManager::m_displayTools`. `ecvGenericDisplayTools::GetInstance()` → `ecvViewManager::instance().displayTools()`. Static helpers delegate to that instance or per-view virtuals. |
...
**ParaView 对齐率**: 91/91 = **100.0%** ALIGNED — 对比矩阵所有项均已对齐。

**已完成的里程碑**:
- ~~Phase P (View Type Registry)~~ — 仅在需要 SpreadSheet/Chart 等视图类型时启动（可选，不计入对齐矩阵）
...
- ~~GAP-S: Global Undo Stack~~ ✅ ALIGNED — `ecvUndoManager` (`QUndoStack`) + `ecvLayoutUndoCommand` / `ecvCameraUndoCommand`，Edit → Undo/Redo；布局与相机撤销并入全局栈
- ~~GAP-T: Source Undo~~ ✅ ALIGNED（2026-05-02）— `ecvPropertyChangeCommand`（模板化属性撤销）+ `ecvTransformCommand`（变换矩阵撤销）+ `ecvEntityAddRemoveCommand`（实体添加/删除撤销，含延迟序列化）+ `ecvScalarFieldEditCommand`（标量场编辑撤销）+ 内存预算系统（500 MB 上限）

**2026-05-02 新修复的运行时对齐 Gap（审计发现，非文档已记录）**:
- ~~Layout camera persistence~~ ✅ RESOLVED — `VtkVis::saveCameraToJson()`/`loadCameraFromJson()` + `ecvViewLayoutProxy` 保存/恢复每视图相机状态
- ~~restoreLayoutState layout leak~~ ✅ RESOLVED — 在 `restoreLayoutState()` 拆除多余 Tab 前调用 `unregisterLayout()`
- ~~zoomOnSelectedEntities view mismatch~~ ✅ RESOLVED — zoom 操作先通过 `findViewForEntity()` 定位实体所属视图
- ~~Plugin model association~~ ✅ RESOLVED — `forceAssociateToView()` 确保已有 display 的对象也能正确绑定
- ~~copyPrimaryViewConfig timing~~ ✅ RESOLVED — 使用显式 `getActiveView()` 替代 `effectiveCtx()` 作为上下文来源
...
vtkSMViewProxy                        ecvGenericGLDisplay (interface)
  ...
  └── Properties (via SM)               │   └── per-view signals
                                        │
pqView (Qt wrapper)                     └── ecvGLView (only concrete view type; N peers)
  ├── pqRenderView                          Each owns: ecvViewContext, VtkVis, QVTKWidgetCustom
  ├── widget() → QWidget                   Coordination: ecvViewManager (owns m_displayTools engine)
  ├── render() / forceRender()             └── VtkDisplayTools: per-ecvGLView engine service (not ecvGenericGLDisplay)
  └── supportsUndo()                       (no m_primaryCtx on a global "primary" view)
```

| Aspect | ParaView | ACloudViewer | Gap |
|--------|----------|-------------|-----|
| View class | `pqRenderView` (all views identical) | `ecvGLView` only — each peer owns full pipeline | **ALIGNED** (Phase M3: no global primary view) |
...
| Undo/Redo | `BEGIN_UNDO_SET` via `pqUndoStack` | `ecvUndoManager` + `beginUndoSet`/`endUndoSet` (`ecvLayoutUndoCommand`) + `VtkVis` camera (`ecvCameraUndoCommand`) | **ALIGNED** |
...
| Display-tools lifecycle | N/A (no singleton tools) | `initDisplayTools()` / `releaseDisplayTools()` on `ecvViewManager`; engine is **not** registered as a view | **ALIGNED** |
...
ACloudViewer Render Flow (current):
  ecvViewManager::redrawAll()
    ├── preRenderHousekeeping() (RemoveWidgets, CheckIfRemove, FontSize, …)
    └── For each ecvGLView in getAllViews():
        └── ScopedRenderOverride(view) → view->redraw()
            ├── getContext(ctx) from m_ctx (per-view)
            ├── VTK background + 3D/2D DB draw via this view's VtkDisplayTools
            ├── DrawColorRamp, Messages, ScaleBar (per-view)
            ├── DrawClickableItems (parameterized with this view's hot zone/items)
            └── renderWindow->Render()
```
...
| Layout camera restore | SM property serialize per view | `saveCameraToJson()`/`loadCameraFromJson()` in layout state | **ALIGNED** |
| Layout teardown cleanup | Proxy unregister on close | `unregisterLayout()` in `restoreLayoutState()` | **ALIGNED** |
| Project file | `.pvsm` (SM state XML) | `.acv` project file | **ALIGNED** |
...
| Aspect | ParaView | ACloudViewer | Status |
|--------|----------|-------------|--------|
| Global undo stack | `pqUndoStack` (wraps `vtkSMUndoStack`) | `ecvUndoManager::undoStack()` + Edit menu Undo/Redo (`QAction` from `QUndoStack`) | **ALIGNED** |
| Layout undo | `BEGIN_UNDO_SET` → global stack | `ecvLayoutUndoCommand` snapshots via `saveState()` / `loadState()` | **ALIGNED** |
| Camera undo | `pqCameraUndoRedoReaction` + global stack coexist | Per-view deque + **`ecvCameraUndoCommand`** on same global stack (`VtkVis::pushCameraState`) | **ALIGNED** |
| Source undo | SM undo elements | `ecvUndoManager` + `ecvPropertyChangeCommand`, `ecvTransformCommand`, `ecvEntityAddRemoveCommand`, `ecvScalarFieldEditCommand` (GAP-T, 2026-05-02) | **ALIGNED** |
...
| **L** | Singleton API removal | No `ecvDisplayTools` singleton | `ecvViewManager` owns engine; `ecvGenericDisplayTools::GetInstance()` → `displayTools()` | **DONE** |
...
Before M1–M3, this dual role caused:
...
### 4.8 Additional Alignment Items Resolved (2026-05-03)

The following operational alignment gaps were identified and resolved:

| Item | Problem | Resolution |
|------|---------|-----------|
| Layout camera persistence | Per-view camera state not saved in layout JSON | `saveCameraToJson()`/`loadCameraFromJson()` in `ecvViewLayoutProxy::saveState()`/`loadState()` |
| `restoreLayoutState` layout leak | Removed tabs not unregistered from `ecvViewManager` | `unregisterLayout()` called in `ecvTabbedMultiViewWidget::restoreLayoutState` |
| `zoomOnSelectedEntities` view mismatch | Zoom applied to wrong view for cross-view selection | Groups entities by owning view via `findViewForEntity()`, zooms per-view |
| Plugin model association | Objects with pre-set display skip active view binding | `forceAssociateToView()` re-binds in `MainWindow::addToDB` |
| `copyPrimaryViewConfig` timing | Could read stale `effectiveCtx()` during tab switch | Uses explicit `getActiveView()->viewContext()` with `effectiveCtx()` fallback |
...
graph TB
    ...
    subgraph ENGINE["Engine Layer — per ecvGLView + VM-owned dispatch"]
        VE["VtkDisplayTools (per view)"]
        DT["ecvDisplayTools dispatch via ecvViewManager::displayTools()"]
    end
...
├──────────────────────────────────────────────────────────────────────┤
│  Engine Layer                                                        │
│  Per ecvGLView: VtkDisplayTools  │  ecvViewManager::displayTools()   │
└──────────────────────────────────────────────────────────────────────┘
```
...
| `ecvViewManager` | Active-object tracking, view registry, signal relay, **owns `m_displayTools`**, global DB/font/removal state | Yes | Active view/source/repr; global batch flags; **`ecvDisplayTools*` engine** |
...
| `VtkDisplayTools` | CC→VTK rendering implementation; per-view instance on **`ecvGLView::m_displayTools`** | No | Bound to owning view's `VtkVis` / widget |
| `ecvDisplayTools` | Abstract engine + static helpers routing to **`ecvViewManager::displayTools()`** | No (**not** `TheInstance()` singleton) | No “primary” context — **`resolveViewContext()`** / per-view **`m_ctx`** |
...
### GAP-S: Global Undo Stack — 全局撤销/重做栈 ✅ ALIGNED

**对应矩阵**: §2.13 Undo/Redo — `pqUndoStack` (wraps `vtkSMUndoStack`)

**实现概要**:
- `ecvUndoManager` 由 `ecvViewManager` 持有，`QUndoStack` 限额 64；`createUndoAction` / `createRedoAction` 绑定 **Edit → Undo / Redo**（`QKeySequence::Undo` / `Redo`）。
- **布局**：`ecvViewLayoutProxy::beginUndoSet` / `endUndoSet` 在嵌套归零时用 `saveState()` / `loadState()` 生成 `ecvLayoutUndoCommand` 并 `push`；每个已注册 layout 通过 `registerLayout()` 获得同一 `undoManager`。新增 Tab (`ecvTabbedMultiViewWidget::createTab`) 现调用 `registerLayout`，关闭 Tab 时 `unregisterLayout`。
- **相机**：`VtkVis::pushCameraState()` 在与内部 deque 一致的「前一帧 / 当前帧」边界上附加 `ecvCameraUndoCommand`（`applyCameraState` + `VTK` 度数 FOV）；`ecvGLView::Create` 对每个视图 `setUndoManager`。Per-view 工具栏 **Camera Undo/Redo** 仍操作内部栈；全局 Edit Undo 可走 `QUndoStack`。
- Display 菜单中独立的「Undo Layout / Redo Layout」已移除（由 Edit 全局项替代）。

**涉及文件**: `ecvUndoManager.*`、`ecvLayoutUndoCommand`、`ecvCameraUndoCommand`、`ecvViewLayoutProxy.*`、`ecvViewManager.cpp`、`VtkVis.*`、`ecvGLView.cpp`、`MainWindow.cpp`、`ecvTabbedMultiViewWidget.cpp`

**优先级**: LOW — **已完成**（2026-05-02）

### ~~GAP-T: Source Undo~~ ✅ — 实体/数据源撤销

**对应矩阵**: §2.13 Undo/Redo — SM undo elements

**实现方案: QUndoCommand 命令模式** (基于 GAP-S):

```
基于 ecvUndoManager 的命令对象:
├── ecvPropertyChangeCommand<T>   // 模板化属性 before/after (可见性、不透明度、点大小、名称等)，实体级 merge ID
├── ecvTransformCommand           // 绝对变换矩阵快照，undo 时应用精确逆矩阵
├── ecvEntityAddRemoveCommand     // 实体添加/删除，延迟序列化到临时文件 (>10 MB 阈值)
└── ecvScalarFieldEditCommand     // 标量场值范围快照 undo
```

**已实现文件**:
| 操作 | 文件 |
|------|------|
| ✅ 新建 | `libs/CV_db/include/ecvPropertyChangeCommand.h` — 模板化，header-only |
| ✅ 新建 | `libs/CV_db/include/ecvTransformCommand.h` — header-only |
| ✅ 新建 | `libs/CV_db/include/ecvEntityAddRemoveCommand.h` + `src/ecvEntityAddRemoveCommand.cpp` |
| ✅ 新建 | `libs/CV_db/include/ecvScalarFieldEditCommand.h` — header-only |
| ✅ 修改 | `app/MainWindow.cpp` — `applyTransformation()` 包装为 `ecvTransformCommand` |
| ✅ 修改 | `app/db_tree/ecvDBRoot.cpp` — `deleteSelectedEntities` 包装为 `ecvEntityAddRemoveCommand`，6 种属性 toggle 包装为 `ecvPropertyChangeCommand` |
| ✅ 修改 | `libs/CV_db/src/ecvUndoManager.cpp` — 内存预算系统 |

**内存优化策略** (已实现):
- **属性变更** (`ecvPropertyChangeCommand<T>`): 仅记录 before/after 值 — 内存开销极小
- **变换操作** (`ecvTransformCommand`): 仅存储变换矩阵 — 固定 ~128 字节
- **实体添加/删除** (`ecvEntityAddRemoveCommand`): 延迟序列化到 `QTemporaryFile`，超过 10 MB 阈值时自动触发
- **内存预算**: 全局 undo 栈上限 500 MB，超限时自动减小 `setUndoLimit()` 丢弃最旧记录，最少保留 10 条
...
GAP-S (Global Undo)    ────→  90/91 = 98.9%  ✅ DONE
GAP-T (Source Undo)    ────→  91/91 = 100.0% ✅ DONE — Full ParaView alignment achieved
       │
       └── GAP-S is prerequisite for GAP-T (✅ cleared)
```

| 阶段 | 对齐率 | 说明 |
|------|--------|------|
| ~~当前（含 GAP-R、GAP-S）~~ | ~~90/91 = 98.9%~~ | ~~仅剩 GAP-T~~ |
| ✅ + GAP-T (2026-05-02) | 91/91 = **100%** | 实体级撤销已完成 |
