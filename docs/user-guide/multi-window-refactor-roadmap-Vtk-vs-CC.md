...
### TODO M3: ecvGLView 成为唯一视图类型 ✅ (M3.1 done) 🔲 (M3.2-M3.4)
...
| M3.3 | 删除 Category A 实现 (`switchActiveView`/`restorePrimaryView`/`adoptNewPrimary`/`resetToBuiltInPipeline`) | 🔲 |
| M3.4 | 简化 `MainWindow.cpp` 中 `dynamic_cast<ecvGLView*>` 分支（所有视图都是 ecvGLView） | 🔲 |
...
- [ ] `switchActiveView`/`restorePrimaryView`/`resetToBuiltInPipeline` 实现代码已删除
