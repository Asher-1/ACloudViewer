...
| ~~CUDA F16 conv_transpose abort~~ **FIXED** | YOLO seg/depth models on CUDA no longer abort | Upstream `ggml_cuda_conv_2d_transpose_p0` is F32-only for input/dst in both v0.18.1 and v0.21.0 (Vulkan has a native F16 pipeline from yolo_merged, CPU consumes F16 directly). Fixed at the graph level in `yolo_graph.cpp` (`GraphBuilder::conv_transpose` F32 detour on the CUDA f16 flow, same runtime-by-backend pattern as `interpolate`); verified 7-model × 3-backend matrix (detect/segment/depth, f16/f32) on CPU + Vulkan + CUDA with matching sanity signals |
