...
if not _build_config["BUILD_JUPYTER_EXTENSION"]:
    raise RuntimeError(
        "CloudViewer WebVisualizer Jupyter extension is not available. To use "
        "WebVisualizer, build CloudViewer with -DBUILD_JUPYTER_EXTENSION=ON.")


@ipywidgets.register
class WebVisualizer(ipywidgets.DOMWidget):
    """CloudViewer Web Visualizer based on WebRTC."""
    ...
    _view_module = traitlets.Unicode('cloudViewer').tag(sync=True)

    # Name of the front-end module containing widget model.
    _model_module = traitlets.Unicode('cloudViewer').tag(sync=True)
    ...
    def _call_http_api(self, entry_point, query_string, data):
        return cv3d.visualization.webrtc_server.call_http_api(
...
def draw(geometry=None,
    ...
    window_uid = async_event_loop.run_sync(
        functools.partial(cv3d.visualization.draw,
                          geometry=geometry,
                          title=title,
                          width=width,
                          height=height,
