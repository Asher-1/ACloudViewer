...
# ----------------------------------------------------------------------------
"""Utility functions for the CloudViewer TensorBoard plugin."""
...
# TODO(@ssheorey) Colormap and LabelLUT are duplicated from CloudViewer-ML. Remove
...
# Setup Python logger to emulate CloudViewer C++ logger.
_log = logging.getLogger("CloudViewer")
...
def _classify_properties(tensormap):
    ...
    for name, tensor in tensormap.items():
        ...
        if (tensor.shape == (exp_size, 1) and
                tensor.dtype not in (cv3d.core.float32, cv3d.core.float64,
                                     cv3d.core.undefined)):
...
class CloudViewerPluginDataReader:
    """Manage TB event data and geometry data for common use by all
    CloudViewerPluginWindow instances. This is thread safe for simultaneous use by
    ...
    def reload_events(self):
        """Reload event file"""
        # Quickly check for CloudViewer data, since adding all runs can take a long
        # time
    ...
    def is_active(self):
        """Do we have any CloudViewer data to display?"""
        with self._event_lock:
    ...
    def read_geometry(self, run, tag, step, batch_idx, step_to_idx):
        ...
        metadata_proto = plugin_data_pb2.CloudViewerPluginData()
        ...
        if geometry is None:  # Read from storage
            ...
            msg_tag, msg_step, geometry = cv3d.io.rpc.data_buffer_to_meta_geometry(
                buf)
            if geometry is None:
        ...
        for prop_ref in metadata_proto.property_references:
            prop = plugin_data_pb2.CloudViewerPluginData.GeometryProperty.Name(
            ...
            if prop_map == "vertex" and not isinstance(
                    geometry, cv3d.t.geometry.TriangleMesh):
                prop_map = "point"
...
def _normalize(tensor):
    ...
    Args:
        tensor: CloudViewer / Numpy float tensor. Int tensors are returned unchanged.
    ...
    """
    if tensor.dtype in (cv3d.core.float32, cv3d.core.float64, np.float32,
                        np.float64):
...
class RenderUpdate:
    ...
    def __init__(self, window_scaling, message, label_to_names):
        from cloudViewer.visualization.async_event_loop import async_event_loop
        ...
        if self._shader == "":
            if (len(inference_result) > 0  # Have BB labels
                ...
                if self._property == "":
                    ...
                    self._index = 0
            elif (isinstance(geometry, cv3d.t.geometry.LineSet) or
    ...
    class BackupRestore():
        ...
        def backup(self, tm, prop, clone=False, shape=None, dtype=None):
            ...
            if "__" + prop in tm:  # __prop -> prop
                ...
                tm[prop] = cv3d.core.Tensor.empty(shape, dtype)
    ...
    def apply(self, cv3dvis, geometry_name, geometry, inference_data_proto=None):
        ...
        Args:
            cv3dvis (O3DVisualizer): Window containing the geometry.
            geometry_name (str): Geometry name in the window.
            geometry (cv3d.t.geometry): Geometry whose rendering is to be
                updated.
        ...
        if cv3dvis.scene.has_geometry(geometry_name):
            updated = self._updated
            # update_geometry() only accepts tPointCloud
            if not isinstance(geometry, cv3d.t.geometry.PointCloud):
                cv3dvis.remove_geometry(geometry_name)
        else:
        ...
        material = cv3dvis.get_geometry_material(geometry_name)
        ...
        if cv3dvis.scene.has_geometry(geometry_name):
            if material_update_flag > 0:
                self._gui.run_sync(
                    cv3dvis.modify_geometry_material, geometry_name, material
                )  # does not do force_redraw(), so also need update_geometry()
            self._gui.run_sync(cv3dvis.update_geometry, geometry_name, geometry,
                               geometry_update_flag)
            _log.debug(
                f"Geometry {geometry_name} updated with flags "
                f"Geo:{geometry_update_flag:b}, Mat:{material_update_flag:b}")
        else:
            self._gui.run_sync(cv3dvis.add_geometry, geometry_name, geometry,
                               material if material_update_flag else None)
        self._gui.run_sync(cv3dvis.post_redraw)
...
def to_dict_batch(cv3d_geometry_list):
    """Convert sequence of identical CloudViewer Eigen geometry types to
    attribute-tensor dictionary. The geometry sequence forms a batch of data.
    Custom attributes are not supported.

    Args:
        cv3d_geometry_list (Iterable): Iterable (list / tuple / sequence
            generator) of CloudViewer Eigen geometry types.

    Returns:
        Dict[str: numpy.array]: Dictionary of property names and corresponding
        Numpy arrays.
    """
    if len(cv3d_geometry_list) == 0:
        return {}
    vertex_positions = []
    vertex_colors = []
    vertex_normals = []
    ...
    elif isinstance(cv3d_geometry_list[0], cv3d.geometry.LineSet):
        for geometry in cv3d_geometry_list:
            vertex_positions.append(np.asarray(geometry.points))
