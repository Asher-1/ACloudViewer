...
# ----------------------------------------------------------------------------
"""CloudViewer visualization plugin for TensorBoard."""
import os
import sys
import threading
import json
...
from cloudViewer.visualization import webrtc_server
from . import plugin_data_pb2
from . import metadata
from .util import CloudViewerPluginDataReader
from .util import RenderUpdate
from .util import _log


class CloudViewerPluginWindow:
    """Create and manage a single CloudViewer WebRTC GUI window."""

    def __init__(self,
                 data_reader,
                 title="CloudViewer for Tensorboard",
                 width=1024,
                 height=768):
        """
        Args:
            data_reader: CloudViewerPluginDataReader object to read Tensorboard event
                files and CloudViewer geometry files.
    ...
    def _create_ui(self, title, width, height):
        """Create new CloudViewer application window and rendering widgets. Must run
        in the GUI thread.
...
class CloudViewerPlugin(base_plugin.TBPlugin):
    """CloudViewer plugin for TensorBoard.
    ...
    def __init__(self, context):
        """Instantiates CloudViewer plugin.

        Args:
            context: A `base_plugin.TBContext` instance.
        """
        self._logdir = context.logdir
        self.data_reader = CloudViewerPluginDataReader(self._logdir)
    ...
    def _serve_js(self, request):
        ...
        if request.is_multiprocess:
            return werkzeug.exceptions.ExpectationFailed(
                "CloudViewer plugin does not run on a multi-process web server.",
                response=self._ERROR_RESPONSE)
