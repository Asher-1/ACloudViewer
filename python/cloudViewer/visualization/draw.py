...
def draw(geometry=None,
    ...
    :class:`cloudViewer.visualization.O3DVisualizer`.
    ...
    Args:
        geometry (List[Geometry] or List[Dict]): The 3D data to be displayed can be provided in different types:
            - A list of any CloudViewer geometry types (``PointCloud``, ``TriangleMesh``, ``LineSet`` or ``TriangleMeshModel``).
            - A list of dictionaries with geometry data and additional metadata. The following keys are used:
                - **name** (str): Geometry name.
                - **geometry** (Geometry): CloudViewer geometry to be drawn.
                - **material** (:class:`cloudViewer.visualization.rendering.MaterialRecord`): PBR material for the geometry.
        ...
        bg_color (array of shape (4,)): Background color float with range [0,1],
            default white.
        bg_image (cloudViewer.geometry.Image): Background image.
        ibl (cloudViewer.geometry.Image): Environment map for image based lighting
            (IBL).
        ...
        rpc_interface (bool or str): Start an RPC interface at this local
            ...
            made with :class:`cloudViewer.visualization.ExternalVisualizer`.
