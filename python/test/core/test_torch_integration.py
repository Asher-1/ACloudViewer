...
def test_tensor_to_pytorch_scope(device):
    ...
    def get_dst_t():
        o3d_t = cv3c.Tensor(src_t, device=device)  # Copy
        # Use to_dlpack() to get capsule, then convert using torch.utils.dlpack
...
def test_tensor_from_to_pytorch(device):
    ...
    # a, b, c share memory.
    a = torch.ones((2, 2))
    if device_type == cv3c.Device.DeviceType.CUDA:
        a = a.cuda(device_id)
    # Use to_dlpack() to get capsule for from_dlpack
    b = cv3c.Tensor.from_dlpack(torch.utils.dlpack.to_dlpack(a))
    ...
    o3_t = cv3c.Tensor.from_dlpack(torch.utils.dlpack.to_dlpack(th_t))
