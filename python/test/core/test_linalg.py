...
def test_matmul(device, dtype):
    ...
    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor.zeros((3, 4, 5), dtype=dtype)
        b = cv3c.Tensor.zeros((4, 5), dtype=dtype)
        c = a @ b
        assert 'Tensor A must be 2D' in str(excinfo.value)

    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor.zeros((3, 4), dtype=dtype)
        b = cv3c.Tensor.zeros((4, 5, 6), dtype=dtype)
        c = a @ b
    ...
    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor.zeros((3, 4), dtype=dtype)
        b = cv3c.Tensor.zeros((3, 7), dtype=dtype)
        c = a @ b
    ...
    for shapes in [((0, 0), (0, 0)), ((2, 0), (0, 3)), ((0, 2), (2, 0)),
        ...
        with pytest.raises(RuntimeError) as excinfo:
            ...
            a = cv3c.Tensor.zeros(a_shape, dtype=dtype, device=device)
            b = cv3c.Tensor.zeros(b_shape, dtype=dtype, device=device)
            c = a @ b
            assert 'dimensions with zero' in str(excinfo.value)


@pytest.mark.parametrize("device", list_devices())
@pytest.mark.parametrize("dtype", [cv3c.float32, cv3c.float64])
def test_addmm(device, dtype):
    ...
    alpha = 1.0
    beta = 1.0
    c = cv3c.addmm(input, a, b, alpha, beta)
    # c = cv3c.matmul(a, b)
    assert c.shape == cv3c.SizeVector([2, 4])
    ...
    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor.zeros((3, 4, 5), dtype=dtype)
        b = cv3c.Tensor.zeros((4, 5), dtype=dtype)
        input = cv3c.Tensor.zeros((3, 5), dtype=dtype)
        c = cv3c.addmm(input, a, b, alpha, beta)
        assert 'Tensor A must be 2D' in str(excinfo.value)
    ...
    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor.zeros((3, 4), dtype=dtype)
        b = cv3c.Tensor.zeros((4, 5), dtype=dtype)
        input = cv3c.Tensor.zeros((3, 7), dtype=dtype)
        c = cv3c.addmm(input, a, b, alpha, beta)
...
def test_det(device, dtype):
    ...
    for shape in [(), [1], (3, 4, 5)]:
        with pytest.raises(RuntimeError) as excinfo:
            a = cv3c.Tensor.zeros(shape, dtype=dtype, device=device)
            a.det()
            assert 'must be 2D' in str(excinfo.value)

    # Non-square
    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor.zeros((2, 3), dtype=dtype, device=device)
        a.det()
...
def test_lu(device, dtype):
    # Skip CUDA device - LU decomposition has a CUDA memory management bug
    ...
    for shape in [(), [1], (3, 4, 5)]:
        with pytest.raises(RuntimeError) as excinfo:
            a_ = cv3c.Tensor.zeros(shape, dtype=dtype, device=device)
            cv3c.lu(a_)
            assert 'must be 2D' in str(excinfo.value)
    ...
    if dtype in [cv3c.int32, cv3c.int64]:
        with pytest.raises(RuntimeError) as excinfo:
            ...
            assert 'Only tensors with Float32 or Float64 are supported' in str(
                excinfo.value)
        return
    ipiv, a_lu = cv3c.lu_ipiv(a)

    a_lu_ = cv3c.Tensor(
        [[3.0, 3.0, 1.0], [0.666667, 2.0, 0.333333], [0.666667, 0.5, 0.166667]],
        dtype=dtype)
    ipiv_ = cv3c.Tensor([2, 3, 3], dtype=dtype)
...
def test_inverse(device, dtype):
    ...
    a = cv3c.Tensor([[1]], dtype=dtype, device=device)
    np.testing.assert_allclose(a.cpu().numpy(), a.inv().cpu().numpy(), 1e-6)

    # Singular condition
    for a in [
            cv3c.Tensor([[0, 0], [0, 1]], dtype=dtype, device=device),
            cv3c.Tensor([[0]], dtype=dtype, device=device)
    ]:
        with pytest.raises(RuntimeError) as excinfo:
            a_inv = a.inv()
...
def test_svd(device, dtype):
    ...
    u, s, vt = cv3c.svd(a)
    assert u.shape == cv3c.SizeVector([4, 4])
    assert s.shape == cv3c.SizeVector([2])
    assert vt.shape == cv3c.SizeVector([2, 2])

    # u and vt are orthogonal matrices
    uut = u @ u.T()
    eye_uut = cv3c.Tensor.eye(4, dtype=dtype)
    np.testing.assert_allclose(uut.cpu().numpy(),
                               eye_uut.cpu().numpy(),
                               atol=1e-6)

    vvt = vt.T() @ vt
    eye_vvt = cv3c.Tensor.eye(2, dtype=dtype)
    np.testing.assert_allclose(vvt.cpu().numpy(),
                               eye_vvt.cpu().numpy(),
                               atol=1e-6)

    usvt = u[:, :2] @ cv3c.Tensor.diag(s) @ vt
    ...
    for shapes in [(0, 0), (0, 2)]:
        with pytest.raises(RuntimeError) as excinfo:
            a = cv3c.Tensor.zeros(shapes, dtype=dtype, device=device)
            a.svd()
            assert 'dimensions with zero' in str(excinfo.value)
    ...
    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor(shapes, dtype=dtype, device=device)
        a.svd()
        assert 'must be 2D' in str(excinfo.value)
...
def test_solve(device, dtype):
    # Test square
    a = cv3c.Tensor([[3, 1], [1, 2]], dtype=dtype, device=device)
    b = cv3c.Tensor([9, 8], dtype=dtype, device=device)
    x = cv3c.solve(a, b)
    ...
    with pytest.raises(RuntimeError) as excinfo:
        a = cv3c.Tensor.zeros((3, 3), dtype=dtype, device=device)
        b = cv3c.Tensor.ones((3,), dtype=dtype, device=device)
        x = cv3c.solve(a, b)
...
def test_thiu(device, dtype):
    ...
    np.testing.assert_allclose(cv3c.triu(a, 1).cpu().numpy(),
...
def test_thil(device, dtype):
    ...
    np.testing.assert_allclose(cv3c.tril(a).cpu().numpy(),
                               ...
                               atol=1e-5)
    # Test positive diagonal value (= 1).
    np.testing.assert_allclose(cv3c.tril(a, 1).cpu().numpy(),
