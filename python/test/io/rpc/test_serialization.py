...
def test_set_mesh_data_deserialization():
    ...
    def set_mesh_data_to_geometry(*args, **kwargs):
        bc = cv3d.io.rpc.BufferConnection()
        cv3d.io.rpc.set_mesh_data(*args, **kwargs, connection=bc)
        return cv3d.io.rpc.data_buffer_to_meta_geometry(bc.get_buffer())
    ...
    o3d_texture_maps = {
        key: cv3d.t.geometry.Image(cv3d.core.Tensor(value))
    ...
    assert path == "pcd"
    assert time == 123
    assert isinstance(geom, cv3d.t.geometry.PointCloud)
    ...
    assert path == "trimesh"
    assert time == 123
    assert isinstance(geom, cv3d.t.geometry.TriangleMesh)
    ...
    assert path == "lines"
    assert time == 123
    assert isinstance(geom, cv3d.t.geometry.LineSet)
    np.testing.assert_equal(geom.point.positions.numpy(), verts)
...
def test_recv_msgpack():
    ...
    out_o3d = cv3d.io.rpc.data_buffer_to_meta_geometry(data)
