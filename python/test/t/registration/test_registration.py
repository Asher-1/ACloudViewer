...
def get_pcds(dtype, device):
    ...
    source = cv3d.t.geometry.PointCloud(device)
    target = cv3d.t.geometry.PointCloud(device)
...
def test_icp_point_to_plane(device):
    ...
    for dtype in supported_dtypes:
        ...
        # Tensor-based ICP uses multi-scale approach which can produce different
...
def test_get_information_matrix(device):
    ...
    for dtype in supported_dtypes:
        ...
        info_matrix_legacy = cv3d.pipelines.registration.get_information_matrix_from_point_clouds(
            source_legacy, target_legacy, max_correspondence_distance,
