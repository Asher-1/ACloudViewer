...
def test_dataset_base():
    default_data_root = Path.home() / "cloudViewer_data"

    ds = cv3d.data.Dataset("some_prefix")
    assert Path(ds.data_root) == default_data_root

    ds_custom = cv3d.data.Dataset("some_prefix", "/my/custom/data_root")
...
def get_test_data_dirs(prefix):
    gt_data_root = Path.home() / "cloudViewer_data"
...
def test_demo_icp_pointclouds():
    ...
    demo_icp = cv3d.data.DemoICPPointClouds()
...
def test_demo_colored_icp_pointclouds():
    ...
    demo_colored_icp = cv3d.data.DemoColoredICPPointClouds()
    assert Path(gt_download_dir).is_dir()
...
def test_demo_crop_pointcloud():
    ...
    demo_crop_pcd = cv3d.data.DemoCropPointCloud()
    assert Path(gt_download_dir).is_dir()
...
def test_demo_feature_matching_point_clouds():
    ...
    demo_feature_matching = cv3d.data.DemoFeatureMatchingPointClouds()
    assert Path(gt_download_dir).is_dir()
...
def test_demo_pose_graph_optimization():
    ...
    demo_pose_optimization = cv3d.data.DemoPoseGraphOptimization()
    assert Path(gt_download_dir).is_dir()
...
def test_ply_point_cloud():
    ...
    ply_pointcloud = cv3d.data.PLYPointCloud()
    assert Path(gt_download_dir).is_dir()
...
def test_sample_nyu_rgbd_image():
    ...
    rgbd_image_nyu = cv3d.data.SampleNYURGBDImage()
    assert Path(gt_download_dir).is_dir()
...
def test_sample_sun_rgbd_image():
    ...
    rgbd_image_sun = cv3d.data.SampleSUNRGBDImage()
    assert Path(gt_download_dir).is_dir()
...
def test_sample_tum_rgbd_image():
    ...
    rgbd_image_tum = cv3d.data.SampleTUMRGBDImage()
    assert Path(gt_download_dir).is_dir()
...
def test_sample_redwood_rgbd_images():
    ...
    rgbd_dataset_redwood = cv3d.data.SampleRedwoodRGBDImages()
    assert Path(gt_download_dir).is_dir()
...
def test_sample_fountain_rgbd_images():
    ...
    fountain_dataset = cv3d.data.SampleFountainRGBDImages()
    assert Path(gt_download_dir).is_dir()
...
def test_flight_helmet():
    ...
    helmet = cv3d.data.FlightHelmetModel()
    assert Path(gt_download_dir).is_dir()
...
def test_avocado():
    ...
    avocado = cv3d.data.AvocadoModel()
    assert Path(gt_download_dir).is_dir()
...
def test_damaged_helmet():
    ...
    damaged_helmet = cv3d.data.DamagedHelmetModel()
    assert Path(gt_download_dir).is_dir()
...
def test_painted_plaster_texture():
    ...
    data = cv3d.data.PaintedPlasterTexture()
    assert Path(gt_download_dir).is_dir()
...
def test_terrazzo_texture():
    ...
    data = cv3d.data.TerrazzoTexture()
    assert Path(gt_download_dir).is_dir()
...
def test_wood_floor_texture():
    ...
    data = cv3d.data.WoodFloorTexture()
    assert Path(gt_download_dir).is_dir()
...
def test_bin_baluster_vase():
    ...
    assert Path(baluster_vase_data.data_root) == gt_data_root
    assert Path(baluster_vase_data.download_dir) == gt_download_dir
    assert Path(baluster_vase_data.extract_dir) == gt_extract_dir
