...
def _draw_webrtc(parser, args):
    ...
    filetype = cv3d.io.read_file_geometry_type(args.filename)
    if (filetype & cv3d.io.CONTAINS_TRIANGLES):
        geometry = cv3d.io.read_triangle_model(args.filename)
    else:
        geometry = cv3d.t.io.read_point_cloud(args.filename)
    ...
    cv3d.visualization.draw(geometry)
