...
public:
	...
	void setNumberOfRows(int rows)
	{
		...
		this->add->setEnabled(rows < ::ecvCustomViewpointButtonDlg::MAXIMUM_NUMBER_OF_ITEMS);
		...
		for (int cc = this->Rows.size(); cc < rows; ++cc)
		{
			...
			arow.ToolTipEdit->setText(::ecvCustomViewpointButtonDlg::DEFAULT_TOOLTIP);
			...
			if (cc >= ::ecvCustomViewpointButtonDlg::MINIMUM_NUMBER_OF_ITEMS)
			{
				...
				arow.DeleteButton->setIcon(QIcon(":/Resources/images/ecvdelete.png"));
	...
	void deleteRow(int index)
	{
		...
		// enable/disable add button.
		this->add->setEnabled(
			this->Rows.size() < ::ecvCustomViewpointButtonDlg::MAXIMUM_NUMBER_OF_ITEMS);
	}
};
...
//------------------------------------------------------------------------------
const QString ecvCustomViewpointButtonDlg::DEFAULT_TOOLTIP = QString("Unnamed Viewpoint");
...
//------------------------------------------------------------------------------
ecvCustomViewpointButtonDlg::ecvCustomViewpointButtonDlg(QWidget* Parent, Qt::WindowFlags flags,
...
void ecvCustomViewpointButtonDlg::importConfigurations()
{
	...
	QStringList selectedFiles =
		QFileDialog::getOpenFileNames(
			this, QString("Load Custom Viewpoints Configuration"),
			currentPath, filters);
...
void ecvCustomViewpointButtonDlg::assignCurrentViewpoint()
{
	...
	if (row >= 0 && row < this->ui->rowCount())
	{
		...
		if (this->ui->toolTip(row) == ecvCustomViewpointButtonDlg::DEFAULT_TOOLTIP)
		{
