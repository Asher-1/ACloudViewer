...
void TwoViewInfoTab::ShowMatches() {
    ...
    if (select->selectedRows().size() > 1) {
        QMessageBox::critical(this, "",
                              tr("Only one image pair may be selected."));
        return;
    }
...
void TwoViewInfoTab::FillTable() {
    // Sort the matched pairs according to number of matches in descending
    // order.
    ...
    for (size_t i = 0; i < sorted_matches_idxs_.size(); ++i) {
        ...
        QTableWidgetItem* image_id_item = new QTableWidgetItem(
                QString::number(matches_[idx].first->ImageId()));
        table_widget_->setItem(i, 0, image_id_item);

        QTableWidgetItem* num_matches_item = new QTableWidgetItem(
                QString::number(matches_[idx].second.size()));
...
void MatchesTab::Reload(const std::vector<Image>& images,
    ...
    for (const auto& image : images) {
        ...
        if (database_->ExistsMatches(image_id, image.ImageId())) {
            const auto matches =
                    database_->ReadMatches(image_id, image.ImageId());
...
OverlappingImagesWidget::OverlappingImagesWidget(QWidget* parent,
    ...
    two_view_geometries_tab_ =
            new TwoViewGeometriesTab(this, options_, database);
...
void OverlappingImagesWidget::ShowMatches(const std::vector<Image>& images,
    ...
    setWindowTitle(QString::fromStdString("Matches for image " +
...
CameraTab::CameraTab(QWidget* parent, Database* database)
    ...
    connect(set_model_button, &QPushButton::released, this,
            &CameraTab::SetModel);
...
void CameraTab::Reload() {
    ...
    for (size_t i = 0; i < cameras_.size(); ++i) {
        ...
        QTableWidgetItem* model_item = new QTableWidgetItem(
                QString::fromStdString(camera.ModelName()));
...
void CameraTab::itemChanged(QTableWidgetItem* item) {
    ...
    switch (item->column()) {
        ...
        case 3:
            ...
            break;
        case 4:
            if (!camera.SetParamsFromString(
                        item->text().toUtf8().constData())) {
                QMessageBox::critical(this, "",
                                      tr("Invalid camera parameters."));
                table_widget_->blockSignals(true);
...
#undef CAMERA_MODEL_CASE
    ...
    camera.InitializeWithName(camera_model.toUtf8().constData(),
                              kDefaultFocalLength, kDefaultWidth,
                              kDefaultHeight);
...
ImageTab::ImageTab(QWidget* parent,
    ...
    QPushButton* split_camera_button =
            new QPushButton(tr("Split camera"), this);
...
void ImageTab::Reload() {
    ...
    // use temp vector to avoid memory problem when directly assigning to member
    // variable std::vector<Image> temp_images = database_->ReadAllImages();
    // images_.clear();
    ...
    for (size_t i = 0; i < images_.size(); ++i) {
        ...
        table_widget_->setItem(
                i, 1,
                new QTableWidgetItem(QString::fromStdString(image.Name())));
        table_widget_->setItem(
                i, 2, new QTableWidgetItem(QString::number(image.CameraId())));
        table_widget_->setItem(
                i, 3,
                new QTableWidgetItem(QString::number(image.QvecPrior(0))));
        table_widget_->setItem(
                i, 4,
                new QTableWidgetItem(QString::number(image.QvecPrior(1))));
        table_widget_->setItem(
                i, 5,
                new QTableWidgetItem(QString::number(image.QvecPrior(2))));
        table_widget_->setItem(
                i, 6,
                new QTableWidgetItem(QString::number(image.QvecPrior(3))));
        table_widget_->setItem(
                i, 7,
                new QTableWidgetItem(QString::number(image.TvecPrior(0))));
        table_widget_->setItem(
                i, 8,
                new QTableWidgetItem(QString::number(image.TvecPrior(1))));
        table_widget_->setItem(
                i, 9,
                new QTableWidgetItem(QString::number(image.TvecPrior(2))));
    }
    table_widget_->resizeColumnsToContents();
...
void ImageTab::SetCamera() {
    ...
    bool ok;
    const camera_t camera_id = static_cast<camera_t>(QInputDialog::getInt(
            this, "", tr("camera_id"), 0, 0, INT_MAX, 1, &ok));
    if (!ok) {
        return;
    }
