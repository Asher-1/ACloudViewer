...
ccUnrollDlg::ccUnrollDlg(QWidget* parent /*=0*/)
    ...
    connect(checkBoxAuto, &QCheckBox::stateChanged, this,
            &ccUnrollDlg::axisAutoStateChanged);
    connect(comboBoxUnrollShapeType,
            static_cast<void (QComboBox::*)(int)>(
                    &QComboBox::currentIndexChanged),
            this, &ccUnrollDlg::shapeTypeChanged);
    connect(comboBoxAxisDimension,
            static_cast<void (QComboBox::*)(int)>(
                    &QComboBox::currentIndexChanged),
            this, &ccUnrollDlg::axisDimensionChanged);
...
ccPointCloud::UnrollMode ccUnrollDlg::getType() const {
    return static_cast<ccPointCloud::UnrollMode>(
            comboBoxUnrollShapeType->currentIndex());
}
...
CCVector3 ccUnrollDlg::getAxisPosition() const {
    return CCVector3(
            static_cast<PointCoordinateType>(doubleSpinBoxAxisX->value()),
            static_cast<PointCoordinateType>(doubleSpinBoxAxisY->value()),
            static_cast<PointCoordinateType>(doubleSpinBoxAxisZ->value()));
}

double ccUnrollDlg::getRadius() const { return radiusDoubleSpinBox->value(); }
...
void ccUnrollDlg::shapeTypeChanged(int index) {
    switch (index) {
        ...
        case ccPointCloud::STRAIGHTENED_CONE:   // straightened cone (fixed
                                                // radius)
...
void ccUnrollDlg::axisDimensionChanged(int index) {
    ...
    if (comboBoxUnrollShapeType->currentIndex() != 0 ||
        checkBoxAuto->checkState() == Qt::Checked) {
        return;
    }
...
void ccUnrollDlg::toPersistentSettings() const {
    ...
    settings.beginGroup("Unroll");
    {
        ...
        settings.setValue("axisDimension",
                          comboBoxAxisDimension->currentIndex());
        ...
        settings.setValue("exportDeviationSF",
                          exportDeviationSFCheckBox->isChecked());
...
void ccUnrollDlg::fromPersistentSettings() {
    ...
    settings.beginGroup("Unroll");
    {
        int shapeType = settings.value("shapeType",
                                       comboBoxUnrollShapeType->currentIndex())
                                .toInt();
        int axisDim = settings.value("axisDimension",
                                     comboBoxAxisDimension->currentIndex())
                              .toInt();
        double angle = settings.value("angle", halfAngleDoubleSpinBox->value())
                               .toDouble();
        double radius = settings.value("radius", radiusDoubleSpinBox->value())
                                .toDouble();
        bool autoCenter =
                settings.value("autoCenter", checkBoxAuto->isChecked())
                        .toBool();
        bool exportDeviationSF =
                settings.value("exportDeviationSF",
                               exportDeviationSFCheckBox->isChecked())
                        .toBool();
