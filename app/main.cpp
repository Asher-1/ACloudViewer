...
int HandleQuickFlags(int argc, char* argv[]) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
            printf(
                "ACloudViewer %s - 3D Point Cloud & Mesh Processing\n"
                "\n"
                "Usage:\n"
                "  ACloudViewer [files...]                  Open files in GUI\n"
                "  ACloudViewer -SILENT [commands...]       Headless CLI mode\n"
                "  ACloudViewer --version | -v              Print version\n"
                "  ACloudViewer --help   | -h               Show this help\n"
                "\n"
                "I/O:\n"
                "  -O <file>                     Open/load a file\n"
                "    -SKIP <n>                     Skip first n lines (ASCII)\n"
                "    -GLOBAL_SHIFT AUTO|FIRST|<x> <y> <z>  Global shift on load\n"
                "  -SAVE_CLOUDS [FILE <path>]    Save point clouds\n"
                "  -SAVE_MESHES [FILE <path>]    Save meshes\n"
                "  -AUTO_SAVE ON|OFF             Toggle auto-save\n"
                "  -NO_TIMESTAMP                 Disable filename timestamps\n"
                "  -LOG_FILE <path>              Write log to file\n"
                "\n"
                "Export format:\n"
                "  -C_EXPORT_FMT <fmt>           Cloud format (PLY,PCD,LAS,E57,BIN,ASC,SBF,DRC,...)\n"
                "  -M_EXPORT_FMT <fmt>           Mesh format (OBJ,STL,OFF,PLY,FBX,DXF,VTK,...)\n"
                "  -H_EXPORT_FMT <fmt>           Hierarchy format\n"
                "  -EXT <ext>                    Override output file extension\n"
                "  -PLY_EXPORT_FMT ASCII|BINARY_LE|BINARY_BE\n"
                "  -PCD_OUTPUT_FORMAT <0|1>      PCD output format (0=ASCII, 1=binary)\n"
                "  -PREC <n>                     ASCII coord precision (default: 12)\n"
                ...
                "  -NORMALS_TO_DIP               Convert normals to dip/dip-dir\n"
                "  -NORMALS_TO_SFS               Convert normals to scalar fields\n"
                "\n"
                "Scalar fields:\n"
                "  -SET_ACTIVE_SF <idx>          Set active scalar field\n"
                "  -SF_ARITHMETIC <idx> <op>     SF arithmetic (ADD,SUB,MUL,...)\n"
                ...
                "  -RGB_CONVERT_TO_SF             Convert RGB to scalar fields\n"
                ...
                "    -VERT_DIR <0|1|2>             Vertical direction (X/Y/Z)\n"
                "    -GRID_STEP <val>              Grid cell size\n"
                "    -OUTPUT_CLOUD                 Output as cloud\n"
                "    -OUTPUT_MESH                  Output as mesh\n"
                "    -OUTPUT_RASTER_Z <file>       Export raster (Z heights)\n"
                ...
                "  -OCTREE_LEVEL <n>             Octree level for comparison\n"
                ...
                "  colmap poisson_mesher            Poisson surface reconstruction\n"
                ...
                "\n"
                "System:\n"
                "  -SILENT                       Headless mode (required first arg)\n"
                ...
                "\n"
                "Examples:\n"
                "  ACloudViewer -SILENT -O in.ply -SS SPATIAL 0.05 -SAVE_CLOUDS\n"
                "  ACloudViewer -SILENT -O in.pcd -C_EXPORT_FMT PLY -SAVE_CLOUDS FILE out.ply\n"
                "  ACloudViewer -SILENT -O mesh.obj -SAMPLE_MESH POINTS 50000 -SAVE_CLOUDS\n"
                "  ACloudViewer -SILENT -O a.ply -O b.ply -ICP -ITER 50 -SAVE_CLOUDS\n"
                "  ACloudViewer -SILENT -O cloud.ply -SOR 6 1.0 -SAVE_CLOUDS\n"
                "  ACloudViewer -SILENT -O cloud.ply -CSF -SAVE_CLOUDS\n"
                "  ACloudViewer -SILENT -O cloud.ply -RANSAC -SAVE_CLOUDS\n"
                "  ACloudViewer -SILENT -HELP    (full list including plugin commands)\n"
                "\n"
...
int main(int argc, char* argv[]) {
    ...
    if (int rc = HandleQuickFlags(argc, argv); rc >= 0)
        return rc;
