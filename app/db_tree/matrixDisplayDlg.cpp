...
MatrixDisplayDlg::MatrixDisplayDlg(QWidget* parent /*=0*/)
    : QWidget(parent), Ui::MatrixDisplayDlg() {
    setupUi(this);

    connect(exportToAsciiPushButton, &QAbstractButton::clicked, this,
            &MatrixDisplayDlg::exportToASCII);
    connect(exportToClipboardPushButton, &QAbstractButton::clicked, this,
            &MatrixDisplayDlg::exportToClipboard);
...
void MatrixDisplayDlg::fillDialogWith(const ccGLMatrix& mat) {
    ...
    // display as rotation vector/angle
    {
        ...
        fillDialogWith(CCVector3d::fromArray(axis3D.u), angle_rad,
                       CCVector3d::fromArray(t3D.u), precision);
    }
}
...
void MatrixDisplayDlg::fillDialogWith(const CCVector3d& axis,
                                      ...
                                      int precision) {
    // center position
    QString centerStr = QString("%0 ; %1 ; %2")
                                .arg(T.x, 0, 'f', precision)
                                .arg(T.y, 0, 'f', precision)
                                .arg(T.z, 0, 'f', precision);
    // rotation axis
    QString axisStr = QString("%0 ; %1 ; %2")
                              .arg(axis.x, 0, 'f', precision)
                              .arg(axis.y, 0, 'f', precision)
                              .arg(axis.z, 0, 'f', precision);
    // rotation angle
    QString angleStr = QString("%1 deg.").arg(
            cloudViewer::RadiansToDegrees(angle_rad), 0, 'f', precision);
...
void MatrixDisplayDlg::exportToASCII() {
    ...
    QSettings settings;
    settings.beginGroup(
            ecvPS::LoadFile());  // use the same folder as the load one
    QString currentPath =
            settings.value(ecvPS::CurrentPath(), ecvFileUtils::defaultDocPath())
                    .toString();

    QString outputFilename = QFileDialog::getSaveFileName(
            this, "Select output file", currentPath, "*.mat.txt");
    ...
    if (m_mat.toAsciiFile(outputFilename)) {
        ...
        CVLog::Error(
                QString("Failed to save matrix as '%1'").arg(outputFilename));
    }
