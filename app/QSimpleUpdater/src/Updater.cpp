...
#include "QSimpleUpdater/include/Updater.h"
...
void Updater::onReply(QNetworkReply *reply)
{
   ...
   if (platform.contains("mandatory"))
      m_mandatoryUpdate = platform.value("mandatory").toBool();
...
void Updater::setUpdateAvailable(const bool available)
{
   ...
   if (updateAvailable() && (notifyOnUpdate() || notifyOnFinish()))
   {
      ...
      {
         text = tr("Would you like to download the update now? This is a mandatory update, "
                   "you should install this version after exiting application!");
      }
      ...
      {
         // do nothing
         CVLog::Print(tr("You can download latest application from %1 mannually").arg(downloadUrl()));
         return;
      }
   }
