...
ccColorLevelsDlg::ccColorLevelsDlg(QWidget* parent,
    ...
    connect(channelComboBox,
            ...
            this, &ccColorLevelsDlg::onChannelChanged);
    connect(buttonBox->button(QDialogButtonBox::Apply), &QPushButton::clicked,
            this, &ccColorLevelsDlg::onApply);
...
void ccColorLevelsDlg::updateHistogram() {
    if (m_histogram) {
        ...
        std::vector<unsigned>* histoValuesR =
                (histoValues[0].empty() ? 0 : histoValues);
        std::vector<unsigned>* histoValuesG =
                (histoValues[1].empty() ? 0 : histoValues + 1);
        std::vector<unsigned>* histoValuesB =
                (histoValues[2].empty() ? 0 : histoValues + 2);
        ...
        // project points
        {
            for (unsigned i = 0; i < pointCount; ++i) {
                ...
                if (histoValuesR) histoValuesR->at(rgb.r)++;
                if (histoValuesG) histoValuesG->at(rgb.g)++;
                if (histoValuesB) histoValuesB->at(rgb.b)++;
            }
        }
...
void ccColorLevelsDlg::onApply() {
    ...
    if (m_cloud &&
        (minInputSpinBox->value() != 0 || maxInputSpinBox->value() != 255 ||
         minOutputSpinBox->value() != 0 || maxOutputSpinBox->value() != 255)) {
        bool applyRGB[3] = {channelComboBox->currentIndex() == RGB ||
                                    channelComboBox->currentIndex() == RED,
                            channelComboBox->currentIndex() == RGB ||
                                    channelComboBox->currentIndex() == GREEN,
                            channelComboBox->currentIndex() == RGB ||
                                    channelComboBox->currentIndex() == BLUE};
        ...
        for (unsigned i = 0; i < pointCount; ++i) {
            ...
            for (unsigned c = 0; c < 3; ++c) {
                if (applyRGB[c]) {
                    ...
                    if (qIn) {
                        double u = (static_cast<double>(rgb.rgb[c]) -
                                    s_inputLevels[0]) /
                                   qIn;
                        newC = s_outputLevels[0] + u * pOut;
                    }
                    newRgb.rgb[c] = static_cast<ColorCompType>(std::max<double>(
                            std::min<double>(newC, ecvColor::MAX), 0.0));
                } else {
            ...
            if (pc) {
                ...
                // DGM FIXME: dirty!
                const_cast<ecvColor::Rgb&>(rgb) = newRgb;
            }
        }
