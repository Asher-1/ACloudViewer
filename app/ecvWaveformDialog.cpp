...
ccWaveWidget::~ccWaveWidget() { clearInternal(); }
...
void ccWaveWidget::setTitle(const QString& str) { m_titleStr = str; }
...
void ccWaveWidget::refresh() {
    ...
    if (m_drawVerticalIndicator)  // vertical hint
    {
        ...
        // horizontal position
        int curvePos = static_cast<int>(curveSize *
        ...
        valueStr = QString("= %0").arg(
                curvePos < curveSize ? m_curveValues[curvePos] : 0);
    ...
    if (event->buttons() & Qt::LeftButton) {
        if (m_curve && !m_curveValues.empty()) {
            ...
            if (roi.contains(event->pos(), false)) {
                ...
                m_verticalIndicatorPositionPercent =
                        static_cast<double>(event->x() - roi.x()) / roi.width();
                refresh();
            }
        }
    } else {
...
ccWaveDialog::ccWaveDialog(ccPointCloud* cloud,
    ...
    if (cloud && cloud->size()) {
        ...
        m_gui->pointIndexSpinBox->setSuffix(
                QString(" / %1").arg(cloud->size() - 1));

        // init m_waveMax
        double waveMin = 0;
        ecvProgressDialog pDlg(parent);
        if (cloud->computeFWFAmplitude(waveMin, m_waveMax, &pDlg)) {
            CVLog::Print(
                    QString("[ccWaveDialog] Cloud '%1': max FWF amplitude = %2")
                            .arg(cloud->getName())
                            .arg(m_waveMax));
        } else {
            CVLog::Warning("[ccWaveDialog] Input cloud has no valid FWF data");
        }
    }

    connect(m_gui->pointIndexSpinBox,
            static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this,
            &ccWaveDialog::onPointIndexChanged);
    connect(m_gui->logScaleCheckBox, &QCheckBox::toggled, this,
            &ccWaveDialog::updateCurrentWaveform);
    connect(m_gui->fixedAmplitudeCheckBox, &QCheckBox::toggled, this,
            &ccWaveDialog::updateCurrentWaveform);
    connect(m_gui->pointPickingToolButton, &QToolButton::toggled, this,
            &ccWaveDialog::onPointPickingButtonToggled);
    connect(m_gui->saveWaveToolButton, &QToolButton::clicked, this,
            &ccWaveDialog::onExportWaveAsCSV);
    connect(this, &QDialog::finished, [&]() {
        m_gui->pointPickingToolButton->setChecked(false);
    });  // auto disable picking mode when the dialog is closed

    // force update
    onPointIndexChanged(0);
}

ccWaveDialog::~ccWaveDialog() { delete m_gui; }
...
void ccWaveDialog::onPointIndexChanged(int index) {
    ...
    m_widget->init(
            m_cloud, static_cast<unsigned>(index),
            m_gui->logScaleCheckBox->isChecked(),
            m_gui->fixedAmplitudeCheckBox->isChecked() ? m_waveMax : 0.0);
    m_widget->refresh();
}
...
void ccWaveDialog::onPointPickingButtonToggled(bool state) {
    ...
    if (state) {
        if (!m_pickingHub->addListener(this)) {
            CVLog::Error(
                    "Another tool is currently using the point picking "
