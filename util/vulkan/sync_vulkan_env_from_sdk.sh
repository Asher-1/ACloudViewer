...
if [[ -z "${VULKAN_SDK:-}" ]]; then
    echo "ERROR: VULKAN_SDK is not set. Install the SDK first (Linux: util/vulkan/install_vulkan_linux.sh, Windows: util/vulkan/install_vulkan_sdk_windows.ps1)." >&2
    exit 1
fi
...
VULKAN_SDK="$(resolve_existing_vulkan_sdk "${VULKAN_SDK}")" || {
    ...
    echo "Windows CI: use util/vulkan/install_vulkan_sdk_windows.ps1 (SDK >= 1.4.313 needs silent install, not 7z)." >&2
    exit 1
}
...
{
    printf '# Generated by util/vulkan/sync_vulkan_env_from_sdk.sh - do not edit by hand.\n'
