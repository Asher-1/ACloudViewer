...
install_glslc_from_source() {
    ...
    # v2026.3 is an annotated tag; the release commit is 2c8cae78..., not the tag object SHA.
    local shaderc_commit="${SHADERC_COMMIT:-2c8cae778eec0283b44acbe7ed1a386865d78799}"
    ...
    if [[ ! -d "${source_dir}/.git" ]]; then
        ...
        git -C "${source_dir}" fetch --depth 1 origin tag "${shaderc_tag}"
        git -C "${source_dir}" checkout "tags/${shaderc_tag}"
    fi
    local actual_commit
...
write_env_script() {
    ...
    {
        printf '# Generated by util/vulkan/install_vulkan_linux.sh - do not edit by hand.\n'
...
verify_environment() {
    ...
    if needs_bundled_vulkan_headers; then
        if [[ -z "${VULKAN_SDK:-}" || ! -f "${VULKAN_SDK}/include/vulkan/vulkan_core.h" ]]; then
            ...
            echo "Re-run: util/vulkan/install_vulkan_env.sh" >&2
            exit 1
        fi
    elif [[ -n "${VULKAN_SDK:-}" && ! -f "${VULKAN_SDK}/include/vulkan/vulkan_core.h" ]]; then
        echo "WARNING: VULKAN_SDK=${VULKAN_SDK} does not exist; unset it and re-run util/vulkan/install_vulkan_env.sh." >&2
    fi
