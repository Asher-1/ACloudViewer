...
run-ci)
    gcloud compute ssh "${GCE_INSTANCE}" --zone "${GCE_INSTANCE_ZONE[$GCE_ZID]}" --command \
        "sudo docker run --detach --interactive --name cloudViewer_gpu_ci --gpus all \
            --env NPROC=$NPROC \
            ...
            --env CLOUDVIEWER_ML_ROOT=/root/ErowCloudViewer/CloudViewer-ML \
            $DC_IMAGE_TAG; \
            sudo docker exec --interactive  cloudViewer_gpu_ci util/run_ci.sh"
    ;;
