...
if [[ $BUILD_TENSORFLOW_OPS == ON || $BUILD_PYTORCH_OPS == ON || \
    ...
    # Without MLOps, ensure CloudViewer works with the lowest supported CUDA version
...
test_wheel() {
    ...
    # fi
    echo
    # FIXME: Needed because CloudViewer-ML master TF and PyTorch is older than dev.
    if [ $BUILD_CUDA_MODULE == ON ]; then
...
test_cpp_example() {
    # Now I am in CloudViewer/build/
    cd ..
    git clone https://github.com/intel-isl/cloudViewer-cmake-find-package.git
    cd cloudViewer-cmake-find-package
    mkdir build
    cd build
    ...
    # Now I am in CloudViewer/cloudViewer-cmake-find-package/build/
    cd ../../build
}
...
install_docs_dependencies() {
    ...
    if [[ -d "$1" ]]; then
        OPEN3D_ML_ROOT="$1"
        echo Installing CloudViewer-ML dependencies from "${OPEN3D_ML_ROOT}"
        python -m pip install -r "${OPEN3D_ML_ROOT}/requirements.txt"
        python -m pip install -r "${OPEN3D_ML_ROOT}/requirements-torch.txt"
        python -m pip install -r "${OPEN3D_ML_ROOT}/requirements-tensorflow.txt" ||
            python -m pip install tensorflow # FIXME: Remove after CloudViewer-ML update
    else
...
build_docs() {
    ...
    if [[ "$DEVELOPER_BUILD" != "OFF" ]]; then # Validate input coming from GHA input field
        ...
        DOC_ARGS="--is_release"
        echo "Building docs for a new CloudViewer release"
        echo
        echo "Building CloudViewer with ENABLE_HEADLESS_RENDERING=ON for Jupyter notebooks"
        echo
    fi
    ...
    make -j$NPROC
    bin/GLInfo
    python -c "from cloudViewer import *; import cloudViewer; print(cloudViewer)"
    cd ../docs # To CloudViewer/docs
    python make_docs.py $DOC_ARGS --clean_notebooks --execute_notebooks=always --pyapi_rst=never
    python -m pip uninstall --yes cloudViewer
    cd ../build
    set +x # Echo commands off
    echo
    echo "Building CloudViewer with BUILD_GUI=ON for visualization.{gui,rendering} documentation"
    echo
    set -x # Echo commands on
    cmake "${cmakeOptions[@]}" \
