...
- If you prefer using native windows (with blocking calls) instead of embedded
  ...
  ``CLOUDVIEWER_DISABLE_WEB_VISUALIZER=true`` before importing CloudViewer.
...
.. code-block:: cpp
    ...
    cloudViewer::visualization::webrtc_server::WebRTCWindowSystem::GetInstance()->EnableWebRTC();
...
Checkout `examples/Cpp/DrawWebRTC.cpp <https://github.com/Asher-1/ACloudViewer/blob/main/examples/Cpp/DrawWebRTC.cpp>`_
...
Checkout `examples/Python/visualization/draw_webrtc.py <https://github.com/Asher-1/ACloudViewer/blob/main/examples/Python/visualization/draw_webrtc.py>`_
for the complete source code.
...
`docs/jupyter/visualization/jupyter_visualization.ipynb <https://github.com/Asher-1/ACloudViewer/blob/main/docs/jupyter/visualization/jupyter_visualization.ipynb>`_.
...
visualization. For instance, ``cloudViewer.web_visualizer.draw`` is used instead of
the regular ``cloudViewer.visualization.draw`` in the example notebook above.
...
``python/cloudViewer/visualization/async_event_loop.py`` for the  ``_AsyncEventLoop`` class and
the implementation of ``cloudViewer.web_visualizer.draw`` as an example.
