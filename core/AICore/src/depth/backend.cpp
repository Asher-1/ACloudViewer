...
struct Backend::Impl {
    ...
    // Extra graph roots to expand (registered via add_graph_root()) but NOT
    // read back. Used for resident K/V write (ggml_cpy) nodes.
...
Backend::Backend() : impl_(new Impl()) {
    ...
    int want_idx = 0;
    ggml_common::parse_device(want, parsed_name, want_idx);
...
void Backend::drop_transient_allocators() {
    if (!impl_) return;
    if (impl_->sched) {
        ggml_backend_sched_free(impl_->sched);
        impl_->sched = nullptr;
    }
    if (impl_->galloc) {
        ggml_gallocr_free(impl_->galloc);
        impl_->galloc = nullptr;
    }
}
...
ggml_tensor* Backend::add_graph_input_nd(ggml_context* ctx,
                                         GraphInputPool& pool,
                                         const float* host,
                                         const int64_t* ne,
                                         int n_dims) {
    ...
    impl_->pending.push_back(
            {t, pool.keep(host, total), total * sizeof(float)});
    return t;
}
...
ggml_tensor* Backend::add_graph_input_nd_borrow(ggml_context* ctx,
    ...
    ggml_set_input(t);
    impl_->pending.push_back(
            {t, host, total * sizeof(float)});  // borrow: no copy
    return t;
}

ggml_tensor* Backend::add_int32_input_nd(ggml_context* ctx,
                                         GraphInputPool& pool,
                                         const int32_t* host,
                                         const int64_t* ne,
                                         int n_dims) {
    ...
    ggml_set_input(t);
    // The upload is a raw, type-agnostic ggml_backend_tensor_set; reuse the
    // same pending-upload list (host pointer reinterpreted as bytes).
...
void Backend::add_graph_root(ggml_tensor* t) { impl_->roots.push_back(t); }
...
bool Backend::compute(const std::function<ggml_tensor*(ggml_context*)>& build,
    ...
    if (!ctx) {
        DA_ERR("Backend::compute: ggml_init failed");
        return false;
    }

    // Reset registrations for this compute, then run the build lambda. The
    // build lambda registers host inputs / captures via member calls on this
    // Backend.
    impl_->pending.clear();
    ...
    if (!output) {
        DA_ERR("Backend::compute: build() returned null output tensor");
        impl_->pending.clear();
        impl_->captures.clear();
        impl_->roots.clear();
        ggml_free(ctx);
    ...
    // Mark the output (and any captured tensors) so the gallocr does not
    // recycle their storage before we read them back, then expand the forward
    // graph.
    ...
    // Expand registered extra graph roots (e.g. resident K/V ggml_cpy writes)
    // in registration order, BEFORE the output, so their side-effect nodes land
    // in the graph. These are not read back.
    for (ggml_tensor* r : impl_->roots) ggml_build_forward_expand(gf, r);
    ...
    // far less than a sched re-plan. The CPU path never sets use_sched, so it
    ...
    if (need_sched) {
        ...
        if (!alloc_ok)
            DA_ERR("Backend::compute: ggml_backend_sched_alloc_graph failed");
    } else {
        // Fast path: CPU, or a GPU whose backend supports every op in this
        // graph. Persistent gallocr over the active backend's buffer type,
        // lazily created and reused on every subsequent call (it only
        // reallocates the underlying buffer when the graph grows beyond the
        // current high-water mark).
        if (!impl_->galloc) {
            impl_->galloc = ggml_gallocr_new(
                    ggml_backend_get_default_buffer_type(impl_->backend));
            if (!impl_->galloc) {
                DA_ERR("Backend::compute: ggml_gallocr_new failed");
                impl_->pending.clear();
                impl_->captures.clear();
                impl_->roots.clear();
                ggml_free(ctx);
        ...
        if (!alloc_ok)
            DA_ERR("Backend::compute: ggml_gallocr_alloc_graph failed");
    }
    if (!alloc_ok) {
