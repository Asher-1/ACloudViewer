...
public:
    ...
    // Like add_graph_input_nd but BORROWS `host` (no copy into the pool). `host`
    // MUST outlive the compute() call (e.g. a persistent cached buffer). Used for
    // the cached UV positional embedding to skip its per-forward memcpy.
    ggml_tensor* add_graph_input_nd_borrow(ggml_context* ctx,
                                    const float* host, const int64_t* ne, int n_dims);
    ...
    // Drop persistent graph-activation buffers (gallocr / scheduler). Model weights
    // uploaded via ModelLoader::offload_weights stay resident. Safe to call
    // between large graph phases (e.g. anyview multiview -> metric branch).
    void release_graph_memory();
    void drop_transient_allocators();
