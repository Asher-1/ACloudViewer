...
//   so they use torch's DEFAULT nn.LayerNorm eps = 1e-5 -- NOT the block's
//   1e-6. Therefore the qk_norm LayerNorm inside attention() must use 1e-5.
...
BlockWeights load_block(const ModelLoader& ml, int i) {
    ...
    BlockWeights w;
    w.n1_w = t("norm1.weight");
    w.n1_b = t("norm1.bias");
    w.n2_w = t("norm2.weight");
    w.n2_b = t("norm2.bias");
    w.ls1 = t("ls1");
    w.ls2 = t("ls2");
    w.swiglu = (ml.config().ffn_type == "swiglu");
    if (w.swiglu) {
        w.w12_w = t("mlp_w12.weight");
        w.w12_b = t("mlp_w12.bias");
        w.w3_w = t("mlp_w3.weight");
        w.w3_b = t("mlp_w3.bias");
    } else {
        w.fc1_w = t("mlp_fc1.weight");
        w.fc1_b = t("mlp_fc1.bias");
        w.fc2_w = t("mlp_fc2.weight");
        w.fc2_b = t("mlp_fc2.bias");
    }
    w.attn = load_attn(ml, i);
    return w;
}
ggml_tensor* vit_block(ggml_context* ctx,
                       ggml_tensor* x,
                       ...
                       ggml_tensor* cosb,
                       ggml_tensor* sinb) {
    // qk_norm uses torch-default eps (1e-5), distinct from the block norm eps
    // (1e-6).
    ggml_tensor* a = attention(ctx, layernorm(ctx, x, w.n1_w, w.n1_b, eps),
                               w.attn, H, D, QK_NORM_EPS, cosb, sinb);
    ...
    if (w.swiglu) {
        ...
        // hidden = silu(x1)*x2; m = w3(hidden). Chunk is along the channel dim
        // (ne0): torch x12.chunk(2,-1) -> x1 = first `hidden`, x2 = next
        // `hidden`.
        ...
        ggml_tensor* x1 = ggml_cont(
                ctx, ggml_view_2d(ctx, x12, hidden, N, x12->nb[1], 0));
        ggml_tensor* x2 =
                ggml_cont(ctx, ggml_view_2d(ctx, x12, hidden, N, x12->nb[1],
