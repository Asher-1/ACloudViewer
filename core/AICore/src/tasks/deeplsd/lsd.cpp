...
static void enlarge_ntuple_list(ntuple_list n_tuple) {
    /* check parameters */
    if (n_tuple == nullptr || n_tuple->values == nullptr ||
        n_tuple->max_size == 0)
...
static image_double gaussian_sampler(image_double in,
    ...
    /* check parameters */
    if (in == nullptr || in->data == nullptr || in->xsize == 0 ||
        in->ysize == 0)
    ...
    if (in->xsize * scale > (double)UINT_MAX ||
        ...
        error("gaussian_sampler: the output image size exceeds the handled "
              "size.");
 ...
 * @param threshold The gradient threshold used to avoid some values in the
 * gradient orientation map
 * @param g The gradient orientation map
 * @param modgrad the module of the gradient
 */
static void grad_angle_orientation(image_double in,
                                   double threshold,
                                   image_double &g,
...
static int isaligned(
        int x, int y, image_double angles, double theta, double prec) {
    double a;
...
static double nfa(int n, int k, double p, double logNT) {
    ...
    if (double_equal(term, 0.0)) /* the first term is almost zero */
    {
        if ((double)k > (double)n * p) /* at begin or end of the tail?  */
            return -log1term / M_LN10 -
                   logNT; /* end: use just the first term  */
        else
...
static double get_theta(struct point *reg,
    ...
    /* compute angle */
    theta = fabs(Ixx) > fabs(Iyy) ? atan2(lambda - Ixx, Ixy)
                                  : atan2(Ixy, lambda - Iyy);
    ...
    while (density < density_th) {
        ...
        for (i = 0; i < *reg_size; i++)
            if (dist(xc, yc, (double)reg[i].x, (double)reg[i].y) > rad) {
                ...
                reg[i].x = reg[*reg_size - 1]
                                   .x; /* if i==*reg_size-1 copy itself */
...
static int refine(struct point *reg,
    ...
    region_grow(reg[0].x, reg[0].y, angles, reg, reg_size, &reg_angle, used,
                tau);
...
double *LineSegmentDetection(int *n_out,
    ...
    if (scale != 1.0) {
        if (grad_nfa)
            ll_angle(scaled_image, rho, &list_pp, &mem_pp, img_gradnorm,
                     img_grad_angle, (unsigned int)n_bins);
        ll_angle(scaled_image, rho, &list_p, &mem_p, modgrad, angles,
                 (unsigned int)n_bins);

    } else {
        if (grad_nfa)
            ll_angle(image, rho, &list_pp, &mem_pp, img_gradnorm,
                     img_grad_angle, (unsigned int)n_bins);
        ll_angle(image, rho, &list_p, &mem_p, modgrad, angles,
                 (unsigned int)n_bins);
    }
    ...
    */
    logNT = 5.0 * (log10((double)xsize) + log10((double)ysize)) / 2.0 +
            log10(11.0);
    min_reg_size =
            (int)(-logNT / log10(p)); /* minimal number of points in region
                                       that can give a meaningful event */

    /* initialize some structures */
    if (reg_img != nullptr && reg_x != nullptr &&
        reg_y != nullptr) /* save region data */
    ...
    for (; list_p != nullptr; list_p = list_p->next)
           ...
           because we are only interested in the exact NOTDEF value */
        {
            ...
            /* Check if the rectangle exceeds the minimal density of
               ...
               Control" by R. Grompone von Gioi, J. Jakubowicz, J.M. Morel, and
               G. Randall. The original algorithm is obtained with density_th =
               0.0.
             */
            ...
            /*
               The gradient was computed with a 2x2 mask, its value corresponds
               to points with an offset of (0.5,0.5), that should be added to
               output. The coordinates origin is at the center of pixel (0,0).
             */
            // rec.x1 += 0.5;
            // rec.y1 += 0.5;
            // rec.x2 += 0.5;
            ...
            if (region != nullptr)
                for (i = 0; i < reg_size; i++)
                    region->data[reg[i].x + reg[i].y * region->xsize] =
                            ls_count;
        }
...
double *lsd_scale_region(int *n_out,
    ...
    // */
    double density_th =
            0.7;       /* Minimal density of region points in rectangle. */
...
double *lsd_scale(int *n_out,
                  ...
                  double scale,
                  double log_eps = 0.0) {
    return lsd_scale_region(n_out, img, X, Y, scale, nullptr, nullptr, nullptr,
                            log_eps);
}
