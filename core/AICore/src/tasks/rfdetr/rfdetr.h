...
typedef enum {
    ...
    RFDETR_ERR_MODEL_LOAD =
            -7, /* tensor binding or inventory (e.g. missing tensors) */
...
typedef void (*rfdetr_log_cb)(rfdetr_log_level lvl,
                              const char* msg,
                              void* user_data);
 ...
 * (thresholded). mask_width/mask_height are the MODEL resolution (e.g.
 * 640x640) — the seg-head output is thresholded in place and NOT upsampled
 * to the image size. Display code (rfdetr_visualize_overlay_mask,
 * rfdetr_render, the qRFDetr plugin) must stretch the mask over the frame;
 * boxes (x1..y2) are already scaled to the original image.
 *
...
typedef struct {
     ...
     *   0 or negative → auto (physical-core estimate = hardware_concurrency()/2
     * — logical SMT cores oversubscribe ggml's matmuls and collapse
     * throughput; see resolve_n_threads in rfdetr.cpp).
     *   >0            → use that many threads. */
    int n_threads;
    /* AICore device request: "auto" (default), "cpu", "cuda", "vulkan",
     * "metal", "cuda:0", ... NULL/empty means "auto". */
    const char* device;
...
/* Lifecycle (not yet implemented; returns RFDETR_ERR_NOT_IMPLEMENTED in this
 * plan) */
rfdetr_context* rfdetr_init(const rfdetr_params* params,
...
/* Effective thread count after the auto (n_threads <= 0) resolution. */
int rfdetr_context_n_threads(const rfdetr_context* ctx);
/* Backend-RESOLVED device name ("CUDA0", "Vulkan0", "cpu", ...). Differs
 * from the request when the GPU lease can't be acquired — surfaces silent
 * CPU fallbacks. Empty string for a null context. */
...
rfdetr_image* rfdetr_image_load_file(const char* path,
                                     rfdetr_status* out_status);
rfdetr_image* rfdetr_image_load_buffer(const uint8_t* bytes,
                                       size_t len,
                                       rfdetr_status* out_status);
 ...
 * (R, G, B, R, G, B, ...), such as the output of
 ...
 * tinted mask region.
 *
 *   - The rectangle is drawn in a deterministic per-class color (palette of
 * 20).
...
void rfdetr_visualize_draw_box(rfdetr_image* img,
...
void rfdetr_visualize_overlay_mask(rfdetr_image* img,
                                   rfdetr_detection det,
                                   float alpha);
...
/* Render annotated image (IMPLEMENTED in this plan, boxes only — labels in Plan
 * 2) */
