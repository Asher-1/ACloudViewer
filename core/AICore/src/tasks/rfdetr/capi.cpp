...
struct aicore_rfdetr_ctx {
    ...
    // Per-detection store from the most recent detect call (masks are PNG
    // encoded at detect time so the caller-owned rfdetr_detection array can
    ...
    struct DetectionStore {
        ...
        std::vector<uint8_t> mask_png;  // empty for detection-only models
    };
...
char* run_detect(aicore_rfdetr_ctx* ctx,
    ...
    *out_rc = -1;
    ...
    size_t n = 0;
    const rfdetr_status st = rfdetr_detect(ctx->engine, img, &dp, &dets, &n);
    if (st != RFDETR_OK) {
    ...
    if (dets != nullptr && n > 0) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            s.x2 = d.x2;
            s.y2 = d.y2;
            if (d.mask != nullptr && d.mask_width > 0 && d.mask_height > 0) {
                rfdetr_encode_gray_png(d.mask, d.mask_width, d.mask_height,
                                       s.mask_png);
            }
...
AICORE_CAPI char* aicore_rfdetr_detect_rgb_json(aicore_rfdetr_ctx* ctx,
    ...
    rfdetr_status st = RFDETR_OK;
    rfdetr_image* img = rfdetr_image_from_rgb_buffer(rgb, width, height, &st);
    if (img == nullptr) {
...
AICORE_CAPI int aicore_rfdetr_detection_mask_png(aicore_rfdetr_ctx* ctx,
    ...
    const auto& mp = ctx->detections[static_cast<size_t>(index)].mask_png;
    if (mp.empty()) return 0;  // detection-only model / no mask
    const int needed = static_cast<int>(mp.size());
    if (buf == nullptr || buf_size < needed) return needed;
    std::memcpy(buf, mp.data(), mp.size());
    return needed;
}
...
AICORE_CAPI char* aicore_rfdetr_info_json(aicore_rfdetr_ctx* ctx) {
    ...
    std::ostringstream o;
    o << "{\"model\":\""
      << json_escape(rfdetr_context_variant(ctx->engine)) << "\","
