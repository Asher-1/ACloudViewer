...
ggml_tensor* convx(ggml_context* ctx,
                   ggml_tensor* x,
                   ggml_tensor* kernel,
                   ggml_tensor* norm_w,
                   ggml_tensor* norm_b) {
...
ggml_tensor* bottleneck(ggml_context* ctx,
                        ggml_tensor* x,
                        ggml_tensor* cv1_k,
                        ggml_tensor* cv1_nw,
                        ggml_tensor* cv1_nb,
...
ggml_tensor* fetch(const Model& m, const std::string& name) {
    ...
    if (it == m.tensors.end()) {
        rfdetr_logf(RFDETR_LOG_ERROR, "projector: missing tensor '%s'",
                    name.c_str());
        return nullptr;
    }
    return it->second;
...
ggml_tensor* projector_forward(ggml_context* ctx,
                               const Model& m,
    ...
    if (n_levels <= 0 || n_levels > (int)bb.multi_scale.size()) {
        rfdetr_logf(RFDETR_LOG_ERROR, "projector: invalid n_levels %d",
                    n_levels);
        return nullptr;
    }
    for (int j = 0; j < n_levels; ++j) {
        if (!bb.multi_scale[j]) {
            rfdetr_logf(RFDETR_LOG_ERROR, "projector: missing multi_scale[%d]",
                        j);
            return nullptr;
        }
    }
    ...
    ggml_tensor* y0 = ggml_cont(
            ctx, ggml_view_4d(ctx, h, W, H, c, 1, h->nb[1], h->nb[2], h->nb[3],
                              /*offset bytes*/ 0));
    ggml_tensor* y1 = ggml_cont(
            ctx, ggml_view_4d(ctx, h, W, H, c, 1, h->nb[1], h->nb[2], h->nb[3],
    ...
    for (uint32_t j = 0; j < pcfg.n_bottlenecks; ++j) {
        ...
        if (!b_cv1_k || !b_cv1_nw || !b_cv1_nb || !b_cv2_k || !b_cv2_nw ||
            !b_cv2_nb) {
            return nullptr;
        }
        cur = bottleneck(ctx, cur, b_cv1_k, b_cv1_nw, b_cv1_nb, b_cv2_k,
                         b_cv2_nw, b_cv2_nb);
