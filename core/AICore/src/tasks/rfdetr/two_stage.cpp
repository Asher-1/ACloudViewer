...
ggml_tensor* fetch(const Model& m, const std::string& name) {
    ...
    if (it == m.tensors.end()) {
        rfdetr_logf(RFDETR_LOG_ERROR, "two_stage: missing tensor '%s'",
                    name.c_str());
        return nullptr;
    }
    return it->second;
...
void compute_proposal_grid(int width, int height, float wh_value, float* out) {
     ...
     * Layout: (4, W*H) F32, with token order (h, w) row-major (h outer, w
     * inner), matching torch's reshape(-1, H*W, 4) on a (H, W, 4) source.
     *
     ...
     * where it masks exactly the one-cell border ring (4*S - 4 tokens).
     * Every smaller grid (all detection variants, seg-nano..seg-large) is
     * bit-unchanged. */
...
TwoStageOutput two_stage_forward(ggml_context* ctx,
                                 const Model& m,
    ...
    if (B != 1) {
        rfdetr_logf(RFDETR_LOG_ERROR, "two_stage: batch != 1 (got %lld)",
                    (long long)B);
        return out;
    }
    if (C != (int64_t)m.config.decoder.model_dim) {
        rfdetr_logf(RFDETR_LOG_ERROR,
                    "two_stage: C %lld != decoder.model_dim %u", (long long)C,
                    m.config.decoder.model_dim);
        return out;
    }
     ...
     * permuted (W, H) pair. */
    ggml_tensor* tokens =
            ggml_cont(ctx, ggml_permute(ctx, projector_out, 1, 2, 0, 3));
    ...
    if (!m.proposals_grid) {
        rfdetr_logf(RFDETR_LOG_ERROR,
                    "two_stage: missing proposals_grid (run "
                    "model_realize_weights first)");
        return out;
    }
    if (m.proposals_grid->ne[0] != 4 || m.proposals_grid->ne[1] != N) {
        rfdetr_logf(RFDETR_LOG_ERROR,
                    ...
                    (long long)m.proposals_grid->ne[1], (long long)N);
        return out;
    }
    ...
    ggml_tensor* valid = ggml_cont(
            ctx, ggml_view_2d(ctx, m.proposals_grid, /*ne0*/ 1, /*ne1*/ N,
                              /*nb1*/ m.proposals_grid->nb[1],
                              /*offset*/ 2 * esz_prop));
    ...
    ggml_tensor* W_b0 =
            fetch(m, "two_stage.enc_out_bbox_embed.0.layers.0.weight");
    ggml_tensor* b_b0 =
            fetch(m, "two_stage.enc_out_bbox_embed.0.layers.0.bias");
    ggml_tensor* W_b1 =
            fetch(m, "two_stage.enc_out_bbox_embed.0.layers.1.weight");
    ggml_tensor* b_b1 =
            fetch(m, "two_stage.enc_out_bbox_embed.0.layers.1.bias");
    ggml_tensor* W_b2 =
            fetch(m, "two_stage.enc_out_bbox_embed.0.layers.2.weight");
    ggml_tensor* b_b2 =
            fetch(m, "two_stage.enc_out_bbox_embed.0.layers.2.bias");
    ...
    ggml_tensor* delta_xy = ggml_cont(
            ctx, ggml_view_2d(ctx, bbox_delta, /*ne0*/ 2, /*ne1*/ N,
                              /*nb1*/ bbox_delta->nb[1], /*offset*/ 0 * esz_d));
    ggml_tensor* delta_wh = ggml_cont(
            ctx, ggml_view_2d(ctx, bbox_delta, /*ne0*/ 2, /*ne1*/ N,
                              /*nb1*/ bbox_delta->nb[1], /*offset*/ 2 * esz_d));
    ggml_tensor* prop_xy = ggml_cont(
            ctx, ggml_view_2d(ctx, m.proposals_grid, /*ne0*/ 2, /*ne1*/ N,
                              /*nb1*/ m.proposals_grid->nb[1],
                              /*offset*/ 0 * esz_p));
    ggml_tensor* prop_wh = ggml_cont(
            ctx, ggml_view_2d(ctx, m.proposals_grid, /*ne0*/ 2, /*ne1*/ N,
                              /*nb1*/ m.proposals_grid->nb[1],
                              /*offset*/ 2 * esz_p));
    ...
    ggml_tensor* cxcy =
            ggml_add(ctx, ggml_mul(ctx, delta_xy, prop_wh), prop_xy);
