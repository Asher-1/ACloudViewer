...
// Rebuilds the LastResult store from the typed engine output.
void store_result(aicore_gkd_ctx* ctx,
                  const gkd::DetectOutput& out,
                  const std::vector<std::string>& prompts,
                  const float* bbox_or_null,
                  int32_t image_w,
                  int32_t image_h) {
    LastResult& last = ctx->last;
    last.valid = true;
...
int detect_impl(aicore_gkd_ctx* ctx,
    ...
    if (!ctx->session->detect(query, req->bbox_xyxy,
                              req->bbox_xyxy != nullptr ? 1 : 0, extra, outputs,
                              &stage)) {
    ...
    store_result(ctx, outputs[0], prompts, req->bbox_xyxy, width, height);
