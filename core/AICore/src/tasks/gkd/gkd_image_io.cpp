...
// Fixed-point precision from Resample.c. Kept at namespace scope: the
// no-capture clip8 lambda below reads it, and MSVC rejects the implicit
// constexpr capture (C3493) that GCC/Clang accept.
static constexpr int kPrecisionBits = 22;  // 32 - 8 - 2 in Resample.c
...
RgbImage resize_bilinear_pil(const RgbImage& src, int dst_w, int dst_h) {
    ...
    auto clip8 = [](int32_t v) {
        ...
        return (uint8_t)std::min(255, std::max(0, (int)(v >> kPrecisionBits)));
    };
        ...
        for (int xx = 0; xx < dst_len; xx++) {
            ...
            for (int x = 0; x < n; x++) {
                ...
                ax.k[(size_t)xx * ax.ksize + x] =
                        (int32_t)(0.5 + w[x] * (float)(1 << kPrecisionBits));
            }
        }
        return ax;
    };
