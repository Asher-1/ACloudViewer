...
// Blocked-layout (nChw16c) AVX-512 register-tiled DIRECT convolution
// microkernel for the ArcFace recognizer's deep-channel/small-spatial 3x3
// stride-1 pad-1 body (the regime where it beats Winograd at high thread
// counts; SCRFD's detector convs stay on Winograd by default -- see fd::conv2d
// for the routing gate). NO im2col, NO Winograd transform: it streams the NCHW
// activation (broadcast over the 16-OC weight vector) and FMA-accumulates a
// register tile of output-width pixels x 16 output channels directly over the
// IC*KH*KW window. The 16 OC sit in zmm lanes; one aligned zmm weight load is
// reused across the whole output-width strip.
//
// Ship-safety mirrors winograd.cpp EXACTLY: the zmm code carries
// __attribute__((target("avx512f,avx512bw,avx512vl"))) (function
// multiversioning) so it is emitted from an AVX2-only TU (default portable
// build, GGML_NATIVE=OFF, -mavx2 -mfma, NO global -mavx512f); it runs ONLY when
// __builtin_cpu_supports reports AVX-512 at run time, else the AVX2 (ymm, 8c)
// fallback runs -- so an AVX2-only CPU never executes a zmm instruction. A
// global -mavx512f build (GGML_NATIVE=ON) defines __AVX512F__ and the attribute
// is dropped.
...
// ------------------------------------------------------------------------
// Persistent per-op state. Caches the blocked-packed weights (computed once
// from w->data). LANE = 16 (AVX-512) or 8 (AVX2/scalar). Weights packed
// [OCB][KH][KW][IC][LANE] so the kh,kw,ic inner loop walks them contiguously
// and each (ocb,kh,kw,ic) yields one aligned LANE-wide load of LANE output
// channels.
// ------------------------------------------------------------------------
struct DConvState {
            ...
            for (int kw = 0; kw < 3; ++kw)
                for (int ic = 0; ic < IC; ++ic) {
                    float* dst = st->Wpack.data() +
                                 ((((size_t)ocb * 9 + kh * 3 + kw) * IC) + ic) *
                                         lane;
                    for (int l = 0; l < lane; ++l) {
                        const int oc = ocb * lane + l;
                        dst[l] = (oc < OC) ? w[(((size_t)oc * IC + ic) * 3 +
                                                kh) * 3 +
                                               kw]
                                           : 0.0f;
                    }
...
template <int CW>
static inline FD_DCONV_AVX512_TARGET void dconv_strip_avx512(const float* x,
                                                             const float* wocb,
                                                             float* outrow,
                                                             int W,
                                                             int H,
    ...
    for (int kh = 0; kh < 3; ++kh) {
        ...
        for (int kw = 0; kw < 3; ++kw) {
            ...
            for (int ic = 0; ic < IC; ++ic) {
                ...
                for (int j = 0; j < CW; ++j)
                    acc[j] =
                            _mm512_fmadd_ps(_mm512_set1_ps(inc[j]), wv, acc[j]);
            }
        }
    }
    for (int j = 0; j < CW; ++j)
...
static inline FD_DCONV_AVX512_TARGET void dconv_strip2_avx512(
    ...
    __m512 a0[CW], a1[CW];
    for (int j = 0; j < CW; ++j) {
        a0[j] = _mm512_setzero_ps();
        a1[j] = _mm512_setzero_ps();
    }
    for (int kh = 0; kh < 3; ++kh) {
        const int iy = ih + kh - pad;
        if (iy < 0 || iy >= H) continue;
...
static inline FD_DCONV_AVX512_TARGET void dconv_edge_avx512(const float* x,
                                                            const float* wocb,
                                                            float* outrow,
                                                            int W,
                                                            int H,
        ...
        for (int kw = 0; kw < 3; ++kw) {
            ...
            for (int ic = 0; ic < IC; ++ic)
                acc = _mm512_fmadd_ps(_mm512_set1_ps(in[(size_t)ic * HW]),
                                      _mm512_loadu_ps(wk + (size_t)ic * 16),
                                      acc);
        }
    }
...
// 2-block strips.
static inline FD_DCONV_AVX512_TARGET void dconv_row2_avx512(const float* x,
                                                            const float* Wpack,
                                                            float* outrow0,
                                                            float* outrow1,
                                                            int W,
    ...
    for (; ow0 + OWB2 - 1 <= hi; ow0 += OWB2)
        dconv_strip2_avx512<OWB2>(x, w0, w1, outrow0, outrow1, W, H, IC, ih,
                                  pad, ow0);
    int rem = hi - ow0 + 1;
    while (rem >= 8) {
        dconv_strip2_avx512<8>(x, w0, w1, outrow0, outrow1, W, H, IC, ih, pad,
                               ow0);
        ow0 += 8;
        rem -= 8;
    }
    while (rem >= 4) {
        dconv_strip2_avx512<4>(x, w0, w1, outrow0, outrow1, W, H, IC, ih, pad,
                               ow0);
        ow0 += 4;
        rem -= 4;
    }
    while (rem >= 2) {
        dconv_strip2_avx512<2>(x, w0, w1, outrow0, outrow1, W, H, IC, ih, pad,
                               ow0);
        ow0 += 2;
        rem -= 2;
    }
    while (rem >= 1) {
        dconv_strip2_avx512<1>(x, w0, w1, outrow0, outrow1, W, H, IC, ih, pad,
                               ow0);
        ow0 += 1;
        rem -= 1;
    }
...
static inline FD_DCONV_AVX512_TARGET void dconv_row_avx512(const float* x,
                                                           const float* Wpack,
                                                           float* outrow,
                                                           int W,
                                                           int H,
    ...
    // Edge columns (ow in [0,pad) and [W-pad,W)): full bounds checks. Two
    // ranges walked as one loop so the zmm intrinsics stay in this
    // target-tagged body (a lambda would not inherit the
    ...
    // Interior columns ow in [pad, W-1-pad]: ix = ow + kw - pad in [0, W-1]
    // always (holds for pad >= 1, which every SCRFD/ArcFace 3x3 conv uses).
    // Bulk strips run the fully-unrolled fixed-OWB kernel; the last partial
    // strip is split into smaller fixed-width kernels (8/4/2/1) so every column
    // stays branchless.
    ...
    int rem = hi - ow0 + 1;
    while (rem >= 8) {
        dconv_strip_avx512<8>(x, wocb, outrow, W, H, IC, ih, pad, ow0);
        ow0 += 8;
        rem -= 8;
    }
    while (rem >= 4) {
        dconv_strip_avx512<4>(x, wocb, outrow, W, H, IC, ih, pad, ow0);
        ow0 += 4;
        rem -= 4;
    }
    while (rem >= 2) {
        dconv_strip_avx512<2>(x, wocb, outrow, W, H, IC, ih, pad, ow0);
        ow0 += 2;
        rem -= 2;
    }
    while (rem >= 1) {
        dconv_strip_avx512<1>(x, wocb, outrow, W, H, IC, ih, pad, ow0);
        ow0 += 1;
        rem -= 1;
    }
}
...
// ========================================================================
#if defined(__AVX2__)
static inline void dconv_row_avx2(const float* x,
                                  const float* Wpack,
                                  float* outrow,
                                  int W,
                                  int H,
            ...
            for (int kw = 0; kw < 3; ++kw) {
                ...
                for (int ic = 0; ic < IC; ++ic)
                    acc = _mm256_fmadd_ps(_mm256_set1_ps(in[(size_t)ic * HW]),
                                          _mm256_loadu_ps(wk + (size_t)ic * 8),
                                          acc);
            }
        }
        ...
        for (int kh = 0; kh < 3; ++kh) {
            ...
            for (int kw = 0; kw < 3; ++kw) {
                ...
                for (int ic = 0; ic < IC; ++ic) {
                    ...
                    const float* inc = in + (size_t)ic * HW;
                    for (int j = 0; j < owc; ++j)
                        acc[j] = _mm256_fmadd_ps(_mm256_set1_ps(inc[j]), wv,
                                                 acc[j]);
                }
            }
        }
...
static void dconv_row_scalar(const float* x,
                             const float* Wpack,
                             float* outrow,
                             int W,
                             int H,
...
static inline void scatter_row(float* y,
                               const float* outrow,
                               int W,
                               size_t HW,
                               int lane,
...
static void dconv_compute(ggml_tensor* dst, int ith, int nth, void* userdata) {
    ...
    // Per-thread blocked output-row scratch (aligned for vector stores). Two
    // rows so the AVX-512 path can emit a pair of OC blocks per pass.
...
#if defined(FD_DCONV_HAVE_AVX512)
    if (lane == 16) {
        // Work items = (OC-block PAIR, ih). Each pass shares one input
        // broadcast across two OC blocks. Odd trailing block falls back to the
        // 1-block row.
        ...
        for (int64_t idx = beg; idx < end; ++idx) {
            ...
            if (ocb1 < OCB) {
                dconv_row2_avx512(x, Wpack, outrow0, outrow1, W, H, IC, ocb0,
                                  ocb1, ih, pad);
...
#if defined(__AVX2__)
        if (lane == 8) {
            dconv_row_avx2(x, Wpack, outrow0, W, H, IC, ocb, ih, pad);
        } else
#endif
        {
...
// every layer. The IResNet stage widths (64/128/256/512) are all multiples of
// 16, so CB = C/16 is exact and there are no padding lanes for buffalo_l/m.
// ========================================================================
constexpr int BLK = 16;  // nChw16c block width (the island is AVX-512 sized)

#if defined(FD_DCONV_HAVE_AVX512)
// Conv epilogue: optional fused bias add (bvec = 16 bias floats for this OC
// block, or nullptr) + optional ReLU, then store. Fusing bias/ReLU into the
// conv's own register tile removes the separate full-buffer blocked-bias /
// blocked-relu passes.
static inline FD_DCONV_AVX512_TARGET void bemit_avx512(float* p,
                                                       __m512 acc,
                                                       const float* bvec,
                                                       bool relu) {
...
// kh,kw,ic accumulation order matches dconv_*  so the result is bit-identical
// to the NCHW direct conv (layout-only change).
...
// Blocked input column stride is BLK floats (vs 1 for NCHW). ix = ow + kw -
// pad.
template <int CW>
static inline FD_DCONV_AVX512_TARGET void bconv3x3_strip_s1_avx512(
    ...
    for (int kh = 0; kh < 3; ++kh) {
        ...
        for (int kw = 0; kw < 3; ++kw) {
            ...
            const float* base =
                    xb + (((size_t)iy * W) + ix0) * BLK;  // cb=0,l=0
            for (int ic = 0; ic < IC; ++ic) {
                const int cb = ic >> 4, l = ic & 15;
                ...
                for (int j = 0; j < CW; ++j)
                    acc[j] = _mm512_fmadd_ps(
                            _mm512_set1_ps(inc[(size_t)j * BLK]), wv, acc[j]);
            }
        }
    }
    for (int j = 0; j < CW; ++j)
...
static inline FD_DCONV_AVX512_TARGET void bconv3x3_strip2_s1_avx512(
        ...
        int pad,
        int ow0,
        const float* bvec0 = nullptr,
        const float* bvec1 = nullptr,
        bool relu = false) {
    __m512 a0[CW], a1[CW];
    for (int j = 0; j < CW; ++j) {
        a0[j] = _mm512_setzero_ps();
...
// stride-1 2-OC-block output-row driver: edges per block (single-OC col
// kernel), interior via the shared-broadcast 2-block strips.
static inline FD_DCONV_AVX512_TARGET void bconv3x3_row2_s1_avx512(
    ...
    for (; ow0 + OWB2 - 1 <= hi; ow0 += OWB2)
        bconv3x3_strip2_s1_avx512<OWB2>(xb, w0, w1, orow0, orow1, W, H, IC, oh,
                                        pad, ow0, b0, b1, relu);
    int rem = hi - ow0 + 1;
    while (rem >= 8) {
        bconv3x3_strip2_s1_avx512<8>(xb, w0, w1, orow0, orow1, W, H, IC, oh,
                                     pad, ow0, b0, b1, relu);
        ow0 += 8;
        rem -= 8;
    }
    while (rem >= 4) {
        bconv3x3_strip2_s1_avx512<4>(xb, w0, w1, orow0, orow1, W, H, IC, oh,
                                     pad, ow0, b0, b1, relu);
        ow0 += 4;
        rem -= 4;
    }
    while (rem >= 2) {
        bconv3x3_strip2_s1_avx512<2>(xb, w0, w1, orow0, orow1, W, H, IC, oh,
                                     pad, ow0, b0, b1, relu);
        ow0 += 2;
        rem -= 2;
    }
    while (rem >= 1) {
        bconv3x3_strip2_s1_avx512<1>(xb, w0, w1, orow0, orow1, W, H, IC, oh,
                                     pad, ow0, b0, b1, relu);
        ow0 += 1;
        rem -= 1;
    }
...
// gather: for each (kh,kw) tap we precompute, per tiled pixel, the
// blocked-input base offset and a 0/1 validity gate (a pixel near a map edge
// has some taps out of bounds). The gate multiplies the broadcast input by 0.0f
// for an invalid tap, which adds an EXACT +0.0 to that accumulator --
// bit-identical to the per-row/per-column path's "skip out-of-bounds tap" (same
// kh,kw,ic order, same fp adds), so parity is unchanged. The 16-OC weight
// vector is still loaded once per ic and reused across the whole pixel strip
// (the master kernel's lever).
#if defined(FD_DCONV_HAVE_AVX512)
...
static inline FD_DCONV_AVX512_TARGET void bconv3x3_flat2_avx512(
    ...
    for (int kh = 0; kh < 3; ++kh) {
        for (int kw = 0; kw < 3; ++kw) {
            ...
            float gate[CW];
            for (int j = 0; j < CW; ++j) {
                const int iy = oh[j] * stride + kh - pad,
                          ix = ow[j] * stride + kw - pad;
                const bool v = (unsigned)iy < (unsigned)H &&
            ...
            for (int ic = 0; ic < IC; ++ic) {
                ...
                for (int j = 0; j < CW; ++j) {
                    const __m512 b =
                            _mm512_set1_ps(xb[base[j] + coff] * gate[j]);
                    a0[j] = _mm512_fmadd_ps(b, w0, a0[j]);
                    a1[j] = _mm512_fmadd_ps(b, w1, a1[j]);
                }
            }
    ...
    for (int kh = 0; kh < 3; ++kh) {
        for (int kw = 0; kw < 3; ++kw) {
            ...
            for (int ic = 0; ic < IC; ++ic) {
                ...
                for (int j = 0; j < CW; ++j)
                    acc[j] = _mm512_fmadd_ps(
                            _mm512_set1_ps(xb[base[j] + coff] * gate[j]), wv,
                            acc[j]);
            }
        }
    }
...
static inline FD_DCONV_AVX512_TARGET void bconv3x3_flat2_range_avx512(
    ...
    for (; f + OWB2 <= pend; f += OWB2)
        bconv3x3_flat2_avx512<OWB2>(xb, w0, w1, y0, y1, W, H, IC, Wout, pad, f,
                                    stride, b0, b1, relu);
    int rem = pend - f;
    while (rem >= 8) {
        bconv3x3_flat2_avx512<8>(xb, w0, w1, y0, y1, W, H, IC, Wout, pad, f,
                                 stride, b0, b1, relu);
        f += 8;
        rem -= 8;
    }
    while (rem >= 4) {
        bconv3x3_flat2_avx512<4>(xb, w0, w1, y0, y1, W, H, IC, Wout, pad, f,
                                 stride, b0, b1, relu);
        f += 4;
        rem -= 4;
    }
    while (rem >= 2) {
        bconv3x3_flat2_avx512<2>(xb, w0, w1, y0, y1, W, H, IC, Wout, pad, f,
                                 stride, b0, b1, relu);
        f += 2;
        rem -= 2;
    }
    while (rem >= 1) {
        bconv3x3_flat2_avx512<1>(xb, w0, w1, y0, y1, W, H, IC, Wout, pad, f,
                                 stride, b0, b1, relu);
        f += 1;
        rem -= 1;
    }
...
static inline FD_DCONV_AVX512_TARGET void bconv3x3_flat_range_avx512(
    ...
    for (; f + OWB <= pend; f += OWB)
        bconv3x3_flat_avx512<OWB>(xb, w0, y0, W, H, IC, Wout, pad, f, stride,
                                  b0, relu);
    int rem = pend - f;
    while (rem >= 8) {
        bconv3x3_flat_avx512<8>(xb, w0, y0, W, H, IC, Wout, pad, f, stride, b0,
                                relu);
        f += 8;
        rem -= 8;
    }
    while (rem >= 4) {
        bconv3x3_flat_avx512<4>(xb, w0, y0, W, H, IC, Wout, pad, f, stride, b0,
                                relu);
        f += 4;
        rem -= 4;
    }
    while (rem >= 2) {
        bconv3x3_flat_avx512<2>(xb, w0, y0, W, H, IC, Wout, pad, f, stride, b0,
                                relu);
        f += 2;
        rem -= 2;
    }
    while (rem >= 1) {
        bconv3x3_flat_avx512<1>(xb, w0, y0, W, H, IC, Wout, pad, f, stride, b0,
                                relu);
        f += 1;
        rem -= 1;
    }
...
// blocked-input broadcast across TWO OC blocks, so the zmm accumulator file
// fills regardless of how narrow each output row is. A 1x1 has a single tap
// always in bounds (output pixel maps to a valid input pixel for pad-0
// stride-s), so no edge gate is needed. Same ic order as bconv1x1_col_avx512 ->
// bit-identical.
template <int CW>
...
static inline FD_DCONV_AVX512_TARGET void bconv1x1_flat_avx512(
    ...
    for (int ic = 0; ic < IC; ++ic) {
        ...
        for (int j = 0; j < CW; ++j)
            acc[j] = _mm512_fmadd_ps(_mm512_set1_ps(xb[base[j] + coff]), wv,
                                     acc[j]);
    }
    for (int j = 0; j < CW; ++j)
        bemit_avx512(y0 + (size_t)(f0 + j) * BLK, acc[j], bvec, relu);
...
static inline FD_DCONV_AVX512_TARGET void bconv1x1_flat2_range_avx512(
    ...
    int f = pbeg;
    for (; f + OWB2 <= pend; f += OWB2)
        bconv1x1_flat2_avx512<OWB2>(xb, w0, w1, y0, y1, W, H, IC, Wout, stride,
                                    f, b0, b1, relu);
    int rem = pend - f;
    while (rem >= 8) {
        bconv1x1_flat2_avx512<8>(xb, w0, w1, y0, y1, W, H, IC, Wout, stride, f,
                                 b0, b1, relu);
        f += 8;
        rem -= 8;
    }
    while (rem >= 4) {
        bconv1x1_flat2_avx512<4>(xb, w0, w1, y0, y1, W, H, IC, Wout, stride, f,
                                 b0, b1, relu);
        f += 4;
        rem -= 4;
    }
    while (rem >= 2) {
        bconv1x1_flat2_avx512<2>(xb, w0, w1, y0, y1, W, H, IC, Wout, stride, f,
                                 b0, b1, relu);
        f += 2;
        rem -= 2;
    }
    while (rem >= 1) {
        bconv1x1_flat2_avx512<1>(xb, w0, w1, y0, y1, W, H, IC, Wout, stride, f,
                                 b0, b1, relu);
        f += 1;
        rem -= 1;
    }
...
static inline FD_DCONV_AVX512_TARGET void bconv1x1_flat_range_avx512(
    ...
    int f = pbeg;
    for (; f + OWB <= pend; f += OWB)
        bconv1x1_flat_avx512<OWB>(xb, w0, y0, W, H, IC, Wout, stride, f, b0,
                                  relu);
    int rem = pend - f;
    while (rem >= 8) {
        bconv1x1_flat_avx512<8>(xb, w0, y0, W, H, IC, Wout, stride, f, b0,
                                relu);
        f += 8;
        rem -= 8;
    }
    while (rem >= 4) {
        bconv1x1_flat_avx512<4>(xb, w0, y0, W, H, IC, Wout, stride, f, b0,
                                relu);
        f += 4;
        rem -= 4;
    }
    while (rem >= 2) {
        bconv1x1_flat_avx512<2>(xb, w0, y0, W, H, IC, Wout, stride, f, b0,
                                relu);
        f += 2;
        rem -= 2;
    }
    while (rem >= 1) {
        bconv1x1_flat_avx512<1>(xb, w0, y0, W, H, IC, Wout, stride, f, b0,
                                relu);
        f += 1;
        rem -= 1;
    }
...
static inline void bconv3x3_col_scalar(const float* xb,
                                       const float* wocb,
                                       float* orow,
                                       int W,
                                       int H,
...
static inline void bconv1x1_col_scalar(const float* xb,
                                       const float* wocb,
                                       float* orow,
                                       int W,
                                       int H,
...
// Blocked 3x3 conv compute: blocked in -> blocked out. dst ne =
// [16,Wout,Hout,OCB].
static void bconv3x3_compute(ggml_tensor* dst,
                             int ith,
                             int nth,
                             void* userdata) {
    DConvState* st = (DConvState*)userdata;
    ...
    const int W = st->W, H = st->H, IC = st->IC, OC = st->OC;
    const int pad = st->pad, stride = st->stride, lane = st->lane,
              OCB = st->OCB;
    ...
    if (lane == 16 && stride == 1) {
        ...
        // tile, the per-row driver below leaves most accumulators idle. Route
        // to the FLATTENED-SPATIAL kernel, which tiles consecutive pixels
        // across rows so the tile fills regardless of row width. Threshold
        // env-overridable (FACEDETECT_DCONV_FLAT_MAXW); default 13: flatten
        // only when a row's interior (Wout-2*pad) cannot fill the 2-OC tile
        // (OWB2=12), i.e. Wout<14. So the IResNet 7x7 stage flattens (big
        ...
        if (Wout <= flat_maxw) {
            ...
            // gets one large contiguous run -- it then tiles into full CW
            // strips (per-pair splitting would hand each thread only Npix/nth
            // pixels, which on a 7x7 map is below the tile width and
            // re-underfills).
            const int64_t total = (int64_t)npairs * Npix;
            const int64_t beg = total * ith / nth,
                          end = total * (ith + 1) / nth;
            if (beg >= end) return;
            for (int p = (int)(beg / Npix);
                 p < npairs && (int64_t)p * Npix < end; ++p) {
                const int ocb0 = 2 * p, ocb1 = 2 * p + 1;
                const int pbeg =
                        (int)std::max<int64_t>(0, beg - (int64_t)p * Npix);
                const int pend =
                        (int)std::min<int64_t>(Npix, end - (int64_t)p * Npix);
                ...
                if (ocb1 < OCB) {
                    ...
                    const float* b1 =
                            Bpack ? Bpack + (size_t)ocb1 * lane : nullptr;
                    float* y1 = y + (size_t)ocb1 * Npix * lane;
                    bconv3x3_flat2_range_avx512(xb, w0, w1, y0, y1, W, H, IC,
                                                Wout, pad, /*stride=*/1, pbeg,
                                                pend, b0, b1, relu);
                } else {
                    bconv3x3_flat_range_avx512(xb, w0, y0, W, H, IC, Wout, pad,
                                               /*stride=*/1, pbeg, pend, b0,
                                               relu);
                }
            }
            return;
        }
        // Work items = (OC-block PAIR, oh): each pass shares one
        // (width-strided) blocked-input broadcast across two OC blocks. Odd
        // trailing block falls back to the single-OC strip path.
        const int npairs = (OCB + 1) / 2;
        ...
        for (int64_t idx = beg; idx < end; ++idx) {
            ...
            const int ocb0 = 2 * p, ocb1 = 2 * p + 1;
            if (ocb1 < OCB) {
                bconv3x3_row2_s1_avx512(xb, Wpack, y, W, H, IC, Wout, Hout,
                                        ocb0, ocb1, oh, pad, Bpack, relu);
            } else {
                float* orow = y + (((size_t)ocb0 * Hout + oh) * Wout) * lane;
                const float* wocb = Wpack + (size_t)ocb0 * 9 * IC * lane;
                const float* bvec =
                        Bpack ? Bpack + (size_t)ocb0 * lane : nullptr;
                int ow = 0;
                for (; ow < pad && ow < Wout; ++ow)
                    bconv3x3_col_avx512(xb, wocb, orow, W, H, IC, oh, ow, pad,
                                        1, bvec, relu);
                const int hi = Wout - 1 - pad;
                int ow0 = std::max(pad, ow);
                for (; ow0 + OWB - 1 <= hi; ow0 += OWB)
                    bconv3x3_strip_s1_avx512<OWB>(xb, wocb, orow, W, H, IC, oh,
                                                  pad, ow0, bvec, relu);
                int rem = hi - ow0 + 1;
                while (rem >= 8) {
                    bconv3x3_strip_s1_avx512<8>(xb, wocb, orow, W, H, IC, oh,
                                                pad, ow0, bvec, relu);
                    ow0 += 8;
                    rem -= 8;
                }
                while (rem >= 4) {
                    bconv3x3_strip_s1_avx512<4>(xb, wocb, orow, W, H, IC, oh,
                                                pad, ow0, bvec, relu);
                    ow0 += 4;
                    rem -= 4;
                }
                while (rem >= 2) {
                    bconv3x3_strip_s1_avx512<2>(xb, wocb, orow, W, H, IC, oh,
                                                pad, ow0, bvec, relu);
                    ow0 += 2;
                    rem -= 2;
                }
                while (rem >= 1) {
                    bconv3x3_strip_s1_avx512<1>(xb, wocb, orow, W, H, IC, oh,
                                                pad, ow0, bvec, relu);
                    ow0 += 1;
                    rem -= 1;
                }
                for (int owe = std::max(Wout - pad, ow0); owe < Wout; ++owe)
                    bconv3x3_col_avx512(xb, wocb, orow, W, H, IC, oh, owe, pad,
                                        1, bvec, relu);
            }
        }
        return;
    }
    // STRIDE-2 (downsample) register-tiled path. The stride>1 case previously
    // ran bconv3x3_col_avx512 per output column -- ONE zmm accumulator, no
    // width tiling and no 2-OC broadcast sharing -- so the four IResNet
    // stage-transition downsample convs ran at ~40 GFLOP/s (~1/5 of the
    // stride-1 small-spatial rate). Route them through the SAME
    // flattened-spatial register-tiled kernel the small-spatial stride-1 path
    // uses (now stride-parameterized): it tiles CW consecutive OUTPUT pixels
    // with 2-OC broadcast sharing and gates out-of- bounds taps with an exact
    // +0.0, so it is bit-identical to the per-column path (same kh,kw,ic order)
    // while filling the zmm accumulator file. The stride>1 output maps
    // (56/28/14/7) are exactly the small-output regime the flat tiling was
    // built for, so no per-row variant is needed.
    if (lane == 16 && stride > 1 && dconv_stride2_flat()) {
        ...
        const int64_t beg = total * ith / nth, end = total * (ith + 1) / nth;
        if (beg >= end) return;
        for (int p = (int)(beg / Npix); p < npairs && (int64_t)p * Npix < end;
             ++p) {
            const int ocb0 = 2 * p, ocb1 = 2 * p + 1;
            ...
            if (ocb1 < OCB) {
                ...
                bconv3x3_flat2_range_avx512(xb, w0, w1, y0, y1, W, H, IC, Wout,
                                            pad, stride, pbeg, pend, b0, b1,
                                            relu);
            } else {
                bconv3x3_flat_range_avx512(xb, w0, y0, W, H, IC, Wout, pad,
                                           stride, pbeg, pend, b0, relu);
            }
        }
        return;
    }
...
#if defined(FD_DCONV_HAVE_AVX512)
        if (lane == 16) {  // stride > 1 (AVX-512 build with global flag, no
                           // flat path above)
...
#endif
        for (int ow = 0; ow < Wout; ++ow)
            bconv3x3_col_scalar(xb, wocb, orow, W, H, IC, oh, ow, pad, stride,
                                lane, bvec, relu);
    }
}

// Blocked 1x1 strided conv compute. dst ne = [16,Wout,Hout,OCB].
static void bconv1x1_compute(ggml_tensor* dst,
                             int ith,
                             int nth,
                             void* userdata) {
    DConvState* st = (DConvState*)userdata;
    ...
    if (lane == 16 && dconv_stride2_flat()) {
        // FLATTENED-SPATIAL register-tiled path (same recovery as the 3x3
        // stride-2 downsample): tile CW consecutive output pixels with 2-OC
        // broadcast sharing instead of the per-column single-zmm path, which
        // left the IResNet 1x1 shortcuts at ~40 GFLOP/s. The downsample 1x1s
        // output small maps (56/28/14/7), exactly the regime that underfills a
        // per-row tile.
        const int npairs = (OCB + 1) / 2;
        const int Npix = Wout * Hout;
        const int64_t total = (int64_t)npairs * Npix;
        ...
        for (int p = (int)(beg / Npix); p < npairs && (int64_t)p * Npix < end;
            ...
            if (ocb1 < OCB) {
                ...
                const float* b1 = Bpack ? Bpack + (size_t)ocb1 * lane : nullptr;
                float* y1 = y + (size_t)ocb1 * Npix * lane;
                bconv1x1_flat2_range_avx512(xb, w0, w1, y0, y1, W, H, IC, Wout,
                                            stride, pbeg, pend, b0, b1, relu);
            } else {
                bconv1x1_flat_range_avx512(xb, w0, y0, W, H, IC, Wout, stride,
                                           pbeg, pend, b0, relu);
            }
        }
        return;
...
#if defined(FD_DCONV_HAVE_AVX512)
            if (lane == 16) {
                bconv1x1_col_avx512(xb, wocb, orow, W, H, IC, oh, ow, stride,
                                    bvec, relu);
                continue;
            }
#endif
            bconv1x1_col_scalar(xb, wocb, orow, W, H, IC, oh, ow, stride, lane,
                                bvec, relu);
        }
    }
}

// reorder NCHW [W,H,C,1] -> blocked [16,W,H,CB] (zero-pads channels
// C..CB*16-1).
...
static void reorder_out_compute(ggml_tensor* dst, int ith, int nth, void*) {
    ...
    const size_t HW = (size_t)H * W;
    const int64_t beg = (int64_t)C * ith / nth,
                  end = (int64_t)C * (ith + 1) / nth;
...
static void bbias_compute(ggml_tensor* dst, int ith, int nth, void*) {
    ...
    const size_t HW = (size_t)H * W;
    const int64_t beg = (int64_t)CB * ith / nth,
                  end = (int64_t)CB * (ith + 1) / nth;
...
// blocked per-channel affine y = x*scale[c] + shift[c] (the IResNet
// pre-activation BatchNorm, host-folded gamma/var into scale and beta/mean into
// shift). Channel c = cb*16+l; padding lanes c>=C pass through unchanged (they
// carry no signal).
...
// blocked per-channel PReLU y = x>0 ? x : alpha[c]*x. Channel c = cb*16+l;
// padding lanes c>=C pass through (zero in, zero out either branch).
...
// fused residual add + ReLU: dst = max(0, src0 + src1). One pass instead of
// two.
...
struct StateKey {
    const void* wdata;
    int W, H, IC, OC, N, pad, stride, kind, flags;
    bool operator==(const StateKey& o) const {
        return wdata == o.wdata && W == o.W && H == o.H && IC == o.IC &&
               OC == o.OC && N == o.N && pad == o.pad && stride == o.stride &&
...
// Blocked-island state: input is the blocked buffer [16,W,H,CB] so W,H come
// from ne[1],ne[2]; IC is the TRUE input channel count (the weight's ne[2]);
// lane is ALWAYS 16 (nChw16c). kind 0 = 3x3, 1 = 1x1. flags encodes fused
// bias/relu so a reused weight with a different epilogue gets its own packed
// state.
static DConvState* get_state_blocked(ggml_tensor* w,
                                     ggml_tensor* xb,
                                     int pad,
                                     int stride,
                                     int kind,
...
ggml_tensor* directconv_conv3x3(ggml_context* ctx,
                                ggml_tensor* w,
                                ggml_tensor* x,
                                int pad) {
    const int OC = (int)w->ne[3];
    ...
    return ggml_custom_4d(ctx, GGML_TYPE_F32, Wout, Hout, OC, N, args, 2,
                          dconv_compute, GGML_N_TASKS_MAX, st);
}
...
ggml_tensor* blocked_reorder_in(ggml_context* ctx, ggml_tensor* x_nchw) {
    const int W = (int)x_nchw->ne[0], H = (int)x_nchw->ne[1],
              C = (int)x_nchw->ne[2];
    const int CB = (C + BLK - 1) / BLK;
    ggml_tensor* args[1] = {x_nchw};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, BLK, W, H, CB, args, 1,
                          reorder_in_compute, GGML_N_TASKS_MAX, nullptr);
}

ggml_tensor* blocked_reorder_out(ggml_context* ctx, ggml_tensor* xb, int C) {
    const int W = (int)xb->ne[1], H = (int)xb->ne[2];
    ggml_tensor* args[1] = {xb};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, W, H, C, 1, args, 1,
                          reorder_out_compute, GGML_N_TASKS_MAX, nullptr);
}

ggml_tensor* blocked_conv3x3(ggml_context* ctx,
                             ggml_tensor* w,
                             ggml_tensor* xb,
                             int pad,
                             int stride,
    ...
    return ggml_custom_4d(ctx, GGML_TYPE_F32, BLK, Wout, Hout, OCB, args,
                          bias ? 3 : 2, bconv3x3_compute, GGML_N_TASKS_MAX, st);
}

ggml_tensor* blocked_conv1x1(ggml_context* ctx,
                             ggml_tensor* w,
                             ggml_tensor* xb,
                             int stride,
                             ggml_tensor* bias,
    ...
    ggml_tensor* args[3] = {xb, w, bias};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, BLK, Wout, Hout, OCB, args,
                          bias ? 3 : 2, bconv1x1_compute, GGML_N_TASKS_MAX, st);
}

ggml_tensor* blocked_bias(ggml_context* ctx,
                          ggml_tensor* xb,
                          ggml_tensor* bias) {
    ggml_tensor* args[2] = {xb, bias};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, xb->ne[0], xb->ne[1], xb->ne[2],
                          xb->ne[3], args, 2, bbias_compute, GGML_N_TASKS_MAX,
                          nullptr);
}
...
ggml_tensor* blocked_relu(ggml_context* ctx, ggml_tensor* xb) {
    ggml_tensor* args[1] = {xb};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, xb->ne[0], xb->ne[1], xb->ne[2],
                          xb->ne[3], args, 1, brelu_compute, GGML_N_TASKS_MAX,
                          nullptr);
}
...
ggml_tensor* blocked_add(ggml_context* ctx, ggml_tensor* a, ggml_tensor* b) {
    ...
    return ggml_custom_4d(ctx, GGML_TYPE_F32, a->ne[0], a->ne[1], a->ne[2],
                          a->ne[3], args, 2, badd_compute, GGML_N_TASKS_MAX,
                          nullptr);
}

ggml_tensor* blocked_add_relu(ggml_context* ctx,
                              ggml_tensor* a,
                              ggml_tensor* b) {
    ggml_tensor* args[2] = {a, b};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, a->ne[0], a->ne[1], a->ne[2],
                          a->ne[3], args, 2, badd_relu_compute,
                          GGML_N_TASKS_MAX, nullptr);
}

ggml_tensor* blocked_scale_shift(ggml_context* ctx,
                                 ggml_tensor* xb,
                                 ggml_tensor* scale,
                                 ggml_tensor* shift) {
    ggml_tensor* args[3] = {xb, scale, shift};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, xb->ne[0], xb->ne[1], xb->ne[2],
                          xb->ne[3], args, 3, bscale_shift_compute,
                          GGML_N_TASKS_MAX, nullptr);
}

ggml_tensor* blocked_prelu(ggml_context* ctx,
                           ggml_tensor* xb,
                           ggml_tensor* alpha) {
    ggml_tensor* args[2] = {xb, alpha};
    return ggml_custom_4d(ctx, GGML_TYPE_F32, xb->ne[0], xb->ne[1], xb->ne[2],
                          xb->ne[3], args, 2, bprelu_compute, GGML_N_TASKS_MAX,
                          nullptr);
}
