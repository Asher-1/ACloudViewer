...
bool landmark_crop(const Image& img,
                   const Detection& d,
                   int size,
                   std::array<float, 6>& M,
                   Image& out) {
    // insightface Landmark.get: _scale = size / (max(w,h) * 1.5), centered on
    // the detection box. face_align.transform with rotation 0 reduces to a pure
    ...
    M = {s, 0.0f, -s * cx + size * 0.5f, 0.0f, s, -s * cy + size * 0.5f};
    return warp_affine(img, M, out, size, size);
}

std::vector<float> landmark_forward(const ModelLoader& ml,
                                    bool three_d,
                                    const Image& crop,
                                    Backend& be) {
    ...
    // scale into its leading bn_data BatchNormalization. swap_rb=false on the
    // RGB crop lands on the R,G,B planes the reference's swapRB=True produces.
    std::vector<float> blob = to_blob(crop, sz, h.input_mean, h.input_std,
                                      /*swap_rb=*/false);
    const std::string prefix = three_d ? "l3d." : "l2d.";
    // bn_eps fallback is unused: every BatchNormalization node carries its own
    // `e=` (2d106det 1e-3, 1k3d68 2e-5) embedded by the converter.
    return run_onnx_graph(ml, prefix, h.graph, h.graph_output, h.graph_input,
                          blob, sz, be, /*bn_eps=*/1e-5f);
}

std::vector<LandmarkPoint> landmark_decode_crop(const ModelLoader& ml,
                                                bool three_d,
    ...
    const float half =
            (float)(h.input_size / 2);  // integer division, like insightface
...
std::vector<LandmarkPoint> landmark_to_image(
        const std::vector<LandmarkPoint>& pts, const std::array<float, 6>& M) {
    // Invert the 2x3 forward affine (cv2.invertAffineTransform), then apply it
    // as insightface trans_points: x,y by the full inverse, z scaled by the
    // inverse's uniform scale. Mirrors the inversion in fd::warp_affine.
...
std::vector<LandmarkPoint> landmarks(const ModelLoader& ml,
                                     bool three_d,
                                     const Image& img,
                                     const Detection& d,
                                     Backend& be) {
