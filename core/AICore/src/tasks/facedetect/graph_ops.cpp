...
// leaves) on EVERY compute is pure graph-size + launch overhead. We fold them
// to host constants ONCE per loaded model and reuse them.
struct BNFold {
...
// Whether the in-progress graph build permits the F(4x4,3x3) Winograd variant
// on its large 3x3 s1p1 convs (SCRFD only; see graph_ops.hpp). thread_local so
...
void bn_fold_params(const ModelLoader& ml,
    ...
    ggml_tensor* mean =
            clone_weight(nullptr, ml, (p + ".running_mean").c_str());
    ggml_tensor* var = clone_weight(nullptr, ml, (p + ".running_var").c_str());
    const int64_t C = ggml_nelements(gamma);
    // 1-D BN stats stay F32 in the GGUF (converter skips quant for ndim<2), so
    // a raw float readback reproduces the reference exactly.
    // ggml_backend_tensor_get is backend-agnostic (works for the CPU-from-ptr
    // buffer and a device buffer).
    ...
    for (int64_t i = 0; i < C; ++i) {
        // Same f32 op order as the prior live-node path: sqrtf(var+eps),
        // divide, then beta - mean*scale. Algebraic identity, no numeric
        // reordering.
...
ggml_tensor* conv2d(ggml_context* ctx,
    ...
    // dominated by 3x3 stride-1 convs). Two CPU conv kernels trade off by
    ...
    // Measured crossover (buffalo_l 640x640 detect, min-of-5, this 16-core
    // box): im2col wins at 1/2/4 threads (spare bandwidth -> GEMM throughput
    // dominates), direct wins ~24% at 8 threads (the 9x stream saturates memory
    // bandwidth and direct's lighter traffic pulls ahead: 135 vs 177 ms). The
    // process-global default is min(hw,8) threads, so on an 8+-logical-core box
    // (the common deployment target) the default lands in direct's win region;
    // smaller boxes default below the crossover and keep im2col. 1x1 convs are
    // pure GEMM (no 9x expansion) so they always stay on im2col. On GPU the
    ...
    // microkernel (a double win), so it beats tinyBLAS sgemm here once the map
    // is big enough to amortize the input/output transforms. Only valid for
    // CPU, KW==KH==3, stride==1, pad==1, F32. The bias add + relu below are
    // unchanged (Winograd applies neither). A/B override via
    // FACEDETECT_WINOGRAD: "on" (all eligible) | "off" | "auto".
    //
    ...
    // GGML_NATIVE=OFF), gating BOTH ArcFace recognizer and SCRFD detect at
    // 1t/8t. Lowering it monotonically improves both at 1t (recognizer
    // 135->78ms, detect 148->121ms) and at 8t down to 14, where the recognizer
    // body's 14x14 map still pays. 7x7 maps do NOT pay at the production
    // default of min(hw,8) threads (recognizer 8t regresses 20.6->24ms, detect
    ...
    // ~4x fewer multiplies than direct (vs F2's 2.25x), but its 1/6,1/24
    // transform entries condition f32 worse, so it is gated by the detect 1px +
    // embedding cosine parity suite and settled by the A/B bench. ONLY SCRFD
    // opts in (it sets the F4 scope around its graph build): the ArcFace
    // recognizer and every small map stay on the parity-exact F(2x2) path. The
    // recognizer parity is independent of this flag, since its golden-landmark
    // gate bypasses detection. (prefer_f4 is finalized from use_wino once the
    // kernel choice below settles.)
    ...
    // Winograd at every thread count -- its large-spatial/low-channel maps
    // favor Winograd's 2.25x fewer multiplies, and directconv regresses SCRFD
    // detect
    // (-46% @1t / -12% @8t). Two-part gate keeps the two models apart:
    //   1. SCRFD-exclusion (!winograd_f4_scope()): SCRFD builds its ENTIRE
    //   graph
    ...
    // directconv needs F32 weights + N==1; the eligible convs satisfy both,
    // else fall back to Winograd. Overrides via FACEDETECT_CONV: "winograd"
    // forces Winograd for ALL eligible convs (directconv off); "directconv"
    // forces directconv for ALL eligible convs (incl. SCRFD, for A/B only);
    // "direct" or "im2col" drop BOTH specialized kernels so the stock ggml
    // kernel runs everywhere. directconv carries its own runtime-CPUID
    // AVX-512/AVX2 dispatch (FACEDETECT_DISABLE_AVX512 forces its AVX2 path,
    // shared with winograd.cpp).
    ...
    // ggml_add + ggml_relu bandwidth pass over the whole [Wout,Hout,OC,N]
    // output. The activation is known here at graph-build time. Only the
    // Winograd path fuses; directconv/im2col/direct keep the post-conv add+relu
    // below.
    const bool wino_fused = use_wino && relu;
    if (dconv)
        x = directconv_conv3x3(ctx, W, x, pad);
    else if (use_wino)
...
void fused_prelu_bias_cpu(ggml_tensor* dst,
                          const ggml_tensor* a,
                          const ggml_tensor* b,
                          const ggml_tensor* c,
                          int ith,
...
ggml_tensor* fused_prelu_bias(ggml_context* ctx,
                              ggml_tensor* x,
                              ggml_tensor* bias,
                              ggml_tensor* alpha) {
...
ggml_tensor* batchnorm(ggml_context* ctx,
                       const ModelLoader& ml,
                       ggml_tensor* x,
                       const char* prefix,
                       float eps,
                       std::vector<std::vector<float>>& keep) {
    (void)keep;  // affine is host-folded now; no per-call eps scalar buffer
                 // needed
