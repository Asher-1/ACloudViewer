...
// for the flat C-API / LocalAI) prefer fd::Model directly to avoid reloading
// the GGUF on every call.

// Embed the primary face in an image file: detect -> align (norm_crop) ->
// ArcFace -> L2-normalized embedding (typically 512-d). Throws
// std::runtime_error on failure (model/image load, no face found, unsupported
// pack, ...).
