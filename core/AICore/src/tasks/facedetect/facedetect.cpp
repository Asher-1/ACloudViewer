...
// Thin convenience wrappers. For repeated use (and for the flat C-API /
// LocalAI) use fd::Model directly to avoid reloading the GGUF on every call.

std::vector<float> embed(const std::string& model_path,
                         const std::string& image_path) {
    std::unique_ptr<Model> model = Model::load(model_path);
    if (!model)
        throw std::runtime_error("facedetect: failed to load model: " +
                                 model_path);
    Image img;
    if (!load_image_rgb(image_path, img))
        throw std::runtime_error("facedetect: failed to load image: " +
                                 image_path);
    return model->embed(img);
}

float verify(const std::string& model_path,
             const std::string& a,
             const std::string& b,
             float /*threshold*/) {
