...
ggml_tensor* conv(ggml_context* ctx,
...
ggml_tensor* conv3(ggml_context* ctx,
                   const ModelLoader& ml,
                   ggml_tensor* x,
                   const char* w,
                   const char* b,
...
// (H*W*num_anchors, C) row order. ct = a*C + c (anchor-major within a
// location).
std::vector<float> flatten_head(
        const std::vector<float>& src, int W, int H, int A, int C) {
...
// det_2.5g / det_500m (buffalo_m / buffalo_s) path: replay the embedded
// detector ONNX topology through the shared metadata-driven interpreter and map
// its 9 named per-stride heads onto ScrfdRawOut. The interpreter already
// flattens each head to the (H*W*num_anchors, C) row layout the host decode
// reads (via the head Transpose+Reshape), and the score heads end in Sigmoid
// in-graph, so the outputs are drop-in for the existing anchor decode + NMS -
// exactly the det_10g layout, just produced metadata-driven instead of
// hand-mapped. Output order is the ONNX graph-output order: scores s8/16/32,
// bbox s8/16/32, kps s8/16/32.
std::vector<ScrfdRawOut> scrfd_forward_interp(const ModelLoader& ml,
                                              const Image& blob_img,
                                              Backend& be) {
    ...
    std::vector<float> blob =
            to_blob(blob_img, S, 127.5f, 128.0f, /*swap_rb=*/false);
    std::vector<std::vector<float>> heads;
    try {
        heads = run_onnx_graph_multi(ml, "det.", cfg.det_graph,
                                     cfg.det_graph_input, cfg.det_graph_outputs,
                                     blob, S, be, 1e-5f);
...
std::vector<ScrfdRawOut> scrfd_forward(const ModelLoader& ml,
                                       const Image& blob_img640,
                                       Backend& be) {
    // buffalo_m (det_2.5g) / buffalo_s (det_500m) carry their detector topology
    // as a GGUF KV graph; replay it metadata-driven. buffalo_l (det_10g) has no
    // such KV and stays on the hand-mapped path below (zero regression).
    ...
    // Detector blob: R,G,B plane order, (px-127.5)/128 (== cv2 blobFromImage
    // with swapRB on a BGR image; our Image is already RGB, so no plane swap).
    std::vector<float> blob =
            to_blob(blob_img640, S, 127.5f, 128.0f, /*swap_rb=*/false);
    ...
    const bool ok = be.compute(
            [&](ggml_context* ctx) -> ggml_tensor* {
                // Permit F(4x4,3x3) Winograd on this detector's large early 3x3
                // s1p1 convs (the 320/160/80 maps). Scoped to this graph build
                // only: the guard clears the flag on return so the
                // recognizer/other graphs keep F(2x2).
                WinogradF4Scope f4scope(true);
                const int64_t ne_in[4] = {S, S, 3, 1};
                ggml_tensor* x = graph_input_tensor(
                        ctx, GGML_TYPE_F32, 4, ne_in, blob.data(),
                        blob.size() * sizeof(float));
                // ---- Backbone (ResNet-style stem + 4 stages)
                // ------------------------
                x = conv(ctx, ml, x, "det.547", "det.549", 2, 1,
                         true);  // 279 stem s2
                x = conv3(ctx, ml, x, "det.551", "det.553", true);  // 282
                x = conv3(ctx, ml, x, "det.555", "det.557", true);  // 285
                ggml_tensor* p =
                        pool2(ctx, x, GGML_OP_POOL_MAX);  // 286 maxpool
                ...
                // layer2 (88ch): downsample block + 3 residual blocks -> stride
                // 8 (C3)
                ...
                // layer3 (88ch): downsample block + 1 residual block -> stride
                // 16 (C4)
                ...
                ggml_tensor* c4 =
                        relu(ctx, add(ctx, y, s16));  // 355 (88ch, /16)

                // layer4 (224ch): downsample block + 2 residual blocks ->
                // stride 32 (C5)
                ...
                ggml_tensor* c5 =
                        relu(ctx, add(ctx, y, s32));  // 379 (224ch, /32)

                // ---- PAFPN neck
                // -----------------------------------------------------
                ggml_tensor* l0 = conv(ctx, ml, c3,
                                       "det.neck.lateral_convs.0.conv.weight",
                                       "det.neck.lateral_convs.0.conv.bias", 1,
                                       0, false);  // 380
                ggml_tensor* la1 = conv(ctx, ml, c4,
                                        "det.neck.lateral_convs.1.conv.weight",
                                        "det.neck.lateral_convs.1.conv.bias", 1,
                                        0, false);  // 381
                ggml_tensor* la2 = conv(ctx, ml, c5,
                                        "det.neck.lateral_convs.2.conv.weight",
                                        "det.neck.lateral_convs.2.conv.bias", 1,
                                        0, false);  // 382

                // top-down: nearest 2x upsample + add
                ggml_tensor* up = ggml_upscale(ctx, la2, 2,
                                               GGML_SCALE_MODE_NEAREST);  // 401
                ggml_tensor* p1m = add(ctx, la1, up);                     // 402
                ggml_tensor* up2 = ggml_upscale(
                        ctx, p1m, 2, GGML_SCALE_MODE_NEAREST);  // 421
                ggml_tensor* p0m = add(ctx, l0, up2);           // 422

                ggml_tensor* f0 =
                        conv3(ctx, ml, p0m, "det.neck.fpn_convs.0.conv.weight",
                              "det.neck.fpn_convs.0.conv.bias", false);  // 423
                // NOTE: the ONNX export feeds fpn_convs.1/.2's bias from
                // downsample_convs 0/1 (fpn_convs.1/.2 carry no own bias).
                // Match that exactly.
                ggml_tensor* f1 = conv3(
                        ctx, ml, p1m, "det.neck.fpn_convs.1.conv.weight",
                        "det.neck.downsample_convs.0.conv.bias", false);  // 424
                ggml_tensor* f2 = conv3(
                        ctx, ml, la2, "det.neck.fpn_convs.2.conv.weight",
                        "det.neck.downsample_convs.1.conv.bias", false);  // 425

                // bottom-up: strided downsample + add
                ggml_tensor* d0 = conv(
                        ctx, ml, f0, "det.neck.downsample_convs.0.conv.weight",
                        "det.neck.downsample_convs.0.conv.bias", 2, 1,
                        false);                      // 426
                ggml_tensor* n1 = add(ctx, f1, d0);  // 427
                ggml_tensor* d1 = conv(
                        ctx, ml, n1, "det.neck.downsample_convs.1.conv.weight",
                        "det.neck.downsample_convs.1.conv.bias", 2, 1,
                        false);                      // 428
                ggml_tensor* n2 = add(ctx, f2, d1);  // 429

                ggml_tensor* pa0 = conv3(
                        ctx, ml, n1, "det.neck.pafpn_convs.0.conv.weight",
                        "det.neck.pafpn_convs.0.conv.bias", false);  // 430
                ggml_tensor* pa1 = conv3(
                        ctx, ml, n2, "det.neck.pafpn_convs.1.conv.weight",
                        "det.neck.pafpn_convs.1.conv.bias", false);  // 431

                // ---- Three shared stride heads (stem 3x3 x3, then
                // cls/reg/kps) -------
                struct Head {
                    ggml_tensor* feat;
                    const char* sw[3];
                    const char* sb[3];  // stem convs
                    const char* cls_w;
                    const char* cls_b;
                    const char* reg_w;
                    const char* reg_b;
                ...
                const Head heads[3] = {
                        {f0,
                         {"det.667", "det.671", "det.675"},
                         {"det.669", "det.673", "det.677"},
                         "det.bbox_head.stride_cls.(8, 8).weight",
                         "det.bbox_head.stride_cls.(8, 8).bias",
                         "det.bbox_head.stride_reg.(8, 8).weight",
                         "det.bbox_head.stride_reg.(8, 8).bias",
                         "det.bbox_head.stride_kps.(8, 8).weight",
                         "det.bbox_head.stride_kps.(8, 8).bias",
                         "det.bbox_head.scales.0.scale"},
                        {pa0,
                         {"det.679", "det.683", "det.687"},
                         {"det.681", "det.685", "det.689"},
                         "det.bbox_head.stride_cls.(16, 16).weight",
                         "det.bbox_head.stride_cls.(16, 16).bias",
                         "det.bbox_head.stride_reg.(16, 16).weight",
                         "det.bbox_head.stride_reg.(16, 16).bias",
                         "det.bbox_head.stride_kps.(16, 16).weight",
                         "det.bbox_head.stride_kps.(16, 16).bias",
                         "det.bbox_head.scales.1.scale"},
                        {pa1,
                         {"det.691", "det.695", "det.699"},
                         {"det.693", "det.697", "det.701"},
                         "det.bbox_head.stride_cls.(32, 32).weight",
                         "det.bbox_head.stride_cls.(32, 32).bias",
                         "det.bbox_head.stride_reg.(32, 32).weight",
                         "det.bbox_head.stride_reg.(32, 32).bias",
                         "det.bbox_head.stride_kps.(32, 32).weight",
                         "det.bbox_head.stride_kps.(32, 32).bias",
                ...
                for (int i = 0; i < 3; ++i) {
                    ...
                    ggml_tensor* h =
                            conv3(ctx, ml, hd.feat, hd.sw[0], hd.sb[0], true);
                    h = conv3(ctx, ml, h, hd.sw[1], hd.sb[1], true);
                    h = conv3(ctx, ml, h, hd.sw[2], hd.sb[2], true);

                    ggml_tensor* cls =
                            conv3(ctx, ml, h, hd.cls_w, hd.cls_b, false);
                    ggml_tensor* score = ggml_sigmoid(ctx, cls);

                    ggml_tensor* reg =
                            conv3(ctx, ml, h, hd.reg_w, hd.reg_b, false);
                    ggml_tensor* scale =
                            clone_weight(ctx, ml, hd.scale);  // ne [1]
                    reg = ggml_mul(ctx, reg, scale);

                    ggml_tensor* kps =
                            conv3(ctx, ml, h, hd.kps_w, hd.kps_b, false);
