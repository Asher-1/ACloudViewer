...
// ONNX BatchNormalization epsilon for genderage.onnx (mxnet export: every bn
// node carries epsilon = 1e-3, NOT the 1e-5 the ArcFace export uses).
...
// fd::batchnorm in graph_ops expects .weight/.bias/.running_mean/.running_var,
// so this head needs its own loader. Math is identical:
//   y = (x - mean)/sqrt(var+eps)*gamma + beta, broadcast per channel (ne[2]).
ggml_tensor* ga_batchnorm(ggml_context* ctx,
    ...
    ggml_tensor* mean =
            clone_weight(ctx, ml, (prefix + "_moving_mean").c_str());
...
// Standard (group=1) conv -> BN -> ReLU. The conv is bias-less (the following
// BN is a separate node, not folded), so conv2d is called with b=nullptr.
ggml_tensor* conv_bn_relu(ggml_context* ctx,
                          const ModelLoader& ml,
                          ggml_tensor* x,
                          ...
                          int pad,
                          std::vector<std::vector<float>>& keep) {
    x = conv2d(ctx, ml, x, conv_w.c_str(), nullptr, stride, pad,
               /*relu=*/false);
...
// ggml_conv_2d_dw_direct yields the same WHCN output layout the rest of the
// graph uses.
ggml_tensor* dwconv_bn_relu(ggml_context* ctx,
                            const ModelLoader& ml,
                            ggml_tensor* x,
                            const std::string& conv_w,
                            const std::string& bn_prefix,
...
// One depthwise-separable block conv_N: depthwise (k3, stride `dw_stride`, pad
// 1) then pointwise (1x1, stride 1, pad 0), each conv followed by its own BN +
// ReLU.
ggml_tensor* sep_block(ggml_context* ctx,
                       const ModelLoader& ml,
                       ggml_tensor* x,
                       int n,
                       int dw_stride,
    ...
    x = dwconv_bn_relu(ctx, ml, x, p + "_dw_conv2d_weight", p + "_dw_batchnorm",
                       dw_stride, 1, keep);
    x = conv_bn_relu(ctx, ml, x, p + "_conv2d_weight", p + "_batchnorm", 1, 0,
                     keep);
    return x;
}
...
ggml_tensor* task_branch(ggml_context* ctx,
                         const ModelLoader& ml,
                         ggml_tensor* trunk,
    ...
    ggml_tensor* x = dwconv_bn_relu(
            ctx, ml, trunk, c13 + "_dw_" + tag + "_conv2d_weight",
    ...
    // [C,1] for the Gemm. Kernel size is the live spatial size (fixed by the
    // 96x96 input: 3x3 here).
    const int64_t W = x->ne[0], H = x->ne[1], C = x->ne[2];
    x = ggml_pool_2d(ctx, x, GGML_OP_POOL_AVG, (int)W, (int)H, (int)W, (int)H,
                     0, 0);
    x = ggml_reshape_2d(ctx, ggml_cont(ctx, x), C, 1);

    // Gemm(transB=1): out = x @ Wfc^T + b. Wfc ggml ne [256, out_dim] = [in,
    // out].
...
std::vector<float> genderage_forward(const ModelLoader& ml,
                                     const Image& crop96,
                                     Backend& be) {
    const int sz = (int)ml.config().genderage_input_size;  // 96
    // The crop is already RGB; genderage's reference blob is
    // blobFromImage(BGR, swapRB=True) == R,G,B planes, mean 0 / std 1 (the
    // net's own (x-127.5)/128 lives inside the graph as scalar_op1/op2).
    // swap_rb=false lands on those R,G,B planes - the same convention as
    // arcface_embed.
    std::vector<float> blob =
            to_blob(crop96, sz, 0.0f, 1.0f, /*swap_rb=*/false);

    std::vector<std::vector<float>>
            keep;  // BN-eps host buffers; must outlive compute
    ...
    const bool ok = be.compute(
            [&](ggml_context* ctx) -> ggml_tensor* {
                ...
                // Built-in input normalization: y = (x - scalar_op1) *
                // scalar_op2.
                ggml_tensor* s1 = ggml_reshape_4d(
                        ctx, clone_weight(ctx, ml, "ga.scalar_op1"), 1, 1, 1,
                        1);
                ggml_tensor* s2 = ggml_reshape_4d(
                        ctx, clone_weight(ctx, ml, "ga.scalar_op2"), 1, 1, 1,
                        1);
                x = ggml_mul(ctx, ggml_sub(ctx, x, s1), s2);

                // Stem: standard conv 3->16, k3 stride 2 pad 1, + BN + ReLU.
                x = conv_bn_relu(ctx, ml, x, "ga.conv_1_conv2d_weight",
                                 "ga.conv_1_batchnorm", 2, 1, keep);
                // 11 depthwise-separable blocks conv_2..conv_12. Depthwise
                // stride is 2 at conv_3 / conv_5 / conv_7 (the spatial
                // downsamples), 1 elsewhere.
                static const int kDwStride[13] = {
                        0,        0,       /*2*/ 1, /*3*/ 2, /*4*/ 1,  /*5*/ 2,
                ...
                // Two task branches off conv_12, then concat to fc1 = [g0, g1,
                // age].
                ggml_tensor* gender = task_branch(ctx, ml, x, "t0",
                                                  "ga.fullyconnected0", keep);
                ggml_tensor* age = task_branch(ctx, ml, x, "t1",
                                               "ga.fullyconnected1", keep);
                return ggml_concat(ctx, gender, age, /*dim=*/0);  // [3, 1]
            },
            out);
    if (!ok)
        throw std::runtime_error("facedetect: genderage graph compute failed");
    return out;
}

std::pair<int, int> genderage(const ModelLoader& ml,
                              const Image& img,
                              const Detection& d,
                              Backend& be) {
    const int sz = (int)ml.config().genderage_input_size;  // 96
    // face_align.transform: scale-about-center the detection box
    // (expanded 1.5x) to fit the square model input, centered. M maps source ->
    // output:
    //   s = sz / (max(w,h) * 1.5);  M = [[s,0, -s*cx + sz/2],[0,s, -s*cy +
    //   sz/2]]
