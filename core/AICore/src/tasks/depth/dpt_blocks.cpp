...
ggml_tensor* conv2d(ggml_context* ctx,
    ...
    // Direct convolution (no im2col 9x expansion) wins for K>1 kernels — the
    // DPT head's 3x3 convs at full resolution. 1x1 convs are pure GEMM, so keep
    // im2col (llamafile sgemm). The historical DA_CONV override
    // (direct|im2col|winograd) was an A/B scaffold and is removed; the routing
    // below is the auto default that won those experiments.
    const bool kgt1 = (w->ne[0] > 1 || w->ne[1] > 1);
    // Winograd is valid for 3x3 stride-1 F32 convs and is the default for
    // them: it beats ggml's direct conv on the warm DPT head, parity-exact
    // (max|d|~1.4e-5). The default kernel is the BLOCKED F(2x2) GEMM
    // (~193ms vs direct ~242ms @504 BASE/16t; ~490ms vs ~625ms on GIANT).
    // See benchmarks/BENCHMARK.md.
    const bool wino_ok = (w->ne[0] == 3 && w->ne[1] == 3 && stride == 1 &&
                          w->type == GGML_TYPE_F32 && x->type == GGML_TYPE_F32);
    // GPU mode: route ALL convs through ggml_conv_2d (im2col + mul_mat). On
    ...
    const bool use_wino = wino_ok && !gpu;  // winograd: CPU 3x3 only
    const bool direct =
            !gpu &&
            kgt1;  // CPU: direct for K>1 non-winograd; GPU: never (im2col)
    ggml_tensor* y;
    if (use_wino)
        y = winograd_conv3x3(ctx, w, x, pad);
    else if (direct)
        y = ggml_conv_2d_direct(ctx, w, x, stride, stride, pad, pad, 1, 1);
    else
        y = ggml_conv_2d(ctx, w, x, stride, stride, pad, pad, 1, 1);
    if (b) y = ggml_add(ctx, y, bias_chw(ctx, b));
    return y;
}

ggml_tensor* conv_transpose2d_p0(ggml_context* ctx,
                                 ggml_tensor* w,
                                 ggml_tensor* b,
                                 ggml_tensor* x,
                                 int stride) {
    // ggml_conv_transpose_2d_p0 expects kernel layout (KW,KH,Cout,Cin) ==
    // [KW,KH,OC,IC], which is exactly the straight dim-reversal of torch
    // ConvTranspose2d (IC,OC,KH,KW). x:[W,H,IC,N] -> [W_out,H_out,OC,N]
    ggml_tensor* y = ggml_conv_transpose_2d_p0(ctx, w, x, stride);
    if (b) y = ggml_add(ctx, y, bias_chw(ctx, b));
    return y;
}

ggml_tensor* interp_bilinear_ac(ggml_context* ctx,
                                ggml_tensor* x,
                                int out_w,
                                int out_h) {
    return ggml_interpolate(
            ctx, x, out_w, out_h, x->ne[2], x->ne[3],
            GGML_SCALE_MODE_BILINEAR | GGML_SCALE_FLAG_ALIGN_CORNERS);
}

ggml_tensor* residual_conv_unit(ggml_context* ctx,
                                ggml_tensor* x,
                                ggml_tensor* c1w,
                                ggml_tensor* c1b,
                                ggml_tensor* c2w,
                                ggml_tensor* c2b) {
    // out = relu(x); out = conv1(out); out = relu(out); out = conv2(out);
    // return out + x
...
ggml_tensor* feature_fusion(ggml_context* ctx,
                            ggml_tensor* top,
                            ggml_tensor* lateral,
                            ggml_tensor* rc1c1w,
                            ggml_tensor* rc1c1b,
    ...
    if (lateral && rc1c1w) {
        ggml_tensor* res = residual_conv_unit(ctx, lateral, rc1c1w, rc1c1b,
                                              rc1c2w, rc1c2b);
