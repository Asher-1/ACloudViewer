...
void build_ray_cloud(const float* ray,
                     const float* ray_conf,
                     int Hy,
                     int Wx,
                     double z_threshold,
    ...
    // Identity-cam-plane grid (unproject_depth, ixt_normalized): for patch
    // index j, x_grid = linspace(dx, 2-dx, Wx)[j]; origin x = x_grid - 1
    // (I_K^{-1} subtracts cx=1). (ixt_normalized branch builds the grid in
    // float32 in torch; we use double here and
    ...
    for (int j = 0; j < Wx; ++j)
        gx[j] = (Wx == 1) ? dx
                          : (dx + (double)j * ((2.0 - dx) - dx) / (Wx - 1));
    for (int i = 0; i < Hy; ++i)
        gy[i] = (Hy == 1) ? dy
                          : (dy + (double)i * ((2.0 - dy) - dy) / (Hy - 1));
...
static bool fit_homography_idx(const std::vector<double>& src,
...
static double score_homography(const std::vector<double>& src,
                               const std::vector<double>& dst,
                               const std::vector<double>& w,
                               int N,
                               const double H[9],
    ...
    for (int n = 0; n < N; ++n) {
        ...
        bool in = (ex * ex + ey * ey) <
                  thr2;  // error < reproj_thr (squared compare)
...
bool solve_ray_pose(const float* ray,
                    const float* ray_conf,
                    int Hy,
                    int Wx,
                    int img_H,
                    int img_W,
                    const int* rand_idx,
                    const int* sorted_idx_in,
                    const int* refit_idx,
                    int n_refit,
                    const RayPoseParams& p,
                    RayPoseOut& out) {
    const int N = Hy * Wx;
    std::vector<double> src, dst, w, origin_half, conf_raw;
    build_ray_cloud(ray, ray_conf, Hy, Wx, p.z_threshold, src, dst, w,
                    origin_half, conf_raw);

    // n_sample is the candidate pool size; clamp to N so a tiny ray field
    // (N < num_sample) can never index sorted_idx out of range.
    const int n_sample = std::min(
            N, std::max(p.num_sample, (int)((double)N * p.sample_ratio)));

    // candidate ordering: argsort(w, descending). Use the fed order if provided
    // (matches torch exactly on the gated path); else compute a deterministic
    // one.
    ...
    // sampling indices into [0, n_sample): fed (gated) or generated
    // (production).
    ...
    for (int it = 0; it < p.n_iter; ++it) {
        ...
        double H[9];
        if (!fit_homography_idx(src, dst, w, sample.data(), p.num_sample, H))
            continue;
        double sc = score_homography(src, dst, w, N, H, p.reproj_threshold,
                                     &tmp_mask);
        if (sc > best_score) {
            best_score = sc;
            best_iter = it;
    ...
    inliers.reserve(N);
    for (int n = 0; n < N; ++n)
        if (best_mask[n]) inliers.push_back(n);
    ...
    if (refit_idx) {
        ...
        // Production: sort inliers by weight desc; subsample if >
        // max_inlier_num.
        std::stable_sort(inliers.begin(), inliers.end(),
                         [&](int a, int b) { return w[a] > w[b]; });
        if ((int)inliers.size() > p.max_inlier_num) {
            int keep_len =
                    std::max((int)(inliers.size() * 0.95), p.max_inlier_num);
    ...
    double A[9];
    if (!fit_homography_idx(src, dst, w, refit.data(), (int)refit.size(), A))
        return false;
    if (linalg::det3(A) < 0)
    ...
    if (std::fabs(csum) < 1e-12)
        return false;  // all-zero confidence -> T undefined
    Tn[0] /= csum;
    Tn[1] /= csum;
    Tn[2] /= csum;
    ...
    // Assemble w2c = [[R, T],[0,0,0,1]] then affine_inverse -> c2w = [[R^T,
    // -R^T T],[..]].
    double Rt[9];
    linalg::mat3_transpose(R, Rt);
    double c2wT[3];
    for (int i = 0; i < 3; ++i)
        c2wT[i] = -(Rt[i * 3 + 0] * Tn[0] + Rt[i * 3 + 1] * Tn[1] +
                    Rt[i * 3 + 2] * Tn[2]);
    ...
    for (int i = 0; i < 12; ++i)
        if (!std::isfinite(out.extrinsics[i])) return false;
    for (int i = 0; i < 9; ++i)
        if (!std::isfinite(out.intrinsics[i])) return false;
    return true;
}
