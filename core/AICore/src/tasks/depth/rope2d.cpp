...
namespace aicore {
namespace depth {
RopeTables build_rope_tables(const std::vector<float>& pos_yx,
                             int tokens,
                             int head_dim,
                             float freq) {
    RopeTables rt;
    rt.head_dim = head_dim;
    rt.tokens = tokens;
...
void build_rope_inputs(ggml_context* ctx,
                       Backend& be,
                       GraphInputPool& pool,
                       const RopeTables& rt,
                       ggml_tensor*& cosb,
...
ggml_tensor* apply_rope(ggml_context* ctx,
    ...
    auto part = [&](int off, int len) {
        return ggml_cont(ctx, ggml_view_3d(ctx, x, len, heads, tok, x->nb[1],
                                           x->nb[2], (size_t)off * x->nb[0]));
    };
    ...
    auto rot_half = [&](ggml_tensor* h) -> ggml_tensor* {
        ggml_tensor* h0 = ggml_cont(ctx, ggml_view_3d(ctx, h, quart, heads, tok,
                                                      h->nb[1], h->nb[2], 0));
        ggml_tensor* h1 = ggml_cont(
                ctx, ggml_view_3d(ctx, h, quart, heads, tok, h->nb[1], h->nb[2],
                                  (size_t)quart * h->nb[0]));
        return ggml_concat(ctx, ggml_neg(ctx, h1), h0, 0);  // [-h1, h0]
    };
    ggml_tensor* rot = ggml_concat(ctx, rot_half(ay), rot_half(ax),
                                   0);  // [head_dim,heads,tok]
