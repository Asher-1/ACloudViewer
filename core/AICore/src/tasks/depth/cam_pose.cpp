...
bool CamPose::pose(const std::vector<float>& cam_token,
                   int H,
                   int W,
    ...
    // cam token width = 2*embed (base 1536, giant 3072); validate against bb0
    // input dim.
    ggml_tensor* bb0 = t("cam.bb0.weight");
    if (cam_token.empty() || !bb0 || (int64_t)cam_token.size() != bb0->ne[0])
        return false;
    ...
    bool ok = be_.compute(
            [&](ggml_context* ctx) -> ggml_tensor* {
                ...
                ggml_tensor* feat = be_.add_graph_input_nd(
                        ctx, pool, cam_token.data(), ne, 1);
                // backbone: relu(linear(bb0)); relu(linear(bb2))
                feat = ggml_relu(ctx, linear(ctx, t("cam.bb0.weight"), feat,
                                             t("cam.bb0.bias")));
                feat = ggml_relu(ctx, linear(ctx, t("cam.bb2.weight"), feat,
                                             t("cam.bb2.bias")));
                // heads: t (no relu), q (no relu), fov (relu AFTER fc_fov)
                ggml_tensor* th = linear(ctx, t("cam.fc_t.weight"), feat,
                                         t("cam.fc_t.bias"));  // [3]
                ggml_tensor* qh = linear(ctx, t("cam.fc_q.weight"), feat,
                                         t("cam.fc_q.bias"));  // [4]
                ggml_tensor* fvh =
                        ggml_relu(ctx, linear(ctx, t("cam.fc_fov.weight"), feat,
                                              t("cam.fc_fov.bias")));  // [2]
    ...
    float R[3][3];
    R[0][0] = 1.0f - s * (qj * qj + qk * qk);
    R[0][1] = s * (qi * qj - qk * qr);
    R[0][2] = s * (qi * qk + qj * qr);
    R[1][0] = s * (qi * qj + qk * qr);
    R[1][1] = 1.0f - s * (qi * qi + qk * qk);
    R[1][2] = s * (qj * qk - qi * qr);
    R[2][0] = s * (qi * qk - qj * qr);
    R[2][1] = s * (qj * qk + qi * qr);
    R[2][2] = 1.0f - s * (qi * qi + qj * qj);
    ...
    K[2][2] = 1.0f;
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c) intrinsics[r * 3 + c] = K[r][c];

    // affine_inverse(c2w) -> ext: [R^T | -R^T * Tc]
    float RT[3][3];
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c) RT[r][c] = R[c][r];
    float negRTt[3];
    for (int r = 0; r < 3; ++r) {
        float acc = 0.0f;
