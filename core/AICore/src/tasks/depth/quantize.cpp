...
bool should_quantize(const std::string& name) {
    static const std::regex re_vit(
            "^vit\\.blk\\.[0-9]+\\.(attn_qkv|attn_proj|mlp_fc1|mlp_fc2|mlp_w12|"
            "mlp_w3)\\.weight$");
...
bool quantize_gguf(const std::string& in_gguf,
    ...
    if (!type_from_string(type, target)) {
        DA_ERR("quantize: unknown type '%s' (expected f16/q8_0/q6_k/q5_k/q4_k)",
               type.c_str());
        return false;
    }
    ...
    if (!out_ctx) {
        DA_ERR("quantize: ggml_init for out_ctx failed");
        gguf_free(out);
        gguf_free(in);
        ggml_free(meta_ctx);
        return false;
    ...
    for (int64_t ti = 0; ti < n_tensors && !failed; ++ti) {
        ...
        if (!src || !src->data) {
            DA_ERR("quantize: tensor '%s' has no data", name);
            failed = true;
            break;
        }
        ...
        if (should_quantize(name) && ggml_n_dims(src) == 2) {
            if (src->ne[0] % 256 == 0) {
                used = target;  // K-quants need 256-divisible rows; DA3 dims ->
                                // OK
                quant = true;
            } else if (is_kquant && src->ne[0] % 32 == 0) {
                used = GGML_TYPE_Q8_0;  // safety fallback (won't trigger for
                                        // DA3 dims)
                quant = true;
                ++n_fallback_q8;
            } else if (!is_kquant && src->ne[0] % 32 == 0) {
        ...
        const int64_t ne[GGML_MAX_DIMS] = {src->ne[0], src->ne[1], src->ne[2],
                                           src->ne[3]};
        ggml_tensor* dst = nullptr;

        if (quant) {
            if (!dequantize_to_f32(src, f32_buf)) {
                DA_ERR("quantize: cannot dequantize '%s' (type=%s)", name,
            ...
            const size_t got = ggml_quantize_chunk(
                    used, f32_buf.data(), bytes.data(),
                    /*start=*/0, nrows, ne0, /*imatrix=*/nullptr);
            if (got != qsz) {
                DA_ERR("quantize: chunk size mismatch '%s' got=%zu want=%zu",
                       name, got, qsz);
                failed = true;
                break;
            }
            ...
            bytes.assign((const uint8_t*)src->data,
                         (const uint8_t*)src->data + nb);
    ...
    if (ok && !gguf_write_to_file(out, out_gguf.c_str(), /*only_meta=*/false)) {
        DA_ERR("quantize: gguf_write_to_file failed for '%s'",
               out_gguf.c_str());
        ok = false;
    }
    ...
    if (ok) {
        if (n_fallback_q8)
            DA_DEBUG_LOG(
                    "quantize: %d quantized (%s), %d kept f32, %d q8_0 "
                    "fallback",
                    n_quant, ggml_type_name(target), n_kept, n_fallback_q8);
        else
            DA_DEBUG_LOG("quantize: %d quantized (%s), %d kept f32", n_quant,
                         ggml_type_name(target), n_kept);
    }
