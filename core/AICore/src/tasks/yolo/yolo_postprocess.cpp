...
std::vector<Detection> postprocess(const std::vector<float>& raw,
    ...
    // sigmoid is monotonic, so argmax and the conf filter run on the raw
    // logits — sigmoid itself is applied only to the few surviving
    // candidates. The class loop is outer so the anchor walk streams raw[c]
    // contiguously instead of hopping na*4B per class.
    ...
    // DFL softmax scratch (reg_max may be any size, e.g. 16 for v8, 1 for
    // yolo26).
    ...
    for (const Cand& cd : cands) {
        ...
        float d[4];
        if (rm > 1) {
            // torch DFL: x.view(B, 4, reg_max, A) — channel k = j*rm + t
            // (edge j outer, bin t inner).
            for (int j = 0; j < 4; j++) {
                float m = -INFINITY;
                for (int t = 0; t < rm; t++)
        ...
        if (meta.end2end) {
            ...
            // dist2bbox xywh: center = (lt + rb) / 2, wh = lt + rb, scaled by
            // stride.
            ...
            dets.push_back({cx - w * 0.5f, cy - h * 0.5f, cx + w * 0.5f,
                            cy + h * 0.5f, cd.score, cd.cls});
        }
    }

    if (meta.end2end) {
        // Detect.postprocess topk (score descending) then the conf filter in
        // the non_max_suppression end2end branch keeps the order; cap at
        // max_det.
    ...
    // Greedy class-aware NMS, matching torchvision.ops.nms with the class
    // offset trick: only same-class boxes can suppress each other,
    // IoU > threshold.
