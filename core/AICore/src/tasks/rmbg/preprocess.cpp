...
// Replaces stb_image / stb_image_write with QImage + QBuffer.
#include <QImage>
#include <QBuffer>
#include <QIODevice>
...
bool decode_preprocess(const void * bytes, int length, int size,
    ...
    if (img.isNull()) { err = "image decode failed"; return false; }
        ...
        for (int x = 0; x < size; ++x) {
            for (int c = 0; c < 3; ++c) {
                const float interpolated = resize_bilinear(
                    [&](int px, int py) { return original_rgba[((size_t) py * width + px) * 4 + c] / 255.f; },
...
bool decode_preprocess_rgb(const uint8_t * rgb, int rgb_w, int rgb_h, int size,
    ...
    for (int y = 0; y < size; ++y) {
        for (int x = 0; x < size; ++x) {
            for (int c = 0; c < 3; ++c) {
                const float interpolated = resize_bilinear(
                    [&](int px, int py) { return rgb[((size_t) py * rgb_w + px) * 3 + c] / 255.f; },
