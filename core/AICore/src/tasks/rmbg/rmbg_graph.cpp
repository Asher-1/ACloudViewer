...
struct GraphBuilder {
    ...
    ggml_tensor *weight(const std::string &name) {
        ...
        if (!source || source->shape.empty() ||
            source->shape.size() > GGML_MAX_DIMS)
            return nullptr;
        const bool matrix_weight =
                source->shape.size() >= 2 && name.size() >= 7 &&
                name.compare(name.size() - 7, 7, "_weight") == 0;
        ...
        const std::vector<float> *f32 =
                preserve_type ? nullptr : weights.get_f32(name.c_str());
        ...
        const enum ggml_type type =
                preserve_type ? source->type : GGML_TYPE_F32;
        ggml_tensor *tensor =
                ggml_new_tensor(stat, type, (int)source->shape.size(), ne);
    ...
    ggml_tensor *weight_f16(const std::string &name) {
        ...
        if (source->type == GGML_TYPE_F16) {
            ...
            for (size_t i = 0; i < f32->size(); ++i)
                converted[i] = ggml_fp32_to_fp16((*f32)[i]);
    ...
    template <typename T>
    ggml_tensor *constant(enum ggml_type type,
                          const std::vector<int64_t> &ne,
                          const std::vector<T> &data,
                          const char *stem) {
        ggml_tensor *tensor =
                ggml_new_tensor(stat, type, (int)ne.size(), ne.data());
        const std::string name =
                std::string(stem) + "_" + std::to_string(constant_id++);
        ggml_set_name(tensor, name.c_str());
        StaticBlob blob;
        blob.tensor = tensor;
    ...
    ggml_tensor *layer_norm(ggml_tensor *x, const std::string &prefix) {
        ...
        ggml_tensor *fused = ggml_custom_4d(
                ctx, GGML_TYPE_F32, x->ne[0], x->ne[1], x->ne[2], x->ne[3],
                args, 3, nullptr, GGML_N_TASKS_MAX, nullptr);
        ggml_set_name(fused, "rmbg_layer_norm");
        if (use_cuda_custom && ggml_backend_supports_op(backend, fused))
            return fused;
    ...
    ggml_tensor *linear(ggml_tensor *x,
    ...
    ggml_tensor *conv2d(ggml_tensor *input,
        ...
        if (use_vulkan_direct_conv) {
            out = ggml_conv_2d_direct(ctx, w, input, stride, stride, pad, pad,
                                      1, 1);
            std::snprintf(custom_name, sizeof(custom_name), "rmbg_conv2d_%s",
                          prefix.c_str());
            ggml_set_name(out, custom_name);
    ...
    ggml_tensor *batch_norm_impl(ggml_tensor *x,
                                 const std::string &prefix,
                                 bool relu) {
        const std::vector<float> *gamma =
                weights.get_f32((prefix + "weight").c_str());
        const std::vector<float> *beta =
                weights.get_f32((prefix + "bias").c_str());
        const std::vector<float> *mean =
                weights.get_f32((prefix + "running_mean").c_str());
        const std::vector<float> *var =
                weights.get_f32((prefix + "running_var").c_str());
        ...
        ggml_tensor *s =
                constant(GGML_TYPE_F32, {1, 1, (int64_t)scale.size(), 1}, scale,
                         "bn_scale");
        ggml_tensor *b =
                constant(GGML_TYPE_F32, {1, 1, (int64_t)shift.size(), 1}, shift,
                         "bn_shift");
    ...
    ggml_tensor *valid_coordinate(ggml_tensor *x, int limit) {
        ...
        ggml_tensor *hi = ggml_step(
                ctx, add_scalar(ggml_neg(ctx, x), (float)limit - 0.5f));
    ...
    ggml_tensor *deform_sample_corner(ggml_tensor *input_tokens,
                                      ggml_tensor *yf,
                                      ggml_tensor *xf,
                                      ggml_tensor *weight_xy,
                                      int H,
                                      int W) {
        ggml_tensor *valid =
                ggml_mul(ctx, valid_coordinate(yf, H), valid_coordinate(xf, W));
        ggml_tensor *yc =
                ggml_clamp(ctx, ggml_dup(ctx, yf), 0.f, (float)H - 1.f);
        ggml_tensor *xc =
                ggml_clamp(ctx, ggml_dup(ctx, xf), 0.f, (float)W - 1.f);
        ...
        ggml_tensor *weight =
                ggml_reshape_2d(ctx, ggml_mul(ctx, weight_xy, valid), 1,
                                ggml_nelements(weight_xy));
    ...
    static void deform_im2col_cpu(ggml_tensor *dst, int ith, int nth, void *) {
        ...
        for (int64_t index = begin; index < end; ++index) {
            ...
            const int xb = p - yb * W;
            const float y =
                    (float)(yb + k / KW - KH / 2) + off[p + HW * (2 * k)];
            const float x =
                    (float)(xb + k % KW - KW / 2) + off[p + HW * (2 * k + 1)];
    ...
    ggml_tensor *deform_im2col_legacy(ggml_tensor *input,
                                      ggml_tensor *offset,
                                      ggml_tensor *modulator,
                                      int H,
                                      int W,
                                      int C,
                                      int KH,
        ...
        ggml_tensor *ox =
                ggml_view_2d(ctx, offset, HW, K, (size_t)2 * HW * sizeof(float),
                             (size_t)HW * sizeof(float));
        ggml_tensor *fy = ggml_add(
                ctx, ggml_add(ctx, oy, ggml_reshape_2d(ctx, by, HW, 1)),
                constant(GGML_TYPE_F32, {1, K}, kernel_y, "kernel_y"));
        ggml_tensor *fx = ggml_add(
                ctx, ggml_add(ctx, ox, ggml_reshape_2d(ctx, bx, HW, 1)),
        ...
        ggml_tensor *s00 =
                deform_sample_corner(tokens, flat(y0), flat(x0),
                                     flat(ggml_mul(ctx, wy0, wx0)), H, W);
        ggml_tensor *s01 = deform_sample_corner(
                tokens, flat(y0), flat(x1), flat(ggml_mul(ctx, wy0, dx)), H, W);
        ggml_tensor *s10 = deform_sample_corner(
                tokens, flat(y1), flat(x0), flat(ggml_mul(ctx, dy, wx0)), H, W);
        ggml_tensor *s11 = deform_sample_corner(
                tokens, flat(y1), flat(x1), flat(ggml_mul(ctx, dy, dx)), H, W);
        ggml_tensor *samples =
                ggml_add(ctx, ggml_add(ctx, s00, s01), ggml_add(ctx, s10, s11));
        ggml_tensor *mask = ggml_reshape_2d(ctx, modulator, 1, HW * K);
        samples = ggml_mul(ctx, samples,
                           ggml_scale(ctx, ggml_sigmoid(ctx, mask), 2.f));
    ...
    ggml_tensor *deform_conv2d(ggml_tensor *input, const std::string &prefix) {
        ggml_tensor *regular =
                weight(prefix + "atrous_conv_regular_conv_weight");
        ...
        const int64_t HW = (int64_t)H * W;
        ggml_tensor *offset =
                conv2d(input, prefix + "atrous_conv_offset_conv_", 1, pad);
        ggml_tensor *modulator =
                conv2d(input, prefix + "atrous_conv_modulator_conv_", 1, pad);
        ...
        ggml_tensor *fused_regular =
                deform_project_enabled
                        ? weight_f16(prefix + "atrous_conv_regular_conv_weight")
                        : regular;
        ggml_tensor *fused_args[] = {input, offset, modulator, fused_regular,
                                     regular_bias};
        ggml_tensor *fused =
                fused_regular
                        ? ggml_custom_4d(ctx, GGML_TYPE_F32, W, H,
                                         fused_regular->ne[3], 1, fused_args, 5,
                                         nullptr, GGML_N_TASKS_MAX, nullptr)
                        : nullptr;
        if (fused) {
            ggml_set_name(fused, deform_project_coop ? "rmbg_deform_project_cm1"
                                                     : "rmbg_deform_project");
            const bool deform_project_supported =
                    use_backend_custom &&
        ...
        ggml_tensor *stacked = ggml_custom_4d(
                ctx, GGML_TYPE_F32, (int64_t)C * K, HW, 1, 1, args, 3,
                deform_im2col_cpu, GGML_N_TASKS_MAX, nullptr);
        ggml_set_name(stacked, "rmbg_deform_im2col");
        if (!use_backend_custom ||
            !ggml_backend_supports_op(backend, stacked)) {
            stacked = deform_im2col_legacy(input, offset, modulator, H, W, C,
                                           KH, KW, pad);
        }
    ...
    ggml_tensor *aspp(ggml_tensor *input, const std::string &prefix) {
        ...
        ggml_tensor *gap =
                ggml_pool_2d(ctx, input, GGML_OP_POOL_AVG, W, H, W, H, 0, 0);
        gap = conv2d(gap, prefix + "global_avg_pool_1_", 1, 0);
        gap = gap ? batch_norm_relu(gap, prefix + "global_avg_pool_2_")
                  : nullptr;
    ...
    ggml_tensor *basic_decoder_block(ggml_tensor *input,
                                     const std::string &prefix) {
    ...
    ggml_tensor *image_patches(ggml_tensor *image, int patch_h, int patch_w) {
        ...
        ggml_tensor *gathered =
                ggml_custom_4d(ctx, GGML_TYPE_F32, 3, gh * gw * P, 1, 1, args,
                               1, nullptr, GGML_N_TASKS_MAX, nullptr);
        ...
        if (!use_backend_custom ||
            !ggml_backend_supports_op(backend, gathered)) {
            ...
            gathered = ggml_get_rows(
                    ctx, spatial_to_tokens(image),
                    constant(GGML_TYPE_I32, {(int64_t)index.size()}, index,
                             "patch_idx"));
        }
        // get_rows produces [c, grid, pixel]. einops' "(c hg wg)" layout
        // requires grid to be the fastest varying component of the channel
        // axis.
    ...
    ggml_tensor *input_concat(ggml_tensor *image,
                              ggml_tensor *feature,
                              const std::string &prefix) {
        ggml_tensor *patches =
                image_patches(image, (int)feature->ne[1], (int)feature->ne[0]);
        ggml_tensor *x = conv2d(patches, prefix + "conv1_", 1, 1);
        x = x ? conv2d(x, prefix + "conv_out_", 1, 1) : nullptr;
        return x ? ggml_concat(ctx, feature, x, 2) : nullptr;
    }

    ggml_tensor *gdt_attention(ggml_tensor *input,
                               const std::string &gdt,
    ...
    EncoderTaps encoder_4scale(ggml_tensor *input,
                               int H,
                               int W,
                               SwinTaps &full_debug) {
        EncoderTaps out;
        ...
        out.x1 = ggml_concat(ctx, full_debug.x1, resize(half.x1, H / 4, W / 4),
                             2);
        out.x2 = ggml_concat(ctx, full_debug.x2, resize(half.x2, H / 8, W / 8),
                             2);
        out.x3 = ggml_concat(ctx, full_debug.x3,
                             resize(half.x3, H / 16, W / 16), 2);
        out.x4 = ggml_concat(ctx, full_debug.x4,
                             resize(half.x4, H / 32, W / 32), 2);
    ...
    ggml_tensor *decoder(ggml_tensor *image,
                         const EncoderTaps &enc,
                         int H,
                         int W) {
    ...
    ggml_tensor *spatial_to_tokens(ggml_tensor *x) {
        ...
        return ggml_reshape_2d(ctx, x, x->ne[0],
    ...
    static void build_shift_mask(
            int H, int W, int ws, int shift, std::vector<float> &out) {
        ...
        for (int win = 0; win < nW; ++win) {
            ...
            for (int q = 0; q < N; ++q) {
                const int qr =
                        regions[(size_t)(wh + q / ws) * Wp + ww + q % ws];
                for (int k = 0; k < N; ++k) {
                    const int kr =
                            regions[(size_t)(wh + k / ws) * Wp + ww + k % ws];
    ...
    ggml_tensor *swin_block(ggml_tensor *tokens,
                            int H,
                            int W,
                            int stage,
                            int block,
                            SwinTaps *taps = nullptr) {
        const std::string p = "bb_layers_" + std::to_string(stage) +
                              "_blocks_" + std::to_string(block) + "_";
        ...
        const std::vector<float> *table = weights.get_f32(
                (p + "attn_relative_position_bias_table").c_str());
        const std::vector<float> *index =
                weights.get_f32((p + "attn_relative_position_index").c_str());
        if (!table || !index || index->size() != (size_t)N * N ||
            table->size() % 529 != 0) {
            return nullptr;
        }
            ...
            for (int qi = 0; qi < N; ++qi)
                for (int ki = 0; ki < N; ++ki)
                    rpb[(size_t)ki + (size_t)N * (qi + (size_t)N * h)] =
                            (*table)[(size_t)(int)(*index)[(size_t)qi * N +
                                                           ki] *
                                             heads +
                                     h];
        ...
        ggml_tensor *args[] = {
                attention_input, qkv_gemm,  qkv_bias,
                proj_gemm,       proj_bias, rpb_tensor,
        };
        ggml_tensor *fused =
                ggml_custom_4d(ctx, GGML_TYPE_F32, C, L, 1, 1, args, 6, nullptr,
                               GGML_N_TASKS_MAX, nullptr);
        char fused_name[GGML_MAX_NAME];
        std::snprintf(fused_name, sizeof(fused_name),
        ...
        const bool can_fuse =
                use_cuda_custom && ggml_backend_supports_op(backend, fused);
        ...
        if (!can_fuse || need_debug_path) {
            ...
            ggml_tensor *windows =
                    ggml_custom_4d(ctx, GGML_TYPE_F32, C, N, nW, 1, pack_args,
                                   1, nullptr, GGML_N_TASKS_MAX, nullptr);
            char pack_name[GGML_MAX_NAME];
            std::snprintf(pack_name, sizeof(pack_name),
                          "rmbg_swin_pack_h%d_w%d_sh%d", H, W, shift);
            ggml_set_name(windows, pack_name);
            if (!use_backend_custom ||
                !ggml_backend_supports_op(backend, windows)) {
            ...
            ggml_tensor *q = ggml_custom_4d(ctx, GGML_TYPE_F32, hd, N, heads,
                                            nW, qkv_args, 2, nullptr,
                                            GGML_N_TASKS_MAX, nullptr);
            ggml_tensor *k = ggml_custom_4d(ctx, GGML_TYPE_F32, hd, N, heads,
                                            nW, qkv_args, 2, nullptr,
                                            GGML_N_TASKS_MAX, nullptr);
            ggml_tensor *v = ggml_custom_4d(ctx, GGML_TYPE_F32, hd, N, heads,
                                            nW, qkv_args, 2, nullptr,
                                            GGML_N_TASKS_MAX, nullptr);
            ...
            if (use_flash) {
                ...
                swin_mask = constant(GGML_TYPE_F16, {N, N, heads, nW}, mask_f16,
                                     "swin_mask");
            }
            ggml_tensor *k_attn = use_flash && flash_coop
                                          ? ggml_cast(ctx, k, GGML_TYPE_F16)
                                          : k;
            ggml_tensor *v_attn = use_flash && flash_coop
                                          ? ggml_cast(ctx, v, GGML_TYPE_F16)
                                          : v;
            ggml_tensor *attended = nullptr;
            if (use_flash) {
                ggml_tensor *attn = ggml_flash_attn_ext(
                        ctx, q, k_attn, v_attn, swin_mask,
                ...
                if (shift) {
                    ...
                    attn_scores =
                            ggml_add(ctx, attn_scores,
                                     constant(GGML_TYPE_F32, {N, N, 1, nW},
                                              strict_mask, "mask_strict"));
                }
            ...
            if (taps && stage == 0 && block == 0)
                taps->block0_attended_windows = windows;

            ggml_tensor *unpack_args[] = {windows};
            ggml_tensor *primitive_tokens =
                    ggml_custom_4d(ctx, GGML_TYPE_F32, C, L, 1, 1, unpack_args,
                                   1, nullptr, GGML_N_TASKS_MAX, nullptr);
            char unpack_name[GGML_MAX_NAME];
            std::snprintf(unpack_name, sizeof(unpack_name),
                          "rmbg_swin_unpack_h%d_w%d_sh%d", H, W, shift);
            ggml_set_name(primitive_tokens, unpack_name);
            if (!use_backend_custom ||
                !ggml_backend_supports_op(backend, primitive_tokens)) {
                ...
                ggml_tensor *flat_windows =
                        ggml_reshape_2d(ctx, windows, C, N * nW);
                primitive_tokens = ggml_get_rows(
                        ctx, flat_windows,
        ...
        if (taps && stage == 0 && block == 0)
            taps->block0_after_attention = tokens;
    ...
    ggml_tensor *patch_merge(ggml_tensor *tokens, int H, int W, int stage) {
        ...
        ggml_tensor *fused =
                ggml_custom_4d(ctx, GGML_TYPE_F32, C * 4, rows, 1, 1, args, 1,
                               nullptr, GGML_N_TASKS_MAX, nullptr);
        char fused_name[GGML_MAX_NAME];
        std::snprintf(fused_name, sizeof(fused_name),
                      "rmbg_patch_merge_h%d_w%d", H, W);
        ggml_set_name(fused, fused_name);
    ...
    SwinTaps backbone(ggml_tensor *input, int H, int W) {
        ...
        for (int stage = 0; stage < 4; ++stage) {
            ...
            taps.raw[stage] = x;
            ggml_tensor *normed =
                    layer_norm(x, "bb_norm" + std::to_string(stage) + "_");
            ggml_tensor *spatial =
                    normed ? tokens_to_spatial(normed, stage_h, stage_w)
                           : nullptr;
...
bool RmbgDeviceGraph::init(ggml_backend_t backend,
    ...
    impl_->encoder4 = b.encoder_4scale(impl_->input, input_size, input_size,
                                       impl_->encoder);
    if (!impl_->encoder.x1 || !impl_->encoder.x2 || !impl_->encoder.x3 ||
        !impl_->encoder.x4) {
        err = "failed to build Swin graph: missing weights or invalid tensor "
              "shape";
        return false;
    }

    if (weights.get_f32("sq0_conv_in_weight")) {
        impl_->alpha = b.decoder(impl_->input, impl_->encoder4, input_size,
                                 input_size);
        if (!impl_->alpha) {
            err = "failed to build decoder graph: missing weights or invalid "
                  "tensor shape";
            return false;
        }
    ...
    impl_->graph =
            ggml_new_graph_custom(impl_->ctx_compute, kMaxGraphNodes, false);
    if (impl_->alpha) {
        ...
        ggml_tensor *debug_outputs[] = {
                ...
                impl_->encoder.x4,
                impl_->encoder.patch,
                impl_->encoder.patch_pre_norm,
                impl_->encoder.block0,
                impl_->encoder.block0_norm1,
                impl_->encoder.block0_windows,
                impl_->encoder.block0_attended_windows,
                impl_->encoder.block0_after_attention,
        };
    ...
    impl_->static_buffer =
            ggml_backend_alloc_ctx_tensors(impl_->ctx_static, backend);
    if (!impl_->static_buffer) {
        err = "static backend buffer allocation failed";
        return false;
    }
    for (const StaticBlob &blob : b.blobs) {
        if (blob.bytes.size() != ggml_nbytes(blob.tensor)) {
            err = std::string("static tensor byte mismatch: ") +
                  ggml_get_name(blob.tensor);
            return false;
        }
        ggml_backend_tensor_set(blob.tensor, blob.bytes.data(), 0,
                                blob.bytes.size());
    }
    ...
    if (!impl_->allocator ||
        !ggml_gallocr_reserve(impl_->allocator, impl_->graph) ||
...
bool RmbgDeviceGraph::forward_encoder(const std::vector<float> &input_nchw,
                                      ...
                                      std::vector<float> &x4,
                                      std::string &err) {
    if (!impl_->graph || !impl_->input) {
        err = "graph not initialized";
        return false;
    }
    const size_t expected = (size_t)3 * impl_->input_size * impl_->input_size;
    if (input_nchw.size() != expected) {
        err = "input tensor size mismatch";
        return false;
    }
    ggml_backend_tensor_set(impl_->input, input_nchw.data(), 0,
                            expected * sizeof(float));
    if (ggml_backend_graph_compute(impl_->backend, impl_->graph) !=
        GGML_STATUS_SUCCESS) {
    ...
    auto read = [](ggml_tensor *tensor, std::vector<float> &out) {
        ...
        ggml_backend_tensor_get(tensor, out.data(), 0,
...
bool RmbgDeviceGraph::forward_swin_debug(const std::vector<float> &input_nchw,
    ...
    if (!forward_encoder(input_nchw, unused1, unused2, unused3, unused4, err))
        return false;
    auto read = [](ggml_tensor *tensor, std::vector<float> &out) {
        out.resize((size_t)ggml_nelements(tensor));
        ggml_backend_tensor_get(tensor, out.data(), 0,
...
bool RmbgDeviceGraph::forward_block0_debug(const std::vector<float> &input_nchw,
    ...
    read(impl_->encoder.block0_attended_windows, attended_window0,
         window_elements);
