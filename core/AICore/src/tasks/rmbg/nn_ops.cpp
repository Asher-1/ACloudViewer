...
float bilinear_sample(const std::vector<float>& in,
                      int C,
                      int H,
                      int W,
                      int c,
...
void conv2d_nchw(const std::vector<float>& in,
                 int N,
                 int C,
                 int H,
                 int W,
                        ...
                        for (int kh = 0; kh < KH; ++kh) {
                            for (int kw = 0; kw < KW; ++kw) {
                                ...
                                if (ih < 0 || ih >= H || iw < 0 || iw >= W)
                                    continue;
                                const size_t wi =
                                        (((size_t)oc * C + ic) * KH + kh) * KW +
                                        kw;
                                s += in[nchw_idx(n, ic, ih, iw, C, H, W)] *
                                     p.weight[wi];
                            }
                        }
                    }
...
void batch_norm2d_nchw(const std::vector<float>& in,
                       int N,
                       int C,
                       int H,
                       int W,
                       const BatchNorm2dParams& p,
                       std::vector<float>& out) {
    out.resize(in.size());
    for (int n = 0; n < N; ++n) {
        for (int c = 0; c < C; ++c) {
            const float scale =
                    p.weight[c] / std::sqrt(p.running_var[c] + p.eps);
...
void bilinear_resize_nchw(const std::vector<float>& in,
                          int N,
                          int C,
                          int H,
                          int W,
...
void concat_nchw_channel(const std::vector<float>& a,
                         int Ca,
    ...
    for (int c = 0; c < Ca; ++c)
        for (int h = 0; h < H; ++h)
            for (int w = 0; w < W; ++w)
                out[nchw_idx(0, c, h, w, C, H, W)] =
                        a[nchw_idx(0, c, h, w, Ca, H, W)];
    for (int c = 0; c < Cb; ++c)
        for (int h = 0; h < H; ++h)
            for (int w = 0; w < W; ++w)
                out[nchw_idx(0, Ca + c, h, w, C, H, W)] =
                        b[nchw_idx(0, c, h, w, Cb, H, W)];
}

void deform_conv2d_nchw(const std::vector<float>& in,
                        int N,
                        int C,
                        int H,
                        int W,
                    ...
                    for (int ic = 0; ic < C; ++ic) {
                        for (int kh = 0; kh < KH; ++kh) {
                            for (int kw = 0; kw < KW; ++kw) {
                                ...
                                const float off_y = offset[nchw_idx(
                                        n, 2 * k + 0, oh, ow, off_c, OH, OW)];
                                const float off_x = offset[nchw_idx(
                                        n, 2 * k + 1, oh, ow, off_c, OH, OW)];
                                const float mask = modulator[nchw_idx(
                                        n, k, oh, ow, mod_c, OH, OW)];
                                const float ih =
                                        (float)(oh * stride + kh - pad) + off_y;
                                const float iw =
                                        (float)(ow * stride + kw - pad) + off_x;
                                const float v = bilinear_sample(in, C, H, W, ic,
                                                                ih, iw);
                                const size_t wi =
                                        (((size_t)oc * C + ic) * KH + kh) * KW +
                                        kw;
...
bool load_conv2d(const WeightMap& w,
                 const std::string& prefix,
                 Conv2dParams& out,
                 std::string& err) {
    const std::vector<float>* wt = w.get_f32((prefix + "weight").c_str());
    if (!wt) {
        err = "missing " + prefix + "weight";
        return false;
    }
    ...
    out.weight = *wt;
    out.bias.clear();
    if (const std::vector<float>* b = w.get_f32((prefix + "bias").c_str()))
        out.bias = *b;
    return true;
}

bool load_batch_norm2d(const WeightMap& w,
                       const std::string& prefix,
                       BatchNorm2dParams& out,
                       std::string& err) {
    const std::vector<float>* wt = w.get_f32((prefix + "weight").c_str());
    ...
    if (!bs) {
        err = "missing " + prefix + "bias";
        return false;
    }
    if (!mn) {
        err = "missing " + prefix + "running_mean";
        return false;
    }
    if (!vr) {
        err = "missing " + prefix + "running_var";
        return false;
    }
    out.weight = *wt;
    out.bias = *bs;
...
bool load_deform_conv2d(const WeightMap& w,
                        const std::string& prefix,
                        DeformConv2dParams& out,
                        std::string& err) {
    out.padding = 0;
    if (!load_conv2d(w, prefix + "atrous_conv_offset_conv_", out.offset, err))
        return false;
    if (!load_conv2d(w, prefix + "atrous_conv_modulator_conv_", out.modulator,
                     err))
        return false;
    if (!load_conv2d(w, prefix + "atrous_conv_regular_conv_", out.regular, err))
        return false;
    if (out.regular.kh > 1) out.padding = out.regular.kh / 2;
    return true;
}

static bool aspp_branch(const std::vector<float>& in,
                        int N,
                        int C,
                        int H,
                        int W,
...
bool aspp_deform_branch_forward(const std::vector<float>& in,
                                int N,
                                int C,
                                int H,
                                int W,
...
bool aspp_deformable_forward(const std::vector<float>& in,
                             int N,
                             int C,
                             int H,
                             int W,
    ...
    if (!aspp_branch(in, N, C, H, W, w, prefix + "aspp1_", b1, err))
        return false;
    if (!aspp_branch(in, N, C, H, W, w, prefix + "aspp_deforms_0_", b0, err))
        return false;
    if (!aspp_branch(in, N, C, H, W, w, prefix + "aspp_deforms_1_", b1d, err))
        return false;
    if (!aspp_branch(in, N, C, H, W, w, prefix + "aspp_deforms_2_", b2, err))
        return false;
    ...
    if (!load_conv2d(w, prefix + "global_avg_pool_1_", gconv, err))
        return false;
    if (!load_batch_norm2d(w, prefix + "global_avg_pool_2_", gbn, err))
        return false;
    std::vector<float> g1;
    ...
    for (int bi = 0; bi < 5; ++bi) {
        ...
        for (int n = 0; n < N; ++n) {
            for (int c = 0; c < branch_c; ++c) {
                for (int h = 0; h < H; ++h) {
                    for (int w_ = 0; w_ < W; ++w_) {
                        cat[nchw_idx(n, bi * branch_c + c, h, w_, cat_c, H,
                                     W)] =
...
bool basic_dec_blk_forward(const std::vector<float>& in,
                           int N,
                           int C,
                           int H,
                           int W,
    ...
    if (!aspp_deformable_forward(x, N, cin.oc, H, W, w, prefix + "dec_att_",
                                 att, err))
        return false;
...
bool lateral_block_forward(const std::vector<float>& in,
                           int N,
                           int C,
                           int H,
                           int W,
...
bool simple_convs_forward(const std::vector<float>& in,
                          int N,
                          int C,
                          int H,
                          int W,
...
void image2patches_split_nchw(const std::vector<float>& image,
                              int H,
                              int W,
                              int patch_h,
                              int patch_w,
...
void add_nchw(const std::vector<float>& a,
              const std::vector<float>& b,
              int C,
              int H,
              int W,
...
bool gdt_attn_forward(const std::vector<float>& in,
                      int N,
                      int C,
                      int H,
                      int W,
                      const WeightMap& w,
                      const std::string& gdt_prefix,
...
bool squeeze_module_forward(const std::vector<float>& in,
                            int N,
                            int C,
                            int H,
                            int W,
