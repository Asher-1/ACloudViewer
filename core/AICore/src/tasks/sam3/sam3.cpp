...
/* Logging: 0=silent, 1=summary timing, 2=verbose progress. Routes through the
...
#define SAM3_LOG(level, ...)                          \
    do {                                              \
        if ((level) <= SAM3_LOG_LEVEL)                \
            AICORE_LOG_PRINT("[sam3] ", __VA_ARGS__); \
    } while (0)
...
struct sam3_hparams {
    ...
    int32_t n_img_tokens() const {
        return n_img_embd() * n_img_embd();
    }  // 5184
    int32_t vit_head_dim() const {
        return vit_embed_dim / vit_num_heads;
    }  // 64
    ...
    int32_t hiera_feat_size() const {
        int s = img_size / 4;
        int n_pools = (hiera_q_pool < hiera_num_stages) ? hiera_q_pool
                                                        : hiera_num_stages - 1;
...
struct sam3_text_encoder {
    ...
    // intentionally not loaded. In SAM3, VETextEncoder discards the pooled
    // output that text_projection operates on — only the full token sequence
    // (through resizer) is used for downstream fusion/decoding.
...
struct sam3_bpe_tokenizer {
    ...
    std::unordered_map<uint8_t, std::string>
            byte_encoder;  // byte → unicode UTF-8
...
struct sam3_state {
    ...
    int encode_img_size =
            0;  // effective img_size for encoding (0 = hp.img_size)
    ...
    // reuse ctx/cgraph/gallocr across frames. Reuse also keeps tensor
    // addresses stable, which is what lets ggml-cuda's CUDA-graph capture
    // (GGML_CUDA_GRAPHS=ON) actually hit instead of re-capturing every frame.
    // The graph references model weight tensors (model->ctx, persistent) and
    // its own input/output tensors (enc_ctx, owned here); it must be freed
    // before the model is. Do NOT read enc_fpn[i] after free.
    struct ggml_context* enc_ctx = nullptr;
    struct ggml_cgraph* enc_graph = nullptr;
    struct ggml_gallocr* enc_galloc = nullptr;
    int enc_img_size = 0;
    int enc_n_fpn = 0;
    struct ggml_tensor* enc_inp = nullptr;  // input image in enc_ctx
    struct ggml_tensor* enc_pe = nullptr;   // pos-embed input in enc_ctx
    struct ggml_tensor* enc_fpn[4] = {};

    // Cached SAM3 ViT encode graph: same rationale as enc_* above, but for
    // the SAM3 model type (which uses a different ViT architecture).  The two
    // cache sets are never active simultaneously because the model type is
    // fixed at load time, but they are kept separate for clarity.
    struct ggml_context* sam3_ctx = nullptr;
    struct ggml_cgraph* sam3_graph = nullptr;
...
struct sam3_tracker {
    ...
    // Per-tensor backend buffers allocated by sam3_encode_memory /
    // sam3_store_obj_ptr. Tracked here so they can be freed on tracker reset.
    std::vector<ggml_backend_buffer_t> owned_buffers;

    // Cached PE / RoPE data — pure functions of fixed hyperparameters, computed
    // once.
    ...
    std::vector<float>
            cached_axial_cis_reord;  // reordered axial CIS for RoPE Q

    // Cached propagate graph: the graph topology is a function of M_total
    // (number of prompt tokens), which depends on the number of selected memory
    // frames.  We cache one graph and rebuild only when M_total changes.
    // This keeps tensor addresses stable across frames, enabling CUDA graph
    // capture and eliminating per-frame graph build + alloc overhead.
    struct ggml_context* prop_ctx = nullptr;
    struct ggml_cgraph* prop_graph = nullptr;
    struct ggml_gallocr* prop_galloc = nullptr;
    int prop_M_total = -1;
    // Saved input tensor pointers (the graph references these by address)
    struct ggml_tensor* prop_inp_curr = nullptr;
    struct ggml_tensor* prop_inp_src_pos = nullptr;
    struct ggml_tensor* prop_inp_prompt = nullptr;
    struct ggml_tensor* prop_inp_prompt_pos = nullptr;
    struct ggml_tensor* prop_inp_rope_q = nullptr;
    struct ggml_tensor* prop_inp_rope_k = nullptr;
    struct ggml_tensor* prop_inp_sparse = nullptr;
    struct ggml_tensor* prop_inp_image_pe = nullptr;
    struct ggml_tensor* prop_inp_dense_emb = nullptr;
    struct ggml_tensor* prop_inp_trk_s0 = nullptr;
    struct ggml_tensor* prop_inp_trk_s1 = nullptr;
    // Saved output tensor pointers
    struct ggml_tensor* prop_out_masks = nullptr;
    struct ggml_tensor* prop_out_iou = nullptr;
    struct ggml_tensor* prop_out_obj = nullptr;
    struct ggml_tensor* prop_out_sam_tok = nullptr;
    struct ggml_tensor* prop_out_mask_toks = nullptr;

    // Cached memory-encoder graph (mask downsampler + pixel fusion + output
    // projection).  Topology is a pure function of (hparams, H), so it is
    // rebuilt only when H changes; reuse keeps tensor addresses stable so the
    // CUDA-graph capture (GGML_CUDA_GRAPHS=ON) can hit instead of re-capturing
    // every frame.  Freed by sam3_tracker_reset.
    struct ggml_context* mem_ctx = nullptr;
    struct ggml_cgraph* mem_graph = nullptr;
    struct ggml_gallocr* mem_galloc = nullptr;
    int mem_H = -1;
    struct ggml_tensor* mem_inp_mask = nullptr;  // [INTERPOL, INTERPOL, 1, 1]
    ...
    std::vector<float> mem_tpos_enc;    // [MD * num_maskmem]
...
// Resolve effective img_size / feat_size from state (which may override hp
// defaults).
...
static ggml_backend_t sam3_backend_init(sam3_device device, bool use_gpu) {
    ...
    if (!dev) {
        AICORE_LOG_ERROR("[sam3] ", "%s: no CPU backend registered\n",
                         __func__);
        return nullptr;
    }
...
static bool sam3_graph_compute(ggml_backend_t backend,
    ...
    if (status != GGML_STATUS_SUCCESS) {
        AICORE_LOG_ERROR("[sam3] ", "%s: graph compute failed: %s\n", __func__,
...
static void sam3_name_tensorf(struct ggml_tensor* t,
                              const char* fmt,
                              int index) {
    if (!t) {
        return;
...
static void sam3_read_f32(struct ggml_tensor* t, float* dst, int64_t n) {
    if (t->type == GGML_TYPE_F32) {
        ...
        AICORE_LOG_ERROR("[sam3] ", "%s: unsupported tensor type %d\n",
                         __func__, (int)t->type);
    }
}
...
static void sam3_init_byte_encoder(
...
static inline std::string sam3_merge_key(const std::string& a,
                                         const std::string& b) {
    std::string k;
...
// Tolerant GGUF KV access — same interface as the other ggml-based projects
// (free-splatter.cpp, OpenPCDet-GGML): batch reads through one `gguf_kv`
// object; errors accumulate in `err` and are checked once by the caller.
// `required` keys that are absent set err; hparams keys are read with
// required=false and keep their defaults, exactly like the old loader.
struct gguf_kv {
    const struct gguf_context* g;
    std::string& err;
    ...
    void i32(const char* key, int32_t& out, bool required = true) const {
        int64_t id = find(key, required);
        if (id >= 0) out = gguf_get_val_i32(g, id);
    }
    ...
    void str_arr(const char* key,
        ...
        if (gguf_get_kv_type(g, id) != GGUF_TYPE_ARRAY ||
            ...
            if (err.empty())
                err = std::string("expected string GGUF array: ") + key;
            return;
        }
        const size_t count = (size_t)gguf_get_arr_n(g, id);
        out.resize(count);
        for (size_t i = 0; i < count; ++i) out[i] = gguf_get_arr_str(g, id, i);
    }
    void i32_arr(const char* key,
        ...
        if (gguf_get_kv_type(g, id) != GGUF_TYPE_ARRAY ||
            ...
            if (err.empty())
                err = std::string("expected int32 GGUF array: ") + key;
            return;
        }
        const size_t count = (size_t)gguf_get_arr_n(g, id);
        out.resize(count);
        const int32_t* a = (const int32_t*)gguf_get_arr_data(g, id);
        std::copy(a, a + count, out.begin());
    }
};

/*
** ── Load embedded BPE tokenizer from GGUF ───────────────────────────────────
*/

// The vocab is stored as a string array indexed by token id
// (sam3.tokenizer.vocab), the merges as "a b" strings in rank order
// (sam3.tokenizer.merges).
static bool sam3_load_bpe_vocab_from_gguf(const gguf_kv& r,
                                          sam3_bpe_tokenizer& tok) {
    std::vector<std::string> vocab, merges;
    r.str_arr("sam3.tokenizer.vocab", vocab, true);
    r.str_arr("sam3.tokenizer.merges", merges, true);
    if (!r.err.empty()) {
        AICORE_LOG_PRINT("[sam3] ", "%s: %s\n", __func__, r.err.c_str());
        return false;
    }
    if (vocab.empty() || merges.empty()) {
        AICORE_LOG_ERROR(
                "[sam3] ",
                "%s: tokenizer KV missing (sam3.tokenizer.vocab/merges)\n",
                __func__);
        return false;
    }
    ...
    tok.encoder.clear();
    tok.decoder.clear();
    tok.encoder.reserve(vocab.size());
    tok.decoder.reserve(vocab.size());
    for (size_t i = 0; i < vocab.size(); ++i) {
        tok.encoder[vocab[i]] = (int)i;
        tok.decoder[(int)i] = vocab[i];
    }

    tok.merges.clear();
    tok.merge_ranks.clear();
    tok.merges.reserve(merges.size());
    for (size_t i = 0; i < merges.size(); ++i) {
        const char* line = merges[i].c_str();
        const char* sep = strchr(line, ' ');
        if (!sep) {
            AICORE_LOG_ERROR("[sam3] ", "%s: malformed merge '%s'\n", __func__,
                             line);
            return false;
        }
        std::string a(line, sep);
        std::string b(sep + 1);
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: loaded %zu vocab entries, %zu merges\n",
...
static std::string sam3_bpe_encode(sam3_bpe_tokenizer& tok,
    ...
    while (true) {
        ...
        for (size_t i = 0; i + 1 < word.size(); ++i) {
            auto it =
                    tok.merge_ranks.find(sam3_merge_key(word[i], word[i + 1]));
        ...
        for (size_t i = 0; i < word.size();) {
            if (i + 1 < word.size() && word[i] == best_first &&
                word[i + 1] == best_second) {
...
// Overwrite an int32 array field of hp from KV when present (tolerant:
// absent arrays keep the hparams default). The gguf_kv reader itself lives
// above, next to the tokenizer.
#define SAM3_KV_HP_ARR(hp, field, n)                              \
    do {                                                          \
        std::vector<int32_t> _a;                                  \
        r.i32_arr("sam3.hparams." #field, _a, false);             \
        for (int _i = 0; _i < (n) && _i < (int)_a.size(); ++_i) { \
            (hp).field[_i] = _a[_i];                              \
        }                                                         \
    } while (0)

static bool sam3_load_hparams(const gguf_kv& r, sam3_hparams& hp) {
    auto rd = [&](const char* name, int32_t def) {
        int32_t v = def;
        r.i32(name, v, false);
        return v;
    };
    hp.img_size = rd("sam3.hparams.img_size", hp.img_size);
    hp.patch_size = rd("sam3.hparams.patch_size", hp.patch_size);
    hp.vit_embed_dim = rd("sam3.hparams.vit_embed_dim", hp.vit_embed_dim);
    hp.vit_depth = rd("sam3.hparams.vit_depth", hp.vit_depth);
    hp.vit_num_heads = rd("sam3.hparams.vit_num_heads", hp.vit_num_heads);
    const int32_t mlp_ratio_x1000 =
            rd("sam3.hparams.vit_mlp_ratio_x1000", 4625);
    hp.vit_mlp_dim = static_cast<int32_t>(hp.vit_embed_dim *
                                          (mlp_ratio_x1000 / 1000.0f));
    hp.vit_window_size = rd("sam3.hparams.vit_window_size", hp.vit_window_size);
    hp.n_global_attn = rd("sam3.hparams.n_global_attn", hp.n_global_attn);
    SAM3_KV_HP_ARR(hp, global_attn_idx, 4);
    hp.text_width = rd("sam3.hparams.text_width", hp.text_width);
    hp.text_heads = rd("sam3.hparams.text_heads", hp.text_heads);
    hp.text_layers = rd("sam3.hparams.text_layers", hp.text_layers);
    hp.text_ctx_len = rd("sam3.hparams.text_ctx_len", hp.text_ctx_len);
    hp.text_vocab_size = rd("sam3.hparams.text_vocab_size", hp.text_vocab_size);
    hp.text_out_dim = rd("sam3.hparams.text_out_dim", hp.text_out_dim);
    hp.neck_dim = rd("sam3.hparams.neck_dim", hp.neck_dim);
    hp.fenc_layers = rd("sam3.hparams.fenc_layers", hp.fenc_layers);
    hp.fenc_heads = rd("sam3.hparams.fenc_heads", hp.fenc_heads);
    hp.fenc_ffn_dim = rd("sam3.hparams.fenc_ffn_dim", hp.fenc_ffn_dim);
    hp.ddec_layers = rd("sam3.hparams.ddec_layers", hp.ddec_layers);
    hp.ddec_heads = rd("sam3.hparams.ddec_heads", hp.ddec_heads);
    hp.ddec_ffn_dim = rd("sam3.hparams.ddec_ffn_dim", hp.ddec_ffn_dim);
    hp.ddec_num_queries =
            rd("sam3.hparams.ddec_num_queries", hp.ddec_num_queries);
    hp.geom_layers = rd("sam3.hparams.geom_layers", hp.geom_layers);
    hp.n_presence_tokens =
            rd("sam3.hparams.n_presence_tokens", hp.n_presence_tokens);
    hp.n_geom_queries = rd("sam3.hparams.n_geom_queries", hp.n_geom_queries);
    hp.sam_embed_dim = rd("sam3.hparams.sam_embed_dim", hp.sam_embed_dim);
    hp.sam_dec_depth = rd("sam3.hparams.sam_dec_depth", hp.sam_dec_depth);
    hp.sam_n_multimask = rd("sam3.hparams.sam_n_multimask", hp.sam_n_multimask);
    hp.sam_iou_head_depth =
            rd("sam3.hparams.sam_iou_head_depth", hp.sam_iou_head_depth);
    hp.mem_out_dim = rd("sam3.hparams.mem_out_dim", hp.mem_out_dim);
...
static bool sam2_load_hparams(const gguf_kv& r, sam3_hparams& hp) {
    ...
    hp.img_size = rd("sam3.hparams.img_size", hp.img_size);
    hp.backbone_type = rd("sam3.hparams.backbone_type", 1);  // 1 = hiera

    hp.hiera_embed_dim = rd("sam3.hparams.hiera_embed_dim", hp.hiera_embed_dim);
    hp.hiera_num_heads = rd("sam3.hparams.hiera_num_heads", hp.hiera_num_heads);
    hp.hiera_num_stages =
            rd("sam3.hparams.hiera_num_stages", hp.hiera_num_stages);
    SAM3_KV_HP_ARR(hp, hiera_stages, 4);
    hp.hiera_global_n = rd("sam3.hparams.hiera_global_n", hp.hiera_global_n);
    SAM3_KV_HP_ARR(hp, hiera_global_idx, 8);
    hp.hiera_q_pool = rd("sam3.hparams.hiera_q_pool", hp.hiera_q_pool);
    SAM3_KV_HP_ARR(hp, hiera_window_spec, 4);
    hp.hiera_pos_embed_bkg_h =
            rd("sam3.hparams.hiera_pos_embed_bkg_h", hp.hiera_pos_embed_bkg_h);
    hp.hiera_pos_embed_bkg_w =
            rd("sam3.hparams.hiera_pos_embed_bkg_w", hp.hiera_pos_embed_bkg_w);
    hp.scalp = rd("sam3.hparams.scalp", hp.scalp);

    hp.neck_dim = rd("sam3.hparams.neck_dim", hp.neck_dim);
    hp.fpn_top_down_n = rd("sam3.hparams.fpn_top_down_n", hp.fpn_top_down_n);
    SAM3_KV_HP_ARR(hp, fpn_top_down_levels, 4);
    ...
    hp.sigmoid_scale_x100 =
            rd("sam3.hparams.sigmoid_scale_x100", hp.sigmoid_scale_x100);
    hp.sigmoid_bias_x100 =
            rd("sam3.hparams.sigmoid_bias_x100", hp.sigmoid_bias_x100);

    hp.use_high_res_features =
            rd("sam3.hparams.use_high_res_features", hp.use_high_res_features);
    hp.use_obj_ptrs_in_encoder = rd("sam3.hparams.use_obj_ptrs_in_encoder",
                                    hp.use_obj_ptrs_in_encoder);
    hp.pred_obj_scores = rd("sam3.hparams.pred_obj_scores", hp.pred_obj_scores);
    hp.use_multimask_token_for_obj_ptr =
            rd("sam3.hparams.use_multimask_token_for_obj_ptr",
               hp.use_multimask_token_for_obj_ptr);
    hp.directly_add_no_mem_embed = rd("sam3.hparams.directly_add_no_mem_embed",
                                      hp.directly_add_no_mem_embed);
    hp.non_overlap_masks_for_mem_enc =
            rd("sam3.hparams.non_overlap_masks_for_mem_enc",
               hp.non_overlap_masks_for_mem_enc);
    hp.binarize_mask_from_pts = rd("sam3.hparams.binarize_mask_from_pts",
                                   hp.binarize_mask_from_pts);
    hp.multimask_output_for_tracking =
            rd("sam3.hparams.multimask_output_for_tracking",
               hp.multimask_output_for_tracking);
    hp.multimask_min_pt_num =
            rd("sam3.hparams.multimask_min_pt_num", hp.multimask_min_pt_num);
    hp.multimask_max_pt_num =
            rd("sam3.hparams.multimask_max_pt_num", hp.multimask_max_pt_num);
    hp.fixed_no_obj_ptr =
            rd("sam3.hparams.fixed_no_obj_ptr", hp.fixed_no_obj_ptr);
    hp.iou_prediction_use_sigmoid =
            rd("sam3.hparams.iou_prediction_use_sigmoid",
               hp.iou_prediction_use_sigmoid);
    hp.use_mask_input_as_output = rd("sam3.hparams.use_mask_input_as_output",
                                     hp.use_mask_input_as_output);
    hp.multimask_output_in_sam = rd("sam3.hparams.multimask_output_in_sam",
                                    hp.multimask_output_in_sam);
    hp.is_sam2_1 = rd("sam3.hparams.is_sam2_1", hp.is_sam2_1);
...
static void sam2_precompute_hiera_metadata(sam3_model& model) {
    ...
    for (int s = 0; s < hp.hiera_num_stages - 1 &&
                    (int)q_pool_blocks.size() < hp.hiera_q_pool;
         ++s) {
    ...
    int cur_stage =
            0;  // Python starts at 1, but we use 0-indexed with same logic
...
static void sam2_register_tensors(sam3_model& model) {
    ...
    auto T2 = [&](const std::string& name, int64_t d0,
                  int64_t d1) -> ggml_tensor* {
    ...
    auto T2f = [&](const std::string& name, int64_t d0,
                   int64_t d1) -> ggml_tensor* {
    ...
    auto T3f = [&](const std::string& name, int64_t d0, int64_t d1,
                   int64_t d2) -> ggml_tensor* {
        auto* t = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, d0, d1, d2);
        ggml_set_name(t, name.c_str());
        tensors[name] = t;
        return t;
    };
    auto T4 = [&](const std::string& name, int64_t d0, int64_t d1, int64_t d2,
                  int64_t d3) -> ggml_tensor* {
        const ggml_type type = (d0 % WBLK == 0) ? WTYPE : GGML_TYPE_F32;
    ...
    auto T4f = [&](const std::string& name, int64_t d0, int64_t d1, int64_t d2,
                   int64_t d3) -> ggml_tensor* {
    ...
    // PyTorch pos_embed [1, E, bkg_h, bkg_w] → ggml reversed [bkg_w, bkg_h, E,
    // 1]
    model.hiera.pos_embed = T4f("hiera.pos_embed", hp.hiera_pos_embed_bkg_w,
                                hp.hiera_pos_embed_bkg_h, E0, 1);
    // PyTorch pos_embed_window [1, E, ws, ws] → ggml reversed [ws, ws, E, 1]
    model.hiera.pos_embed_window =
            T4f("hiera.pos_embed_window", hp.hiera_window_spec[0],
                hp.hiera_window_spec[0], E0, 1);
    ...
    // backbone_channel_list = [stage3_dim, stage2_dim, stage1_dim, stage0_dim]
    // (reversed) convs[0] maps the highest-dim (stage 3), convs[3] maps the
    // lowest-dim (stage 0)
    for (int i = 0; i < 4; ++i) {
    ...
    for (int i = 0; i < 4; ++i)
        model.sam_pe.point_embed[i] =
                T2f("sam_pe.point_embeddings." + std::to_string(i) + ".weight",
                    D, 1);
    model.sam_pe.not_a_point_embed =
            T2f("sam_pe.not_a_point_embed.weight", D, 1);
    ...
    if (hp.pred_obj_scores) {
        model.sam_dec.obj_score_token =
                T2f("sam_dec.obj_score_token.weight", D, 1);
    }
    ...
    for (int i = 0; i < hp.sam_dec_depth; ++i) {
        ...
        auto reg_attn = [&](sam3_sam_attn& a, const std::string& pfx,
    ...
    for (int m = 0; m < 4; ++m) {
        for (int j = 0; j < 3; ++j) {
            ...
            auto bp = "sam_dec.hyper." + std::to_string(m) + ".layers." +
                      std::to_string(j);
    ...
    for (int s = 0; s < 4; ++s) {
        ...
        model.mem_enc.ds_conv_w[s] = T4("mem_enc.ds." + si + ".weight", 3, 3,
                                        ds_channels[s], ds_channels[s + 1]);
        model.mem_enc.ds_conv_b[s] =
                T1f("mem_enc.ds." + si + ".bias", ds_channels[s + 1]);
        auto ni = std::to_string(norm_indices[s]);
        model.mem_enc.ds_norm_w[s] =
                T1f("mem_enc.ds." + ni + ".weight", ds_channels[s + 1]);
        model.mem_enc.ds_norm_b[s] =
                T1f("mem_enc.ds." + ni + ".bias", ds_channels[s + 1]);
    }
...
static void sam3_register_tensors(sam3_model& model) {
    ...
    auto T3 = [&](const std::string& name, int64_t d0, int64_t d1,
                  int64_t d2) -> ggml_tensor* {
        const ggml_type type = (d0 % WBLK == 0) ? WTYPE : GGML_TYPE_F32;
    ...
    model.vit.patch_embed_w = T4("vit.patch_embed.proj.weight", hp.patch_size,
                                 hp.patch_size, 3, E);
    // pos_embed: Hiera stores [1, 24, 24, 1024] at pretrained resolution (no
    // cls token). Conversion script writes reversed dims → ggml [E, 24, 24, 1].
    // Tiled 3x at runtime to [E, 72, 72, 1].
    {
        const int pretrained_grid =
                hp.img_size / hp.patch_size / 3;  // 1008/14/3 = 24
        model.vit.pos_embed =
                T4f("vit.pos_embed", E, pretrained_grid, pretrained_grid, 1);
    }
    ...
    for (int i = 0; i < hp.vit_depth; ++i) {
        ...
        int64_t rope_n = hp.is_global_attn(i)
                                 ? hp.n_img_tokens()
                                 : (hp.vit_window_size * hp.vit_window_size);
        blk.freqs_cis = T3f(p + ".attn.freqs_cis", 2, 32, rope_n);
    }

    // ── Neck (detector + tracker) ────────────────────────────────────────
    // ggml weight layout: conv2d [kW, kH, Cin, Cout], conv_transpose [kW, kH,
    // Cout, Cin]
    auto register_neck = [&](sam3_neck& neck, const std::string& prefix) {
        // scale 0 (4x): ConvTranspose(E→512, k=2, s=2), GELU,
        // ConvTranspose(512→D, k=2, s=2), Conv1x1(D→D), Conv3x3(D→D)
        neck.scales[0].deconv1_w = T4(prefix + "0.dconv_2x2_0.weight", 2, 2,
                                      512, E);  // [kW, kH, Cout=512, Cin=E]
        neck.scales[0].deconv1_b = T1f(prefix + "0.dconv_2x2_0.bias", 512);
        neck.scales[0].deconv2_w = T4(prefix + "0.dconv_2x2_1.weight", 2, 2, D,
                                      512);  // [kW, kH, Cout=D, Cin=512]
        neck.scales[0].deconv2_b = T1f(prefix + "0.dconv_2x2_1.bias", D);
        neck.scales[0].conv1x1_w =
                T4(prefix + "0.conv_1x1.weight", 1, 1, D, D);  // Conv2d(D→D)
        neck.scales[0].conv1x1_b = T1f(prefix + "0.conv_1x1.bias", D);
        neck.scales[0].conv3x3_w =
                T4(prefix + "0.conv_3x3.weight", 3, 3, D, D);  // Conv2d(D→D)
        neck.scales[0].conv3x3_b = T1f(prefix + "0.conv_3x3.bias", D);

        // scale 1 (2x): ConvTranspose(E→512, k=2, s=2), Conv1x1(512→D),
        // Conv3x3(D→D)
        neck.scales[1].deconv1_w = T4(prefix + "1.dconv_2x2.weight", 2, 2, 512,
                                      E);  // ConvTranspose
        neck.scales[1].deconv1_b = T1f(prefix + "1.dconv_2x2.bias", 512);
        neck.scales[1].conv1x1_w = T4(prefix + "1.conv_1x1.weight", 1, 1, 512,
                                      D);  // Conv2d(512→D): Cin=512, Cout=D
        ...
        neck.scales[2].conv1x1_w = T4(prefix + "2.conv_1x1.weight", 1, 1, E,
                                      D);  // Conv2d(E→D): Cin=E, Cout=D
    ...
    auto reg = [&](const std::string& n, int64_t d0, int64_t d1,
                   bool is_f32 = false) {
        const ggml_type rtype =
                (is_f32 || d0 % WBLK != 0) ? GGML_TYPE_F32 : WTYPE;
    ...
    auto reg4 = [&](const std::string& n, int64_t d0, int64_t d1, int64_t d2,
                    int64_t d3) {
    ...
    if (!hp.visual_only) {
        ...
        model.text_enc.token_embed_w =
                T2f("text.token_embed.weight", TW, hp.text_vocab_size);
        ...
        model.text_enc.resizer_w =
                T2("text.resizer.weight", TW, hp.text_out_dim);
        ...
        // script skips it and the loader rejects unknown tensors. See struct
        // comment.
        ...
        // Reference points, norms, bbox embed, ref_point_head, boxRPB,
        // presence_head These use the exact checkpoint names after renaming
        tensors["ddec.reference_points.weight"] =
                ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 4, NQ);
        ggml_set_name(tensors["ddec.reference_points.weight"],
                      "ddec.reference_points.weight");
        ...
        tensors["ddec.ref_point_head.layers.0.weight"] =
                ggml_new_tensor_2d(ctx, WTYPE, 512, D);
        tensors["ddec.ref_point_head.layers.0.bias"] =
                ggml_new_tensor_1d(ctx, GGML_TYPE_F32, D);
        tensors["ddec.ref_point_head.layers.1.weight"] =
                ggml_new_tensor_2d(ctx, WTYPE, D, D);
        tensors["ddec.ref_point_head.layers.1.bias"] =
                ggml_new_tensor_1d(ctx, GGML_TYPE_F32, D);
        for (auto& kv :
             std::vector<std::string>{"ddec.ref_point_head.layers.0.weight",
        ...
        for (const auto& axis : {"x", "y"}) {
            ...
            tensors[bp + ".layers.0.weight"] = ggml_new_tensor_2d(
                    ctx, (2 % WBLK == 0) ? WTYPE : GGML_TYPE_F32, 2, D);
            tensors[bp + ".layers.0.bias"] =
                    ggml_new_tensor_1d(ctx, GGML_TYPE_F32, D);
            tensors[bp + ".layers.1.weight"] =
                    ggml_new_tensor_2d(ctx, WTYPE, D, hp.ddec_heads);
            tensors[bp + ".layers.1.bias"] =
                    ggml_new_tensor_1d(ctx, GGML_TYPE_F32, hp.ddec_heads);
            for (int j = 0; j < 2; ++j) {
        ...
        tensors["ddec.presence_token_out_norm.weight"] =
                ggml_new_tensor_1d(ctx, GGML_TYPE_F32, D);
        tensors["ddec.presence_token_out_norm.bias"] =
                ggml_new_tensor_1d(ctx, GGML_TYPE_F32, D);
        ...
        model.geom_enc.point_proj_w =
                T2("geom.points_direct_project.weight", 2, D);
        model.geom_enc.point_proj_b = T1f("geom.points_direct_project.bias", D);
        model.geom_enc.box_proj_w =
                T2("geom.boxes_direct_project.weight", 4, D);
        model.geom_enc.box_proj_b = T1f("geom.boxes_direct_project.bias", D);
        // Pooling projections
        model.geom_enc.point_pool_proj_w =
                T2("geom.points_pool_project.weight", D, D);
        model.geom_enc.point_pool_proj_b =
                T1f("geom.points_pool_project.bias", D);
        model.geom_enc.box_pool_proj_w =
                T4("geom.boxes_pool_project.weight", 7, 7, D, D);
        model.geom_enc.box_pool_proj_b = T1f("geom.boxes_pool_project.bias", D);
        // Positional encoding projections
        model.geom_enc.point_pos_proj_w =
                T2("geom.points_pos_enc_project.weight", D, D);
        model.geom_enc.point_pos_proj_b =
                T1f("geom.points_pos_enc_project.bias", D);
        model.geom_enc.box_pos_proj_w =
                T2("geom.boxes_pos_enc_project.weight", 258, D);
        model.geom_enc.box_pos_proj_b =
                T1f("geom.boxes_pos_enc_project.bias", D);
        // Label and CLS
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            model.seg_head.up_conv_w[i] =
                    T4("seg.pixel_decoder.conv_layers." + si + ".weight", 3, 3,
                       D, D);
            model.seg_head.up_conv_b[i] =
                    T1f("seg.pixel_decoder.conv_layers." + si + ".bias", D);
            model.seg_head.up_norm_w[i] =
                    T1f("seg.pixel_decoder.norms." + si + ".weight", D);
            model.seg_head.up_norm_b[i] =
                    T1f("seg.pixel_decoder.norms." + si + ".bias", D);
        }

        // Mask predictor (3-layer MLP: 256→256→256→256)
        for (int j = 0; j < 3; ++j) {
            auto bp =
                    "seg.mask_predictor.mask_embed.layers." + std::to_string(j);
            model.seg_head.mask_embed_w =
                    T2(bp + ".weight", D, D);  // overwritten but last one
            model.seg_head.mask_embed_b = T1f(bp + ".bias", D);
        }
        // Re-register properly: all 3 layers with unique names are already in
        // tensors map The struct only has one pointer — use the tensors map at
        // runtime For now, just ensure all 6 tensors are registered (they are
        // via the loop above — each T2/T1f call registers under unique names)

        // Cross-attention to prompt
        model.seg_head.ca_prompt_q_w =
                T2("seg.cross_attend_prompt.in_proj_weight", D, 3 * D);
        model.seg_head.ca_prompt_q_b =
                T1f("seg.cross_attend_prompt.in_proj_bias", 3 * D);
        model.seg_head.ca_prompt_out_w =
                T2("seg.cross_attend_prompt.out_proj.weight", D, D);
        model.seg_head.ca_prompt_out_b =
                T1f("seg.cross_attend_prompt.out_proj.bias", D);
    ...
    // final attention
    auto reg_sam_attn = [&](sam3_sam_attn& a, const std::string& pfx,
                            int in_dim, int out_dim) {
        a.q_w = T2(pfx + ".q_proj.weight", in_dim, out_dim);
        a.q_b = T1f(pfx + ".q_proj.bias", out_dim);
        a.k_w = T2(pfx + ".k_proj.weight", in_dim, out_dim);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        model.mem_enc.fuser_dw_w[i] =
                T4(p + ".dwconv.weight", 7, 7, 1, D);  // groups=256
        model.mem_enc.fuser_dw_b[i] = T1f(p + ".dwconv.bias", D);
        model.mem_enc.fuser_norm_w[i] = T1f(p + ".norm.weight", D);
        model.mem_enc.fuser_norm_b[i] = T1f(p + ".norm.bias", D);
        model.mem_enc.fuser_fc1_w[i] = T2(p + ".pwconv1.weight", D, 1024);
...
// ggml tensors (model.tensors). The registered tensor's type wins: the
// register_* functions may use a wider type than the file (e.g. 1x1 convs
// are registered as F32 even in F16 files because their row length is not
// block-aligned), so convert on load exactly like the legacy loader did.
// A mismatch that has no conversion path is a hard error.
static bool sam3_load_tensors_from_gguf(const char* path,
                                        struct gguf_context* gguf,
                                        struct ggml_context* gguf_ctx,
                                        sam3_model& model) {
    const int64_t n_tensors = gguf_get_n_tensors(gguf);
    if (n_tensors != (int64_t)model.tensors.size()) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: tensor count mismatch: file has %lld, model "
                         "registered %zu\n",
    ...
    for (auto& kv : model.tensors) {
        ...
        // Look up the file tensor — every registered tensor must exist in the
        // file
        struct ggml_tensor* src = ggml_get_tensor(gguf_ctx, name.c_str());
        if (!src) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: tensor '%s' not found in GGUF file\n",
                             __func__, name.c_str());
            fclose(f);
            return false;
        }
        // Resolve the tensor's offset within the GGUF data section.
        const int64_t src_id = gguf_find_tensor(gguf, name.c_str());
        if (src_id < 0) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: tensor '%s' not found in GGUF info\n",
                             __func__, name.c_str());
            fclose(f);
            return false;
        }
        const size_t src_offset = gguf_get_tensor_offset(gguf, src_id);
        // Element-count check: legacy .ggml files never validated per-dim
        // shapes (e.g. sam_pe.pe_gaussian is stored [128,2] while registered
        // as [2,128] — same element count, same flattened layout), so enforce
        // element-count equality, not per-dim equality.
        int64_t src_n_el = 1, dst_n_el = 1;
        for (int i = 0; i < GGML_MAX_DIMS; ++i) {
            src_n_el *= src->ne[i];
            dst_n_el *= tensor->ne[i];
        }
        if (src_n_el != dst_n_el) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: element-count mismatch for '%s' (file=%lld, "
                             "registered=%lld)\n",
                             __func__, name.c_str(), (long long)src_n_el,
                             (long long)dst_n_el);
            fclose(f);
            return false;
        }

        // Read the blob at data_offset + tensor->offset and upload to backend.
        // Convert when the file type differs from the registered type
        // (registered type wins), otherwise copy verbatim.
        const size_t nbytes = ggml_nbytes(src);
        int64_t n_el = 1;
        for (int i = 0; i < GGML_MAX_DIMS; ++i) n_el *= src->ne[i];
        std::vector<char> buf(nbytes);
        if (fseek(f, (long)(data_offset + src_offset), SEEK_SET) != 0 ||
            fread(buf.data(), 1, nbytes, f) != nbytes) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: failed to read tensor '%s' at offset %zu\n",
                             __func__, name.c_str(), data_offset + src_offset);
            fclose(f);
            return false;
        }

        if (src->type == tensor->type) {
            ggml_backend_tensor_set(tensor, buf.data(), 0, nbytes);
        } else if (src->type == GGML_TYPE_F16 &&
                   tensor->type == GGML_TYPE_F32) {
            std::vector<float> f32_buf(n_el);
            ggml_fp16_to_fp32_row(
                    reinterpret_cast<const ggml_fp16_t*>(buf.data()),
                    f32_buf.data(), n_el);
            ggml_backend_tensor_set(tensor, f32_buf.data(), 0,
                                    n_el * sizeof(float));
            ...
            ggml_fp32_to_fp16_row(reinterpret_cast<const float*>(buf.data()),
                                  f16_buf.data(), n_el);
            ggml_backend_tensor_set(tensor, f16_buf.data(), 0,
                                    n_el * sizeof(ggml_fp16_t));
        } else if (ggml_is_quantized(src->type) &&
                   tensor->type == GGML_TYPE_F32) {
            ...
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: unsupported type conversion for '%s' "
                             "(file=%s, registered=%s)\n",
                             __func__, name.c_str(), ggml_type_name(src->type),
    ...
    fclose(f);
    AICORE_LOG_PRINT("[sam3] ", "%s: loaded %d tensors (registered %zu)\n",
                     __func__, n_loaded, model.tensors.size());
    return n_loaded == (int)model.tensors.size();
}
...
std::shared_ptr<sam3_model> sam3_load_model(const sam3_params& params) {
    AICORE_LOG_PRINT("[sam3] ", "%s: loading model from '%s'\n", __func__,
                     params.model_path.c_str());

    // ── Parse GGUF metadata (tensor data is streamed later) ──────────────
    // no_alloc=true: gguf_init only builds the tensor objects (names, shapes,
    ...
    // ── Arch + ftype ─────────────────────────────────────────────────────
    // Architecture comes from the standard `general.architecture` key (same
    // as free-splatter.cpp / OpenPCDet-GGML), falling back to the legacy
    // `sam3.arch` key written by older converters.
    std::string kv_err;
    gguf_kv r{gguf, kv_err};

    std::string arch = r.str("general.architecture", false);
    if (arch.empty()) arch = r.str("sam3.arch", false);
    if (arch.empty()) arch = "sam3";
    int32_t ftype = 1;
    r.i32("sam3.ftype", ftype, false);
    ...
    if (arch == "sam3") {
        ...
        AICORE_LOG_PRINT("[sam3] ", "%s: unknown architecture '%s'\n", __func__,
                         arch.c_str());
        gguf_free(gguf);
        ggml_free(gguf_ctx);
        return nullptr;
    }
    AICORE_LOG_PRINT("[sam3] ", "%s: GGUF %s (ftype %d, %lld tensors)\n",
    ...
    }
    {
        ...
        switch (ftype) {
            ...
            case 8:
                ...
                break;
            default:
                AICORE_LOG_ERROR("[sam3] ", "%s: unsupported ftype: %d\n",
                                 __func__, ftype);
                gguf_free(gguf);
                ggml_free(gguf_ctx);
                return nullptr;
    ...
    // ── Read hyperparameters from KV ─────────────────────────────────────
    if (is_sam2) {
        if (!sam2_load_hparams(r, model->hparams)) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: failed to read SAM2 hyperparameters\n",
                             __func__);
            gguf_free(gguf);
            ggml_free(gguf_ctx);
            return nullptr;
        ...
        if (!sam3_load_hparams(r, model->hparams)) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: failed to read SAM3 hyperparameters\n",
                             __func__);
            gguf_free(gguf);
            ggml_free(gguf_ctx);
            return nullptr;
    ...
    if (!model->ctx) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to init ggml context\n",
                         __func__);
        gguf_free(gguf);
        ggml_free(gguf_ctx);
        return nullptr;
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: registered %zu tensors\n", __func__,
                     model->tensors.size());

    // ── Allocate backend buffer for all tensors ──────────────────────────
    model->buffer = ggml_backend_alloc_ctx_tensors(model->ctx, model->backend);
    if (!model->buffer) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate tensor buffer\n",
                         __func__);
        gguf_free(gguf);
        ggml_free(gguf_ctx);
        return nullptr;
    ...
    AICORE_LOG_PRINT(
            "[sam3] ", "%s: buffer size = %.2f MB\n", __func__,
            ggml_backend_buffer_get_size(model->buffer) / (1024.0 * 1024.0));

    // ── Stream tensor data from the GGUF data section ────────────────────
    if (!sam3_load_tensors_from_gguf(params.model_path.c_str(), gguf, gguf_ctx,
                                     *model)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to load tensors\n", __func__);
        gguf_free(gguf);
        ggml_free(gguf_ctx);
        return nullptr;
    }
    ...
    if (!is_sam2 && !model->hparams.visual_only) {
        if (!sam3_load_bpe_vocab_from_gguf(r, model->tokenizer)) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: failed to load embedded tokenizer\n",
                             __func__);
            gguf_free(gguf);
            ggml_free(gguf_ctx);
            return nullptr;
        ...
        AICORE_LOG_ERROR("[sam3] ", "%s: SAM2 model — no tokenizer\n",
                         __func__);
    } else {
        AICORE_LOG_PRINT("[sam3] ",
                         "%s: visual-only model — skipping tokenizer\n",
                         __func__);
    }

    // GGUF metadata no longer needed — tensor blobs were streamed out
    gguf_free(gguf);
    ggml_free(gguf_ctx);

    AICORE_LOG_PRINT("[sam3] ", "%s: model loaded successfully\n", __func__);
    return model;
}
...
sam3_state_ptr sam3_create_state(const sam3_model& model,
    ...
    const auto& hp = model.hparams;
    int eis =
            (params.encode_img_size > 0) ? params.encode_img_size : hp.img_size;
    state->encode_img_size = eis;
    state->encode_feat_size = sam3_effective_feat_size(hp, eis);
    if (eis != hp.img_size) {
        AICORE_LOG_PRINT("[sam3] ",
                         "%s: encode_img_size=%d (model native=%d), "
                         "feat_size=%d (native=%d)\n",
                         __func__, eis, hp.img_size, state->encode_feat_size,
                         hp.feat_size());
    }
...
void sam3_free_state(sam3_state& state) {
    if (state.enc_galloc) {
        ggml_gallocr_free(state.enc_galloc);
        state.enc_galloc = nullptr;
    }
    if (state.enc_ctx) {
    ...
    // sam3_galloc and sam3_ctx may share pointers with galloc/ctx; NULL them
    // before freeing so double-free is impossible regardless of the order.
    state.sam3_galloc = nullptr;
    state.sam3_graph = nullptr;
    state.sam3_ctx = nullptr;
...
static void sam3_resize_bilinear(const uint8_t* src,
                                 int src_w,
                                 int src_h,
                                 uint8_t* dst,
                                 int dst_w,
                                 int dst_h) {
    // Bilinear resize matching torch.nn.functional.interpolate(bilinear,
    // align_corners=False). ALL arithmetic is in double to get an exact result
    // for uint8 inputs (0-255), ensuring the bilinear result is independent of
    // FMA/SIMD/compiler behavior. The exact double result is then rounded to
    // uint8, matching torch's round().
...
// Preprocess an image: resize to img_size × img_size, convert to float,
// normalize. Returns a float tensor in [C, H, W] layout (channel-first), range
// normalized with mean=0.5, std=0.5 → pixel values in [-1, 1].
static std::vector<float> sam3_preprocess_image(const sam3_image& image,
                                                int img_size) {
    const int C = 3;
    std::vector<float> result(C * img_size * img_size);

    // Resize to img_size × img_size via uint8 bilinear (matching torch
    // pipeline)
    ...
    // Convert to float [C, H, W] with normalization: (pixel / 255.0 - 0.5) /
    // 0.5
    for (int c = 0; c < C; ++c) {
        for (int y = 0; y < img_size; ++y) {
            for (int x = 0; x < img_size; ++x) {
                float v = pixels[(y * img_size + x) * 3 + c] / 255.0f;
                result[c * img_size * img_size + y * img_size + x] =
                        (v - 0.5f) / 0.5f;
            }
        }
    }
...
// Returns [C, H, W] float, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224,
// 0.225].
...
static void sam3_compute_axial_cis(float* out,
    ...
    for (int idx = 0; idx < N; ++idx) {
        ...
        int row =
                idx / end_x;  // intentional integer floor division (row index)
...
// num_pos_feats = d_model / 2 = 128, temperature = 10000, normalize = true,
// scale = 2pi. Returns data in ggml column-major layout for a tensor with ne =
// {d_model, W, H, 1}, i.e. element (c, w, h) at flat index c + w*d_model +
// h*d_model*W. First half channels (0..half-1) encode y, second half
// (half..d_model-1) encode x.
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            ...
            for (int i = 0; i < half; ++i) {
                int paired = i & ~1;  // 0,0,2,2,4,4,… (pairs sin/cos channels,
                                      // matches Python // 2)
                ...
                // element (c, x, y, 0) at flat index: c + x*d_model +
                // y*d_model*W First half channels are y, second half are x.
...
static void sam3_get_1d_sine_pe(float* out,
                                float pos_ind,
                                int dim,
...
//   ne[0] = embed_dim (E=1024), ne[1] = spatial W, ne[2] = spatial H, ne[3] =
//   batch

// Apply RoPE to Q and K tensors using complex multiplication.
// x shape: [head_dim, N, num_heads*B] in ggml layout
// freqs_cis shape: [2, 32, N] in ggml layout — stored as (cos,sin) interleaved
// pairs
//
...
static struct ggml_tensor* sam3_apply_rope(struct ggml_context* ctx,
    ...
    if (sam3_backend_is_cuda() && freqs_cis->type == GGML_TYPE_F32) {
        auto* fused = output_f32 && x->type == GGML_TYPE_F16
    ...
    if (x->type == GGML_TYPE_F16 && freqs_cis->type == GGML_TYPE_F32) {
        freqs_cis = ggml_cast(ctx, freqs_cis, GGML_TYPE_F16);
    }

    // freqs_cis: [2, 32, N] — dim0=2 (cos,sin), dim1=32 (half_head=head_dim/2),
    // dim2=N x: [head_dim, N, num_heads*B] — dim0=64, dim1=N, dim2=batch*heads
    ...
    // as nb1,nb2,nb3 of the view, so the view walks over (half, N, 1)
    // correctly.
    auto* cos_f = ggml_view_4d(ctx, fc, 1, half, N, 1, fc->nb[1], fc->nb[2],
                               fc->nb[3], 0);
    auto* sin_f = ggml_view_4d(ctx, fc, 1, half, N, 1, fc->nb[1], fc->nb[2],
                               fc->nb[3], fc->nb[0]);

    // Extract x_re (offset 0) and x_im (offset 1) from dim0.
    // x_pairs is [2, half, N, nheads_B] — same slicing logic.
    auto* x_re =
            ggml_view_4d(ctx, x_pairs, 1, half, N, nheads_B, x_pairs->nb[1],
                         x_pairs->nb[2], x_pairs->nb[3], 0);
    auto* x_im =
            ggml_view_4d(ctx, x_pairs, 1, half, N, nheads_B, x_pairs->nb[1],
                         x_pairs->nb[2], x_pairs->nb[3], x_pairs->nb[0]);
...
// CUDA's cuBLAS path computes F16-weight GEMMs in F16 and normally converts
// every result to F32. The ViT immediately consumes QKV/MLP intermediates in
// operations that support F16, so keep those results in F16 and only promote
// Q for flash attention. Residuals and layer norms remain F32.
static struct ggml_tensor* sam3_vit_mul_mat(struct ggml_context* ctx,
...
**
** ggml's flash-attention kernels only implement a fixed set of head
** dimensions, and the exact set differs per backend (CUDA: 40/64/72/80/
** 96/112/128/256/576; Metal: 16/32/40/48/56/64/72/80/96/112/128/192/256/
** 320/512/576 after the ggml-patches head-dim addition; Vulkan: shader
** driven). SAM2's mask decoder uses HD=32 (sam_embed_dim=256 / 8 heads)
** and the Hiera backbone uses HD=56, so calling ggml_flash_attn_ext
** directly aborts inside backends that don't implement the head size.
** Route every fattn call through this wrapper instead — we ask the active
** backend whether it supports the op (probe, cached), go through the fast
** path when it does, and otherwise fall back to a manual K^T·Q → softmax
** → V composition that produces the same [HD, NH, N_q, B] output layout.
*****************************************************************************/

// Ask the active backend whether it can execute flash_attn_ext with this
...
static bool sam3_fattn_hd_supported(ggml_backend_t backend, int64_t hd) {
    ...
    static probe_cache cache = {nullptr, {}};

    if (cache.backend != backend) {
        cache.backend = backend;
        memset(cache.tested, 0, sizeof(cache.tested));
    }
    if (cache.tested[hd]) {
        return cache.supported[hd];
    }

    // Minimal flash_attn_ext op in a throwaway context — Q F32 (as the real
    // graph produces it), K/V F16 (model weight type), no mask. no_alloc
    // means tensor data is never allocated (only object headers), so a few
    // KB of context memory covers the probe and the whole thing costs
    // microseconds.
    struct ggml_init_params ip = {4096, /*mem_buffer*/ nullptr,
                                  /*no_alloc*/ true};
    struct ggml_context* pctx = ggml_init(ip);
    struct ggml_tensor* q =
            ggml_new_tensor_4d(pctx, GGML_TYPE_F32, hd, 1, 1, 1);
    struct ggml_tensor* k =
            ggml_new_tensor_4d(pctx, GGML_TYPE_F16, hd, 1, 1, 1);
    struct ggml_tensor* v =
            ggml_new_tensor_4d(pctx, GGML_TYPE_F16, hd, 1, 1, 1);
    struct ggml_tensor* op =
            ggml_flash_attn_ext(pctx, q, k, v, nullptr, 1.0f, 0.0f, 0.0f);
    const bool supported = ggml_backend_supports_op(backend, op);
    ggml_free(pctx);

    cache.tested[hd] = true;
    cache.supported[hd] = supported;
    if (!supported) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: head_dim=%lld unsupported on this backend; using "
                         "manual SDPA\n",
...
static struct ggml_tensor* sam3_attn_ext(
        ...
        struct ggml_tensor*
                V,  // [HD, N_kv, NH, B]  (may be permuted/non-contig)
        struct ggml_tensor* mask,  // nullable, F16 additive mask
        float scale,
        float max_bias,
        float logit_softcap) {
    // ggml's CUDA flash-attention kernel selection only accepts masks with
    // ne[2] == 1, and the Vulkan shader path has the same per-head bias
    ...
    GGML_ASSERT(max_bias == 0.0f &&
                "sam3_attn_ext manual path: ALiBi not supported");
    GGML_ASSERT(logit_softcap == 0.0f &&
                "sam3_attn_ext manual path: logit_softcap not supported");
    ...
    // K^T·Q via ggml_mul_mat(K, Q): A^T·B with shared dim = ne0 = HD → [N_kv,
    // N_q, NH, B].
    ...
    auto* VT = ggml_cont(
            ctx, ggml_permute(ctx, Vc, 1, 0, 2, 3));  // [N_kv, HD, NH, B]
...
static struct ggml_tensor* sam3_win_part(
    ...
    struct ggml_tensor* p = (px == 0 && py == 0)
                                    ? a
                                    : ggml_pad(ctx, a, 0, (int)px, (int)py,
                                               0);  // [E, W', H', 1]
...
static struct ggml_tensor* sam3_win_unpart(
    ...
    auto* p = ggml_reshape_4d(ctx, a, E * w, w, npx,
                              npy);        // split np → (npx, npy)
    ...
    auto* view =
            ggml_view_4d(ctx, p, E, w0, h0, 1, p->nb[1], p->nb[2], p->nb[3], 0);
...
static inline struct ggml_tensor* sam3_win_unpart(struct ggml_context* ctx,
                                                  struct ggml_tensor* a,
                                                  int w0,
                                                  int h0,
                                                  int w) {
...
// AICore links every ggml backend into one library and picks the device at
// runtime, so the compile-time GPU guard above never fires. ggml-cuda and
// ggml-vulkan ship no POOL_1D kernel (only POOL_2D), so on those backends we
...
static inline struct ggml_tensor* sam3_global_mean_dim0(
        struct ggml_context* ctx, struct ggml_tensor* a, int len_dim0) {
    if (sam3_backend_needs_mean_dim0()) {
        return ggml_mean(ctx, a);
    }
...
// Single ViT block forward: pre-norm → attn (window or global, with RoPE) →
// residual → pre-norm → MLP → residual x: [E, W, H, B] in ggml layout
// (following sam.cpp convention)
static struct ggml_tensor* sam3_vit_block_forward(struct ggml_context* ctx,
    ...
    {
        auto* cur = sam3_vit_mul_mat(ctx, blk.qkv_w, x);
        auto* qkv_bias = cur->type == GGML_TYPE_F16
                                 ? ggml_cast(ctx, blk.qkv_b, GGML_TYPE_F16)
                                 : blk.qkv_b;
        ...
        // On CUDA, fuse QKV de-interleave, head layout, and Q/K RoPE into one
        // pass. The output is [HD, N, NH*B, 3], so all three views are already
        // contiguous in the layout consumed by flash attention.
        struct ggml_tensor* packed = nullptr;
        if (sam3_backend_is_cuda() && cur->type == GGML_TYPE_F16 &&
            blk.freqs_cis) {
            auto* candidate =
        ...
        if (packed) {
            const int64_t N = W_cur * H_cur;
            Q = ggml_view_3d(ctx, packed, HD, N, NH * B_cur, packed->nb[1],
                             packed->nb[2], 0);
            K = ggml_view_3d(ctx, packed, HD, N, NH * B_cur, packed->nb[1],
                             packed->nb[2], packed->nb[3]);
            V = ggml_view_3d(ctx, packed, HD, N, NH * B_cur, packed->nb[1],
                             packed->nb[2], 2 * packed->nb[3]);
            V = ggml_reshape_4d(ctx, V, HD, N, NH, B_cur);
        } else {
            // [3*E, W*H, B] -> [E, W*H, B, 3]
            cur = ggml_reshape_4d(ctx, cur, E, 3, W_cur * H_cur, B_cur);
            cur = ggml_cont(ctx, ggml_permute(ctx, cur, 0, 3, 1, 2));

            Q = ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur, cur->nb[1],
                             cur->nb[2], 0);
            K = ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur, cur->nb[1],
                             cur->nb[2], cur->nb[3]);
            V = ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur, cur->nb[1],
                             cur->nb[2], 2 * cur->nb[3]);
            ...
            if (blk.freqs_cis) {
                Q = sam3_apply_rope(ctx, Q, blk.freqs_cis, true);
                K = sam3_apply_rope(ctx, K, blk.freqs_cis);
            }
        }

        // ggml's CUDA flash-attention kernel requires F32 Q, while K and V
        // natively accept F16. This is the only promotion in the QKV path.
        ...
        auto* attn_out =
                sam3_attn_ext(ctx, Q, K, V, nullptr, scale, 0.0f, 0.0f);
    ...
    x = sam3_vit_mul_mat(ctx, blk.mlp_fc1_w, x);
    auto* fc1_bias = x->type == GGML_TYPE_F16
                             ? ggml_cast(ctx, blk.mlp_fc1_b, GGML_TYPE_F16)
                             : blk.mlp_fc1_b;
    x = ggml_add(ctx, x, fc1_bias);
...
static struct ggml_tensor* sam3_build_vit_prefix_graph(
    ...
    // pos_embed [E, 24, 24] is Hiera pretrained resolution — tile 3x3 to [E,
    // 72, 72]
...
// Input is the shared ViT output converted to conv layout [W, H, E, B].
...
static void sam3_build_neck_graph(struct ggml_context* ctx,
    ...
    // Scale 0 (4× upsample)
    {
        auto* s0 = ggml_conv_transpose_2d_p0(
                ctx, sam3_conv_transpose_weight(ctx, neck.scales[0].deconv1_w),
                x, 2);
        s0 = add_bias(s0, neck.scales[0].deconv1_b);
        s0 = ggml_gelu_erf(ctx, s0);
        s0 = ggml_conv_transpose_2d_p0(
                ctx, sam3_conv_transpose_weight(ctx, neck.scales[0].deconv2_w),
                s0, 2);
    ...
    // Scale 1 (2× upsample)
    {
        auto* s1 = ggml_conv_transpose_2d_p0(
                ctx, sam3_conv_transpose_weight(ctx, neck.scales[1].deconv1_w),
                x, 2);
...
// Marked as input — caller must upload data via ggml_backend_tensor_set after
// alloc.
static struct ggml_tensor* sam3_build_causal_mask(struct ggml_context* ctx,
                                                  int L) {
...
// Returns: [E, L]
static struct ggml_tensor* sam3_text_block_forward(
        struct ggml_context* ctx,
        struct ggml_tensor* x,
    ...
    V = ggml_permute(ctx, V, 0, 2, 1,
                     3);  // non-contiguous; flash_attn uses strides

    float scale = 1.0f / sqrtf((float)HD);
    auto* attn_out =
            sam3_attn_ext(ctx, Q, K, V, causal_mask, scale, 0.0f, 0.0f);
...
static struct ggml_tensor* sam3_build_text_encoder_graph(
        struct ggml_context* ctx,
...
// Bicubic interpolation of a [C, H_in, W_in] tensor on CPU to [C, H_out,
// W_out]. Used for background positional embedding interpolation.
static void sam2_bicubic_interpolate_cpu(const float* src,
                                         int C,
                                         int H_in,
                                         int W_in,
                                         float* dst,
                                         int H_out,
                                         int W_out) {
    // Keys cubic kernel with a = -0.75 (matching PyTorch's bicubic
    // interpolation) w(x) = (a+2)|x|^3 - (a+3)|x|^2 + 1,          |x| <= 1 w(x)
    ...
    // Precompute per-row and per-column interpolation weights: they depend only
    // on the output coordinate, not on the channel, so computing them once per
    // (y, x) instead of once per (c, y, x) removes C-1 redundant evaluations of
    // the cubic kernel (C = 768 for Hiera → ~768x fewer cubic() calls).
    std::vector<int> y_src(H_out * 4);
    std::vector<float> y_w(H_out * 4);
    for (int y = 0; y < H_out; ++y) {
        float fy = ((float)y + 0.5f) * (float)H_in / (float)H_out - 0.5f;
        int iy = (int)floorf(fy);
        float dy = fy - iy;
        for (int jj = -1; jj <= 2; ++jj) {
            y_src[y * 4 + (jj + 1)] = std::min(std::max(iy + jj, 0), H_in - 1);
            y_w[y * 4 + (jj + 1)] = cubic(dy - jj);
        }
    }
    std::vector<int> x_src(W_out * 4);
    std::vector<float> x_w(W_out * 4);
    for (int x = 0; x < W_out; ++x) {
        float fx = ((float)x + 0.5f) * (float)W_in / (float)W_out - 0.5f;
        int ix = (int)floorf(fx);
        float dx = fx - ix;
        for (int ii = -1; ii <= 2; ++ii) {
            x_src[x * 4 + (ii + 1)] = std::min(std::max(ix + ii, 0), W_in - 1);
            x_w[x * 4 + (ii + 1)] = cubic(dx - ii);
        }
    }

    const int H_outW_out = H_out * W_out;
    for (int c = 0; c < C; ++c) {
        const float* src_c = src + c * H_in * W_in;
        float* dst_c = dst + c * H_outW_out;
        for (int y = 0; y < H_out; ++y) {
            const int* ys = &y_src[y * 4];
            const float* wy = &y_w[y * 4];
            for (int x = 0; x < W_out; ++x) {
                const int* xs = &x_src[x * 4];
                const float* wx = &x_w[x * 4];
                float val = 0.0f;
                for (int jj = 0; jj < 4; ++jj) {
                    const float* row = src_c + ys[jj] * W_in;
                    const float wyj = wy[jj];
                    val += wyj * (wx[0] * row[xs[0]] + wx[1] * row[xs[1]] +
...
// Returns [E, H, W] in ggml column-major layout: element (e, x, y) at flat
// index e + x*E + y*E*W. PE = bicubic_interp(pos_embed_bkg, (H, W)) +
// tile(pos_embed_window, (H/W0, W/W0))
static std::vector<float> sam2_compute_pos_embed(const sam3_model& model,
                                                 int H,
                                                 int W) {
    const auto& hp = model.hparams;
    ...
    // The PE depends only on the model weights + (H, W), so it is invariant
    // across encodes of the same model at the same resolution. Recomputing it
    // per frame costs hundreds of ms (bicubic over E*H*W) plus two GPU→CPU and
    // one CPU→GPU round-trip — cache it. Keyed on the hparams that shape the
    // output (not the model pointer) so a model reload with the same layout
    // still reuses the entry safely.
    struct pe_cache_key {
        int E, H, W, bkg_h, bkg_w, ws;
        bool operator==(const pe_cache_key& o) const {
            return E == o.E && H == o.H && W == o.W && bkg_h == o.bkg_h &&
                   bkg_w == o.bkg_w && ws == o.ws;
        }
    };
    static pe_cache_key cache_key = {0, 0, 0, 0, 0, 0};
    static std::vector<float> cache_pe;
    if (cache_key == pe_cache_key{E, H, W, bkg_h, bkg_w, ws} &&
        !cache_pe.empty()) {
        return cache_pe;
    }
    cache_key = pe_cache_key{E, H, W, bkg_h, bkg_w, ws};

    // Read background PE from GPU.
    // Registered as [bkg_w, bkg_h, E, 1] in ggml; raw bytes match PyTorch NCHW
    // [1,E,H,W]. ggml flat index for ne=[W,H,E,1]: w + h*W + e*W*H = same as
    // CHW[e,h,w] = e*H*W + h*W + w. So we can use the raw data directly as CHW
    // layout for bicubic interpolation.
    ...
    // Read window PE from GPU: registered [ws, ws, E, 1], raw bytes = CHW [E,
    // ws, ws]
...
//   3. Cont
//   4. Reshape                 -> [C*ws, ws, nW_h, nW_w*B] -- split H into
//   windows
//   5. Permute (0,2,1,3)      -> [C*ws, nW_h, ws, nW_w*B] -- move nW_h before
//   local-H
//      ... NO!  Step 5 swaps local-H with window-H-index, corrupting the data.
//      After step 4, ne[1]=ws is already local-H and ne[2]=nW_h is the window
//      index. Skipping step 5 gives the correct layout where each window
//      contains contiguous spatial data.  Reshape directly to the final shape.
//   5. Reshape                 -> [C, ws, ws, nW*B]        -- split C*ws -> (C,
//   ws)
//
...
//   2. Reshape [C*ws, ws, nW_h, nW_w*B] -> [C*ws, Hp, nW_w, B]  (merge local-H
//   with nW_h)
...
static struct ggml_tensor* sam2_window_unpartition(struct ggml_context* ctx,
    ...
    //   Merges (local_H=ws, nW_h) into Hp=ws*nW_h; splits nW_w*B into (nW_w,
    //   B).
    ...
    if (Hp != orig_H || Wp != orig_W) {
        out = ggml_view_4d(ctx, out, C, orig_W, orig_H, B, out->nb[1],
                           out->nb[2], out->nb[3], 0);
...
static struct ggml_tensor* sam2_hiera_block_forward(struct ggml_context* ctx,
    ...
    normed = ggml_mul(
            ctx, normed,
            ggml_repeat(ctx, ggml_reshape_4d(ctx, blk.norm1_w, C_in, 1, 1, 1),
                        normed));
    normed = ggml_add(
            ctx, normed,
    ...
    if (C_in != C_out) {
        // Linear projection on normed input: [C_in, W, H, B] -> [C_out, W, H,
        // B]
        auto* flat =
                ggml_reshape_2d(ctx, normed, C_in, spatial_W * spatial_H * B);
        ...
        // Q-stride without dim change (unusual but possible for q_pool >
        // stages)
    ...
    if (blk.window_size > 0) {
        attn_input =
                sam2_window_partition(ctx, normed, blk.window_size, pad_hw);
    }
    ...
    auto* q = ggml_view_3d(ctx, qkv, C_out, N_kv, B_win, qkv->nb[1], qkv->nb[2],
                           0);
    auto* k = ggml_view_3d(ctx, qkv, C_out, N_kv, B_win, qkv->nb[1], qkv->nb[2],
                           C_out * ggml_type_size(qkv->type));
    auto* v = ggml_view_3d(ctx, qkv, C_out, N_kv, B_win, qkv->nb[1], qkv->nb[2],
    ...
    // Q: [C_out, N, B_win] → [HD, NH, N, B_win] → permute(0,2,1,3) → cont →
    // [HD, N, NH*B_win]
    ...
    attn_proj = ggml_add(ctx, attn_proj,
                         ggml_reshape_3d(ctx, blk.proj_b, C_out, 1, 1));

    // Reshape back to spatial
    auto* attn_spatial =
            ggml_reshape_4d(ctx, attn_proj, C_out, out_W, out_H, B_win);
    ...
    if (blk.window_size > 0) {
        ...
        if (blk.has_q_stride) {
            ...
            // Recompute padding from target dims (matching Python lines
            // 152-157).
        ...
        attn_result =
                sam2_window_unpartition(ctx, attn_spatial, ws_out,
    ...
    normed2 = ggml_mul(
            ctx, normed2,
            ggml_repeat(ctx, ggml_reshape_4d(ctx, blk.norm2_w, C_out, 1, 1, 1),
                        normed2));
    normed2 = ggml_add(
            ctx, normed2,
            ggml_repeat(ctx, ggml_reshape_4d(ctx, blk.norm2_b, C_out, 1, 1, 1),
                        normed2));
...
static void sam2_build_hiera_graph(struct ggml_context* ctx,
    ...
    x = ggml_add(ctx, x,
                 ggml_reshape_4d(ctx, hiera.patch_embed_b, 1, 1,
    ...
    auto* pe = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, hp.hiera_embed_dim,
    ...
    for (int i = 0; i < hp.hiera_total_blocks(); ++i) {
        ...
        if (i == hiera.stage_ends[stage_idx]) {
            // Store as BCHW: permute [C, W, H, 1] → [C, W, H, 1] (already in
            // this format)
            char name[64];
...
static void sam2_build_fpn_neck_graph(struct ggml_context* ctx,
    ...
    // Use ggml_conv_2d_sk_p0 (same as SAM3 neck) — operates on [W, H, C, B]
    // layout.
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        auto* inp =
                ggml_cont(ctx, ggml_permute(ctx, stage_outs[i], 2, 0, 1, 3));
        auto* conv_out = ggml_conv_2d_sk_p0(
                ctx, model.fpn_neck.levels[conv_idx].conv_w, inp);
        // Add bias: [C_out] → [1, 1, C_out, 1] → broadcasts over (W, H, B).
        auto* bias = ggml_reshape_3d(
                ctx, model.fpn_neck.levels[conv_idx].conv_b, 1, 1,
    ...
    for (int ri = 0; ri < 4; ++ri) {
        ...
        if (is_top_down && prev != nullptr) {
            ...
            // ggml_upscale scales ne[0],ne[1]; we need to scale W,H
            // (ne[1],ne[2]). Permute [D, W, H, 1] -> [W, H, D, 1], upscale,
            // permute back.
            auto* prev_whcb =
                    ggml_cont(ctx, ggml_permute(ctx, prev, 2, 0, 1, 3));
            auto* upsampled =
                    ggml_upscale(ctx, prev_whcb, 2, GGML_SCALE_MODE_NEAREST);
            auto* upsampled_cwh =
                    ggml_cont(ctx, ggml_permute(ctx, upsampled, 1, 2, 0, 3));
...
static bool sam2_encode_image_hiera(sam3_state& state,
    ...
    // The graph topology is a pure function of (model hparams, img_size), so
    // a state that already encoded at this resolution can skip rebuild +
    // realloc entirely (saves ~45 ms/frame) and, crucially, keeps all tensor
    // addresses stable across frames — the precondition for ggml-cuda's
    // CUDA-graph capture (GGML_CUDA_GRAPHS=ON) to hit instead of re-capturing.
    const size_t buf_size =
            ggml_tensor_overhead() * 16384 + ggml_graph_overhead() * 2;
    ...
    struct ggml_context* ctx0 = state.enc_ctx;
    struct ggml_cgraph* graph = state.enc_graph;
    struct ggml_gallocr* galloc = state.enc_galloc;
    struct ggml_tensor* inp = state.enc_inp;
    struct ggml_tensor* pe_tensor = state.enc_pe;
    struct ggml_tensor* fpn_outs[4];
    const bool graph_cached =
            (ctx0 != nullptr) && (state.enc_img_size == img_size);

    if (graph_cached) {
        for (int i = 0; i < 4; ++i) fpn_outs[i] = state.enc_fpn[i];
    } else {
        if (ctx0) {
            // img_size changed: drop the stale graph and rebuild.
        ...
        if (!ggml_gallocr_reserve(galloc, graph)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to reserve graph memory\n",
                             __func__);
            ggml_gallocr_free(galloc);
            ggml_free(ctx0);
            return false;
        ...
        if (!ggml_gallocr_alloc_graph(galloc, graph)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to alloc graph\n",
                             __func__);
            ggml_gallocr_free(galloc);
            ggml_free(ctx0);
            return false;
        ...
        pe_tensor = ggml_graph_get_tensor(graph, "hiera_pos_embed");

        // Cache for the next encode at the same resolution.
        state.enc_ctx = ctx0;
        state.enc_graph = graph;
        state.enc_galloc = galloc;
        state.enc_img_size = img_size;
        state.enc_n_fpn = n_fpn;
        state.enc_inp = inp;
    ...
    // Positional embedding: only upload when the graph was freshly built
    // (first encode or resolution change).  When the graph is cached the PE
    // data persists in the GPU buffer from the initial upload — re-uploading
    // the same constant data is a wasted H2D transfer (~1-2 ms).
    if (!graph_cached) {
        int pe_H = img_size / 4;
        int pe_W = img_size / 4;
    ...
    if (ggml_backend_graph_compute(model.backend, graph) !=
        GGML_STATUS_SUCCESS) {
        AICORE_LOG_ERROR("[sam3] ", "%s: graph compute failed\n", __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    }
    ...
    // resolution), reuse the existing state buffers and do a direct GPU→GPU
    // tensor copy instead of the expensive GPU→CPU→GPU round trip through
    // a host staging buffer.  This avoids per-frame cudaMalloc/cudaFree
    // (~100-200 ms) and host DMA transfers (~100 ms for 104 MB).
    ...
    if (sizes_changed) {
        // First encode or resolution changed — full (re)allocation.
        if (state.buffer) {
            ggml_backend_buffer_free(state.buffer);
            state.buffer = nullptr;
        }
        ...
        for (int i = 0; i < n_fpn; ++i) {
            auto* src = fpn_outs[i];
            state.neck_trk[i] =
                    ggml_new_tensor_4d(state.ctx, GGML_TYPE_F32, src->ne[0],
    ...
    // GPU→GPU copy of FPN outputs into persistent state tensors.
    for (int i = 0; i < n_fpn; ++i) {
        ggml_backend_tensor_copy(fpn_outs[i], state.neck_trk[i]);
    }

    // Compute sinusoidal PE for each FPN level (only on first encode or
    // when resolution changes — the PE is resolution-dependent but
    // frame-invariant).
    if (sizes_changed) {
        if (state.pe_buf) {
            ggml_backend_buffer_free(state.pe_buf);
        ...
        for (int i = 0; i < n_fpn; ++i) {
            ...
            state.neck_trk_pe[i] = ggml_new_tensor_4d(
                    state.pe_ctx, GGML_TYPE_F32, hp.neck_dim, W, H, 1);
            char name[64];
            snprintf(name, sizeof(name), "neck_trk_pe_%d", i);
            ggml_set_name(state.neck_trk_pe[i], name);
        }
        state.pe_buf =
                ggml_backend_alloc_ctx_tensors(state.pe_ctx, model.backend);
        for (int i = 0; i < n_fpn; ++i) {
            int H = (int)state.neck_trk[i]->ne[2];
            int W = (int)state.neck_trk[i]->ne[1];
            auto pe = sam3_sinusoidal_pe_2d(H, W, hp.neck_dim);
            ggml_backend_tensor_set(state.neck_trk_pe[i], pe.data(), 0,
                                    pe.size() * sizeof(float));
        }
    }

    // The encode graph is cached in state (state.enc_ctx/enc_galloc) for reuse
    // at the same img_size, so it must NOT be freed here — sam3_free_state owns
    // its lifetime. Early-exit paths above (reserve/alloc failures) free the
    // freshly-built graph before the cache pointers are set.
...
static bool sam3_encode_image_impl(sam3_state& state,
    ...
    // ── Build or reuse the cached SAM3 encode graph ──────────────────────
    // The graph topology is a pure function of (model hparams, img_size), so
    // a state that already encoded at this resolution can skip rebuild +
    // realloc entirely and, crucially, keeps all tensor addresses stable
    // across frames — the precondition for ggml-cuda's CUDA-graph capture
    // (GGML_CUDA_GRAPHS=ON) to hit instead of re-capturing every frame.
    const bool graph_cached = (state.sam3_ctx != nullptr) &&
                              (state.sam3_img_size == img_size) &&
                              (state.sam3_tracker_only == tracker_only ||
                               model.hparams.visual_only);
    ...
    if (graph_cached) {
        // Reuse cached graph — all tensor addresses stay the same.
        ctx0 = state.sam3_ctx;
        graph = state.sam3_graph;
        galloc = state.sam3_galloc;
        inp = ggml_graph_get_tensor(graph, "input_image");
        vit_out = state.vit_output;
        for (int i = 0; i < 4; ++i) {
        ...
        if (!inp) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: cached graph missing input_image tensor\n",
                             __func__);
            return false;
        }
    } else {
        ...
        const size_t buf_size =
                ggml_tensor_overhead() * 8192 + ggml_graph_overhead() * 2;
        struct ggml_init_params gparams = {
                /*.mem_size   =*/buf_size,
                /*.mem_buffer =*/nullptr,
                /*.no_alloc   =*/true,
        ...
        ctx0 = ggml_init(gparams);
        if (!ctx0) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to init compute context\n",
                             __func__);
            return false;
        }
        ...
        vit_out = sam3_build_vit_graph(ctx0, inp, model);
        ggml_set_name(vit_out, "vit_output");
        ggml_set_output(vit_out);

        // Both necks consume the same convolution-layout tensor. Sharing this
        // materialization avoids a duplicate 20 MiB permute+copy per encode.
        auto* neck_input =
                ggml_cont(ctx0, ggml_permute(ctx0, vit_out, 2, 0, 1, 3));
        if (!model.hparams.visual_only && !tracker_only) {
            sam3_build_neck_graph(ctx0, neck_input, model.neck_det,
                                  neck_det_out);
        }
        ...
        if (!ggml_gallocr_alloc_graph(galloc, graph)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate graph\n",
                             __func__);
            ggml_gallocr_free(galloc);
            ggml_free(ctx0);
            return false;
...

    {
        ...
        SAM3_LOG(1, "%s: graph computed in %.1f ms (%d threads)\n", __func__,
                 compute_ms, state.n_threads);
#endif
    }

    if (!graph_cached) {
        // Cache ctx/galloc/graph for reuse at same resolution.
        // Free stale state.ctx/galloc before overwriting.
        if (state.ctx) {
            ggml_gallocr_free(state.galloc);
            ggml_free(state.ctx);
        }
        ...
        for (int i = 0; i < 4; ++i) {
            state.neck_det[i] = (model.hparams.visual_only || tracker_only)
                                        ? nullptr
                                        : neck_det_out[i];
        ...
        state.sam3_ctx = ctx0;
        state.sam3_graph = graph;
        state.sam3_galloc = galloc;
        state.sam3_img_size = img_size;
        state.sam3_tracker_only = tracker_only;

        // Invalidate the prompt-encoder PE cache so segment reprocesses PEs
        state.pe_cache_valid = false;

        // Neck positional encodings are only consumed by PCS. Building all
        // four levels here costs about 250 ms and 111 MB even for PVS-only
        // callers, so invalidate them now and create them on first PCS use.
        if (state.pe_buf) {
            ggml_backend_buffer_free(state.pe_buf);
            state.pe_buf = nullptr;
        }
...
bool sam3_encode_image_pvs(sam3_state& state,
                           const sam3_model& model,
                           const sam3_image& image) {
    return sam3_encode_image_impl(state, model, image, true);
}

static bool sam3_ensure_neck_pe(sam3_state& state, const sam3_model& model) {
    if (state.neck_det_pe[2]) {
        return true;
    }
    if (!state.neck_det[0]) {
        return false;
    ...
    for (int i = 0; i < 4; ++i) {
        auto* feature = state.neck_det[i];
        if (!feature) {
            continue;
        }
        pe_tensors[i] = ggml_new_tensor_4d(state.pe_ctx, GGML_TYPE_F32,
                                           model.hparams.neck_dim,
                                           feature->ne[1], feature->ne[2], 1);
        char name[64];
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        const int W = (int)pe_tensors[i]->ne[1];
        const int H = (int)pe_tensors[i]->ne[2];
        auto pe_data = sam3_sinusoidal_pe_2d(H, W, model.hparams.neck_dim);
        ggml_backend_tensor_set(pe_tensors[i], pe_data.data(), 0,
                                pe_data.size() * sizeof(float));
...
static bool sam3_mark_named_outputs(
        struct ggml_context* ctx,
    ...
    for (const auto& name : output_tensors) {
        ...
        if (!t) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: requested tensor '%s' was not found\n",
                             __func__, name.c_str());
            return false;
        }
        ggml_set_output(t);
...
bool sam3_encode_vit_from_preprocessed_selective(
        sam3_state& state,
        const sam3_model& model,
    ...
    const size_t buf_size =
            ggml_tensor_overhead() * 4096 + ggml_graph_overhead() * 2;
    struct ggml_init_params gparams = {
            /*.mem_size   =*/buf_size,
            /*.mem_buffer =*/nullptr,
            /*.no_alloc   =*/true,
    ...
    auto* inp =
            ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, img_size, img_size, 3, 1);
    ggml_set_name(inp, "input_image");
    ggml_set_input(inp);
...
static struct ggml_tensor* sam3_new_f32_tensor_4d_from_ne(
        struct ggml_context* ctx, const int64_t ne[4]);
static bool sam3_copy_tensor_to_f32(struct ggml_tensor* t,
                                    std::vector<float>& output);
static struct ggml_tensor* sam3_build_vit_prefix_stage_from_input(
        struct ggml_context* ctx,
        struct ggml_tensor* input,
        const sam3_model& model,
...
bool sam3_test_run_vit_block0_input(const sam3_model& model,
                                    ...
                                    int n_threads) {
    if (!chw_data) {
        AICORE_LOG_ERROR("[sam3] ", "%s: chw_data is null\n", __func__);
        return false;
    }
    ...
    const size_t ctx_size =
            ggml_tensor_overhead() * 256 + ggml_graph_overhead();
    ...
    if (!ctx) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to create ggml context\n",
                         __func__);
        return false;
    }

    struct ggml_tensor* input =
            ggml_new_tensor_4d(ctx, GGML_TYPE_F32, img_size, img_size, 3, 1);
...
bool sam3_test_run_vit_prefix_stage(const sam3_model& model,
    ...
    switch (stage) {
        ...
        case SAM3_VIT_PREFIX_STAGE_PATCH_EMBED:
            if (ne[0] != model.hparams.img_size ||
                ne[1] != model.hparams.img_size || ne[2] != 3) {
                AICORE_LOG_ERROR("[sam3] ",
                                 "%s: patch_embed input shape mismatch "
                                 "[%lld,%lld,%lld,%lld]\n",
                                 __func__, (long long)ne[0], (long long)ne[1],
        ...
        default:
            if (ne[0] != model.hparams.vit_embed_dim ||
                ...
                ne[2] != model.hparams.n_img_embd()) {
                AICORE_LOG_ERROR("[sam3] ",
                                 "%s: prefix feature input shape mismatch "
                                 "[%lld,%lld,%lld,%lld]\n",
                                 __func__, (long long)ne[0], (long long)ne[1],
                                 (long long)ne[2], (long long)ne[3]);
                return false;
    ...
    if (stage == SAM3_VIT_PREFIX_STAGE_PATCH_MULMAT_RAW ||
        ...
        input = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, ne[0], ne[1], ne[2],
                                   ne[3]);
    } else {
    ...
    struct ggml_tensor* output =
            sam3_build_vit_prefix_stage_from_input(ctx, input, model, stage);
    if (!output) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to build prefix stage %d\n",
                         __func__, (int)stage);
        ggml_free(ctx);
        return false;
    }
    ...
    if (input->type == GGML_TYPE_F16) {
        ...
        ggml_backend_tensor_set(input, tmp.data(), 0,
                                (size_t)numel * sizeof(ggml_fp16_t));
    } else {
        ggml_backend_tensor_set(input, input_data, 0,
                                (size_t)ggml_nbytes(input));
    }
    sam3_graph_compute(model.backend, graph, n_threads);
...
bool sam3_test_run_patch_mulmat_host_ref(const sam3_model& model,
    ...
    const int64_t patch_k = model.vit.patch_embed_w->ne[0] *
                            model.vit.patch_embed_w->ne[1] *
                            model.vit.patch_embed_w->ne[2];
    ...
    if (ne[0] != patch_k || ne[1] != n_img || ne[2] != n_img) {
        AICORE_LOG_ERROR(
                ...
                __func__, (long long)ne[0], (long long)ne[1], (long long)ne[2],
                (long long)ne[3]);
        return false;
    }
    ...
    ggml_backend_tensor_get(model.vit.patch_embed_w, weight_f16.data(), 0,
                            weight_f16.size() * sizeof(ggml_fp16_t));
        ...
        for (int64_t p = 0; p < n_patch; ++p) {
            ...
            if (use_double_accum) {
                ...
                for (int64_t k = 0; k < patch_k; ++k) {
                    acc += (double)ggml_fp16_to_fp32(x_row[k]) *
                           (double)ggml_fp16_to_fp32(w_row[k]);
                }
                dst_row[p] = (float)acc;
            } else {
                float acc = 0.0f;
                for (int64_t k = 0; k < patch_k; ++k) {
                    acc += ggml_fp16_to_fp32(x_row[k]) *
                           ggml_fp16_to_fp32(w_row[k]);
                }
                dst_row[p] = acc;
            }
        }
...
bool sam3_test_run_vit_block_linear_host_ref(const sam3_model& model,
                                             ...
                                             std::vector<float>& output_data,
                                             int64_t output_ne[4]) {
    if (!input_data || block_idx < 0 ||
        block_idx >= (int)model.vit.blocks.size()) {
        return false;
    }
    ...
    switch (stage) {
        ...
        case SAM3_VIT_BLOCK_STAGE_MLP_FC2:
            ...
            break;
        default:
            AICORE_LOG_ERROR("[sam3] ", "%s: unsupported stage %d\n", __func__,
                             (int)stage);
            return false;
    }
    ...
    if (ne[0] != k) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: linear input shape mismatch "
                         "[%lld,%lld,%lld,%lld] for K=%lld\n",
                         __func__, (long long)ne[0], (long long)ne[1],
                         (long long)ne[2], (long long)ne[3], (long long)k);
        return false;
    }
    ...
    ggml_backend_tensor_get(w, weight_f16.data(), 0,
                            weight_f16.size() * sizeof(ggml_fp16_t));

    std::vector<float> bias_f32((size_t)out_dim);
    ggml_backend_tensor_get(b, bias_f32.data(), 0,
                            bias_f32.size() * sizeof(float));
        ...
        for (int64_t oc = 0; oc < out_dim; ++oc) {
            ...
            if (use_double_accum) {
                ...
                for (int64_t i = 0; i < k; ++i) {
                    acc += (double)ggml_fp16_to_fp32(w_row[i]) *
                           (double)ggml_fp16_to_fp32(x_col[i]);
                }
                dst_col[oc] = (float)acc;
            } else {
                float acc = bias_f32[oc];
                for (int64_t i = 0; i < k; ++i) {
                    acc += ggml_fp16_to_fp32(w_row[i]) *
                           ggml_fp16_to_fp32(x_col[i]);
                }
                dst_col[oc] = acc;
            }
        }
...
static bool sam3_copy_tensor_to_f32(struct ggml_tensor* t,
    ...
    if (!ggml_is_contiguous(t)) {
        AICORE_LOG_PRINT("[sam3] ", "%s: tensor '%s' is not contiguous\n",
    ...
    if (t->type == GGML_TYPE_F32) {
        ggml_backend_tensor_get(t, output.data(), 0,
    ...
    if (t->type == GGML_TYPE_F16) {
        std::vector<ggml_fp16_t> tmp((size_t)numel);
        ggml_backend_tensor_get(t, tmp.data(), 0,
                                (size_t)numel * sizeof(ggml_fp16_t));
        ggml_fp16_to_fp32_row(tmp.data(), output.data(), numel);
        return true;
    }

    AICORE_LOG_ERROR("[sam3] ", "%s: unsupported tensor type %d for '%s'\n",
...
static struct ggml_tensor* sam3_build_vit_prefix_stage_from_input(
    ...
    switch (stage) {
        case SAM3_VIT_PREFIX_STAGE_PATCH_IM2COL:
            return ggml_im2col(ctx, model.vit.patch_embed_w, input,
                               model.vit.patch_embed_w->ne[0],
                               model.vit.patch_embed_w->ne[1], 0, 0, 1, 1, true,
        ...
        case SAM3_VIT_PREFIX_STAGE_PATCH_MULMAT: {
            struct ggml_tensor* result = ggml_mul_mat(
                    ctx,
                    ggml_reshape_2d(ctx, input, input->ne[0],
                                    input->ne[3] * input->ne[2] * input->ne[1]),
                    ggml_reshape_2d(ctx, model.vit.patch_embed_w,
                                    model.vit.patch_embed_w->ne[0] *
                                            model.vit.patch_embed_w->ne[1] *
                                            model.vit.patch_embed_w->ne[2],
            ...
            result = ggml_reshape_4d(ctx, result, input->ne[1], input->ne[2],
                                     input->ne[3],
                                     model.vit.patch_embed_w->ne[3]);
        ...
        case SAM3_VIT_PREFIX_STAGE_PATCH_EMBED: {
            struct ggml_tensor* x =
                    ggml_conv_2d_sk_p0(ctx, model.vit.patch_embed_w, input);
        ...
        case SAM3_VIT_PREFIX_STAGE_POS_ADD: {
            struct ggml_tensor* pos_target = ggml_new_tensor_4d(
                    ctx, GGML_TYPE_F32, model.hparams.vit_embed_dim,
                    model.hparams.n_img_embd(), model.hparams.n_img_embd(), 1);
            struct ggml_tensor* pos_tiled =
                    ggml_repeat(ctx, model.vit.pos_embed, pos_target);
...
static struct ggml_tensor* sam3_build_vit_attn_core_from_qkv(
        struct ggml_context* ctx,
    ...
    struct ggml_tensor* cur =
            ggml_reshape_4d(ctx, qkv, E, 3, W_cur * H_cur, B_cur);
    cur = ggml_cont(ctx, ggml_permute(ctx, cur, 0, 3, 1, 2));

    struct ggml_tensor* Q = ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur,
                                         cur->nb[1], cur->nb[2], 0);
    struct ggml_tensor* K =
            ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur, cur->nb[1],
                         cur->nb[2], 1 * cur->nb[3]);
    struct ggml_tensor* V =
            ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur, cur->nb[1],
                         cur->nb[2], 2 * cur->nb[3]);
    ...
    return ggml_cont(ctx,
                     ggml_reshape_4d(ctx, attn_out, E, W_cur, H_cur, B_cur));
}

static struct ggml_tensor* sam3_build_vit_block_stage_from_input(
        struct ggml_context* ctx,
        struct ggml_tensor* input,
        const sam3_vit_block& blk,
        const sam3_hparams& hp,
    ...
    switch (stage) {
        ...
        case SAM3_VIT_BLOCK_STAGE_QKV_PROJ:
            return ggml_add(ctx, ggml_mul_mat(ctx, blk.qkv_w, input),
                            blk.qkv_b);
        ...
        case SAM3_VIT_BLOCK_STAGE_ATTN_PROJ:
            return ggml_add(ctx, ggml_mul_mat(ctx, blk.proj_w, input),
                            blk.proj_b);

        case SAM3_VIT_BLOCK_STAGE_WINDOW_UNPART:
            return sam3_win_unpart(ctx, input, hp.n_img_embd(), hp.n_img_embd(),
                                   hp.vit_window_size);
        ...
        case SAM3_VIT_BLOCK_STAGE_MLP_FC1:
            return ggml_add(ctx, ggml_mul_mat(ctx, blk.mlp_fc1_w, input),
                            blk.mlp_fc1_b);
        ...
        case SAM3_VIT_BLOCK_STAGE_MLP_FC2:
            return ggml_add(ctx, ggml_mul_mat(ctx, blk.mlp_fc2_w, input),
                            blk.mlp_fc2_b);
...
bool sam3_test_run_vit_block_stage(const sam3_model& model,
    ...
    if (block_idx < 0 || block_idx >= (int)model.vit.blocks.size()) {
        AICORE_LOG_ERROR("[sam3] ", "%s: invalid block_idx=%d\n", __func__,
                         block_idx);
        return false;
    }
    ...
    switch (stage) {
        ...
        case SAM3_VIT_BLOCK_STAGE_ATTN_CORE:
            if (ne[0] != 3 * hp.vit_embed_dim) {
                AICORE_LOG_PRINT(
                        "[sam3] ", "%s: attn input ne0=%lld expected %d\n",
    ...
    struct ggml_tensor* output =
            sam3_build_vit_block_stage_from_input(ctx, input, blk, hp, stage);
    if (!output) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to build stage %d\n", __func__,
                         (int)stage);
        ggml_free(ctx);
        return false;
    }
    ...
    const size_t input_bytes = (size_t)ne[0] * (size_t)ne[1] * (size_t)ne[2] *
                               (size_t)ne[3] * sizeof(float);
...
// Test-only: encode from pre-preprocessed float data (bypasses C++
// resize/normalize). chw_data is in [C, H, W] layout, already normalized, with
// C=3, H=W=img_size.
bool sam3_encode_image_from_preprocessed(sam3_state& state,
    ...
    if (hp.is_sam2()) {
        ...
        // sam2_encode_image_hiera expects an image struct, but we have raw CHW
        // data. We need to bypass preprocessing and inject the data directly
        // into the graph. Build the Hiera graph, set the input from chw_data
        // (already CHW normalized).
        ...
        auto* inp = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, img_size, img_size,
                                       3, 1);
        ggml_set_name(inp, "input_image");
        ggml_set_input(inp);
        ...
        // Set input image — CHW data needs to go into ggml's [W, H, C, B]
        // layout The input tensor is [img_size, img_size, 3, 1] in ggml = [W,
        // H, C, B] CHW data: element (c, h, w) at c*H*W + h*W + w ggml data:
        // element at w + h*W_stride... but ggml_conv_2d expects [W, H, C, B]
        ...
        ggml_backend_tensor_set(inp, chw_data, 0,
                                3 * img_size * img_size * sizeof(float));
    ...
    // Upload preprocessed data directly (already in CHW = ggml [W, H, C, B]
    // layout)
    const size_t data_bytes = (size_t)3 * img_size * img_size * sizeof(float);
    ggml_backend_tensor_set(inp, chw_data, 0, data_bytes);
    ...
    for (int i = 0; i < 4; ++i) {
        state.neck_det[i] =
                model.hparams.visual_only ? nullptr : neck_det_out[i];
        state.neck_trk[i] = neck_trk_out[i];
    }
    ...
    // Compute sinusoidal PEs
    {
        ...
        state.pe_buf =
                ggml_backend_alloc_ctx_tensors(state.pe_ctx, model.backend);
        if (!state.pe_buf) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate PE buffer\n",
                             __func__);
        } else {
            for (int i = 0; i < 4; ++i) {
                const int S = scale_sizes[i];
...
// q_in, k_in, v_in: [D, N, B]  (if fused_qkv, only q_in is used and contains
// QKV stacked) in_proj_w: [D, 3*D] (fused Q/K/V projection) in_proj_b: [3*D]
// out_proj_w: [D, D], out_proj_b: [D]
// n_heads: number of attention heads
// Returns: [D, N_q, B]
//
// If separate_kv is true, q_in/k_in/v_in are already separate (no fused proj
// needed). The in_proj is applied to form Q from q_in, and K/V from the
// concatenated k/v source.
static struct ggml_tensor* sam3_multihead_attn_fused(
        struct ggml_context* ctx,
        struct ggml_tensor* q_in,  // [D, N_q, B]
        struct ggml_tensor*
                kv_in,  // [D, N_kv, B] (can be same as q_in for self-attn)
        ...
        struct ggml_tensor* attn_mask = nullptr)  // [N_kv, N_q] or nullptr
{
    ...
    auto* k_w = ggml_view_2d(ctx, in_proj_w, D, D, in_proj_w->nb[1],
                             D * in_proj_w->nb[1]);
    auto* v_w = ggml_view_2d(ctx, in_proj_w, D, D, in_proj_w->nb[1],
                             2 * D * in_proj_w->nb[1]);
    ...
    V = ggml_permute(
            ctx, V, 0, 2, 1,
            3);  // [HD, N_kv, NH, B] non-contiguous; flash_attn uses strides
...
static void sam3_sine_encode_box(float* out,
                                 float cx,
                                 float cy,
                                 float w,
                                 float h,
    ...
    for (int i = 0; i < num_pos_feats; ++i) {
        int div_idx = 2 * (i / 2);
        float dim_t =
                powf((float)temperature, (float)div_idx / (float)num_pos_feats);
...
// Features in ggml [C, W, H] layout. Box in XYXY format scaled to feature grid
// coords. Uses sub-sampling matching torchvision's sampling_ratio=0 (auto).
// Output: [C * roi_size * roi_size] in ggml layout.
static void sam3_roi_align_single(
        const float*
                feats,  // [C, W, H] ggml layout (C innermost, then W, then H)
        int C,
        int W_feat,
        int H_feat,
        float x0,
        ...
        float* out)  // [C, roi_size, roi_size]
{
    ...
    auto bilinear_sample = [&](float sx, float sy, int c) -> float {
        ...
        if (sx < -1.0f || sx > (float)W_feat || sy < -1.0f ||
            sy > (float)H_feat)
            return 0.0f;
        ...
        return v00 * (1 - ly) * (1 - lx) + v10 * (1 - ly) * lx +
               v01 * ly * (1 - lx) + v11 * ly * lx;
    };
        ...
        for (int pw = 0; pw < roi_size; ++pw) {
            for (int c = 0; c < C; ++c) {
                ...
                for (int iy = 0; iy < sample_y_count; ++iy) {
                    float sy = y0 + bin_h * ((float)ph +
                                             ((float)iy + 0.5f) /
                                                     (float)sample_y_count);
                    for (int ix = 0; ix < sample_x_count; ++ix) {
                        float sx = x0 + bin_w * ((float)pw +
                                                 ((float)ix + 0.5f) /
                                                         (float)sample_x_count);
        ...
        struct ggml_tensor* img_pe)     // [D, N_img, 1] sinusoidal PE
{
    ...
    const int n_heads = 8;
    const int n_boxes =
            (int)(params.pos_exemplars.size() + params.neg_exemplars.size());
    const int N_geo = n_boxes + 1;  // +1 for CLS

    // Input tensor: pre-computed geometry embeddings after final_proj + norm
    // [D, N_geo, 1] The caller pre-computes: final_proj(box_embeds + CLS) →
    // LayerNorm
    ...
    for (int i = 0; i < (int)ge.layers.size(); ++i) {
        ...
        // 1. Self-attention (pre-norm, pos_enc_at_attn=False)
        {
            ...
            auto* Q = ggml_add(
                    ctx,
                    ggml_mul_mat(ctx,
                                 ggml_view_2d(ctx, ly.sa_in_proj_w, D, D,
                                              ly.sa_in_proj_w->nb[1], 0),
                                 xn),
            ...
            auto* V = ggml_add(
                    ...
                    ggml_mul_mat(ctx,
                                 ggml_view_2d(ctx, ly.sa_in_proj_w, D, D,
                                              ly.sa_in_proj_w->nb[1],
                                              2 * D * ly.sa_in_proj_w->nb[1]),
                                 xn),
                    ggml_view_1d(ctx, ly.sa_in_proj_b, D,
                                 2 * D * sizeof(float)));
        ...
        // 2. Cross-attention (pre-norm, Q from x, K from img+PE, V from img)
        {
            ...
            auto* k_w = ggml_view_2d(ctx, ly.ca_q_w, D, D, ly.ca_q_w->nb[1],
                                     D * ly.ca_q_w->nb[1]);
            auto* v_w = ggml_view_2d(ctx, ly.ca_q_w, D, D, ly.ca_q_w->nb[1],
                                     2 * D * ly.ca_q_w->nb[1]);
            ...
            auto* k_input = ggml_add(ctx, img_feats,
                                     img_pe);  // pos_enc_at_cross_attn_keys
...
static std::vector<float> sam3_precompute_geom_input(
    ...
    std::vector<float> box_pool_proj_w_data(7 * 7 * D * D),
            box_pool_proj_b_data(D);
    ...
    if (n_boxes > 0) {
        read_f32(ge.box_pool_proj_w, box_pool_proj_w_data.data(),
                 7 * 7 * D * D);
    ...
    for (int bi = 0; bi < n_boxes; ++bi) {
        ...
        // out = x @ W^T + b where x=[cx,cy,w,h]
        {
            ...
            for (int d = 0; d < D; ++d) {
                ...
                for (int j = 0; j < 4; ++j)
                    sum += coords[j] *
                           box_proj_w_data[j + d * 4];  // ggml: [4, D] stride
                embed[d] = sum;
            }
        }
        ...
        if (img_feats_data) {
            ...
            // Actually, LayerNorm is applied to the [D] dimension at each
            // spatial location But we need the full feature map, so let's
            ...
            for (int s = 0; s < N_spatial; ++s) {
                ...
                float mean = 0.0f;
                for (int d = 0; d < D; ++d) mean += img_feats_data[d + s * D];
                mean /= D;
                ...
                for (int d = 0; d < D; ++d) {
                    normed_feats[d + s * D] =
                            (img_feats_data[d + s * D] - mean) * inv_std *
                                    img_pre_norm_w_data[d] +
            ...
            sam3_roi_align_single(normed_feats.data(), D, W_feat, H_feat, fx0,
                                  fy0, fx1, fy1, roi_size, roi_data.data());

            // Conv2d(D, D, 7): kernel [7, 7, D, D] in ggml = [D_out, D_in, kH,
            // kW] in PyTorch Since roi_size = 7 = kernel_size, output is [D, 1,
            // 1] This is effectively a matrix multiply: out[d_out] = sum over
            // (d_in, kh, kw)
                ...
                for (int kh = 0; kh < roi_size; ++kh) {
                    for (int kw = 0; kw < roi_size; ++kw) {
                        for (int d_in = 0; d_in < D; ++d_in) {
                            // ggml weight layout: [kW=7, kH=7, D_in=256,
                            // D_out=256]
                            int w_idx = kw + kh * 7 + d_in * 7 * 7 +
                                        d_out * 7 * 7 * D;
                            // roi_data layout: [C, W, H] = [D_in, kW, kH]
                            int r_idx = d_in + kw * D + kh * D * roi_size;
                            sum += box_pool_proj_w_data[w_idx] *
                                   roi_data[r_idx];
                        }
                    }
                }
        ...
        for (int d = 0; d < D; ++d) out[d + bi * D] = embed[d];
    }

    // CLS token goes at position n_boxes (last)
    for (int d = 0; d < D; ++d) out[d + n_boxes * D] = cls_data[d];
    ...
    for (int t = 0; t < N_geo; ++t) {
        float mean = 0.0f;
        for (int d = 0; d < D; ++d) mean += projected[d + t * D];
        mean /= D;
...
// x: [D, N, B] image features (N=5184), prompt: [D, T, B] text/exemplar tokens,
// pos: [D, N, B] Returns: updated x [D, N, B]
static struct ggml_tensor* sam3_fenc_layer_forward(
    ...
    // Self-attention: Q/K get positional encoding, V does not
    {
        ...
        auto* q_w = ggml_view_2d(ctx, ly.sa_in_proj_w, D, D,
                                 ly.sa_in_proj_w->nb[1], 0);
        auto* k_w =
                ggml_view_2d(ctx, ly.sa_in_proj_w, D, D, ly.sa_in_proj_w->nb[1],
                             D * ly.sa_in_proj_w->nb[1]);
        auto* v_w =
                ggml_view_2d(ctx, ly.sa_in_proj_w, D, D, ly.sa_in_proj_w->nb[1],
                             2 * D * ly.sa_in_proj_w->nb[1]);
    ...
    // ca_q_w stores fused [D, 3*D] weights split as Q-proj, K-proj, V-proj.
    {
        ...
        auto* ca_mask = sam3_expand_token_attn_bias(ctx, prompt_attn_bias, N_q,
                                                    n_heads, B);
        float scale = 1.0f / sqrtf((float)HD);
...
// Python reference uses eps=1e-3: x1 = x.clamp(min=eps), x2 =
// (1-x).clamp(min=eps)
static struct ggml_tensor* sam3_inverse_sigmoid(struct ggml_context* ctx,
                                                struct ggml_tensor* x) {
...
// Build sinusoidal positional embedding for 4D reference points in the ggml
// graph. ref_boxes: [4, NQ, B] — (cx, cy, w, h) after sigmoid, B=1 sine_dim_t:
// [1, 64] — pre-computed angle multipliers (2π / 10000^(2i/128)) Returns: [512,
// NQ, B] sinusoidal embedding matching Python gen_sineembed_for_position
static struct ggml_tensor* sam3_build_sine_pos_embed_4d(
    ...
    // Python output order: [cy, cx, w, h] → coord indices from boxes
    // [cx(0),cy(1),w(2),h(3)]
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        auto* coord = ggml_view_2d(ctx, ref_boxes, 1, NQ, ref_boxes->nb[1],
                                   ci * sizeof(float));  // [1, NQ]
    ...
    auto* embed =
            ggml_concat(ctx, coord_embeds[0], coord_embeds[1], 0);  // [256, NQ]
...
// Build query positional encoding from reference boxes via sine embed +
// ref_point_head MLP. ref_boxes: [4, NQ, 1] — after sigmoid sine_dim_t: [1, 64]
// Returns: [D, NQ+1, 1] (zeros for presence token at index 0, MLP output for
// object queries)
static struct ggml_tensor* sam3_build_query_pos(
    ...
    auto* h = ggml_mul_mat(
            ctx, tensors.at("ddec.ref_point_head.layers.0.weight"), sine_embed);
    h = ggml_add(ctx, h, tensors.at("ddec.ref_point_head.layers.0.bias"));
    h = ggml_relu(ctx, h);
    // Layer 1: W1 @ h + b1 (no activation)
    auto* qpos_obj = ggml_mul_mat(
            ctx, tensors.at("ddec.ref_point_head.layers.1.weight"), h);
    qpos_obj = ggml_add(ctx, qpos_obj,
                        tensors.at("ddec.ref_point_head.layers.1.bias"));
    // qpos_obj: [D, NQ]
    if (layer_idx == 0) {
...
// rpb_coords: [feat_hw] — normalized coords [0/H, 1/H, ..., (H-1)/H] (input
// tensor) Returns: bias tensor [N_kv, N_q+1, n_heads, B] for
...
static struct ggml_tensor* sam3_compute_box_rpb(
        ...
        struct ggml_tensor*
                rpb_coords,  // [feat_hw] — pre-filled grid coordinates
        int feat_hw,
        int layer_idx = -1) {
    const int64_t NQ = ref_boxes->ne[1];
    ...
    // ggml_view_2d on strided data is non-contiguous — ggml_scale requires
    // contiguous. Use ggml_cont to make each coordinate slice contiguous.
    auto* cx = ggml_cont(
            ctx, ggml_view_2d(ctx, ref_boxes, 1, NQ, ref_boxes->nb[1], 0));
    auto* cy =
            ggml_cont(ctx, ggml_view_2d(ctx, ref_boxes, 1, NQ, ref_boxes->nb[1],
                                        1 * sizeof(float)));
    auto* bw =
            ggml_cont(ctx, ggml_view_2d(ctx, ref_boxes, 1, NQ, ref_boxes->nb[1],
                                        2 * sizeof(float)));
    ...
    auto* cw_rep =
            ggml_repeat(ctx, cw, shape_wn);  // [W, NQ] (each column = coords)
    auto* x0_t = ggml_cont(ctx, ggml_transpose(ctx, x0));  // [NQ, 1]
    auto* x0_rep =
            ggml_repeat(ctx, ggml_reshape_2d(ctx, x0_t, 1, NQ), shape_wn);
    auto* x1_t = ggml_cont(ctx, ggml_transpose(ctx, x1));
    auto* x1_rep =
            ggml_repeat(ctx, ggml_reshape_2d(ctx, x1_t, 1, NQ), shape_wn);
    auto* y0_t = ggml_cont(ctx, ggml_transpose(ctx, y0));
    auto* y0_rep =
            ggml_repeat(ctx, ggml_reshape_2d(ctx, y0_t, 1, NQ), shape_wn);
    auto* y1_t = ggml_cont(ctx, ggml_transpose(ctx, y1));
    auto* y1_rep =
            ggml_repeat(ctx, ggml_reshape_2d(ctx, y1_t, 1, NQ), shape_wn);
    ...
    auto rpb_mlp = [&](struct ggml_tensor * d,
                       const char* axis) -> struct ggml_tensor* {
        int64_t spatial = d->ne[1];
        int64_t nq = d->ne[2];
        auto* flat = ggml_reshape_2d(ctx, d, 2, spatial * nq);  // [2, W*NQ]
        auto wn0 =
                std::string("ddec.boxRPB_embed_") + axis + ".layers.0.weight";
        auto bn0 = std::string("ddec.boxRPB_embed_") + axis + ".layers.0.bias";
        auto wn1 =
                std::string("ddec.boxRPB_embed_") + axis + ".layers.1.weight";
    ...
    auto* rpb_hw =
            ggml_repeat(ctx, rpb_y_4d,
                        ggml_new_tensor_4d(ctx, GGML_TYPE_F32, NH, W, H, NQ));
    auto* rpb_hw_x =
            ggml_repeat(ctx, rpb_x_4d,
                        ggml_new_tensor_4d(ctx, GGML_TYPE_F32, NH, W, H, NQ));
    ...
    rpb = ggml_cont(ctx,
                    ggml_permute(ctx, rpb, 2, 0, 1, 3));  // [H*W, NQ, NH, 1]

    // Prepend zeros for presence token: mask for presence token has no
    // box-relative bias
...
static struct ggml_tensor* sam3_ddec_layer_forward(
    ...
    //   ly.norm3_w  = ".norm_ca_text.weight"  = Python catext_norm
    //   (post-text-CA) ly.norm1_w  = ".norm1.weight"         = Python norm1
    //   (post-image-CA) ly.norm4_w  = ".norm3.weight"         = Python norm3
    //   (post-FFN)
    ...
    // 2. Cross-attention to text tokens (post-norm)
    {
        ...
        auto* q_w = ggml_view_2d(ctx, ly.ca_text_q_w, D, D,
                                 ly.ca_text_q_w->nb[1], 0);
        auto* k_w =
                ggml_view_2d(ctx, ly.ca_text_q_w, D, D, ly.ca_text_q_w->nb[1],
                             D * ly.ca_text_q_w->nb[1]);
        auto* v_w =
                ggml_view_2d(ctx, ly.ca_text_q_w, D, D, ly.ca_text_q_w->nb[1],
                             2 * D * ly.ca_text_q_w->nb[1]);
        ...
        auto* text_mask = sam3_expand_token_attn_bias(ctx, text_attn_bias, N_q,
                                                      n_heads, B);
        float scale = 1.0f / sqrtf((float)HD);
        auto* ca_out =
                sam3_attn_ext(ctx, Q, K, V, text_mask, scale, 0.0f, 0.0f);
        ca_out = ggml_reshape_3d(ctx, ca_out, D, N_q, B);
    ...
    // 3. Cross-attention to conditioned image features (post-norm)
    {
        ...
        if (rpb_mask) {
            ...
            kq = ggml_soft_max_ext(ctx, kq, rpb_mask, scale,
                                   0.0f);  // [N_kv, N_q, NH, B]

            auto* v_t = ggml_cont(ctx,
                                  ggml_transpose(ctx, V));  // [N_kv, HD, NH, B]
...
//   5. matmul(proj_hs, proj_pooled.unsqueeze(-1)) → dot product → [num_layer,
//   BS, N_q, 1]
//   6. scale by 1/sqrt(D)
//   7. clamp to [-12, 12]
//
...
// text_valid_mask: [T, 1, B] — 1.0 for valid tokens, 0.0 for padding (or
// nullptr for all-valid) Returns: class_scores [N_q, B] (one score per query
// per batch)
        ...
        struct ggml_tensor* text_valid_mask)  // [T, 1, B] or nullptr
{
    ...
    text_mlp = ggml_mul_mat(
            ctx, tensors.at("scoring.prompt_mlp.layers.0.weight"), text_mlp);
    text_mlp = ggml_add(ctx, text_mlp,
                        tensors.at("scoring.prompt_mlp.layers.0.bias"));
    text_mlp = ggml_relu(ctx, text_mlp);
    text_mlp = ggml_mul_mat(
            ctx, tensors.at("scoring.prompt_mlp.layers.1.weight"), text_mlp);
    text_mlp = ggml_add(ctx, text_mlp,
                        tensors.at("scoring.prompt_mlp.layers.1.bias"));
    ...
    // The masked path bakes T/n_valid into the mask values for valid tokens (0
    // for padding), so mean(mask * features) = sum(valid * T/n_valid) / T =
    // sum(valid)/n_valid.
    struct ggml_tensor* text_pooled;
    if (text_valid_mask) {
        auto* tp = ggml_cont(
                ctx, ggml_permute(ctx, text_mlp, 1, 0, 2, 3));  // [T, D, B]
        tp = ggml_mul(ctx, tp,
                      text_valid_mask);  // padding zeroed (scale baked in)
        auto* pooled_t = sam3_global_mean_dim0(ctx, tp, (int)T);  // [1, D, B]
        text_pooled = ggml_cont(
                ctx, ggml_permute(ctx, pooled_t, 1, 0, 2, 3));  // [D, 1, B]
    } else {
    ...
    auto* proj_pooled = ggml_mul_mat(
            ctx, tensors.at("scoring.prompt_proj.weight"), text_pooled);
    proj_pooled =
            ggml_add(ctx, proj_pooled, tensors.at("scoring.prompt_proj.bias"));
    // proj_pooled: [D, 1, B]
    ggml_set_name(proj_pooled, "scoring_proj_pooled");

    // Step 4: Project queries through hs_proj: D→D
    auto* proj_hs = ggml_mul_mat(ctx, tensors.at("scoring.hs_proj.weight"),
                                 query_outputs);
    ...
    // matmul(proj_hs, proj_pooled.unsqueeze(-1)) in Python = batched
    // vector-matrix multiply ggml_mul_mat(A, B) = A^T @ B With A = proj_pooled
    // [D, 1, B], B = proj_hs [D, N_q, B]: result = [1, N_q, B] — each element
    // is dot product of query with pooled prompt
...
struct sam3_ddec_output {
    ...
    struct ggml_tensor*
            presence_feats;  // [D, 1, B] pre-decoder-norm presence token
...
static sam3_ddec_output sam3_build_ddec_graph(
        ...
        struct ggml_tensor*
                sine_dim_t,  // [1, 64] — pre-computed angle multipliers
        struct ggml_tensor*
                rpb_coords,  // [feat_hw] — normalized grid coords (or nullptr)
        struct ggml_tensor* text_attn_bias =
                nullptr,  // [T, 1, B] additive text padding bias
        struct ggml_tensor* text_valid_mask =
                nullptr)  // [T, 1, B] for scoring (or nullptr)
{
    const auto& hp = model.hparams;
    const auto& tensors = model.tensors;
    const int D = hp.neck_dim;            // 256
    ...
    auto* ref_pts_raw = ggml_cont(
            ctx, tensors.at("ddec.reference_points.weight"));  // [4, NQ]
    ...
    // ── Run decoder layers ───────────────────────────────────────────────
    // Per-layer: recompute query_pos from updated ref_boxes (matching Python
    // exactly)
    struct ggml_tensor* last_presence = pres_tok;
    for (int i = 0; i < hp.ddec_layers; ++i) {
        // Recompute query_pos from current ref_boxes via sine embed +
        // ref_point_head MLP
        auto* query_pos =
                sam3_build_query_pos(ctx, model, ref_boxes, sine_dim_t, i);

        // Compute box-relative positional bias for image cross-attention
        struct ggml_tensor* rpb_mask = nullptr;
        if (rpb_coords) {
            rpb_mask = sam3_compute_box_rpb(ctx, model, ref_boxes, rpb_coords,
                                            feat_hw, i);
        }

        queries = sam3_ddec_layer_forward(ctx, model.ddec.layers[i], queries,
                                          query_pos, enc_feats, enc_pos,
                                          text_feats, hp.ddec_heads,
                                          text_attn_bias, rpb_mask, i);

        // Box refinement after each layer (on object queries only, not presence
        // token)
        auto* obj_q = ggml_view_3d(ctx, queries, D, NQ, 1, queries->nb[1],
                                   queries->nb[2], 1 * queries->nb[1]);
        obj_q = ggml_cont(ctx, obj_q);
        sam3_name_tensorf(obj_q, "ddec_layer%d_out", i);

        auto* pres_q = ggml_view_3d(ctx, queries, D, 1, 1, queries->nb[1],
                                    queries->nb[2], 0);
        ...
        // Apply the final decoder norm before box refinement
        // (use_normed_output_consistently)
        auto* obj_q_normed =
                sam3_layer_norm(ctx, obj_q, tensors.at("ddec.norm.weight"),
    ...
    auto* obj_queries = ggml_view_3d(ctx, queries, D, NQ, 1, queries->nb[1],
                                     queries->nb[2], 1 * queries->nb[1]);
    obj_queries = ggml_cont(ctx, obj_queries);
    obj_queries =
            sam3_layer_norm(ctx, obj_queries, tensors.at("ddec.norm.weight"),
                            tensors.at("ddec.norm.bias"));
    ggml_set_name(obj_queries, "ddec_normed_output");

    auto* queries_for_seg = ggml_concat(ctx, last_presence, obj_queries, 1);

    auto* class_scores = sam3_dot_product_scoring(ctx, model, obj_queries,
                                                  text_feats, text_valid_mask);
    // class_scores: [NQ, B]

    // ── Presence score ───────────────────────────────────────────────────
    // Presence token head: LN + 3-layer MLP (D→D→D→1)
    auto* pres_out =
            sam3_layer_norm(ctx, last_presence,
                            tensors.at("ddec.presence_token_out_norm.weight"),
                            tensors.at("ddec.presence_token_out_norm.bias"));

    for (int j = 0; j < 3; ++j) {
        auto wn = "ddec.presence_token_head.layers." + std::to_string(j) +
                  ".weight";
        auto bn = "ddec.presence_token_head.layers." + std::to_string(j) +
                  ".bias";
    ...
    // Keep presence as raw logit (no sigmoid yet — applied during
    // post-processing)
...
//   for bb_feat in backbone_feats[:-1][::-1]:  (iterate from second-lowest to
//   highest)
//       prev_fpn = bb_feat + F.interpolate(prev_fpn, size=bb_feat.shape[-2:],
//       mode="nearest") prev_fpn = conv_layers[i](prev_fpn)    # conv on the
//       MERGED result prev_fpn = F.relu(norms[i](prev_fpn))  # GroupNorm then
//       ReLU
//
// Python uses GroupNorm(8, 256) — we use ggml_group_norm which normalizes ne[2]
// (the channel dim) in groups.  The conv output is [W, H, D, B] with D in
// ne[2].
        ...
        struct ggml_tensor* fpn_feats[3])  // [D, W, H, B] at 3 scales
{
    ...
    auto* prev = ggml_cont(
            ctx, ggml_permute(ctx, feat, 2, 0, 1, 3));  // [72, 72, D, B]
    prev = ggml_upscale(ctx, prev, 2,
                        GGML_SCALE_MODE_NEAREST);  // [144, 144, D, B]
    auto* fpn1 = ggml_cont(ctx, ggml_permute(ctx, fpn_feats[1], 2, 0, 1,
                                             3));  // [144, 144, D, B]
    ...
    {
        auto* b3d = ggml_reshape_3d(ctx, seg.up_conv_b[0], 1, 1,
                                    seg.up_conv_b[0]->ne[0]);
    ...
    {
        auto* w3d = ggml_reshape_3d(ctx, seg.up_norm_w[0], 1, 1,
                                    seg.up_norm_w[0]->ne[0]);
        prev = ggml_mul(ctx, prev, ggml_repeat(ctx, w3d, prev));
        auto* bn3d = ggml_reshape_3d(ctx, seg.up_norm_b[0], 1, 1,
                                     seg.up_norm_b[0]->ne[0]);
    ...
    prev = ggml_upscale(ctx, prev, 2,
                        GGML_SCALE_MODE_NEAREST);  // [288, 288, D, B]
    auto* fpn0 = ggml_cont(ctx, ggml_permute(ctx, fpn_feats[0], 2, 0, 1,
                                             3));  // [288, 288, D, B]
    prev = ggml_add(ctx, fpn0, prev);              // merged
    // Conv 3x3 on the MERGED result
    prev = ggml_conv_2d_s1_ph(ctx, seg.up_conv_w[1], prev);
    {
        auto* b3d = ggml_reshape_3d(ctx, seg.up_conv_b[1], 1, 1,
                                    seg.up_conv_b[1]->ne[0]);
        prev = ggml_add(ctx, prev, ggml_repeat(ctx, b3d, prev));
    }
    // GroupNorm(8, 256) then ReLU
    prev = ggml_group_norm(ctx, prev, 8, 1e-5f);
    {
        auto* w3d = ggml_reshape_3d(ctx, seg.up_norm_w[1], 1, 1,
                                    seg.up_norm_w[1]->ne[0]);
        prev = ggml_mul(ctx, prev, ggml_repeat(ctx, w3d, prev));
        auto* bn3d = ggml_reshape_3d(ctx, seg.up_norm_b[1], 1, 1,
                                     seg.up_norm_b[1]->ne[0]);
        prev = ggml_add(ctx, prev, ggml_repeat(ctx, bn3d, prev));
    }
    prev = ggml_relu(ctx, prev);
    ...
    auto* out = ggml_cont(
            ctx, ggml_permute(ctx, prev, 1, 2, 0, 3));  // [D, 288, 288, B]
    return out;
}
...
//   2. _embed_pixels: replace lowest-res FPN feat with spatial portion of
//   encoder output
...
//
// enc_hidden: [D, N_spatial, B] — fusion encoder output (cross-attended in step
// 1) fpn_feats[3]: the 3 FPN features at different resolutions query_outputs:
// [D, N, B] selected object query outputs text_features: [D, T, B] for
// cross-attention (prompt) Returns: mask_logits [W*H, N, B] (raw logits, not
// sigmoid)
static struct ggml_tensor* sam3_build_seg_head_graph(
        struct ggml_context* ctx,
        const sam3_model& model,
        struct ggml_tensor*
                enc_hidden,  // [D, N_spatial, B] fusion encoder output
        struct ggml_tensor* fpn_feats[3],   // FPN features at 3 scales
        struct ggml_tensor* query_outputs,  // [D, N, B]
        struct ggml_tensor*
                text_features,  // [D, T, B] (for cross-attn, can be nullptr)
        struct ggml_tensor* text_attn_bias = nullptr) {
    const auto& seg = model.seg_head;
    const auto& tensors = model.tensors;
    ...
    auto* enc = enc_hidden;
    if (text_features) {
        auto* ca_norm = sam3_layer_norm(
                ctx, enc, tensors.at("seg.cross_attn_norm.weight"),
                tensors.at("seg.cross_attn_norm.bias"));

        auto* ca_mask = sam3_expand_token_attn_bias(ctx, text_attn_bias,
                                                    enc->ne[1], 8, B);
        auto* ca_out = sam3_multihead_attn_fused(
                ctx, ca_norm, text_features, seg.ca_prompt_q_w,
                seg.ca_prompt_q_b, seg.ca_prompt_out_w, seg.ca_prompt_out_b, 8,
                ca_mask);
    ...
    pf_conv = ggml_conv_2d_sk_p0(
            ctx, tensors.at("seg.instance_seg_head.weight"), pf_conv);
    {
        auto* b3d = ggml_reshape_3d(
                ctx, tensors.at("seg.instance_seg_head.bias"), 1, 1,
                tensors.at("seg.instance_seg_head.bias")->ne[0]);
        pf_conv = ggml_add(ctx, pf_conv, ggml_repeat(ctx, b3d, pf_conv));
    }
    auto* pixel_embed = ggml_cont(
            ctx, ggml_permute(ctx, pf_conv, 1, 2, 0, 3));  // [D, W, H, B]
#ifndef NDEBUG
    auto* pixel_embed_dbg =
            ggml_cont(ctx, ggml_permute(ctx, pixel_embed, 2, 0, 1, 3));
    ...
    for (int j = 0; j < 3; ++j) {
        auto wn = "seg.mask_predictor.mask_embed.layers." + std::to_string(j) +
                  ".weight";
        auto bn = "seg.mask_predictor.mask_embed.layers." + std::to_string(j) +
                  ".bias";
        ...
        struct ggml_tensor* gamma)   // [D]
{
    ...
    auto* dw_w_f32 = (dw_w->type == GGML_TYPE_F32)
                             ? dw_w
                             : ggml_cast(ctx, dw_w, GGML_TYPE_F32);
...
// spatial_tpos[i]: temporal position for slot i (0 = conditioning, >0 =
// non-cond t_pos) obj_ptrs[i]: [D] object pointer data ptr_tpos[i]: temporal
// position for pointer i (<0 = skip)
static sam3_prompt_data sam3_build_prompt_and_pos(
        ...
        int eff_feat_size = 0,
        // Optional caches for the immutable model weights read below; when
    ...
    // Per-slot token count comes from the stored feature tensor.
    ...
    if (model.mem_enc.tpos[0]) {
        if (tpos_enc_cache &&
            tpos_enc_cache->size() >= (size_t)(MD * hp.num_maskmem)) {
            tpos_all.assign(tpos_enc_cache->begin(),
                            tpos_enc_cache->begin() + MD * hp.num_maskmem);
        } else {
            sam3_read_f32(model.mem_enc.tpos[0], tpos_all.data(),
                          MD * hp.num_maskmem);
        }
    }
    ...
    for (size_t s = 0; s < mem_slot_feats.size(); ++s) {
        ...
        pd.prompt.insert(pd.prompt.end(), mem_slot_feats[s].begin(),
                         mem_slot_feats[s].end());
        ...
        if (tpos_idx >= 0 && tpos_idx < hp.num_maskmem) {
            int enc_idx =
                    hp.num_maskmem - tpos_idx -
                    1;  // Python: maskmem_tpos_enc[num_maskmem - tpos - 1]
    ...
    // 2. Object pointer tokens (split each D=256 pointer into D/MD=4 tokens of
    // MD=64)
    ...
    if (model.obj_ptr_tpos_w) {
        if (ptr_tpos_w_cache && ptr_tpos_w_cache->size() >= (size_t)(D * MD) &&
            ptr_tpos_b_cache && ptr_tpos_b_cache->size() >= (size_t)MD) {
            tpos_w.assign(ptr_tpos_w_cache->begin(),
                          ptr_tpos_w_cache->begin() + D * MD);
            tpos_b.assign(ptr_tpos_b_cache->begin(),
                          ptr_tpos_b_cache->begin() + MD);
        } else {
    ...
    for (size_t p = 0; p < obj_ptrs.size(); ++p) {
        ...
        for (int j = 0; j < MD; ++j) {
            float sum = tpos_b[j];
            for (int i = 0; i < D; ++i) sum += tpos_w[i + j * D] * sine_pe[i];
            proj_pe[j] = sum;
        }
...
// curr_tokens: [D, N, 1] — current frame image tokens (flattened from 72x72 =
// 5184) src_pos:     [D, N, 1] — sinusoidal PE for current tokens
// (pos_enc_at_input) prompt:      [MD, M_total, 1] — concatenated memory
// features + split pointer tokens prompt_pos:  [MD, M_total, 1] — positional
// encodings for prompt rope_freqs:  [2, D/2, N] — axial RoPE frequencies for Q
// and self-attn K rope_k_freqs:[2, D/2, M_spatial] — repeated axial RoPE for
// cross-attn spatial K num_obj_ptr_tokens: number of pointer tokens at end of
// prompt (excluded from RoPE) Returns: conditioned tokens [D, N, 1]
static struct ggml_tensor* sam3_build_mem_attn_graph(
    ...
    for (int l = 0; l < (int)ma.layers.size(); ++l) {
        ...
        {
            auto* x_norm = sam3_layer_norm(ctx, x, ly.norm1_w, ly.norm1_b);
            auto* q = ggml_add(ctx, ggml_mul_mat(ctx, ly.sa_q_w, x_norm),
                               ly.sa_q_b);
            auto* k = ggml_add(ctx, ggml_mul_mat(ctx, ly.sa_k_w, x_norm),
                               ly.sa_k_b);
            auto* v = ggml_add(ctx, ggml_mul_mat(ctx, ly.sa_v_w, x_norm),
                               ly.sa_v_b);
            ...
            sa_out = ggml_add(ctx, ggml_mul_mat(ctx, ly.sa_out_w, sa_out),
                              ly.sa_out_b);
        ...
        {
            auto* x_norm = sam3_layer_norm(ctx, x, ly.norm2_w, ly.norm2_b);
            auto* q = ggml_add(ctx, ggml_mul_mat(ctx, ly.ca_q_w, x_norm),
                               ly.ca_q_b);

            // K: project from (prompt + prompt_pos) — pos enc added before
            // projection
            auto* kv_with_pos = ggml_add(ctx, prompt, prompt_pos);
            auto* k = ggml_add(ctx, ggml_mul_mat(ctx, ly.ca_k_w, kv_with_pos),
                               ly.ca_k_b);
            // V: project from prompt only (no pos enc)
            auto* v = ggml_add(ctx, ggml_mul_mat(ctx, ly.ca_v_w, prompt),
                               ly.ca_v_b);

            // Apply RoPE to Q
            q = sam3_apply_rope(ctx, q, rope_freqs);

            // Apply RoPE to spatial K only (exclude num_obj_ptr_tokens tail
            // keys)
            if (M_spatial > 0 && rope_k_freqs) {
                auto* k_spatial =
                        ggml_cont(ctx, ggml_view_3d(ctx, k, D, M_spatial, 1,
                                                    k->nb[1], k->nb[2], 0));
                k_spatial = sam3_apply_rope(ctx, k_spatial, rope_k_freqs);
                if (num_obj_ptr_tokens > 0) {
                    auto* k_ptr = ggml_cont(
                            ctx, ggml_view_3d(ctx, k, D, num_obj_ptr_tokens, 1,
                                              k->nb[1], k->nb[2],
            ...
            ca_out = ggml_add(ctx, ggml_mul_mat(ctx, ly.ca_out_w, ca_out),
                              ly.ca_out_b);
        ...
        {
            auto* x_norm = sam3_layer_norm(ctx, x, ly.norm3_w, ly.norm3_b);
            auto* ffn = ggml_add(ctx, ggml_mul_mat(ctx, ly.ffn_fc1_w, x_norm),
                                 ly.ffn_fc1_b);
            ffn = ggml_relu(ctx, ffn);
            ffn = ggml_add(ctx, ggml_mul_mat(ctx, ly.ffn_fc2_w, ffn),
                           ly.ffn_fc2_b);
                                     ...
                                     float* out_ptr)  // [D]
{
    ...
    if (hp.is_sam2() && hp.fixed_no_obj_ptr) {
        ...
        ggml_backend_tensor_get(model.no_obj_ptr, no_ptr.data(), 0,
                                D * sizeof(float));
        for (int i = 0; i < D; ++i)
    ...
    if (obj_score <= 0.0f) {
        ggml_backend_tensor_get(model.no_obj_ptr, out_ptr, 0,
                                D * sizeof(float));
        return;
    }
...
static std::vector<int> sam3_select_memory_frames(
        const std::vector<sam3_memory_slot>& bank, int max_slots) {
...
static void sam3_fill_holes(uint8_t* mask, int w, int h, int area_threshold) {
    ...
    for (int i = 0; i < n; ++i) {
        if (mask[i] <= 127 && labels[i] >= 0 &&
            component_sizes[labels[i]] <= area_threshold) {
            mask[i] = 255;
        }
    }
}
...
static void sam3_remove_sprinkles(uint8_t* mask,
                                  int w,
                                  int h,
                                  int area_threshold) {
    const int n = w * h;
...
static std::vector<float> sam3_bilinear_interpolate(
    ...
    for (int y = 0; y < dst_h; ++y) {
        ...
        for (int x = 0; x < dst_w; ++x) {
            ...
            float v = (1 - wy) * ((1 - wx) * src[y0 * src_w + x0] +
                                  wx * src[y0 * src_w + x1]) +
                      wy * ((1 - wx) * src[y1 * src_w + x0] +
                            wx * src[y1 * src_w + x1]);
            dst[y * dst_w + x] = v;
        }
    }
...
static sam3_box sam3_cxcywh_to_xyxy(
        float cx, float cy, float w, float h, int img_w, int img_h) {
    sam3_box box;
...
sam3_result sam3_segment_pcs(sam3_state& state,
    ...
    if (!state.neck_det[0]) {
        AICORE_LOG_PRINT(
                "[sam3] ",
                "%s: image not encoded — call sam3_encode_image first\n",
                __func__);
        return result;
    }
    if (!sam3_ensure_neck_pe(state, model)) {
        AICORE_LOG_ERROR(
                "[sam3] ",
                "%s: failed to initialize detector positional encodings\n",
                __func__);
        return result;
    }
    ...
    auto token_ids =
            sam3_tokenize(const_cast<sam3_bpe_tokenizer&>(model.tokenizer),
                          params.text_prompt, L);
    if (token_ids.empty()) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to tokenize text prompt\n",
                         __func__);
        return result;
    }
    ...
    // Read image features and PE from state into CPU buffers (used by multiple
    // stages)
    ...
    for (int i = 0; i < 64; ++i)
        sine_dim_t_cpu[i] =
                2.0f * (float)M_PI / powf(10000.0f, 2.0f * (float)i / 128.0f);
    ...
    for (int i = 0; i < N_geo; ++i) combined_bias_cpu[L + i] = 0.0f;
    ...
    for (int i = 0; i < N_geo; ++i) text_valid_mask_cpu[L + i] = tvm_scale;
    ...
    */
    std::vector<float> text_feats_cpu(D * L);
    {
        ...
        if (!ggml_gallocr_reserve(alloc, graph) ||
            ...
            AICORE_LOG_ERROR("[sam3] ", "%s: text encoder alloc failed\n",
                             __func__);
        ...
        if (inp->buffer) {
            ggml_backend_tensor_set(inp, token_ids.data(), 0,
                                    L * sizeof(int32_t));
        } else {
            AICORE_LOG_ERROR(
                    "[sam3] ",
                    "%s: ERROR: text encoder input tensor has no buffer!\n",
                    __func__);
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return result;
        ...
        if (causal && causal->buffer) {
            ...
            ggml_backend_tensor_set(causal, cm.data(), 0,
                                    L * L * sizeof(ggml_fp16_t));
        }
        ...
        ggml_backend_tensor_get(out, text_feats_cpu.data(), 0,
                                D * L * sizeof(float));
    ...
    */
    std::vector<float> geo_feats_cpu(D * N_geo);
    {
        ...
        if (!ggml_gallocr_reserve(alloc, graph) ||
            !ggml_gallocr_alloc_graph(alloc, graph)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: geometry encoder alloc failed\n",
                             __func__);
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return result;
        ...
        ggml_backend_tensor_set(g_img, img_feats_cpu.data(), 0,
                                D * N_spatial * sizeof(float));
        ggml_backend_tensor_set(g_pe, img_pe_cpu.data(), 0,
                                D * N_spatial * sizeof(float));
    ...
    memcpy(combined_prompt_cpu.data(), text_feats_cpu.data(),
           D * L * sizeof(float));
    ...
    */
    std::vector<float> fenc_output_cpu(D * N_spatial);
    {
        ...
        if (!ggml_gallocr_reserve(alloc, graph) ||
            !ggml_gallocr_alloc_graph(alloc, graph)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: fusion encoder alloc failed\n",
                             __func__);
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return result;
        ...
        ggml_backend_tensor_set(img, img_feats_cpu.data(), 0,
                                D * N_spatial * sizeof(float));
        ggml_backend_tensor_set(pe, img_pe_cpu.data(), 0,
                                D * N_spatial * sizeof(float));
        ggml_backend_tensor_set(prompt, combined_prompt_cpu.data(), 0,
                                D * T * sizeof(float));
        ggml_backend_tensor_set(bias, combined_bias_cpu.data(), 0,
                                T * sizeof(float));
    ...
    */
    ...
    {
        const size_t sz =
                ggml_tensor_overhead() * 32768 + ggml_graph_overhead() * 2;
        struct ggml_init_params gp = {sz, nullptr, true};
        auto* ctx = ggml_init(gp);
        ...
        auto dout = sam3_build_ddec_graph(ctx, model, enc, pe, txt, sdt, rpb,
                                          tab, tvm);
        ...
        if (!ggml_gallocr_reserve(alloc, graph) ||
            !ggml_gallocr_alloc_graph(alloc, graph)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: DETR decoder alloc failed\n",
                             __func__);
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return result;
        ...
        ggml_backend_tensor_set(enc, fenc_output_cpu.data(), 0,
                                D * N_spatial * sizeof(float));
        ggml_backend_tensor_set(pe, img_pe_cpu.data(), 0,
                                D * N_spatial * sizeof(float));
        ggml_backend_tensor_set(txt, combined_prompt_cpu.data(), 0,
                                D * T * sizeof(float));
        ggml_backend_tensor_set(sdt, sine_dim_t_cpu.data(), 0,
                                64 * sizeof(float));
        ggml_backend_tensor_set(rpb, rpb_coords_cpu.data(), 0,
                                H * sizeof(float));
        ggml_backend_tensor_set(tab, combined_bias_cpu.data(), 0,
                                T * sizeof(float));
        ggml_backend_tensor_set(tvm, text_valid_mask_cpu.data(), 0,
                                T * sizeof(float));
        ...
        if (rpbz) {
            int n = (int)(rpbz->ne[0] * rpbz->ne[1] * rpbz->ne[2] *
                          rpbz->ne[3]);
        ...
        ggml_backend_tensor_get(dout.class_scores, scores_data.data(), 0,
                                NQ * sizeof(float));
        ggml_backend_tensor_get(dout.pred_boxes, boxes_data.data(), 0,
                                4 * NQ * sizeof(float));
        ggml_backend_tensor_get(dout.presence_score, &presence_logit, 0,
                                sizeof(float));
    ...
    */
    ...
    std::vector<float> all_masks(NQ * mask_hw * mask_hw);
    {
        ...
        auto* out = sam3_build_seg_head_graph(ctx, model, enc_h, fpn_feats, oq,
                                              txt, tab);
        ggml_set_output(out);
        ...
        if (!ggml_gallocr_reserve(alloc, graph) ||
            !ggml_gallocr_alloc_graph(alloc, graph)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: segmentation head alloc failed\n",
                             __func__);
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return result;
        ...
        // Object queries: extract from DETR queries (skip presence token at
        // slot 0) queries_data is flat [D * 201], presence token is at
        // positions [0..D-1] Object queries start at position [D..D*(NQ+1)-1]
        ggml_backend_tensor_set(oq, queries_data.data() + D, 0,
                                D * NQ * sizeof(float));
        ...
        ggml_backend_tensor_get(out, all_masks.data(), 0,
                                all_masks.size() * sizeof(float));
...
static struct ggml_tensor* sam3_sam_attention(
    ...
    V = ggml_reshape_4d(ctx, V, HD, n_heads, N_kv, B);
    V = ggml_cont(ctx, ggml_permute(ctx, V, 0, 2, 1,
                                    3));  // [HD, N_kv, NH, B] contiguous
...
static void sam3_pe_encode_coord(float* out,
                                 float x_norm,
                                 float y_norm,
                                 const float* pe_gauss,
                                 int num_pos_feats) {
    // Map [0,1] → [-1,1]
...
static void sam3_populate_pe_cache(sam3_state& state, const sam3_model& model) {
    ...
    if (pe.pe_gaussian->type == GGML_TYPE_F16) {
        ...
        ggml_backend_tensor_get(pe.pe_gaussian, tmp.data(), 0,
                                pe_nel * sizeof(ggml_fp16_t));
        ggml_fp16_to_fp32_row(tmp.data(), state.pe_gauss_cache.data(), pe_nel);
    } else {
        ggml_backend_tensor_get(pe.pe_gaussian, state.pe_gauss_cache.data(), 0,
                                pe_nel * sizeof(float));
    }
    ...
    for (int i = 0; i < 4; ++i) {
        if (pe.point_embed[i]->type == GGML_TYPE_F16) {
            ...
            ggml_backend_tensor_get(pe.point_embed[i], tmp.data(), 0,
                                    D * sizeof(ggml_fp16_t));
            ggml_fp16_to_fp32_row(tmp.data(), state.point_emb_cache[i], D);
        } else {
            ggml_backend_tensor_get(pe.point_embed[i], state.point_emb_cache[i],
                                    0, D * sizeof(float));
        }
    }
    ...
    if (pe.not_a_point_embed->type == GGML_TYPE_F16) {
        std::vector<ggml_fp16_t> tmp(D);
        ggml_backend_tensor_get(pe.not_a_point_embed, tmp.data(), 0,
                                D * sizeof(ggml_fp16_t));
        ggml_fp16_to_fp32_row(tmp.data(), state.not_a_point_cache, D);
    } else {
        ggml_backend_tensor_get(pe.not_a_point_embed, state.not_a_point_cache,
                                0, D * sizeof(float));
    }

    if (pe.no_mask_embed->type == GGML_TYPE_F16) {
        std::vector<ggml_fp16_t> tmp(D);
        ggml_backend_tensor_get(pe.no_mask_embed, tmp.data(), 0,
                                D * sizeof(ggml_fp16_t));
        ggml_fp16_to_fp32_row(tmp.data(), state.no_mask_emb_cache, D);
    } else {
        ggml_backend_tensor_get(pe.no_mask_embed, state.no_mask_emb_cache, 0,
                                D * sizeof(float));
    }
    ...
    for (int row = 0; row < H; ++row) {
        for (int col = 0; col < H; ++col) {
            for (int d = 0; d < D; ++d)
                state.dense_nomask_cache[d + col * D + row * D * H] =
                        state.no_mask_emb_cache[d];
        }
    }
...
// are merged into the point stream before calling PromptEncoder(points=...,
// boxes=None).
...
// sparse_out: [D, N_pts, 1] where N_pts = n_pos + n_neg + (use_box ? 2 : 0) +
// pad dense_out:  [D, H, H, 1] (no-mask default or mask downsample)
struct sam3_pe_result {
    struct ggml_tensor* sparse;  // [D, N_pts, 1]
    struct ggml_tensor* dense;   // [D, H, H, 1]
    struct ggml_tensor*
            image_pe;  // [D, H, H, 1] — dense positional encoding grid
    int n_tokens;
};
...
static void sam3_twoway_block_forward(
    ...
    if (skip_first_layer_pe) {
        ...
        queries = sam3_sam_attention(ctx, queries, queries, queries,
                                     blk.self_attn, n_heads);
    } else {
        auto* q = ggml_add(ctx, queries, query_pe);
        auto* attn_out =
                sam3_sam_attention(ctx, q, q, queries, blk.self_attn, n_heads);
...
static sam3_dec_result sam3_build_sam_dec_graph(
    ...
    // When pred_obj_scores=True:  [obj_score(1,D), iou(1,D), masks(4,D)] = 6
    // tokens When pred_obj_scores=False: [iou(1,D), masks(4,D)] = 5 tokens
    // (older SAM2)
    ...
    // Final attention: tokens → image
    {
        ...
        auto* k = ggml_add(ctx, keys, key_pe);
        auto* attn_out =
                sam3_sam_attention(ctx, q, k, keys, dec.final_attn, n_heads);
        queries = ggml_add(ctx, queries, attn_out);
        queries = sam3_layer_norm(ctx, queries, dec.final_norm_w,
                                  dec.final_norm_b);
    ...
    auto* iou_token_out = ggml_view_3d(ctx, queries, D, 1, 1, queries->nb[1],
                                       queries->nb[2], s * queries->nb[1]);
    iou_token_out = ggml_cont(ctx, iou_token_out);  // [D, 1, 1]

    auto* mask_tokens_out =
            ggml_view_3d(ctx, queries, D, num_mask_tokens, 1, queries->nb[1],
                         queries->nb[2], (s + 1) * queries->nb[1]);
    ...
    if (has_obj_score) {
        obj_in = ggml_view_3d(ctx, queries, D, 1, 1, queries->nb[1],
                              queries->nb[2], 0);
        obj_in = ggml_cont(ctx, obj_in);  // [D, 1, 1]
    }

    // SAM output token = first mask token, used for object pointer
    auto* sam_token = ggml_view_2d(ctx, queries, D, 1, queries->nb[1],
                                   (s + 1) * queries->nb[1]);
    ...
    auto* up1 = ggml_conv_transpose_2d_p0(
            ctx, sam3_conv_transpose_weight(ctx, dec.up1_w), src_img, 2);
    up1 = ggml_add(ctx, up1,
                   ggml_reshape_4d(ctx, dec.up1_b, 1, 1,
                                   ggml_nelements(dec.up1_b), 1));
    ...
    up1 = ggml_cont(ctx,
                    ggml_permute(ctx, up1, 2, 0, 1, 3));  // [144, 144, 64, 1]

    // dc2: ConvTranspose2d(64, 32, k=2, s=2) → [288, 288, 32, 1]
    auto* up2 = ggml_conv_transpose_2d_p0(
            ctx, sam3_conv_transpose_weight(ctx, dec.up2_w), up1, 2);
    up2 = ggml_add(ctx, up2,
                   ggml_reshape_4d(ctx, dec.up2_b, 1, 1,
                                   ggml_nelements(dec.up2_b), 1));

    // conv_s0: 1x1 conv on feat_s0 (256→32). feat_s0 is [C, W, H, B] — permute
    // for conv.
    auto* fs0 = ggml_cont(
            ctx, ggml_permute(ctx, feat_s0, 2, 0, 1, 3));     // [W, H, C, B]
    ...
    up2 = ggml_cont(ctx,
                    ggml_permute(ctx, up2, 1, 2, 0, 3));  // [32, 288, 288, 1]
    ...
    // ── IoU prediction ───────────────────────────────────────────────────
    // iou_token_out: [D, 1, 1]
    auto* iou_pred = sam3_mlp_forward(ctx, iou_token_out, dec.iou_head_w,
                                      dec.iou_head_b, 3,
                                      /*sigmoid_output=*/true);
    ...
    if (has_obj_score) {
        ...
        obj_score = sam3_mlp_forward(ctx, obj_in, dec.obj_head_w,
                                     dec.obj_head_b, 3);
        obj_score = ggml_reshape_2d(ctx, obj_score, 1, 1);
    } else {
        // No obj_score prediction — return raw logit 10.0 (sigmoid ≈ 1.0,
...
sam3_result sam3_segment_pvs(sam3_state& state,
    ...
    if (params.pos_points.empty() && !params.use_box) {
        AICORE_LOG_ERROR(
                "[sam3] ",
                "%s: no prompts provided (need at least one point or box)\n",
                __func__);
        return result;
    }
    ...
    // Instead: create fresh input tensors and copy the state data into them.
    ...
    auto dec_out = sam3_build_sam_dec_graph(ctx0, model, image_feats,
                                            pe_out.image_pe, pe_out.sparse,
                                            pe_out.dense, feat_s0, feat_s1, H);
...

    {
        ...
        for (int p = 0; p < N_pts; ++p) {
            // Scale from original image space to model input space, then shift
            // to pixel center
            float px = all_coords[p * 2 + 0] / (float)state.orig_width *
                               (float)eff_img_size +
                       0.5f;
            float py = all_coords[p * 2 + 1] / (float)state.orig_height *
                               (float)eff_img_size +
                       0.5f;
    ...
    // image_feats = neck_trk[2] + no_mem_embed (computed on CPU)
    {
        ...
        ggml_backend_tensor_get(state.neck_trk[2], trk2.data(), 0,
                                n2 * sizeof(float));
        ggml_backend_tensor_get(model.tensors.at("no_mem_embed"),
                                no_mem_data.data(), 0, D * sizeof(float));
    ...
    ggml_backend_tensor_get(dec_out.masks, masks_data.data(), 0,
                            masks_data.size() * sizeof(float));

    // IoU predictions: [4, 1]
    std::vector<float> iou_data(num_mask_tokens);
    ggml_backend_tensor_get(dec_out.iou_pred, iou_data.data(), 0,
...
// Lazily compute and cache PE/RoPE data that is identical across all
// propagation calls.
static void sam3_ensure_tracker_pe_caches(sam3_tracker& tracker,
    ...
    for (int n = 0; n < N; ++n)
        for (int i = 0; i < half_d; ++i) {
            tracker.cached_axial_cis_reord[0 + i * 2 + n * D] =
                    rope_raw[n * D + i * 2 + 0];
            tracker.cached_axial_cis_reord[1 + i * 2 + n * D] =
                    rope_raw[n * D + i * 2 + 1];
        }
...
static sam3_prop_output sam3_propagate_single(
    ...
    if (pd.M_spatial > 0) {
        ...
        // Repeat the HxH axial CIS for each memory frame.
        for (int s = 0; s < pd.M_spatial / N; ++s)
            memcpy(rope_k_data.data() + s * D * N, rope_q_reord.data(),
                   D * N * sizeof(float));
    }

    // Fill the one-time caches for the immutable model weights used by
    // sam3_build_prompt_and_pos and for the frame-invariant propagate inputs
    // (rope_q/rope_k, src_pos, sparse, image_pe, dense_emb).  These are
    // uploaded once after each graph rebuild and reused afterwards.
    if (!tracker.mem_w_cache_valid) {
        tracker.mem_tpos_enc.assign((size_t)MD * hp.num_maskmem, 0.0f);
        if (model.mem_enc.tpos[0]) {
            sam3_read_f32(model.mem_enc.tpos[0], tracker.mem_tpos_enc.data(),
                          MD * hp.num_maskmem);
        }
        tracker.mem_ptr_tpos_w.assign((size_t)D * MD, 0.0f);
        tracker.mem_ptr_tpos_b.assign((size_t)MD, 0.0f);
        if (model.obj_ptr_tpos_w) {
            sam3_read_f32(model.obj_ptr_tpos_w, tracker.mem_ptr_tpos_w.data(),
                          D * MD);
            sam3_read_f32(model.obj_ptr_tpos_b, tracker.mem_ptr_tpos_b.data(),
                          MD);
        }
        tracker.mem_w_cache_valid = true;
    }

    // ── Build or reuse cached propagate graph ──────────────────────────
    // The graph topology is a pure function of M_total (total prompt tokens).
    // M_total stabilises after a few frames (once num_maskmem memory slots
    // are filled) and stays constant for the rest of the video.  Caching
    // eliminates per-frame graph build + alloc overhead and keeps tensor
    // addresses stable so that CUDA graph capture can actually hit.
    const bool prop_cached = (tracker.prop_ctx != nullptr) &&
                             (tracker.prop_M_total == pd.M_total);

    struct ggml_tensor *curr, *src_pos_t, *prompt_t, *prompt_pos_t;
    struct ggml_tensor *rope_q_t, *rope_k_t = nullptr;
    struct ggml_tensor *sparse_in, *image_pe, *dense_emb;
    struct ggml_tensor *trk_s0, *trk_s1;
    sam3_dec_result dec;
    struct ggml_context* ctx0;
    struct ggml_cgraph* graph;
    struct ggml_gallocr* galloc;
    ...
    if (prop_cached) {
        // Restore all tensor pointers from the cached graph.
        ctx0 = tracker.prop_ctx;
        graph = tracker.prop_graph;
        galloc = tracker.prop_galloc;
        curr = tracker.prop_inp_curr;
        src_pos_t = tracker.prop_inp_src_pos;
        prompt_t = tracker.prop_inp_prompt;
        prompt_pos_t = tracker.prop_inp_prompt_pos;
        rope_q_t = tracker.prop_inp_rope_q;
        rope_k_t = tracker.prop_inp_rope_k;
        sparse_in = tracker.prop_inp_sparse;
        image_pe = tracker.prop_inp_image_pe;
        dense_emb = tracker.prop_inp_dense_emb;
        trk_s0 = tracker.prop_inp_trk_s0;
        trk_s1 = tracker.prop_inp_trk_s1;
        dec.masks = tracker.prop_out_masks;
        dec.iou_pred = tracker.prop_out_iou;
        dec.obj_score = tracker.prop_out_obj;
        dec.sam_token = tracker.prop_out_sam_tok;
        dec.mask_tokens = tracker.prop_out_mask_toks;
    } else {
        // Invalidate stale cache
        if (tracker.prop_galloc) {
            ggml_gallocr_free(tracker.prop_galloc);
            tracker.prop_galloc = nullptr;
        }
        if (tracker.prop_ctx) {
            ggml_free(tracker.prop_ctx);
            tracker.prop_ctx = nullptr;
        }
        ...
        prompt_pos_t =
                ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, MD, pd.M_total, 1);
        ...
        if (pd.M_spatial > 0) {
            rope_k_t = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, 2, half_d,
                                          pd.M_spatial);
        ...
        auto* conditioned = sam3_build_mem_attn_graph(
                ctx0, model, curr, src_pos_t, prompt_t, prompt_pos_t, rope_q_t,
                rope_k_t, pd.num_obj_ptr_tokens);
        ...
        dec = sam3_build_sam_dec_graph(ctx0, model, cond_spatial, image_pe,
                                       sparse_in, dense_emb, trk_s0, trk_s1, H);
        ...
        // Cache the graph and all tensor pointers for subsequent frames.
        tracker.prop_ctx = ctx0;
        tracker.prop_graph = graph;
        tracker.prop_galloc = galloc;
        tracker.prop_M_total = pd.M_total;
        tracker.prop_inp_curr = curr;
        tracker.prop_inp_src_pos = src_pos_t;
        tracker.prop_inp_prompt = prompt_t;
        tracker.prop_inp_prompt_pos = prompt_pos_t;
        tracker.prop_inp_rope_q = rope_q_t;
        tracker.prop_inp_rope_k = rope_k_t;
        tracker.prop_inp_sparse = sparse_in;
        tracker.prop_inp_image_pe = image_pe;
        tracker.prop_inp_dense_emb = dense_emb;
        tracker.prop_inp_trk_s0 = trk_s0;
        tracker.prop_inp_trk_s1 = trk_s1;
        tracker.prop_out_masks = dec.masks;
        tracker.prop_out_iou = dec.iou_pred;
        tracker.prop_out_obj = dec.obj_score;
        tracker.prop_out_sam_tok = dec.sam_token;
        tracker.prop_out_mask_toks = dec.mask_tokens;
        tracker.prop_consts_uploaded =
                false;  // new tensors: re-upload constants
    }
    ...
    // All other inputs (rope_q/rope_k, src_pos, sparse, image_pe, dense_emb)
    // are frame-invariant — upload them only once after each graph rebuild.
    if (!tracker.prop_consts_uploaded) {
        ggml_backend_tensor_set(rope_q_t, rope_q_reord.data(), 0,
                                rope_q_reord.size() * sizeof(float));
        if (rope_k_t && !rope_k_data.empty())
            ggml_backend_tensor_set(rope_k_t, rope_k_data.data(), 0,
        ...
        ggml_backend_tensor_set(
                src_pos_t, tracker.cached_sinpe_256.data(), 0,
                tracker.cached_sinpe_256.size() * sizeof(float));

        // Upload not_a_point_embed, image_pe, dense_emb from state PE cache
        sam3_populate_pe_cache(state, model);
        ggml_backend_tensor_set(sparse_in, state.not_a_point_cache, 0,
                                D * sizeof(float));
        ggml_backend_tensor_set(image_pe, state.dense_pe_cache.data(), 0,
                                state.dense_pe_cache.size() * sizeof(float));
        ggml_backend_tensor_set(
                dense_emb, state.dense_nomask_cache.data(), 0,
                state.dense_nomask_cache.size() * sizeof(float));
        tracker.prop_consts_uploaded = true;
    }

    // Copy tracker features from state to fresh input tensors.
    // GPU→GPU direct copy avoids the expensive GPU→CPU→GPU round trip
    // through a host staging buffer (~11 MB of DMA transfers per frame).
    // ggml_backend_tensor_copy requires matching layouts (same ne[] dims),
    // so we reshape the 3-D curr to 4-D [D,H,H,1] to match neck_trk[2].
    // The reshape is a zero-cost view — the graph still sees [D,N,1].
    {
        auto* curr_4d = ggml_reshape_4d(
                ctx0, curr, state.neck_trk[2]->ne[0], state.neck_trk[2]->ne[1],
    ...
    if (use_multimask) {
        ...
        ggml_backend_tensor_get(dec.masks, output.mask_logits.data(),
                                best_idx * mhw * mhw * sizeof(float),
                                mhw * mhw * sizeof(float));
        output.iou_scores.resize(1);
        output.iou_scores[0] = best_iou;
        ggml_backend_tensor_get(dec.obj_score, &output.obj_score, 0,
                                sizeof(float));

        // Object pointer token: use best multimask token if
        ...
        if (hp.use_multimask_token_for_obj_ptr && dec.mask_tokens) {
            ggml_backend_tensor_get(dec.mask_tokens, output.sam_token.data(),
                                    best_idx * D * sizeof(float),
                                    D * sizeof(float));
        } else {
            ggml_backend_tensor_get(dec.sam_token, output.sam_token.data(), 0,
                                    D * sizeof(float));
        }
    } else {
        output.n_masks = 1;
        ...
        output.mask_w = mhw;
        output.mask_logits.resize(mhw * mhw);
        ggml_backend_tensor_get(dec.masks, output.mask_logits.data(), 0,
                                mhw * mhw * sizeof(float));
        output.iou_scores.resize(1);
        ggml_backend_tensor_get(dec.iou_pred, output.iou_scores.data(), 0,
                                sizeof(float));
        ggml_backend_tensor_get(dec.obj_score, &output.obj_score, 0,
                                sizeof(float));
        output.sam_token.resize(D);
...
static bool sam3_encode_memory(sam3_tracker& tracker,
    ...
    // ── Build or reuse the cached memory-encoder graph ──────────────────
    // The topology is a pure function of (hparams, H); rebuild only when H
    // changes.  Reuse keeps tensor addresses stable so CUDA graph capture
    // hits, and eliminates the per-frame ggml_init/build/alloc/free cycle.
    const bool mem_cached =
            (tracker.mem_ctx != nullptr) && (tracker.mem_H == H);
    ...
    if (mem_cached) {
        ...
        // Invalidate stale cache
        if (tracker.mem_galloc) {
            ggml_gallocr_free(tracker.mem_galloc);
            tracker.mem_galloc = nullptr;
        }
        if (tracker.mem_ctx) {
            ggml_free(tracker.mem_ctx);
            tracker.mem_ctx = nullptr;
        }
        ...
        mask_in = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, INTERPOL, INTERPOL, 1,
                                     1);
        ggml_set_name(mask_in, "mem_mask");
        ggml_set_input(mask_in);

        // Learned mask downsampler: 4 stages conv+LN+GELU + final 1×1
        // Conv output is in ggml "conv" layout. permute(1,2,0,3) converts to
        // "internal" layout where ne[0]=channel for LN2d. permute(2,0,1,3)
        // converts back to "conv" layout.
        auto* ds = mask_in;
        for (int s = 0; s < 4; ++s) {
            int out_ch = (int)model.mem_enc.ds_conv_w[s]->ne[3];
            ds = ggml_conv_2d(ctx0, model.mem_enc.ds_conv_w[s], ds, 2, 2, 1, 1,
                              1, 1);
            ds = ggml_add(ctx0, ds,
                          ggml_reshape_4d(ctx0, model.mem_enc.ds_conv_b[s], 1,
                                          1, out_ch, 1));
            ds = ggml_cont(ctx0,
                           ggml_permute(ctx0, ds, 1, 2, 0,
                                        3));  // conv→internal (ne[0]=channel)
            ds = sam3_layer_norm_2d(ctx0, ds, model.mem_enc.ds_norm_w[s],
                                    model.mem_enc.ds_norm_b[s]);
            ds = ggml_gelu(ctx0, ds);
            ds = ggml_cont(
                    ctx0, ggml_permute(ctx0, ds, 2, 0, 1, 3));  // internal→conv
        }
        ds = ggml_conv_2d(ctx0, model.mem_enc.ds_conv_w[4], ds, 1, 1, 0, 0, 1,
                          1);
        ds = ggml_add(
                ctx0, ds,
                ggml_reshape_4d(ctx0, model.mem_enc.ds_conv_b[4], 1, 1, D, 1));
        ds = ggml_cont(ctx0, ggml_permute(ctx0, ds, 1, 2, 0,
                                          3));  // conv→internal [D, ...]

        // Pixel projection — use fresh input tensor to avoid pulling in the
        // entire ViT+neck recomputation from state.neck_trk[2]'s dependency
        // tree
        pix_in_raw = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, D, H, H, 1);
        ggml_set_name(pix_in_raw, "mem_pix_feat");
        ggml_set_input(pix_in_raw);
        auto* pix_in =
                ggml_cont(ctx0, ggml_permute(ctx0, pix_in_raw, 2, 0, 1, 3));
        auto* pix = ggml_conv_2d(ctx0, model.mem_enc.pix_proj_w, pix_in, 1, 1,
                                 0, 0, 1, 1);
        pix = ggml_add(
                ctx0, pix,
                ggml_reshape_4d(ctx0, model.mem_enc.pix_proj_b, 1, 1, D, 1));
        pix = ggml_cont(ctx0, ggml_permute(ctx0, pix, 1, 2, 0, 3));

        // Fusion: ADD (not multiply)
        auto* fused = ggml_add(ctx0, pix, ds);
        for (int i = 0; i < 2; ++i)
            fused = sam3_cxblock_forward(
                    ctx0, fused, model.mem_enc.fuser_dw_w[i],
                    model.mem_enc.fuser_dw_b[i], model.mem_enc.fuser_norm_w[i],
                    model.mem_enc.fuser_norm_b[i], model.mem_enc.fuser_fc1_w[i],
                    model.mem_enc.fuser_fc1_b[i], model.mem_enc.fuser_fc2_w[i],
                    model.mem_enc.fuser_fc2_b[i], model.mem_enc.fuser_gamma[i]);
        auto* fused_out =
                ggml_cont(ctx0, ggml_permute(ctx0, fused, 2, 0, 1, 3));
        auto* mo = ggml_conv_2d(ctx0, model.mem_enc.out_proj_w, fused_out, 1, 1,
                                0, 0, 1, 1);
        mo = ggml_add(
                ctx0, mo,
                ggml_reshape_4d(ctx0, model.mem_enc.out_proj_b, 1, 1, MD, 1));
        mem_out = ggml_cont(ctx0, ggml_permute(ctx0, mo, 1, 2, 0, 3));
        ggml_set_name(mem_out, "mem_out");
    ...
    ggml_backend_tensor_set(mask_in, m_interp.data(), 0,
    ...
    // common path keeps the result on the GPU and copies it into the slot.
    std::vector<float> md;
    const bool need_noobj = (obj_score <= 0.0f && model.no_obj_embed_spatial);
    if (need_noobj) {
        md.resize(MD * H * H);
        ggml_backend_tensor_get(mem_out, md.data(), 0,
                                md.size() * sizeof(float));
        ...
        if (need_noobj) {
            ...
            if (noe->type == GGML_TYPE_F16) {
                ...
                ggml_backend_tensor_get(noe, tmp.data(), 0,
                                        MD * sizeof(ggml_fp16_t));
                ggml_fp16_to_fp32_row(tmp.data(), no_obj_emb.data(), MD);
            } else {
                ggml_backend_tensor_get(noe, no_obj_emb.data(), 0,
                                        MD * sizeof(float));
            }
            for (int i = 0; i < MD * H * H; ++i) md[i] += no_obj_emb[i % MD];
        }
    }
    ...
    // Store raw [MD, H, H] features for SAM2/SAM3.
    // Store spatial features
    auto* st = ggml_new_tensor_4d(tracker.ctx, GGML_TYPE_F32, MD, H, H, 1);
    auto* sb = ggml_backend_alloc_buffer(model.backend,
                                         MD * H * H * sizeof(float));
    ...
    if (need_noobj) {
        ...
        // GPU→GPU direct copy: avoid the GPU→CPU→GPU round trip through a
        // host staging buffer (~1.3 MB of DMA per frame).
...
static void sam3_store_obj_ptr(sam3_tracker& tracker,
                               const sam3_model& model,
                               int inst_id,
                               const float* pd,
                               int frame_idx) {
...
void sam3_tracker_set_text_prompt(sam3_tracker& tracker,
                                  const std::string& text) {
    tracker.params.text_prompt = text;
}
...
sam3_result sam3_track_frame(sam3_tracker& tracker,
    ...
    // Match new PCS detections against ALL tracked objects (both active AND
    // pending) so that the same object isn't detected as a new instance every
    // frame.
    ...
    auto mat =
            sam3_match_detections(all_tracked, nd.detections, all_tracked_masks,
                                  tracker.params.assoc_iou_threshold);
        ...
        if (!det.mask.data.empty()) {
            ...
            for (int r = 0; r < mh; ++r) {
                for (int c = 0; c < mw; ++c) {
                    ...
                    det_logits[r * mw + c] =
                            (det.mask.data[sy * det.mask.width + sx] > 127)
                                    ? 5.0f
                                    : -5.0f;
                }
            }
            ...
            if (nop->type == GGML_TYPE_F16) {
                std::vector<ggml_fp16_t> tmp(D);
                ggml_backend_tensor_get(nop, tmp.data(), 0,
                                        D * sizeof(ggml_fp16_t));
                ggml_fp16_to_fp32_row(tmp.data(), no_ptr.data(), D);
            } else {
                ggml_backend_tensor_get(nop, no_ptr.data(), 0,
                                        D * sizeof(float));
            }
    ...
    for (auto& ml : tracker.pending) {
        ...
        if (it != pm.end() && !it->second.data.empty()) {
            ...
            if (di != pending_det_idx.end() &&
                di->second < (int)nd.detections.size()) {
...
bool sam3_refine_instance(sam3_tracker& tracker,
    ...
    if (!tgt) {
        AICORE_LOG_ERROR("[sam3] ", "%s: instance %d not found\n", __func__,
                         instance_id);
        return false;
    }
    sam3_pvs_params pvs;
...
int sam3_tracker_add_instance(sam3_tracker& tracker,
    ...
    if (r.detections.empty()) {
        AICORE_LOG_ERROR("[sam3] ", "%s: PVS returned no masks\n", __func__);
        return -1;
    }

    const auto& det = r.detections[0];
    if (det.mask.data.empty()) {
        AICORE_LOG_PRINT("[sam3] ", "%s: PVS mask is empty\n", __func__);
        return -1;
    }
    ...
    if (!sam3_encode_memory(tracker, state, model, inst_id, synth_logits.data(),
                            mask_hw, mask_hw, fi, true, obj_score)) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: failed to encode memory for instance %d\n",
                         __func__, inst_id);
        return -1;
    }
    ...
    if (!det.sam_token.empty()) {
        sam3_extract_obj_ptr_cpu(model, det.sam_token.data(), obj_score,
                                 op.data());
    } else {
        std::fill(op.begin(), op.end(), 0.0f);
    }
...
int sam3_tracker_add_instance_from_mask(sam3_tracker& tracker,
    ...
    if (mask.data.empty() || mask.width <= 0 || mask.height <= 0) {
        AICORE_LOG_PRINT("[sam3] ", "%s: input mask is empty\n", __func__);
        return -1;
    }
    if ((int)mask.data.size() < mask.width * mask.height) {
        AICORE_LOG_PRINT("[sam3] ", "%s: mask data (%zu) smaller than %dx%d\n",
    ...
    // scale/bias, so +6/-6 gives sigmoid ~0.9975/~0.0025 — practically
    // identical to real mask-decoder logits. (Same scheme as
    ...
    if (!seeded_ptr) {
        ...
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: seed decode failed, falling back to no_obj_ptr\n",
                         __func__);
...
void sam3_tracker_reset(sam3_tracker& tracker) {
    ...
    // Free cached memory-encoder graph
    if (tracker.mem_galloc) {
        ggml_gallocr_free(tracker.mem_galloc);
        tracker.mem_galloc = nullptr;
    }
    ...
    tracker.prop_graph = nullptr;
    tracker.prop_M_total = -1;
    tracker.pe_caches_valid = false;
    tracker.mem_w_cache_valid = false;
...
sam3_tracker_ptr sam3_create_visual_tracker(
        const sam3_model& model, const sam3_visual_track_params& params) {
...
bool sam3_test_load_tokenizer(const std::string& model_path) {
    ...
    gguf_kv r{gguf, kv_err};
    sam3_hparams hp;
    if (!sam3_load_hparams(r, hp) || hp.visual_only) {
        gguf_free(gguf);
        ggml_free(gguf_ctx);
        return false;
    }

    const bool ok = sam3_load_bpe_vocab_from_gguf(r, g_test_tokenizer);
    gguf_free(gguf);
    ggml_free(gguf_ctx);
    g_test_tokenizer_loaded = ok;
    return ok;
}
...
static bool sam3_dump_tensor_to_path(struct ggml_tensor* t,
    ...
    if (!t) {
        AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' is null\n", __func__,
                         tensor_name.c_str());
        return false;
    }
    ...
    if (ggml_is_contiguous(t)) {
        if (t->type == GGML_TYPE_F16) {
            ...
            ggml_backend_tensor_get(t, f16_data.data(), 0,
                                    numel * sizeof(ggml_fp16_t));
            ...
            for (int64_t i3 = 0; i3 < ne3; ++i3) {
                for (int64_t i2 = 0; i2 < ne2; ++i2) {
                    for (int64_t i1 = 0; i1 < ne1; ++i1) {
                        const size_t row_idx = ((size_t)i3 * ne2 * ne1 +
                                                (size_t)i2 * ne1 + (size_t)i1) *
                                               ne0;
                        const size_t offs =
                                i3 * t->nb[3] + i2 * t->nb[2] + i1 * t->nb[1];
                        ggml_backend_tensor_get(t, row.data(), offs,
                                                ne0 * sizeof(ggml_fp16_t));
                        ggml_fp16_to_fp32_row(row.data(), data.data() + row_idx,
                                              ne0);
                    }
                }
            }
            ...
            for (int64_t i3 = 0; i3 < ne3; ++i3) {
                for (int64_t i2 = 0; i2 < ne2; ++i2) {
                    for (int64_t i1 = 0; i1 < ne1; ++i1) {
                        ...
                        const size_t offs =
                                i3 * t->nb[3] + i2 * t->nb[2] + i1 * t->nb[1];
                        ggml_backend_tensor_get(t, data.data() + row_idx, offs,
                                                ne0 * sizeof(float));
                    }
                }
            }
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: dumped '%s' [", __func__,
                     tensor_name.c_str());
    for (int i = 0; i < ggml_n_dims(t); ++i) {
        if (i > 0) AICORE_LOG_PRINT("[sam3] ", ", ");
        AICORE_LOG_PRINT("[sam3] ", "%lld", (long long)t->ne[i]);
    }
    AICORE_LOG_PRINT("[sam3] ", "] to %s\n", output_path.c_str());
...
static bool sam3_load_ref_f32_data(const std::string& path,
    ...
    if (!shape_f) {
        AICORE_LOG_ERROR("[sam3] ", "%s: missing %s.shape\n", __func__,
                         path.c_str());
        return false;
    }
    ...
    if (expected_numel >= 0 && numel != expected_numel) {
        AICORE_LOG_PRINT("[sam3] ", "%s: %s expected %d elements, got %lld\n",
                         __func__, path.c_str(), expected_numel,
                         (long long)numel);
        return false;
    }
    ...
    if (!data_f) {
        AICORE_LOG_ERROR("[sam3] ", "%s: missing %s.bin\n", __func__,
                         path.c_str());
        return false;
    }
...
static std::vector<float> sam3_reorder_nchw_to_ggml_dwh(
        const std::vector<float>& src, int channels, int height, int width) {
    std::vector<float> dst((size_t)channels * height * width);
    for (int c = 0; c < channels; ++c) {
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                const size_t src_idx =
                        (size_t)c * height * width + (size_t)y * width + x;
                const size_t dst_idx = (size_t)c + (size_t)x * channels +
                                       (size_t)y * channels * width;
...
static bool sam3_load_kv_text_file(const std::string& path,
    ...
    std::ifstream f(path);
    if (!f) {
        AICORE_LOG_ERROR("[sam3] ", "%s: missing %s\n", __func__, path.c_str());
        return false;
    }
...
bool sam3_test_dump_text_encoder(const sam3_model& model,
    ...
    if ((int)token_ids.size() != L) {
        AICORE_LOG_PRINT("[sam3] ", "%s: expected %d token IDs, got %zu\n",
    ...
    auto* text_features =
            sam3_build_text_encoder_graph(ctx0, inp_tokens, model);
    std::vector<std::string> tensor_names = {
            "causal_mask",   "text_token_embed", "text_after_pos_embed",
            "text_final_ln", "text_features_2d",
    };
    ...
    if (!ggml_gallocr_reserve(galloc, graph) ||
        !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: failed to allocate text encoder graph\n",
                         __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    ...
    ggml_backend_tensor_set(inp_tokens, token_ids.data(), 0,
                            L * sizeof(int32_t));

    auto* causal_mask = ggml_get_tensor(ctx0, "causal_mask");
    if (!causal_mask) {
        AICORE_LOG_ERROR("[sam3] ", "%s: causal_mask tensor not found\n",
                         __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    ...
    ggml_backend_tensor_set(causal_mask, mask_data.data(), 0,
                            L * L * sizeof(ggml_fp16_t));
    ...
    for (const auto& name : tensor_names) {
        auto* t = ggml_get_tensor(ctx0, name.c_str());
        if (!t) {
            AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' not found in graph\n",
                             __func__, name.c_str());
            ok = false;
            continue;
        }
...
bool sam3_test_dump_phase5(const sam3_model& model,
    ...
    auto& mutable_state = const_cast<sam3_state&>(state);
    if (!state.neck_det[0] || !sam3_ensure_neck_pe(mutable_state, model)) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: encoded detector features are missing\n",
                         __func__);
        return false;
    }

    const size_t buf_size =
            ggml_tensor_overhead() * 65536 + ggml_graph_overhead() * 2;
    struct ggml_init_params gparams = {
            /*.mem_size   =*/buf_size,
            /*.mem_buffer =*/nullptr,
            /*.no_alloc   =*/true,
    ...
    auto* text_features_snap =
            ggml_cont(ctx0, ggml_reshape_2d(ctx0, text_features_2d, D, L));
    ...
    auto* conditioned = sam3_build_fenc_graph(
            ctx0, model, img_feats, text_features, img_pe, text_attn_bias);
    ...
    auto ddec_out = sam3_build_ddec_graph(ctx0, model, conditioned, img_pe,
                                          text_features, sine_dim_t, rpb_coords,
                                          text_attn_bias, text_valid_mask);
    ...
    auto* obj_queries = ggml_view_3d(
            ctx0, ddec_out.queries, D, NQ, 1, ddec_out.queries->nb[1],
            ddec_out.queries->nb[2], 1 * ddec_out.queries->nb[1]);
    obj_queries = ggml_cont(ctx0, obj_queries);

    auto* mask_logits = sam3_build_seg_head_graph(
            ctx0, model, conditioned, fpn_feats, obj_queries, text_features,
            text_attn_bias);
    ...
    const std::vector<named_tensor> direct_tensors = {
            ...
            {"ddec_ref_pts_raw",
             model.tensors.at("ddec.reference_points.weight")},
    ...
    if (!ggml_gallocr_reserve(galloc, graph) ||
        !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate phase 5 graph\n",
                         __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
...

    {
        ...
        for (int i = 0; i < 64; ++i) {
            dim_t_data[i] = 2.0f * (float)M_PI /
                            powf(10000.0f, 2.0f * (float)i / 128.0f);
        }
        ggml_backend_tensor_set(sine_dim_t, dim_t_data.data(), 0,
                                64 * sizeof(float));
    }

    {
        ...
        ggml_backend_tensor_set(rpb_coords, coords.data(), 0,
                                H * sizeof(float));
    }
    ...
    if (rpb_pz) {
        int n = (int)(rpb_pz->ne[0] * rpb_pz->ne[1] * rpb_pz->ne[2] *
                      rpb_pz->ne[3]);
...

    {
        ...
        ggml_backend_tensor_set(text_valid_mask, valid_mask.data(), 0,
                                L * sizeof(float));
        ggml_backend_tensor_set(text_attn_bias, attn_bias.data(), 0,
                                L * sizeof(float));
    }
    ...
    for (const auto& item : direct_tensors) {
        if (!sam3_dump_tensor_to_path(item.tensor, item.name,
...
bool sam3_test_dump_phase5_from_ref_inputs(
        const sam3_model& model,
        const std::vector<int32_t>& token_ids,
    ...
    if (!sam3_load_ref_f32_data(prephase_ref_dir + "/neck_det_0", neck_det_0,
                                D * H0 * H0) ||
        !sam3_load_ref_f32_data(prephase_ref_dir + "/neck_det_1", neck_det_1,
                                D * H1 * H1) ||
        !sam3_load_ref_f32_data(phase5_ref_dir + "/fenc_img_input",
                                fenc_img_input_data, D * H * H) ||
        !sam3_load_ref_f32_data(phase5_ref_dir + "/fenc_pos_embed",
                                fenc_pos_embed_data, D * H * H) ||
        !sam3_load_ref_f32_data(phase5_ref_dir + "/img_pe_72", img_pe_72_data,
                                D * H * H) ||
        !sam3_load_ref_f32_data(phase5_ref_dir + "/text_features",
                                text_features_data, D * L)) {
        return false;
    }
    ...
    ggml_backend_tensor_set(text_features, text_features_data.data(), 0,
                            text_features_data.size() * sizeof(float));
    ggml_backend_tensor_set(neck0, neck_det_0.data(), 0,
                            neck_det_0.size() * sizeof(float));
    ggml_backend_tensor_set(neck1, neck_det_1.data(), 0,
                            neck_det_1.size() * sizeof(float));
    ggml_backend_tensor_set(img_feats, fenc_img_input_data.data(), 0,
                            fenc_img_input_data.size() * sizeof(float));
    ggml_backend_tensor_set(img_pe, fenc_pos_embed_data.data(), 0,
                            fenc_pos_embed_data.size() * sizeof(float));
    ggml_backend_tensor_set(img_pe_72_cmp, img_pe_72_data.data(), 0,
                            img_pe_72_data.size() * sizeof(float));
...
bool sam3_test_fenc_only(const sam3_model& model,
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: D=%d H=%d N=%d fenc_layers=%d\n", __func__,
                     D, H, N, hp.fenc_layers);
    ...
    if (!sam3_load_ref_f32_data(ref_dir + "/fenc_input_tgt", img_feat_data,
                                D * N)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to load fenc_input_tgt\n",
                         __func__);
        return false;
    }
    if (!sam3_load_ref_f32_data(ref_dir + "/fenc_input_pos", pos_data, D * N)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to load fenc_input_pos\n",
                         __func__);
        return false;
    }
    if (!sam3_load_ref_f32_data(ref_dir + "/fenc_input_prompt", prompt_data)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to load fenc_input_prompt\n",
                         __func__);
        return false;
    }
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: prompt tokens T=%d\n", __func__, T);

    // Attn bias: [1, T] float (0.0 valid, -1e9 padding)
    // If not present, assume all valid (no bias)
    bool have_attn_bias = sam3_load_ref_f32_data(ref_dir + "/fenc_attn_bias",
                                                 attn_bias_data, T);
    if (!have_attn_bias) {
        AICORE_LOG_ERROR(
                "[sam3] ",
                "%s: no fenc_attn_bias found, assuming all tokens valid\n",
                __func__);
    ...
    auto* conditioned = sam3_build_fenc_graph(
            ctx0, model, img_feats, prompt_tokens, pos_enc, text_attn_bias);
    ...
    if (!ggml_gallocr_reserve(galloc, graph) ||
        !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate fenc graph\n",
                         __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    ...
    // ── Compute ──
    AICORE_LOG_PRINT("[sam3] ", "%s: computing fenc graph (%d nodes)...\n",
    ...
    for (const auto& name : output_names) {
        auto* t = ggml_get_tensor(ctx0, name.c_str());
        if (!t) {
            AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' not found\n", __func__,
                             name.c_str());
            ok = false;
            continue;
        }
    ...
    sam3_dump_tensor_to_path(img_feats, "fenc_img_input",
                             output_dir + "/fenc_img_input");
    sam3_dump_tensor_to_path(pos_enc, "fenc_pos_input",
                             output_dir + "/fenc_pos_input");
    sam3_dump_tensor_to_path(prompt_tokens, "fenc_prompt_input",
                             output_dir + "/fenc_prompt_input");
...
static bool sam3_test_dump_phase6_impl(const sam3_model& model,
    ...
    if (!neck0 || !neck1 || !neck2) {
        AICORE_LOG_ERROR("[sam3] ", "%s: missing tracker feature inputs\n",
                         __func__);
        ggml_free(ctx0);
        return false;
    }
    ...
    auto* no_mem =
            ggml_reshape_4d(ctx0, model.tensors.at("no_mem_embed"), D, 1, 1, 1);
    auto* image_feats = ggml_add(ctx0, neck2, no_mem);
    ggml_set_name(image_feats, "sam_dec_image_feats");

    auto dec_out =
            sam3_build_sam_dec_graph(ctx0, model, image_feats, pe_out.image_pe,
                                     pe_out.sparse, pe_out.dense, neck0, neck1);
    ...
    std::vector<std::string> named_outputs = {
            "sam_dec_tokens_initial", "sam_dec_block0_queries",
            "sam_dec_block0_keys",    "sam_dec_block1_queries",
            "sam_dec_block1_keys",    "sam_dec_final_queries",
            "sam_dec_feat_s1_proj",   "sam_dec_feat_s0_proj",
            "sam_dec_upscaled",       "sam_dec_mask_tokens",
            "sam_dec_masks",          "sam_dec_iou",
            "sam_dec_obj_score",
    };
    ...
    if (!ggml_gallocr_reserve(galloc, graph) ||
        !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate phase 6 graph\n",
                         __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    ...
    if (neck0_data && neck1_data && neck2_data) {
        ggml_backend_tensor_set(neck0, neck0_data->data(), 0,
                                neck0_data->size() * sizeof(float));
        ggml_backend_tensor_set(neck1, neck1_data->data(), 0,
                                neck1_data->size() * sizeof(float));
        ggml_backend_tensor_set(neck2, neck2_data->data(), 0,
                                neck2_data->size() * sizeof(float));
    }
...

    {
        ...
        for (int p = 0; p < pe_out.n_tokens; ++p) {
            ...
            float pe_vec[256];
            sam3_pe_encode_coord(pe_vec, x_norm, y_norm,
                                 pe_cache_state.pe_gauss_cache.data(),
                                 num_pos_feats);

            const int label = all_labels[p];
            if (label == -1) {
                for (int d = 0; d < D; ++d) {
                    sparse_data[(size_t)p * D + d] =
                            pe_cache_state.not_a_point_cache[d];
                }
            } else {
                for (int d = 0; d < D; ++d) {
                    sparse_data[(size_t)p * D + d] =
                            pe_vec[d] +
                            pe_cache_state.point_emb_cache[label][d];
                }
            }
        }
        ...
        ggml_backend_tensor_set(
                pe_out.image_pe, pe_cache_state.dense_pe_cache.data(), 0,
                pe_cache_state.dense_pe_cache.size() * sizeof(float));
        ggml_backend_tensor_set(
                pe_out.dense, pe_cache_state.dense_nomask_cache.data(), 0,
    ...
    for (const auto& name : named_outputs) {
        ...
        if (!t) {
            AICORE_LOG_ERROR("[sam3] ",
                             "%s: tensor '%s' not found after compute\n",
                             __func__, name.c_str());
            ok = false;
            continue;
        }
...
bool sam3_test_dump_phase6(const sam3_model& model,
                           ...
                           const std::string& output_dir,
                           int n_threads) {
    return sam3_test_dump_phase6_impl(
            model, state.neck_trk[0], state.neck_trk[1], state.neck_trk[2],
            nullptr, nullptr, nullptr, params, output_dir, n_threads);
}
...
bool sam3_test_dump_phase6_from_ref_inputs(const sam3_model& model,
    ...
    if (!sam3_load_ref_f32_data(prephase_ref_dir + "/neck_trk_0", neck_trk_0,
                                D * H0 * H0) ||
        !sam3_load_ref_f32_data(prephase_ref_dir + "/neck_trk_1", neck_trk_1,
                                D * H1 * H1) ||
        !sam3_load_ref_f32_data(prephase_ref_dir + "/neck_trk_2", neck_trk_2,
                                D * H * H)) {
        return false;
    }
    ...
    return sam3_test_dump_phase6_impl(model, nullptr, nullptr, nullptr,
...
bool sam3_test_dump_geom_enc(const sam3_model& model,
    ...
    if (!sam3_load_ref_f32_data(prephase_ref_dir + "/neck_det_2", neck_det_2,
                                D * H * H)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to load neck_det_2\n",
                         __func__);
        return false;
    }
    ...
    // Compute sinusoidal PE for image features (matching Python
    ...
    std::vector<float> img_pe_nchw(D * H * H);
    {
        ...
        for (int row = 0; row < H; ++row) {
            for (int col = 0; col < H; ++col) {
                float y_embed =
                        ((float)(row + 1) - 0.5f) / ((float)H + eps) * scale;
                float x_embed =
                        ((float)(col + 1) - 0.5f) / ((float)H + eps) * scale;

                for (int i = 0; i < num_pos_feats; ++i) {
                    int div_idx = 2 * (i / 2);
                    float dim_t = powf(10000.0f,
                                       (float)div_idx / (float)num_pos_feats);
                    ...
                    // Output: [1, D, H, W] NCHW — pos_y first half, pos_x
                    // second half
                    img_pe_nchw[i * H * H + row * H + col] = pe_y;
                    img_pe_nchw[(num_pos_feats + i) * H * H + row * H + col] =
                            pe_x;
                }
            }
        }
    ...
    struct ggml_context* ctx0 = ggml_init(gparams);
    if (!ctx0) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to init context\n", __func__);
        return false;
    }
    ...
    auto geom_out = sam3_build_geom_enc_graph(ctx0, model, params, img_feats_t,
                                              img_pe_t);
    ...
    ggml_backend_tensor_set(img_feats_t, neck_det_2_ggml.data(), 0,
                            D * H * H * sizeof(float));
    ggml_backend_tensor_set(img_pe_t, img_pe_ggml.data(), 0,
                            D * H * H * sizeof(float));
    ...
    // Dump the pre-computed input (after final_proj + norm, before transformer)
    {
        ...
        if (pre_proj)
            sam3_dump_tensor_to_path(pre_proj, "post_final_proj",
                                     output_dir + "/post_final_proj");
    }
    ...
    // Dump final output
    {
        ...
        if (out)
            sam3_dump_tensor_to_path(out, "geom_output",
                                     output_dir + "/geom_output");
    }
    ...
    // Also dump the pre-computed input data (before ggml processing)
    {
        ...
        AICORE_LOG_PRINT("[sam3] ",
                         "%s: dumped geom_input_precomputed [%d, %d]\n",
                         __func__, D, N_geo);
    }
...
static bool sam3_test_dump_phase7_mem_slot_current(
    ...
    auto m_hires = sam3_bilinear_interpolate(
            low_res_mask_logits.data(), MASK_HW, MASK_HW, HIGH_RES, HIGH_RES);
    ...
    const size_t buf_size =
            ggml_tensor_overhead() * 16384 + ggml_graph_overhead();
    struct ggml_init_params gparams = {buf_size, nullptr, true};
    struct ggml_context* ctx0 = ggml_init(gparams);
    if (!ctx0) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: failed to init memory encoder context\n",
                         __func__);
        return false;
    }
    ...
    auto* mask_in =
            ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, INTERPOL, INTERPOL, 1, 1);
    ...
    for (int s = 0; s < 4; ++s) {
        int out_ch = (int)model.mem_enc.ds_conv_w[s]->ne[3];
        ds = ggml_conv_2d(ctx0, model.mem_enc.ds_conv_w[s], ds, 2, 2, 1, 1, 1,
                          1);
        ds = ggml_add(ctx0, ds,
                      ggml_reshape_4d(ctx0, model.mem_enc.ds_conv_b[s], 1, 1,
                                      out_ch, 1));
        ds = ggml_cont(ctx0,
                       ggml_permute(ctx0, ds, 1, 2, 0, 3));  // conv→internal
        ds = sam3_layer_norm_2d(ctx0, ds, model.mem_enc.ds_norm_w[s],
                                model.mem_enc.ds_norm_b[s]);
        ds = ggml_gelu(ctx0, ds);
        ds = ggml_cont(ctx0,
                       ggml_permute(ctx0, ds, 2, 0, 1, 3));  // internal→conv
    }
    ...
    auto* pix = ggml_conv_2d(ctx0, model.mem_enc.pix_proj_w, neck2_whcb, 1, 1,
                             0, 0, 1, 1);
    pix = ggml_add(ctx0, pix,
                   ggml_reshape_4d(ctx0, model.mem_enc.pix_proj_b, 1, 1, D, 1));
    pix = ggml_cont(ctx0, ggml_permute(ctx0, pix, 1, 2, 0, 3));
    ...
    for (int i = 0; i < 2; ++i) {
        h = sam3_cxblock_forward(
                ctx0, h, model.mem_enc.fuser_dw_w[i],
                model.mem_enc.fuser_dw_b[i], model.mem_enc.fuser_norm_w[i],
                model.mem_enc.fuser_norm_b[i], model.mem_enc.fuser_fc1_w[i],
                model.mem_enc.fuser_fc1_b[i], model.mem_enc.fuser_fc2_w[i],
                model.mem_enc.fuser_fc2_b[i], model.mem_enc.fuser_gamma[i]);
    ...
    auto* mo = ggml_conv_2d(ctx0, model.mem_enc.out_proj_w, h_whcb, 1, 1, 0, 0,
                            1, 1);
    mo = ggml_add(ctx0, mo,
                  ggml_reshape_4d(ctx0, model.mem_enc.out_proj_b, 1, 1, MD, 1));
    ...
    if (!ggml_gallocr_reserve(galloc, graph) ||
        !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ",
                         "%s: failed to allocate phase 7 memory graph\n",
                         __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    ...
    ggml_backend_tensor_get(mo, mem_out_data.data(), 0,
                            mem_out_data.size() * sizeof(float));
...
bool sam3_test_dump_phase7_from_ref_inputs(const sam3_model& model,
    ...
    if (num_slots <= 0) {
        AICORE_LOG_ERROR("[sam3] ", "%s: no memory slots in %s\n", __func__,
                         case_ref_dir.c_str());
        return false;
    }
    ...
    if (!sam3_load_ref_f32_data(case_ref_dir + "/input_curr_neck_trk_0",
                                neck_trk_0_nchw, D * H0 * H0) ||
        !sam3_load_ref_f32_data(case_ref_dir + "/input_curr_neck_trk_1",
                                neck_trk_1_nchw, D * H1 * H1) ||
        !sam3_load_ref_f32_data(case_ref_dir + "/input_curr_neck_trk_2",
                                neck_trk_2_nchw, D * H * H)) {
        return false;
    }
    ...
    for (int i = 0; i < num_slots; ++i) {
        ...
        if (!sam3_load_ref_f32_data(case_ref_dir + "/input_mem_mask_logits_" +
                                            std::to_string(i),
                                    mask_logits_nchw, MASK_HW * MASK_HW) ||
            !sam3_load_ref_f32_data(
                    case_ref_dir + "/input_mem_sam_token_" + std::to_string(i),
                    sam_token, D) ||
            !sam3_load_ref_f32_data(
                    case_ref_dir + "/input_mem_obj_score_" + std::to_string(i),
                    obj_score, 1)) {
            return false;
        }

        slot_spatial_tpos[i] = sam3_meta_get_int(
                meta, "slot" + std::to_string(i) + "_spatial_tpos", 0);
        slot_ptr_tpos[i] = sam3_meta_get_int(
                meta, "slot" + std::to_string(i) + "_ptr_tpos", -1);

        if (!sam3_test_dump_phase7_mem_slot_current(
                    model, neck_trk_2, mask_logits_nchw, obj_score[0], i,
                    output_dir, mem_slot_outputs[i], mem_slot_pes[i],
                    n_threads)) {
            ok = false;
        }

        std::vector<float> obj_ptr(D, 0.0f);
        sam3_extract_obj_ptr_cpu(model, sam_token.data(), obj_score[0],
                                 obj_ptr.data());
    ...
    for (int i = 0; i < num_slots; ++i) ptr_tpos_vec[i] = slot_ptr_tpos[i];

    auto pd = sam3_build_prompt_and_pos(model, mem_slot_outputs, mem_slot_pes,
                                        slot_spatial_tpos, obj_ptrs,
                                        ptr_tpos_vec);
    ...
    if (pd.M_spatial > 0) {
        ...
        for (int s = 0; s < n_spatial_slots; ++s)
            memcpy(rope_k_data.data() + s * D * N, rope_q_data.data(),
                   D * N * sizeof(float));
    }
    ...
    struct ggml_context* ctx0 = ggml_init(gparams);
    if (!ctx0) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to init phase 7 context\n",
                         __func__);
        return false;
    }
    ...
    if (!ggml_gallocr_reserve(galloc, graph) ||
        !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate phase 7 graph\n",
                         __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    ...
    ggml_backend_tensor_set(neck0, neck_trk_0.data(), 0,
                            neck_trk_0.size() * sizeof(float));
    ggml_backend_tensor_set(neck1, neck_trk_1.data(), 0,
                            neck_trk_1.size() * sizeof(float));
    ggml_backend_tensor_set(neck2, neck_trk_2.data(), 0,
                            neck_trk_2.size() * sizeof(float));
    ...
    ggml_backend_tensor_set(rope_q_t, rope_q_data.data(), 0,
                            rope_q_data.size() * sizeof(float));
    if (rope_k_t && !rope_k_data.empty())
        ggml_backend_tensor_set(rope_k_t, rope_k_data.data(), 0,
                                rope_k_data.size() * sizeof(float));
    ...
    ggml_backend_tensor_set(src_pos_t, src_pos_data.data(), 0,
                            src_pos_data.size() * sizeof(float));
    ...
    // Upload not_a_point_embed for sparse prompt
    {
        ...
        if (model.sam_pe.not_a_point_embed->type == GGML_TYPE_F16) {
            std::vector<ggml_fp16_t> tmp(D);
            ggml_backend_tensor_get(model.sam_pe.not_a_point_embed, tmp.data(),
                                    0, D * sizeof(ggml_fp16_t));
            ggml_fp16_to_fp32_row(tmp.data(), nap, D);
        } else {
            ggml_backend_tensor_get(model.sam_pe.not_a_point_embed, nap, 0,
                                    D * sizeof(float));
        }
    ...
    sam3_populate_pe_cache(pe_cache_state, model);
    ggml_backend_tensor_set(
            image_pe, pe_cache_state.dense_pe_cache.data(), 0,
            pe_cache_state.dense_pe_cache.size() * sizeof(float));
    ggml_backend_tensor_set(
            dense_emb, pe_cache_state.dense_nomask_cache.data(), 0,
            pe_cache_state.dense_nomask_cache.size() * sizeof(float));

    sam3_graph_compute(model.backend, graph, n_threads);

    // Dump the mem_attn_input manually (it's now x = curr + 0.1*src_pos, not
    // just curr) The named tensor "phase7_mem_attn_input" is no longer in the
    // graph since we removed the explicit naming. But all the per-layer outputs
    // are still named.
    for (const auto& name : mem_attn_names) {
        auto* t = ggml_get_tensor(ctx0, name.c_str());
        if (!t || !sam3_dump_tensor_to_path(t, name, output_dir + "/" + name)) {
...
static struct ggml_tensor* sam3_find_state_tensor(
        const sam3_state& state, const std::string& tensor_name) {
...
static bool sam3_fill_tensor_info(struct ggml_tensor* t,
...
bool sam3_get_state_tensor_info(const sam3_state& state,
    ...
    if (!t) {
        AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' not found in state\n",
...
bool sam3_get_model_tensor_info(const sam3_model& model,
    ...
    if (it == model.tensors.end() || !it->second) {
        AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' not found in model\n",
                         __func__, tensor_name.c_str());
        return false;
    }
