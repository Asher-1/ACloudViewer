...
#define SAM3_LOG(level, ...) \
    do { if ((level) <= SAM3_LOG_LEVEL) AICORE_LOG_PRINT("[sam3] ", __VA_ARGS__); } while (0)
...
static ggml_backend_t sam3_backend_init(sam3_device device, bool use_gpu) {
    ...
    switch (device) {
        case SAM3_DEVICE_AUTO:
            ...
            if (!dev) dev = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_GPU);
            break;
    ...
    if (!dev) {
        ...
        if (dev) {
            AICORE_LOG_ERROR("[sam3] ", "%s: requested device unavailable; using CPU backend\n", __func__);
        }
    }
    if (!dev) {
...
struct gguf_kv {
    ...
    void str_arr(const char* key, std::vector<std::string>& out, bool required = true) const {
        int64_t id = find(key, required);
        if (id < 0) return;
        if (gguf_get_kv_type(g, id) != GGUF_TYPE_ARRAY ||
            gguf_get_arr_type(g, id) != GGUF_TYPE_STRING) {
            if (err.empty()) err = std::string("expected string GGUF array: ") + key;
            return;
        }
    ...
    void i32_arr(const char* key, std::vector<int32_t>& out, bool required = true) const {
        int64_t id = find(key, required);
        if (id < 0) return;
        if (gguf_get_kv_type(g, id) != GGUF_TYPE_ARRAY ||
            gguf_get_arr_type(g, id) != GGUF_TYPE_INT32) {
            if (err.empty()) err = std::string("expected int32 GGUF array: ") + key;
            return;
        }
        const size_t count = (size_t) gguf_get_arr_n(g, id);
        out.resize(count);
...
static bool sam3_load_bpe_vocab_from_gguf(const gguf_kv& r, sam3_bpe_tokenizer& tok) {
    ...
    if (vocab.empty() || merges.empty()) {
        AICORE_LOG_ERROR("[sam3] ", "%s: tokenizer KV missing (sam3.tokenizer.vocab/merges)\n", __func__);
        return false;
    }
...
static bool sam3_load_hparams(const gguf_kv& r, sam3_hparams& hp) {
    ...
    const int32_t mlp_ratio_x1000 = rd("sam3.hparams.vit_mlp_ratio_x1000", 4625);
    ...
    hp.ddec_num_queries  = rd("sam3.hparams.ddec_num_queries",  hp.ddec_num_queries);
    hp.geom_layers       = rd("sam3.hparams.geom_layers",       hp.geom_layers);
    hp.n_presence_tokens = rd("sam3.hparams.n_presence_tokens", hp.n_presence_tokens);
    ...
    hp.sam_iou_head_depth = rd("sam3.hparams.sam_iou_head_depth", hp.sam_iou_head_depth);
...
static bool sam2_load_hparams(const gguf_kv& r, sam3_hparams& hp) {
    ...
    hp.hiera_num_stages    = rd("sam3.hparams.hiera_num_stages", hp.hiera_num_stages);
    ...
    hp.hiera_pos_embed_bkg_h = rd("sam3.hparams.hiera_pos_embed_bkg_h", hp.hiera_pos_embed_bkg_h);
    hp.hiera_pos_embed_bkg_w = rd("sam3.hparams.hiera_pos_embed_bkg_w", hp.hiera_pos_embed_bkg_w);
    ...
    hp.sigmoid_scale_x100  = rd("sam3.hparams.sigmoid_scale_x100", hp.sigmoid_scale_x100);
    hp.sigmoid_bias_x100   = rd("sam3.hparams.sigmoid_bias_x100", hp.sigmoid_bias_x100);

    hp.use_high_res_features        = rd("sam3.hparams.use_high_res_features", hp.use_high_res_features);
    hp.use_obj_ptrs_in_encoder      = rd("sam3.hparams.use_obj_ptrs_in_encoder", hp.use_obj_ptrs_in_encoder);
    hp.pred_obj_scores              = rd("sam3.hparams.pred_obj_scores", hp.pred_obj_scores);
    hp.use_multimask_token_for_obj_ptr = rd("sam3.hparams.use_multimask_token_for_obj_ptr", hp.use_multimask_token_for_obj_ptr);
    hp.directly_add_no_mem_embed    = rd("sam3.hparams.directly_add_no_mem_embed", hp.directly_add_no_mem_embed);
    hp.non_overlap_masks_for_mem_enc = rd("sam3.hparams.non_overlap_masks_for_mem_enc", hp.non_overlap_masks_for_mem_enc);
    hp.binarize_mask_from_pts       = rd("sam3.hparams.binarize_mask_from_pts", hp.binarize_mask_from_pts);
    hp.multimask_output_for_tracking = rd("sam3.hparams.multimask_output_for_tracking", hp.multimask_output_for_tracking);
    hp.multimask_min_pt_num         = rd("sam3.hparams.multimask_min_pt_num", hp.multimask_min_pt_num);
    hp.multimask_max_pt_num         = rd("sam3.hparams.multimask_max_pt_num", hp.multimask_max_pt_num);
    hp.fixed_no_obj_ptr             = rd("sam3.hparams.fixed_no_obj_ptr", hp.fixed_no_obj_ptr);
    hp.iou_prediction_use_sigmoid   = rd("sam3.hparams.iou_prediction_use_sigmoid", hp.iou_prediction_use_sigmoid);
    hp.use_mask_input_as_output     = rd("sam3.hparams.use_mask_input_as_output", hp.use_mask_input_as_output);
    hp.multimask_output_in_sam      = rd("sam3.hparams.multimask_output_in_sam", hp.multimask_output_in_sam);
...
static void sam2_precompute_hiera_metadata(sam3_model& model) {
    ...
    for (int i = 0; i < total; ++i) {
        ...
        AICORE_LOG_PRINT("[sam3] ", "  blk %2d: stage=%d dim=%d→%d heads=%d ws=%d q_stride=%d\n",
...
static bool sam3_load_tensors_from_gguf(const char* path, struct gguf_context* gguf,
                                        struct ggml_context* gguf_ctx,
                                        sam3_model& model) {
    const int64_t n_tensors = gguf_get_n_tensors(gguf);
    if (n_tensors != (int64_t) model.tensors.size()) {
        AICORE_LOG_ERROR("[sam3] ", "%s: tensor count mismatch: file has %lld, model registered %zu\n",
    ...
    if (!f) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to reopen '%s' for tensor data\n", __func__, path);
        return false;
    }
    ...
    for (auto& kv : model.tensors) {
        ...
        // Look up the file tensor — every registered tensor must exist in the file
        struct ggml_tensor* src = ggml_get_tensor(gguf_ctx, name.c_str());
        if (!src) {
            AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' not found in GGUF file\n", __func__, name.c_str());
            fclose(f);
            return false;
        }
        ...
        if (src_id < 0) {
            AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' not found in GGUF info\n", __func__, name.c_str());
            fclose(f);
            return false;
        }
        ...
        if (src_n_el != dst_n_el) {
            AICORE_LOG_ERROR("[sam3] ", "%s: element-count mismatch for '%s' (file=%lld, registered=%lld)\n",
                    __func__, name.c_str(), (long long) src_n_el, (long long) dst_n_el);
            fclose(f);
            return false;
        }
        ...
        if (fseek(f, (long) (data_offset + src_offset), SEEK_SET) != 0 ||
            ...
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to read tensor '%s' at offset %zu\n",
        ...
        if (src->type == tensor->type) {
        ...
        } else if (src->type == GGML_TYPE_F16 && tensor->type == GGML_TYPE_F32) {
        ...
        } else if (src->type == GGML_TYPE_F32 && tensor->type == GGML_TYPE_F16) {
        ...
        } else if (ggml_is_quantized(src->type) && tensor->type == GGML_TYPE_F32) {
            const auto* traits = ggml_get_type_traits(src->type);
            if (!traits->to_float) {
                AICORE_LOG_ERROR("[sam3] ", "%s: no dequantize function for '%s' (type=%s)\n",
            ...
            AICORE_LOG_ERROR("[sam3] ", "%s: unsupported type conversion for '%s' (file=%s, registered=%s)\n",
                    __func__, name.c_str(), ggml_type_name(src->type),
...
std::shared_ptr<sam3_model> sam3_load_model(const sam3_params& params) {
    ...
    struct gguf_context* gguf = gguf_init_from_file(params.model_path.c_str(), gparams);
    if (!gguf) {
    ...
    if (is_sam2) {
        if (!sam2_load_hparams(r, model->hparams)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to read SAM2 hyperparameters\n", __func__);
            gguf_free(gguf);
            ggml_free(gguf_ctx);
            return nullptr;
        }
        ...
        if (!sam3_load_hparams(r, model->hparams)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to read SAM3 hyperparameters\n", __func__);
            gguf_free(gguf);
            ggml_free(gguf_ctx);
            return nullptr;
        }
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: buffer size = %.2f MB\n", __func__,
            ggml_backend_buffer_get_size(model->buffer) / (1024.0 * 1024.0));

    // ── Stream tensor data from the GGUF data section ────────────────────
    if (!sam3_load_tensors_from_gguf(params.model_path.c_str(), gguf, gguf_ctx, *model)) {
    ...
    if (!is_sam2 && !model->hparams.visual_only) {
        if (!sam3_load_bpe_vocab_from_gguf(r, model->tokenizer)) {
            AICORE_LOG_ERROR("[sam3] ", "%s: failed to load embedded tokenizer\n", __func__);
            gguf_free(gguf);
            ggml_free(gguf_ctx);
            return nullptr;
        }
        ...
        AICORE_LOG_PRINT("[sam3] ", "%s: visual-only model — skipping tokenizer\n", __func__);
    }
...
sam3_state_ptr sam3_create_state(const sam3_model& model,
    ...
    if (eis != hp.img_size) {
        AICORE_LOG_PRINT("[sam3] ", "%s: encode_img_size=%d (model native=%d), feat_size=%d (native=%d)\n",
...
static struct ggml_tensor* sam3_apply_rope(struct ggml_context* ctx,
    ...
    if (sam3_backend_is_cuda() && freqs_cis->type == GGML_TYPE_F32) {
        auto* fused = output_f32 && x->type == GGML_TYPE_F16
            ? ggml_rope_custom_freqs_cast(ctx, x, freqs_cis, GGML_TYPE_F32)
...
static bool sam3_fattn_hd_supported(ggml_backend_t backend, int64_t hd) {
    ...
    // microseconds.
    struct ggml_init_params ip = { 4096, /*mem_buffer*/ nullptr, /*no_alloc*/ true };
    struct ggml_context* pctx = ggml_init(ip);
    struct ggml_tensor* q = ggml_new_tensor_4d(pctx, GGML_TYPE_F32, hd, 1, 1, 1);
    struct ggml_tensor* k = ggml_new_tensor_4d(pctx, GGML_TYPE_F16, hd, 1, 1, 1);
    struct ggml_tensor* v = ggml_new_tensor_4d(pctx, GGML_TYPE_F16, hd, 1, 1, 1);
    struct ggml_tensor* op = ggml_flash_attn_ext(pctx, q, k, v, nullptr, 1.0f, 0.0f, 0.0f);
    ...
    if (!supported) {
        AICORE_LOG_ERROR("[sam3] ", "%s: head_dim=%lld unsupported on this backend; using manual SDPA\n",
...
static struct ggml_tensor* sam3_vit_block_forward(struct ggml_context* ctx,
...

    {
        ...
        if (sam3_backend_is_cuda() && cur->type == GGML_TYPE_F16 && blk.freqs_cis) {
            auto* candidate = ggml_rope_custom_qkv(ctx, cur, blk.freqs_cis, HD, NH);
        ...
        if (packed) {
            ...
            Q = ggml_view_3d(ctx, packed, HD, N, NH * B_cur,
                             packed->nb[1], packed->nb[2], 0);
            K = ggml_view_3d(ctx, packed, HD, N, NH * B_cur,
                             packed->nb[1], packed->nb[2], packed->nb[3]);
            V = ggml_view_3d(ctx, packed, HD, N, NH * B_cur,
                             packed->nb[1], packed->nb[2], 2 * packed->nb[3]);
            ...
            Q = ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur,
                             cur->nb[1], cur->nb[2], 0);
            K = ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur,
                             cur->nb[1], cur->nb[2], cur->nb[3]);
            V = ggml_view_3d(ctx, cur, E, W_cur * H_cur, B_cur,
...
static std::vector<float> sam2_compute_pos_embed(const sam3_model& model, int H, int W) {
    ...
    if (cache_key == pe_cache_key{E, H, W, bkg_h, bkg_w, ws} && !cache_pe.empty()) {
        return cache_pe;
    }
    ...
    const char* dump_dir = nullptr;  // AICore: no env access (upstream SAM2_DUMP_DIR)
    if (dump_dir) {
        char path[512];
...
static bool sam2_encode_image_hiera(sam3_state& state,
    ...
    const bool timing = false;  // AICore: no env access (upstream SAM3_ENCODE_TIMING)
    auto tmark = [&](const char* what) {
        if (timing) {
            auto now = std::chrono::high_resolution_clock::now();
            AICORE_LOG_PRINT("[sam3] ", "  [hiera-enc] %-22s %8.1f ms\n", what,
    ...
    struct ggml_tensor*   fpn_outs[4];
    const bool graph_cached = (ctx0 != nullptr) && (state.enc_img_size == img_size);
    ...
    if (timing) {
        AICORE_LOG_PRINT("[sam3] ", "  [hiera-enc] inp=%p buf=%p pe=%p pebuf=%p galloc=%p\n",
                (void*)inp, (void*)(inp ? inp->buffer : nullptr),
                (void*)pe_tensor, (void*)(pe_tensor ? pe_tensor->buffer : nullptr),
                (void*)galloc);
    }
    ...
    for (int i = 0; i < n_fpn; ++i) {
        AICORE_LOG_PRINT("[sam3] ", "  neck_trk[%d]: [%lld, %lld, %lld, %lld]\n", i,
...
static bool sam3_encode_image_impl(sam3_state& state,
    ...
    const bool graph_cached = (state.sam3_ctx != nullptr) &&
                              ...
                              (state.sam3_tracker_only == tracker_only || model.hparams.visual_only);
    ...
    if (graph_cached) {
        ...
        if (!inp) {
            AICORE_LOG_ERROR("[sam3] ", "%s: cached graph missing input_image tensor\n", __func__);
            return false;
        }
    } else {
...
static bool sam3_mark_named_outputs(struct ggml_context * ctx,
    ...
    for (const auto & name : output_tensors) {
        ...
        if (!t) {
            AICORE_LOG_ERROR("[sam3] ", "%s: requested tensor '%s' was not found\n", __func__, name.c_str());
            return false;
        }
        ggml_set_output(t);
    }
...
bool sam3_encode_vit_from_preprocessed_selective(sam3_state                    & state,
    ...
    if (img_size != hp.img_size) {
        AICORE_LOG_ERROR("[sam3] ", "%s: img_size mismatch: got %d, expected %d\n",
    ...
    switch (stage) {
        ...
        case SAM3_VIT_PREFIX_STAGE_PATCH_EMBED:
            if (ne[0] != model.hparams.img_size || ne[1] != model.hparams.img_size || ne[2] != 3) {
                AICORE_LOG_ERROR("[sam3] ", "%s: patch_embed input shape mismatch [%lld,%lld,%lld,%lld]\n",
                        __func__,
        ...
        case SAM3_VIT_PREFIX_STAGE_PATCH_MULMAT:
            if (ne[0] != model.vit.patch_embed_w->ne[0] * model.vit.patch_embed_w->ne[1] * model.vit.patch_embed_w->ne[2] ||
                ...
                AICORE_LOG_ERROR("[sam3] ", "%s: patch_mulmat input shape mismatch [%lld,%lld,%lld,%lld]\n",
                        __func__,
                        (long long) ne[0], (long long) ne[1], (long long) ne[2], (long long) ne[3]);
                return false;
            }
        ...
        default:
            if (ne[0] != model.hparams.vit_embed_dim ||
                ne[1] != model.hparams.n_img_embd() ||
                ne[2] != model.hparams.n_img_embd()) {
                AICORE_LOG_ERROR("[sam3] ", "%s: prefix feature input shape mismatch [%lld,%lld,%lld,%lld]\n",
                        __func__,
                        (long long) ne[0], (long long) ne[1], (long long) ne[2], (long long) ne[3]);
                return false;
            }
...
bool sam3_test_run_vit_block_linear_host_ref(const sam3_model         & model,
    ...
    if (!w || !b || w->type != GGML_TYPE_F16 || b->type != GGML_TYPE_F32) {
        AICORE_LOG_ERROR("[sam3] ", "%s: unsupported tensor types for stage %d\n", __func__, (int) stage);
        return false;
    }
    ...
    if (ne[0] != k) {
        AICORE_LOG_ERROR("[sam3] ", "%s: linear input shape mismatch [%lld,%lld,%lld,%lld] for K=%lld\n",
                __func__,
                (long long) ne[0], (long long) ne[1], (long long) ne[2], (long long) ne[3],
                (long long) k);
        return false;
...
bool sam3_test_run_vit_block_stage(const sam3_model        & model,
    ...
    switch (stage) {
        ...
        case SAM3_VIT_BLOCK_STAGE_WINDOW_UNPART:
            if (ne[0] != hp.vit_embed_dim) {
                AICORE_LOG_PRINT("[sam3] ", "%s: window stage input ne0=%lld expected %d\n",
        ...
        case SAM3_VIT_BLOCK_STAGE_QKV_PROJ:
            if (ne[0] != hp.vit_embed_dim) {
                AICORE_LOG_PRINT("[sam3] ", "%s: qkv input ne0=%lld expected %d\n",
                        __func__, (long long) ne[0], hp.vit_embed_dim);
                return false;
            }
            break;
        case SAM3_VIT_BLOCK_STAGE_ATTN_CORE:
            if (ne[0] != 3 * hp.vit_embed_dim) {
                AICORE_LOG_PRINT("[sam3] ", "%s: attn input ne0=%lld expected %d\n",
        ...
        case SAM3_VIT_BLOCK_STAGE_MLP_FC1:
            if (ne[0] != hp.vit_embed_dim) {
                AICORE_LOG_PRINT("[sam3] ", "%s: mlp_fc1 input ne0=%lld expected %d\n",
                        __func__, (long long) ne[0], hp.vit_embed_dim);
                return false;
            }
            break;
        ...
        case SAM3_VIT_BLOCK_STAGE_MLP_FC2:
            if (ne[0] != hp.vit_mlp_dim) {
                AICORE_LOG_PRINT("[sam3] ", "%s: mlp stage input ne0=%lld expected %d\n",
        ...
        default:
            if (ne[0] != hp.vit_embed_dim) {
                AICORE_LOG_PRINT("[sam3] ", "%s: stage input ne0=%lld expected %d\n",
                        __func__, (long long) ne[0], hp.vit_embed_dim);
                return false;
            }
            break;
    ...
    auto * galloc = ggml_gallocr_new(ggml_backend_get_default_buffer_type(model.backend));
    if (!ggml_gallocr_reserve(galloc, graph) || !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate graph for stage %d\n", __func__, (int) stage);
        ggml_gallocr_free(galloc);
        ggml_free(ctx);
        return false;
    }
...
bool sam3_encode_image_from_preprocessed(sam3_state& state,
    ...
    if (hp.is_sam2()) {
        ...
        // Set positional embedding
        {
            ...
            if (dump_dir) {
                ...
                AICORE_LOG_PRINT("[sam3] ", "  [DUMP] cpp_pos_embed: [%d,%d,%d,1]\n",
        ...
        // Dump debug tensors if SAM2_DUMP_DIR is set
        {
            ...
            if (dump_dir) {
                ...
                for (const char* dn : dbg_names) {
                    ...
                    AICORE_LOG_PRINT("[sam3] ", "  [DUMP] %s: [%lld,%lld,%lld,%lld]\n", dn,
                            (long long)t->ne[0], (long long)t->ne[1],
                            (long long)t->ne[2], (long long)t->ne[3]);
                }
            }
        ...
        AICORE_LOG_PRINT("[sam3] ", "%s: SAM2 encoding from preprocessed done\n", __func__);
        return true;
    }
    ...
    // Compute
    {
        ...
        AICORE_LOG_PRINT("[sam3] ", "%s: graph computed in %.1f ms (%d threads)\n",
...
static std::vector<float> sam3_precompute_geom_input(
    ...
    if (n_boxes > 0) {
        AICORE_LOG_PRINT("[sam3] ", "%s: %d exemplar boxes encoded (%d pos, %d neg)\n",
                __func__, n_boxes,
...
static sam3_prompt_data sam3_build_prompt_and_pos(
    ...
    if (model.mem_enc.tpos[0]) {
        if (tpos_enc_cache && tpos_enc_cache->size() >= (size_t)(MD * hp.num_maskmem)) {
    ...
    if (model.obj_ptr_tpos_w) {
        if (ptr_tpos_w_cache && ptr_tpos_w_cache->size() >= (size_t)(D * MD) &&
            ...
            tpos_w.assign(ptr_tpos_w_cache->begin(), ptr_tpos_w_cache->begin() + D * MD);
            tpos_b.assign(ptr_tpos_b_cache->begin(), ptr_tpos_b_cache->begin() + MD);
        } else {
...
sam3_result sam3_segment_pcs(sam3_state& state,
    ...
    if (model.hparams.visual_only || model.hparams.is_sam2()) {
        AICORE_LOG_ERROR("[sam3] ", "%s: ERROR: PCS not available on %s model\n",
    ...
    if (!state.neck_det[0]) {
        AICORE_LOG_PRINT("[sam3] ", "%s: image not encoded — call sam3_encode_image first\n", __func__);
        return result;
    }
    if (!sam3_ensure_neck_pe(state, model)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to initialize detector positional encodings\n", __func__);
        return result;
    }
    ...
    */
    std::vector<float> text_feats_cpu(D * L);
    {
        ...
        if (inp->buffer) {
            ...
            AICORE_LOG_ERROR("[sam3] ", "%s: ERROR: text encoder input tensor has no buffer!\n", __func__);
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return result;
        }
...
static void sam3_populate_pe_cache(sam3_state& state, const sam3_model& model) {
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: PE cache populated (%d embeddings, %.1f KB dense grids)\n",
...
sam3_result sam3_segment_pvs(sam3_state& state,
    ...
    if (params.pos_points.empty() && !params.use_box) {
        AICORE_LOG_ERROR("[sam3] ", "%s: no prompts provided (need at least one point or box)\n", __func__);
        return result;
    }
...

    {
        ...
        {
            const char* dd = nullptr;  // AICore: no env access (upstream SAM2_DUMP_DIR)
            if (dd) {
                char p[512]; snprintf(p, sizeof(p), "%s/cpp_sparse_emb.bin", dd);
                FILE* f = fopen(p, "wb");
                if (f) { fwrite(sparse_data.data(), sizeof(float), N_pts * D, f); fclose(f); }
                AICORE_LOG_PRINT("[sam3] ", "  [DUMP] cpp_sparse_emb: %d tokens x %d dims\n", N_pts, D);
            }
        }
    ...
    // image_feats = neck_trk[2] + no_mem_embed (computed on CPU)
    {
        ...
        // Dump image_feats (with no_mem_embed) if requested
        {
            ...
            if (dd) {
                ...
                AICORE_LOG_PRINT("[sam3] ", "  [DUMP] cpp_image_feats: [%d, %d, %d]\n", D, H, H);
            }
        }
...
static sam3_prop_output sam3_propagate_single(
    ...
    static const bool s_prof = false;  // AICore: no env access (upstream SAM3_PROFILE_PROP)
    ...
    if (prop_cached) {
        ...
        tracker.prop_consts_uploaded = false;  // new tensors: re-upload constants
    }
    ...
    // The reshape is a zero-cost view — the graph still sees [D,N,1].
    {
        auto* curr_4d = ggml_reshape_4d(ctx0, curr,
                                         ...
                                         state.neck_trk[2]->ne[2],
                                         state.neck_trk[2]->ne[3]);
    ...
    if (s_prof) {
        ...
        AICORE_LOG_PRINT("[sam3] ", "  prop: build+download %6.1f ms, upload %6.1f ms\n",
    ...
    if (s_prof) {
        ...
        AICORE_LOG_PRINT("[sam3] ", "  prop: compute %6.1f ms\n",
    ...
    if (s_prof) {
        ...
        AICORE_LOG_PRINT("[sam3] ", "  prop: readback %6.1f ms\n",
...
static bool sam3_encode_memory(
    ...
    const bool mem_cached = (tracker.mem_ctx != nullptr) && (tracker.mem_H == H);
    ...
    if (s_prof) {
        AICORE_LOG_PRINT("[sam3] ", "  memenc: prep %6.1f ms, compute %6.1f ms\n",
...
sam3_tracker_ptr sam3_create_tracker(const sam3_model& model,
    ...
    if (model.hparams.is_sam2()) {
        AICORE_LOG_ERROR("[sam3] ", "%s: ERROR: text-prompted tracker not available for SAM2 "
                "(use sam3_create_visual_tracker instead)\n", __func__);
        return nullptr;
    }
    sam3_tracker_ptr tracker(new sam3_tracker());
    tracker->params = params;
    AICORE_LOG_PRINT("[sam3] ", "%s: tracker created (hotstart=%d, max_keep_alive=%d)\n",
...
sam3_result sam3_track_frame(sam3_tracker& tracker, sam3_state& state,
    ...
    if (model.hparams.visual_only) {
        AICORE_LOG_ERROR("[sam3] ", "%s: ERROR: track_frame not available on visual-only model "
...
int sam3_tracker_add_instance(sam3_tracker& tracker, sam3_state& state,
    ...
    if (!sam3_encode_memory(tracker, state, model, inst_id,
        ...
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to encode memory for instance %d\n", __func__, inst_id);
        return -1;
    }
...
int sam3_tracker_add_instance_from_mask(sam3_tracker& tracker, sam3_state& state,
    ...
    if (!seeded_ptr) {
        ...
        AICORE_LOG_ERROR("[sam3] ", "%s: seed decode failed, falling back to no_obj_ptr\n", __func__);
...
sam3_tracker_ptr sam3_create_visual_tracker(
    ...
    sam3_tracker_ptr tracker(new sam3_tracker());
    tracker->params = vp;
    AICORE_LOG_PRINT("[sam3] ", "%s: visual-only tracker created (max_keep_alive=%d)\n",
...
bool sam3_test_load_tokenizer(const std::string& model_path) {
    ...
    struct gguf_context* gguf = gguf_init_from_file(model_path.c_str(), gparams);
...
static bool sam3_dump_tensor_to_path(struct ggml_tensor* t,
    ...
    if (ggml_is_contiguous(t)) {
        ...
        AICORE_LOG_ERROR("[sam3] ", "%s: unsupported non-contiguous layout for '%s' (nb0=%llu)\n",
    ...
    AICORE_LOG_PRINT("[sam3] ", "%s: dumped '%s' [", __func__, tensor_name.c_str());
...
bool sam3_test_dump_text_encoder(const sam3_model& model,
    ...
    auto* galloc = ggml_gallocr_new(ggml_backend_get_default_buffer_type(model.backend));
    if (!ggml_gallocr_reserve(galloc, graph) || !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate text encoder graph\n", __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    }
...
bool sam3_test_dump_phase5(const sam3_model& model,
    ...
    if (!state.neck_det[0] || !sam3_ensure_neck_pe(mutable_state, model)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: encoded detector features are missing\n", __func__);
        return false;
    }
...
bool sam3_test_fenc_only(const sam3_model& model,
    ...
    if (!have_attn_bias) {
        AICORE_LOG_ERROR("[sam3] ", "%s: no fenc_attn_bias found, assuming all tokens valid\n", __func__);
...
static bool sam3_test_dump_phase6_impl(const sam3_model& model,
    ...
    if ((int)all_labels.size() != pe_out.n_tokens) {
        AICORE_LOG_ERROR("[sam3] ", "%s: prompt token count mismatch (%zu vs %d)\n",
    ...
    for (const auto& name : named_outputs) {
        auto* t = ggml_get_tensor(ctx0, name.c_str());
        if (!t) {
            AICORE_LOG_ERROR("[sam3] ", "%s: tensor '%s' not found after compute\n", __func__, name.c_str());
            ok = false;
            continue;
        }
        if (!sam3_dump_tensor_to_path(t, name, output_dir + "/" + name)) {
...
bool sam3_test_dump_geom_enc(const sam3_model& model,
    ...
    // Also dump the pre-computed input data (before ggml processing)
    {
        ...
        AICORE_LOG_PRINT("[sam3] ", "%s: dumped geom_input_precomputed [%d, %d]\n", __func__, D, N_geo);
    }
...
static bool sam3_test_dump_phase7_mem_slot_current(
    ...
    struct ggml_context* ctx0 = ggml_init(gparams);
    if (!ctx0) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to init memory encoder context\n", __func__);
        return false;
    }
    ...
    auto* galloc = ggml_gallocr_new(ggml_backend_get_default_buffer_type(model.backend));
    if (!ggml_gallocr_reserve(galloc, graph) || !ggml_gallocr_alloc_graph(galloc, graph)) {
        AICORE_LOG_ERROR("[sam3] ", "%s: failed to allocate phase 7 memory graph\n", __func__);
        ggml_gallocr_free(galloc);
        ggml_free(ctx0);
        return false;
    }
