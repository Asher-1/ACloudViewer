...
bool GGUFModel::load(const std::string& path, ggml_backend_buffer_type_t buft) {
    ...
    if (!buf_) {
        LOGE("failed to allocate %zu bytes for weights of %s", total,
             path.c_str());
        return false;
    }
    ...
    for (size_t i = 0; i < n_tensors; i++) {
        ...
        if (mapping.data) {
            if (data_base + offset + nbytes > mapping.size) {
                LOGE("tensor '%s' exceeds the mapping of %s", name,
                     path.c_str());
                if (f) fclose(f);
                return false;
            }
    ...
    LOGD("loaded %s: %zu tensors, %.1f MB", path.c_str(), n_tensors,
         total / 1.0e6);
    return true;
}
...
std::string GGUFModel::str(const std::string& key,
...
uint32_t GGUFModel::u32(const std::string& key, uint32_t def) const {
    ...
    switch (gguf_get_kv_type(gguf_, iid)) {
        case GGUF_TYPE_UINT32:
            return gguf_get_val_u32(gguf_, iid);
        case GGUF_TYPE_INT32:
            return (uint32_t)gguf_get_val_i32(gguf_, iid);
        case GGUF_TYPE_UINT64:
            return (uint32_t)gguf_get_val_u64(gguf_, iid);
        case GGUF_TYPE_INT64:
            return (uint32_t)gguf_get_val_i64(gguf_, iid);
        default:
            return def;
    }
}
