...
ggml_tensor* conv3d(Ctx& c,
    ...
    GGML_ASSERT(oc * ic <= c.max_conv_oc_ic &&
                "invalid SS Conv3D shape metadata");
    if (!c.im2col_shape) {
    ...
    ggml_tensor* kernel_shape = ggml_view_4d(
            c.ctx, c.im2col_shape, 3, 3, 3, oc * ic, c.im2col_shape->nb[1],
            c.im2col_shape->nb[2], c.im2col_shape->nb[3], 0);
    ggml_tensor* cols = ggml_im2col_3d(c.ctx, kernel_shape, x, ic, 1, 1, 1, pad,
                                       pad, pad, 1, 1, 1, GGML_TYPE_F32);
    ...
    if (c.debug_stage && ((prefix == "dec.input_layer" &&
                           strcmp(c.debug_stage, "im2col") == 0) ||
                          im2col_stage == c.debug_stage)) {
        return cols;
    }
    ggml_tensor* cols2d = ggml_reshape_2d(
            c.ctx, cols, cols->ne[0], cols->ne[1] * cols->ne[2] * cols->ne[3]);
    ggml_tensor* acc = ggml_mul_mat(c.ctx, w, cols2d);  // (OC, W*H*D)
    if (c.debug_stage &&
        ((prefix == "dec.input_layer" && strcmp(c.debug_stage, "gemm") == 0) ||
    ...
    if (c.debug_stage && ((prefix == "dec.input_layer" &&
                           strcmp(c.debug_stage, "convout") == 0) ||
...
ggml_tensor* res_block(Ctx& c, const std::string& prefix, ggml_tensor* x) {
    ggml_tensor* h =
            channel_layer_norm(c, x, c.m->get(prefix + ".norm1.weight"),
    ...
    if (c.debug_stage && (prefix + ".conv1" == c.debug_stage ||
                          ...
                          prefix + ".conv1.convout" == c.debug_stage))
        return h;
    ...
    if (c.debug_stage && (prefix + ".conv2" == c.debug_stage ||
                          ...
                          prefix + ".conv2.convout" == c.debug_stage))
        return h;
...
ggml_tensor* SsDecoderGraph::build(ggml_tensor* latent) {
    ...
    if (c.debug_stage &&
        (!strcmp(c.debug_stage, "im2col") || !strcmp(c.debug_stage, "gemm") ||
         !strcmp(c.debug_stage, "convout")))
        return h;
    c.dbg(h);
    ...
    const int n_blocks =
            n_res * (int)channels.size() + ((int)channels.size() - 1);
