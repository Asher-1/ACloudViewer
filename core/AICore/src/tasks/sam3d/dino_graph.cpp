...
static ggml_tensor* pos_probe(ggml_context* ctx,
                              const GGUFModel* m,
...
ggml_tensor* DinoGraph::build(ggml_tensor* img) {
    ...
    GGML_ASSERT(G == Gv && G == 37 &&
                "expect 518x518 input (pos_embed must match)");
    ...
    table_data.push_back(
            f32_buf({1.0f / 0.229f, 1.0f / 0.224f, 1.0f / 0.225f}));
    ...
    ggml_tensor* w2d =
            ggml_view_2d(ctx, w, kdim, C, kdim * ggml_element_size(w), 0);
    ggml_tensor* img1 =
    ...
    ggml_tensor* reg =
            as_f32(ctx, m->get(prefix + ".backbone.register_tokens"));
    ggml_tensor* xfull = ggml_concat(ctx, ggml_reshape_2d(ctx, cls, C, 1), x,
                                     1);  // (C, 1+n_patch)
    ...
    ggml_tensor* pat_view = ggml_view_2d(ctx, xfull, C, n_patch,
    ...
    for (int i = 0; i < (int)depth; i++) {
        ...
        ggml_tensor* q = gb_linear(
                ctx, ggml_view_2d(ctx, wqkv, Cw, C, wqkv->nb[1], 0), bq, h);
        ggml_tensor* k = gb_linear(
                ctx,
                ggml_view_2d(ctx, wqkv, Cw, C, wqkv->nb[1], C * wqkv->nb[1]),
                bk, h);
        ggml_tensor* v = gb_linear(ctx,
                                   ggml_view_2d(ctx, wqkv, Cw, C, wqkv->nb[1],
                                                2 * C * wqkv->nb[1]),
                                   bv, h);
    ...
    ggml_tensor* cls_out = ggml_view_2d(ctx, xfull, C, 1, C * sizeof(float), 0);
    ggml_tensor* pat_out =
            ggml_view_2d(ctx, xfull, C, n_patch, C * sizeof(float),
