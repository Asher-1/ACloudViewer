...
ScopedTimer::~ScopedTimer() { LOGD("%s took %.2f ms", name, now_ms() - t0); }
...
bool save_raw_tensor_f32(const std::string& path,
...
bool save_raw_tensor_i32(const std::string& path,
                         const std::vector<int64_t>& ne,
                         const int32_t* data) {
    RawTensor t;
    t.ne = ne;
...
bool load_raw_tensor(const std::string& path, RawTensor& out) {
    ...
    const size_t nbytes =
            n * ggml_type_size(out.type) / ggml_blck_size(out.type);
...
void copy_tensor_to_float(const struct ggml_tensor* t,
