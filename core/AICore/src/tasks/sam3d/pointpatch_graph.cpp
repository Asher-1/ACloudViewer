...
static void nearest_index_table(std::vector<int32_t>& t,
                                int src_size,
                                int dst_size) {
...
ggml_tensor* PointPatchGraph::build(ggml_tensor* pm) {
    ...
    for (int c = 0; c < 3; c++) {
        ...
        ggml_tensor* plane =
                ggml_view_2d(ctx, pm, 1, src_w * src_h, sizeof(float),
    ...
    ggml_tensor* d = ggml_mul(ctx, ggml_sub(ctx, x, inv_tok), mask_i);
    ...
    ggml_tensor* cls = ggml_reshape_2d(
            ctx, as_f32(ctx, m->get(prefix + ".cls_token")), D, 1);
    ...
    ggml_tensor* pw = ggml_reshape_3d(
            ctx, as_f32(ctx, m->get(prefix + ".pos_embed_window")), D,
            1 + ps * ps, 1);
    ...
    ggml_tensor* wk =
            ggml_view_2d(ctx, wqkv, D, D, wqkv->nb[1], (size_t)D * wqkv->nb[1]);
    ...
    auto to_heads4 = [&](ggml_tensor* t) {
        ggml_tensor* r =
                ggml_reshape_4d(ctx, t, Hd, n_heads, 1 + ps * ps, n_win);
    ...
    h = gb_ffn_gelu(ctx, h, m->get(b + ".mlp.fc1.weight"),
                    m->get(b + ".mlp.fc1.bias"), m->get(b + ".mlp.fc2.weight"),
