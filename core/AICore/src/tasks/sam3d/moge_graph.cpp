                ...
                for (int ky = 0; ky < patch; ++ky) {
                    for (int kx = 0; kx < patch; ++kx) {
                        const int kernel_index =
                                channel * patch * patch + ky * patch + kx;
                        const int source_y = py * patch + ky;
                        const int source_x = px * patch + kx;
                        table[static_cast<size_t>(patch_index) * patch_values +
                              kernel_index] =
...
std::vector<float> make_bicubic_resample_table(int source_size,
                                               int target_size,
                                               float scale_factor) {
    ...
    std::vector<float> table(static_cast<size_t>(source_size) * target_size,
                             0.0f);
    for (int target = 0; target < target_size; ++target) {
        const float source =
                (static_cast<float>(target) + 0.5f) / scale_factor - 0.5f;
        const int first = static_cast<int>(std::floor(source)) - 1;
        for (int tap = 0; tap < 4; ++tap) {
            const int unbounded = first + tap;
            const int clamped =
                    std::max(0, std::min(source_size - 1, unbounded));
...
ggml_tensor* replicate_pad_2d(ggml_context* ctx, ggml_tensor* x, int padding) {
    ...
    ggml_tensor* right =
            ggml_view_4d(ctx, x, 1, height, channels, batch, x->nb[1], x->nb[2],
                         x->nb[3], (width - 1) * x->nb[0]);
    ggml_tensor* padded_x = x;
    for (int i = 0; i < padding; ++i)
        padded_x = ggml_concat(ctx, left, padded_x, 0);
    for (int i = 0; i < padding; ++i)
        padded_x = ggml_concat(ctx, padded_x, right, 0);

    const int64_t padded_width = padded_x->ne[0];
    ggml_tensor* top =
            ggml_view_4d(ctx, padded_x, padded_width, 1, channels, batch,
                         padded_x->nb[1], padded_x->nb[2], padded_x->nb[3], 0);
    ggml_tensor* bottom = ggml_view_4d(
            ctx, padded_x, padded_width, 1, channels, batch, padded_x->nb[1],
            padded_x->nb[2], padded_x->nb[3], (height - 1) * padded_x->nb[1]);
    for (int i = 0; i < padding; ++i)
        padded_x = ggml_concat(ctx, top, padded_x, 1);
    for (int i = 0; i < padding; ++i)
        padded_x = ggml_concat(ctx, padded_x, bottom, 1);
    return padded_x;
}

ggml_tensor* add_channel_bias(ggml_context* ctx,
                              ggml_tensor* x,
                              ggml_tensor* bias) {
...
ggml_tensor* conv2d_replicate(ggml_context* ctx,
                              ggml_tensor* weight,
                              ggml_tensor* bias,
                              ggml_tensor* x) {
...
ggml_tensor* conv2d_1x1(ggml_context* ctx,
                        ggml_tensor* weight,
                        ggml_tensor* bias,
                        ggml_tensor* x) {
    weight = as_f32(ctx, weight);
    ...
    if (weight->ne[2] != x->ne[2]) {
        LOGE("MoGe 1x1 convolution channel mismatch: "
             "weight=[%lld,%lld,%lld,%lld] input=[%lld,%lld,%lld,%lld]",
             static_cast<long long>(weight->ne[0]),
             static_cast<long long>(weight->ne[1]),
             static_cast<long long>(weight->ne[2]),
             static_cast<long long>(weight->ne[3]),
             static_cast<long long>(x->ne[0]), static_cast<long long>(x->ne[1]),
             static_cast<long long>(x->ne[2]),
             static_cast<long long>(x->ne[3]));
    }
    x = ggml_conv_2d_direct(ctx, weight, x, 1, 1, 0, 0, 1, 1);
    return add_channel_bias(ctx, x, bias);
}

ggml_tensor* group_norm(ggml_context* ctx,
                        ggml_tensor* x,
                        ggml_tensor* weight,
                        ggml_tensor* bias,
                        int groups) {
    ...
    weight = as_f32(ctx, weight);
    bias = as_f32(ctx, bias);
    GGML_ASSERT(ggml_nelements(weight) == channels &&
                ggml_nelements(bias) == channels);
    x = ggml_mul(ctx, x, ggml_reshape_4d(ctx, weight, 1, 1, channels, 1));
    return ggml_add(ctx, x, ggml_reshape_4d(ctx, bias, 1, 1, channels, 1));
}

ggml_tensor* residual_conv_block(ggml_context* ctx,
...
ggml_tensor* tokens_to_image(ggml_context* ctx,
                             ggml_tensor* tokens,
                             int64_t width,
                             int64_t height) {
...
MogeOutputs MogeGraph::build(ggml_tensor* img) {
    ...
    GGML_ASSERT(img->ne[0] == 3 && resized_width >= patch &&
    ...
    const float attention_scale =
            1.0f / std::sqrt(static_cast<float>(hidden / heads));
    auto add_f32_input = [&](const char* name,
    ...
    auto add_i32_input = [&](const char* name,
                             std::initializer_list<int64_t> shape,
    ...
    auto mean = std::make_shared<std::vector<float>>(
            std::initializer_list<float>{0.485f, 0.456f, 0.406f});
    auto inverse_std =
            std::make_shared<std::vector<float>>(std::initializer_list<float>{
                    1.0f / 0.229f, 1.0f / 0.224f, 1.0f / 0.225f});
    ggml_tensor* mean_tensor = add_f32_input("moge_mean", {3, 1, 1}, mean);
    ggml_tensor* inverse_std_tensor =
            add_f32_input("moge_inverse_std", {3, 1, 1}, inverse_std);
    ...
    dino_img = ggml_interpolate(
            ctx, dino_img, dino_width, dino_height, 3, 1,
    ...
    auto patch_table = std::make_shared<std::vector<int32_t>>(make_patch_table(
            static_cast<int>(dino_height), static_cast<int>(dino_width),
            static_cast<int>(patch)));
    ggml_tensor* patch_indices = add_i32_input(
            "moge_patch_table", {patch_values * n_patches}, patch_table);
    ggml_tensor* image_vector =
            ggml_view_2d(ctx, img, 1, ggml_nelements(img), sizeof(float), 0);
    ...
    ggml_tensor* patch_weight =
            as_f32(ctx, m->get(prefix + ".backbone.patch_embed.proj.weight"));
    ggml_tensor* patch_bias =
            as_f32(ctx, m->get(prefix + ".backbone.patch_embed.proj.bias"));
    ggml_tensor* x =
            linear(ctx,
                   ggml_view_2d(ctx, patch_weight, patch_values, hidden,
    ...
    ggml_tensor* cls_pos =
            ggml_view_2d(ctx, pos, hidden, 1, hidden * sizeof(float), 0);
    ggml_tensor* patch_pos = ggml_view_4d(
            ctx, pos, hidden, 37, 37, 1, hidden * sizeof(float),
            37 * hidden * sizeof(float), 37 * 37 * hidden * sizeof(float),
            hidden * sizeof(float));
    auto pos_x_values =
            std::make_shared<std::vector<float>>(make_bicubic_resample_table(
                    37, static_cast<int>(patches_w),
                    (static_cast<float>(patches_w) + 0.1f) / 37.0f));
    auto pos_y_values =
            std::make_shared<std::vector<float>>(make_bicubic_resample_table(
                    37, static_cast<int>(patches_h),
                    (static_cast<float>(patches_h) + 0.1f) / 37.0f));
    ggml_tensor* pos_x =
            add_f32_input("moge_pos_resample_x", {37, patches_w}, pos_x_values);
    ggml_tensor* pos_y =
            add_f32_input("moge_pos_resample_y", {37, patches_h}, pos_y_values);
    // [C, W, H] -> [W, C, H], resample W, then [H, W, C], resample H.
    ggml_tensor* pos_w =
            ggml_cont(ctx, ggml_permute(ctx, patch_pos, 1, 0, 2, 3));
    pos_w = ggml_mul_mat(ctx, pos_x,
                         ggml_reshape_2d(ctx, pos_w, 37, hidden * 37));
    pos_w = ggml_reshape_3d(ctx, pos_w, patches_w, hidden, 37);
    ggml_tensor* pos_h = ggml_cont(ctx, ggml_permute(ctx, pos_w, 1, 2, 0, 3));
    pos_h = ggml_mul_mat(ctx, pos_y,
                         ggml_reshape_2d(ctx, pos_h, 37, patches_w * hidden));
    ...
    ggml_tensor* backbone_position_tokens =
            ggml_concat(ctx, cls_pos, patch_pos, 1);
    ...
    for (int i = 0; i < blocks; ++i) {
        ...
        ggml_tensor* attention =
                gb_attention(ctx, as_f32(ctx, q), k, v, attention_scale, true,
                             AttentionOptions{/*.strict_kv=*/true});
        if (i == 0) {
        ...
        attention = ggml_mul(ctx, as_f32(ctx, attention),
                             m->get(block + ".ls1.gamma"));
    ...
    for (int i = 0; i < static_cast<int>(features.size()); ++i) {
        ggml_tensor* feature = tokens_to_image(
                ctx, features[static_cast<size_t>(i)], patches_w, patches_h);
        feature = conv2d_1x1(ctx,
                             m->get(prefix + ".head.projects." +
                                    std::to_string(i) + ".weight"),
                             m->get(prefix + ".head.projects." +
                                    std::to_string(i) + ".bias"),
                             feature);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        const std::string block =
                prefix + ".head.upsample_blocks." + std::to_string(i);
        ...
        ggml_tensor* uv_tensor =
                add_f32_input("moge_up_uv", {width, height, 2, 1}, uv);
        head = ggml_concat(ctx, head, uv_tensor, 2);
        ggml_tensor* transpose_weight =
                as_f32(ctx, m->get(block + ".0.0.weight"));
        head = ggml_conv_transpose_2d_p0(ctx, transpose_weight, head, 2);
        ...
        for (int64_t residual = 0; residual < residual_blocks; ++residual) {
            head = residual_conv_block(
                    ctx, m, block + "." + std::to_string(residual + 1), head);
        }
    }
    head = ggml_interpolate(ctx, head, resized_width, resized_height,
                            head->ne[2], 1, GGML_SCALE_MODE_BILINEAR);
    auto output_uv = std::make_shared<std::vector<float>>(make_uv(
            static_cast<int>(resized_width), static_cast<int>(resized_height),
            static_cast<float>(resized_width) / resized_height));
    ggml_tensor* output_uv_tensor = add_f32_input(
            "moge_output_uv", {resized_width, resized_height, 2, 1}, output_uv);
    head = ggml_concat(ctx, head, output_uv_tensor, 2);

    const std::string output_prefix = prefix + ".head.output_block.";
    ggml_tensor* points =
            conv2d_replicate(ctx, m->get(output_prefix + "0.0.weight"),
    ...
    ggml_tensor* mask =
            conv2d_replicate(ctx, m->get(output_prefix + "1.0.weight"),
    ...
    ggml_tensor* xy = ggml_view_4d(ctx, raw_points, raw_points->ne[0],
                                   raw_points->ne[1], 2, 1, raw_points->nb[1],
                                   raw_points->nb[2], raw_points->nb[3], 0);
    ggml_tensor* z =
            ggml_view_4d(ctx, raw_points, raw_points->ne[0], raw_points->ne[1],
                         1, 1, raw_points->nb[1], raw_points->nb[2],
                         raw_points->nb[3], 2 * raw_points->nb[2]);
    z = ggml_exp(ctx, z);
    xy = ggml_mul(ctx, xy, ggml_repeat(ctx, z, xy));
    return {raw_points,
            ggml_concat(ctx, xy, z, 2),
            mask,
            backbone_image,
            backbone_patch_tokens,
            backbone_position_tokens,
            backbone_input,
            debug_q,
            debug_k,
            debug_v,
            debug_attention_context,
            attention_projection_outputs,
            attention_outputs,
            mlp_fc1_outputs,
            mlp_gelu_outputs,
            mlp_outputs,
