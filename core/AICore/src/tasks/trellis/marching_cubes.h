...
// The 6 tetrahedra of the Freudenthal subdivision, all sharing the 0-6
// diagonal.
static const int TETRA[6][4] = {{0, 1, 2, 6}, {0, 2, 3, 6}, {0, 3, 7, 6},
                                {0, 7, 4, 6}, {0, 4, 5, 6}, {0, 5, 1, 6}};

// A tetrahedron's 6 edges, as (local-corner, local-corner) pairs.
static const int TET_EDGE[6][2] = {{0, 1}, {1, 2}, {2, 0},
                                   {0, 3}, {1, 3}, {2, 3}};
...
inline Mesh extract(const float* field,
                    int nx,
                    int ny,
                    int nz,
                    float iso,
    ...
    // Tie-breaking jitter (Simulation-of-Simplicity style): a corner whose
    // value is *exactly* iso makes the surface pass through that single point,
    // which is shared by every incident tetrahedron — a pinch that no winding
    // can make coherent. A tiny deterministic per-corner perturbation
    // (consistent across cubes, so shared corners still agree -> mesh stays
    // watertight) pushes every corner decisively on/off the surface. Scaled to
    // the field so vertex motion is sub-micro-voxel; continuous fields (e.g.
    // decoder logits) are unaffected.
    double fmax = 0.0;
    ...
    auto at = [&](int x, int y, int z) -> float {
        if (x < 0 || y < 0 || z < 0 || x >= nx || y >= ny || z >= nz)
            return pad;
        return field[(size_t)x + (size_t)y * nx + (size_t)z * nx * ny] +
               eps * jitter(x, y, z);
    };
    ...
    std::unordered_map<int64_t, int>
            vmap;  // quantized-position key -> vertex index
    ...
    // corners, so several distinct edges land on the same point. Position
    // keying welds those into one vertex (killing the degenerate zero-area cap
    // triangles and keeping the boundary watertight), while shared edges across
    // cubes — which already yield identical positions — still merge as before.
    const double QS = 1024.0;  // sub-milli-voxel quantization
    auto edge_vertex = [&](const int ca[3], const int cb[3], float va,
                           float vb) -> int {
        ...
        if (t < 0.0f)
            ...
            t = 1.0f;
        float p[3] = {ca[0] + t * (cb[0] - ca[0]), ca[1] + t * (cb[1] - ca[1]),
                      ca[2] + t * (cb[2] - ca[2])};
        ...
        for (int y = -1; y < ny; ++y)
            for (int x = -1; x < nx; ++x) {
                ...
                for (int t = 0; t < 6; ++t) {
                    ...
                    // Gather this tetra's triangles (1 or 2), then decide their
                    // winding ONCE, together: flip so the summed geometric
                    // normal (snorm) agrees with the summed outward vertex
                    // normal (gsum = sum of -grad over the triangle vertices).
                    // gsum is PARALLEL to the surface normal, so the dot is
                    // never near zero — unlike a centroid-difference proxy,
                    // which can be in-plane at boundary/crease tetra and pick a
                    // random sign. The surface inside a tetra is planar, so a
                    // quad's two triangles are coplanar and flip as a unit (a
                    // per-triangle test would mis-flip a sliver triangle, whose
                    // own geometric normal is ~0).
                    int tribuf[2][3];
                    int ntri = 0;
                    ...
                    for (int e = 0; tri[e] >= 0; e += 3) {
                        ...
                        for (int k = 0; k < 3; ++k) {
                            ...
                            vi[k] = edge_vertex(ccoord[la], ccoord[lb],
                                                cval[la], cval[lb]);
                        }
                        if (vi[0] == vi[1] || vi[1] == vi[2] || vi[0] == vi[2])
                            continue;  // degenerate
                        const float* P0 = &mesh.verts[3 * vi[0]];
                        const float* P1 = &mesh.verts[3 * vi[1]];
                        const float* P2 = &mesh.verts[3 * vi[2]];
                        float u[3] = {P1[0] - P0[0], P1[1] - P0[1],
                                      P1[2] - P0[2]};
                        float v[3] = {P2[0] - P0[0], P2[1] - P0[1],
                                      P2[2] - P0[2]};
                    ...
                    bool flip = (snorm[0] * gsum[0] + snorm[1] * gsum[1] +
                                 snorm[2] * gsum[2]) < 0.0;
                    for (int a = 0; a < ntri; ++a) {
                        int v0 = tribuf[a][0], v1 = tribuf[a][1],
                            v2 = tribuf[a][2];
                        if (flip) {
                            int tmp = v1;
                            v1 = v2;
...
inline bool write_obj(const Mesh& m, const char* path) {
    ...
    for (size_t i = 0; i < m.n_verts(); ++i)
        std::fprintf(f, "v %.6f %.6f %.6f\n", m.verts[3 * i],
                     m.verts[3 * i + 1], m.verts[3 * i + 2]);
    for (size_t i = 0; i < m.n_verts(); ++i)
        std::fprintf(f, "vn %.6f %.6f %.6f\n", m.normals[3 * i],
                     m.normals[3 * i + 1], m.normals[3 * i + 2]);
    for (size_t i = 0; i < m.n_tris(); ++i) {
        int a = m.tris[3 * i] + 1, b = m.tris[3 * i + 1] + 1,
            c = m.tris[3 * i + 2] + 1;
