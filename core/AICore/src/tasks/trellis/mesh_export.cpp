...
static bool decimate_topology(PreparedMesh & mesh, size_t target_tris,
                              float error_limit, std::string & err) {
...
static bool decimate_sloppy(PreparedMesh & mesh, size_t target_tris,
                            float error_limit, float sliver_aspect,
                            std::string & err) {
    const size_t nt = mesh.tris.size() / 3;
    if (nt <= target_tris || nt == 0) return true;
    const size_t nv = mesh.verts.size() / 3;
    ...
    const size_t n = meshopt_simplifySloppy(
        ...
        mesh.verts.data(), nv, sizeof(float) * 3,
        target_tris * 3, decimation_error(error_limit), &result_error);
...
static bool decimate_with_pbr(PreparedMesh & mesh, size_t target_tris,
                              float error_limit, float sliver_aspect,
                              std::string & err) {
    const size_t nt = mesh.tris.size() / 3;
    if (nt <= target_tris || nt == 0) return true;
    ...
    if (n / 3 > target_tris) {
        ...
        return decimate_sloppy(mesh, target_tris, error_limit,
                               sliver_aspect, err);
    }
