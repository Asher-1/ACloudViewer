...
bool encode_png(int w, int h, int comp, const uint8_t * pix, std::vector<uint8_t> & out) {
    ...
    const QImage::Format fmt = comp == 4 ? QImage::Format_RGBA8888
...
static bool decimate_topology(PreparedMesh & mesh, size_t target_tris, std::string & err) {
    ...
    if (n / 3 > target_tris) {
        ...
        n = meshopt_simplifySloppy(
            dst.data(), idx.data(), idx.size(),
            mesh.verts.data(), nv, sizeof(float) * 3,
            nullptr, target_tris * 3, 1e-1f, &result_error);
...
static bool decimate_with_pbr(PreparedMesh & mesh, size_t target_tris, std::string & err) {
    ...
    if (n / 3 > target_tris) {
        ...
        n = meshopt_simplifySloppy(
            ...
            mesh.verts.data(), nv, sizeof(float) * 3,
            nullptr, target_tris * 3, 1e-1f, &result_error);
        if (n == 0) { err = "mesh sloppy decimation with PBR failed"; return false; }
        GLBLOG("sloppy decimated+pbr %zu -> %zu tris (error %.4g)", nt, n / 3, result_error);
    } else {
        GLBLOG("decimated+pbr %zu/%zu tris (error %.4g)", nt, n / 3, result_error);
    }
    ...
    GLBLOG("decimated+pbr %zu/%zu -> %zu/%zu tris (error %.4g)",
