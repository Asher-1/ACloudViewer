...
// giving the CLI / debugging a progress trace).
...
bool encode_png(
    ...
    // QImage::Format_RGB888 requires 32-bit row alignment; build a copy with
    // the padded stride when the input row stride (w*comp) is not aligned.
...
bool bake_atlas_locked(const PreparedMesh &mesh,
    ...
    std::vector<uint32_t> oidx;
    int AW = TS, AH = TS;
    if (!xatlas_unwrap(mesh.verts, mesh.normals, mesh.pbr, dnv, mesh.tris, dnt,
                       opt, TS, opos, onrm, ouv, opbr, oidx, AW, AH, err))
        return false;
...
bool prepare_mesh_locked(const float *verts,
    ...
    // the geometry that will actually be written to the GLB. Was the
    // T2GLB_XATLAS / T2GLB_NOSMOOTH environment variables upstream; now a
    // MeshExportOptions field.
...
static bool decimate_topology(PreparedMesh &mesh,
    ...
    size_t n = meshopt_simplify(dst.data(), idx.data(), idx.size(),
                                ...
                                target_tris * 3, 1e-2f, 0, &result_error);
    if (n == 0) {
    ...
    if (n / 3 > target_tris) {
        result_error = 0.0f;
        n = meshopt_simplifySloppy(dst.data(), idx.data(), idx.size(),
                                   mesh.verts.data(), nv, sizeof(float) * 3,
                                   nullptr, target_tris * 3, 1e-1f,
                                   &result_error);
        if (n == 0) {
        ...
        GLBLOG("decimated %zu -> %zu tris (error %.4g)", nt, n / 3,
               result_error);
    }
    mesh.tris.resize(n);
...
static bool decimate_with_pbr(PreparedMesh &mesh,
    ...
    if (n / 3 > target_tris) {
        result_error = 0.0f;
        n = meshopt_simplifySloppy(idx.data(), idx.data(), idx.size(),
                                   mesh.verts.data(), nv, sizeof(float) * 3,
                                   nullptr, target_tris * 3, 1e-1f,
                                   &result_error);
        if (n == 0) {
            err = "mesh sloppy decimation with PBR failed";
            return false;
        }
        GLBLOG("sloppy decimated+pbr %zu -> %zu tris (error %.4g)", nt, n / 3,
               result_error);
    } else {
        GLBLOG("decimated+pbr %zu/%zu tris (error %.4g)", nt, n / 3,
               result_error);
    }
    ...
    GLBLOG("decimated+pbr %zu/%zu -> %zu/%zu tris (error %.4g)", nv, nt, new_nv,
           n / 3, result_error);
    return true;
}
...
bool mesh_to_glb(const float *verts,
    ...
    const MeshExportOptions &bake_opt = opt;
    ...
    // Was the T2GLB_VERTEX environment variable upstream; now an option.
    ...
    // Without decimation: direct UV atlas bake for pre-simplified meshes.
    // Was the T2GLB_XATLAS environment variable upstream.
    if (bake_opt.taubin_smooth && bake_opt.decimation_target <= 0) {
