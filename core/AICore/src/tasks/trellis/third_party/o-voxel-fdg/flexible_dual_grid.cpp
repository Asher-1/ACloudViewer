...
std::array<int3, 2> quad_to_2tri(const std::vector<float3>& vertices,
    ...
    float angle_ac = std::acos(clamp_val(n_abc.dot(n_acd), -1.0f, 1.0f));

    // diagonal BD
    Eigen::Vector3f n_abd = (b - a).cross(d - a).normalized();
    Eigen::Vector3f n_bcd = (c - b).cross(d - b).normalized();
    float angle_bd = std::acos(clamp_val(n_abd.dot(n_bcd), -1.0f, 1.0f));
...
void face_from_dual_vertices(
    ...
    for (int i = 0; i < dual_vertices.size(); ++i) {
        ...
        if (is_intersected[2] && neigh_indices[0] != kInvalidIndex &&
            ...
            int4 quad_indices{i, static_cast<int>(neigh_indices[0]),
                              static_cast<int>(neigh_indices[2]),
                              static_cast<int>(neigh_indices[1])};
        ...
        if (is_intersected[0] && neigh_indices[1] != kInvalidIndex &&
            ...
            int4 quad_indices{i, static_cast<int>(neigh_indices[1]),
                              static_cast<int>(neigh_indices[5]),
                              static_cast<int>(neigh_indices[3])};
            auto tri_indices = quad_to_2tri(dual_vertices, quad_indices);
            face_indices.insert(face_indices.end(), tri_indices.begin(),
                                tri_indices.end());
        }
        ...
        if (is_intersected[1] && neigh_indices[0] != kInvalidIndex &&
            ...
            int4 quad_indices{i, static_cast<int>(neigh_indices[0]),
                              static_cast<int>(neigh_indices[4]),
                              static_cast<int>(neigh_indices[3])};
            auto tri_indices = quad_to_2tri(dual_vertices, quad_indices);
            face_indices.insert(face_indices.end(), tri_indices.begin(),
                                tri_indices.end());
...
static void solve_qef_dual_vertices(const std::vector<int3>& voxels,
    ...
    for (size_t i = 0; i < voxels.size(); ++i) {
        ...
        if (regularization_weight > 0.0f) {
            ...
            Qreg(3, 3) = p.dot(p);
            Q += regularization_weight * cnt[i] * Qreg;
        }
        ...
        if (!(v_new.x() >= min_corner[0] && v_new.x() <= max_corner[0] &&
            ...
            auto solve_single_constraint = [&](int fixed_axis) {
                ...
                b2 = -B * q;
                x = Asol.solve(b2);
                if (x.x() >= min_corner[ax1] && x.x() <= max_corner[ax1] &&
            ...
            solve_single_constraint(1);
            solve_single_constraint(2);

            auto solve_two_constraint = [&](int free_axis) {
                int ax1 = (free_axis + 1) % 3;
                int ax2 = (free_axis + 2) % 3;
                float a = Q(free_axis, free_axis);
                Eigen::Vector3f b3;
                b3 << Q(free_axis, ax1), Q(free_axis, ax2), Q(free_axis, 3);
                auto try_corner = [&](float c1, float c2) {
                    Eigen::Vector3f q;
                    q << c1, c2, 1;
                    float x = -(b3.dot(q)) / a;
                ...
                try_corner(min_corner[ax1], max_corner[ax2]);
                try_corner(max_corner[ax1], min_corner[ax2]);
                try_corner(max_corner[ax1], max_corner[ax2]);
            };
            solve_two_constraint(0);
            solve_two_constraint(1);
            solve_two_constraint(2);

            for (int xc = 0; xc < 2; ++xc) {
                for (int yc = 0; yc < 2; ++yc) {
                    for (int zc = 0; zc < 2; ++zc) {
                        Eigen::Vector4f p;
                        p[0] = xc ? min_corner[0] : max_corner[0];
                        p[1] = yc ? min_corner[1] : max_corner[1];
...
FdgVoxelGrid mesh_to_flexible_dual_grid_native(const float* verts,
    ...
    Eigen::Vector3i e_grid_min(grid_range[0], grid_range[1], grid_range[2]);
    Eigen::Vector3i e_grid_max(grid_range[3], grid_range[4], grid_range[5]);
    ...
    out.dual_verts.resize(n * 3);
    out.intersected.resize(n * 3);
    for (size_t i = 0; i < n; ++i) {
        out.coords[i * 3 + 0] = voxels[i].x;
        out.coords[i * 3 + 1] = voxels[i].y;
        out.coords[i * 3 + 2] = voxels[i].z;
        out.dual_verts[i * 3 + 0] = dual_vertices[i].x;
        out.dual_verts[i * 3 + 1] = dual_vertices[i].y;
        out.dual_verts[i * 3 + 2] = dual_vertices[i].z;
        out.intersected[i * 3 + 0] = intersected[i].x ? 1 : 0;
        out.intersected[i * 3 + 1] = intersected[i].y ? 1 : 0;
        out.intersected[i * 3 + 2] = intersected[i].z ? 1 : 0;
    }
    return out;
}
