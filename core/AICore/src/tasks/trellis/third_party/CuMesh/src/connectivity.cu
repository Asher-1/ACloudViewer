 ...
 */
static __global__ void get_neighbor_face_cnt_kernel(const int3* faces,
                                                    const int F,
 ...
 * @param faces: the faces of the mesh, shape (F)
 * @param F: the number of faces
 * @param neighbor_face_ids: the buffer for neighbor face ids, shape
 * (total_neighbor_face_cnt)
 * @param neighbor_face_ids_offset: the buffer for neighbor face ids offset,
 * shape (V+1)
 * @param neighbor_face_cnt: the buffer for neighbor face count, shape (V+1)
 */
static __global__ void fill_neighbor_face_ids_kernel(
    ...
    neighbor_face_ids[neighbor_face_ids_offset[f.x] +
                      atomicAdd(&neighbor_face_cnt[f.x], 1)] = tid;
    neighbor_face_ids[neighbor_face_ids_offset[f.y] +
                      atomicAdd(&neighbor_face_cnt[f.y], 1)] = tid;
    neighbor_face_ids[neighbor_face_ids_offset[f.z] +
                      atomicAdd(&neighbor_face_cnt[f.z], 1)] = tid;
}
...
void CuMesh::get_vertex_face_adjacency() {
    ...
    get_neighbor_face_cnt_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                   BLOCK_SIZE, 0, stream>>>(
            this->faces.ptr, F, this->vert2face_cnt.ptr);
    ...
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, this->vert2face_cnt.ptr,
            this->vert2face_offset.ptr, V + 1, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            this->vert2face_cnt.ptr, this->vert2face_offset.ptr, V + 1,
            stream));
    this->vert2face.resize(F * 3);

    // fill neighboring face ids for each vertex
    this->vert2face_cnt.zero();
    fill_neighbor_face_ids_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                    BLOCK_SIZE, 0, stream>>>(
            this->faces.ptr, F, this->vert2face.ptr, this->vert2face_offset.ptr,
 ...
 */
static __global__ void expand_edges_kernel(const int3* faces,
                                           const int F,
                                           uint64_t* edges) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= F) return;
...
void CuMesh::get_edges() {
    ...
    expand_edges_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                          stream>>>(this->faces.ptr, F, this->edges.ptr);
    CUDA_CHECK(cudaGetLastError());
    ...
    CUDA_CHECK(cub::DeviceRadixSort::SortKeys(
            nullptr, temp_storage_bytes, this->edges.ptr,
            reinterpret_cast<uint64_t*>(this->temp_storage.ptr), F * 3, 0,
            sizeof(uint64_t) * 8, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRadixSort::SortKeys(
            this->cub_temp_storage.ptr, temp_storage_bytes, this->edges.ptr,
            reinterpret_cast<uint64_t*>(this->temp_storage.ptr), F * 3, 0,
            sizeof(uint64_t) * 8, stream));
    ...
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            ...
            this->edges.ptr, this->edge2face_cnt.ptr, num_edges, F * 3,
            stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            ...
            this->edges.ptr, this->edge2face_cnt.ptr, num_edges, F * 3,
            stream));
    CUDA_CHECK(cudaMemcpyAsync(&this->edges.size, num_edges, sizeof(int),
 ...
 * @param vert2face: the buffer for neighboring face ids, shape
 * (total_neighbor_face_cnt)
 * @param vert2face_offset: the buffer for neighboring face ids offset, shape
 * (V+1)
 * @param edge2face_offset: the buffer for edge to face adjacency offset, shape
 * (E+1)
 * @param E: the number of edges
 * @param edge2face: the buffer for edge to face adjacency, shape
 * (total_edge_face_cnt)
...
static __global__ void get_edge_face_adjacency_kernel(
    ...
    for (int f = vert2face_offset[e0]; f < vert2face_offset[e0 + 1]; f++) {
        ...
        if (f_vids.x == e1 || f_vids.y == e1 || f_vids.z == e1) {
            ...
            ptr++;
            // fill face2edge
            if (f_vids.x == e0 && f_vids.y == e1 ||
                f_vids.x == e1 && f_vids.y == e0) {
                face2edge[fid].x = tid;
            } else if (f_vids.y == e0 && f_vids.z == e1 ||
                       f_vids.y == e1 && f_vids.z == e0) {
                face2edge[fid].y = tid;
            } else if (f_vids.z == e0 && f_vids.x == e1 ||
                       f_vids.z == e1 && f_vids.x == e0) {
                face2edge[fid].z = tid;
            }
        }
    }
...
void CuMesh::get_edge_face_adjacency() {
    ...
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, this->edge2face_cnt.ptr,
            this->edge2face_offset.ptr, E + 1, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            this->edge2face_cnt.ptr, this->edge2face_offset.ptr, E + 1,
            stream));

    // allocate memory for edge2face
    int total_edge_face_cnt;
    CUDA_CHECK(cudaMemcpyAsync(&total_edge_face_cnt,
                               &this->edge2face_offset.ptr[E], sizeof(int),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    // get edge-face adjacency
    get_edge_face_adjacency_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                     BLOCK_SIZE, 0, stream>>>(
            this->faces.ptr, this->edges.ptr, this->edge2face_cnt.ptr,
            this->vert2face.ptr, this->vert2face_offset.ptr,
            this->edge2face_offset.ptr, E, this->edge2face.ptr,
            this->face2edge.ptr);
    CUDA_CHECK(cudaGetLastError());
}
 ...
 */
static __global__ void get_vertex_edge_cnt_kernel(const uint64_t* edges,
                                                  const int E,
                                                  int* vert2edge_cnt) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= E) return;
 ...
 * @param edges: the buffer for edges, shape (E)
 * @param E: the number of edges
 * @param vert2edge: the buffer for vertex to edge adjacency, shape
 * (total_vertex_edge_cnt)
 * @param vert2edge_offset: the buffer for vertex to edge adjacency offset,
 * shape (V+1)
 * @param vert2edge_cnt: the buffer for vertex adjacent edge number, shape (V)
 */
static __global__ void get_vertex_edge_adjacency_kernel(const uint64_t* edges,
                                                        const int E,
                                                        int* vert2edge,
...
void CuMesh::get_vertex_edge_adjacency() {
    ...
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, this->vert2edge_cnt.ptr,
            this->vert2edge_offset.ptr, V + 1, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            this->vert2edge_cnt.ptr, this->vert2edge_offset.ptr, V + 1,
            stream));

    // get vertex-edge adjacency
    this->vert2edge.resize(2 * E);
    this->vert2edge_cnt.zero();
    get_vertex_edge_adjacency_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                       BLOCK_SIZE, 0, stream>>>(
            this->edges.ptr, E, this->vert2edge.ptr, this->vert2edge_offset.ptr,
            this->vert2edge_cnt.ptr);
    CUDA_CHECK(cudaGetLastError());
}
 ...
 */
static __global__ void set_boundary_vertex_kernel(const uint64_t* edges,
...
void CuMesh::get_boundary_info() {
    ...
    arange_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            cu_edge_idx, E);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cub::DeviceSelect::If(nullptr, temp_storage_bytes, cu_edge_idx,
                                     this->boundaries.ptr, cu_num_boundary, E,
                                     is_boundary_edge{this->edge2face_cnt.ptr},
                                     stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSelect::If(
            this->cub_temp_storage.ptr, temp_storage_bytes, cu_edge_idx,
            this->boundaries.ptr, cu_num_boundary, E,
            is_boundary_edge{this->edge2face_cnt.ptr}, stream));
    CUDA_CHECK(cudaMemcpyAsync(&this->boundaries.size, cu_num_boundary,
    ...
    if (this->boundaries.size > 0) {
        set_boundary_vertex_kernel<<<(this->boundaries.size + BLOCK_SIZE - 1) /
                                             BLOCK_SIZE,
                                     BLOCK_SIZE, 0, stream>>>(
...
static __global__ void get_vertex_boundary_cnt_kernel(const uint64_t* edges,
                                                      const int* boundaries,
                                                      const int B,
                                                      int* vert2bound_cnt) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
 ...
 * @param boundaries: the buffer for boundary edges, shape (B)
 * @param B: the number of boundary edges
 * @param vert2bound: the buffer for vertex to boundary adjacency, shape
 * (total_vertex_boundary_cnt)
 * @param vert2bound_offset: the buffer for vertex to boundary adjacency offset,
 * shape (V+1)
 * @param vert2bound_cnt: the buffer for vertex adjacent boundary number, shape
 * (V)
 */
...
void CuMesh::get_vertex_boundary_adjacency() {
    ...
    this->vert2bound_cnt.resize(V + 1);
    this->vert2bound_cnt.zero();
    get_vertex_boundary_cnt_kernel<<<(B + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                     BLOCK_SIZE, 0, stream>>>(
    ...
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, this->vert2bound_cnt.ptr,
            this->vert2bound_offset.ptr, V + 1, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            this->vert2bound_cnt.ptr, this->vert2bound_offset.ptr, V + 1,
            stream));

    // get vertex-boundary adjacency
    this->vert2bound.resize(2 * B);
    this->vert2bound_cnt.zero();
    get_vertex_boundary_adjacency_kernel<<<(B + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                           BLOCK_SIZE, 0, stream>>>(
            this->edges.ptr, this->boundaries.ptr, B, this->vert2bound.ptr,
            this->vert2bound_offset.ptr, this->vert2bound_cnt.ptr);
    CUDA_CHECK(cudaGetLastError());
}
...
void CuMesh::get_vertex_is_manifold() {
    ...
    get_vertex_is_manifold_kernel<<<(V + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                    BLOCK_SIZE, 0, stream>>>(
            this->vert2edge.ptr, this->vert2edge_offset.ptr,
            this->edge2face_cnt.ptr, V, this->vert_is_manifold.ptr);
    CUDA_CHECK(cudaGetLastError());
}
...
void CuMesh::get_manifold_face_adjacency() {
    ...
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cub::DeviceSelect::If(
            nullptr, temp_storage_bytes, cu_edge_idx, cu_manifold_edge_idx,
            cu_num_manifold_edges, E, is_manifold_edge{this->edge2face_cnt.ptr},
            stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSelect::If(
            this->cub_temp_storage.ptr, temp_storage_bytes, cu_edge_idx,
            cu_manifold_edge_idx, cu_num_manifold_edges, E,
            is_manifold_edge{this->edge2face_cnt.ptr}, stream));
    int manifold_edge_count;
    CUDA_CHECK(cudaMemcpyAsync(&manifold_edge_count, cu_num_manifold_edges,
                               sizeof(int), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    set_manifold_face_adj_kernel<<<(manifold_edge_count + BLOCK_SIZE - 1) /
                                           BLOCK_SIZE,
                                   BLOCK_SIZE, 0, stream>>>(
            cu_manifold_edge_idx, this->edge2face.ptr,
            this->edge2face_offset.ptr, manifold_edge_count,
...
void CuMesh::get_manifold_boundary_adjacency() {
    ...
    arange_kernel<<<(V + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            cu_vert_idx, V);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cub::DeviceSelect::If(
            nullptr, temp_storage_bytes, cu_vert_idx, cu_manifold_vert_idx,
            cu_num_manifold_boundary_verts, V,
            is_manifold_boundary_vertex{this->vert_is_manifold.ptr,
                                        this->vert_is_boundary.ptr},
            stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSelect::If(
            this->cub_temp_storage.ptr, temp_storage_bytes, cu_vert_idx,
            cu_manifold_vert_idx, cu_num_manifold_boundary_verts, V,
            is_manifold_boundary_vertex{this->vert_is_manifold.ptr,
                                        this->vert_is_boundary.ptr},
            stream));
    int manifold_boundary_vert_count;
    CUDA_CHECK(cudaMemcpyAsync(&manifold_boundary_vert_count,
                               cu_num_manifold_boundary_verts, sizeof(int),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    set_manifold_bound_adj_kernel<<<(manifold_boundary_vert_count + BLOCK_SIZE -
                                     1) / BLOCK_SIZE,
                                    BLOCK_SIZE, 0, stream>>>(
            cu_manifold_vert_idx, this->vert2bound.ptr,
            this->vert2bound_offset.ptr, manifold_boundary_vert_count,
...
void CuMesh::get_connected_components() {
    ...
    do {
        ...
        // Hook
        hook_edges_kernel<<<(M + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                            stream>>>(this->manifold_face_adj.ptr, M,
                                      this->conn_comp_ids.ptr, cu_end_flag);
        CUDA_CHECK(cudaGetLastError());

        // Compress
        compress_components_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                     BLOCK_SIZE, 0, stream>>>(
                this->conn_comp_ids.ptr, F);
        CUDA_CHECK(cudaGetLastError());
    ...
    this->num_conn_comps =
            compress_ids(this->conn_comp_ids.ptr, F, this->cub_temp_storage);
}
...
void CuMesh::get_boundary_connected_components() {
    ...
    arange_kernel<<<(B + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            this->bound_conn_comp_ids.ptr, B);
    CUDA_CHECK(cudaGetLastError());
    int* cu_end_flag;
    int h_end_flag;
    CUDA_CHECK(cudaMalloc(&cu_end_flag, sizeof(int)));
    do {
        ...
        // Hook
        hook_edges_kernel<<<(M + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                            stream>>>(this->manifold_bound_adj.ptr, M,
                                      this->bound_conn_comp_ids.ptr,
                                      cu_end_flag);
        CUDA_CHECK(cudaGetLastError());

        // Compress
        compress_components_kernel<<<(B + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                     BLOCK_SIZE, 0, stream>>>(
                this->bound_conn_comp_ids.ptr, B);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaMemcpyAsync(&h_end_flag, cu_end_flag, sizeof(int),
    ...
    // Compresses boundary components
    this->num_bound_conn_comps = compress_ids(this->bound_conn_comp_ids.ptr, B,
                                              this->cub_temp_storage);
}
...
static __global__ void is_bound_conn_comp_loop_kernel(
    ...
    // check if both vertices are connected to another boundary with the same
...
void CuMesh::get_boundary_loops() {
    ...
    CUDA_CHECK(cudaMalloc(&cu_is_bound_conn_comp_loop,
                          this->num_bound_conn_comps * sizeof(int)));
    fill_kernel<<<(this->num_bound_conn_comps + BLOCK_SIZE - 1) / BLOCK_SIZE,
                  BLOCK_SIZE, 0, stream>>>(cu_is_bound_conn_comp_loop,
                                           this->num_bound_conn_comps, 1);
    CUDA_CHECK(cudaGetLastError());
    is_bound_conn_comp_loop_kernel<<<(B + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                     BLOCK_SIZE, 0, stream>>>(
            this->edges.ptr, this->boundaries.ptr,
            this->bound_conn_comp_ids.ptr, this->vert2bound.ptr,
            this->vert2bound_offset.ptr, B, cu_is_bound_conn_comp_loop);
    CUDA_CHECK(cudaGetLastError());
    int* cu_num_bound_loops;
    CUDA_CHECK(cudaMalloc(&cu_num_bound_loops, sizeof(int)));
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceReduce::Sum(
            nullptr, temp_storage_bytes, cu_is_bound_conn_comp_loop,
            cu_num_bound_loops, this->num_bound_conn_comps, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceReduce::Sum(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            cu_is_bound_conn_comp_loop, cu_num_bound_loops,
            this->num_bound_conn_comps, stream));
    CUDA_CHECK(cudaMemcpyAsync(&this->num_bound_loops, cu_num_bound_loops,
                               sizeof(int), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            nullptr, temp_storage_bytes, this->bound_conn_comp_ids.ptr,
            cu_bound_conn_comp_ids_sorted, this->boundaries.ptr,
            cu_bound_sorted, B, 0, sizeof(int) * 8, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            this->bound_conn_comp_ids.ptr, cu_bound_conn_comp_ids_sorted,
            this->boundaries.ptr, cu_bound_sorted, B, 0, sizeof(int) * 8,
            stream));

    // Select loops
    int* cu_bound_is_on_loop;
    CUDA_CHECK(cudaMalloc(&cu_bound_is_on_loop, B * sizeof(int)));
    index_kernel<<<(B + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            cu_is_bound_conn_comp_loop, cu_bound_conn_comp_ids_sorted, B,
            cu_bound_is_on_loop);
    CUDA_CHECK(cudaGetLastError());
    this->loop_boundaries.resize(B);
    int *cu_loop_bound_conn_comp_ids_sorted, *cu_num_bound_on_loop;
    CUDA_CHECK(
            cudaMalloc(&cu_loop_bound_conn_comp_ids_sorted, B * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&cu_num_bound_on_loop, sizeof(int)));
    temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceSelect::Flagged(
            nullptr, temp_storage_bytes, cu_bound_sorted, cu_bound_is_on_loop,
            this->loop_boundaries.ptr, cu_num_bound_on_loop, B, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSelect::Flagged(
            this->cub_temp_storage.ptr, temp_storage_bytes, cu_bound_sorted,
            cu_bound_is_on_loop, this->loop_boundaries.ptr,
            cu_num_bound_on_loop, B, stream));
    int num_bound_on_loop;
    CUDA_CHECK(cudaMemcpyAsync(&num_bound_on_loop, cu_num_bound_on_loop,
                               sizeof(int), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    CUDA_CHECK(cudaFree(cu_is_bound_conn_comp_loop));
    ...
    temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceSelect::Flagged(
            nullptr, temp_storage_bytes, cu_bound_conn_comp_ids_sorted,
            cu_bound_is_on_loop, cu_loop_bound_conn_comp_ids_sorted,
            cu_num_bound_on_loop, B, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSelect::Flagged(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            cu_bound_conn_comp_ids_sorted, cu_bound_is_on_loop,
            cu_loop_bound_conn_comp_ids_sorted, cu_num_bound_on_loop, B,
            stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    CUDA_CHECK(cudaMalloc(&cu_rle_unique_out,
                          this->num_bound_loops * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&cu_rle_num_runs, sizeof(int)));
    temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            nullptr, temp_storage_bytes, cu_loop_bound_conn_comp_ids_sorted,
            cu_rle_unique_out, this->loop_boundaries_offset.ptr,
            cu_rle_num_runs, num_bound_on_loop, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            cu_loop_bound_conn_comp_ids_sorted, cu_rle_unique_out,
            this->loop_boundaries_offset.ptr, cu_rle_num_runs,
            num_bound_on_loop, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, this->loop_boundaries_offset.ptr,
            this->num_bound_loops + 1, stream));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            this->cub_temp_storage.ptr, temp_storage_bytes,
            this->loop_boundaries_offset.ptr, this->num_bound_loops + 1,
            stream));
}
