...
1. Split the mesh into charts
   ...
   - Use a parallel edge collapse algorithm to merge charts based on normal cone
deviation
...
__device__ inline uint64_t pack_key_value_positive(int key, float value) {
    ...
    return (static_cast<uint64_t>(v) << 32) | static_cast<unsigned int>(key);
}

__device__ inline void unpack_key_value_positive(uint64_t key_value,
                                                 int& key,
                                                 float& value) {
...
//     chart_normal_cones[tid] = make_float4(n.x, n.y, n.z, 0.0f); // half angle
//     = 0
// }

static __global__ void init_chart_adj_kernel(const float3* vertices,
                                             const int3* faces,
                                             const int2* face_adj,
                                             const int* chart_ids,
                                             const size_t M,
    ...
    int max_c = max(c0, c1);
    chart_adj[tid] =
            (static_cast<uint64_t>(min_c) << 32) | static_cast<uint64_t>(max_c);
...
static __global__ void get_chart_edge_cnt_kernel(const uint64_t* chart_adj,
...
static __global__ void compute_chart_adjacency_cost_kernel(
    ...
    float new_perim =
            chart_perims[c0] + chart_perims[c1] - 2 * chart_adj_length[tid];
    cost += perimeter_area_ratio_weight * (new_perim * new_perim / new_area);

    chart_adj_costs[tid] = cost;
}

static __global__ void propagate_cost_kernel(const int* chart2edge,
...
static __global__ void collapse_edges_kernel(uint64_t* chart_adj,
                                             const float* edge_collapse_costs,
    ...
    if (axis_angle < 1e-3f) {
        ...
        new_axis =
                axis0 * cosf(new_axis_angle) +
                (axis1 - axis0 * cos_angle).normalized() * sinf(new_axis_angle);
        new_axis.normalize();
    }
    chart_normal_cones[c0] =
            make_float4(new_axis.x, new_axis.y, new_axis.z, new_half_angle);

    // not end of iteration
    *end_flag = 0;
}

static void get_chart_connectivity(CuMesh& mesh, cudaStream_t stream) {
    ...
    init_chart_adj_kernel<<<(M + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                            stream>>>(mesh.vertices.ptr, mesh.faces.ptr,
                                      mesh.manifold_face_adj.ptr,
                                      mesh.atlas_chart_ids.ptr, M,
                                      mesh.atlas_chart_adj.ptr, cu_raw_lengths);
    ...
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            nullptr, temp_storage_bytes, mesh.atlas_chart_adj.ptr,
            reinterpret_cast<uint64_t*>(mesh.temp_storage.ptr), cu_raw_lengths,
            cu_sorted_lengths, M, 0, sizeof(uint64_t) * 8, stream));
    ...
    CUDA_CHECK(cub::DeviceReduce::ReduceByKey(
            ...
            mesh.atlas_chart_adj.ptr, cu_sorted_lengths,
            mesh.atlas_chart_adj_length.ptr, cu_num_chart_adjs, reduce_op, M,
            stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceReduce::ReduceByKey(
            mesh.cub_temp_storage.ptr, temp_storage_bytes,
            ...
            mesh.atlas_chart_adj_length.ptr, cu_num_chart_adjs, reduce_op, M,
            stream));
    CUDA_CHECK(cudaMemcpyAsync(&mesh.atlas_chart_adj.size, cu_num_chart_adjs,
    ...
    if (mesh.atlas_chart_adj.size > 0) {
        CUDA_CHECK(cudaMemcpyAsync(
                &last_key,
                mesh.atlas_chart_adj.ptr + mesh.atlas_chart_adj.size - 1,
    ...
    get_chart_edge_cnt_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE,
                                0, stream>>>(
            mesh.atlas_chart_adj.ptr, mesh.atlas_chart_adj_length.ptr, E,
            mesh.atlas_chart2edge_cnt.ptr, mesh.atlas_chart_perims.ptr);
    CUDA_CHECK(cudaGetLastError());
    // 2.2 Prepare CSR format for chart-edge connectivity
    mesh.atlas_chart2edge_offset.resize(C + 1);
    temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, mesh.atlas_chart2edge_cnt.ptr,
            mesh.atlas_chart2edge_offset.ptr, C + 1, stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            mesh.cub_temp_storage.ptr, temp_storage_bytes,
            mesh.atlas_chart2edge_cnt.ptr, mesh.atlas_chart2edge_offset.ptr,
            C + 1, stream));
    // 2.3 Fill CSR format for chart-edge connectivity
    mesh.atlas_chart2edge.resize(2 * E);  // each edge connects two charts
    mesh.atlas_chart2edge_cnt.zero();
    get_chart_edge_adjacency_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                      BLOCK_SIZE, 0, stream>>>(
            mesh.atlas_chart_adj.ptr, E, mesh.atlas_chart2edge.ptr,
            mesh.atlas_chart2edge_offset.ptr, mesh.atlas_chart2edge_cnt.ptr);
    CUDA_CHECK(cudaGetLastError());
}
...
static __global__ void normalize_kernel(float3* chart_normals,
                                        const int num_charts) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= num_charts) return;
...
static __global__ void normal_diff_kernel(const float3* chart_normals,
...
void compute_chart_normal_cones(CuMesh& mesh, cudaStream_t stream) {
    size_t C = mesh.atlas_num_charts;
    ...
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            nullptr, temp_storage_bytes, mesh.atlas_chart_ids.ptr,
            sorted_chart_ids, faces_ids, argsorted_faces_ids, F, 0,
            sizeof(int) * 8, stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            mesh.cub_temp_storage.ptr, temp_storage_bytes,
            mesh.atlas_chart_ids.ptr, sorted_chart_ids, faces_ids,
            argsorted_faces_ids, F, 0, sizeof(int) * 8, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            nullptr, temp_storage_bytes, sorted_chart_ids, cu_unique_chart_ids,
            cu_chart_size, cu_num_charts, F, stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            mesh.cub_temp_storage.ptr, temp_storage_bytes, sorted_chart_ids,
            cu_unique_chart_ids, cu_chart_size, cu_num_charts, F, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(nullptr, temp_storage_bytes,
                                             ...
                                             C + 1, stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(mesh.cub_temp_storage.ptr,
                                             temp_storage_bytes, cu_chart_size,
                                             cu_chart_offsets, C + 1, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    index_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            mesh.face_areas.ptr, argsorted_faces_ids, F, cu_sorted_face_areas);
    CUDA_CHECK(cudaGetLastError());
    mesh.atlas_chart_areas.resize(C);
    CUDA_CHECK(cub::DeviceSegmentedReduce::Sum(
            nullptr, temp_storage_bytes, cu_sorted_face_areas,
            mesh.atlas_chart_areas.ptr, C, cu_chart_offsets,
            cu_chart_offsets + 1, stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSegmentedReduce::Sum(
            mesh.cub_temp_storage.ptr, temp_storage_bytes, cu_sorted_face_areas,
            mesh.atlas_chart_areas.ptr, C, cu_chart_offsets,
            cu_chart_offsets + 1, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    index_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            mesh.face_normals.ptr, argsorted_faces_ids, F,
    ...
    CUDA_CHECK(cub::DeviceSegmentedReduce::Reduce(
            nullptr, temp_storage_bytes, cu_sorted_face_normals,
            cu_chart_normals, C, cu_chart_offsets, cu_chart_offsets + 1,
            Float3Add(), make_float3(0.0f, 0.0f, 0.0f), stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSegmentedReduce::Reduce(
            mesh.cub_temp_storage.ptr, temp_storage_bytes,
            cu_sorted_face_normals, cu_chart_normals, C, cu_chart_offsets,
            cu_chart_offsets + 1, Float3Add(), make_float3(0.0f, 0.0f, 0.0f),
            stream));
    normalize_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                       stream>>>(cu_chart_normals, C);
    CUDA_CHECK(cudaGetLastError());

    // 4. Compute normal difference
    float* cu_normal_diff;
    CUDA_CHECK(cudaMalloc(&cu_normal_diff, F * sizeof(float)));
    normal_diff_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                         stream>>>(cu_chart_normals, cu_sorted_face_normals,
                                   sorted_chart_ids, F, cu_normal_diff);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    CUDA_CHECK(cub::DeviceSegmentedReduce::Max(
            nullptr, temp_storage_bytes, cu_normal_diff,
            cu_new_cone_half_angles, C, cu_chart_offsets, cu_chart_offsets + 1,
            stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSegmentedReduce::Max(
            mesh.cub_temp_storage.ptr, temp_storage_bytes, cu_normal_diff,
            cu_new_cone_half_angles, C, cu_chart_offsets, cu_chart_offsets + 1,
            stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    update_normal_cones_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE,
                                 0, stream>>>(mesh.atlas_chart_normal_cones.ptr,
                                              cu_chart_normals,
                                              cu_new_cone_half_angles, C);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaStreamSynchronize(stream));
...
static __global__ void refine_charts_kernel(
    ...
    // local register cache for candidate list (triangle has at most 3
    // neighbors, plus self, max 4 candidates)
    int candidates[4];
...
#pragma unroll
    for (int i = 0; i < 3; i++) {
        ...
        // logic: if I add the neighbor's Chart, I can eliminate this edge as a
        // boundary cost
...
__global__ void hook_edges_if_same_chart_kernel(const int2* adj,
                                                const int* chart_ids,
                                                const int M,
                                                int* conn_comp_ids,
                                                int* end_flag) {
...
static void reassign_chart_ids(CuMesh& mesh, cudaStream_t stream) {
    size_t F = mesh.faces.size;
    size_t M = mesh.manifold_face_adj.size;

    mesh.temp_storage.resize(F * sizeof(int));  // Use as parent for DSU
    arange_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            reinterpret_cast<int*>(mesh.temp_storage.ptr), F);
    CUDA_CHECK(cudaGetLastError());
    ...
    do {
        ...
        // Hook
        hook_edges_if_same_chart_kernel<<<(M + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                          BLOCK_SIZE, 0, stream>>>(
                mesh.manifold_face_adj.ptr, mesh.atlas_chart_ids.ptr, M,
                reinterpret_cast<int*>(mesh.temp_storage.ptr), cu_end_flag);
        CUDA_CHECK(cudaGetLastError());
    ...
    mesh.atlas_num_charts =
            compress_ids(mesh.atlas_chart_ids.ptr, F, mesh.cub_temp_storage);
}
...
static __global__ void unpack_faces_kernel(const uint64_t* pack,
                                           const size_t F,
                                           int3* faces) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= F) return;
...
static __global__ void unpack_vertex_ids_kernel(const uint64_t* pack,
                                                const size_t N,
                                                int* vertex_ids,
                                                int* vertex_offsets) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
void construct_chart_mesh(CuMesh& mesh, cudaStream_t stream) {
    size_t F = mesh.faces.size;
    ...
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            nullptr, temp_storage_bytes, mesh.atlas_chart_ids.ptr,
            cu_sorted_chart_ids, cu_face_idx, cu_sorted_face_idx, F, 0,
            sizeof(int) * 8, stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            mesh.cub_temp_storage.ptr, temp_storage_bytes,
            mesh.atlas_chart_ids.ptr, cu_sorted_chart_ids, cu_face_idx,
            cu_sorted_face_idx, F, 0, sizeof(int) * 8, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    int* cu_num_chart;
    int* cu_unique_chart_ids;
    CUDA_CHECK(cudaMalloc(&cu_chart_size,
                          (mesh.atlas_num_charts + 1) * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&cu_num_chart, sizeof(int)));
    CUDA_CHECK(cudaMalloc(&cu_unique_chart_ids,
                          mesh.atlas_num_charts * sizeof(int)));
    temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            nullptr, temp_storage_bytes, cu_sorted_chart_ids,
            cu_unique_chart_ids, cu_chart_size, cu_num_chart, F, stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
            mesh.cub_temp_storage.ptr, temp_storage_bytes, cu_sorted_chart_ids,
            cu_unique_chart_ids, cu_chart_size, cu_num_chart, F, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    CUDA_CHECK(cudaFree(cu_unique_chart_ids));
    ...
    temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, cu_chart_size,
            mesh.atlas_chart_faces_offset.ptr, mesh.atlas_num_charts + 1,
            stream));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            mesh.cub_temp_storage.ptr, temp_storage_bytes, cu_chart_size,
            mesh.atlas_chart_faces_offset.ptr, mesh.atlas_num_charts + 1,
            stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    CUDA_CHECK(cudaFree(cu_chart_size));
    ...
    expand_chart_ids_and_vertex_ids_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                             BLOCK_SIZE, 0, stream>>>(
            cu_sorted_chart_ids, cu_sorted_face_idx, mesh.faces.ptr, F,
            cu_pack);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    int new_num_vertices = compress_ids(cu_pack, 3 * F, mesh.cub_temp_storage,
                                        cu_inverse_pack);
    mesh.atlas_chart_vertex_map.resize(new_num_vertices);
    mesh.atlas_chart_vertex_offset.resize(mesh.atlas_num_charts + 1);
    unpack_vertex_ids_kernel<<<(new_num_vertices + BLOCK_SIZE - 1) / BLOCK_SIZE,
                               BLOCK_SIZE, 0, stream>>>(
            cu_inverse_pack, new_num_vertices, mesh.atlas_chart_vertex_map.ptr,
            mesh.atlas_chart_vertex_offset.ptr);
    CUDA_CHECK(cudaGetLastError());
    unpack_faces_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                          stream>>>(cu_pack, F, mesh.atlas_chart_faces.ptr);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaStreamSynchronize(stream));
...
void CuMesh::compute_charts(float threshold_cone_half_angle_rad,
    ...
    for (int i = 0; i < global_iterations; i++) {
        while (true) {
            ...
            compute_chart_adjacency_cost_kernel<<<
                    (E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
                    this->atlas_chart_adj.ptr,
                    this->atlas_chart_normal_cones.ptr,
                    this->atlas_chart_adj_length.ptr,
                    this->atlas_chart_perims.ptr, this->atlas_chart_areas.ptr,
                    area_penalty_weight, perimeter_area_ratio_weight, E,
            ...
            propagate_cost_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                    BLOCK_SIZE, 0, stream>>>(
                    this->atlas_chart2edge.ptr,
                    this->atlas_chart2edge_offset.ptr,
                    this->edge_collapse_costs.ptr, C,
                    this->propagated_costs.ptr);
            CUDA_CHECK(cudaGetLastError());

            // 5. Collapse edges
            this->vertices_map.resize(C);  // store collapse map
            arange_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                            stream>>>(this->vertices_map.ptr, C);
            collapse_edges_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                    BLOCK_SIZE, 0, stream>>>(
                    this->atlas_chart_adj.ptr, this->edge_collapse_costs.ptr,
                    this->propagated_costs.ptr, threshold_cone_half_angle_rad,
                    E, this->vertices_map.ptr,
                    this->atlas_chart_normal_cones.ptr, cu_end_flag);
            CUDA_CHECK(cudaGetLastError());
            ...
            this->atlas_num_charts = compress_ids(this->vertices_map.ptr, C,
                                                  this->cub_temp_storage);
            this->temp_storage.resize(F * sizeof(int));
            index_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                           stream>>>(
                    this->vertices_map.ptr, this->atlas_chart_ids.ptr, F,
        ...
        for (int j = 0; j < refine_iterations; j++) {
            compute_chart_normal_cones(*this, stream);
            this->temp_storage.resize(F * sizeof(int));
            refine_charts_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                   BLOCK_SIZE, 0, stream>>>(
                    this->atlas_chart_normal_cones.ptr, this->face_normals.ptr,
                    this->vertices.ptr, this->edges.ptr, this->face2edge.ptr,
                    this->edge2face.ptr, this->edge2face_offset.ptr, F,
                    smooth_strength, this->atlas_chart_ids.ptr,
                    reinterpret_cast<int*>(this->temp_storage.ptr));
            CUDA_CHECK(cudaGetLastError());
            swap_buffers(this->atlas_chart_ids, this->temp_storage);
            this->atlas_num_charts = compress_ids(this->atlas_chart_ids.ptr, F,
                                                  this->cub_temp_storage);
        }

        // After refinement, the chart may become disconnected, so we need to
        // re-assign chart ids
...
std::tuple<int,
           ...
           torch::Tensor,
           torch::Tensor>
CuMesh::read_atlas_charts() {
    cudaStream_t stream = current_stream();

    auto chart_ids =
            torch::empty({static_cast<int64_t>(this->faces.size)},
                         torch::dtype(torch::kInt32).device(torch::kCUDA));
    CUDA_CHECK(cudaMemcpyAsync(
            chart_ids.data_ptr<int>(), this->atlas_chart_ids.ptr,
            this->faces.size * sizeof(int), cudaMemcpyDeviceToDevice, stream));
    auto vertex_map = torch::empty(
            {static_cast<int64_t>(this->atlas_chart_vertex_map.size)},
            torch::dtype(torch::kInt32).device(torch::kCUDA));
    CUDA_CHECK(cudaMemcpyAsync(vertex_map.data_ptr<int>(),
                               this->atlas_chart_vertex_map.ptr,
                               this->atlas_chart_vertex_map.size * sizeof(int),
                               cudaMemcpyDeviceToDevice, stream));
    auto chart_faces = torch::empty(
            {static_cast<int64_t>(this->atlas_chart_faces.size), 3},
            torch::dtype(torch::kInt32).device(torch::kCUDA));
    CUDA_CHECK(cudaMemcpyAsync(chart_faces.data_ptr<int>(),
                               this->atlas_chart_faces.ptr,
                               this->atlas_chart_faces.size * 3 * sizeof(int),
                               cudaMemcpyDeviceToDevice, stream));
    auto chart_vertex_offset = torch::empty(
            {static_cast<int64_t>(this->atlas_chart_vertex_offset.size)},
            torch::dtype(torch::kInt32).device(torch::kCUDA));
    CUDA_CHECK(
            cudaMemcpyAsync(chart_vertex_offset.data_ptr<int>(),
                            this->atlas_chart_vertex_offset.ptr,
                            this->atlas_chart_vertex_offset.size * sizeof(int),
                            cudaMemcpyDeviceToDevice, stream));
    auto chart_face_offset = torch::empty(
            {static_cast<int64_t>(this->atlas_chart_faces_offset.size)},
            torch::dtype(torch::kInt32).device(torch::kCUDA));
    CUDA_CHECK(
            cudaMemcpyAsync(chart_face_offset.data_ptr<int>(),
                            this->atlas_chart_faces_offset.ptr,
                            this->atlas_chart_faces_offset.size * sizeof(int),
                            cudaMemcpyDeviceToDevice, stream));
    return std::make_tuple(this->atlas_num_charts, chart_ids, vertex_map,
                           chart_faces, chart_vertex_offset, chart_face_offset);
}
