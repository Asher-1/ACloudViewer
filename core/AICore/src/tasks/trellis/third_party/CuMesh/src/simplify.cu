 ...
 * @param vert2face: the buffer for neighbor face ids, shape
 ...
 */
static __global__ void get_qem_kernel(const float3* vertices,
                                      const int3* faces,
                                      const int* vert2face,
 ...
 * Get the QEM for each vertex
 */
void get_qem(CuMesh& ctx, cudaStream_t stream) {
    size_t V = ctx.vertices.size;
    size_t F = ctx.faces.size;
    ctx.temp_storage.resize(V * sizeof(QEM));
    get_qem_kernel<<<(V + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                     stream>>>(ctx.vertices.ptr, ctx.faces.ptr,
                               ctx.vert2face.ptr, ctx.vert2face_offset.ptr, V,
                               F, reinterpret_cast<QEM*>(ctx.temp_storage.ptr));
...
inline __device__ bool process_incident_tri(
    ...
    // If this triangle contains the other vertex (the edge), it will be
    // removed, skip it
    if (f_vids.x == collapse_other_vert || f_vids.y == collapse_other_vert ||
        f_vids.z == collapse_other_vert)
...
static __global__ void get_edge_collapse_cost_kernel(
    ...
    for (int f = vert2face_offset[e0]; f < vert2face_offset[e0 + 1]; f++) {
        ...
        if (!process_incident_tri(tri_idx, e0, e1, vertices, faces, v,
                                  skinny_cost, num_tri)) {
    ...
    for (int f = vert2face_offset[e1]; f < vert2face_offset[e1 + 1]; f++) {
        int tri_idx = vert2face[f];
        if (!process_incident_tri(tri_idx, e1, e0, vertices, faces, v,
                                  skinny_cost, num_tri)) {
            edge_collapse_costs[tid] = INFINITY;
            return;
        }
 ...
 * Get the cost for each edge collapse
 */
void get_edge_collapse_cost(CuMesh& ctx,
    ...
    get_edge_collapse_cost_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE,
                                    BLOCK_SIZE, 0, stream>>>(
            ctx.vertices.ptr, ctx.faces.ptr, ctx.vert2face.ptr,
            ctx.vert2face_offset.ptr, ctx.edges.ptr, ctx.vert_is_boundary.ptr,
            reinterpret_cast<const QEM*>(ctx.temp_storage.ptr), V, F, E,
            lambda_edge_length, lambda_skinny, ctx.edge_collapse_costs.ptr);
    CUDA_CHECK(cudaGetLastError());
}
 ...
 * @param F: the number of faces
 * @param E: the number of edges
 * @param propagated_costs: the buffer for edge collapse costs propagated, shape
 * (F)
 */
static __global__ void propagate_cost_kernel(const uint64_t* edges,
                                             const int* vert2face,
                                             const int* vert2face_offset,
 ...
 * Propagate cost to neighboring faces
 */
void propagate_cost(CuMesh& ctx, cudaStream_t stream) {
    size_t V = ctx.vertices.size;
    size_t F = ctx.faces.size;
    size_t E = ctx.edges.size;
    ctx.propagated_costs.resize(F);
    ctx.propagated_costs.fill(std::numeric_limits<uint64_t>::max());
    propagate_cost_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                            stream>>>(
            ctx.edges.ptr, ctx.vert2face.ptr, ctx.vert2face_offset.ptr,
            ctx.edge_collapse_costs.ptr, V, F, E, ctx.propagated_costs.ptr);
    CUDA_CHECK(cudaGetLastError());
}
 ...
 */
static __global__ void collapse_edges_kernel(float3* vertices,
                                             int3* faces,
                                             uint64_t* edges,
                                             const int* vert2face,
                                             const int* vert2face_offset,
...
static __global__ void compress_faces_kernel(const int* faces_map,
 ...
 * Collapse edges parallelly
 */
void collapse_edges(CuMesh& ctx, float collapse_thresh, cudaStream_t stream) {
    size_t V = ctx.vertices.size;
    size_t F = ctx.faces.size;
    size_t E = ctx.edges.size;
    ...
    collapse_edges_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                            stream>>>(
            ctx.vertices.ptr, ctx.faces.ptr, ctx.edges.ptr, ctx.vert2face.ptr,
            ctx.vert2face_offset.ptr, ctx.edge_collapse_costs.ptr,
            ctx.propagated_costs.ptr, ctx.vert_is_boundary.ptr, V, F, E,
            collapse_thresh, ctx.vertices_map.ptr, ctx.faces_map.ptr);
    CUDA_CHECK(cudaGetLastError());
    ...
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            nullptr, temp_storage_bytes, ctx.vertices_map.ptr, V + 1, stream));
    ctx.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            ctx.cub_temp_storage.ptr, temp_storage_bytes, ctx.vertices_map.ptr,
            V + 1, stream));
    int new_num_vertices;
    CUDA_CHECK(cudaMemcpyAsync(&new_num_vertices, ctx.vertices_map.ptr + V,
    ...
    compress_vertices_kernel<<<(V + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                               stream>>>(
            ctx.vertices_map.ptr, ctx.vertices.ptr, V,
    ...
    ctx.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(ctx.cub_temp_storage.ptr,
                                             temp_storage_bytes,
                                             ctx.faces_map.ptr, F + 1, stream));
    int new_num_faces;
    CUDA_CHECK(cudaMemcpyAsync(&new_num_faces, ctx.faces_map.ptr + F,
                               sizeof(int), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    // compress faces
    ctx.temp_storage.resize(new_num_faces * sizeof(int3));
    compress_faces_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                            stream>>>(
            ctx.faces_map.ptr, ctx.vertices_map.ptr, ctx.faces.ptr, F,
            reinterpret_cast<int3*>(ctx.temp_storage.ptr));
    CUDA_CHECK(cudaGetLastError());
    swap_buffers(ctx.temp_storage, ctx.faces);
}

std::tuple<int, int> CuMesh::simplify_step(float lambda_edge_length,
                                           float lambda_skinny,
                                           float threshold,
                                           bool timing) {
    ...
    if (timing) {
        ...
        std::cout << "get_vertex_face_adjacency: "
    ...
    if (timing) {
        ...
        std::cout << "get_edge_collapse_cost: "
                  << std::chrono::duration_cast<std::chrono::microseconds>(
                             end - start)
                             .count()
                  << " us" << std::endl;
