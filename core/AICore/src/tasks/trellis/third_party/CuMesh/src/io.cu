...
template <>
struct TorchTypeMapping<uint8_t> {
...
template <>
struct TorchTypeMapping<uint64_t> {
    static constexpr auto scalar_type = torch::kInt32;
    static constexpr int sizeof_scalar = 4;
...
template <>
struct TorchTypeMapping<float2> {
    static constexpr auto scalar_type = torch::kFloat32;
    static constexpr int sizeof_scalar = 4;
    static constexpr int channels = 2;
};
...
template <>
struct TorchTypeMapping<float3> {
    static constexpr auto scalar_type = torch::kFloat32;
    static constexpr int sizeof_scalar = 4;
    static constexpr int channels = 3;
};
...
template <>
struct TorchTypeMapping<float4> {
    static constexpr auto scalar_type = torch::kFloat32;
    static constexpr int sizeof_scalar = 4;
...
static torch::Tensor buffer_to_tensor(const Buffer<T> buffer) {
    ...
    if (sizeof(T) == dst_bytes) {
        CUDA_CHECK(cudaMemcpy(tensor.data_ptr(), buffer.ptr, count * sizeof(T),
                              cudaMemcpyDeviceToDevice));
    } else {
        CUDA_CHECK(cudaMemcpy2D(tensor.data_ptr(), dst_bytes, buffer.ptr,
                                sizeof(T), dst_bytes, count,
                                cudaMemcpyDeviceToDevice));
    }
...
void CuMesh::init(const torch::Tensor& vertices, const torch::Tensor& faces) {
    ...
    CUDA_CHECK(cudaMemcpy2D(this->vertices.ptr, sizeof(float3),
                            vertices.data_ptr<float>(), sizeof(float) * 3,
                            sizeof(float) * 3, num_vertices,
                            cudaMemcpyDeviceToDevice));
    CUDA_CHECK(cudaMemcpy2D(this->faces.ptr, sizeof(int3),
                            faces.data_ptr<int>(), sizeof(int) * 3,
...
std::tuple<int, torch::Tensor> CuMesh::read_boundary_connected_components() {
    auto bound_conn_comp_ids_tensor =
            buffer_to_tensor(this->bound_conn_comp_ids);
    return std::make_tuple(this->num_bound_conn_comps,
                           bound_conn_comp_ids_tensor);
}
...
std::tuple<int, torch::Tensor, torch::Tensor> CuMesh::read_boundary_loops() {
    ...
    auto loop_boundaries_offset_tensor =
            buffer_to_tensor(this->loop_boundaries_offset);
    return std::make_tuple(this->num_bound_loops, loop_boundaries_tensor,
                           loop_boundaries_offset_tensor);
}
...
std::unordered_map<std::string, torch::Tensor> CuMesh::read_all_cache() {
    ...
    cache["atlas_chart_vertex_map"] =
            buffer_to_tensor(this->atlas_chart_vertex_map);
    cache["atlas_chart_faces"] = buffer_to_tensor(this->atlas_chart_faces);
    cache["atlas_chart_faces_offset"] =
            buffer_to_tensor(this->atlas_chart_faces_offset);
    cache["atlas_chart_vertex_offset"] =
            buffer_to_tensor(this->atlas_chart_vertex_offset);
    cache["atlas_chart_uvs"] = buffer_to_tensor(this->atlas_chart_uvs);
    cache["atlas_chart_normal_cones"] =
            buffer_to_tensor(this->atlas_chart_normal_cones);
    cache["atlas_chart_adj"] = buffer_to_tensor(this->atlas_chart_adj);
    cache["atlas_chart_adj_length"] =
            buffer_to_tensor(this->atlas_chart_adj_length);
    ...
    cache["atlas_chart2edge_cnt"] =
            buffer_to_tensor(this->atlas_chart2edge_cnt);
    cache["atlas_chart2edge_offset"] =
            buffer_to_tensor(this->atlas_chart2edge_offset);
