...
template <typename T>
__global__ void scatter_kernel(const int* indices,
                               const T* values,
                               const size_t N,
                               T* output) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename T>
__global__ void index_kernel(const T* values,
                             const int* indices,
                             const size_t N,
                             T* output) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename T>
__global__ void diff_kernel(const T* values, const size_t N, T* output) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= N) return;
...
template <typename T>
__global__ void set_flag_kernel(const int* indices, const size_t N, T* output) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= N) return;
...
__global__ void compare_kernel(const CompT* values,
    ...
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= N) return;
    flag[tid] = op(values[tid], threshold) ? static_cast<FlagT>(1)
                                           : static_cast<FlagT>(0);
}

template <typename Ta, typename Tb>
__global__ void inplace_div_kernel(Ta* a, const Tb* b, const size_t N) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= N) return;
...
template <typename T>
__global__ void get_diff_kernel(const T* ids_sorted, T* ids_diff, const int N) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= N) return;
    if (tid == N - 1) {
        ids_diff[tid] = 1;
...
int compress_ids(T* ids,
    ...
    arange_kernel<<<(N + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            cu_indices, N);
    CUDA_CHECK(cudaGetLastError());
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            nullptr, temp_storage_bytes, ids, cu_ids_sorted, cu_indices,
            cu_indices_argsorted, N, 0, sizeof(T) * 8, stream));
    cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
            cub_temp_storage.ptr, temp_storage_bytes, ids, cu_ids_sorted,
            cu_indices, cu_indices_argsorted, N, 0, sizeof(T) * 8, stream));
    // get diff
    T* cu_new_ids;
    CUDA_CHECK(cudaMalloc(&cu_new_ids, N * sizeof(T)));
    get_diff_kernel<<<(N + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                      stream>>>(cu_ids_sorted, cu_new_ids, N);
    CUDA_CHECK(cudaGetLastError());
    ...
    if (inverse) {
        ...
        CUDA_CHECK(cub::DeviceSelect::Flagged(nullptr, temp_storage_bytes,
                                              cu_ids_sorted, cu_new_ids,
                                              inverse, cu_num, N, stream));
        cub_temp_storage.resize(temp_storage_bytes);
        CUDA_CHECK(cub::DeviceSelect::Flagged(
                cub_temp_storage.ptr, temp_storage_bytes, cu_ids_sorted,
                cu_new_ids, inverse, cu_num, N, stream));
    }
    ...
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
            cub_temp_storage.ptr, temp_storage_bytes, cu_new_ids, N, stream));

    // scatter
    scatter_kernel<<<(N + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0,
                     stream>>>(cu_indices_argsorted, cu_new_ids, N, ids);
    CUDA_CHECK(cudaGetLastError());
    T num_components;
    CUDA_CHECK(cudaMemcpyAsync(&num_components, cu_new_ids + N - 1, sizeof(T),
...
void print_max_val(T* ptr, size_t size) {
    ...
    size_t temp_storage_bytes = 0;
    CUDA_CHECK(cub::DeviceReduce::Max(nullptr, temp_storage_bytes, ptr,
                                      dbg_cu_max_val, size, stream));
    char* temp_storage;
    CUDA_CHECK(cudaMalloc(&temp_storage, temp_storage_bytes));
    CUDA_CHECK(cub::DeviceReduce::Max(temp_storage, temp_storage_bytes, ptr,
                                      dbg_cu_max_val, size, stream));
    T h_max_val;
    CUDA_CHECK(cudaMemcpyAsync(&h_max_val, dbg_cu_max_val, sizeof(T),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
...
void print_val(T* ptr, size_t size) {
    cudaStream_t stream = current_stream();
    T h_ptr[size];
    CUDA_CHECK(cudaMemcpyAsync(h_ptr, ptr, size * sizeof(T),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
