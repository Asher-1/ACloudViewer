...
__device__ __forceinline__ float get_vertex_val(
    ...
    uint32_t idx =
            linear_probing_lookup(hashmap_keys, hashmap_vals, key, N_vert);
    return udf[idx];
}
...
#pragma unroll
        for (int v = 0; v <= 1; ++v) {
            float val1 = get_vertex_val(hashmap_keys, hashmap_vals, udf, N_vert,
                                        vx, vy + u, vz + v, W, H, D);
            float val2 = get_vertex_val(hashmap_keys, hashmap_vals, udf, N_vert,
                                        vx + 1, vy + u, vz + v, W, H, D);
...
#pragma unroll
    ...
    if (intersection_count > 0) {
        out_vertices[thread_id * 3 + 0] =
                intersection_sum.x / intersection_count;
        out_vertices[thread_id * 3 + 1] =
                intersection_sum.y / intersection_count;
        out_vertices[thread_id * 3 + 2] =
                intersection_sum.z / intersection_count;
    } else {
...
/**
 * Isosurfacing a volume defined on vertices of a sparse voxel grid using a
 simple dual contouring algorithm.
 ...
 * @param hashmap_vals  [Nvert] uint32 tensor containing the hashmap values as
 vertex indices
 * @param coords        [Mvox, 3] int32 tensor containing the coordinates of the
 active voxels
 * @param udf           [Mvert] float tensor containing the UDF/SDF values at
 the vertices
 * @param W             the number of width dimensions
 * @param H             the number of height dimensions
 * @param D             the number of depth dimensions
 *
 * @return              [L, 3] float tensor containing the active vertices (Dual
 Vertices) [L, 3] int32 tensor containing the intersected edges (1: intersected,
 0: not intersected)
 */
std::tuple<torch::Tensor, torch::Tensor> cumesh::simple_dual_contour(
    ...
    auto vertices =
            torch::empty({(long)M, 3},
                         torch::dtype(torch::kFloat32).device(coords.device()));
    auto intersected = torch::empty(
            {(long)M, 3}, torch::dtype(torch::kInt32).device(coords.device()));
    ...
    if (hashmap_keys.dtype() == torch::kUInt32) {
        simple_dual_contour_kernel<<<blocks, threads, 0, stream>>>(
                N_vert, M, W, H, D, hashmap_keys.data_ptr<uint32_t>(),
                hashmap_vals.data_ptr<uint32_t>(), coords.data_ptr<int32_t>(),
                udf.data_ptr<float>(), vertices.data_ptr<float>(),
                intersected.data_ptr<int32_t>());
    } else if (hashmap_keys.dtype() == torch::kUInt64) {
        simple_dual_contour_kernel<<<blocks, threads, 0, stream>>>(
                N_vert, M, W, H, D, hashmap_keys.data_ptr<uint64_t>(),
                hashmap_vals.data_ptr<uint32_t>(), coords.data_ptr<int32_t>(),
                udf.data_ptr<float>(), vertices.data_ptr<float>(),
                intersected.data_ptr<int32_t>());
    } else {
