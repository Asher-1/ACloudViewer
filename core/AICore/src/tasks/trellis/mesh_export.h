...
struct MeshExportOptions {
    int   texture_size  = 2048;    // atlas width/height
    ...
    // Was the T2GLB_XATLAS/T2GLB_NOSMOOTH env pair upstream: regularise the
    // geometry with a Taubin smooth before export (opt-in; off by default).
    bool  taubin_smooth = false;
    // Was T2GLB_VERTEX upstream: write the legacy vertex-colour GLB instead of
    // the UV atlas (debug / regression path).
