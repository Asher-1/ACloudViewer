...
struct trellis2_dino_cond {
    ...
    int64_t tokens() const {
        return shape.size() == 3 ? shape[1] : (shape.empty() ? 0 : shape[0]);
    }
...
struct trellis2_ss_flow_hparams {
    ...
    int32_t head_dim() const {
        return num_heads ? model_channels / num_heads : 0;
    }
};
...
TRELLIS2_API trellis2_ss_flow_model* trellis2_ss_flow_load(
...
TRELLIS2_API const char* trellis2_ss_flow_backend_name(
        const trellis2_ss_flow_model* m);

TRELLIS2_API const trellis2_ss_flow_hparams& trellis2_ss_flow_hparams_of(
        const trellis2_ss_flow_model* m);

TRELLIS2_API int trellis2_ss_flow_n_tensors(const trellis2_ss_flow_model* m);
TRELLIS2_API bool trellis2_ss_flow_get_tensor_info(
        const trellis2_ss_flow_model* m, int i, trellis2_tensor_info& out);
...
struct trellis2_ss_sampler_params {
    ...
    bool verbose = false;  // per-step progress log (opt-in)
...
TRELLIS2_API bool trellis2_ss_flow_forward(trellis2_ss_flow_model* m,
                                           const float* x,
                                           float t,
                                           const float* cond,
                                           int cond_tokens,
...
//                (channel-major). If nullptr, standard-normal noise is
//                generated from params->seed.
...
TRELLIS2_API trellis2_ss_dec_model* trellis2_ss_dec_load(
        const std::string& path,
        bool load_tensors = true,
        std::string* error = nullptr,
        const char* device = nullptr);
...
TRELLIS2_API const char* trellis2_ss_dec_backend_name(
        const trellis2_ss_dec_model* m);

TRELLIS2_API const trellis2_ss_dec_hparams& trellis2_ss_dec_hparams_of(
        const trellis2_ss_dec_model* m);

TRELLIS2_API int trellis2_ss_dec_n_tensors(const trellis2_ss_dec_model* m);
TRELLIS2_API bool trellis2_ss_dec_get_tensor_info(
        const trellis2_ss_dec_model* m, int i, trellis2_tensor_info& out);
...
//            checkpoint out_channels=1 and res_out=64, so this is a 64^3 grid
//            of occupancy logits; the voxel scaffold is `out[n] > 0`.
//
// Returns false on failure and (if `error`) fills it with a reason.
TRELLIS2_API bool trellis2_ss_dec_decode(trellis2_ss_dec_model* m,
                                         const float* latent,
                                         float* out,
                                         std::string* error = nullptr);
...
struct trellis2_slat_flow_hparams {
    ...
    // Texture SLAT flow only (concat_cond). 0 on the shape flow. When > 0 the
    // DiT is conditioned on the shape via concatenation: in_channels =
    // out_channels + concat_cond_channels (64 = 32 tex-noise + 32 shape-SLAT).
    // The shape SLAT is normalized by concat_norm_* before being concatenated
    // onto the noise.
...
TRELLIS2_API trellis2_slat_flow_model* trellis2_slat_flow_load(
        const std::string& path,
        bool load_tensors = true,
        std::string* error = nullptr,
        const char* device = nullptr);
...
TRELLIS2_API const char* trellis2_slat_flow_backend_name(
        const trellis2_slat_flow_model* m);
TRELLIS2_API const trellis2_slat_flow_hparams& trellis2_slat_flow_hparams_of(
        const trellis2_slat_flow_model* m);
...
TRELLIS2_API bool trellis2_slat_flow_forward(trellis2_slat_flow_model* m,
                                             const float* x,
                                             int n_voxels,
...
TRELLIS2_API bool trellis2_slat_flow_sample(
        trellis2_slat_flow_model* m,
        int n_voxels,
        const int32_t* coords,
        const float* cond,
        int cond_tokens,
...
// Texture-SLAT flow sampling with concat_cond (requires
// concat_cond_channels>0). Diffuses out_channels (32) and, each step,
// concatenates the shape SLAT (normalized by concat_norm_*) onto the noise so
// the DiT sees in_channels (64).
...
TRELLIS2_API bool trellis2_slat_flow_sample_tex(
        trellis2_slat_flow_model* m,
        int n_voxels,
        const int32_t* coords,
        const float* cond,
...
TRELLIS2_API trellis2_shape_dec_model* trellis2_shape_dec_load(
        const std::string& path,
        bool load_tensors = true,
        std::string* error = nullptr,
        const char* device = nullptr);
...
TRELLIS2_API const char* trellis2_shape_dec_backend_name(
        const trellis2_shape_dec_model* m);
TRELLIS2_API const trellis2_shape_dec_hparams& trellis2_shape_dec_hparams_of(
        const trellis2_shape_dec_model* m);
...
TRELLIS2_API bool trellis2_shape_dec_decode(
...
TRELLIS2_API bool trellis2_shape_dec_decode_with_subs(
        trellis2_shape_dec_model* m,
        const float* slat,
        int n_voxels,
        const int32_t* coords,
...
// without running the output layer — mirrors
// FlexiDualGridVaeDecoder.upsample(), used by the 1024 cascade to turn the LR
// (32^3) scaffold into candidate coords.
//   upsample_times : in [1, n_levels-1] (4 for the cascade: 32^3 -> 512^3)
//   out_coords     : filled with [L_out * 3] int32 at input_res *
//   2^upsample_times
TRELLIS2_API bool trellis2_shape_dec_upsample(trellis2_shape_dec_model* m,
                                              const float* slat,
                                              int n_voxels,
                                              const int32_t* coords,
                                              int upsample_times,
...
TRELLIS2_API trellis2_shape_enc_model* trellis2_shape_enc_load(
        const std::string& path,
        bool load_tensors = true,
        std::string* error = nullptr,
        const char* device = nullptr);
...
TRELLIS2_API const char* trellis2_shape_enc_backend_name(
        const trellis2_shape_enc_model* m);
TRELLIS2_API const trellis2_shape_enc_hparams& trellis2_shape_enc_hparams_of(
        const trellis2_shape_enc_model* m);
...
TRELLIS2_API bool trellis2_shape_enc_encode(
        trellis2_shape_enc_model* m,
        const float* in6,
        int n_voxels,
        const int32_t* coords,
...
/*****************************************************************************
** Public API – Texture-SLAT VAE decoder (SparseUnetVaeDecoder,
*pred_subdiv=False)
**
...
TRELLIS2_API trellis2_shape_dec_model* trellis2_tex_dec_load(
        const std::string& path,
        bool load_tensors = true,
        std::string* error = nullptr,
        const char* device = nullptr);
...
//   subs       : shape decoder or encoder per-level subdivisions (decoder
//   order). out_feats  : [M * 6] PBR, voxel-major, already in [0,1]. out_coords
//   : [M * 3] at R; a permutation-free reproduction of the encoder's
//                input voxel order (so PBR voxel v == mesh vertex v).
TRELLIS2_API bool trellis2_tex_dec_decode(
        trellis2_shape_dec_model* m,
        const float* slat,
        int n_voxels,
        const int32_t* coords,
...
TRELLIS2_API trellis2_dino_model* trellis2_dino_load(
        const std::string& path,
        bool load_tensors = true,
        std::string* error = nullptr,
        const char* device = nullptr);
...
TRELLIS2_API const char* trellis2_dino_backend_name(
        const trellis2_dino_model* m);

TRELLIS2_API const trellis2_dino_hparams& trellis2_dino_hparams_of(
        const trellis2_dino_model* m);
...
//
// Returns false on failure and (if `error`) fills it with a reason.
TRELLIS2_API bool trellis2_dino_encode(trellis2_dino_model* m,
...
TRELLIS2_API bool trellis2_dino_encode_rgb(trellis2_dino_model* m,
                                           const uint8_t* rgb,
                                           int image_size,
                                           trellis2_dino_cond& out,
                                           std::string* error = nullptr);
...
TRELLIS2_API int trellis2_remove_solid_background_rgba(
        uint8_t* rgba, int w, int h, int mode = TRELLIS2_BACKGROUND_AUTO);

/*****************************************************************************
** Public API – RMBG-2.0 AI background removal (optional)
**
** When built with TRELLIS2_HAVE_RMBG, these functions expose the
** BiRefNet/RMBG-2.0 model for high-quality background removal from arbitrary
** images (not limited to solid-color backgrounds).
*****************************************************************************/

#ifdef TRELLIS2_HAVE_RMBG

// In-tree integration: the RMBG-2.0 engine is the AICore rmbg task
// (aicore_rmbg_*), linked once inside libAICore.so — no second copy of the
// model runtime is vendored.

struct trellis2_rmbg_model;

// Load an RMBG-2.0 GGUF model. device: "auto", "cpu", "cuda", "vulkan".
TRELLIS2_API trellis2_rmbg_model* trellis2_rmbg_load(
        const char* gguf_path,
        const char* device = "auto",
        std::string* error = nullptr);

TRELLIS2_API void trellis2_rmbg_free(trellis2_rmbg_model* m);

// Remove background from raw RGBA pixels. On success, writes an RGBA byte
// buffer (w*h*4, background removed) into (out_rgba, out_len) — caller must
// free with trellis2_rmbg_free_buffer. Returns 0 on success.
TRELLIS2_API int trellis2_rmbg_remove_background(trellis2_rmbg_model* m,
                                                 const uint8_t* rgba,
                                                 int w,
                                                 int h,
                                                 uint8_t** out_rgba,
...
TRELLIS2_API void trellis2_rmbg_free_buffer(uint8_t* buf);

TRELLIS2_API bool trellis2_rmbg_available();

#endif  // TRELLIS2_HAVE_RMBG

/*****************************************************************************
** Public API – engine configuration (replaces the upstream environment
** variables; AICore reads no environment variables for logic control)
*****************************************************************************/

// Set the CPU thread count used by subsequently loaded models (0 = hardware
// concurrency). Thread-local: affects the calling thread only. Was
// TRELLIS2_N_THREADS upstream.
TRELLIS2_API void trellis2_set_threads(int n_threads);
...
TRELLIS2_API bool trellis2_preprocess_rgba(const uint8_t* rgba,
                                           int w,
                                           int h,
                                           int out_size,
