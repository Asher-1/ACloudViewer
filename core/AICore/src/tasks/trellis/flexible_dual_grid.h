...
inline Mesh extract(const float* feats,
    ...
    for (int v = 0; v < n; ++v) {
        ...
        for (int a = 0; a < 3; ++a) {
            ...
            V[(size_t)v * 3 + a] =
                    ((float)coords[(size_t)v * 3 + a] + off) * vs + aabb0;
        }
        idx[key(coords[(size_t)v * 3], coords[(size_t)v * 3 + 1],
    ...
    for (int v = 0; v < n; ++v) {
        ...
        // Dual-vertex offset of the current voxel; used to synthesise the
        // missing neighbours' dual vertices below (offsets vary smoothly
        // between adjacent voxels, so this is a good local approximation).
        const float off_cur[3] = {
                (1.0f + 2.0f * margin) / (1.0f + std::exp(-f[0])) - margin,
                (1.0f + 2.0f * margin) / (1.0f + std::exp(-f[1])) - margin,
                (1.0f + 2.0f * margin) / (1.0f + std::exp(-f[2])) - margin};
        for (int axis = 0; axis < 3; ++axis) {
            if (f[3 + axis] <= 0.0f) continue;  // not intersected on this axis
            int q[4];
            int missing[4], nMiss = 0;
            for (int i = 0; i < 4; ++i) {
            ...
            // Skipping the whole quad here is what leaves the jagged cracks
            // that read as "many holes" on the rendered surface. When at most
            // two of the four neighbours are absent, synthesise their dual
            // vertices (current voxel's offset projected onto the missing
            // neighbour's cell) and still emit the quad — the surface stays
            // continuous and the tiny seam between the synthetic and real
            // vertices is sub-pixel. Only fully open edges (>= 3 missing) are
            // left for fill_holes.
            if (nMiss >= 3) continue;
            for (int mi = 0; mi < nMiss; ++mi) {
                const int i = missing[mi];
                const int32_t nx = cx + EDGE_OFF[axis][i][0];
                const int32_t ny = cy + EDGE_OFF[axis][i][1];
                const int32_t nz = cz + EDGE_OFF[axis][i][2];
                const float pos[3] = {((float)nx + off_cur[0]) * vs + aabb0,
                                      ((float)ny + off_cur[1]) * vs + aabb0,
                                      ((float)nz + off_cur[2]) * vs + aabb0};
                q[i] = (int)(m.verts.size() / 3);
                m.verts.push_back(pos[0]);
                m.verts.push_back(pos[1]);
                m.verts.push_back(pos[2]);
            }

            // Choose the quad diagonal by the decoder's learned split_weight
            // (softplus of feat[6]), exactly as the reference eval path does
            // (FlexiDualGridVaeDecoder -> flexible_dual_grid_to_mesh,
            // train=False):
...
// which way the surface crosses the edge, so a large fraction of faces are
// wound opposite to their neighbours. Two failure modes follow: (a) a naive
// area-weighted normal cancels at those seams, and (b) any *sign* fix that
// leaves stray flips is not harmless — the normal is interpolated across the
// triangle *before* the fragment shader's abs(dot), so two adjacent vertices
// with opposite normals make the interpolated normal cross zero mid-face →
// normalize() explodes → speckled/blocky specular. The mesh is too non-manifold
// to 2-colour the winding cleanly (edges shared by >2 faces frustrate it), so
// we don't try to.
//
//   1. Recover a smooth, winding-INDEPENDENT normal *direction* per vertex as
//   the
...
//      and the interpolated normal stays clear of zero. Only genuinely
//      frustrated (odd-cycle / non-manifold) edges are left flipped.
// Final shading is orientation-independent (viewer uses abs(dot)), so only
// local smoothness matters, not a globally correct outward sign. (Winding
// unification and sign diffusion were both tried and are worse on this mesh:
// 2-colouring the >2-face non-manifold edges frustrates more, and Jacobi
// diffusion checkerboards.)
inline std::vector<float> vertex_normals(const Mesh& m) {
    const size_t nv = m.n_verts();
    std::vector<double> A((size_t)nv * 6,
                          0.0);  // sym structure tensor per vertex
    std::vector<float> seed((size_t)nv * 3,
                            0.0f);  // signed area sum: a sign hint
    for (size_t t = 0; t < m.tris.size(); t += 3) {
        ...
        const double area =
                std::sqrt((double)fn[0] * fn[0] + (double)fn[1] * fn[1] +
                          (double)fn[2] * fn[2]);
        if (area <= 1e-20) continue;
        const double inv = 1.0 / area;
        const double xx = fn[0] * fn[0] * inv, yy = fn[1] * fn[1] * inv,
                     zz = fn[2] * fn[2] * inv;
        const double xy = fn[0] * fn[1] * inv, xz = fn[0] * fn[2] * inv,
                     yz = fn[1] * fn[2] * inv;
        for (int i : {i0, i1, i2}) {
    ...
    //     union-find over mesh edges: two edge-adjacent vertices whose
    //     directions are anti-aligned must end up with opposite signs (and vice
    //     versa), so within each connected component neighbours share a
    //     hemisphere. Only genuinely frustrated (odd-cycle / non-manifold)
    //     edges are left flipped.
    std::vector<int> ufp(nv), ufr(nv, 0);
    std::vector<uint8_t> ufb(nv,
                             0);  // parity of a vertex relative to its parent
    ...
    auto join = [&](int a, int b) {
        ...
        const int rel =
                (na[0] * nb[0] + na[1] * nb[1] + na[2] * nb[2]) < 0.0f ? 1 : 0;
    ...
    //     result is deterministic and roughly outward), then emit oriented
    //     normals
    ...
    for (size_t v = 0; v < nv; ++v) {
        ...
        comp_sign[r] += s * (dir[v * 3] * sv[0] + dir[v * 3 + 1] * sv[1] +
                             dir[v * 3 + 2] * sv[2]);
    }
...
// Fill the holes extract() leaves where a dual-grid quad was skipped (a
...
//
// max_loop is intentionally generous (1024): TRELLIS.2 outputs a single
// closed object, so every boundary loop — however long — is a hole to fill,
// not a designed opening. A fan from loop[0] may cut across a concave loop,
// but the added triangle is inside the loop's bounding region and keeps the
// surface watertight, which matters more than perfect concavity here.
inline void fill_holes(Mesh& m, int max_loop = 1024, int max_passes = 8) {
    ...
    for (int pass = 0; pass < max_passes; ++pass) {
        ...
        for (const auto& kv : adj) {
            ...
            for (const int nb0 : kv.second) {
                ...
                // Backtracking DFS instead of a greedy one-way walk: on the
                // dual grid's non-manifold mesh a hole rim frequently runs
                // through a vertex shared by >2 boundary edges (a junction),
                // where a greedy walk picks the wrong branch and dies in an
                // open chain. DFS retries every branch (degree-2 "rim"
                // neighbours first, so the hole is followed before side
...
// Close the open boundary chains (jagged cracks) that survive fill_holes.
// The dual-grid extract skips whole quads when >= 3 of their 4 neighbour
// voxels are missing, leaving open chains of boundary edges — cracks that
// read as holes on the rendered surface. Each chain component (up to
// `max_chain` boundary edges) is fan-triangulated against its centroid: the
// chain vertices plus a synthetic centroid vertex form a closed patch.
//
// A centroid vertex is appended to the mesh — safe for the caller's
// per-vertex attribute arrays (baked PBR) because those are sampled AFTER
// this pass (the new vertex receives its own material sample). Long chains
// (genuine large openings) are left untouched.
inline void fill_open_chains(Mesh& m, int max_chain = 32, int max_passes = 8) {
    if (m.n_tris() == 0) return;
    auto key = [](int a, int b) {
        const uint32_t lo = a < b ? a : b, hi = a < b ? b : a;
        return ((uint64_t)lo << 32) | hi;
    };
    // Iterate: closing one crack moves the boundary edge (a,b) onto the new
    // edge (b,c), which a later pass closes in turn. Repeat until a pass adds
    // nothing (or the pass cap is hit).
    for (int pass = 0; pass < max_passes; ++pass) {
        const size_t before = m.tris.size();

        // Boundary edges + adjacency (same bookkeeping as fill_holes).
        std::vector<uint64_t> ek;
        ek.reserve(m.tris.size());
        for (size_t t = 0; t < m.tris.size(); t += 3) {
            ek.push_back(key(m.tris[t], m.tris[t + 1]));
        ...
        // Vertex -> incident triangles, used to close single-edge cracks
        // (k == 2) with a third vertex taken from the surface around one
        // endpoint.
        std::unordered_map<int, std::vector<int>> vtri;
        for (size_t t = 0; t < m.tris.size(); t += 3) {
            vtri[m.tris[t]].push_back((int)(t / 3));
            vtri[m.tris[t + 1]].push_back((int)(t / 3));
            vtri[m.tris[t + 2]].push_back((int)(t / 3));
        }

        // Walk each chain from a degree-1 tip (or any unused vertex) along
        // unused boundary edges, then fan it against its centroid.
        std::unordered_set<uint64_t> used;
        std::vector<int> start_vertices;
        for (const auto& kv : adj) start_vertices.push_back(kv.first);
        for (const int s : start_vertices) {
            if (adj.find(s) == adj.end()) continue;  // fully consumed
            // Prefer a degree-1 tip as the walk start so the whole chain is
            // followed in one pass; starting mid-chain would leave the other
            // half stranded. Explore along unused boundary edges until a tip.
            int start = s, eprev = -1, ecur = s;
            for (int guard = 0; guard <= max_chain + 1; ++guard) {
                if ((int)adj[ecur].size() == 1) {
                    start = ecur;
                    break;
                }
                int next = -1;
                auto it = adj.find(ecur);
                if (it != adj.end()) {
                    for (const int c : it->second) {
                        if (c == eprev) continue;
                        if (used.count(key(ecur, c))) continue;
                        next = c;
                        break;
                    }
                }
            ...
            for (int guard = 0; guard <= max_chain + 1; ++guard) {
                ...
                if (it != adj.end()) {
                    for (const int c : it->second) {
                        if (c == prev) continue;
                        if (used.count(key(cur, c))) continue;
                        next = c;
                        break;
                    }
                }
                ...
                prev = cur;
                cur = next;
                if ((int)chain.size() > max_chain) break;
            }
            const int k = (int)chain.size();
            for (int i = 0; i + 1 < k; ++i)
                used.insert(key(chain[i], chain[i + 1]));
            if (k == 2) {
                // Single-edge crack: a centroid fan would be degenerate (the
                // two endpoints and the midpoint are collinear). Close it with
                // one triangle; the third vertex is chosen to be a *common
                // neighbour* of both endpoints when one exists — then the new
                // edges (a,c) and (b,c) are already paired by existing faces
                // and the closure introduces no new boundary edge. Without a
                // common neighbour the patch uses any incident surface vertex
                // of a (the boundary edge moves to (b,c), which later passes
                // keep closing like a zipper).
                const int a = chain[0], b = chain[1];
                int c = -1;
                auto ita = vtri.find(a);
                auto itb = vtri.find(b);
                if (ita != vtri.end() && itb != vtri.end()) {
                    // a's incident vertices (excluding a)
                    std::unordered_set<int> anb;
                    for (const int ti : ita->second) {
                        const int* tri = &m.tris[(size_t)ti * 3];
                        for (int j = 0; j < 3; ++j)
                            if (tri[j] != a) anb.insert(tri[j]);
                    }
                    // first b-incident vertex that is also adjacent to a
                    for (const int ti : itb->second) {
                        const int* tri = &m.tris[(size_t)ti * 3];
                        for (int j = 0; j < 3; ++j) {
                            if (tri[j] != b && anb.count(tri[j])) {
                                c = tri[j];
                                break;
                            }
                        }
                ...
                if (c < 0 && ita != vtri.end() && !ita->second.empty()) {
                    // fallback: any incident surface vertex of a
                    const int ti = ita->second.front();
                    const int* tri = &m.tris[(size_t)ti * 3];
                    c = (tri[0] == a)   ? tri[1]
                        : (tri[1] == a) ? tri[2]
                                        : tri[0];
            ...
            if (k < 3 || k > max_chain + 1) continue;
            // Centroid of the chain vertices (a synthetic vertex — see above).
            float cx = 0, cy = 0, cz = 0;
            for (int i = 0; i < k; ++i) {
                const float* p = &m.verts[(size_t)chain[i] * 3];
                cx += p[0];
                cy += p[1];
                cz += p[2];
            }
            const float inv = 1.0f / (float)k;
            const int cidx = (int)(m.verts.size() / 3);
            m.verts.push_back(cx * inv);
            m.verts.push_back(cy * inv);
            m.verts.push_back(cz * inv);
            for (int i = 0; i < k; ++i) {  // fan: centroid + consecutive pair
                const int a = chain[i], b = chain[(i + 1) % k];
                m.tris.push_back(cidx);
                m.tris.push_back(a);
                m.tris.push_back(b);
            }
        }
