...
const char *trellis2_version(void) { return TRELLIS2_VERSION; }

/*****************************************************************************
** Engine configuration (thread-local; replaces upstream env vars)
*****************************************************************************/

void trellis2_set_threads(int n_threads) {
    tls_n_threads = n_threads > 0 ? n_threads : 0;
}
...
bool trellis2_load_dinodata(const std::string &path,
    ...
    if (!read_u32_le(p, end, version) || !read_u32_le(p, end, dtype) ||
    ...
    for (uint32_t i = 0; i < ndim; ++i) {
        ...
        if (dim == 0 || (int64_t)dim > MAX_ELEMS ||
            total > MAX_ELEMS / (int64_t)dim) {
            set_error(error,
                      "implausible shape (zero or overflowing element count)");
            return false;
        }
        shape[i] = (int64_t)dim;
        total *= (int64_t)dim;
    ...
    if (have_bytes < want_bytes) {
        set_error(error, "payload truncated: have " +
                                 std::to_string(have_bytes) + " bytes, need " +
...
trellis2_dino_fingerprint trellis2_dino_fingerprints(
...
struct trellis2_ss_flow_model {
    ...
    // name -> tensor (into ctx); built once at load for O(1) graph wiring
    // later.
...
ggml_backend_t init_best_backend(std::string &name_out,
    ...
    // ggml defaults to 4 threads; use every core unless configured explicitly.
    int n_threads = tls_n_threads > 0
                            ? tls_n_threads
...
trellis2_ss_flow_model *trellis2_ss_flow_load(const std::string &path,
    ...
    hp.model_channels =
            (int32_t)kv_u32(m->gguf, K("model_channels").c_str(), 0);
    ...
    hp.qk_rms_norm_cross =
            kv_bool(m->gguf, K("qk_rms_norm_cross").c_str(), false) ? 1 : 0;
    ...
    if (load_tensors) {
        ...
        for (int64_t i = 0; i < nt; ++i) {
            ...
            if (!fin.read(reinterpret_cast<char *>(buf.data()),
                          (std::streamsize)nb)) {
                set_error(error, std::string("failed reading weight '") + name +
                                         "' from file");
                trellis2_ss_flow_free(m);
                return nullptr;
            }
...
const trellis2_ss_flow_hparams &trellis2_ss_flow_hparams_of(
        const trellis2_ss_flow_model *m) {
    return m->hp;
}
...
**   h = input_layer(x)                                  # [C, N]
**   t_emb = adaLN(SiLU stack)(timestep_embedding(t))    # [6C] shared
*modulation
...
ggml_tensor *sdpa_auto(ggml_context *ctx,
                       ...
                       float scale) {
    ggml_tensor *qp =
            ggml_cont(ctx, ggml_permute(ctx, q3, 0, 2, 1, 3));  // [hd, Lq, H]
    ggml_tensor *kp =
            ggml_cont(ctx, ggml_permute(ctx, k3, 0, 2, 1, 3));  // [hd, Lk, H]
    ggml_tensor *vp =
...
// Layout matches q reshaped to [head_dim, n_heads, N]: cos/sin are [head_dim,
// 1, N] with cos[n*head_dim + 2p] == cos[n*head_dim + 2p+1] == cos(theta_p(n)).
void rope_tables(int res,
                 int head_dim,
                 float freq_min,
                 float freq_base,
...
bool trellis2_ss_flow_forward(trellis2_ss_flow_model *m,
    ...
    if (!m->has_data) {
        set_error(error,
                  "model loaded metadata-only; reload with load_tensors=true");
        return false;
    }
    ...
    const bool t2_timing = tls_timing;
    ...
    const size_t mem = ggml_tensor_overhead() * 32768 +
    ...
    // ── input leaves ─────────────────────────────────────────────────────────
    ggml_tensor *x_t = ggml_new_tensor_2d(
            ctx, GGML_TYPE_F32, N, hp.in_channels);  // channel-major [N, Cin]
    ...
    ggml_tensor *cnd = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, cond_channels,
                                          Lkv);  // [ctx_ch, Lkv]
    ggml_set_input(x_t);
    ggml_set_name(x_t, "x");
    ggml_set_input(temb);
    ...
    auto modulate = [&](ggml_tensor *h, ggml_tensor *scale,
                        ggml_tensor *shift) {
    ...
    auto rope = [&](ggml_tensor *q3) -> ggml_tensor * {
        ...
        ggml_tensor *q0 =
                ggml_cont(ctx, ggml_view_4d(ctx, q4, 1, hd / 2, H, N, q4->nb[1],
                                            q4->nb[2], q4->nb[3], 0));
        ggml_tensor *q1 =
                ggml_cont(ctx, ggml_view_4d(ctx, q4, 1, hd / 2, H, N, q4->nb[1],
                                            q4->nb[2], q4->nb[3], q4->nb[0]));
        ggml_tensor *swap =
                ggml_concat(ctx, ggml_neg(ctx, q1), q0, 0);  // [2,hd/2,H,N]
        swap = ggml_reshape_3d(ctx, swap, hd, H, N);
        return ggml_add(ctx, ggml_mul(ctx, q3, cos_t),
                        ggml_mul(ctx, swap, sin_t));
    };
    // QK-RMSNorm: F.normalize(x)*gamma*sqrt(hd) == rms_norm(x)*gamma (sqrt
    // cancels).
    ...
    for (int b = 0; b < hp.num_blocks; ++b) {
        ...
        ggml_tensor *mods =
                ggml_add(ctx, W(blk + ".modulation"), tmod);  // [6C]
        auto chunk = [&](int idx) {
            return ggml_view_1d(ctx, mods, C, (size_t)idx * C * es);
        };
        ggml_tensor *shift_msa = chunk(0), *scale_msa = chunk(1),
                    *gate_msa = chunk(2);
        ggml_tensor *shift_mlp = chunk(3), *scale_mlp = chunk(4),
                    *gate_mlp = chunk(5);

        // self-attention (norm1 affine-free, modulated; RoPE + QK-RMSNorm)
        ggml_tensor *hn =
                modulate(ggml_norm(ctx, h, 1e-6f), scale_msa, shift_msa);
        ggml_tensor *qkv = lin(hn, blk + ".self_attn.to_qkv");  // [3C, N]
        ggml_tensor *q = ggml_reshape_3d(
                ctx,
                ggml_cont(ctx, ggml_view_2d(ctx, qkv, C, N, qkv->nb[1], 0)), hd,
                H, N);
        ggml_tensor *k = ggml_reshape_3d(
                ctx,
                ggml_cont(ctx, ggml_view_2d(ctx, qkv, C, N, qkv->nb[1],
                                            (size_t)C * es)),
                hd, H, N);
        ...
        h2 = ggml_add(ctx, ggml_mul(ctx, h2, W(blk + ".norm2.weight")),
                      W(blk + ".norm2.bias"));
        ggml_tensor *cq = ggml_reshape_3d(
                ctx, lin(h2, blk + ".cross_attn.to_q"), hd, H, N);
        cq = qk_norm(cq, blk + ".cross_attn.q_rms_norm.gamma");
        ggml_tensor *kv = lin(cond_h, blk + ".cross_attn.to_kv");  // [2C, Lkv]
        ggml_tensor *ck = ggml_reshape_3d(
                ctx,
                ggml_cont(ctx, ggml_view_2d(ctx, kv, C, Lkv, kv->nb[1], 0)), hd,
                H, Lkv);
        ggml_tensor *cv = ggml_reshape_3d(
                ctx,
                ggml_cont(ctx, ggml_view_2d(ctx, kv, C, Lkv, kv->nb[1],
                                            (size_t)C * es)),
                hd, H, Lkv);
        ...
        ggml_tensor *hm =
                modulate(ggml_norm(ctx, h, 1e-6f), scale_mlp, shift_mlp);
    ...
    ggml_tensor *y = ggml_cont(
            ctx, ggml_transpose(ctx, h));  // [N, out_channels], channel-major
    ggml_set_output(y);
    ...
    if (ok) {
        ...
        if (t2_timing) {
            AICORE_LOG_DEBUG("trellis",
                             "[ss_flow] build=%.1f alloc=%.1f upload=%.1f "
                             "compute=%.1f read=%.1f total=%.1f ms",
                             t_ms(t_start, t_build), t_ms(t_build, t_alloc),
                             t_ms(t_alloc, t_upload), t_ms(t_upload, t_compute),
                             t_ms(t_compute, t_now()), t_ms(t_start, t_now()));
        }
    } else {
...
inline void pred_to_xstart(const std::vector<float> &x_t,
                           double t,
                           double sm,
...
inline void xstart_to_pred(const std::vector<float> &x_t,
                           double t,
                           double sm,
...
bool trellis2_ss_flow_sample(trellis2_ss_flow_model *m,
    ...
    const std::vector<float> zero_cond((size_t)cond_tokens * cond_channels,
                                       0.0f);
    std::vector<float> pred_pos(n), pred_neg(n), pred_v(n), x0_pos(n),
            x0_cfg(n);
    ...
    auto fwd = [&](double t, const float *c, std::vector<float> &dst) -> bool {
        return trellis2_ss_flow_forward(m, x_t.data(), (float)(1000.0 * t), c,
                                        cond_tokens, cond_channels, dst.data(),
                                        error);
    };
    ...
    for (int i = 0; i < P.steps; ++i) {
        ...
        const bool in_interval =
                (t >= P.guidance_interval_min && t <= P.guidance_interval_max);
        ...
        // Live preview: the denoised x_0 estimate at this step (best guess of
        // the clean latent), computed from x_t@t before the Euler step
        // overwrites it.
        if (P.preview) {
        ...
        if (P.verbose) {
            AICORE_LOG_DEBUG(
                    "trellis", "[ss sample] step %2d/%d  t=%.4f->%.4f  %s",
...
**   h = middle_block(h)                          # num_res_blocks_middle
*ResBlocks
**   for level i in 0..n_levels-1:
**       h = ResBlock x num_res_blocks            # at channels[i]
**       if i < n_levels-1: h = Upsample(h)       # Conv3d (C->C'*8) +
*pixel_shuffle_3d
**   logits = out_layer(h)                        # ChannelLayerNorm + SiLU +
*Conv3d->out
**
...
trellis2_ss_dec_model *trellis2_ss_dec_load(const std::string &path,
                                            bool load_tensors,
                                            std::string *error,
                                            const char *device) {
    ...
    hp.latent_channels =
            (int32_t)kv_u32(m->gguf, K("latent_channels").c_str(), 8);
    hp.num_res_blocks =
            (int32_t)kv_u32(m->gguf, K("num_res_blocks").c_str(), 2);
    hp.num_res_blocks_middle =
            (int32_t)kv_u32(m->gguf, K("num_res_blocks_middle").c_str(), 2);
    ...
    for (int i = 0; i < hp.n_levels; ++i) {
        hp.channels[i] = (int32_t)kv_u32(
                m->gguf, K(("channels." + std::to_string(i)).c_str()).c_str(),
                0);
    }
...
const trellis2_ss_dec_hparams &trellis2_ss_dec_hparams_of(
        const trellis2_ss_dec_model *m) {
    return m->hp;
}
...
bool trellis2_ss_dec_decode(trellis2_ss_dec_model *m,
    ...
    // Conv3d (stride 1, pad 1) + per-output-channel bias. ic/oc passed
    // explicitly.
    auto conv = [&](ggml_tensor *in, const std::string &pfx, int ic,
                    int oc) -> ggml_tensor * {
        ggml_tensor *w = W(pfx + ".weight");
        ggml_tensor *b = W(pfx + ".bias");
        if (!w) return in;
        ggml_tensor *y = ggml_conv_3d_direct(ctx, w, in, 1, 1, 1, 1, 1, 1, 1, 1,
                                             1, ic, 1, oc);
        if (b) y = ggml_add(ctx, y, ggml_reshape_4d(ctx, b, 1, 1, 1, oc));
        return y;
    };
    // ChannelLayerNorm32: per-voxel LayerNorm over the channel axis (with
    // affine).
    auto clnorm = [&](ggml_tensor *in,
                      const std::string &pfx) -> ggml_tensor * {
        ggml_tensor *p =
                ggml_cont(ctx, ggml_permute(ctx, in, 1, 2, 3, 0));  // [C,W,H,D]
    ...
    auto resblock = [&](ggml_tensor *in, const std::string &pfx,
                        int C) -> ggml_tensor * {
    ...
    // pixel_shuffle_3d(scale 2): [A,A,A, Cout*8] -> [2A,2A,2A, Cout]. Each
    // scale bit (LSB->axis0, mid->axis1, MSB->axis2, matching torch's H/W/D
    // pairing) is peeled out of the channel and interleaved into its spatial
    // axis.
    auto pshuf = [&](ggml_tensor *t, int A0, int A1, int A2,
                     int Co) -> ggml_tensor * {
        // peel s2 (channel LSB) into axis0
        t = ggml_reshape_4d(ctx, t, A0, A1 * A2, 2, Co * 4);
        t = ggml_cont(
                ctx, ggml_permute(ctx, t, 1, 2, 0, 3));  // [2, A0, A1*A2, Co*4]
        t = ggml_reshape_4d(ctx, t, 2 * A0, A1, A2, Co * 4);
        // peel s1 into axis1
        t = ggml_cont(ctx,
                      ggml_permute(ctx, t, 1, 0, 2, 3));  // [A1, 2A0, A2, Co*4]
        t = ggml_reshape_4d(ctx, t, A1, 2 * A0 * A2, 2, Co * 2);
        t = ggml_cont(ctx, ggml_permute(ctx, t, 1, 2, 0,
                                        3));  // [2, A1, 2A0*A2, Co*2]
        t = ggml_reshape_4d(ctx, t, 2 * A1, 2 * A0, A2, Co * 2);
        t = ggml_cont(
                ctx, ggml_permute(ctx, t, 1, 0, 2, 3));  // [2A0, 2A1, A2, Co*2]
        // peel s0 (channel MSB) into axis2
        t = ggml_cont(
                ctx, ggml_permute(ctx, t, 1, 2, 0, 3));  // [A2, 2A0, 2A1, Co*2]
        t = ggml_reshape_4d(ctx, t, A2, 2 * A0 * 2 * A1, 2, Co);
        t = ggml_cont(
                ctx, ggml_permute(ctx, t, 1, 2, 0, 3));  // [2, A2, 2A0*2A1, Co]
        t = ggml_reshape_4d(ctx, t, 2 * A2, 2 * A0, 2 * A1, Co);
        t = ggml_cont(ctx,
                      ggml_permute(ctx, t, 2, 0, 1, 3));  // [2A0, 2A1, 2A2, Co]
        return t;
    };
    ...
    h = ggml_silu(ctx, h);
    h = conv(h, "out_layer.2", hp.channels[hp.n_levels - 1],
             hp.out_channels);  // [Rout,Rout,Rout,Oc]
    ggml_set_output(h);
    ...
    if (ok) {
        ...
        ggml_backend_tensor_get(
                h, out, 0, (size_t)hp.out_channels * Rout * Rout * Rout * es);
    } else {
        set_error(error, "graph compute failed");
    }
...
trellis2_dino_model *trellis2_dino_load(const std::string &path,
                                        bool load_tensors,
                                        std::string *error,
                                        const char *device) {
    ...
    hp.intermediate_size =
            (int32_t)kv_u32(m->gguf, K("intermediate_size").c_str(), 0);
    hp.patch_size = (int32_t)kv_u32(m->gguf, K("patch_size").c_str(), 16);
    hp.num_register_tokens =
            (int32_t)kv_u32(m->gguf, K("num_register_tokens").c_str(), 0);
    hp.layer_norm_eps = kv_f32(m->gguf, K("layer_norm_eps").c_str(), 1e-5f);
    hp.rope_theta = kv_f32(m->gguf, K("rope_theta").c_str(), 100.0f);
    hp.file_type = (int32_t)kv_u32(m->gguf, "general.file_type", 0);
    for (int c = 0; c < 3; ++c) {
        hp.image_mean[c] =
                kv_f32(m->gguf, (K("image_mean.") + std::to_string(c)).c_str(),
                       hp.image_mean[c]);
        hp.image_std[c] =
                kv_f32(m->gguf, (K("image_std.") + std::to_string(c)).c_str(),
                       hp.image_std[c]);
    }
...
const trellis2_dino_hparams &trellis2_dino_hparams_of(
        const trellis2_dino_model *m) {
    return m->hp;
}
...
void dino_rope_tables(int hp_grid,
                      int wp_grid,
                      int head_dim,
                      float theta,
                      std::vector<float> &cos_t,
    ...
    for (int j = 0; j < quarter; ++j) {
        inv_freq[j] = 1.0 / std::pow((double)theta,
                                     (double)j * 4.0 / (double)head_dim);
    }
    ...
    for (int py = 0; py < hp_grid; ++py) {
        ...
        for (int px = 0; px < wp_grid; ++px) {
            const double cx =
                    2.0 * (((double)px + 0.5) / (double)wp_grid) - 1.0;
            ...
            for (int j = 0; j < quarter; ++j) {
                ...
                const float cy_c = (float)std::cos(ay),
                            cy_s = (float)std::sin(ay);
                const float cx_c = (float)std::cos(ax),
                            cx_s = (float)std::sin(ax);
                cos_t[base + j] = cy_c;
...
bool trellis2_dino_encode(trellis2_dino_model *m,
    ...
    // ── input leaves ─────────────────────────────────────────────────────────
    ggml_tensor *pix =
            ggml_new_tensor_4d(ctx, GGML_TYPE_F32, S, S, 3, 1);  // CHW flat
    ...
    auto rope = [&](ggml_tensor *q3) -> ggml_tensor * {
        ...
        ggml_tensor *x1 = ggml_cont(ctx, ggml_view_3d(ctx, q3, hd / 2, H, P,
                                                      q3->nb[1], q3->nb[2], 0));
        ggml_tensor *x2 =
                ggml_cont(ctx, ggml_view_3d(ctx, q3, hd / 2, H, P, q3->nb[1],
                                            q3->nb[2], half_off));
        ggml_tensor *rh =
                ggml_concat(ctx, ggml_neg(ctx, x2), x1, 0);  // [hd,H,P]
        return ggml_add(ctx, ggml_mul(ctx, q3, cos_t),
                        ggml_mul(ctx, rh, sin_t));
    };
    // RoPE on patch tokens only of a [hd, H, N] tensor (prefix passes through).
    auto rope_patches = [&](ggml_tensor *q3) -> ggml_tensor * {
        ggml_tensor *pre = ggml_cont(
                ctx,
                ggml_view_3d(ctx, q3, hd, H, Npre, q3->nb[1], q3->nb[2], 0));
        ggml_tensor *pat = ggml_cont(
                ctx, ggml_view_3d(ctx, q3, hd, H, P, q3->nb[1], q3->nb[2],
                                  (size_t)Npre * q3->nb[2]));
        return ggml_concat(ctx, pre, rope(pat), 2);
    };
    // scaled-dot-product attention; q3/k3/v3 are [hd, H, N].
    auto sdpa = [&](ggml_tensor *q3, ggml_tensor *k3,
                    ggml_tensor *v3) -> ggml_tensor * {
        ggml_tensor *qp = ggml_cont(
                ctx, ggml_permute(ctx, q3, 0, 2, 1, 3));  // [hd, N, H]
    ...
    ggml_tensor *pconv = ggml_conv_2d(
            ctx, W("embeddings.patch_embeddings.weight"), pix, hp.patch_size,
            hp.patch_size, 0, 0, 1, 1);         // [Wp, Wp, C, 1]
    ...
    ggml_tensor *h = ggml_concat(
            ctx, W("embeddings.cls_token"),
            ggml_concat(ctx, W("embeddings.register_tokens"), pconv, 1),
            1);  // [C, N]
    h = ggml_cont(ctx, h);
    tap("embd", h);
    ...
    for (int i = 0; i < hp.n_layers; ++i) {
        ...
        const bool detail =
                taps && detail_first_last && (i == 0 || i == hp.n_layers - 1);
        auto tn = [&](const char *s) {
            return "l" + std::to_string(i) + "." + s;
        };
        (void)tn;
        ...
        ggml_tensor *q = ggml_reshape_3d(
                ctx, lin(hn, blk + ".attention.q_proj"), hd, H, N);
        ggml_tensor *k = ggml_reshape_3d(
                ctx, lin(hn, blk + ".attention.k_proj"), hd, H, N);
        ggml_tensor *v = ggml_reshape_3d(
                ctx, lin(hn, blk + ".attention.v_proj"), hd, H, N);
        q = rope_patches(q);
        k = rope_patches(k);
        ggml_tensor *sa =
                lin(sdpa(q, k, v), blk + ".attention.o_proj");  // [C, N]
    ...
    if (ok) {
        ...
        if (taps) {
            for (auto &nt : tap_list) {
                ...
                ggml_backend_tensor_get(nt.second, buf.data(), 0,
                                        buf.size() * es);
...
bool trellis2_dino_encode_rgb(trellis2_dino_model *m,
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        for (size_t i = 0; i < S * S; ++i) {
            pix[(size_t)c * S * S + i] =
                    ((float)rgb[i * 3 + c] / 255.0f - mean) / sd;
        }
    }
...
void pil_coeffs(int in_size,
    ...
    for (int xx = 0; xx < out_size; ++xx) {
        ...
        for (int x = 0; x < xmax; ++x) {
            const double w =
                    pil_lanczos3(((double)(x + xmin) - center + 0.5) * ss);
            k[(size_t)x] = w;
            ww += w;
        }
        for (int x = 0; x < xmax; ++x) {
...
void pil_resize(const uint8_t *in,
                int w,
                int h,
                int ch,
                int out_w,
        ...
        for (int xx = 0; xx < out_w; ++xx) {
            for (int c = 0; c < ch; ++c) {
                ...
                for (int y = 0; y < ymax; ++y) {
                    ss += (int64_t)tmp[(size_t)(y + ymin) * out_w * ch +
                                       (size_t)xx * ch + c] *
                          k[y];
                }
                orow[(size_t)xx * ch + c] = pil_clip8(ss);
...
int trellis2_remove_solid_background_rgba(uint8_t *rgba,
                                          int w,
                                          int h,
                                          int mode) {
    if (!rgba || w <= 0 || h <= 0 || mode < TRELLIS2_BACKGROUND_AUTO ||
        mode > TRELLIS2_BACKGROUND_WHITE) {
        return -1;
    }
    ...
    if (mode == TRELLIS2_BACKGROUND_AUTO) {
        ...
        mode = is_dark && (!is_light || dark >= light)
                       ? TRELLIS2_BACKGROUND_BLACK
                       : TRELLIS2_BACKGROUND_WHITE;
    }
...
// AICore in-tree RMBG-2.0 backend. The upstream trellis-ggml vendor linked
// the RMBG-2.0-GGML submodule; here the AICore rmbg task (aicore_rmbg_*) is
// the same library, linked once inside libAICore.so. The remove-background
// entry point returns raw RGBA (not PNG bytes) so the caller can feed the
// matte straight into the preprocess stage without a PNG encode/decode round
// trip.

bool trellis2_rmbg_available() { return true; }

trellis2_rmbg_model *trellis2_rmbg_load(const char *gguf_path,
                                        const char *device,
                                        std::string *error) {
    if (!gguf_path || !gguf_path[0]) {
        set_error(error, "empty RMBG GGUF path");
        return nullptr;
    }
    aicore_rmbg_options *opts = aicore_rmbg_options_new();
    if (device && device[0]) aicore_rmbg_options_set_device(opts, device);
    aicore_rmbg_ctx *ctx = aicore_rmbg_load_opts(gguf_path, opts);
    aicore_rmbg_options_free(opts);
    if (!ctx || !aicore_rmbg_is_ready(ctx)) {
        const char *e = ctx ? aicore_rmbg_last_error(ctx) : "load failed";
        set_error(error, e ? e : "unknown RMBG load error");
        if (ctx) aicore_rmbg_free(ctx);
        return nullptr;
    }
    return reinterpret_cast<trellis2_rmbg_model *>(ctx);
}

void trellis2_rmbg_free(trellis2_rmbg_model *m) {
    if (m) aicore_rmbg_free(reinterpret_cast<aicore_rmbg_ctx *>(m));
}

// In:  RGBA bytes (w*h*4). Out: malloc'd RGBA bytes (w*h*4) with the
// background removed; free with trellis2_rmbg_free_buffer.
int trellis2_rmbg_remove_background(trellis2_rmbg_model *m,
                                    const uint8_t *rgba,
                                    int w,
                                    int h,
                                    uint8_t **out_rgba,
                                    ...
                                    std::string *error) {
    if (!m) {
        set_error(error, "null RMBG model");
        return 1;
    }
    if (!rgba || w <= 0 || h <= 0 || !out_rgba || !out_len) {
    ...
    *out_len = 0;
    ...
    // aicore_rmbg consumes tightly packed RGB; drop the alpha channel.
    ...
    int out_sz = 0;
    aicore_rmbg_ctx *ctx = reinterpret_cast<aicore_rmbg_ctx *>(m);
    const int rc = aicore_rmbg_remove_background_rgba(
            ctx, rgb.data(), w, h, &out, &out_w, &out_h, &out_sz);
    if (rc != 0) {
        const char *e = aicore_rmbg_last_error(ctx);
        set_error(error, e ? e : "rmbg failed");
        return 1;
    }
    *out_rgba = out;
    *out_len = out_sz;
...
void trellis2_rmbg_free_buffer(uint8_t *buf) {
    if (buf) aicore_rmbg_free_buffer(buf);
}

#else

bool trellis2_rmbg_available() { return false; }

#endif  // TRELLIS2_HAVE_RMBG

bool trellis2_preprocess_rgba(const uint8_t *rgba,
                              int w,
                              int h,
                              int out_size,
    ...
    if (x1 < 0) {
        set_error(error,
                  "image has no pixels with alpha > 0.8 (fully transparent?)");
        return false;
    }
...
trellis2_slat_flow_model *trellis2_slat_flow_load(const std::string &path,
                                                  bool load_tensors,
                                                  std::string *error,
                                                  const char *device) {
    ...
    for (int c = 0; c < hp.out_channels && c < 64; ++c) {
        hp.norm_mean[c] = kv_f32(
                m->gguf, K("norm_mean." + std::to_string(c)).c_str(), 0.0f);
        hp.norm_std[c] = kv_f32(
                m->gguf, K("norm_std." + std::to_string(c)).c_str(), 1.0f);
    }
    ...
    hp.concat_cond_channels =
            (int32_t)kv_u32(m->gguf, K("concat_cond_channels").c_str(), 0);
    for (int c = 0; c < hp.concat_cond_channels && c < 64; ++c) {
        hp.concat_norm_mean[c] = kv_f32(
                m->gguf, K("concat_norm_mean." + std::to_string(c)).c_str(),
                0.0f);
        hp.concat_norm_std[c] =
                kv_f32(m->gguf,
                       K("concat_norm_std." + std::to_string(c)).c_str(), 1.0f);
    }
...
const trellis2_slat_flow_hparams &trellis2_slat_flow_hparams_of(
        const trellis2_slat_flow_model *m) {
    return m->hp;
}
...
void rope_tables_coords(const int32_t *coords,
                        int L,
                        int head_dim,
                        float freq_min,
                        float freq_base,
    ...
    for (int v = 0; v < L; ++v) {
        for (int p = 0; p < pairs; ++p) {
            float theta = 0.0f;
            if (p < dim * freq_dim) {
                theta = (float)coords[(size_t)v * 3 + p / freq_dim] *
                        freqs[p % freq_dim];
            }
...
bool trellis2_slat_flow_forward(trellis2_slat_flow_model *m,
    ...
    ggml_tensor *x_t = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, hp.in_channels,
                                          N);  // voxel-major [L][Cin]
    ggml_tensor *temb = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 256);
    ggml_tensor *cos_t = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, hd, 1, N);
    ggml_tensor *sin_t = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, hd, 1, N);
    ggml_tensor *cnd =
            ggml_new_tensor_2d(ctx, GGML_TYPE_F32, cond_channels, Lkv);
    ggml_set_input(x_t);
    ggml_set_input(temb);
    ggml_set_input(cos_t);
    ggml_set_input(sin_t);
    ...
    std::vector<float> emb = timestep_embedding(t, 256);
    std::vector<float> cosv, sinv;
    rope_tables_coords(coords, N, hd, hp.rope_freq_min, hp.rope_freq_base, cosv,
                       sinv);
...
bool trellis2_slat_flow_sample(trellis2_slat_flow_model *m,
    ...
    auto fwd = [&](double t, const float *c, std::vector<float> &dst) -> bool {
        return trellis2_slat_flow_forward(m, x_t.data(), n_voxels, coords,
                                          (float)(1000.0 * t), c, cond_tokens,
                                          cond_channels, dst.data(), error);
    };
    ...
    for (int i = 0; i < P.steps; ++i) {
        ...
        if (P.verbose) {
            AICORE_LOG_DEBUG(
                    "trellis", "[slat sample] step %2d/%d  t=%.4f->%.4f  %s",
                    i + 1, P.steps, t, t_prev, in_interval ? "cfg" : "uncond");
        }
        if (P.progress) P.progress(P.progress_user, i + 1, P.steps);
    }
    ...
    if (denormalize) {
        ...
        for (int v = 0; v < n_voxels; ++v) {
            for (int c = 0; c < C; ++c) {
                x_t[(size_t)v * C + c] =
                        x_t[(size_t)v * C + c] * hp.norm_std[c] +
                        hp.norm_mean[c];
            }
        }
    }
...
bool trellis2_slat_flow_sample_tex(trellis2_slat_flow_model *m,
    ...
    const trellis2_slat_flow_hparams &hp = m->hp;
    const int Cin = hp.in_channels, Cout = hp.out_channels,
              Ccat = hp.concat_cond_channels;
    ...
    for (int v = 0; v < n_voxels; ++v)
        for (int c = 0; c < Ccat; ++c)
            shape_n[(size_t)v * Ccat + c] = (shape_slat[(size_t)v * Ccat + c] -
                                             hp.concat_norm_mean[c]) /
                                            hp.concat_norm_std[c];
    ...
    auto fwd = [&](double t, const float *c, std::vector<float> &dst) -> bool {
        for (int v = 0; v < n_voxels; ++v) {
            ...
            std::memcpy(d, x_t.data() + (size_t)v * Cout,
                        (size_t)Cout * sizeof(float));
            std::memcpy(d + Cout, shape_n.data() + (size_t)v * Ccat,
                        (size_t)Ccat * sizeof(float));
        }
    ...
    for (int i = 0; i < P.steps; ++i) {
        ...
        if (P.verbose) {
            AICORE_LOG_DEBUG("trellis",
                             "[tex slat sample] step %2d/%d  t=%.4f->%.4f  %s",
                             i + 1, P.steps, t, t_prev,
                             in_interval ? "cfg" : "uncond");
        }
        if (P.progress) P.progress(P.progress_user, i + 1, P.steps);
    }
    ...
    if (denormalize) {
        for (int v = 0; v < n_voxels; ++v)
            for (int c = 0; c < Cout; ++c)
                x_t[(size_t)v * Cout + c] =
                        x_t[(size_t)v * Cout + c] * hp.norm_std[c] +
                        hp.norm_mean[c];
    }
...
static trellis2_shape_dec_model *dec_load_impl(const std::string &path,
                                               bool load_tensors,
                                               std::string *error,
                                               const char *device,
                                               const char *expect_arch,
    ...
    hp.latent_channels =
            (int32_t)kv_u32(m->gguf, K("latent_channels").c_str(), 0);
    hp.out_channels = (int32_t)kv_u32(m->gguf, K("out_channels").c_str(), 0);
    ...
    for (int i = 0; i < hp.n_levels && i < 8; ++i) {
        hp.channels[i] = (int32_t)kv_u32(
                m->gguf, K("channels." + std::to_string(i)).c_str(), 0);
        hp.num_blocks[i] = (int32_t)kv_u32(
                m->gguf, K("num_blocks." + std::to_string(i)).c_str(), 0);
    }
...
trellis2_shape_dec_model *trellis2_shape_dec_load(const std::string &path,
                                                  bool load_tensors,
                                                  std::string *error,
                                                  const char *device) {
...
trellis2_shape_dec_model *trellis2_tex_dec_load(const std::string &path,
                                                bool load_tensors,
                                                std::string *error,
                                                const char *device) {
    return dec_load_impl(path, load_tensors, error, device, "trellis2-tex-dec",
                         "trellis2.tex_dec.");
}
...
const trellis2_shape_dec_hparams &trellis2_shape_dec_hparams_of(
        const trellis2_shape_dec_model *m) {
    return m->hp;
}
...
inline uint64_t voxel_key(int32_t c1, int32_t c2, int32_t c3) {
    return ((uint64_t)(uint32_t)c1 << 40) | ((uint64_t)(uint32_t)c2 << 20) |
...
// kD -> c1, kH -> c2, kW -> c3.
void build_neighbor_indices(const std::vector<int32_t> &coords,
                            int L,
                            std::vector<std::vector<int32_t>> &idx) {
    std::unordered_map<uint64_t, int32_t> map;
    map.reserve((size_t)L * 2);
    for (int v = 0; v < L; ++v) {
        map[voxel_key(coords[(size_t)v * 3], coords[(size_t)v * 3 + 1],
                      coords[(size_t)v * 3 + 2])] = v;
    }
...
static bool shape_dec_run(trellis2_shape_dec_model *m,
    ...
    // Texture decoder (pred_subdiv=False): the per-level subdivision is
    // supplied by the integrated shape decoder or standalone shape encoder
    // (`guide`) instead of predicted by a to_subdiv head.
    ...
    if (upsample_times >= 0 &&
        (upsample_times < 1 || upsample_times > n_levels - 1)) {
    ...
    for (int lvl = 0; lvl < n_levels; ++lvl) {
        ...
        const size_t mem = ggml_tensor_overhead() * gsize +
                           ggml_graph_overhead_custom(gsize, false);
        ggml_init_params ip{mem, nullptr, true};
        ggml_context *ctx = ggml_init(ip);
        ...
        // mul, mul_mat) stays within ggml kernels that tile the voxel
        // dimension, so the decoder runs on the GPU as well as the CPU.
        std::vector<ggml_tensor *> idx_t(27, nullptr), mask_t(27, nullptr);
        if (needs_conv) {
            for (int k = 0; k < 27; ++k) {
                idx_t[k] = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, L);
                mask_t[k] =
                        ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 1, L);  // [1, L]
        ...
        auto conv = [&](ggml_tensor *x,
            ...
            for (int k = 0; k < 27; ++k) {
                ggml_tensor *wk = ggml_cont(
                        ctx, ggml_view_3d(ctx, w, Ci, 1, Co, w->nb[1], w->nb[2],
                                          (size_t)k * w->nb[1]));
        ...
        if (lvl == 0) {
            ...
            in_a = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, hp.latent_channels,
                                      L);
            ggml_set_input(in_a);
            ...
            in_xch = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, prev_C / 8,
                                        L);  // xch
            ...
            skip = ggml_repeat(
                    ctx, skip,
                    ggml_new_tensor_3d(ctx, GGML_TYPE_F32, r, prev_C / 8, L));
            skip = ggml_reshape_2d(ctx, skip, C, L);

            ggml_tensor *hn =
                    ggml_norm(ctx, in_hch, eps);  // norm2, affine-free
            hn = ggml_silu(ctx, hn);
            hn = conv(hn, up + ".conv2");
        ...
        for (int b = 0; b < hp.num_blocks[lvl]; ++b) {
            const std::string pfx =
                    "blocks." + std::to_string(lvl) + "." + std::to_string(b);
            h = convnext(h, pfx);
        }
        ...
        if (!missing.empty()) {
            set_error(error, "missing tensor: " + missing + " (level " +
                                     std::to_string(lvl) + ")");
            ggml_free(ctx);
            return false;
        }
        ...
        if (!ggml_gallocr_alloc_graph(alloc, gf)) {
            set_error(error, "ggml_gallocr_alloc_graph failed (level " +
                                     std::to_string(lvl) + ")");
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return false;
        ...
        if (needs_conv) {
            ...
            for (int k = 0; k < 27; ++k) {
                ...
                ggml_backend_tensor_set(idx_t[k], clamped.data(), 0,
                                        (size_t)L * sizeof(int32_t));
                ggml_backend_tensor_set(mask_t[k], mask.data(), 0,
                                        (size_t)L * sizeof(float));
            }
        }
        if (lvl == 0) {
            ggml_backend_tensor_set(in_a, slat, 0,
                                    (size_t)hp.latent_channels * L * es);
        } else {
            // pre-gathered by the previous level (host_gather below)
            ggml_backend_tensor_set(in_hch, up_hch.data(), 0,
                                    up_hch.size() * es);
            ggml_backend_tensor_set(in_xch, up_xch.data(), 0,
                                    up_xch.size() * es);
        }
        ...
        if (st != GGML_STATUS_SUCCESS) {
            set_error(error, "graph compute failed (level " +
                                     std::to_string(lvl) + ")");
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return false;
        ...
        for (auto &o : outs) {
            if (o.first == "pre_up") {
                ...
                ggml_backend_tensor_get(o.second, buf.data(), 0,
                                        buf.size() * es);
                cap("lvl" + std::to_string(lvl) + ".pre_up", buf.data(),
                    buf.size());
            }
        }
        ...
        if (has_up) {
            ...
            if (!guide) {  // shape decoder: read the predicted subdivision
                           // logits
                up_subdiv.resize((size_t)8 * L);
                ggml_backend_tensor_get(subdiv_o, up_subdiv.data(), 0,
                                        up_subdiv.size() * es);
                if (taps)
                    cap("lvl" + std::to_string(lvl) + ".subdiv",
                        up_subdiv.data(), up_subdiv.size());
            }

            // The surviving children + their gather index (child slot o +
            // 8*parent, in [0, 8L)). Shape decoder: expand from predicted
            // subdivision logits. Texture decoder: replay the encoder's
            // recorded subdivision (`guide`), which also reproduces the
            // encoder's exact input voxel order.
                ...
                for (int v = 0; v < L; ++v) {
                    for (int o = 0; o < 8; ++o) {
                        if (up_subdiv[(size_t)v * 8 + o] > 0.0f) {
                            ...
                            child_coords.push_back(2 * coords[(size_t)v * 3] +
                                                   (o & 1));
                            child_coords.push_back(
                                    2 * coords[(size_t)v * 3 + 1] +
                                    ((o >> 1) & 1));
                            child_coords.push_back(
                                    2 * coords[(size_t)v * 3 + 2] +
                                    ((o >> 2) & 1));
                        }
                    }
                }
            ...
            auto host_gather = [&](ggml_tensor *t, int chans,
                ...
                if (t->buffer && ggml_backend_buffer_is_host(t->buffer)) {
                    ...
                    for (int j = 0; j < L_child; ++j)
                        std::memcpy(out.data() + (size_t)j * chans,
                                    d + (size_t)cidx[(size_t)j] * chans,
                                    (size_t)chans * es);
                } else {
                    std::vector<float> full((size_t)ggml_nelements(t));
                    ggml_backend_tensor_get(t, full.data(), 0,
                                            full.size() * es);
                    for (int j = 0; j < L_child; ++j)
                        std::memcpy(
                                out.data() + (size_t)j * chans,
                                full.data() + (size_t)cidx[(size_t)j] * chans,
                                (size_t)chans * es);
                }
            };
            auto t_gh = t_now();
            ...
            for (auto &o : outs) {
                if (o.first == "out") {
                    ggml_backend_tensor_get(o.second, out_feats.data(), 0,
                                            out_feats.size() * es);
                }
            }
            if (pbr_scale)  // tex decoder: map to [0,1] like the reference
                            // *0.5+0.5
                for (float &f : out_feats) f = f * 0.5f + 0.5f;
            out_coords = coords;
            if (taps) {
                cap(pbr_scale ? "pbr" : "out7", out_feats.data(),
                    out_feats.size());
        ...
        // level's up-block produced (has_up is always true for lvl <
        // n_levels-1, which upsample_times-1 always is), so return it and skip
        // the rest.
        if (upsample_times >= 0 && lvl == upsample_times - 1) {
            ...
            if (t2_timing)
                AICORE_LOG_DEBUG(
                        "trellis",
                        "[shape_dec] nbr=%.0f graph=%.0f gather=%.0f ms",
...
bool trellis2_shape_dec_decode(trellis2_shape_dec_model *m,
    ...
    return shape_dec_run(m, slat, n_voxels, coords_in, /*upsample_times*/ -1,
                         /*guide*/ nullptr, /*predicted_subs*/ nullptr,
                         /*pbr_scale*/ false, out_feats, out_coords, taps,
                         error);
}

bool trellis2_shape_dec_decode_with_subs(
        trellis2_shape_dec_model *m,
        const float *slat,
        int n_voxels,
        const int32_t *coords_in,
        std::vector<float> &out_feats,
...
bool trellis2_shape_dec_upsample(trellis2_shape_dec_model *m,
    ...
    return shape_dec_run(m, slat, n_voxels, coords, upsample_times,
                         /*guide*/ nullptr, /*predicted_subs*/ nullptr,
                         /*pbr_scale*/ false, unused, out_coords,
                         /*taps*/ nullptr, error);
}
...
trellis2_shape_enc_model *trellis2_shape_enc_load(const std::string &path,
                                                  bool load_tensors,
                                                  std::string *error,
                                                  const char *device) {
...
const trellis2_shape_enc_hparams &trellis2_shape_enc_hparams_of(
        const trellis2_shape_enc_model *m) {
    return m->hp;
}
...
bool trellis2_shape_enc_encode(trellis2_shape_enc_model *m,
    ...
    for (int lvl = 0; lvl < n_levels; ++lvl) {
        ...
        // coords are lexicographically sorted to match the reference's
        // unique().
        std::vector<int32_t> coarse_coords;  // [Lc*3]
        std::vector<int32_t> childidx;  // [Lc*8]  fine row of child (or -1)
        std::vector<int32_t>
                cidx;  // [L]     8*parent_row + subidx (decoder replay)
        int Lc = 0;
        ...
        if (has_down) {
            ...
            for (int v = 0; v < L; ++v)
                fine_map[voxel_key(coords[(size_t)v * 3],
                                   coords[(size_t)v * 3 + 1],
                                   coords[(size_t)v * 3 + 2])] = v;
            // unique parents
            ...
            for (int v = 0; v < L; ++v) {
                const int32_t px = coords[(size_t)v * 3] >>
                                   1,  // // 2 (coords >= 0)
            ...
            for (int p = 0; p < Lc; ++p) {
                ...
                coarse_map[voxel_key(parents[p][0], parents[p][1],
                                     parents[p][2])] = p;
            }
            ...
            // decoder replay: for each fine voxel, its coarse parent row +
            // subidx
        ...
        auto conv = [&](ggml_tensor *x, const std::string &pfx,
                        std::vector<ggml_tensor *> &idxs,
                        std::vector<ggml_tensor *> &masks) -> ggml_tensor * {
            ggml_tensor *w = W(pfx + ".weight");  // ne [Ci, 27, Co]
            ggml_tensor *b = W(pfx + ".bias");
            if (!w || !b) return x;
            const int64_t Ci = w->ne[0], Co = w->ne[2];
        ...
        // input for this level: 6-channel dual grid at lvl 0 (through
        // input_layer), else the previous level's [C_in, L] features.
        ...
        for (int b = 0; b < hp.num_blocks[lvl]; ++b)
            h = convnext(h, "blocks." + std::to_string(lvl) + "." +
                                    std::to_string(b));
        ...
        if (has_down) {
            ...
            auto s2c = [&](ggml_tensor *t, int ch) -> ggml_tensor * {
                ...
                return ggml_reshape_2d(ctx, ggml_cont(ctx, g), (int64_t)ch * 8,
                                       Lc);
            };
            ggml_tensor *h1c = s2c(h1, C_out / 8);  // [C_out, Lc]
            ggml_tensor *xc = s2c(h, C_in);         // [C_in*8, Lc]
            ggml_tensor *hn2 = ggml_silu(
                    ctx, ggml_norm(ctx, h1c, eps));  // norm2 affine-free
            ggml_tensor *h2 =
                    conv(hn2, down + ".conv2", idx_c, mask_c);  // [C_out, Lc]
            ...
            skip = ggml_reshape_2d(ctx, ggml_cont(ctx, ggml_mean(ctx, skip)),
                                   C_out, Lc);
            out_h = ggml_add(ctx, h2, skip);  // [C_out, Lc]
        } else {
            ggml_tensor *hn =
                    ggml_norm(ctx, h, 1e-5f);       // F.layer_norm affine-free
            ggml_tensor *z = lin(hn, "to_latent");  // [2*latent, L]
            // take the mean half (posterior mean): channels [0, latent)
            out_h = ggml_cont(ctx, ggml_view_2d(ctx, z, hp.latent_channels, L,
                                                z->nb[1], 0));
        }
        ...
        ggml_gallocr_t alloc = ggml_gallocr_new(
                ggml_backend_get_default_buffer_type(m->backend));
        if (!ggml_gallocr_alloc_graph(alloc, gf)) {
            set_error(error, "ggml_gallocr_alloc_graph failed (enc level " +
                                     std::to_string(lvl) + ")");
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return false;
        ...
        auto upload_nbr = [&](std::vector<std::vector<int32_t>> &nb, int Ln,
            ...
            for (int k = 0; k < 27; ++k) {
                ...
                ggml_backend_tensor_set(idxs[k], cl.data(), 0,
                                        (size_t)Ln * sizeof(int32_t));
                ggml_backend_tensor_set(masks[k], mk.data(), 0,
                                        (size_t)Ln * sizeof(float));
            }
        };
        upload_nbr(nfine, L, idx_f, mask_f);
        if (lvl == 0)
            ggml_backend_tensor_set(in_a, in_shift.data(), 0,
                                    in_shift.size() * es);
        else
        ...
        if (has_down) {
            ...
            ggml_backend_tensor_set(cidx_t, cl.data(), 0,
                                    cl.size() * sizeof(int32_t));
            ggml_backend_tensor_set(cmask_t, mk.data(), 0,
                                    mk.size() * sizeof(float));
        }

        if (ggml_backend_graph_compute(m->backend, gf) != GGML_STATUS_SUCCESS) {
            set_error(error, "enc graph compute failed (level " +
                                     std::to_string(lvl) + ")");
            ggml_gallocr_free(alloc);
            ggml_free(ctx);
            return false;
        ...
        if (has_down) {
            // record subdivision for the decoder (decoder order = reverse of
            // encode)
            trellis2_subdiv_level &sl = out_subs[(size_t)(n_levels - 2 - lvl)];
            sl.fine_coords =
                    coords;  // this level's (fine) coords, in fine order
