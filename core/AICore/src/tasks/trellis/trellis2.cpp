...
void trellis2_set_threads(int n_threads) { tls_n_threads = n_threads > 0 ? n_threads : 0; }
...
ggml_backend_t init_best_backend(std::string & name_out, const char * device = nullptr) {
    ...
    int n_threads = tls_n_threads > 0 ? tls_n_threads
...
bool trellis2_ss_flow_forward(trellis2_ss_flow_model * m,
    ...
    if (ok) {
        ...
        if (t2_timing) {
            AICORE_LOG_DEBUG("trellis", "[ss_flow] build=%.1f alloc=%.1f upload=%.1f compute=%.1f read=%.1f total=%.1f ms",
...
bool trellis2_ss_flow_sample(trellis2_ss_flow_model * m,
    ...
    for (int i = 0; i < P.steps; ++i) {
        ...
        if (P.verbose) {
            AICORE_LOG_DEBUG("trellis", "[ss sample] step %2d/%d  t=%.4f->%.4f  %s",
...
int trellis2_rmbg_remove_background(trellis2_rmbg_model * m,
                                    ...
                                    uint8_t ** out_rgba, int * out_len,
                                    std::string * error) {
    if (!m) { set_error(error, "null RMBG model"); return 1; }
    if (!rgba || w <= 0 || h <= 0 || !out_rgba || !out_len) {
        set_error(error, "invalid image"); return 1;
    }
    *out_rgba = nullptr;
    *out_len = 0;
...
bool trellis2_slat_flow_sample(trellis2_slat_flow_model * m,
    ...
    for (int i = 0; i < P.steps; ++i) {
        ...
        if (P.verbose) {
            AICORE_LOG_DEBUG("trellis", "[slat sample] step %2d/%d  t=%.4f->%.4f  %s",
                             i + 1, P.steps, t, t_prev, in_interval ? "cfg" : "uncond");
        }
        if (P.progress) P.progress(P.progress_user, i + 1, P.steps);
    }
...
bool trellis2_slat_flow_sample_tex(trellis2_slat_flow_model * m,
    ...
    for (int i = 0; i < P.steps; ++i) {
        ...
        if (P.verbose) {
            AICORE_LOG_DEBUG("trellis", "[tex slat sample] step %2d/%d  t=%.4f->%.4f  %s",
                             i + 1, P.steps, t, t_prev, in_interval ? "cfg" : "uncond");
        }
        if (P.progress) P.progress(P.progress_user, i + 1, P.steps);
    }
...
static bool shape_dec_run(trellis2_shape_dec_model * m,
    ...
    for (int lvl = 0; lvl < n_levels; ++lvl) {
        ...
        if (upsample_times >= 0 && lvl == upsample_times - 1) {
            out_coords = coords;
            if (t2_timing)
                AICORE_LOG_DEBUG("trellis", "[shape_dec] nbr=%.0f graph=%.0f gather=%.0f ms",
