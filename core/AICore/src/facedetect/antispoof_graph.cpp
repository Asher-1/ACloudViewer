...
// ONNX BatchNormalization epsilon (the MiniFASNet exports carry the default
// 1e-5).
...
struct Node {
    ...
    float eps =
            -1.0f;  // per-node BatchNormalization epsilon (<0 -> use bn_eps)
...
// group, e.g. MobileFaceNet's group=64 / 2-in-2-out layer) that is neither
// dense nor depthwise. ggml has no native grouped conv, so split the input
// channels and output filters into `group` equal slices, run a dense conv per
// slice, and concat the results back along the channel axis. W ne =
// [KW,KH,ipg,OC]; x ne = [W,H,IC,1] with IC = group*ipg, OC = group*opg.
ggml_tensor* conv_grouped(ggml_context* ctx,
                          ggml_tensor* W,
                          ggml_tensor* x,
                          int group,
                          int stride,
    ...
    for (int g = 0; g < group; ++g) {
        ggml_tensor* xg =
                ggml_cont(ctx, ggml_view_4d(ctx, x, x->ne[0], x->ne[1], ipg, 1,
                                            x->nb[1], x->nb[2], x->nb[3],
                                            (size_t)g * ipg * x->nb[2]));
        ggml_tensor* wg =
                ggml_cont(ctx, ggml_view_4d(ctx, W, W->ne[0], W->ne[1], ipg,
                                            opg, W->nb[1], W->nb[2], W->nb[3],
                                            (size_t)g * opg * W->nb[3]));
        ggml_tensor* yg =
                ggml_conv_2d(ctx, wg, xg, stride, stride, pad, pad, 1, 1);
        acc = acc ? ggml_concat(ctx, acc, yg, 2) : yg;
    }
    return acc;
}

// One folded Conv+BatchNorm: BN's inference affine (scale =
// gamma/sqrt(var+eps), shift = beta - mean*scale) folded into the PRECEDING
// conv's weights and bias, so the BatchNormalization op vanishes from the graph
// (no per-output full-map mul+add - the ~20% of the SFace recognizer forward
// that the unfused BN cost). W'[oc] = W[oc]*scale[oc]; b'[oc] = b[oc]*scale[oc]
// + shift[oc] (b=0 for the bias-less SFace/MobileFaceNet convs). Algebraically
// identical to the live-BN path; the only delta is f32 rounding order ((w*s)*x
// vs (sum w*x)*s), far inside the recognizer max|d|<=1e-3 / cosine>=0.9999
// gate. F32 conv weights only (the host fold dequantizes nothing; quantized/f16
// packs keep the live-BN path).
struct ConvBNFold {
...
const ConvBNFold& convbn_fold(const ModelLoader& ml,
    ...
    ggml_tensor* gamma =
            clone_weight(nullptr, ml, (prefix + gamma_name).c_str());
    ...
    ggml_backend_tensor_get(W, wsrc.data(), 0,
                            (size_t)OC * inner * sizeof(float));
    std::vector<float> bsrc;
    if (!bias_name.empty() && ml.tensor((prefix + bias_name).c_str())) {
        bsrc.resize(OC);
        ggml_tensor* B =
                clone_weight(nullptr, ml, (prefix + bias_name).c_str());
...
// axis `a` is ggml axis `3-a`. ONNX out axis i takes in axis perm[i]; we want
// the ggml source axis (3-perm[i]) to land at ggml dest (3-i). Returns a
// permuted VIEW (callers ggml_cont it, e.g. the following Reshape).
ggml_tensor* transpose_onnx(ggml_context* ctx,
                            ggml_tensor* x,
...
// Execute ONE parsed node, resolving inputs against `vals` (prior activations)
// or the `prefix`-namespaced GGUF weights. Shared by the single-output runner
// (the MiniFASNet / MobileFaceNet graphs) and the multi-output SCRFD detector
// runner; `keep` owns BN-eps host scalars that must outlive the compute.
ggml_tensor* build_node(
        ggml_context* ctx,
    ...
    if (n.op == "Conv") {
        ...
        // weight is F32, replace W with the host-folded W*scale and add the
        // folded per-channel bias; the matched BatchNormalization node aliases
        // this result (see run_onnx_graph), so the BN op never enters the
        // graph.
        ggml_tensor* Wraw = clone_weight(ctx, ml, (prefix + n.in[1]).c_str());
        const Node* bn = nullptr;
        if (folds) {
            auto it = folds->find(n.out);
            if (it != folds->end()) bn = &it->second;
        }
        if (bn && Wraw->type == GGML_TYPE_F32) {
            const std::string biasname =
                    (n.in.size() > 2) ? n.in[2] : std::string();
            const ConvBNFold& f = convbn_fold(
                    ml, prefix, n.in[1], biasname, bn->in[1], bn->in[2],
                    bn->in[3], bn->in[4], bn->eps >= 0.0f ? bn->eps : bn_eps);
            W = graph_input_tensor(ctx, GGML_TYPE_F32, 4, f.ne, f.w.data(),
                                   f.w.size() * sizeof(float));
            const int64_t bne[4] = {(int64_t)f.bias.size(), 1, 1, 1};
            B = graph_input_tensor(ctx, GGML_TYPE_F32, 1, bne, f.bias.data(),
                                   f.bias.size() * sizeof(float));
        } else {
            W = Wraw;
        ...
        // When the following BN->PReLU chain will be fused (CPU), defer the
        // bias: the fused_prelu_bias op adds it in the same pass as the
        // activation, so skip the standalone broadcast add here and hand the
        // leaf to the PReLU.
        ...
        if (pending_bias) {
            ...
            if (it !=
                pending_bias->end())  // fused (deferred-bias + PReLU) pass
        ...
        // SFace in-graph input normalization: data - scalar_op1 (mean 127.5).
        // The subtrahend is a [1] leaf that broadcasts over the whole [W,H,C,1]
        // input.
        return ggml_sub(ctx, resolve(n.in[0]), resolve(n.in[1]));
    } else if (n.op == "Dropout") {
        // Inference-time Dropout is the identity (the ratio is a no-op at
        // eval).
        return resolve(n.in[0]);
    } else if (n.op == "Identity") {
        // 1k3d68 begins with an Identity passthrough (data -> id) before
        // bn_data.
        return resolve(n.in[0]);
    } else if (n.op == "Mul") {
        ...
        // SCRFD stem MaxPool / residual-shortcut AveragePool. The square 640
        // input keeps every pooled map even, so ceil_mode is moot (ceil ==
        // floor).
        ggml_tensor* x = resolve(n.in[0]);
        const ggml_op_pool op =
                (n.op == "MaxPool") ? GGML_OP_POOL_MAX : GGML_OP_POOL_AVG;
        ...
        // SCRFD FPN top-down upsample: nearest, 2x (the shape-plumbing that fed
        // the dynamic target size was dropped by the converter; on the even 640
        // maps it is exactly 2x, matching the det_10g hand path's
        // ggml_upscale).
        ...
        // SCRFD head reshape to [-1, rc]: ggml ne [rc, nelem/rc], contiguous.
        // After the [2,3,0,1] Transpose the contiguous element order is ONNX
        // row-major (h,w,anchor,c), so this lands the (H*W*anchor, c) layout
        // the host decode reads (== flatten_head), with rc = 1 (score) / 4
        // (bbox) / 10 (kps).
        ...
        if (n.in.size() > 2 && !n.in[2].empty())
            y = ggml_add(ctx, y,
                         clone_weight(ctx, ml, (prefix + n.in[2]).c_str()));
        return y;
    } else if (n.op == "BatchNormalization") {
        // BN over the channel dim, for BOTH 4-D spatial maps ([W,H,C,N], the
        // SFace conv BNs) and 1-D linear outputs ([C,N], the MobileFaceNet /
        // SFace fc1 BN). The per-node epsilon `n.eps` is used when present
        // (SFace mixes 1e-3 conv BNs with 2e-5 bn1/fc1 BNs); otherwise the
        // caller's `bn_eps`.
        ggml_tensor* x = resolve(n.in[0]);
        ...
        // Linear activation: channel is ggml ne[0] ([C,1]); broadcast over the
        // row.
        if (x->ne[0] == C && x->ne[1] == 1 && x->ne[2] == 1) {
            return ggml_add(ctx,
                            ggml_mul(ctx, x, ggml_reshape_2d(ctx, scale, C, 1)),
                            ggml_reshape_2d(ctx, shift, C, 1));
        }
        // Spatial map: channel is ggml ne[2]; broadcast scale/shift per
        // channel.
...
std::vector<float> run_onnx_graph(const ModelLoader& ml,
                                  const std::string& prefix,
    ...
    std::vector<std::vector<float>>
            keep;  // BN-eps host buffers; outlive compute
    ...
    const bool ok = be.compute(
            [&](ggml_context* ctx) -> ggml_tensor* {
                ...
                ggml_tensor* input = graph_input_tensor(
                        ctx, GGML_TYPE_F32, 4, ne, blob.data(),
                        blob.size() * sizeof(float));
                // Seed the input under BOTH a generic alias and the graph's
                // real ONNX input name (e.g. MiniFASNet "input" vs
                // MobileFaceNet "input.1"), so the first node resolves it as an
                // activation rather than a missing weight.
                vals["input"] = input;
                if (!input_name.empty()) vals[input_name] = input;

                // Parse once, then plan Conv+BN folds: a BatchNormalization
                // whose in[0] is produced by a Conv that feeds nothing else
                // (single consumer) folds into that conv (host weight*scale +
                // bias). The matched BN is then a pure alias of the folded conv
                // output. On the SFace recognizer this elides all 29 spatial
                // BNs; the SCRFD detector graph carries no BN nodes (folded at
                // conversion) and the MiniFASNet antispoof BN sits after a
                // MatMul, so neither matches and both keep their exact prior
                // graph.
                std::vector<Node> nodes;
                nodes.reserve(specs.size());
                for (const std::string& spec : specs)
                    nodes.push_back(parse_node(spec, ','));
                ...
                for (const Node& n : nodes) {
                    ...
                    // Fuse the deferred bias-add into the following PReLU when
                    // (a) on CPU, (b) the BN feeds exactly one consumer, and
                ...
                for (const Node& n : nodes) {
                    ...
                    if (ba !=
                        bn_alias.end()) {  // folded BN: alias the conv output
                        ggml_tensor* y = vals.at(ba->second);
                        vals[n.out] = y;
                        // carry the conv's deferred bias leaf onto the BN alias
                        // name, so the PReLU (which reads the BN output) finds
                        // it.
                        auto pb = pending_bias.find(ba->second);
                        if (pb != pending_bias.end())
                            pending_bias[n.out] = pb->second;
                        last = y;
                        continue;
                    }
                    ggml_tensor* y =
                            build_node(ctx, ml, prefix, n, vals, keep, bn_eps,
    ...
    const bool ok = be.compute(
            [&](ggml_context* ctx) -> ggml_tensor* {
                ...
                for (const std::string& spec : specs) {
                    ...
                    vals[n.out] = build_node(ctx, ml, prefix, n, vals, keep,
                                             bn_eps, nullptr, nullptr, nullptr);
                }
                // Capture every requested output (the SCRFD per-stride heads)
                // for host-side readback. Returns the last as the graph's
                // nominal root.
                ggml_tensor* root = nullptr;
                for (size_t i = 0; i < output_names.size(); ++i) {
                    auto it = vals.find(output_names[i]);
                    if (it == vals.end())
                        throw std::runtime_error("onnx_graph: missing output " +
                                                 output_names[i]);
...
std::vector<float> minifasnet_forward(const ModelLoader& ml,
                                      int model_idx,
                                      const std::vector<float>& blob,
                                      int size,
                                      Backend& be) {
...
bool antispoof_crop(const Image& img,
                    const Detection& d,
                    float scale,
                    Image& out,
                    int size) {
    ...
    scale = std::min(std::min((src_h - 1.0f) / box_h, (src_w - 1.0f) / box_w),
                     scale);
    ...
    // Express the integer crop + cv2.resize(INTER_LINEAR, align_corners=False)
    // as a forward affine src->dst and reuse the shared cv2-faithful warp:
...
std::array<double, 3> ensemble_softmax(const Model& m,
                                       const Image& img,
                                       const Detection& d) {
    ...
    for (size_t i = 0; i < sc.size(); ++i) {
        ...
        std::vector<float> blob =
                to_blob(crop, size, 0.0f, 1.0f, /*swap_rb=*/true);
        ...
        double e[3], s = 0.0;
        for (int k = 0; k < 3; ++k) {
            e[k] = std::exp((double)logits[k] - mx);
            s += e[k];
        }
