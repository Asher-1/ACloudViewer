...
public:
    ...
    ggml_tensor *Require(const std::string &name) {
        ggml_tensor *tensor = context_ == nullptr
                                      ? nullptr
                                      : ggml_get_tensor(context_, name.c_str());
...
ggml_tensor *LinearForward(ggml_context *context,
                           const Linear &linear,
                           ggml_tensor *input) {
...
ggml_tensor *FeedForwardBlock(ggml_context *context,
                              const FeedForward &weights,
...
ggml_tensor *RepeatPairs(ggml_context *context,
                         ggml_tensor *input,
...
ggml_tensor *RotateHalf(ggml_context *context,
                        ggml_tensor *input,
                        int64_t head_dim,
                        int64_t tokens,
                        int64_t heads) {
    ...
    ggml_tensor *first =
            ggml_view_4d(context, pairs, 1, head_dim / 2, tokens, heads,
                         pairs->nb[1], pairs->nb[2], pairs->nb[3], 0);
    ggml_tensor *second = ggml_view_4d(context, pairs, 1, head_dim / 2, tokens,
                                       heads, pairs->nb[1], pairs->nb[2],
                                       pairs->nb[3], sizeof(float));
...
ggml_tensor *ApplyRotary(ggml_context *context,
                         ggml_tensor *input,
                         ggml_tensor *cosine,
                         ggml_tensor *sine,
                         int64_t head_dim,
...
Qkv SplitInterleavedQkv(ggml_context *context,
                        ggml_tensor *input,
                        int64_t head_dim,
                        int64_t tokens,
                        int64_t heads) {
...
ggml_tensor *Heads(ggml_context *context,
                   ggml_tensor *input,
                   int64_t head_dim,
                   int64_t tokens,
                   int64_t heads) {
...
Encoding PositionalEncoding(ggml_context *context,
...
ggml_tensor *SelfBlock(ggml_context *context,
    ...
    ggml_tensor *qkv_projection =
            tap(LinearForward(context, weights.self_qkv, descriptors),
                prefix + ".qkv");
    Qkv qkv = SplitInterleavedQkv(context, qkv_projection, head_dim, tokens,
                                  heads);
    qkv.q = ApplyRotary(context, qkv.q, encoding.cosine, encoding.sine,
                        head_dim, tokens, heads);
    qkv.k = ApplyRotary(context, qkv.k, encoding.cosine, encoding.sine,
                        head_dim, tokens, heads);
    tap(qkv.q, prefix + ".q");
    tap(qkv.k, prefix + ".k");
    tap(qkv.v, prefix + ".v");
    ggml_tensor *message =
            Attention(context, qkv.q, qkv.k, qkv.v, 1.0f / std::sqrt(head_dim),
                      fused_attention);
...
std::pair<ggml_tensor *, ggml_tensor *> CrossBlock(ggml_context *context,
                                                   const LayerWeights &weights,
                                                   ggml_tensor *descriptors0,
                                                   ggml_tensor *descriptors1,
                                                   int64_t descriptor_dim,
                                                   int64_t head_dim,
                                                   int64_t tokens0,
                                                   int64_t tokens1,
                                                   ...
                                                   float epsilon,
                                                   bool fused_attention) {
    ggml_tensor *qk0 = Heads(
            context, LinearForward(context, weights.cross_qk, descriptors0),
            head_dim, tokens0, heads);
    ggml_tensor *qk1 = Heads(
            context, LinearForward(context, weights.cross_qk, descriptors1),
            head_dim, tokens1, heads);
    ggml_tensor *value0 = Heads(
            context, LinearForward(context, weights.cross_v, descriptors0),
            head_dim, tokens0, heads);
    ggml_tensor *value1 = Heads(
            context, LinearForward(context, weights.cross_v, descriptors1),
            head_dim, tokens1, heads);
...
public:
    ...
    bool match(const Features &image1,
               const Features &image2,
        ...
        for (size_t i = 0; i < raw.matches0.size(); ++i) {
            if (raw.matches0[i] >= 0) {
                matches->push_back({static_cast<int32_t>(i), raw.matches0[i],
                                    raw.mscores0[i]});
            }
        }
        return true;
    ...
    bool run_raw(const Features &image1,
                 const Features &image2,
        ...
        ggml_tensor *position0 = ggml_new_tensor_2d(context, GGML_TYPE_F32,
                                                    position_dim, tokens0);
        ggml_tensor *position1 = ggml_new_tensor_2d(context, GGML_TYPE_F32,
                                                    position_dim, tokens1);
        ...
        if (input_projection_.weight != nullptr) {
            descriptor0 =
                    LinearForward(context, input_projection_, descriptor0);
            descriptor1 =
                    LinearForward(context, input_projection_, descriptor1);
        }
        ...
        for (size_t layer_index = 0; layer_index < layers_.size();
            ...
            descriptor0 =
                    SelfBlock(context, layer, descriptor0, encoding0, dim,
                              head_dim, tokens0, heads,
            ...
            descriptor1 =
                    SelfBlock(context, layer, descriptor1, encoding1, dim,
                              head_dim, tokens1, heads,
                              file_.hp.layer_norm_epsilon, fused_attention, tap,
                              "layer" + std::to_string(layer_index) + ".self1");
            tap(descriptor1, "layer" + std::to_string(layer_index) + ".self1");
            std::tie(descriptor0, descriptor1) =
                    CrossBlock(context, layer, descriptor0, descriptor1, dim,
                               head_dim, tokens0, tokens1, heads,
                               file_.hp.layer_norm_epsilon, fused_attention);
        ...
        ggml_cgraph *graph =
                ggml_new_graph_custom(context, kMaxGraphNodes, false);
        ...
        if (backend_.is_cpu()) {
            ...
            // Reduce matching on the accelerator. Copying the full MxN
            // assignment matrix to the host dominates CUDA latency at large
            // keypoint counts.
            ...
            ggml_tensor *best1_rows =
                    ggml_reshape_2d(context, device_best1, 1, tokens1);
            device_mutual1 = ggml_get_rows(context, best1_rows, device_best0);

            // Gather scores[i, best0[i]] from the flattened assignment matrix.
            // Linear indices avoid materializing an additional tokens0^2
            // tensor.
            ...
            ggml_tensor *linear_indices = ggml_cast(
                    context,
                    ggml_add(context,
                             ggml_cast(context, device_best0, GGML_TYPE_F32),
                             row_offsets),
                    GGML_TYPE_I32);
    ...
    bool ValidateFeatures(const Features &features, const char *label) {
        ...
        if (features.descriptor_dim != file_.hp.input_dim) {
            error_ = std::string(label) + " descriptor dimension is " +
                     std::to_string(features.descriptor_dim) +
                     ", model expects " + std::to_string(file_.hp.input_dim);
            return false;
        }
        if (features.descriptors.size() !=
            features.keypoints.size() *
                    static_cast<size_t>(features.descriptor_dim)) {
            error_ = std::string(label) +
                     " descriptor buffer has the wrong size";
            return false;
        }
        return true;
    }
    ...
    bool RealizeWeights() {
        ...
        for (int64_t i = 0; i < file_.TensorCount(); ++i) {
            ...
            if (source->type == target->type) {
                ggml_backend_tensor_set(target, source->data, 0,
                                        ggml_nbytes(source));
                ...
                error_ = std::string("unsupported CPU weight conversion for ") +
                         name;
                return false;
            }
        }
        ForEachWeight([&](ggml_tensor *&tensor) {
            if (tensor != nullptr) {
                tensor =
                        ggml_get_tensor(device_context_, ggml_get_name(tensor));
            }
        });
        return true;
    }
        ...
        for (int64_t i = 0; i < tokens0; ++i) {
            ...
            for (int64_t j = 0; j < tokens1; ++j) {
                const float candidate =
                        scores[static_cast<size_t>(j + tokens1 * i)];
                if (candidate > value) {
                    value = candidate;
...
std::unique_ptr<FeatureMatcher> create_feature_matcher(
        const MatchingOptions &options, std::string *error) {
