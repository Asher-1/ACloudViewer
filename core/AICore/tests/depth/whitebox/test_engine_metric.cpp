...
// Feeds the dumped raw_image (224x224x3 uint8) through Engine::depth_metric
// (which runs the nested anyview GIANT + metric ViT-L backbones + DPT/sky heads
// + cam pose + NestedAligner) and compares depth_final / scale_factor /
// extrinsics_final vs the nested dump at 5e-3. SKIP (77) if any artifact is
// absent.
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main() {
    ...
    const char* metric = std::getenv("AICORE_TEST_DEPTH_GGUF_METRIC");
    ...
    if ((int)raw.size() != H * W * 3) {
        std::fprintf(stderr, "bad raw_image %zu\n", raw.size());
        return 1;
    }
    ...
    int oH, oW;
    if (!eng->depth_metric(img, out, oH, oW)) {
        std::fprintf(stderr, "depth_metric failed\n");
        return 1;
    }
    ...
    {
        std::vector<float> ref;
        if (!da_parity::load_baseline(base, "extrinsics_final", ref, s))
            return 1;
