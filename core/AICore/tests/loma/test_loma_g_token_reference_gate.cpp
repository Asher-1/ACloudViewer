...
bool LoadReference(const char* path, Reference* reference) {
    ...
    if (!stream.read(magic, sizeof(magic)) ||
        ...
        !Read(stream, &version) || !Read(stream, &width) || !Read(stream, &height) ||
        !Read(stream, &tokens) || !Read(stream, &dimension) || !Read(stream, &rgb_bytes) ||
        version != 1 || width != kWidth || height != kHeight || tokens != kTokens ||
        dimension != kDimension || rgb_bytes != kWidth * kHeight * 3) {
        return false;
    }
    ...
    return static_cast<bool>(stream.read(reinterpret_cast<char*>(reference->rgb.data()),
                                         reference->rgb.size())) &&
           static_cast<bool>(stream.read(reinterpret_cast<char*>(reference->tokens.data()),
...
int main() {
    ...
    const char* fixture = std::getenv("AICORE_TEST_LOMA_DESCRIPTOR_G_TOKEN_REFERENCE");
    ...
    if (model == nullptr || fixture == nullptr || model[0] == '\0' || fixture[0] == '\0') {
        std::fprintf(stderr, "SKIP: set AICORE_TEST_LOMA_DESCRIPTOR_G_GGUF and "
    ...
    if (const char* block = std::getenv("AICORE_TEST_LOMA_DESCRIPTOR_G_TRACE_BLOCK")) {
        char* end = nullptr;
    ...
    const aicore_loma_rgb_image image = {reference.rgb.data(), static_cast<int32_t>(kWidth),
    ...
    for (size_t index = 0; index < output.size(); ++index) {
        ...
        reference_squared += static_cast<double>(reference.tokens[index]) * reference.tokens[index];
    }
    const double relative_l2 = std::sqrt(squared_error / std::max(reference_squared, 1e-30));
    std::printf("{\"suite\":\"loma-dedode-g-token-reference\",\"device\":\"%s\",\"tokens\":%u,"
                "\"dimension\":%u,\"max_absolute_error\":%.8g,\"relative_l2\":%.8g}\n",
                options.device.c_str(), kTokens, kDimension, max_absolute_error, relative_l2);
