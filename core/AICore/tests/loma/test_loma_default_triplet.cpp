...
int main() {
    ...
    if (detector_model == nullptr || descriptor_model == nullptr ||
        matcher_model == nullptr || detector_model[0] == '\0' ||
        descriptor_model[0] == '\0' || matcher_model[0] == '\0') {
        std::fprintf(stderr,
                     "SKIP: set "
                     "AICORE_TEST_LOMA_{DETECTOR,DESCRIPTOR,MATCHER}_GGUF\n");
        return 77;
    }
    if (device == nullptr || device[0] == '\0') device = "cpu";
    int expected_dimension = 256;
    if (expected_dimension_text != nullptr &&
        expected_dimension_text[0] != '\0') {
    ...
    if (!aicore_loma_detector_is_ready(detector) ||
        ...
        std::fprintf(stderr,
                     "LoMa model-pack load failed: detector=%s descriptor=%s "
                     "matcher=%s\n",
    ...
    const int describe_rc =
            detect_rc == 0
                    ? aicore_loma_descriptor_run(descriptor, &image,
                                                 points.keypoints, points.count,
                                                 kWidth, kHeight, &descriptors)
                    : -1;
    const aicore_loma_features features = {
            points.keypoints,        points.count,
            descriptors.descriptors, descriptors.descriptor_dim,
            points.image_width,      points.image_height};
    const int match_rc =
            describe_rc == 0
                    ? aicore_loma_matcher_run(matcher, &features, &features,
                                              &matches, &match_count)
                    : -1;
    ...
    std::printf(
            "{\"suite\":\"loma-default-triplet\",\"keypoints\":%d,"
    ...
    if (!valid) {
        std::fprintf(stderr,
                     "LoMa default model-pack functional gate failed\n");
        return 1;
    }
    return 0;
}
