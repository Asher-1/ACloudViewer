...
bool LoadReference(const char* path, Reference* reference) {
    ...
    if (!stream.read(magic, sizeof(magic)) ||
        ...
        !Read(stream, &reference->descriptor_dim) || !Read(stream, &rgb_bytes) ||
        !Read(stream, &match_count) || version != 1 || reference->width != 784 ||
        reference->height != 784 || reference->count == 0 ||
        reference->descriptor_dim != 128 || match_count == 0 ||
        rgb_bytes != reference->width * reference->height * 3) {
        return false;
    }
    reference->rgb0.resize(rgb_bytes);
    reference->rgb1.resize(rgb_bytes);
    if (!stream.read(reinterpret_cast<char*>(reference->rgb0.data()), reference->rgb0.size()) ||
        !stream.read(reinterpret_cast<char*>(reference->rgb1.data()), reference->rgb1.size())) {
        return false;
    }
    for (uint32_t index = 0; index < match_count; ++index) {
        ...
        int32_t index1 = -1;
        float score = 0.0f;
        if (!Read(stream, &index0) || !Read(stream, &index1) || !Read(stream, &score) ||
            index0 < 0 || index1 < 0 || index0 >= static_cast<int32_t>(reference->count) ||
            index1 >= static_cast<int32_t>(reference->count) || !std::isfinite(score)) {
            return false;
        }
...
bool Extract(aicore_loma_detector_ctx* detector,
    ...
    const aicore_loma_rgb_image image = {rgb.data(), static_cast<int32_t>(reference.width),
    ...
    *descriptions = {};
    ...
    return aicore_loma_descriptor_run(descriptor, &image, points->keypoints, points->count,
                                      image.width, image.height, descriptions) == 0 &&
           descriptions->count == points->count &&
           descriptions->descriptor_dim == static_cast<int32_t>(reference.descriptor_dim);
}
...
int main() {
    ...
    if (detector_model == nullptr || descriptor_model == nullptr || matcher_model == nullptr ||
        fixture == nullptr || detector_model[0] == '\0' || descriptor_model[0] == '\0' ||
        matcher_model[0] == '\0' || fixture[0] == '\0') {
        std::fprintf(stderr, "SKIP: set AICORE_TEST_LOMA_{DETECTOR,DESCRIPTOR,B128}_GGUF "
                             "and AICORE_TEST_LOMA_B128_REFERENCE\n");
        return 77;
    }
    Reference reference;
    if (!LoadReference(fixture, &reference)) {
        std::fprintf(stderr, "invalid real-image LoMa-B128 reference fixture\n");
        return 1;
    }
    ...
    aicore_loma_detector_ctx* detector = aicore_loma_detector_load(detector_model,
                                                                     detector_options);
    aicore_loma_detector_options_free(detector_options);
    aicore_loma_descriptor_options* descriptor_options = aicore_loma_descriptor_options_new();
    aicore_loma_descriptor_options_set_device(descriptor_options, "cpu");
    aicore_loma_descriptor_ctx* descriptor = aicore_loma_descriptor_load(descriptor_model,
                                                                           descriptor_options);
    ...
    aicore_loma_matcher_ctx* matcher = aicore_loma_matcher_load(matcher_model, matcher_options);
    ...
    if (!aicore_loma_detector_is_ready(detector) ||
        ...
        std::fprintf(stderr, "LoMa-B128 load failure: detector=%s descriptor=%s matcher=%s\n",
    ...
    const bool extracted = Extract(detector, descriptor, reference.rgb0, reference, &points0,
                                   &descriptions0) &&
                           Extract(detector, descriptor, reference.rgb1, reference, &points1,
                                   &descriptions1);
    aicore_loma_match* raw_matches = nullptr;
    int32_t match_count = 0;
    int rc = -1;
    if (extracted) {
        const aicore_loma_features image0 = {points0.keypoints, points0.count,
                                             descriptions0.descriptors,
                                             descriptions0.descriptor_dim,
                                             points0.image_width, points0.image_height};
        const aicore_loma_features image1 = {points1.keypoints, points1.count,
                                             descriptions1.descriptors,
                                             descriptions1.descriptor_dim,
                                             points1.image_width, points1.image_height};
        rc = aicore_loma_matcher_run(matcher, &image0, &image1, &raw_matches, &match_count);
    }
    ...
    for (int32_t index = 0; index < match_count; ++index) {
        const auto pair = std::make_pair(raw_matches[index].idx0, raw_matches[index].idx1);
        if (pair.first >= 0 && pair.second >= 0 && std::isfinite(raw_matches[index].score)) {
    ...
    const double precision = predicted.empty() ? 0.0
            : static_cast<double>(true_positive) / predicted.size();
    const double recall = static_cast<double>(true_positive) / reference.matches.size();
    std::printf("{\"suite\":\"loma-b128-real-e2e\",\"expected\":%zu,"
    ...
    if (!extracted || rc != 0 || precision < 0.995 || recall < 0.995 ||
        ...
        std::fprintf(stderr, "LoMa-B128 real-image end-to-end P/R gate failed\n");
        return 1;
    }
    return 0;
}
