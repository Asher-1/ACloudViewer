...
bool LoadReference(const char* path, Reference* reference) {
    ...
    return static_cast<bool>(stream.read(reinterpret_cast<char*>(reference->rgb.data()),
           ...
           static_cast<bool>(stream.read(reinterpret_cast<char*>(reference->descriptors.data()),
...
int main() {
    ...
    if (model == nullptr || fixture == nullptr || model[0] == '\0' || fixture[0] == '\0') {
        std::fprintf(stderr, "SKIP: set AICORE_TEST_LOMA_DESCRIPTOR_GGUF and "
    ...
    aicore_loma_descriptor_options* options = aicore_loma_descriptor_options_new();
    aicore_loma_descriptor_options_set_device(options, "cpu");
    aicore_loma_descriptor_ctx* descriptor = aicore_loma_descriptor_load(model, options);
    ...
    const int rc = aicore_loma_descriptor_run(
            ...
            static_cast<int32_t>(reference.count), static_cast<int32_t>(reference.width),
            static_cast<int32_t>(reference.height), &output);
    if (rc != 0 || output.count != static_cast<int32_t>(reference.count) ||
        output.descriptor_dim != static_cast<int32_t>(reference.descriptor_dim)) {
    ...
    for (size_t index = 0; index < reference.descriptors.size(); ++index) {
        const float diff = output.descriptors[index] - reference.descriptors[index];
    ...
    std::printf("{\"suite\":\"loma-dedode-reference\",\"count\":%u,"
                "\"dim\":%u,\"max_absolute_error\":%.8g,"
                "\"relative_l2\":%.8g}\n",
                reference.count, reference.descriptor_dim, max_absolute_error, relative_l2);
    aicore_loma_described_features_free(&output);
    aicore_loma_descriptor_free(descriptor);
