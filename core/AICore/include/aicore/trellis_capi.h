...
/** Bake progress: fired at each bake stage boundary with a human-readable
 *  stage description and the seconds elapsed since that stage started. */
typedef void (*aicore_trellis_bake_progress_fn)(const char* stage,
                                                double elapsed_s,
                                                void* user);

/** Variant of aicore_trellis_bake_glb with optional stage progress and a
 *  cooperative cancel flag (polled with relaxed semantics at bake stage
