...
enum aicore_trellis_stage {
    AICORE_TRELLIS_STAGE_PREPROCESS   = 0, /* image decode + crop/premultiply/resize */
    AICORE_TRELLIS_STAGE_DINO         = 1, /* conditioning encoder                   */
    AICORE_TRELLIS_STAGE_SS_FLOW      = 2, /* sparse-structure flow sampling (steps) */
    ...
    AICORE_TRELLIS_STAGE_UPSAMPLE     = 7, /* cascade: LR slat -> HR voxel scaffold  */
    AICORE_TRELLIS_STAGE_SLAT_FLOW_HR = 8, /* cascade: 1024 shape-SLAT flow (steps)  */
...
enum aicore_trellis_pipeline_type {
    AICORE_TRELLIS_PIPE_AUTO   = 0, /* cascade if available, else 512 fine, else coarse */
    AICORE_TRELLIS_PIPE_COARSE = 1, /* 64^3 occupancy -> marching cubes preview        */
...
enum aicore_trellis_background_mode {
    AICORE_TRELLIS_BG_AUTO  = 0, /* detect border-connected near-black/near-white */
    AICORE_TRELLIS_BG_KEEP  = 1, /* preserve the decoded alpha exactly             */
    AICORE_TRELLIS_BG_BLACK = 2, /* force removal of border-connected near-black   */
    AICORE_TRELLIS_BG_WHITE = 3  /* force removal of border-connected near-white   */
};
...
typedef void (*aicore_trellis_progress_fn)(void* user, int stage, int step,
                                           int total);
...
AICORE_CAPI void aicore_trellis_options_set_threads(aicore_trellis_options* opts,
                                                    int n_threads);
...
AICORE_CAPI void aicore_trellis_options_set_rmbg_gguf(aicore_trellis_options* opts,
...
AICORE_CAPI void aicore_trellis_options_set_sdpa_exact(aicore_trellis_options* opts,
                                                       int exact);
...
AICORE_CAPI const char* aicore_trellis_last_error(const aicore_trellis_ctx* ctx);
...
AICORE_CAPI aicore_trellis_mesh* aicore_trellis_generate(
        ...
        aicore_trellis_progress_fn progress, void* progress_user,
...
AICORE_CAPI const float* aicore_trellis_mesh_verts(const aicore_trellis_mesh* r);
AICORE_CAPI const float* aicore_trellis_mesh_normals(const aicore_trellis_mesh* r);
...
AICORE_CAPI const float* aicore_trellis_mesh_grid_feats(const aicore_trellis_mesh* r);
AICORE_CAPI const int* aicore_trellis_mesh_grid_coords(const aicore_trellis_mesh* r);
 ...
 *  *out_len. */
AICORE_CAPI uint8_t* aicore_trellis_bake_glb(const float* verts, int n_verts,
