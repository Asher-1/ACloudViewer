...
bool CVTools::FileMappingWriter(const std::string& filename,
    ...
    HANDLE hFileMapping =
            CreateFileMapping(hFile, NULL, PAGE_READWRITE, 0, size, NULL);
    ...
    TCHAR* pbFile =
            (TCHAR*)MapViewOfFile(hFileMapping, FILE_MAP_WRITE, 0, 0, 0);
    if (pbFile == NULL) {
        CVLog::Error("FileMappingWriter: %d", GetLastError());
        UnmapViewOfFile(pbFile);
        return false;
...
bool CVTools::QMappingReader(const std::string& filename,
    ...
    if (!input.open(QIODevice::ReadOnly)) {
        CVLog::Error(QString("[CVTools::QMappingReader] Cannot open file : %1")
                             .arg(filename.c_str()));
        return false;
    }
...
bool CVTools::QMappingWriter(const std::string& filename,
    ...
    if (!file.resize(static_cast<qint64>(length))) {
        CVLog::Error(
                "[CVTools::QMappingWriter] Reserve space error! May have not "
                "enough space avaliable!");
        return false;
    }

    if (!file.open(QIODevice::ReadWrite)) {
        CVLog::Error(QString("[CVTools::QMappingWriter] Cannot open file : %1")
                             .arg(filename.c_str()));
        return false;
    }
    ...
    if (!fptr) {
        CVLog::Error(
                QString("[CVTools::QMappingWriter] Mapping file(%1) failed!")
                        .arg(filename.c_str()));
    ...
    if (!buf) {
        CVLog::Error(
                QString("[CVTools::QMappingWriter] Converting uchar* to char* "
                        "failed!"));
        return false;
    }
