...
//! Internal structure used by
//! DistanceComputationTools::computeCloud2MeshDistances
...
public:
    ...
    ~OctreeAndMeshIntersection() {
        if (perCellTriangleList.isInitialized()) {
            ...
            for (std::size_t i = 0; i < perCellTriangleList.totalCellCount();
                 ++i, ++data) {
                if (*data) delete (*data);
            }
        }
...
int DistanceComputationTools::computeCloud2CloudDistances(
    ...
    if (params.CPSet && params.maxSearchDist > 0) {
        // we can't use a 'max search distance' criterion if the "Closest Point
        // Set" is requested
        assert(false);
        return DISTANCE_COMPUTATION_RESULTS::
    ...
    SOReturnCode soCode = synchronizeOctrees(comparedCloud, referenceCloud,
                                             comparedOctree, referenceOctree,
                                             params.maxSearchDist, progressCb);
    ...
    // we 'enable' a scalar field  (if it is not already done) to store
    // resulting distances
    ...
    // internally we don't use the maxSearchDist parameters as is, but the
    // square of it
    double maxSearchSquareDistd =
            params.maxSearchDist <= 0
                    ? 0
                    : static_cast<double>(params.maxSearchDist) *
    ...
    if (params.CPSet) {
        ...
        if (!params.CPSet->resize(comparedCloud->size())) {
            // not enough memory
            if (comparedOctree && !compOctree) delete comparedOctree;
            if (referenceOctree && !refOctree) delete referenceOctree;
            return DISTANCE_COMPUTATION_RESULTS::ERROR_OUT_OF_MEMORY;
        }
    }

    // by default we reset any former value stored in the 'enabled' scalar field
    const ScalarType resetValue =
            maxSearchSquareDistd <= 0 ? NAN_VALUE : params.maxSearchDist;
    ...
    // specific case: a max search distance has been defined and octrees are
    // totally disjoint
    ...
    // if necessary we try to guess the best octree level for distances
    // computation
    if (params.octreeLevel == 0 &&
        referenceOctree)  // DGM: referenceOctree can be 0 if the input entities
                          // bounding-boxes are disjoint!
    {
        params.octreeLevel =
                comparedOctree->findBestLevelForComparisonWithOctree(
                        referenceOctree);
    }
    ...
    {
        for (int i = 0; i < 3; ++i) {
            if (params.splitDistances[i] &&
                params.splitDistances[i]->currentSize() ==
                        comparedCloud->size()) {
    ...
    void* additionalParameters[] = {
            reinterpret_cast<void*>(referenceCloud),
    ...
    if (comparedOctree->executeFunctionForAllCellsAtLevel(
                params.octreeLevel,
                params.localModel == NO_MODEL
                        ? computeCellHausdorffDistance
                        : computeCellHausdorffDistanceWithLocalModel,
                additionalParameters, params.multiThread, progressCb,
                "Cloud-Cloud Distance", params.maxThreadCount) == 0) {
        // something went wrong
        result = DISTANCE_COMPUTATION_RESULTS::
...
DistanceComputationTools::synchronizeOctrees(
    ...
    if (nA == 0 || nB == 0) return EMPTY_CLOUD;
    ...
    if (maxDist > 0) {
        // we reduce the bounding box to the intersection of both bounding-boxes
        // enlarged by 'maxDist'
        for (unsigned char k = 0; k < 3; k++) {
            minD.u[k] = std::max(minD.u[k],
                                 std::max(minsA.u[k], minsB.u[k]) - maxDist);
            maxD.u[k] = std::min(maxD.u[k],
                                 std::min(maxsA.u[k], maxsB.u[k]) + maxDist);
    ...
    if (needToRecalculateOctreeA) {
        ...
        if (comparedOctree->build(minD, maxD, &minPoints, &maxPoints,
                                  progressCb) < 1) {
            if (octreeACreated) {
                delete comparedOctree;
                comparedOctree = nullptr;
    ...
    if (needToRecalculateOctreeB) {
        ...
        if (referenceOctree->build(minD, maxD, &minPoints, &maxPoints,
                                   progressCb) < 1) {
            if (octreeACreated) {
                delete comparedOctree;
                comparedOctree = nullptr;
    ...
    assert(comparedOctree->getNumberOfProjectedPoints() != 0 &&
           referenceOctree->getNumberOfProjectedPoints() != 0);
    return SYNCHRONIZED;
}
...
bool DistanceComputationTools::computeCellHausdorffDistance(
        const DgmOctree::octreeCell& cell,
        void** additionalParameters,
        NormalizedProgress* nProgress /*=0*/) {
    // additional parameters
    const GenericIndexedCloudPersist* referenceCloud =
            reinterpret_cast<GenericIndexedCloudPersist*>(
                    additionalParameters[0]);
    const DgmOctree* referenceOctree =
            reinterpret_cast<DgmOctree*>(additionalParameters[1]);
    Cloud2CloudDistancesComputationParams* params =
            reinterpret_cast<Cloud2CloudDistancesComputationParams*>(
                    additionalParameters[2]);
    const double* maxSearchSquareDistd =
            reinterpret_cast<double*>(additionalParameters[3]);
    bool computeSplitDistances =
            *reinterpret_cast<bool*>(additionalParameters[4]);
    ...
    // we can already compute the position of the 'equivalent' cell in the
    // reference octree
    referenceOctree->getCellPos(cell.truncatedCode, cell.level, nNSS.cellPos,
                                true);
    // and we deduce its center
    referenceOctree->computeCellCenter(nNSS.cellPos, cell.level,
                                       nNSS.cellCenter);

    // for each point of the current cell (compared octree) we look for its
    // nearest neighbour in the reference cloud
    unsigned pointCount = cell.points->size();
    for (unsigned i = 0; i < pointCount; i++) {
        cell.points->getPoint(i, nNSS.queryPoint);

        if (params->CPSet ||
            referenceCloud->testVisibility(nNSS.queryPoint) ==
                    POINT_VISIBLE)  // to build the closest point set up we must
                                    // process the point whatever its visibility
                                    // is!
        {
            double squareDist =
                    referenceOctree->findTheNearestNeighborStartingFromCell(
                            nNSS);
            if (squareDist >= 0) {
                ...
                if (params->CPSet) {
                    params->CPSet->setPointIndex(
                            cell.points->getPointGlobalIndex(i),
                            nNSS.theNearestPointIndex);
                }
                ...
                if (computeSplitDistances) {
                    ...
                    if (params->splitDistances[0])
                        params->splitDistances[0]->setValue(
                                index, static_cast<ScalarType>(
                                               nNSS.queryPoint.x - P.x));
                    if (params->splitDistances[1])
                        params->splitDistances[1]->setValue(
                                index, static_cast<ScalarType>(
                                               nNSS.queryPoint.y - P.y));
                    if (params->splitDistances[2])
                        params->splitDistances[2]->setValue(
                                index, static_cast<ScalarType>(
                                               nNSS.queryPoint.z - P.z));
                }
            } else {
                assert(!params->CPSet);
            }
...
//  [2] -> (Cloud2CloudDistancesComputationParams*): parameters
//  [3] -> (ScalarType*): max search distance (squared)
bool DistanceComputationTools::computeCellHausdorffDistanceWithLocalModel(
        const DgmOctree::octreeCell& cell,
        void** additionalParameters,
        NormalizedProgress* nProgress /*=0*/) {
    // additional parameters
    ...
    nNSS.theNearestPointIndex = 0;
    nNSS.maxSearchSquareDistd = *maxSearchSquareDistd;
    // we already compute the position of the 'equivalent' cell in the reference
    // octree
    referenceOctree->getCellPos(cell.truncatedCode, cell.level, nNSS.cellPos,
                                true);
    // and we deduce its center
    ...
    // structures for determining the nearest neighbours of the 'nearest
    // neighbour' (to compute the local model) either inside a sphere or the k
    // nearest
    ...
    if (params->useSphericalSearchForLocalModel) {
        nNSS_Model.prepare(
                ...
                cell.parentOctree->getCellSize(cell.level));
    } else {
        ...
        if (params->CPSet ||
            referenceCloud->testVisibility(nNSS.queryPoint) ==
                    POINT_VISIBLE)  // to build the closest point set up we must
                                    ...
                                    // is!
        {
            // first, we look for the nearest point to "_queryPoint" in the
            // reference cloud
            double squareDistToNearestPoint =
                    referenceOctree->findTheNearestNeighborStartingFromCell(
                            nNSS);

            // if it exists
            if (squareDistToNearestPoint >= 0) {
                ScalarType distToNearestPoint =
                        static_cast<ScalarType>(sqrt(squareDistToNearestPoint));

                CCVector3 nearestPoint;
                referenceCloud->getPoint(nNSS.theNearestPointIndex,
                                         nearestPoint);
                ...
                if (params->reuseExistingLocalModels) {
                    ...
                    for (std::vector<const LocalModel*>::const_iterator it =
                                 models.begin();
                         it != models.end(); ++it) {
                        // we take the first model that 'includes' the nearest
                        // point
                        if (((*it)->getCenter() - nearestPoint).norm2() <=
                            (*it)->getSquareSize()) {
                            lm = *it;
                            break;
                        }
                ...
                if (!lm) {
                    ...
                    // update cell pos information (as the nearestPoint may not
                    // be inside the same cell as the actual query point!)
                    {
                        bool inbounds = false;
                        Tuple3i cellPos;
                        referenceOctree->getTheCellPosWhichIncludesThePoint(
                                &nearestPoint, cellPos, cell.level, inbounds);
                        // if the cell is different or the structure has not yet
                        // been initialized, we reset it!
                        if (cellPos.x != nNSS_Model.cellPos.x ||
                            cellPos.y != nNSS_Model.cellPos.y ||
                            cellPos.z != nNSS_Model.cellPos.z) {
                            nNSS_Model.cellPos = cellPos;
                            referenceOctree->computeCellCenter(
                                    nNSS_Model.cellPos, nNSS_Model.level,
                                    nNSS_Model.cellCenter);
                            assert(inbounds);
                            nNSS_Model.minimalCellsSetToVisit.clear();
                            nNSS_Model.pointsInNeighbourhood.clear();
                            nNSS_Model.alreadyVisitedNeighbourhoodSize =
                                    inbounds ? 0 : 1;
                    ...
                    if (params->useSphericalSearchForLocalModel) {
                        ...
                        // 'reuseExistingLocalModels' optimization warning:
                        // there may be more points at the end of
                        // nNSS.pointsInNeighbourhood than the actual nearest
                        // neighbors (kNN)!
                        kNN = referenceOctree
                                      ->findNeighborsInASphereStartingFromCell(
                                              nNSS_Model,
                                              static_cast<PointCoordinateType>(
                                                      params->radiusForLocalModel),
                    ...
                    if (kNN >= CV_LOCAL_MODEL_MIN_SIZE[params->localModel]) {
                        DgmOctreeReferenceCloud neighboursCloud(
                                &nNSS_Model.pointsInNeighbourhood, kNN);
                        Neighbourhood Z(&neighboursCloud);

                        // Neighbours are sorted, so the farthest is at the end.
                        // It also gives us an approximation of the model 'size'
                        const double& maxSquareDist =
                                nNSS_Model.pointsInNeighbourhood[kNN - 1]
                                        .squareDistd;
                        if (maxSquareDist >
                            0)  // DGM: with duplicate points, all neighbors can
                                // be at the same place :(
                        {
                            lm = LocalModel::New(
                                    params->localModel, Z, nearestPoint,
                                    static_cast<PointCoordinateType>(
                                            maxSquareDist));
                            if (lm && params->reuseExistingLocalModels) {
                                // we add the model to the 'existing models'
                                // list
                                try {
                                    models.push_back(lm);
                                } catch (const std::bad_alloc&) {
                ...
                if (lm) {
                    ...
                    ScalarType distToModel =
                            lm->computeDistanceFromModelToPoint(
                                    &nNSS.queryPoint,
                                    computeSplitDistances ? &nearestModelPoint
                                                          : nullptr);

                    // we take the best estimation between the nearest neighbor
                    // and the model! this way we only reduce any potential
                    // noise (that would be due to sampling) instead of 'adding'
                    // noise if the model is badly shaped
                    ...
                    if (computeSplitDistances) {
                        ...
                        if (params->splitDistances[0])
                            params->splitDistances[0]->setValue(
                                    index,
                                    static_cast<ScalarType>(nNSS.queryPoint.x -
                                                            nearestPoint.x));
                        if (params->splitDistances[1])
                            params->splitDistances[1]->setValue(
                                    index,
                                    static_cast<ScalarType>(nNSS.queryPoint.y -
                                                            nearestPoint.y));
                        if (params->splitDistances[2])
                            params->splitDistances[2]->setValue(
                                    index,
                                    static_cast<ScalarType>(nNSS.queryPoint.z -
                                                            nearestPoint.z));
                    }
                } else {
                    distPt = distToNearestPoint;
                ...
                distPt = static_cast<ScalarType>(
                        sqrt(nNSS.maxSearchSquareDistd));
            }
...
// DistanceComputationTools::computeCloud2MeshDistances
struct CellToTest {
    // Warning: put the non aligned members (< 4 bytes) at the end to avoid too
    // much alignment padding!
...
int DistanceComputationTools::intersectMeshWithOctree(
    ...
    for (unsigned n = 0; n < numberOfTriangles; ++n) {
        ...
        if (GreaterThanEpsilon(AB.norm2()) && GreaterThanEpsilon(BC.norm2()) &&
            GreaterThanEpsilon(CA.norm2())) {
            Tuple3i cellPos[3];
            octree->getTheCellPosWhichIncludesThePoint(triPoints[0], cellPos[0],
                                                       octreeLevel);
            octree->getTheCellPosWhichIncludesThePoint(triPoints[1], cellPos[1],
                                                       octreeLevel);
            octree->getTheCellPosWhichIncludesThePoint(triPoints[2], cellPos[2],
                                                       octreeLevel);
            ...
            for (int k = 0; k < 3; k++) {
                minPos.u[k] =
                        std::min(cellPos[0].u[k],
                                 std::min(cellPos[1].u[k], cellPos[2].u[k]));
                maxPos.u[k] =
                        std::max(cellPos[0].u[k],
            ...
            _currentCell->level =
                    octreeLevel -
                    (maxSize > 1 ? static_cast<unsigned char>(ceil(
                                           log(static_cast<double>(maxSize)) /
                                           LOG_2))
                                 : 0);
            ...
            while (cellsToTestCount != 0) {
                ...
                if (_currentCell->level == octreeLevel) {
                    ...
                    if (CCMiscTools::TriBoxOverlap(AB, halfCellDimensions,
                                                   triPoints)) {
                        if ((currentCellPos.x >=
                                     intersection->minFillIndexes.x &&
                             currentCellPos.x <=
                                     intersection->maxFillIndexes.x) &&
                            (currentCellPos.y >=
                                     intersection->minFillIndexes.y &&
                             currentCellPos.y <=
                                     intersection->maxFillIndexes.y) &&
                            (currentCellPos.z >=
                                     intersection->minFillIndexes.z &&
                             currentCellPos.z <=
                                     intersection->maxFillIndexes.z)) {
                            Tuple3i cellPos = currentCellPos -
                                              intersection->minFillIndexes;

                            if (intersection->perCellTriangleList
                                        .isInitialized()) {
                                TriangleList*& triList =
                                        intersection->perCellTriangleList
                                                .getValue(cellPos);
                                if (!triList) {
                                ...
                                // add the triangle to the current 'intersecting
                                // triangles' list
                                triList->push(n);
                            }

                            if (intersection->distanceTransform) {
                                intersection->distanceTransform->setValue(
                                        cellPos, 1);
                            }
                        }
                    }
                    ...
                    // compute the position of each cell 'neighbors' relatively
                    // to the triangle (3*3*3 = 27, including the cell itself)
                    char pointsPosition[27];
                    {
                        char* _pointsPosition = pointsPosition;
                        for (int i = 0; i < 3; ++i) {
                            AB.x = distanceToOctreeMinBorder.x +
                                   static_cast<PointCoordinateType>(
                                           currentCellPos.x +
                                           i * halfCellSize) *
                                           cellLength;
                            for (int j = 0; j < 3; ++j) {
                                AB.y = distanceToOctreeMinBorder.y +
                                       static_cast<PointCoordinateType>(
                                               currentCellPos.y +
                                               j * halfCellSize) *
                                               cellLength;
                                for (int k = 0; k < 3; ++k) {
                                    AB.z = distanceToOctreeMinBorder.z +
                                           static_cast<PointCoordinateType>(
                                                   currentCellPos.z +
                                                   k * halfCellSize) *
                                                   cellLength;
                    ...
                    // we look at the position of the 8 sub-cubes relatively to
                    // the triangle
                    for (int i = 0; i < 2; ++i) {
                        _newCell->pos.x = currentCellPos.x + i * halfCellSize;
                        // quick test to determine if the cube is potentially
                        // intersecting the triangle's bbox
                        if (static_cast<int>(_newCell->pos.x) + halfCellSize >=
                                    minPos.x &&
                                ...
                                if (static_cast<int>(_newCell->pos.y) +
                                    ...
                                    for (int k = 0; k < 2; ++k) {
                                        _newCell->pos.z = currentCellPos.z +
                                                          k * halfCellSize;
                                        if (static_cast<int>(_newCell->pos.z) +
                                                            halfCellSize >=
                                                    minPos.z &&
                                            static_cast<int>(_newCell->pos.z) <=
                                                    maxPos.z) {
                                            ...
                                            // if all the sub-cube vertices are
                                            // not on the same side, then the
                                            // triangle may intersect the cell
                                            if (abs(sum) < 8) {
                                                // we make newCell point on next
                                                // cell in array (we copy
...
bool ComparePointsAndTriangles(
        ReferenceCloud& Yk,
    ...
    while (trianglesToTestCount != 0) {
        ...
        mesh->getTriangleVertices(trianglesToTest[--trianglesToTestCount],
                                  tri.A, tri.B, tri.C);
        ...
        if (params.signedDistances) {
            ...
            for (unsigned j = 0; j < remainingPoints; ++j) {
                ...
                ScalarType dPTri =
                        DistanceComputationTools::computePoint2TriangleDistance(
                                Yk.getPoint(j), &tri, true, _nearestPoint);
                // keep it if it's smaller
                ScalarType min_d = Yk.getPointScalarValue(j);
                if (!ScalarField::ValidValue(min_d) ||
                    min_d * min_d > dPTri * dPTri) {
                    Yk.setPointScalarValue(j,
                                           params.flipNormals ? -dPTri : dPTri);
                    if (params.CPSet) {
                        // Closest Point Set: save the nearest point as well
                        assert(_nearestPoint);
                        *const_cast<CCVector3*>(params.CPSet->getPoint(
                                Yk.getPointGlobalIndex(j))) = *_nearestPoint;
                    }
                }
            }
        } else  // squared distances
        {
            for (unsigned j = 0; j < remainingPoints; ++j) {
                ...
                ScalarType dPTri =
                        DistanceComputationTools::computePoint2TriangleDistance(
                                Yk.getPoint(j), &tri, false, _nearestPoint);
                // keep it if it's smaller
                ScalarType min_d = Yk.getPointScalarValue(j);
    ...
    // we can 'remove' all the eligible points at the current neighborhood
    // radius
    if (firstComparisonDone) {
        ...
        for (unsigned j = 0; j < remainingPoints; ++j) {
            ...
            if (dPTri <= eligibleDist * eligibleDist) {
                ...
                // and do the same for the 'minDists' array! (see
                // ReferenceCloud::removeCurrentPointGlobalIndex)
...
int ComputeMaxNeighborhoodLength(ScalarType maxSearchDist,
    ...
    return static_cast<int>(
            ceil(maxSearchDist / cellSize +
                 static_cast<ScalarType>((sqrt(2.0) - 1.0) / 2)));
}
...
static cloudViewer::DistanceComputationTools::
        Cloud2MeshDistancesComputationParams s_params_MT;
...
void cloudMeshDistCellFunc_MT(const DgmOctree::IndexAndCode& desc) {
    ...
    s_octree_MT->getPointsInCellByCellIndex(&Yk, desc.theIndex,
                                            s_params_MT.octreeLevel);
    ...
    // get cell pos
    Tuple3i startPos;
    s_octree_MT->getCellPos(desc.theCode, s_params_MT.octreeLevel, startPos,
                            true);

    // get the distance to the nearest and farthest boundaries
    Tuple3i signedDistToLowerBorder, signedDistToUpperBorder;
    signedDistToLowerBorder = startPos - s_intersection_MT->minFillIndexes;
    signedDistToUpperBorder = s_intersection_MT->maxFillIndexes - startPos;

    Tuple3i minDistToGridBoundaries, maxDistToGridBoundaries;
    for (unsigned k = 0; k < 3; ++k) {
        minDistToGridBoundaries.u[k] =
                std::max(std::max(-signedDistToLowerBorder.u[k], 0),
                         std::max(0, -signedDistToUpperBorder.u[k]));
        maxDistToGridBoundaries.u[k] =
                std::max(std::max(signedDistToLowerBorder.u[k], 0),
                         std::max(0, signedDistToUpperBorder.u[k]));
    }
    int minDistToBoundaries = std::max(
            minDistToGridBoundaries.x,
            std::max(minDistToGridBoundaries.y, minDistToGridBoundaries.z));
    int maxDistToBoundaries = std::max(
            maxDistToGridBoundaries.x,
            std::max(maxDistToGridBoundaries.y, maxDistToGridBoundaries.z));
    int maxIntDist = maxDistToBoundaries;

    if (s_params_MT.maxSearchDist > 0) {
        // no need to look farther than 'maxNeighbourhoodLength'
        int maxNeighbourhoodLength = ComputeMaxNeighborhoodLength(
                s_params_MT.maxSearchDist,
                s_octree_MT->getCellSize(s_params_MT.octreeLevel));
    ...
    s_octree_MT->computeCellCenter(startPos, s_params_MT.octreeLevel,
                                   cellCenter);
    ...
    // octree cell size
    const PointCoordinateType& cellLength =
            s_octree_MT->getCellSize(s_params_MT.octreeLevel);
    ...
    for (unsigned j = 0; j < remainingPoints; ++j) {
        ...
        // distance du bord le plus proche = taille de la cellule - distance la
        // plus grande par rapport au centre de la cellule
        minDists[j] = DgmOctree::ComputeMinDistanceToCellBorder(
                *tempPt, cellLength, cellCenter);
        Yk.forwardIterator();
    }

    // let's find the nearest triangles for each point in the neighborhood 'Yk'
    ScalarType maxRadius = 0;
    for (int dist = minDistToBoundaries;
         remainingPoints != 0 && dist <= maxIntDist;
         ++dist, maxRadius += static_cast<ScalarType>(cellLength)) {
            ...
            for (int j = -c; j <= d; j++) {
                ...
                if (imax || abs(j) == dist) {
                    ...
                    for (int k = -e; k <= f; k++) {
                        ...
                        TriangleList* triList =
                                s_intersection_MT->perCellTriangleList.getValue(
                                        cellPos);
                        if (triList) {
                            if (trianglesToTestCount + triList->indexes.size() >
                                trianglesToTestCapacity) {
                                trianglesToTestCapacity = std::max(
                                        trianglesToTestCount +
                                                triList->indexes.size(),
                                        2 * trianglesToTestCount);
                                trianglesToTest.resize(trianglesToTestCapacity);
                            }
                            // let's test all the triangles that intersect this
                            // cell
                            for (unsigned p = 0; p < triList->indexes.size();
                                 ++p) {
                                if (bitArray) {
                                    const unsigned indexTri =
                                            triList->indexes[p];
                                    // if the triangles has not been processed
                                    // yet
                                    ...
                                    trianglesToTest[trianglesToTestCount++] =
                                            triList->indexes[p];
                                }
                            }
                        }
        ...
        ComparePointsAndTriangles(Yk, remainingPoints, s_intersection_MT->mesh,
                                  trianglesToTest, trianglesToTestCount,
                                  minDists, maxRadius, s_params_MT);
    }
...
int DistanceComputationTools::computeCloud2MeshDistanceWithOctree(
    ...
    assert(!params.signedDistances ||
           !intersection->distanceTransform);  // signed distances are not
                                               // compatible with Distance
                                               // Transform acceleration
    assert(!params.multiThread ||
           params.maxSearchDist <= 0);  // maxSearchDist is not compatible with
                                        // parallel processing
...
#endif
    {
        ...
        PointCoordinateType cellLength =
                octree->getCellSize(params.octreeLevel);

        // get the cell indexes at level "octreeLevel"
        DgmOctree::cellsContainer cellCodesAndIndexes;
        if (!octree->getCellCodesAndIndexes(params.octreeLevel,
                                            cellCodesAndIndexes, true)) {
            // not enough memory
            return DISTANCE_COMPUTATION_RESULTS::
        ...
        unsigned numberOfCells =
                static_cast<unsigned>(cellCodesAndIndexes.size());
        bool boundedSearch = (params.maxSearchDist > 0);

        DgmOctree::cellsContainer::const_iterator pCodeAndIndex =
                cellCodesAndIndexes.begin();
        ...
        if (intersection->distanceTransform) {
            ...
            for (unsigned i = 0; i < numberOfCells; ++i, ++pCodeAndIndex) {
                octree->getPointsInCellByCellIndex(&Yk, pCodeAndIndex->theIndex,
                                                   params.octreeLevel);

                // get the cell pos
                Tuple3i cellPos;
                octree->getCellPos(pCodeAndIndex->theCode, params.octreeLevel,
                                   cellPos, true);
                cellPos -= intersection->minFillIndexes;

                // get the Distance Transform distance
                unsigned squareDist =
                        intersection->distanceTransform->getValue(cellPos);

                // assign the distance to all points inside this cell
                ScalarType maxRadius =
                        sqrt(static_cast<ScalarType>(squareDist)) * cellLength;
        ...
        // otherwise we have to compute the distance from each point to its
        // nearest triangle
        ...
        if (progressCb) {
            if (progressCb->textCanBeEdited()) {
                ...
                progressCb->setInfo(buffer);
                progressCb->setMethodTitle(params.signedDistances
        ...
        if (boundedSearch) {
            maxNeighbourhoodLength = ComputeMaxNeighborhoodLength(
                    params.maxSearchDist, cellLength);
        }

        // for each cell
        for (unsigned cellIndex = 1; cellIndex <= numberOfCells;
             ++cellIndex, ++pCodeAndIndex)  // cellIndex = unique ID for the
                                            // current cell
        {
            octree->getPointsInCellByCellIndex(&Yk, pCodeAndIndex->theIndex,
                                               params.octreeLevel);
            ...
            // get the distance to the nearest and farthest boundaries
            Tuple3i signedDistToLowerBorder, signedDistToUpperBorder;
            signedDistToLowerBorder = startPos - intersection->minFillIndexes;
            signedDistToUpperBorder = intersection->maxFillIndexes - startPos;
            ...
            int minDistToBoundaries =
                    std::max(minDistToGridBoundaries.x,
                             std::max(minDistToGridBoundaries.y,
                                      minDistToGridBoundaries.z));
            int maxDistToBoundaries =
                    std::max(maxDistToGridBoundaries.x,
                             std::max(maxDistToGridBoundaries.y,
                                      maxDistToGridBoundaries.z));
            int maxIntDist = maxDistToBoundaries;
            ...
            // for each point, we pre-compute its distance to the nearest cell
            // border (will be handy later)
            for (unsigned j = 0; j < remainingPoints; ++j) {
                const CCVector3* tempPt = Yk.getPointPersistentPtr(j);
                minDists[j] = static_cast<ScalarType>(
                        DgmOctree::ComputeMinDistanceToCellBorder(
                                *tempPt, cellLength, cellCenter));
            }
            ...
            // let's find the nearest triangles for each point in the
            // neighborhood 'Yk'
            ScalarType maxRadius = 0;
            for (int dist = minDistToBoundaries;
                 dist <= maxIntDist && remainingPoints != 0;
                 ++dist, maxRadius += static_cast<ScalarType>(cellLength)) {
                // test the neighbor cells at distance = 'dist'
                // a,b,c,d,e,f are the extents of this neighborhood
                // for the 6 main directions -X,+X,-Y,+Y,-Z,+Z
                                    ...
                                    if (trianglesToTestCount +
                                        ...
                                        trianglesToTestCapacity = std::max(
                                                trianglesToTestCount +
                                                        triList->indexes.size(),
                                                2 * trianglesToTestCount);
                                        trianglesToTest.resize(
                                                trianglesToTestCapacity);
                                    }
                                    // let's test all the triangles that
                                    // intersect this cell
                                    for (std::size_t p = 0;
                                         p < triList->indexes.size(); ++p) {
                                        if (!processTriangles.empty()) {
                                            unsigned indexTri =
                                                    triList->indexes[p];
                                            // if the triangles has not been
                                            // processed yet
                                            if (processTriangles[indexTri] !=
                                                cellIndex) {
                                                trianglesToTest
                                                        [trianglesToTestCount++] =
                                                                indexTri;
                                                processTriangles[indexTri] =
                                                        cellIndex;
                                            }
                                        } else {
                                            trianglesToTest
                ...
                ComparePointsAndTriangles(Yk, remainingPoints, mesh,
                                          trianglesToTest, trianglesToTestCount,
                                          minDists, maxRadius, params);
            }
...
inline void applySqrtToPointDist(const CCVector3& aPoint,
...
static int ComputeNeighborhood2MeshDistancesWithOctree(
        OctreeAndMeshIntersection* intersection,
    ...
    // get the distance to the nearest and farthest boundaries
    Tuple3i localCellPos = cellPos - intersection->minFillIndexes;
    Tuple3i signedDistToLowerBorder = localCellPos;
    Tuple3i signedDistToUpperBorder = intersection->maxFillIndexes -
                                      intersection->minFillIndexes -
                                      localCellPos;
    ...
    octree->computeCellCenter(cellPos, params.octreeLevel, cellCenter);

    // express 'startPos' relatively to the inner grid borders
    Tuple3i startPos = localCellPos;
    ...
    {
        // not enough memory
        return DistanceComputationTools::DISTANCE_COMPUTATION_RESULTS::
                ERROR_OUT_OF_MEMORY;
    }
    ...
    for (unsigned j = 0; j < remainingPoints; ++j) {
        const CCVector3* tempPt = Yk.getPointPersistentPtr(j);
        minDists[j] = static_cast<ScalarType>(
                DgmOctree::ComputeMinDistanceToCellBorder(*tempPt, cellSize,
                                                          cellCenter));
    }
    ...
    const Tuple3ui& gridSize = intersection->perCellTriangleList.size();

    // let's find the nearest triangles for each point in the neighborhood 'Yk'
    ScalarType maxRadius = 0;
    for (int dist = minDistToBoundaries;
         dist <= maxDistToBoundaries && remainingPoints != 0;
         ++dist, maxRadius += static_cast<ScalarType>(cellSize)) {
        // test the neighbor cells at distance = 'dist'
        // a,b,c,d,e,f are the extents of this neighborhood
        // for the 6 main directions -X,+X,-Y,+Y,-Z,+Z
        int a = std::min(dist, signedDistToLowerBorder.x);
        ...
        for (int i = -a; i <= b; i++) {
            ...
            if (localCellPos.x < 0 ||
                static_cast<unsigned>(localCellPos.x) >= gridSize.x)
                continue;

            for (int j = -c; j <= d; j++) {
                localCellPos.y = startPos.y + j;
                if (localCellPos.y < 0 ||
                    static_cast<unsigned>(localCellPos.y) >= gridSize.y)
                    continue;
                ...
                if (imax || std::abs(j) == dist) {
                    ...
                    for (int k = -e; k <= f; k++) {
                        ...
                        if (localCellPos.z < 0 ||
                            static_cast<unsigned>(localCellPos.z) >= gridSize.z)
                            continue;
                        TriangleList* triList =
                                intersection->perCellTriangleList.getValue(
                                        localCellPos);
                        if (triList) {
                            if (ttt.trianglesToTestCount +
                                        triList->indexes.size() >
                                ttt.trianglesToTestCapacity) {
                                ttt.trianglesToTestCapacity = std::max(
                                        ttt.trianglesToTestCount +
                                                triList->indexes.size(),
                                        2 * ttt.trianglesToTestCount);
                                ttt.trianglesToTest.resize(
                                        ttt.trianglesToTestCapacity);
                            }
                            // let's test all the triangles that intersect this
                            // cell
                            for (unsigned indexTri : triList->indexes) {
                                if (!ttt.processTriangles.empty()) {
                                    // if the triangles has not been processed
                                    // yet
                                    if (ttt.processTriangles[indexTri] !=
                                        cellIndex) {
                                        ttt.trianglesToTest
                                                [ttt.trianglesToTestCount++] =
                                                indexTri;
                                        ttt.processTriangles[indexTri] =
                                                cellIndex;
                                    }
                                } else {
                                    ttt.trianglesToTest
                        ...
                        if (triList) {
                            ...
                            for (unsigned triIndex : triList->indexes) {
                                if (!ttt.processTriangles.empty()) {
                                    // if the triangles has not been processed
                                    // yet
                                    if (ttt.processTriangles[triIndex] !=
                                        cellIndex) {
                                        ttt.trianglesToTest
                                                [ttt.trianglesToTestCount++] =
                                                triIndex;
                                        ttt.processTriangles[triIndex] =
                                                cellIndex;
                                    }
                                } else {
                                    ttt.trianglesToTest
        ...
        if (false == ComparePointsAndTriangles(
                             Yk, remainingPoints, intersection->mesh,
                             ttt.trianglesToTest, ttt.trianglesToTestCount,
                             minDists, maxRadius, params)) {
            return DistanceComputationTools::DISTANCE_COMPUTATION_RESULTS::
                    ERROR_OUT_OF_MEMORY;
        }
...
int DistanceComputationTools::computePoint2MeshDistancesWithOctree(
        const CCVector3& P,
        ScalarType& distance,
        OctreeAndMeshIntersection* intersection,
        Cloud2MeshDistancesComputationParams& params) {
    ...
    if (boundedSearch) {
        ...
        maxNeighbourhoodLength =
                ComputeMaxNeighborhoodLength(params.maxSearchDist, cellLength);
    }

    // get cell pos (global coordinates)
    Tuple3i cellPos;
    octree->getTheCellPosWhichIncludesThePoint(&P, cellPos, params.octreeLevel);

    int result = ComputeNeighborhood2MeshDistancesWithOctree(
            intersection, params, Yk, 1, cellPos, ttt, boundedSearch,
            maxNeighbourhoodLength);
...
int DistanceComputationTools::computeCloud2MeshDistancesWithOctree(
    ...
    if (!intersection->distanceTransform &&
        !intersection->perCellTriangleList.isInitialized()) {
        assert(false);
        return DISTANCE_COMPUTATION_RESULTS::
                ERROR_INVALID_OCTREE_AND_MESH_INTERSECTION;
    }
    ...
    if (params.CPSet) {
        ...
        // reserve memory for an associated scalar field (the nearest triangle
        // index)
    ...
    if (params.multiThread) {
        ...
        if (params.maxThreadCount == 1) {
            // if only one thread should/cloud be used, the direct approach is
            // more efficient
            params.multiThread = false;
        }
    }
#endif  // CV_CORE_LIB_USES_QT_CONCURRENT
...
#endif  // ENABLE_CLOUD2MESH_DIST_MT
    {
        ...
        if (intersection->distanceTransform) {
            ...
            for (unsigned i = 0; i < numberOfCells; ++i, ++pCodeAndIndex) {
                ...
                // assign the distance to all points inside this cell
                ScalarType maxRadius =
                        sqrt(static_cast<ScalarType>(squareDist)) * cellSize;
        ...
        for (unsigned cellIndex = 1; cellIndex <= numberOfCells;
             ++cellIndex, ++pCodeAndIndex)  // cellIndex = unique ID for the
                                            // current cell
        {
            if (!octree->getPointsInCellByCellIndex(
                        &Yk, pCodeAndIndex->theIndex, params.octreeLevel)) {
                return DISTANCE_COMPUTATION_RESULTS::
            ...
            int result = ComputeNeighborhood2MeshDistancesWithOctree(
                    intersection, params, Yk, cellIndex, cellPos, ttt,
                    boundedSearch, maxNeighbourhoodLength);
...
#if defined(CV_CORE_LIB_USES_QT_CONCURRENT)
            QCoreApplication::processEvents(
                    QEventLoop::EventLoopExec);  // to allow the GUI to refresh
                                                 // itself
#endif
            if (progressCb && !nProgress.oneStep()) {
                // process cancelled by the user
        ...
        DgmOctree::cellsContainer cellsDescs;
        if (!octree->getCellCodesAndIndexes(params.octreeLevel, cellsDescs,
                                            true)) {
            return DISTANCE_COMPUTATION_RESULTS::
                    ERROR_GET_CELL_CODES_AND_INDEXES_FAILURE;
        }
        ...
        // Note: Multi-threading implementation would go here
        // For now, we'll use single-threaded approach
        // TODO: Implement multi-threading support if needed
...
int DistanceComputationTools::computeCloud2MeshDistances(
    ...
    if (params.signedDistances) {
        // signed distances are incompatible with approximate distances (with
        // Distance Transform)
        params.useDistanceMap = false;
    }
    if (params.CPSet) {
        // Closest Point Set determination is incompatible with distance map
        // approximation and max search distance
        params.useDistanceMap = false;
        params.maxSearchDist = 0;
    }

    // compute the (cubical) bounding box that contains both the cloud and the
    // mehs BBs
    ...
    if (rebuildTheOctree) {
        // build the octree
        if (octree->build(minCubifiedBB, maxCubifiedBB, &cloudMinBB,
                          &cloudMaxBB, progressCb) <= 0) {
    ...
    // we deduce the grid cell size very simply (as the bbox has been
    // "cubified")
    PointCoordinateType cellSize =
            (maxCubifiedBB.x - minCubifiedBB.x) / (1 << params.octreeLevel);
    // we compute grid occupancy ... and we deduce the grid dimensions
    Tuple3ui gridSize;
    {
        for (unsigned char k = 0; k < 3; ++k) {
            intersection.minFillIndexes.u[k] = static_cast<int>(
                    floor((minBB.u[k] - minCubifiedBB.u[k]) / cellSize));
            intersection.maxFillIndexes.u[k] = static_cast<int>(
                    floor((maxBB.u[k] - minCubifiedBB.u[k]) / cellSize));
            gridSize.u[k] =
                    static_cast<unsigned>(intersection.maxFillIndexes.u[k] -
                                          intersection.minFillIndexes.u[k] + 1);
        }
    }

    // if the user (or the current cloud/mesh configuration) requires that we
    // use a Distance Transform
    if (params.useDistanceMap) {
        intersection.distanceTransform = new SaitoSquaredDistanceTransform;
        if (!intersection.distanceTransform ||
            !intersection.distanceTransform->initGrid(gridSize)) {
            return DISTANCE_COMPUTATION_RESULTS::
                    ERROR_INIT_DISTANCE_TRANSFORM_GRID_FAILURE;
        }
        params.multiThread = false;  // not necessary/supported
    } else {
        // we need to build the list of triangles intersecting each cell of the
        // 3D grid
        if (!intersection.perCellTriangleList.init(gridSize.x, gridSize.y,
                                                   gridSize.z, 0, 0)) {
            return DISTANCE_COMPUTATION_RESULTS::
    ...
    int result = intersectMeshWithOctree(&intersection, params.octreeLevel,
                                         progressCb);
    if (result < 0) {
        return result;
    }
    ...
    result = computeCloud2MeshDistanceWithOctree(&intersection, params,
                                                 progressCb);

    // don't forget to compute the square root of the (squared) unsigned
    // distances
    if (result == DISTANCE_COMPUTATION_RESULTS::SUCCESS &&
        !params.signedDistances && !params.useDistanceMap) {
...
ScalarType DistanceComputationTools::computePoint2TriangleDistance(
        const CCVector3* P,
    ...
    if (signedDist) {
        ...
        // we test the sign of the cross product of the triangle normal and the
        // vector AP
...
ScalarType DistanceComputationTools::computePoint2PlaneDistance(
        const CCVector3* P, const PointCoordinateType* planeEquation) {
    // point to plane distance: d = (a0*x+a1*y+a2*z - a3) / sqrt(a0^2+a1^2+a2^2)
    assert(std::abs(CCVector3::vnorm(planeEquation) - PC_ONE) <=
           std::numeric_limits<PointCoordinateType>::epsilon());

    return static_cast<ScalarType>((
            CCVector3::vdot(P->u, planeEquation) -
            planeEquation[3]) /*/CCVector3::vnorm(planeEquation)*/);  // norm
                                                                      // == 1.0!
}

ScalarType DistanceComputationTools::computePoint2LineSegmentDistSquared(
        const CCVector3* p, const CCVector3* start, const CCVector3* end) {
    assert(p && start && end);
...
// This algorithm is a modification of the distance computation between a point
// and a cone from Barbier & Galin's Fast Distance Computation Between a Point
// and Cylinders, Cones, Line Swept Spheres and Cone-Spheres. The modifications
// from the paper are to compute the closest distance when the point is interior
// to the cone. http://liris.cnrs.fr/Documents/Liris-1297.pdf
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if (x <= 0)  // Below the bottom point
        {
            if (yy < rr1) {
                ...
                if (!solutionType) {
                    y = sqrt(yy) - coneR1;
                    d = sqrt((y * y) + xx);  // Below the bottom point not
                                             // within larger disk radius
                } else {
                    d = 2;  // Works
                }
            }
        ...
        } else  // Above the bottom point
        {
            if (yy < rr2)  // within smaller disk radius
            {
                ...
                {
                    if (!solutionType) {
                        d = x - axisLength;  // Above the top point within top
                                             // disk radius
                    } else {
                        d = 3;
                    }
                } else  // inside cone within smaller disk
                {
                    if (!solutionType) {
                        y = sqrt(yy) - coneR1;
                        ry = y * side[0] -
                             x * side[1];  // rotated y value (distance from the
                                           // coneside axis)
                        d = std::min(
                                axisLength - x,
                                x);  // determine whether closer to either radii
                        d = std::min(d, static_cast<double>(
                                                std::abs(ry)));  // or to side
                        d = -d;  // negative inside
                    } else {
                        d = 4;
                    }
            ...
            } else  // Outside smaller disk radius
            {
                ...
                {
                    rx = y * side[1] +
                         x * side[0];  // rotated x value (distance along the
                                       // coneside axis)
                    if (rx < 0) {
                        if (!solutionType) {
                            d = sqrt(y * y + xx);
                        ...
                        if (rx > coneLength) {
                            ...
                            if (!solutionType) {
                                ...
                                if (ry < 0) {  // point is interior to the cone
                                    d = std::min(axisLength - x,
                                                 x);  // determine whether
                                                      // closer to either radii
                                    d = std::min(d,
...
// This algorithm is a modification of the distance computation between a point
// and a cylinder from Barbier & Galin's Fast Distance Computation Between a
// Point and Cylinders, Cones, Line Swept Spheres and Cone-Spheres. The
// modifications from the paper are to compute the closest distance when the
// point is interior to the capped cylinder.
// http://liris.cnrs.fr/Documents/Liris-1297.pdf
// solutionType 1 = exterior to the cylinder and within the bounds of the axis
// solutionType 2 = interior to the cylinder and either closer to an end-cap or
// the cylinder wall solutionType 3 = beyond the bounds of the cylinder's axis
// and radius solutionType 4 = beyond the bounds of the cylinder's axis but
// within the bounds of it's radius
int DistanceComputationTools::computeCloud2CylinderEquation(
    ...
    double cylinderRadius2 =
            static_cast<double>(cylinderRadius) * cylinderRadius;
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if (x <= h) {
            if (yy >= cylinderRadius2) {
                if (!solutionType) {
                    d = sqrt(yy) -
                        cylinderRadius;  // exterior to the cylinder and within
                ...
                if (!solutionType) {
                    d = -std::min(
                            std::abs(sqrt(yy) - cylinderRadius),
                            std::abs(h - x));  // interior to the cylinder and
                                               // either closer to an end-cap or
                                               // the cylinder wall
                } else {
                    d = 2;
                }
            ...
            if (yy >= cylinderRadius2) {
                if (!solutionType) {
                    y = sqrt(yy);
                    d = sqrt(
                            ((y - cylinderRadius) * (y - cylinderRadius)) +
                            ((x - h) * (x - h)));  // beyond the bounds of the
                                                   // cylinder's axis and radius
                } else {
                    d = 3;
                }
            } else {
                if (!solutionType) {
                    d = x - h;  // beyond the bounds of the cylinder's axis but
                                // within the bounds of it's radius
                } else {
                    d = 4;
                }
...
int DistanceComputationTools::computeCloud2SphereEquation(
        GenericIndexedCloudPersist* cloud,
...
int DistanceComputationTools::computeCloud2PlaneEquation(
        GenericIndexedCloudPersist* cloud,
    ...
    // point to plane distance: d = std::abs(a0*x+a1*y+a2*z-a3) /
    // sqrt(a0^2+a1^2+a2^2) <-- "norm" but the norm should always be equal
    // to 1.0!
    PointCoordinateType norm2 = CCVector3::vnorm2(planeEquation);
    assert(std::abs(sqrt(norm2) - PC_ONE) <=
           std::numeric_limits<PointCoordinateType>::epsilon());
    ...
    for (unsigned i = 0; i < count; ++i) {
        const CCVector3* P = cloud->getPoint(i);
        double d = static_cast<double>(CCVector3::vdotd(P->u, planeEquation) -
                                       planeEquation[3]);
        if (signedDistances) {
            cloud->setPointScalarValue(i, static_cast<ScalarType>(d));
        } else {
...
int DistanceComputationTools::computeCloud2BoxEquation(
        GenericIndexedCloudPersist* cloud,
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        CCVector3 p_inBoxCoords(pointCenterDifference.dot(u),
                                pointCenterDifference.dot(v),
                                pointCenterDifference.dot(w));
        bool insideBox = false;
        if (p_inBoxCoords.x > -hu && p_inBoxCoords.x < hu &&
            p_inBoxCoords.y > -hv && p_inBoxCoords.y < hv &&
            p_inBoxCoords.z > -hw && p_inBoxCoords.z < hw) {
            insideBox = true;
        }
...
int DistanceComputationTools::computeCloud2DiscEquation(
        GenericIndexedCloudPersist* cloud,
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if (rProj <= discRadius) {
            ...
            CCVector3 pEdge =
                    discCenter + discRadius * (pProj - discCenter) /
                                         rProj;  // safe as rProj can not be
                                                 // null at this point
            d = (*P - pEdge).normd();  // The distance is the distance between
                                       // the point and the border of the disc
...
int DistanceComputationTools::computeCloud2PolylineEquation(
        GenericIndexedCloudPersist* cloud,
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        for (unsigned j = 0; j < polyline->size() - 1; j++) {
            ...
            // Rejection test
            if (((startXMinusASq >= distSq) && (endXMinusASq >= distSq) &&
                 GreaterThanEpsilon(startXMinusA * endXMinusA)) ||
                ((startYMinusBSq >= distSq) && (endYMinusBSq >= distSq) &&
                 GreaterThanEpsilon(startYMinusB * endYMinusB)) ||
                ((startZMinusCSq >= distSq) && (endZMinusCSq >= distSq) &&
                 GreaterThanEpsilon(startZMinusC * endZMinusC))) {
                continue;
            }
            if (std::isnan(distSq)) {
                distSq = computePoint2LineSegmentDistSquared(p, start, end);
            } else {
                distSq = std::min(distSq, computePoint2LineSegmentDistSquared(
                                                  p, start, end));
            }
        }
        d = sqrt(distSq);
...
ScalarType DistanceComputationTools::computeCloud2PlaneDistanceRMS(
    ...
    // to 1.0!
    PointCoordinateType norm2 = CCVector3::vnorm2(planeEquation);
    if (LessThanEpsilon(norm2)) return NAN_VALUE;
    assert(std::abs(sqrt(norm2) - PC_ONE) <=
           std::numeric_limits<PointCoordinateType>::epsilon());
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        double d =
                static_cast<double>(CCVector3::vdot(P->u, planeEquation) -
                                    planeEquation[3]) /*/norm*/;  // norm == 1.0
...
ScalarType DistanceComputationTools::ComputeCloud2PlaneRobustMax(
        GenericCloud* cloud,
        const PointCoordinateType* planeEquation,
        float percent) {
    assert(cloud && planeEquation);
    ...
    std::size_t tailSize =
            static_cast<std::size_t>(ceil(static_cast<float>(count) * percent));
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        PointCoordinateType d =
                std::abs(CCVector3::vdot(P->u, planeEquation) -
                         planeEquation[3]) /*/norm*/;  // norm == 1.0
        ...
        if (maxPos != 0) {
            ...
            for (std::size_t j = 0; j < maxPos; ++j)
                if (tail[j] < tail[maxIndex]) maxIndex = j;
            // and put it to the back!
            if (maxPos != maxIndex) std::swap(tail[maxIndex], tail[maxPos]);
        }
    }
...
ScalarType DistanceComputationTools::ComputeCloud2PlaneMaxDistance(
        GenericCloud* cloud, const PointCoordinateType* planeEquation) {
    assert(cloud && planeEquation);
...
ScalarType DistanceComputationTools::ComputeCloud2PlaneDistance(
    ...
    switch (measureType) {
        case RMS:
            return cloudViewer::DistanceComputationTools::
                    computeCloud2PlaneDistanceRMS(cloud, planeEquation);

        case MAX_DIST_68_PERCENT:
            return cloudViewer::DistanceComputationTools::
                    ComputeCloud2PlaneRobustMax(cloud, planeEquation, 0.32f);
        case MAX_DIST_95_PERCENT:
            return cloudViewer::DistanceComputationTools::
                    ComputeCloud2PlaneRobustMax(cloud, planeEquation, 0.05f);
        case MAX_DIST_99_PERCENT:
            return cloudViewer::DistanceComputationTools::
                    ComputeCloud2PlaneRobustMax(cloud, planeEquation, 0.01f);

        case MAX_DIST:
            return cloudViewer::DistanceComputationTools::
                    ComputeCloud2PlaneMaxDistance(cloud, planeEquation);
...
bool DistanceComputationTools::computeGeodesicDistances(
        GenericIndexedCloudPersist* cloud,
    ...
    if (n == 0 || seedPointIndex >= n) return false;
    ...
    Tuple3i cellPos;
    octree->getTheCellPosWhichIncludesThePoint(cloud->getPoint(seedPointIndex),
                                               cellPos, octreeLevel);
    fm.setSeedCell(cellPos);

    bool result = false;
    if (fm.propagate() >= 0) result = fm.setPropagationTimingsAsDistances();
...
int DistanceComputationTools::diff(GenericIndexedCloudPersist* comparedCloud,
    ...
    unsigned nA = comparedCloud->size();
    if (nA == 0) return DISTANCE_COMPUTATION_RESULTS::ERROR_EMPTY_COMPAREDCLOUD;
    ...
    int result = computeCloud2CloudDistances(comparedCloud, referenceCloud,
                                             params, progressCb);
    ...
    for (unsigned i = 0; i < nA; ++i) {
        ...
        comparedCloud->setPointScalarValue(
                i, ScalarField::ValidValue(dA) && ScalarField::ValidValue(dB)
                           ? dA - dB
                           : NAN_VALUE);
    }
...
int DistanceComputationTools::computeApproxCloud2CloudDistance(
    ...
    if (synchronizeOctrees(comparedCloud, referenceCloud, octreeA, octreeB,
                           maxSearchDist, progressCb) != SYNCHRONIZED) {
        return DISTANCE_COMPUTATION_RESULTS::ERROR_SYNCHRONIZE_OCTREES_FAILURE;
    }
    ...
    if (dtGrid.initGrid(boxSize)) {
        ...
        ScalarType cellSize =
                static_cast<ScalarType>(octreeA->getCellSize(octreeLevel));
        ...
        while (!theIndexes.empty()) {
            ...
            Tuple3i cellPos;
            octreeA->getCellPos(octreeA->getCellCode(theIndex), octreeLevel,
                                cellPos, false);
            cellPos -= minIndexes;
...
PointCoordinateType DistanceComputationTools::ComputeSquareDistToSegment(
        const CCVector2& P,
        const CCVector2& A,
        const CCVector2& B,
