...
public:
    ...
    CV_LOCAL_MODEL_TYPES getType() const override { return LS; }

    // inherited from LocalModel
    ScalarType computeDistanceFromModelToPoint(
            const CCVector3* P,
            CCVector3* nearestPoint = nullptr) const override {
        ScalarType dist =
...
public:
    //! Constructor
    DelaunayLocalModel(GenericMesh* tri,
                       const CCVector3& center,
                       PointCoordinateType squaredRadius)
    ...
    // inherited from LocalModel
    CV_LOCAL_MODEL_TYPES getType() const override { return TRI; }
            ...
            for (unsigned i = 0; i < numberOfTriangles; ++i) {
                ...
                ScalarType dist2 =
                        DistanceComputationTools::computePoint2TriangleDistance(
                                P, tri, false,
                                nearestPoint ? &triNearestPoint : nullptr);
                if (dist2 < minDist2 || i == 0) {
                    // keep track of the smallest distance
                    minDist2 = dist2;
                    if (nearestPoint) *nearestPoint = triNearestPoint;
                }
            }
        }
...
public:
    ...
    // inherited from LocalModel
    CV_LOCAL_MODEL_TYPES getType() const override { return QUADRIC; }
    ...
    ScalarType computeDistanceFromModelToPoint(
        ...
        PointCoordinateType z =
                m_eq[0] + m_eq[1] * P.u[m_X] + m_eq[2] * P.u[m_Y] +
                m_eq[3] * P.u[m_X] * P.u[m_X] + m_eq[4] * P.u[m_X] * P.u[m_Y] +
                m_eq[5] * P.u[m_Y] * P.u[m_Y];
...
LocalModel::LocalModel(const CCVector3& center,
                       PointCoordinateType squaredRadius)
    : m_modelCenter(center), m_squaredRadius(squaredRadius) {}

LocalModel* LocalModel::New(CV_LOCAL_MODEL_TYPES type,
    ...
    switch (type) {
        ...
        case TRI: {
            ...
            GenericMesh* tri = subset.triangulateOnPlane(
                    Neighbourhood::DUPLICATE_VERTICES,
                    Neighbourhood::IGNORE_MAX_EDGE_LENGTH,
                    errorStr);  //'subset' is potentially associated to a
                                // volatile ReferenceCloud, so we must duplicate
                                // vertices!
            if (tri) {
        ...
        case QUADRIC: {
            ...
            if (eq) {
                return new QuadricLocalModel(
                        eq, dims.x, dims.y, dims.z,
                        *subset.getGravityCenter(),  // should be ok as the
                                                     // quadric computation
                                                     // succeeded!
