...
static double Gamma_cc(double x) {
    ...
    double gr = g[24];
    for (int k = 23; k >= 0; k--) gr = gr * z + g[k];
    double ga = 1.0 / (gr * z);
    if (std::abs(x) > 1.0) {
        ga *= r;
        if (x < 0.0) {
...
WeibullDistribution::WeibullDistribution() { setParameters(0, 0, 0); }
...
bool WeibullDistribution::setParameters(ScalarType a,
    ...
    if (m_a > 0.0 && m_b >= 0.0) {
        ...
        m_sigma2 = static_cast<ScalarType>(
                Gamma_cc(1.0 + 2.0 / m_a) * (m_b * m_b) - (m_mu * m_mu));
...
bool WeibullDistribution::computeParameters(const ScalarContainer& values) {
    ...
    for (ScalarType s : values) {
        if (!ScalarField::ValidValue(s)) continue;
...
double WeibullDistribution::computePfromZero(ScalarType x) const {
    return (x <= m_valueShift
                    ? 0.0
                    : 1.0 - exp(-pow(
                                    static_cast<double>(x - m_valueShift) / m_b,
                                    static_cast<double>(m_a))));
}

double WeibullDistribution::computeP(ScalarType x1, ScalarType x2) const {
    if (x1 < m_valueShift) x1 = m_valueShift;
    if (x2 < m_valueShift) return 0;
    // pi = computeP(minV+(ScalarType(k)+0.5)*step)*step;
    //...instead we take the sampling into account and then integrate
    return exp(-pow(static_cast<double>(x1 - m_valueShift) / m_b,
                    static_cast<double>(m_a))) -
           exp(-pow(static_cast<double>(x2 - m_valueShift) / m_b,
                    static_cast<double>(m_a)));
}
...
double WeibullDistribution::ComputeG(const ScalarContainer& values,
    ...
    if (zeroValues) {
        ...
        const double v_a =
                pow(ZERO_TOLERANCE_D / valueRange, static_cast<double>(r));
        s += ln_v;
        q += v_a * zeroValues;
        p += ln_v * v_a;
        counter += zeroValues;
...
double WeibullDistribution::FindGRoot(const ScalarContainer& values,
    ...
    while (GreaterThanEpsilon(std::abs(v) *
                              100))  // DGM: *100 ?! (can't remember why ;)
    {
        r = (aMin + aMax) / 2;
        double old_v = v;
        v = ComputeG(values, r, valueShift, valueRange);

        if (LessThanEpsilon(std::abs(old_v - v))) return r;
...
double WeibullDistribution::computeChi2Dist(const GenericCloud* cloud,
    ...
    unsigned numberOfElements =
            ScalarFieldTools::countScalarFieldValidValues(cloud);

    if (numberOfElements == 0) return -1.0;

    if (numberOfClasses < 1 ||
        numberOfClasses * numberOfClasses > numberOfElements)
        return -1.0;
    else if (numberOfClasses == 1)
        return 0.0;

    if (!setChi2ClassesPositions(numberOfClasses)) return -1.0;
    ...
    if (!histo) {
        ...
        if (!histo) return -1.0;  // not enough memory
    }
    ...
    for (unsigned i = 0; i < n; ++i) {
        ...
        if (ScalarField::ValidValue(V)) {
            ...
            for (; j < numberOfClasses - 1; ++j)
                if (V < chi2ClassesPositions[j]) break;
    ...
    if (histo && !inputHisto) delete[] histo;
    histo = nullptr;
...
bool WeibullDistribution::setChi2ClassesPositions(unsigned numberOfClasses) {
    ...
    if (!isValid() || numberOfClasses < 2) return false;
    ...
    // we create "numberOfClasses" equiprobable classes (for all of themn
    // nPi>=sqrt(n) if n>=4)
    ...
    for (unsigned i = 1; i < numberOfClasses; ++i) {
        chi2ClassesPositions[i - 1] =
                m_b *
                static_cast<ScalarType>(pow(-log(1.0 - currentArea), invA));
...
void WeibullDistribution::setValueShift(ScalarType vs) {
    if (vs != m_valueShift) setValid(false);
...
double WeibullDistribution::computeSkewness() const {
    if (!isValid() || std::abs(m_a) < std::numeric_limits<double>::epsilon() ||
        m_sigma2 < std::numeric_limits<double>::epsilon()) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    return (Gamma_cc(1.0 + 3.0 / m_a) * (m_b * m_b * m_b) -
            3.0 * m_mu * m_sigma2 - (m_mu * m_mu * m_mu)) /
