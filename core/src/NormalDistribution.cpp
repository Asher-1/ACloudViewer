...
bool NormalDistribution::getParameters(ScalarType& mu,
...
double NormalDistribution::computeP(ScalarType x1, ScalarType x2) const {
    return 0.5 * (ErrorFunction::erf(static_cast<double>(x2 - m_mu) /
                                     sqrt(2 * m_sigma2)) -
                  ErrorFunction::erf(static_cast<double>(x1 - m_mu) /
                                     sqrt(2 * m_sigma2)));
}

double NormalDistribution::computePfromZero(ScalarType x) const {
    return 0.5 * (ErrorFunction::erf(static_cast<double>(x - m_mu) /
                                     sqrt(2 * m_sigma2)) +
                  1.0);
}
...
bool NormalDistribution::computeParameters(const GenericCloud* cloud) {
    ...
    return setParameters(static_cast<ScalarType>(mean),
                         static_cast<ScalarType>(stddev2));
}
...
bool NormalDistribution::computeRobustParameters(const ScalarContainer& values,
                                                 double nSigma) {
    if (!computeParameters(values)) return false;
...
double NormalDistribution::computeChi2Dist(const GenericCloud* cloud,
    ...
    int* _histo = histo;
    if (!_histo) _histo = new int[numberOfClasses];
    if (!_histo) return -1.0;
    ...
    for (unsigned i = 0; i < n; ++i) {
        ...
        if (ScalarField::ValidValue(V)) {
            ...
            for (; j < numberOfClasses - 1; ++j)
                if (V < m_chi2ClassesPositions[j]) break;
