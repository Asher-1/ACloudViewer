...
unsigned CCShareable::GetAliveCount() { return s_aliveSharedObjects.size(); }
...
void CCShareable::link() { ++m_linkCount; }
...
CCShareable::~CCShareable() {
    ...
    for (it = s_aliveSharedObjects.begin(); it != s_aliveSharedObjects.end();
         ++it) {
        if (*it == this) {
