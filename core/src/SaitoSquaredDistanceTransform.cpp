...
bool SaitoSquaredDistanceTransform::EDT_1D(GridElement* slice,
                                           std::size_t r,
                                           std::size_t c) {
    ...
    // 2nd step: horizontal scan
    {
        ...
        for (std::size_t i = 0; i < c; ++i) {
            // fill buffer with column values
            {
                ...
                for (std::size_t j = 0; j < r; ++j, pt += c) colData[j] = *pt;
            }
...
bool SaitoSquaredDistanceTransform::SDT_3D(
        Grid3D<GridElement>& grid, GenericProgressCallback* progressCb /*=0*/) {
    const Tuple3ui& gridSize = grid.size();
    std::size_t r = gridSize.y;
    std::size_t c = gridSize.x;
    ...
    std::size_t diag = static_cast<std::size_t>(
            ceil(sqrt(static_cast<double>(r * r + c * c + p * p))) - 1);
    ...
    const GridElement maxDistance =
            std::numeric_limits<GridElement>::max() -
            static_cast<GridElement>(r * r + c * c + p * p) - 1;
    ...
    if (progressCb) {
        if (progressCb->textCanBeEdited()) {
            ...
            char buffer[256];
            sprintf(buffer, "Box: [%u x %u x %u]", gridSize.x, gridSize.y,
                    gridSize.z);
    ...
    // Now, for each pixel, compute final distance by searching along Z
    // direction
    std::size_t rc = r * c;
    std::vector<GridElement> colData;
    try {
    ...
    for (std::size_t j = 0; j < r; ++j, data += c) {
        for (std::size_t i = 0; i < c; ++i) {
            ...
            for (std::size_t k = 0; k < p; ++k, pt += rc) colData[k] = *pt;
            ...
            for (std::size_t k = 1; k < p; ++k, pt += rc) {
                ...
                if (colData[k] > buffer + 1) {
                    ...
                    if (k + b + 1 > p) b = static_cast<GridElement>(p - 1 - k);

                    GridElement* npt = pt + a * rc;
                    for (GridElement l = a; l <= b; ++l) {
                        GridElement m = buffer + sq[l + 1];
                        if (colData[k + l] <= m) break;  // go to next plane k
                        if (m < *npt) *npt = m;
                        npt += rc;
                    }
                    a = b;
            ...
            for (std::size_t k = p - 2; k != static_cast<std::size_t>(-1);
                 --k, pt -= rc) {
                if (a != 0) --a;
                if (colData[k] > buffer + 1) {
                    GridElement b = (colData[k] - buffer - 1) / 2;
                    if (k < b) b = static_cast<GridElement>(k);

                    GridElement* npt = pt - a * rc;
                    for (GridElement l = a; l <= b; ++l) {
                        GridElement m = buffer + sq[l + 1];
                        if (colData[k - l] <= m) break;  // go to next column k
                        if (m < *npt) *npt = m;
                        npt -= rc;
                    }
                    a = b;
