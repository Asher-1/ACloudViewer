...
public:
    ...
    inline int fetchAndAddRelaxed(int add) {
...
void NormalizedProgress::scale(unsigned totalSteps,
    ...
    if (progressCallback) {
        ...
        if (totalSteps >= 2 * totalPercentage) {
            m_step = static_cast<unsigned>(
                    ceil(static_cast<float>(totalSteps) / totalPercentage));
            assert(m_step != 0 && m_step < totalSteps);
            m_percentAdd =
                    static_cast<float>(totalPercentage) / (totalSteps / m_step);
        } else {
            m_step = 1;
...
bool NormalizedProgress::oneStep() {
    ...
    unsigned currentCount =
            static_cast<unsigned>(m_counter->fetchAndAddRelaxed(1)) + 1;
...
bool NormalizedProgress::steps(unsigned n) {
    ...
    unsigned currentCount =
            static_cast<unsigned>(m_counter->fetchAndAddRelaxed(n)) + n;
