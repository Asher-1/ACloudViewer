...
std::unique_ptr<Engine> Engine::load(const std::string& path, int n_threads){
    ...
    if (!e->ml_.load(path)) { DA_ERR("engine: load failed"); return nullptr; }
    e->be_.set_n_threads(n_threads > 0 ? n_threads : 1);
    if (!e->ml_.offload_weights(e->be_)) { DA_ERR("engine: offload failed"); return nullptr; }
...
std::unique_ptr<Engine> Engine::load_nested(const std::string& anyview_gguf,
    ...
    if (!e) { DA_ERR("engine: anyview load failed"); return nullptr; }
    e->metric_ml_.reset(new ModelLoader());
    e->metric_be_.reset(new Backend());
    if (!e->metric_ml_->load(metric_gguf)) { DA_ERR("engine: metric load failed"); return nullptr; }
    e->metric_be_->set_n_threads(n_threads > 0 ? n_threads : 1);
    if (!e->metric_ml_->offload_weights(*e->metric_be_)) { DA_ERR("engine: metric offload failed"); return nullptr; }
    return e;
}
...
bool Engine::depth(const std::string& image_path, std::vector<float>& depth_out,
    ...
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth: load image failed"); return false; }
    return depth_image(img, depth_out, conf_out, H, W);
}
bool Engine::depth_image(const Image& img, std::vector<float>& depth_out,
                         std::vector<float>& conf_out, int& H, int& W){
    Preprocessed p;
    if (!preprocess(img, ml_.config(), p)) { DA_ERR("depth: preprocess failed"); return false; }
    H = p.H; W = p.W;
    std::vector<std::vector<float>> feats;
    if (!backbone_features(p.chw, H, W, feats)) { DA_ERR("depth: backbone failed"); return false; }
...
bool Engine::depth_mono(const Image& img, std::vector<float>& depth_out,
    ...
    Preprocessed p;
    if (!preprocess_real(img, ml_.config(), p)) { DA_ERR("depth_mono: preprocess_real failed"); return false; }
    H = p.H; W = p.W;
    DinoBackbone bb(ml_, be_);
    std::vector<std::vector<float>> feats, cam_tokens;
    if (!bb.forward(p.chw, H, W, feats, cam_tokens)) { DA_ERR("depth_mono: backbone failed"); return false; }
    DptHead head(ml_, be_);
...
bool Engine::depth_mono_path(const std::string& image_path, std::vector<float>& depth_out,
                             std::vector<float>& sky_out, int& H, int& W){
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth_mono: load image failed"); return false; }
...
bool Engine::depth_relative(const Image& img, std::vector<float>& depth_out, int& H, int& W){
    Preprocessed p;
    if (!preprocess_real(img, ml_.config(), p)) { DA_ERR("depth_relative: preprocess_real failed"); return false; }
    H = p.H; W = p.W;
    DinoBackbone bb(ml_, be_);
    std::vector<std::vector<float>> feats, cam_tokens;
    if (!bb.forward(p.chw, H, W, feats, cam_tokens)) { DA_ERR("depth_relative: backbone failed"); return false; }
    DptHead head(ml_, be_);
...
bool Engine::depth_relative_path(const std::string& image_path, std::vector<float>& depth_out,
                                 int& H, int& W){
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth_relative: load image failed"); return false; }
    return depth_relative(img, depth_out, H, W);
}
bool Engine::depth_native(const std::string& image_path, std::vector<float>& depth_out,
                          std::vector<float>& conf_out, int& H, int& W){
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth_native: load image failed"); return false; }
...
bool Engine::depth_native_unfused(const Image& img, std::vector<float>& depth_out,
    ...
    Preprocessed p;
    auto t0 = now();
    if (!preprocess_real(img, ml_.config(), p)) { DA_ERR("depth_native: preprocess_real failed"); return false; }
    H = p.H; W = p.W;
    auto t1 = now();
    std::vector<std::vector<float>> feats;
    if (!backbone_features(p.chw, H, W, feats)) { DA_ERR("depth_native: backbone failed"); return false; }
    auto t2 = now();
    DptHead head(ml_, be_);
...
bool Engine::depth_native_fused(const Image& img, std::vector<float>& depth_out,
    ...
    auto t2 = now();
    if (!ok) { DA_ERR("depth_native_fused: compute failed"); return false; }
    const size_t HW = (size_t)H * W;
    if (logits.size() != (size_t)output_dim * HW) { DA_ERR("depth_native_fused: bad logits size"); return false; }
...
bool Engine::depth_pose_native(const Image& img, std::vector<float>& depth, std::vector<float>& conf,
    ...
    Preprocessed p;
    if (!preprocess_real(img, ml_.config(), p)) { DA_ERR("depth_pose_native: preprocess_real failed"); return false; }
    H = p.H; W = p.W;
    DinoBackbone bb(ml_, be_);
    std::vector<std::vector<float>> feats, cam_tokens;
    if (!bb.forward(p.chw, H, W, feats, cam_tokens)) { DA_ERR("depth_pose_native: backbone failed"); return false; }
    DptHead head(ml_, be_);
    if (!head.depth(feats, H, W, depth, conf)) { DA_ERR("depth_pose_native: depth head failed"); return false; }
    if (cam_tokens.size() < 4) { DA_ERR("depth_pose_native: missing layer-11 cam token"); return false; }
    CamPose cam(ml_, be_);
    std::array<float,9> pe;
    if (!cam.pose(cam_tokens[3], H, W, pe, ext, intr)) { DA_ERR("depth_pose_native: cam pose failed"); return false; }
    return true;
}
bool Engine::depth_pose_native_path(const std::string& image_path, std::vector<float>& depth,
    ...
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth_pose_native: load image failed"); return false; }
...
bool Engine::depth_pose_rays_native(const Image& img, std::vector<float>& depth, std::vector<float>& conf,
                                    std::array<float,12>& ext, std::array<float,9>& intr, int& H, int& W){
    if (!has_aux()) {
        DA_WARN("depth_pose_rays: this GGUF has no auxiliary ray head (rebuild with --with-aux for --ray-pose)");
        return false;
    }
    Preprocessed p;
    if (!preprocess_real(img, ml_.config(), p)) { DA_ERR("depth_pose_rays: preprocess_real failed"); return false; }
    H = p.H; W = p.W;
    DinoBackbone bb(ml_, be_);
    std::vector<std::vector<float>> feats, cam_tokens;
    if (!bb.forward(p.chw, H, W, feats, cam_tokens)) { DA_ERR("depth_pose_rays: backbone failed"); return false; }
    DptHead head(ml_, be_);
    if (!head.depth(feats, H, W, depth, conf)) { DA_ERR("depth_pose_rays: depth head failed"); return false; }
...
bool Engine::depth_pose_rays_native_path(const std::string& image_path, std::vector<float>& depth,
                                         std::vector<float>& conf, std::array<float,12>& ext,
                                         std::array<float,9>& intr, int& H, int& W){
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth_pose_rays: load image failed"); return false; }
...
bool Engine::depth_pose(const Image& img, std::vector<float>& depth, std::vector<float>& conf,
                        std::array<float,12>& ext, std::array<float,9>& intr, int& H, int& W){
    Preprocessed p;
    if (!preprocess(img, ml_.config(), p)) { DA_ERR("depth_pose: preprocess failed"); return false; }
    H = p.H; W = p.W;
    // Run the backbone ONCE; reuse feats for depth and cam_tokens[3] for pose.
    DinoBackbone bb(ml_, be_);
    std::vector<std::vector<float>> feats, cam_tokens;
    if (!bb.forward(p.chw, H, W, feats, cam_tokens)) { DA_ERR("depth_pose: backbone failed"); return false; }
    DptHead head(ml_, be_);
    if (!head.depth(feats, H, W, depth, conf)) { DA_ERR("depth_pose: depth head failed"); return false; }
    if (cam_tokens.size() < 4) { DA_ERR("depth_pose: missing layer-11 cam token"); return false; }
    CamPose cam(ml_, be_);
    std::array<float,9> pe;
    if (!cam.pose(cam_tokens[3], H, W, pe, ext, intr)) { DA_ERR("depth_pose: cam pose failed"); return false; }
    return true;
}
bool Engine::depth_pose_multi(const std::vector<Image>& imgs, std::vector<ViewResult>& out, int& H, int& W){
    out.clear();
    if (imgs.empty()) { DA_ERR("depth_pose_multi: no images"); return false; }
    ...
    for (size_t i = 0; i < imgs.size(); ++i){
        Preprocessed p;
        if (!preprocess_real(imgs[i], ml_.config(), p)) { DA_ERR("depth_pose_multi: preprocess_real failed"); return false; }
        if (i == 0) { H = p.H; W = p.W; }
        else if (p.H != H || p.W != W) { DA_ERR("depth_pose_multi: views differ in H,W"); return false; }
    ...
    if (!bb.forward_mv(views_chw, H, W, feats, cam_tokens)) { DA_ERR("depth_pose_multi: backbone failed"); return false; }
    if (feats.size() < 4 || cam_tokens.size() < 4) { DA_ERR("depth_pose_multi: missing out layers"); return false; }
    out.resize(S);
    for (int v = 0; v < S; ++v){
        ViewResult& r = out[v];
        ...
        DptHead head(ml_, be_);
        if (!head.depth(feats4_v, H, W, r.depth, r.conf)) { DA_ERR("depth_pose_multi: depth head failed"); return false; }
        CamPose cam(ml_, be_);
        std::array<float,9> pe;
        if (!cam.pose(cam_tokens[3][v], H, W, pe, r.ext, r.intr)) { DA_ERR("depth_pose_multi: cam pose failed"); return false; }
    }
    return true;
}
bool Engine::reconstruct(const Image& img, Gaussians& g, int& H, int& W){
    Preprocessed p;
    if (!preprocess_real(img, ml_.config(), p)) { DA_ERR("reconstruct: preprocess_real failed"); return false; }
    H = p.H; W = p.W;
    // One backbone pass: feats[4] feed depth + gs_head; cam_tokens[3] feeds pose.
    DinoBackbone bb(ml_, be_);
    std::vector<std::vector<float>> feats, cam_tokens;
    if (!bb.forward(p.chw, H, W, feats, cam_tokens)) { DA_ERR("reconstruct: backbone failed"); return false; }
    if (feats.size() < 4 || cam_tokens.size() < 4) { DA_ERR("reconstruct: missing out layers"); return false; }
    DptHead head(ml_, be_);
    std::vector<float> depth, conf;
    if (!head.depth(feats, H, W, depth, conf)) { DA_ERR("reconstruct: depth head failed"); return false; }
    CamPose cam(ml_, be_);
    std::array<float,9> pe; std::array<float,12> ext; std::array<float,9> intr;
    if (!cam.pose(cam_tokens[3], H, W, pe, ext, intr)) { DA_ERR("reconstruct: cam pose failed"); return false; }
    GsHead gs(ml_, be_);
    std::vector<float> raw_gs, gs_conf;
    if (!gs.raw_gaussians(feats, p.chw, H, W, raw_gs, gs_conf)) { DA_ERR("reconstruct: gs_head failed"); return false; }
    GsAdapter ad;
    if (!ad.build(raw_gs, depth, gs_conf, ext, intr, H, W, g)) { DA_ERR("reconstruct: gs_adapter failed"); return false; }
    return true;
}
bool Engine::reconstruct_path(const std::string& image_path, Gaussians& g, int& H, int& W){
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("reconstruct: load image failed"); return false; }
...
bool Engine::depth_pose_path(const std::string& image_path, std::vector<float>& depth, std::vector<float>& conf,
                             std::array<float,12>& ext, std::array<float,9>& intr, int& H, int& W){
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth_pose: load image failed"); return false; }
    return depth_pose(img, depth, conf, ext, intr, H, W);
}
bool Engine::depth_metric(const Image& img, NestedOut& out, int& H, int& W){
    if (!metric_ml_ || !metric_be_) { DA_ERR("depth_metric: engine not loaded via load_nested"); return false; }
    // Both branches consume the SAME preprocessed input x (da3.py NestedDepthAnything3Net).
    Preprocessed p;
    if (!preprocess_real(img, ml_.config(), p)) { DA_ERR("depth_metric: preprocess_real failed"); return false; }
    H = p.H; W = p.W;
    ...
    AnyviewOut any;
    {
        DinoBackbone bb(ml_, be_);
        std::vector<std::vector<float>> feats, cam_tokens;
        if (!bb.forward(p.chw, H, W, feats, cam_tokens)) { DA_ERR("depth_metric: anyview backbone failed"); return false; }
        if (cam_tokens.size() < 4) { DA_ERR("depth_metric: missing cam token"); return false; }
        DptHead head(ml_, be_);
        if (!head.depth(feats, H, W, any.depth, any.depth_conf)) { DA_ERR("depth_metric: anyview depth head failed"); return false; }
        CamPose cam(ml_, be_);
        std::array<float,9> pe;
        if (!cam.pose(cam_tokens[3], H, W, pe, any.extrinsics, any.intrinsics)) { DA_ERR("depth_metric: cam pose failed"); return false; }
    }
    ...
    MetricOut metric;
    {
        ...
        if (!bb.forward(p.chw, H, W, feats_m, cams_m)) { DA_ERR("depth_metric: metric backbone failed"); return false; }
        DptHead head(*metric_ml_, *metric_be_);
        if (!head.depth_sky(feats_m, H, W, metric.depth, metric.sky)) { DA_ERR("depth_metric: metric depth_sky failed"); return false; }
    }
...
bool Engine::depth_metric_path(const std::string& image_path, NestedOut& out, int& H, int& W){
    Image img; if (!load_image_rgb(image_path, img)) { DA_ERR("depth_metric: load image failed"); return false; }
    return depth_metric(img, out, H, W);
}
bool Engine::depth_metric_multi(const std::vector<Image>& imgs, std::vector<NestedOut>& out,
                                int& H, int& W){
    if (!metric_ml_ || !metric_be_) { DA_ERR("depth_metric_multi: engine not loaded via load_nested"); return false; }
    out.clear();
    if (imgs.empty()) { DA_ERR("depth_metric_multi: no images"); return false; }

    std::vector<ViewResult> any_views;
    if (!depth_pose_multi(imgs, any_views, H, W)) { return false; }
...
bool Engine::export_colmap_multi(const std::vector<Image>& imgs,
                                 ...
                                 const std::string& dir, bool binary) {
    ...
    if (is_nested()) {
        std::vector<ViewResult> any_views;
        if (!depth_pose_multi(imgs, any_views, H, W)) {
            return false;
        }
        if (static_cast<int>(any_views.size()) != N) {
        ...
        for (int v = 0; v < N; ++v) {
            Preprocessed p;
            if (!preprocess_real(imgs[v], ml_.config(), p)) {
                DA_ERR("export_colmap_multi: preprocess failed");
                return false;
            }
            MetricOut metric;
            {
                ...
                std::vector<std::vector<float>> feats_m, cams_m;
                if (!bb.forward(p.chw, H, W, feats_m, cams_m)) {
                    DA_ERR("export_colmap_multi: metric backbone failed");
                    return false;
                }
                DptHead head(*metric_ml_, *metric_be_);
                if (!head.depth_sky(feats_m, H, W, metric.depth, metric.sky)) {
                    DA_ERR("export_colmap_multi: metric depth_sky failed");
                    return false;
                }
            }
            NestedAligner::process_mono_sky(metric.depth, metric.sky);
            ...
            any.extrinsics = any_views[v].ext;
            any.intrinsics = any_views[v].intr;
            NestedOut nested = NestedAligner::align(any, metric, H, W);

            std::memcpy(depth_all.data() + static_cast<size_t>(v) * plane,
                        nested.depth.data(), plane * sizeof(float));
            if (!any_views[v].conf.empty()) {
                std::memcpy(conf_all.data() + static_cast<size_t>(v) * plane,
                            any_views[v].conf.data(), plane * sizeof(float));
            }
            K[static_cast<size_t>(v)] = nested.intrinsics;
            E[static_cast<size_t>(v)] = ext12_to_ext16(nested.extrinsics);
        }
    } else {
    ...
    for (int v = 0; v < N; ++v) {
        Preprocessed pp;
        if (!preprocess_real(imgs[v], ml_.config(), pp, &rgb_bufs[static_cast<size_t>(v)]) ||
            pp.H != H || pp.W != W) {
            DA_ERR("export_colmap_multi: capture processed RGB failed");
            return false;
        }
        rgb_ptrs[static_cast<size_t>(v)] = rgb_bufs[static_cast<size_t>(v)].data();
        orig_wh[static_cast<size_t>(v)] = {imgs[v].w, imgs[v].h};
    }

    if (!write_colmap(dir, depth_all, conf_all, K, E, rgb_ptrs, image_names, orig_wh,
                      H, W, N, binary)) {
