...
bool DinoBackbone::forward(const std::vector<float>& input_chw, int H, int W,
    ...
    const float* nwp; const float* nbp;
    if (nw->buffer) {
        nw_host.resize(embed); nb_host.resize(embed);
        ggml_backend_tensor_get(nw, nw_host.data(), 0, embed * sizeof(float));
        ggml_backend_tensor_get(nb, nb_host.data(), 0, embed * sizeof(float));
        nwp = nw_host.data(); nbp = nb_host.data();
    } else {
        nwp = (const float*)nw->data; nbp = (const float*)nb->data;
    }
