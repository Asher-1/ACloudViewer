...
bool quantize_gguf(const std::string& in_gguf, const std::string& out_gguf, const std::string& type) {
    ...
    for (int64_t ti = 0; ti < n_tensors && !failed; ++ti) {
        ...
        if (quant) {
            if (!dequantize_to_f32(src, f32_buf)) {
                DA_ERR("quantize: cannot dequantize '%s' (type=%s)",
    ...
    if (ok && !gguf_write_to_file(out, out_gguf.c_str(), /*only_meta=*/false)) {
        DA_ERR("quantize: gguf_write_to_file failed for '%s'", out_gguf.c_str());
        ok = false;
    }

    if (ok) {
        if (n_fallback_q8)
            DA_LOG("quantize: %d quantized (%s), %d kept f32, %d q8_0 fallback",
                   n_quant, ggml_type_name(target), n_kept, n_fallback_q8);
        else
            DA_LOG("quantize: %d quantized (%s), %d kept f32",
