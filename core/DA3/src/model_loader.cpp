...
bool ModelLoader::load(const std::string& path){
    ...
    if (!gguf_){ DA_ERR("gguf_init_from_file failed: %s", path.c_str()); return false; }
...
bool ModelLoader::offload_weights(Backend& be){
    ...
    if (!backend || !ctx_){ DA_ERR("offload_weights: null backend/ctx"); return false; }
    ...
    if (!device_ctx_){ DA_ERR("offload_weights: device ctx init failed"); return false; }
    ...
    if (!gpu_buf_){ DA_ERR("offload_weights: alloc_ctx_tensors failed"); return false; }
    for (auto& pr : ups)
