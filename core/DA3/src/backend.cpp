...
Backend::Backend() : impl_(new Impl()) {
    ...
    if (!force_cpu) {
        ...
        if (!want.empty() && !impl_->backend)
            DA_WARN("da::Backend: DA_DEVICE=%s not found; falling back to CPU",
                    want.c_str());
    }
    if (!impl_->backend) {              // CPU fallback (or CPU-only build)
        ...
        offloading_  = false;
        if (!impl_->backend) {
            DA_ERR("da::Backend: ggml_backend_cpu_init failed");
            return;
        }
    }
    ...
    if (impl_->use_sched) {
        ...
        if (!impl_->cpu_backend) {
            DA_WARN("da::Backend: CPU fallback init failed; disabling sched");
...
bool Backend::compute(const std::function<ggml_tensor*(ggml_context*)>& build,
    ...
    if (need_sched) {
        ...
        if (!alloc_ok) DA_ERR("Backend::compute: ggml_backend_sched_alloc_graph failed");
    } else {
        ...
        if (!alloc_ok) DA_ERR("Backend::compute: ggml_gallocr_alloc_graph failed");
    }
    if (!alloc_ok) {
        impl_->pending.clear();
        impl_->captures.clear();
    ...
    if (status != GGML_STATUS_SUCCESS) {
        DA_ERR("Backend::compute: ggml_backend_graph_compute failed (status=%d)",
               (int)status);
        impl_->captures.clear();
        impl_->roots.clear();
        ggml_free(ctx);
