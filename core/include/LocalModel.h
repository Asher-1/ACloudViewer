...
public:
    ...
    /** \param type the model type
            \param subset (small) set of points from which to compute the local
    model \param center model "center" \param squaredRadius model max radius
    (squared)
    **/
    static LocalModel* New(CV_LOCAL_MODEL_TYPES type,
    ...
    /** \param[in] P the query point
            ...
            \return the (unsigned) distance (or NAN_VALUE if the computation
    failed)
    **/
    virtual ScalarType computeDistanceFromModelToPoint(
            const CCVector3* P, CCVector3* nearestPoint = nullptr) const = 0;
