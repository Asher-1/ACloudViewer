...
public:
    ...
    /** The points should be associated to scalar values.
            Warning: be sure to activate an OUTPUT scalar field on the input
    cloud The Fast Marching grid will have the same characteristics as the
    octree considered at a given level of subdivision. The local front
    acceleration in each cell is deduced from the scalar values associated to
    the points lying in the octree cell (mean value). \param theCloud the point
    cloud \param theOctree the associated octree \param gridLevel the level of
    subdivision \param constantAcceleration specifies if the acceleration is
    constant or shoul be computed from the cell points scalar values \return a
    ...
    **/
    ...
    //! Returns a list of the points (references to) reached by the propagation
    //! process
    /** Returns a cloud of points (references to) corresponding to the points
    that are lying in cells that have been visited by the last propagation
    process. \param[out] Zk (reference) point cloud
    **/
...
protected:
    ...
    public:
        //! Default constructor
        PropagationCell() : Cell(), f(0), cellCode(0) {}
    ...
    // inherited methods (see FastMarching)
    float computeTCoefApprox(Cell* currentCell,
                             Cell* neighbourCell) const override;
    int step() override;
