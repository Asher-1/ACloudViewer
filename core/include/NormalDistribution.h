 ...
 **/
class CV_CORE_LIB_API NormalDistribution : public GenericDistribution {
public:
    ...
    **/
    ...
    //! Computes the distribution parameters from a point cloud (with scalar
    //! values)
    bool computeParameters(const GenericCloud* cloud);

    //! Computes robust parameters for the distribution from an array of scalar
    //! values
    /** Specific method to compute the parameters directly from an array
            (vector) of scalar values, without associated points. After a first
    pass, only the values close enough to the mean (in terms of nSigma times the
    initial variance) are kept to make a second and more robust evaluation of
    the parameters. \param values the scalar values \param nSigma the values
    filtering interval size ([mu -nSigma * stddev : mu + nSigma * stddev])
...
protected:
    ...
    /** This method is used (internally) to accelerate the Chi2 distance
    computation. \param numberOfClasses the number of classes that will be used
    for Chi2 distance computation \return success
    **/
