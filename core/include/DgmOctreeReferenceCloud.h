...
class CV_CORE_LIB_API DgmOctreeReferenceCloud
    : public GenericIndexedCloudPersist {
public:
    //! Default constructor.
    /** \param associatedSet associated NeighboursSet
            \param count number of values to use (0 = all)
    **/
    DgmOctreeReferenceCloud(DgmOctree::NeighboursSet* associatedSet,
                            unsigned count = 0);
    ...
    // virtual unsigned char testVisibility(const CCVector3& P) const; //not
    // supported
    inline void placeIteratorAtBeginning() override { m_globalIterator = 0; }
    inline const CCVector3* getNextPoint() override {
        return (m_globalIterator < size() ? m_set->at(m_globalIterator++).point
                                          : nullptr);
    }
    inline bool enableScalarField() override {
        return true;
    }  // use DgmOctree::PointDescriptor::squareDistd by default
    ...
    inline void getPoint(unsigned index, double P[3]) const override {
        assert(index < size());
        P[0] = m_set->at(index).point->x;
        P[1] = m_set->at(index).point->y;
        P[2] = m_set->at(index).point->z;
    }
