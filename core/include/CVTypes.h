...
#define CC_PRIMITIVE_BIT \
    0x00000000000008  // Primitive (sphere, plane, torus, cylinder, etc.)
...
#define CC_FACET_BIT \
    0x00000000800000  // Facet (composite object: cloud + 2D1/2 mesh + 2D1/2
                      // polyline)
...
#define CC_COORDINATESYSTEM_BIT \
    0x00000800000000                    // CoordinateSystem (primitive)
...
#define CC_VOXEL_GRID_BIT 0x01000000000000     // VOXEL GRID
#define CC_IMAGE2_BIT 0x02000000000000         // IMAGE for python
#define CC_RGBD_IMAGE_BIT 0x04000000000000     // RGBD IMAGE
#define CC_LINESET_BIT 0x08000000000000  // simple LINESET compared to polyline
#define CC_LINE3D_BIT 0x10000000000000   // LINE3D for computing
#define CC_MESH_BASE_BIT 0x20000000000000       // BASE MESH for python
#define CC_TETRA_MESH_BIT 0x40000000000000      // TETRA MESH for python
#define CC_HALF_EDGE_MESH_BIT 0x80000000000000  // HALF EDGE MESH for python
...
typedef enum : CV_CLASS_ENUM {
    ...
    MESH_GROUP = MESH | CC_GROUP_BIT,  // DEPRECATED; DEFINITION REMAINS FOR
    ...
    OLD_CYLINDER_ID =
            PRIMITIVE | CC_CYLINDER_BIT,  // for backward compatibility
    CYLINDER =
            PRIMITIVE | CC_CYLINDER_BIT | CC_CONE_BIT,  // cylinders are cones
    ...
    NORMAL_INDEXES_ARRAY =
            CC_ARRAY_BIT | CC_COMPRESSED_NORMAL_BIT | CC_LEAF_BIT,
    ...
    TETRA_MESH = MESH_BASE | CC_TETRA_MESH_BIT,
    HALF_EDGE_MESH = MESH_BASE | CC_HALF_EDGE_MESH_BIT,
    ...
    BBOX = HIERARCHY_OBJECT | CC_BBOX_BIT | CC_LEAF_BIT,
    ORIENTED_BBOX = HIERARCHY_OBJECT | CC_ORIENTED_BBOX_BIT | CC_LEAF_BIT,
    IMAGE2 = HIERARCHY_OBJECT | CC_IMAGE2_BIT | CC_LEAF_BIT,
    POINT_OCTREE2 = HIERARCHY_OBJECT | CC_OCTREE2_BIT | CC_LEAF_BIT,
    RGBD_IMAGE = HIERARCHY_OBJECT | CC_RGBD_IMAGE_BIT | CC_LEAF_BIT,

    //  Custom types
    /** Custom objects are typically defined by plugins. They can be inserted in
    an object hierarchy or displayed in an OpenGL context like any other
    ccHObject. To differentiate custom objects, use the meta-data mechanism (see
    ccObject::getMetaData and ccObject::setMetaData). You can also define a
    custom icon (see ccHObject::getIcon).

            It is highly advised to use the ccCustomHObject and
    ccCustomLeafObject interfaces to define a custom types. Carefully read the
    ccCustomHObject::isDeserialized method's description and the warning below!

            Warning: custom objects can't be 'fully' serialized. Don't overload
    the 'ccSerializableObject::toFile' method for them as this would break the
    deserialization mechanism! They can only be serialized as plain ccHObject
    instances (CV_TYPES::HIERARCHY_OBJECT). Hierarchical custom objects
    (CV_TYPES::CUSTOM_H_OBJECT) will be deserialized as ccCustomHObject
            instances. Leaf custom objects (CV_TYPES::CUSTOM_LEAF_OBJECT) will
    be deserialized as ccCustomLeafObject instances.
    **/
