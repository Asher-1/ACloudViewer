...
class ConjugateGradient : MathTools {
public:
    //! Default constructor
    ConjugateGradient() : cg_A(N) {
    ...
    void initConjugateGradient(const Scalar* X0) {
        ...
        // H0 = G0 = A.X0-b
        cg_A.apply(X0, cg_Gn);
        for (unsigned k = 0; k < N; ++k) cg_Hn[k] = (cg_Gn[k] -= cg_b[k]);
    }
    ...
    **/
    Scalar iterConjugateGradient(Scalar* Xn) {
        ...
        d /= e;
        for (k = 0; k < N; ++k) Xn[k] -= cg_Hn[k] * d;

        // we compute Gn+1
        cg_A.apply(Xn, cg_u);  // u = A.Xn+1
        for (k = 0; k < N; ++k)
            cg_Gn[k] = cg_u[k] - cg_b[k];  // Gn+1 =
                                           // A.Xn+1-b

        // sum of square errors
        e = cg_Gn[0] * cg_Gn[0];
        for (k = 1; k < N; ++k) e += cg_Gn[k] * cg_Gn[k];  // t^Gn+1.Gn+1
        ...
        for (k = 0; k < N; ++k)
            cg_Hn[k] =
                    cg_Gn[k] +
                    cg_Hn[k] * d;  // Hn+1 = Gn+1 + Hn.(t^Gn+1.Gn+1)/(t^Gn.Gn)
