...
/** Internally we use 'unsigned short' value to limit memory consumption.
        For computational reasons, the max computable 'distance' is 0xFAFA =
64250.
**/
class CV_CORE_LIB_API ChamferDistanceTransform : public Grid3D<unsigned short>,
                                                 public MathTools {
public:
    ...
    **/
    inline bool init(const Tuple3ui& gridSize) {
        return Grid3D<GridElement>::init(gridSize.x, gridSize.y, gridSize.z, 1,
                                         MAX_DIST);
    }
    ...
    GenericProgressCallback) \return max distance (or -1 if an error occurred)
    **/
    int propagateDistance(CHAMFER_DISTANCE_TYPE type,
