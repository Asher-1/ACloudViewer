...
public:
    ...
    **/
    ...
    /** This structure is generic and can be used in multiple cases.
            It is particularly useful when searching nearest neighbours around
    points that lie in the same octree cell. In this case, several informations
    about this cell should be given to the search algorithm through this
    structure, but only once,before the first search. Then the search algorithm
    can be called multiple times, and only few informations need to be updated
    (the query point, etc.).
    **/
    struct NearestNeighboursSearchStruct {
        ...
        /** used only during multiple neighbours search (see
        findNearestNeighborsStartingFromCell). This is only indicative and not
        guaranteed.
        **/
        unsigned minNumberOfNeighbors;
        //! Position in the octree of the cell including the query point
        /** The position is expressed for the level of subdivision at which the
        search will be processed. Use see DgmOctree::getCellPos to determine
        this position. This information should only be updated if the cell
        changes.
        **/
        Tuple3i cellPos;
        ...
        **/
        ...
        /** The NN search process will stop if it reaches this radius even if it
                hasn't find any neighbour (acceleration). To disable this
        behavior, set the maxSearchSquareDistd to something <= 0).
        **/
        double maxSearchSquareDistd;

        /*** Information to set to 0 before search ***/

        //! List of indexes of the cells that have been already visited by the
        //! algorithm
        /** This field is updated by the search algorithm. It should only be
        emptied if the cell that includes the query points change. Only used by
        the "unique nearest point" search algorithm.
        **/
        cellIndexesContainer minimalCellsSetToVisit;

        //! All the points that belong to the cubical neighbourhood of the
        //! current cell
        /** This structure is only used by the "multiple nearest neighbours"
        search algorithms. The nearest points (relatively to the query point)
        are stored at the beginning of the vector. They are associated to their
        square distance to the query point.
        **/
        NeighboursSet pointsInNeighbourhood;

        //! Size of the cell neighbourhood that has been already visited by the
        //! algorithm
        /** This field is updated by the search algorithm. It should only be
        reset to 0 when the cell that includes the query point changes. A value
        of 0 means that no cell has been visited yet, 1 means that only the cell
        that includes the query point has been visited, 2 means that this cell
        and its 27 neighbourhing cells have been visited, etc.
        **/
        ...
        //! The nearest point
        /** This field is only used by the "unique nearest neighbour" search
        algorithm (see DgmOctree::findTheNearestNeighborStartingFromCell).
        **/
    ...
    struct NearestNeighboursSphericalSearchStruct
        ...
        //! All the points that belong to the spherical neighbourhood of the
        //! current cell
        NeighboursSet pointsInSphericalNeighbourhood;

        //! Meta data describing cells neighbourhood (associated to
        //! pointsInNeighbourhoodUnsorted)
        NeighbourCellsSet cellsInNeighbourhood;

        //! max SQUARE distance from query point to cell center (to be sure of
        //! total inclusion)
        PointCoordinateType maxInD2;

        //! min SQUARE distance from query point to cell center (to be sure of
        //! total exclusion)
    ...
    **/
    struct IndexAndCode {
        ...
        **/
        ...
        /** \param a first IndexAndCode structure
                \param b second IndexAndCode structure
                \return whether the index of 'a' is smaller than the index of
        'b'
        **/
    ...
    //! Generic form of a function that can be applied automatically to all
    //! cells of the octree
    ...
    **/
    ...
    GenericProgressCallback) \return the number of points projected in the
    octree
    **/
    int build(GenericProgressCallback* progressCb = nullptr);

    //! Builds the structure with constraints
    /** Octree spatial limits must be specified. Also, if specified, points
    falling outside the "pointsFilter" limits won't be projected into the octree
    structure. Otherwise, all points will be taken into account. Octree 3D
    limits in space should be cubical. \param octreeMin the lower limits for the
    octree cells along X, Y and Z \param octreeMax the upper limits for the
    octree cells along X, Y and Z \param pointsMinFilter the lower limits for
    the projected points along X, Y and Z (is specified) \param pointsMaxFilter
    the upper limits for the projected points along X, Y and Z (is specified)
            \param progressCb the client application can get some notification
    of the process progress through this callback mechanism (see
    GenericProgressCallback) \return the number of points projected in the
    octree
    ...
    **/
    ...
    //! Returns the lowest cell positions in the octree along all dimensions and
    //! for a given level of subdivision
    /** For example, at a level	n, the octree length is 2^n cells along each
            dimension. The lowest cell position along each dimension will be
    expressed between 0 and 2^n-1. \param level the level of subdivision \return
    ...
    **/
    ...
    //! Returns the highest cell positions in the octree along all dimensions
    //! and for a given level of subdivision
    /** For example, at a level	n, the octree length is 2^n cells along each
            dimension. The highest cell position along each dimension will be
    expressed between 0 and 2^n-1. \param level the level of subdivision \return
    ...
    **/
    ...
    /** As the octree is cubical, cells are cubical.
            \param level the level of subdivision (up to MAX_OCTREE_LEVEL+1 for
    convenience) \return the cell size
    **/
    inline const PointCoordinateType& getCellSize(unsigned char level) const {
        return m_cellSize[level];
    }

    //! Returns distance form a cell to the filled octree borders in all
    //! directions.
    ...
    **/
    ...
    //! Returns distance from cell center to cell neighbourhood INSIDE filled
    //! octree
    ...
    **/
    ...
    /** Each cell at a given level of subdivision can be recognized by the index
            in the DgmOctree structure of the first point that lies inside it.
    By construction, we are assured that every point lying in the same cell for
            a given level of subdivision are next to each others in the octree
            structure (which is the vector "m_thePointsAndTheirCellCodes" in
    practical). This is the quickest way to access the points inside a given
    cell (but its kind of hard to know directly what is the index of a given
    cell ;) \param cloud ReferenceCloud to store the points lying inside the
    cell \param cellIndex the cell index \param level the level of subdivision
            \param clearOutputCloud whether to clear the input cloud prior to
    inserting the points or not \return success
    **/
    ...
    /** \param cellCode the unique cell code
            \param level the level of subdivision
            \param[out] subset set of points lying in the cell (references, no
    duplication) \param isCodeTruncated specifies if the code is given in a
    truncated form or not \param clearOutputCloud whether to clear or not the
    output cloud (subest) if no points lie in the specified cell \return success
    **/
    ...
    /** Cells are recognized here by their unique "code". They should be sorted
            along by codes, with an ascendant order. See
    DgmOctree::getPointsInCellByCellIndex for more information. \param cellCodes
    the cells codes \param level the level of subdivision \param[out] subset set
    of points lying in the cell (references, no duplication) \param
    areCodesTruncated specifies if the codes are given in a truncated form or
    not \return the set of points lying in the cell (references, no duplication)
    **/
    ...
    /** This is the simplest form of the nearest neighbour search algorithm.
            It should only be used for unique/few requests as it is not
    optimized for repetitive search around points lying in the same octree cell
    (see DgmOctree::findNearestNeighborsStartingFromCell for example). Moreover,
            ...
            \param level the subdivision level of the octree at which to perform
    the search \param maxSquareDist the square distance between the farthest
    "nearest neighbour" and the query point \param maxSearchDist the maximum
    search distance (ignored if <= 0) \param[out] the final neighborhood
    (half)size (optional) \return the number of neighbours found
    **/
    ...
    //! Advanced form of the nearest neighbour search algorithm (unique
    //! neighbour)
    /** This version is optimized for a unique nearest-neighbour search.
            ...
            \return the square distance between the query point and its nearest
    neighbour (or -1 if none was found - i.e. maxSearchDist was reached)
    **/
    double findTheNearestNeighborStartingFromCell(
            NearestNeighboursSearchStruct& nNSS) const;

    //! Advanced form of the nearest neighbours search algorithm (multiple
    //! neighbours)
    /** This version is optimized for a multiple nearest neighbours search
            that is applied around several query points included in the same
    octree cell. See DgmOctree::NearestNeighboursSearchStruct for more details.
            \param nNSS NN search parameters
            \param getOnlyPointsWithValidScalar whether to ignore points having
    an invalid associated scalar value \return the number of neighbours found
    **/
    ...
    /** This version is optimized for a spatially bounded search instead of
            ...
            \warning the number of points in the output buffer
    (nNSS.pointsInNeighbourhood) may be greater than the actual count of closest
    points inside the sphere! (which is returned by the method). Only the 'k'
    first points are actually inside the sphere (the others are not removed for
    the sake of performance). \param nNSS a pack of parameters \param radius the
    sphere radius \param sortValues specifies if the neighbours needs to be
    sorted by their distance to the query point or not \return the number of
    neighbours found
    **/
...
public:  // extraction of points inside geometrical volumes (sphere, cylinder,
    ...
    ;). \param sphereCenter center \param radius radius \param[out] neighbours
    points falling inside the sphere \param level subdivision level at which to
    apply the extraction process \return the number of extracted points
    **/
    int getPointsInSphericalNeighbourhood(const CCVector3& sphereCenter,
    ...
    //! Input/output parameters structure for
    ...
    struct CylindricalNeighbourhood {
        ...
        unsigned char level;
        //! Whether to look in both directions or only along the positive
        //! direction (i.e. half cylinder)
    ...
    **/
    ...
    struct ProgressiveCylindricalNeighbourhood : CylindricalNeighbourhood {
        ...
        /** Candidates are points close enough to the cylinder's axis but too
        far from its actual center.
        **/
    ...
    /** Can be called multiple times (the 'currentHalfLength' parameter will
    increase each time until 'maxHalfLength' is reached).
    **/
...
public: /***** CELLS POSITION HANDLING *****/
    //! Generates the truncated cell code of a cell given its position at a
    //! given level of subdivision
    ...
    **/
    ...
    //! Returns the position FOR THE DEEPEST LEVEL OF SUBDIVISION of the cell
    //! that includes a given point
    /** The cell coordinates can be negative or greater than
    2^MAX_OCTREE_LEVEL-1 as the point can lie outside the octree bounding-box.
    ...
    **/
    ...
    //! Returns the position for a given level of subdivision of the cell that
    ...
    **/
    ...
    /** The cell coordinates can be negative or greater than 2^N-1  (where N
            ...
            \param cellPos the computed position
            \param level the level of subdivision
            \param inBounds indicates if the query point is inside or outside
    the octree bounding-box
    **/
    inline void getTheCellPosWhichIncludesThePoint(const CCVector3* thePoint,
                                                   Tuple3i& cellPos,
                                                   unsigned char level,
    ...
    //! Returns the cell position for a given level of subdivision of a cell
    //! designated by its code
    /** \param code the cell code
            \param level the level of subdivision
            \param cellPos the computed position
            \param isCodeTruncated indicates if the given code is truncated or
    not
    **/
    ...
    //! Returns the cell center for a given level of subdivision of a cell
    //! designated by its code
    /** \param code the cell code
            \param level the level of subdivision
    ...
    **/
    ...
    //! Determines the best level of subdivision of the octree at which to apply
    //! the nearest neighbours search algorithm (inside a sphere) depending on
    //! the sphere radius
    ...
    **/
    ...
    //! Determines the best level of subdivision of the octree at which to apply
    //! a cloud-2-cloud distance computation algorithm
    /** The octree instance on which is "applied" this method should be the
    compared cloud's one. "theOtherOctree" should be the reference cloud's
    octree. \param theOtherOctree the octree of the other cloud \return the
    'best' level
    **/
    unsigned char findBestLevelForComparisonWithOctree(
            const DgmOctree* theOtherOctree) const;

    //! Determines the best subdivision level of the octree that gives the
    //! average population per cell closest to the input value
    /** \param indicativeNumberOfPointsPerCell 'desired' average number of
    points per cell \return the 'best' level
    **/
    unsigned char findBestLevelForAGivenPopulationPerCell(
            unsigned indicativeNumberOfPointsPerCell) const;

    //! Determines the best subdivision level of the octree to match a given
    //! number of cells
    ...
    **/
    ...
    //! Returns the list of codes corresponding to the octree cells for a given
    //! level of subdivision
    /** Only the non empty cells are represented in the octree structure.
            \param level the level of subdivision
            \param vec the list of codes
            \param truncatedCodes indicates if the resulting codes should be
    truncated or not \return false if an error occurred (e.g. not enough memory)
    **/
    bool getCellCodes(unsigned char level,
                      cellCodesContainer& vec,
                      bool truncatedCodes = false) const;

    //! Returns the list of indexes corresponding to the octree cells for a
    //! given level of subdivision
    /** Only the non empty cells are represented in the octree structure.
            Cell indexes are expressed relatively to the DgmOctree structure.
    They correspond to the indexes of the first points of each cell. \param
    level the level of subdivision \param vec the list of indexes \return false
    if an error occurred (e.g. not enough memory)
    **/
    bool getCellIndexes(unsigned char level, cellIndexesContainer& vec) const;

    //! Returns the list of indexes and codes corresponding to the octree cells
    //! for a given level of subdivision
    /** Only the non empty cells are represented in the octree structure.
            Cell indexes are expressed relatively to the DgmOctree structure.
    They correspond to the indexes of the first points of each cell. \param
    level the level of subdivision \param vec the list of codes & indexes \param
    ...
    **/
    ...
    //! Returns the cells that differ between two octrees (for a same implicit
    //! level of subdivision)
    /** Warning: the two octrees should have been computed with the same
    bounding-box. \param codesA the cell codes of the first octree for the
    implicit level \param codesB the cell codes of the second octree for the
    implicit level \param diffA the cells of the first octree that are not in
    the second octree \param diffB the cells of the second octree that are not
    in the first octree
    **/
    ...
    //! Returns the differences (in terms of number of cells) between two
    //! octrees for a given level of subdivision
    /** Warning: the two octrees should have been computed with the same
    bounding-box. \param octreeLevel the octree level \param codesA the cell
    codes (and point index) of the first octree \param codesB the cell codes
    (and point index) of the second octree \param diffA the number of cells of
    the first octree that are not in the second octree \param diffB the number
    of cells of the second octree that are not in the first octree \param cellsA
    the number of cells of the first octree for the given number of subdivision
            \param cellsB the number of cells of the second octree for the given
    number of subdivision \return false if it could not calculate the
    differences
    **/
    ...
    //! Computes mean octree density (point/cell) at a given level of
    //! subdivision
    /** \param level the level of subdivision
    ...
    **/
    ...
    //! Computes the minimal distance between a point and the borders (faces) of
    //! the cell (cube) in which it is included
    /** \param queryPoint the point
            \param cs the cell size (as cells are cubical, it's the same along
    every dimension) \param cellCenter the cell center \return the minimal
    distance
    **/
    ...
    //! Computes the connected components (considering the octree cells only)
    //! for a given level of subdivision (partial)
    /** The octree is seen as a regular 3D grid, and each cell of this grid is
    either set to 0 (if no points lies in it) or to 1 (if some points lie in it,
    e.g. if it is indeed a cell of this octree). This version of the algorithm
    can be applied by considering only a specified list of octree cells
    (ignoring the others). \param cellCodes the cell codes to consider for the
    CC computation \param level the level of subdivision at which to perform the
    algorithm \param sixConnexity indicates if the CC's 3D connexity should be 6
    (26 otherwise) \param progressCb the client application can get some
    notification of the process progress through this callback mechanism (see
    GenericProgressCallback) \return error code:
    ...
    **/
    ...
    //! Computes the connected components (considering the octree cells only)
    //! for a given level of subdivision (complete)
    /** The octree is seen as a regular 3D grid, and each cell of this grid is
    either set to 0 (if no points lies in it) or to 1 (if some points lie in it,
    e.g. if it is indeed a cell of this octree). This version of the algorithm
    is directly applied on the whole octree. \param level the level of
    subdivision at which to perform the algorithm \param sixConnexity indicates
    if the CC's 3D connexity should be 6 (26 otherwise) \param progressCb the
    client application can get some notification of the process progress through
    this callback mechanism (see GenericProgressCallback) \return error code:
                    - '>= 0' = number of components
                    - '-1' = no cells (input)
                    - '-2' = not enough memory
                    - '-3' = no CC found
    **/
    ...
    //! Method to apply automatically a specific function to each cell of the
    //! octree
    /** The function to apply should be of the form DgmOctree::octreeCellFunc.
    In this case the octree cells are scanned one by one at the same level of
    subdivision, but the scan can also be sometimes done (inside the cell) at
    deeper levels, in order to limit the number of points in a cell. Thanks to
    this, the function is applied on a limited number of points, avoiding great
    loss of performances. The only limitation is when the level of subdivision
    is deepest level. In this case no more splitting is possible.
    ...
    a cell (indicative) \param maxNumberOfPointsPerCell maximum number of points
    inside a cell (indicative) \param multiThread whether to use parallel
    processing or not \param progressCb the client application can get some
    notification of the process progress through this callback mechanism (see
    GenericProgressCallback) \param functionTitle function title \param
    maxThreadCount the maximum number of threads to use (0 = all). Ignored if
    'multiThread' is false. \return the number of processed cells (or 0 is
    ...
    **/
    ...
    //! Returns whether multi-threading (parallel) computation is supported or
    //! not
...
protected:
    ...
    //! Nearest power of 2 less than the number of points (used for binary
    //! search)
    ...
    //! Min coordinates of the bounding-box of the set of points projected in
    //! the octree
    CCVector3 m_pointsMin;
    //! Max coordinates of the bounding-box of the set of points projected in
    //! the octree
    CCVector3 m_pointsMax;

    //! Cell dimensions for all subdivision levels
    PointCoordinateType m_cellSize[MAX_OCTREE_LEVEL + 2];
    //! Min and max occupied cells indexes, for all dimensions and every
    //! subdivision level
    ...
    **/
    ...
    //! Updates the tables containing the octree cells length for each level of
    //! subdivision
    void updateCellSizeTable();

    //! Updates the tables containing the number of octree cells for each level
    //! of subdivision
    ...
    **/
    ...
    //! Returns the indexes of the neighbourhing (existing) cells of a given
    //! cell
    /** This function is used by the nearest neighbours search algorithms.
            ...
            \param neighbourhoodLength the distance (in terms of cells) at which
    to look for neighbour cells \param level the level of subdivision
    **/
    ...
    /** \param nNSS NN search parameters (from which are used: cellPos,
    pointsInNeighbourCells and level) \param neighbourhoodLength the new
    distance (in terms of cells) at which to look for neighbour cells \param
    getOnlyPointsWithValidScalar whether to ignore points having an invalid
    ...
    **/
    ...
    /** The index is found thanks to a binary search. The index of an existing
    cell is between 0 and the number of points projected in the octree minus 1.
    If the cell code cannot be found in the octree structure, then the method
    returns an index equal to the number of projected points
    (m_numberOfProjectedPoints). \param truncatedCellCode truncated cell code
    (i.e. original cell code shifted of 'bitDec' bits) \param bitDec binary
    shift corresponding to the level of subdivision (see GET_BIT_SHIFT) \return
    ...
    **/
    ...
    //! Returns the index of a given cell represented by its code
    /** Same algorithm as the other "getCellIndex" method, but in an optimized
    form. The binary search can be performed on a sub-part of the DgmOctree
    structure. \param truncatedCellCode truncated cell code (i.e. original cell
    code shifted of 'bitDec' bits) \param bitDec binary shift corresponding to
    the level of subdivision (see GET_BIT_SHIFT) \param begin first index of the
    sub-list in which to perform the binary search \param end last index of the
    sub-list in which to perform the binary search \return the index of the cell
    (or 'm_numberOfProjectedPoints' if none found)
    **/
    unsigned getCellIndex(CellCode truncatedCellCode,
