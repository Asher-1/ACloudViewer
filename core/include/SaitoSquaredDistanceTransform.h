...
//! Class to compute a Squared Distance Field with the Saito algorithm on a 3D
//! grid
class CV_CORE_LIB_API SaitoSquaredDistanceTransform : public Grid3D<unsigned>,
                                                      public MathTools {
public:
    //! Default constructor
    ...
    /** The memory for the grid must be explicitelty reserved prior to any
    action. \return true if the initialization succeeded
    **/
    inline bool initGrid(const Tuple3ui& gridSize) {
        return Grid3D<GridElement>::init(
                gridSize.x, gridSize.y, gridSize.z, 0,
...
protected:
    ...
    static bool SDT_3D(Grid3D<GridElement>& image,
                       GenericProgressCallback* progressCb = nullptr);
};
