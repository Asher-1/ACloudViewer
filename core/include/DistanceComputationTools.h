...
//! Several entity-to-entity distances computation algorithms (cloud-cloud,
//! cloud-mesh, point-triangle, etc.)
...
public:  // distance to clouds or meshes
    ...
    struct Cloud2CloudDistancesComputationParams {
        ...
        /** Set to -1 to deactivate (default).
                \warning Not compatible with closest point set determination
        (see CPSet)
        **/
        ...
        //! Whether to use a fixed number of neighbors or a (sphere) radius for
        ...
        **/
        ...
        /** The Closest Point Set corresponds to (the reference to) each
        compared point's closest neighbour. \warning Not compatible with max
        search distance (see maxSearchDist)
        **/
        ...
        /** By default, any previous distances/scalar values stored in the
        'enabled' scalar field will be reset before computing them again.
        **/
    ...
    //! Computes the "nearest neighbour distance" between two point clouds
    //! (formerly named "Hausdorff distance")
    /** The main algorithm and its different versions (with or without local
    modeling) are described in Daniel Girardeau-Montaut's PhD manuscript
    (Chapter 2, section 2.3). It is the standard way to compare directly two
    dense (and globally close) point clouds. \warning The current scalar field
    of the compared cloud should be enabled. By default it will be reset to
            NAN_VALUE but one can avoid this by defining the
    Cloud2CloudDistancesComputationParams::resetFormerDistances parameters to
    false. But even in this case, only values above
    Cloud2CloudDistancesComputationParams::maxSearchDist will remain untouched.
            \warning Max search distance
    (Cloud2CloudDistancesComputationParams::maxSearchDist > 0) is not compatible
    with the determination of the Closest Point Set
    (Cloud2CloudDistancesComputationParams::CPSet) \param comparedCloud the
    compared cloud (the distances will be computed on these points) \param
    referenceCloud the reference cloud (the distances will be computed
    relatively to these points) \param params distance computation parameters
    ...
    GenericProgressCallback) \param compOctree the pre-computed octree of the
    compared cloud (warning: both octrees must have the same cubical
    bounding-box - it is automatically computed if 0) \param refOctree the
    pre-computed octree of the reference cloud (warning: both octrees must have
    the same cubical bounding-box - it is automatically computed if 0) \return 0
    if ok, a negative value otherwise
    **/
    ...
    struct Cloud2MeshDistancesComputationParams {
        //! The level of subdivision of the octree at witch to apply the
        //! algorithm
        unsigned char octreeLevel;
        ...
        /** If true the distances will be aproximated by a Distance Transform.
                \warning Incompatible with signed distances or Closest Point
        Set.
        **/
        bool useDistanceMap;

        //! Whether to compute signed distances or not
        /** If true, the computed distances will be signed (in this case, the
        Distance Transform can't be used and therefore useDistanceMap will be
        ignored)
        **/
        bool signedDistances;

        //! Whether triangle normals should be computed in the 'direct' order
        //! (true) or 'indirect' (false)
        bool flipNormals;
        ...
        /** The cloud should be initialized but empty on input. It will have the
        same size as the compared cloud on output. \warning Not compatible with
        maxSearchDist > 0.
        **/
        PointCloud* CPSet;
    ...
    /** The algorithm, inspired from METRO by Cignoni et al., is described
            in Daniel Girardeau-Montaut's PhD manuscript (Chapter 2,
    section 2.2). It is the general way to compare a point cloud with a
    triangular mesh. \param pointCloud the compared cloud (the distances will be
    computed on these points) \param mesh the reference mesh (the distances will
    be computed relatively to its triangles) \param params parameters \param
    progressCb the client application can get some notification of the process
    progress through this callback mechanism (see GenericProgressCallback)
            \param cloudOctree the pre-computed octree of the compared cloud
...
public:  // approximate distances to clouds or meshes
    ...
    /** This methods uses an exact Distance Transform to approximate the real
    distances. Therefore, the greater the octree level is (it is used to
    determine the grid step), the finer the result will be (but more memory and
    time will be needed). \param comparedCloud the compared cloud \param
    referenceCloud the reference cloud \param octreeLevel the octree level at
    which to compute the Distance Transform \param maxSearchDist max search
    distance (or any negative value if no max distance is defined) \param
    progressCb the client application can get some notification of the process
    progress through this callback mechanism (see GenericProgressCallback)
            \param compOctree the pre-computed octree of the compared cloud
    (warning: both octrees must have the same cubical bounding-box - it is
    automatically computed if 0) \param refOctree the pre-computed octree of the
    reference cloud (warning: both octrees must have the same cubical
    bounding-box - it is automatically computed if 0) \return negative error
    code or a positive value in case of success
    **/
...
public:  // distance to simple entities (triangles, planes, etc.)
    ...
    /** WARNING: if not signed, the returned distance is SQUARED!
            ...
            \param signedDist whether to compute the signed or positive
    (SQUARED) distance \param nearestP optional: returns the nearest point on
    the triangle \return the distance between the point and the triangle
    **/
    ...
    norm(a,bc)==1 \return the signed distance between the point and the plane
    **/
    ...
    **/
    ...
    /** \param cloud a 3D point cloud
            ...
            \param signedDist whether to compute the signed or positive
    (absolute) distance (optional) \param solutionType if true the scalar field
    will be set to which solution was selected 1-4 (optional) \param[out] rms
    will be set with the Root Mean Square (RMS) distance between a cloud and a
    cylinder (optional) \return negative error code or a positive value in case
    of success
    **/
    ...
    **/
    ...
    Mean Square (RMS) distance between a cloud and a sphere (optional) \return
    ...
    **/
    ...
    /** \param cloud a 3D point cloud
            \param planeEquation plane equation: [a,b,c,d] as 'ax+by+cz=d' with
    norm(a,bc)==1 \param signedDist whether to compute the signed or positive
    (absolute) distance (optional) \param[out] rms will be set with the Root
    Mean Square (RMS) distance between a cloud and a plane (optional) \return
    negative error code or a positive value in case of success
    **/
    ...
    /** \param cloud a 3D point cloud
            ...
            \param[out] rms will be set with the Root Mean Square (RMS) distance
    between a cloud and a plane (optional) \return negative error code or a
    ...
    **/
    ...
    //! Computes the "distance" (see ERROR_MEASURES) between a point cloud and a
    //! plane
    ...
    **/
    ...
    /** WARNING: this method uses the cloud global iterator
            ...
            \return the max distance @ 'percent' % between the point and the
    plane
    **/
    ...
    **/
    ...
    /** Sums the squared distances between each point of the cloud and the
    plane, then computes the mean value. WARNING: this method uses the cloud
    global iterator \param cloud a point cloud \param planeEquation plane
    equation: [a,b,c,d] as 'ax+by+cz=d' \return the RMS of distances (or NaN if
    an error occurred)
    **/
    ...
    /** \param P 3D point
            ...
            \param onlyOrthogonal computes distance only if P lies 'in front' of
    AB (returns -1.0 otherwise) \return squared distance (or potentially -1.0 if
    onlyOrthogonal is true)
    **/
...
public:  // other methods
    //! Computes geodesic distances over a point cloud "surface" (starting from
    //! a seed point)
    /** This method uses the FastMarching algorithm. Thereofre it needs an
    octree level as input parameter in order to create the corresponding 3D
    grid. The greater this level is, the finer the result will be, but more
    memory will be required as well. Moreover to get an interesting result the
    cells size should not be too small (the propagation will be stoped more
    easily on any encountered 'hole'). \param cloud the point cloud \param
    seedPointIndex the index of the point from where to start the propagation
            \param octreeLevel the octree at which to perform the Fast Marching
    propagation \param progressCb the client application can get some
    notification of the process progress through this callback mechanism (see
    GenericProgressCallback) \return true if the method succeeds
    **/
    ...
    //! Computes the differences between two scalar fields associated to
    //! equivalent point clouds
    /** The compared cloud should be smaller or equal to the reference cloud.
    Its points should be at the same position in space as points in the other
    cloud. The algorithm simply computes the difference between the scalar
    values associated to each couple of equivalent points. \warning The result
    is stored in the active scalar field (input) of the comparedCloud. \warning
    Moreover, the output scalar field should be different than the input scalar
    field! \warning Be sure to activate an OUTPUT scalar field on both clouds
    ...
    **/
    ...
    /** Initializes the octrees before computing the distance between two
    clouds. Check if both octree have the same sizes and limits (in 3D) and
    rebuild them if necessary. \param comparedCloud the cloud corresponding to
    the first octree \param referenceCloud the cloud corresponding to the second
    octree \param comparedOctree the first octree \param referenceOctree the
    second octree \param maxSearchDist max search distance (or any negative
    value if no max distance is defined) \param progressCb the client method can
    get some notification of the process progress through this callback
    mechanism (see GenericProgressCallback) \return return code
    **/
...
protected:
    ...
    /** This method is used by computeCloud2MeshDistances.
            \param theIntersection a specific structure to store the result of
    the intersection \param octreeLevel the octree subdivision level
    corresponding to the grid \param progressCb the client method can get some
    notification of the process progress through this callback mechanism (see
    GenericProgressCallback)
    **/
    ...
    intersectMeshWithOctree has been called. \param theIntersection a specific
    structure corresponding the intersection of the mesh with the grid \param
    params parameters \param progressCb the client method can get some
    notification of the process progress through this callback mechanism (see
    GenericProgressCallback) \return -1 if an error occurred (e.g. not enough
    ...
    **/
    ...
    /** This method has the generic syntax of a "cellular function" (see
    DgmOctree::localFunctionPtr). Specific parameters are transmitted via the
    "additionalParameters" structure. There are 3 additional parameters :
            - (GenericCloud*) the compared cloud
            - (GenericCloud*) the reference cloud
            - (DgmOctree*) the octree corresponding to the compared cloud
            \param cell structure describing the cell on which processing is
    applied \param additionalParameters see method description \param nProgress
    optional (normalized) progress notification (per-point)
    **/
    ...
    //! Computes the "nearest neighbour distance" with local modeling for all
    //! points of an octree cell
    /** This method has the generic syntax of a "cellular function" (see
    DgmOctree::localFunctionPtr). Specific parameters are transmitted via the
    "additionalParameters" structure. There are 4 additional parameters :
            - (GenericCloud*) the compared cloud
            - (GenericCloud*) the reference cloud
            - (DgmOctree*) the octree corresponding to the compared cloud
