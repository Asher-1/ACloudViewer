...
//! A specific point could structure to handle subsets of points, provided with
//! several geometric processings
...
public:
    ...
    //! Geometric properties/elements that can be computed from the set of
    //! points (see Neighbourhood::getGeometricalElement)
    enum GeomElement {
        FLAG_DEPRECATED = 0,
    ...
    /** Cloud selection is first projected on the best least-square plane.
            \param duplicateVertices whether to duplicate vertices (a new point
    cloud is created) or to use the associated one) \param maxEdgeLength max
    edge length for output triangles (IGNORE_MAX_EDGE_LENGTH = ignored) \param
    ...
    ***/
    ...
    //! Fit a quadric on point set (see getQuadric) then triangulates it inside
    //! bounding box
    ...
    /** Projected points are stored in the points2D vector.
            ...
            \param planeEquation custom plane equation (otherwise the default
    Neighbouhood's one is used) \param O if set, the local plane base origin
    will be output here \param X if set, the local plane base X vector will be
    output here \param Y if set, the local plane base Y vector will be output
    here \param useOXYasBase whether O, X and Y should be used as input base
            \return success
    **/
    ...
    /** Most of them are defined in "Contour detection in unstructured 3D point
    clouds", Hackel et al, 2016 PCA1 and PCA2 are defined in "3D terrestrial
    lidar data classification of complex natural scenes using a multi-scale
    dimensionality criterion: Applications in geomorphology", Brodu and Lague,
    2012
    **/
    enum GeomFeature {
        EigenValuesSum = 1,
    ...
    //! Computes the 1st order moment of a set of point (based on the
    //! eigenvalues)
    /** \return 1st order moment at a given position P (between 0 and 1)
     **/
    ScalarType computeMomentOrder1(const CCVector3& P);

    //! Computes the roughness of a point (by fitting a 2D plane on its
    //! neighbors)
    /** \param P point for which to compute the roughness value
            \param roughnessUpDir up direction to compute a signed roughness
    value (optional) \return roughness value at a given position P \warning The
    point P shouldn't be in the set of points
    **/
    ...
    /** \return curvature value at a given position P or NAN_VALUE if the
    computation failed \warning The curvature value is always unsigned
    **/
    ...
    **/
    ...
    /** This corresponds to the largest eigen value (i.e. the largest cloud
    ...
    **/
    ...
    /** This corresponds to the second largest eigen value (i.e. the second
    largest cloud dimension) \return 0 if computation failed
    **/
    const CCVector3* getLSPlaneY();
    //! Returns best interpolating plane (Least-square) normal vector
    /** This corresponds to the smallest eigen value (i.e. the second largest
    cloud dimension) \return 0 if computation failed
    **/
    ...
    **/
    ...
    /** \param[out] quadricEquation an array of 10 coefficients
    [a,b,c,d,e,f,g,l,m,n] such as
                             a.x^2+b.y^2+c.z^2+2e.x.y+2f.y.z+2g.z.x+2l.x+2m.y+2n.z+d
    = 0 \return success
    **/
    ...
    //! Returns the set 'radius' (i.e. the distance between the gravity center
    //! and the its farthest point)
    PointCoordinateType computeLargestRadius();

protected:
    //! 2.5D Quadric equation
    /** Array [a,b,c,d,e,f] such that Z = a + b.X + c.Y + d.X^2 + e.X.Y + f.Y^2.
            \warning: 'X','Y' and 'Z' are defined by
    'm_quadricEquationDirections' Only valid if 'structuresValidity & QUADRIC !=
    0'.
    **/
