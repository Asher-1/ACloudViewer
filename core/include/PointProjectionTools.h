...
//! Several point cloud re-projection algorithms ("developpee", translation,
//! rotation, etc.)
...
public:
    ...
    struct Transformation {
        ...
        /** \warning THIS METHOD IS NOT COMPATIBLE WITH PARALLEL STRATEGIES
                \warning The caller should invalidate the bounding-box manually
        afterwards
        **/
    ...
    /** Generates a "developpee" of a cylinder-shaped point cloud.
            ...
            \param dim the dimension along which the cylinder axis is aligned
    (X=0, Y=1, Z=2) \param center a 3D point (as a 3 values array) belonging to
    the cylinder axis \param progressCb the client application can get some
    notification of the process progress through this callback mechanism (see
    GenericProgressCallback) \return the "developed" cloud
    **/
    ...
    /** Generates a "developpee" of a cone-shaped point cloud.
            WARNING: this method uses the cloud global iterator
            \param cloud the point cloud to be developed
            \param dim the dimension along which the cone axis is aligned (X=0,
    Y=1, Z=2) \param baseRadius the radius of the base of the cone \param alpha
    the angle of the cone "opening" \param center the 3D point corresponding to
    the intersection between the cone axis and its base \param progressCb the
    client application can get some notification of the process progress through
    this callback mechanism (see GenericProgressCallback) \return the
    "developed" cloud
    **/
    ...
    **/
    ...
    /** The triangulation can be either computed on the points projected
            ...
            \param maxEdgeLength max edge length for output triangles (0 =
    ignored) \param dim projection dimension (for axis-aligned meshes) \param
    errorStr error (if any) [optional] \return a mesh
    **/
    ...
    **/
    ...
    /** Inspired from JIN-SEO PARK AND SE-JONG OH, "A New Concave Hull Algorithm
            ...
            \param points input set of points
            \param hullPoints output points (on the convex hull)
            \param maxSquareLength maximum square length (ignored if <= 0, in
    which case the method simply returns the convex hull!) \return success
    **/
