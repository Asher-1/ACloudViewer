...
//! Square matrix
/** Row-major ordered matrix (i.e. elements are accessed with
 *'values[row][column]')
 **/
...
public:
    ...
    /** The matrix dimension is automatically set to 4.
            ...
            \param M16f a table of 16 floats (OpenGL float transformation
    matrix) \param rotationOnly consider only the roation part (3x3 matrix)
    **/
    ...
    /** The matrix dimension is automatically set to 4.
            It can be forced to 3 (size_3 = true). In this
            case, only the rotation part will be 'imported'.
            \param M16d a table of 16 floats (OpenGL double transformation
    matrix) \param rotationOnly consider only the roation part (3x3 matrix)
    **/
    ...
    **/
    ...
    //! Converts a 3*3 or 4*4 matrix to an OpenGL-style double matrix
    //! (double[16])
    ...
    //! SVD decomposition (inspired from
