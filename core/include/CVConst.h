...
[[deprecated(
        "Use ZERO_TOLERANCE_F/ZERO_TOLERANCE_D or "
        "cloudViewer::LessThanEpsilon()/"
        "cloudViewer::GreaterThanEpsilon()")]] constexpr double ZERO_TOLERANCE =
        std::numeric_limits<double>::epsilon();
#endif

//! ZERO_TOLERANCE_F is used to set or compare a float variable to "close to
//! zero".
constexpr float ZERO_TOLERANCE_F = std::numeric_limits<float>::epsilon();

//! ZERO_TOLERANCE_D is used to set or compare a double variable to "close to
//! zero". It is defined as std::numeric_limits<float>::epsilon() because using
//! std::numeric_limits<double>::epsilon() results in numbers that are too small
//! for our purposes.
...
//! ZERO_TOLERANCE_POINT_COORDINATE is used to set or compare a
//! PointCoordinateType variable to "close to zero".
...
constexpr unsigned char POINT_VISIBLE =
        0; /**< Point visibility state: visible **/
constexpr unsigned char POINT_HIDDEN =
        1; /**< Point visibility state: hidden (e.g. behind other points) **/
constexpr unsigned char POINT_OUT_OF_RANGE =
        2; /**< Point visibility state: out of range **/
constexpr unsigned char POINT_OUT_OF_FOV =
        4; /**< Point visibility state: out of field of view **/
...
//! Types of local models (no model, least square best fitting plan, Delaunay
//! 2D1/2 triangulation, height function)
