...
public:
    ...
    /** It uses Rutishauser's modfications of the classical Jacobi rotation
        ...
        for (unsigned it = 0; it < maxIterationCount; ++it) {
            ...
            for (unsigned p = 0; p < n; ++p) {
                for (unsigned q = p + 1; q < n; ++q) {
                    ...
                    if (it > 4 && termp == std::abs(eigenValues[p]) &&
                    ...
                    // Otherwise, apply a rotation
                        ...
                        // Rotate, using information from the upper triangle of
                        // A only.
                        unsigned j;
                        for (j = 0; j < p; ++j) {
    ...
    static bool ComputeEigenValuesAndVectors(const SquareMatrix& matrix,
        ...
        // init
        {
            for (unsigned ip = 0; ip < n; ip++) {
                b[ip] = d[ip] =
                        matrix.m_values[ip][ip];  // Initialize b and d to the
                                                  // diagonal of a.
        ...
        for (unsigned i = 1; i <= maxIterationCount; i++) {
            ...
            if (sm == 0)  // The normal return, which relies on quadratic
                          // convergence to machine underflow.
            {
                if (absoluteValues) {
                    // we only need the absolute values of eigenvalues
                    for (unsigned ip = 0; ip < n; ip++) d[ip] = std::abs(d[ip]);
                }
            ...
            Scalar tresh = 0;
            if (i < 4) {
                tresh = sm / static_cast<Scalar>(
                                     5 * matrixSquareSize);  //...on the first
                                                             // three sweeps.
            }
            ...
            for (unsigned ip = 0; ip < n - 1; ip++) {
                for (unsigned iq = ip + 1; iq < n; iq++) {
                    ...
                    if (i > 4 &&
                        ...
                        static_cast<float>(std::abs(d[iq]) + g) ==
                                static_cast<float>(std::abs(d[iq]))) {
                        matrix.m_values[ip][iq] = 0;
                    } else if (std::abs(matrix.m_values[ip][iq]) > tresh) {
                        Scalar h = d[iq] - d[ip];
                        Scalar t = 0;
                        if (static_cast<float>(std::abs(h) + g) ==
                            static_cast<float>(std::abs(h))) {
                            t = matrix.m_values[ip][iq] / h;
                        } else {
                            Scalar theta =
                                    h /
                                    (2 *
                                     matrix.m_values[ip][iq]);  // Equation
                                                                // (11.1.10).
                            t = 1 / (std::abs(theta) + sqrt(1 + theta * theta));
                            if (theta < 0) t = -t;
                        }
    ...
    //! Sorts the eigenvectors in the decreasing order of their associated
    //! eigenvalues
    /** \param eigenVectors eigenvectors (as a square matrix)
            \param eigenValues eigenvalues
            \return success
            **/
    static bool SortEigenValuesAndVectors(SquareMatrix& eigenVectors,
                                          EigenValues& eigenValues) {
        ...
        for (unsigned i = 0; i < n - 1; i++) {
            ...
            for (unsigned j = i + 1; j < n; j++)
                if (eigenValues[j] > eigenValues[maxValIndex]) maxValIndex = j;

            if (maxValIndex != i) {
                std::swap(eigenValues[i], eigenValues[maxValIndex]);
                for (unsigned j = 0; j < n; ++j)
                    std::swap(eigenVectors.m_values[j][i],
                              eigenVectors.m_values[j][maxValIndex]);
            }
        }
    ...
    /** \param eigenVectors eigenvectors (as a square matrix)
            \param index requested eigenvector index (< eigenvectors matrix
       size) \param eigenVector output vector (size = matrix size) \return
       success
            **/
    static bool GetEigenVector(const SquareMatrix& eigenVectors,
                               unsigned index,
                               Scalar eigenVector[]) {
    ...
    /** \param eigenVectors eigenvectors (as a square matrix)
            ...
            \param maxEigenVector eigenvector vector corresponding to the
       biggest eigenvalue \return success
            **/
    static bool GetMaxEigenValueAndVector(const SquareMatrix& eigenVectors,
        ...
        for (unsigned i = 1; i < eigenVectors.size(); ++i)
            if (eigenValues[i] > eigenValues[maxIndex]) maxIndex = i;
    ...
    /** \param eigenVectors eigenvectors (as a square matrix)
            ...
            \param minEigenVector eigenvector vector corresponding to the
       smallest eigenvalue \return success
            **/
