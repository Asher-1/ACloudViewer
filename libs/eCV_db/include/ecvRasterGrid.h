...
//! Raster grid cell
struct ECV_DB_LIB_API ccRasterCell {
    //! Default constructor
    ccRasterCell()
...
//! Raster grid type
struct ECV_DB_LIB_API ccRasterGrid {
    //! Default constructor
    ccRasterGrid();
    ...
    /** Since version 2.8, we now use the "PixelIsArea" convention by default
    (as GDAL) This means that the height is computed at the center of the grid
    cell.
    **/
    ...
    //! type
    static InterpolationType InterpolationTypeFromEmptyCellFillOption(
            EmptyCellFillOption option);
    ...
    std::pair<int, int> computeCellPos(const CCVector3& P,
                                       unsigned char X,
    ...
    CCVector2d computeCellCenter(int i,
                                 int j,
                                 unsigned char X,
                                 unsigned char Y) const {
        return {minCorner.u[X] + (i + 0.5) * gridStep,
                minCorner.u[Y] + (j + 0.5) * gridStep};
    }
