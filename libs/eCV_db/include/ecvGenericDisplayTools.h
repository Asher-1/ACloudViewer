...
public:
	...
	inline virtual void toWorldPoint(const CCVector3d& input2D, CCVector3d& output3D) { /* do nothing */ }
	//inline static void ToWorldPoint(CCVector3 & p) { s_tools->toWorldPoint(p); }
	inline virtual void toWorldPoint(const CCVector3& input2D, CCVector3d& output3D) { /* do nothing */ }

	template <typename iType, typename oType>
	inline static void ToDisplayPoint(const Vector3Tpl<iType>& input3D, Vector3Tpl<oType>& output2D) { GetInstance()->toDisplayPoint(input3D, output2D); }
	inline virtual void toDisplayPoint(const CCVector3d & input3D, CCVector3d & output2D) { /* do nothing */ }
	//inline static void ToDisplayPoint(const CCVector3 & worldPos, CCVector3d & displayPos) { s_tools->toDisplayPoint(worldPos, displayPos); }
	inline virtual void toDisplayPoint(const CCVector3 & input3D, CCVector3d & output2D) { /* do nothing */ }
...
struct ECV_DB_LIB_API ccGLCameraParameters
{
	...
	inline bool project(const CCVector3d& input3D, CCVector3d& output2D, bool* inFrustum = nullptr) const { return ecvGenericDisplayTools::Project<double, double>(input3D, modelViewMat.data(), projectionMat.data(), viewport, output2D, inFrustum); }
	//! Projects a 3D point in 2D (+ normalized 'z' coordinate)
	inline bool project(const CCVector3& input3D, CCVector3d& output2D, bool* inFrustum = nullptr) const { return ecvGenericDisplayTools::Project<PointCoordinateType, double>(input3D, modelViewMat.data(), projectionMat.data(), viewport, output2D, inFrustum); }

	//! Unprojects a 2D point (+ normalized 'z' coordinate) in 3D
	inline bool unproject(const CCVector3d& input2D, CCVector3d& output3D) const { return ecvGenericDisplayTools::Unproject<double, double>(input2D, modelViewMat.data(), projectionMat.data(), viewport, output3D); }
	//! Unprojects a 2D point (+ normalized 'z' coordinate) in 3D
	inline bool unproject(const CCVector3& input2D, CCVector3d& output3D) const { return ecvGenericDisplayTools::Unproject<PointCoordinateType, double>(input2D, modelViewMat.data(), projectionMat.data(), viewport, output3D); }
