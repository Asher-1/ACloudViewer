...
class ECV_DB_LIB_API ccGLMatrix : public ccGLMatrixTpl<float> {
public:
    //! Default constructor
...
class ECV_DB_LIB_API ccGLMatrixd : public ccGLMatrixTpl<double> {
public:
    //! Default constructor
    /** Matrix is set to identity (see ccGLMatrixTpl::toIdentity) by default.
     **/
...
//! Constructor from a rotation center G, a 3x3 rotation matrix R and a vector
//! Tr
template <typename Tin, typename Tout>
ccGLMatrixTpl<Tout> FromCCLibMatrix(const cloudViewer::SquareMatrixTpl<Tin>& R,
                                    const Vector3Tpl<Tin>& Tr,
