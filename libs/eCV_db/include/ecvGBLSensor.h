...
/** An implementation of the ccSensor interface that can be used to represent a
depth sensor relying on 2 rotations relatively to two perpendicular axes, such
as ground based laser scanners typically.
**/
class ECV_DB_LIB_API ccGBLSensor : public ccSensor {
public:
    //! The order of inner-rotations of the sensor (body/mirrors)
    /** Either the first rotation is made around the Z axis (yaw) then around
    the lateral axis (pitch) as most scanners do today (Leica, Riegl, Faro,
    etc.). Othewise the opposite order is used (as the very old Mensi Soisic).
    **/
    ...
    **/
    ...
    CV_CLASS_ENUM getClassID() const override { return CV_TYPES::GBL_SENSOR; }
    ...
    //! Determines a 3D point "visibility" relatively to the sensor field of
    //! view
    /** Relies on the sensor associated depth map (see
    ccGBLSensor::computeDepthBuffer). The depth map is used to determine the
    "visiblity" of a 3D point relatively to the laser scanner field of view.
    This can be useful for filtering out points that shouldn't be compared while
    computing the distances between two point clouds for instance (for more
    information on this	particular topic, refer to Daniel Girardeau-Montaut's
    PhD manuscript - Chapter 2, section 2.3.3). \param P the point to test
            \return the point's visibility (POINT_VISIBLE, POINT_HIDDEN,
    POINT_OUT_OF_RANGE or POINT_OUT_OF_FOV)
    **/
    ...
    /** Errors codes are returned by ccGBLSensor::computeDepthBuffer or
     *ccDepthBuffer::fillHoles for instance.
     **/
...
public:  // setters and getters
    ...
    **/
    ...
    //! Returns whether the pitch angles are shifted (i.e. between [0 ; 2pi]
    //! instead of [-pi ; pi])
    ...
    **/
    void setYawRange(PointCoordinateType minTheta,
    ...
    //! Returns whether the yaw angles are shifted (i.e. between [0 ; 2pi]
    //! instead of [-pi ; pi])
...
public:  // projection tools
    ...
    /** \param[in] sourcePoint 3D point to project
            \param[out] destPoint projected point in polar coordinates:
    (theta,phi) = (yaw,pitch) (angles between [-pi,+pi] or [0 ; 2pi] if the
    corresponding angle is 'shifted') \param[out] depth distance between the
    sensor optical center and the 3D point \param[in] posIndex (optional) sensor
    position index (see ccIndexedTransformationBuffer)
    **/
    ...
    /** WARNING: this method uses the cloud global iterator
            ...
            \param norms the normals vectors (should have the same size and
    order as the point cloud) \param posIndex (optional) sensor position index
    (see ccIndexedTransformationBuffer) \return a bidimensional array of 3D
    vectors (same size as the depth buffer)
    **/
    ...
    //! Projects a set of point cloud colors in the sensor frame defined by this
    //! instance
    /** WARNING: this method uses the cloud global iterator
            \param cloud a point cloud
            \param rgbColors the RGB colors (should have the same size and order
    as the point cloud) \return a set of RGB colors organized as a bidimensional
    grid (same size as the depth buffer)
    **/
    ColorGrid* projectColors(cloudViewer::GenericCloud* cloud,
                             const ColorGrid& rgbColors) const;

public:  // depth buffer management
    //! Projects a point cloud along the sensor point of view defined by this
    //! instance
    /** WARNING: this method uses the cloud global iterator
            \param cloud a point cloud
            \param errorCode error code in case the returned cloud is 0
            \param projectedCloud optional (empty) cloud to store the projected
    points \return whether the depth buffer was successfully created or not
    **/
    bool computeDepthBuffer(cloudViewer::GenericCloud* cloud,
                            int& errorCode,
                            ccPointCloud* projectedCloud = nullptr);

    //! Returns the associated depth buffer
    /** Call ccGBLSensor::computeDepthBuffer first otherwise the returned buffer
     *will be 0.
     **/
...
protected:
    ...
    //! Converts 2D angular coordinates (yaw,pitch) in integer depth buffer
    //! coordinates
    bool convertToDepthMapCoords(PointCoordinateType yaw,
    ...
    //! Whether the pitch angular range is shifted (i.e in [0 ; 2pi] instead of
    //! [-pi ; pi])
    ...
    //! Whether the yaw range is shifted (i.e in [0 ; 2pi] instead of [-pi ;
    //! pi]))
