...
//! Triangular mesh
class ECV_DB_LIB_API ccMesh : public ccGenericMesh
{
public:
...
public: // some cloudViewer interface
	...
	bool hasAdjacencyList() const {
		return getVerticeSize() > 0 && adjacency_list_.size() == getVerticeSize();
	}
	...
	bool hasTriangleMaterialIds() const {
		return hasTriangles() &&
			triangle_material_ids_.size() == size();
	}
	...
	inline virtual bool isEmpty() const override { return !hasVertices() || !hasTriangles(); }
   ...
   /// triangle indices the given edge belongs to.
	...
	/// \param point_weight The importance that interpolation of the point samples 
	...
	/// \param samples_per_node The minimum number of sample points that should fall 
	/// within an octree node as the octree construction is adapted to sampling density.
	...
	static std::shared_ptr<ccMesh> CreateTetrahedron(double radius = 1.0);
	...
	static std::shared_ptr<ccMesh> CreateIcosahedron(double radius = 1.0);
	...
	static std::shared_ptr<ccMesh> CreatePlane(
		double width = 1.0,
		double height = 1.0);
	...
	/// longitude lines.)
	static std::shared_ptr<ccMesh> CreateSphere(
		double radius = 1.0,
		int resolution = 20);
	...
	/// segments.
	static std::shared_ptr<ccMesh> CreateCylinder(
		double radius = 1.0,
		double height = 2.0,
		int resolution = 20,
		int split = 4);
	...
	/// segments \param split defines that the height will be split into split
	/// segments.
	static std::shared_ptr<ccMesh> CreateCone(
		double radius = 1.0,
		double height = 2.0,
		int resolution = 20,
		int split = 1);
	...
	static std::shared_ptr<ccMesh> CreateTorus(
	...
	static std::shared_ptr<ccMesh> CreateArrow(
	...
	static std::shared_ptr<ccMesh> CreateMoebius(
		int length_split = 70,
		int width_split = 15,
		int twists = 1,
		double radius = 1,
