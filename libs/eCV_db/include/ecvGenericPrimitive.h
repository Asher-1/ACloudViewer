...
class ECV_DB_LIB_API ccGenericPrimitive : public ccMesh {
public:
    //! Default constructor
    /** Warning: the associated transformation is purely for display purpose by
    default. Use 'ccHObject::applyGLTransformation_recursive' to 'apply' it to
    the mesh). \param name name \param transMat optional 3D transformation
    applied to the primitive vertices (can be set afterwards with
    ccDrawableObject::setGLTransformation +
    ccDrawableObject::applyGLTransformation_recursive)
    **/
    ...
    //! Add operator
    /** Warning: simply copies the input primitive vertices/triangles to this
     *primitive mesh!
     **/
...
protected:
    ...
    **/
    ...
    /** Warning: resets all!
     **/
    bool init(unsigned vertCount,
              bool vertNormals,
              unsigned faceCount,
              unsigned faceNormCount);
