...
class ECV_DB_LIB_API ccNormalVectors {
public:
    ...
    enum Orientation {
        ...
        PLUS_BARYCENTER =
                6,  //!< Normals always opposite to the cloud barycenter
        ...
        PLUS_SENSOR_ORIGIN =
                11,  //!< Normals opposite to the associated sensor origin (if
                     //!< any, and if multiple, the first one will be used)
        MINUS_SENSOR_ORIGIN =
                12,  //!< Normals towards the associated sensor origin (if any,
                     //!< and if multiple, the first one will be used)
    ...
    /** \param cloud point cloud on which to process the normals.
            ...
            \param localModel which kind of model to use for the computation (LS
    = plane, QUADRIC = quadratic Height Function, TRI = triangulation) \param
    localRadius local neighborhood radius (not necessary for TRI) \param
    preferredOrientation specifies a preferred orientation for normals
    (optional) \param progressCb progress notification (optional) \param
    inputOctree inputOctree input cloud octree (optional). \return success
    **/
    ...
    //! Tries to guess a very naive 'local radius' for normals computation (see
    //! ComputeCloudNormals)
    /** \param cloud point cloud on which to process the normals.
    ...
    **/
    ...
    //! Tries to guess the best 'local radius' for normals computation (see
    //! ComputeCloudNormals)
    /** \param cloud point cloud on which to process the normals.
            \param cloudOctree input cloud octree (optional)
            \param progressCb progress notification (optional)
            \return the best radius (strictly positive value) or 0 if an error
    occurred
    **/
    ...
    /** \param theCloud point cloud on which to process the normals.
            \param theNormsCodes array in which the normals indexes are stored
            \param preferredOrientation specifies a preferred orientation for
    normals \return success
    **/
    static bool UpdateNormalOrientations(ccGenericPointCloud* theCloud,
                                         NormsIndexesTableType& theNormsCodes,
                                         Orientation preferredOrientation);

    //! Converts a normal vector to geological 'strike & dip' parameters
    //! (N[dip]°E - [strike]°)
    ...
    **/
    ...
    //! Converts a couple of geological 'dip direction & dip' parameters to a
    //! unit normal vector
    /** \param[in] dip_deg value (in degrees)
            \param[in] dipDir_deg dip direction value(in degrees)
            \param[in] upward whether the output normal vector should point
    towards Z+ (true) or Z- (false) \return unit normal vector
    **/
    static CCVector3 ConvertDipAndDipDirToNormal(PointCoordinateType dip_deg,
                                                 PointCoordinateType dipDir_deg,
                                                 bool upward = true);

    //! Converts geological 'strike & dip' parameters (N[dip]°E - [strike]°) to
    //! a string
    ...
    **/
    ...
    /** \param[in] dip_deg dip angle value (in degrees)
            \param[in] dipDir_deg dip direction value (in degrees)
            \return formatted string "Dip direction: [dipDir]° - Dip angle:
    [dip]°"
    **/
