...
//! Waveform descriptor
class ECV_DB_LIB_API WaveformDescriptor : public ccSerializableObject {
public:
    //! Default constructor
    WaveformDescriptor();
    ...
    double digitizerGain;      //!< Digitizer gain (VOLTS = OFFSET + GAIN *
                               //!< Raw_Waveform_Amplitude)
    double digitizerOffset;    //!< Digitizer offset (VOLTS = OFFSET + GAIN *
                               //!< Raw_Waveform_Amplitude)
 ...
 **/
class ECV_DB_LIB_API ccWaveform : public ccSerializableObject {
public:
    //! Default constructor
    ...
    /** \warning A value of zero indicates that there is no associated waveform
     *data.
     **/
    ...
    static bool ToASCII(const QString& filename,
                        std::vector<double>& values,
                        uint32_t samplingRate_ps);

    //! Returns the sample position in 3D
    CCVector3 getSamplePos(float i,
                           const CCVector3& P0,
...
protected:  // members
    ...
    /** Parametric line equation for extrapolating points along the associated
    waveform: X = X0 + X(t) Y = Y0 + Y(t) Z = Z0 + Z(t)
    **/
    ...
    /** The offset in picoseconds from the first digitized value to the location
            within the waveform packet that the associated return pulse was
    detected.
    **/
    float m_echoTime_ps;
 ...
 **/
class ECV_DB_LIB_API ccWaveformProxy {
public:
    //! Default constructor
    ccWaveformProxy(const ccWaveform& w,
                    const WaveformDescriptor& d,
                    const uint8_t* storage)
        : m_w(w), m_d(d), m_storage(storage) {}
