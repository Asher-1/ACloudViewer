...
**/
class ECV_DB_LIB_API ccGenericPointCloud
    : public ccShiftedObject,
      public cloudViewer::GenericIndexedCloudPersist {
    friend class ccMesh;
...
public:
    ...
    ***************************************************/
    ...
    and the points visibility information. \param destCloud destination cloud
    can be provided here (must be of the exact same type as the cloned cloud!)
            \param ignoreChildren [optional] whether to ignore the cloud's
    children or not (in which case they will be cloned as well) \return a copy
    of this entity
    **/
    virtual ccGenericPointCloud* clone(ccGenericPointCloud* destCloud = nullptr,
                                       bool ignoreChildren = false) = 0;
    ...
    ***************************************************/
    ...
    /** The octree bounding-box is automatically defined as the smallest
            ...
            \param progressCb the caller can get some notification of the
    process progress through this callback mechanism (see cloudViewer
    documentation) \param autoAddChild whether to automatically add the computed
    octree as child of this cloud or not \return the computed octree
    **/
    virtual ccOctree::Shared computeOctree(
    ...
    ***************************************************/
    ...
    /** The returned value depends on the current scalar field display
    parameters. It may even be 0 if the value shouldn't be displayed. WARNING:
    scalar field must be enabled! (see
    ...
    **/
    ...
    ccDrawableObject::hasDisplayedScalarField)
    **/
    virtual const ecvColor::Rgb* getPointScalarValueColor(
            unsigned pointIndex) const = 0;

    //! Returns scalar value associated to a given point
    /** The returned value is taken from the current displayed scalar field
            WARNING: scalar field must be enabled! (see
    ccDrawableObject::hasDisplayedScalarField)
    **/
    ...
    /** WARNING: normals array must be enabled! (see
     *ccDrawableObject::hasNormals)
     **/
    virtual const CompressedNormType& getPointNormalIndex(
            unsigned pointIndex) const = 0;
    ...
    ***************************************************/
    ...
    /** \param visTable visibility table (optional, otherwise the cloud's
    default one will be used) \param silent don't issue warnings if no visible
    point is present \param selection input reference cloud to be used
    (optional) \return the visible points as a ReferenceCloud
    **/
    virtual cloudViewer::ReferenceCloud* getTheVisiblePoints(
    ...
    ***************************************************/
    ...
    //! Creates a new point cloud with only the 'visible' points (as defined by
    //! the visibility array)
    /** \param[in]  removeSelectedPoints if true, 'visible' points are also
    removed from the current point cloud \param[in]  visTable visibility table
    (optional, otherwise the cloud's default one will be used) \param[out]
    newIndexesOfRemainingPoints the new indexes of the remaining points (if
    removeSelectedPoints is true - optional). Must be initially empty or have
    the same size as the original cloud. \param[in]  silent don't issue a
    warning message if there's no point to keep \param[out] selection the
    corresponding point selection \return new point cloud with the 'visible'
    points (or the same cloud if all points are visible)
    **/
    virtual ccGenericPointCloud* createNewCloudFromVisibilitySelection(
            bool removeSelectedPoints = false,
            VisibilityTableType* visTable = nullptr,
            std::vector<int>* newIndexesOfRemainingPoints = nullptr,
            bool silent = false,
            cloudViewer::ReferenceCloud* selection = nullptr) = 0;

    //! Removes all the 'visible' points (as defined by the visibility array)
    /** \param visTable visibility table (optional, otherwise the cloud's
    default one will be used) \param newIndexes optional: stores the new indexes
    of the points (either an index >= 0 if kept, or -1 if not). Must be
    initially empty or have the same size as the original cloud. \return success
    **/
    virtual bool removeVisiblePoints(
            VisibilityTableType* visTable = nullptr,
    ...
    **/
    virtual cloudViewer::ReferenceCloud* crop(const ccBBox& box,
                                              bool inside = true) = 0;
