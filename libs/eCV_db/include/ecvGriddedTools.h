...
class ECV_DB_LIB_API ccGriddedTools {
public:
    ...
    /** \param cloud associated cloud
            ...
            \param verbose whether the process should output some detailed
    information in the log/console or not \param cloudToSensorTrans
    transformation from cloud coordinate system to the sensor coordinate system
    ...
    **/
    ...
    //! Determines the (TLS) sensor parameters from the relative position of
    //! gridded points
    /** \param cloud cloud on which to compute the sensor parameters (should be
    a single grid) \param grid scan grid \param cloudToSensorTrans
    transformation from cloud coordinate system to the sensor coordinate system
    (optional) \return sensor (if successful) or 0 otherwise
    **/
