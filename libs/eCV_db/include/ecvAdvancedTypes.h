...
public:
	...
	NormsIndexesTableType* clone() override
	{
		...
		if (!copy(*cloneArray))
		{
			CVLog::Warning("[NormsIndexesTableType::clone] Failed to clone array (not enough memory)");
			cloneArray->release();
			return nullptr;
		}
    ...
    ECV_DB_LIB_API bool fromFile_MeOnly(QFile& in, short dataVersion, int flags, LoadedIDMap& oldToNewIDMap) override;
};
...
public:
	...
	//inherited from ccArray/ccHObject
	CV_CLASS_ENUM getClassID() const override { return CV_TYPES::NORMALS_ARRAY; }
	...
	NormsTableType* clone() override
	{
		...
		if (!copy(*cloneArray))
		{
			CVLog::Warning("[NormsTableType::clone] Failed to clone array (not enough memory)");
			cloneArray->release();
			return nullptr;
		}
		cloneArray->setName(getName());
...
public:
	//! Default constructor
	ColorsTableType() : ccArray<ecvColor::Rgb, 3, ColorCompType>("RGB colors") {}
	virtual ~ColorsTableType() = default;

	//inherited from ccArray/ccHObject
	CV_CLASS_ENUM getClassID() const override { return CV_TYPES::RGB_COLOR_ARRAY; }
	...
	ColorsTableType* clone() override
	{
		...
		if (!copy(*cloneArray))
		{
			CVLog::Warning("[ColorsTableType::clone] Failed to clone array (not enough memory)");
			cloneArray->release();
			return nullptr;
		}
		cloneArray->setName(getName());
...
public:
	...
	//inherited from ccArray/ccHObject
	CV_CLASS_ENUM getClassID() const override { return CV_TYPES::TEX_COORDS_ARRAY; }
	...
	TextureCoordsContainer* clone() override
	{
		...
		if (!copy(*cloneArray))
		{
			CVLog::Warning("[TextureCoordsContainer::clone] Failed to clone array (not enough memory)");
			cloneArray->release();
			return nullptr;
		}
		cloneArray->setName(getName());
