...
**/
class ECV_DB_LIB_API ccSensor : public ccHObject {
public:
    //! Default constructor
    ...
    **/
    ...
    /** Precise definition of point's visibility can be found in Daniel
    Girardeau-Montaut's PhD manuscript (Chapter 2, section 2-3-3). In fact it
    can be anything, assuming that a point that is not POINT_VISIBLE won't be
    compared during a point-to-cloud distance computation process. \deprecated
    This method is deprecated and should be ignored. \param P a 3D point \return
    ...
    **/
    ...
    //! Returns the absolute transformation between the world and the "optical"
    //! center (shortcut)
    /** Absolute transformation corresponds to the rigid transformation
            multiplied by the associated transformation interpolated at the
    given index.
    **/
    bool getAbsoluteTransformation(ccIndexedTransformation& trans,
                                   double index) const;
    ...
    //! Sets the rigid transformation between this sensor and its associated
    //! positions
    /** Rigid transformation goes from the sensor position(s) to the sensor
     *"optical" center.
     **/
    ...
    //! Returns the rigid transformation between this sensor and its associated
    //! positions
    ...
    /** \param win 3D view to which to apply the sensor viewport (or the
    associated 'display' if 0) \return success
    **/
...
protected:
    ...
    /** The transformation goes from the sensor position(s) to the sensor
     *"optical" center.
     **/
