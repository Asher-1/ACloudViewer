...
/** A color scale is defined by several 'steps' corresponding to given colors.
        The color between each step is linearly interpolated. A valid color
scale must have at least 2 steps, one at relative position 0.0 (scale start) and
        one at relative position 1.0 (scale end). Steps can't be defined outside
        this interval.

        For faster access, a array of interpolated colors is maintained
internally. Be sure that the 'refresh' method has been called after any
modification(s) of the scale steps (position or color).
**/
class ECV_DB_LIB_API ccColorScale : public ccSerializableObject {
public:
    //! Shared pointer type
    ...
    /** \param name scale name
            ...
            Scale are 'relative' by default (can be changed afterwards, see
    setAbsolute). On construction they already have the two extreme steps
    defined (at position 0.0 and 1.0).
    **/
    ...
    struct Label {
        Label(double v) : value(v) {}

        Label(double v, const QString& t) : value(v), text(t) {}
    ...
    **/
    ...
    //! Returns relative position of a given value (wrt to scale absolute min
    //! and max)
    /** Warning: only valid with absolute scales! Use 'getColorByRelativePos'
     *otherwise.
     **/
    ...
    /** Warning: only valid with absolute scales!
            \param value value
            \param outOfRangeColor default color to return if relativePos if out
    of [0;1] \return corresponding color
    **/
    inline const ecvColor::Rgb* getColorByValue(
            double value,
            const ecvColor::Rgb* outOfRangeColor = nullptr) const {
        assert(m_updated && !m_relative);
        double relativePos = getRelativePosition(value);
        return (relativePos >= 0.0 && relativePos <= 1.0
                        ? getColorByRelativePos(relativePos)
                        : outOfRangeColor);
    }
    ...
    of [0;1] \return corresponding color
    **/
    inline const ecvColor::Rgb* getColorByRelativePos(
            double relativePos,
            const ecvColor::Rgb* outOfRangeColor = nullptr) const {
        assert(m_updated);
        if (relativePos >= 0.0 && relativePos <= 1.0)
            return &getColorByIndex(
                    static_cast<unsigned>(relativePos * (MAX_STEPS - 1)));
        else
    ...
    /** \param relativePos relative position (must be between 0 and 1!)
            \param steps desired resolution (must be greater than 1 and smaller
    than MAX_STEPS) \param outOfRangeColor default color to return if
    relativePos if out of [0;1] \return corresponding color
    **/
    inline const ecvColor::Rgb* getColorByRelativePos(
            double relativePos,
            unsigned steps,
    ...
    **/
    inline const ecvColor::Rgb& getColorByIndex(unsigned index) const {
