...
/** 3D plane primitive
 **/
class ECV_DB_LIB_API ccPlane : public ccGenericPrimitive,
...
public:
    ...
    **/
    ...
    void setXWidth(PointCoordinateType w, bool autoUpdate = true) {
        m_xWidth = w;
        if (autoUpdate) updateRepresentation();
    }
    ...
    static bool SetQuadTexture(ccMesh* quadMesh,
                               QImage image,
    ...
    object, i.e. even a ccPolyline object for instance. \param[in] cloud input
    cloud \param[out] rms plane fitting rms (optional) \return plane primitive
    (if successful)
    **/
    static ccPlane* Fit(cloudViewer::GenericIndexedCloudPersist* cloud,
                        double* rms = 0);
