...
protected:
	...
	uint8_t fillNode(ccPointCloudLOD::Node& node) const
	{
		...
		const unsigned char bitDec = CVLib::DgmOctree::GET_BIT_SHIFT(node.level);
		const CVLib::DgmOctree::CellCode currentTruncatedCellCode = (cellCodes[node.firstCodeIndex].theCode >> bitDec);
	...
	virtual void run()
	{
		...
		CVLog::Print(QString("[LoD] Preparing LoD acceleration structure for cloud '%1' [%2 points]...").arg(m_cloud.getName()).arg(pointCount));
		QElapsedTimer timer;
		timer.start();
		...
		if (!m_octree)
		{
			...
			{
				//not enough memory
				CVLog::Warning(QString("[LoD] Failed to compute octree on cloud '%1' (not enough memory)").arg(m_cloud.getName()));
				m_lod.setState(ccPointCloudLOD::BROKEN);
				return;
			}
		...
		{
			//not enough memory
			CVLog::Warning(QString("[LoD] Failed to compute LOD structure on cloud '%1' (not enough memory)").arg(m_cloud.getName()));
			m_lod.setState(ccPointCloudLOD::BROKEN);
			return;
		}
		...
		assert(m_maxLevel <= CVLib::DgmOctree::MAX_OCTREE_LEVEL);
		...
		for (uint8_t currentLevel = 0; currentLevel < m_maxLevel; ++currentLevel)
		{
			...
			CVLog::Print(QString("[LoD] Level %1: %2 cells").arg(currentLevel).arg(level.data.size()));
		...
		if (true)
		{
			...
			for (uint8_t currentLevel = 0; currentLevel < biggestLevel; ++currentLevel)
			{
				...
				CVLog::Print(QString("[LoD][pass 2] Level %1: %2 cells (+%3)").arg(currentLevel+1).arg(cellCountAfter).arg(cellCountAfter - cellCountBefore));
			}
		...
		CVLog::Print(QString("[LoD] Acceleration structure ready for cloud '%1' (max level: %2 / mem. = %3 Mb / duration: %4 s.)")
			.arg(m_cloud.getName())
			.arg(m_maxLevel)
...
bool ccPointCloudLOD::initInternal(ccOctree::Shared octree)
{
	...
	try
	{
		assert(CVLib::DgmOctree::MAX_OCTREE_LEVEL <= 255);
		m_levels.resize(CVLib::DgmOctree::MAX_OCTREE_LEVEL + 1);
	}
