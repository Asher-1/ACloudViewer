...
ccFacet* ccFacet::clone() const
{
	...
	if (m_originPoints)
	{
		...
		{
			CVLog::Warning(QString("[ccFacet::clone][%1] Failed to clone origin points!").arg(getName()));
			//delete facet;
			//return 0;
		}
		else
											...
											const PointCoordinateType* planeEquation/*=0*/)
{
	...
	if (!planeEquation)
	{
		...
		if (!planeEquation)
		{
			CVLog::Warning("[ccFacet::createInternalRepresentation] Failed to compute the LS plane passing through the input points!");
			return false;
		}
	}
	...
	//try to get the points on the convex/concave hull to build the contour and the polygon
	{
		...
		{
			CVLog::Error("[ccFacet::createInternalRepresentation] Failed to compute the convex hull of the input points!");
		}
		...
		{
			CVLog::Warning("[ccFacet::createInternalRepresentation] Not enough memory to create the contour mesh!");
		}
		...
		if (!hullPointsVector.empty() && CVLib::Delaunay2dMesh::Available())
		{
			...
			if (dm.buildMesh(hullPointsVector, 0, errorStr))
			{
				...
				if (m_polygonMesh->reserve(triCount))
				{
					...
					{
						CVLog::Warning("[ccFacet::createInternalRepresentation] Not enough memory to create the polygon mesh's normals!");
					}
			...
			{
				CVLog::Warning(QString("[ccFacet::createInternalRepresentation] Failed to create the polygon mesh (third party lib. said '%1'").arg(errorStr));
			}
		}
	}
