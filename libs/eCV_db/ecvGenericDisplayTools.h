...
#ifndef GENERIC_DISPLAY_TOOLS_HEADER
#define GENERIC_DISPLAY_TOOLS_HEADER
...
class ECV_DB_LIB_API GenericDispalyTools : public CVLib::CVToolbox
{

public:

	//! Default constructor
	GenericDispalyTools() = default;

	static GenericDispalyTools* s_genericTools;

	//! Default destructor
	virtual ~GenericDispalyTools() {}
public:
	...
	inline static bool GetPerspectiveState() { assert(s_genericTools);  return s_genericTools->getPerspectiveState(); }
	inline virtual bool getPerspectiveState() { return false; }

	template <typename iType, typename oType>
	inline static void ToWorldPoint(const Vector3Tpl<iType>& input2D, Vector3Tpl<oType>& output3D) { s_genericTools->toWorldPoint(input2D, output3D); }
	...
	template <typename iType, typename oType>
	inline static void ToDisplayPoint(const Vector3Tpl<iType>& input3D, Vector3Tpl<oType>& output2D) { s_genericTools->toDisplayPoint(input3D, output2D); }
...
struct ECV_DB_LIB_API ccGLCameraParameters
{
	...
	//! Unprojects a 2D point (+ normalized 'z' coordinate) in 3D
	inline bool unproject(const CCVector3& input2D, CCVector3d& output3D) const { return GenericDispalyTools::Unproject<PointCoordinateType, double>(input2D, modelViewMat.data(), projectionMat.data(), viewport, output3D); }
...
#endif // GENERIC_DISPLAY_TOOLS_HEADER