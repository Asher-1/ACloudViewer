...
bool ccOctree::intersectWithFrustum(ccCameraSensor* sensor, std::vector<unsigned>& inCameraFrustum)
{
	...
	if (!m_frustumIntersector)
	{
		...
		{
			CVLog::Warning("[ccOctree::intersectWithFrustum] Not enough memory!");
			return false;
		}
	}
...
void ccOctree::draw(CC_DRAW_CONTEXT& context)
{
	...
	else
	{
		...
		{
			//ccGL::Color3v(glFunc, ecvColor::white.rgb);
		}
								...
								CVLib::NormalizedProgress* nProgress/*=0*/)
{
	...
	if (glParams->showSF)
	{
		...
		const ecvColor::Rgb* col = cloud->geScalarValueColor(dist);
									...
									CVLib::NormalizedProgress* nProgress/*=0*/)
{
	...
	{
		ScalarType dist = CVLib::ScalarFieldTools::computeMeanScalarValue(cell.points);
		const ecvColor::Rgb* rgb = cloud->geScalarValueColor(dist);
		if (rgb)
...
void ccOctree::ComputeAverageColor(CVLib::ReferenceCloud* subset, ccGenericPointCloud* sourceCloud, ColorCompType meanCol[])
{
	...
	{
		const ecvColor::Rgb& _theColor = sourceCloud->getPointColor(subset->getPointGlobalIndex(i));
		sum.x += _theColor.r;
		sum.y += _theColor.g;
		sum.z += _theColor.b;
	}
