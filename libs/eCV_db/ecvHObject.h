...
public: //construction
	...
	**/
	...
	static void ResizeAndPaintUniformColor(
		std::vector<Eigen::Vector3d>& colors,
		const size_t size, const Eigen::Vector3d& color);
	...
	virtual Eigen::Vector3d getGeometryCenter() const { return Eigen::Vector3d(); }
	...
	virtual ccHObject& transform(const Eigen::Matrix4d& transformation) { return *this; }
	/// \brief Apply translation to the geometry coordinates.
	///
	/// \param translation A 3D vector to transform the geometry.
	/// \param relative If `true`, the \p translation is directly applied to the
	/// geometry. Otherwise, the geometry center is moved to the \p translation.
	virtual ccHObject& translate(const Eigen::Vector3d& translation, bool relative = true) { return *this; }
	...
	virtual ccHObject& scale(const double s, 
							 const Eigen::Vector3d& center) { return *this; }
	virtual ccHObject& scale(const double s) { 
...
public: //base members access
	...
	**/
	inline CV_CLASS_ENUM getClassID() const override { return CV_TYPES::HIERARCHY_OBJECT; }

	//! Returns whether the instance is a group
	inline bool isGroup() const { return getClassID() == static_cast<CV_CLASS_ENUM>(CV_TYPES::HIERARCHY_OBJECT); }
