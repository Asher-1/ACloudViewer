...
void cc2DLabel::drawMeOnly3D(CC_DRAW_CONTEXT& context)
{
		...
		//display point marker as spheres
		{
			if (!c_unitTriMarker)
			{
				CVLib::GenericIndexedCloudPersist* cloud = nullptr;
				{
					...
					*A = *(m_points[0].cloud->getPoint(m_points[0].index));
					*B = *(m_points[1].cloud->getPoint(m_points[1].index));
					*C = *(m_points[2].cloud->getPoint(m_points[2].index));
	...
	case 2:
	{
		if (count == 2)
		{
			...
			const CCVector3* lineSt = m_points[0].cloud->getPoint(m_points[0].index);
			const CCVector3* lineEd = m_points[1].cloud->getPoint(m_points[1].index);
