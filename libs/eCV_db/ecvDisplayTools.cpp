...
void ecvDisplayTools::Init(ecvDisplayTools* displayTools, ecvMainAppInterface* app, QMainWindow* win)
{
	...
	GenericDispalyTools::s_genericTools = s_tools.instance;
													 ...
													 ProjectionMetrics* metrics/*=0*/, double* eyeOffset/*=0*/)
{
	...
	if (s_tools.instance->m_viewportParams.perspectiveView)
	{
		...
		return GenericDispalyTools::Frustum(-xMax - frustumAsymmetry, xMax - frustumAsymmetry, -yMax, yMax, zNear, zFar);
	}
	else
	{
		...
		return GenericDispalyTools::Ortho(halfW, halfH, maxDist_pix);
	}
}
