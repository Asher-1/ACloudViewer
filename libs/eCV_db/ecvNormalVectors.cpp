	...
	case PREVIOUS:
		{
			...
			{
				CVLog::Warning("[UpdateNormalOrientations] Can't orient the new normals with the previous ones... as the cloud has no normals!");
				return false;
			}
		}
		break;
														...
														CVLib::GenericProgressCallback* progressCb/*=0*/)
{
	...
	if (!octree)
	{
		...
		{
			delete octree;
			CVLog::Warning("[GuessBestRadius] Failed to compute the cloud octree");
			return 0;
		}
	}
	...
	//we are now going to sample the cloud so as to compute statistics on the density
	{
		...
		for (size_t attempt = 0; attempt < 10; ++attempt)
		{
			...
			CVLog::Print(QString("[GuessBestRadius] Radius = %1 -> samples population in [%2 ; %3] (mean %4 / std. dev. %5 / %6% above mininmum)")
										.arg(radius)
										.arg(minPop)
										.arg(maxPop)
										.arg(meanPop)
			...
			if (fabs(meanPop - aimedPop) < s_aimedPopRange)
			{
				...
				{
					//CVLog::Warning("[GuessBestRadius] The cloud density is very inhomogeneous! You may have to increase the radius to get valid normals everywhere... but the result will be smoother");
...
void ccNormalVectors::ConvertNormalToStrikeAndDip(const CCVector3& N, PointCoordinateType& strike_deg, PointCoordinateType& dip_deg)
{
	...
	{
		strike_deg = 180.0 - atan2(N.y, N.x)*CV_RAD_TO_DEG;		//atan2 output is between -180 and 180! So strike is always positive here
		PointCoordinateType x = sqrt(N.x*N.x + N.y*N.y);		//x is the horizontal magnitude
		dip_deg = atan2(x, N.z)*CV_RAD_TO_DEG;
	}
	else
	{
...
void ccNormalVectors::ConvertNormalToDipAndDipDir(const CCVector3& N, PointCoordinateType& dip_deg, PointCoordinateType& dipDir_deg)
{
	...
	if (N.norm2d() > std::numeric_limits<PointCoordinateType>::epsilon())
	{
		...
		dipDir_deg = static_cast<PointCoordinateType>(dipDir_rad * CV_RAD_TO_DEG);
		dip_deg = static_cast<PointCoordinateType>(dip_rad * CV_RAD_TO_DEG);
	}
	else
	{
...
CCVector3 ccNormalVectors::ConvertDipAndDipDirToNormal(PointCoordinateType dip_deg, PointCoordinateType dipDir_deg, bool upward/*=true*/)
{
	...
	double Nz = cos(dip_deg * CV_DEG_TO_RAD);
	double Nxy = sqrt(1.0 - Nz * Nz);
	double dipDir_rad = dipDir_deg * CV_DEG_TO_RAD;
