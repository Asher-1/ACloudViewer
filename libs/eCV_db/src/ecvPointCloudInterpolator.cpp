															...
															unsigned char octreeLevel/*=0*/)
{
	...
	{
		CVLog::Warning("[InterpolateScalarFieldsFrom] Invalid/empty input cloud(s)!");
		return false;
	}
	...
	{
		CVLog::Warning("[InterpolateScalarFieldsFrom] Clouds are too far from each other! Can't proceed.");
		return false;
	}
	...
	for (size_t i = 0; i < inSFIndexes.size(); ++i)
	{
		...
		{
			//invalid index
			CVLog::Warning(QString("[InterpolateScalarFieldsFrom] Source cloud has no scalar field with index #%1").arg(inSFIndex));
			assert(false);
			return false;
		}
	...
	else
	{
		...
		try
		{
																...
																0) == 0)
			{
				...
				CVLog::Warning("[InterpolateScalarFieldsFrom] Failed to perform the interpolation");
				return false;
			}
		}
		catch (const std::bad_alloc&)
	...
	if (overwrite)
	{
		CVLog::Warning("[InterpolateScalarFieldsFrom] Some scalar fields with the same names have been overwritten");
	}
