...
bool WaveformDescriptor::fromFile(QFile& in,
    ...
    if (dataVersion < 44) return false;
...
uint32_t ccWaveform::getRawSample(uint32_t i,
    ...
    switch (descriptor.bitsPerSample) {
        ...
        default:  // other 'strange' bps values ;)
        {
            ...
            uint32_t lastBitIndex =
                    firstBitIndex + descriptor.bitsPerSample - 1;
...
bool ccWaveform::toASCII(const QString& filename,
    ...
    if (!decodeSamples(values, descriptor, dataStorage)) {
        CVLog::Warning(QString("[ccWaveform::toASCII] Not enough memory"));
        return false;
    }
...
bool ccWaveform::ToASCII(const QString& filename,
    ...
    if (!file.open(QFile::Text | QFile::WriteOnly)) {
        CVLog::Warning(QString("[ccWaveform::toASCII] Failed to open file '%1' "
                               "for writing")
                               .arg(filename));
        return false;
    }
    ...
    file.close();
    CVLog::Print(QString("[ccWaveform::toASCII] File '%1' has been saved "
                         "successfully")
                         .arg(filename));
...
CCVector3 ccWaveform::getSamplePos(float index,
                                   const CCVector3& P0,
...
bool ccWaveform::toFile(QFile& out) const {
    ...
    if (m_descriptorID != 0)  // no need to save invalid waveforms
    {
        ...
        outStream << static_cast<quint64>(
                m_dataOffset);  // see comment below (in 'fromFile')
...
bool ccWaveform::fromFile(QFile& in,
    ...
    if (dataVersion < 46) return false;
