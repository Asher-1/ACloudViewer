...
bool ccWaveform::ToASCII(const QString& filename, std::vector<double>& values, uint32_t samplingRate_ps)
{
	...
	{
		CVLog::Warning(QString("[ccWaveform::toASCII] Failed to open file '%1' for writing").arg(filename));
		return false;
	}
	...
	file.close();
	CVLog::Print(QString("[ccWaveform::toASCII] File '%1' has been saved successfully").arg(filename));
