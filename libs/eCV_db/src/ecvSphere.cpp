...
ccSphere::ccSphere(PointCoordinateType radius,
    ...
    : ccGenericPrimitive(name, transMat), m_radius(radius) {
...
ccGenericPrimitive* ccSphere::clone() const {
    return finishCloneJob(new ccSphere(m_radius, &m_transformation, getName(),
                                       m_drawPrecision));
}
...
bool ccSphere::buildUp() {
    ...
    if (!init(count, true, faces, 0)) {
        CVLog::Error("[ccSphere::buildUp] Not enough memory");
        return false;
    }
        ...
        for (unsigned j = 1; j < steps; ++j) {
            ...
            for (unsigned i = 0; i < steps; ++i) {
                PointCoordinateType phi =
                        static_cast<PointCoordinateType>(2 * i) *
                        angle_rad_step;
