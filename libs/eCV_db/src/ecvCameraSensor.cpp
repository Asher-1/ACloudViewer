...
bool ccCameraSensor::FrustumInformation::initFrustumHull()
{
	...
	{
		CVLog::Warning("[ccCameraSensor::FrustumInformation::initFrustumHull] Corners are not initialized!");
		return false;
	}
	...
	{
		CVLog::Warning("[ccCameraSensor::FrustumInformation::initFrustumHull] Not enough memory!");
		delete frustumHull;
        frustumHull = nullptr;
		return false;
	}
...
bool ccCameraSensor::applyViewport()
{
	...
	{
		CVLog::Warning("[ccCameraSensor::applyViewport] No associated display!");
		return false;
	}
...
bool ccCameraSensor::computeUncertainty(cloudViewer::ReferenceCloud* points, std::vector< Vector3Tpl<ScalarType> >& accuracy/*, bool lensCorrection*/)
{
	...
	{
		CVLog::Warning("[ccCameraSensor::computeUncertainty] Internal error: invalid input cloud");
		return false;
	}

	if (!m_distortionParams || m_distortionParams->getModel() != BROWN_DISTORTION)
	{
		CVLog::Warning("[ccCameraSensor::computeUncertainty] Sensor has no associated uncertainty model! (Brown, etc.)");
		return false;
	}
	...
	{
		CVLog::Warning("[ccCameraSensor::computeUncertainty] Not enough memory!");
		return false;
	}
...
QImage ccCameraSensor::undistort(const QImage& image) const
{
	...
	case EXTENDED_RADIAL_DISTORTION:
		{
			...
			if (k1 == 0 && k2 == 0)
			{
				CVLog::Warning("[ccCameraSensor::undistort] Invalid radial distortion coefficients!");
				return QImage();
			}
			float k3 = 0;
			...
			if (newImage.isNull())
			{
				CVLog::Warning("[ccCameraSensor::undistort] Not enough memory!");
				return QImage();
			}
			newImage.fill(0);
	...
	CVLog::Warning("[ccCameraSensor::undistort] Can't undistort the image with the current distortion model!");
...
ccImage* ccCameraSensor::undistort(ccImage* image, bool inplace/*=true*/) const
{
	...
	{
		CVLog::Warning("[ccCameraSensor::undistort] Invalid/empty input image!");
        return nullptr;
	}
...
CCVector3 ccCameraSensor::computeUpperLeftPoint() const
{
    ...
    float ar = m_intrinsicParams.arrayHeight != 0 ?
                static_cast<float>(m_intrinsicParams.arrayWidth) / m_intrinsicParams.arrayHeight : 1.0f;
...
bool ccCameraSensor::computeFrustumCorners()
{
	if (m_intrinsicParams.arrayHeight == 0)
	{
		CVLog::Warning("[ccCameraSensor::computeFrustumCorners] Sensor height is 0!");
		return false;
	}
	...
	{
		CVLog::Warning("[ccCameraSensor::computeFrustumCorners] Not enough memory!");
		return false;
	}
...
{
    CCVector3d upperLeftPointd = CCVector3d::fromArray(computeUpperLeftPoint().data());

    // near plane
    m_nearPlane.clear();
    m_nearPlane.points_.push_back(Eigen::Vector3d(upperLeftPointd.x,  upperLeftPointd.y,  -upperLeftPointd.z));
    m_nearPlane.points_.push_back(Eigen::Vector3d(-upperLeftPointd.x, upperLeftPointd.y,  -upperLeftPointd.z));
    m_nearPlane.points_.push_back(Eigen::Vector3d(-upperLeftPointd.x, -upperLeftPointd.y, -upperLeftPointd.z));
    m_nearPlane.points_.push_back(Eigen::Vector3d(upperLeftPointd.x,  -upperLeftPointd.y, -upperLeftPointd.z));
    ...
    m_sideLines.points_.push_back(Eigen::Vector3d(upperLeftPointd.x,  upperLeftPointd.y, -upperLeftPointd.z));
    m_sideLines.points_.push_back(Eigen::Vector3d(-upperLeftPointd.x,  upperLeftPointd.y, -upperLeftPointd.z));
    m_sideLines.points_.push_back(Eigen::Vector3d(-upperLeftPointd.x, -upperLeftPointd.y, -upperLeftPointd.z));
    m_sideLines.points_.push_back(Eigen::Vector3d(upperLeftPointd.x, -upperLeftPointd.y, -upperLeftPointd.z));
    ...
    // arrow
    m_arrow.clear();
    m_arrow.points_.push_back(Eigen::Vector3d(-baseHalfWidth, baseHeight, -upperLeftPointd.z));
    m_arrow.points_.push_back(Eigen::Vector3d(baseHalfWidth, baseHeight, -upperLeftPointd.z));
    m_arrow.points_.push_back(Eigen::Vector3d(baseHalfWidth, upperLeftPointd.y, -upperLeftPointd.z));
    m_arrow.points_.push_back(Eigen::Vector3d(-baseHalfWidth, upperLeftPointd.y, -upperLeftPointd.z));
    ...
    m_arrow.points_.push_back(Eigen::Vector3d(-arrowHalfWidth, baseHeight,  -upperLeftPointd.z));
    m_arrow.points_.push_back(Eigen::Vector3d(arrowHalfWidth, baseHeight,  -upperLeftPointd.z));
    m_arrow.points_.push_back(Eigen::Vector3d(0, arrowHeight, -upperLeftPointd.z));
...
void ccCameraSensor::drawMeOnly(CC_DRAW_CONTEXT& context)
{
    ...
    // we must remove the 'push name flag' so that the primitives don't push their own!
    ...
    if (m_frustumInfos.drawFrustum || m_frustumInfos.drawSidePlanes)
    {
        ...
        if (m_frustumInfos.frustumCorners && m_frustumInfos.frustumCorners->size() >= 8)
        {
            ...
            if (m_frustumInfos.drawFrustum)
            {
                ...
                m_sideLines.points_.push_back(CCVector3d::fromArray(*P5)); // 10
                m_sideLines.points_.push_back(CCVector3d::fromArray(*P6)); // 11
                m_sideLines.points_.push_back(CCVector3d::fromArray(*P7)); // 12
...
float ccCameraSensor::ConvertFocalPixToMM(float focal_pix, float ccdPixelSize_mm)
{
	...
	{
		CVLog::Warning("[ccCameraSensor::convertFocalPixToMM] Invalid CCD pixel size! (<= 0)");
		return -1.0f;
	}
...
float ccCameraSensor::ConvertFocalMMToPix(float focal_mm, float ccdPixelSize_mm)
{
	if (ccdPixelSize_mm < FLT_EPSILON)
	{
		CVLog::Warning("[ccCameraSensor::convertFocalMMToPix] Invalid CCD pixel size! (<= 0)");
		return -1.0f;
	}
	...
	double c_out[3]) const
{
	...
	//conjugate gradient iterations
	{
		...
		for (unsigned i = 0; i < 1500; ++i)
		{
			...
			{
				CVLog::PrintDebug(QString("[computeOrthoRectificationParams] Convergence reached in %1 iteration(s) (error: %2)").arg(i + 1).arg(lastError));
				break;
			}
		}
	}
	...
	std::vector<KeyPoint>& keypointsImage) const
{
	...
	if (realCount == 0)
	{
		CVLog::Warning(QString("[orthoRectifyAsCloud] Image '%1' was black, nothing to project!").arg(image->getName()));
		delete proj;
        proj = nullptr;
	}
	else
