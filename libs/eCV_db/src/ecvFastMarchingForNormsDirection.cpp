													...
													ecvProgressDialog* progressCb)
{
	if (!cloud || !cloud->normals())
	{
		...
		CVLog::Warning(QString("[orientNormalsWithFM] Cloud '%1' is invalid (or cloud has no normals)").arg( name ));
		assert(false);
		return 0;
	}
	...
	if (!cloud->getOctree())
	{
		...
		{
			CVLog::Warning(QString("[orientNormalsWithFM] Could not compute octree on cloud '%1'").arg(cloud->getName()));
			return 0;
		}
	}
	...
	{
		CVLog::Warning("[orientNormalsWithFM] Couldn't create temporary scalar field! Not enough memory?");
		return -3;
	}

	if (!cloud->enableScalarField())
	{
		CVLog::Warning("[orientNormalsWithFM] Couldn't enable temporary scalar field! Not enough memory?");
	...
	if (result < 0)
	{
		CVLog::Error("[orientNormalsWithFM] Something went wrong during initialization...");
		cloud->deleteScalarField(sfIdx);
		cloud->setCurrentScalarField(oldSfIdx);
		return -6;
	}
	...
	while (success)
	{
		...
		{
			CVLog::Error("An error occurred during front propagation! Process cancelled...");
			success = false;
		}
	}
