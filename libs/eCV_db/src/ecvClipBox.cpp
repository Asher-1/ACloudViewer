...
void DrawUnitArrow(int ID, const CCVector3& start, const CCVector3& direction, PointCoordinateType scale, const ecvColor::Rgb& col, CC_DRAW_CONTEXT& context)
{
	...
	if (ps < 1)
	{
		if (ps > -1)
		{
			...
			angle_deg = acos(ps) * static_cast<PointCoordinateType>(CV_RAD_TO_DEG);
...
//Unused function
//static void DrawUnitSphere(int ID, const CCVector3& center, PointCoordinateType radius, const ecvColor::Rgb& col, CC_DRAW_CONTEXT& context)
//{
...
static void DrawUnitCross(int ID, const CCVector3& center, PointCoordinateType scale, const ecvColor::Rgb& col, CC_DRAW_CONTEXT& context)
{
	...
	DrawUnitArrow(10*ID+2, center, CCVector3( 0,-1, 0), scale, col, context);
	DrawUnitArrow(10*ID+3, center, CCVector3( 0, 1, 0), scale, col, context);
	DrawUnitArrow(10*ID+4, center, CCVector3( 0, 0,-1), scale, col, context);
...
void ccClipBox::hideShowAllInterators(bool visibility)
{
	...
	ecvDisplayTools::HideShowEntities(removeViewIds, ENTITY_TYPE::ECV_MESH, visibility);
}
...
void ccClipBox::drawMeOnly(CC_DRAW_CONTEXT& context)
{
	...
	//draw the interactors
	{
		...
		//we must remove the 'push name flag' so that the arows don't push their own!
		componentContext.drawingFlags &= (~CC_DRAW_ENTITY_NAMES);

		pushName = true;
		DrawUnitArrow(X_MINUS_ARROW*pushName, CCVector3(minC.x, center.y, center.z), CCVector3(-1.0, 0.0, 0.0), scale, ecvColor::red, componentContext);
		DrawUnitArrow(X_PLUS_ARROW*pushName, CCVector3(maxC.x, center.y, center.z), CCVector3(1.0, 0.0, 0.0), scale, ecvColor::red, componentContext);
		DrawUnitArrow(Y_MINUS_ARROW*pushName, CCVector3(center.x, minC.y, center.z), CCVector3(0.0, -1.0, 0.0), scale, ecvColor::green, componentContext);
		DrawUnitArrow(Y_PLUS_ARROW*pushName, CCVector3(center.x, maxC.y, center.z), CCVector3(0.0, 1.0, 0.0), scale, ecvColor::green, componentContext);
		DrawUnitArrow(Z_MINUS_ARROW*pushName, CCVector3(center.x, center.y, minC.z), CCVector3(0.0, 0.0, -1.0), scale, ecvColor::blue, componentContext);
		DrawUnitArrow(Z_PLUS_ARROW*pushName, CCVector3(center.x, center.y, maxC.z), CCVector3(0.0, 0.0, 1.0), scale, ecvColor::blue, componentContext);
		DrawUnitCross(CROSS*pushName, minC - CCVector3(scale, scale, scale) / 2.0, scale, ecvColor::yellow, componentContext);
		//DrawUnitSphere(SPHERE*pushName, maxC + CCVector3(scale, scale, scale) / 2.0, scale / 2.0, ecvColor::yellow, componentContext);
