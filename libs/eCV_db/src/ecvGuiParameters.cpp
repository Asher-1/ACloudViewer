...
void ecvGui::ReleaseInstance() { s_gui.release(); }
...
ecvGui::ParamStruct::ParamStruct() { reset(); }
...
void ecvGui::ParamStruct::fromPersistentSettings() {
    ...
    lightAmbientColor = ecvColor::Rgbaf(reinterpret_cast<float*>(
            settings.value("lightAmbientColor",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::darkest.rgba,
                                   c_fColorArraySize))
                    .toByteArray()
                    .data()));
    lightSpecularColor = ecvColor::Rgbaf(reinterpret_cast<float*>(
            settings.value("lightSpecularColor",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::darker.rgba,
                                   c_fColorArraySize))
                    .toByteArray()
                    .data()));
    lightDiffuseColor = ecvColor::Rgbaf(reinterpret_cast<float*>(
            settings.value("lightDiffuseColor",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::bright.rgba,
                                   c_fColorArraySize))
                    .toByteArray()
                    .data()));
    meshFrontDiff = ecvColor::Rgbaf(reinterpret_cast<float*>(
            settings.value("meshFrontDiff",
                           QByteArray::fromRawData(
                                   (const char*)
                                           ecvColor::defaultMeshFrontDiff.rgba,
                                   c_fColorArraySize))
                    .toByteArray()
                    .data()));
    meshBackDiff = ecvColor::Rgbaf(reinterpret_cast<float*>(
            settings.value("meshBackDiff",
                           QByteArray::fromRawData(
                                   (const char*)
                                           ecvColor::defaultMeshBackDiff.rgba,
                                   c_fColorArraySize))
                    .toByteArray()
                    .data()));
    meshSpecular = ecvColor::Rgbaf(reinterpret_cast<float*>(
            settings.value("meshSpecular",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::middle.rgba,
                                   c_fColorArraySize))
                    .toByteArray()
                    .data()));
    pointsDefaultCol = ecvColor::Rgbaub(reinterpret_cast<unsigned char*>(
            settings.value("pointsDefaultColor",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::defaultColor.rgb,
                                   c_ubColorArraySize))
                    .toByteArray()
                    .data()));
    textDefaultCol = ecvColor::Rgbaub(reinterpret_cast<unsigned char*>(
            settings.value("textDefaultColor",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::defaultColor.rgb,
                                   c_ubColorArraySize))
                    .toByteArray()
                    .data()));
    backgroundCol = ecvColor::Rgbaub(reinterpret_cast<unsigned char*>(
            settings.value("backgroundColor",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::defaultBkgColor.rgb,
                                   c_ubColorArraySize))
                    .toByteArray()
                    .data()));
    labelBackgroundCol = ecvColor::Rgbaub(reinterpret_cast<unsigned char*>(
            settings.value("labelBackgroundColor",
                           QByteArray::fromRawData(
                                   (const char*)
                                           ecvColor::defaultLabelBkgColor.rgb,
                                   c_ubColorArraySize))
                    .toByteArray()
                    .data()));
    labelMarkerCol = ecvColor::Rgbaub(reinterpret_cast<unsigned char*>(
            settings.value("labelMarkerColor",
                           QByteArray::fromRawData(
                                   (const char*)
                                           ecvColor::defaultLabelMarkerColor
                                                   .rgb,
                                   c_ubColorArraySize))
                    .toByteArray()
                    .data()));
    bbDefaultCol = ecvColor::Rgbaub(reinterpret_cast<unsigned char*>(
            settings.value("bbDefaultColor",
                           QByteArray::fromRawData(
                                   (const char*)ecvColor::yellow.rgb,
                                   c_ubColorArraySize))
                    .toByteArray()
                    .data()));
    ...
    labelMarkerSize = static_cast<unsigned>(
            std::max(0, settings.value("labelMarkerSize", 5).toInt()));
    colorScaleShowHistogram =
            settings.value("colorScaleShowHistogram", true).toBool();
    colorScaleUseShader = settings.value("colorScaleUseShader", false).toBool();
    // colorScaleShaderSupported	= not saved
    colorScaleRampWidth = static_cast<unsigned>(
            std::max(0, settings.value("colorScaleRampWidth", 50).toInt()));
    defaultFontSize = static_cast<unsigned>(
            std::max(0, settings.value("defaultFontSize", 10).toInt()));
    labelFontSize = static_cast<unsigned>(
            std::max(0, settings.value("labelFontSize", 8).toInt()));
    displayedNumPrecision = static_cast<unsigned>(
            std::max(0, settings.value("displayedNumPrecision", 6).toInt()));
    labelOpacity = static_cast<unsigned>(
            std::max(0, settings.value("labelOpacity", 75).toInt()));
    zoomSpeed = settings.value("zoomSpeed", 1.0).toDouble();
    autoComputeOctree = static_cast<ComputeOctreeForPicking>(
            settings.value("autoComputeOctree", ASK_USER).toInt());
...
void ecvGui::ParamStruct::toPersistentSettings() const {
    ...
    settings.setValue(
            "lightDiffuseColor",
            QByteArray((const char*)lightDiffuseColor.rgba, c_fColorArraySize));
    settings.setValue(
            "lightAmbientColor",
            QByteArray((const char*)lightAmbientColor.rgba, c_fColorArraySize));
    settings.setValue("lightSpecularColor",
                      QByteArray((const char*)lightSpecularColor.rgba,
                                 c_fColorArraySize));
    settings.setValue(
            "meshFrontDiff",
            QByteArray((const char*)meshFrontDiff.rgba, c_fColorArraySize));
    settings.setValue("meshBackDiff", QByteArray((const char*)meshBackDiff.rgba,
                                                 c_fColorArraySize));
    settings.setValue("meshSpecular", QByteArray((const char*)meshSpecular.rgba,
                                                 c_fColorArraySize));
    settings.setValue(
            "pointsDefaultColor",
            QByteArray((const char*)pointsDefaultCol.rgb, c_ubColorArraySize));
    settings.setValue(
            "textDefaultColor",
            QByteArray((const char*)textDefaultCol.rgb, c_ubColorArraySize));
    settings.setValue(
            "backgroundColor",
            QByteArray((const char*)backgroundCol.rgb, c_ubColorArraySize));
    settings.setValue("labelBackgroundColor",
                      QByteArray((const char*)labelBackgroundCol.rgb,
                                 c_ubColorArraySize));
    settings.setValue(
            "labelMarkerColor",
            QByteArray((const char*)labelMarkerCol.rgb, c_ubColorArraySize));
    settings.setValue(
            "bbDefaultColor",
