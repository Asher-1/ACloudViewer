...
bool ecvViewportParameters::fromFile(QFile& in, short dataVersion, int flags)
{
    ...
    if (dataVersion < 51 && zoom != 1.0f)
    {
        ...
        CVLog::Warning("[ecvViewportParameters] Approximate focal distance (sorry, the parameters of viewport objects have changed!)");
    }
...
void ecvViewportParameters::setPivotPoint(const CCVector3d& P, bool autoUpdateFocal)
{
    pivotPoint = P;
...
void ecvViewportParameters::setCameraCenter(const CCVector3d& C, bool autoUpdateFocal)
{
    cameraCenter = C;
    if (autoUpdateFocal && objectCenteredView)
    {
