...
bool ccMaterialSet::saveAsMTL(QString path,
    ...
    stream << "# Generated by CLOUDVIEWER " << QtCompat::endl;
    ...
    for (ccMaterialSet::const_iterator it = begin(); it != end();
        ...
        stream << QtCompat::endl
               << "newmtl " << mtl->getName() << QtCompat::endl;
        ...
        stream << "Ka " << Ka.r << " " << Ka.g << " " << Ka.b << QtCompat::endl;
        stream << "Kd " << Kd.r << " " << Kd.g << " " << Kd.b << QtCompat::endl;
        stream << "Ks " << Ks.r << " " << Ks.g << " " << Ks.b << QtCompat::endl;

        // Emission color (if not black/default)
        if (Ke.r > 0.0f || Ke.g > 0.0f || Ke.b > 0.0f) {
            stream << "Ke " << Ke.r << " " << Ke.g << " " << Ke.b
                   << QtCompat::endl;
        }
        ...
        if (transparency < 1.0f) {
            stream << "d " << transparency << QtCompat::endl;
        }

        // Illumination model
        stream << "illum " << mtl->getIllum() << QtCompat::endl;

        // Shininess
        stream << "Ns " << mtl->getShininessFront() << QtCompat::endl;

        // PBR scalar parameters (only save if non-default values)
        if (mtl->getMetallic() > 0.0f) {
            stream << "Pm " << mtl->getMetallic() << QtCompat::endl;
        }
        if (mtl->getRoughness() != 0.5f) {
            stream << "Pr " << mtl->getRoughness() << QtCompat::endl;
        }
        if (mtl->getSheen() > 0.0f) {
            stream << "Ps " << mtl->getSheen() << QtCompat::endl;
        }
        if (mtl->getClearcoat() > 0.0f) {
            stream << "Pc " << mtl->getClearcoat() << QtCompat::endl;
        }
        if (mtl->getClearcoatRoughness() > 0.0f) {
            stream << "Pcr " << mtl->getClearcoatRoughness() << QtCompat::endl;
        }
        if (mtl->getAnisotropy() > 0.0f) {
            stream << "aniso " << mtl->getAnisotropy() << QtCompat::endl;
        }
        if (mtl->getAmbientOcclusion() > 0.0f) {
            stream << "Pa " << mtl->getAmbientOcclusion() << QtCompat::endl;
        }
        ...
        for (const auto& texPair : allTextures) {
            ...
            stream << mapCommand << " " << relativeFilename << QtCompat::endl;
        }
        ...
        if (mtl->hasTexture() &&
            ...
            if (!relativeFilename.isEmpty()) {
                stream << "map_Kd " << relativeFilename << QtCompat::endl;
            }
        }
    }
