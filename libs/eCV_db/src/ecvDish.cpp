...
ccDish::ccDish(PointCoordinateType radius,
    ...
    if (radius2 == 0)
        m_height = std::min(
                height,
                radius);  // in spherical mode, the height can't be superior to
                          // the radius! (dishes are at most hemispheres)
...
ccGenericPrimitive* ccDish::clone() const {
    return finishCloneJob(new ccDish(m_baseRadius, m_height, m_secondRadius,
...
bool ccDish::buildUp() {
    ...
    if (m_height <= 0 || m_baseRadius <= 0 ||
        m_secondRadius < 0)  // invalid parameters
        return false;
    ...
    if (m_secondRadius == 0 &&
        m_height < m_baseRadius)  // partial spherical mode
    {
        realRadius = (m_height * m_height + m_baseRadius * m_baseRadius) /
                     (2 * m_height);
    ...
    unsigned sectionSteps = static_cast<unsigned>(ceil(
            (endAngle_rad - startAngle_rad) * m_drawPrecision / (2.0 * M_PI)));
    double sectionAngleStep_rad =
            (endAngle_rad - startAngle_rad) / sectionSteps;
    ...
    if (!init(vertCount, true, faceCount, 0)) {
        CVLog::Error("[ccDish::buildUp] Not enough memory");
        return false;
    }
    ...
    // then, angular sweep
    {
        for (unsigned j = 1; j <= sectionSteps; ++j) {
            PointCoordinateType theta = static_cast<PointCoordinateType>(
                    endAngle_rad -
                    j * sectionAngleStep_rad);  // we start from north pole!
            ...
            for (unsigned i = 0; i < steps;
                 ++i)  // then we make a full revolution
            {
                PointCoordinateType phi =
                        static_cast<PointCoordinateType>(i * angleStep_rad);
...
bool ccDish::fromFile_MeOnly(QFile& in,
                             short dataVersion,
                             int flags,
