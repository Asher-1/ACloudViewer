...
TetraMesh &TetraMesh::RemoveDuplicatedVertices() {
    ...
    std::unordered_map<Coordinate3, size_t,
                       cloudViewer::utility::hash_tuple<Coordinate3>>
            point_to_old_index;
    ...
    for (size_t tetra_i = 0; tetra_i < tetras_.size(); ++tetra_i) {
        ...
        for (int i = 0; i < num_verts; ++i) {
            if (intersecting_edges.count(keys[i]) == 0) {
                ...
                baseVertices->addEigenPoint(verts[i]);
            } else {
        ...
        if (3 == num_verts) {
            ...
            if (order[0] != -1) {
                ...
                triangle_mesh->addTriangle(tris[0]);
                triangle_mesh->addTriangle(tris[1]);
            } else {
