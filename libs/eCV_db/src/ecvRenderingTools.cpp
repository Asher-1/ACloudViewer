...
void ccRenderingTools::ShowDepthBuffer(ccGBLSensor* sensor, QWidget* parent/*=0*/, unsigned maxDim/*=1024*/)
{
	...
	QImage bufferImage(depthBuffer.width, depthBuffer.height, QImage::Format_RGB32);
	{
		...
		assert(colorScale);
        ScalarType coef = cloudViewer::LessThanEpsilon( maxDist - minDist ) ? 0 : static_cast<ScalarType>(ccColorScale::MAX_STEPS - 1) / (maxDist - minDist);
		...
		for (unsigned y = 0; y < depthBuffer.height; ++y)
		{
			...
			{
				const ecvColor::Rgb& col = (*_zBuff >= minDist ? colorScale->getColorByIndex(static_cast<unsigned>((std::min(maxDist, *_zBuff) - minDist)*coef)) : ecvColor::black);
...
void ccRenderingTools::DrawColorRamp(const CC_DRAW_CONTEXT& context)
{
	...
	QWidget* display = static_cast<QWidget*>(ecvDisplayTools::GetCurrentScreen());
...
void ccRenderingTools::DrawColorRamp(const CC_DRAW_CONTEXT& context, const ccScalarField* sf, QWidget* win, int glW, int glH, float renderZoom/*=1.0f*/)
{
	...
	//const ecvGui::ParamStruct& displayParams = win->getDisplayParameters();
