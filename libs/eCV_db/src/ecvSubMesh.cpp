...
bool ccSubMesh::interpolateColors(unsigned triIndex, const CCVector3& P, ecvColor::Rgb& rgb)
{
	if (m_associatedMesh && triIndex < size())
...
bool ccSubMesh::getColorFromMaterial(unsigned triIndex, const CCVector3& P, ecvColor::Rgb& rgb, bool interpolateColorIfNoTexture)
{
	if (m_associatedMesh && triIndex < size())
...
bool ccSubMesh::getVertexColorFromMaterial(unsigned triIndex, unsigned char vertIndex, ecvColor::Rgb& rgb, bool returnColorIfNoTexture)
{
	if (m_associatedMesh && triIndex < size())
...
void ccSubMesh::getTriangleVertices(unsigned triangleIndex, double A[3], double B[3], double C[3]) const
{
...
ccSubMesh* ccSubMesh::createNewSubMeshFromSelection(bool removeSelectedFaces, IndexMap* indexMap/*=0*/)
{
	...
	{
		CVLog::Error(QString("[Sub-mesh %1] Internal error: vertex visibility table not instantiated!").arg(getName()));
		return nullptr;
	}
	...
	{
		CVLog::Error("[ccSubMesh::createNewSubMeshFromSelection] Not enough memory!");
		return nullptr;
	}
