...
ScalarType ccScalarField::normalize(ScalarType d) const
{
	...
	{
        ScalarType dLog = log10(std::max(static_cast<ScalarType>(fabs(d)), ZERO_TOLERANCE_SCALAR));
...
void ccScalarField::setLogScale(bool state)
{
	if (m_logScale != state)
	{
		...
		{
			CVLog::Warning("[ccScalarField] Scalar field contains negative values! Log scale will only consider absolute values...");
		}
...
void ccScalarField::computeMinAndMax()
{
	...
	//update histogram
	{
		...
		else
		{
			...
			{
				CVLog::Warning("[ccScalarField::computeMinAndMax] Failed to update associated histogram!");
				m_histogram.clear();
			}
...
void ccScalarField::updateSaturationBounds()
{
	if (!m_colorScale || m_colorScale->isRelative()) //Relative scale (default)
	{
		...
		//if (m_logScale)
		{
            ScalarType minSatLog = log10(std::max(minAbsVal, ZERO_TOLERANCE_SCALAR));
            ScalarType maxSatLog = log10(std::max(maxAbsVal, ZERO_TOLERANCE_SCALAR));
...
bool ccScalarField::fromFile(QFile& in, short dataVersion, int flags)
{
	...
	if (dataVersion < 27)
	{
		...
		if (!autoBoundaries)
		{
			CVLog::Warning("[ccScalarField] Former 'released' boundaries are deprecated!");
			CVLog::Warning("[ccScalarField] You'll have to create the corresponding 'absolute' color scale (see the Color Scale Manager) and replace the file.");
		}
	}
	...
	//color scale
	{
		...
		{
			CVLog::Warning("[ccScalarField::fromFile] Failed to access color scales manager?!");
			assert(false);
		}
		...
		if (dataVersion < 27)
		{
			...
			case ccColorScalesManager::RW:
				...
				break;
			default:
				CVLog::Warning("[ccScalarField::fromFile] Color scale is no more supported!");
				break;
			}
