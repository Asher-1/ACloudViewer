...
std::shared_ptr<ccMesh> ccMesh::SimplifyVertexClustering(
    ...
    if (contraction == SimplificationContraction::Average) {
        for (const auto& voxel : voxel_vertices) {
            ...
            baseVertices->setPoint(static_cast<size_t>(vox_vidx),
                                   AvgVertex(voxel.second));
            if (has_vert_normal) {
                baseVertices->setPointNormal(static_cast<size_t>(vox_vidx),
                                             AvgNormal(voxel.second));
            }
            if (has_vert_color) {
                baseVertices->setPointColor(static_cast<size_t>(vox_vidx),
                                            AvgColor(voxel.second));
            }
        }
...
std::shared_ptr<ccMesh> ccMesh::SimplifyQuadricDecimation(
    ...
    auto AddPerpPlaneQuadric = [&](int vidx0, int vidx1, int vidx2,
        ...
        const auto& vert2 =
                baseVertices->getEigenPoint(static_cast<size_t>(vidx2));
    ...
    while (n_triangles > target_number_of_triangles && !queue.empty()) {
        ...
        for (int tidx : vert_to_triangles[vidx1]) {
            ...
            Eigen::Vector3d vert0 =
                    baseVertices->getEigenPoint(static_cast<size_t>(tria(0)));
            Eigen::Vector3d vert1 =
                    baseVertices->getEigenPoint(static_cast<size_t>(tria(1)));
            Eigen::Vector3d vert2 =
                    baseVertices->getEigenPoint(static_cast<size_t>(tria(2)));
        ...
        for (int tidx : vert_to_triangles[vidx1]) {
            ...
            cloudViewer::VerticesIndexes* tria =
                    mesh->getTriangleVertIndexes(static_cast<unsigned>(tidx));
        ...
        Qs[vidx0] += Qs[vidx1];
        if (has_vert_normal) {
            baseVertices->setEigenNormal(
                    static_cast<size_t>(vidx0),
                    0.5 * (baseVertices->getEigenNormal(
                                   static_cast<size_t>(vidx0)) +
                           baseVertices->getEigenNormal(
                                   static_cast<size_t>(vidx1))));
        }
        if (has_vert_color) {
            baseVertices->setEigenColor(
                    static_cast<size_t>(vidx0),
                    0.5 * (baseVertices->getEigenColor(
                                   static_cast<size_t>(vidx0)) +
                           baseVertices->getEigenColor(
                                   static_cast<size_t>(vidx1))));
        }
    ...
    for (unsigned int idx = 0; idx < baseVertices->size(); ++idx) {
        if (!vertices_deleted[idx]) {
            ...
            if (has_vert_normal) {
                baseVertices->setPointNormal(next_free,
                                             baseVertices->getPointNormal(idx));
            }
            if (has_vert_color) {
                baseVertices->setPointColor(next_free,
                                            baseVertices->getPointColor(idx));
            }
            next_free++;
        }
    }
    ...
    for (size_t idx = 0; idx < mesh->size(); ++idx) {
        if (!triangles_deleted[idx]) {
            ...
            mesh->setTriangle(next_free,
                              Eigen::Vector3i(vert_remapping[tria(0)],
