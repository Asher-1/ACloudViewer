...
//#define COLOR_PATCHES
	...
	try
	{
		...
		CVLog::Print(QString("[ResolveNormalsWithMST] Patches = %1 / Inversions: %2").arg(patchCount).arg(inversionCount));
	}
	catch (const std::bad_alloc&)
	{
		//not enough memory
															...
															ecvProgressDialog* progressDlg/*=0*/)
{
	...
	if (!cloud->hasNormals())
	{
		CVLog::Warning(QString("Cloud '%1' has no normals!").arg(cloud->getName()));
		return false;
	}
	...
	if (!cloud->getOctree())
	{
		...
		{
			CVLog::Warning(QString("[orientNormalsWithMST] Could not compute octree on cloud '%1'").arg(cloud->getName()));
			return false;
		}
	}
	...
	try
	{
														...
														"Build Spanning Tree") == 0)
		{
			...
			CVLog::Warning(QString("Failed to compute Spanning Tree on cloud '%1'").arg(cloud->getName()));
			result = false;
		}
		else
		{
			...
			{
				//something went wrong
				CVLog::Warning(QString("Failed to compute Minimum Spanning Tree on cloud '%1'").arg(cloud->getName()));
				result = false;
			}
		}
#else
		...
		{
			//something went wrong
			CVLog::Warning(QString("Failed to resolve normals orientation with Minimum Spanning Tree on cloud '%1'").arg(cloud->getName()));
			result = false;
		}
#endif
	}
	...
	{
		CVLog::Error(QString("Process failed on cloud '%1'").arg(cloud->getName()));
		result = false;
	}
