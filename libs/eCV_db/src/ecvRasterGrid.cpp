									...
									unsigned& gridHeight)
{
	...
	{
		CVLog::Warning("[ccRasterGrid::ComputeGridSize] Invalid cloud bounding box!");
		return false;
	}
								...
								ecvProgressDialog* progressDialog/*=0*/)
{
	...
	if (interpolateSF)
	{
		if (pc && pc->hasScalarFields())
		{
			...
			catch (const std::bad_alloc&)
			{
				//not enough memory
				scalarFields.resize(0);
				CVLog::Warning("[Rasterize] Failed to allocate memory for scalar fields!");
			}
		}
		else
		{
	...
	if (interpolateEmptyCells)
	{
		...
		{
			CVLog::Warning("[Rasterize] Not enough non-empty cells for interpolation!");
		}
		else if (nonEmptyCellCount < width * height) //otherwise it's useless!
		{
			...
			{
				//out of memory
				CVLog::Warning("[Rasterize] Not enough memory to interpolate empty cells!");
			}
		}
		...
		if (!the2DPoints.empty())
		{
			...
			{
				CVLog::Warning(QString("[Rasterize] Empty cells interpolation failed: Triangle lib. said '%1'").arg(errorStr));
			}
		}
	}
											...
											bool exportToOriginalCS) const
{
	...
	if (resampleInputCloudXY)
	{
		if (!inputCloud)
		{
			CVLog::Warning("[Rasterize] Internal error: no input clouds specified (for resampling)");
			assert(false);
			return nullptr;
		}
		...
		cloudGrid = inputCloud->isA(CV_TYPES::POINT_CLOUD) ? static_cast<ccPointCloud*>(inputCloud)->partialClone(&refCloud) : ccPointCloud::From(&refCloud, inputCloud);
		if (!cloudGrid)
		{
	...
	if (!exportedFields.empty())
	{
		...
		for (size_t i = 0; i < exportedFields.size(); ++i)
		{
			...
			case PER_CELL_HEIGHT_RANGE:
			{
				...
				if (sfIndex >= 0)
				{
					CVLog::Warning(QString("[Rasterize] Scalar field '%1' already exists. It will be overwritten.").arg(sfName));
				}
				else
				{
			...
			if (sfIndex < 0)
			{
				CVLog::Warning("[Rasterize] Couldn't allocate scalar field(s)! Try to free some memory ...");
				break;
			}
	...
	if (!resampleInputCloudXY || fillEmptyCells)
	{
		...
		{
			CVLog::Warning("[Rasterize] Not enough memory to interpolate colors!");
		...
		for (unsigned i = 0; i < width; ++i, ++aCell)
		{
			if (std::isfinite(aCell->h)) //valid cell (could have been interpolated)
			{
				...
				if (!resampleInputCloudXY || aCell->nbPoints == 0)
				{
					...
					{
						ecvColor::Rgb col(	static_cast<ColorCompType>(std::min(255.0, aCell->color.x)),
	...
	if (!resampleInputCloudXY)
	{
		...
		if (interpolateSF && inputCloud && inputCloud->isA(CV_TYPES::POINT_CLOUD))
		{
			...
			for (size_t k = 0; k < scalarFields.size(); ++k)
			{
				...
				if (sfIdx < 0)
				{
					CVLog::Warning("[Rasterize] Failed to allocate a new scalar field for storing SF '%s' values! Try to free some memory ...", formerSf->getName());
				}
				else
				{
