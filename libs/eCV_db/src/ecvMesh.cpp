...
{
	if (!m_associatedCloud || !m_associatedCloud->isA(CV_TYPES::POINT_CLOUD)) //TODO
	{
		CVLog::Warning("[ccMesh::computePerVertexNormals] Vertex set is not a standard cloud?!");
		return false;
	}
	...
	if (vertCount < 3)
	{
		CVLog::Warning("[ccMesh::computePerVertexNormals] Not enough vertices! (<3)");
		return false;
	}
...
bool ccMesh::computePerTriangleNormals()
{
	...
	{
		normIndexes->release();
		CVLog::Warning("[ccMesh::computePerTriangleNormals] Not enough memory!");
		return false;
	}
...
void ccMesh::transformTriNormals(const ccGLMatrix& trans)
{
	...
	if (m_triNormals && (!getParent() || !getParent()->isKindOf(CV_TYPES::MESH)))
	{
                                    ...
                                    cloudViewer::NormalizedProgress* nProgress/*=0*/)
{
    ...
    static const PointCoordinateType c_defaultSearchRadius = static_cast<PointCoordinateType>(sqrtf(ZERO_TOLERANCE_F));
                                     ...
                                     QWidget* parentWidget/*=nullptr*/)
{
    ...
    {
        CVLog::Warning("[ccMesh::mergeDuplicatedVertices] No triangle or no vertex");
        return false;
    }
    ...
    try
    {
        ...
        // tag the duplicated vertices
        {
            ...
            if (result == 0)
            {
                CVLog::Warning("[MergeDuplicatedVertices] Duplicated vertices removal algorithm failed?!");
                return false;
            }
        }
        ...
        //update face indexes
        {
            ...
            if (newFaceCount == 0)
            {
                CVLog::Warning("[MergeDuplicatedVertices] After vertex fusion, all triangles would collapse! We'll keep the non-fused version...");
                delete newVertices;
                newVertices = nullptr;
            }
            else
        ...
        CVLog::Print("[MergeDuplicatedVertices] Remaining vertices after auto-removal of duplicate ones: %i", vertCount);
        CVLog::Print("[MergeDuplicatedVertices] Remaining faces after auto-removal of duplicate ones: %i", size());
        return false;
    }
    catch (const std::bad_alloc&)
    {
        CVLog::Warning("[MergeDuplicatedVertices] Not enough memory: could not remove duplicated vertices!");
    }
...
ccMesh& ccMesh::rotate(const Eigen::Matrix3d & R, const Eigen::Vector3d &center)
{
	ccPointCloud* cloud = ccHObjectCaster::ToPointCloud(m_associatedCloud);
	if (!cloud)
	{
...
ccMesh::selectByIndex(const std::vector<size_t>& indices, bool cleanup) const
{
	...
	for (const auto &sel_vidx : indices) {
		...
		if (has_vertex_normals)
		{
			...
			baseVertices->addNorm(
				getAssociatedCloud()->getPointNormal(static_cast<unsigned>(sel_vidx)));
		}
		if (has_vertex_colors)
		{
			...
			baseVertices->addRGBColor(
				getAssociatedCloud()->getPointColor(static_cast<unsigned>(sel_vidx)));
		}
	}
	...
	cloudViewer::utility::LogDebug(
		...
		(int)output->getVerticeSize(), (int)output->size());
	...
	if (m_triNormals && m_triNormalIndexes)
	{
		...
		if (cloneMesh->reservePerTriangleNormalIndexes())
		{
			...
			if (!clonedNormsTable)
			{
				...
				{
					CVLog::Warning("[ccMesh::clone] Not enough memory: failed to clone per-triangle normals!");
		...
		{
			CVLog::Warning("[ccMesh::clone] Not enough memory: failed to clone per-triangle normal indexes!");
		}
	}
	...
	if (m_materials && m_triMtlIndexes)
	{
		...
		if (cloneMesh->reservePerTriangleMtlIndexes())
		{
			...
			if (!clonedMaterials)
			{
				...
				{
					CVLog::Warning("[ccMesh::clone] Not enough memory: failed to clone materials set!");
		...
		{
			CVLog::Warning("[ccMesh::clone] Not enough memory: failed to clone per-triangle materials!");
		}
	}
	...
	if (m_texCoords && m_texCoordIndexes)
	{
		...
		if (cloneMesh->reservePerTriangleTexCoordIndexes())
		{
			...
			if (!cloneTexCoords)
			{
				...
				if (!cloneTexCoords)
				{
					CVLog::Warning("[ccMesh::clone] Not enough memory: failed to clone texture coordinates!");
		...
		{
			CVLog::Warning("[ccMesh::clone] Not enough memory: failed to clone per-triangle texture info!");
		}
	}
...
ccMesh* ccMesh::TriangulateTwoPolylines(
	ccPolyline* p1, ccPolyline* p2, 
	CCVector3* projectionDir/*=0*/)
{
	...
	else
	{
		...
		if (!N.getLSPlane())
		{
			CVLog::Warning("[ccMesh::TriangulateTwoPolylines] Failed to fit a plane through both polylines");
			delete vertices;
			return nullptr;
		}
	...
	//remove the points oustide of the 'concave' hull
	{
		...
		try
		{
			...
			{
				CVLog::Warning("[ccMesh::TriangulateTwoPolylines] Failed to compute the convex hull (can't clean the mesh borders)");
			}
		}
		catch (const std::bad_alloc&)
		{
			CVLog::Warning("[ccMesh::TriangulateTwoPolylines] Not enough memory to clean the mesh borders");
		}
							...
							unsigned char dim/*=2*/)
{
	...
	if (!dummyMesh)
	{
		CVLog::Warning(QString("[ccMesh::Triangulate] Failed to construct Delaunay mesh (Triangle lib error: %1)").arg(errorStr));
		return nullptr;
	}
	...
	if (!mesh)
	{
		CVLog::Warning("[ccMesh::Triangulate] An error occurred while computing mesh! (not enough memory?)");
		return nullptr;
	}
...
bool ccMesh::merge(const ccMesh* mesh, bool createSubMesh)
{
	...
	if (	!mesh->getAssociatedCloud()
		...
		||	!m_associatedCloud
		||	!m_associatedCloud->isA(CV_TYPES::POINT_CLOUD))
	{
		assert(false);
		CVLog::Warning("[ccMesh::merge] Requires meshes with standard vertices!");
		return false;
	}
	...
	if (createSubMesh)
	{
		...
		{
			CVLog::Warning(QString("[Merge] Not enough memory to create the sub-mesh corresponding to mesh '%1'!").arg(mesh->getName()));
			delete subMesh;
			subMesh = nullptr;
		}
	}
...
void ccMesh::getTriangleVertices(unsigned triangleIndex, double A[3], double B[3], double C[3]) const
{
	assert(triangleIndex < m_triVertIndexes->size());
...
Eigen::Vector3d ccMesh::getVertice(size_t index) const
{
	...
	return ccHObjectCaster::ToPointCloud(getAssociatedCloud())->getEigenPoint(index);
}
...
Eigen::Vector3d ccMesh::getVertexNormal(size_t index) const
{
	...
	return ccHObjectCaster::ToPointCloud(m_associatedCloud)->getEigenNormal(index);
}
...
Eigen::Vector3d ccMesh::getVertexColor(size_t index) const
{
	...
	return ccHObjectCaster::ToPointCloud(m_associatedCloud)->getEigenColor(index);
}
...
std::vector<Eigen::Vector3d> ccMesh::getEigenVertices() const
{
	...
	return ccHObjectCaster::ToPointCloud(getAssociatedCloud())->getEigenPoints();
}
...
void ccMesh::addEigenVertices(const std::vector<Eigen::Vector3d>& vertices)
{
	...
	{
		ccPointCloud* cloud = ccHObjectCaster::ToPointCloud(m_associatedCloud);
		if (cloud->reserveThePointsTable(static_cast<unsigned>(cloud->size() + vertices.size())))
		{
	...
	if (MACRO_Draw3D(context))
	{
		...
		if (glParams.showSF)
		{
			...
			greyForNanScalarValues = (
				...
				cloud->getCurrentDisplayedScalarField()->areNaNValuesShownInGrey());
		...
		if (glParams.showColors)
		{
			...
			{
				assert(m_associatedCloud->isA(CV_TYPES::POINT_CLOUD));
				context.defaultMeshColor = static_cast<ccPointCloud*>(m_associatedCloud)->rgbColors()->getValue(0);
			}
		}
		else {
...
ccMesh* ccMesh::createNewMeshFromSelection(bool removeSelectedFaces)
{
	...
	{
		CVLog::Error(QString("[Mesh %1] Internal error: vertex visibility table not instantiated!").arg(getName()));
		return nullptr;
	}
	...
	if (!newVertices)
	{
		CVLog::Error(QString("[Mesh %1] Failed to create segmented mesh vertices! (not enough memory)").arg(getName()));
		return nullptr;
	}
	else if (newVertices->size() == 0)
	{
		CVLog::Error(QString("[Mesh %1] Failed to create segmented mesh vertices! (no visible point in selection)").arg(getName()));
		delete newVertices;
		return nullptr;
	}
	assert(newVertices);
		...
		else
		{
			...
			if (addFeatures)
			{
				...
				if (m_triNormals && m_triNormalIndexes)
				{
					...
					{
						CVLog::Warning("[ccMesh::createNewMeshFromSelection] Failed to create new normals subset! (not enough memory)");
				...
				if (m_texCoords && m_texCoordIndexes)
				{
					...
					{
						CVLog::Warning("[ccMesh::createNewMeshFromSelection] Failed to create new texture indexes subset! (not enough memory)");
				...
				if (m_materials && m_triMtlIndexes)
				{
					...
					{
						CVLog::Warning("[ccMesh::createNewMeshFromSelection] Failed to create new material subset! (not enough memory)");
						...
						verticesVisibility[tsi.i3] == POINT_VISIBLE)
					{
						...
						if (newTriTexIndexes)
						{
							...
							for (unsigned j = 0; j < 3; ++j)
							{
								if (triTexIndexes.u[j] >= 0 && newTexIndexes[triTexIndexes.u[j]] < 0)
								{
									...
									{
										CVLog::Error("Failed to create new texture coordinates subset! (not enough memory)");
										newMesh->removePerTriangleTexCoordIndexes();
										newTriTexIndexes->release();
										newTriTexIndexes = nullptr;
										break;
						...
						if (newMaterials)
						{
							...
							if (triMatIndex >= 0 && newMatIndexes[triMatIndex] < 0)
							{
								...
								{
									CVLog::Warning("[ccMesh::createNewMeshFromSelection] Failed to create new materials subset! (not enough memory)");
									newMesh->removePerTriangleMtlIndexes();
									newMaterials->release();
									newMaterials = nullptr;
								}
	...
	if (m_triNormals && m_triNormalIndexes && m_triNormalIndexes->size() > triangleIndex)
	{
		...
		if (indexes.u[0] >= 0)
		{
			const PointCoordinateType* n1 =
				ccNormalVectors::GetUniqueInstance()->getNormal(m_triNormals->getValue(indexes.u[0])).u;
		...
		if (indexes.u[1] >= 0)
		{
			const PointCoordinateType* n2 = 
				ccNormalVectors::GetUniqueInstance()->getNormal(m_triNormals->getValue(indexes.u[1])).u;
		...
		if (indexes.u[2] >= 0)
		{
			const PointCoordinateType* n3 = 
				ccNormalVectors::GetUniqueInstance()->getNormal(m_triNormals->getValue(indexes.u[2])).u;
...
Eigen::Vector3d ccMesh::getTriangleNorm(size_t index) const
{
	if (index >= m_triNormals->size()) 
		return Eigen::Vector3d(0.0, 0.0, 0.0);
	const PointCoordinateType* n1 = 
		ccNormalVectors::GetUniqueInstance()->getNormal(m_triNormals->getValue(index)).u;
	return Eigen::Vector3d(n1[0], n1[1], n1[2]);
}

bool ccMesh::setTriangleNorm(size_t index, const Eigen::Vector3d& triangle_normal)
{
	...
	CompressedNormType nIndex = 
		ccNormalVectors::GetNormIndex(CCVector3::fromArray(triangle_normal));
	setTriangleNormalIndexes(index, nIndex);
	return true;
}

bool ccMesh::setTriangleNormalIndexes(size_t triangleIndex, CompressedNormType value)
{
...
{
	bool success = true;
	if (resize(triangle_normals.size()) && 
		m_triNormals && m_triNormals->resizeSafe(triangle_normals.size()))
	{
...
bool ccMesh::getVertexColorFromMaterial(unsigned triIndex, unsigned char vertIndex, ecvColor::Rgb& rgb, bool returnColorIfNoTexture)
{
	assert(triIndex < size());
	...
	if (vertIndex > 2)
	{
		CVLog::Error("[ccMesh::getVertexColorFromMaterial] Internal error: invalid vertex index!");
		return false;
	}
...
bool ccMesh::getColorFromMaterial(unsigned triIndex, const CCVector3& P, ecvColor::Rgb& rgb, bool interpolateColorIfNoTexture)
{
	assert(triIndex < size());
...
bool ccMesh::pushSubdivide(/*PointCoordinateType maxArea, */unsigned indexA, unsigned indexB, unsigned indexC)
{
	...
	if (!getAssociatedCloud() || !getAssociatedCloud()->isA(CV_TYPES::POINT_CLOUD))
	{
		CVLog::Error("[ccMesh::pushSubdivide] Vertices set must be a true point cloud!");
		return false;
	}
...
ccMesh* ccMesh::subdivide(PointCoordinateType maxArea) const
{
	...
	ccPointCloud* resultVertices = vertices->isA(CV_TYPES::POINT_CLOUD) ? static_cast<ccPointCloud*>(vertices)->cloneThis() : ccPointCloud::From(vertices,vertices);
	if (!resultVertices)
	{
...
bool ccMesh::convertMaterialsToVertexColors()
{
	if (!hasMaterials())
	{
		CVLog::Warning("[ccMesh::convertMaterialsToVertexColors] Mesh has no material!");
		return false;
	}

	if (!m_associatedCloud->isA(CV_TYPES::POINT_CLOUD))
	{
		CVLog::Warning("[ccMesh::convertMaterialsToVertexColors] Need a true point cloud as vertices!");
		return false;
	}
	...
	{
		CVLog::Warning("[ccMesh::convertMaterialsToVertexColors] Failed to resize vertices color table! (not enough memory?)");
		return false;
	}
