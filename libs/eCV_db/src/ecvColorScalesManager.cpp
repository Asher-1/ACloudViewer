...
void ccColorScalesManager::fromPersistentSettings()
{
	...
	CVLog::Print(QString("[ccColorScalesManager] Found %1 custom scale(s) in persistent settings").arg(scales.size()));
	...
	for (int j = 0; j < scales.size(); ++j)
	{
		...
		{
			CVLog::Warning(QString("[ccColorScalesManager] Failed to load scale '%1' (not enough memory)").arg(scale->getName()));
			scale.clear();
		}
...
void ccColorScalesManager::removeScale(QString UUID)
{
	...
	if (it != m_scales.constEnd())
	{
		if ((*it)->isLocked())
		{
			CVLog::Warning(QString("[ccColorScalesManager::addScale] Can't remove a locked scale (%1)!").arg(UUID));
		}
		else
		{
...
ccColorScale::Shared ccColorScalesManager::Create(DEFAULT_SCALES scaleType)
{
	...
	if ( name.isNull() )
	{
		CVLog::Error(QStringLiteral("Unhandled pre-defined scale (%1)").arg(scaleType));
