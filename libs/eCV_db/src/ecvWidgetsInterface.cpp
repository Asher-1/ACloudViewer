...
namespace DBLib
{
	...
	**/
	...
	QWidget * ecvWidgetsInterface::LoadWidget(const VTK_WIDGETS_TYPE widgetType, Shared widgetInterface)
	{
		if (!widgetInterface)
		{
			CVLog::Error(QString("[Load] Internal error (invalid input widget type : %1)").arg(widgetType));
			assert(false);
			return nullptr;
		}
	...
	QWidget * ecvWidgetsInterface::LoadWidget(const VTK_WIDGETS_TYPE widgetType)
	{
		...
		if (!widgetInterface)
		{
			CVLog::Error(QString("[Load] Internal error: no widget interface corresponds to widget type '%1'").arg(widgetType));
			return nullptr;
		}
	...
	bool ecvWidgetsInterface::SetInput(const ccHObject * obj, const VTK_WIDGETS_TYPE widgetType)
	{
		Shared widgetInterface(nullptr);
		widgetInterface = GetWigetInterface(widgetType);
		if (!widgetInterface)
	...
	void ecvWidgetsInterface::Register(Shared widgetInterface)
	{
		...
		for (InterfaceContainer::const_iterator it = s_widgetInterfaces.begin(); it != s_widgetInterfaces.end(); ++it)
		{
			bool error = false;
			if (*it == widgetInterface)
			{
				CVLog::Warning(QStringLiteral("[ecvWidgetsInterface::Register] widget Interface '%1' is already registered").arg(interfaceName));
				error = true;
			}
			else
			{
				...
				for (int i = 0; i < widgetName.size(); ++i)
				{
					...
					{
						const QString otherInterfaceName = widgetInterface->getInterfaceName();
						CVLog::Warning(QStringLiteral(
							"[ecvWidgetsInterface::Register] Internal error: widget '%1' of  widget Interface '%2' "
							"is already handled by another widget interface ('%3')!").
							arg(widgetName[i], interfaceName, otherInterfaceName));
						error = true;
						break;
					}
				}
	...
	{
		for (InterfaceContainer::iterator it = s_widgetInterfaces.begin(); it != s_widgetInterfaces.end(); ++it)
		{
			(*it)->unregister();
		}
	...
	ecvWidgetsInterface::Shared ecvWidgetsInterface::GetWigetInterface(const VTK_WIDGETS_TYPE widgetID)
	{
		for (InterfaceContainer::const_iterator it = s_widgetInterfaces.begin(); it != s_widgetInterfaces.end(); ++it)
		{
			if ((*it)->getWidgtByID(widgetID))
				return *it;
		}
	...
	const ecvWidgetsInterface::InterfaceContainer& ecvWidgetsInterface::GetWigetInterfaces()
	{
