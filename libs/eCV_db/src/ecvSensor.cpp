...
bool ccSensor::addPosition(ccGLMatrix& trans, double index) {
    ...
    bool sort =
            (!m_posBuffer->empty() && m_posBuffer->back().getIndex() > index);
    try {
...
bool ccSensor::getAbsoluteTransformation(ccIndexedTransformation& trans,
                                         double index) const {
    trans.toIdentity();
    if (m_posBuffer)
...
bool ccSensor::getActiveAbsoluteTransformation(
        ccIndexedTransformation& trans) const {
    if (!getAbsoluteTransformation(trans, m_activeIndex)) {
        CVLog::Warning(
                "[ccSensor::getActiveAbsoluteTransformation] Failed to get a "
                "valid transformation for active index!");
        return false;
    }
...
bool ccSensor::toFile_MeOnly(QFile& out) const {
    ...
    // we can't save the associated position buffer (as it may be shared by
    // multiple sensors) so instead we save it's unique ID (dataVersion>=34)
    // WARNING: the buffer must be saved in the same BIN file! (responsibility
    // of the caller)
    uint32_t bufferUniqueID =
            (m_posBuffer ? (uint32_t)m_posBuffer->getUniqueID() : 0);
...
bool ccSensor::fromFile_MeOnly(QFile& in,
    ...
    if (dataVersion < 34) return false;
    ...
    // as the associated position buffer can't be saved directly (as it may be
    // shared by multiple sensors) we only store its unique ID (dataVersion>=34)
    ...
    if (in.read((char*)&bufferUniqueID, 4) < 0) return ReadError();
...
bool ccSensor::applyViewport() {
    // not supported by default, must be reimplemented by the child class
    CVLog::Warning("[ccSensor::applyViewport] Unhandled sensor type!");
    return false;
}
