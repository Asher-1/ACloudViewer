...
void cc2DLabel::clear3Dviews()
{
	...
	{
		ecvDisplayTools::RemoveWidgets(
			WIDGETS_PARAMETER(WIDGETS_TYPE::WIDGET_POLYLINE, m_contourIdfix));
		ecvDisplayTools::RemoveWidgets(
			WIDGETS_PARAMETER(WIDGETS_TYPE::WIDGET_POLYGONMESH, m_surfaceIdfix));
	}
}
...
void cc2DLabel::clear2Dviews()
{
	if (!m_historyMessage.isEmpty())
	{
		for (const QString& text : m_historyMessage)
		{
			...
			ecvDisplayTools::RemoveWidgets(WIDGETS_PARAMETER(WIDGETS_TYPE::WIDGET_T2D, text));
			ecvDisplayTools::RemoveWidgets(WIDGETS_PARAMETER(WIDGETS_TYPE::WIDGET_RECTANGLE_2D, text));
		}
...
void cc2DLabel::update3DLabelView(
	CC_DRAW_CONTEXT& context,
...
void cc2DLabel::update2DLabelView(
	CC_DRAW_CONTEXT& context, 
	bool updateScreen /* = true */)
{
...
void cc2DLabel::updateName()
{
	...
	case 2:
		if (m_pickedPoints[0].cloud == m_pickedPoints[1].cloud)
			...
			setName(QString("Vector #") + POINT_INDEX_0 + QString("@") + ENTITY_INDEX_0
				+ QString(" - #") + POINT_INDEX_1 + QString("@") + ENTITY_INDEX_1);
		break;
	case 3:
		if (m_pickedPoints[0].cloud == m_pickedPoints[2].cloud && m_pickedPoints[1].cloud == m_pickedPoints[2].cloud)
			setName(QString("Triplet #") + POINT_INDEX_0 + QString(" - #") + POINT_INDEX_1 + QString(" - #") + POINT_INDEX_2);
		else
			setName(QString("Triplet #") + POINT_INDEX_0 + QString("@") + ENTITY_INDEX_0
				+ QString(" - #") + POINT_INDEX_1 + QString("@") + ENTITY_INDEX_1
				+ QString(" - #") + POINT_INDEX_2 + QString("@") + ENTITY_INDEX_2);
		break;
	}
}
...
bool cc2DLabel::addPickedPoint(ccGenericMesh * mesh, unsigned triangleIndex, const CCVector2d & uv, bool entityCenter)
{
...
void cc2DLabel::drawMeOnly3D(CC_DRAW_CONTEXT& context)
{
		...
		//display point marker as spheres
		{
			if (!c_unitTriMarker)
			{
				cloudViewer::GenericIndexedCloudPersist* cloud = nullptr;
				{
					...
					CCVector3* A = const_cast<CCVector3*>(m_polyVertices->getPointPersistentPtr(0));
					...
					*A = *(m_pickedPoints[0].cloud->getPoint(m_pickedPoints[0].index));
					*B = *(m_pickedPoints[1].cloud->getPoint(m_pickedPoints[1].index));
					*C = *(m_pickedPoints[2].cloud->getPoint(m_pickedPoints[2].index));
				...
				c_unitTriMarker = QSharedPointer<ccFacet>(
					ccFacet::Create(cloud));

				if (c_unitTriMarker)
				{
					c_unitTriMarker->getPolygon()->setOpacity(0.5);
					c_unitTriMarker->getPolygon()->setTempColor(ecvColor::yellow);
			...
			CCVector3* A = const_cast<CCVector3*>(c_unitTriMarker->getContourVertices()->getPointPersistentPtr(0));
			CCVector3* B = const_cast<CCVector3*>(c_unitTriMarker->getContourVertices()->getPointPersistentPtr(1));
			CCVector3* C = const_cast<CCVector3*>(c_unitTriMarker->getContourVertices()->getPointPersistentPtr(2));
			...
			*C = *(m_pickedPoints[2].cloud->getPoint(m_pickedPoints[2].index));
			...
			//we must remove the 'push name flag' so that the sphere doesn't push its own!
	...
	case 2:
	{
		if (count == 2)
		{
			...
			ecvColor::Rgb lineColor = isSelected() ? ecvColor::red :
				context.labelDefaultMarkerCol/*ecvColor::green.rgba*/;
			float lineWidth = c_sizeFactor * context.renderZoom;

			const CCVector3* lineSt = m_pickedPoints[0].cloud->getPoint(m_pickedPoints[0].index);
			const CCVector3* lineEd = m_pickedPoints[1].cloud->getPoint(m_pickedPoints[1].index);

			//we draw the line
			WIDGETS_PARAMETER param(WIDGETS_TYPE::WIDGET_LINE_3D, m_lineID);
			param.setLineWidget(LineWidget(*lineSt, *lineEd, lineWidth, lineColor));
	...
	case 1:
	{
		//display point marker as spheres
		{
			...
			const ecvViewportParameters& viewportParams = ecvDisplayTools::GetViewportParameters();
			for (size_t i = 0; i < count; i++)
			{
				const CCVector3* P = m_pickedPoints[i].cloud->getPoint(m_pickedPoints[i].index);
				//markerContext.transformInfo.setTranslationStart(CCVector3(P->x, P->y, P->z));
				//markerContext.transformInfo.setPostion(CCVector3(P->x, P->y, P->z));
				...
				WIDGETS_PARAMETER param(WIDGETS_TYPE::WIDGET_SPHERE, QString::number(i) + m_sphereIdfix);
				param.radius = scale;
				param.center = CCVector3(P->x, P->y, P->z);
				param.color = ecvColor::FromRgba(ecvColor::ored);
				ecvDisplayTools::DrawWidgets(param, false);
				//markerContext.transformInfo.setScale(CCVector3(scale, scale, scale));
				//markerContext.viewID = QString::number(i) + m_sphereIdfix;
	...
	//in 2D so that they always appear above the entities
	{
		...
		if (!pushName)
		{
			...
			for (size_t i = 0; i < count; i++)
			{
				...
				const CCVector3* P3D = m_pickedPoints[i].cloud->getPoint(m_pickedPoints[i].index);
		...
		if (visibleCount)
		{
			...
			if (m_dispPointsLegend && !pushName)
			{
				...
				for (size_t j = 0; j < count; j++)
				{
					...
					ecvDisplayTools::DisplayText(title,
...
#define DRAW_CONTENT_AS_TAB
	...
	{
		/*** label border ***/
		bodyFont = ecvDisplayTools::GetLabelDisplayFont(); //takes rendering zoom into account!
	...
	if (!highlighted)
	{
		...
		defaultBorderColor = ecvColor::Rgbaub(context.labelDefaultBkgCol, halfAlpha);
	}
	...
	if (!pushName)
	{
		...
		int iArrowDestY = static_cast<int>(arrowDest2D.y - yStart);
		{
			if (iArrowDestX < 0/*m_labelROI.left()*/) //left
				arrowBaseConfig += 0;
			else if (iArrowDestX > m_labelROI.width() /*m_labelROI.right()*/) //Right
				arrowBaseConfig += 2;
			else  //Middle
				arrowBaseConfig += 1;
			...
			if (iArrowDestY > 0/*-m_labelROI.top()*/) //Top
				arrowBaseConfig += 0;
			else if (iArrowDestY < -m_labelROI.height()/*-m_labelROI.bottom()*/) //Bottom
				arrowBaseConfig += 6;
			else  //Middle
				arrowBaseConfig += 3;
		}
		...
		if (arrowBaseConfig != 4) //4 = label above point!
		{
			...
			case 0: //top-left corner
			{
				...
				//triangleParam.p2 = QPoint(m_labelROI.left(), newTop - 2 * arrowBaseSize);
				//triangleParam.p3 = QPoint(m_labelROI.left(), newTop);
				//triangleParam.p4 = QPoint(m_labelROI.left() + 2 * arrowBaseSize, newTop);
				break;
			case 1: //top-middle edge
			{
				triangleParam.p2 = QPoint(m_labelROI.center().x(), newTop);
			}
				//triangleParam.p2 = QPoint(std::max(m_labelROI.left(), iArrowDestX - arrowBaseSize), newTop);
				//triangleParam.p3 = QPoint(std::min(m_labelROI.right(), iArrowDestX + arrowBaseSize), newTop);
				break;
			case 2: //top-right corner
			{
				triangleParam.p2 = QPoint(m_labelROI.right(), newTop);
			}
				//triangleParam.p2 = QPoint(m_labelROI.right(), newTop - 2 * arrowBaseSize);
				//triangleParam.p3 = QPoint(m_labelROI.right(), newTop);
				//triangleParam.p4 = QPoint(m_labelROI.right() - 2 * arrowBaseSize, newTop);
				break;
			case 3: //middle-left edge
			{
				triangleParam.p2 = QPoint(m_labelROI.left(), m_labelROI.center().y());
			}
				//triangleParam.p2 = QPoint(m_labelROI.left(), std::min(newTop, iArrowDestY + arrowBaseSize));
				//triangleParam.p3 = QPoint(m_labelROI.left(), std::max(newBottom, iArrowDestY - arrowBaseSize));
				break;
			case 4: //middle of rectangle!
				break;
			case 5: //middle-right edge
			{
				triangleParam.p2 = QPoint(m_labelROI.right(), m_labelROI.center().y());
			}
				//triangleParam.p2 = QPoint(m_labelROI.right(), std::min(newTop, iArrowDestY + arrowBaseSize));
				//triangleParam.p3 = QPoint(m_labelROI.right(), std::max(newBottom, iArrowDestY - arrowBaseSize));
				break;
			case 6: //bottom-left corner
			{
				triangleParam.p2 = QPoint(m_labelROI.left(), newBottom);
			}
				//triangleParam.p2 = QPoint(m_labelROI.left(), newBottom + 2 * arrowBaseSize);
				//triangleParam.p3 = QPoint(m_labelROI.left(), newBottom);
				//triangleParam.p4 = QPoint(m_labelROI.left() + 2 * arrowBaseSize, newBottom);
				break;
			case 7: //bottom-middle edge
			{
				triangleParam.p2 = QPoint(m_labelROI.center().x(), newBottom);
			}
				/*triangleParam.p2 = QPoint(std::max(m_labelROI.left(), iArrowDestX - arrowBaseSize), newBottom);
				triangleParam.p3 = QPoint(std::min(m_labelROI.right(), iArrowDestX + arrowBaseSize), newBottom);*/
				break;
			case 8: //bottom-right corner
			{
				triangleParam.p2 = QPoint(m_labelROI.right(), newBottom);
			}
				//triangleParam.p2 = QPoint(m_labelROI.right(), newBottom + 2 * arrowBaseSize);
				//triangleParam.p3 = QPoint(m_labelROI.right(), newBottom);
				//triangleParam.p4 = QPoint(m_labelROI.right() - 2 * arrowBaseSize, newBottom);
				break;
			}
	...
	if (!pushName)
	{
		...
		if (m_showFullBody)
		{
			...
			for (int c = 0; c < tab.colCount; ++c)
			{
				...
				bool labelCol = ((c & 1) == 0);
				const unsigned char* textColor = labelCol ? ecvColor::white.rgb : defaultTextColor.rgb;
				...
				for (int r = 0; r < actualRowCount; ++r)
				{
					...
					if (labelCol)
					{
						...
						param.rect = QRect(
							m_labelROI.x() + xCol,
							m_labelROI.y() + m_labelROI.height() + yRow - height,
							width, height);
						ecvDisplayTools::DrawWidgets(param, false);
					}
					...
					m_historyMessage << str;
					ecvDisplayTools::DisplayText(str,
