...
{
    ecvDisplayTools::HideShowEntities(this, context.visible && m_showAxisPlanes);
    ecvDisplayTools::HideShowEntities(&m_axis, context.visible && m_showAxisLines);
}
...
//    zxPlane.setColor(ecvColor::FromRgbfToRgb(ecvColor::Rgbf(0.0f, 0.7f, 1.0f)));
...
void ccCoordinateSystem::drawMeOnly(CC_DRAW_CONTEXT& context)
{
    ...
    if (MACRO_Draw3D(context))
	{
        if (m_showAxisLines)
        {
            ...
            if (m_width != 0.0f)
            {
                tempContext.currentLineWidth = static_cast<unsigned char>(m_width);
            }

            // coordinate system axis
            m_axis.clear();
            m_axis.points_.push_back(Eigen::Vector3d(0.0, 0.0, 0.0));
            m_axis.points_.push_back(Eigen::Vector3d(static_cast<double>(m_DisplayScale*2), 0.0, 0.0));
            m_axis.points_.push_back(Eigen::Vector3d(0.0, static_cast<double>(m_DisplayScale*2), 0.0));
            m_axis.points_.push_back(Eigen::Vector3d(0.0, 0.0, static_cast<double>(m_DisplayScale*2)));
            ...
            // transformation
            {
                Eigen::Matrix4d transformation = ccGLMatrixd::ToEigenMatrix4(m_transformation);
