										...
										bool autoComputeOctree/*=false*/)
{
	...
	if (pickWidth == pickHeight)
	{
		...
		{
			ecvProgressDialog pDlg(false, ecvDisplayTools::GetMainWindow() ? ecvDisplayTools::GetMainWindow() : nullptr);
		...
		if (octree)
		{
			...
			{
				CVLog::Warning("[Point picking] Failed to use the octree. We'll fall back to the slow process...");
			}
		}
	}
...
cloudViewer::ReferenceCloud* ccGenericPointCloud::getTheVisiblePoints(const VisibilityTableType* visTable/*=nullptr*/, bool silent/*=false*/) const
{
	...
	{
		assert(false);
		CVLog::Warning("[ccGenericPointCloud::getTheVisiblePoints] No visibility table instantiated!");
		return nullptr;
	}
	...
	if (pointCount)
	{
		...
		{
			CVLog::Warning("[ccGenericPointCloud::getTheVisiblePoints] Not enough memory!");
			delete rc;
			rc = nullptr;
		}
	}
	...
	{
		CVLog::Warning("[ccGenericPointCloud::getTheVisiblePoints] No point in selection");
	}
