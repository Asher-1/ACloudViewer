	...
	if (MACRO_Draw2D(context))
	{
		if (MACRO_Foreground(context) && !context.sfColorScaleToDisplay)
		{
			if (sfShown())
			{
				...
				if (parent && parent->isKindOf(CV_TYPES::MESH) && (ccHObjectCaster::ToGenericMesh(parent)->getAssociatedCloud() == vertices))
					return;
											...
											CVLib::GenericProgressCallback* pDlg/*=nullptr*/)
{
	...
	if (sampledCloud)
	{
		...
		{
			CVLog::Warning("[ccGenericMesh::samplePoints] No point was generated (sampling density is too low?)");
		}
		else
		{
	...
	if (withFeatures && triIndices && triIndices->size() >= cloud->size())
	{
		...
		if (withNormals && hasNormals())
		{
			...
			{
				CVLog::Warning("[ccGenericMesh::samplePoints] Failed to interpolate normals (not enough memory?)");
			}
		}
		...
		if (withTexture && hasMaterials())
		{
			...
			{
				CVLog::Warning("[ccGenericMesh::samplePoints] Failed to export texture colors (not enough memory?)");
			}
		}
		else if (withRGB && hasColors())
		{
			...
			{
				CVLog::Warning("[ccGenericMesh::samplePoints] Failed to interpolate colors (not enough memory?)");
			}
		}
	}
									...
									CCVector3d& nearestPoint)
{
	...
	{
		CVLib::VerticesIndexes* tsi = getTriangleVertIndexes(i);
