...
struct CVPLUGIN_LIB_API CLEntityDesc {
    QString basename;
    QString path;
    int indexInFile;
    ...
    CLEntityDesc(const QString& _basename,
                 const QString& _path,
                 int _indexInFile = -1);
...
struct CVPLUGIN_LIB_API CLGroupDesc : CLEntityDesc {
...
struct CVPLUGIN_LIB_API CLCloudDesc : CLEntityDesc {
    ccPointCloud* pc;
...
struct CVPLUGIN_LIB_API CLMeshDesc : CLEntityDesc {
    ccGenericMesh* mesh;
...
class CVPLUGIN_LIB_API ccCommandLineInterface {
public:  // constructor
    //! Default constructor
...
public:  // commands
    ...
    struct CVPLUGIN_LIB_API Command {
        //! Shared type
...
public:  // virtual methods
    ...
    virtual QString exportEntity(CLEntityDesc& entityDesc,
                                 ...
                                 ccCommandLineInterface::ExportOptions options =
                                         ExportOption::NoOptions) = 0;

    //! Saves all clouds
    /** \param suffix optional suffix
            \param allAtOnce whether to save all clouds in the same file or one
    cloud per file \return success
    **/
    virtual bool saveClouds(QString suffix = QString(),
                            bool allAtOnce = false,
                            const QString* allAtOnceFileName = nullptr) = 0;

    //! Saves all meshes
    /** \param suffix optional suffix
            \param allAtOnce whether to save all meshes in the same file or one
    mesh per file \return success
    **/
    virtual bool saveMeshes(QString suffix = QString(),
                            bool allAtOnce = false,
                            const QString* allAtOnceFileName = nullptr) = 0;
...
public:  // file I/O
    ...
    struct CVPLUGIN_LIB_API CLLoadParameters
        : public FileIOFilter::LoadParameters {
        CLLoadParameters();
    ...
    /** Automatically dispatches the entities between the clouds and meshes
     *sets.
     **/
    virtual bool importFile(
            QString filename,
            const GlobalShiftOptions& globalShiftOptions,
            FileIOFilter::Shared filter = FileIOFilter::Shared(nullptr)) = 0;
    ...
    //! Returns the current hierarchy(ies) export extension (warning: can be
    //! anything)
...
public:  // logging
    ...
    virtual bool error(
            const QString& message) const = 0;  // must always return false!
...
public:  // access to data
    ...
    //! Sets whether files should be automatically saved (after each process) or
    //! not
    void toggleAutoSaveMode(bool state);
    //! Returns whether files should be automatically saved (after each process)
    //! or not
    bool autoSaveMode() const;

    //! Sets whether a timestamp should be automatically added to output files
    //! or not
    void toggleAddTimestamp(bool state);
    //! Returns whether a timestamp should be automatically added to output
    //! files or not
...
public:  // Global shift management
    ...
    //! Check the current command line argument stack against the
