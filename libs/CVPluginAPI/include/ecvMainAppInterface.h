...
public:
    ...
    /** Overlay dialogs are displayed in the central MDI area, above the 3D
    views. The position (pos) is defined relatively to the MDI area (as one of
    its 4 corners). And it is automatically updated when the main window is
    moved or resized. Registered dialogs are automatically released when
    CLOUDVIEWER  stops.

    Notes:
    - it may be necessary to call 'updateOverlayDialogsPlacement' after calling
    this method
    - it's a good idea to freeez the UI when the tool starts to avoid other
    overlay dialogs to appear (don't forget to unfreeze the UI afterwards)
    **/
    virtual void registerOverlayDialog(ccOverlayDialog* dlg,
                                       Qt::Corner pos) = 0;

    //! Unregisters a MDI area 'overlay' dialog
    /** \warning Original overlay dialog object will be deleted (see
     *QObject::deleteLater)
     **/
    ...
    **/
    ...
    /** This method should be called once modifications to the db tree are
    finished (see removeObjectTemporarilyFromDBTree).
    **/
    ...
    **/
    ...
    ///** \param only2D whether to redraw everything (false) or only the 2D
    /// layer (true)
    //**/
    ...
    **/
    ...
    //! state
    /** This method should be called whenever a change is made to any selected
     *entity
     **/
    ...
    // virtual int getDevicePixelRatio() const = 0;
