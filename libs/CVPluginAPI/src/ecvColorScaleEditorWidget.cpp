...
ColorScaleElementSlider::ColorScaleElementSlider(
    ...
    if (m_orientation == Qt::Horizontal)
        setFixedSize(DEFAULT_SLIDER_SYMBOL_SIZE,
                     2 * DEFAULT_SLIDER_SYMBOL_SIZE);
    else
        setFixedSize(2 * DEFAULT_SLIDER_SYMBOL_SIZE,
                     DEFAULT_SLIDER_SYMBOL_SIZE);
}
...
void ColorScaleElementSlider::paintEvent(QPaintEvent* e) {
    ...
    QRect box(0, 0, DEFAULT_SLIDER_SYMBOL_SIZE - 1,
              DEFAULT_SLIDER_SYMBOL_SIZE - 1);
    QPolygon pointyHead;
    if (m_orientation == Qt::Horizontal) {
        box.moveTop(DEFAULT_SLIDER_SYMBOL_SIZE - 1);
        pointyHead << QPoint(0, DEFAULT_SLIDER_SYMBOL_SIZE - 1)
                   << QPoint(DEFAULT_SLIDER_SYMBOL_SIZE / 2, 0)
                   << QPoint(DEFAULT_SLIDER_SYMBOL_SIZE - 1,
                             DEFAULT_SLIDER_SYMBOL_SIZE - 1);
    } else {
        box.moveLeft(DEFAULT_SLIDER_SYMBOL_SIZE - 1);
        pointyHead << QPoint(DEFAULT_SLIDER_SYMBOL_SIZE - 1, 0)
                   << QPoint(0, DEFAULT_SLIDER_SYMBOL_SIZE / 2)
                   << QPoint(DEFAULT_SLIDER_SYMBOL_SIZE - 1,
                             DEFAULT_SLIDER_SYMBOL_SIZE - 1);
    }
...
void ColorBarWidget::mousePressEvent(QMouseEvent* e) {
    if (e->button() == Qt::LeftButton) {
        ...
        if (contentRect.contains(e->pos(), true)) {
            ...
            if (m_orientation == Qt::Horizontal) {
                relativePos =
                        static_cast<double>(e->pos().x() - contentRect.left()) /
                        contentRect.width();
            } else {
                relativePos =
                        static_cast<double>(e->pos().y() - contentRect.top()) /
                        contentRect.height();
            }
...
void ColorBarWidget::paintEvent(QPaintEvent* e) {
    if (m_sliders && m_sliders->size() >= 2) {
        ...
        assert(m_sliders->front()->getRelativePos() == 0.0 &&
               m_sliders->back()->getRelativePos() == 1.0);

        // color gradient
        {
            QLinearGradient gradient;
            if (m_orientation == Qt::Horizontal)
                gradient = QLinearGradient(contentRect.left(), 0,
                                           contentRect.right(), 0);
            else
                gradient = QLinearGradient(0, contentRect.bottom(), 0,
                                           contentRect.top());

            // fill gradient with sliders
            {
                for (int i = 0; i < m_sliders->size(); i++) {
                    gradient.setColorAt(m_sliders->at(i)->getRelativePos(),
                                        m_sliders->at(i)->getColor());
                }
            }
        ...
        // draw a line for each slider position (apart from the first and the
        // last one)
        {
            ...
            for (int i = 0; i < m_sliders->size(); i++) {
                ...
                if (m_orientation == Qt::Horizontal) {
                    int pos =
                            contentRect.left() +
                            static_cast<int>(relativePos * contentRect.width());
                    A.setX(pos);
                    B.setX(pos);
                } else {
                    int pos = contentRect.top() +
...
void SlidersWidget::select(int index, bool silent /*=false*/) {
    ...
    if (index >= 0) m_sliders->at(index)->setSelected(true);

    if (!silent) emit sliderSelected(index);
}

ColorScaleElementSlider* SlidersWidget::addNewSlider(double relativePos,
                                                     QColor color) {
    select(-1);  // be sure to deselect any selected slider before modifying the
                 // global set contents!

    ColorScaleElementSlider* slider = new ColorScaleElementSlider(
            relativePos, color, this, m_orientation);
...
void SlidersWidget::resizeEvent(QResizeEvent* e) { updateAllSlidersPos(); }

void SlidersWidget::updateAllSlidersPos() {
    if (!m_sliders || m_sliders->size() < 2) return;

    int rectLength = length();

    for (ColorScaleElementSliders::iterator it = m_sliders->begin();
         it != m_sliders->end(); ++it) {
...
void SlidersWidget::updateSliderPos(int index) {
    if (!m_sliders || m_sliders->size() < 2 || index < 0) return;
...
void SlidersWidget::mouseMoveEvent(QMouseEvent* e) {
    ...
    double relativePos = static_cast<double>(pos - DEFAULT_MARGIN) /
    ...
    if (relativePos > 0.0 && relativePos < 1.0) {
        ...
        if (activeSliderIndex > 0 &&
            activeSliderIndex + 1 <
                    m_sliders->size())  // first and last sliders can't move!
        {
...
SliderLabelWidget::SliderLabelWidget(
        SharedColorScaleElementSliders sliders,
        QWidget* parent /*=0*/,
        Qt::Orientation orientation /*=Qt::Horizontal*/)
    : ColorScaleEditorBaseWidget(sliders, orientation, DEFAULT_MARGIN, parent),
    ...
    if (m_sliders) {
        ...
        if (m_orientation == Qt::Horizontal) {
            ...
            for (int i = 0; i < m_sliders->size(); i++) {
                ...
                QString label = QString("%1 %").arg(
                        val * 100.0, 0, 'f',
                        std::max(m_precision - 2,
                                 0));  // display as a percentage
            ...
            {
                QString firstLabel = QString::number(
                        m_sliders->first()->getRelativePos(), 'f', m_precision);
                QString lastLabel = QString::number(
                        m_sliders->last()->getRelativePos(), 'f', m_precision);
                // int labelWidth =
                // std::max(fm.width(firstLabel),fm.width(lastLabel))+2*DEFAULT_TEXT_MARGIN;
                int labelWidth = std::max(fm.horizontalAdvance(firstLabel),
                                          fm.horizontalAdvance(lastLabel)) +
                                 2 * DEFAULT_TEXT_MARGIN;
            ...
            for (int i = 0; i < m_sliders->size(); i++) {
                ...
                painter.drawText(DEFAULT_TEXT_MARGIN,
                                 pos + m_sliders->at(i)->height(), label);
            }
        }
    }
...
ccColorScaleEditorWidget::ccColorScaleEditorWidget(
        QWidget* parent /*=0*/, Qt::Orientation orientation /*=Qt::Horizontal*/)
    : ColorScaleEditorBaseWidget(
              SharedColorScaleElementSliders(new ColorScaleElementSliders),
              orientation,
              0,
              parent) {
    // size and margin
    ...
    // layout
    setLayout(m_orientation == Qt::Horizontal
    ...
    // color bar
    {
        ...
        m_colorBarWidget->setSizePolicy(QSizePolicy::Expanding,
                                        QSizePolicy::Expanding);
        m_colorBarWidget->setContentsMargins(0, 0, 0, 0);
        layout()->addWidget(m_colorBarWidget);

        connect(m_colorBarWidget, SIGNAL(pointClicked(double)), this,
                SLOT(onPointClicked(double)));
    }
    ...
    // sliders widget
    {
        ...
        connect(m_slidersWidget, SIGNAL(sliderModified(int)), this,
                SLOT(onSliderModified(int)));
        connect(m_slidersWidget, SIGNAL(sliderSelected(int)), this,
                SLOT(onSliderSelected(int)));
    }
    ...
    // Labels widget
    {
        ...
        if (m_orientation == Qt::Horizontal) {
            m_labelsWidget->setSizePolicy(QSizePolicy::Expanding,
                                          QSizePolicy::Fixed);
            m_labelsWidget->setFixedHeight(DEFAULT_LABEL_HEIGHT);
        } else {
            m_labelsWidget->setSizePolicy(QSizePolicy::Fixed,
                                          QSizePolicy::Expanding);
...
void ccColorScaleEditorWidget::onPointClicked(double relativePos) {
    ...
    const double maxDist = static_cast<double>(DEFAULT_SLIDER_SYMBOL_SIZE) /
                           m_colorBarWidget->length();
    for (int i = 0; i < m_sliders->size(); ++i) {
...
void ccColorScaleEditorWidget::onSliderModified(int sliderIndex) {
    ...
    if (m_slidersWidget) m_slidersWidget->update();
    if (m_labelsWidget) m_labelsWidget->update();

    emit stepModified(sliderIndex);
}

void ccColorScaleEditorWidget::setSliders(
        SharedColorScaleElementSliders sliders) {
    if (m_sliders) {
        // onSliderSelected(-1);
    ...
    for (int i = 0; i < sliders->size(); ++i)
        m_slidersWidget->addNewSlider(sliders->at(i)->getRelativePos(),
                                      sliders->at(i)->getColor());
...
void ccColorScaleEditorWidget::exportColorScale(
        ccColorScale::Shared& destScale) const {
    if (!destScale) return;
...
void ccColorScaleEditorWidget::deleteStep(int index) {
    ...
    if (index >= 0) {
        if (m_sliders->at(index)->isSelected()) onSliderSelected(-1);
...
void ccColorScaleEditorWidget::setSelectedStepIndex(int index,
                                                    bool silent /*=false*/) {
    if (m_slidersWidget) m_slidersWidget->select(index, silent);
}
