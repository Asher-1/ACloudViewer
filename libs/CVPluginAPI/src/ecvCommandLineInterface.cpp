...
ecvProgressDialog *ccCommandLineInterface::progressDialog() { return nullptr; }
...
bool ccCommandLineInterface::processGlobalShiftCommand(
    ...
    if (arguments().empty()) {
        return error(QObject::tr("Missing parameter: global shift vector or %1 "
                                 "or %2 after '%3'")
                             .arg(COMMAND_OPEN_SHIFT_ON_LOAD_AUTO,
                                  COMMAND_OPEN_SHIFT_ON_LOAD_FIRST,
                                  COMMAND_OPEN_SHIFT_ON_LOAD));
    }
    ...
    if (firstParam.toUpper() == COMMAND_OPEN_SHIFT_ON_LOAD_AUTO) {
        ...
        if (!ok)
            return error(QObject::tr("Invalid parameter: X coordinate of the "
                                     "global shift vector after '%1'")
                                 .arg(COMMAND_OPEN_SHIFT_ON_LOAD));
        shiftOnLoadVec.y = arguments().takeFirst().toDouble(&ok);
        if (!ok)
            return error(QObject::tr("Invalid parameter: Y coordinate of the "
                                     "global shift vector after '%1'")
                                 .arg(COMMAND_OPEN_SHIFT_ON_LOAD));
        shiftOnLoadVec.z = arguments().takeFirst().toDouble(&ok);
        if (!ok)
            return error(QObject::tr("Invalid parameter: Z coordinate of the "
                                     "global shift vector after '%1'")
                                 .arg(COMMAND_OPEN_SHIFT_ON_LOAD));
...
ccCommandLineInterface::CLLoadParameters::CLLoadParameters()
    ...
    shiftHandlingMode = ecvGlobalShiftManager::NO_DIALOG;
