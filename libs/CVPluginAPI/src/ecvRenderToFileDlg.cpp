...
ccRenderToFileDlg::ccRenderToFileDlg(unsigned baseWidth,
    ...
    if (list.size() < 1) {
        CVLog::Error("No supported image format on this platform?!");
        reject();
        return;
    }
    ...
    for (int i = 0; i < list.size(); ++i) {
        filters.append(QString("%1 image (*.%2)\n")
                               .arg(QString(list[i].data()).toUpper())
                               .arg(list[i].data()));
    ...
    QString currentPath =
            settings.value("currentPath",
                           QStandardPaths::writableLocation(
                                   QStandardPaths::DocumentsLocation))
                    .toString();
    QString selectedExtension =
            settings.value("selectedExtension", firstExtension).toString();
    QString baseFilename = settings.value("baseFilename", "capture").toString();
    bool dontScale =
            settings.value("dontScaleFeatures", dontScalePoints()).toBool();
    bool doRenderOverlayItems =
            settings.value("renderOverlayItems", renderOverlayItems()).toBool();
    settings.endGroup();

    m_ui->dontScaleFeaturesCheckBox->setChecked(dontScale);
    m_ui->renderOverlayItemsCheckBox->setChecked(doRenderOverlayItems);
    m_ui->filenameLineEdit->setText(currentPath + QString("/") + baseFilename +
                                    QString(".") + selectedExtension);

    m_ui->zoomDoubleSpinBox->setValue(s_renderZoom);

    connect(m_ui->chooseFileButton, SIGNAL(clicked()), this,
            SLOT(chooseFile()));
    connect(m_ui->zoomDoubleSpinBox, SIGNAL(valueChanged(double)), this,
            SLOT(updateInfo()));
...
void ccRenderToFileDlg::chooseFile() {
    QString selectedFileName = QFileDialog::getSaveFileName(
            this, tr("Save Image"), m_ui->filenameLineEdit->text(), filters,
            &selectedFilter);

    // if operation is canceled, selectedFileName is empty
    if (selectedFileName.size() < 1) return;

    m_ui->filenameLineEdit->setText(selectedFileName);
}

ccRenderToFileDlg::~ccRenderToFileDlg() { delete m_ui; }
