	...
	, m_ui(new Ui::ColorScaleEditorDlg)
{
	...
	//main combo box
	connect(m_ui->rampComboBox, SIGNAL(activated(int)), this, SLOT(colorScaleChanged(int)));

	//import/export buttons
	connect(m_ui->exportToolButton,		SIGNAL(clicked()),				this,	SLOT(exportCurrentScale()));
	connect(m_ui->importToolButton,		SIGNAL(clicked()),				this,	SLOT(importScale()));

	//upper buttons
	connect(m_ui->renameToolButton,		SIGNAL(clicked()),				this,	SLOT(renameCurrentScale()));
	connect(m_ui->saveToolButton,			SIGNAL(clicked()),				this,	SLOT(saveCurrentScale()));
	connect(m_ui->deleteToolButton,		SIGNAL(clicked()),				this,	SLOT(deleteCurrentScale()));
	connect(m_ui->copyToolButton,			SIGNAL(clicked()),				this,	SLOT(copyCurrentScale()));
	connect(m_ui->newToolButton,			SIGNAL(clicked()),				this,	SLOT(createNewScale()));
	connect(m_ui->scaleModeComboBox,		SIGNAL(activated(int)),			this,	SLOT(relativeModeChanged(int)));
	...
	//slider editor
	connect(m_ui->deleteSliderToolButton,	SIGNAL(clicked()),				this,	SLOT(deletecSelectedStep()));
	connect(m_ui->colorToolButton,		SIGNAL(clicked()),				this,	SLOT(changeSelectedStepColor()));
	connect(m_ui->valueDoubleSpinBox,		SIGNAL(valueChanged(double)),	this,	SLOT(changeSelectedStepValue(double)));

	//labels list widget
	connect(m_ui->customLabelsGroupBox,		SIGNAL(toggled(bool)),		this,	SLOT(toggleCustomLabelsList(bool)));
	connect(m_ui->customLabelsPlainTextEdit,	SIGNAL(textChanged()),		this,	SLOT(onCustomLabelsListChanged()));
...
void ccColorScaleEditorDialog::onStepSelected(int index)
{
	...
	m_ui->deleteSliderToolButton->setEnabled(index >= 1 && index+1 < m_scaleWidget->getStepCount()); 
...
bool ccColorScaleEditorDialog::checkCustomLabelsList(bool showWarnings)
{
	...
	for (int i=0; i<items.size(); ++i)
	{
		...
		{
			if (showWarnings)
				CVLog::Error(QString("Invalid label value: '%1'").arg(items[i]));
			return false;
		}
	}
...
bool ccColorScaleEditorDialog::saveCurrentScale()
{
	...
	if ((!isRelative || isRelative != wasRelative) && m_mainApp && m_mainApp->dbRootObject())
	{
		...
		m_mainApp->dbRootObject()->filterChildren(clouds, true, CV_TYPES::POINT_CLOUD, true);
...
void ccColorScaleEditorDialog::exportCurrentScale()
{
	...
	{
		CVLog::Print(QString("[ColorScale] Scale '%1' successfully exported in '%2'").arg(m_colorScale->getName(),filename));
	}
}
		...
		if (m_manager)
		{
			...
			if (otherScale)
			{
				...
				if (QMessageBox::question	(this,
											...
											QMessageBox::No) == QMessageBox::No)
				{
					CVLog::Warning("[ccColorScaleEditorDialog::importScale] Importation cancelled due to a conflicting UUID (color scale may already be in store)");
					return;
				}
				//generate a new UUID
			...
			m_manager->addScale(scale);
			CVLog::Print(QString("[ccColorScaleEditorDialog::importScale] Color scale '%1' successfully imported").arg(scale->getName()));
		}
