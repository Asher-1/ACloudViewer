...
void ccPickingHub::togglePickingMode(bool state) {
    // CVLog::Warning(QString("Toggle picking mode: ") + (state ? "ON" : "OFF")
    ...
    if (!m_listeners.empty()) {
        if (m_exclusive)  // a previous listener is exclusive
        {
            ...
            if (m_listeners.find(listener) == m_listeners.end()) {
                CVLog::Warning(
                        "[ccPickingHub::addListener] Exclusive listener "
                        "already registered: stop the other tool relying on "
                        "point picking first");
                return false;
            }
        } else if (exclusive)  // this new listener is exclusive
        {
            if (m_listeners.size() > 1 ||
                m_listeners.find(listener) == m_listeners.end()) {
                CVLog::Warning(
                        "[ccPickingHub::addListener] Attempt to register an "
                        "exclusive listener while other listeners are already "
                        "registered");
                return false;
            }
            ...
            if (m_listeners.size() > 1 ||
                m_listeners.find(listener) == m_listeners.end()) {
                CVLog::Warning(
                        "[ccPickingHub::addListener] Other listeners are "
                        "already registered with a different picking mode");
                return false;
            }
        }
    }
