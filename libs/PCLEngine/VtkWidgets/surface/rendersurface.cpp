...
void RenderSurface::init()
{
    ...
    m_tableModel = new VtkUtils::TableModel(3, 10, this);
...
bool RenderSurface::initTableModel(const ccHObject * obj)
{
	if (!m_tableModel)
	{
...
bool RenderSurface::setInput(const ccHObject * obj)
{
...
void RenderSurface::reset()
{
	...
	m_ui->showEdgeCheckBox->setChecked(false);
	showScalarBar(false);
	m_ui->scalarBarGroupBox->setChecked(false);
	m_ui->showFrameCheckBox->setChecked(false);
	showOutline(false);
	m_ui->genFacesCheckBox->setChecked(false);
	m_ui->outlineGroupBox->setChecked(false);
	showContour(false);
	m_ui->showContourLinesCheckBox->setChecked(false);
	m_ui->showLabelsCheckBox->setChecked(false);
	m_ui->contourGroupBox->setChecked(false);
	showCubeAxes(false);
	m_ui->cubeAxesGroupBox->setChecked(false);
}

ccHObject * RenderSurface::getOutput() const
{
	return nullptr;
}

void RenderSurface::renderSurfaceFromPoints(const QVector<VtkUtils::Point3F>& points, const QVector<Tuple3ui>& vertices/* = QVector<Tuple3ui>()*/)
{
	if (points.isEmpty())
		return;

	VtkUtils::PointsToPolyDataConverter* ptpdConverter = new VtkUtils::PointsToPolyDataConverter(points, vertices);
...
{
	VtkUtils::ModelToPointsConverter* mtpConverter = new VtkUtils::ModelToPointsConverter(m_tableModel);
...
{
	VtkUtils::ModelToPointsConverter* mtpConverter = qobject_cast<VtkUtils::ModelToPointsConverter*>(sender());
	QVector<Tuple3ui> vertices = mtpConverter->vertices();
	meshMode = vertices.isEmpty() ? false : true;
    renderSurfaceFromPoints(mtpConverter->points().toVector(), vertices);
...
void RenderSurface::onPointsToPolyDataConverterFinished()
{
	...
	m_polyData = ptpdConverter->polyData();
	...
	VtkUtils::SignalBlocker sb(m_ui->rowsSpinBox);
...
{
	VtkUtils::SignalBlocker sb(m_ui->bgColorButton);
...
{
	VtkUtils::PointsReader* pointsReader = new VtkUtils::PointsReader(file);
...
void RenderSurface::onPointReaderFinished()
{
	...
	renderSurfaceFromPoints(points.toVector());

	VtkUtils::PointsToModelConverter* converter = new VtkUtils::PointsToModelConverter(points, m_tableModel);
...
void RenderSurface::showOutline(bool show)
{
    ...
    outline->SetInputData(m_polyData);
...
void RenderSurface::showContourLabels(bool show)
{
    ...
    contour->SetInputData(m_polyData);
...
void RenderSurface::showEdges(bool show)
{
    ...
    extractEdge->SetInputData(m_polyData);
