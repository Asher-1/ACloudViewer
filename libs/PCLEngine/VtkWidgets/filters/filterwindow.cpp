...
FilterWindow::FilterWindow(QWidget* parent)
    : QWidget(parent), m_ui(new Ui::GeneralFilterWindow) {
...
QString FilterWindow::fileName() const { return m_fileName; }
...
ccHObject* FilterWindow::getOutput() const {
    ...
    if (polyCount > 0) {
        ...
        const vtkIdType* tempIndexes = indexes;
#else   // VTK_CELL_ARRAY_V2
        vtkIdType* tempIndexes = indexes;
#endif  // VTK_CELL_ARRAY_V2
        vtkIdType iPt = 0;
        while (cell_array->GetNextCell(iPt, tempIndexes) != 0) {
            if (mesh->size() + 1 > mesh->capacity()) {
                if (!mesh->reserve(mesh->size() + 1 +
                                   256))  // take some advance to avoid too many
                                          // allocations
                {
...
bool FilterWindow::initTableModel(const ccHObject* obj) {
    ...
    m_tableModel->setHorizontalHeaderData(QVariantList() << "X"
                                                         << "Y"
                                                         << "Z");
...
void FilterWindow::onObjFileReaderFinished() {
    VtkUtils::ObjFileReader* stlReader =
            qobject_cast<VtkUtils::ObjFileReader*>(sender());
    createActorFromData<vtkPolyData, vtkPolyDataMapper>(
            stlReader->dataObject());
...
void FilterWindow::onFluentFileReaderFinished() {
    VtkUtils::FluentFileReader* fluentReader =
            qobject_cast<VtkUtils::FluentFileReader*>(sender());
    vtkMultiBlockDataSet* output = vtkMultiBlockDataSet::New();
    output->ShallowCopy(fluentReader->dataObject());

    createActorFromData<vtkUnstructuredGrid, vtkDataSetMapper>(
            output->GetBlock(0));
...
void FilterWindow::onStlFileReaderFinished() {
    VtkUtils::StlFileReader* stlReader =
            qobject_cast<VtkUtils::StlFileReader*>(sender());
    createActorFromData<vtkPolyData, vtkPolyDataMapper>(
            stlReader->dataObject());
    stlReader->deleteLater();
}
...
void FilterWindow::onVtkFileReaderFinished() {
    VtkUtils::VtkFileReader* vtkReader =
            qobject_cast<VtkUtils::VtkFileReader*>(sender());
    ...
    if (dataSet)
        createActorFromData<vtkDataSet, vtkDataSetMapper>(
                vtkReader->dataObject());
    else if (polyData)
        createActorFromData<vtkPolyData, vtkPolyDataMapper>(
                vtkReader->dataObject());
...
void FilterWindow::onNastranFileReaderFinished() {
    VtkUtils::NastranFileReader* nastranReader =
            qobject_cast<VtkUtils::NastranFileReader*>(sender());
    createActorFromData<vtkDataSet, vtkDataSetMapper>(
            nastranReader->dataObject());
    nastranReader->deleteLater();
}

void FilterWindow::onAnsysFileReaderFinished() {
    VtkUtils::AnsysFileReader* ansysReader =
            qobject_cast<VtkUtils::AnsysFileReader*>(sender());
    vtkDataSet* dataSet = vtkDataSet::SafeDownCast(ansysReader->dataObject());
    vtkPolyData* polyData =
            vtkPolyData::SafeDownCast(ansysReader->dataObject());
    if (dataSet)
        createActorFromData<vtkDataSet, vtkDataSetMapper>(
                ansysReader->dataObject());
    else if (polyData)
        createActorFromData<vtkPolyData, vtkPolyDataMapper>(
                ansysReader->dataObject());
...
void FilterWindow::onPlyFileReaderFinished() {
    VtkUtils::PlyFileReader* plyReader =
            qobject_cast<VtkUtils::PlyFileReader*>(sender());
    createActorFromData<vtkPolyData, vtkPolyDataMapper>(
            plyReader->dataObject());
...
void FilterWindow::readFile() {
    ...
    if (ext == "obj") {
        ...
        connect(fileReader, SIGNAL(finished()), this,
                SLOT(onObjFileReaderFinished()));
    } else if (ext == "vtk") {
        fileReader = new VtkUtils::VtkFileReader();
        connect(fileReader, SIGNAL(finished()), this,
                SLOT(onVtkFileReaderFinished()));
    } else if (ext == "stl") {
        fileReader = new VtkUtils::StlFileReader();
        connect(fileReader, SIGNAL(finished()), this,
                SLOT(onStlFileReaderFinished()));
    } else if (ext == "cdb") {
        fileReader = new VtkUtils::AnsysFileReader();
        connect(fileReader, SIGNAL(finished()), this,
                SLOT(onAnsysFileReaderFinished()));
    } else if (ext == "bdf") {
        fileReader = new VtkUtils::NastranFileReader();
        connect(fileReader, SIGNAL(finished()), this,
                SLOT(onNastranFileReaderFinished()));
    } else if (ext == "cas") {
        fileReader = new VtkUtils::FluentFileReader();
        connect(fileReader, SIGNAL(finished()), this,
                SLOT(onFluentFileReaderFinished()));
    } else if (ext == "vrml") {
    } else if (ext == "ply") {
        fileReader = new VtkUtils::PlyFileReader();
        connect(fileReader, SIGNAL(finished()), this,
                SLOT(onPlyFileReaderFinished()));
    }
...
QString FilterWindow::fileFilter() const {
    static QString filter =
            "ANSYS Files(*.obj);;VTK Files(*.vtk);;STL Files(*.stl);;ANSYS "
            "Files(*.cdb);;"
...
void FilterWindow::setResultData(vtkDataObject* data) { m_resultData = data; }

vtkDataObject* FilterWindow::resultData() const { return m_resultData; }
...
QColor FilterWindow::color1() const { return m_color1; }

QColor FilterWindow::color2() const { return m_color2; }
...
double FilterWindow::scalarMin() const { return m_scalarMin; }

double FilterWindow::scalarMax() const { return m_scalarMax; }

vtkSmartPointer<vtkLookupTable> FilterWindow::createLookupTable(double min,
                                                                double max) {
    double hsv1[3];
    double hsv2[3];
...
void FilterWindow::fireupModelToPointsConverter() {
    VtkUtils::ModelToPointsConverter* converter =
            new VtkUtils::ModelToPointsConverter(m_tableModel);
    converter->setAutoDelete(false);
    connect(converter, SIGNAL(finished()), this,
            SLOT(onModelToPointsConverterFinished()));
    QThreadPool::globalInstance()->start(converter);
}

void FilterWindow::onModelToPointsConverterFinished() {
    VtkUtils::ModelToPointsConverter* lastConverter =
            qobject_cast<VtkUtils::ModelToPointsConverter*>(sender());
    ...
    VtkUtils::PointsToPolyDataConverter* newConverter =
            new VtkUtils::PointsToPolyDataConverter(points, vertices);
    newConverter->setAutoDelete(false);
    connect(newConverter, SIGNAL(finished()), this,
            SLOT(onPointsToPolyDataConverterFinished()));
...
void FilterWindow::onPointsToPolyDataConverterFinished() {
    ...
    VtkUtils::PointsToPolyDataConverter* converter =
            qobject_cast<VtkUtils::PointsToPolyDataConverter*>(sender());
    vtkPolyData* polydata = converter->polyData();

    VtkUtils::vtkInitOnce(m_modelActor);
    VTK_CREATE(vtkPolyDataMapper, mapper);

    if (!m_dataObject) m_dataObject = vtkPolyData::New();
    ...
    if (meshMode) {
        ...
        if (!vertices) vertices = vtkSmartPointer<vtkCellArray>::New();

        vtkSmartPointer<vtkIdTypeArray> cells = vertices->GetData();

        // If no init cells and cells has not been initialized...
        if (!cells) cells = vtkSmartPointer<vtkIdTypeArray>::New();
        ...
        if (cells->GetNumberOfTuples() < nr_points) {
            ...
            // If the number of tuples is still too small, we need to recreate
            // the array
            ...
            // The assumption here is that the current set of cells has more
            // data than needed
            cells->SetNumberOfComponents(2);
            cells->SetNumberOfTuples(nr_points);
        }
...
void FilterWindow::createActorFromData(vtkDataObject* dataObj) {
    ...
    m_modelActor->SetNumberOfCloudPoints(
