...
CutWindow::CutWindow(QWidget* parent) : FilterWindow(parent) { createUi(); }
...
void CutWindow::setRadius(double radius) { m_radius = radius; }
...
CutWindow::CutType CutWindow::cutType() const { return m_cutType; }
...
void CutWindow::showContourLines(bool show) {
    ...
    //        for (int i = 0; i < numOfArrays; ++i)
    //            if (pointData->GetArray(i)->GetNumberOfTuples() > 0) // skip
    //            empty data array
...
void CutWindow::createUi() {
    ...
    VtkUtils::SignalBlocker sb(m_configUi->rowsSpinBox);
...
void CutWindow::on_randomButton_clicked() { randomTableModel(); }
