...
void StreamlineWindow::apply() {
    ...
    setResultData(streamline->GetOutput());
...
void StreamlineWindow::updateLineActor() {
    ...
    m_lineWidget->SetPoint2(m_point2);
    m_lineWidget->On();
    update();
}
...
void StreamlineWindow::on_integrationDirectionCombo_currentIndexChanged(
        int index) {
...
void StreamlineWindow::onPointPositionChanged(double* position) {
    if (Utils::ArrayComparator<double>()(m_center, position)) return;
