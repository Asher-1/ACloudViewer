...
QStringList SmallWidgetsInterface::getWidgtsNames() const {
    ...
    widgetsName << "ContourWidgetWindow"
                << "DistanceWidgetWindow"
                << "AngleWidgetWindow";
    return widgetsName;
}
...
QWidget *SmallWidgetsInterface::getWidgtByID(
...
const QString SmallWidgetsInterface::getWidgtNameByID(
        const VTK_WIDGETS_TYPE widgetID) const {
...
bool SmallWidgetsInterface::setInput(const ccHObject *obj,
...
ccHObject *SmallWidgetsInterface::getOutput(
        const DBLib::VTK_WIDGETS_TYPE widgetType) {
    if (m_widgetsMap.isEmpty() || m_NamesMap.isEmpty()) {
        return nullptr;
    }

    BaseWidgetWindow *baseWidget =
            static_cast<BaseWidgetWindow *>(getWidgtByID(widgetType));
    if (!baseWidget) {
        return nullptr;
    }
...
void SmallWidgetsInterface::initWidgets() {
    ...
    for (QString &name : widgetNames) {
        if (name == QString("ContourWidgetWindow")) {
            m_widgetsMap.insert(VTK_WIDGETS_TYPE::VTK_CONTOUR_WIDGET,
                                new ContourWidgetWindow());
            m_NamesMap.insert(VTK_WIDGETS_TYPE::VTK_CONTOUR_WIDGET, name);
        } else if (name == QString("DistanceWidgetWindow")) {
            m_widgetsMap.insert(VTK_WIDGETS_TYPE::VTK_DISTANCE_WIDGET,
                                new DistanceWidgetWindow());
            m_NamesMap.insert(VTK_WIDGETS_TYPE::VTK_DISTANCE_WIDGET, name);
        } else if (name == QString("AngleWidgetWindow")) {
            m_widgetsMap.insert(VTK_WIDGETS_TYPE::VTK_ANGLE_WIDGET,
                                new AngleWidgetWindow());
