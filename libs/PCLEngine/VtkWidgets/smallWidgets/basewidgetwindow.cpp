...
BaseWidgetWindow::BaseWidgetWindow(QWidget *parent)
    : QWidget(parent), m_ui(new Ui::BaseWidgetWindow) {}

BaseWidgetWindow::~BaseWidgetWindow() { delete m_ui; }

bool BaseWidgetWindow::setInput(const ccHObject *obj) {
    return obj ? true : false;
}

ccHObject *BaseWidgetWindow::getOutput() const { return nullptr; }

void BaseWidgetWindow::createUi() {
    m_vtkWidget = new VtkUtils::VtkWidget(this);
    m_ui->previewLayout->addWidget(m_vtkWidget);

    m_theActor = vtkSmartPointer<vtkActor>::Take(
            VtkUtils::createSourceActor<vtkConeSource>());
