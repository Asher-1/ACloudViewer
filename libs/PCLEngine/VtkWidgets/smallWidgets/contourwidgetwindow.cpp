...
static const char* sLabels[] = {"Books", "New and Popular", "Periodical",
                                "Audiobook", "Video"};

ContourWidgetWindow::ContourWidgetWindow(QWidget* parent)
    : BaseWidgetWindow(parent) {
    createUi();
    createWidget();
}

ContourWidgetWindow::~ContourWidgetWindow() {
    if (m_configUi) {
        delete m_configUi;
        m_configUi = nullptr;
    }
...
void ContourWidgetWindow::createWidget() {
    ...
    VtkUtils::ContourWidgetObserver* observer =
            new VtkUtils::ContourWidgetObserver(this);
    observer->attach(m_contour);
    connect(observer, SIGNAL(dataChanged(vtkPolyData*)), this,
            SLOT(onDataChanged(vtkPolyData*)));
}

void ContourWidgetWindow::createUi() {
    m_configUi = new Ui::ContourWidgetConfig;
    setupConfigWidget(m_configUi);
    m_configUi->clearButton->show();
    m_configUi->showNodesCheckBox->show();
