...
DistanceWidgetWindow::DistanceWidgetWindow(QWidget* parent)
    : BaseWidgetWindow(parent) {
    createUi();
    createWidget();
}

DistanceWidgetWindow::~DistanceWidgetWindow() {
    if (m_configUi) {
        delete m_configUi;
        m_configUi = nullptr;
    }
...
void DistanceWidgetWindow::onWorldPoint1Changed(double* pos) {
    VtkUtils::SignalBlocker sb(m_configUi->point1XSpinBox);
...
void DistanceWidgetWindow::onWorldPoint2Changed(double* pos) {
    VtkUtils::SignalBlocker sb(m_configUi->point2XSpinBox);
...
void DistanceWidgetWindow::hookWidget(
        const vtkSmartPointer<vtkDistanceWidget>& widget) {
    VtkUtils::DistanceWidgetObserver* distWidgetObserver =
            new VtkUtils::DistanceWidgetObserver(this);
    distWidgetObserver->attach(widget);
    connect(distWidgetObserver, SIGNAL(distanceChanged(double)), this,
            SLOT(onDistanceChanged(double)));
    connect(distWidgetObserver, SIGNAL(worldPoint1Changed(double*)), this,
            SLOT(onWorldPoint1Changed(double*)));
    connect(distWidgetObserver, SIGNAL(worldPoint2Changed(double*)), this,
            SLOT(onWorldPoint2Changed(double*)));
}