...
ModelToVectorsConverter::ModelToVectorsConverter(TableModel* model)
    : m_model(model) {}
...
void ModelToVectorsConverter::run() {
    ...
    for (int i = 0; i < rows; ++i) {
        Vector4F vec{m_model->data(i, 0), m_model->data(i, 1),
                     m_model->data(i, 2), m_model->data(i, 3)};
...
QList<Vector4F> ModelToVectorsConverter::vectors() const { return m_vectors; }
