...
void ImageVis::changeOpacity(const std::string& viewID, double opacity) {
    ...
    if (layer) {
        ...
        int index = 0;
        while (layer->actor->GetScene()->GetItem(index)) {
            pcl::visualization::PCLContextItem* context =
                    reinterpret_cast<pcl::visualization::PCLContextItem*>(
                            layer->actor->GetScene()->GetItem(index));
            if (context) {
...
void ImageVis::addRGBImage(const unsigned char* rgb_data,
                           unsigned x,
                           unsigned y,
                           unsigned width,
                           unsigned height,
    ...
    // to the visualizer?)
    pcl::visualization::ImageViewer::LayerMap::iterator am_it = std::find_if(
            layer_map_.begin(), layer_map_.end(), LayerComparator(layer_id));
    if (am_it == layer_map_.end()) {
        // PCL_DEBUG("[pcl::visualization::ImageViewer::addRGBImage] No layer
