...
int vtkBoundedPlaneSource::RequestData(vtkInformation*,
    ...
    if (!bbox.IsValid()) {
        vtkErrorMacro(
                "Invalid bounding box specified. Please choose a valid "
                "BoundingBox.");
        return 0;
    }
    ...
    if (this->RefinementMode == USE_RESOLUTION) {
        if (!vtkBoundedVolumeSource::SetImageParameters(
                    image, bbox, vtkVector3i(this->Resolution))) {
            vtkErrorMacro("Failed to determine image parameters.");
            return 0;
        }
    } else {
        if (!vtkBoundedVolumeSource::SetImageParameters(image, bbox,
                                                        this->CellSize)) {
            vtkErrorMacro("Failed to determine image parameters.");
            return 0;
        }
    ...
    // AllocateScalars, alas, is needed since vtkFlyingEdgesPlaneCutter cannot
    // work without scalars.
...
void vtkBoundedPlaneSource::PrintSelf(ostream& os, vtkIndent indent) {
    ...
    os << indent << "Normal: " << this->Normal[0] << ", " << this->Normal[1]
       << ", " << this->Normal[2] << endl;
    os << indent << "BoundingBox: " << this->BoundingBox[0] << ", "
       << this->BoundingBox[1] << ", " << this->BoundingBox[2] << ", "
       << this->BoundingBox[3] << ", " << this->BoundingBox[4] << ", "
