...
void vtkDiscretizableColorTransferFunctionCustom::SetAnnotationsInFullSet(
    ...
    if ((values && !annotations) || (!values && annotations)) return;

    if (values && annotations &&
        values->GetNumberOfTuples() != annotations->GetNumberOfTuples()) {
        vtkErrorMacro(<< "Values and annotations do not have the same number "
                         "of tuples ("
                      << values->GetNumberOfTuples() << " and "
                      << annotations->GetNumberOfTuples()
    ...
    if (this->AnnotatedValuesInFullSet && !values) {
    ...
    } else if (values) {  // Ensure arrays are of the same type before copying.
        if (this->AnnotatedValuesInFullSet) {
            if (this->AnnotatedValuesInFullSet->GetDataType() !=
                values->GetDataType()) {
                this->AnnotatedValuesInFullSet->Delete();
                this->AnnotatedValuesInFullSet = 0;
            }
        ...
        if (!this->AnnotatedValuesInFullSet) {
            this->AnnotatedValuesInFullSet =
                    vtkAbstractArray::CreateArray(values->GetDataType());
        }
    }
...
vtkIdType vtkDiscretizableColorTransferFunctionCustom::SetAnnotationInFullSet(
    ...
    if (valid) {
        ...
        // NOTE: This prevents the value "" in vtkStringArrays from being
        // annotated. Hopefully, that isn't a desired use case.
        return -1;
    }
...
//-----------------------------------------------------------------------------
void vtkDiscretizableColorTransferFunctionCustom::SetActiveAnnotatedValue(
        std::string value) {
...
//-----------------------------------------------------------------------------
void vtkDiscretizableColorTransferFunctionCustom::
        SetNumberOfIndexedColorsInFullSet(int n) {
    if (n !=
        static_cast<int>(this->IndexedColorsInFullSet->GetNumberOfTuples())) {
...
void vtkDiscretizableColorTransferFunctionCustom::SetIndexedColorInFullSet(
    ...
    if (index >= static_cast<unsigned int>(
                         this->IndexedColorsInFullSet->GetNumberOfTuples())) {
        this->SetNumberOfIndexedColorsInFullSet(static_cast<int>(index + 1));
        this->Modified();
    }

    // double *currentRGB =
    // static_cast<double*>(this->IndexedColorsInFullSet->GetVoidPointer(index));
    double currentRGB[3];
...
//-----------------------------------------------------------------------------
int vtkDiscretizableColorTransferFunctionCustom::
...
//-----------------------------------------------------------------------------
void vtkDiscretizableColorTransferFunctionCustom::
        SetNumberOfIndexedOpacitiesInFullSet(int n) {
    if (n != static_cast<int>(
                     this->IndexedOpacitiesInFullSet->GetNumberOfTuples())) {
...
void vtkDiscretizableColorTransferFunctionCustom::Build() {
    ...
    if (this->AnnotatedValuesInFullSet) {
        ...
        for (vtkIdType i = 0;
             i < this->AnnotatedValuesInFullSet->GetNumberOfTuples(); ++i) {
            std::string annotation = this->AnnotationsInFullSet->GetValue(i);
            vtkVariant value =
                    this->AnnotatedValuesInFullSet->GetVariantValue(i);
            ...
            if (useAnnotation) {
                ...
                if (i < this->IndexedColorsInFullSet->GetNumberOfTuples()) {
                    ...
                    if (this->EnableOpacityMapping &&
                        i < this->IndexedOpacitiesInFullSet
                                        ->GetNumberOfTuples()) {
        ...
        this->SetAnnotations(builtValues.GetPointer(),
                             builtAnnotations.GetPointer());
    }
