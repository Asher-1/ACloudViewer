...
double vtkMinkowskiFilter::compute_C(vtkPolyhedron* cell)
{
  ...
  std::vector< std::vector<double> > face_normals(num_faces, std::vector<double>(3));

  std::vector< std::vector<int> > edge_faces(num_edges, std::vector<int>(2));
  ...
  for (i = 0; i < num_faces; i++)
  {
    ...
    compute_normal(f, face_normals[i].data());
  }
