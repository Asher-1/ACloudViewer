 ...
 * @param in2outMapping Indices mapping: for each output point, index of input
 * point
 * @param overwrite Whether to overwrite existing fields with same names
 * (default: true)
 *
 * This function copies all scalar fields from the input cloud to the output
 * cloud using the provided index mapping. The mapping allows for point
 * reordering or subsampling during the copy operation.
 *
 ...
 * If input cloud has no colors, the operation completes successfully without
 * effect.
 *
 * @note The output cloud must be pre-allocated with correct size.
