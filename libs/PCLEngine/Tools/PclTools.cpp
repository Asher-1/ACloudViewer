...
void showCameras (pcl::texture_mapping::CameraVector cams, pcl::PointCloud<PointT>::Ptr &cloud)
{
...
bool readCamPoseFile(std::string filename, pcl::TextureMapping<PointT>::Camera &cam)
{
...
bool ConvertToPCLCamPose(const camera::PinholeCameraParameters& pinholeCamera, pcl::TextureMapping<PointT>::Camera &cam)
{
                           ...
                           const PCLTextureMesh &tex_mesh, unsigned precision)
{
  ...
  for (int m = 0; m < nr_meshes; ++m)
  {
    ...
    CVLog::Print("%d vertex textures in submesh %d", tex_mesh.tex_coordinates[m].size (), m);
  ...
  for (int m = 0; m < nr_meshes; ++m)
  {
    ...
    CVLog::Print("%d faces in mesh %d ", tex_mesh.tex_polygons[m].size () , m);
                                                    ...
                                                    bool show_cameras, bool verbose)
{
    ...
    if (show_cameras)
    {
        CVLog::Print("Displaying cameras. Press \'q\' to continue texture mapping");
    ...
    pcl::TextureMapping<PointT> tm; // TextureMapping object that will perform the sort
