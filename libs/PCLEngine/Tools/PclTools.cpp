...
bool PclTools::SaveOBJFile(const std::string& file_name,
    ...
    if (win_separator_index > linux_separator_index) {
        ...
        mtl_file_name_nopath.erase(0, linux_separator_index + 1);
    }
    ...
    for (int i = 0; i < nr_points; ++i) {
        ...
        for (std::size_t d = 0; d < tex_mesh.cloud.fields.size(); ++d) {
            // adding vertex
            if ((tex_mesh.cloud.fields[d].datatype ==
                ...
                float value;
                memcpy(&value,
                       &tex_mesh.cloud.data[i * point_size +
                                            tex_mesh.cloud.fields[d].offset],
                       sizeof(float));
                fs << value;
                if (++xyz == 3) break;
                fs << " ";
        ...
        if (xyz != 3) {
            CVLog::Error(
                    "[pcl::io::saveOBJFile] Input point cloud has no XYZ "
                    "data!");
            return false;
        }
        fs << std::endl;
    ...
    for (int m = 0; m < nr_meshes; ++m) {
        if (tex_mesh.tex_coordinates.empty()) continue;

        CVLog::Print("%d vertex textures in submesh %d",
                     tex_mesh.tex_coordinates[m].size(), m);
        fs << "# " << tex_mesh.tex_coordinates[m].size()
           << " vertex textures in submesh " << m << std::endl;
    ...
    for (int m = 0; m < nr_meshes; ++m) {
        ...
        for (std::size_t i = 0; i < tex_mesh.tex_polygons[m].size(); ++i) {
            // Write faces with "f"
            fs << "f";
            // There's one UV per vertex per face, i.e., the same vertex can
            // have different UV depending on the face.
            for (std::size_t j = 0;
                 j < tex_mesh.tex_polygons[m][i].vertices.size(); ++j) {
                unsigned int idx = tex_mesh.tex_polygons[m][i].vertices[j] + 1;
                fs << " " << idx << "/" << 3 * (i + f_idx) + j + 1 << "/"
                   << idx;  // vertex index in obj file format starting with 1
            }
            fs << std::endl;
        }
        CVLog::Print("%d faces in mesh %d ", tex_mesh.tex_polygons[m].size(),
                     m);
    ...
    if (tex_mesh.tex_materials.empty()) return true;
    ...
    for (int m = 0; m < nr_meshes; ++m) {
        ...
        m_fs << "Ka " << tex_mesh.tex_materials[m].tex_Ka.r << " "
             << tex_mesh.tex_materials[m].tex_Ka.g << " "
             << tex_mesh.tex_materials[m].tex_Ka.b
             << std::endl;  // defines the ambient color of the material to be
                            // (r,g,b).
        m_fs << "Kd " << tex_mesh.tex_materials[m].tex_Kd.r << " "
             << tex_mesh.tex_materials[m].tex_Kd.g << " "
             << tex_mesh.tex_materials[m].tex_Kd.b
             << std::endl;  // defines the diffuse color of the material to be
                            // (r,g,b).
        m_fs << "Ks " << tex_mesh.tex_materials[m].tex_Ks.r << " "
             << tex_mesh.tex_materials[m].tex_Ks.g << " "
             << tex_mesh.tex_materials[m].tex_Ks.b
             << std::endl;  // defines the specular color of the material to be
                            // (r,g,b). This color shows up in highlights.
        m_fs << "d " << tex_mesh.tex_materials[m].tex_d
             << std::endl;  // defines the transparency of the material to be
                            // alpha.
        ...
        m_fs << "illum " << tex_mesh.tex_materials[m].tex_illum
             << std::endl;  // denotes the illumination model used by the
                            // material.
        // illum = 1 indicates a flat material with no specular highlights, so
        // the value of Ks is not used. illum = 2 denotes the presence of
        // specular highlights, and so a specification for Ks is required.
