...
namespace ecvTools
{
	...
	static ccPolyline* GetPolylines(
		PCLCloud::Ptr &curve_sm, 
		...
		bool randomColors = true)
	{
		...
		else
		{
			...
			QString cloudName = QString("%1 %2 (size=%3)").arg(label).
				arg(ecvGroup->getChildrenNumber()).arg(out_cloud_cc->size());
		...
		bool& error)
	{
		...
		ccHObject* ecvGroup = new ccHObject(
			ccCloud ? ccCloud->getName() : "" + QString(" [clusters]-"));
		...
		for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it)
		{
			...
			AddPointCloud(ecvGroup, sm_cloud, error, "cluster", ccCloud, randomColors);
		}
		...
		bool& error)
	{
		...
		for (auto &cluster : cluster_indices)
		{
			...
			if (cloud)
			{
				...
				QString cloudName = QString("%1 %2 (size=%3)").arg("cluster").
					arg(ecvGroup->getChildrenNumber()).arg(cloud->size());
		...
		bool& error)
	{
		...
		if (cloudRemained)
		{
			...
			if (sm_cloud->height * sm_cloud->width != 0)
			{
				AddPointCloud(ecvGroup, sm_cloud, error, "remaining", ccCloud, randomColors);
			}
			else
			{
				error = true;
		...
		for (std::vector<PointCloudT::Ptr>::const_iterator it = cloudExtractions.begin(); it != cloudExtractions.end(); ++it)
		{
			PCLCloud::Ptr sm_cloud(new PCLCloud);
			...
			AddPointCloud(ecvGroup, sm_cloud, error, "segment", ccCloud, randomColors);
		}
