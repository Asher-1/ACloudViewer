...
void cvRenderViewSelectionTool::enable() {
    ...
    // ParaView-style: Enter selection mode for cache optimization
    // This tells the pipeline to cache selection render buffers
...
void cvRenderViewSelectionTool::disable() {
    ...
    // ParaView-style: Exit selection mode to release cached buffers
    cvSelectionPipeline* pipeline = getSelectionPipeline();
    if (pipeline) {
