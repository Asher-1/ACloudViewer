...
cvProbeFilter::cvProbeFilter(QWidget* parent) 
...
#ifdef USE_TBB
		tbb::parallel_for(0, numOfTuples, [&](int dataIndex)
#else
...
void cvProbeFilter::showInteractor(bool state)
{
	...
	case WidgetType::WT_ImplicitPlane:
		...
		{
			state ? m_implicitPlaneWidget->On() : safeOff(m_implicitPlaneWidget);
		}
		break;
...
ccHObject * cvProbeFilter::getOutput()
{
	...
	if (m_plotWidget)
	{
		...
		if (result)
		{
			CVLog::Print(tr("Probe figure has been saved in %1").arg(selectedFilename));
		}
		else
		{
