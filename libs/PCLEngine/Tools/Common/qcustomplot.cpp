...
void QCPPainter::setAntialiasing(bool enabled) {
    ...
    if (mIsAntialiasing != enabled) {
        ...
        if (!mModes.testFlag(
                    pmVectorized))  // antialiasing half-pixel shift only needed
                                    // for rasterized outputs
        {
            if (mIsAntialiasing)
                translate(0.5, 0.5);
            else
...
void QCPScatterStyle::drawShape(QCPPainter *painter, double x, double y) const {
    ...
    switch (mShape) {
        ...
        case ssStar: {
            painter->drawLine(QLineF(x - w, y, x + w, y));
            painter->drawLine(QLineF(x, y + w, x, y - w));
            painter->drawLine(QLineF(x - w * 0.707, y - w * 0.707,
                                     x + w * 0.707, y + w * 0.707));
            painter->drawLine(QLineF(x - w * 0.707, y + w * 0.707,
                                     x + w * 0.707, y - w * 0.707));
            break;
        ...
        case ssTriangle: {
            painter->drawLine(
                    QLineF(x - w, y + 0.755 * w, x + w, y + 0.755 * w));
        ...
        case ssCrossCircle: {
            painter->drawLine(QLineF(x - w * 0.707, y - w * 0.707,
                                     x + w * 0.670, y + w * 0.670));
            painter->drawLine(QLineF(x - w * 0.707, y + w * 0.670,
                                     x + w * 0.670, y - w * 0.707));
            painter->drawEllipse(QPointF(x, y), w, w);
            break;
        }
        ...
        case ssPixmap: {
            painter->drawPixmap(x - mPixmap.width() * 0.5,
                                y - mPixmap.height() * 0.5, mPixmap);
            break;
        }
        case ssCustom: {
...
QCPLayer::~QCPLayer() {
    ...
    // try to reach back to this then invalid layer once they get deleted/moved
    // themselves. This only happens when layers are deleted directly, like in
    // the QCustomPlot destructor. (The regular layer removal procedure for the
    // user is to call QCustomPlot::removeLayer, which moves all layerables off
    ...
    if (mParentPlot->currentLayer() == this)
        qDebug() << Q_FUNC_INFO
                 << "The parent plot's mCurrentLayer will be a dangling "
                    "pointer. Should have been set to a valid layer or 0 "
                    "beforehand.";
}
...
/*!
  ...
  QObject-parent (for memory management) of this layerable, \a plot does.
*/
...
QCPRange QCPRange::sanitizedForLogScale() const {
    ...
    if (sanitizedRange.lower == 0.0 && sanitizedRange.upper != 0.0) {
        ...
        // find out whether negative or positive interval is wider to decide
        // which sign domain will be chosen
...
int QCPMarginGroup::commonMargin(QCP::MarginSide side) const {
    ...
    for (int i = 0; i < elements.size(); ++i) {
        ...
        int m = qMax(
                elements.at(i)->calculateAutoMargin(side),
...
QCPLayoutElement::QCPLayoutElement(QCustomPlot *parentPlot)
    : QCPLayerable(parentPlot),  // parenthood is changed as soon as layout
                                 // element gets inserted into a layout (except
                                 // for top level layout)
      mParentLayout(0),
      mMinimumSize(),
...
QCPLayoutElement::~QCPLayoutElement() {
    ...
    if (qobject_cast<QCPLayout *>(
                mParentLayout))  // the qobject_cast is just a safeguard in case
                                 // the layout forgets to call clear() in its
                                 // dtor and this dtor is called by QObject dtor
...
void QCPLayoutElement::setMarginGroup(QCP::MarginSides sides,
    ...
    for (int i = 0; i < sideVector.size(); ++i) {
        ...
        if (marginGroup(side) != group) {
            ...
            if (!group)  // if setting to 0, remove hash entry. Else set hash
                         // entry to new group and register there
            {
...
void QCPLayoutElement::update(UpdatePhase phase) {
    if (phase == upMargins) {
        if (mAutoMargins != QCP::msNone) {
            // set the margins of this layout element according to automatic
            // margin calculation, either directly or via a margin group:
            QMargins newMargins = mMargins;
            QList<QCP::MarginSide> allMarginSides =
                    QList<QCP::MarginSide>() << QCP::msLeft << QCP::msRight
                                             << QCP::msTop << QCP::msBottom;
            foreach (QCP::MarginSide side, allMarginSides) {
                if (mAutoMargins.testFlag(side))  // this side's margin shall be
                    ...
                    if (mMarginGroups.contains(side))
                        QCP::setMarginValue(
                                ...
                                mMarginGroups[side]->commonMargin(
                                        side));  // this side is part of a
                                                 // margin group, so get the
                                                 // margin value from that group
                    else
                        QCP::setMarginValue(
                                newMargins, side,
                                calculateAutoMargin(
                                        side));  // this side is not part of a
                                                 // group, so calculate the
                                                 // value directly
...
QVector<int> QCPLayout::getSectionSizes(QVector<int> maxSizes,
    ...
    if (totalSize < minSizeSum) {
        // new stretch factors are minimum sizes and minimum sizes are set to
        // zero:
    ...
    while (!unfinishedSections.isEmpty() &&
           outerIterations <
                   sectionCount *
                           2)  // the iteration check ist just a failsafe in
                               // case something really strange happens
    {
                                   ...
                                   // case something really strange happens
        {
            ...
            for (int i = 0; i < unfinishedSections.size(); ++i) {
                ...
                double hitsMaxAt =
                        (maxSizes.at(secId) - sectionSizes.at(secId)) /
            ...
            // check if that maximum is actually within the bounds of the total
            // size (i.e. can we stretch all remaining sections so far that the
            // found section actually hits its maximum, without exceeding the
            // total size when we add up all sections)
            ...
            if (nextMax <
                nextMaxLimit)  // next maximum is actually hit, move forward to
                               // that point and fix the size of that section
            {
                for (int i = 0; i < unfinishedSections.size(); ++i) {
                    ...
                    freeSize -= nextMax *
                                stretchFactors.at(unfinishedSections.at(i));
                }
                unfinishedSections.removeOne(
                        nextId);  // exclude the section that is now at maximum
                                  // from further changes
            } else  // next maximum isn't hit, just distribute rest of free
                    // space on remaining sections
            {
                for (int i = 0; i < unfinishedSections.size(); ++i)
                    sectionSizes[unfinishedSections.at(i)] +=
                            nextMaxLimit *
                            stretchFactors.at(unfinishedSections.at(
                                    i));  // increment all sections
        ...
        for (int i = 0; i < sectionSizes.size(); ++i) {
            ...
            if (sectionSizes.at(i) <
                minSizes.at(i))  // section violates minimum
            {
                sectionSizes[i] = minSizes.at(i);  // set it to minimum
                foundMinimumViolation = true;  // make sure we repeat the whole
        ...
        if (foundMinimumViolation) {
            ...
            for (int i = 0; i < sectionCount; ++i) {
                if (!minimumLockedSections.contains(
                            i))  // only put sections that haven't hit their
                                 // minimum back into the pool
                    unfinishedSections.append(i);
                else
                    freeSize -= sectionSizes.at(
                            i);  // remove size of minimum locked sections from
                                 // available space in next round
            }
            // reset all section sizes to zero that are in unfinished sections
            // (all others have been set to their minimum):
            for (int i = 0; i < unfinishedSections.size(); ++i)
...
QCPLayoutGrid::~QCPLayoutGrid() {
    ...
    // specific layouts know how to handle removing elements (clear calls
    // virtual removeAt method to do that).
    clear();
}
        ...
        for (int col = 0; col < columnCount(); ++col) {
            ...
            if (mElements.at(row).at(col))
                mElements.at(row).at(col)->setOuterRect(
                        QRect(xOffset, yOffset, colWidths.at(col),
                              rowHeights.at(row)));
        }
    }
}
...
void QCPLayoutGrid::simplify() {
    ...
    for (int row = rowCount() - 1; row >= 0; --row) {
        ...
        if (!hasElements) {
            ...
            if (mElements.isEmpty())  // removed last element, also remove
                                      // stretch factor (wouldn't happen below
                                      // because also columnCount changed to 0
                                      // now)
...
QSize QCPLayoutGrid::maximumSizeHint() const {
    ...
    for (int i = 0; i < maxColWidths.size(); ++i)
        result.setWidth(
                qMin(result.width() + maxColWidths.at(i), QWIDGETSIZE_MAX));
    ...
    for (int row = 0; row < rowCount(); ++row) {
        for (int col = 0; col < columnCount(); ++col) {
            if (mElements.at(row).at(col)) {
                ...
                QSize final(max.width() < QWIDGETSIZE_MAX ? max.width()
                                                          : maxHint.width(),
                            max.height() < QWIDGETSIZE_MAX ? max.height()
                                                           : maxHint.height());
...
void QCPLayoutInset::updateLayout() {
    for (int i = 0; i < mElements.size(); ++i) {
        ...
        if (mInsetPlacement.at(i) == ipFree) {
            insetRect =
                    QRect(rect().x() + rect().width() * mInsetRect.at(i).x(),
...
double QCPLayoutInset::selectTest(const QPointF &pos,
    ...
    for (int i = 0; i < mElements.size(); ++i) {
        // inset layout shall only return positive selectTest, if actually an
        // inset object is at pos else it would block the entire underlying
        // QCPAxisRect with its surface.
...
double QCPLineEnding::boundingDistance() const {
    switch (mStyle) {
        ...
        case esHalfBar:
            return mWidth *
                   1.42;  // items that only have a width -> width*sqrt(2)
    }
    return 0;
}
    ...
    switch (mStyle) {
        ...
        case esSkewedBar: {
            if (qFuzzyIsNull(painter->pen().widthF()) &&
                ...
                // if drawing with cosmetic pen (perfectly thin stroke, happens
                // only in vector exports), draw bar exactly on tip of line
                painter->drawLine((pos + widthVec +
                                   lengthVec * 0.2f * (mInverted ? -1 : 1))
                                          .toPointF(),
                                  (pos - widthVec -
                                   lengthVec * 0.2f * (mInverted ? -1 : 1))
                                          .toPointF());
            } else {
                // if drawing with thick (non-cosmetic) pen, shift bar a little
                // in line direction to prevent line from sticking through bar
                // slightly
                painter->drawLine(
                        (pos + widthVec +
                         lengthVec * 0.2f * (mInverted ? -1 : 1) +
                         dir.normalized() *
                                 qMax(1.0f, (float)painter->pen().widthF()) *
                                 0.5f)
                                .toPointF(),
                        (pos - widthVec -
                         lengthVec * 0.2f * (mInverted ? -1 : 1) +
...
void QCPGrid::drawGridLines(QCPPainter *painter) const {
    ...
    if (mParentAxis->orientation() == Qt::Horizontal) {
        ...
        if (mZeroLinePen.style() != Qt::NoPen &&
            mParentAxis->mRange.lower < 0 && mParentAxis->mRange.upper > 0) {
            applyAntialiasingHint(painter, mAntialiasedZeroLine,
                                  QCP::aeZeroLine);
            painter->setPen(mZeroLinePen);
            double epsilon = mParentAxis->range().size() *
                             1E-6;  // for comparing double to zero
            for (int i = lowTick; i <= highTick; ++i) {
                if (qAbs(mParentAxis->mTickVector.at(i)) < epsilon) {
                    zeroLineIndex = i;
                    t = mParentAxis->coordToPixel(
                            mParentAxis->mTickVector.at(i));  // x
                    painter->drawLine(QLineF(t,
                                             mParentAxis->mAxisRect->bottom(),
                                             t, mParentAxis->mAxisRect->top()));
                    break;
                }
            }
        }
        ...
        if (mZeroLinePen.style() != Qt::NoPen &&
            ...
            applyAntialiasingHint(painter, mAntialiasedZeroLine,
                                  QCP::aeZeroLine);
            painter->setPen(mZeroLinePen);
            double epsilon = mParentAxis->mRange.size() *
                             1E-6;  // for comparing double to zero
            for (int i = lowTick; i <= highTick; ++i) {
                if (qAbs(mParentAxis->mTickVector.at(i)) < epsilon) {
                    zeroLineIndex = i;
                    t = mParentAxis->coordToPixel(
                            mParentAxis->mTickVector.at(i));  // y
                    painter->drawLine(QLineF(mParentAxis->mAxisRect->left(), t,
                                             mParentAxis->mAxisRect->right(),
                                             t));
                    break;
                }
            }
...
void QCPGrid::drawSubGridLines(QCPPainter *painter) const {
    ...
    if (mParentAxis->orientation() == Qt::Horizontal) {
        for (int i = 0; i < mParentAxis->mSubTickVector.size(); ++i) {
            t = mParentAxis->coordToPixel(
                    mParentAxis->mSubTickVector.at(i));  // x
            painter->drawLine(QLineF(t, mParentAxis->mAxisRect->bottom(), t,
                                     mParentAxis->mAxisRect->top()));
        }
    } else {
        for (int i = 0; i < mParentAxis->mSubTickVector.size(); ++i) {
            t = mParentAxis->coordToPixel(
                    mParentAxis->mSubTickVector.at(i));  // y
            painter->drawLine(QLineF(mParentAxis->mAxisRect->left(), t,
                                     mParentAxis->mAxisRect->right(), t));
        }
    }
...
QCPAxis::QCPAxis(QCPAxisRect *parent, AxisType type)
    ...
    setAntialiased(false);
    setLayer(mParentPlot
                     ->currentLayer());  // it's actually on that layer already,
...
void QCPAxis::setScaleLogBase(double base) {
    if (base > 1) {
        ...
        mScaleLogBaseLogInv =
                1.0 /
                qLn(mScaleLogBase);  // buffer for faster baseLog() calculation
        mCachedMarginValid = false;
    } else
        qDebug() << Q_FUNC_INFO
...
void QCPAxis::setTickVectorLabels(const QVector<QString> &vec) {
    // don't check whether mTickVectorLabels != vec here, because it takes
    // longer than we would save
...
void QCPAxis::setTickLengthOut(int outside) {
    if (mAxisPainter->tickLengthOut != outside) {
        ...
        mCachedMarginValid =
                false;  // only outside tick length can change margin
    }
}
...
void QCPAxis::setSelectedTickLabelFont(const QFont &font) {
    if (font != mSelectedTickLabelFont) {
        ...
        // don't set mCachedMarginValid to false here because margin calculation
        // is always done with non-selected fonts
    }
}
...
void QCPAxis::rescale(bool onlyVisiblePlottables) {
    ...
    for (int i = 0; i < p.size(); ++i) {
        ...
        QCPAbstractPlottable::SignDomain signDomain =
                QCPAbstractPlottable::sdBoth;
        if (mScaleType == stLogarithmic)
    ...
    if (haveRange) {
        if (!QCPRange::validRange(
                    newRange))  // likely due to range being zero (plottable has
                                // only constant data in this axis dimension),
                                // shift current range to at least center the
                                // plottable
        {
            double center = (newRange.lower + newRange.upper) *
                            0.5;  // upper and lower should be equal anyway, but
                                  // just to make sure, incase validRange
                                  // returned false for other reason
            if (mScaleType == stLinear) {
...
double QCPAxis::pixelToCoord(double value) const {
    if (orientation() == Qt::Horizontal) {
        if (mScaleType == stLinear) {
            if (!mRangeReversed)
                return (value - mAxisRect->left()) /
                               (double)mAxisRect->width() * mRange.size() +
                       mRange.lower;
            else
                return -(value - mAxisRect->left()) /
                               (double)mAxisRect->width() * mRange.size() +
        ...
        } else  // mScaleType == stLogarithmic
        {
            if (!mRangeReversed)
                return qPow(mRange.upper / mRange.lower,
                            (value - mAxisRect->left()) /
                                    (double)mAxisRect->width()) *
                       mRange.lower;
            else
                return qPow(mRange.upper / mRange.lower,
                            (mAxisRect->left() - value) /
                                    (double)mAxisRect->width()) *
                       mRange.upper;
        }
    } else  // orientation() == Qt::Vertical
    {
        if (mScaleType == stLinear) {
            if (!mRangeReversed)
                return (mAxisRect->bottom() - value) /
                               (double)mAxisRect->height() * mRange.size() +
                       mRange.lower;
            else
                return -(mAxisRect->bottom() - value) /
                               (double)mAxisRect->height() * mRange.size() +
...
double QCPAxis::coordToPixel(double value) const {
    if (orientation() == Qt::Horizontal) {
        if (mScaleType == stLinear) {
            if (!mRangeReversed)
                return (value - mRange.lower) / mRange.size() *
                               mAxisRect->width() +
                       mAxisRect->left();
            else
                return (mRange.upper - value) / mRange.size() *
                               mAxisRect->width() +
                       mAxisRect->left();
        } else  // mScaleType == stLogarithmic
        {
            if (value >= 0 &&
                ...
                return !mRangeReversed ? mAxisRect->right() + 200
                     ...
                     mRange.upper > 0)  // invalid value for logarithmic scale,
    ...
    } else  // orientation() == Qt::Vertical
    {
        if (mScaleType == stLinear) {
            if (!mRangeReversed)
                return mAxisRect->bottom() - (value - mRange.lower) /
                                                     mRange.size() *
                                                     mAxisRect->height();
            else
                return mAxisRect->bottom() - (mRange.upper - value) /
                                                     mRange.size() *
                                                     mAxisRect->height();
        } else  // mScaleType == stLogarithmic
        {
...
void QCPAxis::setupTickVectors() {
        ...
        if (mTickLabelType == ltNumber) {
            for (int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i)
                mTickVectorLabels[i] = mParentPlot->locale().toString(
                        mTickVector.at(i), mNumberFormatChar.toLatin1(),
                        mNumberPrecision);
        } else if (mTickLabelType == ltDateTime) {
            for (int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i) {
#if QT_VERSION <               \
        QT_VERSION_CHECK(4, 7, \
                         0)  // use fromMSecsSinceEpoch function if available,
                             // to gain sub-second accuracy on tick labels (e.g.
                             // for format "hh:mm:ss:zzz")
                mTickVectorLabels[i] = mParentPlot->locale().toString(
                        QDateTime::fromTime_t(mTickVector.at(i))
    ...
    {
        if (mAutoTicks)  // ticks generated automatically, but not ticklabels,
                         // so emit ticksRequest here for labels
        {
            emit ticksRequest();
        }
...
void QCPAxis::generateAutoTicks() {
    if (mScaleType == stLinear) {
        if (mAutoTickStep) {
            ...
            mTickStep = mRange.size() /
                        (double)(mAutoTickCount +
                                 1e-10);  // mAutoTickCount ticks on average,
                                          // the small addition is to prevent
                                          // jitter on exact integers
            double magnitudeFactor =
                    qPow(10.0, qFloor(qLn(mTickStep) /
                                      qLn(10.0)));  // get magnitude factor e.g.
                                                    // 0.01, 1, 10, 1000 etc.
            ...
            if (tickStepMantissa < 5) {
                ...
                mTickStep =
                        (int)(tickStepMantissa / 2.0) * 2.0 * magnitudeFactor;
            }
        }
        if (mAutoSubTicks) mSubTickCount = calculateAutoSubTickCount(mTickStep);
        // Generate tick positions according to mTickStep:
        qint64 firstStep =
                floor(mRange.lower / mTickStep);  // do not use qFloor here, or
                                                  // we'll lose 64 bit precision
        qint64 lastStep =
                ceil(mRange.upper / mTickStep);  // do not use qCeil here, or
                                                 // we'll lose 64 bit precision
    ...
    } else  // mScaleType == stLogarithmic
    {
        ...
        if (mRange.lower > 0 && mRange.upper > 0)  // positive range
        {
            ...
            while (currentMag < mRange.upper &&
                   currentMag > 0)  // currentMag might be zero for ranges
        ...
        } else  // invalid range for logarithmic scale, because lower and upper
                // have different sign
        {
            mTickVector.clear();
            qDebug() << Q_FUNC_INFO
...
int QCPAxis::calculateAutoSubTickCount(double tickStep) const {
    int result = mSubTickCount;  // default to current setting, if no proper
                                 // value can be found

    // get mantissa of tickstep:
    double magnitudeFactor = qPow(
            10.0,
            qFloor(qLn(tickStep) / qLn(10.0)));  // get magnitude factor e.g.
                                                 // 0.01, 1, 10, 1000 etc.
    ...
    if (fracPart < epsilon || 1.0 - fracPart < epsilon) {
        ...
        if (qAbs(fracPart - 0.5) < epsilon)  // *.5 mantissa
        {
            switch (intPart) {
                ...
                case 5:
                    result = 4;
                    break;  // 5.5 -> 1.1 substep (won't occur with autoTickStep
                            // from here on)
                case 6:
                    result = 4;
        ...
        // if mantissa fraction isnt 0.0 or 0.5, don't bother finding good sub
        // tick marks, leave default
    }
...
void QCPAxis::draw(QCPPainter *painter) {
    ...
    if (mTicks) {
        ...
        if (mSubTickCount > 0) {
            ...
            for (int i = 0; i < subTickCount;
                 ++i)  // no need to check bounds because subticks are always
                       // only created inside current mRange
    ...
    // needs to draw the axis. Note that some axis painter properties are
    ...
    mAxisPainter->substituteExponent = mAutoTickLabels &&
...
QFont QCPAxis::getLabelFont() const {
    return mSelectedParts.testFlag(spAxisLabel) ? mSelectedLabelFont
                                                : mLabelFont;
}
...
int QCPAxis::calculateMargin() {
    if (!mVisible)  // if not visible, directly return 0, don't cache 0 because
                    // we can't react to setVisible in QCPAxis
        return 0;

    if (mCachedMarginValid) return mCachedMargin;

    // run through similar steps as QCPAxis::draw, and caluclate margin needed
    // to fit axis and its labels
    int margin = 0;
...
void QCPAxisPainterPrivate::draw(QCPPainter *painter) {
    ...
    double xCor = 0,
           yCor = 0;  // paint system correction, for pixel exact matches
                      // (affects baselines and ticks of top/right axes)
    switch (type) {
        case QCPAxis::atTop:
            yCor = -1;
            break;
    ...
    if (!tickPositions.isEmpty()) {
        ...
        int tickDir = (type == QCPAxis::atBottom || type == QCPAxis::atRight)
                              ? -1
                              : 1;  // direction of ticks ("inward" is right for
                                    // left axis and left for right axis)
        if (QCPAxis::orientation(type) == Qt::Horizontal) {
            for (int i = 0; i < tickPositions.size(); ++i)
                painter->drawLine(
                        QLineF(tickPositions.at(i) + xCor,
    ...
    if (!subTickPositions.isEmpty()) {
        ...
        // direction of ticks ("inward" is right for left axis and left for
        // right axis)
        int tickDir = (type == QCPAxis::atBottom || type == QCPAxis::atRight)
                              ? -1
                              : 1;
        if (QCPAxis::orientation(type) == Qt::Horizontal) {
            ...
            for (int i = 0; i < subTickPositions.size(); ++i)
                painter->drawLine(
                        QLineF(origin.x() - subTickLengthOut * tickDir + xCor,
    ...
    // tick labels:
    QRect oldClipRect;
    if (tickLabelSide == QCPAxis::lsInside)  // if using inside labels, clip
                                             // them to the axis rect
    {
        oldClipRect = painter->clipRegion().boundingRect();
        painter->setClipRect(axisRect);
    }
    QSize tickLabelsSize(0, 0);  // size of largest tick label, for offset
                                 // calculation of axis label
    ...
    if (!label.isEmpty()) {
        ...
        labelBounds = painter->fontMetrics().boundingRect(
                0, 0, 0, 0, Qt::TextDontClip, label);
    ...
    if (type == QCPAxis::atLeft) {
        ...
        mTickLabelsSelectionBox.setCoords(
                origin.x() - selTickLabelOffset - selTickLabelSize,
                axisRect.top(), origin.x() - selTickLabelOffset,
                axisRect.bottom());
        mLabelSelectionBox.setCoords(
                origin.x() - selLabelOffset - selLabelSize, axisRect.top(),
        ...
        mAxisSelectionBox.setCoords(
                axisRect.left(), origin.y() - selAxisOutSize, axisRect.right(),
        ...
        mLabelSelectionBox.setCoords(
                axisRect.left(), origin.y() - selLabelOffset - selLabelSize,
...
int QCPAxisPainterPrivate::size() const {
    ...
    if (tickLabelSide == QCPAxis::lsOutside) {
        ...
        if (!tickLabels.isEmpty()) {
            for (int i = 0; i < tickLabels.size(); ++i)
                getMaxTickLabelSize(tickLabelFont, tickLabels.at(i),
                                    &tickLabelsSize);
    ...
    // calculate size of axis label (only height needed, because left/right
    // labels are rotated by 90 degrees):
    if (!label.isEmpty()) {
...
void QCPAxisPainterPrivate::placeTickLabel(QCPPainter *painter,
    ...
    switch (type) {
        case QCPAxis::atLeft:
            labelAnchor = QPointF(axisRect.left() - distanceToAxis - offset,
                                  position);
            break;
        case QCPAxis::atRight:
            labelAnchor = QPointF(axisRect.right() + distanceToAxis + offset,
                                  position);
            break;
        case QCPAxis::atTop:
            labelAnchor =
                    QPointF(position, axisRect.top() - distanceToAxis - offset);
            break;
        case QCPAxis::atBottom:
            labelAnchor = QPointF(position,
                                  axisRect.bottom() + distanceToAxis + offset);
            break;
    }
                ...
                QCPPainter::pmNoCaching))  // label caching enabled
    {
        ...
        if (!cachedLabel)                // no cached label existed, create it
        {
            ...
            drawTickLabel(
                    &cachePainter, -labelData.rotatedTotalBounds.topLeft().x(),
                    -labelData.rotatedTotalBounds.topLeft().y(), labelData);
        }
        // if label would be partly clipped by widget border on sides, don't
        // draw it (only for outside tick labels):
        ...
        mLabelCache.insert(text,
                           cachedLabel);  // return label to cache or insert for
                                          // the first time if newly created
    } else  // label caching disabled, draw text directly on surface:
    {
        ...
        if (!labelClippedByBorder) {
            drawTickLabel(painter, finalPosition.x(), finalPosition.y(),
                          labelData);
...
void QCPAxisPainterPrivate::drawTickLabel(
                 ...
                 .isEmpty())  // indicator that beautiful powers must be used
    {
        ...
        painter->drawText(labelData.baseBounds.width() + 1, 0,
                          ...
                          labelData.expBounds.height(), Qt::TextDontClip,
                          labelData.expPart);
    } else {
        painter->setFont(labelData.baseFont);
...
QCPAxisPainterPrivate::TickLabelData QCPAxisPainterPrivate::getTickLabelData(
    ...
    if (substituteExponent) {
        ...
        if (ePos > -1) useBeautifulPowers = true;
    }
    ...
    if (result.baseFont.pointSizeF() >
        ...
        result.baseFont.setPointSizeF(
                ...
                0.05);  // QFontMetrics.boundingRect has a bug for exact point
                        // sizes that make the results oscillate due to internal
                        // rounding
    if (useBeautifulPowers) {
        // split text into parts of number/symbol that will be drawn normally
        // and part that will be drawn as exponent:
        ...
        if (abbreviateDecimalPowers && result.basePart == QLatin1String("1"))
            ...
            result.basePart += (numberMultiplyCross ? QString(QChar(215))
                                                    : QString(QChar(183))) +
                               QLatin1String("10");
        ...
        while (result.expPart.length() > 2 &&
               result.expPart.at(1) ==
                       QLatin1Char('0'))  // length > 2 so we leave one zero
                                          // when numberFormatChar is 'e'
            result.expPart.remove(1, 1);
        if (!result.expPart.isEmpty() &&
            result.expPart.at(0) == QLatin1Char('+'))
        ...
        result.baseBounds = QFontMetrics(result.baseFont)
                                    .boundingRect(0, 0, 0, 0, Qt::TextDontClip,
                                                  result.basePart);
        ...
        result.totalBounds = result.baseBounds.adjusted(
                ...
                0);  // +2 consists of the 1 pixel spacing between base and
                     // exponent (see drawTickLabel) and an extra pixel to
                     // include AA
    ...
    result.totalBounds.moveTopLeft(QPoint(
            0,
            0));  // want bounding box aligned top left at origin, independent
                  // of how it was created, to make further processing simpler
    ...
    if (!qFuzzyIsNull(tickLabelRotation)) {
        ...
        result.rotatedTotalBounds =
                transform.mapRect(result.rotatedTotalBounds);
    }
                 ...
                 QCPAxis::lsInside))  // Anchor at right side of tick label
    {
        if (doRotation) {
            ...
            y = -labelData.totalBounds.height() / 2.0;
                ...
                tickLabelSide == QCPAxis::lsOutside) ||
               (type == QCPAxis::atLeft &&
                tickLabelSide == QCPAxis::lsInside))  // Anchor at left side of
                                                      // tick label
    {
        if (doRotation) {
            if (tickLabelRotation > 0) {
            ...
            y = -labelData.totalBounds.height() / 2.0;
               ...
               (type == QCPAxis::atBottom &&
                tickLabelSide == QCPAxis::lsInside))  // Anchor at bottom side
                                                      // of tick label
    {
        if (doRotation) {
            if (tickLabelRotation > 0) {
...
QCPItemAnchor::~QCPItemAnchor() {
    ...
    foreach (QCPItemPosition *child, mChildrenX.values()) {
        if (child->parentAnchorX() == this)
            child->setParentAnchorX(0);  // this acts back on this anchor and
                                         // child removes itself from mChildrenX
    }
    foreach (QCPItemPosition *child, mChildrenY.values()) {
        if (child->parentAnchorY() == this)
            child->setParentAnchorY(0);  // this acts back on this anchor and
                                         // child removes itself from mChildrenY
    }
}
...
void QCPItemPosition::setTypeX(QCPItemPosition::PositionType type) {
    if (mPositionTypeX != type) {
        // if switching from or to coordinate type that isn't valid (e.g.
        // because axes or axis rect were deleted), don't try to recover the
        // pixelPoint() because it would output a qDebug warning.
...
bool QCPItemPosition::setParentAnchorX(QCPItemAnchor *parentAnchor,
    ...
    while (currentParent) {
        if (QCPItemPosition *currentParentPos =
            ...
            // is a QCPItemPosition, might have further parent, so keep
            // iterating
            ...
            // is a QCPItemAnchor, can't have further parent. Now make sure the
            // parent items aren't the same, to prevent a position being child
            // of an anchor which itself depends on the position, because
            // they're both on the same item:
        ...
        case ptPlotCoords: {
            if (mKeyAxis && mKeyAxis.data()->orientation() == Qt::Vertical)
                result.ry() = mKeyAxis.data()->coordToPixel(mKey);
                     ...
                     mValueAxis.data()->orientation() == Qt::Vertical)
...
QCustomPlot::QCustomPlot(QWidget *parent)
    ...
    mPlotLayout->setParent(this);  // important because if parent is QWidget,
                                   // QCPLayout::sizeConstraintsChanged will
    ...
    setViewport(
            rect());  // needs to be called after mPlotLayout has been created
...
bool QCustomPlot::addPlottable(QCPAbstractPlottable *plottable) {
    ...
    if (!plottable->layer())  // usually the layer is already set in the
                              // constructor of the plottable (via QCPLayerable
                              // constructor)
...
QCPAbstractPlottable *QCustomPlot::plottableAt(const QPointF &pos,
    ...
    double resultDistance =
            mSelectionTolerance;  // only regard clicks with distances smaller
                                  // than mSelectionTolerance as selections, so
                                  // initialize with that value

    foreach (QCPAbstractPlottable *plottable, mPlottables) {
        if (onlySelectable &&
            !plottable->selectable())  // we could have also passed
                                       // onlySelectable to the selectTest
                                       // function, but checking here is faster,
        ...
        if ((plottable->keyAxis()->axisRect()->rect() &
             plottable->valueAxis()->axisRect()->rect())
                    .contains(pos.toPoint()))  // only consider clicks inside
                                               // the rect that is spanned by
                                               // the plottable's key/value axes
        {
...
QCPAbstractItem *QCustomPlot::itemAt(const QPointF &pos,
    ...
    foreach (QCPAbstractItem *item, mItems) {
        if (onlySelectable &&
            !item->selectable())  // we could have also passed onlySelectable to
        ...
        if (!item->clipToAxisRect() ||
            item->clipRect().contains(
                    pos.toPoint()))  // only consider clicks inside axis
                                     // cliprect of the item if actually clipped
                                     // to it
        {
...
bool QCustomPlot::addLayer(const QString &name,
    ...
    if (layer(name)) {
        qDebug() << Q_FUNC_INFO << "A layer exists already with the name"
                 << name;
        return false;
    }
...
bool QCustomPlot::removeLayer(QCPLayer *layer) {
    ...
    // append all children of this layer to layer below (if this is lowest
    // layer, prepend to layer above)
...
QList<QCPAxisRect *> QCustomPlot::axisRects() const {
    ...
    while (!elementStack.isEmpty()) {
        foreach (QCPLayoutElement *element,
...
void QCustomPlot::showPointToolTip(QMouseEvent *event) {
    ...
    int evtX = event->pos().x();
    int evtY = event->pos().y();

    int x = this->xAxis->pixelToCoord(evtX);
    int y = this->yAxis->pixelToCoord(evtY);
    ...
    if (selectedGraphs().count() > 0) {
        ...
        double dbottom =
                graph->valueAxis()->range().lower;  // Yaxis bottom value
        double dtop = graph->valueAxis()->range().upper;

        // Yaxis top value
        long ptop = graph->valueAxis()->axisRect()->top();  // graph top margin
        long pbottom = graph->valueAxis()
                               ->axisRect()
                               ->bottom();  // graph bottom position
        // result for Y axis
        double valueY = (evtY - ptop) / (double)(pbottom - ptop) *
                                (double)(dbottom - dtop) +
                        dtop;

        // or shortly for X-axis
        double valueX = (evtX - graph->keyAxis()
                                        ->axisRect()
                                        ->left());  // graph width in pixels
        double ratio = (double)(graph->keyAxis()->axisRect()->right() -
                                graph->keyAxis()->axisRect()->left()) /
                       (double)(graph->keyAxis()->range().lower -
                                graph->keyAxis()
                                        ->range()
                                        .upper);  // ratio px->graph width

        // and result for X-axis
        valueX = -valueX / ratio + graph->keyAxis()->range().lower;

        QPoint gPos = mapToGlobal(QPoint(evtX, evtY));
        QToolTip::showText(gPos, QString("%1 , %2").arg(valueX).arg(valueY));
    }
}
/*!
...
void QCustomPlot::mouseDoubleClickEvent(QMouseEvent *event) {
    ...
    if (QCPAbstractPlottable *ap =
        ...
        emit axisDoubleClick(ax, details.value<QCPAxis::SelectablePart>(),
                             event);
    ...
    // QWidget::mouseDoubleClickEvent(event); don't call base class
    // implementation because it would just cause a mousePress/ReleaseEvent,
    // which we don't want.
}
...
void QCustomPlot::mousePressEvent(QMouseEvent *event) {
    ...
    mMousePressPos = event->pos();  // need this to determine in releaseEvent
                                    // whether it was a click (no position
        ...
        5)  // determine whether it was a click operation
    {
        if (event->button() == Qt::LeftButton) {
                ...
                foreach (QCPLayer *layer, mLayers) {
                    foreach (QCPLayerable *layerable, layer->children()) {
                        if (layerable != clickedLayerable &&
                            mInteractions.testFlag(
                                    layerable->selectionCategory())) {
            ...
            if (clickedLayerable &&
                ...
                bool selChanged = false;
                clickedLayerable->selectEvent(event, additive, details,
                                              &selChanged);
                selectionStateChanged |= selChanged;
            }
            if (selectionStateChanged) {
        ...
        QCPLayerable *clickedLayerable = layerableAt(
                event->pos(), false,
                &details);  // for these signals, selectability is ignored,
                            // that's why we call this again with onlySelectable
                            // set to false
        if (QCPAbstractPlottable *ap =
                    qobject_cast<QCPAbstractPlottable *>(clickedLayerable))
            ...
            emit legendClick(lg, 0, event);
                         ...
                         qobject_cast<QCPAbstractLegendItem *>(
                                 clickedLayerable))
            emit legendClick(li->parentLegend(), li, event);
        else if (QCPPlotTitle *pt =
                         qobject_cast<QCPPlotTitle *>(clickedLayerable))
        ...
        if (mBackgroundScaled) {
            ...
            if (mScaledBackgroundPixmap.size() != scaledSize)
                mScaledBackgroundPixmap = mBackgroundPixmap.scaled(
                        mViewport.size(), mBackgroundScaledMode,
                        Qt::SmoothTransformation);
            painter->drawPixmap(
                    mViewport.topLeft(), mScaledBackgroundPixmap,
                    QRect(0, 0, mViewport.width(), mViewport.height()) &
                            mScaledBackgroundPixmap.rect());
        } else {
            painter->drawPixmap(
                    mViewport.topLeft(), mBackgroundPixmap,
                    QRect(0, 0, mViewport.width(), mViewport.height()));
        }
    }
}
...
void QCustomPlot::axisRemoved(QCPAxis *axis) {
    ...
    // Note: No need to take care of range drag axes and range zoom axes,
    // because they are stored in smart pointers
}
...
QCPLayerable *QCustomPlot::layerableAt(const QPointF &pos,
    ...
    for (int layerIndex = mLayers.size() - 1; layerIndex >= 0; --layerIndex) {
        const QList<QCPLayerable *> layerables =
        ...
        for (int i = layerables.size() - 1; i >= 0; --i) {
            ...
            QVariant details;
            double dist =
                    layerables.at(i)->selectTest(pos, onlySelectable, &details);
...
QPixmap QCustomPlot::toPixmap(int width, int height, double scale) {
    // this method is somewhat similar to toPainter. Change something here, and
    // a change in toPainter might be necessary, too.
    int newWidth, newHeight;
    if (width == 0 || height == 0) {
        newWidth = this->width();
        newHeight = this->height();
    ...
    result.fill(mBackgroundBrush.style() == Qt::SolidPattern
                        ? mBackgroundBrush.color()
                        : Qt::transparent);  // if using non-solid pattern, make
                                             // transparent now and draw brush
                                             // pattern later
    QCPPainter painter;
    ...
    if (painter.isActive()) {
        ...
        if (!qFuzzyCompare(scale, 1.0)) {
            if (scale > 1.0)  // for scale < 1 we always want cosmetic pens
                              // where possible, because else lines might
                              // disappear for very small scales
                painter.setMode(QCPPainter::pmNonCosmetic);
            painter.scale(scale, scale);
        }
        if (mBackgroundBrush.style() != Qt::SolidPattern &&
            mBackgroundBrush.style() !=
                    Qt::NoBrush)  // solid fills were done a few lines above
                                  // with QPixmap::fill
            painter.fillRect(mViewport, mBackgroundBrush);
        draw(&painter);
        setViewport(oldViewport);
...
void QCPColorGradient::colorize(const double *data,
    ...
    if (!logarithmic) {
        ...
        if (mPeriodic) {
            ...
            for (int i = 0; i < n; ++i) {
                int index = (data[dataIndexFactor * i] - range.lower) *
                            posToIndexFactor;
                if (index < 0)
                    index = 0;
...
void QCPColorGradient::updateColorBuffer() {
    ...
    if (mColorStops.size() > 1) {
        ...
        for (int i = 0; i < mLevelCount; ++i) {
            ...
            if (it == mColorStops.constEnd())  // position is on or after last
                                               // stop, use color of last stop
            {
                mColorBuffer[i] = (it - 1).value().rgb();
            } else if (it ==
                       mColorStops
                               .constBegin())  // position is on or before first
                                               // stop, use color of first stop
            {
                mColorBuffer[i] = it.value().rgb();
            } else  // position is in between stops (or on an intermediate
                    // stop), interpolate color
            {
                ...
                switch (mColorInterpolation) {
                    case ciRGB: {
                        mColorBuffer[i] = qRgb((1 - t) * low.value().red() +
                                                       t * high.value().red(),
                                               (1 - t) * low.value().green() +
                                                       t * high.value().green(),
                                               (1 - t) * low.value().blue() +
    ...
    while (it.hasNext()) {
        ...
        if (it.value().contains(axis)) {
            ...
            if (qobject_cast<QCustomPlot *>(
                        parentPlot()))  // make sure this isn't called from
                                        // QObject dtor when QCustomPlot is
                                        // already destructed (happens when the
                                        // axis rect is not in any layout and
...
QList<QCPAbstractPlottable *> QCPAxisRect::plottables() const {
    // Note: don't append all QCPAxis::plottables() into a list, because we
...
QList<QCPGraph *> QCPAxisRect::graphs() const {
    // Note: don't append all QCPAxis::graphs() into a list, because we might
...
QList<QCPAbstractItem *> QCPAxisRect::items() const {
    // Note: don't just append all QCPAxis::items() into a list, because we
    // might get duplicate entries
...
void QCPAxisRect::update(UpdatePhase phase) {
    ...
    // pass update call on to inset layout (doesn't happen automatically,
    // because QCPAxisRect doesn't derive from QCPLayout):
...
void QCPAxisRect::drawBackground(QCPPainter *painter) {
    ...
    if (!mBackgroundPixmap.isNull()) {
        if (mBackgroundScaled) {
            ...
            if (mScaledBackgroundPixmap.size() != scaledSize)
                mScaledBackgroundPixmap = mBackgroundPixmap.scaled(
                        mRect.size(), mBackgroundScaledMode,
                        Qt::SmoothTransformation);
...
void QCPAxisRect::updateAxesOffset(QCPAxis::AxisType type) {
    ...
    bool isFirstVisible =
            !axesList.first()
                     ->visible();  // if the first axis is visible, the second
                                   // axis (which is where the loop starts)
                                   // isn't the first visible axis, so
                                   // initialize with false
    for (int i = 1; i < axesList.size(); ++i) {
        int offset = axesList.at(i - 1)->offset() +
                     axesList.at(i - 1)->calculateMargin();
        if (axesList.at(i)->visible())  // only add inner tick length to offset
                                        // if this axis is visible and it's not
                                        // the first visible one (might happen
...
void QCPAxisRect::mousePressEvent(QMouseEvent *event) {
    mDragStart = event->pos();  // need this even when not LeftButton is
                                // pressed, to determine in releaseEvent whether
                                // it was a full click (no position change
...
void QCPAxisRect::mouseMoveEvent(QMouseEvent *event) {
    ...
    if (mDragging && mParentPlot->interactions().testFlag(QCP::iRangeDrag)) {
        if (mRangeDrag.testFlag(Qt::Horizontal)) {
            if (QCPAxis *rangeDragHorzAxis = mRangeDragHorzAxis.data()) {
                if (rangeDragHorzAxis->mScaleType == QCPAxis::stLinear) {
                    double diff =
                            rangeDragHorzAxis->pixelToCoord(mDragStart.x()) -
        ...
        if (mRangeDrag != 0)  // if either vertical or horizontal drag was
                              // enabled, do a replot
        {
            if (mParentPlot->noAntialiasingOnDrag())
...
void QCPPlottableLegendItem::draw(QCPPainter *painter) {
    ...
    int textHeight =
            qMax(textRect.height(),
                 iconSize.height());  // if text has smaller height than icon,
                                      // center text vertically in icon height,
                                      // else align tops
    painter->drawText(
...
QCPLegend::~QCPLegend() {
    clearItems();
    if (qobject_cast<QCustomPlot *>(
                mParentPlot))  // make sure this isn't called from QObject dtor
                               // when QCustomPlot is already destructed
                               // (happens when the legend is not in any layout
                               // and thus QObject-child of QCustomPlot)
...
void QCPLegend::setSelectedParts(const SelectableParts &selected) {
    ...
    mSelectedParts = this->selectedParts();  // update mSelectedParts in case
    ...
    if (mSelectedParts != newSelected) {
        ...
        if (mSelectedParts.testFlag(spItems) &&
            !newSelected.testFlag(spItems))  // spItems flag was unset, so clear
                                             // item selection
        {
            for (int i = 0; i < itemCount(); ++i) {
...
QPen QCPLegend::getBorderPen() const {
    return mSelectedParts.testFlag(spLegendBox) ? mSelectedBorderPen
                                                : mBorderPen;
}
    ...
    if (details.value<SelectablePart>() == spLegendBox &&
        ...
        setSelectedParts(
                additive ? mSelectedParts ^ spLegendBox
                         : mSelectedParts |
                                   spLegendBox);  // no need to unset spItems in
                                                  // !additive case, because
                                                  // they will be deselected by
                                                  // QCustomPlot (they're normal
                                                  // QCPLayerables with own
                                                  // deselectEvent)
        if (selectionStateChanged)
            *selectionStateChanged = mSelectedParts != selBefore;
    }
...
QCPPlotTitle::QCPPlotTitle(QCustomPlot *parentPlot, const QString &text)
    : QCPLayoutElement(parentPlot),
      mText(text),
      mFont(QFont(parentPlot->font().family(),
                  parentPlot->font().pointSize() * 1.5,
                  QFont::Bold)),
      mTextColor(Qt::black),
      mSelectedFont(QFont(parentPlot->font().family(),
                          parentPlot->font().pointSize() * 1.6,
                          QFont::Bold)),
      mSelectedTextColor(Qt::blue),
      mSelectable(false),
      mSelected(false) {
...
QCPColorScale::QCPColorScale(QCustomPlot *parentPlot)
    ...
    setMinimumMargins(QMargins(
            0, 6, 0,
            6));  // for default right color scale types, keep some room at
...
bool QCPColorScale::rangeDrag() const {
    ...
    return mAxisRect.data()->rangeDrag().testFlag(
...
bool QCPColorScale::rangeZoom() const {
    ...
    return mAxisRect.data()->rangeZoom().testFlag(
                   QCPAxis::orientation(mType)) &&
...
void QCPColorScale::setType(QCPAxis::AxisType type) {
    ...
    if (mType != type) {
        ...
        QList<QCPAxis::AxisType> allAxisTypes =
                QList<QCPAxis::AxisType>()
                << QCPAxis::atLeft << QCPAxis::atRight << QCPAxis::atBottom
                << QCPAxis::atTop;
        ...
        mColorAxis.data()->setRange(
                rangeTransfer);  // transfer range of old axis to new one
                                 // (necessary if axis changes from vertical to
                                 // horizontal or vice versa)
        mColorAxis.data()->setLabel(labelTransfer);
        mColorAxis.data()->setScaleLogBase(
                logBaseTransfer);  // scaleType is synchronized among axes in
                                   // realtime via signals (connected in
                                   // QCPColorScale ctor), so we only need to
                                   // take care of log base here
        ...
        mAxisRect.data()->setRangeDragAxes(
                QCPAxis::orientation(mType) == Qt::Horizontal
                        ? mColorAxis.data()
                        : 0,
                QCPAxis::orientation(mType) == Qt::Vertical ? mColorAxis.data()
                                                            : 0);
    }
}
...
void QCPColorScale::rescaleDataRange(bool onlyVisibleMaps) {
    ...
    bool haveRange = false;
    int sign =
            0;  // TODO: should change this to QCPAbstractPlottable::SignDomain
    ...
    if (haveRange) {
        if (!QCPRange::validRange(
                    newRange))  // likely due to range being zero (plottable has
                                // only constant data in this dimension), shift
                                // current range to at least center the data
        {
            double center = (newRange.lower + newRange.upper) *
                            0.5;  // upper and lower should be equal anyway, but
                                  // just to make sure, incase validRange
            ...
            {
                newRange.lower =
                        center / qSqrt(mDataRange.upper / mDataRange.lower);
                newRange.upper =
                        center * qSqrt(mDataRange.upper / mDataRange.lower);
            }
        }
...
void QCPColorScaleAxisRectPrivate::updateGradientImage() {
    ...
    if (mParentColorScale->mType == QCPAxis::atBottom ||
        ...
        mParentColorScale->mGradient.colorize(
                data.constData(), QCPRange(0, n - 1), pixels.first(), n);
        for (int y = 1; y < h; ++y)
...
/*! \class QCPGraph
  ...
  Usually QCustomPlot creates graphs internally via QCustomPlot::addGraph and
  the resulting instance is accessed via QCustomPlot::graph.
...
void QCPGraph::rescaleValueAxis(bool onlyEnlarge, bool includeErrorBars) const {
    // this code is a copy of QCPAbstractPlottable::rescaleValueAxis with the
    // only change is that getValueRange is passed the includeErrorBars value.
    if (mData->isEmpty()) return;
...
void QCPGraph::draw(QCPPainter *painter) {
    ...
    for (it = mData->constBegin(); it != mData->constEnd(); ++it) {
        if (QCP::isInvalidData(it.value().key, it.value().value) ||
            QCP::isInvalidData(it.value().keyErrorPlus,
                               it.value().keyErrorMinus) ||
    ...
    if (mLineStyle != lsNone) {
        ...
        painter->drawLine(QLineF(
                ...
                rect.top() + rect.height() /
                                     2.0));  // +5 on x2 else last segment is
    ...
    if (!mScatterStyle.isNone()) {
        ...
        if (mScatterStyle.shape() == QCPScatterStyle::ssPixmap &&
            ...
            scaledStyle.setPixmap(scaledStyle.pixmap().scaled(
                    rect.size().toSize(), Qt::KeepAspectRatio,
                    Qt::SmoothTransformation));
...
void QCPGraph::getLinePlotData(QVector<QPointF> *linePixelData,
    ...
    linePixelData->reserve(
            lineData.size() +
            2);  // added 2 to reserve memory for lower/upper fill base points
...
void QCPGraph::getImpulsePlotData(QVector<QPointF> *linePixelData,
    ...
    getPreparedData(&lineData, scatterData);
    linePixelData->resize(lineData.size() *
                          2);  // no need to reserve 2 extra points because
                               // impulse plot has no fill
        ...
        if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
            ...
            while (i < lineDataSize &&
                   (qIsNaN(lineData->at(i).y()) ||
                    qIsNaN(lineData->at(i)
                                   .x())))  // make sure first point is not NaN
                ++i;
            ++i;  // because drawing works in 1 point retrospect
            while (i < lineDataSize) {
                if (!qIsNaN(lineData->at(i).y()) &&
                    !qIsNaN(lineData->at(i)
                                    .x()))  // NaNs create a gap in the line
                {
                    if (!lastIsNan)
            ...
            while (i < lineDataSize) {
                                   ...
                                   .x()))  // NaNs create a gap in the line
                {
                    painter->drawPolyline(
                            ...
                            i - segmentStart);  // i, because we don't want to
                                                // include the current NaN point
                    segmentStart = i + 1;
                }
                ++i;
            }
                                ...
                                // least two points per pixel on average
    {
        if (lineData) {
            ...
            int reversedFactor =
                    keyAxis->rangeReversed() !=
                                    (keyAxis->orientation() == Qt::Vertical)
                            ? -1
                            : 1;  // is used to calculate keyEpsilon pixel into
                                  // the correct direction
            int reversedRound =
                    keyAxis->rangeReversed() !=
                                    (keyAxis->orientation() == Qt::Vertical)
                            ? 1
                            : 0;  // is used to switch between floor (normal)
                                  // and ceil (reversed) rounding of
            ...
            double keyEpsilon = qAbs(
                    ...
                    keyAxis->pixelToCoord(
                            ...
                            1.0 * reversedFactor));  // interval of one pixel on
                                                     // screen when mapped to
                                                     // plot key coordinates
            bool keyEpsilonVariable =
                    keyAxis->scaleType() ==
                    QCPAxis::stLogarithmic;  // indicates whether keyEpsilon
                                             // needs to be updated after every
                                             // interval (for log axes)
            int intervalDataCount = 1;
            ++it;  // advance iterator to second data point because adaptive
            ...
            while (it != upperEnd) {
                ...
                } else  // new pixel interval started
                {
                    if (intervalDataCount >=
                        2)  // last pixel had multiple data points, consolidate
                            // them to a cluster
                    {
                        if (lastIntervalEndKey <
                            currentIntervalStartKey -
                                    keyEpsilon)  // last point is further away,
                                                 // so first point of this
                                                 // cluster must be at a real
                                                 // data point
                            lineData->append(QCPData(
                                    currentIntervalStartKey + keyEpsilon * 0.2,
                                    currentIntervalFirstPoint.value().value));
                        lineData->append(QCPData(
                                currentIntervalStartKey + keyEpsilon * 0.25,
                                minValue));
                        lineData->append(QCPData(
                                currentIntervalStartKey + keyEpsilon * 0.75,
                                maxValue));
                        if (it.key() >
                            currentIntervalStartKey +
                                    keyEpsilon *
                                            2)  // new pixel started further
                                                // away from previous cluster,
                                                // so make sure the last point
                                                // of the cluster is at a real
                                                // data point
                            lineData->append(QCPData(
                                    currentIntervalStartKey + keyEpsilon * 0.8,
                                    (it - 1).value().value));
                    } else
                        lineData->append(QCPData(
                                currentIntervalFirstPoint.key(),
                                currentIntervalFirstPoint.value().value));
                    ...
                    currentIntervalStartKey = keyAxis->pixelToCoord(
                            (int)(keyAxis->coordToPixel(it.key()) +
                                  reversedRound));
                    if (keyEpsilonVariable)
                        keyEpsilon =
                                qAbs(currentIntervalStartKey -
                                     keyAxis->pixelToCoord(
                                             keyAxis->coordToPixel(
                                                     currentIntervalStartKey) +
                                             1.0 * reversedFactor));
                    intervalDataCount = 1;
                }
                                         ...
                                         // consolidate them to a cluster
            {
                if (lastIntervalEndKey <
                    currentIntervalStartKey -
                            keyEpsilon)  // last point wasn't a cluster, so
                                         // first point of this cluster must be
                                         // at a real data point
                    lineData->append(
                ...
                lineData->append(
                        QCPData(currentIntervalFirstPoint.key(),
                                currentIntervalFirstPoint.value().value));
        }
        ...
        if (scatterData) {
            ...
            while (it != upperEnd) {
                ...
                {
                    if (intervalDataCount >= 2)  // last pixel had multiple data
                                                 // points, consolidate them
                    {
                        // determine value pixel span and add as many points in
                        // interval to maintain certain vertical data density
                        // (this is specific to scatter plot):
                        double valuePixelSpan =
                                qAbs(valueAxis->coordToPixel(minValue) -
                                     valueAxis->coordToPixel(maxValue));
                        int dataModulo = qMax(
                                1, qRound(intervalDataCount /
                                          (valuePixelSpan /
                                           4.0)));  // approximately every 4
                                                    // value pixels one data
                                                    // point on average
                        QCPDataMap::const_iterator intervalIt =
                                currentIntervalStart;
                        int c = 0;
                        while (intervalIt != it) {
                            if ((c % dataModulo == 0 ||
                                 intervalIt == minValueIt ||
                            ...
                            ++intervalIt;
                                       ...
                                       valueMinRange &&
                               currentIntervalStart.value().value <
                                       valueMaxRange)
            ...
            {
                // determine value pixel span and add as many points in interval
                // to maintain certain vertical data density (this is specific
                // to scatter plot):
                ...
                int dataModulo = qMax(
                        1, qRound(intervalDataCount /
                                  (valuePixelSpan /
                                   4.0)));  // approximately every 4 value
            ...
            // from the map into the output parameters
    {
        ...
        if (dataVector) {
            ...
            dataVector->reserve(dataCount +
                                2);  // +2 for possible fill end points
            while (it != upperEnd) {
    ...
    if (keyAxis->orientation() == Qt::Vertical) {
        ...
        if (mErrorType == etKey || mErrorType == etBoth) {
            ...
            if (mErrorBarSkipSymbol) {
                if (a - y >
                    skipSymbolMargin)  // don't draw spine if error is so small
...
void QCPGraph::getVisibleDataBounds(QCPDataMap::const_iterator &lower,
    ...
    bool lowoutlier =
            lbound != mData->constBegin();  // indicates whether there exist
                                            // points below axis range
    bool highoutlier =
            ubound != mData->constEnd();  // indicates whether there exist
                                          // points above axis range

    lower = (lowoutlier
                     ? lbound - 1
                     : lbound);  // data point range that will be actually drawn
    upper = (highoutlier ? ubound : ubound - 1);  // data point range that will
                                                  // be actually drawn
}
...
QPointF QCPGraph::lowerFillBasePoint(double lowerKey) const {
    ...
    {
        // In logarithmic scaling we can't just draw to value zero so we just
        // fill all the way to the axis which is in the direction towards zero
        if (keyAxis->orientation() == Qt::Vertical) {
...
const QPolygonF QCPGraph::getChannelFillPolygon(
    ...
    if (mChannelFillGraph.data()->mKeyAxis.data()->orientation() !=
        keyAxis->orientation())
        return QPolygonF();  // don't have same axis orientation, can't fill
                             // that (Note: if keyAxis fits, valueAxis will fit
                             // too, because it's always orthogonal to keyAxis)
    ...
    // crop both vectors to ranges in which the keys overlap (which coord is
    // key, depends on axisType):
    if (keyAxis->orientation() == Qt::Horizontal) {
        // x is key
        ...
        croppedData->remove(highBound + 1,
                            croppedData->size() - (highBound + 1));
    ...
    {
        // y is key
        // similar to "x is key" but switched x,y. Further, lower/upper meaning
        // is inverted compared to x, because in pixel coordinates, y increases
        // from top to bottom, not bottom to top like data coordinate. if an
        // axis range is reversed, the data point keys will be descending.
        // Reverse them, since following algorithm assumes ascending keys:
...
double QCPGraph::pointDistance(const QPointF &pixelPoint) const {
    ...
    if (mLineStyle == lsNone) {
        ...
        if (scatterData.size() > 0) {
            ...
            for (int i = 0; i < scatterData.size(); ++i) {
                double currentDistSqr =
                        QVector2D(coordsToPixels(scatterData.at(i).key,
                                                 scatterData.at(i).value) -
                                  pixelPoint)
                                .lengthSquared();
        ...
        getPlotData(&lineData, 0);  // unlike with getScatterPlotData we get
            ...
            1)  // at least one line segment, compare distance to line segments
        {
            double minDistSqr = std::numeric_limits<double>::max();
            if (mLineStyle == lsImpulse) {
                // impulse plot differs from other line styles in that the
                // lineData points are only pairwise connected:
                for (int i = 0; i < lineData.size() - 1;
                     i += 2)  // iterate pairs
                {
                    double currentDistSqr = distSqrToLine(
                            lineData.at(i), lineData.at(i + 1), pixelPoint);
            ...
            return qSqrt(minDistSqr);
        } else if (lineData.size() > 0)  // only single data point, calculate
    ...
    if (inSignDomain == sdBoth)  // range may be anywhere
    {
        ...
        while (it != mData->constEnd()) {
            if (!qIsNaN(it.value().value)) {
                ...
                currentErrorMinus =
                        (includeErrors ? it.value().keyErrorMinus : 0);
                currentErrorPlus =
                        (includeErrors ? it.value().keyErrorPlus : 0);
        ...
        while (it != mData->constEnd()) {
            if (!qIsNaN(it.value().value)) {
                ...
                if (includeErrors)  // in case point is in valid sign domain but
                                    // errobars stretch beyond it, we still want
                                    // to geht that point.
                {
...
QCPRange QCPGraph::getValueRange(bool &foundRange,
    ...
    if (inSignDomain == sdBoth)  // range may be anywhere
    {
        ...
        while (it != mData->constEnd()) {
            ...
            if (!qIsNaN(current)) {
                currentErrorMinus =
                        (includeErrors ? it.value().valueErrorMinus : 0);
                currentErrorPlus =
                        (includeErrors ? it.value().valueErrorPlus : 0);
                if (current - currentErrorMinus < range.lower || !haveLower) {
                    range.lower = current - currentErrorMinus;
                    haveLower = true;
                }
...
QCPCurve::QCPCurve(QCPAxis *keyAxis, QCPAxis *valueAxis)
    ...
    mSelectedPen.setColor(
            QColor(80, 80, 255));  // lighter than Qt::blue of mPen
...
void QCPCurve::setData(const QVector<double> &key,
    ...
    QCPCurveData newData;
    for (int i = 0; i < n; ++i) {
        newData.t = i;  // no t vector given, so we assign t the index of the
                        // key/value pair
        newData.key = key[i];
        newData.value = value[i];
        qtCompatMapInsertMulti(mData, newData.t, newData);
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const {
    ...
    double rectLeft = keyAxis->pixelToCoord(
            ...
            strokeMargin * ((keyAxis->orientation() == Qt::Vertical) !=
                                            keyAxis->rangeReversed()
                                    ? -1
                                    : 1));
    ...
    int prevRegion = getRegion(prevIt.value().key, prevIt.value().value,
                               rectLeft, rectTop, rectRight, rectBottom);
    QVector<QPointF>
            trailingPoints;  // points that must be applied after all other
                             // points (are generated only when handling first
                             // point to get virtual segment between last and
                             // first point right)
    while (it != mData->constEnd()) {
        currentRegion = getRegion(it.value().key, it.value().value, rectLeft,
                                  rectTop, rectRight, rectBottom);
        if (currentRegion !=
            prevRegion)  // changed region, possibly need to add some optimized
                         // edge points or original points if entering R
        {
            if (currentRegion !=
                5)  // segment doesn't end in R, so it's a candidate for removal
            {
                    ...
                    5)  // we're coming from R, so add this point optimized
                {
                    lineData->append(getOptimizedPoint(
                            currentRegion, it.value().key, it.value().value,
                            prevIt.value().key, prevIt.value().value, rectLeft,
                            rectTop, rectRight, rectBottom));
                    // in the situations 5->1/7/9/3 the segment may leave R and
                    // directly cross through two outer regions. In these cases
                    // we need to add an additional corner point
                    *lineData << getOptimizedCornerPoints(
                            prevRegion, currentRegion, prevIt.value().key,
                            prevIt.value().value, it.value().key,
                            it.value().value, rectLeft, rectTop, rectRight,
                            rectBottom);
                } else if (mayTraverse(prevRegion, currentRegion) &&
                           getTraverse(prevIt.value().key, prevIt.value().value,
                                       it.value().key, it.value().value,
                                       rectLeft, rectTop, rectRight, rectBottom,
                                       crossA, crossB)) {
                    // add the two cross points optimized if segment crosses R
                    // and if segment isn't virtual zeroth segment between last
                    // and first curve point:
                    QVector<QPointF> beforeTraverseCornerPoints,
                            afterTraverseCornerPoints;
                    getTraverseCornerPoints(prevRegion, currentRegion, rectLeft,
                                            rectTop, rectRight, rectBottom,
                ...
                } else  // doesn't cross R, line is just moving around in
                        // outside regions, so only need to add optimized
                        // point(s) at the boundary corner(s)
                {
                    *lineData << getOptimizedCornerPoints(
                            prevRegion, currentRegion, prevIt.value().key,
                            prevIt.value().value, it.value().key,
            ...
            } else  // segment does end in R, so we add previous point optimized
                    // and this point at original position
            {
                if (it ==
                    mData->constBegin())  // it is first point in curve and
                                          // prevIt is last one. So save
                                          // optimized point for adding it to
                                          // the lineData in the end
                    trailingPoints << getOptimizedPoint(
                            prevRegion, prevIt.value().key,
                            prevIt.value().value, it.value().key,
                            it.value().value, rectLeft, rectTop, rectRight,
                            rectBottom);
                else
        ...
        } else  // region didn't change
        {
            ...
            {
                // see how this is not doing anything? That's the main
                // optimization...
            }
        }
        prevIt = it;
    ...
    switch (otherRegion) {
        case 1:  // top and left edge
        {
            ...
            if (intersectKey < rectLeft ||
                intersectKey > rectRight)  // doesn't intersect, so must
                                           // intersect other:
            {
                intersectKey = rectLeft;
                intersectValue = otherValue + (value - otherValue) /
                                                      (key - otherKey) *
        ...
        case 5: {
            break;  // case 5 shouldn't happen for this function but we add it
                    // anyway to prevent potential discontinuity in branch table
        }
        case 6:  // bottom edge
        {
            intersectValue = rectBottom;
    ...
    switch (prevRegion) {
        case 1: {
            switch (currentRegion) {
                ...
                case 9: {  // in this case we need another distinction of cases:
                           // segment may pass below or above rect, requiring
                           // either bottom right or top left corner points
                    if ((value - prevValue) / (key - prevKey) *
                                        (rectLeft - key) +
                                value <
        ...
        case 3: {
            switch (currentRegion) {
                ...
                case 7: {  // in this case we need another distinction of cases:
                           // segment may pass below or above rect, requiring
                           // either bottom right or top left corner points
                    if ((value - prevValue) / (key - prevKey) *
                                        (rectRight - key) +
        ...
        case 7: {
            switch (currentRegion) {
                ...
                case 3: {  // in this case we need another distinction of cases:
                           // segment may pass below or above rect, requiring
                           // either bottom right or top left corner points
                    if ((value - prevValue) / (key - prevKey) *
                                        (rectRight - key) +
        ...
        case 9: {
            switch (currentRegion) {
                ...
                case 1: {  // in this case we need another distinction of cases:
                           // segment may pass below or above rect, requiring
                           // either bottom right or top left corner points
                    if ((value - prevValue) / (key - prevKey) *
                                        (rectLeft - key) +
...
bool QCPCurve::getTraverse(double prevKey,
    ...
    {
        // due to region filter in mayTraverseR(), if line is parallel to value
        // or key axis, R is traversed here
        intersections.append(
                QPointF(key, rectBottom));  // direction will be taken care of
                                            // at end of method
    ...
    } else if (qFuzzyIsNull(value - prevValue))  // line is parallel to key axis
    {
        ...
        intersections.append(QPointF(
                rectLeft,
                value));  // direction will be taken care of at end of method
    ...
    if (intersections.size() > 2) {
        // line probably goes through corner of rect, and we got duplicate
        // points there. single out the point pair with greatest distance in
        // between:
        double distSqrMax = 0;
        QPointF pv1, pv2;
        for (int i = 0; i < intersections.size() - 1; ++i) {
            for (int k = i + 1; k < intersections.size(); ++k) {
                ...
                double distSqr = distPoint.x() * distPoint.x() + distPoint.y() +
                                 distPoint.y();
...
void QCPBarsGroup::clear() {
    foreach (QCPBars *bars, mBars)  // since foreach takes a copy, removing bars
                                    // in the loop is okay
                   ...
                   (baseBars.size() - 1) / 2.0)  // bar is to the left of center
        {
            ...
            } else              // uneven number of bars
            {
                ...
                baseBars.at((baseBars.size() - 1) / 2)
                        ->getPixelWidth(keyCoord, lowerPixelWidth,
                                        upperPixelWidth);
                result -= qAbs(upperPixelWidth - lowerPixelWidth) *
                          0.5;  // half of center bar
                result -=
                        getPixelSpacing(baseBars.at((baseBars.size() - 1) / 2),
                                        keyCoord);  // center bar spacing
            }
            for (int i = startIndex; i > index;
                 --i)  // add widths and spacings of bars in between center and
                       // our bars
            {
...
double QCPBarsGroup::getPixelSpacing(const QCPBars *bars, double keyCoord) {
    switch (mSpacingType) {
        ...
        case stPlotCoords: {
            ...
            return bars->keyAxis()->coordToPixel(keyCoord + mSpacing) -
                   keyPixel;
        }
    }
    return 0;
...
void QCPBars::moveBelow(QCPBars *bars) {
    ...
    connectBars(mBarBelow.data(), mBarAbove.data());  // Note: also works if one
...
void QCPBars::getVisibleDataBounds(
    ...
    // walk left from lbound to find lower bar that actually is completely
    ...
    switch (mWidthType) {
        ...
        case wtAxisRectRatio: {
            if (mKeyAxis && mKeyAxis.data()->axisRect()) {
                if (mKeyAxis.data()->orientation() == Qt::Horizontal)
                    ...
                    upper = mKeyAxis.data()->axisRect()->height() * mWidth *
                            0.5;
                lower = -upper;
                if (mKeyAxis &&
                    (mKeyAxis.data()->rangeReversed() ^
        ...
        case wtPlotCoords: {
            if (mKeyAxis) {
                ...
                upper = mKeyAxis.data()->coordToPixel(key + mWidth * 0.5) -
                        keyPixel;
                lower = mKeyAxis.data()->coordToPixel(key - mWidth * 0.5) -
                        keyPixel;
                // no need to qSwap(lower, higher) when range reversed, because
                // higher/lower are gained by coordinate transform which
                // includes range direction
            } else
...
double QCPBars::getStackedBaseValue(double key, bool positive) const {
    if (mBarBelow) {
        ...
        // one:
        double epsilon = qAbs(key) * 1e-6;  // should be safe even when changed
...
void QCPBars::connectBars(QCPBars *lower, QCPBars *upper) {
    ...
    if (!lower)  // disconnect upper at bottom
    {
        ...
        if (upper->mBarBelow &&
            upper->mBarBelow.data()->mBarAbove.data() == upper)
    ...
    } else if (!upper)  // disconnect lower at top
    {
        ...
        if (lower->mBarAbove &&
            lower->mBarAbove.data()->mBarBelow.data() == lower)
...
QCPRange QCPBars::getKeyRange(bool &foundRange, SignDomain inSignDomain) const {
    ...
    // determine exact range of bars by including bar width and barsgroup
    // offset:
...
QCPRange QCPBars::getValueRange(bool &foundRange,
    ...
    foundRange = true;  // return true because bar charts always have the 0-line
                        // visible
    return range;
}
...
QCPRange QCPStatisticalBox::getValueRange(bool &foundRange,
                                          SignDomain inSignDomain) const {
    QVector<double> values;  // values that must be considered (i.e. all
                             // outliers and the five box-parameters)
    values.reserve(mOutliers.size() + 5);
    values << mMaximum << mUpperQuartile << mMedian << mLowerQuartile
           << mMinimum;
    values << mOutliers;
...
double QCPColorMapData::data(double key, double value) {
    int keyCell = (key - mKeyRange.lower) /
...
void QCPColorMapData::setSize(int keySize, int valueSize) {
    if (keySize != mKeySize || valueSize != mValueSize) {
        ...
        if (!mIsEmpty) {
#ifdef __EXCEPTIONS
            try {  // 2D arrays get memory intensive fast. So if the allocation
                   // fails, at least output debug message
#endif
...
/*! \class QCPColorMap
  ...
  axes on the QCustomPlot surface that you specify in the QColorMap constructor.
  The \a data dimension however is encoded as the color of the point at (\a key,
  \a value).
...
void QCPColorMap::setColorScale(QCPColorScale *colorScale) {
    if (mColorScale)  // unconnect signals from old color scale
    {
        ...
        disconnect(mColorScale.data(),
                   SIGNAL(gradientChanged(QCPColorGradient)), this,
    ...
    if (mColorScale)  // connect signals to new color scale
    {
        ...
        connect(mColorScale.data(), SIGNAL(dataRangeChanged(QCPRange)), this,
                SLOT(setDataRange(QCPRange)));
        connect(mColorScale.data(), SIGNAL(gradientChanged(QCPColorGradient)),
...
void QCPColorMap::updateLegendIcon(Qt::TransformationMode transformMode,
    ...
    if (mMapImage.isNull() && !data()->isEmpty())
        updateMapImage();  // try to update map image if it's null (happens if
    ...
    {
        bool mirrorX =
                (keyAxis()->orientation() == Qt::Horizontal ? keyAxis()
                                                            : valueAxis())
                        ->rangeReversed();
        bool mirrorY = (valueAxis()->orientation() == Qt::Vertical ? valueAxis()
                               ...
                               ->rangeReversed();
        mLegendIcon =
                QPixmap::fromImage(mMapImage.mirrored(mirrorX, mirrorY))
...
void QCPColorMap::updateMapImage() {
    ...
    int keyOversamplingFactor =
            mInterpolate
                    ? 1
                    : (int)(1.0 +
                            100.0 / (double)keySize);  // make mMapImage have at
                                                       // least size 100, factor
                                                       // becomes 1 if size >
                                                       // 200 or interpolation
                                                       // is on
    int valueOversamplingFactor =
            mInterpolate
                    ? 1
                    : (int)(1.0 +
                            100.0 / (double)valueSize);  // make mMapImage have
                                                         // at least size 100,
                                                         // factor becomes 1 if
                                                         // size > 200 or
                                                         // interpolation is on
    ...
    QImage *localMapImage =
            &mMapImage;  // this is the image on which the colorization
                         // operates. Either the final mMapImage, or if we need
    ...
    if (keyOversamplingFactor > 1 || valueOversamplingFactor > 1) {
        ...
        mUndersampledMapImage =
                QImage();  // don't need oversampling mechanism anymore (map
                           // size has changed) but mUndersampledMapImage still
                           // has nonzero size, free it
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        ...
        for (int line = 0; line < lineCount; ++line) {
            QRgb *pixels = reinterpret_cast<QRgb *>(localMapImage->scanLine(
                    lineCount - 1 -
                    line));  // invert scanline index because QImage counts
                             // scanlines from top, but our vertical index
                             // counts from bottom (mathematical coordinate
                             // system)
...
void QCPColorMap::draw(QCPPainter *painter) {
    ...
    QRectF mapBufferTarget;  // the rect in absolute widget coordinates where
                             // the visible map portion/buffer will end up in
    QPixmap mapBuffer;
    ...
    if (useBuffer) {
        ...
        mapBuffer = QPixmap(
                (mapBufferTarget.size() * mapBufferPixelRatio).toSize());
    ...
    if (keyAxis()->orientation() == Qt::Horizontal) {
        ...
        if (mMapData->valueSize() > 1)
            halfCellHeight = 0.5 * imageRect.height() /
                             (double)(mMapData->valueSize() - 1);
    } else  // keyAxis orientation is Qt::Vertical
    {
        if (mMapData->keySize() > 1)
            halfCellHeight = 0.5 * imageRect.height() /
                             (double)(mMapData->keySize() - 1);
        if (mMapData->valueSize() > 1)
            halfCellWidth = 0.5 * imageRect.width() /
                            (double)(mMapData->valueSize() - 1);
    }
    ...
    if (mTightBoundary) {
        ...
        QRectF tightClipRect =
                QRectF(coordsToPixels(mMapData->keyRange().lower,
                                      mMapData->valueRange().lower),
                       coordsToPixels(mMapData->keyRange().upper,
                                      mMapData->valueRange().upper))
                        .normalized();
    ...
    if (useBuffer)  // localPainter painted to mapBuffer, so now draw buffer
                    // with original painter
    {
        delete localPainter;
...
void QCPColorMap::drawLegendIcon(QCPPainter *painter,
    ...
    if (!mLegendIcon.isNull()) {
        QPixmap scaledIcon =
                mLegendIcon.scaled(rect.size().toSize(), Qt::KeepAspectRatio,
                                   Qt::FastTransformation);
...
double QCPFinancial::selectTest(const QPointF &pos,
    ...
    if (mKeyAxis.data()->axisRect()->rect().contains(pos.toPoint())) {
        ...
        QCPFinancialDataMap::const_iterator lower,
                upper;  // note that upper is the actual upper point, and not 1
...
QCPFinancialDataMap QCPFinancial::timeSeriesToOhlc(const QVector<double> &time,
    ...
    for (int i = 0; i < count; ++i) {
            ...
            index)  // data point still in current bin, extend high/low:
        {
            ...
            if (i ==
                count - 1)  // last data point is in current bin, finalize bin:
            {
        ...
        } else  // data point not anymore in current bin, set close of old and
                // open of new bin, and add old to map:
        {
    ...
    if (mChartStyle == csOhlc) {
        ...
        if (mTwoColored) {
            ...
            painter->setClipRegion(QRegion(QPolygon()
                                           ...
                                           << rect.topRight().toPoint()
                                           << rect.topLeft().toPoint()));
            painter->drawLine(QLineF(0, rect.height() * 0.5,
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
            if (mSelected)
                ...
                linePen = it.value().close >= it.value().open ? mPenPositive
                                                              : mPenNegative;
            else
                linePen = mPen;
            ...
            double keyWidthPixels =
                    ...
                    keyAxis->coordToPixel(
                            it.value().key -
                            mWidth * 0.5);  // sign of this makes sure
                                            // open/close are on correct sides
...
void QCPFinancial::drawCandlestickPlot(
    ...
    if (keyAxis->orientation() == Qt::Horizontal) {
        for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
            ...
            double keyWidthPixels =
                    keyPixel -
                    keyAxis->coordToPixel(it.value().key - mWidth * 0.5);
            painter->drawRect(
                    QRectF(QPointF(keyPixel - keyWidthPixels, closePixel),
    ...
    } else  // keyAxis->orientation() == Qt::Vertical
    {
        for (QCPFinancialDataMap::const_iterator it = begin; it != end; ++it) {
            ...
            double keyWidthPixels =
                    ...
                    keyAxis->coordToPixel(it.value().key - mWidth * 0.5);
            painter->drawRect(
                    QRectF(QPointF(closePixel, keyPixel - keyWidthPixels),
        ...
        for (it = begin; it != end; ++it) {
                ...
                boxValueRange.contains(posValue))  // is in open-close-box
            {
                ...
                double highLineDistSqr = distSqrToLine(
                        QPointF(keyPixel,
                                valueAxis->coordToPixel(it.value().high)),
                        QPointF(keyPixel,
                                valueAxis->coordToPixel(qMax(
    ...
    } else  // keyAxis->orientation() == Qt::Vertical
    {
        for (it = begin; it != end; ++it) {
                ...
                boxValueRange.contains(posValue))  // is in open-close-box
            {
                ...
                double keyPixel = keyAxis->coordToPixel(it.value().key);
                double highLineDistSqr = distSqrToLine(
                        QPointF(valueAxis->coordToPixel(it.value().high),
                                keyPixel),
                        QPointF(valueAxis->coordToPixel(qMax(it.value().open,
                                                             it.value().close)),
                                keyPixel),
                        pos);
                double lowLineDistSqr = distSqrToLine(
                        QPointF(valueAxis->coordToPixel(it.value().low),
                                keyPixel),
                        QPointF(valueAxis->coordToPixel(qMin(it.value().open,
                                                             it.value().close)),
                                keyPixel),
                        pos);
                currentDistSqr = qMin(highLineDistSqr, lowLineDistSqr);
...
QLineF QCPItemStraightLine::getRectClippedStraightLine(
    ...
    if (qFuzzyIsNull(vec.x()))  // line is vertical
    {
        ...
        if (gamma >= 0 && gamma <= rect.width())
            result.setLine(bx + gamma, rect.top(), bx + gamma,
                           rect.bottom());  // no need to check bottom because
                                            // we know line is vertical
    } else if (qFuzzyIsNull(vec.y()))       // line is horizontal
    {
        ...
        if (gamma >= 0 && gamma <= rect.height())
            result.setLine(rect.left(), by + gamma, rect.right(),
                           by + gamma);  // no need to check right because we
    ...
    } else                               // line is skewed
    {
        ...
        if (pointVectors.size() == 2) {
            ...
            // line probably goes through corner of rect, and we got two points
            // there. single out the point pair with greatest distance:
            double distSqrMax = 0;
            QVector2D pv1, pv2;
            for (int i = 0; i < pointVectors.size() - 1; ++i) {
                for (int k = i + 1; k < pointVectors.size(); ++k) {
                    double distSqr = (pointVectors.at(i) - pointVectors.at(k))
                                             .lengthSquared();
                    if (distSqr > distSqrMax) {
...
void QCPItemRect::draw(QCPPainter *painter) {
    ...
    if (boundingRect.intersects(
                clipRect()))  // only draw if bounding rect of rect item is
                              // visible in cliprect
    {
        painter->setPen(mainPen());
        painter->setBrush(mainBrush());
...
double QCPItemText::selectTest(const QPointF &pos,
    ...
    // rotated coordinate system, so we can use the normal rectSelectTest
    // function for non-rotated rects:
...
void QCPItemText::draw(QCPPainter *painter) {
    ...
    QPointF textPos =
            getTextDrawPoint(QPointF(0, 0), textBoxRect,
                             mPositionAlignment);  // 0, 0 because the transform
...
QPointF QCPItemEllipse::anchorPixelPoint(int anchorId) const {
    ...
    switch (anchorId) {
        case aiTopLeftRim:
            return rect.center() +
                   (rect.topLeft() - rect.center()) * 1 / qSqrt(2);
        case aiTop:
            return (rect.topLeft() + rect.topRight()) * 0.5;
        case aiTopRightRim:
            return rect.center() +
                   (rect.topRight() - rect.center()) * 1 / qSqrt(2);
        case aiRight:
            return (rect.topRight() + rect.bottomRight()) * 0.5;
        ...
        case aiBottomLeftRim:
            return rect.center() +
                   (rect.bottomLeft() - rect.center()) * 1 / qSqrt(2);
        case aiLeft:
            return (rect.topLeft() + rect.bottomLeft()) * 0.5;
        case aiCenter:
...
*/
void QCPItemPixmap::updateScaledPixmap(QRect finalRect,
                                       bool flipHorz,
                                       bool flipVert) {
    ...
    switch (mStyle) {
        ...
        case tsPlus: {
            if (clipRect().intersects(
                ...
                return qSqrt(qMin(distSqrToLine(center + QPointF(-w, 0),
                                  ...
                                  distSqrToLine(center + QPointF(0, -w),
                                                center + QPointF(0, w), pos)));
            break;
        }
        case tsCrosshair: {
            return qSqrt(qMin(
                    distSqrToLine(QPointF(clip.left(), center.y()),
        ...
        case tsSquare: {
            if (clip.intersects(
                ...
                QRectF rect =
                        QRectF(center - QPointF(w, w), center + QPointF(w, w));
                bool filledRect = mBrush.style() != Qt::NoBrush &&
                                  mBrush.color().alpha() != 0;
                return rectSelectTest(rect, pos, filledRect);
            }
            break;
    ...
    switch (mStyle) {
        ...
        case tsPlus: {
            if (clip.intersects(
                        QRectF(center - QPointF(w, w), center + QPointF(w, w))
                                .toRect())) {
                painter->drawLine(QLineF(center + QPointF(-w, 0),
                                         center + QPointF(w, 0)));
                painter->drawLine(QLineF(center + QPointF(0, -w),
                                         center + QPointF(0, w)));
            }
            break;
        }
    ...
    if (mGraph) {
        if (mParentPlot->hasPlottable(mGraph)) {
            if (mGraph->data()->size() > 1) {
                ...
                QCPDataMap::const_iterator last =
                ...
                if (mGraphKey < first.key())
                    ...
                    QCPDataMap::const_iterator it =
                            mGraph->data()->lowerBound(mGraphKey);
                    if (it !=
                        first)  // mGraphKey is somewhere between iterators
                    {
                        QCPDataMap::const_iterator prevIt = it - 1;
                        if (mInterpolating) {
                            // interpolate between iterators around mGraphKey:
                            double slope = 0;
                            if (!qFuzzyCompare((double)it.key(),
                                               (double)prevIt.key()))
                                slope = (it.value().value -
                                         prevIt.value().value) /
                                        (it.key() - prevIt.key());
                            position->setCoords(
                                    mGraphKey,
...
double QCPItemBracket::selectTest(const QPointF &pos,
    ...
    switch (mStyle) {
        ...
        case QCPItemBracket::bsRound: {
            ...
            double b =
                    distSqrToLine((centerVec - widthVec + lengthVec).toPointF(),
                                  (centerVec - widthVec).toPointF(), pos);
            double c =
                    distSqrToLine((centerVec + widthVec + lengthVec).toPointF(),
                                  (centerVec + widthVec).toPointF(), pos);
        ...
        case QCPItemBracket::bsCalligraphic: {
            double a = distSqrToLine(
                    (centerVec - widthVec * 0.75f + lengthVec * 0.15f)
                            .toPointF(),
    ...
    if (clip.intersects(boundingPoly.boundingRect())) {
        ...
        switch (mStyle) {
            ...
            case bsCalligraphic: {
                ...
                path.cubicTo(
                        (centerVec + widthVec - lengthVec * 0.8f).toPointF(),
                        (centerVec + 0.4f * widthVec + 0.8f * lengthVec)
                                .toPointF(),
                        centerVec.toPointF());
                path.cubicTo(
                        (centerVec - 0.4f * widthVec + 0.8f * lengthVec)
                ...
                path.cubicTo(
                        ...
                        (centerVec - 0.2f * widthVec + 1.2f * lengthVec)
                                .toPointF(),
