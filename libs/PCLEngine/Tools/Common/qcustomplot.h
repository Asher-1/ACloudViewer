...
enum AntialiasedElement {
    ...
    ,
    aePlottables =
            0x0020  ///< <tt>0x0020</tt> Main lines of plottables (excluding
                    ///< error bars, see element \ref aeErrorBars)
    ,
    ...
    ,
    aeFills = 0x0200  ///< <tt>0x0200</tt> Borders of fills (e.g. under or
                      ///< between graphs)
    ,
    aeZeroLine = 0x0400  ///< <tt>0x0400</tt> Zero-lines, see \ref
...
enum PlottingHint {
    ...
    ,
    phFastPolylines = 0x001  ///< <tt>0x001</tt> Graph/Curve lines are drawn
                             ///< with a faster method. This reduces the quality
    ///<                especially of the line segment joins. (Only relevant for
    ///<                solid line pens.)
    ,
    phForceRepaint =
            0x002  ///< <tt>0x002</tt> causes an immediate repaint() instead of
                   ///< a soft update() when QCustomPlot::replot() is called
                   ///< with parameter \ref QCustomPlot::rpHint.
    ///<                This is set by default to prevent the plot from freezing
    ///<                on fast consecutive replots (e.g. user drags ranges with
    ///<                mouse).
    ,
...
enum Interaction {
    ...
    ,
    iSelectOther =
            0x080  ///< <tt>0x080</tt> All other objects are selectable (e.g.
                   ///< your own derived layerables, the plot title,...)
};
...
public:
  ...
  */
    ...
    QCPScatterStyle(const QPainterPath &customPath,
                    const QPen &pen,
...
public:
  ...
  */
    enum PainterMode {
        pmDefault = 0x00  ///< <tt>0x00</tt> Default mode for painting on screen
                          ///< devices
        ,
...
protected:
    ...
    void applyAntialiasingHint(QCPPainter *painter,
...
protected:
    ...
    QVector<int> getSectionSizes(QVector<int> maxSizes,
...
class QCP_LIB_DECL QCPAxis : public QCPLayerable {
    ...
    Q_PROPERTY(
            QString dateTimeFormat READ dateTimeFormat WRITE setDateTimeFormat)
    Q_PROPERTY(
            Qt::TimeSpec dateTimeSpec READ dateTimeSpec WRITE setDateTimeSpec)
...
public:
  ...
  */
    enum LabelType {
        ...
        ,
        ltDateTime  ///< Tick coordinate is regarded as a date/time (seconds
                    ///< since 1970-01-01T00:00:00 UTC) and will be displayed
                    ///< and formatted as such. (for details, see \ref
                    ///< setDateTimeFormat)
    };
    Q_ENUMS(LabelType)
    /*!
  ...
  */
    enum ScaleType {
        ...
        stLogarithmic  ///< Logarithmic scaling with correspondingly transformed
                       ///< plots and (major) tick marks at every base power
                       ///< (see \ref setScaleLogBase).
    };
    Q_ENUMS(ScaleType)
    /*!
...
protected:
    ...
    virtual void placeTickLabel(QCPPainter *painter,
                                double position,
                                int distanceToAxis,
                                const QString &text,
                                QSize *tickLabelsSize);
    virtual void drawTickLabel(QCPPainter *painter,
                               double x,
                               double y,
    ...
    virtual QPointF getTickLabelDrawOffset(
            const TickLabelData &labelData) const;
    virtual void getMaxTickLabelSize(const QFont &font,
                                     const QString &text,
...
protected:
  ...
  */
    enum SignDomain {
        sdNegative  ///< The negative sign domain, i.e. numbers smaller than
                    ///< zero
        ,
        sdBoth  ///< Both sign domains, including zero, i.e. all (rational)
                ///< numbers
        ,
        sdPositive  ///< The positive sign domain, i.e. numbers greater than
                    ///< zero
    };
    ...
    double distSqrToLine(const QPointF &start,
                         const QPointF &end,
...
public:
  ...
  */
    enum PositionType {
        ...
        ///< bottom right corner, setCoords(0.5, 0) will be horizontally
        ...
        ptAxisRectRatio  ///< Static positioning given by a fraction of the axis
                         ...
                         ///< the top
        ///< left corner of the axis rect. setCoords(1, 1) will be at the bottom
        ///< right corner, setCoords(0.5, 0) will be horizontally centered and
        ///< vertically at the top of the axis rect, etc. You can also go beyond
        ///< the axis rect by providing negative coordinates or coordinates
        ///< larger than 1.
        ,
    ...
    QCPItemPosition(QCustomPlot *parentPlot,
                    QCPAbstractItem *parentItem,
                    const QString name);
...
protected:
    ...
    double distSqrToLine(const QPointF &start,
                         const QPointF &end,
                         const QPointF &point) const;
    double rectSelectTest(const QRectF &rect,
                          const QPointF &pos,
...
public:
  ...
  */
    enum RefreshPriority {
        ...
        ,
        rpQueued  ///< Queues the refresh such that it is performed at a
                  ///< slightly delayed point in time after the replot, by
                  ///< calling QWidget::update() after the replot
        ,
        rpHint  ///< Whether to use immediate repaint or queued update depends
                ///< on whether the plotting hint \ref QCP::phForceRepaint is
    ...
    bool saveBmp(const QString &fileName,
                 ...
                 int height = 0,
                 double scale = 1.0);
    bool saveRastered(const QString &fileName,
                      int width,
                      int height,
                      double scale,
                      const char *format,
...
protected:
    ...
    QCPLayerable *layerableAt(const QPointF &pos,
                              bool onlySelectable,
...
protected:
    ...
    QPointer<QCPAxis> mRangeDragHorzAxis, mRangeDragVertAxis,
...
public:
  ...
  */
    enum ErrorType {
        ...
        ,
        etValue  ///< Error bars for the value dimension of the data point are
                 ///< shown
        ,
        etBoth  ///< Error bars for both key and value dimensions of the data
                ///< point are shown
    };
    Q_ENUMS(ErrorType)
    ...
    void setDataKeyError(const QVector<double> &key,
...
protected:
    ...
    // introduced virtual methods:
    virtual void drawFill(QCPPainter *painter,
    ...
    void getImpulsePlotData(QVector<QPointF> *linePixelData,
                            QVector<QCPData> *scatterData) const;
    void drawError(QCPPainter *painter,
                   double x,
                   double y,
    ...
    const QPolygonF getChannelFillPolygon(
...
public:
  ...
  */
    ...
    void addData(double key, double value);
    void addData(const QVector<double> &ts,
...
protected:
    ...
    int getRegion(double x,
                  double y,
                  double rectLeft,
                  double rectTop,
                  double rectRight,
                  double rectBottom) const;
    QPointF getOptimizedPoint(int prevRegion,
                              double prevKey,
                              double prevValue,
                              double key,
                              double value,
                              ...
                              double rectRight,
                              double rectBottom) const;
    QVector<QPointF> getOptimizedCornerPoints(int prevRegion,
                                              int currentRegion,
                                              double prevKey,
                                              double prevValue,
                                              double key,
    ...
    bool getTraverse(double prevKey,
                     double prevValue,
                     double key,
                     double value,
                     double rectLeft,
    ...
    void getTraverseCornerPoints(int prevRegion,
                                 int currentRegion,
                                 double rectLeft,
                                 double rectTop,
                                 double rectRight,
...
public:
  ...
  */
    ...
    void addData(
            double key, double open, double high, double low, double close);
    void addData(const QVector<double> &key,
                 const QVector<double> &open,
                 const QVector<double> &high,
                 const QVector<double> &low,
                 const QVector<double> &close);
...
protected:
    ...
    // non-virtual methods:
    double distToStraightLine(const QVector2D &point1,
                              const QVector2D &vec,
...
protected:
    ...
    // non-virtual methods:
    QLineF getRectClippedLine(const QVector2D &start,
                              const QVector2D &end,
                              const QRect &rect) const;
    QPen mainPen() const;
};
...
protected:
    ...
    // non-virtual methods:
    QPointF getTextDrawPoint(const QPointF &pos,
                             const QRectF &rect,
...
protected:
    ...
    // non-virtual methods:
    void updateScaledPixmap(QRect finalRect = QRect(),
                            bool flipHorz = false,
                            bool flipVert = false);
