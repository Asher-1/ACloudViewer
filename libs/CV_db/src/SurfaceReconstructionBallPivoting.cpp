...
public:
    BallPivoting(const ccPointCloud& pcd)
        ...
        if (!baseVertices->reserveThePointsTable(pcd.size())) {
            utility::LogError("[BallPivoting] not enough memory!");
        }

        baseVertices->addPoints(pcd.getPoints());
        if (pcd.hasNormals()) {
            if (baseVertices->reserveTheNormsTable()) {
                baseVertices->addNorms(pcd.getPointNormals());
            }
        }

        if (pcd.hasColors()) {
            if (baseVertices->reserveTheRGBTable()) {
                baseVertices->addRGBColors(pcd.getPointColors());
            }
        }

        for (size_t vidx = 0; vidx < pcd.size(); ++vidx) {
            vertices.emplace_back(new BallPivotingVertex(
                    static_cast<int>(vidx), pcd.getEigenPoint(vidx),
                    pcd.getEigenNormal(vidx)));
        }
    }
    ...
    void CreateTriangle(const BallPivotingVertexPtr& v0,
        ...
        if (face_normal.dot(v0->normal_) > -1e-16) {
            mesh_->addTriangle(v0->idx_, v1->idx_, v2->idx_);
        } else {
            mesh_->addTriangle(v0->idx_, v2->idx_, v1->idx_);
        }
        mesh_->addTriangleNorm(face_normal);
    }
    ...
    BallPivotingVertexPtr FindCandidateVertex(
        ...
        utility::LogDebug("[FindCandidateVertex] src={} => [{}, {}, {}]",
                          src->idx_, src->point_(0), src->point_(1),
                          src->point_(2));
        utility::LogDebug("[FindCandidateVertex] tgt={} => [{}, {}, {}]",
                          tgt->idx_, tgt->point_(0), tgt->point_(1),
                          tgt->point_(2));
        utility::LogDebug("[FindCandidateVertex] opp={} => [{}, {}, {}]",
                          opp->idx_, opp->point_(0), opp->point_(1),
                          opp->point_(2));

        Eigen::Vector3d mp = 0.5 * (src->point_ + tgt->point_);
        utility::LogDebug(
                "[FindCandidateVertex] edge=({}, {}), mp=[{}, {}, {}]",
                edge->source_->idx_, edge->target_->idx_, mp(0), mp(1), mp(2));

        BallPivotingTrianglePtr triangle = edge->triangle0_;
        const Eigen::Vector3d& center = triangle->ball_center_;
        utility::LogDebug(
                "[FindCandidateVertex] edge=({}, {}), center=[{}, {}, {}]",
                edge->source_->idx_, edge->target_->idx_, center(0), center(1),
                center(2));
        ...
        for (auto nbidx : indices) {
            ...
            utility::LogDebug(
                    "[FindCandidateVertex] candidate={:d} => [{}, {}, {}]",
                    candidate->idx_, candidate->point_(0), candidate->point_(1),
                    candidate->point_(2));

            bool coplanar =
                    cloudViewer::utility::IntersectionTest::PointsCoplanar(
                            src->point_, tgt->point_, opp->point_,
                            candidate->point_);
            if (coplanar &&
                (cloudViewer::utility::IntersectionTest::
            ...
            utility::LogDebug(
                    ...
                    candidate->idx_, new_center(0), new_center(1),
                    new_center(2));

            Eigen::Vector3d b = new_center - mp;
            b /= b.norm();
            utility::LogDebug(
                    "[FindCandidateVertex] candidate {:d} v=[{},{},{}], "
                    "a=[{},{},{}], b=[{},{},{}]",
                    candidate->idx_, v(0), v(1), v(2), a(0), a(1), a(2), b(0),
                    b(1), b(2));
    ...
    std::shared_ptr<ccMesh> Run(const std::vector<double>& radii) {
        ...
        for (double radius : radii) {
            ...
            utility::LogDebug("[Run] mesh_ has {:d} triangles", mesh_->size());
            utility::LogDebug("[Run] ################################");
        }
        ...
        {
            mesh_->shrinkToFit();
            NormsIndexesTableType* normals = mesh_->getTriNormsTable();
            if (normals) {
...
private:
    bool has_normals_;
    cloudViewer::geometry::KDTreeFlann kdtree_;
...
std::shared_ptr<ccMesh> ccMesh::CreateFromPointCloudBallPivoting(
