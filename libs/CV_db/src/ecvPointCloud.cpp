...
void ccPointCloud::setPointColor(size_t pointIndex, const ecvColor::Rgb& col) {
    ...
    if (m_pointColorAlphas.size() < m_rgbColors->currentSize()) {
...
//! "Cellular" function to apply a gaussian filter on the RGB values of points
//! inside an octree cell
...
automatically switched on if its sigmaSF parameter in additionalParameters is
positive.
