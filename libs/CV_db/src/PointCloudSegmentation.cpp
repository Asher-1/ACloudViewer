...
RANSACResult EvaluateRANSACBasedOnDistance(const std::vector<CCVector3> &points,
...
RANSACResult EvaluateRANSACBasedOnDistance(
        ...
        double distance_threshold,
        double error) {
    return EvaluateRANSACBasedOnDistance(CCVector3::fromArrayContainer(points),
                                         plane_model, inliers,
                                         distance_threshold, error);
}
...
Eigen::Vector4d GetPlaneFromPoints(const std::vector<CCVector3> &points,
...
std::tuple<Eigen::Vector4d, std::vector<size_t>> ccPointCloud::SegmentPlane(
    ...
    for (int itr = 0; itr < num_iterations; itr++) {
        ...
        // inliers.
        plane_model = ccMesh::ComputeTrianglePlane(getEigenPoint(inliers[0]),
                                                   getEigenPoint(inliers[1]),
                                                   getEigenPoint(inliers[2]));
    ...
    cloudViewer::utility::LogDebug(
            "RANSAC | Inliers: {:d}, Fitness: {:e}, RMSE: {:e}", inliers.size(),
...
std::shared_ptr<ccPointCloud> ccPointCloud::Crop(
        const ecvOrientedBBox &bbox) const {
    if (bbox.IsEmpty()) {
        CVLog::Warning(
