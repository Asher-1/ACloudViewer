...
class CV_DB_LIB_API ccSerializableObject {
public:
    //! Destructor
    ...
    **/
    ...
    enum DeserializationFlags {
        DF_POINT_COORDS_64_BITS =
                1, /**< Point coordinates are stored as 64 bits double
                      (otherwise 32 bits floats) **/
        // DGM: inversion is 'historical' ;)
        DF_SCALAR_VAL_32_BITS = 2, /**< Scalar values are stored as 32 bits
    ...
    **/
    ...
    /** Shortcut for returning a standardized error message in the toFile
    method. \return always false
    **/
    static bool WriteError() {
        CVLog::Error("Write error (disk full or no access right?)");
        return false;
    }

    //! Sends a custom error message (read error) and returns 'false'
    /** Shortcut for returning a standardized error message in the fromFile
    method. \return always false
    **/
    static bool ReadError() {
        CVLog::Error("Read error (corrupted file or no access right?)");
        return false;
    }
    ...
    **/
    static bool CorruptError() {
        CVLog::Error("File seems to be corrupted");
        return false;
    }
};

//! Serialization helpers
class CV_DB_LIB_API ccSerializationHelper {
public:
