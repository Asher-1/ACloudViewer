 ...
 **/
class CV_DB_LIB_API ccScalarField : public cloudViewer::ScalarField,
...
public:
    ...
    **/
    ...
    public:
        ...
        // setters
        void setBounds(ScalarType minVal,
                       ScalarType maxVal,
        ...
        inline void setStart(ScalarType value) {
            ...
            if (m_stop < m_start) m_stop = m_start;
            updateRange();
        }
    ...
    protected:
        ...
        inline void updateRange() {
            m_range = std::max(m_stop - m_start, ZERO_TOLERANCE_SCALAR);
        }
        ...
        ScalarType
                m_range; /**< Actual range: start-stop (but can't be ZERO!) **/
    };
    ...
    **/
    ...
    //! Returns the color corresponding to a given value (wrt to the current
    //! display parameters)
    ...
    //! Returns whether the scalar field in its current configuration MAY have
    //! 'hidden' values or not
    /** 'Hidden' values are typically NaN values or values outside of the
    'displayed' intervale while those values are not displayed in grey (see
...
protected:  // methods
    ...
    /**	\param val scalar value
            \return a number between 0 and 1 if inside displayed range or -1
    otherwise
    **/
