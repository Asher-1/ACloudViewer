 ...
 **/
class CV_DB_LIB_API ccQuadric : public ccGenericPrimitive {
public:
    ...
    /** Quadric orthogonal dimension is 'Z' by default
            ...
            \param eq equation coefficients ( Z = a + b.X + c.Y + d.X^2 + e.X.Y
    + f.Y^2) \param dims optional dimension indexes \param transMat optional 3D
    ...
    **/
    ...
    //! Returns the quadric equation 'coordinate system' (X,Y,Z dimensions
    //! indexes)
    ...
    /** \param P input 3D point
            \param[out] Q position of the input point in the quadric coordinate
    system \return elevation of the input point (in the coordinate system
    quadric)
    **/
    PointCoordinateType projectOnQuadric(const CCVector3& P,
                                         CCVector3& Q) const;
    ...
    /** The cloud can be any cloudViewer::GenericIndexedCloudPersist-derived
    object. \param[in] cloud input cloud \param[out] rms quadric fitting rms
    (optional) \return quadric primitive (if successful)
    **/
    static ccQuadric* Fit(cloudViewer::GenericIndexedCloudPersist* cloud,
                          double* rms /*=0*/);
