...
public:
    DL_CreationInterface() { extrusion = new DL_Extrusion; }
    virtual ~DL_CreationInterface() { delete extrusion; }

    /**
     * Called for every code / value tuple of the DXF file. The complete DXF
     * file contents can be handled by the implemetation of this function.
     */
    virtual void processCodeValuePair(unsigned int groupCode,
     ...
     */
    virtual void setVariableString(const std::string& key,
     ...
     */
    virtual void setVariableDouble(const std::string& key,
                                   double value,
                                   int code) = 0;

#ifdef DL_COMPAT
    virtual void setVariableVector(
            const char* key, double v1, double v2, double v3, int code) = 0;
    ...
    void setAttributes(const DL_Attributes& attrib) { attributes = attrib; }

    /** @return the current attributes used for new entities. */
    DL_Attributes getAttributes() { return attributes; }
    ...
    /** @return the current attributes used for new entities. */
    DL_Extrusion* getExtrusion() { return extrusion; }
