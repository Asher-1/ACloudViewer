...
bool VTKFilter::canSave(CV_CLASS_ENUM type,
                        bool& multiple,
...
CC_FILE_ERROR VTKFilter::saveToFile(ccHObject* entity,
    ...
    if (mesh) {
        ...
        triCount = mesh->size();
        if (triCount == 0) {
            CVLog::Warning("[VTK] Input mesh has no triangle?!");
    ...
    if (!vertices) {
        CVLog::Warning("[VTK] No point cloud nor mesh in input selection!");
        return CC_FERR_BAD_ENTITY_TYPE;
    }
    unsigned ptsCount = vertices->size();
    if (!ptsCount) {
        CVLog::Warning("[VTK] No point/vertex to save?!");
        return CC_FERR_NO_SAVE;
    }
    ...
    // write header
    outFile << "# vtk DataFile Version 3.0" << QtCompat::endl;
    outFile << "vtk output" << QtCompat::endl;
    outFile << "ASCII" << QtCompat::endl;
    outFile << "DATASET " << (mesh ? "POLYDATA" : "UNSTRUCTURED_GRID")
            << QtCompat::endl;
    ...
    // write the points
    {
        outFile << "POINTS " << ptsCount << " " << floatType << QtCompat::endl;
        for (unsigned i = 0; i < ptsCount; ++i) {
            const CCVector3* P = vertices->getPoint(i);
            CCVector3d Pglobal = vertices->toGlobal3d<PointCoordinateType>(*P);
            outFile << Pglobal.x << " " << Pglobal.y << " " << Pglobal.z
                    << QtCompat::endl;
        }
    }
    ...
    // write triangles
    if (mesh) {
        outFile << "POLYGONS " << triCount << " " << 4 * triCount
                << QtCompat::endl;
        ...
        for (unsigned i = 0; i < triCount; ++i) {
            const cloudViewer::VerticesIndexes* tsi =
                    mesh->getNextTriangleVertIndexes();  // DGM:
                                                         ...
                                                         // is faster for mesh
                                                         // groups!
            outFile << "3 " << tsi->i1 << " " << tsi->i2 << " " << tsi->i3
                    << QtCompat::endl;
        }
    } else {
        // write cell data
        outFile << "CELLS " << ptsCount << " " << 2 * ptsCount
                << QtCompat::endl;
        for (unsigned i = 0; i < ptsCount; ++i)
            outFile << "1 " << i << QtCompat::endl;

        outFile << "CELL_TYPES " << ptsCount << QtCompat::endl;
        for (unsigned i = 0; i < ptsCount; ++i)
            outFile << "1 " << QtCompat::endl;
    }
    ...
    if (withNormals) {
        ...
        if (withTriNormals) {
            ...
            if (normsTable && triCount > 0) {
                outFile << "CELL_DATA " << triCount << QtCompat::endl;
                outFile << "NORMALS TriangleNormals " << floatType
                        << QtCompat::endl;
                ...
                for (unsigned i = 0; i < triCount; ++i) {
                    ...
                    if (validNormals > 0) {
                        ...
                        outFile << avgNormal.x << " " << avgNormal.y << " "
                                << avgNormal.z << QtCompat::endl;
                    } else {
                        // fallback: output a default normal
                        outFile << "0 0 1" << QtCompat::endl;
                    }
                }
            }
        }
        // per-vertex normals (only if no per-triangle normals)
            ...
            outFile << "POINT_DATA " << ptsCount << QtCompat::endl;
            outFile << "NORMALS Normals " << floatType << QtCompat::endl;
            for (unsigned i = 0; i < ptsCount; ++i) {
                const CCVector3& N = vertices->getPointNormal(i);
                outFile << N.x << " " << N.y << " " << N.z << QtCompat::endl;
            }
        }
    }
    ...
    if (vertices->hasColors()) {
        outFile << "COLOR_SCALARS RGB 3" << QtCompat::endl;
        for (unsigned i = 0; i < ptsCount; ++i) {
            const ecvColor::Rgb& C = vertices->getPointColor(i);
            outFile << static_cast<float>(C.r) / ecvColor::MAX << " "
                    << static_cast<float>(C.g) / ecvColor::MAX << " "
                    << static_cast<float>(C.b) / ecvColor::MAX
                    << QtCompat::endl;
        }
    }
    ...
    if (vertices->isA(CV_TYPES::POINT_CLOUD)) {
        ...
        for (unsigned i = 0; i < sfCount; ++i) {
            ccScalarField* sf =
                    static_cast<ccScalarField*>(pointCloud->getScalarField(i));

            outFile << "SCALARS " << QString(sf->getName()).replace(" ", "_")
                    << (sizeof(ScalarType) == 4 ? " float" : " double") << " 1"
                    << QtCompat::endl;
            outFile << "LOOKUP_TABLE default" << QtCompat::endl;

            for (unsigned j = 0; j < ptsCount; ++j) {
                outFile << sf->getGlobalShift() + sf->getValue(j)
                        << QtCompat::endl;
            }
        }
    } else  // virtual point cloud, we only have access to its currently
            // displayed scalar field
    {
        if (vertices->hasScalarFields()) {
            outFile << "SCALARS ScalarField"
                    << (sizeof(ScalarType) == 4 ? " float" : " double") << " 1"
                    << QtCompat::endl;
            outFile << "LOOKUP_TABLE default" << QtCompat::endl;

            for (unsigned j = 0; j < ptsCount; ++j)
                outFile << vertices->getPointDisplayedDistance(j)
                        << QtCompat::endl;
        }
    }
...
CC_FILE_ERROR VTKFilter::loadFile(const QString& filename,
                                  ccHObject& container,
    ...
    if (!nextline.startsWith("# vtk")) return CC_FERR_MALFORMED_FILE;

    // comment
    nextline = inFile.readLine();
    CVLog::Print(QString("[VTK] ") + nextline);
    ...
    if (fileType.startsWith("BINARY")) {
        ...
        CVLog::Error("VTK binary format not supported yet!");
        ...
        if (!dataType.startsWith("DATASET")) return CC_FERR_MALFORMED_FILE;
        dataType.remove(0, 8);
        if (dataType.startsWith("POLYDATA")) {
            ...
            CVLog::Error(
                    QString("VTK entity '%1' is not supported!").arg(dataType));
            return CC_FERR_WRONG_FILE_TYPE;
        }
    }
    ...
    while (error == CC_FERR_NO_ERROR) {
        ...
        if (nextline.startsWith("POINTS")) {
            ...
            //{
            //	CVLog::Error(QString("Non floating point data (%1) is not
            // supported!").arg(dataFormat)); 	error = CC_FERR_WRONG_FILE_TYPE;
            //	break;
            // }
            ...
            while (iPt < ptsCount) {
                ...
                for (int i = 0; i < parts.size(); ++i) {
                    ...
                    if (!ok) {
                        CVLog::Warning(
                                "[VTK] Element #%1 of POINTS data is "
                                "corrupted!",
                                iPt);
                        error = CC_FERR_MALFORMED_FILE;
                        iPt = ptsCount;
                    ...
                    if (coordIndex == 2) {
                        ...
                        if (iPt == 0) {
                            ...
                            if (HandleGlobalShift(Pd, Pshift,
                                ...
                                CVLog::Warning(
                                        "[VTKFilter::loadFile] Cloud has been "
                                        "recentered! Translation: (%.2f ; %.2f "
                                        "; %.2f)",
            ...
            assert(mesh);
            if (!mesh) {
                CVLog::Warning(QString("[VTK] We found %1 data while file is "
                                       "not composed of POLYDATA!")
                                       .arg(typeName));
                mesh = new ccMesh(
                        vertices);  // however, we can still try to load it?
            }
            ...
            for (unsigned i = 0; i < elemCount; ++i) {
                ...
                if (!ok || static_cast<int>(vertCount) >= parts.size()) {
                    ...
                    CVLog::Warning(QString("[VTK] Element #%1 of %2 data is "
                                           "corrupted! (not enough indexes)")
                                           .arg(i)
                                           .arg(typeName));
                }
                ...
                if (isPolygon &&
                    (vertCount != 3 &&
                     vertCount != 4))  // quads are easy to handle as well!
                {
                    CVLog::Warning(QString("[VTK] POLYGON element #%1 has an "
                                           "unhandled size (> 4 vertices)")
                                           .arg(i));
                    continue;
                }
                ...
                for (unsigned j = 0; j < vertCount; ++j) {
                    ...
                    if (!ok) {
                        CVLog::Warning(
                                QString("[VTK] Element #%1 of %2 data is "
                                        "corrupted! (invalid index value)")
                                        .arg(i)
                                        .arg(typeName));
                        error = CC_FERR_MALFORMED_FILE;
                        break;
                ...
                // add the triangles
                {
                    ...
                    if (mesh->size() + triCount > mesh->capacity()) {
                        if (!mesh->reserve(mesh->size() + triCount +
                                           256))  // take some advance to avoid
                    ...
                    if (isPolygon) {
                        ...
                        if (vertCount == 4)
                            mesh->addTriangle(indexes[0], indexes[2],
                                              indexes[3]);
                    } else {
                        // triangle strip
                        for (unsigned j = 0; j < triCount; ++j)
                            mesh->addTriangle(indexes[j], indexes[j + 1],
                                              indexes[j + 2]);
                    }
                }
            }
            ...
            if (lastDataSize == 0) lastDataSize = vertices->size();
            if (lastDataSize == 0) {
                error = CC_FERR_MALFORMED_FILE;
                break;
            }
            ...
            while (iNorm < lastDataSize) {
                ...
                for (int i = 0; i < parts.size(); ++i) {
                    bool ok;
                    N.u[coordIndex] = static_cast<PointCoordinateType>(
                            parts[i].toDouble(&ok));
                    if (!ok) {
                        CVLog::Warning(
                                "[VTK] Element #%1 of NORMALS data is "
                                "corrupted!",
                                iNorm);
                        error = CC_FERR_MALFORMED_FILE;
                        iNorm = lastDataSize;
            ...
            if (!loadRGBColors)
                CVLog::Warning("[VTK] Not enough memory to load RGB colors!");
            ...
            while (iCol < lastDataSize) {
                ...
                for (int i = 0; i < parts.size(); ++i) {
                    bool ok;
                    rgb.rgb[coordIndex] = static_cast<ColorCompType>(
                            parts[i].toDouble(&ok) * ecvColor::MAX);
                    if (!ok) {
                        CVLog::Warning(
                                "[VTK] Element #%1 of COLOR_SCALARS data is "
                                "corrupted!",
                                iCol);
                        error = CC_FERR_MALFORMED_FILE;
                        iCol = lastDataSize;
                    ...
                    if (coordIndex == 2) {
                        if (loadRGBColors) vertices->addRGBColor(rgb);
                        coordIndex = 0;
                        ++iCol;
                    } else {
                        ++coordIndex;
            ...
            // SF already exists?
            if (vertices->getScalarFieldIndexByName(qPrintable(lastSfName)) >=
                0)
                lastSfName += QString(" (%1)").arg(
                        vertices->getNumberOfScalarFields());
            // end of SCALARS
        } else if (nextline.startsWith("LOOKUP_TABLE") ||
                   nextline.startsWith("VECTORS")) {
            bool expected = (lastDataSize != 0);
            assert(!acceptLookupTables ||
            ...
            if (parts.size() > 2) {
                ...
                if (ok) lastDataSize = valCount;
            } else if (!expected) {
                CVLog::Warning(
                        QString("[VTK] field %1 has no size?!").arg(itemName));
                error = CC_FERR_MALFORMED_FILE;
                break;
            }

            bool createSF =
                    (vertices->size() == lastDataSize && vertices->size() != 0);
            if (acceptLookupTables && !createSF) {
                CVLog::Warning(QString("[VTK] field %1 has not the right "
                                       "number of points (will be ignored)")
                                       .arg(itemName));
            }
            createSF &= (acceptLookupTables || expected);
            if (createSF && lastSfName.isNull()) {
                CVLog::Warning(
                        QString("[VTK] field %1 has no name (will be ignored)")
                                .arg(itemName));
                createSF = false;
            } else if (!expected) {
                CVLog::Warning(QString("[VTK] field %1 was not expected (will "
                                       "be ignored)")
                                       .arg(itemName));
            }
            ...
            while (iScal < lastDataSize) {
                ...
                if (expected) {
                    for (int i = 0; i < parts.size(); ++i) {
                        bool ok;
                        ScalarType d =
                                static_cast<ScalarType>(parts[i].toDouble(&ok));
                        if (!ok) {
                            CVLog::Warning(
                                    "[VTK] Element #%1 of LOOKUP_TABLE/VECTORS "
                                    "data is corrupted!",
                                    iScal);
                            error = CC_FERR_MALFORMED_FILE;
                            if (sf) {
                        ...
                        if (sf) sf->addElement(d);
                        ++iScal;
                    }
                } else {
                    // hard to guess the right format, but an unexpected field
                    // seem to always be organized as 'one element per line'
                    ++iScal;
                }
            }
            lastDataSize = 0;  // lastDataSize is "consumed"
            ...
            elements *= 2;  // we don't know how to handle those properly but at
                            // least
        ...
        } else  // unhandled property (CELLS, CELL_TYPES, etc.)
        {
            ...
            if (nextline.startsWith("CELL_DATA")) {
                ...
                if (nextline.startsWith("SCALARS") ||
                    ...
                    acceptLookupTables =
                            false;  // this property is for triangles!
                    continue;
                }
            }
        ...
        if (error != CC_FERR_NO_ERROR) break;
    }
    ...
    if (vertices && vertices->size() == 0) {
        delete vertices;
        vertices = 0;
        if (error == CC_FERR_NO_ERROR) error = CC_FERR_NO_LOAD;
    }
    ...
    if (mesh) {
        ...
        vertices->setLocked(true);  // DGM: no need to lock it as it is only
                                    // used by one mesh!

        // DGM: normals can be per-vertex or per-triangle so it's better to let
        // the user do it himself later Moreover it's not always good idea if
        // the user doesn't want normals (especially in ccViewer!)
        if (!mesh->hasNormals()) {
            //	mesh->computeNormals();
            CVLog::Warning(
                    "[VTK] Mesh has no normal! You can manually compute them "
        ...
        if (vertices->hasColors()) mesh->showColors(true);
    } else if (vertices) {
