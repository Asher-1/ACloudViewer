...
PTXFilter::PTXFilter()
    : FileIOFilter({"_PTX Filter",
                    5.0f,  // priority
                    QStringList{"ptx"}, "ptx", QStringList{"PTX cloud (*.ptx)"},
                    QStringList(), Import}) {}

void CleanMatrix(ccGLMatrixd& mat) {
    // make the transform a little bit cleaner (necessary as it's read from
    // ASCII!)
    {
        // #ifdef QT_DEBUG
...
CC_FILE_ERROR PTXFilter::loadFile(const QString& filename,
    ...
    for (unsigned cloudIndex = 0;
        ...
        // read header
        {
            ...
            if (line.isNull() &&
                container.getChildrenNumber() != 0)  // end of file?
                break;

            // read the width (number of columns) and the height (number of
            // rows) on the two first lines (DGM: we transpose the matrix right
            // away)
            bool ok;
            height = line.toUInt(&ok);
            if (!ok) return CC_FERR_MALFORMED_FILE;
            line = inFile.readLine();
            width = line.toUInt(&ok);
            if (!ok) return CC_FERR_MALFORMED_FILE;

            CVLog::Print(QString("[PTX] Scan #%1 - grid size: %2 x %3")
                                 .arg(cloudIndex + 1)
                                 .arg(height)
                                 .arg(width));
            ...
            // make the transform a little bit cleaner (necessary as it's read
            // from ASCII!)
            ...
            if (cloudIndex == 0) {
                if (HandleGlobalShift(cloudTransD.getTranslationAsVec3D(),
                                      PshiftTrans, preserveCoordinateShift,
                                      parameters)) {
                    CVLog::Warning(
                            "[PTXFilter::loadFile] Cloud has be recentered! "
                            "Translation: (%.2f ; %.2f ; %.2f)",
                            PshiftTrans.x, PshiftTrans.y, PshiftTrans.z);
                }
            }

            //'remove' global shift from the sensor and cloud transformation
            // matrices
            cloudTransD.setTranslation(cloudTransD.getTranslationAsVec3D() +
                                       PshiftTrans);
            sensorTransD.setTranslation(sensorTransD.getTranslationAsVec3D() +
                                        PshiftTrans);
        }
        ...
        if (container.getChildrenNumber() == 0) {
            ...
            if (container.getChildrenNumber() == 1) {
                container.getChild(0)->setName(
                        "unnamed - Cloud 1");  // update previous cloud name!
            }
            cloud->setName(QString("unnamed - Cloud %1")
                                   .arg(container.getChildrenNumber() + 1));
        }
        ...
        // intensities
        ccScalarField* intensitySF =
                new ccScalarField(CC_PTX_INTENSITY_FIELD_NAME);
        if (!intensitySF->reserveSafe(static_cast<unsigned>(gridSize))) {
            CVLog::Warning("[PTX] Not enough memory to load intensities!");
            intensitySF->release();
            intensitySF = nullptr;
        }
        ...
        // read points
        {
            cloudViewer::NormalizedProgress nprogress(pDlg.data(), gridSize);
            if (pDlg) {
                pDlg->setInfo(qPrintable(
                        QString("Number of cells: %1").arg(gridSize)));
                pDlg->start();
            }
                ...
                for (unsigned i = 0; i < width; ++i, ++gridIndex) {
                    ...
                    if (firstPoint) {
                        ...
                        if (hasColors) {
                            ...
                            if (!loadColors) {
                                CVLog::Warning(
                                        "[PTX] Not enough memory to load RGB "
                                        "colors!");
                            } else if (hasIndexGrid) {
                                // we also load the colors into the grid (as
                                // invalid/missing points can have colors!)
                                try {
                                    grid->colors.resize(gridSize,
                                                        ecvColor::Rgb(0, 0, 0));
                                    loadGridColors = true;
                                } catch (const std::bad_alloc&) {
                                    CVLog::Warning(
                                            "[PTX] Not enough memory to load "
                                            "the grid colors");
                                }
                            }
                        }
                    }
                    if ((hasColors && tokens.size() != 7) ||
                        (!hasColors && tokens.size() != 4)) {
                    ...
                    // we skip "empty" cells
                    bool pointIsValid =
                            (CCVector3d::fromArray(values).norm2() != 0);
                    if (pointIsValid) {
                        const double* Pd = values;
                        // first point: check for 'big' coordinates
                        if (firstPoint) {
                            if (cloudIndex == 0 &&
                                !cloud->isShifted())  // in case the trans.
                                                      // matrix was ok!
                            {
                                CCVector3d P(Pd);
                                if (HandleGlobalShift(P, PshiftCloud,
                                    ...
                                    CVLog::Warning(
                                            "[PTXFilter::loadFile] Cloud has "
                                            "been recentered! Translation: "
                                            "(%.2f ; %.2f ; %.2f)",
                                            PshiftCloud.x, PshiftCloud.y,
                                            PshiftCloud.z);
                                }
                            }
                            firstPoint = false;
                        ...
                        // update index grid
                        if (hasIndexGrid) {
                            grid->indexes[gridIndex] = static_cast<int>(
                                    cloud->size());  // = index (default value =
                                                     // -1, means no point)
                        }

                        // add point
                        cloud->addPoint(CCVector3(
                                static_cast<PointCoordinateType>(Pd[0] +
                                                                 PshiftCloud.x),
                                static_cast<PointCoordinateType>(Pd[1] +
                                                                 PshiftCloud.y),
                        ...
                        // add intensity
                        if (intensitySF) {
                            intensitySF->addElement(
                                    static_cast<ScalarType>(values[3]));
                        }
                    }
        ...
        if (cloud->size() == 0) {
            ...
            CVLog::Warning(
                    QString("[PTX] Scan #%1 is empty?!").arg(cloudIndex + 1));
        } else {
            if (result == CC_FERR_NO_LOAD)
                result = CC_FERR_NO_ERROR;  // to make clear that we have loaded
                                            // at least something!
            ...
            if (intensitySF) {
                ...
                if (container.getChildrenNumber() == 0) {
                    ...
                    minIntensity =
                            std::min(minIntensity, intensitySF->getMin());
                    maxIntensity =
                            std::max(maxIntensity, intensitySF->getMax());
                }
            ...
            if (hasIndexGrid && result != CC_FERR_CANCELED_BY_USER) {
                ...
                ccGLMatrix cloudToSensorTrans(
                        (sensorTransD.inverse() * cloudTransD).data());
                sensor = ccGriddedTools::ComputeBestSensor(cloud, grid,
                                                           &cloudToSensorTrans);
            }
            ...
            if (hasIndexGrid) {
                ...
                if (parameters.autoComputeNormals) {
                    cloud->computeNormalsWithGrids(1.0,
                                                   normalsProgressDlg.data());
                }
            }
    ...
    // update scalar fields saturation (globally!)
    {
        ...
        if (minIntensity < 0 || maxIntensity > 1.0) {
            CVLog::Warning(
                    "[PTX] Intensity values are invalid (they should all fall "
                    "in [0 ; 1])");
        ...
        for (unsigned i = 0; i < container.getChildrenNumber(); ++i) {
            ...
            assert(obj && obj->isA(CV_TYPES::POINT_CLOUD));
            cloudViewer::ScalarField* sf =
                    static_cast<ccPointCloud*>(obj)->getScalarField(0);
            if (sf) {
                ccScalarField* ccSF = static_cast<ccScalarField*>(sf);
                ccSF->setColorScale(ccColorScalesManager::GetDefaultScale(
                        validIntensityRange
