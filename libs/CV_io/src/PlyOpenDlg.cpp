...
void PlyOpenDlg::ResetApplyAll() { s_lastContext.applyAll = false; }

bool PlyOpenDlg::restorePreviousContext(bool& hasAPreviousContext) {
    hasAPreviousContext = s_lastContext.valid;
    if (!hasAPreviousContext) return false;

    int unassignedProps = 0;
    int mismatchProps = 0;
    bool restored =
            restoreContext(&s_lastContext, unassignedProps, mismatchProps);
...
void PlyOpenDlg::saveContext(PlyLoadingContext* context) {
    ...
    if (m_standardCombos.front())
        for (int i = 1; i < m_standardCombos.front()->count();
             ++i)  // the first item is always 'NONE'
            context->allProperties.append(
                    m_standardCombos.front()->itemText(i));
    assert(m_listCombos.front());
    if (m_listCombos.front())
        for (int i = 1; i < m_listCombos.front()->count();
             ++i)  // the first item is always 'NONE'
            context->allProperties.append(m_listCombos.front()->itemText(i));
    assert(m_singleCombos.front());
    if (m_singleCombos.front())
        for (int i = 1; i < m_singleCombos.front()->count();
             ++i)  // the first item is always 'NONE'
    ...
    try {
        // standard combos
        {
            ...
            std::fill(context->standardCombosProperties.begin(),
                      context->standardCombosProperties.end(), QString());
            for (size_t i = 0; i < m_standardCombos.size(); ++i) {
                if (m_standardCombos[i] &&
                    m_standardCombos[i]->currentIndex() >
                            0)  // currentIndex == 0 means 'NONE'!!!
                {
                    context->standardCombosProperties[i] =
                            m_standardCombos[i]->currentText();
                    ++assignedProps;
                }
            }
        }
        // list combos
        {
            ...
            std::fill(context->listCombosProperties.begin(),
                      context->listCombosProperties.end(), QString());
            for (size_t i = 0; i < m_listCombos.size(); ++i) {
                if (m_listCombos[i] && m_listCombos[i]->currentIndex() > 0) {
                    context->listCombosProperties[i] =
                            m_listCombos[i]->currentText();
                    ++assignedProps;
                }
            }
        }
        // single combos
        {
            ...
            std::fill(context->singleCombosProperties.begin(),
                      context->singleCombosProperties.end(), QString());
            for (size_t i = 0; i < m_singleCombos.size(); ++i) {
                if (m_singleCombos[i] &&
                    m_singleCombos[i]->currentIndex() > 0) {
                    context->singleCombosProperties[i] =
                            m_singleCombos[i]->currentText();
                    ++assignedProps;
                }
            }
        }
        // additional SF combos
        {
            ...
            for (size_t i = 0; i < m_sfCombos.size(); ++i) {
                ...
                if (m_sfCombos[i] && m_sfCombos[i]->currentIndex() > 0) {
                    context->sfCombosProperties.push_back(
                            m_sfCombos[i]->currentText());
                    ++assignedProps;
                }
            }
        }
...
bool PlyOpenDlg::restoreContext(PlyLoadingContext* context,
    ...
    int totalProps = 0;
    {
        ...
        if (m_standardCombos.front())
            ...
            {
                ++totalProps;
                if (!context->allProperties.contains(
                            m_standardCombos.front()->itemText(i)))
                    ++mismatchProps;
            }
        assert(m_listCombos.front());
        ...
        if (m_singleCombos.front())
                 ...
                 ++i)  // the first item is always 'NONE'
            {
                ++totalProps;
                if (!context->allProperties.contains(
                            m_singleCombos.front()->itemText(i)))
                    ++mismatchProps;
            }
    }
        ...
        for (size_t i = 0; i < m_standardCombos.size(); ++i) {
            if (m_standardCombos[i]) {
                ...
                if (!context->standardCombosProperties[i].isNull()) {
                    ...
                    int idx = m_standardCombos[i]->findText(
                            context->standardCombosProperties[i]);
                    if (idx >= 0) {
                        ++assignedEntries;
        ...
        for (size_t i = 0; i < m_listCombos.size(); ++i) {
            if (m_listCombos[i]) {
                ...
                if (!context->listCombosProperties[i].isNull()) {
                    // try to find it in the new property list!
                    int idx = m_listCombos[i]->findText(
                            context->listCombosProperties[i]);
                    if (idx >= 0) {
                        ++assignedEntries;
        ...
        for (size_t i = 0; i < m_singleCombos.size(); ++i) {
            if (m_singleCombos[i]) {
                ...
                if (!context->singleCombosProperties[i].isNull()) {
                    // try to find it in the new property list!
                    int idx = m_singleCombos[i]->findText(
                            context->singleCombosProperties[i]);
                    if (idx >= 0) {
                        ++assignedEntries;
...
bool PlyOpenDlg::isValid(bool displayErrors /*=true*/) const {
    ...
    for (size_t j = 0; j < m_listCombos.size(); ++j)
        ++assignedIndexCount[m_listCombos[j]->currentIndex() > 0
                                     ? n + m_listCombos[j]->currentIndex()
                                     : 0];
    for (size_t k = 0; k < m_singleCombos.size(); ++k)
        ++assignedIndexCount[m_singleCombos[k]->currentIndex() > 0
                                     ? n + p + m_singleCombos[k]->currentIndex()
                                     : 0];
