...
static inline bool isValidESRIShapeCode(int32_t code) {
    if (code < static_cast<int32_t>(ESRI_SHAPE_TYPE::NULL_SHAPE)) return false;
    if (code > static_cast<int32_t>(ESRI_SHAPE_TYPE::MULTI_PATCH)) return false;
...
static bool isValidEsriPartType(int32_t code) {
    if (code < static_cast<int32_t>(ESRI_PART_TYPE::TRIANGLE_STRIP))
        return false;
    if (code > static_cast<int32_t>(ESRI_PART_TYPE::RING)) return false;
...
static inline bool isESRINoData(double m) { return m <= ESRI_NO_DATA; }
...
static int32_t sizeofPolyLine(ESRI_SHAPE_TYPE polylineType,
                              size_t numPoints,
                              size_t numParts = 1) {
    switch (polylineType) {
        case ESRI_SHAPE_TYPE::POLYLINE:
...
static bool areVerticesCounterClockwise(
        const cloudViewer::GenericIndexedCloudPersist *vertices,
        int32_t numPoints,
        unsigned char dim1,
        unsigned char dim2) {
    ...
    for (int32_t i = 0; i + 1 < numPoints; ++i) {
        ...
        sum += (static_cast<double>(P2.u[dim1]) -
                static_cast<double>(P1.u[dim1])) *
               (static_cast<double>(P2.u[dim2]) +
                static_cast<double>(P1.u[dim2]));
    }
...
static CCVector2d minMaxOfEnabledScalarField(
        const cloudViewer::GenericIndexedCloudPersist *cloud) {
    CCVector2d minMax(std::numeric_limits<double>::max(),
...
static ccHObject::GlobalBoundingBox bBoxOfHObjectContainer(
        const ccHObject::Container &objects) {
...
CCVector2d mRangeOfContainer(ccHObject::Container &objects) {
    CCVector2d range(std::numeric_limits<double>::max(),
                     std::numeric_limits<double>::min());

    auto updateRange =
            [&range](const cloudViewer::GenericIndexedCloudPersist *cloud) {
                if (!cloud->isScalarFieldEnabled()) return;
...
CC_FILE_ERROR ShapeFileHeader::readFrom(QDataStream &sin) {
    ...
    if (fileCode != ESRI_SHAPE_FILE_CODE) {
        CVLog::Warning("[SHP] wrong file code (%d), is this a shape file?",
                       fileCode);
    ...
    if (!isValidESRIShapeCode(shapeTypeInt)) {
        CVLog::Warning("[SHP] invalid shape type code in header (%d)",
                       shapeTypeInt);
        return CC_FERR_MALFORMED_FILE;
    }
    ...
    if (sin.status() != QDataStream::Ok) {
        CVLog::Warning("[SHP] Something went wrong reading the shp header");
...
ShpFilter::ShpFilter()
    : FileIOFilter(
              {"_Shape Filter",
               14.0f,  // priority
               QStringList{"shp"}, "shp", QStringList{"SHP entity (*.shp)"},
               QStringList{"SHP entity (*.shp)"}, Import | Export | BuiltIn}) {}

bool ShpFilter::canSave(CV_CLASS_ENUM type,
                        bool &multiple,
                        bool &exclusive) const {
    if (type == CV_TYPES::POLY_LINE || type == CV_TYPES::POINT_CLOUD ||
        type == CV_TYPES::HIERARCHY_OBJECT || type == CV_TYPES::MESH) {
        multiple = true;
        exclusive = true;
        return true;
    }
...
static void GetSupportedShapes(ccHObject *baseEntity,
                               ccHObject::Container &shapes,
    ...
    switch (baseEntity->getClassID()) {
        case CV_TYPES::POINT_CLOUD: {
            unsigned count =
                    ccHObjectCaster::ToGenericPointCloud(baseEntity)->size();
            if (count != 0) {
        ...
        case CV_TYPES::HIERARCHY_OBJECT:
            ...
            if (baseEntity->getChildrenNumber()) {
                ...
                for (unsigned i = 1; i < baseEntity->getChildrenNumber(); ++i) {
                    if (baseEntity->getChild(i) &&
                        baseEntity->getChild(i)->getClassID() !=
                                child->getClassID()) {
                ...
                if (shapeType == ESRI_SHAPE_TYPE::NULL_SHAPE) return;

                // then add the remaining children
                for (unsigned i = 1; i < baseEntity->getChildrenNumber(); ++i) {
                    ESRI_SHAPE_TYPE otherShapeType =
                            ESRI_SHAPE_TYPE::NULL_SHAPE;
                    ...
                    if (otherShapeType != shapeType) {
                        if (child)
                            CVLog::Warning(
                                    QString("[SHP] Entity %1 has not the same "
                                            "type (%2) as the others in the "
                                            "selection (%3)! Can't mix "
                                            "types...")
                                            .arg(child->getName())
...
static std::vector<int32_t> readParts(QDataStream &shpStream,
                                      int32_t numParts) {
...
static std::vector<CCVector3> readPoints(QDataStream &shpStream,
                                         int32_t numPoints,
...
static std::vector<ScalarType> readMeasures(QDataStream &shpStream,
                                            int32_t numPoints) {
    ...
    if (!scalarValues.empty()) {
        for (int32_t i = 0; i < numPoints; ++i) {
            ...
            shpStream >> m;
            scalarValues[i] =
                    (isESRINoData(m) ? NAN_VALUE : static_cast<ScalarType>(m));
        }
    } else {
...
ccPointCloud *buildVertices(const std::vector<CCVector3> &points,
                            int32_t firstIndex,
                            int32_t lastIndex) {
...
//! Creates the ccMesh
ccMesh *createMesh(const std::vector<CCVector3> &points,
    ...
    if (!scalarValues.empty()) {
        ...
        if (firstIndex < scalarValues.size() &&
            lastIndex < scalarValues.size()) {
            areAllValuesOfPartsNans =
                    std::all_of(scalarValues.begin() + firstIndex,
                                scalarValues.begin() + lastIndex,
                                [](ScalarType val) { return std::isnan(val); });
        }
        if (!areAllValuesOfPartsNans) {
            auto *sf = new ccScalarField("Measures");
            if (!sf->reserveSafe(vertCount)) {
                CVLog::Warning(
                        QString("[SHP] Mesh: not enough memory to load scalar "
                                "values!"));
                sf->release();
            } else {
                for (int32_t j = 0; j < vertCount; ++j) {
...
CC_FILE_ERROR buildPatches(ccHObject &container,
    ...
    for (int32_t i = 0; i < numParts; ++i) {
        if (!isValidEsriPartType(partTypes[i])) {
            CVLog::Warning(
                    "[SHP] Multipatch part %d has an invalid part type (%d)", i,
                    partTypes[i]);
            continue;
        }
        ...
        const int32_t &lastIndex = static_cast<const int32_t &>(
                (i + 1 < numParts ? startIndexes[i + 1] : numPoints) - 1);
        const int32_t vertCount = lastIndex - firstIndex + 1;

        switch (type) {
            case ESRI_PART_TYPE::TRIANGLE_STRIP: {
                ccMesh *mesh =
                        createMesh(points, scalarValues, firstIndex, lastIndex);
            ...
            default:
                CVLog::Print(QString("[SHP] Cannot handle Patch of type: %1")
                                     .arg(ToString(type)));
...
static CC_FILE_ERROR LoadMultiPatch(QDataStream &shpStream,
    ...
    return buildPatches(container, startIndexes, partTypes, points,
                        scalarValues);
}
 ...
 */
static void save3DCloud(QDataStream &stream,
                        const ccGenericPointCloud *cloud,
                        const CCVector3d &bbMing,
                        const CCVector3d &bbMaxg) {
...
/**
 * Tells wether the mesh's vertices are organised in triangle fan or triangle
 * strip fashion.
 * @param mesh
 * @param[out] type
 * @return CC_FERR_BAD_ENTITY_TYPE if the mesh is neither a triangle fan or
 * triangle strip
 */
CC_FILE_ERROR findTriangleOrganisation(ccMesh *mesh, ESRI_PART_TYPE &type) {
    const cloudViewer::VerticesIndexes *firstVert =
            mesh->getNextTriangleVertIndexes();
    if (!isTriangleFan(firstVert) && !isTriangleStrip(firstVert))
        return CC_FERR_BAD_ENTITY_TYPE;

    const cloudViewer::VerticesIndexes *secondVert =
            mesh->getNextTriangleVertIndexes();
    if (isTriangleStrip(secondVert)) {
        for (unsigned i = 2; i < mesh->size(); ++i) {
            cloudViewer::VerticesIndexes *idx =
                    mesh->getNextTriangleVertIndexes();
            if (!isTriangleStrip(idx)) return CC_FERR_BAD_ENTITY_TYPE;
        }
        ...
        for (unsigned i = 2; i < mesh->size(); ++i) {
            cloudViewer::VerticesIndexes *idx =
                    mesh->getNextTriangleVertIndexes();
            if (!isTriangleFan(idx)) return CC_FERR_BAD_ENTITY_TYPE;
        }
...
CC_FILE_ERROR SaveMesh(ccMesh *mesh,
                       QDataStream &stream,
                       int32_t recordNumber,
                       int32_t &recordSize) {
    ESRI_PART_TYPE triangleType;
    if (findTriangleOrganisation(mesh, triangleType) ==
        CC_FERR_BAD_ENTITY_TYPE) {
        return CC_FERR_BAD_ENTITY_TYPE;
    }

    CVLog::Print(
            QString("[SHP] Triangle type: %1").arg(ToString(triangleType)));
    ...
    stream << globalBB.minCorner().x << globalBB.minCorner().y
           << globalBB.maxCorner().x << globalBB.maxCorner().y;
...
static CC_FILE_ERROR LoadPolyline(QDataStream &shpStream,
    ...
    // FIXME: we should use this information and create as many polylines as
    // necessary!
    ...
    if (hasMeasurements(shapeType)) {
        ...
        if (scalarValues.empty()) {
            ...
            CVLog::Warning(QString("[SHP] Polyline #%1: not enough memory to "
                                   "load scalar values!")
                                   .arg(index));
        }
    }
    ...
    for (int32_t i = 0; i < numParts; ++i) {
        ...
        if (vertCount > 2 &&
            cloudViewer::LessThanEpsilon(
                    (points[firstIndex] - points[lastIndex]).norm())) {
            vertCount--;
            isClosed = true;
        }
        ...
        if (preserveCoordinateShift) {
            poly->setGlobalShift(
                    Pshift);  // shouldn't be necessary but who knows ;)
        }
        ...
        if (numParts != 1) name += QString(".%1").arg(i + 1);
        poly->setName(name);
        ...
        if (!scalarValues.empty()) {
            ...
            if (!allNans) {
                auto *sf = new ccScalarField("Measures");
                if (!sf->reserveSafe(vertCount)) {
                    CVLog::Warning(QString("[SHP] Polyline #%1.%2: not enough "
                                           "memory to load scalar values!")
                                           .arg(index)
                                           .arg(i + 1));
                    sf->release();
                    sf = nullptr;
...
static CC_FILE_ERROR SavePolyline(ccPolyline *poly,
    ...
    switch (outputShapeType) {
        ...
        case ESRI_SHAPE_TYPE::POLYGON_Z:
            if (realVertexCount < 3) {
                CVLog::Warning(
                        QObject::tr("[SHP] Polyline %1 does not have enough "
                                    "vertices to be saved as polygon entity")
                                .arg(poly->getName()));
                return CC_FERR_BAD_ENTITY_TYPE;
            }
            break;
        ...
        case ESRI_SHAPE_TYPE::POLYLINE_Z:
            if (realVertexCount < 2) {
                CVLog::Warning(
                        QObject::tr("[SHP] Polyline %1 does not have enough "
                                    "vertices to be saved as polyline entity")
                                .arg(poly->getName()));
                return CC_FERR_BAD_ENTITY_TYPE;
            }
            break;
    ...
    if (static_cast<int64_t>(realVertexCount) + 1 >
        std::numeric_limits<int32_t>::max()) {
        CVLog::Warning(
                QObject::tr("[SHP] Polyline %1 has too many points to be saved")
                        .arg(poly->getName()));
        return CC_FERR_BAD_ENTITY_TYPE;
    }
    ...
    // Ymax (24 bytes)
    out << globalBB.minCorner().u[X] << globalBB.minCorner().u[Y]
        << globalBB.maxCorner().u[X] << globalBB.maxCorner().u[Y];
    ...
    if (outputShapeType == ESRI_SHAPE_TYPE::POLYGON ||
        ...
        if (outputShapeType == ESRI_SHAPE_TYPE::POLYGON_Z) {
            ...
            if (diag.u[2] < diag.u[minDim]) minDim = 2;
            dim1 = minDim == 2 ? 0 : minDim + 1;
            dim2 = dim1 == 2 ? 0 : dim1 + 1;
        }

        inverseOrder =
                areVerticesCounterClockwise(vertices, numPoints, dim1, dim2);
    }

    // Points (An array of length NumPoints)
    for (int32_t i = 0; i < numPoints; ++i) {
        int32_t ii = (inverseOrder ? numPoints - 1 - i : i);
        const CCVector3 *P = vertices->getPoint(
                ii % iRealVertexCount);  // warning: handle loop if polyline is
                                         // closed
    ...
    if (isESRIShape3D(outputShapeType)) {
        ...
        for (int32_t i = 0; i < numPoints; ++i) {
            int32_t ii = (inverseOrder ? numPoints - 1 - i : i);
            const CCVector3 *P = vertices->getPoint(
                    ii % iRealVertexCount);  // warning: handle loop if polyline
                                             // is closed
            CCVector3d Pg = poly->toGlobal3d(*P);
            out << Pg.u[Z];
        }
    ...
    if (hasMeasurements(outputShapeType)) {
        ...
        if (hasSF) {
            for (int32_t i = 0; i < numPoints; ++i) {
                ScalarType scalar =
                        vertices->getPointScalarValue(i % iRealVertexCount);
                out << (ccScalarField::ValidValue(scalar)
                                ? ESRI_NO_DATA
...
static CC_FILE_ERROR LoadCloud(QDataStream &shpStream,
    ...
    for (int32_t i = 0; i < numPoints; ++i) {
        ...
        CCVector3 P(static_cast<PointCoordinateType>(x + Pshift.x),
                    static_cast<PointCoordinateType>(y + Pshift.y), 0);
        cloud->addPoint(P);
    }
    ...
    if (isESRIShape3D(shapeType)) {
        ...
        for (int32_t i = 0; i < numPoints; ++i) {
            ...
            const_cast<CCVector3 *>(P)->z =
                    static_cast<PointCoordinateType>(z + Pshift.z);
        }
    ...
    if (hasMeasurements(shapeType)) {
        ...
        if (mMin != ESRI_NO_DATA && mMax != ESRI_NO_DATA) {
            ...
            if (!sf->reserveSafe(numPoints)) {
                CVLog::Warning(
                        "[SHP] Not enough memory to load scalar values!");
                sf->release();
                sf = nullptr;
            }
        }
        ...
        if (sf) {
            for (int32_t i = 0; i < numPoints; ++i) {
                double m;
                shpStream >> m;
                ScalarType s = isESRINoData(m) ? NAN_VALUE
                                               : static_cast<ScalarType>(m);
                sf->addElement(s);
            }
            bool allNans =
                    std::all_of(sf->begin(), sf->end(),
                                [](ScalarType s) { return std::isnan(s); });
            if (!allNans) {
                sf->computeMinAndMax();
...
static CC_FILE_ERROR SaveAsCloud(ccGenericPointCloud *cloud,
                                 QDataStream &out,
                                 int32_t recordNumber,
                                 int32_t &recordSize) {
    if (!cloud) {
    ...
    if (cloud->size() >
        ...
        CVLog::Print("[SHP] Cloud is to big to be saved");
        return CC_FERR_BAD_ENTITY_TYPE;
    }
    ...
    out << globalBB.minCorner().x << globalBB.minCorner().y
        << globalBB.maxCorner().x << globalBB.maxCorner().y;
...
static CC_FILE_ERROR LoadSinglePoint(QDataStream &shpStream,
    ...
    // make sure to reserve the point cloud memory AFTER declaring the scalar
    // field (otherwise the SF won't be reserved...)
    if (singlePoints->size() == singlePoints->capacity() &&
        !singlePoints->reserve(singlePoints->size() +
                               256))  // 256 each time because it appears some
                                      // SHP files have many isolated points...
    {
        delete singlePoints;
...
CC_FILE_ERROR ShpFilter::saveToFile(ccHObject *entity,
...
CC_FILE_ERROR ShpFilter::saveToFile(
        ccHObject *entity,
        const std::vector<GenericDBFField *> &fields,
        const QString &filename,
        const SaveParameters &parameters) {
    if (!entity) return CC_FERR_BAD_ENTITY_TYPE;
    ...
    if (parameters.alwaysDisplaySaveDialog &&
        ...
        ssfDlg.save3DPolyHeightInDBFCheckBox->setChecked(
                m_save3DPolyHeightInDBF);
        ssfDlg.dimComboBox->setCurrentIndex(s_poly2DVertDim);

        if (!ssfDlg.exec()) return CC_FERR_CANCELED_BY_USER;
        ...
        save3DPolyHeightInDBF =
                ssfDlg.save3DPolyHeightInDBFCheckBox->isChecked();
    }
    ...
    // Promote to polygon
    if (m_closedPolylinesAsPolygons &&
        outputShapeType == ESRI_SHAPE_TYPE::POLYLINE_Z) {
        auto isClosed = [](const ccHObject *obj) {
            return static_cast<const ccPolyline *>(obj)->isClosed();
        };
    ...
    // Demote to 2D polyline/polygon and remove "measure" dimension if not
    // needed
    if (save3DPolysAs2D) {
    ...
    CVLog::Print("[SHP] Output type: " + ToString(outputShapeType));
    ...
    if (!file.open(QIODevice::WriteOnly)) return CC_FERR_WRITING;

    // index file (same base name + ".shx")
    QString indexFilename = baseFileName + QString(".shx");
    QFile indexFile(indexFilename);
    if (!indexFile.open(QIODevice::WriteOnly)) return CC_FERR_WRITING;
    ...
    ShapeFileHeader hdr;
    hdr.pointMin =
            CCVector3d(globalBB.minCorner().u[X], globalBB.minCorner().u[Y],
                       globalBB.minCorner().u[Z]);
    hdr.pointMax =
            CCVector3d(globalBB.maxCorner().u[X], globalBB.maxCorner().u[Y],
                       globalBB.maxCorner().u[Z]);
    ...
    for (ccHObject *child : toSave) {
        ...
        if (child->isA(CV_TYPES::POLY_LINE)) {
            if (static_cast<ccPolyline *>(child)->size() < 2) {
                CVLog::Warning(QString("Polyline '%1' is too small! It won't "
                                       "be saved...")
                                       .arg(child->getName()));
                continue;
            }
        ...
        switch (outputShapeType) {
            ...
            case ESRI_SHAPE_TYPE::POLYGON_Z:
            case ESRI_SHAPE_TYPE::POLYGON_M:
                assert(child->isKindOf(CV_TYPES::POLY_LINE));
                error = SavePolyline(static_cast<ccPolyline *>(child),
                                     shpStream, recordSize, shapeIndex,
                                     outputShapeType, s_poly2DVertDim);
                break;
            case ESRI_SHAPE_TYPE::MULTI_POINT_Z:
                assert(child->isKindOf(CV_TYPES::POINT_CLOUD));
                error = SaveAsCloud(ccHObjectCaster::ToGenericPointCloud(child),
                                    shpStream, shapeIndex, recordSize);
                break;
            case ESRI_SHAPE_TYPE::MULTI_PATCH:
                error = SaveMesh(ccHObjectCaster::ToMesh(child), shpStream,
                                 shapeIndex, recordSize);
                break;
            default:
                assert(false);
                break;
        ...
        if (error != CC_FERR_NO_ERROR) return error;
        ...
        CVLog::PrintDebug("[SHP] Saved shape #%d (%d bytes)", shapeIndex,
                          recordSize * 2);
        shapeIndex++;
    }
    ...
    if (dbfHandle) {
        while (true)  // trick: we use 'while' to be able to break anytime
        {
            ...
            {
                int fieldIdx =
                        DBFAddField(dbfHandle, "local_idx", FTInteger, 6, 0);
                if (fieldIdx >= 0) {
                    for (size_t i = 0; i < toSave.size(); ++i)
                        DBFWriteIntegerAttribute(dbfHandle, static_cast<int>(i),
                                                 fieldIdx,
                                                 static_cast<int>(i) + 1);
                } else {
                    CVLog::Warning(QString(
                            "[SHP] Failed to save field 'index' (default)"));
            ...
            if (save3DPolyHeightInDBF) {
                ...
                if (fieldIdx >= 0) {
                    for (size_t i = 0; i < toSave.size(); ++i) {
                        ...
                        DBFWriteDoubleAttribute(dbfHandle, static_cast<int>(i),
                                                fieldIdx, height);
                    }
                } else {
                    CVLog::Warning(
                            QString("[SHP] Failed to save field 'height' (3D "
                                    "polylines height)"));
                    result = CC_FERR_WRITING;
                    break;
                }
            ...
            for (GenericDBFField *field : fields) {
                ...
                {
                    int xFieldIdx = DBFAddField(
                            dbfHandle,
                            qPrintable(field->name() + QString("_x")),
                            field->type(), field->width(), field->decimal());
                    int yFieldIdx = DBFAddField(
                            dbfHandle,
                            qPrintable(field->name() + QString("_y")),
                            field->type(), field->width(), field->decimal());
                    int zFieldIdx = DBFAddField(
                            dbfHandle,
                            qPrintable(field->name() + QString("_z")),
                            field->type(), field->width(), field->decimal());
                    if (xFieldIdx >= 0 && yFieldIdx >= 0 && zFieldIdx >= 0) {
                        if (!field->save(dbfHandle, xFieldIdx, yFieldIdx,
                                         zFieldIdx))
                            xFieldIdx = -1;
                    }
                    ...
                    if (xFieldIdx < 0) {
                        CVLog::Warning(
                                QString("[SHP] Failed to save field '%1'")
                                        .arg(field->name()));
                        result = CC_FERR_WRITING;
                        break;
                    }
                ...
                {
                    int fieldIdx = DBFAddField(
                            dbfHandle, qPrintable(field->name()), field->type(),
                            field->width(), field->decimal());
                    if (fieldIdx >= 0) {
                        if (!field->save(dbfHandle, fieldIdx)) fieldIdx = -1;
                    }
...
CC_FILE_ERROR ShpFilter::loadFile(const QString &filename,
                                  ccHObject &container,
                                  LoadParameters &parameters) {
    QFile file(filename);
    if (!file.open(QIODevice::ReadOnly)) return CC_FERR_READING;

    if (file.size() < ESRI_HEADER_SIZE) {
        CVLog::Warning("[SHP] File is too small to be valid");
        return CC_FERR_MALFORMED_FILE;
    }
    ...
    if (HandleGlobalShift(Pmin, Pshift, preserveCoordinateShift, parameters)) {
        CVLog::Warning(
                "[SHP] Entities will be recentered! Translation: (%.2f ; %.2f "
                "; %.2f)",
    ...
    while (hdr.fileLength - shpStream.device()->pos() > 0) {
        if (shpStream.status() != QDataStream::Ok) {
            CVLog::Warning("[SHP] Something went wrong reading the file");
            return CC_FERR_READING;
        }
        int32_t recordNumber;
        int32_t recordSize;
        ...
        if (!isValidESRIShapeCode(shapeTypeInt)) {
            CVLog::Warning("[SHP] Shape %d has an invalid shape code (%d)",
                           recordNumber, shapeTypeInt);
            return CC_FERR_READING;
        }
        auto shapeType = static_cast<ESRI_SHAPE_TYPE>(shapeTypeInt);

        if (recordNumber < 64)
            CVLog::Print(QString("[SHP] Record #%1 - type: %2 (%3 bytes)")
                                 .arg(recordNumber)
                                 .arg(ToString(shapeType))
                                 .arg(recordSize));
        else if (recordNumber == 64)
            CVLog::Print(
                    "[SHP] Records won't be displayed in the Console anymore "
        ...
        switch (shapeType) {
            ...
            case ESRI_SHAPE_TYPE::POLYGON_M: {
                ...
                error = LoadPolyline(shpStream, container, recordNumber,
                                     shapeType, Pshift,
                                     preserveCoordinateShift);
                if (error == CC_FERR_NO_ERROR &&
                    shapeType == ESRI_SHAPE_TYPE::POLYLINE) {
                    unsigned childCountAfter = container.getChildrenNumber();
                    // warning: we can load mutliple polylines for a single
                    // record!
                    for (unsigned i = childCountBefore; i < childCountAfter;
                         ++i) {
                        ccHObject *child = container.getChild(i);
                        assert(child && child->isA(CV_TYPES::POLY_LINE));
                        polyIDs[static_cast<ccPolyline *>(child)] =
                                recordNumber;
                        if (recordNumber > maxPolyID) maxPolyID = recordNumber;
                    }
                }
            } break;
            case ESRI_SHAPE_TYPE::MULTI_POINT_Z:
            case ESRI_SHAPE_TYPE::MULTI_POINT_M:
                is3DShape = true;
            case ESRI_SHAPE_TYPE::MULTI_POINT:
                error = LoadCloud(shpStream, container, recordNumber, shapeType,
                                  Pshift, preserveCoordinateShift);
                break;
            case ESRI_SHAPE_TYPE::POINT_Z:
            case ESRI_SHAPE_TYPE::POINT_M:
                is3DShape = true;
            case ESRI_SHAPE_TYPE::POINT:
                error = LoadSinglePoint(shpStream, singlePoints, shapeType,
                                        Pshift, preserveCoordinateShift);
            ...
            default:
                ...
                CVLog::Warning("[SHP] Unhandled type!");
                break;
        }
    ...
    // try to load the DBF to see if there's a 'height' field or something
    // similar for polylines
    ...
    if (!is3DShape && error == CC_FERR_NO_ERROR &&
        ...
        if (dbfHandle) {
            ...
            if (fieldCount == 0) {
                CVLog::Warning("[SHP] No field in the associated DBF file!");
            } else if (hasPolylines &&
                       recordCount < static_cast<int>(maxPolyID)) {
                CVLog::Warning(
                        "[SHP] No enough records in the associated DBF file!");
            } else if (hasPoints &&
                       recordCount < static_cast<int>(singlePoints->size())) {
                CVLog::Warning(
                        "[SHP] No enough records in the associated DBF file!");
            } else {
                ...
                for (int i = 0; i < fieldCount; ++i) {
                    char fieldName[256];
                    DBFFieldType fieldType = DBFGetFieldInfo(
                            dbfHandle, i, fieldName, nullptr, nullptr);
                    if (fieldType == FTDouble || fieldType == FTInteger) {
                        candidateFields.push_back(
                                FieldIndexAndName(i, QString(fieldName)));
                    }
                }
                ...
                if (!candidateFields.empty() &&
                    ...
                    for (QList<FieldIndexAndName>::const_iterator it =
                                 candidateFields.begin();
                         it != candidateFields.end(); ++it) {
                    ...
                    if (lsfDlg.exec()) {
                        s_dbfFieldImportScale =
                                lsfDlg.scaleDoubleSpinBox->value();

                        // look for the selected index
                        int index = -1;
                        for (int i = 0; i < candidateFields.size(); ++i) {
                            /*							if
                               (lsfDlg.listWidget->isItemSelected(lsfDlg.listWidget->item(i)))
                                                                                    {
                        ...
                        if (index >= 0) {
                            ...
                            DBFFieldType fieldType =
                                    DBFGetFieldInfo(dbfHandle, index, nullptr,
                                                    nullptr, nullptr);

                            if (hasPolylines) {
                                // for each poyline
                                for (QMap<ccPolyline *, int32_t>::iterator it =
                                             polyIDs.begin();
                                     it != polyIDs.end(); ++it) {
                                    // get the height
                                    double z = 0.0;
                                    if (fieldType == FTDouble)
                                        z = DBFReadDoubleAttribute(
                                                dbfHandle, it.value() - 1,
                                                index);
                                    ...
                                    if (poly) {
                                        poly->applyGLTransformation_recursive(
                                                &trans);
                                        // this transformation is of no interest
                                        // for the user
                                        // poly->resetGLTransformationHistory_recursive();
                                        // add the 'const altitude' meta-data as
                                        // well
                                        poly->setMetaData(
                                                ccPolyline::
                                ...
                                // for each point
                                for (unsigned i = 0; i < singlePoints->size();
                                     ++i) {
                                    // get the height
                                    double z = 0.0;
                                    if (fieldType == FTDouble)
                                        z = DBFReadDoubleAttribute(
                                                dbfHandle, static_cast<int>(i),
                                                index);
                                    else  // if (fieldType == FTInteger)
                                        z = static_cast<double>(
                                                DBFReadIntegerAttribute(
                    ...
                    CVLog::Warning(
                            "[SHP] No numerical field in the associated DBF "
                            "file!");
                }
            }
        } else {
            CVLog::Warning(
                    QString("[SHP] Failed to load associated DBF file ('%1')")
                            .arg(dbfFilename));
        }
    }
