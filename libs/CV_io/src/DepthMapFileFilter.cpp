...
DepthMapFileFilter::DepthMapFileFilter()
    : FileIOFilter({"_Depth Map Filter",
                    ...
                    QStringList{GetFileFilter()}, Export | BuiltIn}) {}

bool DepthMapFileFilter::canSave(CV_CLASS_ENUM type,
                                 bool& multiple,
                                 bool& exclusive) const {
    if (type == CV_TYPES::GBL_SENSOR) {
        multiple = true;
        exclusive = true;
        return true;
    }
...
CC_FILE_ERROR DepthMapFileFilter::saveToFile(ccHObject* entity,
    ...
    if (entity->isKindOf(CV_TYPES::GBL_SENSOR))
        ...
        entity->filterChildren(sensors, true, CV_TYPES::GBL_SENSOR);

    if (sensors.empty()) {
        CVLog::Error("No sensor in input selection!");
    ...
    for (size_t i = 0; i < sensorCount && result == CC_FERR_NO_ERROR; ++i) {
        ...
        QString sensorFilename = (sensorCount > 1 ? QString("%1_%2.%3")
                                                            .arg(baseName)
                                                            .arg(i)
                                                            .arg(extension)
                                                  : filename);
        ...
        if (sensor) {
            ...
            if (result != CC_FERR_NO_SAVE && result != CC_FERR_NO_ERROR) break;
        }
    }
...
CC_FILE_ERROR DepthMapFileFilter::saveToFile(const QString& filename,
                                             ccGBLSensor* sensor) {
    assert(sensor);
    if (!sensor) {
    ...
    if (db.zBuff.empty()) {
        CVLog::Warning(QString("[DepthMap] sensor '%1' has no associated depth "
                               "map (you must compute it first)")
                               .arg(sensor->getName()));
        return CC_FERR_NO_SAVE;  // this is not a severe error (the process can
                                 // go on)
    }

    ccGenericPointCloud* cloud =
            ccHObjectCaster::ToGenericPointCloud(sensor->getParent());
    if (!cloud) {
        CVLog::Warning(QString("[DepthMap] sensor '%1' is not associated to a "
                               "point cloud!")
                               .arg(sensor->getName()));
    ...
    if (CheckForSpecialChars(filename)) {
        CVLog::Warning(QString(
                "[DepthMap] Output filename contains special characters. It "
                "might be scrambled or rejected by the I/O filter..."));
    }
    ...
    fprintf(fp, "// Pitch  = %f [ %f : %f ]\n", sensor->getPitchStep(),
            sensor->getMinPitch(), sensor->getMaxPitch());
    fprintf(fp, "// Yaw   = %f [ %f : %f ]\n", sensor->getYawStep(),
            sensor->getMinYaw(), sensor->getMaxYaw());
    ...
    if (cloud && cloud->isA(CV_TYPES::POINT_CLOUD)) {
        ...
        if (nbPoints == 0) {
            CVLog::Warning(QString("[DepthMap] sensor '%1' is associated to an "
                                   "empty cloud?!")
                                   .arg(sensor->getName()));
            // this is not a severe error (the process can go on)
        } else {
            ...
            if (pc->hasNormals()) {
                ...
                try {
                    ...
                    CVLog::Warning(QString("[DepthMap] not enough memory to "
                                           "load normals on sensor '%1'!")
                                           .arg(sensor->getName()));
                }
            }
            ...
            if (pc->hasColors()) {
                try {
                    std::vector<ecvColor::Rgb> rgbColors;
    ...
    for (unsigned k = 0; k < db.height; ++k) {
        for (unsigned j = 0; j < db.width; ++j, ++_zBuff, ++index) {
            ...
            // color
            if (theColors) {
                const ecvColor::Rgb& C = theColors->at(index);
