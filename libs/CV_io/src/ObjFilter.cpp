...
ObjFilter::ObjFilter()
    : FileIOFilter({"_OBJ Filter",
                    8.0f,  // priority
                    QStringList{"obj"}, "obj", QStringList{"OBJ mesh (*.obj)"},
                    QStringList{"OBJ mesh (*.obj)"}, Import | Export}) {}

bool ObjFilter::canSave(CV_CLASS_ENUM type,
                        bool& multiple,
...
CC_FILE_ERROR ObjFilter::saveToFile(ccHObject* entity,
    ...
    if (!entity->isKindOf(CV_TYPES::MESH)) {
        CVLog::Warning(
                "[OBJ] This filter can only save one mesh (optionally with "
                "sub-meshes) at a time!");
    ...
    if (!mesh || mesh->size() == 0) {
        CVLog::Warning("[OBJ] Input mesh is empty!");
    ...
    if (!vertices || vertices->size() == 0) {
        CVLog::Warning("[OBJ] Input mesh has no vertices?!");
        return CC_FERR_NO_SAVE;
    }
    ...
    QFile file(filename);
    if (!file.open(QFile::Text | QFile::WriteOnly)) return CC_FERR_WRITING;
    ...
    if (parameters.parentWidget) {
        ...
        pDlg->setMethodTitle(
                QObject::tr("Saving mesh [%1]").arg(mesh->getName()));
        pDlg->setInfo(QObject::tr("Writing %1 vertices").arg(nbPoints));
        pDlg->setAutoClose(
                false);  // don't close dialogue when progress bar is full
        pDlg->start();
    }
    ...
    for (unsigned i = 0; i < nbPoints; ++i) {
        ...
        if (pDlg &&
            !nprogress
                     .oneStep())  // update progress bar, check cancel requested
    ...
    if (withNormals) {
        ...
        if (withTriNormals) {
            ...
            if (normsTable) {
                ...
                if (pDlg)
                    pDlg->setInfo(QObject::tr("Writing $1 triangle normals")
                                          .arg(numTriangleNormals));
        ...
        else  // if (withVertNormals)
        {
            ...
            if (pDlg)
                pDlg->setInfo(
                        QObject::tr("Writing %1 vertex normals").arg(nbPoints));
    ...
    if (withMaterials) {
        ...
        if (pDlg)
            pDlg->setInfo(
                    QObject::tr("Writing %1 materials").arg(materials->size()));
        nprogress.scale(1);
        nprogress.reset();
    ...
    bool withTexCoordinates =
            withMaterials && mesh->hasPerTriangleTexCoordIndexes();
    if (withTexCoordinates) {
        ...
        if (texCoords) {
            ...
            if (pDlg) {
                pDlg->setInfo(QObject::tr("Writing %1 texture coordinates")
                                      .arg(numTexCoords));
            }
    ...
    // look for sub-meshes
    mesh->filterChildren(subMeshes, false, CV_TYPES::SUB_MESH);
    // check that the number of facets is the same as the full mesh!
    {
        unsigned faceCount = 0;
        for (ccHObject::Container::const_iterator it = subMeshes.begin();
             it != subMeshes.end(); ++it)
            faceCount += static_cast<ccSubMesh*>(*it)->size();

        // if there's no face (i.e. no sub-mesh) or less face than the total
        // mesh, we save the full mesh!
    ...
    if (pDlg) {
        ...
        pDlg->setAutoClose(
                true);  //(re-enable) close dialogue when progress bar is full
    }
    ...
    for (ccHObject::Container::const_iterator it = subMeshes.begin();
        ...
        for (unsigned i = 0; i < triNum; ++i) {
            if (withMaterials) {
                ...
                if (withTexCoordinates) {
                    mesh->getTriangleTexCoordinatesIndexes(indexShift + i, t1,
                                                           t2, t3);
                    if (t1 >= 0) ++t1;
                    if (t2 >= 0) ++t2;
                    if (t3 >= 0) ++t3;
...
static bool UpdatePointIndex(int& vIndex, int maxIndex) {
    if (vIndex == 0 || -vIndex > maxIndex) return false;
...
struct facetElement {
    ...
    //! Default constructor
    facetElement() : vIndex(0), tcIndex(0), nIndex(0) {}
    ...
    inline bool updateTexCoordIndex(int maxIndex) {
        if (-tcIndex > maxIndex) return false;
    ...
    inline bool updateNormalIndex(int maxIndex) {
        if (-nIndex > maxIndex) return false;
...
CC_FILE_ERROR ObjFilter::loadFile(const QString& filename,
                                  ccHObject& container,
                                  LoadParameters& parameters) {
    CVLog::Print(QString("[OBJ] ") + filename);
    ...
    // common warnings that can appear multiple time (we avoid to send too many
        ...
        while (!currentLine.isNull()) {
            ...
            if (tokens.front() == "v") {
                ...
                if (vertices->size() == vertices->capacity()) {
                    if (!vertices->reserve(vertices->capacity() +
                                           ccChunk::SIZE)) {
                ...
                CCVector3d Pd(tokens[1].toDouble(), tokens[2].toDouble(),
                              tokens[3].toDouble());
                ...
                if (pointsRead == 0) {
                    ...
                    if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
                        ...
                        CVLog::Warning(
                                "[OBJ] Cloud has been recentered! Translation: "
                                "(%.2f ; %.2f ; %.2f)",
            ...
            /*** new vertex texture coordinates ***/
                ...
                // create and reserve memory for tex. coords container if
                // necessary
                if (!texCoords) {
                ...
                if (texCoords->currentSize() == texCoords->capacity()) {
                    if (!texCoords->reserveSafe(texCoords->capacity() +
                                                ccChunk::SIZE)) {
                        objWarnings[NOT_ENOUGH_MEMORY] = true;
                        error = true;
                        break;
                ...
                if (tokens.size() >
                    2)  // OBJ specification allows for only one value!!!
                {
            ...
            /*** new vertex normal ***/
                     ...
                     "vn")  //--> in fact it can also be a facet normal!!!
            {
                ...
                if (normals->currentSize() == normals->capacity()) {
                    if (!normals->reserveSafe(normals->capacity() +
                                              ccChunk::SIZE)) {
                        objWarnings[NOT_ENOUGH_MEMORY] = true;
                        error = true;
                        break;
                ...
                CCVector3 N(
                        static_cast<PointCoordinateType>(tokens[1].toDouble()),
                ...
                normals->addElement(
                        nIndex);  // we don't know yet if it's per-vertex or
                                  // per-triangle normal...
                ++normsRead;
            }
            /*** new group ***/
                ...
                // get the group name
                QString groupName =
                        (tokens.size() > 1 && !tokens[1].isEmpty() ? tokens[1]
                                                                   : "default");
                ...
                if (!groups.empty() && groups.back().first == totalFacesRead)
                    groups.back().second =
                            groupName;  // simply replace the group name if the
                                        // previous group was empty!
                else
            ...
            /*** new face ***/
                ...
                std::vector<facetElement> currentFace;
                {
                    ...
                    for (int i = 1; i < tokens.size(); ++i) {
                        ...
                        if (vertexTokens.size() == 0 ||
                            vertexTokens[0].isEmpty()) {
                            objWarnings[INVALID_LINE] = true;
                            error = true;
                            break;
                        } else {
                            ...
                            if (vertexTokens.size() > 1 &&
                                !vertexTokens[1].isEmpty())
                                fe.tcIndex = vertexTokens[1].toInt();
                            if (vertexTokens.size() > 2 &&
                                !vertexTokens[2].isEmpty())
                ...
                if (currentFace.size() < 3) {
                    CVLog::Warning(
                            "[OBJ] Malformed file: polygon on line %1 has less "
                            "than 3 vertices!",
                            lineCount);
                    error = true;
                    break;
                ...
                // the very first vertex of the group tells us about the whole
                // sequence
                if (facesRead == 0) {
                    // we have a tex. coord index as second vertex element!
                    if (!hasTexCoords && A->tcIndex != 0 &&
                        !materialsLoadFailed) {
                ...
                if (shouldTesselate) {
                    for (const facetElement& fe : currentFace) {
                        if (fe.vIndex < 0 ||
                            vertices->size() <=
                                    static_cast<unsigned>(fe.vIndex)) {
                            // we haven't loaded all the vertices?! Too bad, we
                            // can't tesselate properly :(
                ...
                if (shouldTesselate) {
                    try {
                        ...
                        contour.reserve(
                                static_cast<unsigned>(currentFace.size()));
                        ...
                        if (dMesh) {
                            ...
                            if (baseMesh->size() + triCount >=
                                baseMesh->capacity()) {
                                if (!baseMesh->reserve(
                                            baseMesh->size() +
                                            std::max(triCount, 4096u))) {
                                    objWarnings[NOT_ENOUGH_MEMORY] = true;
                                    error = true;
                                    break;
                                }
                            ...
                            // push new triangle
                            const int* _triIndexes =
                                    dMesh->getTriangleVertIndexesArray();
                            // determine if the triangles must be flipped or not
                            bool flip = false;
                            {
                                for (unsigned i = 0; i < triCount;
                                     ++i, _triIndexes += 3) {
                                    int i1 = _triIndexes[0];
                                    int i2 = _triIndexes[1];
                                    int i3 = _triIndexes[2];
                                    // by definition the first edge of the
                                    // original polygon should be in the same
                                    // 'direction' of the triangle that uses it
                                    if ((i1 == 0 || i2 == 0 || i3 == 0) &&
                                        (i1 == 1 || i2 == 1 || i3 == 1)) {
                                        if ((i1 == 1 && i2 == 0) ||
                                            (i2 == 1 && i3 == 0) ||
                            ...
                            for (unsigned i = 0; i < triCount;
                                 ++i, _triIndexes += 3) {
                                const facetElement& f1 =
                                        currentFace[_triIndexes[0]];
                                facetElement f2 = currentFace[_triIndexes[1]];
                                facetElement f3 = currentFace[_triIndexes[2]];

                                if (flip) std::swap(f2, f3);

                                baseMesh->addTriangle(f1.vIndex, f2.vIndex,
                                                      f3.vIndex);
                                ...
                                if (hasTexCoords)
                                    baseMesh->addTriangleTexCoordIndexes(
                                            f1.tcIndex, f2.tcIndex, f3.tcIndex);

                                if (normalsPerFacet)
                                    baseMesh->addTriangleNormalIndexes(
                                            f1.nIndex, f2.nIndex, f3.nIndex);
                ...
                if (!shouldTesselate) {
                    ...
                    for (; C != currentFace.end(); ++B, ++C) {
                        ...
                        if (hasTexCoords)
                            baseMesh->addTriangleTexCoordIndexes(
                                    A->tcIndex, B->tcIndex, C->tcIndex);

                        if (normalsPerFacet)
                            baseMesh->addTriangleNormalIndexes(
                                    A->nIndex, B->nIndex, C->nIndex);
                    }
                }
            }
            /*** polyline ***/
                ...
                if (!polyline->reserve(
                            static_cast<unsigned>(tokens.size() - 1))) {
                    // not enough memory
                    objWarnings[NOT_ENOUGH_MEMORY] = true;
                    delete polyline;
                    polyline = nullptr;
                ...
                for (int i = 1; i < tokens.size(); ++i) {
                    ...
                    if (vertexTokens.size() == 0 || vertexTokens[0].isEmpty()) {
                        ...
                        int index =
                                vertexTokens[0].toInt();  // we ignore normal
                                                          // index (if any!)
                ...
                QString name = groups.empty() ? QString("Line")
                                              : groups.back().second +
                                                        QString(".line");
            ...
            else if (tokens.front() == "usemtl")  // see 'MTL file' below
            {
                if (materials)  // otherwise we have failed to load MTL file!!!
                {
                    ...
                    // DGM: in case there's space characters in the material
                    // name, we must read it again from the original line buffer
                    // QString mtlName = (tokens.size() > 1 &&
                    // !tokens[1].isEmpty() ? tokens[1] : "");
                    currentMaterial =
                            (!mtlName.isEmpty()
                                     ? materials->findMaterialByName(mtlName)
                                     : -1);
            ...
            /*** material file (MTL) ***/
                ...
                if (tokens.size() < 2 || tokens[1].isEmpty()) {
                    ...
                    // DGM: in case there's space characters in the filename, we
                    // must read it again from the original line buffer QString
                    ...
                    CVLog::Print(QString("[OBJ] Material file: ") +
                                 mtlFilename);
                    ...
                    if (!errors.empty()) {
                        for (int i = 0; i < errors.size(); ++i)
                            CVLog::Warning(QString("[OBJ::Load::MTL parser] ") +
                                           errors[i]);
                    }
                    if (materials->empty()) {
                        materials->release();
    ...
    if (!error && pointsRead == 0) {
        ...
        CVLog::Warning("[OBJ] Malformed file: no vertex in file!");
        error = true;
    }

    if (!error) {
        CVLog::Print("[OBJ] %i points, %u faces", pointsRead, totalFacesRead);
        if (texCoordsRead > 0 || normsRead > 0)
            CVLog::Print("[OBJ] %i tex. coords, %i normals", texCoordsRead,
                         normsRead);

        // do some cleaning
        vertices->shrinkToFit();
        if (normals) normals->shrink_to_fit();
        if (texCoords) texCoords->shrink_to_fit();
        if (baseMesh->size() == 0) {
            delete baseMesh;
            baseMesh = nullptr;
        } else {
        ...
        if (maxVertexIndex >= pointsRead || maxTexCoordIndex >= texCoordsRead ||
            maxTriNormIndex >= normsRead) {
            // hum, we've got a problem here
            CVLog::Warning(
                    "[OBJ] Malformed file: indexes go higher than the number "
                    "of elements! (v=%i/tc=%i/n=%i)",
                    maxVertexIndex, maxTexCoordIndex, maxTriNormIndex);
        ...
        if (!error && baseMesh) {
            ...
            if (texCoords) {
                if (materials) {
                    ...
                    CVLog::Warning(
                            "[OBJ] Texture coordinates were defined but no "
                            "material could be loaded!");
                }
            }

            // normals: if the obj file doesn't provide any, should we compute
            // them?
            if (!normals) {
                CVLog::Warning(
                        "[OBJ] Mesh has no normal! You can manually compute "
                        "them (select it then call \"Tools > Normals > "
                        "Compute\")");
            }
            ...
            // DGM: we can't deactive the vertices if it has children! (such as
            // polyline)
        ...
        if (!baseMesh && vertices->size() != 0) {
            ...
            // we hide the vertices if the entity has children (probably
            // polylines!)
            if (vertices->getChildrenNumber() != 0) {
                vertices->setVisible(false);
            }
        ...
        if (normals && !normalsPerFacet) {
            if (normsRead == pointsRead)  // must be 'per-vertex' normals
            {
                ...
                if (baseMesh) baseMesh->showNormals(true);
            } else {
                CVLog::Warning(
                        "File contains normals which seem to be neither "
                        "per-vertex nor per-face!!! We had to ignore them...");
            }
        }
    }

    if (error) {
        if (baseMesh) delete baseMesh;
    ...
    if (objWarnings[INVALID_NORMALS])
        CVLog::Warning(
                "[OBJ] Some normals in file were invalid. You should "
                "re-compute them (select entity, then \"Tools > Normals > "
                "Compute\")");
    if (objWarnings[INVALID_INDEX])
        CVLog::Warning("[OBJ] File is malformed! Check indexes...");
    if (objWarnings[NOT_ENOUGH_MEMORY])
        CVLog::Warning("[OBJ] Not enough memory!");
    if (objWarnings[INVALID_LINE])
        CVLog::Warning("[OBJ] File is malformed! Missing data.");
