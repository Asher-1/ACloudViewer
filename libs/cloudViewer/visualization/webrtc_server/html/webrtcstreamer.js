...
let WebRtcStreamer = (function() {
    ...
    function WebRtcStreamer(videoElt, srvurl, onClose, commsFetch = null) {
        ...
        // CloudViewer-specific functions.
     ...
     * @param {boolean} useComms If true, CloudViewer's Jupyter "COMMS"
     *         interface will be used for WebRTC handshake. Otherwise, fetch()
     *         will be used and an additional web server is required to process
     *         the http requests.
     */
    ...
    WebRtcStreamer._getModifiers = function(event) {
        // See cloudViewer/visualization/gui/Events.h.
        var modNone = 0;
        var modShift = 1 << 0;
        var modCtrl = 1 << 1;
    ...
    WebRtcStreamer.prototype.addEventListeners = function(windowUID) {
        if (this.videoElt) {
            ...
            this.videoElt.addEventListener('mousedown', (event) => {
                ...
                this.sendJsonData(cloudViewerMouseEvent);
            }, false);
