...
bool TextureSimpleShader::BindGeometry(const ccHObject &geometry,
...
bool TextureSimpleShader::RenderGeometry(const ccHObject &geometry,
                                         const RenderOption &option,
                                         const ViewControl &view) {
...
bool TextureSimpleShaderForTriangleMesh::PrepareBinding(
    ...
    for (size_t i = 0; i < mesh.size(); i++) {
        ...
        for (size_t j = 0; j < 3; j++) {
            size_t idx = i * 3 + j;
            size_t vi = triangle->i[j];
            tmp_points[mi].push_back(mesh.getVertice(vi).cast<float>());
