...
PointCloudBuffersBuilder::PointCloudBuffersBuilder(const ccPointCloud& geometry)
    : geometry_(geometry) {}
...
GeometryBuffersBuilder::Buffers PointCloudBuffersBuilder::ConstructBuffers() {
    ...
    const size_t n_vertices = geometry_.size();
    ...
    if (geometry_.hasNormals()) {
        ...
        for (size_t i = 0; i < n_vertices; ++i) {
            normals[i] = geometry_.getEigenNormal(i).cast<float>();
        }
    ...
    for (size_t i = 0; i < geometry_.size(); ++i) {
        ...
        element.SetVertexPosition(geometry_.getEigenPoint(i));
        if (geometry_.hasColors()) {
            element.SetVertexColor(geometry_.getEigenColor(i),
