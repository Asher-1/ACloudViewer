...
std::tuple<vbdata, ibdata> CreatePlainBuffers(const math::quatf* tangents,
                                              const ccMesh& geometry) {
    vbdata vertex_data;
    ibdata index_data;

    vertex_data.vertices_count = geometry.getVerticeSize();
    ...
    for (size_t i = 0; i < vertex_data.vertices_count; ++i) {
        ...
        SetVertexPosition(element, geometry.getVertice(i));
        if (tangents != nullptr) {
    ...
    index_data.byte_count = geometry.size() * 3 * index_data.stride;
    index_data.bytes = static_cast<GeometryBuffersBuilder::IndexType*>(
            malloc(index_data.byte_count));
    for (size_t i = 0; i < geometry.size(); ++i) {
        const auto& triangle = geometry.getTriangle(i);
...
// Transfers ownership on return for vbdata.bytes and ibdata.bytes
std::tuple<vbdata, ibdata> CreateColoredBuffers(const math::quatf* tangents,
                                                const ccMesh& geometry) {
    vbdata vertex_data;
    ibdata index_data;
    ...
    for (size_t i = 0; i < vertex_data.vertices_count; ++i) {
        ...
        if (geometry.hasColors()) {
            SetVertexColor(element, geometry.getVertexColor(i));
        } else {
...
// Transfers ownership on return for vbdata.bytes and ibdata.bytes
std::tuple<vbdata, ibdata> CreateTexturedBuffers(const math::quatf* tangents,
                                                 const ccMesh& geometry) {
    vbdata vertex_data;
    ibdata index_data;
    ...
    vertex_data.byte_count = geometry.size() * 3 * sizeof(TexturedVertex);
    vertex_data.bytes = malloc(vertex_data.byte_count);
    ...
    for (size_t i = 0; i < geometry.size(); ++i) {
        ...
        for (size_t j = 0; j < 3; ++j) {
            ...
            auto pos = geometry.getVertice(index);
            ...
            if (found != index_lookup.end()) {
                ...
                if (geometry.hasColors()) {
                    SetVertexColor(element,
                                   geometry.getVertexColor(source_idx));
                } else {
                    element.color = kDefault.color;
                }
            }
...
TriangleMeshBuffersBuilder::TriangleMeshBuffersBuilder(const ccMesh& geometry)
    : geometry_(geometry) {}
...
GeometryBuffersBuilder::Buffers TriangleMeshBuffersBuilder::ConstructBuffers() {
    ...
    const size_t n_vertices = geometry_.getVerticeSize();
    ...
    if (geometry_.hasNormals()) {
        ...
        for (size_t i = 0; i < n_vertices; ++i) {
            normals[i] = geometry_.getVertexNormal(i).cast<float>();
        }
...
filament::Box TriangleMeshBuffersBuilder::ComputeAABB() {
    ...
    const filament::math::float3 min(geometry_aabb.minCorner().x,
                                     ...
                                     geometry_aabb.minCorner().z);
    const filament::math::float3 max(geometry_aabb.maxCorner().x,
                                     geometry_aabb.maxCorner().y,
                                     geometry_aabb.maxCorner().z);
