...
inline CLOUDVIEWER_HOST_DEVICE float HuberDeriv(float r, float delta) {
    float abs_r = abs(r);
    return abs_r < delta ? r : delta * Sign(r);
}

inline CLOUDVIEWER_HOST_DEVICE float HuberLoss(float r, float delta) {
    float abs_r = abs(r);
    return abs_r < delta ? 0.5 * r * r : delta * abs_r - 0.5 * delta * delta;
}

CLOUDVIEWER_HOST_DEVICE inline bool GetJacobianPointToPoint(
        int x,
        int y,
...
CLOUDVIEWER_HOST_DEVICE inline bool GetJacobianPointToPlane(
        int x,
        int y,
...
CLOUDVIEWER_HOST_DEVICE inline bool GetJacobianIntensity(
        int x,
        int y,
        const float depth_outlier_trunc,
...
CLOUDVIEWER_HOST_DEVICE inline bool GetJacobianHybrid(
        int x,
        int y,
        const float depth_outlier_trunc,
        const NDArrayIndexer& source_depth_indexer,
