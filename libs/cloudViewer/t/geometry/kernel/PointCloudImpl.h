...
#include "cloudViewer/core/linalg/kernel/SVD3x3.h"
...
template <typename scalar_t>
inline CLOUDVIEWER_HOST_DEVICE void Swap(scalar_t* x, scalar_t* y) {
    scalar_t tmp = *x;
    *x = *y;
    *y = tmp;
}
...
template <typename scalar_t>
inline CLOUDVIEWER_HOST_DEVICE void Heapify(scalar_t* arr, int n, int root) {
    int largest = root;
    int l = 2 * root + 1;
    int r = 2 * root + 2;
...
template <typename scalar_t>
CLOUDVIEWER_HOST_DEVICE void EstimatePointWiseRobustNormalizedCovarianceKernel(
...
template <typename scalar_t>
CLOUDVIEWER_HOST_DEVICE void EstimatePointWiseNormalsWithFastEigen3x3(
...
template <typename scalar_t>
CLOUDVIEWER_HOST_DEVICE void EstimatePointWiseColorGradientKernel(
        const scalar_t* points_ptr,
