...
#pragma omp parallel for schedule(static)
    for (int i = 0; i < (int)output->size(); i++) {
        const auto Rx = GetRotationFromE1ToX(output->getEigenNormal(i));
...
double TransformationEstimationForGeneralizedICP::ComputeRMSE(
    ...
    for (const auto &c : corres) {
        const Eigen::Vector3d vs = source.getEigenPoint(c[0]);
        const Eigen::Matrix3d &Cs = source.covariances_[c[0]];
        const Eigen::Vector3d vt = target.getEigenPoint(c[1]);
