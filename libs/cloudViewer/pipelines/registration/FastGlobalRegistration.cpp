...
static std::vector<std::pair<int, int>> InitialMatching(
        const utility::Feature& src_features,
...
static std::vector<std::pair<int, int>> AdvancedMatching(
        const ccPointCloud& src_point_cloud,
    ...
    for (i = 0; i < number_of_trial; i++) {
        ...
        Eigen::Vector3d pti0 = src_point_cloud.getEigenPoint(idi0);
        Eigen::Vector3d pti1 = src_point_cloud.getEigenPoint(idi1);
        Eigen::Vector3d pti2 = src_point_cloud.getEigenPoint(idi2);
        double li0 = (pti0 - pti1).norm();
        double li1 = (pti1 - pti2).norm();
        double li2 = (pti2 - pti0).norm();

        // collect 3 points from dest fragment
        Eigen::Vector3d ptj0 = dst_point_cloud.getEigenPoint(idj0);
        Eigen::Vector3d ptj1 = dst_point_cloud.getEigenPoint(idj1);
        Eigen::Vector3d ptj2 = dst_point_cloud.getEigenPoint(idj2);
