...
struct Point {
    ...
    CLOUDVIEWER_HOST_DEVICE Point(float x, float y) : x_(x), y_(y) {}
...
CLOUDVIEWER_HOST_DEVICE inline float IoUBev2DWithMinAndMax(
        const float *box_a,
        const float *box_b,
...
CLOUDVIEWER_HOST_DEVICE inline float IoUBev2DWithCenterAndSize(
        const float *box_a,
        const float *box_b,
        bool intersection_only = false) {
    float box_a_new[5];
