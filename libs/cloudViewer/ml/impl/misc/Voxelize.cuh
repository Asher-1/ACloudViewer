...
__global__ void ComputeHashKernel(
        ...
        const cloudViewer::utility::MiniVec<T, NDIM> points_range_min_vec,
        const cloudViewer::utility::MiniVec<T, NDIM> points_range_max_vec,
        const cloudViewer::utility::MiniVec<T, NDIM> inv_voxel_size,
        const cloudViewer::utility::MiniVec<int64_t, NDIM> strides,
        const int64_t batch_hash,
...
void VoxelizeCUDA(const cudaStream_t& stream,
    ...
    if (!get_temp_size) {
        if (max_points_per_voxel < static_cast<int64_t>(num_points)) {
