...
bool gpuSupportCUDA(int minComputeCapabilityMajor,
    ...
    if (success != cudaSuccess) {
        utility::LogError("cudaGetDeviceCount failed: ",
    ...
    if (nbDevices > 0) {
        for (int i = 0; i < nbDevices; ++i) {
            ...
            if (cudaGetDeviceProperties(&deviceProperties, i) != cudaSuccess) {
                utility::LogError(
                        "Cannot get properties for CUDA gpu device {}", i);
                continue;
            }
            ...
            if ((deviceProperties.major > minComputeCapabilityMajor ||
                ...
                deviceProperties.totalGlobalMem >=
                        static_cast<std::size_t>(minTotalDeviceMemory * 1024 *
                                                 1024)) {
                utility::LogInfo("Supported CUDA-Enabled GPU detected.");
                return true;
            } else {
                utility::LogError(
                        "CUDA-Enabled GPU detected, but the compute "
                        "capabilities is not enough.\n - Device {} : {}.{}, "
                        "global memory: {} MB\n - Requirements: {}.{}, global "
                        "memory: {} MB\n",
                        i, deviceProperties.major, deviceProperties.minor,
                        int(deviceProperties.totalGlobalMem / (1024 * 1024)),
                        minComputeCapabilityMajor, minComputeCapabilityMinor,
                        minTotalDeviceMemory);
            }
        }
...
std::string gpuInformationCUDA() {
    ...
    if (cudaGetDeviceCount(&nbDevices) != cudaSuccess) {
        utility::LogWarning(
                "Could not determine number of CUDA cards in this system");
        nbDevices = 0;
    }
    ...
    if (nbDevices > 0) {
        ...
        for (int i = 0; i < nbDevices; ++i) {
            ...
            if (cudaMemGetInfo(&avail, &total) != cudaSuccess) {
                ...
                total = 0;
                utility::LogWarning(
                        "Cannot get available memory information for CUDA gpu "
                        "device {}.",
                        i);
            }
            ...
            deviceSS << "Device information:" << std::endl
                     ...
                     << deviceProperties.sharedMemPerBlock << std::endl
                     << "\t- warp size:               "
                     << deviceProperties.warpSize << std::endl
                     << "\t- max threads per block:   "
                     << deviceProperties.maxThreadsPerBlock << std::endl
                     ...
                     << deviceProperties.maxTexture2DLinear[2] << "}"
                     << std::endl
                     << "\t- max 2D layered texture:  "
                     << "{" << deviceProperties.maxTexture2DLayered[0] << ","
                     << deviceProperties.maxTexture2DLayered[1] << ","
                     << deviceProperties.maxTexture2DLayered[2] << "}"
                     << std::endl
                     << "\t- number of SM(x)s:        "
                     << deviceProperties.multiProcessorCount << std::endl
                     << "\t- registers per SM(x):     "
                     << deviceProperties.regsPerMultiprocessor << std::endl
                     << "\t- registers per block:     "
                     << deviceProperties.regsPerBlock << std::endl
                     << "\t- concurrent kernels:      "
                     ...
                     << std::endl
                     << "\t- texture alignment:       "
                     << deviceProperties.textureAlignment << " byte"
                     << std::endl
                     << "\t- pitch alignment:         "
                     << deviceProperties.texturePitchAlignment << " byte"
                     << std::endl;
