...
# of the point cloud.
pcd.point.positions = cv3d.core.Tensor([[0, 0, 0],
                                    [1, 1, 1],
                                    [2, 2, 2]], dtype, device)
...
pcd.point.normals = cv3d.core.Tensor([[0, 0, 1],
                                  [0, 1, 0],
                                  [1, 0, 0]], dtype, device)
pcd.point.colors = cv3d.core.Tensor([[0.0, 0.0, 0.0],
                                 [0.1, 0.1, 0.1],
...
# dtype, e.g.,
pcd.point.intensities = cv3d.core.Tensor([0.3, 0.1, 0.4], dtype, device)
pcd.point.labels = cv3d.core.Tensor([3, 1, 4], cv3d.core.int32, device)
)");
...
Example:
    ...
    We will load the Eagle dataset, downsample it, and show the result::
        ...
        pcd = cv3d.t.io.read_point_cloud(eagle.path)
        pcd_down = pcd.voxel_down_sample(voxel_size=0.05)
        cv3d.visualization.draw([{'name': 'pcd', 'geometry': pcd}, {'name': 'pcd_down', 'geometry': pcd_down}])
    )doc");
...
Example:
    ...
    Due to the high density of the Bunny point cloud available in CloudViewer a larger
    ...
    https://eugeniovaretti.github.io/meshreco for these cases.::
        ...
        import numpy as np
        # Load point cloud
        data = cv3d.data.BunnyMesh()

        # Case 1, Hoppe (raw):
        pcd = cv3d.io.read_point_cloud(data.path)
        ...
        pcd_robust = cv3d.io.read_point_cloud(data.path)
        ...
        cv3d.visualization.draw_geometries([poisson_mesh_robust]) )");
    pointcloud.def(
...
Example:
    ...
    We use armadillo mesh to compute the visible points from given camera::
        ...
        camera = cv3d.core.Tensor([0, 0, diameter], cv3d.core.float32)
        radius = diameter * 100

        # Get all points that are visible from given view point.
        pcd = cv3d.t.geometry.PointCloud.from_legacy(pcd)
...
Example:
    ...
    We use Redwood dataset for demonstration::
        ...
        pcd = cv3d.t.io.read_point_cloud(sample_ply_data.path)
        ...
        colors = cv3d.core.Tensor(colors[:, :3], cv3d.core.float32)
        colors[labels < 0] = 0
...
Example:
    ...
    We use Redwood dataset to compute its plane model and inliers::
        ...
        pcd = cv3d.t.io.read_point_cloud(sample_pcd_data.path)
...
Example:
    We will load the Eagle dataset, compute and display it's convex hull::
        ...
        cv3d.visualization.draw([{'name': 'eagle', 'geometry': pcd}, {'name': 'convex hull', 'geometry': hull}])
    )doc");
...
Example:
    We will load the DemoCropPointCloud dataset, compute its boundary points::
        ...
        ply_point_cloud = cv3d.data.DemoCropPointCloud()
        pcd = cv3d.t.io.read_point_cloud(ply_point_cloud.point_cloud_path)
        boundaries, mask = pcd.compute_boundary_points(radius, max_nn)
        boundaries.paint_uniform_color([1.0, 0.0, 0.0])
        cv3d.visualization.draw([pcd, boundaries])
    )doc");
...
Example:
    ...
    This code generates a number of helices from a point cloud::
        ...
        import cloudViewer as cv3d
        import numpy as np
        pcd = cv3d.t.geometry.PointCloud(np.random.rand(10,3))
        helices = pcd.extrude_rotation(3*360, [0,1,0], resolution=3*16, translation=2)
        cv3d.visualization.draw([{'name': 'helices', 'geometry': helices}])
...
Example:
    ...
    This code generates a set of straight lines from a point cloud::
        ...
        cv3d.visualization.draw([{'name': 'lines', 'geometry': lines}])
...
Example:
    ...
    contains at most 20 points::
        ...
        import cloudViewer as cv3d
        import numpy as np
        pcd = cv3d.t.geometry.PointCloud(np.random.rand(100,3))
...
Example::
    ...
    from cv3d.t.geometry import TriangleMesh, PointCloud, Metric, MetricParameters
    ...
    metric_params = MetricParameters(
        fscore_radius=cv3d.utility.FloatVector((0.01, 0.11, 0.15, 0.18)))
