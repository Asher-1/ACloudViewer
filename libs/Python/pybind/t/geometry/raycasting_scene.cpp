...
#include "t/geometry/RaycastingScene.h"
...
Args:
    vertex_positions (cloudViewer.core.Tensor): Vertices as Tensor of dim {N,3} and dtype
        Float32.
    triangle_indices (cloudViewer.core.Tensor): Triangles as Tensor of dim {M,3} and dtype
        UInt32.
...
Lists the intersections of the rays with the scene::
    ...
    scene = cv3d.t.geometry.RaycastingScene()
    d = cv3d.data.MonkeyModel()
    mesh = cv3d.t.io.read_triangle_mesh(d.path)
    ...
    x = cv3d.t.geometry.PointCloud(positions=c)
    cv3d.visualization.draw([mesh, lines, x], point_size=8)
