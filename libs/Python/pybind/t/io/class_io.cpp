...
Returns the mesh object. On failure an empty mesh is returned.
    ...
    m_io.def(
            ...
            [](const std::string &filename) {
                py::gil_scoped_release release;
                return ReadNpy(filename);
            },
            "Read a numpy .npy file into a Tensor.", "filename"_a);
    docstring::FunctionDocInject(m_io, "read_numpy",
                                 map_shared_argument_docstrings);

    m_io.def(
            "write_numpy",
            [](const std::string &filename, const core::Tensor &tensor) {
                py::gil_scoped_release release;
                WriteNpy(filename, tensor);
            },
            "Write a Tensor to a numpy .npy file.", "filename"_a, "tensor"_a);
    docstring::FunctionDocInject(m_io, "write_numpy",
                                 map_shared_argument_docstrings);
    ...
    m_io.def(
            ...
            [](const std::string &filename) {
                py::gil_scoped_release release;
                return ReadNpz(filename);
            },
            "Read a numpy .npz file into a dict[str, Tensor].", "filename"_a);
    docstring::FunctionDocInject(m_io, "read_npz",
                                 map_shared_argument_docstrings);
    ...
    m_io.def(
            ...
            [](const std::string &filename,
               const std::unordered_map<std::string, core::Tensor>
                       &tensor_map) {
                py::gil_scoped_release release;
                WriteNpz(filename, tensor_map);
            },
            "Write a dict[str, Tensor] to a numpy .npz file.", "filename"_a,
            "tensor_map"_a);
    docstring::FunctionDocInject(m_io, "write_npz",
                                 map_shared_argument_docstrings);
