...
def {fn_name}({fn_args}):
{docstring}
    return _torch.ops.cloudViewer.{fn_name}({args_fwd})
...
def {fn_name}({fn_args}):
{docstring}
    return return_types.{fn_name}(*_torch.ops.cloudViewer.{fn_name}({args_fwd}))
...
def parse_schema_from_docstring(docstring):
    ...
    cloudViewer::my_function(int a, Tensor b, Tensor c) -> (Tensor d, Tensor e)
...
def main():
    ...
    parser.add_argument("--lib",
                        type=str,
                        required=True,
                        help="path to cloudViewer_torch_ops.so")
    ...
    for schema in torch._C._jit_get_all_schemas():
        if not schema.name.startswith('cloudViewer::'):
            continue
