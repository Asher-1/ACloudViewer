...
void pybind_trianglemesh(py::module& m) {
    ...
    trianglemesh
            ...
            .def(py::init([](std::shared_ptr<ccGenericPointCloud> cloud) {
                     if (cloud) {
                         ...
                         ccPointCloud* vertices =
                                 ccHObjectCaster::ToPointCloud(cloud.get());
            ...
            .def(py::init([](std::shared_ptr<cloudViewer::GenericIndexedMesh>
                                     index_mesh,
                 ...
                 "Create a triangle mesh from index_mesh and cloud",
                 "index_mesh"_a, "cloud"_a)
            .def("__repr__",
                 [](const ccMesh& mesh) {
            ...
            .def(
                    ...
                    "Returns vertices number.", "input_mesh"_a,
                    "create_submesh"_a)
            ...
            .def("reserve", &ccMesh::reserve,
                 "Reserves the memory to store the vertex indexes (3 per "
                 "triangle).",
                 "triangles_number"_a)
            .def("resize", &ccMesh::resize,
                 "If the new number of elements is smaller than the actual "
                 "size,"
                 "the overflooding elements will be deleted.",
                 "triangles_number"_a)
            .def("flip_triangles", &ccMesh::flipTriangles,
                 "Flips the triangle.")
            .def("shrink_triangles", &ccMesh::shrinkToFit,
                 "Removes unused triangle capacity.")
            .def("shrink_vertexes", &ccMesh::shrinkVertexToFit,
                 "Removes unused vertex capacity.")
            .def("clear_triangle_normals", &ccMesh::clearTriNormals,
                 "Removes per-triangle normals.")
            .def("are_triangle_normals_enabled",
                 &ccMesh::arePerTriangleNormalsEnabled,
                 "Returns whether per triangle normals are enabled.")
            .def("reserve_triangle_normal_indexes",
                 &ccMesh::reservePerTriangleNormalIndexes,
                 "Reserves memory to store per-triangle triplets of normal "
                 "indexes.")
            .def("add_triangle_normal_indexes",
                 &ccMesh::addTriangleNormalIndexes,
                 "Adds a triplet of normal indexes for next triangle.")
            .def("set_triangle_normal_indexes",
                 py::overload_cast<std::size_t, CompressedNormType>(
                         &ccMesh::setTriangleNormalIndexes),
                 "Adds a triplet of normal indexes for next triangle.",
                 "triangle_index"_a, "value"_a)
            .def("set_triangle_normal_indexes",
                 py::overload_cast<unsigned, int, int, int>(
                         &ccMesh::setTriangleNormalIndexes),
                 "Adds a triplet of normal indexes for next triangle.",
                 "triangle_index"_a, "i1"_a, "i2"_a, "i3"_a)
            .def("get_triangle_normal_indexes",
                 py::overload_cast<std::size_t>(
                         &ccMesh::getTriangleNormalIndexes),
                 "Returns a compressed normal indexes for a given triangle "
                 "index (if any).",
                 "triangle_index"_a)
            .def("remove_triangle_normal_indexes",
                 &ccMesh::removePerTriangleNormalIndexes,
                 "Removes any per-triangle triplets of normal indexes.")
            .def("convert_materials_to_vertex_colors",
                 &ccMesh::convertMaterialsToVertexColors,
            ...
            .def("reserve_material_indexes",
                 &ccMesh::reservePerTriangleMtlIndexes,
                 "Reserves memory to store per-triangle material index.")
            .def("remove_material_indexes",
                 &ccMesh::removePerTriangleMtlIndexes,
            ...
            .def("set_material_index", &ccMesh::setTriangleMtlIndex,
                 "Adds triangle material index for next triangle.",
                 "triangle_index"_a, "material_index"_a)
            .def("reserve_texture_coord_indexes",
                 &ccMesh::reservePerTriangleTexCoordIndexes,
                 "Reserves memory to store per-triangle triplets of tex coords "
                 "indexes.")
            .def("remove_texture_coord_indexes",
                 &ccMesh::removePerTriangleTexCoordIndexes,
                 "Remove per-triangle tex coords indexes.")
            .def("add_texture_coord_indexes",
                 &ccMesh::addTriangleTexCoordIndexes,
                 "Adds a triplet of tex coords indexes for next triangle.",
                 "i1"_a, "i2"_a, "i3"_a)
            .def("set_texture_coord_indexes",
                 &ccMesh::setTriangleTexCoordIndexes,
                 "Sets a triplet of tex coords indexes for a given triangle.",
                 "triangle_index"_a, "i1"_a, "i2"_a, "i3"_a)
            .def("compute_normals", &ccMesh::computeNormals,
                 "Computes normals.", "per_vertex"_a)
            .def(
                    "laplacian_smooth",
                    [](ccMesh& mesh, unsigned iterations,
                       PointCoordinateType factor) {
                        return mesh.laplacianSmooth(iterations, factor,
                                                    nullptr);
                    },
                    "Laplacian smoothing.", "iterations"_a = 100,
                    "factor"_a = 0.01f)
            .def("process_scalar_field", &ccMesh::processScalarField,
                 "Applies process to the mesh scalar field (the one associated "
                 "to its vertices in fact).",
                 "process_type"_a)
            .def(
                    "subdivide",
                    [](const ccMesh& mesh, PointCoordinateType max_area) {
                        return std::shared_ptr<ccMesh>(
                                mesh.subdivide(max_area));
                    },
                    "Subdivides mesh (so as to ensure that all triangles are "
                    "falls below 'max_area').",
                    "max_area"_a)
            .def(
                    "create_mesh_from_selection",
                    [](ccMesh& mesh, bool remove_selected_faces) {
                        return std::shared_ptr<ccMesh>(
                                mesh.createNewMeshFromSelection(
            ...
            .def(
                    "transform_triangle_normals",
                    [](ccMesh& mesh, const Eigen::Matrix4d& transformation) {
                        mesh.transformTriNormals(
            ...
            .def(
                    ...
                    [](ccMesh& mesh, ccGenericPointCloud& cloud) {
                        if (cloud.isKindOf(CV_TYPES::POINT_CLOUD)) {
                            ccPointCloud* vertices =
            ...
            .def("has_vertex_normals", &ccMesh::hasNormals,
            ...
            .def("has_triangle_material_ids", &ccMesh::hasTriangleMaterialIds,
            ...
            .def("deform_as_rigid_as_possible",
                 ...
                 "max_iter"_a,
                 "energy"_a = cloudViewer::GenericMesh::
            ...
            .def_static("compute_triangle_plane", &ccMesh::ComputeTrianglePlane,
                        "Function that computes the plane equation from the "
                        "three points.",
                        "p0"_a, "p1"_a, "p2"_a)
            .def_static("get_ordered_edge", &ccMesh::GetOrderedEdge,
                        "Helper function to get an edge with ordered vertex "
                        "indices.",
                        "vidx0"_a, "vidx1"_a)
            .def_static("get_eigne_ordered_triangle",
                        &ccMesh::GetEigneOrderedTriangle,
                        "Returns eigne ordered triangle.", "vidx0"_a, "vidx1"_a,
                        "vidx2"_a)
            .def_static(
                    "triangulate",
                    [](ccGenericPointCloud& cloud,
                       ...
                       unsigned char dim) {
                        return std::shared_ptr<ccMesh>(ccMesh::Triangulate(
                                &cloud, type, update_normals, max_edge_length,
                                dim));
                    },
            ...
            .def_static(
                    ...
                    [](const ccPointCloud& pcd, double alpha) {
                        return ccMesh::CreateFromPointCloudAlphaShape(pcd,
                                                                      alpha);
                    },
            ...
            .def_static("create_from_point_cloud_poisson",
                        ...
                        "linear_fit"_a = false, "point_weight"_a = 2.0,
                        "samples_per_node"_a = 1.5, "boundary_type"_a = 2,
                        "n_threads"_a = -1)
            ...
            .def_static("create_tetrahedron", &ccMesh::CreateTetrahedron,
            ...
            .def_static("create_octahedron", &ccMesh::CreateOctahedron,
                        ...
                        "radius to the center.",
                        "radius"_a = 1.0, "create_uv_map"_a = false)
            .def_static("create_icosahedron", &ccMesh::CreateIcosahedron,
            ...
            .def_static("create_sphere", &ccMesh::CreateSphere,
                        ...
                        "create_uv_map"_a = false)
            .def_static("create_cylinder", &ccMesh::CreateCylinder,
            ...
            .def_readwrite("triangle_uvs", &ccMesh::triangle_uvs_,
    ...
    docstring::ClassMethodDocInject(m, "ccMesh", "has_triangle_material_ids");
    ...
    docstring::ClassMethodDocInject(m, "ccMesh", "get_non_manifold_vertices");
    ...
    docstring::ClassMethodDocInject(m, "ccMesh", "remove_duplicated_vertices");
    docstring::ClassMethodDocInject(m, "ccMesh", "remove_duplicated_triangles");
    docstring::ClassMethodDocInject(m, "ccMesh",
                                    "remove_unreferenced_vertices");
    docstring::ClassMethodDocInject(m, "ccMesh", "remove_degenerate_triangles");
    docstring::ClassMethodDocInject(m, "ccMesh", "remove_non_manifold_edges");
    docstring::ClassMethodDocInject(
    ...
    docstring::ClassMethodDocInject(m, "ccMesh", "cluster_connected_triangles");
    docstring::ClassMethodDocInject(
            m, "ccMesh", "remove_triangles_by_index",
            {{"triangle_indices",
              "1D array of triangle indices that should be removed from the "
              "TriangleMesh."}});
    docstring::ClassMethodDocInject(m, "ccMesh", "remove_triangles_by_mask",
                                    {{"triangle_mask",
    ...
    docstring::ClassMethodDocInject(
            ...
            {{"vertex_indices",
              ...
              "TriangleMesh."}});
    docstring::ClassMethodDocInject(m, "ccMesh", "remove_vertices_by_mask",
                                    {{"vertex_mask",
                                      "1D bool array, True values indicate "
    ...
    docstring::ClassMethodDocInject(
            ...
            {{"cloud", "a point cloud."},
             ...
             {"max_edge_length",
              "max edge length for output triangles (0 = ignored)."},
    ...
    docstring::ClassMethodDocInject(
            ...
            {{"pcd",
             ...
             {"samples_per_node",
              ...
              "an octree node as the octree construction is adapted to "
              "sampling density."
              "This parameter specifies the minimum number of points that "
              "should fall"
              "within an octree node. For noise-free samples, small values in "
              "the range [1.0 - 5.0]"
              "can be used. For more noisy samples, larger values in the range "
              "[15.0 - 20.0]"
              "may be needed to provide a smoother, noise-reduced, "
              "reconstruction."},
    ...
    docstring::ClassMethodDocInject(
            m, "ccMesh", "create_plane",
            {{"width", "The width of this plane."},
             {"height", "The height of this plane."},
