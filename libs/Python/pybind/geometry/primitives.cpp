...
void pybind_primitives(py::module& m) {
    ...
    py::class_<ccGenericPrimitive, PyGenericPrimitive<ccGenericPrimitive>,
    ...
    primitive
            .def("__repr__",
                 [](const ccGenericPrimitive& prim) {
                     std::string info = fmt::format(
                             ...
                             prim.getTypeName().toStdString(),
                             prim.getDrawingPrecision());
                     return info;
                 })
            .def(
                    "clone",
                    [](const ccGenericPrimitive& prim) {
                        return std::shared_ptr<ccGenericPrimitive>(
                                prim.clone());
                    },
                    "Clones primitive.")
            .def(
            ...
            .def(
                    ...
                    "Sets primitive color (shortcut).", "color"_a)
            .def("has_drawing_precision",
            ...
            .def(
                    "get_transformation",
                    [](const ccGenericPrimitive& prim) {
                        return ccGLMatrixd::ToEigenMatrix4(
                                prim.getTransformation());
                    },
                    "Returns the transformation that is currently applied to "
                    "the vertices.")
            .def(
                    "get_transformation_history",
                    [](const ccGenericPrimitive& prim) {
                        return ccGLMatrixd::ToEigenMatrix4(
                                prim.getGLTransformationHistory());
                    },
    ...
    docstring::ClassMethodDocInject(m, "ccGenericPrimitive", "set_color",
                                    {{"color", "rgb color."}});
    docstring::ClassMethodDocInject(m, "ccGenericPrimitive",
                                    "has_drawing_precision");
    docstring::ClassMethodDocInject(
            m, "ccGenericPrimitive", "set_drawing_precision",
            {{"steps",
              "Warnings: "
              "- steps should always be >= "
              "ccGenericPrimitive::MIN_DRAWING_PRECISION"
              "- changes primitive content(calls "
    ...
    py::class_<ccPlane, ccGenericPrimitive, std::shared_ptr<ccPlane>,
               ccPlanarEntityInterface>
            pyplane(m, "ccPlane", py::multiple_inheritance(),
    ...
    pyplane.def(py::init([](const std::string& name) {
            ...
            .def(py::init([](PointCoordinateType width,
                     ...
                     auto prim =
                             new ccPlane(width, height, &matrix, name.c_str());
            ...
            .def("__repr__",
                 [](ccPlane& plane) {
                     ...
                     std::string info = fmt::format(
                             "ccPlane with faces {}, width {}, height {} and "
                             "equation {}x + {}y + {}z = {}",
                             plane.size(), plane.getXWidth(), plane.getYWidth(),
                             eq[0], eq[1], eq[2], eq[3]);
                     return info;
                 })
            ...
            .def(
                    "set_width",
                    [](ccPlane& plane, PointCoordinateType width,
                       bool auto_update) {
            ...
            .def(
                    "set_height",
                    [](ccPlane& plane, PointCoordinateType height,
                       bool auto_update) {
            ...
            .def(
                    ...
                    "inherited from ccPlanarEntityInterface, returns the plane "
                    "normal.")
            .def(
                    "get_equation",
                    [](const ccPlane& plane) {
                        ...
                        return std::make_tuple(CCVector3d::fromArray(N),
                                               constVal);
                    },
                    "Returns the equation of the plane. Equation: N.P + "
                    "constVal = 0; "
    ...
    py::class_<ccBox, PyGenericPrimitive<ccBox>, std::shared_ptr<ccBox>,
               ccGenericPrimitive>
    ...
    pybox.def(py::init([](const std::string& name) {
            ...
            .def(py::init([](const Eigen::Vector3d& dims,
                     ...
                     const ccGLMatrix matrix =
                             ccGLMatrix::FromEigenMatrix(trans_matrix);
                     auto prim = new ccBox(CCVector3::fromArray(dims), &matrix,
                                           name.c_str());
                     prim->clearTriNormals();
                     return prim;
                 }),
                 "Box dimensions axis along each dimension are defined in a "
                 "single 3D vector: "
                 ...
                 "\n"
                 "-param name name.",
                 "dims"_a, "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "name"_a = "Box")
            .def("__repr__",
                 [](const ccBox& box) {
            ...
            .def(
                    "set_dimensions",
                    [](ccBox& box, const Eigen::Vector3d& dims) {
    ...
    pysphere.def(py::init([](const std::string& name) {
            ...
            .def(py::init([](PointCoordinateType radius,
                     ...
                     const ccGLMatrix matrix =
                             ccGLMatrix::FromEigenMatrix(trans_matrix);
                     auto prim = new ccSphere(radius, &matrix, name.c_str(),
                                              precision);
                     prim->clearTriNormals();
                     return prim;
                 }),
                 ...
                 "precision"_a = 24, "name"_a = "Sphere")
            .def("__repr__",
                 [](const ccSphere& sphere) {
                     std::string info =
                             fmt::format("ccSphere with faces {} and radius {}",
                                         sphere.size(), sphere.getRadius());
                     return info;
                 })
            .def("get_radius", &ccSphere::getRadius, "Returns sphere radius.")
            .def("set_radius", &ccSphere::setRadius, "Sets sphere radius.",
                 "radius"_a);

    docstring::ClassMethodDocInject(m, "ccSphere", "get_radius");
    docstring::ClassMethodDocInject(
            m, "ccSphere", "set_radius",
            {{"radius",
              "sphere radius, warning changes primitive content "
              "(calls ccGenericPrimitive::updateRepresentation)."}});

    // cloudViewer.geometry.ccTorus
    py::class_<ccTorus, PyGenericPrimitive<ccTorus>, std::shared_ptr<ccTorus>,
               ccGenericPrimitive>
    ...
    pytorus.def(py::init([](const std::string& name) {
            ...
            .def(py::init([](PointCoordinateType inside_radius,
                             ...
                             double angle_rad, bool rectangular_section,
                     ...
                     auto prim = new ccTorus(inside_radius, outside_radius,
                                             angle_rad, rectangular_section,
                                             rect_section_height, &matrix,
                 ...
                 "\n"
                 "-param rectangular_section whether section is rectangular or "
                 "round.",
                 "\n"
                 "-param rect_section_height section height (if rectangular "
                 "torus).",
                 "\n"
                 "-param trans_matrix optional 3D transformation (can be set "
                 "afterwards with ccDrawableObject::setGLTransformation); "
                 ...
                 "\n"
                 "-param name name.",
                 "inside_radius"_a, "outside_radius"_a,
                 "angle_rad"_a = 2.0 * M_PI, "rectangular_section"_a = false,
                 "rect_section_height"_a = 0,
                 "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "precision"_a = 24, "name"_a = "Torus")
            .def("__repr__",
                 [](const ccTorus& torus) {
                     std::string info = fmt::format(
                             "ccTorus with faces {}, inside-radius {}, "
                             "outside-radius {} and height {}",
                             torus.size(), torus.getInsideRadius(),
                             torus.getOutsideRadius(),
                             torus.getRectSectionHeight());
                     return info;
                 })
            .def("get_inside_radius", &ccTorus::getInsideRadius,
                 "Returns the torus inside radius.")
            .def("get_outside_radius", &ccTorus::getOutsideRadius,
                 "Returns the torus outside radius.")
            .def("get_rect_section_height", &ccTorus::getRectSectionHeight,
                 "Returns the torus rectangular section height (along Y-axis) "
                 "if applicable.")
            .def("get_rectangular_section", &ccTorus::getRectSection,
                 "Returns whether torus has a rectangular (true) or circular "
                 "(false) section.")
            .def("get_angle_rad", &ccTorus::getAngleRad,
    ...
    pyquadric
            ...
            .def(py::init([](const Eigen::Vector2d& min_corner,
                     ...
                     const ccGLMatrix matrix =
                             ccGLMatrix::FromEigenMatrix(trans_matrix);
                     CCVector2 minCorner(
                             static_cast<PointCoordinateType>(min_corner(0)),
                             static_cast<PointCoordinateType>(min_corner(1)));
                     CCVector2 maxCorner(
                             static_cast<PointCoordinateType>(max_corner(0)),
                     ...
                     auto prim =
                             new ccQuadric(minCorner, maxCorner, eq, &dims,
                 ...
                 "\n"
                 "-param min_corner min corner of the 'representation' base "
                 "area; "
                 "\n"
                 "-param max_corner max corner of the 'representation' base "
                 "area; "
                 "\n"
                 "-param equation equation coefficients ( Z = a + b.X + c.Y + "
                 "d.X^2 + e.X.Y + f.Y^2); "
                 "\n"
                 ...
                 "\n"
                 "-param name name.",
                 "min_corner"_a, "max_corner"_a, "equation"_a,
                 "dimensions"_a = Eigen::Vector3i::Ones(),
                 "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "precision"_a = 24, "name"_a = "Quadric")
            .def("__repr__",
                 [](const ccQuadric& quadric) {
                     std::string info = fmt::format(
                             "ccQuadric with faces {} and equations: {}",
                             quadric.size(),
                             quadric.getEquationString().toStdString());
                     return info;
                 })
            .def(
                    "get_min_corner",
                    [](const ccQuadric& quadric) {
                        return Eigen::Vector2d(quadric.getMinCorner().x,
                                               quadric.getMinCorner().y);
                    },
                    "Returns the quadric min corner.")
            .def(
                    "get_max_corner",
                    [](const ccQuadric& quadric) {
                        return Eigen::Vector2d(quadric.getMaxCorner().x,
                                               quadric.getMaxCorner().y);
                    },
                    "Returns the quadric max corner.")
            .def(
                    "get_equation_coefficient",
                    [](const ccQuadric& quadric) {
                        const PointCoordinateType* equation =
                                quadric.getEquationCoefs();
            ...
            .def(
                    ...
                    [](const ccQuadric& quadric) {
                        ...
                        return Eigen::Vector3i(static_cast<int>(dims.x),
                                               static_cast<int>(dims.y),
                                               static_cast<int>(dims.z));
                    },
                    "Returns the quadric equation 'coordinate system' (X,Y,Z "
                    "dimensions indexes).")
            .def(
                    "get_equation_string",
                    [](const ccQuadric& quadric) {
            ...
            .def(
                    "project_on_quadric",
                    [](const ccQuadric& quadric, const Eigen::Vector3d& point) {
                        const CCVector3 P = CCVector3::fromArray(point);
                        CCVector3 Q;
                        PointCoordinateType elevation =
                                quadric.projectOnQuadric(P, Q);
                        return std::make_tuple(elevation,
            ...
            .def_static(
                    ...
                    [](cloudViewer::GenericIndexedCloudPersist& cloud) {
                        ...
                        return std::make_tuple(
                                std::shared_ptr<ccQuadric>(quadric), rms);
                    },
    ...
    py::class_<ccCone, PyGenericPrimitive<ccCone>, std::shared_ptr<ccCone>,
               ccGenericPrimitive>
    ...
    pycone.def(py::init([](const std::string& name) {
            ...
            .def(py::init([](PointCoordinateType bottom_radius,
                 ...
                 "\n"
                 "-param height cone height (transformation should point to "
                 "the axis center); "
                 "\n"
                 ...
                 "bottom_radius"_a, "top_radius"_a, "height"_a, "x_off"_a = 0,
                 "y_off"_a = 0, "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "precision"_a = 24, "name"_a = "Cone")
            .def("__repr__",
                 [](const ccCone& cone) {
                     std::string info = fmt::format(
                             "ccCone with faces {}, bottom radius {}, top "
                             "radius {} and height {}",
                             cone.size(), cone.getBottomRadius(),
                             cone.getTopRadius(), cone.getHeight());
                     return info;
                 })
            .def("get_height", &ccCone::getHeight, "Returns cone height.")
            .def("set_height", &ccCone::setHeight, "Sets cone height",
                 "height"_a)
            .def("get_bottom_radius", &ccCone::getBottomRadius,
                 "Returns cone bottom radius.")
            .def("set_bottom_radius", &ccCone::setBottomRadius,
                 "Sets cone bottom radius.", "radius"_a)
            .def("get_top_radius", &ccCone::getTopRadius,
                 "Returns cone top radius.")
            .def("set_top_radius", &ccCone::setTopRadius,
                 "Sets cone top radius.", "radius"_a)
            .def(
                    "get_bottom_center",
                    [](const ccCone& cone) {
                        CCVector3d::fromArray(cone.getBottomCenter());
                    },
                    "Returns cone axis bottom end point after applying "
                    "transformation")
            .def(
                    "get_top_center",
                    [](const ccCone& cone) {
                    ...
                    "Returns cone axis top end point after applying "
                    "transformation")
            .def(
                    "get_small_center",
                    [](const ccCone& cone) {
                    ...
                    "Returns cone axis end point associated with whichever "
                    "radii is smaller")
            .def(
                    "get_large_center",
                    [](const ccCone& cone) {
                    ...
                    "Returns cone axis end point associated with whichever "
                    "radii is larger")
            .def("get_small_radius", &ccCone::getSmallRadius,
                 "Returns whichever cone radii is smaller")
            .def("get_large_radius", &ccCone::getLargeRadius,
                 "Returns whichever cone radii is larger")
            .def("is_snout_mode", &ccCone::isSnoutMode,
    ...
    pycylinder
            ...
            .def(py::init([](PointCoordinateType radius,
                     ...
                     const ccGLMatrix matrix =
                             ccGLMatrix::FromEigenMatrix(trans_matrix);
                     auto prim = new ccCylinder(radius, height, &matrix,
                                                name.c_str(), precision);
                     prim->clearTriNormals();
                     return prim;
                 }),
                 "Cylinder axis corresponds to the 'Z' dimension: "
                 "Internally represented by a cone with the same top and "
                 "bottom radius. "
                 "\n"
                 "-param radius cylinder radius"
                 "\n"
                 "-param height cylinder height(transformation should point to "
                 "the axis center)"
                 "\n"
                 "-param trans_matrix optional 3D transformation (can be set "
                 "afterwards with ccDrawableObject::setGLTransformation); "
                 ...
                 "radius"_a, "height"_a,
                 "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "precision"_a = 24, "name"_a = "Cylinder")
            .def("__repr__", [](const ccCylinder& cylinder) {
                std::string info = fmt::format(
                        "ccCylinder with faces {}, radius {} and height {}",
                        cylinder.size(), cylinder.getBottomRadius(),
                        cylinder.getHeight());
                return info;
            });

    // cloudViewer.geometry.ccDish
    py::class_<ccDish, PyGenericPrimitive<ccDish>, std::shared_ptr<ccDish>,
               ccGenericPrimitive>
    ...
    pydish.def(py::init([](const std::string& name) {
            ...
            .def(py::init([](PointCoordinateType base_radius,
                             PointCoordinateType height,
                     ...
                     const ccGLMatrix matrix =
                             ccGLMatrix::FromEigenMatrix(trans_matrix);
                     auto prim = new ccDish(base_radius, height, second_radius,
                                            &matrix, name.c_str(), precision);
                     prim->clearTriNormals();
                     return prim;
                 }),
                 "dish dimensions axis along each dimension are defined in a "
                 "single 3D vector: "
                 ...
                 "\n"
                 "-param radius2 If radius2 is zero, dish is drawn as a "
                 "section of sphere. If radius2 is >0, dish is defined as half "
                 "of an ellipsoid.; "
                 "\n"
                 "-param trans_matrix optional 3D transformation (can be set "
                 "afterwards with ccDrawableObject::setGLTransformation); "
                 ...
                 "\n"
                 "-param name name.",
                 "base_radius"_a, "height"_a, "second_radius"_a = 0,
                 "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "precision"_a = 24, "name"_a = "Dish")
            .def("__repr__",
                 [](const ccDish& dish) {
                     std::string info = fmt::format(
                             "ccDish with faces {}, R1 {}, R2 {} and heigth {}",
                             dish.size(), dish.getBaseRadius(),
                             dish.getSecondRadius(), dish.getHeight());
                     return info;
                 })
            .def("get_base_radius", &ccDish::getBaseRadius,
                 "Returns the dish base radius.")
            .def("get_second_radius", &ccDish::getSecondRadius,
    ...
    py::class_<ccExtru, PyGenericPrimitive<ccExtru>, std::shared_ptr<ccExtru>,
               ccGenericPrimitive>
    ...
    pyextru.def(py::init([](const std::string& name) {
            ...
            .def(py::init([](const std::vector<Eigen::Vector2d>& profile,
                     ...
                     auto prim = new ccExtru(tempProfile, height, &matrix,
                                             name.c_str());
                     prim->clearTriNormals();
                     return prim;
                 }),
                 "extru dimensions axis along each dimension are defined in a "
                 "single 3D vector: "
                 ...
                 "\n"
                 "-param name name.",
                 "profile"_a, "height"_a,
                 "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "name"_a = "Extrusion")
            .def("__repr__",
                 [](const ccExtru& extru) {
                     std::string info =
                             fmt::format("ccExtru with faces {} and height {}",
                                         extru.size(), extru.getThickness());
                     return info;
                 })
            .def("get_thickness", &ccExtru::getThickness,
                 "Returns extrusion thickness.")
            .def(
                    "get_profile",
                    [](const ccExtru& extru) {
                        const std::vector<CCVector2>& profile =
                                extru.getProfile();
                        std::vector<Eigen::Vector2d> outProfile;
                        for (const auto& pro : profile) {
                            outProfile.emplace_back(
                                    Eigen::Vector2d(pro.x, pro.y));
                        }
                        return outProfile;
                    },
    ...
    py::class_<ccCoordinateSystem, PyGenericPrimitive<ccCoordinateSystem>,
               std::shared_ptr<ccCoordinateSystem>, ccGenericPrimitive>
            pyCS(m, "ccCoordinateSystem", "The 3D Coordinate System.");
    py::detail::bind_default_constructor<ccCoordinateSystem>(pyCS);
    py::detail::bind_copy_functions<ccCoordinateSystem>(pyCS);
    pyCS.def(py::init([](const std::string& name) {
                 return new ccCoordinateSystem(name.c_str());
             }),
             "Simplified constructor", "name"_a = "CoordinateSystem")
            .def(py::init([](PointCoordinateType display_scale,
                             PointCoordinateType axis_width,
                             const Eigen::Matrix4d& trans_matrix,
                             const std::string& name) {
                     const ccGLMatrix matrix =
                             ccGLMatrix::FromEigenMatrix(trans_matrix);
                     auto prim = new ccCoordinateSystem(
                             display_scale, axis_width, &matrix, name.c_str());
                     prim->clearTriNormals();
                     return prim;
                 }),
                 "Coordinate System is essentially just a way to visualize a "
                 "transform matrix. param transMat optional "
                 "3D transformation (can be set afterwards with "
                 "ccDrawableObject::setGLTransformation) param name name.",
                 "display_scale"_a, "axis_width"_a,
                 "trans_matrix"_a = Eigen::Matrix4d::Identity(),
                 "name"_a = "Box")
            .def(py::init([](const Eigen::Matrix4d& trans_matrix,
                             const std::string& name) {
                     const ccGLMatrix matrix =
                             ccGLMatrix::FromEigenMatrix(trans_matrix);
                     auto prim = new ccCoordinateSystem(&matrix, name.c_str());
                     prim->clearTriNormals();
                     return prim;
                 }),
                 "Coordinate System is essentially just a way to visualize a "
                 ...
                 "name"_a = "Box")
            .def("__repr__",
                 [](const ccCoordinateSystem& cs) {
                     const CCVector3 origin = cs.getOrigin();
                     PointCoordinateType axis_width = cs.getAxisWidth();
                     PointCoordinateType display_scale = cs.getDisplayScale();
                     std::string info = fmt::format(
                             "ccCoordinateSystem with origin ({}, {}, {}) "
                             "display scale {} and axis width {}",
                             origin.x, origin.y, origin.z, display_scale,
                             axis_width);
                     return info;
                 })
            .def("axis_planes_shown", &ccCoordinateSystem::axisPlanesAreShown,
                 "Returns whether axis planes are Shown.")
            .def("show_axis_planes", &ccCoordinateSystem::ShowAxisPlanes,
                 "Sets whether axis planes are Shown.", "show"_a)
            .def("axis_lines_shown", &ccCoordinateSystem::axisLinesAreShown,
                 "Returns whether axis lines are Shown.")
            .def("show_axis_lines", &ccCoordinateSystem::ShowAxisLines,
                 "Sets whether axis lines are Shown.", "show"_a)
            .def("get_axis_width", &ccCoordinateSystem::getAxisWidth,
                 "Returns axis width.")
            .def("set_axis_width", &ccCoordinateSystem::setAxisWidth,
                 "Sets axis width.", "width"_a)
            .def("get_display_scale", &ccCoordinateSystem::getDisplayScale,
                 "Returns display scale.")
            .def("set_display_scale", &ccCoordinateSystem::setDisplayScale,
                 "Sets display scale.", "scale"_a)
            .def("get_xy_plane", &ccCoordinateSystem::getXYplane,
                 "Returns xy plane.")
            .def("get_yz_plane", &ccCoordinateSystem::getYZplane,
                 "Returns yz plane.")
            .def("get_zx_plane", &ccCoordinateSystem::getZXplane,
                 "Returns zx plane.")
            .def(
                    "get_origin",
                    [](ccCoordinateSystem& cs) {
                        return CCVector3d::fromArray(cs.getOrigin());
                    },
                    "Returns CoordinateSystem origin.");

    docstring::ClassMethodDocInject(m, "ccCoordinateSystem",
                                    "axis_planes_shown");
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem", "show_axis_planes",
                                    {{"show", "axis planes shown flag."}});
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem",
                                    "axis_lines_shown");
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem", "show_axis_lines",
                                    {{"show", "axis lines shown flag."}});

    docstring::ClassMethodDocInject(m, "ccCoordinateSystem", "get_axis_width");
    docstring::ClassMethodDocInject(
            m, "ccCoordinateSystem", "set_axis_width",
            {{"width", "Coordinate system axis width."}});
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem",
                                    "get_display_scale");
    docstring::ClassMethodDocInject(
            m, "ccCoordinateSystem", "set_display_scale",
            {{"scale", "Coordinate system display scale."}});
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem", "get_xy_plane");
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem", "get_yz_plane");
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem", "get_zx_plane");
    docstring::ClassMethodDocInject(m, "ccCoordinateSystem", "get_origin");
}
