...
void pybind_matrix(py::module &m) {
    ...
    py::class_<utility::Matrix<PointCoordinateType>> te_p2p(
            m, "Matrix", "Class to interface of numpy and std::vector.");

    py::detail::bind_default_constructor<utility::Matrix<PointCoordinateType>>(
            te_p2p);
    py::detail::bind_copy_functions<utility::Matrix<PointCoordinateType>>(
            te_p2p);

    te_p2p.def(py::init([](const std::vector<size_t> &shape,
                           const PointCoordinateType *data) {
            ...
            .def(
                    "shape",
                    [](const utility::Matrix<PointCoordinateType> &s,
                       size_t ndim = 0) { return s.shape(ndim); },
                    "ndim"_a, "Function to get matrix shape")
            .def(
                    "strides",
                    [](const utility::Matrix<PointCoordinateType> &s,
                       bool bytes = false) { return s.strides(bytes); },
