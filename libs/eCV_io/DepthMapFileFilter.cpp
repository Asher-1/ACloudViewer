...
bool DepthMapFileFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
CC_FILE_ERROR DepthMapFileFilter::saveToFile(const QString& filename, ccGBLSensor* sensor)
{
	...
	if (db.zBuff.empty())
	{
		CVLog::Warning(QString("[DepthMap] sensor '%1' has no associated depth map (you must compute it first)").arg(sensor->getName()));
	...
	if (!cloud)
	{
		CVLog::Warning(QString("[DepthMap] sensor '%1' is not associated to a point cloud!").arg(sensor->getName()));
		//this is not a severe error (the process can go on)
	}

	if (CheckForSpecialChars(filename))
	{
		CVLog::Warning(QString("[DepthMap] Output filename contains special characters. It might be scrambled or rejected by the I/O filter..."));
	}
	...
	if (!fp)
	{
		CVLog::Error(QString("[DepthMap] Can't open file '%1' for writing!").arg(filename));
	...
	if (cloud && cloud->isA(CV_TYPES::POINT_CLOUD))
	{
		...
		if (nbPoints == 0)
		{
			CVLog::Warning(QString("[DepthMap] sensor '%1' is associated to an empty cloud?!").arg(sensor->getName()));
			//this is not a severe error (the process can go on)
		}
		else
		{
			...
			if (pc->hasNormals())
			{
				...
				{
					CVLog::Warning(QString("[DepthMap] not enough memory to load normals on sensor '%1'!").arg(sensor->getName()));
				}
			}
			...
			if (pc->hasColors())
			{
				...
				{
					CVLog::Warning(QString("[DepthMap] not enough memory to load colors on sensor '%1'!").arg(sensor->getName()));
				}
			}
		}
	}
