...
bool RasterGridFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
	//not supported yet
	return false;
}
...
CC_FILE_ERROR RasterGridFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		if( poDataset != nullptr )
		{
			...
			{
				CVLog::Print( "Origin = (%.6f,%.6f)", adfGeoTransform[0], adfGeoTransform[3] );
				CVLog::Print( "Pixel Size = (%.6f,%.6f)", adfGeoTransform[1], adfGeoTransform[5] );
			}
			...
			//check for 'big' coordinates
			{
				...
				if (HandleGlobalShift(origin, Pshift, preserveCoordinateShift, parameters))
				{
					...
					CVLog::Warning("[RasterFilter::loadFile] Raster has been recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
				}
			}
			...
			for (int i = 1; i <= rasterCount; ++i)
			{
				...
				CVLog::Print( "[GDAL] Block=%dx%d, Type=%s, ColorInterp=%s", nBlockXSize, nBlockYSize, GDALGetDataTypeName(poBand->GetRasterDataType()), GDALGetColorInterpretationName(colorInterp) );
				...
				CVLog::Print( "[GDAL] Min=%.3fd, Max=%.3f", adfMinMax[0], adfMinMax[1] );
				...
				else //colors
				{
					...
					case GCI_AlphaBand:
						...
						{
							CVLog::Warning(QString("Alpha band ignored as it has a unique value (%1)").arg(adfMinMax[0]));
						}
						break;
					default:
						isScalar = true;
					...
					if (isRGB || isPalette)
					{
						...
						{
							CVLog::Warning(QString("Band is declared as a '%1' but no palette is associated!").arg(GDALGetColorInterpretationName(colorInterp)));
						}
						else
						{
							//instantiate memory for RBG colors if necessary
							if (!loadAsTexturedQuad && !pc->hasColors() && !pc->setRGBColor(ecvColor::MAX, ecvColor::MAX, ecvColor::MAX))
							{
								CVLog::Warning(QString("Failed to instantiate memory for storing color band '%1'!").arg(GDALGetColorInterpretationName(colorInterp)));
							}
							else
							{
									...
									for (int k = 0; k < nXSize; ++k)
									{
										...
										if (loadAsTexturedQuad || pointIndex <= pc->size())
										{
											...
											if (loadAsTexturedQuad)
											{
												...
												C = ecvColor::FromQRgba(origColor);
											}
											else
											{
												const ecvColor::Rgb& origColor = pc->getPointColor(pointIndex);
												C = ecvColor::Rgba(origColor, ecvColor::MAX);
											}
											...
											case GCI_PaletteIndex:
												assert(colTable);
												{
													...
													C.r = static_cast<ColorCompType>(col.c1 & ecvColor::MAX);
													C.g = static_cast<ColorCompType>(col.c2 & ecvColor::MAX);
													C.b = static_cast<ColorCompType>(col.c3 & ecvColor::MAX);
												}
												break;

											case GCI_RedBand:
												C.r = static_cast<ColorCompType>(colIndexes[k] & ecvColor::MAX);
												break;
											case GCI_GreenBand:
												C.g = static_cast<ColorCompType>(colIndexes[k] & ecvColor::MAX);
												break;
											case GCI_BlueBand:
												C.b = static_cast<ColorCompType>(colIndexes[k] & ecvColor::MAX);
												break;

											case GCI_AlphaBand:
												C.a = static_cast<ColorCompType>(colIndexes[k] & ecvColor::MAX);
												break;
					...
					else if (isScalar && !loadAsTexturedQuad)
					{
						...
						{
							CVLog::Warning(QString("Failed to instantiate memory for storing '%1' as a scalar field!").arg(sf->getName()));
							sf->release();
							sf = nullptr;
						}
						else
			...
			else if (pc)
			{
				if (!zRasterProcessed)
				{
					CVLog::Warning("Raster has no height (Z) information: you can convert one of its scalar fields to Z with 'Edit > Scalar Fields > Set SF as coordinate(s)'");
				}
				else if (zInvalid != 0 && zInvalid < pc->size())
				{
					...
					if (QMessageBox::question(0, "Remove NaN points?", "This raster has pixels with invalid heights. Shall we remove them?", QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes)
					{
						...
						if (error)
						{
							CVLog::Error("Not enough memory to remove the points with invalid heights!");
						}
					}
				}
