...
public:
    ...
    CCVector3 convertPoint(double x, double y, double z)
    {
        ...
        if (m_firstPoint)
        {
            if (	(!m_preserveCoordinateShift || ecvGlobalShiftManager::NeedShift(P + m_globalShift))
                &&	FileIOFilter::HandleGlobalShift(P, m_globalShift, m_preserveCoordinateShift, m_loadParameters) )
            {
                CVLog::Warning("[DxfImporter] All points/vertices will be recentered! Translation: (%.2f ; %.2f ; %.2f)", m_globalShift.x, m_globalShift.y, m_globalShift.z);
            }
            m_firstPoint = false;
        }
    ...
    void add3dFace(const DL_3dFaceData& face) override
    {
        ...
        if (faceCol)
        {
            ...
            else if (vertices->setRGBColor(ecvColor::white))
            {
                ...
                for (unsigned i = 0; i < createdVertCount; ++i)
                    vertices->setPointColor(vertCount - 1 - i, ecvColor::Rgba(*faceCol, ecvColor::MAX));
