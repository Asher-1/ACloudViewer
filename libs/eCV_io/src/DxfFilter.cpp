...
bool DxfFilter::canSave(CV_CLASS_ENUM type,
                        bool& multiple,
                        bool& exclusive) const {
    if (type == CV_TYPES::POLY_LINE || type == CV_TYPES::MESH ||
...
public:
    ...
    CCVector3 convertPoint(double x, double y, double z) {
        ...
        if (m_firstPoint) {
            if ((!m_preserveCoordinateShift ||
                 ecvGlobalShiftManager::NeedShift(P + m_globalShift)) &&
                FileIOFilter::HandleGlobalShift(P, m_globalShift,
                                                m_preserveCoordinateShift,
                                                m_loadParameters)) {
                CVLog::Warning(
                        "[DxfImporter] All points/vertices will be recentered! "
                        "Translation: (%.2f ; %.2f ; %.2f)",
                        m_globalShift.x, m_globalShift.y, m_globalShift.z);
            }
            m_firstPoint = false;
        }
    ...
    void applyGlobalShift() {
        if (m_preserveCoordinateShift) {
            if (m_points) m_points->setGlobalShift(m_globalShift);
            if (m_polyVertices) m_polyVertices->setGlobalShift(m_globalShift);
        }
    }
    ...
    void addPoint(const DL_PointData& P) override {
        ...
        if (getCurrentColour(col)) {
            ...
            if (m_points->hasColors()) {
                ...
                // reserve memory (and fill the previous points with a default
                // color if necessary)
                ...
                m_points->setPointColor(
                        m_points->size() - 1,
                        ecvColor::white);  // replace the last color
            }
    ...
    void addPolyline(const DL_PolylineData& poly) override {
        ...
        if (!m_polyVertices->reserve(poly.number) ||
            !m_poly->reserve(poly.number)) {
            CVLog::Error("[DxfImporter] Not enough memory!");
            delete m_poly;
            m_polyVertices = nullptr;
            m_poly = nullptr;
        ...
        // m_poly->set2DMode(poly.flags & 8); //DGM: "2D" polylines in CC
    ...
    void addVertex(const DL_VertexData& vertex) override {
        ...
        if (m_poly && m_polyVertices
            ...
            m_polyVertices->addPoint(
                    convertPoint(vertex.x, vertex.y, vertex.z));
    ...
    void add3dFace(const DL_3dFaceData& face) override {
        ...
        if (!m_faces) {
            ...
            // vertices->setLocked(true);  //DGM: no need to lock it as it is
            // only used by one mesh!
            if (m_preserveCoordinateShift) {
        ...
        ccPointCloud* vertices =
                dynamic_cast<ccPointCloud*>(m_faces->getAssociatedCloud());
        if (!vertices) {
            assert(false);
            return;
        }
        ...
        if (getCurrentColour(col)) faceCol = &col;
            ...
            for (unsigned i = 0; i < addedVertCount; ++i) {
                for (unsigned j = 0; j < vertCount; ++j) {
                    ...
                    if (P[i].x == Pj->x && P[i].y == Pj->y && P[i].z == Pj->z) {
                        ...
                        // We must also check that the color is the same (if
                        // any)
                        if (faceCol || vertices->hasColors()) {
                            const ecvColor::Rgb* _faceCol =
                                    faceCol ? faceCol : &ecvColor::white;
                            const ecvColor::Rgb* _vertCol =
                                    vertices->hasColors()
                                            ? &vertices->getPointColor(j)
                                            : &ecvColor::white;
                            useCurrentVertex = (_faceCol->r == _vertCol->r &&
                                                _faceCol->g == _vertCol->g &&
                                                _faceCol->b == _vertCol->b);
                        }
        ...
        {
            for (unsigned i = 0; i < addedVertCount; ++i)
                if (vertIndexes[i] < 0) ++createdVertCount;
        }
        ...
        if (addedVertCount == 4)
            m_faces->addTriangle(vertIndexes[0], vertIndexes[2],
                                 vertIndexes[3]);
        ...
        // add per-triangle normals
        {
            ...
            if (!triNormsTable->reserveSafe(triNormsTable->currentSize() +
                                            addTriCount)) {
                CVLog::Error("[DxfImporter] Not enough memory!");
                return;
            }
        ...
        if (faceCol) {
            ...
            // otherwise, reserve memory and set all previous points to white by
            // default
            else if (vertices->setRGBColor(ecvColor::white)) {
                // then replace the last color(s) by the current one
                for (unsigned i = 0; i < createdVertCount; ++i)
                    vertices->setPointColor(
                            vertCount - 1 - i,
                            ecvColor::Rgba(*faceCol, ecvColor::MAX));
    ...
    void addRay(const DL_RayData&) override {
        CVLog::Warning("[DxfImporter] Ray not supported");
    }
    ...
    void addArc(const DL_ArcData& data) override {
        ...
        unsigned vertexCount =
                ...
                static_cast<unsigned>(std::max(
                        1.0, arcLength_deg));  // we use a 1 degree resolution
                                               // by default for now
        double step_deg = 1.0;
        ...
        if (!polyVertices->reserve(vertexCount) ||
        ...
        for (unsigned i = 0; i < vertexCount; ++i) {
            ...
            double angle_rad = cloudViewer::DegreesToRadians(angle_deg);
            CCVector3 P =
                    Clocal + CCVector3(static_cast<PointCoordinateType>(
                                               data.radius * cos(angle_rad)),
                                       static_cast<PointCoordinateType>(
                                               data.radius * sin(angle_rad)),
                                       0);
    ...
    void addCircle(const DL_CircleData&) override {
        CVLog::Warning("[DxfImporter] Circle not supported");
    }

    void addEllipse(const DL_EllipseData&) override {
        CVLog::Warning("[DxfImporter] Ellipse not supported");
    }

    void addSpline(const DL_SplineData&) override {
        CVLog::Warning("[DxfImporter] Spline not supported");
    }

    void addControlPoint(const DL_ControlPointData&) override {
        CVLog::Warning("[DxfImporter] Control point not supported");
    }

    void addFitPoint(const DL_FitPointData&) override {
        CVLog::Warning("[DxfImporter] Fit point not supported");
    }

    void addKnot(const DL_KnotData&) override {
        CVLog::Warning("[DxfImporter] Knot not supported");
    }

    void addInsert(const DL_InsertData&) override {
        CVLog::Warning("[DxfImporter] Insert not supported");
    }

    void addSolid(const DL_SolidData&) override {
        CVLog::Warning("[DxfImporter] Solid not supported");
    }

    void addXLine(const DL_XLineData&) override {
        CVLog::Warning("[DxfImporter] XLine not supported");
    }
...
private:
    //! Returns current colour (either the current data's colour or the current
    //! layer's one)
    bool getCurrentColour(ecvColor::Rgb& ccColour) {
        ...
        if (colourIndex == 0) {
            ...
            // an attribute of 256 means the colours are BYLAYER, so grab it
            // from our map instead
            ...
            if (colourIndex == defaultIndex) return false;
        }

        ccColour.r = static_cast<ColorCompType>(dxfColors[colourIndex][0] *
                                                ecvColor::MAX);
        ccColour.g = static_cast<ColorCompType>(dxfColors[colourIndex][1] *
                                                ecvColor::MAX);
        ccColour.b = static_cast<ColorCompType>(dxfColors[colourIndex][2] *
                                                ecvColor::MAX);

        return true;
    }

    //! Keep track of the colour of each layer in case the colour attribute is
    //! set to BYLAYER
...
CC_FILE_ERROR DxfFilter::saveToFile(ccHObject* root,
                                    const QString& filename,
                                    const SaveParameters& parameters) {
#ifndef CV_DXF_SUPPORT

    CVLog::Error(
            "[DXF] DXF format not supported! Check compilation parameters!");
    return CC_FERR_CONSOLE_ERROR;

#else

    if (!root || filename.isEmpty()) return CC_FERR_BAD_ARGUMENT;

    ccHObject::Container polylines;
    root->filterChildren(polylines, true, CV_TYPES::POLY_LINE);
    if (root->isKindOf(CV_TYPES::POLY_LINE)) polylines.push_back(root);
    ccHObject::Container meshes;
    root->filterChildren(meshes, true, CV_TYPES::MESH);
    if (root->isKindOf(CV_TYPES::MESH)) meshes.push_back(root);
    ccHObject::Container clouds;
    root->filterChildren(clouds, true, CV_TYPES::POINT_CLOUD,
                         true);  // we don't want polylines!
    if (root->isKindOf(CV_TYPES::POINT_CLOUD)) clouds.push_back(root);
            ...
            for (ccHObject* cloud : clouds) {
                ...
                if (parent) {
                    if (parent->isA(CV_TYPES::POLY_LINE)) {
                        if (std::find(polylines.begin(), polylines.end(),
                                      parent) != polylines.end()) {
                            // the parent is already in the 'polylines' set
                            continue;
                        }
                    } else if (parent->isKindOf(CV_TYPES::MESH)) {
                        if (std::find(meshes.begin(), meshes.end(), parent) !=
                            meshes.end()) {
    ...
    if (polyCount + meshCount + cloudCount == 0) return CC_FERR_NO_SAVE;
    ...
    ccHObject::GlobalBoundingBox globalBB;
    {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            for (size_t i = 0; i < container.size(); ++i) {
                ccHObject::GlobalBoundingBox bb =
                        container[i]->getOwnGlobalBB();
                // update global BB
                globalBB += bb;
            }
        }
    ...
    if (CheckForSpecialChars(filename)) {
        CVLog::Warning(QString(
                "[DXF] Output filename contains special characters. It might "
                "be scrambled or rejected by the third party library..."));
    }
    ...
    try {
        ...
        // Writing the Linetypes (all by default)
        {
            ...
            dxf.writeLinetype(*dw, DL_LinetypeData("CONTINUOUS", "Continuous",
                                                   0, 0, 0.0));
            dw->tableEnd();
        }

        // Writing the Layers
        dw->tableLayers(static_cast<int>(polyCount + meshCount + cloudCount) +
                        1);
        ...
        QStringList pointLayerNames;
        {
            ...
            for (unsigned i = 0; i < polyCount; ++i) {
                ...
                QString layerName =
                        QString("POLYLINE_%1").arg(i + 1, 3, 10, QChar('0'));
                ...
                dxf.writeLayer(
                        *dw,
                        ...
                        DL_Attributes(std::string(""), DL_Codes::green,
                                      static_cast<int>(lineWidth), "CONTINUOUS",
                                      1.0));
            }
            ...
            for (unsigned j = 0; j < meshCount; ++j) {
                ...
                // but it can't be longer than 31 characters (R14 limit)
                QString layerName =
                        QString("MESH_%1").arg(j + 1, 3, 10, QChar('0'));
                ...
                dxf.writeLayer(
                        *dw,
                        DL_LayerData(qPrintable(layerName),
                                     0),  // DGM: warning, toStdString doesn't
                                          // preserve "local" characters
                        DL_Attributes(std::string(""), DL_Codes::magenta,
                                      static_cast<int>(lineWidth), "CONTINUOUS",
                                      1.0));
            }
            ...
            for (unsigned j = 0; j < cloudCount; ++j) {
                ...
                // but it can't be longer than 31 characters (R14 limit)
                QString layerName =
                        QString("CLOUD_%1").arg(j + 1, 3, 10, QChar('0'));
        ...
        // Writing the Entities Section
        {
            ...
            for (unsigned i = 0; i < polyCount; ++i) {
                ...
                if (!poly->is2DMode()) flags |= 8;  // 3D polyline
                dxf.writePolyline(
                        *dw,
                        DL_PolylineData(static_cast<int>(vertexCount), 0, 0,
                                        flags),
                        DL_Attributes(
                                qPrintable(polyLayerNames[i]),
                                DL_Codes::bylayer, -1, "BYLAYER",
                                1.0));  // DGM: warning, toStdString doesn't
                                        // preserve "local" characters
            ...
            for (unsigned j = 0; j < meshCount; ++j) {
                ...
                for (unsigned f = 0; f < triCount; ++f) {
                    const cloudViewer::GenericTriangle* tri =
                            mesh->_getNextTriangle();
                    ...
                    dxf.write3dFace(
                            *dw,
                            DL_3dFaceData(A.x, A.y, A.z, B.x, B.y, B.z, C.x,
                                          C.y, C.z, C.x, C.y, C.z, lineWidth),
                            DL_Attributes(
                                    qPrintable(meshLayerNames[j]),
                                    DL_Codes::bylayer, -1, "BYLAYER",
                                    1.0));  // DGM: warning, toStdString doesn't
                                            // preserve "local" characters
                }
            ...
            for (unsigned i = 0; i < cloudCount; ++i) {
                const ccPointCloud* cloud =
                        static_cast<ccPointCloud*>(clouds[i]);
                ...
                for (unsigned j = 0; j < pointCount; ++j) {
                    ...
                    dxf.writePoint(
                            *dw, DL_PointData(Pg.x, Pg.y, Pg.z),
                            DL_Attributes(
                                    qPrintable(pointLayerNames[i]),
                                    DL_Codes::bylayer, -1, "BYLAYER",
                                    1.0));  // DGM: warning, toStdString doesn't
                                            // preserve "local" characters
                }
...
CC_FILE_ERROR DxfFilter::loadFile(const QString& filename,
                                  ccHObject& container,
    ...
    try {
        ...
        if (CheckForSpecialChars(filename)) {
            CVLog::Warning(
                    "[DXF] Input file contains special characters. It might be "
                    "rejected by the third party library...");
        }

        if (DL_Dxf().in(qPrintable(filename),
                        &importer))  // DGM: warning, toStdString doesn't
                                     // preserve "local" characters
        {
            importer.applyGlobalShift();  // apply the (potential) global shift
                                          // to shared clouds
            if (container.getChildrenNumber() != 0) result = CC_FERR_NO_ERROR;
        } else {
