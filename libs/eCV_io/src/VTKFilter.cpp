...
CC_FILE_ERROR VTKFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	if (vertices->hasColors())
	{
		...
		for (unsigned i = 0; i < ptsCount; ++i)
		{
			...
			outFile << static_cast<float>(C.r) / ecvColor::MAX << " " << static_cast<float>(C.g) / ecvColor::MAX << " " << static_cast<float>(C.b) / ecvColor::MAX << endl;
		}
	}
...
CC_FILE_ERROR VTKFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	else if (fileType.startsWith("ASCII"))
	{
		...
		{
			CVLog::Error(QString("VTK entity '%1' is not supported!").arg(dataType));
			return CC_FERR_WRONG_FILE_TYPE;
		}
	}
	...
	while (error == CC_FERR_NO_ERROR)
	{
		...
		if (nextline.startsWith("POINTS"))
		{
			...
			//{
			//	CVLog::Error(QString("Non floating point data (%1) is not supported!").arg(dataFormat));
			...
			while (iPt < ptsCount)
			{
				...
				for (int i = 0; i < parts.size(); ++i)
				{
					...
					if (!ok)
					{
						CVLog::Warning("[VTK] Element #%1 of POINTS data is corrupted!", iPt);
						error = CC_FERR_MALFORMED_FILE;
						iPt = ptsCount;
						break;
					}
					...
					if (coordIndex == 2)
					{
						...
						if (iPt == 0)
						{
							...
							if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, parameters))
							{
								...
								CVLog::Warning("[VTKFilter::loadFile] Cloud has been recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
							}
						}
		...
		else if (nextline.startsWith("POLYGONS") || nextline.startsWith("TRIANGLE_STRIPS"))
		{
			...
			if (!mesh)
			{
				CVLog::Warning(QString("[VTK] We found %1 data while file is not composed of POLYDATA!").arg(typeName));
			...
			for (unsigned i = 0; i < elemCount; ++i)
			{
				...
				{
					CVLog::Warning(QString("[VTK] Element #%1 of %2 data is corrupted! (not enough indexes)").arg(i).arg(typeName));
				}

				if (isPolygon && (vertCount != 3 && vertCount != 4)) //quads are easy to handle as well!
				{
					CVLog::Warning(QString("[VTK] POLYGON element #%1 has an unhandled size (> 4 vertices)").arg(i));
					continue;
				}
				...
				for (unsigned j = 0; j < vertCount; ++j)
				{
					...
					if (!ok)
					{
						CVLog::Warning(QString("[VTK] Element #%1 of %2 data is corrupted! (invalid index value)").arg(i).arg(typeName));
						error = CC_FERR_MALFORMED_FILE;
						break;
					}
				}
		...
		else if (nextline.startsWith("NORMALS"))
		{
			...
			while (iNorm < lastDataSize)
			{
				...
				for (int i = 0; i < parts.size(); ++i)
				{
					...
					if (!ok)
					{
						CVLog::Warning("[VTK] Element #%1 of NORMALS data is corrupted!", iNorm);
						error = CC_FERR_MALFORMED_FILE;
						iNorm = lastDataSize;
						break;
					}
		...
		else if (nextline.startsWith("COLOR_SCALARS"))
		{
			...
			while (iCol < lastDataSize)
			{
				...
				{
					bool ok;
					rgb.rgb[coordIndex] = static_cast<ColorCompType>(parts[i].toDouble(&ok) * ecvColor::MAX);
					if (!ok)
					{
						CVLog::Warning("[VTK] Element #%1 of COLOR_SCALARS data is corrupted!", iCol);
						error = CC_FERR_MALFORMED_FILE;
						iCol = lastDataSize;
						break;
					}
		...
		else if (nextline.startsWith("LOOKUP_TABLE") || nextline.startsWith("VECTORS"))
		{
			...
			{
				CVLog::Warning(QString("[VTK] field %1 has no size?!").arg(itemName));
				error = CC_FERR_MALFORMED_FILE;
				break;
			}
			...
			{
				CVLog::Warning(QString("[VTK] field %1 has not the right number of points (will be ignored)").arg(itemName));
			}
			createSF &= (acceptLookupTables || expected);
			if (createSF && lastSfName.isNull())
			{
				CVLog::Warning(QString("[VTK] field %1 has no name (will be ignored)").arg(itemName));
				createSF = false;
			}
			else if (!expected)
			{
				CVLog::Warning(QString("[VTK] field %1 was not expected (will be ignored)").arg(itemName));
			}
			...
			if (createSF)
			{
				...
				{
					CVLog::Warning(QString("[VTK] Not enough memory to load scalar field' %1' (will be ignored)").arg(lastSfName));
					sf->release();
					sf = nullptr;
				}
			}
			...
			while (iScal < lastDataSize)
			{
				...
				if (expected)
				{
					for (int i = 0; i < parts.size(); ++i)
					{
						...
						if (!ok)
						{
							CVLog::Warning("[VTK] Element #%1 of LOOKUP_TABLE/VECTORS data is corrupted!", iScal);
							error = CC_FERR_MALFORMED_FILE;
							if (sf)
							{
								sf->release();
		...
		else //unhandled property (CELLS, CELL_TYPES, etc.)
		{
			...
			if (parts.size() < 2)
			{
				CVLog::Warning(QString("[VTK] Unhandled element: %1").arg(parts[0]));
				error = CC_FERR_MALFORMED_FILE;
				break;
			}
	...
	if (mesh)
	{
		...
		{
			//	mesh->computeNormals();
			CVLog::Warning("[VTK] Mesh has no normal! You can manually compute them (select it then call \"Tools > Normals > Compute\")");
		}
