...
BinFilter::BinFilter()
    : FileIOFilter({"_CloudCompare BIN Filter",
                    1.0f,  // priority
                    QStringList{"bin"}, "bin", QStringList{GetFileFilter()},
                    QStringList{GetFileFilter()}, Import | Export | BuiltIn}) {}

bool BinFilter::canSave(CV_CLASS_ENUM type,
                        bool& multiple,
...
static int ReadEntityHeader(QFile& in,
    ...
    // number of points
    uint32_t ptsCount;
    if (in.read((char*)&ptsCount, 4) < 0) return -1;
    numberOfPoints = (unsigned)ptsCount;

    // flags (colors, etc.)
    uint8_t flag;
    if (in.read((char*)&flag, 1) < 0) return -1;
...
CC_FILE_ERROR BinFilter::saveToFile(ccHObject* root,
    ...
    QFile out(filename);
    if (!out.open(QIODevice::WriteOnly)) return CC_FERR_WRITING;
    ...
    // concurrent call using lambda (no global variables)
...
CC_FILE_ERROR BinFilter::SaveFileV2(QFile& out, ccHObject* object) {
    if (!object) return CC_FERR_BAD_ARGUMENT;
    ...
    while (!toCheck.empty()) {
        ...
        // we check objects that have links to other entities (meshes,
        // polylines, etc.)
        std::unordered_set<const ccHObject*> dependencies;
        if (currentObject->isA(CV_TYPES::MESH) ||
            currentObject->isKindOf(CV_TYPES::PRIMITIVE)) {
            ...
            cloudViewer::GenericIndexedCloudPersist* cloud =
                    static_cast<ccPolyline*>(currentObject)
                            ->getAssociatedCloud();
            ccPointCloud* pc = dynamic_cast<ccPointCloud*>(cloud);
            if (pc)
                dependencies.insert(pc);
            else
                CVLog::Warning(QString("[BIN] Poyline '%1' is associated to an "
                                       "unhandled vertices structure?!")
                                       .arg(currentObject->getName()));
        } else if (currentObject->isKindOf(CV_TYPES::SENSOR)) {
            ccIndexedTransformationBuffer* buffer =
                    static_cast<ccSensor*>(currentObject)->getPositions();
            if (buffer) dependencies.insert(buffer);
        } else if (currentObject->isA(CV_TYPES::LABEL_2D)) {
            ...
            if (facet->getPolygon()) dependencies.insert(facet->getPolygon());
            if (facet->getContour()) dependencies.insert(facet->getContour());
        } else if (currentObject->isKindOf(CV_TYPES::IMAGE)) {
            ccImage* image = static_cast<ccImage*>(currentObject);
            if (image->getAssociatedSensor())
                dependencies.insert(image->getAssociatedSensor());
        }

        for (std::unordered_set<const ccHObject*>::const_iterator it =
                     dependencies.begin();
             it != dependencies.end(); ++it) {
            if (!object->find((*it)->getUniqueID())) {
                CVLog::Warning(
                        QString("[BIN] Dependency broken: entity '%1' must "
                                "also be in selection in order to save '%2'")
                                .arg((*it)->getName(),
...
CC_FILE_ERROR BinFilter::loadFile(const QString& filename,
    ...
    CVLog::Print(QString("[BIN] Opening file '%1'...").arg(filename));

    // opening file
    QFile in(filename);
    if (!in.open(QIODevice::ReadOnly)) return CC_FERR_READING;

    uint32_t firstBytes = 0;
    if (in.read((char*)&firstBytes, 4) < 0) return CC_FERR_READING;
    bool v1 = (strncmp((char*)&firstBytes, "CCB", 3) != 0);

    if (v1) {
        return LoadFileV1(
                in, container, static_cast<unsigned>(firstBytes),
                parameters);  // firstBytes == number of scans for V1 files!
    } else {
        // Since ver 2.5.2, the 4th character of the header corresponds to 'load
        // flags'
        int flags = 0;
        {
            ...
            if (!ok || flags > 8) {
                CVLog::Error(QString("Invalid file header (4th byte is '%1'?!)")
                                     .arg(c));
...
inline bool Match(ccHObject* object,
                  unsigned uniqueID,
                  CV_CLASS_ENUM expectedType) {
    return object && object->getUniqueID() == uniqueID &&
           object->isKindOf(expectedType);
}
...
ccHObject* FindRobust(ccHObject* root,
    ...
    while (it != oldToNewIDMap.end() && it.key() == oldUniqueID) {
        ...
        if (source) {
            ...
            if (Match(parent, uniqueID, expectedType)) return parent;

            // now test the children
            for (unsigned i = 0; i < source->getChildrenNumber(); ++i) {
                ccHObject* child = source->getChild(i);
                if (Match(child, uniqueID, expectedType)) return child;
            }
        }
...
CC_FILE_ERROR BinFilter::LoadFileV2(QFile& in,
                                    ccHObject& container,
                                    int flags,
                                    bool parallel,
    ...
    QString coordsFormat =
            ((flags & ccSerializableObject::DF_POINT_COORDS_64_BITS) ? "double"
                                                                     : "float");
    QString scalarFormat =
            ((flags & ccSerializableObject::DF_SCALAR_VAL_32_BITS) ? "float"
                                                                   : "double");
    CVLog::Print(QString("[BIN] Version %1.%2 (coords: %3 / scalar: %4)")
                         .arg(binVersion / 10)
                         .arg(binVersion % 10)
                         .arg(coordsFormat)
                         .arg(scalarFormat));
    ...
    CV_CLASS_ENUM classID =
            ccObject::ReadClassIDFromFile(in, static_cast<short>(binVersion));
    ...
    if (parallel && parentWidget) {
        pDlg.reset(new ecvProgressDialog(false, parentWidget));
        pDlg->setMethodTitle(QObject::tr("BIN file"));
    ...
    if (classID == CV_TYPES::CUSTOM_H_OBJECT) {
        ...
        root->fromFileNoChildren(in, static_cast<short>(binVersion), flags,
                                 oldToNewIDMap);  // this will load it
    ...
    if (parallel) {
        ...
        QFuture<bool> future = QtConcurrent::run([&]() {
            return root->fromFile(in, static_cast<short>(binVersion), flags,
                                  oldToNewIDMap);
        });
        ...
        while (!future.isFinished()) {
            ...
            // pDlg.setValue(static_cast<int>(in.pos())); //DGM: in fact, the
            // file reading part is just half of the work!
            QApplication::processEvents();
        }

        success = future.result();
    } else {
        success = root->fromFile(in, static_cast<short>(binVersion), flags,
                                 oldToNewIDMap);
    }
    ...
    while (!toCheck.empty()) {
        ...
        // we check objects that have links to other entities (meshes,
        // polylines, etc.)
        if (currentObject->isKindOf(CV_TYPES::MESH)) {
            // specific case: mesh groups are deprecated!
            if (currentObject->isA(CV_TYPES::MESH_GROUP)) {
                // TODO
                CVLog::Warning(QString("[BIN] Mesh groups are deprecated! "
                                       "Entity %1 should be ignored...")
                                       .arg(currentObject->getName()));
            } else if (currentObject->isA(CV_TYPES::SUB_MESH)) {
                ccSubMesh* subMesh = ccHObjectCaster::ToSubMesh(currentObject);

                // normally, the associated mesh should be the sub-mesh's
                // parent! however we have its ID so we will look for it just to
                // be sure
                intptr_t meshID = (intptr_t)subMesh->getAssociatedMesh();
                if (meshID > 0) {
                    ccHObject* mesh = FindRobust(root, subMesh, oldToNewIDMap,
                                                 static_cast<unsigned>(meshID),
                                                 CV_TYPES::MESH);
                    if (mesh) {
                        subMesh->setAssociatedMesh(
                                ccHObjectCaster::ToMesh(mesh),
                                false);  //'false' because previous mesh is not
                                         // null (= real mesh ID)!!!
                    } else {
                        // we have a problem here ;)
                        // normally, the associated mesh should be the
                        // sub-mesh's parent!
                        if (subMesh->getParent() &&
                            subMesh->getParent()->isA(CV_TYPES::MESH)) {
                            subMesh->setAssociatedMesh(
                                    ccHObjectCaster::ToMesh(
                                            subMesh->getParent()),
                                    false);  //'false' because previous mesh is
                                             // not null (= real mesh ID)!!!
                        } else {
                            subMesh->setAssociatedMesh(
                                    nullptr,
                                    false);  //'false' because previous mesh is
                            ...
                            // ;) delete subMesh;
                            CVLog::Warning(
                                    QString("[BIN] Couldn't find associated "
                                            "mesh (ID=%1) for sub-mesh '%2' in "
                                            "the file!")
                                            .arg(meshID)
                                            .arg(subMesh->getName()));
            ...
            } else if (currentObject->isA(CV_TYPES::MESH) ||
                       currentObject->isKindOf(
                               CV_TYPES::PRIMITIVE))  // CV_TYPES::MESH or
                                                      // CV_TYPES::PRIMITIVE!
            {
                ccMesh* mesh = ccHObjectCaster::ToMesh(currentObject);
                assert(mesh);
                ...
                if (cloudID > 0) {
                    ccHObject* cloud =
                            FindRobust(root, mesh, oldToNewIDMap,
                                       static_cast<unsigned>(cloudID),
                                       CV_TYPES::POINT_CLOUD);
                    if (cloud) {
                        ccGenericPointCloud* genericCloud =
                                ccHObjectCaster::ToGenericPointCloud(cloud);
                        assert(genericCloud);
                        ...
                        CVLog::Warning(
                                QString("[BIN] Couldn't find vertices (ID=%1) "
                                        "for mesh '%2' in the file!")
                                        .arg(cloudID)
                                        .arg(mesh->getName()));
                        if (root == mesh &&
                            !ContinueAfterError(forceLoadAfterError)) {
                            return CC_FERR_MALFORMED_FILE;
                        }
                        currentObject = mesh = nullptr;
                    }
                } else {
                    CVLog::Warning(QString("[BIN] Mesh '%1' has no vertices! "
                                           "It will be ignored.")
                                           .arg(mesh->getName()));
                    if (mesh->getParent()) {
                        mesh->getParent()->removeChild(mesh);
                ...
                if (mesh) {
                    ...
                    if (matSetID > 0) {
                        materials = FindRobust(root, mesh, oldToNewIDMap,
                                               static_cast<unsigned>(matSetID),
                                               CV_TYPES::MATERIAL_SET);
                        if (materials) {
                            mesh->setMaterialSet(
                                    static_cast<ccMaterialSet*>(materials),
                                    false);
                        } else {
                            // we have a (less severe) problem here ;)
                            mesh->setMaterialSet(nullptr, false);
                            mesh->showMaterials(false);
                            CVLog::Warning(QString("[BIN] Couldn't find shared "
                                                   "materials set (ID=%1) for "
                                                   "mesh '%2' in the file!")
                                                   .arg(matSetID)
                                                   .arg(mesh->getName()));
                            result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
                    ...
                    intptr_t triNormsTableID =
                            (intptr_t)mesh->getTriNormsTable();
                    if (triNormsTableID > 0) {
                        triNormsTable = FindRobust(
                                root, mesh, oldToNewIDMap,
                                static_cast<unsigned>(triNormsTableID),
                                CV_TYPES::NORMAL_INDEXES_ARRAY);
                        if (triNormsTable) {
                            mesh->setTriNormsTable(
                                    static_cast<NormsIndexesTableType*>(
                                            triNormsTable),
                                    false);
                        } else {
                            // we have a (less severe) problem here ;)
                    ...
                    intptr_t texCoordArrayID =
                            (intptr_t)mesh->getTexCoordinatesTable();
                    if (texCoordArrayID > 0) {
                        texCoordsTable = FindRobust(
                                root, mesh, oldToNewIDMap,
                                static_cast<unsigned>(texCoordArrayID),
                                CV_TYPES::TEX_COORDS_ARRAY);
                        if (texCoordsTable) {
                            mesh->setTexCoordinatesTable(
                                    static_cast<TextureCoordsContainer*>(
                                            texCoordsTable),
                                    false);
                        } else {
                            // we have a (less severe) problem here ;)
                            ...
                            CVLog::Warning(
                                    QString("[BIN] Couldn't find shared "
                                            "texture coordinates (ID=%1) for "
                                            "mesh '%2' in the file!")
                                            .arg(texCoordArrayID)
                                            .arg(mesh->getName()));
                            result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
                    ...
                    if (checkErrors) {
                        ...
                        for (unsigned i = 0; i < faceCount; ++i) {
                            ...
                            if (tri->i1 >= vertCount || tri->i2 >= vertCount ||
                                tri->i3 >= vertCount) {
                                CVLog::Warning(
                                        QString("[BIN] File is corrupted: "
                                                "missing vertices for mesh "
                                                "'%1'!")
                                                .arg(mesh->getName()));

                                // add cloud to the 'orphans' set
                                pc->setName(mesh->getName() + QString(".") +
                                            pc->getName());
                                orphans->addChild(pc);
                                if (texCoordsTable) {
                                    texCoordsTable->setName(
                                            mesh->getName() + QString(".") +
                                            texCoordsTable->getName());
                                    orphans->addChild(texCoordsTable);
                                }
                                if (triNormsTable) {
                                    triNormsTable->setName(
                                            mesh->getName() + QString(".") +
                                            triNormsTable->getName());
                                    orphans->addChild(triNormsTable);
                                }
                                if (materials) {
                                    materials->setName(mesh->getName() +
                                                       QString(".") +
                                                       materials->getName());
            ...
            poly->cloudViewer::Polyline::setAssociatedCloud(
                    nullptr);  // we have to bypass the automatic removal of
                               // flags, as the current vertices pointer is
                               // 'invalid'

            ccHObject* cloudEntity = FindRobust(root, poly, oldToNewIDMap,
                                                static_cast<unsigned>(cloudID),
                                                CV_TYPES::POINT_CLOUD);
            if (cloudEntity) {
                ccGenericPointCloud* cloud =
                        ccHObjectCaster::ToGenericPointCloud(cloudEntity);
                ...
                for (unsigned i = 0; i < poly->size(); ++i) {
                    if (poly->getPointGlobalIndex(i) >= pointCount) {
                        CVLog::Warning(
                                QString("[BIN] Polyline '%1' (ID=%2) seems "
                                        "corrupted!")
                                        .arg(poly->getName())
                                        .arg(poly->getUniqueID()));
                        delete poly;
                ...
                // delete root;
                currentObject = nullptr;
                CVLog::Warning(QString("[BIN] Couldn't find vertices (ID=%1) "
                                       "for polyline '%2' in the file!")
                                       .arg(cloudID)
                                       .arg(poly->getName()));
                if (!ContinueAfterError(forceLoadAfterError)) {
                    return CC_FERR_MALFORMED_FILE;
            ...
            if (bufferID > 0) {
                ccHObject* buffer = FindRobust(root, sensor, oldToNewIDMap,
                                               static_cast<unsigned>(bufferID),
                                               CV_TYPES::TRANS_BUFFER);
                if (buffer) {
                    sensor->setPositions(
                            ccHObjectCaster::ToTransBuffer(buffer));
                } else {
                    // we have a problem here ;)
                    ...
                    currentObject = nullptr;
                    CVLog::Warning(
                            QString("[BIN] Couldn't find trans. buffer (ID=%1) "
                                    "for sensor '%2' in the file!")
                                    .arg(bufferID)
                                    .arg(sensor->getName()));

                    // positions are optional, so we can simply set them to
                    // nullptr and go ahead, we do not need to return. return
            ...
            for (unsigned i = 0; !invalidLabel && i < label->size(); ++i) {
                ...
                if (pp.cloud) {
                    ...
                    ccHObject* cloud =
                            FindRobust(root, label, oldToNewIDMap,
                                       static_cast<unsigned>(cloudID),
                                       CV_TYPES::POINT_CLOUD);
                    if (cloud) {
                        ...
                        correctedPickedPoints.push_back(cc2DLabel::PickedPoint(
                                genCloud, pp.index, pp.entityCenterPoint));
                    } else {
                        // we have a problem here ;)
                        CVLog::Warning(QString("[BIN] Couldn't find cloud "
                                               "(ID=%1) associated to label "
                                               "ID=%2 in the file!")
                                               .arg(cloudID)
                                               .arg(label->getUniqueID()));
                        if (label->getParent())
                    ...
                    intptr_t meshID = (intptr_t)pp.mesh;
                    ccHObject* mesh = FindRobust(root, label, oldToNewIDMap,
                                                 static_cast<unsigned>(meshID),
                                                 CV_TYPES::MESH);
                    if (mesh) {
                        ccGenericMesh* genMesh =
                                ccHObjectCaster::ToGenericMesh(mesh);
                        assert(genMesh->size() > pp.index);
                        correctedPickedPoints.push_back(
                                cc2DLabel::PickedPoint(genMesh, pp.index, pp.uv,
                                                       pp.entityCenterPoint));
                    } else {
                        // we have a problem here ;)
                        CVLog::Warning(QString("[BIN] Couldn't find mesh "
                                               "(ID=%1) associated to label "
                                               "ID=%2 in the file!")
                                               .arg(meshID)
                                               .arg(label->getUniqueID()));
            ...
            if (label)  // correct label data
            {
                ...
                label->clear(true);
                for (const cc2DLabel::PickedPoint& cpp :
                     correctedPickedPoints) {
                    if (cpp.cloud) {
                        label->addPickedPoint(cpp.cloud, cpp.index,
                                              cpp.entityCenterPoint);
                    } else if (cpp.mesh) {
                        label->addPickedPoint(cpp.mesh, cpp.index, cpp.uv,
                                              cpp.entityCenterPoint);
                    }
                }
            ...
            // origin points
            {
                ...
                if (cloudID > 0) {
                    ccHObject* cloud =
                            FindRobust(root, facet, oldToNewIDMap,
                                       static_cast<unsigned>(cloudID),
                                       CV_TYPES::POINT_CLOUD);
                    if (cloud && cloud->isA(CV_TYPES::POINT_CLOUD)) {
                        facet->setOriginPoints(
                                ccHObjectCaster::ToPointCloud(cloud));
                    } else {
                        // we have a problem here ;)
                        facet->setOriginPoints(nullptr);
                        currentObject = nullptr;
                        CVLog::Warning(
                                QString("[BIN] Couldn't find origin points "
                                        "(ID=%1) for facet '%2' in the file!")
                                        .arg(cloudID)
                                        .arg(facet->getName()));
                    }
                }
            ...
            // contour points
            {
                ...
                if (cloudID > 0) {
                    ...
                    if (cloud) {
                        ...
                        currentObject = nullptr;
                        CVLog::Warning(
                                QString("[BIN] Couldn't find contour points "
                                        "(ID=%1) for facet '%2' in the file!")
                                        .arg(cloudID)
                                        .arg(facet->getName()));
                    }
            ...
            // contour polyline
            {
                ...
                if (polyID > 0) {
                    ccHObject* poly = FindRobust(root, facet, oldToNewIDMap,
                                                 static_cast<unsigned>(polyID),
                                                 CV_TYPES::POLY_LINE);
                    if (poly) {
                        ...
                        currentObject = nullptr;
                        CVLog::Warning(
                                QString("[BIN] Couldn't find contour polyline "
                                        "(ID=%1) for facet '%2' in the file!")
                                        .arg(polyID)
                                        .arg(facet->getName()));
                    }
            ...
            // polygon mesh
            {
                ...
                if (polyID > 0) {
                    ...
                    if (poly) {
                        ...
                        currentObject = nullptr;
                        CVLog::Warning(
                                QString("[BIN] Couldn't find polygon mesh "
                                        "(ID=%1) for facet '%2' in the file!")
                                        .arg(polyID)
                                        .arg(facet->getName()));
                    }
            ...
            if (sensorID > 0) {
                ccHObject* sensor = FindRobust(root, image, oldToNewIDMap,
                                               static_cast<unsigned>(sensorID),
                                               CV_TYPES::CAMERA_SENSOR);
                if (sensor) {
                    image->setAssociatedSensor(
                            ccHObjectCaster::ToCameraSensor(sensor));
                } else {
                    // we have a problem here ;)
                    ...
                    currentObject = nullptr;
                    CVLog::Warning(
                            QString("[BIN] Couldn't find camera sensor (ID=%1) "
                                    "for image '%2' in the file!")
                                    .arg(sensorID)
                                    .arg(image->getName()));
                    // return CC_FERR_MALFORMED_FILE;
                }
        ...
        if (currentObject) {
            const ccShiftedObject* shifted =
                    ccHObjectCaster::ToShifted(currentObject);
            if (shifted) {
                // it may be interesting to re-use the Global Shift when loading
                // other files
                ecvGlobalShiftManager::StoreShift(shifted->getGlobalShift(),
                                                  shifted->getGlobalScale());

                // TODO: we should also check that other entities with global
                // shift not too far away have not already been loaded. In which
                // case we should 'translate' the current entity?
            }
...
CC_FILE_ERROR BinFilter::LoadFileV1(QFile& in,
                                    ccHObject& container,
    ...
    for (unsigned k = 0; k < nbScansTotal; k++) {
        ...
        // Console::print("[BinFilter::loadModelFromBinaryFile] Entity %i : %i
        // points, color=%i, norms=%i,
        ...
        if (pDlg) {
            pDlg->reset();
            pDlg->setInfo(QObject::tr("cloud %1/%2 (%3 points)")
                                  .arg(k + 1)
                                  .arg(nbScansTotal)
                                  .arg(nbOfPoints));
            pDlg->start();
        ...
        if (header.name) {
            for (int i = 0; i < 256; ++i) {
                if (in.read(cloudName + i, 1) < 0) {
                    // Console::print("[BinFilter::loadModelFromBinaryFile]
                    // Error reading the cloud name!\n");
                    return CC_FERR_READING;
                }
                if (cloudName[i] == 0) {
                    break;
        ...
        cloudViewer::ScalarField* loadedCloudSF = nullptr;
        if (!loadedCloud) return CC_FERR_NOT_ENOUGH_MEMORY;

        unsigned fileChunkPos = 0;
        unsigned fileChunkSize =
                std::min(nbOfPoints, CC_MAX_NUMBER_OF_POINTS_PER_CLOUD);
        ...
        if (header.colors) {
            if (loadedCloud->reserveTheRGBTable()) {
                ...
                CVLog::Warning(
                        QString("Failed to allocate RGB colors on cloud '%1'")
                                .arg(loadedCloud->getName()));
                delete loadedCloud;
                return CC_FERR_NOT_ENOUGH_MEMORY;
            }
        }
        if (header.normals) {
            if (loadedCloud->reserveTheNormsTable()) {
                ...
                CVLog::Warning(
                        QString("Failed to allocate normals on cloud '%1'")
                                .arg(loadedCloud->getName()));
                delete loadedCloud;
                return CC_FERR_NOT_ENOUGH_MEMORY;
            }
        ...
        if (header.scalarField) {
            if (loadedCloud->enableScalarField()) {
                ...
                CVLog::Warning(
                        QString("Failed to allocate scalar field on cloud '%1'")
                                .arg(loadedCloud->getName()));
            }
        }
        ...
        for (unsigned i = 0; i < nbOfPoints; ++i) {
            if (lineRead == fileChunkPos + fileChunkSize) {
                ...
                if (header.colors) {
                    if (loadedCloud->reserveTheRGBTable()) {
                        ...
                        CVLog::Warning(QString("Failed to allocate RGB colors "
                                               "on cloud '%1'")
                                               .arg(loadedCloud->getName()));
                        delete loadedCloud;
                        return CC_FERR_NOT_ENOUGH_MEMORY;
                ...
                if (header.normals) {
                    if (loadedCloud->reserveTheNormsTable()) {
                        ...
                        CVLog::Warning(
                                QString("Failed to allocate normals on cloud "
                                        "'%1'")
                                        .arg(loadedCloud->getName()));
                        delete loadedCloud;
                        return CC_FERR_NOT_ENOUGH_MEMORY;
                ...
                if (header.scalarField) {
                    if (loadedCloud->enableScalarField()) {
                        ...
                        CVLog::Warning(QString("Failed to allocate scalar "
                                               "field on cloud '%1'")
                                               .arg(loadedCloud->getName()));
                    }
                }
            ...
            if (in.read((char*)Pf, sizeof(float) * 3) < 0) {
                // Console::print("[BinFilter::loadModelFromBinaryFile] Error
                // reading the %ith entity point !\n",k);
                return CC_FERR_READING;
            }
            ...
            if (header.colors) {
                ...
                if (in.read((char*)C.rgb, sizeof(ColorCompType) * 3) < 0) {
                    // Console::print("[BinFilter::loadModelFromBinaryFile]
                    // Error reading the %ith entity colors !\n",k);
                    return CC_FERR_READING;
                }
            ...
            if (header.normals) {
                ...
                if (in.read((char*)N.u, sizeof(float) * 3) < 0) {
                    // Console::print("[BinFilter::loadModelFromBinaryFile]
                    // Error reading the %ith entity norms !\n",k);
                    return CC_FERR_READING;
                }
            ...
            if (header.scalarField) {
                ...
                if (in.read((char*)&D, sizeof(double)) < 0) {
                    // Console::print("[BinFilter::loadModelFromBinaryFile]
                    // Error reading the %ith entity distance!\n",k);
                    return CC_FERR_READING;
                }
                if (loadedCloudSF) {
        ...
        if (loadedCloudSF) {
            ...
            loadedCloud->setCurrentDisplayedScalarField(
                    loadedCloud->getCurrentInScalarFieldIndex());
