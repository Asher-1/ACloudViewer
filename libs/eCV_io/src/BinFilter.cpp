...
CC_FILE_ERROR BinFilter::SaveFileV2(QFile& out, ccHObject* object)
{
	...
	while (!toCheck.empty())
	{
		...
		if (currentObject->isA(CV_TYPES::MESH) || currentObject->isKindOf(CV_TYPES::PRIMITIVE))
		{
		...
		else if (currentObject->isKindOf(CV_TYPES::POLY_LINE))
		{
			...
			if (pc)
				...
				CVLog::Warning(QString("[BIN] Poyline '%1' is associated to an unhandled vertices structure?!").arg(currentObject->getName()));
		}
		...
		for (std::unordered_set<const ccHObject*>::const_iterator it = dependencies.begin(); it != dependencies.end(); ++it)
		{
			...
			{
				CVLog::Warning(QString("[BIN] Dependency broken: entity '%1' must also be in selection in order to save '%2'").arg((*it)->getName(),currentObject->getName()));
...
CC_FILE_ERROR BinFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	else
	{
		...
		int flags = 0;
		{
			...
			if (!ok || flags > 8)
			{
				CVLog::Error(QString("Invalid file header (4th byte is '%1'?!)").arg(c));
...
inline bool Match(ccHObject* object, unsigned uniqueID, CV_CLASS_ENUM expectedType)
{
	return object && object->getUniqueID() == uniqueID && object->isKindOf(expectedType);
}

ccHObject* FindRobust(ccHObject* root, ccHObject* source,
                      const ccObject::LoadedIDMap& oldToNewIDMap,
                      unsigned oldUniqueID, CV_CLASS_ENUM expectedType)
{
...
CC_FILE_ERROR BinFilter::LoadFileV2(QFile& in, ccHObject& container, int flags)
{
	...
	CVLog::Print(QString("[BIN] Version %1.%2 (coords: %3 / scalar: %4)").arg(binVersion / 10).arg(binVersion % 10).arg(coordsFormat).arg(scalarFormat));
	...
	CV_CLASS_ENUM classID = ccObject::ReadClassIDFromFile(in, static_cast<short>(binVersion));
	...
	while (!toCheck.empty())
	{
		...
		if (currentObject->isKindOf(CV_TYPES::MESH))
		{
			...
			{
				//TODO
				CVLog::Warning(QString("[BIN] Mesh groups are deprecated! Entity %1 should be ignored...").arg(currentObject->getName()));
			}
			else if (currentObject->isA(CV_TYPES::SUB_MESH))
			{
				...
				if (meshID > 0)
				{
                    ccHObject* mesh = FindRobust(root, subMesh, oldToNewIDMap, static_cast<unsigned>(meshID), CV_TYPES::MESH);
					if (mesh)
					{
					...
					else
					{
						...
						//normally, the associated mesh should be the sub-mesh's parent!
						if (subMesh->getParent() && subMesh->getParent()->isA(CV_TYPES::MESH))
						{
						...
						else
						{
							...
							//DGM: can't delete it, too dangerous (bad pointers ;)
							//delete subMesh;
							CVLog::Warning(QString("[BIN] Couldn't find associated mesh (ID=%1) for sub-mesh '%2' in the file!").arg(meshID).arg(subMesh->getName()));
							return CC_FERR_MALFORMED_FILE;
						}
					}
				}
			}
			else if (currentObject->isA(CV_TYPES::MESH) || currentObject->isKindOf(CV_TYPES::PRIMITIVE)) //CV_TYPES::MESH or CV_TYPES::PRIMITIVE!
			{
				ccMesh* mesh = ccHObjectCaster::ToMesh(currentObject);
				assert(mesh);
				...
				if (cloudID > 0)
				{
                    ccHObject* cloud = FindRobust(root, mesh, oldToNewIDMap, static_cast<unsigned>(cloudID), CV_TYPES::POINT_CLOUD);
					if (cloud)
					{
					...
					else
					{
						...
						CVLog::Warning(QString("[BIN] Couldn't find vertices (ID=%1) for mesh '%2' in the file!").arg(cloudID).arg(mesh->getName()));
						if (root == mesh)
						{
							return CC_FERR_MALFORMED_FILE;
						}
				...
				{
					CVLog::Warning(QString("[BIN] Mesh '%1' has no vertices! It will be ignored.").arg(mesh->getName()));
					if (mesh->getParent())
					{
						mesh->getParent()->removeChild(mesh);
					}
				...
				if (mesh)
				{
					...
					if (matSetID > 0)
					{
                        materials = FindRobust(root, mesh, oldToNewIDMap, static_cast<unsigned>(matSetID), CV_TYPES::MATERIAL_SET);
						if (materials)
						{
						...
						else
						{
							...
							CVLog::Warning(QString("[BIN] Couldn't find shared materials set (ID=%1) for mesh '%2' in the file!").arg(matSetID).arg(mesh->getName()));
							result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
					...
					{
                        triNormsTable = FindRobust(root, mesh, oldToNewIDMap, static_cast<unsigned>(triNormsTableID), CV_TYPES::NORMAL_INDEXES_ARRAY);
						if (triNormsTable)
						{
						...
						else
						{
							...
							CVLog::Warning(QString("[BIN] Couldn't find shared normals (ID=%1) for mesh '%2' in the file!").arg(triNormsTableID).arg(mesh->getName()));
							result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
					...
					{
                        texCoordsTable = FindRobust(root, mesh, oldToNewIDMap, static_cast<unsigned>(texCoordArrayID), CV_TYPES::TEX_COORDS_ARRAY);
						if (texCoordsTable)
						{
						...
						else
						{
							...
							CVLog::Warning(QString("[BIN] Couldn't find shared texture coordinates (ID=%1) for mesh '%2' in the file!").arg(texCoordArrayID).arg(mesh->getName()));
							result = CC_FERR_BROKEN_DEPENDENCY_ERROR;
					...
					if (checkErrors)
					{
						...
						for (unsigned i = 0; i < faceCount; ++i)
						{
							...
							if (	tri->i1 >= vertCount
								...
								||	tri->i3 >= vertCount )
							{
								CVLog::Warning(QString("[BIN] File is corrupted: missing vertices for mesh '%1'!").arg(mesh->getName()));
		...
		else if (currentObject->isKindOf(CV_TYPES::POLY_LINE))
		{
            ...
            ccHObject* cloud = FindRobust(root, poly, oldToNewIDMap, static_cast<unsigned>(cloudID), CV_TYPES::POINT_CLOUD);
			if (cloud)
			{
			...
			else
			{
				...
				//delete root;
                currentObject = nullptr;
				CVLog::Warning(QString("[BIN] Couldn't find vertices (ID=%1) for polyline '%2' in the file!").arg(cloudID).arg(poly->getName()));
				return CC_FERR_MALFORMED_FILE;
			}
		}
		else if (currentObject->isKindOf(CV_TYPES::SENSOR))
		{
			...
			if (bufferID > 0)
			{
                ccHObject* buffer = FindRobust(root, sensor, oldToNewIDMap, static_cast<unsigned>(bufferID), CV_TYPES::TRANS_BUFFER);
				if (buffer)
				{
				...
				else
				{
                    ...
                    currentObject = nullptr;
					CVLog::Warning(QString("[BIN] Couldn't find trans. buffer (ID=%1) for sensor '%2' in the file!").arg(bufferID).arg(sensor->getName()));
		...
		else if (currentObject->isA(CV_TYPES::LABEL_2D))
		{
			...
			for (unsigned i = 0; i < label->size(); ++i)
			{
                ...
                ccHObject* cloud = FindRobust(root, label, oldToNewIDMap, static_cast<unsigned>(cloudID), CV_TYPES::POINT_CLOUD);
				if (cloud)
				{
				...
				{
					//we have a problem here ;)
					CVLog::Warning(QString("[BIN] Couldn't find cloud (ID=%1) associated to label '%2' in the file!").arg(cloudID).arg(label->getName()));
					if (label->getParent())
			...
			if (label) //correct label data
			{
				...
				for (unsigned i = 0; i < correctedPickedPoints.size(); ++i)
					label->addPickedPoint(correctedPickedPoints[i].cloud, correctedPickedPoints[i].index);
		...
		else if (currentObject->isA(CV_TYPES::FACET))
		{
			...
			//origin points
			{
				...
				if (cloudID > 0)
				{
                    ccHObject* cloud = FindRobust(root, facet, oldToNewIDMap, static_cast<unsigned>(cloudID), CV_TYPES::POINT_CLOUD);
					...
					else
					{
                        ...
                        currentObject = nullptr;
						CVLog::Warning(QString("[BIN] Couldn't find origin points (ID=%1) for facet '%2' in the file!").arg(cloudID).arg(facet->getName()));
					}
				}
			}
			//contour points
			{
				...
				if (cloudID > 0)
				{
					...
					else
					{
                        ...
                        currentObject = nullptr;
						CVLog::Warning(QString("[BIN] Couldn't find contour points (ID=%1) for facet '%2' in the file!").arg(cloudID).arg(facet->getName()));
					}
				}
			}
			//contour polyline
			{
				...
				if (polyID > 0)
				{
                    ccHObject* poly = FindRobust(root, facet, oldToNewIDMap, static_cast<unsigned>(polyID), CV_TYPES::POLY_LINE);
					if (poly)
					{
					...
					else
					{
                        ...
                        facet->setContourVertices(nullptr);
                        currentObject = nullptr;
						CVLog::Warning(QString("[BIN] Couldn't find contour polyline (ID=%1) for facet '%2' in the file!").arg(polyID).arg(facet->getName()));
					}
				}
			}
			//polygon mesh
			{
				...
				if (polyID > 0)
				{
                    ccHObject* poly = FindRobust(root, facet, oldToNewIDMap, static_cast<unsigned>(polyID), CV_TYPES::MESH);
					if (poly)
					{
					...
					else
					{
                        ...
                        currentObject = nullptr;
						CVLog::Warning(QString("[BIN] Couldn't find polygon mesh (ID=%1) for facet '%2' in the file!").arg(polyID).arg(facet->getName()));
					}
				}
			}
		}
		else if (currentObject->isKindOf(CV_TYPES::IMAGE))
		{
			...
			if (sensorID > 0)
			{
                ccHObject* sensor = FindRobust(root, image, oldToNewIDMap, static_cast<unsigned>(sensorID), CV_TYPES::CAMERA_SENSOR);
				if (sensor)
				{
				...
				else
				{
                    ...
                    currentObject = nullptr;
					CVLog::Warning(QString("[BIN] Couldn't find camera sensor (ID=%1) for image '%2' in the file!").arg(sensorID).arg(image->getName()));
					//return CC_FERR_MALFORMED_FILE;
				}
			}
		}
		...
		if (currentObject)
		{
			...
			if (shifted)
			{
				...
				ecvGlobalShiftManager::StoreShift(shifted->getGlobalShift(), shifted->getGlobalScale());
...
//			{
//				CVLog::Warning(QString("[BIN] Duplicate 'unique ID' found! (ID = %1)").arg(uniqueID));
