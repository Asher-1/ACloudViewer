...
static double s_maxCloudSizeDoubleSpinBoxValue =
        (CC_MAX_NUMBER_OF_POINTS_PER_CLOUD / 1.0e6);
...
struct AsciiOpenContext {
    ...
    void save(Ui_AsciiOpenDialog* ui) {
        extractSFNameFrom1stLine =
                ui->extractSFNamesFrom1stLineCheckBox->isChecked();
    ...
    void load(Ui_AsciiOpenDialog* ui) const {
        ui->extractSFNamesFrom1stLineCheckBox->setChecked(
                extractSFNameFrom1stLine);
...
AsciiOpenDlg::AsciiOpenDlg(QWidget* parent)
    ...
    connect(m_ui->applyButton, &QPushButton::clicked, this,
            &AsciiOpenDlg::apply);
    connect(m_ui->applyAllButton, &QPushButton::clicked, this,
            &AsciiOpenDlg::applyAll);
    connect(m_ui->cancelButton, &QPushButton::clicked, this,
            &AsciiOpenDlg::reject);
    connect(m_ui->lineEditSeparator, &QLineEdit::textChanged, this,
            &AsciiOpenDlg::onSeparatorChange);
    connect(m_ui->commaDecimalCheckBox, &QCheckBox::toggled, this,
            &AsciiOpenDlg::commaDecimalCheckBoxToggled);
    connect(m_ui->spinBoxSkipLines,
            static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this,
            &AsciiOpenDlg::setSkippedLines);

    // shortcut buttons
    connect(m_ui->toolButtonShortcutSpace, &QToolButton::clicked, this,
            &AsciiOpenDlg::shortcutButtonPressed);
    connect(m_ui->toolButtonShortcutComma, &QToolButton::clicked, this,
            &AsciiOpenDlg::shortcutButtonPressed);
    connect(m_ui->toolButtonShortcutSemicolon, &QToolButton::clicked, this,
            &AsciiOpenDlg::shortcutButtonPressed);

    m_ui->maxCloudSizeDoubleSpinBox->setMaximum(
            CC_MAX_NUMBER_OF_POINTS_PER_CLOUD / 1.0e6);
...
void AsciiOpenDlg::autoFindBestSeparator() {
    const QList<QChar> separators{QChar(' '), QChar(','), QChar(';')};
...
static bool CouldBeX(const QString& colHeader) {
    return colHeader.startsWith(AsciiHeaderColumns::X().toUpper());
}
static bool CouldBeY(const QString& colHeader) {
    return colHeader.startsWith(AsciiHeaderColumns::Y().toUpper());
}
static bool CouldBeZ(const QString& colHeader) {
    return colHeader.startsWith(AsciiHeaderColumns::Z().toUpper());
}
...
static bool CouldBeR(const QString& colHeader) {
    return colHeader == AsciiHeaderColumns::R().toUpper() ||
           colHeader.contains("RED");
}
static bool CouldBeG(const QString& colHeader) {
    return colHeader == AsciiHeaderColumns::G().toUpper() ||
           colHeader.contains("GREEN");
}
static bool CouldBeB(const QString& colHeader) {
    return colHeader == AsciiHeaderColumns::B().toUpper() ||
           colHeader.contains("BLUE");
}
static bool CouldBeA(const QString& colHeader) {
    return colHeader == AsciiHeaderColumns::A().toUpper() ||
           colHeader.contains("ALPHA");
}
static bool CouldBeNx(const QString& colHeader) {
    return colHeader.startsWith(AsciiHeaderColumns::Nx().toUpper()) ||
           (colHeader.contains("NORM") && colHeader.contains("X"));
}
static bool CouldBeNy(const QString& colHeader) {
    return colHeader.startsWith(AsciiHeaderColumns::Ny().toUpper()) ||
           (colHeader.contains("NORM") && colHeader.contains("Y"));
}
static bool CouldBeNz(const QString& colHeader) {
    return colHeader.startsWith(AsciiHeaderColumns::Nz().toUpper()) ||
           (colHeader.contains("NORM") && colHeader.contains("Z"));
}
...
static bool CouldBeScal(const QString& colHeader) {
    return colHeader.contains("SCALAR");
}
...
static const unsigned MAX_COLUMNS =
        512;  // maximum number of columns that can be handled
static const unsigned LINES_READ_FOR_STATS =
        200;                                 // number of lines read for stats
...
static int EnabledBits(unsigned bitField) {
    int count = 0;
    if (bitField & X_BIT) ++count;
    if (bitField & Y_BIT) ++count;
    if (bitField & Z_BIT) ++count;
    if (bitField & W_BIT) ++count;
...
void AsciiOpenDlg::onSeparatorChange(const QString& separator) {
    ...
    m_ui->asciiCodeLabel->setText(
            QString("(ASCII code: %1)").arg(m_separator.unicode()));
...
bool AsciiOpenDlg::useCommaAsDecimal() const {
    return m_ui->commaDecimalCheckBox->isEnabled() &&
           m_ui->commaDecimalCheckBox->isChecked();
}
...
void AsciiOpenDlg::updateTable() {
    ...
    // m_ui->extractSFNamesFrom1stLineCheckBox->setEnabled(false); //we can't do
    // that here (as we may have restored the state of this checkbox before
    // calling updateTable)
    ...
    m_ui->tableWidget->setRowCount(DISPLAYED_LINES +
                                   1);  //+1 for first line shifting
    ...
    std::vector<bool>
            valueIsNumber;  // identifies columns with numbers only [mandatory]
    std::vector<bool> valueIsBelowOne;  // identifies columns with values
                                        // between -1 and 1 only
    std::vector<bool>
            valueIsInteger;  // identifies columns with integer values only
    std::vector<bool> valueIsBelow255;  // identifies columns with integer
        ...
        if (!currentLine.startsWith(
            ...
            if (lineCount < DISPLAYED_LINES) {
                unsigned partsCount = std::min(
                        MAX_COLUMNS, static_cast<unsigned>(parts.size()));
                ...
                if (columnCountHasIncreased) {
                    ...
                    if (m_ui->tableWidget->columnCount() <
                        static_cast<int>(partsCount)) {
                        // DGM: at this stage we must not reduce the table!
                        // The first line is sometimes smaller than the next
                        // ones and we want to keep the widgets/configuration
                        // for the other columns!
                        m_ui->tableWidget->setColumnCount(partsCount);
                    } else if (m_ui->tableWidget->columnCount() >
                               static_cast<int>(partsCount)) {
                        // remove the unnecessary cells!
                        for (int i = static_cast<int>(partsCount);
                             i < m_ui->tableWidget->columnCount(); ++i) {
                            m_ui->tableWidget->setItem(lineCount + 1, i,
                                                       nullptr);
                        }
                    }
                ...
                for (unsigned i = 0; i < partsCount; ++i) {
                    ...
                    if (!isANumber) {
                        ...
                        if (columnCountHasIncreased ||
                            (lineCount == 1 && !valueIsNumber[i])) {
                            // the previous lines were probably header lines
                            // we can forget about their content otherwise it
                            // will prevent us from detecting the right pattern
                        ...
                        valueIsBelowOne[i] =
                                valueIsBelowOne[i] && (std::abs(value) <= 1.0);
                        valueIsInteger[i] = valueIsInteger[i] &&
                                            !parts[i].contains(decimalPoint);
                        valueIsBelow255[i] = valueIsBelow255[i] &&
                                             valueIsInteger[i] &&
                    ...
                    m_ui->tableWidget->setItem(
                            lineCount + 1, i,
                            newItem);  //+1 for first line shifting
                }
            }
            ...
            if (m_skippedLines == 0 && commentLines == 0) {
                // if the very first line is a comment, then we force the user
                // to skip it! this way it will be considered as a header
    ...
    if (commentLines) {
        m_ui->commentLinesSkippedLabel->setText(
                QString("+ %1 comment line(s) skipped").arg(commentLines));
    }
    ...
    unsigned approximateTotalLineCount =
            static_cast<unsigned>(file.size() / m_averageLineSize);
    ...
    // setup table and widgets
    {
        ...
        static const QIcon xIcon(
                QString::fromUtf8(":/Resources/images/typeXCoordinate.png"));
        static const QIcon yIcon(
                QString::fromUtf8(":/Resources/images/typeYCoordinate.png"));
        static const QIcon zIcon(
                QString::fromUtf8(":/Resources/images/typeZCoordinate.png"));
        static const QIcon NormIcon(
                QString::fromUtf8(":/Resources/images/typeNormal.png"));
        static const QIcon RGBIcon(
                QString::fromUtf8(":/Resources/images/typeRgbCcolor.png"));
        static const QIcon GreyIcon(
                QString::fromUtf8(":/Resources/images/typeGrayColor.png"));
        static const QIcon ScalarIcon(
                QString::fromUtf8(":/Resources/images/typeSF.png"));
        static const QIcon LabelIcon(
                QString::fromUtf8(":/Resources/images/dbLabelSymbol.png"));
        ...
        for (unsigned i = 0; i < columnsCount; i++) {
            QComboBox* columnHeaderWidget = static_cast<QComboBox*>(
            ...
            if (!columnHeaderWidget) {
                ...
                columnHeaderWidget->setMaxVisibleItems(
                ...
                columnHeaderWidget->setItemIcon(ASCII_OPEN_DLG_Label,
                                                LabelIcon);
                columnHeaderWidget->setItemIcon(ASCII_OPEN_DLG_Scalar,
                                                ScalarIcon);

                connect(columnHeaderWidget,
                        static_cast<void (QComboBox::*)(int)>(
                                &QComboBox::currentIndexChanged),
                        this, &AsciiOpenDlg::columnsTypeHasChanged);
            }
            ...
            if (!valueIsNumber[i]) {
                ...
                // Line2   : $ id name x y z
                // Line3   : 500
                // Line4   : 0    0001.JPG  753811.417453   4307200.381522
                // 1957.803955 Linex   : ...... Line503 : 499  0500.JPG
                // 753630.672714   4307195.433217      1957.803955
                //
                // Description:
                // once we open the file, we will get a %m_columnType with 5
                // values of "TEXT" then if we choose to skip the 3 first lines,
                // we get a %valueIsNumber with 5 "true" but the %m_columnType
                // is still with 5 values of "TEXT" which leads to the failure!
    ...
    // auto-detect columns 'roles'
    {
        ...
        m_ui->extractSFNamesFrom1stLineCheckBox->setEnabled(
                validHeader);  // can we consider the first ignored line as a
                               // header?
        if (!validHeader) {
            ...
            // std::fill(m_columnType.begin(), m_columnType.end(), UNKNOWN);
            // //if the header has changed, we force update of all columns!
        }
        ...
        if (validHeader) {
            for (unsigned i = 0; i < columnsCount; i++) {
                // we try to guess columns if we have a valid header for the
                // first time!
                if (m_columnType[i] == UNKNOWN || m_columnType[i] == IGNORED) {
                    ...
                    if ((assignedXYZFlags & X_BIT) == 0 &&
                        CouldBeX(colHeader)) {
                        // X
                        columnHeaderWidget->setCurrentIndex(ASCII_OPEN_DLG_X);
                        assignedXYZFlags |=
                                X_BIT;  // update bit field accordingly
                        m_columnType[i] = VALID;
                    } else if ((assignedXYZFlags & Y_BIT) == 0 &&
                               CouldBeY(colHeader)) {
                        // Y
                        columnHeaderWidget->setCurrentIndex(ASCII_OPEN_DLG_Y);
                        assignedXYZFlags |=
                                Y_BIT;  // update bit field accordingly
                        m_columnType[i] = VALID;
                    } else if ((assignedXYZFlags & Z_BIT) == 0 &&
                               CouldBeZ(colHeader)) {
                        // Z
                        columnHeaderWidget->setCurrentIndex(ASCII_OPEN_DLG_Z);
                        assignedXYZFlags |=
                                Z_BIT;  // update bit field accordingly
                        m_columnType[i] = VALID;
                        ...
                        assignedRGBFlags |=
                                Z_BIT;  // update bit field accordingly
                        m_columnType[i] = VALID;
                    } else if ((assignedRGBFlags & W_BIT) == 0 &&
                               CouldBeA(colHeader)) {
                        // Blue
                        columnHeaderWidget->setCurrentIndex(ASCII_OPEN_DLG_A);
                        assignedRGBFlags |=
                                W_BIT;  // update bit field accordingly
                        m_columnType[i] = VALID;
                        ...
                        // Intensity
                        columnHeaderWidget->setCurrentIndex(
                                ASCII_OPEN_DLG_Grey);
                        m_columnType[i] = VALID;
                        ...
                        m_columnType[i] = VALID;
                               ...
                               approximateTotalLineCount < s_maxLabelCount &&
                               CouldBeLabel(colHeader))  // no need to promote
                                                         // labels if there are
                                                         // too many of them ;)
                    {
                        // label
        ...
        // now for the auto-detection whithout header
        {
            for (unsigned i = 0; i < columnsCount; i++) {
                if (m_columnType[i] == TEXT) {
                    if (!labelColumnAssigned && columnsCount > 1 &&
                        approximateTotalLineCount < s_maxLabelCount) {
                        QComboBox* columnHeaderWidget = static_cast<QComboBox*>(
                                m_ui->tableWidget->cellWidget(0, i));
                        assert(columnHeaderWidget);
                ...
                if (m_columnType[i] == UNKNOWN && columnsCount > 1) {
                    ...
                    // by default, we assume that the first columns are always
                    // X,Y and Z
                    if (assignedXYZFlags < XYZ_BITS) {
                        // in rare cases, the first column is an index
                        if (columnsCount > 3 && i == 0 &&
                            (EnabledBits(assignedXYZFlags) == 0) &&
                            valueIsInteger[i] && i + 1 < columnsCount &&
                            !valueIsInteger[i + 1]) {
                        ...
                        // looks like RGB?
                        if (valueIsBelow255[i] && assignedRGBFlags < XYZ_BITS &&
                            (i + 2 - EnabledBits(assignedRGBFlags)) <
                                    columnsCount  // make sure that we can put
                                                  // all values there!
                            &&
                            (EnabledBits(assignedRGBFlags) > 0 ||
                             (valueIsBelow255[i + 1] &&
                              valueIsBelow255[i + 2]))  // make sure that next
                                                        // values are also ok!
                        ) {
                        ...
                        // looks like a normal vector?
                                 ...
                                 (i + 2 - EnabledBits(assignedNormFlags) <
                                  columnsCount)  // make sure that we can put
                                                 // all values there!
                                 &&
                                 (EnabledBits(assignedNormFlags) > 0 ||
                                  (valueIsBelowOne[i + 1] &&
                                   valueIsBelowOne[i + 2]))  // make sure that
                                                             // next values are
                                                             // also ok!
    ...
    checkSelectedColumnsValidity();  // will eventually enable of disable the
                                     // "OK" button
...
void AsciiOpenDlg::checkSelectedColumnsValidity() {
    ...
    bool m_selectedInvalidColumns = false;
    {
        ...
        assert(m_ui->tableWidget->columnCount() >=
               static_cast<int>(m_columnsCount));
        ...
        for (unsigned i = 0; i < m_columnsCount; i++) {
            ...
            } else if (m_columnType[i] == TEXT &&
                       columnHeaderWidget->currentIndex() != 0) {
...
bool AsciiOpenDlg::CheckOpenSequence(const AsciiOpenDlg::Sequence& sequence,
    ...
    // check for doublons
    {
        for (size_t i = 1; i < ASCII_OPEN_DLG_Scalar; i++) {
            if (counters[i] > 1) {
                errorMessage = QString("'%1' defined at least twice!")
                                       .arg(ASCII_OPEN_DLG_TYPES_NAMES[i]);
                return false;
            }
        }
    }
...
bool AsciiOpenDlg::apply() {
    ...
    if (!CheckOpenSequence(getOpenSequence(), errorMessage)) {
        ...
        s_maxCloudSizeDoubleSpinBoxValue =
                m_ui->maxCloudSizeDoubleSpinBox->value();
        accept();
        return true;
    }
}
...
void AsciiOpenDlg::ResetApplyAll() { s_asciiOpenContext.applyAll = false; }

bool AsciiOpenDlg::restorePreviousContext() {
    if (!s_asciiOpenContext.applyAll) return false;

    // restore previous dialog state
    s_asciiOpenContext.load(m_ui);
    m_separator = s_asciiOpenContext.separator;
    m_skippedLines =
            static_cast<unsigned>(std::max(0, s_asciiOpenContext.skipLines));
    updateTable();

    // saved sequence and cloud content don't match!!!
    if (static_cast<size_t>(m_columnsCount) !=
        s_asciiOpenContext.sequence.size()) {
    ...
    for (unsigned i = 0; i < m_columnsCount; i++) {
        QComboBox* combo =
                static_cast<QComboBox*>(m_ui->tableWidget->cellWidget(0, i));
        if (!combo)  // yes, it happens if all lines are skipped!
        {
            s_asciiOpenContext.applyAll =
                    false;  // cancel the 'Apply All' effect
            return false;
        }
    ...
    if (m_columnsCount != 0) {
        ...
        for (unsigned i = 0; i < m_columnsCount; i++) {
            ...
            if (!combo)  // yes, it happens if all lines are skipped!
                break;
            seq.emplace_back(
                    static_cast<CC_ASCII_OPEN_DLG_TYPES>(combo->currentIndex()),
                    headerParts.size() > static_cast<int>(i) ? headerParts[i]
                                                             : QString());
        }
    }
...
bool AsciiOpenDlg::safeSequence() const {
    ...
    if (getColumnsCount() < 6) return true;

    // no header
    if (m_headerLine.isEmpty()) return false;
    ...
    for (int i = 0; i < headerParts.size(); ++i) {
        ...
        switch (seq[i].type) {
            ...
            case ASCII_OPEN_DLG_X:
                if (!CouldBeX(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_Y:
                if (!CouldBeY(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_Z:
                if (!CouldBeZ(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_NX:
                if (!CouldBeNx(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_NY:
                if (!CouldBeNy(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_NZ:
                if (!CouldBeNz(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_R:
            case ASCII_OPEN_DLG_Rf:
                if (!CouldBeR(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_G:
            case ASCII_OPEN_DLG_Gf:
                if (!CouldBeG(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_B:
            case ASCII_OPEN_DLG_Bf:
                if (!CouldBeB(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_A:
            case ASCII_OPEN_DLG_Af:
                if (!CouldBeA(colHeader)) return false;
                break;
            case ASCII_OPEN_DLG_Grey:
                if (!CouldBeGrey(colHeader) && !colHeader.contains("INT"))
                    return false;
                break;
            ...
            case ASCII_OPEN_DLG_RGB32i:
                if (!CouldBeRGBi(colHeader) && !colHeader.contains("RGB"))
                    return false;
                break;
            case ASCII_OPEN_DLG_RGB32f:
                if (!CouldBeRGBf(colHeader) && !colHeader.contains("RGB"))
                    return false;
                break;
            case ASCII_OPEN_DLG_Label:
                if (!CouldBeLabel(colHeader)) return false;
                break;
            default:
                // unhandled case?!
                assert(false);
        ...
        if (changedCombo == combo) {
            if (index == int(ASCII_OPEN_DLG_X) ||
                ...
                if (i + 2 < m_columnsCount) {
                    QComboBox* nextCombo = static_cast<QComboBox*>(
                            m_ui->tableWidget->cellWidget(0, i + 1));
                    QComboBox* nextNextCombo = static_cast<QComboBox*>(
                            m_ui->tableWidget->cellWidget(0, i + 2));
                    // if the two next columns have no assigned type, we set
                    // them auto.
                    if (nextCombo->currentIndex() == int(ASCII_OPEN_DLG_None) &&
                        nextNextCombo->currentIndex() ==
                                int(ASCII_OPEN_DLG_None)) {
                   ...
                   ASCII_OPEN_DLG_Scalar)  // check that the other combo as the
                                           // same index (apart from SF)
        {
...
unsigned AsciiOpenDlg::getMaxCloudSize() const {
    return static_cast<unsigned>(
            floor(m_ui->maxCloudSizeDoubleSpinBox->value() * 1.0e6));
}

bool AsciiOpenDlg::showLabelsIn2D() const {
    return m_ui->show2DLabelsCheckBox->isEnabled() &&
           m_ui->show2DLabelsCheckBox->isChecked();
}
