...
CC_FILE_ERROR PlyFilter::saveToFile(ccHObject* entity, QString filename, e_ply_storage_mode storageType)
{
	...
	if (mesh && mesh->hasMaterials())
	{
		...
		//if (!mesh->hasColors())
		{
			...
			if (textureCount == 1)
			{
				...
				else if (materials->size() > 1)
				{
					if (mesh->hasColors())
					{
						CVLog::Error("PLY files can't handle multiple materials/textures!\nOnly the RGB field will be saved.");
					}
					else
					{
						if (mesh->isA(CV_TYPES::MESH))
						{
														...
														QMessageBox::Yes) == QMessageBox::No)
							{
								...
								{
									CVLog::Error("Conversion failed! (not enough memory?)");
								}
							}
						}
		...
		//else
		//{
		//	CVLog::Warning("[PLY] PLY files can't handle materials/textures! RGB field will be saved instead");
		//	CVLog::Warning("[PLY] Note: you can convert materials/textures to RGB if necessary (see 'Edit > Mesh' menu)");
		//}
	}
	...
	if (material)
	{
		...
		if (!material->hasTexture())
		{
			...
			uniqueColor[0] = static_cast<ColorCompType>(diffuse.r * ecvColor::MAX);
		...
		if (triNum>0 && ply_add_element(ply, "face", triNum))
		{
			...
			if (material)
			{
				...
				else
				{
					...
					CVLog::Print(QString("[PLY] Texture file: %1").arg(textureFilePath));
				}
			}
		}
		else result = 0;
	...
	if (flags & ELEM_EOL)
	{
		...
		if (s_PointCount == 0)
		{
			...
			if (FileIOFilter::HandleGlobalShift(s_Point, s_Pshift, preserveCoordinateShift, s_loadParameters))
			{
				...
				CVLog::Warning("[PLYFilter::loadFile] Cloud (vertices) has been recentered! Translation: (%.2f ; %.2f ; %.2f)", s_Pshift.x, s_Pshift.y, s_Pshift.z);
			}
		}
...
static int rgb_cb(p_ply_argument argument)
{
	...
	case PLY_FLOAT32:
	case PLY_FLOAT64:
		s_color.rgb[flags & POS_MASK] = static_cast<ColorCompType>(std::min(std::max(0.0, ply_get_argument_value(argument)), 1.0) * ecvColor::MAX);
		break;
	case PLY_INT8:
	case PLY_UINT8:
	case PLY_CHAR:
...
static int grey_cb(p_ply_argument argument)
{
	...
	case PLY_FLOAT32:
	case PLY_FLOAT64:
		G = static_cast<ColorCompType>(std::min(std::max(0.0, ply_get_argument_value(argument)), 1.0) * ecvColor::MAX);
		break;
	case PLY_INT8:
	case PLY_UINT8:
	case PLY_CHAR:
...
CC_FILE_ERROR PlyFilter::loadFile(const QString& filename, const QString& inputTextureFilename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		while ((lastElement.elem = ply_get_next_element(ply, lastElement.elem)))
		{
			...
			{
				CVLog::Warning("[PLY] Element '%s' was ignored as it has 0 instance!", lastElement.elementName);
				continue;
			}
	...
	if (stdPropsCount < 2)
	{
		CVLog::Warning("[PLY] This ply file has less than 2 properties defined! (not even X and Y ;)");
	...
	else
	{
			...
			||	singlePropsCount > assignedSingleProperties + 1)
		{
			...
			if (!pod.restorePreviousContext(hasAPreviousContext))
			{
				...
				{
					CVLog::Warning("[PLY] Too many differences with the previous file, we reset the dialog.");
				}
	...
	if (yIndex > 0)
	{
		...
		if (numberOfPoints > 0)
		{
			...
			{
				CVLog::Warning("[PLY] Bad/uncompatible assignation of point properties!");
				delete cloud;
				ply_close(ply);
				return CC_FERR_BAD_ENTITY_TYPE;
			}
	...
	if (numberOfNormals > 0)
	{
		...
		{
			CVLog::Warning("[PLY] The number of normals doesn't match the number of points!");
			delete cloud;
			ply_close(ply);
			return CC_FERR_BAD_ENTITY_TYPE;
		}
	...
	if (iIndex > 0)
	{
		if (numberOfColors > 0)
		{
			CVLog::Error("Can't import colors AND intensity (intensities will be ignored)!");
	...
	if (numberOfColors > 0)
	{
		...
		{
			CVLog::Warning("The number of colors doesn't match the number of points!");
			delete cloud;
			ply_close(ply);
			return CC_FERR_BAD_ENTITY_TYPE;
		}
	...
	/* SCALAR FIELDS (SF) */
	{
		for (size_t i = 0; i < sfPropIndexes.size(); ++i)
		{
			...
			{
				CVLog::Error(QString("Scalar field #%1: the number of scalars doesn't match the number of points (they will be ignored)!").arg(i + 1));
				CVLog::Warning(QString("[PLY] Scalar field #%1 ignored!").arg(i + 1));
			}
			else
			{
				...
				if (sfIdx < 0)
				{
					CVLog::Error(QString("Scalar field #%1: not enough memory to load scalar field (they will be ignored)!").arg(i + 1));
					CVLog::Warning(QString("[PLY] Scalar field #%1 ignored!").arg(i + 1));
				}
			}
		}
	...
	if (facesIndex > 0)
	{
		...
		{
			CVLog::Error("Not enough memory to load facets (they will be ignored)!");
	...
	if (texCoordsIndex > 0)
	{
		...
		{
			CVLog::Error("Not enough memory to load texture coordinates (they will be ignored)!");
	...
	if (texNumberIndex > 0)
	{
		...
		{
			CVLog::Error("Not enough memory to load texture indexes (they will be ignored)!");
	...
	if (texIndexes)
	{
		if (!texCoords)
		{
			CVLog::Error("No texture coordinates were loaded (texture indexes will be ignored)");
			texIndexes->release();
			texIndexes = 0;
		}
		else if (texIndexes->currentSize() < mesh->size())
		{
			...
			else
			{
				...
				{
					CVLog::Warning("Not enough memory to store texture indexes");
					texIndexes->release();
					texIndexes = 0;
				}
			}
	...
	if (mesh)
	{
		...
		if (texCoords)
		{
			...
			if (!textureFileNames.isEmpty())
			{
				...
				for (int ti = 0; ti < textureFileNames.size(); ++ti)
				{
					...
					if (material->loadAndSetTexture(textureFilePath))
					{
						...
						CVLog::Print(QString("[PLY][Texture] Successfully loaded texture '%1' (%2x%3 pixels)").arg(textureFileName).arg(texture.width()).arg(texture.height()));
					...
					{
						CVLog::Warning(QString("[PLY][Texture] Failed to load texture '%1'").arg(textureFilePath));
					}
				}
			...
			{
				CVLog::Warning("[PLY][Texture] Texture coordinates loaded without an associated image! (we look for the 'TextureFile' keyword in comments)");
			}
			...
			if (materials)
			{
				...
				{
					CVLog::Warning("[PLY][Texture] Failed to reserve per-triangle texture coordinates! (not enough memory?)");
					materials->release();
					materials = 0;
				}
			}
		...
		else
		{
			...
			//mesh->computeNormals();
			CVLog::Warning("[PLY] Mesh has no normal! You can manually compute them (select it then call \"Tools > Normals > Compute\")");
		}
