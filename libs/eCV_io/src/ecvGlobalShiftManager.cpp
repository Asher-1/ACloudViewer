...
void ecvGlobalShiftManager::StoreShift(const CCVector3d& shift, double scale, bool preserve/*=true*/)
{
									...
									bool* applyAll/*=0*/)
{
	...
	//default scale
	double scale = (coordinatesScale ? std::max(*coordinatesScale, ZERO_TOLERANCE_D) : 1.0);
	...
	if (mode == NO_DIALOG && !useInputCoordinatesShiftIfPossible)
	{
		...
		{
			CVLog::Warning("[ecvGlobalShiftManager] Entity has very big coordinates: original accuracy may be lost! (you should apply a Global Shift or Scale)");
		}
