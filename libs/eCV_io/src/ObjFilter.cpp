...
CC_FILE_ERROR ObjFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	{
		CVLog::Warning("[OBJ] This filter can only save one mesh (optionally with sub-meshes) at a time!");
...
CC_FILE_ERROR ObjFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		while (!currentLine.isNull())
		{
			...
			if (tokens.front() == "v")
			{
				...
				if (pointsRead == 0)
				{
					...
					if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, parameters))
					{
						...
						CVLog::Warning("[OBJ] Cloud has been recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
					}
				}
			...
			else if (tokens.front().startsWith('f'))
			{
				...
				{
					CVLog::Warning("[OBJ] Malformed file: polygon on line %1 has less than 3 vertices!",lineCount);
					error = true;
					break;
				}
			...
			else if (tokens.front() == "mtllib")
			{
				...
				else
				{
					...
					CVLog::Print(QString("[OBJ] Material file: ") + mtlFilename);
					...
					{
						CVLog::Print("[OBJ] %i materials loaded", materials->size() - oldSize);
						materialsLoadFailed = false;
					}
					else
					{
						CVLog::Error(QString("[OBJ] Failed to load material file! (should be in '%1')").arg(mtlPath + '/' + QString(mtlFilename)));
					...
					{
						for (int i = 0; i < errors.size(); ++i)
							CVLog::Warning(QString("[OBJ::Load::MTL parser] ") + errors[i]);
					}
					if (materials->empty())
					{
						materials->release();
	...
	if (!error)
	{
		...
		if (texCoordsRead > 0 || normsRead > 0)
			CVLog::Print("[OBJ] %i tex. coords, %i normals",texCoordsRead,normsRead);
			...
			||	maxTriNormIndex >= normsRead)
		{
			...
			CVLog::Warning("[OBJ] Malformed file: indexes go higher than the number of elements! (v=%i/tc=%i/n=%i)",maxVertexIndex,maxTexCoordIndex,maxTriNormIndex);
			if (maxVertexIndex >= pointsRead)
			{
				error = true;
			}
		...
		if (!error && baseMesh)
		{
			...
			if (texCoords)
			{
				...
				{
					CVLog::Warning("[OBJ] Texture coordinates were defined but no material could be loaded!");
				}
			}
		...
		if (normals && !normalsPerFacet)
		{
			...
			{
				CVLog::Warning("File contains normals which seem to be neither per-vertex nor per-face!!! We had to ignore them...");
			}
		}
	}
	...
	if (objWarnings[INVALID_NORMALS])
		CVLog::Warning("[OBJ] Some normals in file were invalid. You should re-compute them (select entity, then \"Tools > Normals > Compute\")");
