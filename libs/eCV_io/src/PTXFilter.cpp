									...
									LoadParameters& parameters)
{
	...
	{
		normalsProgressDlg.reset(new ecvProgressDialog(true, parameters.parentWidget));
	...
	for (unsigned cloudIndex = 0; result == CC_FERR_NO_ERROR || result == CC_FERR_NO_LOAD; cloudIndex++)
	{
		...
		//read header
		{
			...
			CVLog::Print(QString("[PTX] Scan #%1 - grid size: %2 x %3").arg(cloudIndex + 1).arg(height).arg(width));
			...
			if (cloudIndex == 0)
			{
				...
				{
					CVLog::Warning("[PTXFilter::loadFile] Cloud has be recentered! Translation: (%.2f ; %.2f ; %.2f)", PshiftTrans.x, PshiftTrans.y, PshiftTrans.z);
				}
			}
		...
		{
			CVLog::Warning("[PTX] Not enough memory to load the grid structure");
			hasIndexGrid = false;
		}
				...
				for (unsigned i = 0; i < width; ++i, ++gridIndex)
				{
					...
					if (firstPoint)
					{
						...
						if (hasColors)
						{
							...
							if (!loadColors)
							{
								CVLog::Warning("[PTX] Not enough memory to load RGB colors!");
							}
							else if (hasIndexGrid)
							{
								...
								try
								{
									grid->colors.resize(gridSize, ecvColor::Rgb(0, 0, 0));
									loadGridColors = true;
								}
								catch (const std::bad_alloc&)
								{
									CVLog::Warning("[PTX] Not enough memory to load the grid colors");
								}
							}
						}
					}
						...
						if (firstPoint)
						{
							if (cloudIndex == 0 && !cloud->isShifted()) //in case the trans. matrix was ok!
							{
								...
								if (HandleGlobalShift(P, PshiftCloud, preserveCoordinateShift, parameters))
								{
									...
									CVLog::Warning("[PTXFilter::loadFile] Cloud has been recentered! Translation: (%.2f ; %.2f ; %.2f)", PshiftCloud.x, PshiftCloud.y, PshiftCloud.z);
								}
							}
							firstPoint = false;
						}
		...
		if (cloud->size() == 0)
		{
			...
			CVLog::Warning(QString("[PTX] Scan #%1 is empty?!").arg(cloudIndex+1));
		}
		else
		{
	...
	//update scalar fields saturation (globally!)
	{
		...
		{
			CVLog::Warning("[PTX] Intensity values are invalid (they should all fall in [0 ; 1])");
		...
		for (unsigned i = 0; i < container.getChildrenNumber(); ++i)
		{
			...
			cloudViewer::ScalarField* sf = static_cast<ccPointCloud*>(obj)->getScalarField(0);
			if (sf)
			{
