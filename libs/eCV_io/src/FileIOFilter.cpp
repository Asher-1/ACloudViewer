										...
										CC_FILE_ERROR& result)
{
	if (!filter)
	{
		CVLog::Error(QString("[Load] Internal error (invalid input filter)[%1]").arg(filename));
	...
	{
		CVLog::Warning(QString("[I/O] CC has caught an exception while loading file '%1'").arg(filename));
	...
	{
		CVLog::Warning(QString("[I/O] CC has caught an unhandled exception while loading file '%1'").arg(filename));
		if (container)
		{
			container->removeAllChildren();
		}
	...
	{
		CVLog::Print(QString("[I/O] File '%1' loaded successfully").arg(filename));
	}
	else
	{
										...
										const QString& fileFilter/*=QString()*/)
{
	...
	if (!fileFilter.isEmpty())
	{
		...
		if (!filter)
		{
			CVLog::Error(QString("[Load] Internal error: no I/O filter corresponds to filter '%1'").arg(fileFilter));
			result = CC_FERR_CONSOLE_ERROR;
			return nullptr;
		}
	}
	else //we need to guess the I/O filter based on the file format
	{
		...
		if (!filter)
		{
			CVLog::Error(QString("[Load] Can't guess file format: unhandled file extension '%1'").arg(extension));
			result = CC_FERR_CONSOLE_ERROR;
			return nullptr;
		}
	}
										...
										Shared filter)
{
	...
	{
		CVLog::Warning(QString("[I/O] CC has caught an unhandled exception while saving file '%1'").arg(filename));
		result = CC_FERR_CONSOLE_ERROR;
	}
										...
										bool useInputCoordinatesShiftIfPossible/*=false*/)
{
	...
	if (	sizeof(PointCoordinateType) < 8
		&&	ecvGlobalShiftManager::Handle(	P,
											0,
											...
											&applyAll) )
	{
		//we save coordinates shift information
                if ((applyAll || loadParameters.shiftHandlingMode == ecvGlobalShiftManager::NO_DIALOG_AUTO_SHIFT) //in command line mode, with the 'AUTO GLOBAL SHIFT' option, we want to retrieve the applied coordinate shift
