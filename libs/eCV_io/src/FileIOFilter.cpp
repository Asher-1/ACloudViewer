...
void FileIOFilter::setImportFileFilterStrings(
        const QStringList& filterStrings) {
    m_filterInfo.importFileFilterStrings = filterStrings;
}

void FileIOFilter::setExportFileFilterStrings(
        const QStringList& filterStrings) {
...
void FileIOFilter::checkFilterInfo() const {
    ...
    if (m_filterInfo.features & Import) {
        if (m_filterInfo.importFileFilterStrings.isEmpty()) {
            CVLog::Warning(QStringLiteral("I/O filter marked as import, but no "
                                          "filter strings set: %1")
                                   .arg(m_filterInfo.id));
        }
        ...
        if (!m_filterInfo.importFileFilterStrings.isEmpty()) {
            CVLog::Warning(QStringLiteral("I/O filter not marked as import, "
                                          "but filter strings are set: %1")
                                   .arg(m_filterInfo.id));
        }

        if (!m_filterInfo.importExtensions.isEmpty()) {
            CVLog::Warning(QStringLiteral("I/O filter not marked as import, "
                                          "but extensions are set: %1")
                                   .arg(m_filterInfo.id));
        }
    }
    ...
    if (m_filterInfo.features & Export) {
        if (m_filterInfo.exportFileFilterStrings.isEmpty()) {
            CVLog::Warning(QStringLiteral("I/O filter marked as export, but no "
                                          "filter strings set: %1")
                                   .arg(m_filterInfo.id));
        }
    } else {
        if (!m_filterInfo.exportFileFilterStrings.isEmpty()) {
            CVLog::Warning(QStringLiteral("I/O filter not marked as export, "
                                          "but filter strings are set: %1")
                                   .arg(m_filterInfo.id));
        }
    }
...
void FileIOFilter::ResetSesionCounter() { s_sessionCounter = 0; }

unsigned FileIOFilter::IncreaseSesionCounter() { return ++s_sessionCounter; }
...
void FileIOFilter::Register(Shared filter) {
    ...
    if (std::any_of(s_ioFilters.cbegin(), s_ioFilters.cend(), compareFilters)) {
        CVLog::Warning(QStringLiteral("[FileIOFilter] I/O filter already "
    ...
    auto comparePriorities = [](const Shared& filter1,
                                const Shared& filter2) -> bool {
    ...
    auto pos = std::upper_bound(s_ioFilters.begin(), s_ioFilters.end(), filter,
                                comparePriorities);
...
FileIOFilter::Shared FileIOFilter::GetFilter(const QString& fileFilter,
    ...
    if (!fileFilter.isEmpty()) {
        for (FilterContainer::const_iterator it = s_ioFilters.begin();
             it != s_ioFilters.end(); ++it) {
            QStringList otherFilters = (*it)->getFileFilters(onImport);
            if (otherFilters.contains(fileFilter)) return *it;
        }
    }
...
FileIOFilter::Shared FileIOFilter::FindBestFilterForExtension(
        const QString& ext) {
...
ccHObject* FileIOFilter::LoadFromFile(const QString& filename,
    ...
    if (!filter) {
        CVLog::Error(QString("[Load] Internal error (invalid input filter)[%1]")
                             .arg(filename));
    ...
    if (!fi.exists()) {
        CVLog::Error(QString("[Load] File '%1' doesn't exist!").arg(filename));
        result = CC_FERR_CONSOLE_ERROR;
        return nullptr;
    }
    ...
    try {
        ...
        CVLog::Warning(QString("[I/O] CC has caught an exception while loading "
                               "file '%1'")
                               .arg(filename));
        CVLog::Warning(QString("[I/O] Exception: %1").arg(e.what()));
        if (container) {
            container->removeAllChildren();
        }
        result = CC_FERR_CONSOLE_ERROR;
    } catch (...) {
        CVLog::Warning(QString("[I/O] CC has caught an unhandled exception "
                               "while loading file '%1'")
                               .arg(filename));
        if (container) {
            container->removeAllChildren();
        }
    ...
    if (result == CC_FERR_NO_ERROR) {
        CVLog::Print(
                QString("[I/O] File '%1' loaded successfully").arg(filename));
    } else {
    ...
    if (childCount != 0) {
        ...
        container->setName(
                QString("%1 (%2)").arg(fi.fileName(), fi.absolutePath()));
        for (unsigned i = 0; i < childCount; ++i) {
            ccHObject* child = container->getChild(i);
            child->setBaseName(fi.baseName());
            child->setFullPath(filename);
            QString newName = child->getName();
            if (newName.startsWith("unnamed")) {
                // we automatically replace occurrences of 'unnamed' in entities
                // names by the base filename (no path, no extension)
...
ccHObject* FileIOFilter::LoadFromFile(
    ...
    if (!fileFilter.isEmpty()) {
        ...
        if (!filter) {
            CVLog::Error(QString("[Load] Internal error: no I/O filter "
                                 "corresponds to filter '%1'")
                                 .arg(fileFilter));
            result = CC_FERR_CONSOLE_ERROR;
            return nullptr;
        }
    } else  // we need to guess the I/O filter based on the file format
    {
        ...
        if (extension.isEmpty()) {
            CVLog::Error("[Load] Can't guess file format: no file extension");
            result = CC_FERR_CONSOLE_ERROR;
            return nullptr;
        }
        ...
        // unknown extension?
        if (!filter) {
            CVLog::Error(QString("[Load] Can't guess file format: unhandled "
                                 "file extension '%1'")
                                 .arg(extension));
            result = CC_FERR_CONSOLE_ERROR;
            return nullptr;
...
CC_FILE_ERROR FileIOFilter::SaveToFile(ccHObject* entities,
                                       ...
                                       Shared filter) {
    if (!entities || filename.isEmpty() || !filter) return CC_FERR_BAD_ARGUMENT;
...
CC_FILE_ERROR FileIOFilter::SaveToFile(ccHObject* entities,
    ...
    if (fileFilter.isEmpty()) return CC_FERR_BAD_ARGUMENT;

    Shared filter = GetFilter(fileFilter, false);
    if (!filter) {
        CVLog::Error(QString("[Load] Internal error: no filter corresponds to "
                             "filter '%1'")
                             .arg(fileFilter));
...
void FileIOFilter::DisplayErrorMessage(CC_FILE_ERROR err,
    ...
    switch (err) {
        ...
        case CC_FERR_FILE_WAS_WRITTEN_BY_UNKNOWN_PLUGIN:
            errorStr =
                    "the file was written by a plugin but none of the loaded "
                    "plugins can deserialize it";
            break;
        case CC_FERR_THIRD_PARTY_LIB_FAILURE:
            errorStr =
                    "the third-party library in charge of saving/loading the "
                    "file has failed to perform the operation";
            break;
    ...
    QString outputString =
            QString("An error occurred while %1 '%2': ").arg(action, filename) +
            errorStr;
    if (warning)
        CVLog::Warning(outputString);
    else
        CVLog::Error(outputString);
}
...
bool FileIOFilter::HandleGlobalShift(
        const CCVector3d& P,
        CCVector3d& Pshift,
        bool& preserveCoordinateShift,
        LoadParameters& loadParameters,
        bool useInputCoordinatesShiftIfPossible /*=false*/) {
    bool shiftAlreadyEnabled = (loadParameters.coordinatesShiftEnabled &&
                                *loadParameters.coordinatesShiftEnabled &&
                                loadParameters.coordinatesShift);
    ...
    if (sizeof(PointCoordinateType) < 8 &&
        ecvGlobalShiftManager::Handle(
                P, 0, loadParameters.shiftHandlingMode,
                shiftAlreadyEnabled || useInputCoordinatesShiftIfPossible,
                Pshift, &preserveCoordinateShift, nullptr, &applyAll)) {
        // we save coordinates shift information
        if ((applyAll ||
             loadParameters.shiftHandlingMode ==
                     ecvGlobalShiftManager::
                             NO_DIALOG_AUTO_SHIFT)  // in command line mode,
                                                    // with the 'AUTO GLOBAL
                                                    // SHIFT' option, we want to
