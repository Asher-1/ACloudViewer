...
OFFFilter::OFFFilter()
    : FileIOFilter({"_OFF Filter",
                    11.0f,  // priority
                    QStringList{"off"}, "off", QStringList{"OFF mesh (*.off)"},
                    QStringList{"OFF mesh (*.off)"}, Import | Export}) {}

bool OFFFilter::canSave(CV_CLASS_ENUM type,
                        bool& multiple,
...
CC_FILE_ERROR OFFFilter::saveToFile(ccHObject* entity,
                                    const QString& filename,
                                    const SaveParameters& parameters) {
    if (!entity) return CC_FERR_BAD_ARGUMENT;

    if (!entity->isKindOf(CV_TYPES::MESH)) {
        CVLog::Warning("[OFF] This filter can only save one mesh at a time!");
        return CC_FERR_BAD_ENTITY_TYPE;
    }

    ccGenericMesh* mesh = ccHObjectCaster::ToGenericMesh(entity);
    if (!mesh || mesh->size() == 0) {
        CVLog::Warning("[OFF] Input mesh is empty!");
        return CC_FERR_NO_SAVE;
    }
    ccGenericPointCloud* vertices = mesh->getAssociatedCloud();
    if (!vertices || vertices->size() == 0) {
        CVLog::Warning("[OFF] Input mesh has no vertices?!");
        return CC_FERR_NO_SAVE;
    }
...
static QString GetNextLine(QTextStream& stream) {
    ...
    do {
        ...
        // end of file?
        if (currentLine.isNull()) return QString();
    } while (currentLine.startsWith("#") || currentLine.isEmpty());

    return currentLine;
}

CC_FILE_ERROR OFFFilter::loadFile(const QString& filename,
                                  ccHObject& container,
                                  LoadParameters& parameters) {
    // try to open file
    QFile fp(filename);
    if (!fp.open(QIODevice::ReadOnly | QIODevice::Text)) return CC_FERR_READING;

    QTextStream stream(&fp);

    QString currentLine = stream.readLine();
    if (!currentLine.toUpper().startsWith("OFF")) return CC_FERR_MALFORMED_FILE;
        ...
        for (unsigned i = 0; i < vertCount; ++i) {
            ...
            CCVector3d Pd(0, 0, 0);
            {
                ...
                if (vertexIsOk) {
                    ...
                    if (vertexIsOk) Pd.z = tokens[2].toDouble(&vertexIsOk);
                }
                if (!vertexIsOk) {
                    delete vertices;
                    return CC_FERR_MALFORMED_FILE;
            ...
            if (i == 0) {
                ...
                if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift,
                    ...
                    CVLog::Warning(
                            "[OFF] Cloud has been recentered! Translation: "
                            "(%.2f ; %.2f ; %.2f)",
    ...
    // load triangles
    {
        ...
        for (unsigned i = 0; i < triCount; ++i) {
            ...
            if (polyVertCount == 3 || polyVertCount == 4) {
                ...
                if (mesh->size() + polyTriCount > mesh->capacity()) {
                    if (!mesh->reserve(mesh->size() + polyTriCount +
                                       256))  // use some margin to avoid too
                                              // many allocations
                    {
                        delete mesh;
                        return CC_FERR_NOT_ENOUGH_MEMORY;
        ...
        if (ignoredPolygons) {
            CVLog::Warning(
                    "[OFF] Some polygons with an unhandled size (i.e. > 4) "
                    "were ignored!");
        }
    }

    if (mesh->size() == 0) {
        CVLog::Warning("[OFF] Failed to load any polygon!");
        ...
        // the user doesn't want normals (especially in ccViewer!) if
        // (mesh->computeNormals()) 	mesh->showNormals(true); else
        //	CVLog::Warning("[OFF] Failed to compute per-vertex normals...");
        CVLog::Warning(
                "[OFF] Mesh has no normal! You can manually compute them "
