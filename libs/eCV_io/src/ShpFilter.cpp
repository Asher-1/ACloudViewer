...
CC_FILE_ERROR ShapeFileHeader::readFrom(QDataStream &sin)
{
	...
	{
		CVLog::Warning("[SHP] wrong file code (%d), is this a shape file?", fileCode);
	...
	{
		CVLog::Warning("[SHP] invalid shape type code in header (%d)", shapeTypeInt);
		return CC_FERR_MALFORMED_FILE;
	}
...
bool ShpFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
	if (type == CV_TYPES::POLY_LINE ||
		type == CV_TYPES::POINT_CLOUD ||
		type == CV_TYPES::HIERARCHY_OBJECT ||
		type == CV_TYPES::MESH)
	{
		multiple = true;
		exclusive = true;
		...
		case CV_TYPES::HIERARCHY_OBJECT:
			...
			if (baseEntity->getChildrenNumber())
			{
				...
				for (unsigned i = 1; i < baseEntity->getChildrenNumber(); ++i)
				{
					...
					{
						if (child)
							CVLog::Warning(QString("[SHP] Entity %1 has not the same type (%2) as the others in the selection (%3)! Can't mix types...")
							.arg(child->getName())
		...
		int32_t lastIndex)
{
	...
	if (!scalarValues.empty())
	{
		...
		if (!areAllValuesOfPartsNans)
		{
			...
			{
				CVLog::Warning(QString("[SHP] Mesh: not enough memory to load scalar values!"));
				sf->release();
			} else
			{
				for (int32_t j = 0; j < vertCount; ++j)
		...
		const std::vector<ScalarType> &scalarValues)
{
	...
	for (int32_t i = 0; i < numParts; ++i)
	{
		...
		{
			CVLog::Warning("[SHP] Multipatch part %d has an invalid part type (%d)", i, partTypes[i]);
			continue;
		}
		...
		switch (type)
		{
			...
			default:
				CVLog::Print(QString("[SHP] Cannot handle Patch of type: %1").arg(ToString(type)));
								  ...
								  bool load2DPolyAs3DPoly = true)
{
	...
	if (hasMeasurements(shapeType))
	{
		...
		if (scalarValues.empty())
		{
			...
			CVLog::Warning(QString("[SHP] Polyline #%1: not enough memory to load scalar values!").arg(index));
		}
	}
	...
	for (int32_t i = 0; i < numParts; ++i)
	{
		...
		if (!scalarValues.empty())
		{
			...
			if (!allNans)
			{
				...
				if (!sf->reserveSafe(vertCount))
				{
					CVLog::Warning(QString("[SHP] Polyline #%1.%2: not enough memory to load scalar values!").arg(index).arg(i + 1));
					sf->release();
					sf = nullptr;
				}
				for (int32_t j = 0; j < vertCount; ++j)
							   ...
							   bool preserveCoordinateShift)
{
	...
	if (hasMeasurements(shapeType))
	{
		...
		if (mMin != ESRI_NO_DATA && mMax != ESRI_NO_DATA)
		{
			...
			{
				CVLog::Warning("[SHP] Not enough memory to load scalar values!");
				sf->release();
				sf = nullptr;
			}
		}
...
CC_FILE_ERROR ShpFilter::saveToFile(ccHObject* entity, const std::vector<GenericDBFField*>& fields, const QString& filename, const SaveParameters& parameters)
{
	...
	for (ccHObject *child : toSave)
	{
		...
		if (child->isA(CV_TYPES::POLY_LINE))
		{
			...
			{
				CVLog::Warning(QString("Polyline '%1' is too small! It won't be saved...").arg(child->getName()));
				continue;
			}
		}
		...
		CVLog::PrintDebug("[SHP] Saved shape #%d (%d bytes)", shapeIndex, recordSize * 2);
		shapeIndex++;
	}
	...
	if (dbfHandle)
	{
		while (true) //trick: we use 'while' to be able to break anytime
		{
			//always write an 'index' table
			{
				...
				{
					CVLog::Warning(QString("[SHP] Failed to save field 'index' (default)"));
			...
			if (save3DPolyHeightInDBF)
			{
				...
				{
					CVLog::Warning(QString("[SHP] Failed to save field 'height' (3D polylines height)"));
					result = CC_FERR_WRITING;
					break;
				}
			}
			...
			for (GenericDBFField* field: fields)
			{
				if (field->is3D()) //3D case
				{
					...
					if (xFieldIdx < 0)
					{
						CVLog::Warning(QString("[SHP] Failed to save field '%1'").arg(field->name()));
						result = CC_FERR_WRITING;
						break;
					}
				}
...
CC_FILE_ERROR ShpFilter::loadFile(const QString &filename, ccHObject &container, LoadParameters &parameters)
{
	...
	{
		CVLog::Warning("[SHP] Entities will be recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
	}
	...
	while (hdr.fileLength - shpStream.device()->pos() > 0)
	{
		...
		if (!isValidESRIShapeCode(shapeTypeInt))
		{
			CVLog::Warning("[SHP] Shape %d has an invalid shape code (%d)", recordNumber, shapeTypeInt);
			return CC_FERR_READING;
		}
		auto shapeType = static_cast<ESRI_SHAPE_TYPE >(shapeTypeInt);

		if (recordNumber < 64)
			CVLog::Print(QString("[SHP] Record #%1 - type: %2 (%3 bytes)").arg(recordNumber).arg(ToString(shapeType)).arg(recordSize));
		else if (recordNumber == 64)
			CVLog::Print("[SHP] Records won't be displayed in the Console anymore to avoid flooding it...");
	...
	if (!is3DShape && error == CC_FERR_NO_ERROR && (hasPolylines || hasPoints))
	{
		...
		if (dbfHandle)
		{
			...
			{
				CVLog::Warning("[SHP] No enough records in the associated DBF file!");
			}
			...
			else
			{
				...
				{
					CVLog::Warning("[SHP] No numerical field in the associated DBF file!");
				}
			}
		}
		else
		{
			CVLog::Warning(QString("[SHP] Failed to load associated DBF file ('%1')").arg(dbfFilename));
		}
	}
