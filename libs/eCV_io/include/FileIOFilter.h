...
public:  // static methods
    ...
    **/
    ECV_IO_LIB_API static ccHObject* LoadFromFile(
            const QString& filename,
            LoadParameters& parameters,
            CC_FILE_ERROR& result,
    ...
    **/
    ECV_IO_LIB_API static CC_FILE_ERROR SaveToFile(
            ccHObject* entities,
            const QString& filename,
            const SaveParameters& parameters,
            Shared filter);
    ...
    **/
    ECV_IO_LIB_API static bool HandleGlobalShift(
            const CCVector3d& P,
            CCVector3d& Pshift,
...
public:  // loading "sessions" management
    ...
    ECV_IO_LIB_API static void ResetSesionCounter();

    //! Indicates to the I/O filters that a new loading/saving action has
    //! started
    /** \return the updated session counter
     **/
    ECV_IO_LIB_API static unsigned IncreaseSesionCounter();

public:  // global filters registration mechanism
    //! Init internal filters (should be called once)
    ECV_IO_LIB_API static void InitInternalFilters();

    //! Registers a new filter
    ECV_IO_LIB_API static void Register(Shared filter);

    //! Returns the filter corresponding to the given 'file filter'
    ECV_IO_LIB_API static Shared GetFilter(const QString& fileFilter,
                                           bool onImport);
     ...
     **/
    ECV_IO_LIB_API static void UnregisterAll();
