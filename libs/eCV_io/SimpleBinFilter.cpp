...
CC_FILE_ERROR SimpleBinFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	//header
	{
		...
		//8 bytes = point count
		{
			...
			{
				CVLog::Warning("[SBF] Inconsistent number of points between the header and the data file!");
			}
		...
		//2 bytes = sf count
		{
			...
			{
				CVLog::Warning("[SBF] Inconsistent number of scalar fields between the header and the data file!");
			}
	...
	for (size_t i = 0; i < descriptor.pointCount; ++i)
	{
		...
		if (i == 0)
		{
			...
			if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, parameters, true))
			{
				...
				CVLog::Warning("[SBF] Cloud has been recentered! Translation: (%.2f ; %.2f ; %.2f)", descriptor.globalShift.x, descriptor.globalShift.y, descriptor.globalShift.z);
			}
