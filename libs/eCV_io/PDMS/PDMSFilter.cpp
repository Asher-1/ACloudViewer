...
bool PDMSFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
	//not supported yet
	return false;
}
	...
	if (parser.parseSessionContent())
	{
		...
		while (!treeSync.empty())
		{
			...
			else
			{
				...
				{
					CVLog::Warning(QString("[PDMSFilter] Primitive '%1' not supported yet!").arg(unsupportedPrimitiveStr));
				}
			}
