...
bool ecvShiftAndScaleCloudDlg::loadInfoFromFile(QString filename)
{
	...
	while (true)
	{
		...
		{
			//invalid file
			CVLog::Warning(QString("[ecvShiftAndScaleCloudDlg::loadInfoFromFile] File '%1' is malformed (5 items expected per line)").arg(filename));
		...
		{
			//invalid file
			CVLog::Warning(QString("[ecvShiftAndScaleCloudDlg::loadInfoFromFile] File '%1' is malformed (wrong item type encountered on line %2)").arg(filename).arg(lineNumber));
			m_defaultInfos.resize(originalSize);
			return false;
		}
		...
		{
			//not enough memory
			CVLog::Warning(QString("[ecvShiftAndScaleCloudDlg::loadInfoFromFile] Not enough memory to read file '%1'").arg(filename));
			m_defaultInfos.resize(originalSize);
			return false;
		}
	}
...
void ecvShiftAndScaleCloudDlg::updateLocalSystem()
{
	...
	m_ui->xDestLabel->setStyleSheet(ecvGlobalShiftManager::NeedShift(localPoint.x) ? QString("color: red;") : QString());
	m_ui->yDestLabel->setText(QString("y = %1").arg(localPoint.y, 0, 'f', prec));
	m_ui->yDestLabel->setStyleSheet(ecvGlobalShiftManager::NeedShift(localPoint.y) ? QString("color: red;") : QString());
	m_ui->zDestLabel->setText(QString("z = %1").arg(localPoint.z, 0, 'f', prec));
	m_ui->zDestLabel->setStyleSheet(ecvGlobalShiftManager::NeedShift(localPoint.z) ? QString("color: red;") : QString());

	m_ui->diagDestLabel->setText(QString("diagonal = %1").arg(localDiagonal, 0, 'f', prec));
	m_ui->diagDestLabel->setStyleSheet(ecvGlobalShiftManager::NeedRescale(localDiagonal) ? QString("color: red;") : QString());
}
...
bool ecvShiftAndScaleCloudDlg::getInfo(size_t index, ecvGlobalShiftManager::ShiftInfo& info) const
{
...
int ecvShiftAndScaleCloudDlg::addShiftInfo(const std::vector<ecvGlobalShiftManager::ShiftInfo>& infos)
{
