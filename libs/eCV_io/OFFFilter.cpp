...
bool OFFFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
CC_FILE_ERROR OFFFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
		...
		for (unsigned i = 0; i < vertCount; ++i)
		{
			...
			if (i == 0)
			{
				...
				if (HandleGlobalShift(Pd, Pshift, preserveCoordinateShift, parameters))
				{
					...
					CVLog::Warning("[OFF] Cloud has been recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
				}
			}
	...
	//load triangles
	{
		...
		if (ignoredPolygons)
		{
			CVLog::Warning("[OFF] Some polygons with an unhandled size (i.e. > 4) were ignored!");
		}
	}
	...
	else
	{
		...
		CVLog::Warning("[OFF] Mesh has no normal! You can manually compute them (select it then call \"Tools > Normals > Compute\")");
