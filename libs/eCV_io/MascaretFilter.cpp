...
CC_FILE_ERROR MascaretFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	try
	{
		...
		for (size_t i=0; i<candidates.size(); ++i)
		{
			...
			{
				CVLog::Warning(QString("[Mascaret] Polyline '%1' is not a valid profile (missing meta-data)").arg(poly->getName()));
				break;
			}
			else
			{
	...
	for (size_t i=0; i<profiles.size(); ++i)
	{
		...
		{
			//invalid size
			CVLog::Warning(QString("[Mascaret] Polyline '%1' does not have enough vertices")
		...
		if (!ok)
		{
			CVLog::Warning(QString("[Mascaret] At least one of the meta-data entry of polyline '%1' is invalid?!").arg(poly->getName()));
			continue;
		}
