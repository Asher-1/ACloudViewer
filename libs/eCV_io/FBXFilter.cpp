...
bool FBXFilter::canSave(CV_CLASS_ENUM type, bool& multiple, bool& exclusive) const
{
...
static FbxNode* ToFbxMesh(ccGenericMesh* mesh, FbxScene* pScene, QString filename, size_t meshIndex)
{
	...
	{
		CVLog::Warning("[FBX] Mesh has no normal! You can manually compute them (select it then call \"Edit > Normals > Compute\")");
	}
			...
			if (hasTextures && mat->hasTexture())
			{
				...
				if (!texFilenames.contains(texFilename))
				{
					...
					if (!texDir.exists())
					{
						...
						{
							textDirName = QString();
							CVLog::Warning("[FBX] Failed to create subfolder '%1' to store texture files (files will be stored next to the .fbx file)");
						}
					}
					...
					CVLog::PrintDebug(QString("[FBX] Material '%1' texture: %2").arg(mat->getName()).arg(absoluteFilename));
	...
	// Create polygons
	{
		for (unsigned j = 0; j < faceCount; ++j)
		{
			const CVLib::VerticesIndexes* tsi = mesh->getTriangleVertIndexes(j);
...
static bool SaveScene(FbxManager* pManager, FbxDocument* pScene, const char* pFilename, int pFileFormat = -1, bool pEmbedMedia = false)
{
	...
	if (lExporter->Initialize(pFilename, pFileFormat, pManager->GetIOSettings()) == false)
	{
		...
		CVLog::Warning("[FBX] Error returned: %s", lExporter->GetStatus().GetErrorString());
		return false;
	}
...
CC_FILE_ERROR FBXFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
			...
			if (child->isKindOf(CV_TYPES::MESH))
			{
				...
				if (child->hasMetaData(FBX_SCALE_METADATA_KEY))
				{
					...
					if (ok)
					{
						...
						{
							CVLog::Warning("[FBX] Attempt to save mutliple meshes with different units!");
						}
					}
					else
					{
						CVLog::Warning("[FBX] Internal error: invalid FBX scale meta-data?!");
						scaleFactor = 1.0;
					}
				}
				else
				{
					...
					CVLog::Warning("[FBX] Reminder: default FBX units are 'cm'");
					scaleFactor = 1.0;
				}
			}
		}
	...
	{
		CVLog::Print("[FBX] Autodesk FBX SDK version %s", lSdkManager->GetVersion());
	}
	...
	try
	{
		...
		FbxNode* lRootNode = lScene->GetRootNode();
		{
			for (size_t i = 0; i < meshes.size(); ++i)
			{
				...
				if (meshNode)
					...
					CVLog::Warning(QString("[FBX] Failed to convert mesh '%1' to FBX mesh/node!").arg(meshes[i]->getName()));
			}
		}
				...
				else
				{
					...
					for (int lFormatIndex = 0; lFormatIndex < lFormatCount; lFormatIndex++)
					{
						if (pSdkManager->GetIOPluginRegistry()->WriterIsFBX(lFormatIndex))
						{
							...
							{
								CVLog::Print(QString("[FBX] Default output file format: %1").arg(sanitizedDesc));
					...
					if (fileFormat < 0)
					{
						CVLog::Warning(QString("[FBX] File format '%1' not supported").arg(s_defaultOutputFormat));
						...
						for (int lFormatIndex = 0; lFormatIndex < lFormatCount; lFormatIndex++)
						{
							...
							{
								FbxString lDesc = pSdkManager->GetIOPluginRegistry()->GetWriterFormatDescription(lFormatIndex);
								CVLog::Print(QString("\t- %1").arg(SanitizeFBXFormatString(lDesc.Buffer())));
							}
						}
					}
		...
		{
			CVLog::Warning(QString("[FBX] Output filename contains special characters. It might be rejected by the third party library..."));
		}
...
static ccMesh* FromFbxMesh(FbxMesh* fbxMesh, FileIOFilter::LoadParameters& parameters)
{
	...
	//as we can't load all polygons (yet ;) we already look if we can load any!
	{
		...
		if (triCount == 0)
		{
			CVLog::Warning(QString("[FBX] No triangle or quad found in mesh '%1'! (polygons with more than 4 vertices are not supported for the moment)").arg(fbxMesh->GetName()));
			return 0;
		}
		else if (skipped != 0)
		{
			CVLog::Warning(QString("[FBX] Some polygons in mesh '%1' were ignored (%2): polygons with more than 4 vertices are not supported for the moment)").arg(fbxMesh->GetName()).arg(skipped));
			return 0;
		}
	}
	...
	if (vertCount <= 0)
	{
		CVLog::Warning(QString("[FBX] Mesh '%1' has no vetex or no polygon?!").arg(fbxMesh->GetName()));
		return 0;
	}
	...
	{
		CVLog::Warning(QString("[FBX] Not enough memory to load mesh '%1'!").arg(fbxMesh->GetName()));
		delete mesh;
		return 0;
	}
		...
		for (int l = 0; l < fbxMesh->GetElementVertexColorCount(); l++)
		{
			...
			if (vertColor->GetMappingMode() == FbxGeometryElement::eByControlPoint)
			{
					...
					|| vertColor->GetReferenceMode() == FbxGeometryElement::eIndexToDirect)
				{
					if (vertices->reserveTheRGBTable())
					{
						...
						case FbxGeometryElement::eDirect:
						{
							for (int i = 0; i < vertCount; ++i)
							{
								...
								vertices->addRGBColor(static_cast<ColorCompType>(c.mRed	* ecvColor::MAX),
									static_cast<ColorCompType>(c.mGreen	* ecvColor::MAX),
									static_cast<ColorCompType>(c.mBlue	* ecvColor::MAX));
							}
						}
						break;
					...
					{
						CVLog::Warning(QString("[FBX] Not enough memory to load mesh '%1' colors!").arg(fbxMesh->GetName()));
					}
				}
				else
				{
					CVLog::Warning(QString("[FBX] Color field #%i of mesh '%1' will be ignored (unhandled type)").arg(l).arg(fbxMesh->GetName()));
				}
			}
			else
			{
	...
	if (perPointNormals >= 0)
	{
		...
		{
			CVLog::Warning(QString("[FBX] Wrong number of normals on mesh '%1'!").arg(fbxMesh->GetName()));
			perPointNormals = -1;
		}
		else if (!vertices->reserveTheNormsTable())
		{
			CVLog::Warning(QString("[FBX] Not enough memory to load mesh '%1' normals!").arg(fbxMesh->GetName()));
			perPointNormals = -1;
		}
		else
		{
	...
	ccMaterialSet* materials = 0;
	{
		...
		for (int i = 0; i < lMaterialCount; i++)
		{
						...
						if (lProperty.IsValid())
						{
							...
							if (texture)
							{
								...
								if (lFileTexture)
								{
									...
									CVLog::PrintDebug(QString("[FBX] Texture absolue filename: %1").arg(texAbsoluteFilename));
									if (texAbsoluteFilename != 0 && texAbsoluteFilename[0] != 0)
									{
										if (!mat->loadAndSetTexture(texAbsoluteFilename))
										{
											CVLog::Warning(QString("[FBX] Failed to load texture file: %1").arg(texAbsoluteFilename));
										}
									}
								}
							}
			...
			{
				CVLog::Warning(QString("[FBX] Material '%1' has an unhandled type").arg(lBaseMaterial->GetName()));
			}
		}
	}
		...
		for (int l = 0; l < fbxMesh->GetElementUVCount(); ++l)
		{
			...
			if (leUV->GetMappingMode() == FbxGeometryElement::eByPolygonVertex)
			{
				...
				if (!vertTexUVTable->reserveSafe(uvCount) || !mesh->reservePerTriangleTexCoordIndexes())
				{
					...
					vertTexUVTable = 0;
					CVLog::Warning(QString("[FBX] Not enough memory to load mesh '%1' UV coordinates!").arg(fbxMesh->GetName()));
				}
				else
				{
					...
					{
						CVLog::Warning(QString("[FBX] UV coordinates for mesh '%1' are encoded in an unhandled mode!").arg(fbxMesh->GetName()));
						vertTexUVTable->release();
						vertTexUVTable = 0;
					}
				}
	...
	//import polygons
	{
		...
		for (int i = 0; i < polyCount; ++i)
		{
			...
			if (vertTexUVTable)
			{
				...
				{
					CVLog::Warning(QString("[FBX] Mesh '%1': UV coordinates indexes mismatch!").arg(fbxMesh->GetName()));
					vertTexUVTable->release();
					vertTexUVTable = 0;
				}
			}
		...
		if (mesh->size() == 0)
		{
			CVLog::Warning(QString("[FBX] No triangle found in mesh '%1'! (only triangles are supported for the moment)").arg(fbxMesh->GetName()));
			delete mesh;
			return 0;
		}
	}
		...
		for (int i = 0; i < vertCount; ++i, ++fbxVertices)
		{
			...
			if (i == 0)
			{
				...
				if (FileIOFilter::HandleGlobalShift(P, Pshift, preserveCoordinateShift, parameters))
				{
					...
					CVLog::Warning("[FBX] Mesh has been recentered! Translation: (%.2f ; %.2f ; %.2f)", Pshift.x, Pshift.y, Pshift.z);
				}
			}
	...
	if (materials)
	{
		...
		for (int i = 0; i < fbxMatCount; ++i)
		{
				...
				&&	lMaterialElement->GetIndexArray().GetCount() == fbxMesh->GetPolygonCount())
			{
				...
				{
					CVLog::Warning("[FBX] Not enough memory to load materials!");
				}
				break;
			}
...
CC_FILE_ERROR FBXFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	try
	{
		...
		if (CheckForSpecialChars(filename))
		{
			CVLog::Warning(QString("[FBX] Input filename contains special characters. It might be rejected by the third party library..."));
		}

		// Use the first argument as the filename for the importer.
		if (!lImporter->Initialize(qPrintable(filename), -1, lSdkManager->GetIOSettings()))
		{
			CVLog::Warning(QString("[FBX] Error: %1").arg(lImporter->GetStatus().GetErrorString()));
...
#ifdef QT_DEBUG
					CVLog::Print(QString("Node: %1 - %2 properties").arg(nodeName).arg(lNode->GetNodeAttributeCount()));
#endif
...
#ifdef QT_DEBUG
						CVLog::Print(QString("\tProp. #%1").arg(GetAttributeTypeName(type)));
#endif
