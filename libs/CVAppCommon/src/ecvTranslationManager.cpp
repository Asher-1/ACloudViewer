...
void ccTranslationManager::loadTranslations() {
    ...
    for (const auto &fileInfo : info) {
        ...
        bool loaded = translator->load(locale, fileInfo.prefix,
                                       QStringLiteral("_"), fileInfo.path);
...
void ccTranslationManager::populateMenu(QMenu *menu,
    ...
    connect(action, &QAction::triggered, this,
            [this]() { setLanguagePref(QStringLiteral("en")); });
...
ccTranslationManager::LanguageList ccTranslationManager::availableLanguages(
        const QString &appName, const QString &pathToTranslationFiles) {
    ...
    QRegularExpression regExp(
            QStringLiteral("%1_(?<langCode>.{2}).*.qm").arg(appName));
    ...
    for (const auto &cFileName : cFileNames) {
        ...
        if (language.isEmpty()) {
            qWarning() << "Language not found for translation file"
                       << cFileName;
            continue;
        }
