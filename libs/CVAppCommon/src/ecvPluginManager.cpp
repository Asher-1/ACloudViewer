    ...
    if (metaObject.isEmpty()) {
        ...
        CVLog::Warning(
                QStringLiteral(
                        ...
                        "Q_PLUGIN_METADATA (see one of the example plugins).")
                        .arg(fileName));
...
static bool sMetaDataValid(QPluginLoader *loader) {
    ...
    if (metaObject.isEmpty() || metaObject["MetaData"].toObject().isEmpty()) {
        CVLog::Error(
                QStringLiteral(
                        "%1 does not supply meta data in the Q_PLUGIN_METADATA "
                        "(see one of the example plugins).")
                        .arg(fileName));
        ...
        if (!validTypes.contains(pluginType)) {
            CVLog::Error(
                    QStringLiteral(
                            "%1 does not supply a valid plugin type in its "
                            "info.json.\n\nFound: %2\n\nIt must be one of: %3")
...
ccPluginManager &ccPluginManager::get() { return *sPluginManager; }

void ccPluginManager::setPaths(const QStringList &paths) {
    m_pluginPaths = paths;
}

QStringList ccPluginManager::pluginPaths() { return m_pluginPaths; }

void ccPluginManager::loadPlugins() {
    m_pluginList.clear();

    if (m_pluginPaths.empty()) {
        qWarning() << "There are no plugin paths set. Maybe missing a call to "
                      "ccPluginManager::setPaths()?";
    }
    ...
    for (QObject *plugin : pluginInstances) {
        ...
        CVLog::Print(
                tr("[Plugin] Found: %1 (STATIC)").arg(ccPlugin->getName()));
    ...
    for (ccPluginInterface *plugin : m_pluginList) {
        ...
        // Disable if we are not running on the command line and it's in the
        // disabled list
        ...
        switch (plugin->getType()) {
            case ECV_STD_PLUGIN: {
                ...
                ccExternalFactory *factory =
                        stdPlugin->getCustomObjectsFactory();
                if (factory) {
            ...
            case ECV_IO_FILTER_PLUGIN:  // I/O filter
            {
                ...
                if (!ioExtensions.empty()) {
                    ...
                    CVLog::Print(tr("[Plugin][%1] New file extensions "
                                    "registered: %2")
                                         .arg(ioPlugin->getName(),
                                              ioExtensions.join(' ')));
                }
    ...
    for (const QString &path : paths) {
        ...
        for (const QString &fileName : fileNames) {
            ...
            if (!metaDataValid || pluginIID.isNull()) {
                CVLog::Warning(
                        tr("\t%1 has invalid meta data\t").arg(fileName));
            ...
            ccPluginInterface *ccPlugin =
                    qobject_cast<ccPluginInterface *>(plugin);

            if ((plugin == nullptr) || (ccPlugin == nullptr)) {
                if (plugin == nullptr) {
                    CVLog::Warning(
                            tr("\t%1 does not seem to be a valid plugin\t(%2)")
                                    .arg(fileName, loader->errorString()));
                } else {
                    CVLog::Warning(tr("\t%1 does not seem to be a valid plugin "
                                      "or it is not supported by this version")
                                           .arg(fileName));
                }
            ...
            if (ccPlugin->getName().isEmpty()) {
                CVLog::Error(tr("\tPlugin %1 has a blank name").arg(fileName));
            ...
            QPluginLoader *previousLoader =
                    pluginIIDToLoaderMap.value(pluginIID);

            // If we have already loaded a plugin with this file name, unload it
            // and replace the interface in the plugin list
            if (previousLoader != nullptr) {
                ccPluginInterface *pluginInterface =
                        qobject_cast<ccPluginInterface *>(
                                previousLoader->instance());
                ...
                CVLog::Warning(tr("\t%1 overridden").arg(fileName));
            } else {
                m_pluginList.push_back(ccPlugin);
            }
            ...
            CVLog::Print(tr("\tPlugin found: %1 (%2)")
                                 .arg(ccPlugin->getName(), fileName));
        }
    }
