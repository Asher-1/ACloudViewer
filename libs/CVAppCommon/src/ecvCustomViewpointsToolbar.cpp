...
void ecvCustomViewpointsToolbar::constructor() {
    ...
    pixPaint.drawPixmap(0, 0, 48, 48,
                        QPixmap(":/Resources/images/svg/pqCamera.png"));

    // Create plus pixmap
    this->PlusPixmap = this->BasePixmap.copy();
    QPainter pixWithPlusPaint(&this->PlusPixmap);
    pixWithPlusPaint.drawPixmap(32, 32, 32, 32,
                                QPixmap(":/Resources/images/svg/pqPlus.png"));
    this->ConfigPixmap = this->BasePixmap.copy();
    QPainter pixWithConfigPaint(&this->ConfigPixmap);
    pixWithConfigPaint.drawPixmap(
            32, 32, 32, 32, QPixmap(":/Resources/images/svg/pqWrench.png"));
    ...
    this->connect(settings, SIGNAL(modified()),
                  SLOT(updateCustomViewpointActions()));
}
...
void ecvCustomViewpointsToolbar::updateEnabledState() {
    ...
    actions[actions.size() - 2]->setEnabled(
            actions.size() <
            ecvCustomViewpointButtonDlg::MAXIMUM_NUMBER_OF_ITEMS + 2);
}
...
void ecvCustomViewpointsToolbar::updateCustomViewpointActions() {
    ...
    if (!this->ConfigAction) {
        this->ConfigAction = this->addAction(
                QIcon(this->ConfigPixmap), tr("Configure custom viewpoints"),
                this, SLOT(ConfigureCustomViewpoints()));
        this->ConfigAction->setObjectName("ConfigAction");
    }

    if (!this->PlusAction) {
        this->PlusAction = this->addAction(
                QIcon(this->PlusPixmap),
                tr("Add current viewpoint as custom viewpoint"), this,
    ...
    for (int cc = 0; cc < tooltips.size(); cc++) {
        ...
        if (this->ViewpointActions.size() > cc) {
            ...
            pixWithNumberPaint.setPen(
                    QApplication::palette().windowText().color());
            pixWithNumberPaint.drawText(pixmap.rect(),
                                        Qt::AlignRight | Qt::AlignBottom,
                                        QString::number(cc + 1));

            QAction* action = this->addAction(QIcon(pixmap), "", this,
...
#else
    CVLog::Warning(
            "[ecvCustomViewpointsToolbar::ApplyCustomViewpoint] please use pcl "
            "as backend and then try again!");
#endif
}
