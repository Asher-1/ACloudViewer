...
namespace
{
	QStringList getListOfStrings(ecvSettingManager* settings, const QString& defaultTxt, int min, int max)
	{
		QStringList val;
...
public:
	...
	void updateCustomViewpointButtons(::ecvCameraParamEditDlg* self)
	{
		...
		if (toolTips.size() < ecvCustomViewpointButtonDlg::MAXIMUM_NUMBER_OF_ITEMS)
		{
			self->connect(
...
ecvCameraParamEditDlg::ecvCameraParamEditDlg(QWidget* parent, ccPickingHub* pickingHub)
	//: ccOverlayDialog(parent, pickingHub ? Qt::FramelessWindowHint | Qt::Tool : Qt::Tool) //pickingHub = ACloudViewer / otherwise = ccViewer
	...
	, m_pickingHub(pickingHub)
{
	...
	QObject::connect(this->Internal->AutoPickCenterOfRotation, SIGNAL(toggled(bool)), this,
	...
	QObject::connect(this->Internal->saveCameraConfiguration,
		SIGNAL(clicked()), this, SLOT(saveCameraConfiguration()));

	QObject::connect(this->Internal->loadCameraConfiguration, 
		SIGNAL(clicked()), this, SLOT(loadCameraConfiguration()));

	QObject::connect(this->Internal->rcxDoubleSpinBox, static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), this, &ecvCameraParamEditDlg::pivotChanged);
	QObject::connect(this->Internal->rcyDoubleSpinBox, static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), this, &ecvCameraParamEditDlg::pivotChanged);
	QObject::connect(this->Internal->rczDoubleSpinBox, static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), this, &ecvCameraParamEditDlg::pivotChanged);
	...
	QObject::connect(this->Internal->configureCustomViewpoints, 
		SIGNAL(clicked()), this, SLOT(ConfigureCustomViewpoints()));

	this->Internal->AutoPickCenterOfRotation->setChecked(ecvDisplayTools::AutoPickPivotAtCenter());

	QObject::connect(this->Internal->updatePushButton, SIGNAL(clicked()), this, SLOT(updateCamera()));
	QObject::connect(this->Internal->pivotPickingToolButton, &QAbstractButton::toggled, this, &ecvCameraParamEditDlg::pickPointAsPivot);
	QObject::connect(ecvSettingManager::TheInstance(), SIGNAL(modified()), SLOT(updateCustomViewpointButtons()));
	QObject::connect(ecvDisplayTools::TheInstance(), &ecvDisplayTools::cameraParamChanged, this, &ecvCameraParamEditDlg::cameraChanged);

	// load custom view buttons with any tool tips set by the user in a previous session.
...
void ecvCameraParamEditDlg::processPickedItem(ccHObject* entity, unsigned, int, int, const CCVector3& P)
{
...
bool ecvCameraParamEditDlg::linkWith(QWidget* win)
{
	...
	if (oldWin != m_associatedWin && this->Internal->pivotPickingToolButton->isChecked())
	{
	...
	if (m_associatedWin)
	{
		...
		connect(ecvDisplayTools::TheInstance(), &ecvDisplayTools::pivotPointChanged, this, &ecvCameraParamEditDlg::updatePivotPoint);
...
void ecvCameraParamEditDlg::updateCamera()
{
	...
	m_tool->CurrentCameraParam.rotationFactor = this->Internal->rotationFactor->value();
	...
	m_tool->CurrentCameraParam.clippRange.x = this->Internal->nearClipping->value();
	m_tool->CurrentCameraParam.clippRange.y = this->Internal->farClipping->value();
...
{
	ecvDisplayTools::SendAutoPickPivotAtCenter(this->Internal->AutoPickCenterOfRotation->isChecked());
...
void ecvCameraParamEditDlg::updateUi()
{
	...
	this->Internal->nearClipping->setValue(m_tool->CurrentCameraParam.clippRange.x);
	this->Internal->farClipping->setValue(m_tool->CurrentCameraParam.clippRange.y);
	...
	this->Internal->rcyDoubleSpinBox->blockSignals(true);
	this->Internal->rczDoubleSpinBox->blockSignals(true);
	this->Internal->rcxDoubleSpinBox->setValue(m_tool->CurrentCameraParam.pivot.x);
	this->Internal->rcyDoubleSpinBox->setValue(m_tool->CurrentCameraParam.pivot.y);
	this->Internal->rczDoubleSpinBox->setValue(m_tool->CurrentCameraParam.pivot.z);
	this->Internal->rcxDoubleSpinBox->blockSignals(false);
	this->Internal->rcyDoubleSpinBox->blockSignals(false);
	this->Internal->rczDoubleSpinBox->blockSignals(false);

	this->Internal->factorHorizontalSlider->blockSignals(true);
	this->Internal->factorHorizontalSlider->setValue(qFloor(m_tool->CurrentCameraParam.rotationFactor * 10.0));
	this->Internal->factorHorizontalSlider->blockSignals(false);

	this->Internal->rotationFactor->blockSignals(true);
	this->Internal->rotationFactor->setValue(m_tool->CurrentCameraParam.rotationFactor);
...
//-----------------------------------------------------------------------------
void ecvCameraParamEditDlg::resetViewDirection(
	double look_x, double look_y, double look_z, double up_x, double up_y, double up_z)
{
	if (this->m_tool)
	{
		this->m_tool->resetViewDirection(look_x, look_y, look_z, up_x, up_y, up_z);
		this->cameraChanged();
	}
}
...
//-----------------------------------------------------------------------------
void ecvCameraParamEditDlg::adjustCamera(CameraAdjustmentType enType, double value)
{
	if (this->m_tool)
	{
		this->m_tool->adjustCamera(ecvGenericCameraTool::CameraAdjustmentType(enType), value);
		this->cameraChanged();
	}
}
...
{
	this->adjustCamera(CameraAdjustmentType::Roll, this->Internal->rollAngle->value());
}

//-----------------------------------------------------------------------------
void ecvCameraParamEditDlg::applyCameraElevation()
{
	this->adjustCamera(CameraAdjustmentType::Elevation, this->Internal->elevationAngle->value());
}

//-----------------------------------------------------------------------------
void ecvCameraParamEditDlg::applyCameraAzimuth()
{
	this->adjustCamera(CameraAdjustmentType::Azimuth, this->Internal->azimuthAngle->value());
}

//-----------------------------------------------------------------------------
void ecvCameraParamEditDlg::applyCameraZoomIn()
{
	this->adjustCamera(CameraAdjustmentType::Zoom, this->Internal->zoomFactor->value());
}

//-----------------------------------------------------------------------------
void ecvCameraParamEditDlg::applyCameraZoomOut()
{
	this->adjustCamera(CameraAdjustmentType::Zoom, 1.0 / this->Internal->zoomFactor->value());
}
...
bool ecvCameraParamEditDlg::ConfigureCustomViewpoints(QWidget* parentWidget)
{
	...
	QStringList configs = ecvCameraParamEditDlg::CustomViewpointConfigurations();
...
bool ecvCameraParamEditDlg::SetToCurrentViewpoint(int CustomViewpointIndex)
{
	...
	settings->beginGroup("CustomViewButtons");
	settings->beginGroup("Configurations");
	settings->setValue(QString::number(CustomViewpointIndex), 
		QString(ecvGenericCameraTool::CurrentCameraParam.toString().c_str()));
	settings->endGroup();
	settings->endGroup();
	settings->alertSettingsModified();
	return true;
...
QStringList ecvCameraParamEditDlg::CustomViewpointConfigurations()
{
	...
	const QStringList configs =
		getListOfStrings(settings, ecvCustomViewpointButtonDlg::DEFAULT_TOOLTIP,
...
void ecvCameraParamEditDlg::saveCameraConfiguration()
{
	...
	QString selectedFilename = QFileDialog::getSaveFileName(this,
		tr("Save Custom Viewpoints Configuration"),
		currentPath,
		filters);
...
void ecvCameraParamEditDlg::loadCameraConfiguration()
{
	...
	QStringList selectedFiles =
		QFileDialog::getOpenFileNames(
			this,
			QString("Load Custom Camera Configuration"),
			currentPath,
			filters);
...
void ecvCameraParamEditDlg::pickPointAsPivot(bool state)
{
	...
	else if (m_associatedWin)
	{
		if (state)
		{
			ecvDisplayTools::SetPickingMode(ecvDisplayTools::POINT_OR_TRIANGLE_PICKING);
			connect(ecvDisplayTools::TheInstance(), &ecvDisplayTools::itemPicked, this, &ecvCameraParamEditDlg::processPickedItem);
		}
		else
		{
			ecvDisplayTools::SetPickingMode(ecvDisplayTools::DEFAULT_PICKING);
			disconnect(ecvDisplayTools::TheInstance(), &ecvDisplayTools::itemPicked, this, &ecvCameraParamEditDlg::processPickedItem);
		}
	}
