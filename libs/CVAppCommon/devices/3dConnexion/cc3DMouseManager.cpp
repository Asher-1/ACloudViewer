...
cc3DMouseManager::cc3DMouseManager(ecvMainAppInterface* appInterface,
                                   QObject* parent)
    : QObject(parent), m_appInterface(appInterface), m3dMouseInput(nullptr) {
    setupMenu();
    ...
    if (state) {
        ...
        if (m3dMouseInput->connect(m_appInterface->getMainWindow(),
            ...
            connect(m3dMouseInput, &Mouse3DInput::sigReleased, this,
                    &cc3DMouseManager::on3DMouseReleased);
            connect(m3dMouseInput, &Mouse3DInput::sigOn3dmouseKeyDown, this,
                    &cc3DMouseManager::on3DMouseKeyDown);
            connect(m3dMouseInput, &Mouse3DInput::sigOn3dmouseKeyUp, this,
                    &cc3DMouseManager::on3DMouseKeyUp);
            connect(m3dMouseInput, &Mouse3DInput::sigOn3dmouseCMDKeyDown, this,
                    &cc3DMouseManager::on3DMouseCMDKeyDown);
            connect(m3dMouseInput, &Mouse3DInput::sigOn3dmouseCMDKeyUp, this,
                    &cc3DMouseManager::on3DMouseCMDKeyUp);
        } else {
            ...
            if (!silent) {
                CVLog::Error(
                        "[3D Mouse] No device found");  // warning message has
                                                        // already been issued
                                                        // by
        ...
        CVLog::Warning("[3D Mouse] Device has been disabled");
    }
...
void cc3DMouseManager::releaseDevice() {
    if (m3dMouseInput == nullptr) return;
...
void cc3DMouseManager::on3DMouseKeyDown(int key) {
    switch (key) {
            // CVLog::Print(QString("on3DMouseKeyDown Key = %1").arg(key));
        ...
        default:
            CVLog::Warning("[3D mouse] This button is not handled (yet)");
            // TODO
            break;
    }
}
...
void cc3DMouseManager::on3DMouseCMDKeyDown(int cmd) {
    switch (cmd) {
        // CVLog::Print(QString("on3DMouseCMDKeyDown Cmd = %1").arg(cmd));
