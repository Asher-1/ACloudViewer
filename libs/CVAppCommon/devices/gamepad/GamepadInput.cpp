...
GamepadInput::GamepadInput(QObject* parent /*=nullptr*/)
    ...
    connect(&m_timer, &QTimer::timeout, this,
            &GamepadInput::updateInternalState);
}

GamepadInput::~GamepadInput() {}

void GamepadInput::start() { m_timer.start(0); }

void GamepadInput::stop() { m_timer.stop(); }
...
void GamepadInput::updateInternalState() {
    ...
    // rotation
    {
        CCVector3d u(axisRightX() * s_gamepadRotSpeed,
                     -axisRightY() * s_gamepadRotSpeed, 1);
        u.normalize();
        ...
        if (buttonL2() || buttonR2()) {
            double angle_deg = buttonL2() ? 20 * s_gamepadRotSpeed
                                          : -20 * s_gamepadRotSpeed;
            ccGLMatrixd rot;
            rot.initFromParameters(cloudViewer::DegreesToRadians(angle_deg),
                                   CCVector3d(0, 0, 1), CCVector3d(0, 0, 0));
    ...
    // translation
    {
        ...
        if (x != 0 || z != 0) {
            m_translation =
                    CCVector3(x * s_gamepadRotSpeed, 0, z * s_gamepadRotSpeed);
