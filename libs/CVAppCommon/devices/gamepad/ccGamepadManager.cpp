...
ccGamepadManager::ccGamepadManager( ecvMainAppInterface *appInterface, QObject *parent )
	: QObject(parent)
...
void ccGamepadManager::enableDevice(bool state, bool silent, int deviceID/*=-1*/)
{
	...
	if (state)
	{
		for (int step = 0; step < 1; ++step) //fake loop for easy break
		{
			...
			if (deviceID < 0)
			{
				...
				{
					CVLog::Print(QString("[Gamepad] Found %1 gamepad(s)").arg(gamepads.size()));
				}
			...
			{
				CVLog::Print(QString("[Gamepad] Device %1 is now enabled").arg(deviceID));
	...
	else if (m_gamepadInput)
	{
		...
		m_gamepadInput->stop();
		CVLog::Warning(QString("[Gamepad] Device %1 is now disabled").arg(m_gamepadInput->deviceId()));
	}
...
void ccGamepadManager::setupGamepadInput()
{
	...
	if (manager)
	{
		...
		{
			CVLog::Print(QString("gamepad device %1 has been connected").arg(deviceId));
		...
		{
			CVLog::Print(QString("gamepad device %1 has been disconnected").arg(deviceId));
