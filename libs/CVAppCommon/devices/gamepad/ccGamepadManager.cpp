...
ccGamepadManager::ccGamepadManager(ecvMainAppInterface* appInterface,
    ...
    setupMenu();
    setupGamepadInput();  // DGM:at this point we can only init the gamepadInput
                          // structure (Qt will send a signal when the gamepad
                          // is connected - at least from its point of view)
    enableDevice(true, true);  // DGM: but if it's already connected, we'll have
...
void ccGamepadManager::enableDevice(bool state,
    ...
    if (state) {
        for (int step = 0; step < 1; ++step)  // fake loop for easy break
        {
            ...
            if (deviceID < 0) {
                CVLog::Print("[Gamepad] Looking for connected gamepads...");
                QList<int> gamepads;
                for (int i = 0; i < 3; ++i) {
                ...
                if (gamepads.empty()) {
                    ...
                    CVLog::Print(QString("[Gamepad] Found %1 gamepad(s)")
                                         .arg(gamepads.size()));
                }
                ...
                if (!silent && gamepads.size() > 1) {
                    ...
                    ccPickOneElementDlg poeDlg(
                            "Gamepad", "Connected gamepads",
                            m_appInterface ? m_appInterface->getMainWindow()
                                           : nullptr);
                    for (int id : gamepads) {
            ...
            if (m_gamepadInput->isConnected()) {
                CVLog::Print(QString("[Gamepad] Device %1 is now enabled")
                                     .arg(deviceID));
                m_gamepadInput->start();
                break;
            } else {
                showMessage(QString("[Gamepad] Device %1 is not connected")
                                    .arg(deviceID),
                            silent);
                state = false;
                break;
        ...
        m_gamepadInput->stop();
        CVLog::Warning(QString("[Gamepad] Device %1 is now disabled")
                               .arg(m_gamepadInput->deviceId()));
    }
...
void ccGamepadManager::releaseDevice() {
    if (m_gamepadInput == nullptr) return;
...
void ccGamepadManager::setupGamepadInput() {
    ...
    connect(m_gamepadInput, &GamepadInput::updated, this,
            &ccGamepadManager::onGamepadInput, Qt::DirectConnection);
    ...
    connect(m_gamepadInput, &GamepadInput::buttonStartChanged, this,
            [=](bool state) {
                if (state) {
                    m_appInterface->setGlobalZoom();
                }
            });
    connect(m_gamepadInput, &GamepadInput::buttonAChanged, this,
            [=](bool state) {
                if (state) {
                    m_appInterface->toggleActiveWindowViewerBasedPerspective();
                }
            });
    connect(m_gamepadInput, &GamepadInput::buttonBChanged, this,
            [=](bool state) {
                if (state) {
    ...
    QGamepadManager* manager = QGamepadManager::instance();
    if (manager) {
        connect(manager, &QGamepadManager::gamepadConnected, this,
                [&](int deviceId) {
                    CVLog::Print(QString("gamepad device %1 has been connected")
                                         .arg(deviceId));
        ...
        connect(manager, &QGamepadManager::gamepadDisconnected, this,
                [&](int deviceId) {
                    CVLog::Print(
                            QString("gamepad device %1 has been disconnected")
                                    .arg(deviceId));
                    // auto-disable the device (if this device is the one
                    // currently enabled)
                    if (m_actionEnable && m_actionEnable->isChecked() &&
                        m_gamepadInput &&
                        m_gamepadInput->deviceId() == deviceId) {
...
void ccGamepadManager::setupMenu() {
    ...
    if (!m_actionEnable) {
        ...
        connect(m_actionEnable, &QAction::toggled,
                [this](bool state) { enableDevice(state, false); });
...
void ccGamepadManager::onGamepadInput() {
    ...
    QMainWindow* win = m_appInterface->getMainWindow();
    if (win) {
