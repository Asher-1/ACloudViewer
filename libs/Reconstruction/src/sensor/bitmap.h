...
public:
    ...
    // Construct bitmap with given dimensions (upstream parity).
    Bitmap(const int width, const int height, const bool as_rgb);
    ...
    // Upstream-parity overload returning std::optional.
    std::optional<BitmapColor<uint8_t>> GetPixel(const int x,
                                                 const int y) const;
