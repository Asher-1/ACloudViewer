...
Bitmap::Bitmap(const int width, const int height, const bool as_rgb) {
  THROW_CHECK(Allocate(width, height, as_rgb))
      << "Failed to allocate bitmap of size " << width << "x" << height;
}
