...
#define CAMERA_MODEL_SHARED_DEFINITIONS(model_id_val, model_name_val,    \
                                        params_info_val, num_params_val, \
    ...
    static inline bool ImgFromCamWithJac(                                \
            const double* params, const double& u, const double& v,      \
            const double& w, double* x, double* y, double* J_params,     \
            double* J_uvw, bool check_cheirality = true);
#endif
...
#define PERSPECTIVE_CAMERA_MODEL_PARAM_DEFINITIONS(                    \
    ...
    static constexpr std::array<size_t, (num_focal_params_val)>        \
            focal_length_idxs =                                        \
                    internal::IotaArray<(num_focal_params_val), 0>();  \
    static constexpr std::array<size_t, (num_pp_params_val)>           \
            principal_point_idxs =                                     \
                    internal::IotaArray<(num_pp_params_val),           \
...
#define SPHERICAL_CAMERA_MODEL_PARAM_DEFINITIONS(num_metadata_params_val)  \
    ...
    static constexpr std::array<size_t, (num_metadata_params_val)>         \
            metadata_idxs =                                                \
                    internal::IotaArray<(num_metadata_params_val), 0>();
#endif

// Convenience composition macros used in the model class declarations.
#ifndef PERSPECTIVE_CAMERA_MODEL_DEFINITIONS
#define PERSPECTIVE_CAMERA_MODEL_DEFINITIONS(                                \
        model_id_val, model_name_val, params_info_val, num_focal_params_val, \
        num_pp_params_val, num_extra_params_val,                             \
        has_img_from_cam_with_jac_val)                                       \
    CAMERA_MODEL_SHARED_DEFINITIONS(                                         \
            model_id_val, model_name_val, params_info_val,                   \
...
#define SPHERICAL_CAMERA_MODEL_DEFINITIONS(                                   \
        model_id_val, model_name_val, params_info_val,                        \
        num_metadata_params_val, has_img_from_cam_with_jac_val)               \
    CAMERA_MODEL_SHARED_DEFINITIONS(model_id_val, model_name_val,             \
                                    params_info_val, num_metadata_params_val, \
                                    has_img_from_cam_with_jac_val)            \
...
inline bool HasProjectableDepth(const T& w, const bool check_cheirality) {
    return check_cheirality
                   ? w >= std::numeric_limits<T>::epsilon()
...
struct BasePerspectiveCameraModel : public BaseCameraModel<CameraModel> {
    ...
    static inline bool HasBogusParams(const std::vector<T>& params,
        ...
        return CameraModel::HasBogusPrincipalPoint(params, width, height) ||
               CameraModel::HasBogusFocalLength(params, width, height,
    ...
    static inline bool IterativeUndistortion(const double* params,
        ...
        for (size_t i = 0; i < kNumIterations; ++i) {
            ...
            CameraModel::Distortion(params_jet, x_jet[0], x_jet[1], &dx_jet[0],
                                    &dx_jet[1]);
            dx[0] = dx_jet[0].a;
            dx[1] = dx_jet[1].a;
    ...
    static inline void Rescale(double scale_x,
        ...
        if constexpr (CameraModel::num_focal_params == 1) {
            (*params)[CameraModel::focal_length_idxs[0]] *=
                    0.5 * (scale_x + scale_y);
        } else {
...
struct BaseSphericalCameraModel : public BaseCameraModel<CameraModel> {
    // Rescale the parameters in-place for a new image resolution. Only the
    // image dimensions (w, h), carried by the metadata group, track the
    // rescaled image; any extra parameters are resolution independent and left
    // untouched.
    static inline void Rescale(double scale_x,
                               double scale_y,
                               std::vector<double>* params) {
