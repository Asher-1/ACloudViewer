...
struct EquirectangularCameraModel
    ...
    // Projects camera-frame point (u, v, w) onto the equirectangular image
    // plane. Unlike pinhole/fisheye models that require w > 0, EQUIRECTANGULAR
    // accepts any non-zero direction — all 4π of the sphere are representable.
    template <typename T>
    static inline bool ImgFromCam(const T* params,
        ...
        // Elevation φ ∈ [-π/2, π/2], measured from the equator. -Y (up) is
        // +π/2.
    ...
    // coordinates (u = X/Z, v = Y/Z) of the pixel's ray, valid only when the
    // ray falls in the forward hemisphere (Z > 0). Back-hemisphere pixels
    // return false; use CamRayFromImg for the full-sphere 3D bearing.
    ...
    // normalized camera-coordinate thresholds using the angular resolution at
    // the equator (2π rad per W pixels in azimuth).
    template <typename T>
    static inline T CamFromImgThreshold(const T* params, T threshold) {
        return threshold * T(2.0 * EIGEN_PI) / params[0];
    }

    // The base's default CamRayFromImg goes through the 2D CamFromImg which
    // fails for back-hemisphere pixels. EQUIRECTANGULAR can produce valid unit
    // bearings for any pixel in the equirectangular image, so we compute the
    // ray directly from the azimuth/elevation parametrization.
