...
#include "sensor/models/runtime.h"
...
inline void FisheyeProjectionWithJac(
    ...
    *vv = s * b;
    if (J_fisheye) {
        // With s = atan(r) / r and r = sqrt(a^2 + b^2), the Jacobian of (uu,
        // vv) = (s * a, s * b) w.r.t. (a, b) is s * I + g * outer((a, b), (a,
        // b)), where g = (ds/dr) / r = (r / (1 + r^2) - atan(r)) / r^3.
...
bool RadialCameraModel::ImgFromCamWithJac(const double* params,
    ...
    *y = f * yd + c2;
    ...
    if (J_uvw) {
        ...
        // radial = k1 * r2 + k2 * r2^2, r2 = uu^2 + vv^2, the distortion
        // Jacobian in normalized coordinates (uu, vv) is:
...
bool OpenCVCameraModel::ImgFromCamWithJac(const double* params,
    ...
    *y = f2 * yd + c2;
    ...
    if (J_uvw) {
        ...
        // normalized coordinates (uu, vv), with d_radial_d_r2 = k1 + 2 * k2 *
        // r2:
        ...
        // The chain rule through (uu, vv) = (u/w, v/w) yields the columns
        // below.
        const double d_radial_d_r2 = k1 + 2.0 * k2 * r2;
        const double cross = 2.0 * uv * d_radial_d_r2;
        const double du_duu = radial + 2.0 * uu2 * d_radial_d_r2 +
                              2.0 * p1 * vv + 6.0 * p2 * uu;
        const double du_dvv = cross + 2.0 * p1 * uu + 2.0 * p2 * vv;
        const double dv_duu = cross + 2.0 * p2 * vv + 2.0 * p1 * uu;
        const double dv_dvv = radial + 2.0 * vv2 * d_radial_d_r2 +
                              2.0 * p2 * uu + 6.0 * p1 * vv;
...
bool FullOpenCVCameraModel::ImgFromCamWithJac(const double* params,
    ...
    *y = f2 * yd + c2;
    ...
    if (J_uvw) {
        // J_uvw is a 2x3 matrix (row-major): d(x, y) / d(u, v, w).
        // With xd = uu * radial + tangential_x, yd = vv * radial +
        // tangential_y, and radial = num / den, the derivative of the rational
        // radial term is
        ...
        const double cross = 2.0 * uv * d_radial_d_r2;
        const double xd_duu = radial + 2.0 * uu2 * d_radial_d_r2 +
                              2.0 * p1 * vv + 6.0 * p2 * uu;
        const double xd_dvv = cross + 2.0 * p1 * uu + 2.0 * p2 * vv;
        const double yd_duu = cross + 2.0 * p2 * vv + 2.0 * p1 * uu;
        const double yd_dvv = radial + 2.0 * vv2 * d_radial_d_r2 +
                              2.0 * p2 * uu + 6.0 * p1 * vv;
    ...
    if (J_params) {
        ...
        // The numerator coefficients enter as d(radial)/d(k_i) = r^(2i) / den;
        // the denominator coefficients as d(radial)/d(k_j) = -num * r^(2j) /
        // den^2.
...
bool FOVCameraModel::ImgFromCamWithJac(const double* params,
    ...
    // The distortion scales (a, b) by a radially symmetric factor. We compute
    // the factor and its partials factor_r = d(factor)/d(radius2) and
    // factor_omega = d(factor)/d(omega), matching whichever branch
    // FOVCameraModel::Distortion selects so that the analytic Jacobian agrees
    // with autodiff everywhere.
    double factor = 0;
    double factor_r = 0;
    ...
    *y = f2 * dv + c2;
    ...
    if (J_uvw) {
        // d(du, dv) / d(a, b) with du = a * factor, dv = b * factor and factor
        // a function of radius2 = a^2 + b^2.
    ...
    if (J_params) {
        // J_params is a 2x5 matrix (row-major): d(x, y) / d(fx, fy, cx, cy,
        // omega).
        J_params[0] = du;
        J_params[1] = 0.0;
        J_params[2] = 1.0;
...
bool SimpleRadialFisheyeCameraModel::ImgFromCamWithJac(
    ...
    internal::FisheyeProjectionWithJac(a, b, &uu, &vv,
                                       J_uvw ? J_fisheye : nullptr);
    ...
    *y = f * vv_d + c2;
    ...
    if (J_uvw) {
        ...
        const double ipjd[4] = {1.0 + radial + two_k * uu2, two_k * uu * vv,
                                two_k * uu * vv, 1.0 + radial + two_k * vv2};
        double m[4];
...
bool RadialFisheyeCameraModel::ImgFromCamWithJac(const double* params,
    ...
    *y = f * vv_d + c2;
    ...
    if (J_uvw) {
        ...
        const double ipjd[4] = {1.0 + radial + 2.0 * uu2 * d_radial, cross,
                                cross, 1.0 + radial + 2.0 * vv2 * d_radial};
        double m[4];
        internal::MatMul2x2(ipjd, J_fisheye, m);
        const double J_ab[4] = {f * m[0], f * m[1], f * m[2], f * m[3]};
        internal::UvwJacFromAbJac(J_ab, a, b, inv_w, J_uvw);
    ...
    *y = f2 * vv_d + c2;
    ...
    if (J_uvw) {
        const double d_radial =
                k1 + 2.0 * k2 * t2 + 3.0 * k3 * t4 + 4.0 * k4 * t6;
        const double cross = 2.0 * uu * vv * d_radial;
        const double ipjd[4] = {1.0 + radial + 2.0 * uu2 * d_radial, cross,
                                cross, 1.0 + radial + 2.0 * vv2 * d_radial};
        double m[4];
...
bool ThinPrismFisheyeCameraModel::ImgFromCamWithJac(
    ...
    *y = f2 * vv_d + c2;
    ...
    if (J_uvw) {
        const double d_radial =
                k1 + 2.0 * k2 * r2 + 3.0 * k3 * r4 + 4.0 * k4 * r6;
...
bool RadTanThinPrismFisheyeModel::ImgFromCamWithJac(
    ...
    // derivative th_radial' w.r.t. theta2 and the powers theta2^(i+1) used by
    // the per-coefficient parameter Jacobians.
    ...
    *y = f2 * Y + c2;
    ...
    if (J_uvw || J_params) {
        // B = d(X, Y) / d(xr, yr) (tangential + thin-prism stage), used by both
        // the point/pose and the radial-coefficient Jacobians.
        const double B00 = 1.0 + 2.0 * p1 * yr + 6.0 * p0 * xr + 2.0 * s0 * xr +
                           4.0 * s1 * xr * r2;
        const double B01 = 2.0 * p1 * xr + 2.0 * p0 * yr + 2.0 * s0 * yr +
                           4.0 * s1 * yr * r2;
        const double B10 = 2.0 * p0 * yr + 2.0 * p1 * xr + 2.0 * s2 * xr +
                           4.0 * s3 * xr * r2;
        ...
        if (J_uvw) {
            ...
            const double A[4] = {th_radial + 2.0 * uu * uu * d_th_radial, cross,
                                 cross,
        ...
        if (J_params) {
            ...
            // Radial coefficients k0..k5 (params 4..9): dxr/dk_i = uu *
            // theta2^(i+1), dyr/dk_i = vv * theta2^(i+1), propagated through
            // the tangential/prism stage B.
            for (int i = 0; i < 6; ++i) {
...
bool SimpleFisheyeCameraModel::ImgFromCamWithJac(const double* params,
    ...
    *y = f * vv + c2;
    ...
    if (J_uvw) {
        const double J_ab[4] = {f * J_fisheye[0], f * J_fisheye[1],
                                f * J_fisheye[2], f * J_fisheye[3]};
        internal::UvwJacFromAbJac(J_ab, a, b, inv_w, J_uvw);
    }
...
bool FisheyeCameraModel::ImgFromCamWithJac(const double* params,
    ...
    *y = f2 * vv + c2;
    ...
    if (J_uvw) {
        const double J_ab[4] = {f1 * J_fisheye[0], f1 * J_fisheye[1],
                                f2 * J_fisheye[2], f2 * J_fisheye[3]};
        internal::UvwJacFromAbJac(J_ab, a, b, inv_w, J_uvw);
    }
...
bool SimpleDivisionCameraModel::ImgFromCamWithJac(
    ...
    if (!internal::DivisionScaleWithJac(u, v, w, k, &r,
                                        with_jac ? &dr_du : nullptr, &dr_dv,
                                        &dr_dw, &dr_dk)) {
        return false;
    }
...
bool EquirectangularCameraModel::ImgFromCamWithJac(
    ...
    *y = (0.5 - phi * kInvPi) * height;
    ...
    if (J_uvw) {
        ...
        // Hoist the shared reciprocals: R2 and N2*horizontal each divide more
        // than one derivative, and without -ffast-math the compiler cannot
        // factor the repeated runtime division out on its own.
