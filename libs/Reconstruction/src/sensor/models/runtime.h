...
#include "sensor/models/base.h"
#include "sensor/models/division.h"
#include "sensor/models/eucm.h"
#include "sensor/models/fisheye.h"
#include "sensor/models/fisheye_radial.h"
#include "sensor/models/fov.h"
#include "sensor/models/opencv.h"
#include "sensor/models/pinhole.h"
#include "sensor/models/radial.h"
#include "sensor/models/spherical.h"
#include "sensor/models/thin_prism.h"
...
//
// @param model_id     Unique identifier of camera model.
// Fork adaptation: upstream returns span<const size_t>; the fork has no
// span type and its models.cc hands out references to the static per-model
// index arrays instead.
const std::vector<size_t>& CameraModelFocalLengthIdxs(CameraModelId model_id);
const std::vector<size_t>& CameraModelPrincipalPointIdxs(
        CameraModelId model_id);
const std::vector<size_t>& CameraModelExtraParamsIdxs(CameraModelId model_id);
...
#define CAMERA_MODEL_CASE(CameraModel)                                        \
    case CameraModel::model_id:                                               \
        if (CameraModel::ImgFromCam(params.data(), uvw.x(), uvw.y(), uvw.z(), \
                                    &xy.x(), &xy.y(), check_cheirality)) {    \
            return xy;                                                        \
        }                                                                     \
        break;
...
#define CAMERA_MODEL_CASE(CameraModel)                                       \
    case CameraModel::model_id:                                              \
        static_assert(CameraModel::has_img_from_cam_with_jac, #CameraModel   \
                      " does not provide an analytic "                       \
                      "ImgFromCamWithJac, which this dispatch "              \
                      "requires. Implement it in "                           \
                      "models/jacobian.h.");                                 \
        if (CameraModel::ImgFromCamWithJac(params.data(), uvw.x(), uvw.y(),  \
                                           uvw.z(), &xy.x(), &xy.y(),        \
                                           /*J_params=*/nullptr, J_uvw_ptr,  \
                                           check_cheirality)) {              \
            if (J_uvw != nullptr) {                                          \
                *J_uvw = Eigen::Map<                                         \
...
std::optional<Eigen::Matrix3x2d> CamRayFromImgJac(
    ...
    // the ray, so for a unit ray |alpha| == ||g_x x g_y|| and alpha^2 is
    // exactly det(J J^T), the product of the squared singular values. Requiring
    // |alpha| > kMinRelAlpha * (||g_x||^2 + ||g_y||^2) therefore rejects
    // singular value ratios below kMinRelAlpha, i.e. condition numbers worse
    // than ~1e6. Relative, so the test is invariant to focal length.
...
#define CAMERA_MODEL_CASE(CameraModel)                                      \
    case CameraModel::model_id:                                             \
        if (CameraModel::CamFromImg(params.data(), xy.x(), xy.y(), &uv.x(), \
                                    &uv.y())) {                             \
            return uv;                                                      \
        }                                                                   \
        break;
...
#define CAMERA_MODEL_CASE(CameraModel)                                  \
    case CameraModel::model_id:                                         \
        if (CameraModel::CamRayFromImg(params.data(), xy.x(), xy.y(),   \
                                       &ray.x(), &ray.y(), &ray.z())) { \
            return ray;                                                 \
        }                                                               \
        break;
...
#define CAMERA_MODEL_CASE(CameraModel) \
    case CameraModel::model_id:        \
        return std::is_base_of_v<      \
                BasePerspectiveFisheyeCameraModel<CameraModel>, CameraModel>;
...
#define CAMERA_MODEL_CASE(CameraModel) \
    case CameraModel::model_id:        \
        return std::is_base_of_v<      \
                BasePerspectivePinholeCameraModel<CameraModel>, CameraModel>;
