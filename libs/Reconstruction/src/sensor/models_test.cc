...
void TestCamToCamFromImg(const std::vector<double>& params,
  ...
  // Fork note: the direct call and the runtime dispatch are inlined into
  // different contexts, so transcendental models (fisheye/division/EUCM)
  // can differ by 1 ULP from FMA contraction. Compare with a pixel-domain
  // tolerance that is far below any meaningful reprojection error.
  EXPECT_NEAR(x, xy->x(), 1e-9);
  EXPECT_NEAR(y, xy->y(), 1e-9);
