...
// Upstream port (COLMAP dbb41680 util/file_test.cc). Fork adaptations:
// CopyType naming, GetParentDir("/") == "" (misc_test-verified fork
// behavior), WriteBinaryBlob taking a vector (no span type in the fork),
// and the URI/download surface declared in util/download.h.
...
TEST(GetParentDir, Nominal) {
  ...
  EXPECT_EQ(GetParentDir("/test"), "/");
  // Fork parity (misc_test): the parent of "/" is "" rather than "/".
  EXPECT_EQ(GetParentDir("/"), "");
...
TEST(FileCopy, Nominal) {
  ...
  FileCopy(src_path, dst_path, CopyType::COPY);
  ...
  FileCopy(src_path, dst_hard_link_path, CopyType::HARD_LINK);
  EXPECT_TRUE(ExistsFile(dst_hard_link_path));

  const auto dst_soft_link_path = dir / "destination_soft_link.txt";
  FileCopy(src_path, dst_soft_link_path, CopyType::SOFT_LINK);
...
TEST(ReadWriteBinaryBlob, Nominal) {
  ...
  WriteBinaryBlob(file_path, data);
...
TEST(DownloadFile, Nominal) {
  ...
  WriteBinaryBlob(file_path, {data.begin(), data.end()});
...
TEST(MaybeDownloadAndCacheFile, Nominal) {
  ...
  WriteBinaryBlob(server_file_path, {data.begin(), data.end()});
