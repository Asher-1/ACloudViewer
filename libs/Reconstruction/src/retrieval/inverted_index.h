...
public:
    ...
    using WordIds = Eigen::
            Matrix<int64_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;
...
void InvertedIndex<kDescType, kDescDim, kEmbeddingDim>::Write(
    ...
    for (const auto& constant : normalization_constants_) {
        ...
        out->write(reinterpret_cast<const char*>(&constant.second),
                   sizeof(float));
    }
}
