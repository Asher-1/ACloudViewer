...
struct ImageReaderOptions {
    // Path to database in which to store the extracted data (fork extension
    // kept for the app-side readers that construct the options before opening
    ...
    // as the image has below image_path. The filename must be equal, aside from
    // the added extension .png. For example, for an image
