...
// ----------------------------------------------------------------------------
//
// Upstream COLMAP dbb41680 (controllers/global_pipeline.cc) port. Fork
// adaptations: include paths, Camera::HasPriorFocalLength(), the fork's
// value-based ReconstructionManager (Get returns a reference, so the
// non-owning shared_ptr wrapper keeps the upstream shared_ptr call shape),
// and ExtractColorsForAllImages without the num_threads argument.
...
std::optional<Reconstruction> GlobalPipeline::ReconstructSingleComponent(
    const std::shared_ptr<const DatabaseCache>& database_cache,
    const GlobalMapperOptions& mapper_options) {
  // Fork parity: the fork's ReconstructionManager stores Reconstruction by
  // value, so wrap the manager-owned reference in a non-owning shared_ptr to
  // keep the upstream call shape.
  Reconstruction& reconstruction_ref =
      reconstruction_manager_->Get(reconstruction_manager_->Add());
  std::shared_ptr<Reconstruction> reconstruction(
      &reconstruction_ref, [](Reconstruction*) {});
  ...
  // Move the result out of the manager slot so later component runs (which
  // append to the manager and may reallocate it) cannot interact with it.
  return std::move(reconstruction_ref);
}
...
void GlobalPipeline::Run() {
  ...
  if (options_.multiple_models) {
    ...
    const std::optional<Reconstruction> reconstruction =
    ...
    if (!reconstruction.has_value()) {
      ...
      // Re-insert the moved-out reconstruction into the manager slot.
  ...
  for (size_t i = first_reconstruction_idx; i < reconstruction_manager_->Size();
       ++i) {
    reconstructions.push_back(std::move(reconstruction_manager_->Get(i)));
  }
  std::sort(reconstructions.begin(),
            reconstructions.end(),
            [](const Reconstruction& lhs, const Reconstruction& rhs) {
...
GlobalPipeline::ReconstructionStats GlobalPipeline::ReconstructMultiComponents(
  ...
  for (size_t component_idx = 0; component_idx < components.size();
    ...
    PrintHeading1(
        StringPrintf("Reconstructing component %d / %d with %d images",
