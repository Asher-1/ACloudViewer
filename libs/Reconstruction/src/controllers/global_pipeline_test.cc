...
// ----------------------------------------------------------------------------
//
// Upstream COLMAP dbb41680 (controllers/global_pipeline_test.cc) port. Fork
// adaptations: concrete Database construction, include paths, and the fork's
// value-based ReconstructionManager (Get returns a reference).
...
TEST(GlobalPipeline, Nominal) {
  // Pin the global PRNG before synthesizing so the fixture geometry is
  // deterministic and toolchain-independent (libstdc++ std::shuffle /
  // uniform_int_distribution sequences differ across gcc versions; a track
  // whose GT minimum triangulation angle falls below min_tri_angle_deg would
  // otherwise legitimately be filtered on some toolchains and make the
...
TEST(GlobalPipeline, SfMWithRandomSeedStability) {
  ...
  // Multi-threaded execution.
  {
    ...
    auto reconstruction_manager1 =
        run_mapper(/*num_threads=*/3, /*random_seed=*/kRandomSeed);
    // Same seed should produce similar results, up to floating-point
    // variations in optimization. The thresholds are sanity-level (bit-exact
    // reproducibility depends on the toolchain's FMA/vectorization behavior).
    EXPECT_THAT(reconstruction_manager0->Get(0),
...
std::vector<FlatHashSet<image_t>> GroupImageIdsByRig(
  ...
  for (const auto& [frame_id, frame] : reconstruction.Frames()) {
    // Fork parity: Frame::ImageIds() yields the image identifiers directly.
    for (const image_t image_id : frame.ImageIds()) {
      images_by_rig[frame.RigId()].insert(image_id);
    }
  }
    ...
    const bool in_group =
        std::any_of(frame.ImageIds().begin(),
                    ...
                    [&](const image_t image_id) {
                      return group_image_ids.count(image_id) > 0;
                    });
    if (!in_group) {
