...
public:
    struct Options {
        ...
        // Maximum angular error to re-triangulate under-reconstructed image
        // pairs.
    ...
    // graph and the reconstruction objects must live as long as the
    // triangulator.
    ...
    //
    // Image pairs are under-reconstructed if less than
    // `Options::tri_re_min_ratio > tri_ratio`, where `tri_ratio` is the number
    // of triangulated matches over inlier matches between the image pair.
