...
struct GlobalMapperOptions {
    // Number of threads for parallel feature matching and bundle adjustment.
    int num_threads = -1;

    // Random seed for the whole pipeline. If negative, the PRNG is not reset.
    int random_seed = -1;
    ...
    // Max pixel distance between observations of the same track within one
    // image.
...
public:
    ...
    // Run the global SfM pipeline. The optional `on_progress` callback is
    // invoked after global positioning, after each bundle-adjustment iteration,
    // and after retriangulation/refinement; it returns true if a stop has been
    // requested, in which case the pipeline terminates early and keeps the
    // current result.
    ...
    // Run iterative bundle adjustment to refine poses and structure. The
    // optional `on_progress` callback is invoked after each iteration and
    // returns true if a stop has been requested, in which case the iteration
    // terminates early.
