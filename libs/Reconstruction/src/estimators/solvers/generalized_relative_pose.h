...
struct GRNPObservation {
    ...
    // Bearing in the camera frame, bundled with its Jacobian d(ray) / d(pixel)
    // so Residuals can score in pixel units with the tangent Sampson error.
    // Only the residual uses the Jacobian; the solvers read the ray alone.
...
public:
    ...
    // The estimated rig2_from_rig1 relative pose between the generalized
    // cameras.
    using M_t = Rigid3d;
