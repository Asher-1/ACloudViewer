...
public:
    ...
    // The estimated model: a calibrated essential matrix plus the recovered
    // focal length of the first (uncalibrated) view.
    struct M_t {
    ...
    // Estimate relative pose and the first view's focal from >= 6
    // correspondences by wrapping poselib::relpose_6pt_onesided_focal (uses the
    // first six).
    ...
    // loransac.h).
    //
    // Returns true and overwrites *model with the refined model on success. On
    // a degenerate decomposition (or non-positive focal) it returns false and
