...
public:
    ...
    // camera's pixels: the first ray is transferred through H and projected
    // back into the image. Unlike the essential matrix, a homography transfers
    // a point to a point rather than to a line, so this is the exact geometric
    // error and needs no linearization. Rays with no image point, i.e.
    // transferred behind a perspective camera, are scored at the maximum
    // residual.
    //
    // @param cam_rays1  First set of corresponding rays.
    // @param cam_rays2  Second set of corresponding rays and image points.
