...
public:
    ...
    struct M_t {
        ...
        // The focal lengths (fx, fy) of the camera. Equal when the focal length
        // is shared (e.g. single-focal camera models).
    ...
    // (suitable for single-focal camera models, e.g. SIMPLE_PINHOLE).
    // Otherwise, separate focal lengths for x and y are estimated (e.g.
    // PINHOLE, OPENCV).
...
private:
    ...
    double ComputeTotalError(const Eigen::Matrix3d& R,
