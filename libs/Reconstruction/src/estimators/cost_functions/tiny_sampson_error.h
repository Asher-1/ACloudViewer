...
public:
    ...
    bool operator()(const double* params,
        ...
        Eigen::Matrix3d dE[7];
        if (jacobian != nullptr) {
            const double x = params[0], y = params[1], z = params[2],
                         w = params[3];
            Eigen::Matrix3d dR[4];
            ...
            dR[2] << -4 * z, -2 * w, 2 * x, 2 * w, -4 * z, 2 * y, 2 * x, 2 * y,
                    0;  // dR/dqz
            dR[3] << 0, -2 * z, 2 * y, 2 * z, 0, -2 * x, -2 * y, 2 * x,
                    0;  // dR/dqw
...
public:
    ...
    // it stores the functor by reference, so the wrapped functor must outlive
    // it.
    ...
    TinyFocalSampsonErrorCostFunctor(
    ...
    bool operator()(const T* const params, T* residuals) const {
        ...
        // fundamental matrix F = diag(inv_f, inv_f, 1) * E * diag(inv_f, inv_f,
        // 1).
        ...
        for (size_t i = 0; i < points1_.size(); ++i) {
            residuals[i] = SampsonError<T>(F, points1_[i].cast<T>(),
                                           points2_[i].cast<T>());
        }
        return true;
    }
...
public:
    ...
    // ceres::TinySolver-compatible autodiff wrapper for this functor. Note that
    // it stores the functor by reference, so the wrapped functor must outlive
    // it. The solver uses the analytic Jacobian below, so this only serves to
    // pin that Jacobian to a second implementation in tests.
    ...
    template <typename T>
    bool operator()(const T* const params, T* residuals) const {
        // Build E once from the pose, then apply the unknown focal to the
        // columns to obtain the matrix relating view-1 image points to view-2
        // rays.
        ...
        for (size_t i = 0; i < img_points1_.size(); ++i) {
            residuals[i] = TangentSampsonError<T>(
                    M, img_points1_[i].cast<T>().homogeneous(), J1,
    ...
    bool operator()(const double* params,
        ...
        if (jacobian != nullptr) {
            ...
            dM[5] = ey * R;  // dE/dty
            dM[6] = ez * R;  // dE/dtz
            // The pose derivatives are of E, so carry them through the same
            // column scaling that turns E into M.
            for (int l = 0; l < 7; ++l) dM[l].leftCols<2>() *= inv_f1;
            // d(E * diag(1/f1, 1/f1, 1)) / d(log_f1) negates the scaled columns
            // and leaves the third one, which carries no focal, unchanged.
            dM[7] = -M;
            dM[7].col(2).setZero();
        }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            // Constraint gradients in view-1 and view-2 pixels. The former
            // needs no Jacobian, as d(x, y, 1)/d(x, y) merely selects the first
            // two rows.
            ...
            if (jacobian != nullptr) {
                ...
                //         - (num/denom^1.5) (ray2 [g1; 0]^T + (J2 g2)
                //         point1^T).
                ...
                const Eigen::Matrix3d drdM =
                        ...
                        coef * (ray2 * J1g1.transpose() +
                                J2g2 * point1.transpose());
                for (int l = 0; l < 8; ++l) {
...
public:
    ...
    bool operator()(const double* params,
        ...
        for (int i = 0; i < n; ++i) {
            ...
            // Only the first two components of each constraint gradient enter,
            // as the homogeneous third coordinate is not a free variable.
            const double denom = Fpoint1.head<2>().squaredNorm() +
                                 Ftpoint2.head<2>().squaredNorm();
            const double sqrt_denom = std::sqrt(denom);
            if (sqrt_denom == 0.0) {
                residuals[i] = 0.0;
                if (jacobian != nullptr) {
            ...
            if (jacobian != nullptr) {
                ...
                //         - (num/denom^1.5) (g2 point1^T + point2 g1^T), where
                //         g1 and
                // g2 are F^T point2 and F point1 with the third component
                // dropped.
