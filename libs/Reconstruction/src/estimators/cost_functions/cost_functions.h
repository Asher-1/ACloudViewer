...
public:
    ...
    bool operator()(const T* const qvec,
        ...
        // Distort and transform to pixel space (upstream parity: the model
        // API performs the depth division and rejects degenerate/cheirality-
        // violating points by zeroing the residual, keeping the autodiff
        // chain well-defined).
        if (CameraModel::ImgFromCam(camera_params, projection[0], projection[1],
                                    projection[2], &residuals[0],
                                    &residuals[1])) {
            // Re-projection error.
