...
inline Eigen::Vector4d ComputeFetzerPolynomialCoefficients(
        ...
        const int u,
        const int v) {
    return {ai(u) * aj(v) - ai(v) * aj(u), ai(u) * bj(v) - ai(v) * bj(u),
            bi(u) * aj(v) - bi(v) * aj(u), bi(u) * bj(v) - bi(v) * bj(u)};
}
...
inline std::array<Eigen::Vector4d, 2> DecomposeFundamentalMatrixForFetzer(
    ...
    // Factoring out the principal points before the SVD appears to be
    // numerically more stable than the method described in the paper.
...
public:
    ...
    static ceres::CostFunction* Create(
            ...
            const Eigen::Vector2d& principal_point_i,
            const Eigen::Vector2d& principal_point_j) {
        return new ceres::AutoDiffCostFunction<FetzerFocalLengthCostFunctor, 2,
                                               1, 1>(
                new FetzerFocalLengthCostFunctor(j_F_i, principal_point_i,
                                                 principal_point_j));
    }
...
public:
    ...
    static ceres::CostFunction* Create(const Eigen::Matrix3d& j_F_i,
        ...
        return new ceres::AutoDiffCostFunction<
                FetzerFocalLengthSameCameraCostFunctor, 2, 1>(
                new FetzerFocalLengthSameCameraCostFunctor(j_F_i,
                                                           principal_point));
    }
