...
public:
    ...
    //                                 outlive the estimator and be the same
    //                                 data passed to RANSAC).
    // @param points2                  Full second-image correspondences.
    // @param sampson_max_residual     Squared max Sampson error used when
    // scoring
    //                                 completion candidates (typically the
    //                                 RANSAC max_error squared).
    ...
    // @param max_plane_parallax_trials Off-plane pairs sampled during
    // completion.
    ...
    // non-minimal >= 8-point). For each hypothesis whose sample is
    // H-degenerate, the plane-corrupted model is replaced by a
    // plane-and-parallax completion, or dropped if no usable off-plane parallax
    // exists.
    void Estimate(const std::vector<X_t>& sample_points1,
                  const std::vector<Y_t>& sample_points2,
                  std::vector<M_t>* models) const;

    // Legacy RANSAC/LORANSAC interface adapter in this fork: the optim layer
    // invokes Estimate(X, Y) and expects the models as the return value.
    ...
    // Estimate(), then polishes by minimizing the Sampson error, unless the
    // refit is plane-degenerate: the Sampson cost is then nearly flat along the
...
struct FundamentalMatrixDegensacOptions {
    ...
    // Maximum pixel error for a correspondence to count as lying on the
    // dominant plane, used for the degeneracy test and the homography refit.
    // Squared internally. Deliberately looser than the inlier threshold so
    // noisy plane points are still recognized as coplanar. If <= 0, derived as
    ...
    // high-parallax points constrain the epipole (near-plane, low-parallax
    // points are ignored). If <= 0, derived as 10 * ransac.max_error (100x
    // squared).
    double off_plane_min_error = -1;

    // Fraction of a sample that must be consistent with a single plane
    // homography for the sample to be considered H-degenerate. The default
    // corresponds to the paper's "at least 5 of 7" criterion for a minimal
    // sample.
    ...
    // anyway. The value 25 sits at the knee of a runtime/accuracy sweep: fewer
    // trials erode success on the hardest >=98%-plane scenes (t=5: -3pts) with
    // no runtime win (the inner pair loop is not the bottleneck), while more
    // trials (or dynamic termination) yield no accuracy gain at added cost.
