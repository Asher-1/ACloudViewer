// Upstream port (COLMAP d3ccaf35 estimators/generalized_pose_test.cc). Fork
// adaptation: accessor-style scene APIs (XY()/XYZ()/Params()/Width()/
// Height()), fork camera-model enums, and Frame::ImageIds() yielding image
...
#include "geometry/sim3_matchers.h"
#include "math/math.h"
  ...
  for (const image_t image_id : reconstruction.RegImageIds()) {
    ...
    for (size_t point2D_idx = 0; point2D_idx < image.NumPoints2D();
      ...
      if (point2D.HasPoint3D()) {
        ...
        problem.point3D_ids.push_back(point2D.Point3DId());
...
TEST(EstimateScaledGeneralizedAbsolutePose, SingleCenterConsensusFails) {
  ...
  const Camera camera = Camera::CreateFromModelId(
      /*camera_id=*/1, PinholeCameraModel::model_id, 500, 640, 480);
...
TEST(RefineScaledGeneralizedAbsolutePose, StaleInliersAreIgnored) {
  ...
  // Inlier observations that do not project in front of their camera
  // contribute a zero residual and must not bias the refinement: the
  // remaining observations are exact, so it must still converge to the ground
  // truth.
...
GeneralizedRelativePoseProblem BuildGeneralizedRelativePoseProblem(
  ...
  // Fork adaptation: Frame::ImageIds() yields image identifiers directly.
  FlatHashMap<point3D_t, std::vector<std::pair<const Image*, point2D_t>>>
      observations2;
  for (const image_t image_id2 : frame2.ImageIds()) {
    const auto& image = reconstruction.Image(image_id2);
    for (size_t point2D_idx = 0; point2D_idx < image.NumPoints2D();
         ++point2D_idx) {
      const auto& point2D = image.Point2D(point2D_idx);
      if (point2D.HasPoint3D()) {
        observations2[point2D.Point3DId()].emplace_back(&image, point2D_idx);
      }
    }
  ...
  for (const image_t image_id1 : frame1.ImageIds()) {
    ...
    for (size_t point2D_idx1 = 0; point2D_idx1 < image1.NumPoints2D();
      ...
      const auto observation_it = observations2.find(point2D1.Point3DId());
...
StructureLessAbsolutePoseProblem BuildStructureLessAbsolutePoseProblem(
  ...
  for (const image_t world_image_id : reconstruction.RegImageIds()) {
    ...
    for (size_t point2D_idx = 0; point2D_idx < world_image.NumPoints2D();
      ...
      if (point2D.HasPoint3D()) {
        world_obs[point2D.Point3DId()].emplace_back(&world_image, point2D_idx);
      }
    }
  }
  ...
  for (size_t point2D_idx = 0; point2D_idx < query_image.NumPoints2D();
    ...
    const auto world_obs_it = world_obs.find(query_point2D.Point3DId());
