...
struct LeastAbsoluteDeviationSolver {
    struct Options {
        ...
        // Absolute and relative solution thresholds, as suggested by Boyd et
        // al.
        ...
        // CHOLMOD's supernodal Cholesky may report "matrix not positive
        // definite". Default 0 disables regularization (no computational
        // overhead).
    ...
    // (e.g., the system is rank deficient or numerically not positive
    // definite), in which case Solve always returns false without producing NaN
    // output.
...
private:
    ...
    const std::shared_ptr<LeastAbsoluteDeviationLinearSolverImpl>
            linear_solver_;
    bool valid_ = false;
};
