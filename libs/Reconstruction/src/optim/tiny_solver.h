...
struct EuclideanManifold {
    ...
    // Identity Jacobian (row-major, N x N). Unused by the solver on the
    // Euclidean fast path, but provided so the manifold satisfies the full
    // concept.
...
public:
    ...
    static_assert(
            static_cast<int>(NUM_PARAMETERS) == Manifold::kAmbientSize,
    ...
    enum Status {
        ...
        // The user cost function returned false (failed to evaluate) at the
        // initial
    ...
    const Summary& Solve(const Function& function,
        ...
        // Bail out cleanly if the cost function cannot be evaluated at the
        // initial point; there is nothing meaningful the solver can do in that
        // case.
        ...
        for (summary_.iterations = 1;
            ...
            for (int i = 0; i < dx_.rows(); ++i) {
                jtj_regularized_(i, i) +=
                        u * (std::min)((std::max)(jtj_(i, i), min_diagonal),
                                       max_diagonal);
            }

            linear_solver_.compute(jtj_regularized_);
            // If the factorization failed, the regularized normal equations
            // were not solvable. Treat this like a rejected step: shrink the
            // trust region (which increases the regularization) and try again.
            ...
            // If the cost function fails to evaluate at the trial point, reject
            // the step and shrink the trust region rather than acting on
            // garbage.
            ...
            const Scalar model_cost_change =
                    lm_step_.dot(2 * g_ - jtj_ * lm_step_);

            // rho is the ratio of the actual reduction in error to the
            // reduction in error that would be obtained if the problem was
            // linear. See [1] for details.
            ...
            if (rho > 0) {
                ...
                // The cost function already evaluated successfully at x_new_ ==
                // x above, so re-evaluating (now also for the Jacobian) is not
                // expected to fail; guard against it regardless.
                if (!Update(function, x)) {
                    summary_.status = COST_FUNCTION_FAILED;
                    break;
                }
                ...
                // Reject the update because either the normal equations failed
                // to solve or the local linear model was not good (rho < 0).
...
private:
    bool Update(const Function& function, const Parameters& x) {
        ...
        // parameters, then project it into the tangent space of the manifold at
        // x: J_tangent = J_ambient * d(Plus(x, delta))/d(delta) |_{delta = 0}.
        if constexpr (Manifold::kIsEuclidean) {
            // The tangent space equals the ambient space, so the cost function
            // writes its Jacobian straight into jacobian_ (no projection, no
            // copy).
            ...
            if (!function(x.data(), residuals_.data(),
                          jacobian_ambient_.data())) {
                return false;
            }
            residuals_ = -residuals_;
            ...
            Eigen::Matrix<Scalar, NUM_PARAMETERS, NUM_TANGENT, kOrder>
                    plus_jacobian;
    ...
    // Preallocate everything, including temporary storage needed for solving
    // the linear system. This allows reusing the intermediate storage across
    // solves.
    //
    // The (potentially over-aligned) fixed-size Eigen members are declared
    // first and the scalars last so the compiler does not insert padding
    // between an aligned matrix and a scalar
    // (clang-analyzer-optin.performance.Padding).
    Parameters x_new_;
    Tangent dx_, g_, jacobi_scaling_, lm_step_;
    Eigen::Matrix<Scalar, NUM_RESIDUALS, 1> residuals_, f_x_new_;
    // jacobian_ is the tangent-space Jacobian used to form the normal
    // equations. For a non-Euclidean manifold the cost function writes the
    // ambient Jacobian into jacobian_ambient_, which is then projected into
    // jacobian_. For the Euclidean identity manifold the cost function writes
    // jacobian_ directly, so jacobian_ambient_ is unused and sized to zero
    // columns.
    static constexpr int kAmbientJacobianCols =
            Manifold::kIsEuclidean ? 0 : static_cast<int>(NUM_PARAMETERS);
    Eigen::Matrix<Scalar, NUM_RESIDUALS, kAmbientJacobianCols>
            jacobian_ambient_;
