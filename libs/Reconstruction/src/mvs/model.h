...
struct Model {
    ...
    // For each image, determine the maximally overlapping images, sorted based
    // on the number of shared points subject to a minimum robust average
    ...
    const std::vector<std::vector<int>>& GetMaxOverlappingImagesFromPMVS()
            const;

    // Compute the robust minimum and maximum depths from the sparse point
    // cloud.
