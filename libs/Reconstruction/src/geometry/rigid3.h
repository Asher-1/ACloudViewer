...
public:
    ...
    inline Eigen::Matrix6d Adjoint() const {
        ...
        adjoint.block<3, 3>(3, 0) = adjoint.block<3, 3>(0, 0).colwise().cross(
                -translation());  // t x R
    ...
    inline Eigen::Matrix6d AdjointInverse() const {
        ...
        adjoint_inv.block<3, 3>(0, 0) =
                rotation().toRotationMatrix().transpose();
        adjoint_inv.block<3, 3>(0, 3).setZero();
        adjoint_inv.block<3, 3>(3, 0) = -adjoint_inv.block<3, 3>(0, 0) *
...
inline Rigid3d operator*(const Rigid3d& c_from_b, const Rigid3d& b_from_a) {
    ...
    c_from_a.translation() = c_from_b.translation() +
                             (c_from_b.rotation() * b_from_a.translation());
    return c_from_a;
}
