...
struct SyntheticDatasetOptions {
    ...
    // points (dense visibility). If > 0, observations are pruned to exactly
    // this many per point. Must be -1 or >= 2.
    int track_length = -1;
    ...
    enum class MatchConfig {
        ...
        CHAINED = 2,
        // Sparse matches with controllable sparsity, removing edges randomly
        // while
