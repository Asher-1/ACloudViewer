...
public:
    ...
    bool MatchAndExplain(
        ...
        if (lhs.NumRigs() != rhs_.NumRigs()) {
            *listener << " have different number of rigs: " << lhs.NumRigs()
                      << " vs " << rhs_.NumRigs();
            return false;
        }
        if (lhs.NumCameras() != rhs_.NumCameras()) {
            *listener << " have different number of cameras: "
                      << lhs.NumCameras() << " vs " << rhs_.NumCameras();
            return false;
        }
        ...
        if ((lhs_num_obs != 0 || rhs_num_obs != 0) &&
            std::max(std::abs(1 -
                              lhs_num_obs / static_cast<double>(rhs_num_obs)),
                     std::abs(1 -
                              rhs_num_obs / static_cast<double>(lhs_num_obs))) >
                    num_obs_tolerance_) {
            *listener << " have different number of observations: "
                      << lhs_num_obs << " vs " << rhs_num_obs;
            return false;
        }
...
testing::PolymorphicMatcher<ReconstructionNearMatcher<T>> ReconstructionNear(
        ...
        bool align = true) {
    return testing::MakePolymorphicMatcher(ReconstructionNearMatcher<T>(
            std::forward<T>(rhs), max_rotation_error_deg, max_proj_center_error,
            max_scale_error, num_obs_tolerance, align));
}
