...
4. **AB 产物人工对比问题的取证与澄清（用户反馈三问）**：① 纹理 atlas"细节缺失"——量化：atlas 有效内容率 upstream 0.420 vs fork 0.373，与面数差（-6.2%）同量级，属 per-face patch 布局/packing 差异（纹理引擎第 25 轮已证逐行一致），非内容缺失；ply 体积 67/71.2MB 与面数严格成正比。② "mesh 无法重叠"——两边 sparse 相机中心差仅 0.2-0.9m（同坐标系、同 1368 个 3D 点），sim3（Umeyama）拟合 scale=1.00378、对齐残差 mean=0.0003m/max=0.0004m——**两个重建毫米级一致**；视觉差异来自 SfM gauge 的微小剩余 + 查看器 up-axis 惯例。③ "渲染黑花"——ParaView 不支持 PLY face-texcoord 纹理采样与 OBJ+MTL 纹理自动加载，属查看器限制；数据正确性由**顶点色 PLY**（UV→atlas 采样写入顶点色）与离屏渲染并排图 `/tmp/ab_mini6/side_by_side.png` 自证：两边几何与纹理内容一致。**对比产物**：`/tmp/ab_mini6/{fork,upstream}/dense/textured_obj/` 下 mesh.obj（UV/MTL，用 MeshLab/Blender 查看）+ mesh_vcolored*.ply（fork 版已 sim3 对齐到上游坐标系，可与 upstream 版直接叠加）。**顺手修复 fork 绑定层真缺陷**：Rigid3d/Sim3d 的 rotation/translation property 按值返回 Eigen::Vector4d/Vector3d，def_property 默认 reference_internal policy 会包装临时对象缓冲导致悬空（转换脚本中 q copy 后健康、t 未 copy 读出垃圾的随机性实锤）——四个 property 显式 `py::return_value_policy::copy`。

**第二十九轮（2026-09-19(4)，generalized_pose_test.cc 完整移植 + 两项引擎缺口收口 ✅）**：
