...
// template<int i>	 __device__ float Conv(float *data)		{    return
// Conv<i-1>(data) + data[i]*d_kernel[i];} template<>		__device__ float
// Conv<0>(float *data)	{    return data[0] * d_kernel[0];
// }

//////////////////////////////////////////////////////////////
template <int FW>
__global__ void FilterH(float* d_result, int width) {
    ...
    for (int j = 0; j < CACHE_COUNT; ++j) {
        if (cache_index < CACHE_WIDTH) {
            int fetch_index =
                    src_index < index_min
                            ? index_min
                            : (src_index > index_max ? index_max : src_index);
...
////////////////////////////////////////////////////////////////////
template <int FW>
__global__ void FilterV(float* d_result, int width, int height) {
    const int HALF_WIDTH = FW >> 1;
...
// add some extra space to avoid bank conflict
    ...
    const int EXTRA = (TEMP == 2 || TEMP == 1 || TEMP == 0)
                              ? 2 - TEMP
                              : (TEMP == 15 ? 3 : 14 - TEMP);
#elif FILTERV_TILE_WIDTH == 4
    // make the stride 16 * n +/- 4
    const int EXTRA = (TEMP >= 0 && TEMP <= 4)
                              ? 4 - TEMP
                              : (TEMP > 12 ? 20 - TEMP : 12 - TEMP);
#else
#error
#endif
    const int CACHE_TRUE_WIDTH = CACHE_WIDTH + EXTRA;
    const int CACHE_COUNT =
            (CACHE_WIDTH + FILTERV_BLOCK_HEIGHT - 1) / FILTERV_BLOCK_HEIGHT;
    const int WRITE_COUNT = (FILTERV_TILE_HEIGHT + FILTERV_BLOCK_HEIGHT - 1) /
                            FILTERV_BLOCK_HEIGHT;
        ...
        for (int i = 0; i < CACHE_COUNT; ++i) {
            if (cache_col_start < CACHE_WIDTH - i * FILTERV_BLOCK_HEIGHT) {
                int fetch_index =
                        data_index < col
                                ? col
                                : (data_index > data_index_max ? data_index_max
                                                               : data_index);
                data[cache_index + i * FILTERV_BLOCK_HEIGHT] =
                        tex1Dfetch(texData, fetch_index);
...
#pragma unroll
    for (int i = 0; i < WRITE_COUNT; ++i, row += FILTERV_BLOCK_HEIGHT,
             index_start += FILTERV_BLOCK_HEIGHT) {
        if (row < height) {
...
template <int LOG_SCALE>
__global__ void UpsampleKernel(float* d_result, int width) {
...
void ProgramCU::SampleImageU(CuTexImage* dst, CuTexImage* src, int log_scale) {
    ...
    dim3 grid((width + FILTERH_TILE_WIDTH - 1) / FILTERH_TILE_WIDTH,
              height << log_scale);
...
template <int LOG_SCALE>
__global__ void DownsampleKernel(float* d_result,
                                 int src_width,
                                 int dst_width) {
...
void ProgramCU::SampleImageD(CuTexImage* dst, CuTexImage* src, int log_scale) {
    ...
    src->BindTexture(texData);
    dim3 grid((dst_width + FILTERH_TILE_WIDTH - 1) / FILTERH_TILE_WIDTH,
              dst->GetImgHeight());
    dim3 block(FILTERH_TILE_WIDTH);
    switch (log_scale) {
        case 1:
            DownsampleKernel<1><<<grid, block>>>((float*)dst->_cuData,
                                                 src_width, dst_width);
            break;
        case 2:
            DownsampleKernel<2><<<grid, block>>>((float*)dst->_cuData,
                                                 src_width, dst_width);
            break;
        case 3:
            DownsampleKernel<3><<<grid, block>>>((float*)dst->_cuData,
                                                 src_width, dst_width);
            break;
        default:
            DownsampleKernel<<<grid, block>>>((float*)dst->_cuData, src_width,
                                              dst_width, log_scale);
    }
}
...
void ProgramCU::ReduceToSingleChannel(CuTexImage* dst,
                                      CuTexImage* src,
                                      int convert_rgb) {
...
template <int FW>
void ProgramCU::FilterImage(CuTexImage* dst, CuTexImage* src, CuTexImage* buf) {
    int width = src->GetImgWidth(), height = src->GetImgHeight();
    ...
    dim3 gridv((width + FILTERV_TILE_WIDTH - 1) / FILTERV_TILE_WIDTH,
               (height + FILTERV_TILE_HEIGHT - 1) / FILTERV_TILE_HEIGHT);
...
// OpenGL version : 18ms
// CUDA version: 28 ms
void ProgramCU::FilterImage(CuTexImage* dst,
                            CuTexImage* src,
                            CuTexImage* buf,
                            float sigma) {
    ...
    cudaMemcpyToSymbol(d_kernel, filter_kernel, width * sizeof(float), 0,
                       cudaMemcpyHostToDevice);

    switch (width) {
        case 5:
            FilterImage<5>(dst, src, buf);
            break;
        case 7:
            FilterImage<7>(dst, src, buf);
            break;
        case 9:
            FilterImage<9>(dst, src, buf);
            break;
        case 11:
            FilterImage<11>(dst, src, buf);
            break;
        case 13:
            FilterImage<13>(dst, src, buf);
            break;
        case 15:
            FilterImage<15>(dst, src, buf);
            break;
        case 17:
            FilterImage<17>(dst, src, buf);
            break;
        case 19:
            FilterImage<19>(dst, src, buf);
            break;
        case 21:
            FilterImage<21>(dst, src, buf);
            break;
        case 23:
            FilterImage<23>(dst, src, buf);
            break;
        case 25:
            FilterImage<25>(dst, src, buf);
            break;
        case 27:
            FilterImage<27>(dst, src, buf);
            break;
        case 29:
            FilterImage<29>(dst, src, buf);
            break;
        case 31:
            FilterImage<31>(dst, src, buf);
            break;
        case 33:
            FilterImage<33>(dst, src, buf);
            break;
        default:
            break;
    }
...
void __global__ ComputeDOG_Kernel(float* d_dog,
                                  float2* d_got,
                                  int width,
                                  int height) {
...
void ProgramCU::ComputeDOG(CuTexImage* gus, CuTexImage* dog, CuTexImage* got) {
    ...
    dim3 grid((width + DOG_BLOCK_DIMX - 1) / DOG_BLOCK_DIMX,
              (height + DOG_BLOCK_DIMY - 1) / DOG_BLOCK_DIMY);
    dim3 block(DOG_BLOCK_DIMX, DOG_BLOCK_DIMY);
    gus->BindTexture(texC);
    (gus - 1)->BindTexture(texP);
    if (got->_cuData)
        ComputeDOG_Kernel<<<grid, block>>>(
                (float*)dog->_cuData, (float2*)got->_cuData, width, height);
    else
...
void __global__ ComputeKEY_Kernel(float4* d_key,
                                  int width,
                                  int colmax,
                                  int rowmax,
...
#endif
    {
        ...
        if (subpixel_localization) {
            ...
            float fxs = 0.25f *
                        (datan[1][2] + datap[1][0] - datan[1][0] - datap[1][2]);
            float fys = 0.25f *
                        (datan[2][1] + datap[0][1] - datan[0][1] - datap[2][1]);

            // need to solve dx, dy, ds;
            //  |-fx|     | fxx fxy fxs |   |dx|
            //  |-fy|  =  | fxy fyy fys | * |dy|
            //  |-fs|     | fxs fys fss |   |ds|
            float4 A0 = fxx > 0 ? make_float4(fxx, fxy, fxs, -fx)
                                : make_float4(-fxx, -fxy, -fxs, fx);
            float4 A1 = fxy > 0 ? make_float4(fxy, fyy, fys, -fy)
                                : make_float4(-fxy, -fyy, -fys, fy);
            float4 A2 = fxs > 0 ? make_float4(fxs, fys, fss, -fs)
            ...
            if (maxa >= 1e-10) {
                ...
                if (abs(A1.y) >= 1e-10) {
                    ...
                    if (abs(A2.z) >= 1e-10) {
                        ...
                        offset_test_passed =
                                fabs(data[1][1] +
                                     0.5f * (dx * fx + dy * fy + ds * fs)) >
                                        dog_threshold &&
                                fabs(ds) < 1.0f && fabs(dx) < 1.0f &&
                                fabs(dy) < 1.0f;
                    }
                }
            }
...
void ProgramCU::ComputeKEY(CuTexImage* dog,
                           CuTexImage* key,
                           float Tdog,
                           float Tedge) {
...
#ifdef KEY_OFFSET_ONE
    dim3 grid((width - 1 + KEY_BLOCK_DIMX - 1) / KEY_BLOCK_DIMX,
              (height - 1 + KEY_BLOCK_DIMY - 1) / KEY_BLOCK_DIMY);
#else
    dim3 grid((width + KEY_BLOCK_DIMX - 1) / KEY_BLOCK_DIMX,
              (height + KEY_BLOCK_DIMY - 1) / KEY_BLOCK_DIMY);
#endif
    ...
    ComputeKEY_Kernel<<<grid, block>>>((float4*)key->_cuData, width, width - 1,
                                       height - 1, Tdog1, Tdog, Tedge,
...
void __global__ ComputeOrientation_Kernel(float4* d_list,
    ...
    const float radius_per_ten_degrees =
            1.0 / 5.7295779513082320876798154814105;
...
#pragma unroll
        for (int i = 0; i < 36; ++i) {
            ...
            if (vote[i] > vote_threshold && vote[i] > pre && vote[i] > next) {
                float di =
                        0.5f * FDIV(next - pre, vote[i] + vote[i] - next - pre);
                float rot = i + di + 0.5f;
                float weight = vote[i];
                ///
                if (weight > max_vot[1]) {
        ...
        if (fr1 < 0) fr1 += 1.0f;
        unsigned short us1 =
                ocount == 0 ? 65535 : ((unsigned short)floorf(fr1 * 65535.0f));
...
void ProgramCU::ComputeOrientation(CuTexImage* list,
                                   CuTexImage* got,
                                   CuTexImage* key,
                                   float sigma,
                                   float sigma_step,
    ...
    const int block_width = len < ORIENTATION_COMPUTE_PER_BLOCK
                                    ? 16
                                    : ORIENTATION_COMPUTE_PER_BLOCK;
    dim3 grid((len + block_width - 1) / block_width);
    dim3 block(block_width);

    ComputeOrientation_Kernel<<<grid, block>>>(
            (float4*)list->_cuData, len, width, height, sigma, sigma_step,
            GlobalUtil::_OrientationGaussianFactor,
            GlobalUtil::_OrientationGaussianFactor *
                    GlobalUtil::_OrientationWindowFactor,
            GlobalUtil::_FixedOrientation ? 0 : GlobalUtil::_MaxOrientation,
            existing_keypoint, GlobalUtil::_SubpixelLocalization,
            GlobalUtil::_KeepExtremumSign);

    ProgramCU::CheckErrorCUDA("ComputeOrientation");
}

template <bool DYNAMIC_INDEXING>
void __global__ ComputeDescriptor_Kernel(
        float4* d_des, int num, int width, int height, float window_factor) {
    ...
    float s, c;
    __sincosf(key.w, &s, &c);
    float anglef = key.w > 3.14159265358979323846
                           ? key.w - (2.0 * 3.14159265358979323846)
                           : key.w;
...
template <bool DYNAMIC_INDEXING>
void __global__ ComputeDescriptorRECT_Kernel(
        float4* d_des, int num, int width, int height, float window_factor) {
    const float rpi = 4.0 / 3.14159265358979323846;
    int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
    int fidx = idx >> 4;
...
void ProgramCU::ComputeDescriptor(CuTexImage* list,
                                  CuTexImage* got,
                                  CuTexImage* dtex,
                                  int rect,
                                  int stream) {
    ...
    if (rect) {
        if (GlobalUtil::_UseDynamicIndexing)
            ComputeDescriptorRECT_Kernel<true><<<grid, block>>>(
                    (float4*)dtex->_cuData, num, width, height,
                    GlobalUtil::_DescriptorWindowFactor);
        else
            ComputeDescriptorRECT_Kernel<false><<<grid, block>>>(
                    (float4*)dtex->_cuData, num, width, height,
                    GlobalUtil::_DescriptorWindowFactor);

    } else {
        if (GlobalUtil::_UseDynamicIndexing)
            ComputeDescriptor_Kernel<true><<<grid, block>>>(
                    (float4*)dtex->_cuData, num, width, height,
                    GlobalUtil::_DescriptorWindowFactor);
        else
            ComputeDescriptor_Kernel<false><<<grid, block>>>(
                    (float4*)dtex->_cuData, num, width, height,
                    GlobalUtil::_DescriptorWindowFactor);
    }
    if (GlobalUtil::_NormalizedSIFT) {
        ...
        dim3 block(block_width);
        NormalizeDescriptor_Kernel<<<grid, block>>>((float4*)dtex->_cuData,
                                                    num);
    }
...
void __global__ ConvertKEY_Kernel(float4* d_result, int width, int height) {
    ...
    if (col < width && row < height) {
        ...
        float v =
                inside ? saturate(0.5 + 20 * tex1Dfetch(texData, index)) : 0.5;
        d_result[index] =
                is_key && inside
                        ? (keyv.x > 0 ? make_float4(1.0f, 0, 0, 1.0f)
                                      : make_float4(0.0f, 1.0f, 0.0f, 1.0f))
                        : make_float4(v, v, v, 1.0f);
    }
}
void ProgramCU::DisplayConvertKEY(CuTexImage* key,
                                  CuTexImage* dog,
                                  CuTexImage* out) {
    if (out->_cuData == NULL) return;
...
inline void CuTexImage::BindTexture2D(textureReference& texRef) {
    ...
    cudaBindTexture2D(0, &texRef, _cuData, &texRef.channelDesc, _imgWidth,
                      _imgHeight, _imgWidth * _numChannel * sizeof(float));
#else
    ...
    if (cudaGetDeviceCount(&count) != cudaSuccess || count <= 0) {
        ...
        if (cudaGetDeviceProperties(&deviceProp, 0) != cudaSuccess ||
            ...
            if (GlobalUtil::_verbose)
                fprintf(stdout,
                        "NOTE: changing maximum texture dimension to %d\n",
...
void __global__ MultiplyDescriptor_Kernel(int* d_result,
                                          int num1,
                                          int num2,
                                          int3* d_temp) {
...
void ProgramCU::MultiplyDescriptor(CuTexImage* des1,
                                   CuTexImage* des2,
                                   CuTexImage* texDot,
                                   CuTexImage* texCRT) {
    ...
    if (texCRT)
        texCRT->InitTexture(num2,
                            (num1 + MULT_BLOCK_DIMY - 1) / MULT_BLOCK_DIMY, 32);
...
void __global__ MultiplyDescriptorG_Kernel(int* d_result,
                                           int num1,
                                           int num2,
                                           int3* d_temp,
                                           Matrix33 H,
    ...
    for (int i = 0; i < MULT_BLOCK_DIMY; ++i) {
        if (idx1 + i < num1) {
            ...
            if (hdist < hdistmax) {
                ...
                ftx2[0] = F.mat[0][0] * loc2.x + F.mat[1][0] * loc2.y +
                          F.mat[2][0];
                ftx2[1] = F.mat[0][1] * loc2.x + F.mat[1][1] * loc2.y +
                          F.mat[2][1];
...
void ProgramCU::MultiplyDescriptorG(CuTexImage* des1,
                                    CuTexImage* des2,
                                    CuTexImage* loc1,
                                    CuTexImage* loc2,
                                    CuTexImage* texDot,
...
void __global__ RowMatch_Kernel(
        int* d_dot, int* d_result, int num2, float distmax, float ratiomax) {
    ...
    for (int i = 0; i < num2; i += ROWMATCH_BLOCK_WIDTH) {
        if (threadIdx.x + i < num2) {
            int v = d_dot[base_address + threadIdx.x +
                          i];  // tex1Dfetch(texDOT, base_address + threadIdx.x
                               // + i);
...
#pragma unroll
    for (int step = ROWMATCH_BLOCK_WIDTH / 2; step > 0; step /= 2) {
        if (threadIdx.x < step) {
            ...
            bool test = v2 > v1;
            dotnxt[threadIdx.x] = test ? max(v1, dotnxt[threadIdx.x + step])
                                       : max(dotnxt[threadIdx.x], v2);
            dotidx[threadIdx.x] =
                    test ? dotidx[threadIdx.x + step] : dotidx[threadIdx.x];
...
void ProgramCU::GetRowMatch(CuTexImage* texDot,
                            CuTexImage* texMatch,
                            float distmax,
                            float ratiomax) {
...
void __global__ ColMatch_Kernel(int3* d_crt,
                                int* d_result,
                                int height,
                                int num2,
                                float distmax,
...
void ProgramCU::GetColMatch(CuTexImage* texCRT,
                            CuTexImage* texMatch,
                            float distmax,
                            float ratiomax) {
