...
__global__ void vector_max_kernel(const float* x,
                                  int len,
                                  int blen,
                                  float* result) {
...
__global__ void vector_norm_kernel(const float* x,
                                   int len,
                                   int blen,
                                   float* result) {
    __shared__ float value[256];
...
__global__ void vector_sum_kernel(const float* x,
                                  int len,
                                  int blen,
                                  float* result) {
    __shared__ float value[256];
...
float ProgramCU::ComputeVectorSum(CuTexImage& vector,
                                  CuTexImage& buf,
                                  int skip) {
    const unsigned int nblock = REDUCTION_NBLOCK;
    unsigned int bsize = 256;
...
__global__ void vector_dotproduct_kernel(
        const float* a, const float* b, int len, int blen, float* result) {
    __shared__ float value[256];
    int bstart = blen * blockIdx.x;
    int start = bstart + threadIdx.x;
    int end = min(len, bstart + blen);
...
double ProgramCU::ComputeVectorDot(CuTexImage& vector1,
                                   CuTexImage& vector2,
                                   CuTexImage& buf) {
    const unsigned int nblock = REDUCTION_NBLOCK;
    unsigned int bsize = 256;
...
__global__ void vector_weighted_norm_kernel(
        const float* vec, const float* w, int len, int blen, float* result) {
    __shared__ float value[256];
    int bstart = blen * blockIdx.x;
    int start = bstart + threadIdx.x;
    int end = min(len, bstart + blen);
...
double ProgramCU::ComputeVectorNormW(CuTexImage& vector,
                                     CuTexImage& weight,
                                     CuTexImage& buf) {
    if (weight.IsValid()) {
        const unsigned int nblock = REDUCTION_NBLOCK;
        ...
        vector_weighted_norm_kernel<<<grid, block>>>(
                vector.data(), weight.data(), len, blen, buf.data());
...
// return a * x + y
__global__ void saxpy_kernel(const float a,
                             const float* x,
                             const float* y,
                             float* result,
                             unsigned int len) {
...
__global__ void saxpy_kernel_large(const float a,
                                   const float* x,
                                   const float* y,
                                   float* result,
                                   unsigned int len,
                                   unsigned int rowsz) {
    unsigned int idx =
            threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * rowsz;
    if (idx < len) result[idx] = a * x[idx] + y[idx];
}

void ProgramCU::ComputeSAXPY(float a,
                             CuTexImage& texX,
                             CuTexImage& texY,
                             CuTexImage& result) {
    ...
    if (nblock > MAX_BLOCKLEN) {
        ...
        dim3 grid(nblock), block(bsize);
        saxpy_kernel<<<grid, block>>>(a, texX.data(), texY.data(),
                                      result.data(), len);
    }
    ProgramCU::CheckErrorCUDA("ComputeSAXPY");
}

__global__ void sxypz_kernel_large(float a,
                                   const float* x,
                                   const float* y,
                                   const float* z,
                                   float* result,
...
void ProgramCU::ComputeSXYPZ(float a,
                             CuTexImage& texX,
                             CuTexImage& texY,
                             CuTexImage& texZ,
                             CuTexImage& result) {
...
__global__ void vxy_kernel(const float* x,
                           float* y,
                           float* result,
                           unsigned int len) {
    unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void vxy_kernel_large(const float* x,
                                 float* y,
                                 float* result,
                                 unsigned int len,
                                 unsigned int rowsz) {
    unsigned int idx =
            blockIdx.x * blockDim.x + threadIdx.x + rowsz * blockIdx.y;
    if (idx < len) result[idx] = x[idx] * y[idx];
}

void ProgramCU::ComputeVXY(CuTexImage& texX,
                           CuTexImage& texY,
                           CuTexImage& result,
                           unsigned int part,
                           unsigned int skip) {
    ...
    if (nblock > MAX_BLOCKLEN) {
        ...
        GetBlockConfiguration(nblock, bw, bh);
        dim3 grid(bw, bh), block(bsize);
        vxy_kernel_large<<<grid, block>>>(
                texX.data() + skip, texY.data() + skip, result.data() + skip,
                len, bsize * bw);
    } else {
        dim3 grid(nblock), block(bsize);
...
__global__ void sqrt_kernel_large(float* x,
                                  unsigned int len,
                                  unsigned int rowsz) {
    unsigned int idx =
            threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * rowsz;
...
__global__ void rsqrt_kernel_large(float* x,
                                   unsigned int len,
                                   unsigned int rowsz) {
    unsigned int idx =
            threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * rowsz;
...
__global__ void sax_kernel(const float a,
                           const float* x,
                           float* result,
                           unsigned int len) {
    unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void sax_kernel_large(const float a,
                                 const float* x,
                                 float* result,
                                 unsigned int len,
                                 unsigned int rowsz) {
    unsigned int idx =
            blockIdx.x * blockDim.x + threadIdx.x + blockIdx.y * rowsz;
    if (idx < len) result[idx] = a * x[idx];
}
...
__global__ void jacobian_frt_kernel(
    ...
    if (pd) {
        ...
        if (scaling == false) {
            if (jc) {
                ...
                jc[jc_pos] =
                        make_float4(p0_p2 * jfc, f_p2_x, 0, -f_p2_x * p0_p2);
                jc[jc_pos + 1] = make_float4(-f_p2_x * p0_p2 * y0,
                                             f_p2_x * (z0 + x0 * p0_p2),
                                             -f_p2_x * y0, ft_x_pn * p0_p2);
                jc[jc_pos + 2] =
                        make_float4(p1_p2 * jfc, 0, f_p2_y, -f_p2 * p1_p2);
                jc[jc_pos + 3] = make_float4(-f_p2_y * (z0 + y0 * p1_p2),
                                             f_p2_y * x0 * p1_p2, f_p2_y * x0,
                                             ft_x_pn * p1_p2);
                JFRT_SET_JC_END
            }
            ////////////////////
            ...
            jp[(tidx << 1) + 1] =
                    make_float4(f_p2_y * (r[3] - r[6] * p1_p2),
            ...
            if (jc) {
                ...
                jc[jc_pos] = make_float4(p0_p2 * jfc * sc1.x, f_p2_x * sc1.y, 0,
                                         -f_p2_x * p0_p2 * sc1.w);
                jc[jc_pos + 2] =
                        make_float4(p1_p2 * jfc * sc1.x, 0, f_p2_y * sc1.z,
                                    -f_p2_y * p1_p2 * sc1.w);

                float4 sc2 = tex1Dfetch(tex_jacobian_sj, proj.x + 1);
                jc[jc_pos + 1] = make_float4(-sc2.x * f_p2_x * p0_p2 * y0,
                                             sc2.y * f_p2_x * (z0 + x0 * p0_p2),
                                             -sc2.z * f_p2_x * y0,
                                             ft_x_pn * p0_p2 * sc2.w);
                jc[jc_pos + 3] = make_float4(
                        -sc2.x * f_p2_y * (z0 + y0 * p1_p2),
                        sc2.y * f_p2_y * x0 * p1_p2, sc2.z * f_p2_y * x0,
                        ft_x_pn * p1_p2 * sc2.w);
                JFRT_SET_JC_END
            }
            ...
            jp[(tidx << 1)] =
                    make_float4(sc3.x * f_p2_x * (r[0] - r[6] * p0_p2),
        ...
        if (scaling == false) {
            if (jc) {
                ...
                jc[jc_pos + 1] = make_float4(-f_p2 * p0_p2 * y0,
                                             ...
                                             -f_p2 * y0, -ms.x * msn);
                jc[jc_pos + 2] =
                        make_float4(p1_p2 * jic, 0, f_p2, -f_p2 * p1_p2);
                jc[jc_pos + 3] =
                        make_float4(-f_p2 * (z0 + y0 * p1_p2),
            ...
            if (jc) {
                ...
                jc[jc_pos] = make_float4(p0_p2 * jic * sc1.x, f_p2 * sc1.y, 0,
                                         -f_p2 * p0_p2 * sc1.w);
                jc[jc_pos + 2] =
                        make_float4(p1_p2 * jic * sc1.x, 0, f_p2 * sc1.z,
                                    -f_p2 * p1_p2 * sc1.w);
                ...
                float msn = (ms.x * ms.x + ms.y * ms.y) * jic;
                jc[jc_pos + 1] =
                        make_float4(-sc2.x * f_p2 * p0_p2 * y0,
            ...
            float4 sc3 = tex1Dfetch(tex_jacobian_sj, proj.y + ptx);
            jp[(tidx << 1)] =
                    make_float4(sc3.x * f_p2 * (r[0] - r[6] * p0_p2),
...
/////////////////////////////////
void ProgramCU::ComputeJacobian(CuTexImage& camera,
                                CuTexImage& point,
                                CuTexImage& jc,
                                CuTexImage& jp,
                                CuTexImage& proj_map,
    ...
    if (radial_distortion == -1) {
        ...
        if (sj.IsValid()) {
            ...
            if (shuffle)
                ...
                jacobian_frt_kernel<false, false, false, false>
                        <<<grid, block>>>(
                                (float4*)jc.data(), (float4*)jp.data(), len,
                                camera.GetImgWidth() * 2, bw * bsize, jfc);
        }
...
void ProgramCU::UncompressCamera(int ncam,
                                 CuTexImage& camera,
                                 CuTexImage& result) {
...
__global__ void compress_frt_kernel(int ncam, float4* zcam) {
    ...
    if (a >= 1.0) {
        ...
        zcam[write_index + 1] = make_float4(
                b * (r2.w - r2.y), b * (r1.z - r2.z), b * (r1.w - r1.y), r3.y);
    }
}

void ProgramCU::CompressCamera(int ncam,
                               CuTexImage& camera0,
                               CuTexImage& result) {
    unsigned int len = ncam;
    unsigned int bsize = 64;
...
__device__ inline void uncompress_rodrigues_rotation(float rx,
                                                     float ry,
                                                     float rz,
                                                     float* r) {
    float rx_rx = rx * rx;
...
__global__ void update_camera_kernel(int ncam, float4* newcam) {
    ...
    int index1 = tidx << 1;
    {
        ...
        float4 c2 = make_float4(max(c1.x + d1.x, 1e-10f), c1.y + d1.y,
                                c1.z + d1.z, c1.w + d1.w);
        newcam[index0] = c2;
    }
    {
        ...
        newcam[index0 + 3] =
                make_float4(dr[6] * r[2] + dr[7] * r[5] + dr[8] * r[8],
                            r3.y + dd.w, r3.z, r3.w);
    }
}

void ProgramCU::UpdateCameraPoint(int ncam,
                                  CuTexImage& camera,
                                  CuTexImage& point,
                                  CuTexImage& delta,
...
void ProgramCU::ComputeProjection(CuTexImage& camera,
                                  CuTexImage& point,
                                  CuTexImage& meas,
                                  CuTexImage& proj_map,
                                  CuTexImage& proj,
    ...
    if (radial == -1)
        projection_frt_kernel<true, false>
                <<<grid, block>>>(len, bw * bsize, (float2*)proj.data());
    else if (radial)
        projection_frt_kernel<false, true>
                <<<grid, block>>>(len, bw * bsize, (float2*)proj.data());
    else
        projection_frt_kernel<false, false>
                <<<grid, block>>>(len, bw * bsize, (float2*)proj.data());
...
void ProgramCU::ComputeProjectionX(CuTexImage& camera,
                                   CuTexImage& point,
                                   CuTexImage& meas,
                                   CuTexImage& proj_map,
                                   CuTexImage& proj,
    ...
    meas.BindTexture(tex_projection_mea);
    if (radial == -1)
        projectionx_frt_kernel<true, false>
                <<<grid, block>>>(len, bw * bsize, (float2*)proj.data());
    else if (radial)
        projectionx_frt_kernel<false, true>
                <<<grid, block>>>(len, bw * bsize, (float2*)proj.data());
    else
        projectionx_frt_kernel<false, false>
                <<<grid, block>>>(len, bw * bsize, (float2*)proj.data());
    CheckErrorCUDA("ComputeProjection");
}
...
__global__ void jte_cam_kernel(int num, float* jc, float* jte) {
    ...
    // write back
    if (offset < 8)
        jte[(cam << 3) + offset] = (result + value[threadIdx.x + 8]);
}
...
void ProgramCU::ComputeJtE(CuTexImage& pe,
                           CuTexImage& jc,
                           CuTexImage& cmap,
                           CuTexImage& cmlist,
                           CuTexImage& jp,
    ...
    if (mode == 2) {
        ...
        jte_cam_vec32_kernel<bheight, true>
                <<<grid1, block1>>>(ncam, jc.data(), jte.data());
    else
        jte_cam_vec32_kernel<bheight, false>
                <<<grid1, block1>>>(ncam, jc.data(), jte.data());
...
__global__ void jtjd_cam_vec32_kernel(
...
template <int TEXN>
__global__ void jtjd_point_kernel(int num,
                                  int rowsz,
                                  float4* jtjd,
                                  float4* jtjdi) {
    ////////////////////////////
...
void ProgramCU::ComputeDiagonal(CuTexImage& jc,
                                CuTexImage& cmap,
                                CuTexImage& jp,
                                CuTexImage& pmap,
                                CuTexImage& cmlist,
    ...
    if (jc_transpose) {
        if (radial)
            jtjd_cam_vec32_kernel<8, bheight, true>
                    <<<grid1x, block1x>>>(ncam, add_existing_diagc, jc.data(),
                                          jtjd.data(), jtjdi.data());
        else
            jtjd_cam_vec32_kernel<7, bheight, true>
                    <<<grid1x, block1x>>>(ncam, add_existing_diagc, jc.data(),
                                          jtjd.data(), jtjdi.data());
    } else {
        cmlist.BindTexture(tex_jtjd_cmlist);
        if (radial)
            jtjd_cam_vec32_kernel<8, bheight, false>
                    <<<grid1x, block1x>>>(ncam, add_existing_diagc, jc.data(),
                                          jtjd.data(), jtjdi.data());
        else
            jtjd_cam_vec32_kernel<7, bheight, false>
                    <<<grid1x, block1x>>>(ncam, add_existing_diagc, jc.data(),
                                          jtjd.data(), jtjdi.data());
    }
    ...
    if (jp.GetDataSize() > MAX_TEXSIZE) {
        ...
        jtjd_point_kernel<2><<<grid2, block2>>>(
                len2, (bw * bsize2), ((float4*)jtjd.data()) + 2 * ncam,
                ((float4*)jtjdi.data()) + 2 * ncam);
    } else {
        jp.BindTexture(tex_jtjd_jp);
        jtjd_point_kernel<1><<<grid2, block2>>>(
                len2, (bw * bsize2), ((float4*)jtjd.data()) + 2 * ncam,
                ((float4*)jtjdi.data()) + 2 * ncam);
    }
...
__global__ void jtjd_cam_q_kernel(int num, int rowsz, float* qw, float4* diag) {
    ...
    if (SJ) {
        ...
        float4 dj =
                tid == 0 ? make_float4(ws, 0, 0, 0) : make_float4(0, 0, 0, ws);
        diag[index] = dj;
    }
}

void ProgramCU::ComputeDiagonalQ(CuTexImage& qlistw,
                                 CuTexImage& sj,
                                 CuTexImage& diag) {
    ...
    if (sj.IsValid()) {
        ...
        jtjd_cam_q_kernel<true><<<grid, block>>>(
                len, (bw * bsize), qlistw.data(), (float4*)diag.data());
    } else {
        jtjd_cam_q_kernel<false><<<grid, block>>>(
                len, (bw * bsize), qlistw.data(), (float4*)diag.data());
    }
...
template <int VN, int KH, bool JT>
__global__ void jtjd_cam_block_vec32_kernel(int num,
                                            float lambda1,
                                            float lambda2,
                                            float* jc,
                                            float* diag,
...
template <int TEXN>
__global__ void jtjd_point_block_kernel(int num,
                                        int rowsz,
                                        float lambda1,
                                        float lambda2,
                                        float4* diag,
...
__global__ void jtjd_cam_block_invert_kernel(int num, float4* blocks) {
    ...
    for (int i = 0; i < JTJD_BLOCK_CAM_INVERT_KWIDTH * VN;
        ...
        value[threadIdx.x + i] =
                ((float*)blocks)[block_read_pos + threadIdx.x + i];
    __syncthreads();
...
void ProgramCU::ComputeDiagonalBlock(float lambda,
                                     bool dampd,
                                     CuTexImage& jc,
                                     CuTexImage& cmap,
                                     CuTexImage& jp,
    ...
    if (mode == 2) {
        ...
        if (jc_transpose) {
            jtjd_cam_block_vec32_kernel<8, bheight, true><<<grid1x, block1x>>>(
                    ncam, lambda1, lambda2, jc.data(), diag.data(),
    ...
    if (mode == 1) {
        ...
        jp.BindTexture2(tex_jtjd_jp, tex_jtjd_jp2);
        jtjd_point_block_kernel<2>
                <<<grid2, block2>>>(len2, (bw * bsize2), lambda1, lambda2,
                                    ((float4*)diag.data()) + offsetd,
                                    ((float4*)blocks.data()) + offsetb);
    } else {
        jp.BindTexture(tex_jtjd_jp);
        jtjd_point_block_kernel<1>
                <<<grid2, block2>>>(len2, (bw * bsize2), lambda1, lambda2,
                                    ((float4*)diag.data()) + offsetd,
                                    ((float4*)blocks.data()) + offsetb);
    }
    ...
    if (mode != 2) {
        ...
        if (radial_distortion)
            jtjd_cam_block_invert_kernel<8>
                    <<<grid3, block3>>>(len3, (float4*)blocks.data());
        else
            jtjd_cam_block_invert_kernel<7>
                    <<<grid3, block3>>>(len3, (float4*)blocks.data());
...
__global__ void multiply_block_conditioner_kernel(
        int num, int rowsz, float* blocks, float* x, float* result) {
    ...
    if (index >= num) return;
    float* ac =
            mat + (threadIdx.x >> BBIT) * (BSZ * VSZ) + (threadIdx.x & BMASK);
...
void ProgramCU::MultiplyBlockConditioner(int ncam,
                                         int npoint,
                                         CuTexImage& blocks,
                                         CuTexImage& vector,
                                         CuTexImage& result,
    ...
    if (mode != 2) {
        ...
        if (radial)
            multiply_block_conditioner_kernel<bsize1, 3, 8>
                    <<<grid1, block1>>>(len1, (bw * bsize1), blocks.data(),
                                        vector.data(), result.data());
        else
            multiply_block_conditioner_kernel<bsize1, 3, 7>
                    <<<grid1, block1>>>(len1, (bw * bsize1), blocks.data(),
                                        vector.data(), result.data());
    ...
    if (mode != 1) {
        ...
        multiply_block_conditioner_kernel<bsize2, 2, 3><<<grid2, block2>>>(
                len2, (bw * bsize2), blocks.data() + offsetb,
                vector.data() + offsetd, result.data() + offsetd);
...
template <int TEXN>
__global__ void shuffle_camera_jacobian_kernel(int num,
                                               int bwidth,
                                               float4* jc) {
    ...
    if (TEXN == 1) {
        jc[index] =
                tex1Dfetch(tex_shuffle_jc, (fetch_idx << 2) + (index & 0x3));
    }
}

bool ProgramCU::ShuffleCameraJacobian(CuTexImage& jc,
                                      CuTexImage& map,
                                      CuTexImage& result) {
    ...
    if (szjc > 2 * MAX_TEXSIZE) {
        ...
        shuffle_camera_jacobian_kernel<2>
                <<<grid, block>>>(len, (bw * bsize), (float4*)result.data());
    } else {
        jc.BindTexture(tex_shuffle_jc);
        unsigned int bw, bh;
        GetBlockConfiguration(nblock, bw, bh);
        dim3 grid(bw, bh), block(bsize);
        shuffle_camera_jacobian_kernel<1>
                <<<grid, block>>>(len, (bw * bsize), (float4*)result.data());
    }
...
template <int TEXN>
__global__ void multiply_jx_kernel(int num,
                                   int bwidth,
                                   int offset,
                                   float* result) {
    int index = threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * bwidth;
    ...
    if (TEXN == 4 && (index >> 24) == 3) {
        ...
        result[index] = jc1.x * xc1.x + jc1.y * xc1.y + jc1.z * xc1.z +
                        ...
                        jc2.z * xc2.z + jc2.w * xc2.w + jp.x * xp.x +
                        jp.y * xp.y + jp.z * xp.z;
...
template <int TEXN>
__global__ void multiply_jpx_kernel(int num,
                                    int bwidth,
                                    int offset,
                                    float* result) {
    int index = threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * bwidth;
...
template <int KW>
__global__ void multiply_jx_notex2_kernel(int num,
                                          int bwidth,
                                          int offset,
                                          float* jcx,
                                          float* jpx,
    ...
    result[index] = jc[0] * xc1.x + jc[1] * xc1.y + jc[2] * xc1.z +
                    ...
                    jc[6] * xc2.z + jc[7] * xc2.w + jp[0] * xp.x +
                    jp[1] * xp.y + jp[2] * xp.z;
}

template <int KW>
__global__ void multiply_jpx_notex2_kernel(
        int num, int bwidth, int offset, float* jpx, float* result) {
    int bindex = blockIdx.x * blockDim.x + blockIdx.y * bwidth;
    int index = threadIdx.x + bindex;
...
template <int KW>
__global__ void multiply_jcx_notex2_kernel(int num,
                                           int bwidth,
                                           float* jcx,
                                           float* result) {
    int bindex = blockIdx.x * blockDim.x + blockIdx.y * bwidth;
...
void ProgramCU::ComputeJX(int point_offset,
                          CuTexImage& x,
                          CuTexImage& jc,
                          CuTexImage& jp,
                          CuTexImage& jmap,
    ...
    if (mode == 0) {
        ...
        if (TEX_TOOBIG4(szjc)) {
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bw, bh), block(bsize);
            multiply_jx_notex2_kernel<64>
                    <<<grid, block>>>(len, (bw * bsize), point_offset,
                                      jc.data(), jp.data(), result.data());
        } else if (szjc > 2 * MAX_TEXSIZE) {
            jp.BindTexture2(tex_mjx_jp, tex_mjx_jp2);
            jc.BindTexture4(tex_mjx_jc, tex_mjx_jc2, tex_mjx_jc3, tex_mjx_jc4);
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bw, bh), block(bsize);
            multiply_jx_kernel<4><<<grid, block>>>(len, (bw * bsize),
                                                   point_offset, result.data());
        } else if (szjc > MAX_TEXSIZE) {
            jp.BindTexture(tex_mjx_jp);
            jc.BindTexture2(tex_mjx_jc, tex_mjx_jc2);
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bw, bh), block(bsize);
            multiply_jx_kernel<2><<<grid, block>>>(len, (bw * bsize),
                                                   point_offset, result.data());
        } else {
            jp.BindTexture(tex_mjx_jp);
            ...
            multiply_jx_kernel<1><<<grid, block>>>(len, (bh * bsize),
                                                   point_offset, result.data());
        }
        ...
        if (TEX_TOOBIG4(szjc)) {
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bw, bh), block(bsize);
            multiply_jcx_notex2_kernel<64><<<grid, block>>>(
                    len, (bw * bsize), jc.data(), result.data());
        } else if (szjc > 2 * MAX_TEXSIZE) {
            jc.BindTexture4(tex_mjx_jc, tex_mjx_jc2, tex_mjx_jc3, tex_mjx_jc4);
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bw, bh), block(bsize);
            multiply_jcx_kernel<4>
                    <<<grid, block>>>(len, (bw * bsize), result.data());
        } else if (szjc > MAX_TEXSIZE) {
            jc.BindTexture2(tex_mjx_jc, tex_mjx_jc2);
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bw, bh), block(bsize);
            multiply_jcx_kernel<2>
                    <<<grid, block>>>(len, (bw * bsize), result.data());
        } else {
            jc.BindTexture(tex_mjx_jc);
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bh, bw), block(bsize);
            multiply_jcx_kernel<1>
                    <<<grid, block>>>(len, (bh * bsize), result.data());
        }
        ...
        if (szjp > MAX_TEXSIZE) {
            ...
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bw, bh), block(bsize);
            multiply_jpx_kernel<2><<<grid, block>>>(
                    len, (bw * bsize), point_offset, result.data());
        } else {
            jp.BindTexture(tex_mjx_jp);
            GetBlockConfiguration(nblock, bw, bh);
            dim3 grid(bh, bw), block(bsize);
            multiply_jpx_kernel<1><<<grid, block>>>(
                    len, (bh * bsize), point_offset, result.data());
        }
...
template <bool md, bool pd>
__device__ void jacobian_internal(int camera_pos,
                                  int pt_pos,
                                  int tidx,
                                  float* r,
                                  float jic,
...
template <bool md, bool pd>
__device__ void jacobian_camera_internal(int camera_pos,
                                         int pt_pos,
                                         int tidx,
                                         float* r,
                                         float jic,
...
template <bool pd>
__device__ void jacobian_point_internal(int camera_pos,
                                        int pt_pos,
                                        int tidx,
                                        float* r,
                                        float* jxp,
...
template <bool md, bool pd>
__global__ void multiply_jx_noj_kernel(
        int num, int bwidth, int offset, float jic, float2* result) {
    int index = threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * bwidth;
    if (index >= num) return;
    ...
    jacobian_internal<md, pd>(proj.x << 1, proj.y, index,
                              data + 9 * threadIdx.x, jic, jxc, jyc, jxp, jyp);

    /////////////////////////////////////
    result[index] = make_float2(
            jxc[0] * xc1.x + jxc[1] * xc1.y + jxc[2] * xc1.z + jxc[3] * xc1.w +
                    jxc[4] * xc2.x + jxc[5] * xc2.y + jxc[6] * xc2.z +
                    jxc[7] * xc2.w + jxp[0] * xp.x + jxp[1] * xp.y +
                    jxp[2] * xp.z,
            jyc[0] * xc1.x + jyc[1] * xc1.y + jyc[2] * xc1.z + jyc[3] * xc1.w +
                    jyc[4] * xc2.x + jyc[5] * xc2.y + jyc[6] * xc2.z +
                    jyc[7] * xc2.w + jyp[0] * xp.x + jyp[1] * xp.y +
                    jyp[2] * xp.z);
}

template <bool md, bool pd>
__global__ void multiply_jcx_noj_kernel(int num,
                                        int bwidth,
                                        float jic,
                                        float2* result) {
    int index = threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * bwidth;
...
template <bool pd>
__global__ void multiply_jpx_noj_kernel(int num,
                                        int bwidth,
                                        int offset,
                                        float2* result) {
    int index = threadIdx.x + blockIdx.x * blockDim.x + blockIdx.y * bwidth;
...
void ProgramCU::ComputeJX_(CuTexImage& x,
                           CuTexImage& jx,
                           CuTexImage& camera,
                           CuTexImage& point,
                           CuTexImage& meas,
...
__global__ void jte_cam_vec_noj_kernel(int num,
                                       int rowsz,
                                       float jic,
                                       float* jte) {
    ...
    valuec[threadIdx.x] =
            (valuec[threadIdx.x] + valuec[threadIdx.x + 32] +
...
void ProgramCU::ComputeJtE_(CuTexImage& e,
                            CuTexImage& jte,
                            CuTexImage& camera,
                            CuTexImage& point,
                            CuTexImage& meas,
    ...
    if (mode == 2) {
        ...
        jte_cam_vec_noj_kernel<true, false, bheight1>
                <<<grid, block>>>(ncam, bw * bheight1, jfc, jte.data());
    else if (radial_distortion)
        jte_cam_vec_noj_kernel<false, true, bheight1>
                <<<grid, block>>>(ncam, bw * bheight1, jfc, jte.data());
    else
        jte_cam_vec_noj_kernel<false, false, bheight1>
                <<<grid, block>>>(ncam, bw * bheight1, jfc, jte.data());
...
__global__ void jtjd_cam_block_noj_kernel(int num,
                                          int rowsz,
                                          float lambda1,
                                          float lambda2,
                                          float jic,
...
#define STORE_ROWS(k)                                                          \
    ...
    if (threadIdx.x < VN - 1)
        value[threadIdx.x * 8 + 8] = value[threadIdx.x + 1];
    if (threadIdx.x < VN - 2)
...
__global__ void jtjd_point_block_noj_kernel(int num,
                                            int rowsz,
                                            float lambda1,
                                            float lambda2,
                                            float4* diag,
...
void ProgramCU::ComputeDiagonalBlock_(float lambda,
                                      bool dampd,
                                      CuTexImage& camera,
                                      CuTexImage& point,
                                      CuTexImage& meas,
    ...
    if (mode == 2) {
        ...
        sj.BindTexture(tex_jacobian_sj);
        if (radial_distortion == -1)
            jtjd_cam_block_noj_kernel<bheight1, true, false, true>
                    <<<grid1, block1>>>(ncam, bw * bheight1, lambda1, lambda2,
                                        jfc, diag.data(), blocks.data(),
                                        add_existing_diagc);
        else if (radial_distortion)
            jtjd_cam_block_noj_kernel<bheight1, false, true, true>
                    <<<grid1, block1>>>(ncam, bw * bheight1, lambda1, lambda2,
                                        jfc, diag.data(), blocks.data(),
                                        add_existing_diagc);
        else
            jtjd_cam_block_noj_kernel<bheight1, false, false, true>
                    <<<grid1, block1>>>(ncam, bw * bheight1, lambda1, lambda2,
                                        jfc, diag.data(), blocks.data(),
                                        add_existing_diagc);
    } else {
        if (radial_distortion == -1)
            jtjd_cam_block_noj_kernel<bheight1, true, false, false>
                    <<<grid1, block1>>>(ncam, bw * bheight1, lambda1, lambda2,
                                        jfc, diag.data(), blocks.data(),
                                        add_existing_diagc);
        else if (radial_distortion)
            jtjd_cam_block_noj_kernel<bheight1, false, true, false>
                    <<<grid1, block1>>>(ncam, bw * bheight1, lambda1, lambda2,
                                        jfc, diag.data(), blocks.data(),
                                        add_existing_diagc);
        else
            jtjd_cam_block_noj_kernel<bheight1, false, false, false>
                    <<<grid1, block1>>>(ncam, bw * bheight1, lambda1, lambda2,
                                        jfc, diag.data(), blocks.data(),
                                        add_existing_diagc);
    }
    ...
    if (mode == 1) {
        ...
        if (sj.IsValid()) {
            sj.BindTexture(tex_jacobian_sj);
            if (radial_distortion && radial_distortion != -1)
                jtjd_point_block_noj_kernel<bsize2, true, true>
                        <<<grid2, block2>>>(
                                len2, (bw * bsize2), lambda1, lambda2,
                                ((float4*)diag.data()) + offsetd,
                                ((float4*)blocks.data()) + offsetb, offsetd);
            else
                jtjd_point_block_noj_kernel<bsize2, false, true>
                        <<<grid2, block2>>>(
                                len2, (bw * bsize2), lambda1, lambda2,
                                ((float4*)diag.data()) + offsetd,
                                ((float4*)blocks.data()) + offsetb, offsetd);
            ...
            if (radial_distortion && radial_distortion != -1)
                jtjd_point_block_noj_kernel<bsize2, true, false>
                        <<<grid2, block2>>>(
                                len2, (bw * bsize2), lambda1, lambda2,
                                ((float4*)diag.data()) + offsetd,
                                ((float4*)blocks.data()) + offsetb, 0);
            else
                jtjd_point_block_noj_kernel<bsize2, false, false>
                        <<<grid2, block2>>>(
                                len2, (bw * bsize2), lambda1, lambda2,
                                ((float4*)diag.data()) + offsetd,
                                ((float4*)blocks.data()) + offsetb, 0);
...
void ProgramCU::ComputeProjectionQ(CuTexImage& camera,
                                   CuTexImage& qmap,
                                   CuTexImage& qw,
                                   CuTexImage& proj,
                                   int offset) {
...
void ProgramCU::ComputeJQX(CuTexImage& x,
                           CuTexImage& qmap,
                           CuTexImage& wq,
                           CuTexImage& sj,
                           CuTexImage& jx,
    ...
    if (sj.IsValid()) {
        sj.BindTexture(tex_jacobian_sj);
        multiply_jqx_kernel<true><<<grid, block>>>(
                len, (bw * bsize), ((float2*)jx.data()) + offset);
    } else {
        multiply_jqx_kernel<false><<<grid, block>>>(
                len, (bw * bsize), ((float2*)jx.data()) + offset);
    }
}
...
void ProgramCU::ComputeJQtEC(CuTexImage& pe,
                             CuTexImage& qlist,
                             CuTexImage& wq,
                             CuTexImage& sj,
                             CuTexImage& jte) {
    ...
    if (sj.IsValid()) {
        ...
        jte_cam_q_kernel<false>
                <<<grid, block>>>(ncam, (bw * bsize), jte.data());
    }
}
