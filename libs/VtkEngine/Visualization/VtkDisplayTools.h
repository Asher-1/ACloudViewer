...
/// @file VtkDisplayTools.h
/// @brief Display tools bridging CloudViewer entities to VTK 3D and 2D
/// viewers.
...
public:  // inherit from ecvDisplayTools
     ...
     *  @param displayPos Output display coordinates
     */
    ...
    /** @param state true to show 2D viewer
     */
    ...
    /** @param param Property parameters to apply
     */
    ...
    /** @param viewID Output: entity view ID if update needed
...
public:
    ...
    inline virtual void updateCamera() override {
        // No-op: PCL's updateCamera() was deprecated. VTK rendering handles
        // camera updates automatically when the render window is refreshed.
    }
    ...
    inline virtual void getCameraPos(double* pos, int viewport = 0) override {
        const auto cam = m_visualizer3D->getCamera(viewport);
        pos[0] = cam.pos[0];
        pos[1] = cam.pos[1];
        pos[2] = cam.pos[2];
    }
...
public:
    ...
    // Note: These are VtkDisplayTools-specific methods, not part of
    // ecvDisplayTools interface They delegate to VtkVis for VTK-level
    // implementation
    ...
    /** @return true if camera orientation widget is visible
     */
