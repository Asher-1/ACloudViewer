    ...
    void GetTimeToNextTimer(timeval& tv) {
        ...
        if (!this->LocalToTimer.empty()) {
            ...
            for (auto& timer : this->LocalToTimer) {
                uint64_t delta =
                        (ctv.tv_sec - timer.second.lastFire.tv_sec) * 1000000 +
                        ctv.tv_usec - timer.second.lastFire.tv_usec;
        ...
        if (!this->LocalToTimer.empty()) {
            ...
            for (auto& timer : this->LocalToTimer) {
                int64_t delta =
                        ...
                        ctv.tv_usec - timer.second.lastFire.tv_usec;
                if (delta / 1000 >=
                    static_cast<int64_t>(timer.second.duration)) {
...
std::set<vtkXRenderWindowInteractor*>
        vtkXRenderWindowInteractorInternals::Instances;
...
void vtkXRenderWindowInteractor::TerminateApp() {
    ...
    // event loop.  This "wakes up" the event loop.  Otherwise, it might sit
    ...
    client.message_type =
            XInternAtom(this->DisplayId, "VTK_BreakXtLoop", False);
...
void vtkXRenderWindowInteractor::ProcessEvents() {
    ...
    rwiFileDescriptors.reserve(
            vtkXRenderWindowInteractorInternals::Instances.size());
    ...
    for (auto rwi = vtkXRenderWindowInteractorInternals::Instances.begin();
        ...
        if (XPending((*rwi)->DisplayId) == 0) {
            ...
            // If events are pending, dispatch them to the right
        ...
        // If current RenderWindowInteractor have been terminated, handle its
        // last event, then remove it from the Instance vector
        if ((*rwi)->Done) {
    ...
    if (wait && !done) {
        ...
        int maxFileDescriptor = *std::max_element(rwiFileDescriptors.begin(),
                                                  rwiFileDescriptors.end());
...
void vtkXRenderWindowInteractor::Enable() {
    ...
    XChangeProperty(this->DisplayId, this->WindowId, xdndAwareAtom, XA_ATOM, 32,
                    PropModeReplace, &XDND_VERSION, 1);
    this->XdndURIListAtom =
            XInternAtom(this->DisplayId, "text/uri-list", False);
    this->XdndTypeListAtom =
            XInternAtom(this->DisplayId, "XdndTypeList", False);
    this->XdndEnterAtom = XInternAtom(this->DisplayId, "XdndEnter", False);
    this->XdndPositionAtom =
            XInternAtom(this->DisplayId, "XdndPosition", False);
    this->XdndDropAtom = XInternAtom(this->DisplayId, "XdndDrop", False);
    this->XdndActionCopyAtom =
            XInternAtom(this->DisplayId, "XdndActionCopy", False);
    this->XdndStatusAtom = XInternAtom(this->DisplayId, "XdndStatus", False);
    this->XdndFinishedAtom =
            XInternAtom(this->DisplayId, "XdndFinished", False);
...
void vtkXRenderWindowInteractor::UpdateSizeNoXResize(int x, int y) {
    ...
    if ((x != this->Size[0]) || (y != this->Size[1])) {
        this->Size[0] = x;
        this->Size[1] = y;
        // static_cast<vtkXOpenGLRenderWindow*>(this->RenderWindow)->SetSizeNoXResize(x,
        // y);
        this->RenderWindow->SetSize(x, y);
    }
}
...
void vtkXRenderWindowInteractor::DispatchEvent(XEvent* event) {
    ...
    switch (event->type) {
        case Expose: {
            ...
            XEvent result;
            while (XCheckTypedWindowEvent(this->DisplayId, this->WindowId,
                                          Expose, &result)) {
        ...
        case ConfigureNotify: {
            ...
            if (width != this->Size[0] || height != this->Size[1]) {
                ...
                if (this->Enabled) {
                    ...
                    if (resizeSmaller) {
                        // Don't call Render when the window is resized to be
                        // larger:
                        //
                        // - if the window is resized to be larger, an Expose
                        // event will be triggered by the X server which will
                        // trigger a call to Render().
                        // - if the window is resized to be smaller, no Expose
                        // event will be triggered by the X server, as no new
                        // area become visible. only in this case, we need to
                        // explicitly call Render() in ConfigureNotify.
                        this->Render();
                    }
                }
            }
        ...
        case ButtonPress: {
            ...
            int ctrl = ((reinterpret_cast<XButtonEvent*>(event))->state &
                        ControlMask)
                               ? 1
                               : 0;
            int shift = ((reinterpret_cast<XButtonEvent*>(event))->state &
                         ShiftMask)
                                ? 1
                                : 0;
            int alt =
                    ((reinterpret_cast<XButtonEvent*>(event))->state & Mod1Mask)
                            ? 1
                            : 0;
            xp = (reinterpret_cast<XButtonEvent*>(event))->x;
            yp = (reinterpret_cast<XButtonEvent*>(event))->y;
            ...
            int eventTime = static_cast<int>(
                    reinterpret_cast<XButtonEvent*>(event)->time);
        ...
        case EnterNotify: {
            ...
            XSetInputFocus(this->DisplayId, this->WindowId, RevertToPointerRoot,
                           CurrentTime);
            if (this->Enabled) {
                ...
                this->SetEventInformationFlipY(e->x, e->y,
                                               ...
                                               (e->state & ShiftMask) != 0);
                this->SetAltKey(
                        ((reinterpret_cast<XButtonEvent*>(event))->state &
                         Mod1Mask)
                                ? 1
                                : 0);
        ...
        case KeyPress: {
            ...
            int alt =
                    ((reinterpret_cast<XButtonEvent*>(event))->state & Mod1Mask)
                            ...
                            : 0;
            vtkKeySym keySym =
                    XLookupKeysym(reinterpret_cast<XKeyEvent*>(event), 0);
            ...
            this->SetEventInformationFlipY(xp, yp, ctrl, shift, keyCode, 1,
        ...
        case MotionNotify: {
            ...
            // Note that even though the (x,y) location of the pointer is event
            // structure, we must call XQueryPointer for the hints (motion event
            // compression) to work properly.
        ...
        case SelectionNotify: {
            ...
            XGetWindowProperty(this->DisplayId, event->xselection.requestor,
                               event->xselection.property, 0, LONG_MAX, False,
                               event->xselection.target, &actualType,
                               &actualFormat, &itemCount, &bytesAfter,
                               reinterpret_cast<unsigned char**>(&data));

            // Conversion checks
            if ((event->xselection.target != AnyPropertyType &&
                 actualType != event->xselection.target) ||
                itemCount == 0) {
                return;
            }
            ...
            while (std::getline(uris, uri, '\n')) {
                if (vtksys::SystemTools::ParseURL(uri, protocol, unused0,
                                                  unused1, hostname, unused3,
                                                  filePath, true)) {
                    if (protocol == "file" &&
                        (hostname.empty() || hostname == "localhost")) {
                        // The uris can be crlf delimited, remove ending \r if
                        // any
                        if (filePath.back() == '\r') {
                            filePath.pop_back();
                        }
            ...
            XSendEvent(this->DisplayId, this->XdndSource, False, NoEventMask,
                       &reply);
        ...
        case ClientMessage: {
            if (event->xclient.message_type == this->XdndEnterAtom) {
                ...
                if (list) {
                    ...
                    XGetWindowProperty(
                            this->DisplayId, this->XdndSource,
                            this->XdndTypeListAtom, 0, LONG_MAX, False, XA_ATOM,
                            &actualType, &actualFormat, &count, &bytesAfter,
                            reinterpret_cast<unsigned char**>(&formats));
                } else {
                    count = 3;
                    formats =
                            reinterpret_cast<Atom*>(event->xclient.data.l + 2);
                }
            ...
            if (event->xclient.message_type == this->XdndPositionAtom) {
                ...
                if (this->XdndSource !=
                    static_cast<Window>(event->xclient.data.l[0])) {
                    vtkWarningMacro(
                            "Only one dnd action at a time is supported");
                    return;
                }
                ...
                Window child;
                XTranslateCoordinates(this->DisplayId, root, this->WindowId,
                                      xRoot, yRoot, &xWindow, &yWindow, &child);
                ...
                this->InvokeEvent(vtkCommand::UpdateDropLocationEvent,
                                  location);
                ...
                reply.xclient.format = 32;
                reply.xclient.data.l[0] = this->WindowId;
                reply.xclient.data.l[1] =
                        1;  // Always accept the dnd with no rectangle
                reply.xclient.data.l[2] = 0;  // Specify an empty rectangle
                reply.xclient.data.l[3] = 0;
                reply.xclient.data.l[4] = this->XdndActionCopyAtom;

                XSendEvent(this->DisplayId, this->XdndSource, False,
                           NoEventMask, &reply);
                XFlush(this->DisplayId);
                ...
                if (this->XdndFormatAtom) {
                    // Ask for a conversion of the selection. This will trigger
                    // a SelectionNotify event later.
                    Atom xdndSelectionAtom = XInternAtom(
                            this->DisplayId, "XdndSelection", False);
                    XConvertSelection(this->DisplayId, xdndSelectionAtom,
                                      this->XdndFormatAtom, xdndSelectionAtom,
                                      this->WindowId, CurrentTime);
            ...
            } else if (static_cast<Atom>(event->xclient.data.l[0]) ==
                       this->KillAtom) {
                this->ExitCallback();
            }
        } break;
...
void vtkXRenderWindowInteractor::GetMousePosition(int* x, int* y) {
    ...
    XQueryPointer(this->DisplayId, this->WindowId, &root, &child, &root_x,
                  &root_y, x, y, &keys);
...
//------------------------------------------------------------------------------
// void vtkXRenderWindowInteractor::Timer(XtPointer client_data, XtIntervalId*
// id)
// {
