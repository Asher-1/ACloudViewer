  ...
  vtkMultiProcessController* contr, vtkMultiBlockDataSet* data)
{
  ...
  std::vector<int> all_num_regions_local;
  all_num_regions_local.resize(tb);

  memset(all_num_regions_local.data(), 0, sizeof(int) * tb);
  ...
  std::vector<int> all_num_regions;
  all_num_regions.resize(tb);
  contr->AllReduce(all_num_regions_local.data(), all_num_regions.data(), tb, vtkCommunicator::SUM_OP);
