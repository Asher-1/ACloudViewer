...
/**
 * @file vtkUndoStack.h
 * @brief Stack of undo/redo sets for command history
 */
...
class QVTK_ENGINE_LIB_API vtkUndoStack : public vtkObject {
public:
    ...
    /**
     * Performs an Undo using the set on the top of the undo stack. The set is
     * poped from the undo stack and pushed at the top of the redo stack. Before
     * undo begins, it fires vtkCommand::StartEvent and when undo completes, it
     * fires vtkCommand::EndEvent. \returns the status of the operation.
     */
    virtual int Undo();

    /**
     * Performs a Redo using the set on the top of the redo stack. The set is
     * poped from the redo stack and pushed at the top of the undo stack. Before
     * redo begins, it fires vtkCommand::StartEvent and when redo completes, it
     * fires vtkCommand::EndEvent. \returns the status of the operation.
     */
    virtual int Redo();

    /**
     * Pop the undo stack. The UndoElement on the top of the undo stack is
     * popped from the undo stack and pushed on the redo stack. This is same as
     * Undo() except that the vtkUndoElement::Undo() is not invoked.
     */
    void PopUndoStack();

    /**
     * Pop the redo stack. The UndoElement on the top of the redo stack is
     * popped and then pushed on the undo stack. This is same as Redo() except
     * that vtkUndoElement::Redo() is not invoked.
     */
    void PopRedoStack();
