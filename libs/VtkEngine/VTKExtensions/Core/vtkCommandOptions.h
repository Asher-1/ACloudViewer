 ...
 * @brief Parses command-line arguments, supports XML config and process types
 */
class QVTK_ENGINE_LIB_API vtkCommandOptions : public vtkObject {
public:
...
protected:
     ...
     */
    typedef int (*CallbackType)(const char* argument,
                                const char* value,
                                void* call_data);
     ...
     */
    void AddBooleanArgument(const char* longarg,
                            const char* shortarg,
                            int* var,
                            const char* help,
                            int type = EVERYBODY);
    void AddDeprecatedArgument(const char* longarg,
                               const char* shortarg,
                               const char* help,
                               int type = EVERYBODY);
    void AddArgument(const char* longarg,
                     const char* shortarg,
                     int* var,
                     const char* help,
                     int type = EVERYBODY);
    ...
    void AddCallback(const char* longarg,
                     const char* shortarg,
    ...
    /**
     * This method is called when a deprecated argument is found. If it returns
     * 0, then the parsing will fail.
     */
...
private:
    ...
    static int DeprecatedArgumentHandler(const char* argument,
                                         const char* value,
                                         void* call_data);
};
