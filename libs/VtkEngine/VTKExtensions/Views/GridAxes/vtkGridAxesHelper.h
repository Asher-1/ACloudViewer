...
#include "vtkVectorCompat.h"  // VTK 9.x compat (ABI namespace + operators)

VTK_ABI_NAMESPACE_BEGIN
class vtkMatrix4x4;
class vtkViewport;

class QVTK_ENGINE_LIB_API vtkGridAxesHelper : public vtkObject {
public:
    ...
    /**
     * Returns which of the 3 coordinate axes for the 2 axes for this plane: 0
     * for X axis, 1, for Y axis, and 3 for Z axis. The two axes are specified
     * in order so that together with the face normal (which is point outwards
     * from the box defined by GridBounds), they form a right-handed coordinate
     * system.
     */
    ...
    ///@{
    /**
     * Set the transform matrix to use to transform the points. The matrix's
     * MTime will be used to determine if the transformed points needed to be
    ...
    /**
     * Call this method before accessing any of the attributes in viewport
     * space. This computes the location of the plane in the viewport space
     * using the specified viewport. This method should typically be called once
     * per render.
     */
