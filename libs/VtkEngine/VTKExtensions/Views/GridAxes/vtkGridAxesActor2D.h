...
class QVTK_ENGINE_LIB_API vtkGridAxesActor2D : public vtkProp3D {
public:
     ...
     * vtkGridAxesActor2D::LabelMasks to label multiple axes. Any request to
     * label an axes not present on the selected `Face` is ignored e.g. if
     * `Face` is `MIN_YZ`, then `LabelMask` set to `MIN_X` will have no effect.
     *
     * Default is 0xff.
     */
     ...
     * The positions are deep copied. Set to nullptr to not use custom tick
     * positions for the axis.
     *
     * Default is nullptr.
     */
...
private:
    ...
    std::function<double(double)> TickLabelFunction[3] = {nullptr, nullptr,
                                                          nullptr};
};
