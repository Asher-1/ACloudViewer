...
class QVTK_ENGINE_LIB_API vtkGridAxesPlaneActor2D : public vtkProp3D {
public:
    ...
    ///@{
    /**
     * Get/Set whether to generate the polydata for the plane's edges. Default
     * is true.
     */
    ...
    ///@{
    /**
     * Get/Set whether to generate tick markers for the tick positions. Default
     * is true.
     */
     ...
     * and hence need not be specified. Pass nullptr for data will clear the
     * ticks positions for that axis. Note: This creates a deep-copy of the
     * values in \c data and stores that.
     *
     * Default is an empty vector.
     */
    void SetTickPositions(int axis, vtkDoubleArray* data);
    const std::deque<double>& GetTickPositions(int axis) {
        return (axis >= 0 && axis < 3) ? this->TickPositions[axis]
                                       : this->EmptyVector;
    }
    ///@}
