...
/**
 * @file vtkScalarBarActorCustom.h
 * @brief Extended scalar bar actor with aspect ratio, tick marks, and range
 * labels.
 */
...
/**
 * @class vtkScalarBarActorCustom
 * @brief Scalar bar actor with custom aspect ratio, tick marks, and annotation
 * layout.
 */
class QVTK_ENGINE_LIB_API vtkScalarBarActorCustom : public vtkScalarBarActor {
public:
     ...
     * @param pos Position along the bar
     * @param diff Minimum spacing from other labels
     */
    virtual void AddValueLabelIfUnoccluded(double value,
                                           double pos,
                                           double diff);
...
protected:
     ...
     * virtual void PositionScalarBar(const int propSize[2], vtkViewport
     * *viewport);
     */
    ...
    /// @param viewport Viewport for sizing
    /// @return Index of the created label
    virtual int CreateLabel(double value,
                            int minDigits,
                            int targetWidth,
                            int targetHeight,
    ...
    /**
     * Space, in pixels, between the labels and the bar itself.  Currently set
     * in PositionTitle.
     */
    int LabelSpace;
