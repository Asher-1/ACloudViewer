...
void QRoundProgressBar::setRange(double min, double max) {
    ...
    if (m_max < m_min) qSwap(m_max, m_min);
    ...
    if (!m_gradientData.isEmpty()) m_rebuildBrush = true;

    update();
}

void QRoundProgressBar::setMinimum(double min) { setRange(min, m_max); }

void QRoundProgressBar::setMaximum(double max) { setRange(m_min, max); }
...
void QRoundProgressBar::setValue(int val) { setValue(double(val)); }
...
void QRoundProgressBar::paintEvent(QPaintEvent * /*event*/) {
    ...
    QImage buffer(outerRadius, outerRadius,
...
void QRoundProgressBar::drawBase(QPainter &p, const QRectF &baseRect) {
    switch (m_barStyle) {
        ...
        case StyleLine:
            ...
            p.drawEllipse(baseRect.adjusted(
...
void QRoundProgressBar::drawValue(QPainter &p,
    ...
    if (m_barStyle == StyleLine) {
        ...
        p.setBrush(Qt::NoBrush);
        p.drawArc(baseRect.adjusted(
                          m_outlinePenWidth / 2, m_outlinePenWidth / 2,
                          -m_outlinePenWidth / 2, -m_outlinePenWidth / 2),
                  m_nullPosition * 16, -arcLength * 16);
        return;
    }
...
void QRoundProgressBar::drawText(QPainter &p,
                                 ...
                                 double innerRadius,
                                 double value) {
    if (m_format.isEmpty()) return;

    // !!! to revise
    QFont f(font());
    f.setPixelSize(innerRadius *
                   qMax(0.05, (0.35 - (double)m_decimals * 0.08)));
    p.setFont(f);
...
QString QRoundProgressBar::valueToText(double value) const {
    ...
    if (m_updateFlags & UF_MAX)
        textToDraw.replace("%m",
                           QString::number(m_max - m_min + 1, 'f', m_decimals));
...
void QRoundProgressBar::valueFormatChanged() {
    ...
    if (m_format.contains("%v")) m_updateFlags |= UF_VALUE;

    if (m_format.contains("%p")) m_updateFlags |= UF_PERCENT;

    if (m_format.contains("%m")) m_updateFlags |= UF_MAX;
...
void QRoundProgressBar::rebuildDataBrushIfNeeded() {
    if (m_rebuildBrush) {
        ...
        for (int i = 0; i < m_gradientData.count(); i++) {
            dataBrush.setColorAt(1.0 - m_gradientData.at(i).first,
                                 m_gradientData.at(i).second);
        }
