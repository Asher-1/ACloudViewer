...
/**
 * @file Annotaion.cpp
 * @brief Implementation of annotation tools with box widgets.
 */
...
Annotation::Annotation(vtkPoints* points,
                       vector<int>& slice,
                       std::string type_) {
    double p1[3];
    double p2[3];
    ComputeOBB(points, slice, p1, p2);
    BoxLabel label(p1, p2, type_);

    setAnchorPoint(points, slice);
...
void Annotation::setAnchorPoint(vtkPoints* points,
                                const std::vector<int>& slice) {
    if (!points || points->GetNumberOfPoints() == 0 || slice.empty()) return;
...
#pragma omp parallel for
        ...
        for (int dataIndex = 0; dataIndex < num; ++dataIndex)
#endif
                {
                    ...
                    vtkIdType idx = static_cast<vtkIdType>(
                            slice[static_cast<std::size_t>(dataIndex)]);
...
void Annotation::ComputeOBB(vtkPoints* points,
    ...
    for (auto i : slice) {
        ...
        p1[0] = std::min(p1[0], pt[0]);
        p1[1] = std::min(p1[1], pt[1]);
        p1[2] = std::min(p1[2], pt[2]);

        p2[0] = std::max(p2[0], pt[0]);
        p2[1] = std::max(p2[1], pt[1]);
        p2[2] = std::max(p2[2], pt[2]);
    }
}
...
void Annotaions::preserve(size_t num) {
    ...
    if (m_labeledCloudIndex && m_capacity > 0) {
