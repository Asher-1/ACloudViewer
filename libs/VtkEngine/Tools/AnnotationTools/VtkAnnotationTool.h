...
/**
 * @file VtkAnnotationTool.h
 * @brief VTK-based annotation tool for bounding box and point cloud labeling.
 */

// LOCAL
#include <vtkSmartPointer.h>

#include "Tools/ColorTools/VtkPointCloudColorHandlerLUT.h"
#include "qVTK.h"
...
/**
 * @class VtkAnnotationTool
 * @brief Annotation tool implementing ecvGenericAnnotationTool for VTK point
 * clouds.
 */
class QVTK_ENGINE_LIB_API VtkAnnotationTool : public ecvGenericAnnotationTool {
    Q_OBJECT
public:
    explicit VtkAnnotationTool(
            AnnotationMode mode = AnnotationMode::BOUNDINGBOX);
    explicit VtkAnnotationTool(
            ecvGenericVisualizer3D* viewer,
            AnnotationMode mode = AnnotationMode::BOUNDINGBOX);
    virtual ~VtkAnnotationTool() override;
...
private:
     ...
     */
    vtkSmartPointer<vtkPolyData> m_baseCloud;

    /**
     * @brief state of each point to identity color or selectable
     */
    int* m_cloudLabel;
    AnnotationColorHandler m_colorHandler;
