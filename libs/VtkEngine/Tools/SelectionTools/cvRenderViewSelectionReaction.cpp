...
void cvRenderViewSelectionReaction::beginSelection() {
    ...
    switch (m_mode) {
        ...
        default:
            // CRITICAL: Disable VtkVis's PointPickingCallback to prevent
            // conflicts
...
void cvRenderViewSelectionReaction::endSelection() {
    ...
    // Clear hover state and visual highlight
    m_hoveredId = -1;
...
Visualization::VtkVis* cvRenderViewSelectionReaction::getVtkVis() const {
    if (!m_viewer) return nullptr;

    // Dynamic cast to VtkVis
    return dynamic_cast<Visualization::VtkVis*>(m_viewer);
}
