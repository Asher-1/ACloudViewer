...
void cvSelectionPropertiesWidget::updateAttributeCombo() {
    ...
    // thing) VtkVis.cpp uses "RGB" for point clouds and cc2sm.cpp uses "Colors"
    // for meshes
    QSet<QString> colorArrayVariants = {"RGB", "Colors", "rgba", "rgb", "RGBA"};

    // Add VTK "active" arrays first (Normals, TCoords, Scalars/Colors)
    // These are set in VtkVis draw functions via SetNormals(), SetTCoords(),
    // SetScalars() Field names in VtkVis.cpp:
    ...
    if (attrData) {
        ...
        // Set by updateShadingMode() and addTextureMesh() in VtkVis.cpp
        ...
        // Set by addTextureMesh() in VtkVis.cpp, named "TCoords0", "TCoords1"
        ...
        // Set by addTextureMesh() in VtkVis.cpp
        ...
        if (scalars) {
            ...
            // different code paths (VtkVis uses "RGB", cc2sm uses "Colors")
