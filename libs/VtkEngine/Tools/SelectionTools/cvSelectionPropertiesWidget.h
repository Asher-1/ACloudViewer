...
/**
 * @file cvSelectionPropertiesWidget.h
 * @brief Comprehensive selection properties and management widget (ParaView
 * Find Data panel style).
 */

// clang-format off
// Qt - must be included before qVTK.h for MOC to work correctly
 ...
 */
class QVTK_ENGINE_LIB_API cvSelectionPropertiesWidget : public QWidget,
...
public:
    ...
    /**
     * @param highlighter Selection highlighter for visual feedback
     */
    void setHighlighter(cvSelectionHighlighter* highlighter);
    /**
     * @param manager Selection manager for coordinating selections
     */
    void setSelectionManager(cvViewSelectionManager* manager);
    void syncUIWithHighlighter();

    /**
     * @param selectionData New selection data to display
     * @param polyData Optional polyData for spreadsheet (nullptr to use
     * default)
