...
/**
 * @file VtkTransformTool.cpp
 * @brief Implementation of transform tool for point clouds and meshes.
 */

#include "VtkTransformTool.h"
...
VtkTransformTool::VtkTransformTool(ecvGenericVisualizer3D* viewer)
...
VtkTransformTool::~VtkTransformTool() {}

void VtkTransformTool::setVisualizer(ecvGenericVisualizer3D* viewer) {
    if (viewer) {
        m_viewer = reinterpret_cast<VtkVis*>(viewer);
        if (!m_viewer) {
            CVLog::Warning("[VtkTransformTool::setVisualizer] viewer is Null!");
        }
    } else {
        CVLog::Warning("[VtkTransformTool::setVisualizer] viewer is Null!");
    }
...
bool VtkTransformTool::setInputData(ccHObject* entity, int viewport) {
...
void VtkTransformTool::addActors() {
...
void VtkTransformTool::showInteractor(bool state) {
    if (!m_boxWidgetTransformer) {
        CVLog::Warning(
                QString("[VtkTransformTool::showInteractor] boxWidget has not "
                        "been initialized!"));
        return;
    }
...
bool VtkTransformTool::start() {
    ...
    // &VtkTransformTool::onTransform);
    m_boxWidgetTransformer->On();

    return true;
}

void VtkTransformTool::stop() {
    if (m_boxWidgetTransformer) {
        reset();
...
void VtkTransformTool::clear() {}

void VtkTransformTool::setTranlationMode(TranslationMOde mode) {
    if (!m_boxWidgetTransformer) {
        CVLog::Warning(
                QString("[VtkTransformTool::setTranlationMode] boxWidget has "
...
void VtkTransformTool::setRotationMode(RotationMode mode) {
    if (!m_boxWidgetTransformer) {
        CVLog::Warning(
                QString("[VtkTransformTool::setRotationMode] boxWidget has "
                        "not been initialized!"));
        return;
    }
...
void VtkTransformTool::setScaleEnabled(bool state) {
    if (!m_boxWidgetTransformer) {
        CVLog::Warning(
                QString("[VtkTransformTool::setScaleEnabled] boxWidget has "
                        "not been initialized!"));
        return;
    }
...
void VtkTransformTool::setShearEnabled(bool state) {
    if (!m_boxWidgetTransformer) {
        CVLog::Warning(
                QString("[VtkTransformTool::setShearEnabled] boxWidget has "
                        "not been initialized!"));
        return;
    }
...
const ccGLMatrixd VtkTransformTool::getFinalTransformation() {
    if (!m_boxWidgetTransformer) {
        return ccGLMatrixd();
    }
...
void VtkTransformTool::getOutput(std::vector<ccHObject*>& out) {
    if (!m_boxWidgetTransformer) {
        return;
    }
    ...
    for (vtkActor* actor : m_modelActors) {
        ...
        if (baseEntity->isKindOf(CV_TYPES::POINT_CLOUD)) {
            result = Converters::Vtk2Cc::ConvertToPointCloud(dataObject);
        } else if (baseEntity->isKindOf(CV_TYPES::MESH)) {
            result = Converters::Vtk2Cc::ConvertToMesh(dataObject);
        } else if (baseEntity->isKindOf(CV_TYPES::POLY_LINE)) {
            result = Converters::Vtk2Cc::ConvertToPolyline(dataObject);
        } else {
