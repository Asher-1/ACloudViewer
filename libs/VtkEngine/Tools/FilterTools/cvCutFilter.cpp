...
void cvCutFilter::createUi() {
    ...
    connect(m_configUi->previewCheckBox, &QCheckBox::toggled, this,
            &cvCutFilter::onPreview);
}
    ...
    switch (m_cutType) {
        case Plane: {
            if (!m_planeWidget) {
                ...
                m_planeWidget->SetOutlineTranslation(
                ...
                connect(observer, SIGNAL(originChanged(double*)), this,
                        SLOT(onOriginChanged(double*)));
                connect(observer, SIGNAL(normalChanged(double*)), this,
                        SLOT(onNormalChanged(double*)));
        ...
        case Sphere: {
            if (!m_sphereWidget) {
                ...
                connect(observer, SIGNAL(centerChanged(double*)), this,
                        SLOT(onCenterChanged(double*)));
                connect(observer, SIGNAL(radiusChanged(double)), this,
        ...
        case Box: {
            if (!m_boxWidget) {
                ...
                VtkUtils::BoxWidgetObserver* observer =
                        new VtkUtils::BoxWidgetObserver(this);
                observer->attach(m_boxWidget);
                connect(observer, SIGNAL(planesChanged(vtkPlanes*)), this,
                        SLOT(onPlanesChanged(vtkPlanes*)));
            }
...
void cvCutFilter::setNormal(double normal[3]) {
    if (Utils::ArrayComparator<double>()(normal, m_normal)) return;
...
void cvCutFilter::setRadius(double radius) { m_radius = radius; }
...
cvCutFilter::CutType cvCutFilter::cutType() const { return m_cutType; }
...
void cvCutFilter::initFilter() {
    ...
    m_meshMode&& m_preview ? m_configUi->displayEffectCombo->setCurrentIndex(
...
void cvCutFilter::onOriginChanged(double* origin) {
    if (Utils::ArrayComparator<double>()(m_origin, origin)) return;
...
void cvCutFilter::onNormalChanged(double* normal) {
    if (Utils::ArrayComparator<double>()(m_normal, normal)) return;
...
void cvCutFilter::onCenterChanged(double* center) {
    if (Utils::ArrayComparator<double>()(m_center, center)) return;
...
void cvCutFilter::showContourLines(bool show) {
    ...
    for (int i = 0; i < numOfArrays; ++i) {
        ...
        scalarNameDataMap.insert(
                contourName,
                pointData->GetArray(contourName.toStdString().c_str()));
    }
    ...
    double delta =
            (range[1] - range[0]) / static_cast<double>(numberOfContours - 1);
    ...
    for (int i = 0; i < numberOfContours; i++) {
        ...
        if (i == 0)
            ...
            clippersLo[i]->SetInputConnection(
                    clippersHi[i - 1]->GetOutputPort(1));
        ...
        clippersHi[i]->Update();
        if (clippersHi[i]->GetOutput()->GetNumberOfCells() == 0) continue;
        ...
        appendFilledContours->AddInputConnection(
                clippersHi[i]->GetOutputPort());
    }
    ...
    if (!m_contourLinesActor) m_contourLinesActor = vtkActor::New();
...
void cvCutFilter::resetPlaneWidget() {
    ...
    if (!m_planeWidget) {
        CVLog::Error(
                QString("cvCutFilter::resetPlaneWidget: null plane widget."));
        return;
    }
...
void cvCutFilter::resetSphereWidget() {
    ...
    if (!m_sphereWidget) {
        CVLog::Error(
                QString("cvCutFilter::resetSphereWidget: null sphere widget."));
        return;
    }
...
void cvCutFilter::on_showPlaneCheckBox_toggled(bool checked) {
    if (m_planeWidget) m_planeWidget->SetDrawPlane(checked);
}
