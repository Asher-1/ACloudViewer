#pragma once
#include <sam3d_spconv/cumm/common/TensorViewNVRTC.h>
#include <sam3d_spconv/sam3d/spconv/Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK/mma/mma_ns_wmma/tensorop/MmaSync.h>
namespace sam3d_spconv {
namespace sam3d {
namespace spconv {
namespace Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK {
namespace mma {
namespace mma_ns_wmma {
using TensorViewNVRTC = sam3d_spconv::cumm::common::TensorViewNVRTC;
using InstMma = sam3d_spconv::sam3d::spconv::Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::mma::mma_ns_wmma::tensorop::MmaSync;
struct WarpMmaTuring {
  __forceinline__ __device__ void operator()(tv::array<tv::half_t, 64, 0>& D, tv::array<tv::half_t, 16, 0> const & A, tv::array<tv::half_t, 8, 0> const & B, tv::array<tv::half_t, 64, 0> const & C)   {

    InstMma mma;
    D = C;
    tv::array<tv::half_t, 4, 0> const *ptr_A = reinterpret_cast<tv::array<tv::half_t, 4, 0> const *>(&A);
    tv::array<tv::half_t, 2, 0> const *ptr_B = reinterpret_cast<tv::array<tv::half_t, 2, 0> const *>(&B);
    tv::array<tv::half_t, 4, 0> *ptr_D = reinterpret_cast<tv::array<tv::half_t, 4, 0> *>(&D);
    #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ < 800))
      TV_PRAGMA_UNROLL
      for (int n = 0; n < 4; ++n){
        TV_PRAGMA_UNROLL
        for (int m = 0; m < 4; ++m){
          int m_serpentine = ((n % 2) ? (4 - 1 - m) : m);
          mma(ptr_D[m_serpentine + n * 4],
              ptr_A[m_serpentine],
              ptr_B[n],
              ptr_D[m_serpentine + n * 4]);
        }
      }
    #elif (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
      TV_PRAGMA_UNROLL
      for (int m = 0; m < 4; ++m){
        TV_PRAGMA_UNROLL
        for (int n = 0; n < 4; ++n){
          int n_serpentine = ((m % 2) ? (4 - 1 - n) : n);
          mma(ptr_D[m + n_serpentine * 4],
              ptr_A[m],
              ptr_B[n_serpentine],
              ptr_D[m + n_serpentine * 4]);
        }
      }
    #endif
  }
};
} // namespace mma_ns_wmma
} // namespace mma
} // namespace Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK
} // namespace spconv
} // namespace sam3d
} // namespace sam3d_spconv