...
#include <sam3d_spconv/sam3d/spconv/Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK/inpiterb/gload/GlobalLoad.h>
#include <sam3d_spconv/sam3d/spconv/Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK/inpiterb/tmap/PitchLinearWarpRaked.h>
...
using GlobalLoad = sam3d_spconv::sam3d::spconv::Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::inpiterb::gload::GlobalLoad;
using ThreadMap = sam3d_spconv::sam3d::spconv::Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::inpiterb::tmap::PitchLinearWarpRaked;
...
using Params = sam3d_spconv::sam3d::spconv::cpAmpere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::iterb_p::WeightOptParams;
using Layout = sam3d_spconv::sam3d::spconv::cpAmpere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::lb::TensorGeneric;
struct WeightIteratorDP4A {
  ...
  __forceinline__ __device__ const tv::array<tv::half_t, 8> * get(int stride, int contig, int ss)  const {

    return reinterpret_cast<const tv::array<tv::half_t, 8> *>(pointer_ + contig * 32 * 16 / 8);
  }
  __forceinline__ __device__ void load_with_pointer_offset(tv::array<tv::half_t, 16, 0>& frag, int32_t pointer_offset)   {

    frag.clear();
    tv::array<tv::half_t, 8> *frag_ptr = reinterpret_cast<tv::array<tv::half_t, 8> *>(&frag);
    TV_PRAGMA_UNROLL
    for (int s = 0; s < 2; ++s){
      TV_PRAGMA_UNROLL
      for (int c = 0; c < 1; ++c){
        ...
        for (int ss = 0; ss < 1; ++ss){
          ...
          for (int v = 0; v < 1; ++v){
            int idx = s * 1 +
                c * 1 + ss * 1 + v;
            tv::array<tv::half_t, 8> const *access_ptr = get(s, c, ss) + v + pointer_offset / 8;
            // tv::gemm::global_load<tv::array<tv::half_t, 8>, sizeof(tv::array<tv::half_t, 8>)>(
  ...
  __forceinline__ __device__ const tv::array<tv::half_t, 8>* load_ptr_with_param(int s, int c, bool& valid_ref)   {

    tv::array<tv::half_t, 8> const *access_ptr = get(s, c, 0) + 0;
