...
__global__ void conv_kernel(ConvParams params)   {
    ...
    tv::array<int, 2> block_offset_A{tile_offset_m * 128, tile_offset_k * 32};
    ...
    TV_PRAGMA_UNROLL
    for (int i = 0; i < 4; ++i){
        if (tile_offset_m * 128 + i * 32 + lane_idx < params.m){
            masks[i] = params.mask_ptr[tile_offset_m * 128 + i * 32 + lane_idx];
        }
    }
    TV_PRAGMA_UNROLL
    for (int i = 0; i < 4; ++i){
    ...
    tv::array<int, 2> block_offset_C{tile_offset_m * 128,
                                    tile_offset_n * 64};
