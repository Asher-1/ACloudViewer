...
#include <sam3d_spconv/sam3d/spconv/Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK/ConvKernel.h>
...
using ConvParamsAmpere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK = sam3d_spconv::sam3d::spconv::cpAmpere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::ConvParams;
using ConvAmpere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK = sam3d_spconv::sam3d::spconv::Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::ConvKernel;
void ConvMainUnitTest::matmul_split_Ampere_f16f16f16_0(tv::gemm::ConvParams params)   {
  ...
  if (algo_desp.trans_a() == false && algo_desp.trans_b() == true && algo_desp.trans_c() == false){
    if (algo_desp.tile_shape == std::array<int, 3>{128, 64, 32}){
      if (algo_desp.warp_tile_shape == std::array<int, 3>{64, 32, 32}){
        if (algo_desp.num_stage == 2 && algo_desp.dacc == 7 && algo_desp.dcomp == 7){
                  ...
                  if (static_cast<int>(algo_desp.layout_i) == 1 && static_cast<int>(algo_desp.layout_w) == 1 && static_cast<int>(algo_desp.layout_o) == 1){
                    if (algo_desp.interleave_i == 1 && algo_desp.interleave_w == 1 && algo_desp.interleave_o == 1){
                      if (algo_desp.mask_sparse == true && algo_desp.increment_k_first == true && algo_desp.is_int8_inference == false && algo_desp.dynamic_mask == false){
                        TV_ASSERT_RT_ERR("algo don't support splitk but you provide split_k_slices > 1.", split_k_slices);
                        // Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK
                        found = true;
                        ...
                        tv::check_shape(indices, {kernel_volume, -1});
                        N = indices.dim(1);
                        cpAmpere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::ConvProblem problem(N, C, K, kernel_volume,
                            tv::gemm::ConvMode::kCrossCorrelation, split_k_slices, groups);
                        TV_ASSERT_RT_ERR(N == output.dim(0), "error");
                        TV_ASSERT_RT_ERR(int64_t(N) * int64_t(C) * 16 / 8 < std::numeric_limits<int32_t>::max(),
                            "your data exceed int32 range. this will be fixed in cumm + nvrtc (spconv 2.2/2.3).");
                        if (!algo_desp.is_int8_inference){
                            TV_ASSERT_INVALID_ARG(output_add.empty(), "only int8 inference support output_add not empty ")
                        }
                        auto source_ptr = algo_desp.is_int8_inference ? c_ten.data_ptr<const tv::half_t>() : (bias.empty() ? c_ten.data_ptr<const tv::half_t>() : bias.data_ptr<const tv::half_t>());
                        if (!output_add.empty()){
                            source_ptr = output_add.data_ptr<const tv::half_t>();
                        }
                        ConvParamsAmpere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK ker_params(problem, a_ten.data_ptr<const tv::half_t>(), b_ten.data_ptr<const tv::half_t>(), c_ten.data_ptr<tv::half_t>(), source_ptr, mask.data_ptr<const uint32_t>(), mask_argsort.data_ptr<const int32_t>(), indices.data_ptr<const int32_t>(), mask_output.empty() ? nullptr : mask_output.data_ptr<uint32_t>(), params.mask_filter, params.reverse_mask, tv::half_t(params.alpha), tv::half_t(params.beta), tv::half_t(params.act_alpha), tv::half_t(params.act_beta), params.act_type, 1, d_is_bias);
                        tv::cuda::Launch launcher(ker_params.grid_dims, dim3(128),
                                                    24576, reinterpret_cast<cudaStream_t>(params.stream));
                        cudaError_t result;
                        if (24576 >= (48 << 10)) {
                            result = cudaFuncSetAttribute(Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::conv_kernel,
                                                            cudaFuncAttributeMaxDynamicSharedMemorySize,
                                                            24576);
                            TV_ASSERT_RT_ERR(result == cudaSuccess, "error");
                            result = cudaFuncSetAttribute(
                                Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::conv_kernel,
                        ...
                        {
                            tv::CUDAKernelTimerGuard timerguard("Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK", evtimer, reinterpret_cast<cudaStream_t>(params.stream));
                            launcher(Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::conv_kernel, ker_params);
                        }
                        TV_CHECK_CUDA_ERR_V2("Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK", "error with params", input.shape(), weight.shape(), output.shape(),
                            indices.shape(), mask.shape(), mask_argsort.shape(), mask_output.shape(), mask_width);
                        if (params.verbose){
                            cudaFuncAttributes attr;
                            checkCudaErrors(
                                cudaFuncGetAttributes(&attr, Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK::conv_kernel));
                            tv::ssprint("Ampere_f16f16f16f16f16tnt_m128n64k32m64n32k32A1T1688_200_C301LLL_SK kernel num regs:", attr.numRegs, "time:", timer.report() / 1000.0);
                        }
                        return;
                      }
                    }
