...
// Matthias Teschner, Bruno Heidelberger, Matthias Mueller, Danat Pomeranets,
// Markus Gross. Optimized Spatial Hashing for Collision Detection of Deformable
// Objects. 2003 John McDonald, Mark Kilgard. Crack-Free Point-Normal Triangles
// using Adjacent Edge Normals. 2010 John Hable. Variable Rate Shading with
// Visibility Buffer Rendering. 2024
namespace meshopt {

static unsigned int hashUpdate4(unsigned int h,
...
struct VertexStreamHasher {
    ...
    bool equal(unsigned int lhs, unsigned int rhs) const {
        for (size_t i = 0; i < stream_count; ++i) {
            ...
            if (memcmp(data + lhs * s.stride, data + rhs * s.stride, s.size) !=
                0)
                return false;
        }
...
struct VertexCustomHasher {
    ...
    bool equal(unsigned int lhs, unsigned int rhs) const {
        ...
        if (lp[0] != rp[0] || lp[1] != rp[1] || lp[2] != rp[2]) return false;
...
static void buildPositionRemap(unsigned int* remap,
    ...
    for (size_t i = 0; i < vertex_count; ++i) {
        ...
        unsigned int* entry = hashLookup(vertex_table, vertex_table_size,
                                         vertex_hasher, index, ~0u);
...
static size_t generateVertexRemap(unsigned int* remap,
...
static void remapVertices(void* destination,
                          const void* vertices,
                          size_t vertex_count,
                          size_t vertex_size,
                          const unsigned int* remap) {
    ...
    for (size_t i = 0; i < vertex_count; ++i)
        if (remap[i] != ~0u) {
            ...
            memcpy(static_cast<unsigned char*>(destination) +
                           remap[i] * block_size,
                   static_cast<const unsigned char*>(vertices) + i * block_size,
                   block_size);
        }
}

template <typename Hash>
static void generateShadowBuffer(unsigned int* destination,
                                 const unsigned int* indices,
                                 size_t index_count,
                                 size_t vertex_count,
                                 const Hash& hash,
    ...
    for (size_t i = 0; i < index_count; ++i) {
        ...
        if (remap[index] == ~0u) {
            unsigned int* entry =
                    hashLookup(table, table_size, hash, index, ~0u);
...
static size_t filterIndexBuffer(unsigned int* destination,
                                const unsigned int* indices,
                                size_t index_count,
                                const unsigned int* remap,
                                size_t vertex_count,
...
size_t meshopt_generateVertexRemap(unsigned int* destination,
    ...
    return generateVertexRemap(destination, indices, index_count, vertex_count,
                               hasher, allocator);
}
...
size_t meshopt_generateVertexRemapCustom(unsigned int* destination,
                                         const unsigned int* indices,
                                         size_t index_count,
                                         const float* vertex_positions,
                                         size_t vertex_count,
    ...
    meshopt_Allocator allocator;
    VertexCustomHasher hasher = {vertex_positions,
...
void meshopt_remapVertexBuffer(void* destination,
                               const void* vertices,
                               size_t vertex_count,
                               size_t vertex_size,
                               const unsigned int* remap) {
    ...
    if (destination == vertices) {
        unsigned char* vertices_copy =
                allocator.allocate<unsigned char>(vertex_count * vertex_size);
        memcpy(vertices_copy, vertices, vertex_count * vertex_size);
        vertices = vertices_copy;
    }

    // specialize the loop for common vertex sizes to ensure memcpy is compiled
    // as an inlined intrinsic
    switch (vertex_size) {
        case 4:
            return remapVertices<4>(destination, vertices, vertex_count,
                                    vertex_size, remap);

        case 8:
            return remapVertices<8>(destination, vertices, vertex_count,
                                    vertex_size, remap);

        case 12:
            return remapVertices<12>(destination, vertices, vertex_count,
                                     vertex_size, remap);

        case 16:
            return remapVertices<16>(destination, vertices, vertex_count,
                                     vertex_size, remap);

        default:
            return remapVertices<0>(destination, vertices, vertex_count,
                                    vertex_size, remap);
    }
}
...
size_t meshopt_filterIndexBuffer(unsigned int* destination,
                                 const unsigned int* indices,
                                 size_t index_count,
                                 const void* vertices,
                                 size_t vertex_count,
    ...
    unsigned int* remap = allocator.allocate<unsigned int>(vertex_count);
    generateVertexRemap(remap, indices, index_count, vertex_count, hasher,
                        allocator);

    return filterIndexBuffer(destination, indices, index_count, remap,
                             vertex_count, allocator);
}

size_t meshopt_filterIndexBufferMulti(unsigned int* destination,
                                      const unsigned int* indices,
                                      size_t index_count,
                                      size_t vertex_count,
                                      const struct meshopt_Stream* streams,
...
void meshopt_generateShadowIndexBuffer(unsigned int* destination,
    ...
    generateShadowBuffer(destination, indices, index_count, vertex_count,
                         hasher, allocator);
}
...
void meshopt_generatePositionRemap(unsigned int* destination,
    ...
    meshopt_Allocator allocator;
    VertexCustomHasher hasher = {vertex_positions,
                                 vertex_positions_stride / sizeof(float), NULL,
                                 NULL};
    ...
    for (size_t i = 0; i < vertex_count; ++i) {
        unsigned int* entry =
                hashLookup(table, table_size, hasher, unsigned(i), ~0u);

        if (*entry == ~0u) *entry = unsigned(i);
...
void meshopt_generateAdjacencyIndexBuffer(unsigned int* destination,
    ...
    // build edge set; this stores all triangle edges but we can look these up
    // by any other wedge
    ...
    for (size_t i = 0; i < index_count; i += 3) {
        for (int e = 0; e < 3; ++e) {
            ...
            unsigned long long* entry = hashLookup(edge_table, edge_table_size,
    ...
    for (size_t i = 0; i < index_count; i += 3) {
        ...
        for (int e = 0; e < 3; ++e) {
            ...
            unsigned long long* oppe = hashLookup(edge_table, edge_table_size,
                                                  edge_hasher, edge, ~0ull);

            patch[e * 2 + 0] = i0;
            patch[e * 2 + 1] = (*oppe == ~0ull)
                                       ? i0
...
size_t meshopt_generateProvokingIndexBuffer(unsigned int* destination,
    ...
    // note: we use 8-bit counters for performance; for outlier vertices the
    // valence is incorrect but that just affects the heuristic
    ...
    for (size_t i = 0; i < index_count; i += 3) {
        ...
        // try to rotate triangle such that provoking vertex hasn't been seen
        // before if multiple vertices are new, prioritize the one with least
        // valence this reduces the risk that a future triangle will have all
        // three vertices seen
        ...
        // recording remap[a] makes it possible to remap future references to
        // the same index, conserving space
        if (remap[a] == ~0u) remap[a] = newidx;

        // we need to clone the provoking vertex to get a unique index
        // if all three are used the choice is arbitrary since no future
        // triangle will be able to reuse any of these
        reorder[reorder_offset++] = a;

        // note: first vertex is final, the other two will be fixed up in next
        // pass
    ...
    // remap or clone non-provoking vertices (iterating to skip provoking
    // vertices)
    int step = 1;
    ...
    for (size_t i = 1; i < index_count; i += step, step ^= 3) {
        ...
        if (remap[index] == ~0u) {
            ...
            // to maintain the reference to the original vertex we need to clone
            // it
            unsigned int newidx = reorder_offset;
