...
def lineset_from_pose_graph(pose_graph):
    ...
    lineset = cv3d.geometry.LineSet()
    lineset.points = cv3d.utility.Vector3dVector(np.vstack(points))
    lineset.lines = cv3d.utility.Vector2iVector(np.vstack(lines).astype(int))
    lineset.colors = cv3d.utility.Vector3dVector(np.vstack(colors))
...
if __name__ == '__main__':
    ...
    scene = cv3d.io.read_point_cloud(path_to_scene)

    geometries = [scene]
    if args.poses:
        path_to_posegraph = args.poses
        pose_graph = cv3d.io.read_pose_graph(path_to_posegraph)

        lineset = lineset_from_pose_graph(pose_graph)
        geometries.append(lineset)

    cv3d.visualization.draw_geometries(geometries)
