...
import cloudViewer.t.pipelines.registration as o3d_reg
...
def run_doppler_icp(args):
    ...
    device = cv3d.core.Device('CUDA:0' if args.device == 'cuda' else 'CPU:0')

    # Load the point clouds.
    demo_sequence = cv3d.data.DemoDopplerICPSequence()
    source = cv3d.t.io.read_point_cloud(demo_sequence.paths[args.source])
    target = cv3d.t.io.read_point_cloud(demo_sequence.paths[args.target])
