...
def draw_registration_result(source, target, transformation):
    ...
    cv3d.visualization.draw([source_temp, target])


print("Load two point clouds and show initial pose ...")
ply_data = cv3d.data.DemoColoredICPPointClouds()
source = cv3d.io.read_point_cloud(ply_data.paths[0])
target = cv3d.io.read_point_cloud(ply_data.paths[1])
