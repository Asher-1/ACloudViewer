...
def pointcloud_generator():
    ...
    mesh = cv3d.geometry.ccMesh.create_torus().scale(5)
    ...
    mesh += cv3d.geometry.ccMesh.create_moebius(twists=1).translate(
        (0, -d, 0))
    mesh += cv3d.geometry.ccMesh.create_moebius(twists=2).translate(
        (d, -d, 0))
...
if __name__ == "__main__":
    ...
    for pcl_name, pcl, eps in pointcloud_generator():
        ...
        colors[labels < 0] = 0
        pcl.colors = cv3d.utility.Vector3dVector(colors[:, :3])
        cv3d.visualization.draw_geometries([pcl])
