        ...
        if config["icp_method"] == "point_to_point":
            ...
            if config["icp_method"] == "color":
                result_icp = cv3d.registration.registration_colored_icp(
                    ...
                    current_transformation,
                    cv3d.registration.ICPConvergenceCriteria(
                        relative_fitness=1e-6,
                        relative_rmse=1e-6,
                        max_iteration=iter))
        current_transformation = result_icp.transformation
        if i == len(max_iter) - 1:
            information_matrix = cv3d.registration.get_information_matrix_from_point_clouds(
...
def make_posegraph_for_refined_scene(ply_file_names, config):
    pose_graph = cv3d.io.read_pose_graph(
    ...
    pose_graph_new.nodes.append(cv3d.registration.PoseGraphNode(odometry))
    for r in matching_results:
