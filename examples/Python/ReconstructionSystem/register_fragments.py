...
def register_point_cloud_fpfh(source, target, source_fpfh, target_fpfh, config):
    ...
    if config["global_registration"] == "fgr":
        result = cv3d.registration.registration_fast_based_on_feature_matching(
            source, target, source_fpfh, target_fpfh,
            cv3d.registration.FastGlobalRegistrationOption(
                maximum_correspondence_distance=distance_threshold))
    if config["global_registration"] == "ransac":
        result = cv3d.registration.registration_ransac_based_on_feature_matching(
            source, target, source_fpfh, target_fpfh, distance_threshold,
            cv3d.registration.TransformationEstimationPointToPoint(False), 4, [
                cv3d.registration.CorrespondenceCheckerBasedOnEdgeLength(0.9),
                cv3d.registration.CorrespondenceCheckerBasedOnDistance(
                    distance_threshold)
            ], cv3d.registration.RANSACConvergenceCriteria(4000000, 500))
