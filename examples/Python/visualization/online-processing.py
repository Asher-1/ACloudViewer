...
class PipelineModel:
    ...
    def __init__(self,
        ...
        if device:
            ...
            self.device = 'cuda:0' if cv3d.core.cuda.is_available() else 'cpu:0'
        self.o3d_device = cv3d.core.Device(self.device)
        ...
        if rgbd_video:  # Video file
            self.video = cv3d.t.io.RGBDVideoReader.create(rgbd_video)
            ...
            filename = f"{now}.bag"
            self.camera = cv3d.t.io.RealSenseSensor()
        ...
        # RGBD -> PCD
        self.extrinsics = cv3d.core.Tensor.eye(4,
                                               dtype=cv3d.core.Dtype.Float32,
                                               device=self.o3d_device)
        self.intrinsic_matrix = cv3d.core.Tensor(
    ...
    def run(self):
        ...
        while (not self.flag_exit and
            ...
            try:
                ...
                self.pcd_frame = cv3d.t.geometry.PointCloud.create_from_rgbd_image(
    ...
    def save_pcd(self):
        ...
        self.pcd_frame.point['colors'] = (self.pcd_frame.point['colors'] *
                                          255).to(cv3d.core.Dtype.UInt8)
        self.executor.submit(cv3d.t.io.write_point_cloud,
                             filename,
                             self.pcd_frame,
                             write_ascii=False,
                             compressed=True,
    ...
    def save_rgbd(self):
        ...
        self.executor.submit(cv3d.t.io.write_image, filename,
...
class PipelineView:
    ...
    def __init__(self, vfov=60, max_pcd_vertices=1 << 20, **callbacks):
        ...
        self.window = gui.Application.instance.create_window(
            "CloudViewer || Online RGBD Video Processing", 1280, 960)
        ...
        self.pcdview.scene = rendering.CloudViewerScene(self.window.renderer)
        ...
        self.color_video = gui.ImageWidget(
            cv3d.geometry.Image(np.zeros(video_size, dtype=np.uint8)))
    ...
    def update(self, frame_elements):
        ...
        if not self.flag_gui_init:
            ...
            dummy_pcd = cv3d.t.geometry.PointCloud({
                'points':
                    cv3d.core.Tensor.zeros((self.max_pcd_vertices, 3),
