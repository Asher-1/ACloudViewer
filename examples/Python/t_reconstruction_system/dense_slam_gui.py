...
class ReconstructionWindow:
    ...
    def _on_start(self):
        ...
        pcd_placeholder = cv3d.t.geometry.PointCloud(
        ...
        self.model = cv3d.t.pipelines.slam.Model(
    ...
    def update_main(self):
        ...
        while not self.is_done:
            ...
            depth = cv3d.t.io.read_image(depth_file_names[self.idx]).to(device)
            color = cv3d.t.io.read_image(color_file_names[self.idx]).to(device)
            ...
            frustum = cv3d.geometry.LineSet.create_camera_visualization(
