...
def integrate(depth_file_names, color_file_names, intrinsic, extrinsics,
    ...
    if os.path.exists(config.path_npz):
        ...
        vbg = cv3d.t.geometry.VoxelBlockGrid.load(config.path_npz)
    ...
    device = cv3d.core.Device(config.device)
    ...
    if config.integrate_color:
        vbg = cv3d.t.geometry.VoxelBlockGrid(
            ('tsdf', 'weight', 'color'),
    ...
    for i in tqdm(range(n_files)):
        depth = cv3d.t.io.read_image(depth_file_names[i]).to(device)
        ...
        if config.integrate_color:
            color = cv3d.t.io.read_image(color_file_names[i]).to(device)
...
if __name__ == '__main__':
    ...
    cv3d.visualization.draw([mesh.to_legacy()])
