...
- All RPC methods are in the dispatch table (including `colmap.reconstruct`, `file.convert`)
...
### Level 4 — GUI RPC
- WebSocket ping/pong
- methods.list returns ≥25 methods with correct names (including `colmap.reconstruct`, `transform.apply`, `entity.rename`)
...
### Level 5 — MCP Server
- MCP SDK importable
- Tool listing returns ≥28 tools
- Core tool names present (open_file, convert_format, subsample, etc.)
- All 12 Colmap MCP tools present (colmap_auto_reconstruct, colmap_extract_features, colmap_match_features, colmap_sparse_reconstruct, colmap_undistort, colmap_dense_stereo, colmap_stereo_fusion, colmap_poisson_mesh, colmap_delaunay_mesh, colmap_image_texturer, colmap_model_converter, colmap_analyze_model)
- All 4 SIBR MCP tools present (sibr_tool, sibr_prepare_colmap, sibr_texture_mesh, sibr_unwrap_mesh)
