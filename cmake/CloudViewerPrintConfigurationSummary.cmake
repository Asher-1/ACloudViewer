...
function(cloudViewer_print_configuration_summary)
    ...
    cloudViewer_aligned_print("AICore" "${AICore_ENABLED}")
    if(AICore_ENABLED)
        cloudViewer_aligned_print("AICore dynamic backends" "${GGML_DYNAMIC_BACKENDS}")
        cloudViewer_aligned_print("AICore backends built" "${GGML_BACKEND_LIST}")
        if(GGML_AUTO_BACKEND_ORDER)
            cloudViewer_aligned_print("AICore Auto device order" "${GGML_AUTO_BACKEND_ORDER}")
        endif()
        # Developer opt-in backends (defaults OFF); release path is in backends built above.
        set(_aicore_dev_backends)
        if(AICore_USE_CUDA)
            list(APPEND _aicore_dev_backends "CUDA")
        endif()
        if(AICore_USE_SYCL)
            list(APPEND _aicore_dev_backends "SYCL")
        endif()
        if(AICore_USE_OPENCL)
            list(APPEND _aicore_dev_backends "OpenCL")
        endif()
        if(_aicore_dev_backends)
            string(JOIN ", " _aicore_dev_backends_text ${_aicore_dev_backends})
            cloudViewer_aligned_print("AICore developer backends" "${_aicore_dev_backends_text}")
        endif()
        if(AICore_USE_CUDA)
            cloudViewer_aligned_print("AICore CUDA runtime bundle" "${AICore_BUNDLE_CUDA_RUNTIME}")
            cloudViewer_aligned_print("AICore FaceDetect cuDNN" "${AICore_USE_CUDNN}")
        endif()
    endif()
    ...
    cloudViewer_aligned_print("Use OpenCV" "${BUILD_OPENCV}")
    cloudViewer_aligned_print("Build Documentation" "${BUILD_DOCUMENTATION}")
