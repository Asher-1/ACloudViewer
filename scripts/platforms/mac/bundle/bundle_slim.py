...
# ---------------------------------------------------------------------------
# MoltenVK discovery — follows the same mechanism the Vulkan loader uses
# ---------------------------------------------------------------------------

_ICD_SEARCH_DIRS: tuple[Path, ...] = (
    Path("/usr/local/share/vulkan/icd.d"),
    Path("/opt/homebrew/share/vulkan/icd.d"),
    Path("/etc/vulkan/icd.d"),
    Path.home() / ".local/share/vulkan/icd.d",
)


def _resolve_icd_library(manifest_path: Path, library_path_str: str) -> Path | None:
    """Resolve ``library_path`` from an ICD manifest to an absolute path."""
    lp = Path(library_path_str)
    if lp.is_absolute():
        return lp if lp.is_file() else None
    resolved = (manifest_path.parent / lp).resolve()
    return resolved if resolved.is_file() else None


def discover_moltenvk() -> tuple[Path | None, dict | None]:
    """Find ``libMoltenVK.dylib`` via Vulkan ICD manifests.

    Returns (library_path, icd_data) where *icd_data* is the parsed JSON
    from the manifest.  Falls back to a direct file search if no manifest
    is found.
    """
    ...
    if vk_icd:
        for p in vk_icd.split(":"):
            fp = Path(p)
            if fp.is_file():
                manifest_files.append(fp)

    vk_driver = os.environ.get("VK_DRIVER_FILES", "")
    if vk_driver:
        for p in vk_driver.split(":"):
            fp = Path(p)
            if fp.is_file():
                manifest_files.append(fp)
    ...
    if vulkan_sdk:
        sdk_icd = Path(vulkan_sdk) / "share" / "vulkan" / "icd.d"
        if sdk_icd.is_dir():
            manifest_files.extend(sorted(sdk_icd.glob("*.json")))

    for search_dir in _ICD_SEARCH_DIRS:
        if search_dir.is_dir():
            manifest_files.extend(sorted(search_dir.glob("*.json")))
    ...
    for mf in manifest_files:
        try:
            ...
            continue
        icd = data.get("ICD", {})
        lib_path_str = icd.get("library_path", "")
        if "MoltenVK" not in lib_path_str and "moltenvk" not in lib_path_str.lower():
            continue
        resolved = _resolve_icd_library(mf, lib_path_str)
        if resolved:
            logger.info("MoltenVK discovered via ICD manifest %s -> %s", mf, resolved)
            return resolved, data
        logger.warning("ICD manifest %s references %s but file not found", mf, lib_path_str)
    ...
    if vulkan_sdk:
        direct_search.append(Path(vulkan_sdk) / "lib" / "libMoltenVK.dylib")
    for candidate in direct_search:
        if candidate.is_file():
            logger.info("MoltenVK found via direct search: %s", candidate)
